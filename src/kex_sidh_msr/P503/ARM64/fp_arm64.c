...
void mp_add503x2_asm(const digit_t *a, const digit_t *b, digit_t *c) { // Multiprecision addition, c = a+b, where lng(a) = lng(b) = nwords. Returns the carry bit.
	unsigned int i, carry = 0;
...
digit_t mp_sub503x2_asm(const digit_t *a, const digit_t *b, digit_t *c) { // Multiprecision subtraction, c = a-b, where lng(a) = lng(b) = nwords. Returns the borrow bit.
...
__inline void fpadd503(const digit_t *a, const digit_t *b, digit_t *c) { // Modular addition, c = a+b mod p503.
	...
	for (i = 0; i < NWORDS_FIELD; i++) {
		ADDC_slow(carry, a[i], b[i], carry, c[i]);
	}

	carry = 0;
	for (i = 0; i < NWORDS_FIELD; i++) {
		SUBC_slow(carry, c[i], ((digit_t *) p503x2)[i], carry, c[i]);
	}
	mask = 0 - (digit_t) carry;

	carry = 0;
	for (i = 0; i < NWORDS_FIELD; i++) {
		ADDC_slow(carry, c[i], ((digit_t *) p503x2)[i] & mask, carry, c[i]);
	}
}
...
__inline void fpsub503(const digit_t *a, const digit_t *b, digit_t *c) { // Modular subtraction, c = a-b mod p503.
	...
	for (i = 0; i < NWORDS_FIELD; i++) {
		SUBC_slow(borrow, a[i], b[i], borrow, c[i]);
	}
	mask = 0 - (digit_t) borrow;

	borrow = 0;
	for (i = 0; i < NWORDS_FIELD; i++) {
		ADDC_slow(borrow, c[i], ((digit_t *) p503x2)[i] & mask, borrow, c[i]);
	}
}
...
__inline void fpneg503(digit_t *a) { // Modular negation, a = -a mod p503.
	...
	for (i = 0; i < NWORDS_FIELD; i++) {
		SUBC_slow(borrow, ((digit_t *) p503x2)[i], a[i], borrow, a[i]);
	}
}
...
void fpdiv2_503(const digit_t *a, digit_t *c) { // Modular division by two, c = a/2 mod p503.
	...
	for (i = 0; i < NWORDS_FIELD; i++) {
		ADDC_slow(carry, a[i], ((digit_t *) p503)[i] & mask, carry, c[i]);
	}
...
void fpcorrection503(digit_t *a) { // Modular correction to reduce field element a in [0, 2*p503-1] to [0, p503-1].
	...
	for (i = 0; i < NWORDS_FIELD; i++) {
		SUBC_slow(borrow, a[i], ((digit_t *) p503)[i], borrow, a[i]);
	}
	mask = 0 - (digit_t) borrow;

	borrow = 0;
	for (i = 0; i < NWORDS_FIELD; i++) {
		ADDC_slow(borrow, a[i], ((digit_t *) p503)[i] & mask, borrow, a[i]);
	}
}
...
void mp_mul(const digit_t *a, const digit_t *b, digit_t *c, const unsigned int nwords) { // Multiprecision comba multiply, c = a*b, where lng(a) = lng(b) = nwords.
	...
	for (i = 0; i < nwords; i++) {
		for (j = 0; j <= i; j++) {
			MUL_slow(a[j], b[i - j], UV + 1, UV[0]);
			ADDC_slow(0, UV[0], v, carry, v);
			ADDC_slow(carry, UV[1], u, carry, u);
			t += carry;
		}
		c[i] = v;
		v = u;
...
void rdc_mont(const digit_t *ma, digit_t *mc) { // Efficient Montgomery reduction using comba and exploiting the special form of the prime p503.
	...
	for (i = 0; i < NWORDS_FIELD; i++) {
		for (j = 0; j < i; j++) {
			if (j < (i - p503_ZERO_WORDS + 1)) {
				MUL_slow(mc[j], ((digit_t *) p503p1)[i - j], UV + 1, UV[0]);
				ADDC_slow(0, UV[0], v, carry, v);
				ADDC_slow(carry, UV[1], u, carry, u);
				t += carry;
			}
		}
		ADDC_slow(0, v, ma[i], carry, v);
		ADDC_slow(carry, u, 0, carry, u);
		t += carry;
		mc[i] = v;
		v = u;
		u = t;
	...
	ADDC_slow(0, v, ma[2 * NWORDS_FIELD - 1], carry, v);
	mc[NWORDS_FIELD - 1] = v;
}
