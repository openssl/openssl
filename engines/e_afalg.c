...
static int afalg_fin_cipher_aio(afalg_aio *ptr, int sfd, unsigned char *buf,
				size_t len);
...
static int afalg_ciphers(ENGINE *e, const EVP_CIPHER **cipher, const int **nids,
			 int nid);
				    ...
				    struct timespec *timeout)
{
	...
	if (sizeof(*timeout) != sizeof(struct __timespec32)) {
		int ret = syscall(__NR_io_pgetevents_time64, ctx, min, max,
				  events, timeout, NULL);
	...
	if (sizeof(*timeout) == sizeof(struct __timespec32))
         ...
         * __NR_io_getevents
         */
		return syscall(__NR_io_getevents, ctx, min, max, events,
			       timeout);
	else {
		/*
...
static int afalg_setup_async_event_notification(afalg_aio *aio)
{
	...
	if ((job = ASYNC_get_current_job()) != NULL) {
		...
		if (waitctx == NULL) {
			ALG_WARN("%s(%d): ASYNC_get_wait_ctx error", __FILE__,
				 __LINE__);
			return 0;
		}
		/* Get waitfd from ASYNC_WAIT_CTX if it is already set */
		ret = ASYNC_WAIT_CTX_get_fd(waitctx, engine_afalg_id, &aio->efd,
					    &custom);
		if (ret == 0) {
			/*
			...
			if (aio->efd == -1) {
				ALG_PERR("%s(%d): Failed to get eventfd : ",
					 __FILE__, __LINE__);
				AFALGerr(
					AFALG_F_AFALG_SETUP_ASYNC_EVENT_NOTIFICATION,
					AFALG_R_EVENTFD_FAILED);
				return 0;
			}
			ret = ASYNC_WAIT_CTX_set_wait_fd(waitctx,
							 engine_afalg_id,
							 aio->efd, custom,
							 afalg_waitfd_cleanup);
			if (ret == 0) {
				ALG_WARN("%s(%d): Failed to set wait fd",
					 __FILE__, __LINE__);
				close(aio->efd);
				return 0;
			}
			...
			if (fcntl(aio->efd, F_SETFL, O_NONBLOCK) != 0) {
				ALG_WARN(
					"%s(%d): Failed to set event fd as NONBLOCKING",
					__FILE__, __LINE__);
			}
		}
		aio->mode = MODE_ASYNC;
			...
			if (r > 0) {
				...
				if (events[0].res < 0) {
					...
					if (events[0].res == -EBUSY &&
						...
						if (r < 0) {
							ALG_PERR(
								"%s(%d): retry %d for io_read failed : ",
								__FILE__,
								__LINE__,
								retry);
							return 0;
				     ...
				     const int klen)
{
	...
	if (ret < 0) {
		ALG_PERR("%s(%d): Failed to set socket option : ", __FILE__,
			 __LINE__);
			   ...
			   const char *ciphername)
{
	...
	if (actx->bfd == -1) {
		ALG_PERR("%s(%d): Failed to open socket : ", __FILE__,
			 __LINE__);
	...
	if (r < 0) {
		ALG_PERR("%s(%d): Failed to bind socket : ", __FILE__,
			 __LINE__);
				 ...
				 unsigned int enc)
{
	...
	int ret;
#endif
	char cbuf[CMSG_SPACE(ALG_IV_LEN(ALG_AES_IV_LEN)) +
	...
	if (sbytes < 0) {
		ALG_PERR(
			"%s(%d): sendmsg failed for zero copy cipher operation : ",
			__FILE__, __LINE__);
		return 0;
	}
			     ...
			     const unsigned char *iv, int enc)
{
	...
	if (ALG_AES_IV_LEN != EVP_CIPHER_CTX_get_iv_length(ctx)) {
		ALG_WARN("%s(%d): Unsupported IV length :%d\n", __FILE__,
			 __LINE__, EVP_CIPHER_CTX_get_iv_length(ctx));
		return 0;
	}
			   ...
			   const unsigned char *in, size_t inl)
{
	...
	if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
		memcpy(EVP_CIPHER_CTX_iv_noconst(ctx),
		       out + (inl - ALG_AES_IV_LEN), ALG_AES_IV_LEN);
	} else {
...
static const EVP_CIPHER *afalg_aes_cbc(int nid)
{
	...
	if (cipher_handle->_hidden == NULL &&
	    ((cipher_handle->_hidden = EVP_CIPHER_meth_new(
		      nid, AES_BLOCK_SIZE, cipher_handle->key_size)) == NULL ||
