...
static const EVP_MD *digest_md5(void)
{
	if (_hidden_md5_md == NULL) {
		...
		if ((md = EVP_MD_meth_new(NID_md5, NID_md5WithRSAEncryption)) ==
			    NULL ||
		    !EVP_MD_meth_set_result_size(md, MD5_DIGEST_LENGTH) ||
		    !EVP_MD_meth_set_input_blocksize(md, MD5_CBLOCK) ||
		    !EVP_MD_meth_set_app_datasize(
			    md, sizeof(EVP_MD *) + sizeof(MD5_CTX)) ||
...
static const EVP_MD *digest_sha256(void)
{
	if (_hidden_sha256_md == NULL) {
		...
		if ((md = EVP_MD_meth_new(NID_sha256,
...
static const EVP_MD *digest_sha384(void)
{
	if (_hidden_sha384_md == NULL) {
		...
		if ((md = EVP_MD_meth_new(NID_sha384,
...
static const EVP_MD *digest_sha512(void)
{
	if (_hidden_sha512_md == NULL) {
		...
		if ((md = EVP_MD_meth_new(NID_sha512,
...
static const EVP_CIPHER *ossltest_aes_128_cbc(void)
{
	if (_hidden_aes_128_cbc == NULL &&
	    ((_hidden_aes_128_cbc =
		      EVP_CIPHER_meth_new(NID_aes_128_cbc, 16 /* block size */,
					  16 /* key len */)) == NULL ||
...
static const EVP_CIPHER *ossltest_aes_128_gcm(void)
{
	if (_hidden_aes_128_gcm == NULL &&
	    ((_hidden_aes_128_gcm =
		      EVP_CIPHER_meth_new(NID_aes_128_gcm, 1 /* block size */,
					  16 /* key len */)) == NULL ||
...
static const EVP_CIPHER *ossltest_aes_128_cbc_hmac_sha1(void)
{
	if (_hidden_aes_128_cbc_hmac_sha1 == NULL &&
	    ((_hidden_aes_128_cbc_hmac_sha1 = EVP_CIPHER_meth_new(
		      NID_aes_128_cbc_hmac_sha1, 16 /* block size */,
		      16 /* key len */)) == NULL ||
	     ...
	     !EVP_CIPHER_meth_set_set_asn1_params(
		     _hidden_aes_128_cbc_hmac_sha1,
		     EVP_CIPH_FLAG_DEFAULT_ASN1 ? NULL :
				      ...
				      const unsigned char *in, size_t inl)
{
	...
	ret = EVP_CIPHER_meth_get_do_cipher(EVP_aes_128_cbc())(ctx, out, in,
							       inl);
...
{
	/* Pass the ctrl down */
	int ret = EVP_CIPHER_meth_get_ctrl(EVP_aes_128_gcm())(ctx, type, arg,
							      ptr);
