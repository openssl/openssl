...
static int bind_dasync(ENGINE *e)
{
	...
	if (_hidden_sha1_md == NULL ||
	    ...
	    !EVP_MD_meth_set_flags(_hidden_sha1_md,
...
static void dummy_pause_job(void)
{
	...
	if (ASYNC_WAIT_CTX_get_fd(waitctx, engine_dasync_id, &pipefds[0],
		...
		*writefd = pipefds[1];
		...
		if (!ASYNC_WAIT_CTX_set_wait_fd(waitctx, engine_dasync_id,
						pipefds[0], writefd,
						wait_cleanup)) {
			wait_cleanup(waitctx, engine_dasync_id, pipefds[0],
				     writefd);
			return;
		}
	}
				     ...
				     const EVP_CIPHER *ciph)
{
	...
	struct dasync_pipeline_ctx *pipe_ctx =
		(struct dasync_pipeline_ctx *)EVP_CIPHER_CTX_get_cipher_data(
			ctx);
	...
	case EVP_CTRL_AEAD_SET_MAC_KEY:
		...
		ret = EVP_CIPHER_meth_get_ctrl(
			EVP_aes_128_cbc_hmac_sha1())(ctx, type, arg, ptr);
...
static int dasync_rsa_paramgen_init(EVP_PKEY_CTX *ctx)
{
	...
	if (pparamgen_init == NULL)
		EVP_PKEY_meth_get_paramgen(dasync_rsa_orig, &pparamgen_init,
					   NULL);
...
static int dasync_rsa_encrypt_init(EVP_PKEY_CTX *ctx)
{
	...
	if (pencrypt_init == NULL)
		EVP_PKEY_meth_get_encrypt(dasync_rsa_orig, &pencrypt_init,
					  NULL);
...
static int dasync_rsa_decrypt_init(EVP_PKEY_CTX *ctx)
{
	...
	if (pdecrypt_init == NULL)
		EVP_PKEY_meth_get_decrypt(dasync_rsa_orig, &pdecrypt_init,
					  NULL);
