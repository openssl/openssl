...
static int cipher_ctrl(EVP_CIPHER_CTX *ctx, int type, int p1, void *p2)
{
	...
	case EVP_CTRL_COPY:
		...
		to_cipher_ctx =
			(struct cipher_ctx *)EVP_CIPHER_CTX_get_cipher_data(
				to_ctx);
...
static void prepare_cipher_methods(void)
{
	...
	for (i = 0, known_cipher_nids_amount = 0; i < OSSL_NELEM(cipher_data);
		...
		if ((known_cipher_methods[i] = EVP_CIPHER_meth_new(
		    ...
		    !EVP_CIPHER_meth_set_init(known_cipher_methods[i],
					      cipher_init) ||
		    !EVP_CIPHER_meth_set_do_cipher(
			    ...
			    cipher_mode == EVP_CIPH_CTR_MODE ?
				    ctr_do_cipher :
				    cipher_do_cipher) ||
		    !EVP_CIPHER_meth_set_ctrl(known_cipher_methods[i],
					      cipher_ctrl) ||
...
static void dump_cipher_info(void)
{
	...
	for (i = 0; i < OSSL_NELEM(cipher_data); i++) {
		...
		if (cipher_driver_info[i].status ==
		    DEVCRYPTO_STATUS_NO_CIOCGSESSION) {
			fprintf(stderr,
...
static int digest_update(EVP_MD_CTX *ctx, const void *data, size_t count)
{
	...
	if (EVP_MD_CTX_test_flags(ctx, EVP_MD_CTX_FLAG_ONESHOT)) {
		if (digest_op(digest_ctx, data, count, digest_ctx->digest_res,
			      0) >= 0)
			return 1;
	} else if (digest_op(digest_ctx, data, count, NULL, COP_FLAG_UPDATE) >=
		   0) {
		return 1;
	}
...
static void prepare_digest_methods(void)
{
	...
	for (i = 0, known_digest_nids_amount = 0; i < OSSL_NELEM(digest_data);
		...
		if ((known_digest_methods[i] = EVP_MD_meth_new(
			     digest_data[i].nid, NID_undef)) == NULL ||
		    !EVP_MD_meth_set_input_blocksize(known_digest_methods[i],
						     digest_data[i].blocksize) ||
		    !EVP_MD_meth_set_result_size(known_digest_methods[i],
						 digest_data[i].digestlen) ||
		    !EVP_MD_meth_set_init(known_digest_methods[i],
					  digest_init) ||
		    !EVP_MD_meth_set_update(known_digest_methods[i],
					    digest_update) ||
		    !EVP_MD_meth_set_final(known_digest_methods[i],
					   digest_final) ||
		    !EVP_MD_meth_set_copy(known_digest_methods[i],
					  digest_copy) ||
		    !EVP_MD_meth_set_cleanup(known_digest_methods[i],
					     digest_cleanup) ||
...
static int cryptodev_select_digest_cb(const char *str, int len, void *usr)
{
	...
	else if ((i = find_digest_data_index(EVP_MD_get_type(EVP))) !=
		 (size_t)-1)
		digest_list[i] = 1;
	else
	...
	for (i = 0; i < OSSL_NELEM(digest_data); i++) {
		...
		fprintf(stderr,
			...
			digest_driver_info[i].driver_name ?
				digest_driver_info[i].driver_name :
				"unknown");
		if (digest_driver_info[i].status ==
		    DEVCRYPTO_STATUS_NO_CIOCGSESSION) {
			fprintf(stderr,
...
static int devcrypto_ctrl(ENGINE *e, int cmd, long i, void *p, void (*f)(void))
{
	...
	case DEVCRYPTO_CMD_CIPHERS:
		...
		if (OPENSSL_strcasecmp((const char *)p, "ALL") == 0) {
			...
			if (!CONF_parse_list(p, ',', 1,
					     cryptodev_select_cipher_cb,
					     new_list)) {
				OPENSSL_free(new_list);
				return 0;
			}
			memcpy(selected_ciphers, new_list,
	...
	case DEVCRYPTO_CMD_DIGESTS:
		...
		if (OPENSSL_strcasecmp((const char *)p, "ALL") == 0) {
			...
			if (!CONF_parse_list(p, ',', 1,
					     cryptodev_select_digest_cb,
					     new_list)) {
				OPENSSL_free(new_list);
				return 0;
			}
			memcpy(selected_digests, new_list,
