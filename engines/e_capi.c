 ...
 */
	...
	defined(CERT_STORE_PROV_SYSTEM_A) && defined(CERT_STORE_READONLY_FLAG)
...
typedef PCCERT_CONTEXT(WINAPI *CERTDLG)(HCERTSTORE, HWND, LPCWSTR, LPCWSTR,
					DWORD, DWORD, void *);
...
static const ENGINE_CMD_DEFN capi_cmd_defns[] = {
	{ CAPI_CMD_LIST_CERTS, "list_certs", "List all certificates in store",
	  ENGINE_CMD_FLAG_NO_INPUT },
	{ CAPI_CMD_LOOKUP_CERT, "lookup_cert", "Lookup and output certificates",
	  ENGINE_CMD_FLAG_STRING },
	{ CAPI_CMD_DEBUG_LEVEL, "debug_level",
	  "debug level (1=errors, 2=trace)", ENGINE_CMD_FLAG_NUMERIC },
	{ CAPI_CMD_DEBUG_FILE, "debug_file", "debugging filename)",
	  ENGINE_CMD_FLAG_STRING },
	{ CAPI_CMD_KEYTYPE, "key_type",
	  "Key type: 1=AT_KEYEXCHANGE (default), 2=AT_SIGNATURE",
	  ENGINE_CMD_FLAG_NUMERIC },
	{ CAPI_CMD_LIST_CSPS, "list_csps", "List all CSPs",
	  ENGINE_CMD_FLAG_NO_INPUT },
	{ CAPI_CMD_SET_CSP_IDX, "csp_idx", "Set CSP by index",
	  ENGINE_CMD_FLAG_NUMERIC },
	{ CAPI_CMD_SET_CSP_NAME, "csp_name",
	  "Set CSP name, (default CSP used if not specified)",
	  ENGINE_CMD_FLAG_STRING },
	{ CAPI_CMD_SET_CSP_TYPE, "csp_type",
	  "Set CSP type, (default RSA_PROV_FULL)", ENGINE_CMD_FLAG_NUMERIC },
	{ CAPI_CMD_LIST_CONTAINERS, "list_containers", "list container names",
	  ENGINE_CMD_FLAG_NO_INPUT },
	{ CAPI_CMD_LIST_OPTIONS, "list_options",
	  "Set list options (1=summary,2=friendly name, 4=full printout, 8=PEM output, 16=XXX, "
	  "32=private key info)",
	  ENGINE_CMD_FLAG_NUMERIC },
	{ CAPI_CMD_LOOKUP_METHOD, "lookup_method",
	  "Set key lookup method (1=substring, 2=friendlyname, 3=container name)",
	  ENGINE_CMD_FLAG_NUMERIC },
	{ CAPI_CMD_STORE_NAME, "store_name",
	  "certificate store name, default \"MY\"", ENGINE_CMD_FLAG_STRING },
	{ CAPI_CMD_STORE_FLAGS, "store_flags",
...
static int capi_init(ENGINE *e)
{
	...
	if (capi_idx < 0) {
		...
		if (!RSA_meth_set_pub_enc(capi_rsa_method,
		    ...
		    !RSA_meth_set_priv_enc(capi_rsa_method,
					   capi_rsa_priv_enc) ||
		    !RSA_meth_set_priv_dec(capi_rsa_method,
					   capi_rsa_priv_dec) ||
...
#ifdef OPENSSL_CAPIENG_DIALOG
	{
		...
		if (cryptui)
			ctx->certselectdlg = (CERTDLG)GetProcAddress(
				cryptui,
				"CryptUIDlgSelectCertificateFromStore");
		if (kernel)
			ctx->getconswindow = (GETCONSWIN)GetProcAddress(
				kernel, "GetConsoleWindow");
				   ...
				   UI_METHOD *ui_method, void *callback_data)
{
	...
	if (!ctx) {
		CAPIerr(CAPI_F_CAPI_LOAD_PRIVKEY,
...
int capi_rsa_priv_enc(int flen, const unsigned char *from, unsigned char *to,
		      RSA *rsa, int padding)
{
...
int capi_rsa_priv_dec(int flen, const unsigned char *from, unsigned char *to,
		      RSA *rsa, int padding)
{
	int i;
				 ...
				 DSA *dsa)
{
	...
	if (!CryptCreateHash(capi_key->hprov, CALG_SHA1, 0, 0, &hash)) {
		CAPIerr(CAPI_F_CAPI_DSA_DO_SIGN,
	...
	if (!CryptSignHash(hash, capi_key->keyspec, NULL, 0, csigbuf, &slen)) {
		...
		if (r == NULL || s == NULL || !lend_tobn(r, csigbuf, 20) ||
...
static void capi_dump_cert(CAPI_CTX *ctx, BIO *out, PCCERT_CONTEXT cert)
{
	...
	if (flags & CAPI_DMP_SUMMARY) {
		...
		X509_NAME_print_ex(out, X509_get_subject_name(x), 0,
				   XN_FLAG_ONELINE);
		BIO_printf(out, "\n  Issuer: ");
		X509_NAME_print_ex(out, X509_get_issuer_name(x), 0,
				   XN_FLAG_ONELINE);
		BIO_printf(out, "\n");
	}
...
static HCERTSTORE capi_open_store(CAPI_CTX *ctx, char *storename)
{
	...
	hstore = CertOpenStore(CERT_STORE_PROV_SYSTEM_A, 0, 0, ctx->store_flags,
			       storename);
	if (!hstore) {
...
static CAPI_KEY *capi_get_key(CAPI_CTX *ctx, const WCHAR *contname,
			      const WCHAR *provname, DWORD ptype, DWORD keyspec)
{
	DWORD dwFlags = 0;
...
static CAPI_KEY *capi_get_cert_key(CAPI_CTX *ctx, PCCERT_CONTEXT cert)
{
	...
	if (pinfo != NULL)
		key = capi_get_key(ctx, pinfo->pwszContainerName,
				   pinfo->pwszProvName, pinfo->dwProvType,
				   pinfo->dwKeySpec);
	...
	case CAPI_LU_CONTNAME: {
		...
		if ((len = MultiByteToWideChar(CP_ACP, 0, id, -1, NULL, 0)) &&
		    ...
		    (provname = alloca(len * sizeof(WCHAR)),
		     MultiByteToWideChar(CP_ACP, 0, ctx->cspname, -1, provname,
					 len)))
				 ...
				 int check)
{
	...
	if (check) {
		...
		if ((len = MultiByteToWideChar(CP_ACP, 0, pname, -1, NULL,
					       0))) {
			name = alloca(len * sizeof(WCHAR));
			MultiByteToWideChar(CP_ACP, 0, pname, -1, (WCHAR *)name,
					    len);
		}
		if (name == NULL ||
		    !CryptAcquireContextW(&hprov, NULL, name, type,
					  CRYPT_VERIFYCONTEXT)) {
...
static int cert_select_dialog(ENGINE *e, SSL *ssl, STACK_OF(X509) *certs)
{
	...
	for (i = 0; i < sk_X509_num(certs); i++) {
		...
		if (!CertAddCertificateContextToStore(
			    dstore, key->pcert, CERT_STORE_ADD_NEW, NULL)) {
			CAPIerr(CAPI_F_CERT_SELECT_DIALOG,
	...
	if (cert) {
		for (i = 0; i < sk_X509_num(certs); i++) {
			...
			if (CertCompareCertificate(
				    ...
				    cert->pCertInfo, key->pcert->pCertInfo)) {
				idx = i;
				break;
			}
		}
