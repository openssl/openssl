			   ...
			   void *data)
{
	...
	if ((prompt = UI_construct_prompt(ui, desc, info)) == NULL) {
		...
		case -2:
			ATTICerr(0,
				 ATTIC_R_UI_PROCESS_INTERRUPTED_OR_CANCELLED);
			pass = NULL;
			break;
		case -1:
			ATTICerr(0, ERR_R_UI_LIB);
				     ...
				     BUF_MEM *embedded)
{
	...
	data->blob = embedded;
	data->pem_name = new_pem_name == NULL ? NULL :
		...
		if ((p12 = d2i_PKCS12(NULL, &blob, (long)len)) != NULL) {
			...
			*matchcount = 1;
			...
			if (!PKCS12_mac_present(p12) ||
				...
				if ((pass = file_get_pass(ui_method, tpass,
							  PEM_BUFSIZE,
							  "PKCS12 import", uri,
							  ui_data)) == NULL) {
					ATTICerr(
						0,
						ATTIC_R_PASSPHRASE_CALLBACK_ERROR);
					goto p12_end;
				}
			...
			if (PKCS12_parse(p12, pass, &pkey, &cert, &chain)) {
				...
				if ((ctx = sk_OSSL_STORE_INFO_new_null()) !=
				    NULL) {
					if (pkey != NULL) {
						if ((osi_pkey =
	...
	void *ui_data, const char *uri, OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if (len > LONG_MAX ||
	    (p8 = d2i_X509_SIG(NULL, &blob, (long)len)) == NULL)
		return NULL;
	...
	*matchcount = 1;
	...
	if (!PKCS12_pbe_crypt(dalg, pass, (int)strlen(pass), doct->data,
			      doct->length, &new_data, &new_data_len, 0))
		goto nop8;
	...
	if (pem_name != NULL) {
		if (strcmp(pem_name, PEM_STRING_PKCS8INF) == 0) {
			...
			*matchcount = 1;
			...
			if ((slen = check_suffix(pem_name, "PRIVATE KEY")) >
				    0 &&
			    (ameth = EVP_PKEY_asn1_find_str(NULL, pem_name,
							    slen)) != NULL &&
			    EVP_PKEY_asn1_get0_info(&pkey_id, NULL, NULL, NULL,
						    NULL, ameth)) {
				*matchcount = 1;
				pkey = d2i_PrivateKey_ex(pkey_id, NULL, &blob,
							 (long)len, libctx,
							 propq);
			}
		}
		...
		while (curengine != NULL) {
			...
			if (asn1meths != NULL) {
				const int *nids = NULL;
				int nids_n =
					asn1meths(curengine, NULL, &nids, 0);
				...
				for (i = 0; i < nids_n; i++) {
					...
					if (!asn1meths(curengine, &ameth2, NULL,
						       nids[i]) ||
					    !EVP_PKEY_asn1_get0_info(
						    &pkey_id, NULL, &pkey_flags,
						    NULL, NULL, ameth2) ||
					    (pkey_flags & ASN1_PKEY_ALIAS) != 0)
						continue;

					ERR_set_mark(); /* prevent flooding error queue */
					tmp_pkey = d2i_PrivateKey_ex(
						pkey_id, NULL, &tmp_blob,
						(long)len, libctx, propq);
					if (tmp_pkey != NULL) {
						if (pkey != NULL)
		...
		for (i = 0; i < EVP_PKEY_asn1_get_count(); i++) {
			...
			if (!EVP_PKEY_asn1_get0_info(&pkey_id, NULL,
			...
			ERR_set_mark(); /* prevent flooding error queue */
			tmp_pkey = d2i_PrivateKey_ex(pkey_id, NULL, &tmp_blob,
						     (long)len, libctx, propq);
			if (tmp_pkey != NULL) {
				if (pkey != NULL)
					EVP_PKEY_free(tmp_pkey);
...
static FILE_HANDLER PrivateKey_handler = { "PrivateKey",
		  ...
		  const char *uri, OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if (len > LONG_MAX ||
	    (pkey = d2i_PUBKEY(NULL, &blob, (long)len)) != NULL) {
		*matchcount = 1;
		  ...
		  const char *uri, OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if (pem_name != NULL) {
		...
		for (i = 0; i < EVP_PKEY_asn1_get_count(); i++) {
			...
			tmp_pkey = d2i_KeyParams(pkey_id, NULL, &tmp_blob,
						 (long)len);
...
static int file_find_type(OSSL_STORE_LOADER_CTX *ctx)
{
	...
	if (BIO_buffer_peek(ctx->_.file.file, peekbuf, sizeof(peekbuf) - 1) >
	    0) {
		     ...
		     const OSSL_STORE_SEARCH *search)
{
	...
	if (OSSL_STORE_SEARCH_get_type(search) == OSSL_STORE_SEARCH_BY_NAME) {
		...
		if (ctx->type != is_dir) {
			ATTICerr(0,
				 ATTIC_R_SEARCH_ONLY_SUPPORTED_FOR_DIRECTORIES);
			return 0;
		}

		hash = X509_NAME_hash_ex(OSSL_STORE_SEARCH_get0_name(search),
					 NULL, NULL, NULL);
		BIO_snprintf(ctx->_.dir.search_name,
			     sizeof(ctx->_.dir.search_name), "%08lx", hash);
		return 1;
	}
...
again: {
	...
	const FILE_HANDLER **matching_handlers = OPENSSL_zalloc(
		sizeof(*matching_handlers) * OSSL_NELEM(file_handlers));
	...
	*matchcount = 0;
	for (i = 0; i < OSSL_NELEM(file_handlers); i++) {
		...
		ERR_set_mark();
		tmp_result = handler->try_decode(pem_name, pem_header, data,
						 len, &tmp_handler_ctx,
						 &try_matchcount, ui_method,
						 ui_data, ctx->uri, ctx->libctx,
						 ctx->propq);
		...
		if (try_matchcount > 0) {
			...
			if (result == NULL)
				result = tmp_result;
			if (result ==
			    NULL) /* e.g., PKCS#12 file decryption error */
				break;
		}
	}
					     ...
					     void *ui_data)
{
	...
	if (ctx->_.file.last_handler != NULL) {
		result = ctx->_.file.last_handler->try_decode(
			NULL, NULL, NULL, 0, &ctx->_.file.last_handler_ctx,
			&try_matchcount, ui_method, ui_data, ctx->uri,
...
static OSSL_STORE_INFO *file_try_read_msblob(BIO *bp, int *matchcount)
{
	...
	{
		EVP_PKEY *tmp = ispub ? b2i_PublicKey_bio(bp) :
					  ...
					  int *matchcount)
{
...

	{
		...
		if (BIO_buffer_peek(bp, peekbuf, sizeof(peekbuf)) <= 0)
			return 0;
		if (!ossl_do_PVK_header(&p, sizeof(peekbuf), 0, &isdss,
					&saltlen, &keylen))
			return 0;
	}
...

	{
		...
		if (!file_fill_pem_pass_data(&pass_data, "PVK pass phrase", uri,
					     ui_method, ui_data) ||
		    (tmp = b2i_PVK_bio(bp, file_get_pem_pass, &pass_data)) ==
			    NULL ||
		    (result = OSSL_STORE_INFO_new_PKEY(tmp)) == NULL) {
			EVP_PKEY_free(tmp);
			return 0;
			    ...
			    char **data)
{
	...
	assert(data != NULL);
	{
		...
		long calculated_length =
			(long)(strlen(ctx->uri) + strlen(pathsep) +
			       strlen(name) + 1 /* \0 */);
		...
		do {
			...
			if (ctx->_.dir.last_entry == NULL) {
				if (!ctx->_.dir.end_reached) {
					...
					ERR_raise(ERR_LIB_SYS,
						  ctx->_.dir.last_errno);
					ctx->errcnt++;
				}
				return NULL;
			}
			...
			if (ctx->_.dir.last_entry[0] != '.' &&
			    ...
			    !file_name_to_uri(ctx, ctx->_.dir.last_entry,
					      &newname))
				return NULL;
			...
			ctx->_.dir.last_entry =
				OPENSSL_DIR_read(&ctx->_.dir.ctx, ctx->uri);
			ctx->_.dir.last_errno = errno;
			if (ctx->_.dir.last_entry == NULL &&
			    ctx->_.dir.last_errno == 0)
				ctx->_.dir.end_reached = 1;

			if (newname != NULL &&
			    (result = OSSL_STORE_INFO_new_NAME(newname)) ==
				    NULL) {
...
again:
		...
		do {
			...
			if (ctx->type == is_pem) {
				...
				if ((result = file_try_read_msblob(
					     ctx->_.file.file, &matchcount)) !=
					    NULL ||
				    (result = file_try_read_PVK(
					     ctx->_.file.file, ui_method,
					     ui_data, ctx->uri, &matchcount)) !=
					    NULL)
					goto endloop;

				if (!file_read_asn1(ctx->_.file.file, &data,
						    &len)) {
					ctx->errcnt++;
					goto endloop;
				}
			...
			if (matchcount > 1) {
				...
				if (ERR_peek_error() == 0) {
					ATTICerr(
						0,
						ATTIC_R_UNSUPPORTED_CONTENT_TYPE);
					if (pem_name != NULL)
						ERR_add_error_data(
							3, "PEM type is '",
							pem_name, "'");
...
endloop:
			pem_free_flag(pem_name,
				      (ctx->flags & FILE_FLAG_SECMEM) != 0, 0);
			pem_free_flag(pem_header,
				      (ctx->flags & FILE_FLAG_SECMEM) != 0, 0);
			pem_free_flag(data,
