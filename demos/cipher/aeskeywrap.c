...
static int aes_wrap_encrypt(void)
{
     ...
     */
	if (!EVP_EncryptInit_ex2(ctx, cipher, wrap_key, wrap_iv,
				 /* params */ NULL))
		goto err;
	...
	if (sizeof(wrap_ct) == outlen &&
	    !CRYPTO_memcmp(outbuf, wrap_ct, outlen))
...
static int aes_wrap_decrypt(void)
{
     ...
     * We are not setting any custom params so let params be just NULL.
     */
	if (!EVP_DecryptInit_ex2(ctx, cipher, wrap_key, wrap_iv,
				 /* params */ NULL))
		goto err;
	...
	if (sizeof(wrap_pt) == outlen &&
	    !CRYPTO_memcmp(outbuf, wrap_pt, outlen))
