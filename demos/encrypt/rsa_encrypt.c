...
static void set_optional_params(OSSL_PARAM *p, const char *propq)
{
	...
	*p++ = OSSL_PARAM_construct_octet_string(
		OSSL_ASYM_CIPHER_PARAM_OAEP_LABEL, label, sizeof(label));
	/* "SHA1" is used if this is not set */
	*p++ = OSSL_PARAM_construct_utf8_string(
		OSSL_ASYM_CIPHER_PARAM_OAEP_DIGEST, "SHA256", 0);
	/*
	...
	if (propq != NULL)
		*p++ = OSSL_PARAM_construct_utf8_string(
			OSSL_ASYM_CIPHER_PARAM_OAEP_DIGEST_PROPS, (char *)propq,
			0);
...
int main(void)
{
	...
	if (CRYPTO_memcmp(msg, decrypted, decrypted_len) != 0) {
		fprintf(stderr,
			"Decrypted data does not match expected value\n");
		goto cleanup;
	}
	ret = EXIT_SUCCESS;
