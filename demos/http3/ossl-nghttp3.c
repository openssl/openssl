	...
	const nghttp3_settings *settings, void *user_data)
{
	...
	if ((conn->streams = lh_OSSL_DEMO_H3_STREAM_new(h3_stream_hash,
							h3_stream_eq)) == NULL)
		goto err;
...
static void h3_conn_pump_stream(OSSL_DEMO_H3_STREAM *s, void *conn_)
{
	...
	for (;;) {
		...
		if (s->buf_cur == s->buf_total) {
			/* Need more data. */
			ec = SSL_read_ex(s->s, s->buf, sizeof(s->buf),
					 &num_bytes);
			if (ec <= 0) {
				num_bytes = 0;
				if (SSL_get_error(s->s, ec) ==
				    SSL_ERROR_ZERO_RETURN) {
					/* Stream concluded normally. Pass FIN to HTTP/3 stack. */
					ec = nghttp3_conn_read_stream(
						conn->h3conn, s->id, NULL, 0,
						/*fin=*/1);
					if (ec < 0) {
						ERR_raise_data(
							ERR_LIB_USER,
					...
					ec = nghttp3_conn_close_stream(
						conn->h3conn, s->id, aec);
					if (ec < 0) {
						ERR_raise_data(
							ERR_LIB_USER,
							ERR_R_INTERNAL_ERROR,
		...
		ec = nghttp3_conn_read_stream(conn->h3conn, s->id,
					      s->buf + s->buf_cur,
					      s->buf_total - s->buf_cur,
					      /*fin=*/0);
		if (ec < 0) {
			ERR_raise_data(
				ERR_LIB_USER, ERR_R_INTERNAL_ERROR,
...
int OSSL_DEMO_H3_CONN_handle_events(OSSL_DEMO_H3_CONN *conn)
{
	...
	for (;;) {
		if ((snew = SSL_accept_stream(
			     conn->qconn, SSL_ACCEPT_STREAM_NO_BLOCK)) == NULL)
			break;
	 ...
	 * after all data are written.
		...
		key.id = stream_id;
		if ((s = lh_OSSL_DEMO_H3_STREAM_retrieve(conn->streams,
							 &key)) == NULL) {
			ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR,
		...
		for (i = 0; i < num_vecs; ++i) {
			...
			if (s->s == NULL) {
				...
				written = vecs[i].len;
			} else if (!SSL_write_ex2(s->s, vecs[i].base,
						  vecs[i].len, flags,
						  &written)) {
				if (SSL_get_error(s->s, 0) ==
                     ...
                     */
					written = 0;
					nghttp3_conn_block_stream(conn->h3conn,
								  stream_id);
				} else {
					ERR_raise_data(
						ERR_LIB_USER,
                 ...
                 */
				nghttp3_conn_unblock_stream(conn->h3conn,
							    stream_id);
			}
			...
			if (written > 0) {
                 ...
                 */
				ec = nghttp3_conn_add_write_offset(
					conn->h3conn, stream_id, written);
				if (ec < 0)
					return 0;
				     ...
				     void *user_data)
{
	...
	if ((s_req = h3_conn_create_stream(
		     conn, OSSL_DEMO_H3_STREAM_TYPE_REQ)) == NULL)
		goto err;
