...
static int read_from_ssl_ids(nghttp3_conn **curh3conn, struct h3ssl *h3ssl)
{
	...
	for (i = 0, item = items; i < numitem; i++, item++) {
		...
		if (item->revents & SSL_POLL_EVENT_IC) {
			SSL *conn =
				SSL_accept_connection(item->desc.value.ssl, 0);
			SSL *oldconn;
			...
			if (nghttp3_conn_server_new(curh3conn, &callbacks,
						    &settings, mem, h3ssl)) {
				fprintf(stderr,
					"nghttp3_conn_client_new failed!\n");
				exit(1);
			}
			h3conn = *curh3conn;
			hassomething++;
			...
			if (!SSL_set_incoming_stream_policy(
				    conn, SSL_INCOMING_STREAM_POLICY_ACCEPT,
				    0)) {
				fprintf(stderr,
		...
		if ((item->revents & SSL_POLL_EVENT_ISB) ||
			...
			SSL *stream =
				SSL_accept_stream(item->desc.value.ssl, 0);
			uint64_t new_id;
			int r;
		...
		if (item->revents != processed_event) {
			...
			printf("revent %llu (%d) on %llu NOT PROCESSED!\n",
			       ...
			       SSL_POLL_EVENT_W, (unsigned long long)id);
		}
	}
	ret = hassomething;
err:
			     ...
			     size_t *written)
{
	...
	for (i = 0; i < MAXSSL_IDS; i++) {
		if (ssl_ids[i].id == streamid) {
			if (!SSL_write_ex2(ssl_ids[i].s, buff, len, flags,
					   written) ||
			    *written != len) {
				fprintf(stderr,
					"couldn't write on connection\n");
				ERR_print_errors_fp(stderr);
				return 0;
			}
...
restart:
		...
		while (!h3ssl.end_headers_received) {
			...
			if (hassomething == -1) {
				fprintf(stderr,
					"read_from_ssl_ids hassomething failed\n");
				goto err;
		...
		if (nghttp3_conn_submit_response(h3conn, h3ssl.id_bidi, resp,
						 num_nv, &dr)) {
			fprintf(stderr,
				"nghttp3_conn_submit_response failed!\n");
			goto err;
		}
		...
		for (;;) {
			...
			for (i = 0; i < sveccnt; i++) {
				...
				printf("quic_server_write on %llu for %ld\n",
				       (unsigned long long)streamid,
				       (unsigned long)vec[i].len);
				if (fin && i == sveccnt - 1)
					flagwrite = SSL_WRITE_FLAG_CONCLUDE;
				if (!quic_server_write(&h3ssl, streamid,
						       vec[i].base, vec[i].len,
						       flagwrite, &numbytes)) {
					fprintf(stderr,
						"quic_server_write failed!\n");
					goto err;
				}
			}
			if (nghttp3_conn_add_write_offset(
				    h3conn, streamid,
				    (size_t)nghttp3_vec_len(vec,
							    (size_t)sveccnt))) {
				fprintf(stderr,
					"nghttp3_conn_add_write_offset failed!\n");
				goto err;
			}
		}
		...
		if (h3ssl.datadone) {
             ...
             */
			if (!h3ssl.close_done) {
				set_id_status(h3ssl.id_bidi, SERVERCLOSED,
					      &h3ssl);
				h3ssl.close_wait = 1;
			}
		} else {
