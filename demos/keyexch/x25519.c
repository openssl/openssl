				 ...
				 PEER_DATA *local_peer)
{
	...
	if (kat_privk_data != NULL)
		local_peer->privk = EVP_PKEY_new_raw_private_key_ex(
			libctx, "X25519", propq, kat_privk_data,
	...
	if (EVP_PKEY_get_octet_string_param(
		    local_peer->privk, OSSL_PKEY_PARAM_PUB_KEY,
		    local_peer->pubk_data, sizeof(local_peer->pubk_data),
		    &pubk_data_len) == 0) {
				...
				const unsigned char *remote_peer_pubk_data)
{
	...
	BIO_dump_indent_fp(stdout, local_peer->secret,
			   (int)local_peer->secret_len, 2);
	putchar('\n');
