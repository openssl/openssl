...
CN="Test Revoked Cert" $OPENtls req -config ca.cnf -nodes \
	-keyout revkey.pem -out rreq.pem -newkey rsa:1024
# Sign using intermediate CA
$OPENtls x509 -req -in rreq.pem -CA intca.pem -CAkey intkey.pem -days 3600 \
	-extfile ca.cnf -extensions usr_cert -CAcreateserial -out rev.pem

# OCSP responder certificate: request first
CN="Test OCSP Responder Cert" $OPENtls req -config ca.cnf -nodes \
	-keyout respkey.pem -out respreq.pem -newkey rsa:1024
# Sign using intermediate CA and responder extensions
$OPENtls x509 -req -in respreq.pem -CA intca.pem -CAkey intkey.pem -days 3600 \
...
[ -f dhp.pem ] || $OPENtls genpkey -genparam -algorithm DH -pkeyopt dh_paramgen_prime_len:1024 -out dhp.pem

# Now a DH private key
$OPENtls genpkey -paramfile dhp.pem -out dhskey.pem
# Create DH public key file
$OPENtls pkey -in dhskey.pem -pubout -out dhspub.pem
# Certificate request, key just reuses old one as it is ignored when the
# request is signed.
CN="Test Server DH Cert" $OPENtls req -config ca.cnf -new \
	-key skey.pem -out dhsreq.pem
# Sign request: end entity DH extensions
$OPENtls x509 -req -in dhsreq.pem -CA root.pem -days 3600 \
	-force_pubkey dhspub.pem \
	-extfile ca.cnf -extensions dh_cert -CAcreateserial -out dhserver.pem

# DH client certificate

$OPENtls genpkey -paramfile dhp.pem -out dhckey.pem
$OPENtls pkey -in dhckey.pem -pubout -out dhcpub.pem
CN="Test Client DH Cert" $OPENtls req -config ca.cnf -new \
	-key skey.pem -out dhcreq.pem
$OPENtls x509 -req -in dhcreq.pem -CA root.pem -days 3600 \
...
$OPENtls ca -valid server.pem -keyfile root.pem -cert root.pem \
		-config ca.cnf -md sha1
$OPENtls ca -valid client.pem -keyfile root.pem -cert root.pem \
		-config ca.cnf -md sha1
$OPENtls ca -valid rev.pem -keyfile root.pem -cert root.pem \
		-config ca.cnf -md sha1
# Generate a CRL.
$OPENtls ca -gencrl -keyfile root.pem -cert root.pem -config ca.cnf \
		-md sha1 -crldays 1 -out crl1.pem
# Revoke a certificate
opentls ca -revoke rev.pem -crl_reason superseded \
