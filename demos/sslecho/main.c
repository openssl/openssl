...
/**
 * This program implements an SSL echo client/server.
 *
 * Spin up the server, then the client. The client sends lines from keyboard
 * input to the server, which then echos the lines back to the client.
 *
 * Revision History
 *
 * 11-Dec-2021      RCW
 *
 * 1.0.0    Initial version; working with 2 instances on one machine.
 *
 * 1.0.1    Binding server to INADDR_ANY (instead of 127.0.0.1).
 *          Client needs server ip on command line.
 *          Using ERR_print_errors_fp() for all SSL errors.
 *          Added kill command to terminate server.
 *
 * 1.0.2    Replaced 'server-' & 'client-' pem files with default
