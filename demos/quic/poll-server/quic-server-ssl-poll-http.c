...
{
	static const char *names[] = { "none",	      "listener",
				       "connection",  "stream (bidi)",
...
static struct poll_event *new_qconn_pe(SSL *ssl_qconn)
{
	...
	if (qconn_pe != NULL) {
		...
		qconn_pe->pe_want_events = SSL_POLL_EVENT_ISB |
					   SSL_POLL_EVENT_ISU;
		qconn_pe->pe_want_events |= SSL_POLL_EVENT_EC |
					    SSL_POLL_EVENT_ECD;
		/*
...
static int handle_write_stream_state(struct poll_event *pe)
{
	...
	default:
		DPRINTF(stderr,
			"%s error %s on stream, the %p (%s) should be destroyed\n",
			__func__, stream_state_str(state), pe,
			pe_type_to_name(pe));
		rv = -1;
	}
...
static int rebuild_poll_set(struct poll_manager *pm)
{
	...
	if (pe_num > pm->pm_poll_set_sz) {
         ...
         */
		new_poll_set = OPENSSL_realloc_array(
			pm->pm_poll_set, pm->pm_poll_set_sz + POLL_GROW,
         ...
         */
		new_poll_set = OPENSSL_realloc_array(
			pm->pm_poll_set, pm->pm_poll_set_sz - POLL_DOWNSIZ,
			sizeof(struct poll_event));
		if (new_poll_set == NULL)
			return -1;
		pm->pm_poll_set = new_poll_set;
...
static int parse_request(struct poll_event_stream *pes)
{
	...
	*dst = '\0';
	...
	pe->pe_appdata =
		new_txt_full_respoonse(file_name, get_fsize(file_name));
...
static void app_destroy_qconn(struct poll_event *pe)
{
	...
	OSSL_LIST_FOREACH_DELSAFE(peccx, peccx_save, peccx,
				  &pec->pec_unistream_cx)
	{
...
static int run_quic_server(SSL_CTX *ctx, struct poll_manager *pm, int fd)
{
	...
	while (pm->pm_continue) {
		...
		ok = SSL_poll((SSL_POLL_ITEM *)pm->pm_poll_set,
			      pm->pm_event_count, sizeof(struct poll_event),
			      NULL, 0, &poll_items);
		...
		for (i = 0; i < pm->pm_event_count; i++) {
			...
			DPRINTF(stderr, "%s %s (%p) " POLL_FMT "\n", __func__,
				...
				POLL_PRINTA(pe->pe_poll_item.revents));
			pe->pe_self->pe_poll_item.revents =
				pe->pe_poll_item.revents;
