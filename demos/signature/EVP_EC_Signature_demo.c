		       ...
		       size_t sig_len, unsigned char *sig_value)
{
	...
	/* Verify */
	if (!EVP_DigestVerifyInit_ex(verify_context, NULL, sig_name, libctx,
				     NULL, pub_key, NULL)) {
     ...
     * to include additional data.
     */
	if (!EVP_DigestVerifyUpdate(verify_context, hamlet_1,
				    strlen(hamlet_1))) {
		fprintf(stderr, "EVP_DigestVerifyUpdate(hamlet_1) failed.\n");
		goto cleanup;
	}
	if (!EVP_DigestVerifyUpdate(verify_context, hamlet_2,
				    strlen(hamlet_2))) {
