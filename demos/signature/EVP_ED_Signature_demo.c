		       ...
		       OSSL_LIB_CTX *libctx)
{
	...
	if (!EVP_DigestVerifyInit_ex(verify_context, NULL, NULL, libctx, NULL,
				     pub, NULL)) {
     ...
     */
	if (!EVP_DigestVerify(verify_context, sig_value, sig_len, tbs,
			      tbs_len)) {
...
int main(void)
{
	...
	if (!demo_verify(pub, hamlet, sizeof(hamlet), sig_value, sig_len,
			 libctx)) {
