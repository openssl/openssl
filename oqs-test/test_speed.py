...
def test_sig_speed(ossl, ossl_config, test_artifacts_dir, sig_name, worker_id):
    helpers.run_subprocess([ossl, 'speed', '-seconds', '1', sig_name])
...
@pytest.mark.parametrize('kem_name', [i for i in oqs_algorithms.key_exchanges if (not i.startswith("p256_") and i!="oqs_kem_default")])
def test_kem_speed(ossl, ossl_config, test_artifacts_dir, kem_name, worker_id):
    helpers.run_subprocess([ossl, 'speed', '-seconds', '1', kem_name])
