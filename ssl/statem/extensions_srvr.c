...
#define MAX_COOKIE_SIZE                                        \
	(2 + 2 + 2 + 2 + 2 + 1 + 8 + 2 + EVP_MAX_MD_SIZE + 1 + \
				  ...
				  size_t chainidx)
{
	...
	if (PACKET_remaining(&exts) > 0) {
		...
		sk_X509_EXTENSION_pop_free(s->ext.ocsp.exts,
					   X509_EXTENSION_free);
		s->ext.ocsp.exts = d2i_X509_EXTENSIONS(
			NULL, &ext_data, (int)PACKET_remaining(&exts));
		  ...
		  size_t *keyshares_cnt, size_t *keyshares_max)
{
	...
	*encoded_pubkey_arr = OPENSSL_malloc_array(
		*keyshares_max, sizeof(**encoded_pubkey_arr));
	...
	while (PACKET_remaining(key_share_list) > 0) {
		...
		if (key_share_pos < previous_key_share_pos)
			OSSL_TRACE1(
				TLS,
				"key share group id %d is out of RFC 8446 order\n",
				group_id);
		...
		if (s->s3.group_id != 0) {
             ...
             */
			if (!tls_accept_ksgroup(s, s->s3.group_id,
						&encoded_pubkey))
			  ...
			  int *candidate_group_idx, uint16_t *selected_group)
{
	...
	*selected_group = 0;
	...
	for (current_group = 0; current_group < candidate_num_groups;
	     current_group++) {
		if (!check_in_list(s, candidate_groups[current_group],
				   prio_groups, prio_num_groups, 1,
				   &new_group_idx) ||
			     ...
			     unsigned int context, X509 *x, size_t chainidx)
{
	...
	ks_extraction_result = extract_keyshares(
		...
		srvr_num_groups, &keyshares_arr, &encoded_pubkey_arr,
	...
	for (current_tuple = 0; current_tuple < srvr_num_tuples;
	     current_tuple++) {
		...
		if (s->options & SSL_OP_SERVER_PREFERENCE) {
			...
			check_overlap(s, first_group_in_tuple,
				      number_of_groups_in_tuple, keyshares_arr,
				      keyshares_cnt, &prio_group_idx,
				      &candidate_group_idx,
				      &group_id_candidate);
			...
			check_overlap(s, keyshares_arr, keyshares_cnt,
				      first_group_in_tuple,
				      number_of_groups_in_tuple,
			  ...
			  X509 *x, size_t chainidx)
{
	...
	if (sctx->verify_stateless_cookie_cb(
		    SSL_CONNECTION_GET_USER_SSL(s), PACKET_data(&appcookie),
		       ...
		       X509 *x, size_t chainidx)
{
	...
	for (id = 0; PACKET_remaining(&identities) != 0; id++) {
		...
		if (sess == NULL && s->psk_server_callback != NULL &&
			...
			pskdatalen = s->psk_server_callback(
				ussl, pskid, pskdata, sizeof(pskdata));
			OPENSSL_free(pskid);
		...
		if (sess != NULL) {
			...
			if ((s->options & SSL_OP_NO_TICKET) != 0 ||
				...
				ret = tls_get_stateful_ticket(s, &identity,
							      &sess);
			else
				ret = tls_decrypt_ticket(
					s, PACKET_data(&identity),
			...
			age = ossl_time_subtract(
				ossl_ms2time(ticket_agel),
			...
			if (id == 0 &&
			    ...
			    ossl_time_compare(
				    ossl_time_add(age, TICKET_AGE_ALLOWANCE),
				    expire) >= 0) {
				/*
	...
	binderoffset =
		PACKET_data(pkt) - (const unsigned char *)s->init_buf->data;
		...
		if (tls_valid_group(s, group, version, version, 0, NULL) &&
			...
			if (first) {
				...
				if (!WPACKET_put_bytes_u16(
					    pkt, TLSEXT_TYPE_supported_groups)
					...
					size_t chainidx)
{
	...
	if (!ginf->is_kem) {
		...
		if (!WPACKET_sub_memcpy_u16(pkt, encoded_pubkey,
					    encoded_pubkey_len) ||
		    !WPACKET_close(pkt)) {
			SSLfatal(s, SSL_AD_INTERNAL_ERROR,
				 ERR_R_INTERNAL_ERROR);
				     ...
				     size_t chainidx)
{
	...
	if (sctx->gen_stateless_cookie_cb(ussl, appcookie1, &appcookielen) ==
	    0) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR,
	...
	if (!ossl_assert(totcookielen <=
			 MAX_COOKIE_SIZE - SHA256_DIGEST_LENGTH)) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
		return EXT_RETURN_FAIL;
	}
				    ...
				    size_t chainidx)
{
	...
	sc->ext.server_cert_type_ctos = reconcile_cert_type(
		sc->server_cert_type, sc->server_cert_type_len, data, len,
