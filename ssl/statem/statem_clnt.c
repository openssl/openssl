...
#error Internal DTLS version error
			...
			if (!SSL_CONNECTION_IS_DTLS(s) &&
                 ...
                 */
				if (!tls13_restore_handshake_digest_for_pha(
					    s)) {
...
static int do_compressed_cert(SSL_CONNECTION *sc)
{
	...
	return sc->ext.client_cert_type == TLSEXT_cert_type_x509 &&
	       sc->ext.compress_certificate_from_peer[0] !=
...
static WRITE_TRAN ossl_statem_client13_write_transition(SSL_CONNECTION *s)
{
	...
	case TLS_ST_CW_CERT:
		...
		st->hand_state = (s->s3.tmp.cert_req == 1) ?
					 TLS_ST_CW_CERT_VRFY :
					 TLS_ST_CW_FINISHED;
		return WRITE_TRAN_CONTINUE;
...
WORK_STATE ossl_statem_client_pre_work(SSL_CONNECTION *s, WORK_STATE wst)
{
	...
	case TLS_ST_CW_CHANGE:
		if (SSL_CONNECTION_IS_DTLS(s)) {
			...
			if (BIO_dgram_is_sctp(
				    SSL_get_wbio(SSL_CONNECTION_GET_SSL(s)))) {
...
WORK_STATE ossl_statem_client_post_work(SSL_CONNECTION *s, WORK_STATE wst)
{
	...
	case TLS_ST_CW_FINISHED:
#ifndef OPENSSL_NO_SCTP
		if (wst == WORK_MORE_A && SSL_CONNECTION_IS_DTLS(s) &&
		    s->hit == 0) {
			/*
             * Change to new shared key of SCTP-Auth, will be ignored if
             * no SCTP used.
...
size_t ossl_statem_client_max_message_size(SSL_CONNECTION *s)
{
	...
	case TLS_ST_CR_SESSION_TICKET:
		return (SSL_CONNECTION_IS_TLS13(s)) ?
...
CON_FUNC_RETURN tls_construct_client_hello(SSL_CONNECTION *s, WPACKET *pkt)
{
	...
	if (s->new_session || s->session->ssl_version == TLS1_3_VERSION) {
		...
		if (s->version == TLS1_3_VERSION) {
			s->tmp_session_id_len = sess_id_len;
			memcpy(s->tmp_session_id, s->session->session_id,
			       sess_id_len);
		}
	}
...
MSG_PROCESS_RETURN tls_process_server_hello(SSL_CONNECTION *s, PACKET *pkt)
{
	...
	if (s->version == TLS1_3_VERSION && sversion == TLS1_2_VERSION &&
		...
		if (!PACKET_copy_bytes(pkt, s->s3.server_random,
				       SSL3_RANDOM_SIZE)) {
			SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_R_LENGTH_MISMATCH);
			goto err;
		}
	...
	if (s->hit) {
		if (s->sid_ctx_length != s->session->sid_ctx_length ||
		    memcmp(s->session->sid_ctx, s->sid_ctx,
			   s->sid_ctx_length)) {
			/* actually a client application bug */
			SSLfatal(
				s, SSL_AD_ILLEGAL_PARAMETER,
							     ...
							     PACKET *extpkt)
{
	...
	s->rlayer.wrlmethod->set_protocol_version(s->rlayer.wrl,
						  TLS1_3_VERSION);
	...
	for (chainidx = 0; PACKET_remaining(pkt); chainidx++) {
		...
		if (SSL_CONNECTION_IS_TLS13(s)) {
			...
			if (!tls_collect_extensions(
				    s, &extensions, SSL_EXT_TLS1_3_CERTIFICATE,
				    &rawexts, NULL, chainidx == 0) ||
			    !tls_parse_all_extensions(
				    s, SSL_EXT_TLS1_3_CERTIFICATE, rawexts, x,
...
static int tls_process_ske_dhe(SSL_CONNECTION *s, PACKET *pkt, EVP_PKEY **pkey)
{
	...
	g = BN_bin2bn(PACKET_data(&generator),
				 ...
				 EVP_PKEY **pkey)
{
	...
	if (EVP_PKEY_set1_encoded_public_key(
		    s->s3.peer_tmp, PACKET_data(&encoded_pt),
...
MSG_PROCESS_RETURN tls_process_key_exchange(SSL_CONNECTION *s, PACKET *pkt)
{
	...
	if (pkey != NULL) {
		...
		rv = EVP_DigestVerify(md_ctx, PACKET_data(&signature),
				      PACKET_remaining(&signature), tbs,
				      tbslen);
		OPENSSL_free(tbs);
		if (rv <= 0) {
						   ...
						   PACKET *pkt)
{
	...
	if (SSL_CONNECTION_IS_TLS13(s)) {
		...
		if (!PACKET_get_length_prefixed_1(pkt, &reqctx) ||
		    !PACKET_memdup(&reqctx, &s->pha_context,
				   &s->pha_context_len)) {
			SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_R_LENGTH_MISMATCH);
			return MSG_PROCESS_ERROR;
		}
		...
		if (!tls_collect_extensions(s, &extensions,
					    ...
					    &rawexts, NULL, 1) ||
		    !tls_parse_all_extensions(
			    s, SSL_EXT_TLS1_3_CERTIFICATE_REQUEST, rawexts,
			    NULL, 0, 1)) {
			/* SSLfatal() already called */
			OPENSSL_free(rawexts);
			return MSG_PROCESS_ERROR;
		...
		if (!PACKET_memdup(&ctypes, &s->s3.tmp.ctype,
				   &s->s3.tmp.ctype_len)) {
			SSLfatal(s, SSL_AD_INTERNAL_ERROR,
				 ERR_R_INTERNAL_ERROR);
			return MSG_PROCESS_ERROR;
				 ...
				 PACKET *pkt)
{
	...
	if (PACKET_remaining(pkt) > 0) {
		...
		if (resplen > 0) {
			...
			sk_OCSP_RESPONSE_insert(s->ext.ocsp.resp_ex, resp,
						(int)chainidx);
		}
	}
...
static int tls_construct_cke_psk_preamble(SSL_CONNECTION *s, WPACKET *pkt)
{
	...
	if (psklen > PSK_MAX_PSK_LEN) {
		...
		psklen =
			PSK_MAX_PSK_LEN; /* Avoid overrunning the array on cleanse */
		goto err;
	} else if (psklen == 0) {
		SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE,
...
int ossl_gost_ukm(const SSL_CONNECTION *s, unsigned char *dgst_buf)
{
	...
	unsigned int md_len;
	SSL_CTX *sctx = SSL_CONNECTION_GET_CTX(s);
	const EVP_MD *md = ssl_evp_md_fetch(
		sctx->libctx, NID_id_GostR3411_2012_256, sctx->propq);
	...
	if ((hash = EVP_MD_CTX_new()) == NULL ||
	    ...
	    EVP_DigestUpdate(hash, s->s3.client_random, SSL3_RANDOM_SIZE) <=
		    0 ||
	    EVP_DigestUpdate(hash, s->s3.server_random, SSL3_RANDOM_SIZE) <=
		    0 ||
							    ...
							    WPACKET *pkt)
{
	...
	comp_len = COMP_compress_block(comp, WPACKET_get_curr(pkt),
				       (int)max_length,
...
int ssl_do_client_cert_cb(SSL_CONNECTION *s, X509 **px509, EVP_PKEY **ppkey)
{
	...
	if (sctx->client_cert_cb)
		i = sctx->client_cert_cb(SSL_CONNECTION_GET_USER_SSL(s), px509,
					 ppkey);
	return i;
}
...
#error Max cipher length too short
	...
	for (i = 0; i < sk_SSL_CIPHER_num(sk) && totlen < maxlen; i++) {
		...
		if (!maxverok) {
			int minproto = SSL_CONNECTION_IS_DTLS(s) ? c->min_dtls :
								   c->min_tls;
			int maxproto = SSL_CONNECTION_IS_DTLS(s) ? c->max_dtls :
								   c->max_tls;

			if (ssl_version_cmp(s, maxproto, s->s3.tmp.max_ver) >=
				    0 &&
			    ssl_version_cmp(s, minproto, s->s3.tmp.max_ver) <=
				    0)
				maxverok = 1;
		}
	...
	if (totlen == 0 || !maxverok) {
		...
		SSLfatal_data(s, SSL_AD_INTERNAL_ERROR,
			      SSL_R_NO_CIPHERS_AVAILABLE, maxvertext);
		return 0;
	}
	...
	if (totlen != 0) {
		if (empty_reneg_info_scsv) {
			static const SSL_CIPHER scsv = {
				0, NULL, NULL, SSL3_CK_SCSV, 0, 0, 0, 0, 0, 0,
				0, 0,	 0
			};
			if (!ssl->method->put_cipher_by_char(&scsv, pkt,
							     &len)) {
				SSLfatal(s, SSL_AD_INTERNAL_ERROR,
					 ERR_R_INTERNAL_ERROR);
				return 0;
		...
		if (s->mode & SSL_MODE_SEND_FALLBACK_SCSV) {
			static const SSL_CIPHER scsv = {
				0, NULL, NULL, SSL3_CK_FALLBACK_SCSV,
				0, 0,	 0,    0,
				0, 0,	 0,    0,
				0
			};
