...
int custom_ext_parse(SSL_CONNECTION *s, unsigned int context,
		     unsigned int ext_type, const unsigned char *ext_data,
		     size_t ext_size, X509 *x, size_t chainidx)
{
	int al = 0;
	...
	if ((context & (SSL_EXT_CLIENT_HELLO | SSL_EXT_TLS1_2_SERVER_HELLO)) !=
	    0)
	...
	if ((context &
		...
		if ((meth->ext_flags & SSL_EXT_FLAG_SENT) == 0) {
			SSLfatal(s, TLS1_AD_UNSUPPORTED_EXTENSION,
				 SSL_R_BAD_EXTENSION);
			return 0;
		}
	}
     ...
     */
	if ((context &
	     (SSL_EXT_CLIENT_HELLO | SSL_EXT_TLS1_3_CERTIFICATE_REQUEST)) != 0)
	...
	if (meth->parse_cb(SSL_CONNECTION_GET_USER_SSL(s), ext_type, context,
			   ext_data, ext_size, x, chainidx, &al,
			   meth->parse_arg) <= 0) {
		   ...
		   size_t chainidx, int maxversion)
{
	...
	for (i = 0; i < exts->meths_count; i++) {
		...
		if (!should_add_extension(s, meth->context, context,
					  maxversion))
			continue;
		...
		if (meth->add_cb != NULL) {
			int cb_retval = meth->add_cb(
				SSL_CONNECTION_GET_USER_SSL(s), meth->ext_type,
				context, &out, &outlen, x, chainidx, &al,
				meth->add_arg);
		...
		if (!WPACKET_put_bytes_u16(pkt, meth->ext_type) ||
			...
			if (meth->free_cb != NULL)
				meth->free_cb(SSL_CONNECTION_GET_USER_SSL(s),
			   ...
			   const custom_ext_methods *src)
{
	...
	for (i = 0; i < src->meths_count; i++, methsrc++) {
		custom_ext_method *methdst = custom_ext_find(
			dst, methsrc->role, methsrc->ext_type, NULL);
...
static void custom_ext_copy_old_cb(custom_ext_method *methdst,
				   const custom_ext_method *methsrc, int *err)
{
...
int custom_exts_copy(custom_ext_methods *dst, const custom_ext_methods *src)
{
	...
	if (src->meths_count > 0) {
		dst->meths = OPENSSL_memdup(
			src->meths, sizeof(*src->meths) * src->meths_count);
		if (dst->meths == NULL)
			return 0;
		dst->meths_count = src->meths_count;

		for (i = 0; i < src->meths_count; i++)
			custom_ext_copy_old_cb(&dst->meths[i], &src->meths[i],
					       &err);
	}
			  ...
			  const custom_ext_methods *src)
{
	...
	if (src->meths_count > 0) {
		...
		if (meths_count > 0) {
			custom_ext_method *methdst = OPENSSL_realloc(
				dst->meths, (dst->meths_count + meths_count) *
			...
			for (i = 0; i < dst->meths_count; i++)
				custom_ext_copy_old_cb(&methdst[i],
						       &dst->meths[i], &err);
			...
			for (i = 0; i < src->meths_count; i++) {
				...
				if ((methsrc->ext_flags & SSL_EXT_FLAG_CONN) ==
				    0)
					continue;

				memcpy(methdst, methsrc,
				       sizeof(custom_ext_method));
...
int ossl_tls_add_custom_ext_intern(
	SSL_CTX *ctx, custom_ext_methods *exts, ENDPOINT role,
	unsigned int ext_type, unsigned int context,
	...
	void *add_arg, SSL_custom_ext_parse_cb_ex parse_cb, void *parse_arg)
{
	...
	if (ext_type == TLSEXT_TYPE_signed_certificate_timestamp &&
	    (context & SSL_EXT_CLIENT_HELLO) != 0 && ctx != NULL &&
	...
	if (custom_ext_find(exts, role, ext_type, NULL))
		return 0;
	tmp = OPENSSL_realloc(exts->meths, (exts->meths_count + 1) *
						   sizeof(custom_ext_method));
	if (tmp == NULL)
		return 0;
			      ...
			      custom_ext_parse_cb parse_cb, void *parse_arg)
{
	...
	ret = ossl_tls_add_custom_ext_intern(
		ctx, NULL, role, ext_type, context, custom_ext_add_old_cb_wrap,
		custom_ext_free_old_cb_wrap, add_cb_wrap,
		custom_ext_parse_old_cb_wrap, parse_cb_wrap);
...
int SSL_CTX_add_client_custom_ext(SSL_CTX *ctx, unsigned int ext_type,
				  ...
				  custom_ext_parse_cb parse_cb, void *parse_arg)
{
	return add_old_custom_ext(
		ctx, ENDPOINT_CLIENT, ext_type,
...
int SSL_CTX_add_custom_ext(SSL_CTX *ctx, unsigned int ext_type,
			   ...
			   SSL_custom_ext_free_cb_ex free_cb, void *add_arg,
			   SSL_custom_ext_parse_cb_ex parse_cb, void *parse_arg)
{
	return ossl_tls_add_custom_ext_intern(ctx, NULL, ENDPOINT_BOTH,
					      ext_type, context, add_cb,
