 ...
 */
static int custom_ext_add_old_cb_wrap(tls *s, unsigned int ext_type,
...
static void custom_ext_free_old_cb_wrap(tls *s, unsigned int ext_type,
                                        unsigned int context,
...
static int custom_ext_parse_old_cb_wrap(tls *s, unsigned int ext_type,
                                        unsigned int context,
 ...
 * be set independently on the same tls_CTX. It is set to ENDPOINT_SERVER if we
...
int custom_ext_parse(tls *s, unsigned int context, unsigned int ext_type,
                     ...
                     size_t chainidx)
{
    ...
    if ((context & (tls_EXT_CLIENT_HELLO | tls_EXT_TLS1_2_SERVER_HELLO)) != 0)
    ...
    if ((context & (tls_EXT_TLS1_2_SERVER_HELLO
                    | tls_EXT_TLS1_3_SERVER_HELLO
                    | tls_EXT_TLS1_3_ENCRYPTED_EXTENSIONS)) != 0) {
        /*
         * If it's ServerHello or EncryptedExtensions we can't have any
         * extensions not sent in ClientHello.
         */
        if ((meth->ext_flags & tls_EXT_FLAG_SENT) == 0) {
            tlsfatal(s, TLS1_AD_UNSUPPORTED_EXTENSION, tls_F_CUSTOM_EXT_PARSE,
                     tls_R_BAD_EXTENSION);
            return 0;
        }
    }
     ...
     * tls_EXT_FLAG_RECEIVED. This is so we know to add the equivalent
     * extensions in the ServerHello/EncryptedExtensions message
     */
    if ((context & tls_EXT_CLIENT_HELLO) != 0)
        meth->ext_flags |= tls_EXT_FLAG_RECEIVED;
    ...
    if (meth->parse_cb(s, ext_type, context, ext_data, ext_size, x, chainidx,
                       &al, meth->parse_arg) <= 0) {
        tlsfatal(s, al, tls_F_CUSTOM_EXT_PARSE, tls_R_BAD_EXTENSION);
        return 0;
    }
 ...
 * buffer.
 */
int custom_ext_add(tls *s, int context, WPACKET *pkt, X509 *x, size_t chainidx,
                   int maxversion)
{
    custom_ext_methods *exts = &s->cert->custext;
    custom_ext_method *meth;
    ...
    for (i = 0; i < exts->meths_count; i++) {
        ...
        if ((context & (tls_EXT_TLS1_2_SERVER_HELLO
                        ...
                        | tls_EXT_TLS1_3_CERTIFICATE
                        | tls_EXT_TLS1_3_HELLO_RETRY_REQUEST)) != 0) {
            /* Only send extensions present in ClientHello. */
            if (!(meth->ext_flags & tls_EXT_FLAG_RECEIVED))
                continue;
        }
        /*
         ...
         */
        if ((context & tls_EXT_CLIENT_HELLO) == 0 && meth->add_cb == NULL)
            continue;
        ...
        if (meth->add_cb != NULL) {
            ...
            if (cb_retval < 0) {
                tlsfatal(s, al, tls_F_CUSTOM_EXT_ADD, tls_R_CALLBACK_FAILED);
                return 0;       /* error */
            }
            if (cb_retval == 0)
        ...
        if (!WPACKET_put_bytes_u16(pkt, meth->ext_type)
                ...
                || !WPACKET_close(pkt)) {
            tlsfatal(s, tls_AD_INTERNAL_ERROR, tls_F_CUSTOM_EXT_ADD,
        ...
        if ((context & tls_EXT_CLIENT_HELLO) != 0) {
             ...
             */
            if (!otls_assert((meth->ext_flags & tls_EXT_FLAG_SENT) == 0)) {
                tlsfatal(s, tls_AD_INTERNAL_ERROR, tls_F_CUSTOM_EXT_ADD,
                         ERR_R_INTERNAL_ERROR);
                return 0;
            }
             ...
             */
            meth->ext_flags |= tls_EXT_FLAG_SENT;
        }
...
int custom_exts_copy(custom_ext_methods *dst, const custom_ext_methods *src)
{
    ...
    if (src->meths_count > 0) {
        ...
        for (i = 0; i < src->meths_count; i++) {
            ...
            methdst->add_arg = OPENtls_memdup(methsrc->add_arg,
                                              sizeof(custom_ext_add_cb_wrap));
            methdst->parse_arg = OPENtls_memdup(methsrc->parse_arg,
...
void custom_exts_free(custom_ext_methods *exts)
{
    ...
    for (i = 0, meth = exts->meths; i < exts->meths_count; i++, meth++) {
        ...
        OPENtls_free(meth->add_arg);
        OPENtls_free(meth->parse_arg);
    }
    OPENtls_free(exts->meths);
}

/* Return true if a client custom extension exists, false otherwise */
int tls_CTX_has_client_custom_ext(const tls_CTX *ctx, unsigned int ext_type)
{
    return custom_ext_find(&ctx->cert->custext, ENDPOINT_CLIENT, ext_type,
                           NULL) != NULL;
}

static int add_custom_ext_intern(tls_CTX *ctx, ENDPOINT role,
                                 ...
                                 void *parse_arg)
{
     ...
     */
    if (tls_extension_supported(ext_type)
    ...
    if (custom_ext_find(exts, role, ext_type, NULL))
        return 0;
    tmp = OPENtls_realloc(exts->meths,
...
static int add_old_custom_ext(tls_CTX *ctx, ENDPOINT role,
                              unsigned int ext_type,
                              unsigned int context,
                              custom_ext_add_cb add_cb,
                              custom_ext_free_cb free_cb,
                              ...
                              custom_ext_parse_cb parse_cb, void *parse_arg)
{
    custom_ext_add_cb_wrap *add_cb_wrap
        = OPENtls_malloc(sizeof(*add_cb_wrap));
    custom_ext_parse_cb_wrap *parse_cb_wrap
        = OPENtls_malloc(sizeof(*parse_cb_wrap));
    int ret;

    if (add_cb_wrap == NULL || parse_cb_wrap == NULL) {
        OPENtls_free(add_cb_wrap);
        OPENtls_free(parse_cb_wrap);
        return 0;
    }
...
int tls_CTX_add_client_custom_ext(tls_CTX *ctx, unsigned int ext_type,
                                  custom_ext_add_cb add_cb,
                                  custom_ext_free_cb free_cb,
                                  void *add_arg,
                                  custom_ext_parse_cb parse_cb, void *parse_arg)
...
int tls_CTX_add_server_custom_ext(tls_CTX *ctx, unsigned int ext_type,
                                  custom_ext_add_cb add_cb,
                                  custom_ext_free_cb free_cb,
                                  void *add_arg,
                                  custom_ext_parse_cb parse_cb, void *parse_arg)
...
int tls_CTX_add_custom_ext(tls_CTX *ctx, unsigned int ext_type,
                           unsigned int context,
                           tls_custom_ext_add_cb_ex add_cb,
                           tls_custom_ext_free_cb_ex free_cb,
                           void *add_arg,
                           tls_custom_ext_parse_cb_ex parse_cb, void *parse_arg)
{
    return add_custom_ext_intern(ctx, ENDPOINT_BOTH, ext_type, context, add_cb,
                                 free_cb, add_arg, parse_cb, parse_arg);
}

int tls_extension_supported(unsigned int ext_type)
{
    switch (ext_type) {
