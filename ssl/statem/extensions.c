...
static int tls_init_compress_certificate(SSL_CONNECTION *sc,
					 unsigned int context);
static EXT_RETURN tls_construct_compress_certificate(SSL_CONNECTION *sc,
						     WPACKET *pkt,
						     unsigned int context,
...
#define INVALID_EXTENSION \
	{ TLSEXT_TYPE_invalid, 0, NULL, NULL, NULL, NULL, NULL, NULL }
static const EXTENSION_DEFINITION ext_defs[] = {
	...
	{ TLSEXT_TYPE_server_name,
	  ...
	  init_server_name, tls_parse_ctos_server_name,
	  tls_parse_stoc_server_name, tls_construct_stoc_server_name,
	  tls_construct_ctos_server_name, final_server_name },
...
#ifndef OPENSSL_NO_SRP
	{ TLSEXT_TYPE_srp, SSL_EXT_CLIENT_HELLO | SSL_EXT_TLS1_2_AND_BELOW_ONLY,
	  init_srp, tls_parse_ctos_srp, NULL, NULL, tls_construct_ctos_srp,
	  NULL },
#else
	INVALID_EXTENSION,
#endif
	{ TLSEXT_TYPE_ec_point_formats,
	  ...
	  tls_parse_stoc_ec_pt_formats, tls_construct_stoc_ec_pt_formats,
	  tls_construct_ctos_ec_pt_formats, final_ec_pt_formats },
	{ /*
	...
	{ TLSEXT_TYPE_client_cert_type,
	  SSL_EXT_CLIENT_HELLO | SSL_EXT_TLS1_3_ENCRYPTED_EXTENSIONS |
		  SSL_EXT_TLS1_2_SERVER_HELLO,
	  init_client_cert_type, tls_parse_ctos_client_cert_type,
	  tls_parse_stoc_client_cert_type, tls_construct_stoc_client_cert_type,
	  tls_construct_ctos_client_cert_type, NULL },
	{ TLSEXT_TYPE_server_cert_type,
	  SSL_EXT_CLIENT_HELLO | SSL_EXT_TLS1_3_ENCRYPTED_EXTENSIONS |
		  SSL_EXT_TLS1_2_SERVER_HELLO,
	  init_server_cert_type, tls_parse_ctos_server_cert_type,
	  tls_parse_stoc_server_cert_type, tls_construct_stoc_server_cert_type,
	  tls_construct_ctos_server_cert_type, NULL },
	{ TLSEXT_TYPE_signature_algorithms,
	  SSL_EXT_CLIENT_HELLO | SSL_EXT_TLS1_3_CERTIFICATE_REQUEST,
	  init_sig_algs, tls_parse_ctos_sig_algs, tls_parse_ctos_sig_algs,
	  tls_construct_ctos_sig_algs, tls_construct_ctos_sig_algs,
	  final_sig_algs },
	...
	{ TLSEXT_TYPE_compress_certificate,
	  SSL_EXT_CLIENT_HELLO | SSL_EXT_TLS1_3_CERTIFICATE_REQUEST |
		  SSL_EXT_TLS_IMPLEMENTATION_ONLY | SSL_EXT_TLS1_3_ONLY,
	  tls_init_compress_certificate, tls_parse_compress_certificate,
	  tls_parse_compress_certificate, tls_construct_compress_certificate,
	...
	{ /* Must be immediately before pre_shared_key */
	  TLSEXT_TYPE_padding, SSL_EXT_CLIENT_HELLO, NULL,
			      ...
			      RAW_EXTENSION *exts)
{
	...
	for (thisext = exts, i = 0; i < num_exts; i++, thisext++) {
		...
		if (i < builtin_num) {
			...
			meth = custom_ext_find(&s->cert->custext, role,
					       thisext->type, &offset);
			  ...
			  unsigned int thisctx)
{
	...
	if ((SSL_CONNECTION_IS_DTLS(s) &&
	    ...
	    (s->version == SSL3_VERSION && (extctx & SSL_EXT_SSL3_ALLOWED) == 0)
	    /*
...
int tls_collect_extensions(SSL_CONNECTION *s, PACKET *packet,
			   unsigned int context, RAW_EXTENSION **res,
			   size_t *len, int init)
{
	...
	*res = NULL;
	...
	num_exts =
		OSSL_NELEM(ext_defs) + (exts != NULL ? exts->meths_count : 0);
	...
	if (init) {
		...
		for (thisexd = ext_defs, i = 0; i < OSSL_NELEM(ext_defs);
		     i++, thisexd++) {
			if (thisexd->init != NULL &&
			    (thisexd->context & context) != 0 &&
			    extension_is_relevant(s, thisexd->context,
						  context) &&
			...
			RAW_EXTENSION *exts, X509 *x, size_t chainidx)
{
	...
	int (*parser)(SSL_CONNECTION *s, PACKET *pkt, unsigned int context,
		      X509 *x, size_t chainidx) = NULL;
	...
	return custom_ext_parse(s, context, currext->type,
				...
				PACKET_remaining(&currext->data), x, chainidx);
}
			     ...
			     unsigned int context, X509 *x, size_t chainidx)
{
	...
	for (i = 0, thisexd = ext_defs; i < OSSL_NELEM(ext_defs);
		...
		/* Skip if not relevant for our context */
		if (!should_add_extension(s, thisexd->context, context,
					  max_version))
			continue;
...
static int final_renegotiate(SSL_CONNECTION *s, unsigned int context, int sent)
{
	...
	if (s->renegotiate &&
	    !(s->options & SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION) && !sent) {
		SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE,
			 SSL_R_UNSAFE_LEGACY_RENEGOTIATION_DISABLED);
		return 0;
	}
...
static int final_server_name(SSL_CONNECTION *s, unsigned int context, int sent)
{
	...
	if (!ossl_assert(sctx != NULL) ||
	    !ossl_assert(s->session_ctx != NULL)) {
	...
	if (sctx->ext.servername_cb != NULL)
		...
		ret = s->session_ctx->ext.servername_cb(
			ussl, &altmp, s->session_ctx->ext.servername_arg);
	...
	if (SSL_IS_FIRST_HANDSHAKE(s) && sctx != s->session_ctx &&
		...
		ssl_tsan_decr(s->session_ctx,
			      &s->session_ctx->stats.sess_accept);
	}
     ...
     */
	if (ret == SSL_TLSEXT_ERR_OK && s->ext.ticket_expected && was_ticket &&
			       ...
			       int sent)
{
     ...
     */
	if (s->ext.ecpointformats != NULL && s->ext.ecpointformats_len > 0 &&
...
static int init_status_request(SSL_CONNECTION *s, unsigned int context)
{
	if (s->server) {
		...
		sk_OCSP_RESPONSE_pop_free(s->ext.ocsp.resp_ex,
					  OCSP_RESPONSE_free);
	...
	if (s->server) {
		if (s->s3.peer_tmp != NULL) {
			...
			if (s->hello_retry_request == SSL_HRR_NONE && sent &&
			    (!s->hit || (s->ext.psk_kex_mode &
					 TLSEXT_KEX_MODE_FLAG_KE_DHE) != 0)) {
				/* Did we detect group overlap in tls_parse_ctos_key_share ? */
				if (s->s3.group_id_candidate != 0) {
					/* A shared group exists so send a HelloRetryRequest */
					s->s3.group_id =
						s->s3.group_id_candidate;
					s->hello_retry_request =
						SSL_HRR_PENDING;
					return 1;
				}
			}
			if (!s->hit || (s->ext.psk_kex_mode &
					TLSEXT_KEX_MODE_FLAG_KE) == 0) {
				/* Nothing left we can do - just fail */
				SSLfatal(s,
					 sent ? SSL_AD_HANDSHAKE_FAILURE :
...
int tls_psk_do_binder(SSL_CONNECTION *s, const EVP_MD *md,
		      const unsigned char *msgstart, size_t binderoffset,
		      const unsigned char *binderin, unsigned char *binderout,
		      SSL_SESSION *sess, int sign, int external)
{
	...
	if (external && s->early_data_state == SSL_EARLY_DATA_CONNECTING &&
	    s->session->ext.max_early_data == 0 && sess->ext.max_early_data > 0)
		usepskfored = 1;
     ...
     */
	mctx = EVP_MD_CTX_new();
	if (mctx == NULL || EVP_DigestInit_ex(mctx, md, NULL) <= 0 ||
	...
	if (!tls13_derive_finishedkey(s, md, binderkey, finishedkey,
				      hashsize)) {
		/* SSLfatal() already called */
		goto err;
	}
	...
	mackey = EVP_PKEY_new_raw_private_key_ex(
		sctx->libctx, "HMAC", sctx->propq, finishedkey, hashsize);
	if (mackey == NULL) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
		goto err;
	}
	...
	bindersize = hashsize;
	if (EVP_DigestSignInit_ex(mctx, NULL, EVP_MD_get0_name(md),
				  sctx->libctx, sctx->propq, mackey,
				  NULL) <= 0 ||
...
static int final_early_data(SSL_CONNECTION *s, unsigned int context, int sent)
{
	...
	if (!s->server) {
		if (context == SSL_EXT_TLS1_3_ENCRYPTED_EXTENSIONS && sent &&
		    !s->ext.early_data_ok) {
			/*
	...
	if (s->max_early_data == 0 || !s->hit ||
	    s->early_data_state != SSL_EARLY_DATA_ACCEPTING ||
	    !s->ext.early_data_ok || s->hello_retry_request != SSL_HRR_NONE ||
				...
				int sent)
{
	...
	if (s->session->ext.max_fragment_len_mode ==
	    TLSEXT_max_fragment_length_UNSPECIFIED)
		s->session->ext.max_fragment_len_mode =
			TLSEXT_max_fragment_length_DISABLED;

	if (USE_MAX_FRAGMENT_LENGTH_EXT(s->session)) {
		s->rlayer.rrlmethod->set_max_frag_len(
			s->rlayer.rrl, GET_MAX_FRAGMENT_LENGTH(s->session));
		s->rlayer.wrlmethod->set_max_frag_len(
			s->rlayer.wrl, ssl_get_max_send_fragment(s));
	}
...
int tls_parse_compress_certificate(SSL_CONNECTION *sc, PACKET *pkt,
				   unsigned int context, X509 *x,
				   size_t chainidx)
{
#ifndef OPENSSL_NO_COMP_ALG
