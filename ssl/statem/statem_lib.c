...
const unsigned char hrrrandom[] = { 0xcf, 0x21, 0xad, 0x74, 0xe5, 0x9a, 0x61,
				    0x11, 0xbe, 0x1d, 0x8c, 0x02, 0x1e, 0x65,
				    0xb8, 0x91, 0xc2, 0xa2, 0x11, 0x16, 0x7a,
				    0xbb, 0x8c, 0x5e, 0x07, 0x9e, 0x09, 0xe2,
				    0xc8, 0xa8, 0x33, 0x9c };
...
int ssl3_do_write(SSL_CONNECTION *s, uint8_t type)
{
	...
	if (s->statem.mutate_handshake_cb != NULL &&
	    !s->statem.write_in_progress && type == SSL3_RT_HANDSHAKE &&
	    s->init_num >= SSL3_HM_HEADER_LENGTH) {
		unsigned char *msg;
		size_t msglen;

		if (!s->statem.mutate_handshake_cb(
			    (unsigned char *)s->init_buf->data, s->init_num,
			    &msg, &msglen, s->statem.mutatearg))
			return -1;
...
int tls_setup_handshake(SSL_CONNECTION *s)
{
	...
	if (sctx->ssl_digest_methods[SSL_MD_MD5_SHA1_IDX] == NULL) {
		...
		if (ssl_version_cmp(s, ver_max, md5sha1_needed_maxversion) <=
		    0) {
			SSLfatal_data(
		...
		if (ssl_version_cmp(s, ver_min, negotiated_minversion) < 0)
			ok = SSL_set_min_proto_version(ssl,
	...
	if (s->server) {
		...
		for (i = 0; i < sk_SSL_CIPHER_num(ciphers); i++) {
			...
			if (ssl_version_cmp(s, ver_max, cipher_minprotover) >=
				    0 &&
			    ssl_version_cmp(s, ver_max, cipher_maxprotover) <=
				    0) {
				ok = 1;
				break;
			}
		...
		if (SSL_IS_FIRST_HANDSHAKE(s)) {
			...
			ssl_tsan_counter(sctx,
					 &sctx->stats.sess_accept_renegotiate);
		...
		if (SSL_IS_FIRST_HANDSHAKE(s))
			ssl_tsan_counter(s->session_ctx,
					 &s->session_ctx->stats.sess_connect);
		else
			ssl_tsan_counter(
				s->session_ctx,
				    ...
				    void **hdata, size_t *hdatalen)
{
	...
	if (SSL_CONNECTION_IS_TLS13(s)) {
		...
		if (s->statem.hand_state == TLS_ST_CR_CERT_VRFY ||
			...
			memcpy(tls13tbs + TLS13_TBS_PREAMBLE_SIZE,
			       s->cert_verify_hash, s->cert_verify_hash_len);
			hashlen = s->cert_verify_hash_len;
		} else if (!ssl_handshake_hash(
				   s, tls13tbs + TLS13_TBS_PREAMBLE_SIZE,
...
CON_FUNC_RETURN tls_construct_cert_verify(SSL_CONNECTION *s, WPACKET *pkt)
{
	...
	if (EVP_DigestSignInit_ex(mctx, &pctx,
				  ...
				  sctx->libctx, sctx->propq, pkey, NULL) <= 0) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_EVP_LIB);
		goto err;
	}
...
MSG_PROCESS_RETURN tls_process_cert_verify(SSL_CONNECTION *s, PACKET *pkt)
{
	...
	if (EVP_DigestVerifyInit_ex(
		    mctx, &pctx, md == NULL ? NULL : EVP_MD_get0_name(md),
		    sctx->libctx, sctx->propq, pkey, NULL) <= 0) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_EVP_LIB);
		goto err;
	}
...
CON_FUNC_RETURN tls_construct_finished(SSL_CONNECTION *s, WPACKET *pkt)
{
	...
	if (SSL_CONNECTION_IS_TLS13(s) && !s->server &&
	    ...
	    s->s3.tmp.cert_req == 0 &&
	    (!ssl->method->ssl3_enc->change_cipher_state(
	...
	finish_md_len = ssl->method->ssl3_enc->final_finish_mac(
		s, sender, slen, s->s3.tmp.finish_md);
	if (finish_md_len == 0) {
		/* SSLfatal() already called */
		return CON_FUNC_ERROR;
	}
...
int ssl3_take_mac(SSL_CONNECTION *s)
{
	...
	s->s3.tmp.peer_finish_md_len = ssl->method->ssl3_enc->final_finish_mac(
...
MSG_PROCESS_RETURN tls_process_finished(SSL_CONNECTION *s, PACKET *pkt)
{
	...
	ok = CRYPTO_memcmp(PACKET_data(pkt), s->s3.tmp.peer_finish_md, md_len);
#ifdef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
	if (ok != 0) {
	...
	if (SSL_CONNECTION_IS_TLS13(s)) {
		if (s->server) {
			...
			size_t dummy;
			if (!ssl->method->ssl3_enc->generate_master_secret(
	...
	if (was_first && !SSL_IS_FIRST_HANDSHAKE(s) &&
...
CON_FUNC_RETURN tls_construct_change_cipher_spec(SSL_CONNECTION *s,
						 WPACKET *pkt)
{
...
static int ssl_add_cert_to_wpacket(SSL_CONNECTION *s, WPACKET *pkt, X509 *x,
				   int chain, int for_comp)
{
	int len;
			      ...
			      int for_comp)
{
	...
	if (chain_store != NULL) {
		X509_STORE_CTX *xs_ctx =
			X509_STORE_CTX_new_ex(sctx->libctx, sctx->propq);
...
int tls_process_rpk(SSL_CONNECTION *sc, PACKET *pkt, EVP_PKEY **peer_rpk)
{
	...
	if (SSL_CONNECTION_IS_TLS13(sc)) {
		if (!PACKET_get_length_prefixed_1(pkt, &context)) {
			SSLfatal(sc, SSL_AD_DECODE_ERROR,
		...
		if (sc->server) {
			if (sc->pha_context == NULL) {
				...
				if (!PACKET_equal(&context, sc->pha_context,
						  sc->pha_context_len)) {
					SSLfatal(sc, SSL_AD_DECODE_ERROR,
						 SSL_R_INVALID_CONTEXT);
					goto err;
	...
	spkistart = spki;
	if ((pkey = d2i_PUBKEY_ex(NULL, &spki, spki_len, sctx->libctx,
				  sctx->propq)) == NULL ||
...
unsigned long tls_output_rpk(SSL_CONNECTION *sc, WPACKET *pkt, CERT_PKEY *cpk)
{
	...
	if (SSL_CONNECTION_IS_TLS13(sc)) {
		...
		if (!tls_construct_extensions(
			    sc, pkt, SSL_EXT_TLS1_3_RAW_PUBLIC_KEY, x509, 0)) {
			/* SSLfatal() already called */
			goto err;
		}
				...
				int clearbufs, int stop)
{
		...
		if (s->server) {
			...
			if (SSL_CONNECTION_IS_TLS13(s)) {
				...
				if ((s->session_ctx->session_cache_mode &
					...
					SSL_CTX_remove_session(s->session_ctx,
							       s->session);
			} else {
				/*
	...
	if (cb != NULL) {
		if (cleanuphand || !SSL_CONNECTION_IS_TLS13(s) ||
...
int tls_get_message_header(SSL_CONNECTION *s, int *mt)
{
	...
	do {
		while (s->init_num < SSL3_HM_HEADER_LENGTH) {
			i = ssl->method->ssl_read_bytes(
				ssl, SSL3_RT_HANDSHAKE, &recvd_type,
				&p[s->init_num],
...
#define SERVER_HELLO_RANDOM_OFFSET (SSL3_HM_HEADER_LENGTH + 2)
		...
		if (!SSL_CONNECTION_IS_TLS13(s) ||
			...
			if (s->s3.tmp.message_type != SSL3_MT_SERVER_HELLO ||
			    s->init_num < SERVER_HELLO_RANDOM_OFFSET +
						  SSL3_RANDOM_SIZE ||
			    memcmp(hrrrandom,
				   s->init_buf->data +
			  ...
			  const SSL_METHOD **meth)
{
	...
	for (vent = table; vent->version != 0 &&
		...
		if (thismeth != NULL &&
		    ...
		    (!s->server || version != TLS1_3_VERSION ||
		     is_tls13_capable(s))) {
			if (meth != NULL)
				*meth = thismeth();
			return 1;
...
int ssl_set_version_bound(int method_version, int version, int *bound)
{
	...
	valid_tls = version >= SSL3_VERSION &&
		    version <= TLS_MAX_VERSION_INTERNAL;
	valid_dtls =
			      ...
			      DOWNGRADE *dgrd)
{
	...
	if (suppversions->present && !SSL_CONNECTION_IS_DTLS(s)) {
		...
		if (!PACKET_as_length_prefixed_1(&suppversions->data,
						 &versionslist)) {
...
int create_synthetic_message_hash(SSL_CONNECTION *s,
				  const unsigned char *hashval, size_t hashlen,
				  const unsigned char *hrr, size_t hrrlen)
{
							...
							BUF_MEM *buf)
{
	...
	if (sc->cert_comp_prefs[0] != TLSEXT_comp_cert_none) {
		for (i = 0; sc->cert_comp_prefs[i] != TLSEXT_comp_cert_none;
		     i++) {
	...
	if (!ossl_comp_has_alg(comp_alg)) {
		SSLfatal(sc, SSL_AD_BAD_CERTIFICATE,
			 SSL_R_BAD_COMPRESSION_ALGORITHM);
		goto err;
	}
	switch (comp_alg) {
	...
	if (!BUF_MEM_grow(buf, expected_length) ||
	    !PACKET_buf_init(tmppkt, (unsigned char *)buf->data,
			     expected_length) ||
	    COMP_expand_block(comp, (unsigned char *)buf->data,
			      (int)expected_length,
