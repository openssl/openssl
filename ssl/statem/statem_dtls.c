...
int dtls1_do_write(SSL_CONNECTION *s, uint8_t type)
{
	...
	if (s->init_off == 0 && type == SSL3_RT_HANDSHAKE) {
		if (!ossl_assert(s->init_num == s->d1->w_msg_hdr.msg_len +
	...
	while (s->init_num > 0) {
		...
		if (type == SSL3_RT_HANDSHAKE && s->msg_callback &&
		    s->init_off != 0)
		...
		if (ret <= 0) {
			...
			if (retry && BIO_ctrl(SSL_get_wbio(ssl),
					      BIO_CTRL_DGRAM_MTU_EXCEEDED, 0,
					      NULL) > 0) {
				if (!(SSL_get_options(ssl) &
				      SSL_OP_NO_QUERY_MTU)) {
					if (!dtls1_query_mtu(s))
						return -1;
					/* Have one more go */
			...
			if (type == SSL3_RT_HANDSHAKE &&
                 ...
                 */
				unsigned char *p = (unsigned char *)&s->init_buf
							   ->data[s->init_off];
	...
	if (*mt == SSL3_MT_CHANGE_CIPHER_SPEC) {
		if (s->msg_callback) {
			s->msg_callback(0, s->version,
					SSL3_RT_CHANGE_CIPHER_SPEC, p, 1,
	...
	if (frag->reassembly == NULL) {
		...
		while (frag_len) {
			i = ssl->method->ssl_read_bytes(
				...
				frag_len > sizeof(devnull) ? sizeof(devnull) :
							     frag_len,
				0, &readbytes);
			if (i <= 0)
				goto err;
...
int dtls1_set_handshake_header(SSL_CONNECTION *s, WPACKET *pkt, int htype)
{
	...
	if (htype == SSL3_MT_CHANGE_CIPHER_SPEC) {
         ...
         * filled in later
         */
		if (!WPACKET_allocate_bytes(pkt, DTLS1_HM_HEADER_LENGTH,
					    &header) ||
...
int dtls1_close_construct_packet(SSL_CONNECTION *s, WPACKET *pkt, int htype)
{
	...
	if (htype != DTLS1_MT_HELLO_VERIFY_REQUEST) {
		...
		if (!dtls1_buffer_message(
			    s, htype == SSL3_MT_CHANGE_CIPHER_SPEC ? 1 : 0))
			return 0;
	}
