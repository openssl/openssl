...
#ifndef OPENSSL_NO_TLS1_3
static int add_key_share(SSL_CONNECTION *s, WPACKET *pkt, unsigned int curve_id, size_t loop_num)
{
    ...
    s->s3.tmp.ks_group_id[loop_num] = curve_id;
    ...
    if (s->s3.group_id != 0) {
        ...
        if (s->ext.supportedgroups == NULL) { /* use default */
            ...
            for (i = 0; i < num_groups; i++) {
                ...
                if (!add_key_share(s, pkt, curve_id, valid_keyshare)) {
                    /* SSLfatal() already called */
                    return EXT_RETURN_FAIL;
                }
                valid_keyshare++;
