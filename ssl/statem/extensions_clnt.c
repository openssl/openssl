...
#error Internal DTLS version error
		...
		if (!SSL_CONNECTION_IS_DTLS(s) &&
		    (s->min_proto_version >= TLS1_3_VERSION ||
		     (ssl_security(s, SSL_SECOP_VERSION, 0, TLS1_VERSION,
				   NULL) &&
...
static int use_ecc(SSL_CONNECTION *s, int min_version, int max_version)
{
	...
	for (i = 0; i < end; i++) {
		...
		if ((alg_k & (SSL_kECDHE | SSL_kECDHEPSK)) ||
		    (alg_a & SSL_aECDSA) || c->min_tls >= TLS1_3_VERSION) {
			ret = 1;
			break;
		}
	}
	...
	for (j = 0; j < num_groups; j++) {
		...
		if (tls_valid_group(s, ctmp, min_version, max_version, 1,
				    NULL) &&
					       ...
					       size_t chainidx)
{
	...
	for (i = 0; i < num_groups; i++) {
		...
		if (tls_valid_group(s, ctmp, min_version, max_version, 0,
				    &okfortls13) &&
		    tls_group_allowed(s, ctmp, SSL_SECOP_CURVE_SUPPORTED)) {
	...
	if (tls13added == 0 && max_version == TLS1_3_VERSION) {
		SSLfatal_data(
			s, SSL_AD_INTERNAL_ERROR, SSL_R_NO_SUITABLE_GROUPS,
			"No groups enabled for max supported SSL/TLS version");
		return EXT_RETURN_FAIL;
	}
...
#ifndef OPENSSL_NO_TLS1_3
static int add_key_share(SSL_CONNECTION *s, WPACKET *pkt, unsigned int group_id,
			 size_t loop_num)
{
					...
					size_t chainidx)
{
	...
	if (num_groups == 1 &&
	    pgroups[0] == 0) { /* Indication that no * prefix was used */
		tls1_get_supported_groups(s, &pgroups, &num_groups);
		add_only_one = 1;
	}
					 ...
					 size_t chainidx)
{
	...
	if (psksess == NULL && s->psk_client_callback != NULL) {
		...
		psklen = s->psk_client_callback(ussl, NULL, identity,
						sizeof(identity) - 1, psk,
						sizeof(psk));
		...
		if (psklen > PSK_MAX_PSK_LEN) {
			...
			const unsigned char tls13_aes128gcmsha256_id[] = {
				0x13, 0x01
			};
			...
			if (psksess == NULL ||
			    !SSL_SESSION_set1_master_key(psksess, psk,
							 psklen) ||
			    !SSL_SESSION_set_cipher(psksess, cipher) ||
			    !SSL_SESSION_set_protocol_version(psksess,
							      TLS1_3_VERSION)) {
				SSLfatal(s, SSL_AD_INTERNAL_ERROR,
					 ERR_R_INTERNAL_ERROR);
				      ...
				      size_t chainidx)
{
	...
	if (s->session->ssl_version == TLS1_3_VERSION &&
		...
		if (md != NULL) {
			...
			if (md_size <= 0)
				return EXT_RETURN_FAIL;
			hlen += PSK_PRE_BINDER_OVERHEAD +
				s->session->ext.ticklen + md_size;
		}
	}
...
EXT_RETURN tls_construct_ctos_post_handshake_auth(
	SSL_CONNECTION *s, WPACKET *pkt, ossl_unused unsigned int context,
	ossl_unused X509 *x, ossl_unused size_t chainidx)
{
#ifndef OPENSSL_NO_TLS1_3
	if (!s->pha_enabled)
		return EXT_RETURN_NOT_SENT;
		       ...
		       X509 *x, size_t chainidx)
{
	...
	if (sctx->ext.npn_select_cb(
		    SSL_CONNECTION_GET_USER_SSL(s), &selected, &selected_len,
			...
			X509 *x, size_t chainidx)
{
	...
	if (!s->hit) {
		...
		s->session->ext.alpn_selected = OPENSSL_memdup(
			    ...
			    unsigned int context, X509 *x, size_t chainidx)
{
	...
	if (!PACKET_get_net_2(pkt, &ct) || ct != 2 ||
	    !PACKET_get_net_2(pkt, &id) || !PACKET_get_1(pkt, &mki) ||
		       ...
		       X509 *x, size_t chainidx)
{
     ...
     */
	if (identity == 0 &&
	    (s->psksession == NULL || s->ext.tick_identity == 2)) {
		s->hit = 1;
		SSL_SESSION_free(s->psksession);
		s->psksession = NULL;
		return 1;
	...
	if ((s->early_data_state != SSL_EARLY_DATA_WRITE_RETRY &&
		...
		memcpy(s->early_secret, s->psksession->early_secret,
		       EVP_MAX_MD_SIZE);
					       ...
					       size_t chainidx)
{
	...
	if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_client_cert_type) ||
	    !WPACKET_start_sub_packet_u16(pkt) ||
	    !WPACKET_sub_memcpy_u8(pkt, sc->client_cert_type,
				    ...
				    size_t chainidx)
{
	...
	if (memchr(sc->client_cert_type, type, sc->client_cert_type_len) ==
	    NULL) {
					       ...
					       size_t chainidx)
{
	...
	if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_server_cert_type) ||
	    !WPACKET_start_sub_packet_u16(pkt) ||
	    !WPACKET_sub_memcpy_u8(pkt, sc->server_cert_type,
				    ...
				    size_t chainidx)
{
	...
	/* Given back a value we didn't configure */
	if (memchr(sc->server_cert_type, type, sc->server_cert_type_len) ==
	    NULL) {
		SSLfatal(sc, SSL_AD_DECODE_ERROR, SSL_R_BAD_VALUE);
		return 0;
	}
