...
WORK_STATE ossl_statem_server_post_work(SSL_CONNECTION *s, WORK_STATE wst)
{
	...
	case TLS_ST_SW_SRVR_HELLO:
		...
		if (SSL_CONNECTION_IS_DTLS(s) && s->hit) {
			...
			if (SSL_export_keying_material(
				    ssl, sctpauthkey, sizeof(sctpauthkey),
	...
	case TLS_ST_SW_CHANGE:
		...
		if (SSL_CONNECTION_IS_TLS13(s)) {
			...
			if (s->rlayer.rrlmethod->set_plain_alerts != NULL)
				s->rlayer.rrlmethod->set_plain_alerts(
					s->rlayer.rrl, 1);
			break;
		}
	...
	case TLS_ST_SW_CERT_REQ:
		if (s->post_handshake_auth == SSL_PHA_REQUEST_PENDING) {
			...
			if (!SSL_CONNECTION_IS_TLS13(s) ||
			    (s->options &
			     SSL_OP_NO_TX_CERTIFICATE_COMPRESSION) != 0)
				 ...
				 const CLIENTHELLO_MSG *hello)
{
	...
	ext_len = TLS1_get_client_version(SSL_CONNECTION_GET_SSL(s)) >=
				  TLS1_2_VERSION ?
...
MSG_PROCESS_RETURN tls_process_client_hello(SSL_CONNECTION *s, PACKET *pkt)
{
	...
	if (clienthello->isv2) {
		...
		/* Regular ClientHello. */
		if (!PACKET_copy_bytes(pkt, clienthello->random,
				       SSL3_RANDOM_SIZE) ||
...
static int tls_early_post_process_client_hello(SSL_CONNECTION *s)
{
	...
	if (sctx->client_hello_cb != NULL) {
		...
		switch (sctx->client_hello_cb(ussl, &al,
					      sctx->client_hello_cb_arg)) {
     ...
     */
	{
		...
		if (ssl_fill_hello_random(s, 1, pos, SSL3_RANDOM_SIZE, dgrd) <=
		    0) {
			SSLfatal(s, SSL_AD_INTERNAL_ERROR,
				 ERR_R_INTERNAL_ERROR);
			goto err;
	...
	if (!s->hit && s->version >= TLS1_VERSION &&
	    !SSL_CONNECTION_IS_TLS13(s) && !SSL_CONNECTION_IS_DTLS(s) &&
		...
		if (s->ext.session_secret_cb(ussl, s->session->master_key,
			...
			if (pref_cipher == NULL)
				pref_cipher = ssl3_choose_cipher(
					s, s->peer_ciphers,
					SSL_get_ciphers(ssl));
			if (pref_cipher == NULL) {
				SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE,
			...
			s->cipher_list_by_id =
				sk_SSL_CIPHER_dup(s->peer_ciphers);
		}
	}
		...
		if (s->s3.tmp.cert != NULL) {
			...
			case SSL_TLSEXT_ERR_OK:
				...
				if (s->ext.ocsp.resp_ex != NULL &&
				    sk_OCSP_RESPONSE_num(s->ext.ocsp.resp_ex) >
					    0)
...
int tls_handle_alpn(SSL_CONNECTION *s)
{
	...
	if (sctx->ext.alpn_select_cb != NULL && s->s3.alpn_proposed != NULL) {
		int r = sctx->ext.alpn_select_cb(
			SSL_CONNECTION_GET_USER_SSL(s), &selected,
		...
		if (r == SSL_TLSEXT_ERR_OK) {
			...
			if (s->session->ext.alpn_selected == NULL ||
				...
				if (!s->hit) {
					...
					s->session->ext.alpn_selected_len =
						selected_len;
				}
			}
						  ...
						  WPACKET *pkt)
{
		...
		if (type & (SSL_kDHE | SSL_kDHEPSK)) {
			...
			if ((pkdhp == NULL) && (s->cert->dh_tmp_cb != NULL)) {
				pkdh = ssl_dh_to_pkey(s->cert->dh_tmp_cb(
					SSL_CONNECTION_GET_USER_SSL(s), 0,
					1024));
				if (pkdh == NULL) {
					SSLfatal(s, SSL_AD_INTERNAL_ERROR,
						 ERR_R_INTERNAL_ERROR);
			...
			if (!ssl_security(s, SSL_SECOP_TMP_DH,
					  EVP_PKEY_get_security_bits(pkdhp), 0,
					  pkdhp)) {
				SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE,
			...
			freer = 1;
			if (!EVP_PKEY_get_bn_param(s->s3.tmp.pkey,
			...
			encodedlen = EVP_PKEY_get1_encoded_public_key(
				s->s3.tmp.pkey, &encodedPoint);
			if (encodedlen == 0) {
				SSLfatal(s, SSL_AD_INTERNAL_ERROR,
					 ERR_R_EC_LIB);
				goto err;
	...
	for (i = 0; i < 4 && r[i] != NULL; i++) {
		...
		if ((i == 2) && (type & (SSL_kDHE | SSL_kDHEPSK))) {
			...
			if (len > 0) {
				if (!WPACKET_allocate_bytes(pkt, len,
							    &binval)) {
					SSLfatal(s, SSL_AD_INTERNAL_ERROR,
						 ERR_R_INTERNAL_ERROR);
					goto err;
	...
	if (lu != NULL) {
		...
		if (SSL_USE_SIGALGS(s) &&
		    !WPACKET_put_bytes_u16(pkt, lu->sigalg)) {
			SSLfatal(s, SSL_AD_INTERNAL_ERROR,
				 ERR_R_INTERNAL_ERROR);
			goto err;
		}
		...
		if (EVP_DigestSign(md_ctx, NULL, &siglen, tbs, tbslen) <= 0 ||
		    ...
		    EVP_DigestSign(md_ctx, sigbytes1, &siglen, tbs, tbslen) <=
			    0 ||
						  ...
						  WPACKET *pkt)
{
	if (SSL_CONNECTION_IS_TLS13(s)) {
		...
		if (!tls_construct_extensions(
			    s, pkt, SSL_EXT_TLS1_3_CERTIFICATE_REQUEST, NULL,
			    0)) {
			/* SSLfatal() already called */
			return CON_FUNC_ERROR;
		}
...
static int tls_process_cke_rsa(SSL_CONNECTION *s, PACKET *pkt)
{
	...
	if (EVP_PKEY_decrypt_init(ctx) <= 0 ||
	    EVP_PKEY_CTX_set_rsa_padding(ctx, RSA_PKCS1_WITH_TLS_PADDING) <=
		    0) {
						...
						WORK_STATE wst)
{
	...
	if (wst == WORK_MORE_A) {
		if (SSL_CONNECTION_IS_DTLS(s)) {
			...
			if (SSL_export_keying_material(
				    SSL_CONNECTION_GET_SSL(s), sctpauthkey,
						  ...
						  PACKET *pkt)
{
	...
	for (chainidx = 0; PACKET_remaining(&spkt) > 0; chainidx++) {
		...
		if (d2i_X509(&x, (const unsigned char **)&certbytes, l) ==
		    NULL) {
						  ...
						  unsigned char *tick_nonce)
{
...
#endif
	{
		...
		EVP_CIPHER *cipher = EVP_CIPHER_fetch(
			sctx->libctx, "AES-256-CBC", sctx->propq);
						 ...
						 WPACKET *pkt)
{
	...
	if (SSL_CONNECTION_IS_TLS13(s)) {
		...
		if (RAND_bytes_ex(SSL_CONNECTION_GET_CTX(s)->libctx,
				  age_add_u.age_add_c, sizeof(age_add_u),
				  0) <= 0) {
			SSLfatal(s, SSL_AD_INTERNAL_ERROR,
				 ERR_R_INTERNAL_ERROR);
			goto err;
	...
	if (SSL_CONNECTION_IS_TLS13(s) &&
		...
		if (!construct_stateful_ticket(s, pkt, age_add_u.age_add,
					       tick_nonce)) {
			/* SSLfatal() already called */
			goto err;
		}
				   ...
				   WPACKET *pkt)
{
	...
	if ((resp = sk_OCSP_RESPONSE_value(s->ext.ocsp.resp_ex,
					   (int)chainidx)) != NULL) {
		/*
		...
		if (OCSP_response_status(resp) ==
			...
			if (((bs = OCSP_response_get1_basic(resp)) != NULL) &&
				...
				cid = (OCSP_CERTID *)OCSP_SINGLERESP_get0_id(
					sr);

				OCSP_id_get0_info(&respIssuerNameHash,
						  &cert_id_md_oid, NULL,
						  &respSerial, cid);
				if (cert_id_md_oid != NULL)
					cert_id_md = EVP_get_digestbyobj(
						cert_id_md_oid);
				else
					cert_id_md = EVP_sha1();
				...
				if (!X509_NAME_digest(certIssuerName,
						      cert_id_md, md, &len) ||
				    !(ASN1_OCTET_STRING_set(certIssuerNameHash,
							    md, len))) {
				...
				for (i = 0; i < num; i++) {
					...
					if (!ASN1_INTEGER_cmp(certSerial,
							      respSerial) &&
