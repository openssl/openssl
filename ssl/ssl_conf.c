...
static int cmd_Options(SSL_CONF_CTX *cctx, const char *value)
{
	static const ssl_flag_tbl ssl_option_list[] = {
		...
		SSL_FLAG_TBL("IgnoreUnexpectedEOF",
...
static int cmd_Certificate(SSL_CONF_CTX *cctx, const char *value)
{
	...
	if (rv > 0 && c != NULL &&
	    cctx->flags & SSL_CONF_FLAG_REQUIRE_PRIVATE) {
		...
		if (fileidx >= cctx->num_cert_filename) {
			...
			char **pfilename =
				&cctx->cert_filename[c->key - c->pkeys];
...
static int cmd_PrivateKey(SSL_CONF_CTX *cctx, const char *value)
{
	...
	if (!(cctx->flags & SSL_CONF_FLAG_CERTIFICATE))
		return -2;
	if (cctx->ctx)
		rv = SSL_CTX_use_PrivateKey_file(cctx->ctx, value,
						 SSL_FILETYPE_PEM);
	if (cctx->ssl)
		rv = SSL_use_PrivateKey_file(cctx->ssl, value,
					     SSL_FILETYPE_PEM);
	return rv > 0;
}
		    ...
		    const char *CAstore, int verify_store)
{
	...
	if (CAfile != NULL &&
	    !X509_STORE_load_file_ex(*st, CAfile, libctx, propq))
		return 0;
...
static int cmd_DHParameters(SSL_CONF_CTX *cctx, const char *value)
{
	...
	if (cctx->ctx != NULL || cctx->ssl != NULL) {
		...
		decoderctx = OSSL_DECODER_CTX_new_for_pkey(
			&dhpkey, "PEM", NULL, "DH",
			OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS, sslctx->libctx,
			sslctx->propq);
		if (decoderctx == NULL)
			goto end;
		ERR_set_mark();
		while (!OSSL_DECODER_from_bio(decoderctx, in) &&
		       dhpkey == NULL && !BIO_eof(in))
			;
...
static const ssl_conf_cmd_tbl ssl_conf_cmds[] = {
	...
	SSL_CONF_CMD(ClientCAPath, NULL,
		     SSL_CONF_FLAG_SERVER | SSL_CONF_FLAG_CERTIFICATE,
		     SSL_CONF_TYPE_DIR),
	SSL_CONF_CMD(RequestCAStore, "requestCAStore",
		     SSL_CONF_FLAG_CERTIFICATE, SSL_CONF_TYPE_STORE),
						   ...
						   const char *cmd)
{
	...
	for (i = 0, t = ssl_conf_cmds; i < OSSL_NELEM(ssl_conf_cmds);
	     i++, t++) {
		if (ssl_conf_cmd_allowed(cctx, t)) {
			if (cctx->flags & SSL_CONF_FLAG_CMDLINE) {
				if (t->str_cmdline &&
				    strcmp(t->str_cmdline, cmd) == 0)
					return t;
			}
			if (cctx->flags & SSL_CONF_FLAG_FILE) {
				if (t->str_file &&
				    OPENSSL_strcasecmp(t->str_file, cmd) == 0)
					return t;
			}
		}
	}
...
unknown_cmd:
	if (cctx->flags & SSL_CONF_FLAG_SHOW_ERRORS)
		ERR_raise_data(ERR_LIB_SSL, SSL_R_UNKNOWN_CMD_NAME, "cmd=%s",
			       cmd);
...
void SSL_CONF_CTX_set_ssl(SSL_CONF_CTX *cctx, SSL *ssl)
{
	...
	if (ssl != NULL) {
		...
		cctx->cert_filename = OPENSSL_zalloc(
			sc->ssl_pkey_num * sizeof(*cctx->cert_filename));
...
void SSL_CONF_CTX_set_ssl_ctx(SSL_CONF_CTX *cctx, SSL_CTX *ctx)
{
	...
	if (ctx) {
		...
		if (cctx->cert_filename != NULL)
			cctx->num_cert_filename =
				SSL_PKEY_NUM + ctx->sigalg_list_len;
	} else {
		cctx->poptions = NULL;
		cctx->min_version = NULL;
		cctx->max_version = NULL;
