...
static int cmd_CipherString(SSL_CONF_CTX *cctx, const char *value)
{
    ...
    if (cctx->ctx) {
        rv = SSL_CTX_set_cipher_list_and_mask(cctx->ctx, value,
                                              cctx->version_mask);
        if (rv > 0 && SSL_CTX_count_ciphers_by_version(cctx->ctx, TLS1_3_VERSION) > 0)
            rv = SSL_CTX_set_ciphersuites(cctx->ctx, "");
    }
    if (cctx->ssl) {
        rv = SSL_set_cipher_list_and_mask(cctx->ssl, value,
                                          cctx->version_mask);
        if (rv > 0 && SSL_count_ciphers_by_version(cctx->ssl, TLS1_3_VERSION) > 0)
            rv = SSL_set_ciphersuites(cctx->ssl, "");
    }
    return rv > 0;
}
...
static int cmd_CiphVersionMask(SSL_CONF_CTX *cctx, const char *value)
{
    ...
    if (cctx->ssl)
        rv = SSL_set_ciphersuites(cctx->ssl, "");
    return rv > 0 && OPENSSL_version_list(value, &cctx->version_mask) > 0;
}
