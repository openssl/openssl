...
int ssl_srp_server_param_with_username_intern(SSL_CONNECTION *s, int *ad)
{
	...
	*ad = SSL_AD_UNKNOWN_PSK_IDENTITY;
	if ((s->srp_ctx.TLS_ext_srp_username_callback != NULL) &&
	    ((al = s->srp_ctx.TLS_ext_srp_username_callback(
		      SSL_CONNECTION_GET_USER_SSL(s), ad,
	...
	*ad = SSL_AD_INTERNAL_ERROR;
	...
	return ((s->srp_ctx.B = SRP_Calc_B_ex(
			 s->srp_ctx.b, s->srp_ctx.N, s->srp_ctx.g, s->srp_ctx.v,
				...
				const char *grp)
{
	...
	if (!SRP_create_verifier_BN_ex(
		    user, pass, &sc->srp_ctx.s, &sc->srp_ctx.v, sc->srp_ctx.N,
		    sc->srp_ctx.g, s->ctx->libctx, s->ctx->propq))
		return -1;
			     ...
			     BIGNUM *sa, BIGNUM *v, char *info)
{
	...
	if (!(sc->srp_ctx.N) || !(sc->srp_ctx.g) || !(sc->srp_ctx.s) ||
	    !(sc->srp_ctx.v))
		return -1;
...
int srp_generate_server_master_secret(SSL_CONNECTION *s)
{
	...
	if ((u = SRP_Calc_u_ex(s->srp_ctx.A, s->srp_ctx.B, s->srp_ctx.N,
			       sctx->libctx, sctx->propq)) == NULL)
		goto err;
	if ((K = SRP_Calc_server_key(s->srp_ctx.A, s->srp_ctx.v, u,
				     s->srp_ctx.b, s->srp_ctx.N)) == NULL)
		goto err;
...
int srp_generate_client_master_secret(SSL_CONNECTION *s)
{
	...
	if ((passwd = s->srp_ctx.SRP_give_srp_client_pwd_callback(
		     SSL_CONNECTION_GET_USER_SSL(s), s->srp_ctx.SRP_cb_arg)) ==
	    NULL) {
...
int srp_verify_server_param(SSL_CONNECTION *s)
{
	...
	if (srp->SRP_verify_param_callback) {
		if (srp->SRP_verify_param_callback(
			    SSL_CONNECTION_GET_USER_SSL(s), srp->SRP_cb_arg) <=
		    0) {
			SSLfatal(s, SSL_AD_INSUFFICIENT_SECURITY,
...
int ssl_srp_calc_a_param_intern(SSL_CONNECTION *s)
{
	...
	if (RAND_priv_bytes_ex(SSL_CONNECTION_GET_CTX(s)->libctx, rnd,
			       sizeof(rnd), 0) <= 0)
		return 0;
...
{
	return tls1_ctx_ctrl(ctx, SSL_CTRL_SET_TLS_EXT_SRP_PASSWORD, 0,
			     password);
}
