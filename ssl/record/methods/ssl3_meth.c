				 ...
				 COMP_METHOD *comp)
{
     ...
     */
	if (EVP_CIPHER_get0_provider(EVP_CIPHER_CTX_get0_cipher(ciph_ctx)) !=
		    NULL &&
		       ...
		       size_t macsize)
{
	...
	if (provided) {
		...
		if (!sending && mac != NULL) {
			...
			*p++ = OSSL_PARAM_construct_octet_ptr(
				OSSL_CIPHER_PARAM_TLS_MAC, (void **)&mac->mac,
				macsize);
		...
		if (EVP_Cipher(ds, rec->data, rec->input, (unsigned int)l) <
		    1) {
			/* Shouldn't happen */
		...
		if (!sending)
			return ssl3_cbc_remove_padding_and_mac(
				&rec->length, rec->orig_len, rec->data,
				(mac != NULL) ? &mac->mac : NULL,
				(mac != NULL) ? &mac->alloced : NULL, bs,
				macsize, rl->libctx);
	}
		    ...
		    unsigned char *md, int sending)
{
	...
	if (!sending &&
		...
		if (ssl3_cbc_digest_record(EVP_MD_CTX_get0_md(hash), md,
					   &md_size, header, rec->input,
					   rec->length, rec->orig_len, mac_sec,
					   md_size, 1) <= 0)
			return 0;
#endif
	} else {
...
const struct record_functions_st ssl_3_0_funcs = {
	ssl3_set_crypto_state, ssl3_cipher, ssl3_mac,
	tls_default_set_protocol_version, tls_default_read_n,
	tls_get_more_records, tls_default_validate_record_header,
	tls_default_post_process_record, tls_get_max_records_default,
	tls_write_records_default,
	/* These 2 functions are defined in tls1_meth.c */
	tls1_allocate_write_buffers, tls1_initialise_write_packets, NULL,
	tls_prepare_record_header_default, NULL,
	tls_prepare_for_encryption_default,
	tls_post_encryption_processing_default, NULL
};
