				  ...
				  COMP_METHOD *comp)
{
	...
	/* Integrity only */
	if (EVP_CIPHER_is_a(ciph, "NULL") && mactype == NID_hmac &&
	    md != NULL) {
...
end_mac:
	...
	if (sending) {
		/* Add the tag */
		if (EVP_CIPHER_CTX_ctrl(enc_ctx, EVP_CTRL_AEAD_GET_TAG,
					(int)rl->taglen,
					rec->data + rec->length) <= 0) {
			RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR,
				    ERR_R_INTERNAL_ERROR);
				    ...
				    WPACKET *thispkt, TLS_RL_RECORD *thiswr)
{
	...
	if (rlen < rl->max_frag_len) {
		...
		if (rl->padding != NULL) {
             ...
             */
			if (thistempl->type == SSL3_RT_HANDSHAKE &&
			    rl->hs_padding > 0)
				bp = rl->hs_padding;
			else if (thistempl->type == SSL3_RT_ALERT &&
				 rl->hs_padding > 0)
				bp = rl->hs_padding;
