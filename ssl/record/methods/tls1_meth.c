				 ...
				 COMP_METHOD *comp)
{
	...
	if ((EVP_CIPHER_get_flags(ciph) & EVP_CIPH_FLAG_AEAD_CIPHER) == 0) {
		if (mactype == EVP_PKEY_HMAC) {
			mac_key = EVP_PKEY_new_raw_private_key_ex(
				rl->libctx, "HMAC", rl->propq, mackey,
				mackeylen);
		} else {
			/*
		...
		if (mac_key == NULL ||
		    EVP_DigestSignInit_ex(rl->md_ctx, NULL,
					  EVP_MD_get0_name(md), rl->libctx,
					  rl->propq, mac_key, NULL) <= 0) {
		       ...
		       size_t macsize)
{
			...
			for (ctr = 0; ctr < n_recs; ctr++) {
				if (recs[ctr].data != recs[ctr].input) {
					RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR,
						    ERR_R_INTERNAL_ERROR);
					return 0;
				} else if (RAND_bytes_ex(rl->libctx,
							 recs[ctr].input, ivlen,
							 0) <= 0) {
					RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR,
						    ERR_R_INTERNAL_ERROR);
					return 0;
	...
	for (ctr = 0; ctr < n_recs; ctr++) {
		...
		if ((EVP_CIPHER_get_flags(EVP_CIPHER_CTX_get0_cipher(ds)) &
			...
			for (loop = reclen[ctr]; loop < reclen[ctr] + padnum;
			     loop++)
	...
	if (provided) {
		...
		if (!sending) {
			...
			} else if (EVP_CIPHER_get_mode(enc) ==
				   EVP_CIPH_CCM_MODE) {
		...
		if (!sending) {
			for (ctr = 0; ctr < n_recs; ctr++) {
                 ...
                 */
				if (!tls1_cbc_remove_padding_and_mac(
					    &recs[ctr].length,
					    recs[ctr].orig_len, recs[ctr].data,
					    (macs != NULL) ? &macs[ctr].mac :
							     NULL,
					    (macs != NULL) ?
						    &macs[ctr].alloced :
						    NULL,
					    bs, pad ? (size_t)pad : macsize,
...
#define MAX_PREFIX_LEN                                                    \
	((SSL3_ALIGN_PAYLOAD - 1) + SSL3_RT_SEND_MAX_ENCRYPTED_OVERHEAD + \
	 SSL3_RT_HEADER_LENGTH + SSL3_RT_MAX_COMPRESSED_OVERHEAD)
#else
#define MAX_PREFIX_LEN                                                    \
	((SSL3_ALIGN_PAYLOAD - 1) + SSL3_RT_SEND_MAX_ENCRYPTED_OVERHEAD + \
...
#ifndef OPENSSL_NO_COMP
#define MAX_PREFIX_LEN                                                 \
	(SSL3_RT_SEND_MAX_ENCRYPTED_OVERHEAD + SSL3_RT_HEADER_LENGTH + \
...
const struct record_functions_st dtls_1_funcs = {
	tls1_set_crypto_state, tls1_cipher, tls1_mac,
	tls_default_set_protocol_version, tls_default_read_n,
	dtls_get_more_records, NULL, NULL, NULL, tls_write_records_default,
	/*
	...
	tls_initialise_write_packets_default, NULL, dtls_prepare_record_header,
	NULL, tls_prepare_for_encryption_default,
