...
again:
	...
	if ((rl->rstate != SSL_ST_READ_BODY) ||
		...
		rret = rl->funcs->read_n(rl, DTLS1_RT_HEADER_LENGTH,
					 TLS_BUFFER_get_len(&rl->rbuf), 0, 1,
					 &n);
		...
		if (rl->msg_callback != NULL)
			rl->msg_callback(0, rr->rec_version, SSL3_RT_HEADER,
		...
		if (ssl_major != (rl->version == DTLS_ANY_VERSION ?
					  DTLS1_VERSION_MAJOR :
					  rl->version >> 8)) {
         ...
         */
		if (rr->length >
		    rl->max_frag_len + SSL3_RT_MAX_ENCRYPTED_OVERHEAD) {
			/* record too long, silently discard it */
			rr->length = 0;
			rl->packet_length = 0;
			goto again;
...
static int dtls_free(OSSL_RECORD_LAYER *rl)
{
	...
	if (rl->unprocessed_rcds != NULL) {
		while ((item = pqueue_pop(rl->unprocessed_rcds)) != NULL) {
			...
			ret &= BIO_write_ex(rl->next, rdata->packet,
					    rdata->packet_length, &written);
			OPENSSL_free(rdata->rbuf.buf);
			OPENSSL_free(item->data);
			pitem_free(item);
		}
