			   ...
			   size_t firstlen, size_t nextlen)
{
	...
	if (firstlen == 0 || (numwpipes > 1 && nextlen == 0)) {
		...
		defltlen = maxalign + headerlen + rl->eivlen +
			   rl->max_frag_len + contenttypelen +
...
int tls_setup_read_buffer(OSSL_RECORD_LAYER *rl)
{
	...
	if (b->buf == NULL) {
		len = rl->max_frag_len + SSL3_RT_MAX_ENCRYPTED_OVERHEAD +
		      headerlen + maxalign;
#ifndef OPENSSL_NO_COMP
		if (tls_allow_compression(rl))
...
int tls_get_more_records(OSSL_RECORD_LAYER *rl)
{
	...
	if (num_recs == 1 &&
         ...
         */
		if (rl->msg_callback != NULL)
			rl->msg_callback(0, TLS1_3_VERSION,
	...
	if (rl->md_ctx != NULL) {
		...
		if (tmpmd != NULL) {
			...
			if (!ossl_assert(imac_size > 0 &&
					 imac_size <= EVP_MAX_MD_SIZE)) {
				RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR,
					    ERR_R_EVP_LIB);
				return OSSL_RECORD_RETURN_FATAL;
			}
	...
	if (rl->enc_ctx != NULL && !rl->use_etm &&
		...
		for (j = 0; j < num_recs; j++) {
			...
			if (i == 0 || thismb == NULL || thismb->mac == NULL ||
			    CRYPTO_memcmp(md, thismb->mac, (size_t)mac_size) !=
				    0)
				enc_err = 0;
			if (thisrr->length >
...
int tls_do_compress(OSSL_RECORD_LAYER *rl, TLS_RL_RECORD *wr)
{
	...
	i = COMP_compress_block(
		rl->compctx, wr->data,
		(int)(wr->length + SSL3_RT_MAX_COMPRESSED_OVERHEAD), wr->input,
		(int)wr->length);
	if (i < 0)
		return 0;
...
int tls_do_uncompress(OSSL_RECORD_LAYER *rl, TLS_RL_RECORD *rec)
{
	...
	if (rec->comp == NULL) {
		rec->comp = (unsigned char *)OPENSSL_malloc(
...
int tls13_common_post_process_record(OSSL_RECORD_LAYER *rl, TLS_RL_RECORD *rec)
{
	if (rec->type != SSL3_RT_APPLICATION_DATA &&
	    rec->type != SSL3_RT_ALERT && rec->type != SSL3_RT_HANDSHAKE) {
		RLAYERfatal(rl, SSL_AD_UNEXPECTED_MESSAGE,
	...
	if (rl->msg_callback != NULL)
		rl->msg_callback(0, rl->version, SSL3_RT_INNER_CONTENT_TYPE,
				 &rec->type, 1, rl->cbarg);
...
int tls_set_options(OSSL_RECORD_LAYER *rl, const OSSL_PARAM *options)
{
	...
	if (rl->direction == OSSL_RECORD_DIRECTION_READ) {
		p = OSSL_PARAM_locate_const(
			options, OSSL_LIBSSL_RECORD_LAYER_READ_BUFFER_LEN);
		if (p != NULL &&
		...
		if (p != NULL &&
		    !OSSL_PARAM_get_size_t(p, &rl->block_padding)) {
			ERR_raise(ERR_LIB_SSL, SSL_R_FAILED_TO_GET_PARAMETER);
			return 0;
		}
		p = OSSL_PARAM_locate_const(
			options, OSSL_LIBSSL_RECORD_LAYER_PARAM_HS_PADDING);
	...
	if (rl->level == OSSL_RECORD_PROTECTION_LEVEL_APPLICATION) {
         ...
         */
		p = OSSL_PARAM_locate_const(
			options, OSSL_LIBSSL_RECORD_LAYER_PARAM_READ_AHEAD);
...
int tls_int_new_record_layer(
	...
	int direction, int level, const EVP_CIPHER *ciph, size_t taglen,
	const EVP_MD *md, COMP_METHOD *comp, BIO *prev, BIO *transport,
	BIO *next, const OSSL_PARAM *settings, const OSSL_PARAM *options,
...
int tls_free(OSSL_RECORD_LAYER *rl)
{
	...
	if (left > 0) {
         ...
         */
		ret = BIO_write_ex(rl->next, rbuf->buf + rbuf->offset, left,
				   &written);
	}
	tls_int_free(rl);
					 ...
					 size_t *wpinited)
{
	...
	for (j = 0; j < numtempl; j++) {
		...
		align += rl->isdtls ? DTLS1_RT_HEADER_LENGTH :
				      SSL3_RT_HEADER_LENGTH;
				       ...
				       WPACKET *thispkt, TLS_RL_RECORD *thiswr)
{
	...
	if (!WPACKET_reserve_bytes(
		    thispkt, SSL3_RT_SEND_MAX_ENCRYPTED_OVERHEAD - mac_size,
		    NULL)
	    /*
					   ...
					   TLS_RL_RECORD *thiswr)
{
	...
	if (rl->msg_callback != NULL) {
		...
		rl->msg_callback(1, thiswr->rec_version, SSL3_RT_HEADER,
				 recordstart, headerlen, rl->cbarg);

		if (rl->version == TLS1_3_VERSION && rl->enc_ctx != NULL) {
			unsigned char ctype = thistempl->type;

			rl->msg_callback(1, thiswr->rec_version,
					 SSL3_RT_INNER_CONTENT_TYPE, &ctype, 1,
					 rl->cbarg);
		}
	}
			      ...
			      OSSL_RECORD_TEMPLATE *templates, size_t numtempl)
{
	...
	if (!rl->funcs->allocate_write_buffers(rl, templates, numtempl,
					       &prefix)) {
		/* RLAYERfatal() already called */
		goto err;
	}
	...
	for (j = 0; j < numtempl + prefix; j++) {
		...
		thispkt = &pkt[j];
		thiswr = &wr[j];
		thistempl = (j < prefix) ? &prefixtempl :
					   &templates[j - prefix];
		...
		if (!rl->funcs->prepare_record_header(rl, thispkt, thistempl,
		...
		TLS_RL_RECORD_set_input(thiswr,
					(unsigned char *)thistempl->buf);
		...
		if (rl->compctx != NULL) {
			if (!tls_do_compress(rl, thiswr) ||
			    !WPACKET_allocate_bytes(thispkt, thiswr->length,
						    NULL)) {
				RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR,
			...
			if (!WPACKET_memcpy(thispkt, thiswr->input,
					    thiswr->length)) {
				RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR,
					    ERR_R_INTERNAL_ERROR);
				goto err;
		...
		if (!rl->funcs->prepare_for_encryption(rl, mac_size, thispkt,
						       thiswr)) {
			/* RLAYERfatal() already called */
			goto err;
		}
	...
	if (rl->funcs->cipher(rl, wr + prefix, numtempl, 1, NULL, mac_size) <
	    1) {
		if (rl->alert == SSL_AD_NO_ALERT) {
			RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR,
				    ERR_R_INTERNAL_ERROR);
	...
	for (j = 0; j < numtempl + prefix; j++) {
		...
		/* now let's set up wb */
		TLS_BUFFER_set_left(&rl->wbuf[j],
				    TLS_RL_RECORD_get_length(thiswr));
	}
	...
	for (;;) {
		...
		if (rl->bio != NULL) {
			if (rl->funcs->prepare_write_bio != NULL) {
				ret = rl->funcs->prepare_write_bio(
					rl, thiswb->type);
				if (ret != OSSL_RECORD_RETURN_SUCCESS)
					return ret;
			}
