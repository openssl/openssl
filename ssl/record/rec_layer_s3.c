...
int RECORD_LAYER_clear(RECORD_LAYER *rl)
{
	...
	while (rl->curr_rec < rl->num_recs)
		ret &= ssl_release_record(rl->s, &(rl->tlsrecs[rl->curr_rec++]),
					  0);
...
int RECORD_LAYER_reset(RECORD_LAYER *rl)
{
	...
	ret &= ssl_set_new_record_layer(rl->s,
					...
					OSSL_RECORD_PROTECTION_LEVEL_NONE, NULL,
		     ...
		     size_t *written)
{
	...
	if (i < 0) {
		...
		i = HANDLE_RLAYER_WRITE_RETURN(
			s, s->rlayer.wrlmethod->retry_write_records(
				   s->rlayer.wrl));
		if (i <= 0) {
			s->rlayer.wnum = tot;
			return i;
     ...
     */
	recversion = (s->version == TLS1_3_VERSION) ? TLS1_2_VERSION :
						      s->version;
	if (SSL_get_state(ssl) == TLS_ST_CW_CLNT_HELLO && !s->renegotiate &&
	...
	for (;;) {
        ...
        */
		maxpipes = s->rlayer.wrlmethod->get_max_records(
			s->rlayer.wrl, type, n, max_send_fragment,
			&split_send_fragment);
		/*
        ...
        */
		...
		i = HANDLE_RLAYER_WRITE_RETURN(
			s, s->rlayer.wrlmethod->write_records(s->rlayer.wrl,
							      tmpls, maxpipes));
		if (i <= 0) {
			/* SSLfatal() already called if appropriate */
			s->rlayer.wnum = tot;
	...
	if (ret == OSSL_RECORD_RETURN_RETRY) {
		...
		if (ret == OSSL_RECORD_RETURN_EOF) {
			if (writing) {
                 ...
                 */
				ossl_statem_fatal(
					s, SSL_AD_DECODE_ERROR,
					SSL_R_UNEXPECTED_EOF_WHILE_READING,
					NULL);
			}
		} else if (ret == OSSL_RECORD_RETURN_FATAL) {
			int al = s->rlayer.rrlmethod->get_alert_code(
				s->rlayer.rrl);

			if (al != SSL_AD_NO_ALERT) {
				ERR_new();
				ERR_set_debug(file, line, 0);
				ossl_statem_fatal(s, al,
         ...
         */
		if (ret == OSSL_RECORD_RETURN_NON_FATAL_ERR ||
...
int ssl_release_record(SSL_CONNECTION *s, TLS_RECORD *rr, size_t length)
{
	...
	if (rr->rechandle != NULL) {
		...
		if (HANDLE_RLAYER_READ_RETURN(
			    s, s->rlayer.rrlmethod->release_record(
				       s->rlayer.rrl, rr->rechandle, length)) <=
		    0) {
...
start:
	...
	if (s->rlayer.curr_rec >= s->rlayer.num_recs) {
		...
		do {
			...
			ret = HANDLE_RLAYER_READ_RETURN(
				s, s->rlayer.rrlmethod->read_record(
					   s->rlayer.rrl, &rr->rechandle,
					   &rr->version, &rr->type, &rr->data,
					   &rr->length, NULL, NULL));
			if (ret <= 0) {
				/* SSLfatal() already called if appropriate */
				return ret;
			...
			rr->off = 0;
			s->rlayer.num_recs++;
		} while (s->rlayer.rrlmethod->processed_read_pending(
				 s->rlayer.rrl) &&
	...
	if (s->rlayer.handshake_fragment_len > 0 &&
	    rr->type != SSL3_RT_HANDSHAKE && SSL_CONNECTION_IS_TLS13(s)) {
	...
	if (type == rr->type ||
		...
		do {
			...
			totalbytes += n;
			 ...
			 curr_rec < s->rlayer.num_recs && totalbytes < len);
		if (totalbytes == 0) {
...
static const OSSL_RECORD_METHOD *
ssl_select_next_record_layer(SSL_CONNECTION *s, int direction, int level)
{
...
static int ssl_post_record_layer_select(SSL_CONNECTION *s, int direction)
{
	...
	if (SSL_IS_FIRST_HANDSHAKE(s) &&
	    thismethod->set_first_handshake != NULL)
			     ...
			     const EVP_MD *kdfdigest)
{
	...
	*opts++ = OSSL_PARAM_construct_uint64(
		OSSL_LIBSSL_RECORD_LAYER_PARAM_OPTIONS, &s->options);
	*opts++ = OSSL_PARAM_construct_uint32(
		OSSL_LIBSSL_RECORD_LAYER_PARAM_MODE, &s->mode);
	if (direction == OSSL_RECORD_DIRECTION_READ) {
		*opts++ = OSSL_PARAM_construct_size_t(
	...
	*opts = OSSL_PARAM_construct_end();
     ...
     */
	if (direction == OSSL_RECORD_DIRECTION_READ && s->session != NULL &&
	    USE_MAX_FRAGMENT_LENGTH_EXT(s->session))
		maxfrag = GET_MAX_FRAGMENT_LENGTH(s->session);

	if (maxfrag != SSL3_RT_MAX_PLAIN_LENGTH)
		*set++ = OSSL_PARAM_construct_uint(
			OSSL_LIBSSL_RECORD_LAYER_PARAM_MAX_FRAG_LEN, &maxfrag);
	...
	if (use_early_data) {
		...
		if (max_early_data != 0)
			*set++ = OSSL_PARAM_construct_uint32(
	...
	for (;;) {
		...
		if (direction == OSSL_RECORD_DIRECTION_READ) {
			...
			if (SSL_CONNECTION_IS_DTLS(s) &&
				...
				epoch = dtls1_get_epoch(
					s, SSL3_CC_READ); /* new epoch */
		...
		rlret = meth->new_record_layer(
			sctx->libctx, sctx->propq, version, s->server,
			direction, level, epoch, secret, secretlen, key, keylen,
			iv, ivlen, mackey, mackeylen, ciph, taglen, mactype, md,
			compm, kdfdigest, prev, thisbio, next, NULL, NULL,
			settings, options, rlayer_dispatch_tmp, s,
