...
start:
	...
	if (sc->rlayer.curr_rec >= sc->rlayer.num_recs) {
		...
		do {
			...
			rr->off = 0;
			sc->rlayer.num_recs++;
		} while (sc->rlayer.rrlmethod->processed_read_pending(
				 sc->rlayer.rrl) &&
	...
	if (rr->type == SSL3_RT_ALERT) {
		...
		if (sc->msg_callback)
			sc->msg_callback(0, sc->version, SSL3_RT_ALERT,
					 alert_bytes, 2, s,
					 sc->msg_callback_arg);
		...
		if (alert_level == SSL3_AL_WARNING) {
			...
			if (alert_descr == SSL_AD_CLOSE_NOTIFY) {
                 ...
                 */
				SSLfatal(sc, SSL_AD_HANDSHAKE_FAILURE,
