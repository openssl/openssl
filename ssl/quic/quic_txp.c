			  ...
			  uint32_t enc_level, size_t max_ppl, size_t reserve)
{
	...
	if (max_ppl > h->txp->el[enc_level].scratch_len) {
		...
		scratch =
			OPENSSL_realloc(h->txp->el[enc_level].scratch, max_ppl);
		if (scratch == NULL)
			return 0;
...
static int tx_helper_commit(struct tx_helper *h)
{
	...
	if (h->txp->msg_callback != NULL && l > 0) {
		...
		h->txp->msg_callback(1, OSSL_QUIC1_VERSION, ctype, h->txn.data,
				     l, h->txp->msg_callback_ssl,
				     ...
				     QUIC_TXP_STATUS *status)
{
	...
	if (need_padding) {
		...
		for (enc_level = QUIC_ENC_LEVEL_INITIAL;
		     enc_level < QUIC_ENC_LEVEL_NUM; ++enc_level)
				  ...
				  struct txp_pkt_geom *geom)
{
	...
	if (!txp_determine_ppl_from_pl(txp, cmpl, enc_level, hdr_len,
				       &geom->cmppl))
		return 0;
				  ...
				  uint32_t *conn_close_enc_level)
{
	...
	if (enc_level != QUIC_ENC_LEVEL_0RTT)
		for (cfq_item = ossl_quic_cfq_get_priority_head(txp->args.cfq,
								pn_space);
		     cfq_item != NULL;
...
static int sstream_is_pending(QUIC_SSTREAM *sstream)
{
	...
	return ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov,
						  &num_iov);
}
			    ...
			    QUIC_TXPIM_PKT *pkt, void *arg)
{
	...
	case OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA: {
		QUIC_STREAM *s = ossl_quic_stream_map_get_by_id(txp->args.qsm,
								stream_id);
					       ...
					       OSSL_QUIC_TX_PACKETISER *txp)
{
	...
	pkt->geom.pkt_overhead =
		hdr_len + ciphertext_len - pkt->h.bytes_appended;
	return 1;
}
			      ...
			      QUIC_TXPIM_PKT *pkt, void *arg)
{
	...
	case OSSL_QUIC_FRAME_TYPE_RESET_STREAM: {
		...
		/*
                 ...
                 */
		ossl_quic_stream_map_notify_reset_stream_acked(txp->args.qsm,
							       s);
		ossl_quic_stream_map_update_state(txp->args.qsm, s);
	} break;
	default:
				      ...
				      int *have_ack_eliciting)
{
	...
	for (i = 0;; ++i) {
		...
		if (!ossl_quic_sstream_get_stream_frame(
			    txp->args.crypto[pn_space], i, &shdr, iov,
			    &num_stream_iovec))
         ...
         */
		ossl_quic_sstream_adjust_iov((size_t)chdr.len, iov,
					     num_stream_iovec);
				 ...
				 struct chunk_info *chunk, uint64_t consumed)
{
	...
	chunk->valid = ossl_quic_sstream_get_stream_frame(
		sstream, skip, &chunk->shdr, chunk->iov,
...
static int txp_generate_stream_frames(
	...
	uint64_t *new_credit_consumed, uint64_t conn_consumed)
{
	int rc = 0;
     ...
     */
	for (i = 0; i < 2; ++i) {
		if (!txp_plan_stream_chunk(txp, h, sstream, stream_txfc, i,
					   &chunks[i], conn_consumed))
			goto err;
	...
	for (i = 0;; ++i) {
		...
		if (i > 0)
			...
			if (!txp_plan_stream_chunk(txp, h, sstream, stream_txfc,
						   i + 1, &chunks[(i + 1) % 2],
						   conn_consumed))
				goto err;
		...
		use_explicit_len =
			!((can_fill_payload || !could_have_following_chunk) &&
			  !pkt->force_pad);
		...
		/* Truncate IOVs to match our chosen length. */
		ossl_quic_sstream_adjust_iov((size_t)shdr->len,
					     chunks[i % 2].iov,
				       ...
				       QUIC_STREAM **tmp_head)
{
	...
	for (ossl_quic_stream_iter_init(&it, txp->args.qsm, 1);
		...
		if (stream->want_stop_sending) {
			...
			if (!ossl_quic_wire_encode_frame_stop_sending(wpkt,
								      &f)) {
		...
		if (stream->want_reset_stream) {
			...
			if (!ossl_quic_stream_send_get_final_size(
				    stream, &f.final_size))
				return 0; /* should not be possible */

			if (!ossl_quic_wire_encode_frame_reset_stream(wpkt,
								      &f)) {
				tx_helper_rollback(h); /* can't fit */
				txp_enlink_tmp(tmp_head, stream);
				break;
		...
		if (ossl_quic_stream_has_send_buffer(stream) &&
			...
			if (!txp_generate_stream_frames(
				    txp, pkt, stream->id, stream->sstream,
				    &stream->txfc, snext, have_ack_eliciting,
				    &packet_full,
				    &stream->txp_txfc_new_credit_consumed,
				    conn_consumed)) {
			  ...
			  uint32_t archetype, int *txpim_pkt_reffed)
{
	...
	*txpim_pkt_reffed = 0;
	...
	for (stream = pkt->stream_head; stream != NULL;
	     stream = stream->txp_next)
		if (stream->txp_sent_stop_sending ||
		    stream->txp_sent_reset_stream) {
	...
	*txpim_pkt_reffed = 1;
	...
	for (stream = pkt->stream_head; stream != NULL;
		...
		if (ossl_quic_stream_has_send_buffer(stream) &&
             ...
             * sent all data.
             */
			ossl_quic_stream_map_notify_all_data_sent(txp->args.qsm,
								  stream);
	}
