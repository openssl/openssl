...
static int ch_init(QUIC_CHANNEL *ch)
{
	...
	if (ch->qrx != NULL) {
         ...
         */
		if (!ossl_qrx_set_late_validation_cb(ch->qrx, rx_late_validate,
						     ch))
			goto err;
	...
	for (pn_space = QUIC_PN_SPACE_INITIAL; pn_space < QUIC_PN_SPACE_NUM;
	     ++pn_space) {
		ch->crypto_recv[pn_space] =
			ossl_quic_rstream_new(NULL, NULL, 0);
...
static void ch_cleanup(QUIC_CHANNEL *ch)
{
	...
	if (ch->ackm != NULL)
		for (pn_space = QUIC_PN_SPACE_INITIAL;
		     pn_space < QUIC_PN_SPACE_NUM; ++pn_space)
...
void ossl_quic_channel_bind_qrx(QUIC_CHANNEL *tserver_ch, OSSL_QRX *qrx)
{
	if (tserver_ch->qrx == NULL && tserver_ch->is_tserver_ch == 1) {
		tserver_ch->qrx = qrx;
		ossl_qrx_set_late_validation_cb(tserver_ch->qrx,
						rx_late_validate, tserver_ch);
...
QUIC_CHANNEL *ossl_quic_channel_alloc(const QUIC_CHANNEL_ARGS *args)
{
	...
	if (ch->use_qlog && args->qlog_title != NULL) {
		if ((ch->qlog_title = OPENSSL_strdup(args->qlog_title)) ==
		    NULL) {
			OPENSSL_free(ch);
			return NULL;
		}
					 ...
					 size_t token_len)
{
	...
	cfq_item = ossl_quic_cfq_add_frame(ch->cfq, 1, QUIC_PN_SPACE_APP,
...
static int ch_on_crypto_release_record(size_t bytes_read, void *arg)
{
	...
	uint32_t rx_pn_space =
		ossl_quic_enc_level_to_pn_space(ch->rx_enc_level);
					...
					size_t secret_len, void *arg)
{
	...
	if (direction) {
		...
		if (!ossl_qtx_provide_secret(ch->qtx, enc_level, suite_id, md,
					     secret, secret_len))
			return 0;
		...
		if (!ossl_qrx_provide_secret(ch->qrx, enc_level, suite_id, md,
					     secret, secret_len))
			return 0;
				  ...
				  size_t params_len, void *arg)
{
	...
	while (PACKET_remaining(&pkt) > 0) {
		...
		case QUIC_TPARAM_ORIG_DCID:
			...
			if (!ossl_quic_wire_decode_transport_param_cid(
				    &pkt, NULL, &cid)) {
		...
		case QUIC_TPARAM_INITIAL_SCID:
			...
			if (!ossl_quic_conn_id_eq(&ch->init_scid, &cid)) {
				reason = TP_REASON_EXPECTED_VALUE(
					"INITIAL_SCID");
				goto malformed;
			}
		...
		case QUIC_TPARAM_INITIAL_MAX_DATA:
			...
			if (!ossl_quic_wire_decode_transport_param_int(
				    &pkt, &id, &v)) {
				reason =
		...
		case QUIC_TPARAM_INITIAL_MAX_STREAM_DATA_BIDI_REMOTE:
			...
			ossl_quic_stream_map_visit(&ch->qsm, txfc_bump_cwm_bidi,
						   &v);
		...
		case QUIC_TPARAM_INITIAL_MAX_STREAM_DATA_UNI:
			...
			/* Apply to all existing streams. */
			ossl_quic_stream_map_visit(&ch->qsm, txfc_bump_cwm_uni,
						   &v);
		...
		case QUIC_TPARAM_STATELESS_RESET_TOKEN:
			...
			body = ossl_quic_wire_decode_transport_param_bytes(
				&pkt, &id, &len);
			if (body == NULL ||
			    len != QUIC_STATELESS_RESET_TOKEN_LEN) {
				reason = TP_REASON_MALFORMED(
					"STATELESS_RESET_TOKEN");
				goto malformed;
			}
			if (!ossl_quic_srtm_add(
				    ch->srtm, ch, ch->cur_remote_seq_num,
				    (const QUIC_STATELESS_RESET_TOKEN *)body)) {
				reason = TP_REASON_INTERNAL_ERROR(
					"STATELESS_RESET_TOKEN");
				goto malformed;
			}
		...
		case QUIC_TPARAM_PREFERRED_ADDR:
			...
			if (!ossl_quic_wire_decode_transport_param_preferred_addr(
				    &pkt, &pfa)) {
	...
	if (got_active_conn_id_limit)
		QLOG_U64("active_connection_id_limit",
			 ch->rx_active_conn_id_limit);
	if (got_stateless_reset_token)
...
static int ch_generate_transport_params(QUIC_CHANNEL *ch)
{
	...
	if (ch->is_server) {
		...
		if (!ossl_quic_wire_encode_transport_param_cid(
			    &wpkt, QUIC_TPARAM_INITIAL_SCID,
			    &ch->cur_local_cid))
			goto err;
		if (ch->odcid.id_len != 0)
			if (!ossl_quic_wire_encode_transport_param_cid(
				    &wpkt, QUIC_TPARAM_RETRY_SCID,
				    &ch->init_dcid))
				goto err;
	} else {
		if (!ossl_quic_wire_encode_transport_param_cid(
			       ...
			       uint32_t flags)
{
	...
	if (!ch->port->engine->inhibit_tick) {
		...
		if (ossl_time_compare(now, ch->ping_deadline) >= 0) {
			int pn_space = ossl_quic_enc_level_to_pn_space(
				ch->tx_enc_level);
...
static void ch_rx_handle_packet(QUIC_CHANNEL *ch, int channel_only)
{
	...
	if (ossl_quic_pkt_type_is_encrypted(ch->qrx_pkt->hdr->type)) {
		if (!ch->have_received_enc_pkt) {
			ch->cur_remote_dcid = ch->init_scid =
             ...
             * DCID.
             */
			ossl_quic_tx_packetiser_set_cur_dcid(ch->txp,
							     &ch->init_scid);
		}

		enc_level =
			ossl_quic_pkt_type_to_enc_level(ch->qrx_pkt->hdr->type);
		if ((ch->el_discarded & (1U << enc_level)) != 0)
	...
	if (ch->qrx_pkt->hdr->type == QUIC_PKT_TYPE_VERSION_NEG) {
		...
		while (PACKET_remaining(&vpkt) > 0) {
			...
			if (supported_ver == QUIC_VERSION_1) {
                 ...
                 */
				ossl_quic_tx_packetiser_set_protocol_version(
					ch->txp, QUIC_VERSION_1);
				...
				if (!ch_restart(ch))
					ossl_quic_channel_raise_protocol_error(
						ch,
						OSSL_QUIC_ERR_INTERNAL_ERROR, 0,
	...
	case QUIC_PKT_TYPE_1RTT:
		if (ch->is_server &&
		    ch->qrx_pkt->hdr->type == QUIC_PKT_TYPE_HANDSHAKE)
			/*
...
static OSSL_TIME ch_determine_next_tick_deadline(QUIC_CHANNEL *ch)
{
	...
	if (ch->rxku_in_progress)
		deadline =
			ossl_time_min(deadline, ch->rxku_update_end_deadline);
		    ...
		    int drop_later_pn)
{
	...
	if (!ossl_quic_tx_packetiser_set_initial_token(
         ...
         */
		ossl_quic_channel_raise_protocol_error(
			ch, OSSL_QUIC_ERR_INVALID_TOKEN, 0,
				 ...
				 int force_immediate)
{
	...
	case QUIC_CHANNEL_STATE_TERMINATING_CLOSING:
		if (force_immediate)
             ...
             */
			ch_record_state_transition(
				ch, QUIC_CHANNEL_STATE_TERMINATING_DRAINING);
	...
	int src_line, const char *src_func)
{
	...
	if (frame_type != 0) {
		...
		ERR_raise_data(ERR_LIB_SSL, err_reason,
			       ...
			       (unsigned long long)error_code, err_str_pfx,
			       err_str, err_str_sfx, reason);
	}
				 ...
				 const QUIC_CONN_ID *peer_odcid)
{
	...
	if (!ossl_quic_tx_packetiser_set_cur_dcid(ch->txp,
						  &ch->cur_remote_dcid))
		return 0;
	...
	if (!ossl_quic_lcidm_enrol_odcid(ch->lcidm, ch,
					 peer_odcid == NULL ? &ch->init_dcid :
							      peer_odcid))
		return 0;
				  ...
				  const QUIC_CONN_ID *peer_dcid)
{
	...
	if (!ossl_quic_lcidm_generate_initial(ch->lcidm, ch,
					      &ch->cur_local_cid))
		return 0;
			      ...
			      int can_recv)
{
	...
	if (can_send)
		if ((qs->sstream = ossl_quic_sstream_new(INIT_APP_BUF_LEN)) ==
		    NULL)
			goto err;
							...
							int is_uni)
{
	...
	p_next_ordinal = ch_get_local_stream_next_ordinal_ptr(
		(QUIC_CHANNEL *)ch, is_uni);
...
QUIC_STREAM *ossl_quic_channel_new_stream_local(QUIC_CHANNEL *ch, int is_uni)
{
	...
	if ((qs = ossl_quic_stream_map_alloc(&ch->qsm, stream_id, type)) ==
	    NULL)
		return NULL;
					...
					SSL *msg_callback_ssl)
{
	...
	if (ch->is_tserver_ch == 0)
		ossl_qrx_set_msg_callback(ch->qrx, msg_callback,
					  msg_callback_ssl);
}
