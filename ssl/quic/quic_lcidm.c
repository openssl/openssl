				...
				const QUIC_CONN_ID *initial_odcid)
{
	...
	if (initial_odcid == NULL ||
	    initial_odcid->id_len < QUIC_MIN_ODCID_LEN ||
	...
	if ((lcid_obj = lcidm_conn_new_lcid(lcidm, conn, initial_odcid)) ==
	    NULL)
		return 0;
...
{
	return lcidm_generate(lcidm, opaque, LCID_TYPE_INITIAL, initial_lcid,
			      NULL);
}
			     ...
			     OSSL_QUIC_FRAME_NEW_CONN_ID *ncid_frame)
{
	...
	return lcidm_generate(lcidm, opaque, LCID_TYPE_NCID,
			      &ncid_frame->conn_id, &ncid_frame->seq_num);
}
