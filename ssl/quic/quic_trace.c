...
int ossl_quic_trace(int write_p, int version, int content_type, const void *buf,
		    size_t msglen, SSL *ssl, void *arg)
{
	BIO *bio = arg;
	PACKET pkt;
	size_t id_len = 0;
	...
	case SSL3_RT_QUIC_PACKET: {
		...
		if (ossl_quic_wire_decode_pkt_hdr(&pkt, id_len, 0, 1, &hdr,
						  NULL, NULL) != 1)
			return 0;
