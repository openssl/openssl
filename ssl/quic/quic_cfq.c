...
static void list_insert_after(QUIC_CFQ_ITEM_LIST *l, QUIC_CFQ_ITEM_EX *ref,
			      QUIC_CFQ_ITEM_EX *n)
{
	n->prev = ref;
	n->next = ref->next;
...
static void clear_item(QUIC_CFQ_ITEM_EX *item)
{
	if (item->free_cb != NULL) {
		item->free_cb(item->encoded, item->encoded_len,
			      item->free_cb_arg);
...
QUIC_CFQ_ITEM *ossl_quic_cfq_add_frame(QUIC_CFQ *cfq, uint32_t priority,
				       uint32_t pn_space, uint64_t frame_type,
				       uint32_t flags,
				       const unsigned char *encoded,
				       size_t encoded_len, cfq_free_cb *free_cb,
				       void *free_cb_arg)
{
