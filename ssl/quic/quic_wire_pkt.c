				 ...
				 size_t quic_hp_key_len)
{
	...
	if (!EVP_CipherInit_ex(hpr->cipher_ctx, hpr->cipher, NULL, quic_hp_key,
			       NULL, 1)) {
		ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
		goto err;
	}
			     ...
			     unsigned char *mask)
{
	...
	if (hpr->cipher_id == QUIC_HDR_PROT_CIPHER_AES_128 ||
		...
		if (!EVP_CipherInit_ex(hpr->cipher_ctx, NULL, NULL, NULL, NULL,
				       1) ||
		...
		if (!EVP_CipherInit_ex(hpr->cipher_ctx, NULL, NULL, NULL,
				       sample, 1) ||
		    !EVP_CipherUpdate(hpr->cipher_ctx, mask, &l, zeroes,
				      sizeof(zeroes))) {
			ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
			return 0;
		}
...
{
	return ossl_quic_hdr_protector_decrypt_fields(hpr, ptrs->raw_sample,
						      ptrs->raw_sample_len,
						      ptrs->raw_start,
						      ptrs->raw_pn);
}
...
int ossl_quic_hdr_protector_encrypt(QUIC_HDR_PROTECTOR *hpr,
				    QUIC_PKT_HDR_PTRS *ptrs)
{
	return ossl_quic_hdr_protector_encrypt_fields(hpr, ptrs->raw_sample,
						      ptrs->raw_sample_len,
						      ptrs->raw_start,
						      ptrs->raw_pn);
}
...
int ossl_quic_wire_decode_pkt_hdr(PACKET *pkt, size_t short_conn_id_len,
				  int partial, int nodata, QUIC_PKT_HDR *hdr,
				  QUIC_PKT_HDR_PTRS *ptrs, uint64_t *fail_cause)
{
	unsigned int b0;
	...
	if (l < QUIC_MIN_VALID_PKT_LEN || !PACKET_get_1(pkt, &b0))
		return 0;
	...
	if ((b0 & 0x80) == 0) {
		...
		if (!PACKET_copy_bytes(pkt, hdr->dst_conn_id.id,
				       short_conn_id_len))
			return 0;
		...
		if (!PACKET_get_1(pkt, &dst_conn_id_len) ||
		    ...
		    !PACKET_copy_bytes(pkt, hdr->dst_conn_id.id,
				       dst_conn_id_len) ||
		    !PACKET_get_1(pkt, &src_conn_id_len) ||
		    src_conn_id_len > QUIC_MAX_CONN_ID_LEN ||
		    !PACKET_copy_bytes(pkt, hdr->src_conn_id.id,
				       src_conn_id_len))
			return 0;
		...
		if (version == 0) {
			...
			if (hdr->type == QUIC_PKT_TYPE_INITIAL) {
				...
				if (!PACKET_get_quic_vlint(pkt, &token_len) ||
				    token_len > SIZE_MAX ||
				    !PACKET_get_bytes(pkt, &hdr->token,
						      (size_t)token_len))
					return 0;
			...
			if (hdr->type == QUIC_PKT_TYPE_RETRY) {
				...
				memset(hdr->pn, 0, sizeof(hdr->pn));
				if (partial) {
					if (!PACKET_forward(pkt,
							    sizeof(hdr->pn)))
						return 0;

					hdr->len =
						(size_t)(len - sizeof(hdr->pn));
				} else {
					if (!PACKET_copy_bytes(pkt, hdr->pn,
							       hdr->pn_len))
						return 0;
	...
	if (ptrs != NULL) {
		...
		if (pn != NULL) {
			...
			ptrs->raw_sample_len =
				PACKET_end(pkt) - ptrs->raw_sample;
		}
	}
				  ...
				  QUIC_PKT_HDR_PTRS *ptrs)
{
	...
	if (hdr->partial || (hdr->type == QUIC_PKT_TYPE_1RTT &&
	...
	if (hdr->type == QUIC_PKT_TYPE_1RTT) {
		...
		b0 = (hdr->spin_bit << 5) | (hdr->key_phase << 2) |
		     (hdr->pn_len - 1) | (hdr->reserved << 3) |
		     0x40; /* fixed bit */
		...
		if (hdr->type == QUIC_PKT_TYPE_VERSION_NEG) {
			if (hdr->len > 0 &&
			    !WPACKET_reserve_bytes(pkt, hdr->len, NULL))
				return 0;
...
int ossl_quic_wire_decode_pkt_hdr_pn(const unsigned char *enc_pn,
				     ...
				     QUIC_PN *res_pn)
{
	int64_t expected_pn, truncated_pn, candidate_pn, pn_win, pn_hwin,
		pn_mask;
	...
	case 1:
		...
		break;
	case 2:
		truncated_pn = ((QUIC_PN)enc_pn[0] << 8) | (QUIC_PN)enc_pn[1];
		break;
	case 3:
		truncated_pn = ((QUIC_PN)enc_pn[0] << 16) |
			       ((QUIC_PN)enc_pn[1] << 8) | (QUIC_PN)enc_pn[2];
		break;
	case 4:
		truncated_pn = ((QUIC_PN)enc_pn[0] << 24) |
			       ((QUIC_PN)enc_pn[1] << 16) |
			       ((QUIC_PN)enc_pn[2] << 8) | (QUIC_PN)enc_pn[3];
		break;
	default:
		return 0;
	}
...
{
	uint64_t num_unacked = (largest_acked == QUIC_PN_INVALID) ?
				       pn + 1 :
				       pn - largest_acked;
...
int ossl_quic_validate_retry_integrity_tag(
	OSSL_LIB_CTX *libctx, const char *propq, const QUIC_PKT_HDR *hdr,
	const QUIC_CONN_ID *client_initial_dcid)
{
	...
	if (!ossl_quic_calculate_retry_integrity_tag(
		    libctx, propq, hdr, client_initial_dcid, expected_tag))
		return 0;
...
/* RFC 9001 s. 5.8 */
static const unsigned char retry_integrity_key[] = { 0xbe, 0x0c, 0x69, 0x0b,
						     0x9f, 0x66, 0x57, 0x5a,
						     0x1d, 0x76, 0x6b, 0x54,
						     0xe3, 0x68, 0xc8, 0x4e };

static const unsigned char retry_integrity_nonce[] = { 0x46, 0x15, 0x99, 0xd3,
						       0x5d, 0x63, 0x2b, 0xf2,
						       0x23, 0x98, 0x25, 0xbb };

int ossl_quic_calculate_retry_integrity_tag(
	OSSL_LIB_CTX *libctx, const char *propq, const QUIC_PKT_HDR *hdr,
	const QUIC_CONN_ID *client_initial_dcid, unsigned char *tag)
{
	...
	if (hdr->type != QUIC_PKT_TYPE_RETRY || hdr->version == 0 ||
	    hdr->len < QUIC_RETRY_INTEGRITY_TAG_LEN || hdr->data == NULL ||
	...
	if (!EVP_CipherInit_ex(cctx, cipher, NULL, retry_integrity_key,
			       retry_integrity_nonce, /*enc=*/1)) {
		ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
		goto err;
	}
	...
	if (EVP_CIPHER_CTX_ctrl(cctx, EVP_CTRL_AEAD_GET_TAG,
				QUIC_RETRY_INTEGRITY_TAG_LEN, tag) != 1) {
		ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
		goto err;
	}
