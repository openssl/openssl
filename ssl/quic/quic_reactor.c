				  ...
				  const BIO_POLL_DESCRIPTOR *r)
{
	...
	rtor->can_poll_r = ossl_quic_reactor_can_support_poll_descriptor(
		rtor, &rtor->poll_r);
}
				  ...
				  const BIO_POLL_DESCRIPTOR *w)
{
	...
	rtor->can_poll_w = ossl_quic_reactor_can_support_poll_descriptor(
		rtor, &rtor->poll_w);
}
				       ...
				       uint32_t flags)
{
	...
	notifier_fd = (rtor->have_notifier ?
			       ossl_rio_notifier_as_fd(&rtor->notifier) :
			       INVALID_SOCKET);
...
void ossl_quic_reactor_leave_blocking_section(QUIC_REACTOR *rtor)
{
	...
	if (rtor->have_notifier && rtor->signalled_notifier) {
		if (rtor->cur_blocking_waiters == 0) {
			...
			while (rtor->signalled_notifier)
				ossl_crypto_condvar_wait(rtor->notifier_cv,
							 rtor->mutex);
		}
	}
}
