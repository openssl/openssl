					    ...
					    QUIC_STREAM **result)
{
	...
	is_remote_init =
		((stream_id & QUIC_STREAM_INITIATOR_MASK) == peer_role);
	...
	if (is_remote_init) {
		...
		if (ossl_quic_rxfc_get_error(max_streams_fc, 0) !=
			...
			ossl_quic_channel_raise_protocol_error(
				...
				frame_type, "exceeded maximum allowed streams");
			return 0;
		}
		...
		while (*p_next_ordinal_remote <= stream_ordinal) {
			uint64_t cur_stream_id =
				...
				(stream_id & (QUIC_STREAM_DIR_MASK |
					      QUIC_STREAM_INITIATOR_MASK));

			stream = ossl_quic_channel_new_stream_remote(
				ch, cur_stream_id);
			if (stream == NULL) {
				ossl_quic_channel_raise_protocol_error(
					ch, OSSL_QUIC_ERR_INTERNAL_ERROR,
		...
		*result = stream;
		...
		p_next_ordinal_local =
			is_uni ? &ch->next_local_stream_ordinal_uni :
				  ...
				  uint64_t frame_type, uint64_t *datalen)
{
	...
	*datalen = 0;
	...
	if (!ossl_quic_rxfc_on_rx_stream_frame(
		    &stream->rxfc, frame_data.offset + frame_data.len,
		    frame_data.is_fin)) {
		ossl_quic_channel_raise_protocol_error(
			ch, OSSL_QUIC_ERR_INTERNAL_ERROR, frame_type,
			"internal error (flow control)");
					   ...
					   OSSL_ACKM_RX_PKT *ackm_data)
{
	...
	if (!depack_do_implicit_stream_create(
		    ch, stream_id, OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA,
		    &stream))
		return 0; /* error already raised for us */
					       ...
					       OSSL_ACKM_RX_PKT *ackm_data)
{
     ...
     */
	if (!depack_do_implicit_stream_create(
		    ch, stream_id, OSSL_QUIC_FRAME_TYPE_STREAM_DATA_BLOCKED,
		    &stream))
		return 0; /* error already raised for us */
				 ...
				 OSSL_ACKM_RX_PKT *ackm_data)
{
	...
	while (PACKET_remaining(pkt) > 0) {
		...
		if (!was_minimal) {
			ossl_quic_channel_raise_protocol_error(
				ch, OSSL_QUIC_ERR_PROTOCOL_VIOLATION,
				frame_type, "non-minimal frame type encoding");
			return 0;
		}
		...
		case OSSL_QUIC_FRAME_TYPE_PING:
			...
			if (!depack_do_frame_ping(pkt, ch, enc_level,
						  ackm_data))
				return 0;
			break;
		case OSSL_QUIC_FRAME_TYPE_PADDING:
		...
		case OSSL_QUIC_FRAME_TYPE_ACK_WITH_ECN:
			...
			if (!depack_do_frame_ack(pkt, ch, packet_space,
						 received, frame_type,
						 parent_pkt))
				return 0;
			break;
		...
		case OSSL_QUIC_FRAME_TYPE_CRYPTO:
			...
			if (!depack_do_frame_crypto(pkt, ch, parent_pkt,
						    ackm_data, &datalen))
				return 0;
			break;
		...
		case OSSL_QUIC_FRAME_TYPE_STREAM_OFF_LEN_FIN:
			...
			if (!depack_do_frame_stream(pkt, ch, parent_pkt,
						    ackm_data, frame_type,
						    &datalen))
				return 0;
			break;
		...
		case OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA:
			...
			if (!depack_do_frame_max_stream_data(pkt, ch,
							     ackm_data))
				return 0;
			break;
		...
		case OSSL_QUIC_FRAME_TYPE_STREAM_DATA_BLOCKED:
			...
			if (!depack_do_frame_stream_data_blocked(pkt, ch,
								 ackm_data))
				return 0;
			break;
		...
		if (ch->msg_callback != NULL) {
			...
			ch->msg_callback(0, OSSL_QUIC1_VERSION, ctype, sof,
					 framelen, ch->msg_callback_ssl,
					 ch->msg_callback_arg);
		}
	}
...
int ossl_quic_handle_frames(QUIC_CHANNEL *ch, OSSL_QRX_PKT *qpacket)
{
	...
	if (enc_level == QUIC_ENC_LEVEL_HANDSHAKE)
		...
		ossl_quic_tx_packetiser_add_unvalidated_credit(ch->txp,
							       dgram_len);
