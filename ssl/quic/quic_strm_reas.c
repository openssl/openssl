    ...
    struct stream_range_t *sr, UINT_RANGE *r, const unsigned char **data)
{
    ...
    if (head_sc == tail_sc
        ...
        if (rsize <= DIRECT_STORAGE_SZ) {
            ...
            *data += rsize;
            ...
            DEBUG_PRINT(stderr,
                ...
                OPENSSL_FUNC,
                head_sc, head_sc->sc_range.start, head_sc->sc_range.end,
                new_sc, new_sc->sc_range.start, new_sc->sc_range.end);
        ...
        DEBUG_PRINT(stderr, "\trange: %p [ %llu, %llu ] -> [ %llu, %llu ]\n",
            sr, sr->sr_range.start, sr->sr_range.end, r->start, r->end);
        ...
        DEBUG_PRINT(stderr, "%s append: @in %p [ %llu, %llu ]\n",
            OPENSSL_FUNC, *data, r->start, r->end);

        if (r->start < tail_sc->sc_range.end) {
            *data += tail_sc->sc_range.end - r->start;
        ...
        *data += rsize;
        ...
        DEBUG_PRINT(stderr, "%s prpend: @in %p [ %llu, %llu ]\n",
            OPENSSL_FUNC, *data, r->start, r->end);
    ...
    struct stream_chunk_t *sc)
{
    ...
    DEBUG_PRINT(stderr, "%s range: %p [ %llu, %llu ] -> [ %llu, %llu ]\n",
        OPENSSL_FUNC, sr, sr->sr_range.start, sr->sr_range.end,
        sc->sc_range.start, sc->sc_range.end);
    sr->sr_range.start = sc->sc_range.start;
    sr->sr_range.end = sc->sc_range.end;
}
    ...
    int *fin)
{
    ...
    if (sc == NULL) {
        ...
        *iterator = NULL;
         ...
         */
        if (can_read && (sr == NULL || OSSL_RBT_NEXT(srange, sr) == NULL))
            *fin = fs->fin;
        else
            *fin = 0;
        ...
        OSSL_LIST_FOREACH_FROM(sc, sc, ossl_list_sc_head(&sr->sr_chunks))
        {
            if (sc->sc_st == ST_TYPE_PKT) {
                if (SCHUNK_SIZE(sc) <= DIRECT_STORAGE_SZ) {
                    ...
                    buf = OPENSSL_malloc(UINT64_TO_SIZE_T(SCHUNK_SIZE(sc)));
                    if (buf == NULL)
                        return 0;
...
static int flatten_range(SFRAME_SET *fs, struct stream_range_t *sr)
{
    ...
    sz = SRANGE_SIZE(sr);
    buf = OPENSSL_malloc(UINT64_TO_SIZE_T(sz));
    if (buf == NULL)
        return 0;
    w = buf;
    sc = ossl_list_sc_head(&sr->sr_chunks);
    ...
    /*
     * use the head chunk for the buf,
