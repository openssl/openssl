				     ...
				     size_t alloc_len)
{
	return e->alloc_len < alloc_len ?
		       demux_resize_urxe(demux, e, alloc_len) :
		       e;
}
...
static int demux_recv(QUIC_DEMUX *demux)
{
	...
	ERR_clear_last_mark();
	now = demux->now != NULL ? demux->now(demux->now_arg) :
				   ossl_time_zero();
			   ...
			   const BIO_ADDR *local)
{
	...
	urxe->time = demux->now != NULL ? demux->now(demux->now_arg) :
					  ossl_time_zero();
...
{
	assert(ossl_list_urxe_prev(e) == NULL &&
	       ossl_list_urxe_next(e) == NULL);
