				    ...
				    size_t datagram_len)
{
	...
	if (ossl_qrl_enc_level_set_have_el(&qrx->el_set,
					   QUIC_ENC_LEVEL_INITIAL) != 1)
		goto malformed;
	...
	if (rxe->hdr.type == QUIC_PKT_TYPE_INITIAL) {
         ...
         */
		if (!qrx_relocate_buffer(qrx, &rxe, &i, &token,
					 rxe->hdr.token_len))
			goto malformed;
	...
	el = ossl_qrl_enc_level_set_get(&qrx->el_set, QUIC_ENC_LEVEL_INITIAL,
					1);
	...
	if (ossl_quic_wire_decode_pkt_hdr(&pkt, 0, 0, 0, &rxe->hdr, NULL,
					  NULL) != 1)
		goto malformed;
	...
	if ((rxe = qrx_reserve_rxe(&qrx->rx_free, rxe, rxe->hdr.len + i)) ==
	    NULL)
		goto malformed;
	...
	if (!qrx_decrypt_pkt_body(qrx, dst, rxe->hdr.data, rxe->hdr.len,
				  &dec_len, sop, aad_len, rxe->pn,
				  QUIC_ENC_LEVEL_INITIAL, rxe->hdr.key_phase,
				  &rx_key_epoch))
		goto malformed;
			    ...
			    const unsigned char *secret, size_t secret_len)
{
	...
	if (!ossl_qrl_enc_level_set_provide_secret(
		    &qrx->el_set, qrx->libctx, qrx->propq, enc_level, suite_id,
		    md, secret, secret_len, qrx->init_key_phase_bit,
		    /*is_tx=*/0))
		return 0;
...
static void qrx_recycle_rxe(OSSL_QRX *qrx, RXE *rxe)
{
	...
	assert(ossl_list_rxe_prev(rxe) == NULL &&
	       ossl_list_rxe_next(rxe) == NULL);
	rxe->pkt.hdr = NULL;
	rxe->pkt.peer = NULL;
	rxe->pkt.local = NULL;
	ossl_list_rxe_insert_tail(&qrx->rx_free, rxe);
			       ...
			       const unsigned char **pptr, size_t buf_len)
{
	...
	if ((rxe = qrx_reserve_rxe(&qrx->rx_free, *prxe, *pi + buf_len)) ==
	    NULL)
		return 0;
	...
	*is_old_key = 0;
	...
	case QRL_EL_STATE_PROV_NORMAL:
		...
		*rx_key_epoch = el->key_epoch +
				((el->key_epoch & 1) ^ (uint64_t)key_phase_bit);
		break;
				...
				uint64_t *rx_key_epoch)
{
	...
	if (EVP_CipherUpdate(cctx, dst, &l, src,
			     (int)(src_len - el->tag_len)) != 1)
		return 0;
			   ...
			   size_t datagram_len)
{
	...
	*pkt = orig_pkt;
	...
	if (qrx->msg_callback != NULL)
		qrx->msg_callback(0, OSSL_QUIC1_VERSION, SSL3_RT_QUIC_PACKET,
				  sop, eop - sop - rxe->hdr.len,
				  qrx->msg_callback_ssl, qrx->msg_callback_arg);
				...
				const unsigned char *data, size_t data_len)
{
	...
	for (; PACKET_remaining(&pkt) > 0; ++pkt_idx) {
         ...
         */
		if (qrx_process_pkt(qrx, e, &pkt, pkt_idx, &first_dcid,
				    data_len))
			have_deferred = 1;
	}
...
int ossl_qrx_read_pkt(OSSL_QRX *qrx, OSSL_QRX_PKT **ppkt)
{
	...
	rxe->pkt.peer = BIO_ADDR_family(&rxe->peer) != AF_UNSPEC ? &rxe->peer :
								   NULL;
	rxe->pkt.local =
...
{
	OSSL_QRL_ENC_LEVEL *el = ossl_qrl_enc_level_set_get(
		&qrx->el_set, QUIC_ENC_LEVEL_1RTT, 1);
