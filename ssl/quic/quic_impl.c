...
static int expect_quic_as(const SSL *s, QCTX *ctx, uint32_t flags)
{
	...
	if (s == NULL) {
		QUIC_RAISE_NON_NORMAL_ERROR(NULL, ERR_R_PASSED_NULL_PARAMETER,
					    NULL);
		goto err;
	}
	...
	case SSL_TYPE_QUIC_CONNECTION:
		...
		ctx->qd = qc->domain;
		ctx->ql =
			qc->listener; /* never changes, so can be read without lock */
		ctx->qc = qc;
		...
		if ((flags & QCTX_AUTO_S) != 0 && qc->default_xso == NULL) {
			if (!quic_mutation_allowed(qc, /*req_active=*/0)) {
				QUIC_RAISE_NON_NORMAL_ERROR(
					ctx, SSL_R_PROTOCOL_IS_SHUTDOWN, NULL);
				goto err;
			}
			...
			if ((flags & QCTX_REMOTE_INIT) != 0) {
				if (!qc_wait_for_default_xso_for_read(
					    ctx, /*peek=*/0))
					goto err;
			} else {
...
SSL *ossl_quic_new(SSL_CTX *ctx)
{
	...
	if (ctx->method == OSSL_QUIC_server_method()) {
		QUIC_RAISE_NON_NORMAL_ERROR(
			NULL, ERR_R_SHOULD_NOT_HAVE_BEEN_CALLED, NULL);
		return NULL;
	}
	...
	if (qc->tls == NULL ||
	    (sc = SSL_CONNECTION_FROM_SSL(qc->tls)) == NULL) {
		QUIC_RAISE_NON_NORMAL_ERROR(NULL, ERR_R_INTERNAL_ERROR, NULL);
		goto err;
	}
	...
	ossl_quic_channel_set_msg_callback(qc->ch, ctx->msg_callback,
					   &qc->obj.ssl);
	...
	qc->default_ssl_options = qc->obj.ssl.ctx->options &
				     ...
				     uint64_t or_value)
{
	...
	if (!ctx.is_stream) {
		...
		ctx.qc->default_ssl_options =
			((ctx.qc->default_ssl_options & ~mask_value) |
			 or_value) &
			OSSL_QUIC_PERMITTED_OPTIONS;
	}
			    ...
			    const SSL_SHUTDOWN_EX_ARGS *args, size_t args_len)
{
	...
	if (!wait_peer) {
         ...
         */
		if (args != NULL) {
			ossl_quic_channel_set_tcause(ctx.qc->ch,
	...
	if (wait_peer && !ossl_quic_channel_is_term_any(ctx.qc->ch)) {
		if (!no_block && qctx_blocking(&ctx)) {
			ret = block_until_pred(&ctx, quic_shutdown_peer_wait,
					       ctx.qc, 0);
			if (ret < 1) {
				ret = 0;
				goto err;
...
int ossl_quic_set_connect_state(SSL *s, int raiseerrs)
{
	...
	if (ctx.qc->started) {
		if (raiseerrs)
			QUIC_RAISE_NON_NORMAL_ERROR(NULL, SSL_R_INVALID_COMMAND,
						    NULL);
		return 0;
	}
...
static int ensure_channel_started(QCTX *ctx)
{
	...
	if (!qc->started) {
		...
		if (qc->is_thread_assisted)
			if (!ossl_quic_thread_assist_init_start(
				    &qc->thread_assist, qc->ch)) {
				QUIC_RAISE_NON_NORMAL_ERROR(
					ctx, ERR_R_INTERNAL_ERROR,
...
static int quic_do_handshake(QCTX *ctx)
{
	...
	if (!qc->started && ossl_quic_port_is_addressed_w(port) &&
		...
		if (!csm_analyse_init_peer_addr(net_wbio, &qc->init_peer_addr))
			...
			ossl_quic_channel_set_peer_addr(qc->ch,
							&qc->init_peer_addr);
	}
	...
	if (!qc->started && ossl_quic_port_is_addressed_w(port) &&
         ...
         */
		QUIC_RAISE_NON_NORMAL_ERROR(
			ctx, SSL_R_REMOTE_PEER_ADDRESS_NOT_SET, NULL);
		return -1; /* Non-protocol error */
	}
...
static int quic_wait_for_stream(void *arg)
{
	...
	if (!quic_mutation_allowed(args->qc, /*req_active=*/1)) {
		...
		QUIC_RAISE_NON_NORMAL_ERROR(args->ctx,
					    SSL_R_PROTOCOL_IS_SHUTDOWN, NULL);
		return -1;
	}
...
static int qc_wait_for_default_xso_for_read(QCTX *ctx, int peek)
{
	...
	if (qs == NULL)
		qs = ossl_quic_stream_map_get_by_id(
			ossl_quic_channel_get_qsm(qc->ch),
	...
	if (qs == NULL) {
		...
		qs = ossl_quic_stream_map_get_by_id(
			ossl_quic_channel_get_qsm(qc->ch), expect_id);
	}
	...
	if (qs == NULL) {
		...
		if (ossl_quic_channel_is_term_any(qc->ch)) {
			...
			return QUIC_RAISE_NORMAL_ERROR(ctx,
						       SSL_ERROR_WANT_READ);
		}
...
static QUIC_XSO *create_xso_from_stream(QUIC_CONNECTION *qc, QUIC_STREAM *qs)
{
	...
	xso->ssl_options = qc->default_ssl_options &
			   OSSL_QUIC_PERMITTED_OPTIONS_STREAM;
...
static int quic_new_stream_wait(void *arg)
{
	...
	if (ossl_quic_channel_is_new_local_stream_admissible(qc->ch,
							     args->is_uni))
		return 1;
...
static SSL *quic_conn_stream_new(QCTX *ctx, uint64_t flags, int need_lock)
{
	...
	if (!advance &&
		...
		if (no_blocking || !qctx_blocking(ctx)) {
			QUIC_RAISE_NON_NORMAL_ERROR(
				ctx, SSL_R_STREAM_COUNT_LIMITED, NULL);
			goto err;
		}
...
int ossl_quic_want(const SSL *s)
{
	...
	w = error_to_want(ctx.is_stream ? ctx.xso->last_error :
					  ctx.qc->last_error);
			    ...
			    uint64_t flags, int do_tick)
{
	...
	if (do_tick)
		ossl_quic_reactor_tick(
			ossl_quic_channel_get_reactor(xso->conn->ch), 0);
}
...
static int quic_write_again(void *arg)
{
	...
	if (!xso_sstream_append(args->xso, args->buf, args->len,
				&actual_written))
		return -2;
			       ...
			       uint64_t flags, size_t *written)
{
	...
	quic_post_write(xso, actual_written > 0, actual_written == len, flags,
			1);
				      ...
				      uint64_t flags, size_t *written)
{
	...
	if (xso->aon_write_in_progress) {
		...
		if ((!accept_moving_buffer && xso->aon_buf_base != buf) ||
             ...
             */
			return QUIC_RAISE_NON_NORMAL_ERROR(
				ctx, SSL_R_BAD_WRITE_RETRY, NULL);
...
static int quic_validate_for_write(QUIC_XSO *xso, int *err)
{
	...
	case QUIC_SSTREAM_STATE_READY:
		...
		if (!ossl_quic_stream_map_ensure_send_part_id(qsm,
							      xso->stream)) {
			*err = ERR_R_INTERNAL_ERROR;
			return 0;
		}
	...
	case QUIC_SSTREAM_STATE_DATA_SENT:
		if (ossl_quic_sstream_get_final_size(xso->stream->sstream,
						     NULL)) {
			  ...
			  size_t *written)
{
	...
	*written = 0;
	...
	if (len == 0) {
		...
		if (!expect_quic_with_stream_lock(s, /*remote_init=*/0,
						  /*io=*/1, &ctx))
			return 0;
	}
	...
	if ((flags & ~SSL_WRITE_FLAG_CONCLUDE) != 0) {
		ret = QUIC_RAISE_NON_NORMAL_ERROR(
			&ctx, SSL_R_UNSUPPORTED_WRITE_FLAG, NULL);
		goto out;
	}
	...
	if (qctx_blocking(&ctx))
		...
		ret = quic_write_nonblocking_epw(&ctx, buf, len, flags,
						 written);
	else
		ret = quic_write_nonblocking_aon(&ctx, buf, len, flags,
						 written);
	...
	if (!peek) {
		if (*bytes_read > 0) {
			...
			ossl_statm_get_rtt_info(
				ossl_quic_channel_get_statm(qc->ch), &rtt_info);

			if (!ossl_quic_rxfc_on_retire(&stream->rxfc,
						      *bytes_read,
		...
		if (*bytes_read > 0)
			ossl_quic_stream_map_update_state(
				ossl_quic_channel_get_qsm(qc->ch), stream);
	}
		     ...
		     int peek)
{
	...
	*bytes_read = 0;
	...
	if (*bytes_read > 0) {
		...
		/* Try the read again. */
		if (!quic_read_actual(&ctx, ctx.xso->stream, buf, len,
				      bytes_read, peek)) {
			ret = 0; /* quic_read_actual raised error here */
			goto out;
		}
		...
		if (*bytes_read > 0)
			...
			ret = QUIC_RAISE_NORMAL_ERROR(&ctx,
						      SSL_ERROR_WANT_READ);
	}
...
static size_t ossl_quic_pending_int(const SSL *s, int check_channel)
{
	...
	if (ctx.xso == NULL) {
		...
		if (qc_wait_for_default_xso_for_read(&ctx, /*peek=*/1)) {
			...
			QUIC_RAISE_NON_NORMAL_ERROR(&ctx, SSL_R_NO_STREAM,
						    NULL);
			goto out;
		}
	}
...
int ossl_quic_get_stream_type(SSL *s)
{
	...
	if (ossl_quic_stream_is_server_init(ctx.xso->stream) !=
	    ctx.qc->as_server)
...
uint64_t ossl_quic_get_stream_id(SSL *s)
{
	...
	if (!expect_quic_with_stream_lock(s, /*remote_init=*/-1, /*io=*/0,
					  &ctx))
		return UINT64_MAX;
...
int ossl_quic_set_default_stream_mode(SSL *s, uint32_t mode)
{
	...
	default:
		qctx_unlock(&ctx);
		return QUIC_RAISE_NON_NORMAL_ERROR(
			&ctx, ERR_R_PASSED_INVALID_ARGUMENT,
				  ...
				  uint64_t *p_value_out, uint64_t *p_value_in)
{
	...
	case SSL_VALUE_CLASS_FEATURE_REQUEST:
		value_out = ossl_quic_channel_get_max_idle_timeout_request(
			ctx->qc->ch);
		...
		if (p_value_in != NULL) {
			...
			if (ossl_quic_channel_have_generated_transport_params(
				    ctx->qc->ch)) {
				QUIC_RAISE_NON_NORMAL_ERROR(
					ctx, SSL_R_FEATURE_NOT_RENEGOTIABLE,
					NULL);
				goto err;
			}
	...
	case SSL_VALUE_CLASS_FEATURE_NEGOTIATED:
		...
		if (!ossl_quic_channel_is_handshake_complete(ctx->qc->ch)) {
			QUIC_RAISE_NON_NORMAL_ERROR(
				ctx, SSL_R_FEATURE_NEGOTIATION_NOT_COMPLETE,
				NULL);
			goto err;
		}
			       ...
			       int is_remote, uint64_t *value)
{
	...
	*value = is_remote ? ossl_quic_channel_get_remote_stream_count_avail(
				     ctx->qc->ch, is_uni) :
			     ...
			     uint64_t *value)
{
	...
	case SSL_VALUE_QUIC_STREAM_BIDI_LOCAL_AVAIL:
		return qc_get_stream_avail(&ctx, class_, /*uni=*/0,
					   /*remote=*/0, value);
	case SSL_VALUE_QUIC_STREAM_BIDI_REMOTE_AVAIL:
		return qc_get_stream_avail(&ctx, class_, /*uni=*/0,
					   /*remote=*/1, value);
	case SSL_VALUE_QUIC_STREAM_UNI_LOCAL_AVAIL:
		return qc_get_stream_avail(&ctx, class_, /*uni=*/1,
					   /*remote=*/0, value);
...
SSL *ossl_quic_accept_stream(SSL *s, uint64_t flags)
{
	...
	if ((flags & SSL_ACCEPT_STREAM_UNI) &&
	    !(flags & SSL_ACCEPT_STREAM_BIDI)) {
	...
	if (qs == NULL) {
		if (qctx_blocking(&ctx) &&
			...
			ret = block_until_pred(&ctx, wait_for_incoming_stream,
					       &args, 0);
			if (ret == 0) {
				QUIC_RAISE_NON_NORMAL_ERROR(
					&ctx, ERR_R_INTERNAL_ERROR, NULL);
			   ...
			   size_t args_len)
{
	...
	if (!expect_quic_with_stream_lock(ssl, /*remote_init=*/0, /*io=*/0,
					  &ctx))
		return 0;
				 ...
				 uint64_t *app_error_code)
{
	...
	if (!ossl_quic_stream_is_bidi(qs) && local_init != is_write) {
		...
		*app_error_code = !is_write ? qs->stop_sending_aec :
					      qs->reset_stream_aec;
		...
		*app_error_code = !is_write ? qs->peer_reset_stream_aec :
...
static int quic_get_stream_state(SSL *ssl, int is_write)
{
	...
	if (!expect_quic_with_stream_lock(ssl, /*remote_init=*/-1, /*io=*/0,
					  &ctx))
...
int ossl_quic_set_write_buffer_size(SSL *ssl, size_t size)
{
	...
	if (!ossl_quic_sstream_set_buffer_size(ctx.xso->stream->sstream,
					       size)) {
		QUIC_RAISE_NON_NORMAL_ERROR(&ctx, ERR_R_INTERNAL_ERROR, NULL);
		goto out;
	}
...
int ossl_quic_key_update(SSL *ssl, int update_type)
{
	...
	if (!ossl_quic_channel_trigger_txku(ctx.qc->ch)) {
		QUIC_RAISE_NON_NORMAL_ERROR(&ctx, SSL_R_TOO_MANY_KEY_UPDATES,
					    NULL);
		qctx_unlock(&ctx);
		return 0;
	}
...
SSL *ossl_quic_new_from_listener(SSL *ssl, uint64_t flags)
{
	...
	if (ssl->ctx->tokencache == NULL)
		if ((ssl->ctx->tokencache = ossl_quic_new_token_store()) ==
		    NULL)
			goto err;
...
/* create channel */
	...
	/* Create the handshake layer. */
	qc->tls = ossl_ssl_connection_new_int(ql->obj.ssl.ctx, NULL,
					      TLS_method());
	if (qc->tls == NULL ||
	    (sc = SSL_CONNECTION_FROM_SSL(qc->tls)) == NULL) {
		QUIC_RAISE_NON_NORMAL_ERROR(NULL, ERR_R_INTERNAL_ERROR, NULL);
	...
	ossl_quic_channel_set_msg_callback_arg(
		qc->ch, ql->obj.ssl.ctx->msg_callback_arg);
	...
	if (!ossl_quic_obj_init(&qc->obj, ql->obj.ssl.ctx,
				SSL_TYPE_QUIC_CONNECTION, &ql->obj.ssl, NULL,
				NULL)) {
		QUIC_RAISE_NON_NORMAL_ERROR(NULL, ERR_R_INTERNAL_ERROR, NULL);
		goto err;
	}
...
SSL *ossl_quic_accept_connection(SSL *ssl, uint64_t flags)
{
	...
	if (new_ch == NULL && ossl_quic_port_is_running(ctx.ql->port)) {
		...
		ossl_quic_reactor_tick(
			ossl_quic_engine_get0_reactor(ctx.ql->engine), 0);
						     ...
						     QUIC_CHANNEL *ch)
{
	...
	ossl_quic_channel_get_peer_addr(ch,
	...
	qc->default_stream_mode = SSL_DEFAULT_STREAM_MODE_AUTO_BIDI;
	qc->default_ssl_options = ql->obj.ssl.ctx->options &
				  OSSL_QUIC_PERMITTED_OPTIONS;
	qc->incoming_stream_policy = SSL_INCOMING_STREAM_POLICY_AUTO;
	qc->last_error = SSL_ERROR_NONE;
	qc_update_reject_policy(qc);
					     ...
					     size_t token_len)
{
	...
	new_token =
		OPENSSL_zalloc(sizeof(QUIC_TOKEN) + hashkey_len + token_len);
	if (new_token == NULL)
		return NULL;
...
static int test_poll_event_r(QUIC_XSO *xso)
{
	...
	return ossl_quic_stream_has_recv_buffer(xso->stream) &&
	       ossl_quic_rstream_available(xso->stream->rstream, &avail,
					   &fin) &&
