...
void ossl_qlog_event_connectivity_connection_started(
	QLOG *qlog, const QUIC_CONN_ID *init_dcid)
{
...
#ifndef OPENSSL_NO_QLOG
static const char *map_state_to_qlog(uint32_t state, int handshake_complete,
...
void ossl_qlog_event_connectivity_connection_state_updated(
	QLOG *qlog, uint32_t old_state, uint32_t new_state,
	int handshake_complete, int handshake_confirmed)
{
#ifndef OPENSSL_NO_QLOG
	const char *state_s;
...
void ossl_qlog_event_connectivity_connection_closed(
	QLOG *qlog, const QUIC_TERMINATE_CAUSE *tcause)
{
#ifndef OPENSSL_NO_QLOG
 ...
 */
static int log_frame_actual(QLOG *qlog_instance, PACKET *pkt, size_t *need_skip)
{
	uint64_t frame_type;
	...
	case OSSL_QUIC_FRAME_TYPE_PADDING:
		...
		QLOG_U64("payload_length", ossl_quic_wire_decode_padding(pkt));
		break;
	...
	case OSSL_QUIC_FRAME_TYPE_ACK_WITH_ECN:
		...
		if (!ossl_quic_wire_decode_frame_ack(pkt, 3, &ack,
						     &total_ranges))
			goto unknown;
	...
	case OSSL_QUIC_FRAME_TYPE_STREAM_OFF_LEN_FIN: {
		...
		if (f.is_fin)
			QLOG_BOOL("fin", 1);
		*need_skip = f.has_explicit_len ? *need_skip + (size_t)f.len :
						  SIZE_MAX;
	} break;
	...
	case OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA: {
		...
		if (!ossl_quic_wire_decode_frame_max_stream_data(
			    pkt, &stream_id, &max_data))
			goto unknown;
	...
	case OSSL_QUIC_FRAME_TYPE_PATH_CHALLENGE: {
		...
		if (!ossl_quic_wire_decode_frame_path_challenge(pkt,
								&challenge))
			goto unknown;
	...
	case OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID: {
		...
		QLOG_BIN("stateless_reset_token", f.stateless_reset.token,
	...
	case OSSL_QUIC_FRAME_TYPE_NEW_TOKEN: {
		...
		if (!ossl_quic_wire_decode_frame_new_token(pkt, &token,
							   &token_len))
			goto unknown;
...
static void log_frame(QLOG *qlog_instance, PACKET *pkt, size_t *need_skip)
{
...
static int log_frames(QLOG *qlog_instance, const OSSL_QTX_IOVEC *iovec,
		      size_t num_iovec)
{
	size_t i;
	PACKET pkt;
...
static void log_packet(QLOG *qlog_instance, const QUIC_PKT_HDR *hdr, QUIC_PN pn,
...
void ossl_qlog_event_transport_packet_sent(QLOG *qlog, const QUIC_PKT_HDR *hdr,
					   QUIC_PN pn,
...
void ossl_qlog_event_transport_packet_received(
	QLOG *qlog, const QUIC_PKT_HDR *hdr, QUIC_PN pn,
	const OSSL_QTX_IOVEC *iovec, size_t num_iovec, uint64_t datagram_id)
{
#ifndef OPENSSL_NO_QLOG
