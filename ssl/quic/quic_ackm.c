...
{
	OSSL_ACKM_TX_PKT *acked_pkts = NULL, **fixup = &acked_pkts, *pkt,
			 *pprev;
	...
	for (; pkt != NULL; pkt = pprev) {
		...
		for (;; ++ridx) {
			...
			if (range_contains(&ack->ack_ranges[ridx],
					   pkt->pkt_num)) {
...
static OSSL_TIME ackm_get_pto_time_and_space(OSSL_ACKM *ackm, int *space)
{
	...
	duration = ossl_time_multiply(duration,
	...
	for (i = QUIC_PN_SPACE_INITIAL; i < QUIC_PN_SPACE_NUM; ++i) {
		...
		if (i == QUIC_PN_SPACE_APP) {
			...
			if (!ossl_time_is_infinite(ackm->rx_max_ack_delay)) {
				uint64_t factor = (uint64_t)1
						  << min_u32(ackm->pto_count,
							     MAX_PTO_COUNT);

				duration = ossl_time_add(
					duration,
					ossl_time_multiply(
						ackm->rx_max_ack_delay,
						factor));
			}
		}
         ...
         */
		if (!ossl_time_is_zero(
			    ackm->time_of_last_ack_eliciting_pkt[i])) {
			t = ossl_time_add(
				ackm->time_of_last_ack_eliciting_pkt[i],
				duration);
						 ...
						 OSSL_TIME deadline)
{
	...
	if (ackm->loss_detection_deadline_cb != NULL)
		ackm->loss_detection_deadline_cb(
			deadline, ackm->loss_detection_deadline_cb_arg);
}
	...
	for (p = lpkt; p != NULL; p = pnext) {
		...
		if (p->is_inflight) {
			...
			if (p->is_ack_eliciting)
				ackm->ack_eliciting_bytes_in_flight
					[p->pkt_space] -= p->num_bytes;
...
int ossl_ackm_on_tx_packet(OSSL_ACKM *ackm, OSSL_ACKM_TX_PKT *pkt)
{
	...
	if (ossl_time_is_zero(pkt->time) ||
	    ossl_time_compare(
		    ackm->time_of_last_ack_eliciting_pkt[pkt->pkt_space],
		    pkt->time) > 0)
		return 0;
			      ...
			      int pkt_space, OSSL_TIME rx_time)
{
	...
	if (na_pkts->pkt_num == ack->ack_ranges[0].end &&
		...
		if (ackm->handshake_confirmed)
			ack_delay = ossl_time_min(ack_delay,
						  ackm->rx_max_ack_delay);
				    ...
				    OSSL_TIME deadline)
{
	...
	if (ackm->ack_deadline_cb != NULL)
		ackm->ack_deadline_cb(ossl_ackm_get_ack_deadline(ackm,
								 pkt_space),
...
int ossl_ackm_on_rx_packet(OSSL_ACKM *ackm, const OSSL_ACKM_RX_PKT *pkt)
{
	...
	if (ossl_ackm_is_rx_pn_processable(ackm, pkt->pkt_num,
					   pkt->pkt_space) != 1)
     ...
     */
	if (pkt->is_ack_eliciting)
		ackm_on_rx_ack_eliciting(ackm, pkt->time, pkt->pkt_space,
					 was_missing);
