			   ...
			   size_t ikm_len, unsigned char *out, size_t out_len)
{
     ...
     */
	*p++ = OSSL_PARAM_construct_int(OSSL_KDF_PARAM_FIPS_KEY_CHECK,
					&key_check);
	*p++ = OSSL_PARAM_construct_int(OSSL_KDF_PARAM_MODE, &mode);
	*p++ = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_DIGEST,
						(char *)md_name, 0);
	*p++ = OSSL_PARAM_construct_octet_string(
		OSSL_KDF_PARAM_SALT, (unsigned char *)salt, salt_len);
				     ...
				     struct ossl_qtx_st *qtx)
{
	...
	if (((qtx != NULL && tx_secret == client_initial_secret) ||
	    ...
	    !tls13_hkdf_expand_ex(
		    ...
		    sizeof(quic_client_in_label), NULL, 0,
		    client_initial_secret, sizeof(client_initial_secret), 1))
		goto err;
	...
	if (((qtx != NULL && tx_secret == server_initial_secret) ||
	    ...
	    !tls13_hkdf_expand_ex(
		    ...
		    sizeof(quic_server_in_label), NULL, 0,
		    server_initial_secret, sizeof(server_initial_secret), 1))
		goto err;
