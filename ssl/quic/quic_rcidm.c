...
static void rcidm_check_rcid(QUIC_RCIDM *rcidm, RCID *rcid)
{
	...
	assert(rcid->cid.id_len > 0 &&
	       rcid->cid.id_len <= QUIC_MAX_CONN_ID_LEN);
				  ...
				  unsigned int state)
{
	...
	if (rcidm->cur_rcid != NULL && state == RCID_STATE_CUR) {
		rcidm_transition_rcid(rcidm, rcidm->cur_rcid,
				      RCID_STATE_RETIRING);
				     ...
				     const QUIC_CONN_ID *rcid)
{
	...
	rcid_obj = rcidm_create_rcid(rcidm, INITIAL_SEQ_NUM, rcid,
				     RCID_TYPE_INITIAL);
	if (rcid_obj == NULL)
		return 0;
