 ...
 *       ossl_pq_node_in_queue(&rcid->pqn) (debug assert only);
...
typedef struct rcid_st {
    ...
    OSSL_PQ_NODE pqn; /* embedded priority-queue node (valid iff PENDING) */
...
static void rcidm_check_rcid(QUIC_RCIDM *rcidm, RCID *rcid)
{
    assert(rcid->state == RCID_STATE_PENDING
        ...
        || rcid->state == RCID_STATE_RETIRING);
    assert((rcid->state == RCID_STATE_PENDING)
        == ossl_pq_node_in_queue(&rcid->pqn));
...
static ossl_inline RCID *rcid_peek(OSSL_PQ *pq)
{
    OSSL_PQ_NODE *n = ossl_pq_peek(pq);

    return n != NULL ? CONTAINER_OF(n, RCID, pqn) : NULL;
}

static void rcid_free_node(OSSL_PQ_NODE *n)
{
...
QUIC_RCIDM *ossl_quic_rcidm_new(const QUIC_CONN_ID *initial_odcid)
{
    ...
    if ((rcidm->rcids = ossl_pq_new(rcid_cmp)) == NULL) {
        OPENSSL_free(rcidm);
        return NULL;
    }
...
void ossl_quic_rcidm_free(QUIC_RCIDM *rcidm)
{
    ...
    ossl_pq_pop_free(rcidm->rcids, rcid_free_node);
    ...
    unsigned int type)
{
    ...
    if (cid->id_len < 1 || cid->id_len > QUIC_MAX_CONN_ID_LEN
        ...
        || ossl_pq_num(rcidm->rcids) + rcidm->num_retiring
            > MAX_NUMBERED_RCIDS)
        return NULL;
    ...
    ossl_pq_node_init(&rcid->pqn);

    if (rcid->seq_num >= rcidm->retire_prior_to) {
        rcid->state = RCID_STATE_PENDING;

        if (!ossl_pq_push(rcidm->rcids, &rcid->pqn)) {
            OPENSSL_free(rcid);
            return NULL;
        }
    } else {
    ...
    unsigned int state)
{
    ...
    if (old_state == RCID_STATE_PENDING)
        ossl_pq_remove(rcidm->rcids, &rcid->pqn);
...
{
    return ossl_pq_num(rcidm->rcids)
