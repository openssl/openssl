...
uint64_t ossl_quic_txfc_get_credit(QUIC_TXFC *txfc, uint64_t consumed)
{
	...
	if (txfc->parent != NULL) {
		...
		conn_r =
			ossl_quic_txfc_get_credit_local(txfc->parent, consumed);
		if (conn_r < r)
			r = conn_r;
	}
...
int ossl_quic_txfc_consume_credit(QUIC_TXFC *txfc, uint64_t num_bytes)
{
	...
	if (txfc->parent != NULL) {
		assert(txfc->parent->parent == NULL);
		if (!ossl_quic_txfc_consume_credit_local(txfc->parent,
							 num_bytes))
			return 0;
	}
...
int ossl_quic_rxfc_on_rx_stream_frame(QUIC_RXFC *rxfc, uint64_t end, int is_fin)
{
	...
	if (end > rxfc->hwm) {
		...
		if (rxfc->parent != NULL)
			on_rx_controlled_bytes(rxfc->parent,
					       delta); /* result ignored */
...
static int rxfc_cwm_bump_desired(QUIC_RXFC *rxfc)
{
	...
	uint64_t threshold = safe_muldiv_uint64_t(rxfc->cur_window_size,
						  WINDOW_THRESHOLD_NUM,
...
int ossl_quic_rxfc_on_retire(QUIC_RXFC *rxfc, uint64_t num_bytes, OSSL_TIME rtt)
{
	...
	if (!rxfc->standalone)
		rxfc_on_retire(rxfc->parent, num_bytes, rxfc->cur_window_size,
			       rtt);
