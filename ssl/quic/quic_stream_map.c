...
{
	assert(n->prev != NULL && n->next != NULL && n->prev != n &&
	       n->next != n);
...
void ossl_quic_stream_map_release(QUIC_STREAM_MAP *qsm, QUIC_STREAM *stream)
{
	...
	if (stream->ready_for_gc_node.next != NULL)
		list_remove(&qsm->ready_for_gc_list,
			    &stream->ready_for_gc_node);
...
void ossl_quic_stream_map_update_state(QUIC_STREAM_MAP *qsm, QUIC_STREAM *s)
{
	...
	if (!s->ready_for_gc) {
		...
		if (s->ready_for_gc)
			list_insert_tail(&qsm->ready_for_gc_list,
					 &s->ready_for_gc_node);
	}
						...
						uint64_t final_size)
{
	...
	case QUIC_RSTREAM_STATE_SIZE_KNOWN:
	case QUIC_RSTREAM_STATE_DATA_RECVD:
		if (ossl_quic_stream_recv_get_final_size(qs,
							 &prev_final_size) &&
