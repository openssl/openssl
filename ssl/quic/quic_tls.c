...
static int quic_new_record_layer(
	OSSL_LIB_CTX *libctx, const char *propq, int vers, int role,
	int direction, int level, uint16_t epoch, unsigned char *secret,
	size_t secretlen, unsigned char *key, size_t keylen, unsigned char *iv,
	size_t ivlen, unsigned char *mackey, size_t mackeylen,
	const EVP_CIPHER *ciph, size_t taglen, int mactype, const EVP_MD *md,
	COMP_METHOD *comp, const EVP_MD *kdfdigest, BIO *prev, BIO *transport,
	BIO *next, BIO_ADDR *local, BIO_ADDR *peer, const OSSL_PARAM *settings,
	const OSSL_PARAM *options, const OSSL_DISPATCH *fns, void *cbarg,
	void *rlarg, OSSL_RECORD_LAYER **retrl)
{
	...
	*retrl = rl;
	...
	if (!rl->qtls->args.yield_secret_cb(
		    level, qdir, suite_id, (EVP_MD *)kdfdigest, secret,
		    secretlen, rl->qtls->args.yield_secret_cb_arg)) {
		QUIC_TLS_FATAL(rl, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
...
static int quic_write_records(OSSL_RECORD_LAYER *rl,
			      OSSL_RECORD_TEMPLATE *template, size_t numtempl)
{
	size_t consumed;
	unsigned char alert;
	...
	if (rl->msg_callback != NULL) {
		...
		if (rl->level != OSSL_RECORD_PROTECTION_LEVEL_NONE) {
			rl->msg_callback(1, TLS1_3_VERSION,
	...
	case SSL3_RT_ALERT:
		...
		if (!rl->qtls->args.alert_cb(rl->qtls->args.alert_cb_arg,
					     alert)) {
			QUIC_TLS_FATAL(rl, SSL_AD_INTERNAL_ERROR,
				       ERR_R_INTERNAL_ERROR);
			return OSSL_RECORD_RETURN_FATAL;
	...
	case SSL3_RT_HANDSHAKE:
         ...
         * failure)
         */
		if (!rl->qtls->args.crypto_send_cb(
			    template->buf + rl->written,
			    template->buflen - rl->written, &consumed,
		...
		if (consumed + rl->written != template->buflen) {
			if (!ossl_assert(consumed + rl->written <
					 template->buflen)) {
				QUIC_TLS_FATAL(rl, SSL_AD_INTERNAL_ERROR,
					       ERR_R_INTERNAL_ERROR);
				return OSSL_RECORD_RETURN_FATAL;
			    ...
			    uint16_t *epoch, unsigned char *seq_num)
{
	...
	if (!rl->qtls->args.crypto_recv_rcd_cb(
		    data, datalen, rl->qtls->args.crypto_recv_rcd_cb_arg)) {
		QUIC_TLS_FATAL(rl, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
		return OSSL_RECORD_RETURN_FATAL;
	}
	...
	*type = SSL3_RT_HANDSHAKE;
	...
	if (rl->msg_callback != NULL) {
		...
		rl->msg_callback(0, TLS1_3_VERSION, SSL3_RT_HEADER, dummyrec,
				 SSL3_RT_HEADER_LENGTH, rl->cbarg);
		rl->msg_callback(0, TLS1_3_VERSION, SSL3_RT_INNER_CONTENT_TYPE,
				 type, 1, rl->cbarg);
	}
			       ...
			       size_t length)
{
	...
	if (rl->recunreleased == length) {
		if (!rl->qtls->args.crypto_release_rcd_cb(
			    rl->recread,
				     ...
				     void *parse_arg)
{
	...
	return qtls->args.got_transport_params_cb(
		in, inlen, qtls->args.got_transport_params_cb_arg);
}
...
QUIC_TLS *ossl_quic_tls_new(const QUIC_TLS_ARGS *args)
{
	...
	if (args->crypto_send_cb == NULL || args->crypto_recv_rcd_cb == NULL ||
	...
	if (args->ossl_quic &&
	    (qtls->error_state = OSSL_ERR_STATE_new()) == NULL) {
		OPENSSL_free(qtls);
		return NULL;
	}
...
static int raise_error(QUIC_TLS *qtls, uint64_t error_code,
		       const char *error_msg, const char *src_file,
		       int src_line, const char *src_func)
{
	/*
	...
	ERR_set_error(
		ERR_LIB_SSL, SSL_R_QUIC_HANDSHAKE_LAYER_ERROR,
...
#define RAISE_ERROR(qtls, error_code, error_msg)                     \
	raise_error((qtls), (error_code), (error_msg), OPENSSL_FILE, \
...
#else
#define RAISE_INTERNAL_ERROR(qtls) RAISE_ERROR((qtls), 0x01, "internal error")
#endif
...
int ossl_quic_tls_configure(QUIC_TLS *qtls)
{
	...
	if (sc == NULL ||
	    !SSL_set_min_proto_version(qtls->args.s, TLS1_3_VERSION))
	...
	if (!ossl_tls_add_custom_ext_intern(
		    NULL, &sc->cert->custext,
		    qtls->args.is_server ? ENDPOINT_SERVER : ENDPOINT_CLIENT,
		    TLSEXT_TYPE_quic_transport_parameters,
		    SSL_EXT_TLS1_3_ONLY | SSL_EXT_CLIENT_HELLO |
			    SSL_EXT_TLS1_3_ENCRYPTED_EXTENSIONS,
		    add_transport_params_cb, free_transport_params_cb, qtls,
	...
	if (!qtls->configured) {
		...
		if (qtls->args.is_server) {
			...
			if (sc->ext.alpn == NULL || sc->ext.alpn_len == 0)
				return RAISE_ERROR(
					qtls, OSSL_QUIC_ERR_CRYPTO_NO_APP_PROTO,
...
int ossl_quic_tls_set_early_data_enabled(QUIC_TLS *qtls, int enabled)
{
	...
	if ((sc->session == NULL ||
	     sc->session->ext.max_early_data != 0xffffffff) &&
