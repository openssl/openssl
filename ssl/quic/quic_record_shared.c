...
static void el_teardown_keyslot(OSSL_QRL_ENC_LEVEL_SET *els, uint32_t enc_level,
				size_t keyslot)
{
	OSSL_QRL_ENC_LEVEL *el = ossl_qrl_enc_level_set_get(els, enc_level, 0);

	if (!ossl_qrl_enc_level_set_has_keyslot(els, enc_level, el->state,
						keyslot))
		return;
			    ...
			    const unsigned char *secret, size_t secret_len)
{
	...
	if (!ossl_assert(el != NULL &&
			 ossl_qrl_enc_level_set_has_keyslot(
				 els, enc_level, tgt_state, keyslot))) {
	...
	if (!tls13_hkdf_expand_ex(el->libctx, el->propq, el->md, secret,
				  ...
				  NULL, 0, el->iv[keyslot], iv_len, 1))
		goto err;

	/* Derive "quic key" key. */
	if (!tls13_hkdf_expand_ex(el->libctx, el->propq, el->md, secret,
				  quic_v1_key_label, sizeof(quic_v1_key_label),
				  NULL, 0, key, key_len, 1))
		goto err;

	/* Create and initialise cipher context. */
	if ((cipher = EVP_CIPHER_fetch(el->libctx, cipher_name, el->propq)) ==
	    NULL) {
	...
	unsigned char init_key_phase_bit, int is_tx)
{
	...
	if (el == NULL || md_name == NULL || init_key_phase_bit > 1 ||
	    is_tx < 0 || is_tx > 1 ||
	...
	/* Derive "quic hp" key. */
	if (!tls13_hkdf_expand_ex(libctx, propq, md, secret, quic_v1_hp_label,
				  sizeof(quic_v1_hp_label), NULL, 0, hpr_key,
				  hpr_key_len, 1))
		goto err;
	...
	if (enc_level == QUIC_ENC_LEVEL_1RTT) {
		...
		if (!tls13_hkdf_expand_ex(
			    libctx, propq, md, secret, quic_v1_ku_label,
			    sizeof(quic_v1_ku_label), NULL, 0,
		...
		if (!is_tx) {
			...
			if (!el_setup_keyslot(
				    els, enc_level, QRL_EL_STATE_PROV_NORMAL,
			...
			if (!tls13_hkdf_expand_ex(
				    libctx, propq, md, ku_key, quic_v1_ku_label,
				    sizeof(quic_v1_ku_label), NULL, 0, el->ku,
				    secret_len, 1))
				goto err;
		}
	}
	...
	if (!ossl_quic_hdr_protector_init(
		    &el->hpr, libctx, propq,
		    ossl_qrl_get_suite_hdr_prot_cipher_id(suite_id), hpr_key,
		    hpr_key_len))
		goto err;
				      ...
				      uint32_t enc_level)
{
	...
	if (!tls13_hkdf_expand_ex(el->libctx, el->propq, el->md, el->ku,
				  quic_v1_ku_label, sizeof(quic_v1_ku_label),
				  NULL, 0, new_ku, secret_len, 1))
		return 0;

	el_teardown_keyslot(els, enc_level, 0);

	/* Setup keyslot for CURRENT "quic ku" key. */
	if (!el_setup_keyslot(els, enc_level, QRL_EL_STATE_PROV_NORMAL, 0,
			      el->ku, secret_len))
		return 0;
