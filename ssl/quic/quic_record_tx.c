			 ...
			 QUIC_PKT_HDR_PTRS *ptrs)
{
	...
	if (!WPACKET_init_static_len(&wpkt, data,
	...
	if (qtx->msg_callback != NULL)
		qtx->msg_callback(1, OSSL_QUIC1_VERSION, SSL3_RT_QUIC_PACKET,
				  data, l, qtx->msg_callback_ssl,
				...
				QUIC_PKT_HDR_PTRS *ptrs)
{
	...
	for (;;) {
		...
		if (EVP_CipherUpdate(cctx, txe_data(txe) + txe->data_len, &l,
				     src, (int)src_len) != 1) {
			ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
			return 0;
		}
		     ...
		     const OSSL_QTX_IOVEC *iovec, size_t num_iovec)
{
	...
	if (!ossl_quic_pkt_type_is_encrypted(hdr->type)) {
		...
		if (!ossl_assert(el !=
				 NULL)) /* should already have been checked */
			return 0;
	}
	...
	if (ossl_quic_pkt_type_has_pn(hdr->type)) {
		if (!ossl_quic_wire_encode_pkt_hdr_pn(pkt->pn, hdr->pn,
						      hdr->pn_len)) {
			ret = QTX_FAIL_GENERIC;
			goto err;
		}
			    ...
			    uint32_t enc_level)
{
	...
	if (ret == 1)
		ossl_qlog_event_transport_packet_sent(qtx_get_qlog(qtx), hdr,
						      pkt->pn, iovec, num_iovec,
						      qtx->datagram_count);
...
static void txe_to_msg(TXE *txe, BIO_MSG *msg)
{
	...
	msg->flags = 0;
	msg->peer = BIO_ADDR_family(&txe->peer) != AF_UNSPEC ? &txe->peer :
							       NULL;
	msg->local = BIO_ADDR_family(&txe->local) != AF_UNSPEC ? &txe->local :
								 NULL;
}
...
int ossl_qtx_flush_net(OSSL_QTX *qtx)
{
	...
	for (;;) {
		...
		for (i = 0; i < wr; ++i) {
			if (qtx->msg_callback != NULL)
				qtx->msg_callback(1, OSSL_QUIC1_VERSION,
