...
int ossl_quic_wire_encode_frame_ack(WPACKET *pkt, uint32_t ack_delay_exponent,
				    const OSSL_QUIC_FRAME_ACK *ack)
{
	uint64_t frame_type = ack->ecn_present ?
	...
	delay = ossl_time_divide(ossl_time_divide(ack->delay_time,
						  OSSL_TIME_US),
...
size_t
ossl_quic_wire_get_encoded_frame_len_crypto_hdr(const OSSL_QUIC_FRAME_CRYPTO *f)
{
	size_t a, b, c;
...
size_t
ossl_quic_wire_get_encoded_frame_len_stream_hdr(const OSSL_QUIC_FRAME_STREAM *f)
{
	size_t a, b, c, d;
...
int ossl_quic_wire_encode_frame_max_data(WPACKET *pkt, uint64_t max_data)
{
...
{
	if (!encode_frame_hdr(pkt,
			      is_uni ? OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_UNI :
...
int ossl_quic_wire_encode_frame_data_blocked(WPACKET *pkt, uint64_t max_data)
{
...
{
	if (!encode_frame_hdr(
		    pkt, is_uni ? OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_UNI :
...
{
	if (f->conn_id.id_len < 1 || f->conn_id.id_len > QUIC_MAX_CONN_ID_LEN)
		return 0;
...
int ossl_quic_wire_encode_frame_retire_conn_id(WPACKET *pkt, uint64_t seq_num)
{
...
int ossl_quic_wire_encode_frame_path_challenge(WPACKET *pkt, uint64_t data)
{
...
int ossl_quic_wire_encode_frame_path_response(WPACKET *pkt, uint64_t data)
{
...
int ossl_quic_wire_encode_transport_param_cid(WPACKET *wpkt, uint64_t id,
					      const QUIC_CONN_ID *cid)
{
	if (cid->id_len > QUIC_MAX_CONN_ID_LEN)
		return 0;

	if (ossl_quic_wire_encode_transport_param_bytes(wpkt, id, cid->id,
							cid->id_len) == NULL)
		return 0;
...
static int expect_frame_header_mask(PACKET *pkt, uint64_t expected_frame_type,
				    uint64_t mask_bits,
					     ...
					     uint64_t *total_ranges)
{
	...
	if (!expect_frame_header_mask(
		    &pkt, OSSL_QUIC_FRAME_TYPE_ACK_WITHOUT_ECN, 1, NULL) ||
	    !PACKET_skip_quic_vlint(&pkt) || !PACKET_skip_quic_vlint(&pkt) ||
				    ...
				    uint64_t *total_ranges)
{
	...
	if (ack != NULL) {
		int err = 0;
		ack->delay_time = ossl_time_multiply(
				       ...
				       OSSL_QUIC_FRAME_STREAM *f)
{
	...
	f->has_explicit_len =
		((frame_type & OSSL_QUIC_FRAME_FLAG_STREAM_LEN) != 0);
...
int ossl_quic_wire_decode_frame_max_data(PACKET *pkt, uint64_t *max_data)
{
...
int ossl_quic_wire_decode_frame_max_streams(PACKET *pkt, uint64_t *max_streams)
{
	/* This call matches both MAX_STREAMS_BIDI and MAX_STREAMS_UNI. */
	if (!expect_frame_header_mask(
		    pkt, OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_BIDI, 1, NULL) ||
...
int ossl_quic_wire_decode_frame_data_blocked(PACKET *pkt, uint64_t *max_data)
{
						...
						uint64_t *max_streams)
{
	...
	if (!expect_frame_header_mask(
		    pkt, OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_BIDI, 1, NULL) ||
	    !PACKET_get_quic_vlint(pkt, max_streams))
		return 0;
					    ...
					    OSSL_QUIC_FRAME_NEW_CONN_ID *f)
{
	...
	if (!expect_frame_header(pkt, OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID) ||
	    ...
	    f->seq_num < f->retire_prior_to || !PACKET_get_1(pkt, &len) ||
	    len < 1 || len > QUIC_MAX_CONN_ID_LEN)
		return 0;
...
int ossl_quic_wire_decode_frame_retire_conn_id(PACKET *pkt, uint64_t *seq_num)
{
...
int ossl_quic_wire_decode_frame_path_challenge(PACKET *pkt, uint64_t *data)
{
...
int ossl_quic_wire_decode_frame_path_response(PACKET *pkt, uint64_t *data)
{
					   ...
					   OSSL_QUIC_FRAME_CONN_CLOSE *f)
{
	...
	if (!PACKET_get_quic_vlint(pkt, &reason_len) || reason_len > SIZE_MAX)
		return 0;
					      ...
					      uint64_t *value)
{
	...
	sub.curr = ossl_quic_wire_decode_transport_param_bytes(pkt, id,
							       &sub.remaining);
	if (sub.curr == NULL)
		return 0;
...
int ossl_quic_wire_decode_transport_param_cid(PACKET *pkt, uint64_t *id,
					      QUIC_CONN_ID *cid)
{
							 ...
							 QUIC_PREFERRED_ADDR *p)
{
	...
	body = ossl_quic_wire_decode_transport_param_bytes(pkt, &id, &len);
	if (body == NULL || len < QUIC_MIN_ENCODED_PREFERRED_ADDR_LEN ||
	...
	if (!PACKET_copy_bytes(&pkt2, p->ipv4, sizeof(p->ipv4)) ||
	    ...
	    !PACKET_get_1(&pkt2, &cidl) || cidl > QUIC_MAX_CONN_ID_LEN ||
