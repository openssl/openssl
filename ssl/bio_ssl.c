...
static int ssl_read(BIO *b, char *buf, size_t size, size_t *readbytes)
{
    ...
    case SSL_ERROR_NONE:
        ...
        if ((sb->renegotiate_timeout > 0) && (!r)) {
            ...
            if (OPENSSL_time_t_compare(sb->last_time, sb->renegotiate_timeout, now, 0) < 0) {
                sb->last_time = now;
                sb->num_renegotiates++;
                SSL_renegotiate(ssl);
            }
...
static int ssl_write(BIO *b, const char *buf, size_t size, size_t *written)
{
    ...
    case SSL_ERROR_NONE:
        ...
        if ((bs->renegotiate_timeout > 0) && (!r)) {
            ...
            if (OPENSSL_time_t_compare(bs->last_time, bs->renegotiate_timeout, now, 0) < 0) {
                bs->last_time = now;
                bs->num_renegotiates++;
                SSL_renegotiate(ssl);
            }
