...
CERT *ssl_cert_dup(CERT *cert)
{
	...
	for (i = 0; i < ret->ssl_pkey_num; i++) {
		...
		if (cpk->serverinfo != NULL) {
			...
			rpk->serverinfo = OPENSSL_memdup(
				cpk->serverinfo, cpk->serverinfo_length);
			if (rpk->serverinfo == NULL)
				goto err;
			rpk->serverinfo_length = cpk->serverinfo_length;
		}
#ifndef OPENSSL_NO_COMP_ALG
		for (j = TLSEXT_comp_cert_none; j < TLSEXT_comp_cert_limit;
		     j++) {
	...
	if (cert->conf_sigalgs) {
		ret->conf_sigalgs = OPENSSL_malloc_array(
	...
	if (cert->client_sigalgs) {
		ret->client_sigalgs = OPENSSL_malloc_array(
	...
	if (cert->psk_identity_hint) {
		ret->psk_identity_hint =
			OPENSSL_strdup(cert->psk_identity_hint);
			       ...
			       EVP_PKEY *rpk)
{
	...
	X509_VERIFY_PARAM_set_auth_level(
		param, SSL_get_security_level(SSL_CONNECTION_GET_SSL(s)));
...
STACK_OF(X509_NAME) *SSL_dup_CA_list(const STACK_OF(X509_NAME) *sk)
{
	...
	for (i = 0; i < num; i++) {
		...
		sk_X509_NAME_push(ret,
				  name); /* Cannot fail after reserve call */
	}
	return ret;
}
...
static int add_uris_recursive(STACK_OF(X509_NAME) *stack, const char *uri,
			      int depth)
{
	int ok = 1;
	...
	while (!OSSL_STORE_eof(ctx) && !OSSL_STORE_error(ctx)) {
		...
		if (infotype == OSSL_STORE_INFO_NAME) {
             ...
             */
			if (depth > 0)
				ok = add_uris_recursive(
					stack, OSSL_STORE_INFO_get0_NAME(info),
					depth - 1);
...
int ssl_security(const SSL_CONNECTION *s, int op, int bits, int nid,
		 void *other)
{
	return s->cert->sec_cb(SSL_CONNECTION_GET_USER_SSL(s), NULL, op, bits,
			       nid, other, s->cert->sec_ex);
}
