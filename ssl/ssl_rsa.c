...
int SSL_use_PrivateKey_file(SSL *ssl, const char *file, int type)
{
	...
	if (type == SSL_FILETYPE_PEM) {
		...
		pkey = PEM_read_bio_PrivateKey_ex(
			in, NULL, sc->default_passwd_callback,
			sc->default_passwd_callback_userdata, ssl->ctx->libctx,
			ssl->ctx->propq);
...
int SSL_CTX_use_PrivateKey_file(SSL_CTX *ctx, const char *file, int type)
{
	...
	if (type == SSL_FILETYPE_PEM) {
		...
		pkey = PEM_read_bio_PrivateKey_ex(
			...
			ctx->default_passwd_callback_userdata, ctx->libctx,
			ctx->propq);
	} else if (type == SSL_FILETYPE_ASN1) {
		j = ERR_R_ASN1_LIB;
				   ...
				   X509 *x, size_t chainidx, int *al, void *arg)
{
	...
	if ((ssl_get_server_cert_serverinfo(sc, &serverinfo,
		...
		int retval = serverinfo_find_extension(
			serverinfo, serverinfo_length, ext_type, out, outlen);
		if (retval == -1) {
			*al = SSL_AD_INTERNAL_ERROR;
			return -1; /* Error */
		}
	...
	while (PACKET_remaining(&pkt)) {
		...
		if (version == SSL_SERVERINFOV1 || context == SYNTHV1CONTEXT) {
			if (!SSL_CTX_add_server_custom_ext(
				    ctx, ext_type, serverinfo_srv_add_cb, NULL,
				    NULL, serverinfo_srv_parse_cb, NULL))
				return 0;
		} else {
			if (!SSL_CTX_add_custom_ext(
				    ctx, ext_type, context,
				    serverinfoex_srv_add_cb, NULL, NULL,
				    serverinfoex_srv_parse_cb, NULL))
				return 0;
		}
	}
...
static size_t extension_append_length(unsigned int version,
				      size_t extension_length)
{
			      ...
			      size_t serverinfo_length)
{
	...
	if (version == SSL_SERVERINFOV1) {
         ...
         */
		const size_t sinfo_length = extension_append_length(
			SSL_SERVERINFOV1, serverinfo_length);
		unsigned char *sinfo;
		int ret;
		...
		extension_append(SSL_SERVERINFOV1, serverinfo,
				 serverinfo_length, sinfo);
	...
	new_serverinfo =
		OPENSSL_realloc(ctx->cert->key->serverinfo, serverinfo_length);
...
int SSL_CTX_use_serverinfo_file(SSL_CTX *ctx, const char *file)
{
	...
	for (num_extensions = 0;; num_extensions++) {
		...
		if (PEM_read_bio(bin, &name, &header, &extension,
				 &extension_length) == 0) {
			/*
		...
		append_length =
			extension_append_length(version, extension_length);
		tmp = OPENSSL_realloc(serverinfo,
...
static int ssl_set_cert_and_key(SSL *ssl, SSL_CTX *ctx, X509 *x509,
				EVP_PKEY *privatekey, STACK_OF(X509) *chain,
				int override)
{
	int ret = 0;
	size_t i;
	...
	if (privatekey == NULL) {
		...
		if (EVP_PKEY_missing_parameters(privatekey)) {
			if (EVP_PKEY_missing_parameters(pubkey)) {
				...
				if (!EVP_PKEY_copy_parameters(privatekey,
							      pubkey)) {
					ERR_raise(ERR_LIB_SSL,
	...
	if (!override &&
	    (c->pkeys[i].x509 != NULL || c->pkeys[i].privatekey != NULL ||
...
{
	return ssl_set_cert_and_key(ssl, NULL, x509, privatekey, chain,
				    override);
}
...
int SSL_CTX_use_cert_and_key(SSL_CTX *ctx, X509 *x509, EVP_PKEY *privatekey,
			     STACK_OF(X509) *chain, int override)
{
	return ssl_set_cert_and_key(NULL, ctx, x509, privatekey, chain,
				    override);
}
