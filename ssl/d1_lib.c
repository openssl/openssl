...
const SSL3_ENC_METHOD DTLSv1_enc_data = { tls1_setup_key_block,
...
const SSL3_ENC_METHOD DTLSv1_2_enc_data = { tls1_setup_key_block,
					    tls1_generate_master_secret,
					    tls1_change_cipher_state,
					    tls1_final_finish_mac,
					    TLS_MD_CLIENT_FINISH_CONST,
...
int dtls1_handle_timeout(SSL_CONNECTION *s)
{
	...
	if (s->d1->timer_cb != NULL)
		s->d1->timeout_duration_us =
			s->d1->timer_cb(SSL_CONNECTION_GET_USER_SSL(s),
					s->d1->timeout_duration_us);
	else
...
int DTLSv1_listen(SSL *ssl, BIO_ADDR *client)
{
	...
	if (buf == NULL)
		return -1;
	wbuf = OPENSSL_malloc(DTLS1_RT_HEADER_LENGTH +
	...
	do {
		...
		clear_sys_error();
		n = BIO_read(rbio, buf,
			     SSL3_RT_MAX_PLAIN_LENGTH + DTLS1_RT_HEADER_LENGTH);
		if (n <= 0) {
		...
		if (s->msg_callback)
			s->msg_callback(0, (versmajor << 8) | versminor,
					SSL3_RT_HEADER, buf,
         ...
         */
		if (DTLS_VERSION_LT(clientvers,
				    (unsigned int)ssl->method->version) &&
		...
		if (PACKET_remaining(&cookiepkt) == 0) {
			...
			if (ssl->ctx->app_verify_cookie_cb(
				    ssl, PACKET_data(&cookiepkt),
		...
		if (next == LISTEN_SEND_VERIFY_REQUEST) {
			...
			if (ssl->ctx->app_gen_cookie_cb == NULL ||
			    ssl->ctx->app_gen_cookie_cb(ssl, cookie,
							&cookielen) == 0 ||
			    cookielen > 255) {
				ERR_raise(ERR_LIB_SSL,
             ...
             */
			version = (ssl->method->version == DTLS_ANY_VERSION) ?
					  DTLS1_VERSION :
					  s->version;

			/* Construct the record and message headers */
			if (!WPACKET_init_static_len(
				    &wpkt, wbuf,
			    ...
			    || !WPACKET_put_bytes_u8(
				       &wpkt, DTLS1_MT_HELLO_VERIFY_REQUEST)
			    /*
			    ...
			    || !dtls_raw_hello_verify_request(&wpkt, cookie,
							      cookielen)
			    /* Close message body */
			    || !WPACKET_close(&wpkt)
			    /* Close record body */
			...
			if (BIO_write(wbio, wbuf, (int)wreclen) <
			    (int)wreclen) {
     ...
     */
	if (!ssl_set_new_record_layer(
		    s, DTLS_ANY_VERSION, OSSL_RECORD_DIRECTION_READ,
		    OSSL_RECORD_PROTECTION_LEVEL_NONE, NULL, 0, NULL, 0, NULL,
		    0, NULL, 0, NULL, 0, NID_undef, NULL, NULL, NULL)) {
		/* SSLfatal already called */
		ret = -1;
		goto end;
	}
...
int dtls1_query_mtu(SSL_CONNECTION *s)
{
	...
	if (s->d1->mtu < dtls1_min_mtu(s)) {
		if (!(SSL_get_options(ssl) & SSL_OP_NO_QUERY_MTU)) {
			s->d1->mtu = BIO_ctrl(SSL_get_wbio(ssl),
					      BIO_CTRL_DGRAM_QUERY_MTU, 0,
					      NULL);
