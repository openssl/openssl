...
void ssl_sort_cipher_list(void)
{
	...
	qsort(ssl3_ciphers, SSL3_NUM_CIPHERS, sizeof(ssl3_ciphers[0]),
	      cipher_compare);
	qsort(ssl3_scsvs, SSL3_NUM_SCSVS, sizeof(ssl3_scsvs[0]),
	      cipher_compare);
}
...
long ssl3_ctrl(SSL *s, int cmd, long larg, void *parg)
{
	...
	case SSL_CTRL_SET_TLSEXT_HOSTNAME:
		...
		if (larg == TLSEXT_NAMETYPE_host_name) {
			...
			if ((sc->ext.hostname = OPENSSL_strdup((char *)parg)) ==
			    NULL) {
	...
	case SSL_CTRL_SET_TLSEXT_STATUS_REQ_OCSP_RESP:
		...
		sk_OCSP_RESPONSE_pop_free(sc->ext.ocsp.resp_ex,
					  OCSP_RESPONSE_free);
		sc->ext.ocsp.resp_ex = NULL;

		if (parg != NULL) {
			sc->ext.ocsp.resp_ex =
				sk_OCSP_RESPONSE_new_reserve(NULL, 1);
			if (sc->ext.ocsp.resp_ex == NULL)
				return 0;

			p = parg;
			resp = d2i_OCSP_RESPONSE(
				NULL, (const unsigned char **)&p, larg);
			if (resp != NULL)
				sk_OCSP_RESPONSE_push(sc->ext.ocsp.resp_ex,
						      resp);
		}
#endif
		break;
	...
	case SSL_CTRL_CHAIN:
		if (larg)
			return ssl_cert_set1_chain(sc, NULL,
						   (STACK_OF(X509) *)parg);
		else
			return ssl_cert_set0_chain(sc, NULL,
						   (STACK_OF(X509) *)parg);
	...
	case SSL_CTRL_GET_NEGOTIATED_GROUP: {
		...
		if (SSL_CONNECTION_IS_TLS13(sc) && sc->s3.did_kex)
			...
			id = (sc->session != NULL) ? sc->session->kex_group :
						     NID_undef;
	...
	case SSL_CTRL_GET_IANA_GROUPS: {
		if (parg != NULL) {
			*(uint16_t **)parg =
				(uint16_t *)sc->ext.peer_supportedgroups;
		}
...
long ssl3_callback_ctrl(SSL *s, int cmd, void (*fp)(void))
{
	...
	case SSL_CTRL_SET_TLSEXT_DEBUG_CB:
		sc->ext.debug_cb = (void (*)(
			SSL *, int, int, const unsigned char *, int, void *))fp;
		ret = 1;
		break;
...
long ssl3_ctx_ctrl(SSL_CTX *ctx, int cmd, long larg, void *parg)
{
	...
	case SSL_CTRL_SET_TLS_EXT_SRP_USERNAME:
		...
		if ((ctx->srp_ctx.login = OPENSSL_strdup((char *)parg)) ==
		    NULL) {
			ERR_raise(ERR_LIB_SSL, ERR_R_INTERNAL_ERROR);
			return 0;
		}
	...
	case SSL_CTRL_SET_TLS_EXT_SRP_PASSWORD:
		...
		if ((ctx->srp_ctx.info = OPENSSL_strdup((char *)parg)) ==
		    NULL) {
			ERR_raise(ERR_LIB_SSL, ERR_R_INTERNAL_ERROR);
			return 0;
		}
	...
	case SSL_CTRL_SET_GROUPS:
		return tls1_set_groups(&ctx->ext.supportedgroups,
				       ...
				       &ctx->ext.keyshares,
				       &ctx->ext.keyshares_len,
				       &ctx->ext.tuples, &ctx->ext.tuples_len,
				       parg, larg);
	...
	case SSL_CTRL_GET0_IMPLEMENTED_GROUPS:
		return tls1_get0_implemented_groups(
			ctx->min_proto_version, ctx->max_proto_version,
			ctx->group_list, ctx->group_list_len, larg, parg);
	...
	case SSL_CTRL_CHAIN_CERT:
		if (larg)
			return ssl_cert_add1_chain_cert(NULL, ctx,
							(X509 *)parg);
		else
			return ssl_cert_add0_chain_cert(NULL, ctx,
							(X509 *)parg);
...
const SSL_CIPHER *ssl3_get_cipher_by_std_name(const char *stdname)
{
	...
	size_t i, j,
		tblsize[] = { TLS13_NUM_CIPHERS, SSL3_NUM_CIPHERS,
			      SSL3_NUM_SCSVS };
				     ...
				     STACK_OF(SSL_CIPHER) *srvr)
{
	...
	if (tls1_suiteb(s)) {
		...
		if (s->options & SSL_OP_PRIORITIZE_CHACHA &&
			...
			if (c->algorithm_enc == SSL_CHACHA20POLY1305) {
				...
				if (found) {
					prio_chacha = sk_SSL_CIPHER_new_reserve(
						NULL, num);
					/* if reserve fails, then there's likely a memory issue */
					if (prio_chacha != NULL) {
						/* Put all ChaCha20 at the top, starting with the one we just found */
						sk_SSL_CIPHER_push(prio_chacha,
								   c);
						for (i++; i < num; i++) {
	...
	if (SSL_CONNECTION_IS_TLS13(s)) {
		...
		if (s->psk_server_callback != NULL) {
			for (j = 0;
			     j < s->ssl_pkey_num && !ssl_has_cert(s, (int)j);
			     j++)
				;
			if (j == s->ssl_pkey_num) {
	...
	for (i = 0; i < sk_SSL_CIPHER_num(prio); i++) {
		...
		minversion = SSL_CONNECTION_IS_DTLS(s) ? c->min_dtls :
							 c->min_tls;
		maxversion = SSL_CONNECTION_IS_DTLS(s) ? c->max_dtls :
							 c->max_tls;
		...
		if (!SSL_CONNECTION_IS_TLS13(s)) {
			...
			OSSL_TRACE7(TLS_CIPHER,
				    ...
				    alg_k, alg_a, mask_k, mask_a, (void *)c,
				    c->name);
		...
		if (ii >= 0) {
			...
			if (prefer_sha256) {
				const SSL_CIPHER *tmp =
					sk_SSL_CIPHER_value(allow, ii);
				const EVP_MD *md =
			  ...
			  size_t len, DOWNGRADE dgrd)
{
	...
	if (send_time) {
		...
		l2n(Time, p);
		ret = RAND_bytes_ex(SSL_CONNECTION_GET_CTX(s)->libctx, p,
				    len - 4, 0);
	} else {
		ret = RAND_bytes_ex(SSL_CONNECTION_GET_CTX(s)->libctx, result,
				    len, 0);
	}
	...
	if (ret > 0) {
		...
		if (dgrd == DOWNGRADE_TO_1_2)
			memcpy(result + len - sizeof(tls12downgrade),
			       tls12downgrade, sizeof(tls12downgrade));
		else if (dgrd == DOWNGRADE_TO_1_1)
			memcpy(result + len - sizeof(tls11downgrade),
			       tls11downgrade, sizeof(tls11downgrade));
	}
...
int ssl_gensecret(SSL_CONNECTION *s, unsigned char *pms, size_t pmslen)
{
	...
	if (SSL_CONNECTION_IS_TLS13(s)) {
         ...
         */
		if (!s->hit)
			rv = tls13_generate_secret(
				s, ssl_handshake_md(s), NULL, NULL, 0,
