 ...
 * OSSL_PRIOQ_NODE embedded in the user's struct; the node tracks its current
...
struct ossl_prioq_st {
    OSSL_PRIOQ_NODE **heap;
    int (*cmp)(const OSSL_PRIOQ_NODE *, const OSSL_PRIOQ_NODE *);
...
static const size_t max_nodes = SIZE_MAX / sizeof(OSSL_PRIOQ_NODE *);
...
static ossl_inline void pq_set(OSSL_PRIOQ *pq, size_t i, OSSL_PRIOQ_NODE *n)
{
    pq->heap[i] = n;
    n->posn = i;
}

static ossl_inline void pq_swap(OSSL_PRIOQ *pq, size_t i, size_t j)
{
    OSSL_PRIOQ_NODE *a = pq->heap[i], *b = pq->heap[j];
...
static void pq_sift_up(OSSL_PRIOQ *pq, size_t n)
{
    ASSERT_USED(pq, n);
    while (n > 0) {
        size_t p = (n - 1) / 2;
 ...
 */
static void pq_force_top(OSSL_PRIOQ *pq, size_t n)
{
    ASSERT_USED(pq, n);
    while (n > 0) {
        size_t p = (n - 1) / 2;
...
static void pq_sift_down(OSSL_PRIOQ *pq, size_t n)
{
    size_t p = n * 2 + 1;
...
int ossl_prioq_push(OSSL_PRIOQ *pq, OSSL_PRIOQ_NODE *n)
{
    ...
    if (pq == NULL || n == NULL || n->posn != OSSL_PRIOQ_NOT_IN_QUEUE)
        return 0;

    if (!ossl_prioq_reserve(pq, 1))
        return 0;
...
OSSL_PRIOQ_NODE *ossl_prioq_pop(OSSL_PRIOQ *pq)
{
    ...
    ASSERT_USED(pq, 0);
    res = pq->heap[0];
    res->posn = OSSL_PRIOQ_NOT_IN_QUEUE;
...
OSSL_PRIOQ_NODE *ossl_prioq_remove(OSSL_PRIOQ *pq, OSSL_PRIOQ_NODE *n)
{
    ...
    i = n->posn;
    if (i >= pq->htop || pq->heap[i] != n) {
        assert(0 && "ossl_prioq_remove: node not in this queue");
        return NULL;
    }
