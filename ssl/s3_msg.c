...
int ssl3_dispatch_alert(SSL *s)
{
	...
	if (RECORD_LAYER_write_pending(&sc->rlayer)) {
		...
		i = HANDLE_RLAYER_WRITE_RETURN(
			sc, sc->rlayer.wrlmethod->retry_write_records(
				    sc->rlayer.wrl));
		if (i <= 0) {
	...
	i = HANDLE_RLAYER_WRITE_RETURN(sc, sc->rlayer.wrlmethod->write_records(
						   sc->rlayer.wrl, &templ, 1));
	...
	if (i <= 0) {
		...
		if (sc->msg_callback)
			sc->msg_callback(1, sc->version, SSL3_RT_ALERT,
					 sc->s3.send_alert, 2, s,
					 sc->msg_callback_arg);
