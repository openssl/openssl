...
SSL3_ENC_METHOD const TLSv1_enc_data = { tls1_setup_key_block,
...
SSL3_ENC_METHOD const TLSv1_1_enc_data = { tls1_setup_key_block,
					   tls1_generate_master_secret,
					   tls1_change_cipher_state,
					   tls1_final_finish_mac,
					   TLS_MD_CLIENT_FINISH_CONST,
...
SSL3_ENC_METHOD const TLSv1_2_enc_data = { tls1_setup_key_block,
					   tls1_generate_master_secret,
					   tls1_change_cipher_state,
					   tls1_final_finish_mac,
					   TLS_MD_CLIENT_FINISH_CONST,
...
SSL3_ENC_METHOD const TLSv1_3_enc_data = { tls13_setup_key_block,
...
static int add_provider_groups(const OSSL_PARAM params[], void *data)
{
	...
	if (ctx->group_list_max_len == ctx->group_list_len) {
		...
		if (tmp == NULL)
			return 0;
		ctx->group_list = tmp;
		memset(tmp + ctx->group_list_max_len, 0,
		       sizeof(TLS_GROUP_INFO) *
			       TLS_GROUP_LIST_MALLOC_BLOCK_SIZE);
...
static int add_provider_sigalgs(const OSSL_PARAM params[], void *data)
{
	...
	keytype = (sinf->keytype != NULL ?
			   sinf->keytype :
			   (sinf->sig_name != NULL ? sinf->sig_name :
						     sinf->sigalg_name));
	...
	if (keymgmt != NULL) {
		...
		if (EVP_KEYMGMT_get0_provider(keymgmt) == provider) {
			...
			if ((OBJ_txt2nid(sinf->sigalg_name) == NID_undef) ||
			    (OBJ_nid2obj(OBJ_txt2nid(sinf->sigalg_name)) ==
			     NULL)) {
			...
			if (sinf->keytype != NULL)
				OBJ_create(sinf->keytype_oid, sinf->keytype,
					   NULL);
			if (sinf->hash_name != NULL)
				OBJ_create(sinf->hash_oid, sinf->hash_name,
					   NULL);
...
int ssl_load_sigalgs(SSL_CTX *ctx)
{
	...
	if (ctx->sigalg_list_len > 0) {
		...
		ctx->ssl_cert_info =
			OPENSSL_calloc(ctx->sigalg_list_len, sizeof(lu));
		if (ctx->ssl_cert_info == NULL)
			return 0;
		for (i = 0; i < ctx->sigalg_list_len; i++) {
			ctx->ssl_cert_info[i].nid =
				OBJ_txt2nid(ctx->sigalg_list[i].sigalg_name);
...
const char *tls1_group_id2name(SSL_CTX *ctx, uint16_t group_id)
{
	const TLS_GROUP_INFO *tls_group_info =
		tls1_group_id_lookup(ctx, group_id);
...
int tls_valid_group(SSL_CONNECTION *s, uint16_t group_id, int minversion,
		    int maxversion, int isec, int *okfortls13)
{
	const TLS_GROUP_INFO *ginfo =
		tls1_group_id_lookup(SSL_CONNECTION_GET_CTX(s), group_id);
	int ret;
	...
	group_minversion = SSL_CONNECTION_IS_DTLS(s) ? ginfo->mindtls :
						       ginfo->mintls;
	group_maxversion = SSL_CONNECTION_IS_DTLS(s) ? ginfo->maxdtls :
						       ginfo->maxtls;
	...
	ret &= !isec || strcmp(ginfo->algorithm, "EC") == 0 ||
...
{
	const TLS_GROUP_INFO *ginfo =
		tls1_group_id_lookup(SSL_CONNECTION_GET_CTX(s), group);
	...
	return ssl_security(s, op, ginfo->secbits,
			    tls1_group_id2nid(ginfo->group_id, 0),
			    (void *)gtmp);
}
...
uint16_t tls1_shared_group(SSL_CONNECTION *s, int nmatch)
{
	...
	for (k = 0, i = 0; i < num_pref; i++) {
		...
		minversion = SSL_CONNECTION_IS_DTLS(s) ? inf->mindtls :
							 inf->mintls;
		maxversion = SSL_CONNECTION_IS_DTLS(s) ? inf->maxdtls :
							 inf->maxtls;
		if (maxversion == -1)
			continue;
		if ((minversion != 0 &&
...
int tls1_set_groups(uint16_t **grpext, size_t *grpextlen, uint16_t **ksext,
		    size_t *ksextlen, size_t **tplext, size_t *tplextlen,
 ...
 */
#define GROUPLIST_INCREMENT \
	32 /* Memory allocation chunk size (64 Bytes chunks ~= cache line) */
...
static const char prefixes[] = {
	TUPLE_DELIMITER_CHARACTER,	  GROUP_DELIMITER_CHARACTER,
	IGNORE_UNKNOWN_GROUP_CHARACTER,	  KEY_SHARE_INDICATOR_CHARACTER,
	REMOVE_GROUP_INDICATOR_CHARACTER, '\0'
};
...
static int gid_cb(const char *elem, int len, void *arg)
{
	...
	while (continue_while_loop && len > 0 &&
		...
		OPENSSL_strncasecmp(current_prefix = (char *)
					    DEFAULT_GROUPNAME_FIRST_CHARACTER,
				    elem, 1) == 0)) {
		...
		default:
             ...
             */
			for (i = 0; i < OSSL_NELEM(default_group_strings);
			     i++) {
				if ((size_t)len ==
					    (strlen(default_group_strings[i]
							    .list_name)) &&
				    OPENSSL_strncasecmp(
					    default_group_strings[i].list_name,
					    elem, len) == 0) {
					/*
                     ...
                     */
					restored_default_group_string = OPENSSL_malloc(
						1 /* max prefix length */ +
						strlen(default_group_strings[i]
							       .group_string) +
						1 /* \0 */);
					if (restored_default_group_string ==
					    NULL)
						return 0;
					if (add_keyshare
					    /* Remark: we tolerate a duplicated keyshare indicator here */
					    &&
					    default_group_strings[i]
							    .group_string[0] !=
					...
					memcpy(restored_default_group_string +
						       restored_prefix_index,
					       default_group_strings[i]
						       .group_string,
					       strlen(default_group_strings[i]
	...
	if (garg->ignore_unknown_default ==
	...
	if (garg->gidcnt == garg->gidmax) {
		uint16_t *tmp = OPENSSL_realloc_array(
			garg->gid_arr, garg->gidmax + GROUPLIST_INCREMENT,
			sizeof(*garg->gid_arr));
	...
	if (garg->ksidcnt == garg->ksidmax) {
		uint16_t *tmp = OPENSSL_realloc_array(
			garg->ksid_arr, garg->ksidmax + GROUPLIST_INCREMENT,
	...
	if (gid == 0) {
		...
		for (i = 0; i < OSSL_NELEM(name2id_arr); i++) {
			if (OPENSSL_strcasecmp(
				    etmp, name2id_arr[i].group_name) == 0) {
	...
	if (remove_group) {
		...
		if (found_group) {
			...
			for (j = 0; j < garg->tplcnt; j++) {
				...
				if (k >
				    i) { /* remove from one of the previous tuples */
					garg->tuplcnt_arr[j]--;
...
static int tuple_cb(const char *tuple, int len, void *arg)
{
	...
	if (garg->tplcnt == garg->tplmax) {
		size_t *tmp = OPENSSL_realloc_array(
			garg->tuplcnt_arr, garg->tplmax + GROUPLIST_INCREMENT,
	...
	retval = CONF_parse_list(restored_tuple_string,
				 GROUP_DELIMITER_CHARACTER, 1, gid_cb, arg);

	/* We don't need the \o-terminated string anymore */
	OPENSSL_free(restored_tuple_string);

	if (garg->tuplcnt_arr[garg->tplcnt] >
 ...
 */
int tls1_set_groups_list(SSL_CTX *ctx, uint16_t **grpext, size_t *grpextlen,
			 uint16_t **ksext, size_t *ksextlen, size_t **tplext,
			 size_t *tplextlen, const char *str)
{
	size_t i = 0, j;
	...
	if (gcb.gid_arr == NULL)
		goto end;
	gcb.tuplcnt_arr =
		OPENSSL_malloc_array(gcb.tplmax, sizeof(*gcb.tuplcnt_arr));
     ...
     */
	parse_ret = CONF_parse_list(str, TUPLE_DELIMITER_CHARACTER, 1, tuple_cb,
				    &gcb);
...
static const SIGALG_LOOKUP sigalg_lookup_tbl[] = {
	{ TLSEXT_SIGALG_ecdsa_secp256r1_sha256_name, "ECDSA+SHA256",
	  TLSEXT_SIGALG_ecdsa_secp256r1_sha256, NID_sha256, SSL_MD_SHA256_IDX,
	  EVP_PKEY_EC, SSL_PKEY_ECC, NID_ecdsa_with_SHA256,
	  NID_X9_62_prime256v1, 1, 0, TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0 },
	{ TLSEXT_SIGALG_ecdsa_secp384r1_sha384_name, "ECDSA+SHA384",
	  TLSEXT_SIGALG_ecdsa_secp384r1_sha384, NID_sha384, SSL_MD_SHA384_IDX,
	  EVP_PKEY_EC, SSL_PKEY_ECC, NID_ecdsa_with_SHA384, NID_secp384r1, 1, 0,
	  TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0 },
	{ TLSEXT_SIGALG_ecdsa_secp521r1_sha512_name, "ECDSA+SHA512",
	  TLSEXT_SIGALG_ecdsa_secp521r1_sha512, NID_sha512, SSL_MD_SHA512_IDX,
	  EVP_PKEY_EC, SSL_PKEY_ECC, NID_ecdsa_with_SHA512, NID_secp521r1, 1, 0,
	  TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0 },

	{ TLSEXT_SIGALG_ed25519_name, NULL, TLSEXT_SIGALG_ed25519, NID_undef,
	  -1, EVP_PKEY_ED25519, SSL_PKEY_ED25519, NID_undef, NID_undef, 1, 0,
	  TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0 },
	{ TLSEXT_SIGALG_ed448_name, NULL, TLSEXT_SIGALG_ed448, NID_undef, -1,
	  EVP_PKEY_ED448, SSL_PKEY_ED448, NID_undef, NID_undef, 1, 0,
	  TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0 },

	{ TLSEXT_SIGALG_ecdsa_sha224_name, "ECDSA+SHA224",
	  TLSEXT_SIGALG_ecdsa_sha224, NID_sha224, SSL_MD_SHA224_IDX,
	  EVP_PKEY_EC, SSL_PKEY_ECC, NID_ecdsa_with_SHA224, NID_undef, 1, 0,
	  TLS1_2_VERSION, TLS1_2_VERSION, DTLS1_2_VERSION, DTLS1_2_VERSION },
	{ TLSEXT_SIGALG_ecdsa_sha1_name, "ECDSA+SHA1", TLSEXT_SIGALG_ecdsa_sha1,
	  NID_sha1, SSL_MD_SHA1_IDX, EVP_PKEY_EC, SSL_PKEY_ECC,
	  NID_ecdsa_with_SHA1, NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION,
	  DTLS1_2_VERSION, DTLS1_2_VERSION },

	{ TLSEXT_SIGALG_ecdsa_brainpoolP256r1_sha256_name,
	  TLSEXT_SIGALG_ecdsa_brainpoolP256r1_sha256_alias,
	  TLSEXT_SIGALG_ecdsa_brainpoolP256r1_sha256, NID_sha256,
	  SSL_MD_SHA256_IDX, EVP_PKEY_EC, SSL_PKEY_ECC, NID_ecdsa_with_SHA256,
	  NID_brainpoolP256r1, 1, 0, TLS1_3_VERSION, 0, -1, -1 },
	{ TLSEXT_SIGALG_ecdsa_brainpoolP384r1_sha384_name,
	  TLSEXT_SIGALG_ecdsa_brainpoolP384r1_sha384_alias,
	  TLSEXT_SIGALG_ecdsa_brainpoolP384r1_sha384, NID_sha384,
	  SSL_MD_SHA384_IDX, EVP_PKEY_EC, SSL_PKEY_ECC, NID_ecdsa_with_SHA384,
	  NID_brainpoolP384r1, 1, 0, TLS1_3_VERSION, 0, -1, -1 },
	{ TLSEXT_SIGALG_ecdsa_brainpoolP512r1_sha512_name,
	  TLSEXT_SIGALG_ecdsa_brainpoolP512r1_sha512_alias,
	  TLSEXT_SIGALG_ecdsa_brainpoolP512r1_sha512, NID_sha512,
	  SSL_MD_SHA512_IDX, EVP_PKEY_EC, SSL_PKEY_ECC, NID_ecdsa_with_SHA512,
	  NID_brainpoolP512r1, 1, 0, TLS1_3_VERSION, 0, -1, -1 },

	{ TLSEXT_SIGALG_rsa_pss_rsae_sha256_name, "PSS+SHA256",
	  TLSEXT_SIGALG_rsa_pss_rsae_sha256, NID_sha256, SSL_MD_SHA256_IDX,
	  EVP_PKEY_RSA_PSS, SSL_PKEY_RSA, NID_undef, NID_undef, 1, 0,
	  TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0 },
	{ TLSEXT_SIGALG_rsa_pss_rsae_sha384_name, "PSS+SHA384",
	  TLSEXT_SIGALG_rsa_pss_rsae_sha384, NID_sha384, SSL_MD_SHA384_IDX,
	  EVP_PKEY_RSA_PSS, SSL_PKEY_RSA, NID_undef, NID_undef, 1, 0,
	  TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0 },
	{ TLSEXT_SIGALG_rsa_pss_rsae_sha512_name, "PSS+SHA512",
	  TLSEXT_SIGALG_rsa_pss_rsae_sha512, NID_sha512, SSL_MD_SHA512_IDX,
	  EVP_PKEY_RSA_PSS, SSL_PKEY_RSA, NID_undef, NID_undef, 1, 0,
	  TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0 },

	{ TLSEXT_SIGALG_rsa_pss_pss_sha256_name, NULL,
	  TLSEXT_SIGALG_rsa_pss_pss_sha256, NID_sha256, SSL_MD_SHA256_IDX,
	  EVP_PKEY_RSA_PSS, SSL_PKEY_RSA_PSS_SIGN, NID_undef, NID_undef, 1, 0,
	  TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0 },
	{ TLSEXT_SIGALG_rsa_pss_pss_sha384_name, NULL,
	  TLSEXT_SIGALG_rsa_pss_pss_sha384, NID_sha384, SSL_MD_SHA384_IDX,
	  EVP_PKEY_RSA_PSS, SSL_PKEY_RSA_PSS_SIGN, NID_undef, NID_undef, 1, 0,
	  TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0 },
	{ TLSEXT_SIGALG_rsa_pss_pss_sha512_name, NULL,
	  TLSEXT_SIGALG_rsa_pss_pss_sha512, NID_sha512, SSL_MD_SHA512_IDX,
	  EVP_PKEY_RSA_PSS, SSL_PKEY_RSA_PSS_SIGN, NID_undef, NID_undef, 1, 0,
	  TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0 },

	{ TLSEXT_SIGALG_rsa_pkcs1_sha256_name, "RSA+SHA256",
	  TLSEXT_SIGALG_rsa_pkcs1_sha256, NID_sha256, SSL_MD_SHA256_IDX,
	  EVP_PKEY_RSA, SSL_PKEY_RSA, NID_sha256WithRSAEncryption, NID_undef, 1,
	  0, TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0 },
	{ TLSEXT_SIGALG_rsa_pkcs1_sha384_name, "RSA+SHA384",
	  TLSEXT_SIGALG_rsa_pkcs1_sha384, NID_sha384, SSL_MD_SHA384_IDX,
	  EVP_PKEY_RSA, SSL_PKEY_RSA, NID_sha384WithRSAEncryption, NID_undef, 1,
	  0, TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0 },
	{ TLSEXT_SIGALG_rsa_pkcs1_sha512_name, "RSA+SHA512",
	  TLSEXT_SIGALG_rsa_pkcs1_sha512, NID_sha512, SSL_MD_SHA512_IDX,
	  EVP_PKEY_RSA, SSL_PKEY_RSA, NID_sha512WithRSAEncryption, NID_undef, 1,
	  0, TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0 },

	{ TLSEXT_SIGALG_rsa_pkcs1_sha224_name, "RSA+SHA224",
	  TLSEXT_SIGALG_rsa_pkcs1_sha224, NID_sha224, SSL_MD_SHA224_IDX,
	  EVP_PKEY_RSA, SSL_PKEY_RSA, NID_sha224WithRSAEncryption, NID_undef, 1,
	  0, TLS1_2_VERSION, TLS1_2_VERSION, DTLS1_2_VERSION, DTLS1_2_VERSION },
	{ TLSEXT_SIGALG_rsa_pkcs1_sha1_name, "RSA+SHA1",
	  TLSEXT_SIGALG_rsa_pkcs1_sha1, NID_sha1, SSL_MD_SHA1_IDX, EVP_PKEY_RSA,
	  SSL_PKEY_RSA, NID_sha1WithRSAEncryption, NID_undef, 1, 0,
	  TLS1_2_VERSION, TLS1_2_VERSION, DTLS1_2_VERSION, DTLS1_2_VERSION },

	{ TLSEXT_SIGALG_dsa_sha256_name, "DSA+SHA256", TLSEXT_SIGALG_dsa_sha256,
	  NID_sha256, SSL_MD_SHA256_IDX, EVP_PKEY_DSA, SSL_PKEY_DSA_SIGN,
	  NID_dsa_with_SHA256, NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION,
	  DTLS1_2_VERSION, DTLS1_2_VERSION },
	{ TLSEXT_SIGALG_dsa_sha384_name, "DSA+SHA384", TLSEXT_SIGALG_dsa_sha384,
	  NID_sha384, SSL_MD_SHA384_IDX, EVP_PKEY_DSA, SSL_PKEY_DSA_SIGN,
	  NID_undef, NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION,
	  DTLS1_2_VERSION, DTLS1_2_VERSION },
	{ TLSEXT_SIGALG_dsa_sha512_name, "DSA+SHA512", TLSEXT_SIGALG_dsa_sha512,
	  NID_sha512, SSL_MD_SHA512_IDX, EVP_PKEY_DSA, SSL_PKEY_DSA_SIGN,
	  NID_undef, NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION,
	  DTLS1_2_VERSION, DTLS1_2_VERSION },
	{ TLSEXT_SIGALG_dsa_sha224_name, "DSA+SHA224", TLSEXT_SIGALG_dsa_sha224,
	  NID_sha224, SSL_MD_SHA224_IDX, EVP_PKEY_DSA, SSL_PKEY_DSA_SIGN,
	  NID_undef, NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION,
	  DTLS1_2_VERSION, DTLS1_2_VERSION },
	{ TLSEXT_SIGALG_dsa_sha1_name, "DSA+SHA1", TLSEXT_SIGALG_dsa_sha1,
	  NID_sha1, SSL_MD_SHA1_IDX, EVP_PKEY_DSA, SSL_PKEY_DSA_SIGN,
	  NID_dsaWithSHA1, NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION,
	  DTLS1_2_VERSION, DTLS1_2_VERSION },

#ifndef OPENSSL_NO_GOST
	{ TLSEXT_SIGALG_gostr34102012_256_intrinsic_alias, /* RFC9189 */
	  TLSEXT_SIGALG_gostr34102012_256_intrinsic_name,
	  TLSEXT_SIGALG_gostr34102012_256_intrinsic, NID_id_GostR3411_2012_256,
	  SSL_MD_GOST12_256_IDX, NID_id_GostR3410_2012_256, SSL_PKEY_GOST12_256,
	  NID_undef, NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION,
	  DTLS1_2_VERSION, DTLS1_2_VERSION },
	{ TLSEXT_SIGALG_gostr34102012_256_intrinsic_alias, /* RFC9189 */
	  TLSEXT_SIGALG_gostr34102012_256_intrinsic_name,
	  TLSEXT_SIGALG_gostr34102012_512_intrinsic, NID_id_GostR3411_2012_512,
	  SSL_MD_GOST12_512_IDX, NID_id_GostR3410_2012_512, SSL_PKEY_GOST12_512,
	  NID_undef, NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION,
	  DTLS1_2_VERSION, DTLS1_2_VERSION },

	{ TLSEXT_SIGALG_gostr34102012_256_gostr34112012_256_name, NULL,
	  TLSEXT_SIGALG_gostr34102012_256_gostr34112012_256,
	  NID_id_GostR3411_2012_256, SSL_MD_GOST12_256_IDX,
	  NID_id_GostR3410_2012_256, SSL_PKEY_GOST12_256, NID_undef, NID_undef,
	  1, 0, TLS1_2_VERSION, TLS1_2_VERSION, DTLS1_2_VERSION,
	  DTLS1_2_VERSION },
	{ TLSEXT_SIGALG_gostr34102012_512_gostr34112012_512_name, NULL,
	  TLSEXT_SIGALG_gostr34102012_512_gostr34112012_512,
	  NID_id_GostR3411_2012_512, SSL_MD_GOST12_512_IDX,
	  NID_id_GostR3410_2012_512, SSL_PKEY_GOST12_512, NID_undef, NID_undef,
	  1, 0, TLS1_2_VERSION, TLS1_2_VERSION, DTLS1_2_VERSION,
	  DTLS1_2_VERSION },
	{ TLSEXT_SIGALG_gostr34102001_gostr3411_name, NULL,
	  TLSEXT_SIGALG_gostr34102001_gostr3411, NID_id_GostR3411_94,
	  SSL_MD_GOST94_IDX, NID_id_GostR3410_2001, SSL_PKEY_GOST01, NID_undef,
	  NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION, DTLS1_2_VERSION,
	  DTLS1_2_VERSION },
#endif
};
/* Legacy sigalgs for TLS < 1.2 RSA TLS signatures */
static const SIGALG_LOOKUP legacy_rsa_sigalg = { "rsa_pkcs1_md5_sha1",
						 NULL,
						 0,
						 NID_md5_sha1,
						 SSL_MD_MD5_SHA1_IDX,
...
int ssl_setup_sigalgs(SSL_CTX *ctx)
{
	...
	for (i = 0; i < ctx->sigalg_list_len; i++) {
		...
		cache[cache_idx].hash =
			si.hash_name ? OBJ_txt2nid(si.hash_name) : NID_undef;
		cache[cache_idx].hash_idx =
			ssl_get_md_idx(cache[cache_idx].hash);
...
char *SSL_get1_builtin_sigalgs(OSSL_LIB_CTX *libctx)
{
	...
	for (i = 0, lu = sigalg_lookup_tbl; i < OSSL_NELEM(sigalg_lookup_tbl);
		...
		if (lu->hash != NID_undef) {
			EVP_MD *hash = EVP_MD_fetch(libctx,
						    OBJ_nid2ln(lu->hash), NULL);
		...
		if (enabled) {
			...
			if (sa != NULL) {
				if (strlen(sa) + strlen(retval) + 1 >=
				    maxretlen) {
					char *tmp;

					maxretlen += SIGLEN_BUF_INCREMENT;
					tmp = OPENSSL_realloc(retval,
							      maxretlen);
					if (tmp == NULL) {
						OPENSSL_free(retval);
						return NULL;
						   ...
						   int idx)
{
	if (idx == -1) {
		if (s->server) {
             ...
             * */
			if (idx == SSL_PKEY_GOST01 &&
			    s->s3.tmp.new_cipher->algorithm_auth !=
				    SSL_aGOST01) {
				int real_idx;

				for (real_idx = SSL_PKEY_GOST12_512;
				     real_idx >= SSL_PKEY_GOST01; real_idx--) {
					if (s->cert->pkeys[real_idx]
						    .privatekey != NULL) {
						idx = real_idx;
						break;
					}
	...
	if (SSL_USE_SIGALGS(s) || idx != SSL_PKEY_RSA) {
		const SIGALG_LOOKUP *lu = tls1_lookup_sigalg(
			SSL_CONNECTION_GET_CTX(s), tls_default_sigalg[idx]);
	...
	if (!tls12_sigalg_allowed(s, SSL_SECOP_SIGALG_SUPPORTED,
				  &legacy_rsa_sigalg))
		return NULL;
...
int tls1_set_peer_legacy_sigalg(SSL_CONNECTION *s, const EVP_PKEY *pkey)
{
	...
	if (ssl_cert_lookup_by_pkey(pkey, &idx, SSL_CONNECTION_GET_CTX(s)) ==
	    NULL)
		return 0;
...
int tls_check_sigalg_curve(const SSL_CONNECTION *s, int curve)
{
	...
	for (i = 0; i < siglen; i++) {
		...
		if (lu == NULL)
			continue;
		if (lu->sig == EVP_PKEY_EC && lu->curve != NID_undef &&
		    curve == lu->curve)
			return 1;
	}
...
int tls12_check_peer_sigalg(SSL_CONNECTION *s, uint16_t sig, EVP_PKEY *pkey)
{
	...
	if (!ssl_cert_lookup_by_nid((pkeyid == EVP_PKEY_RSA_PSS) ?
...
int ssl_set_client_disabled(SSL_CONNECTION *s)
{
	...
	if (ssl_get_min_max_version(s, &s->s3.tmp.min_ver, &s->s3.tmp.max_ver,
				    NULL) != 0)
		return 0;
 ...
 */
int ssl_cipher_disabled(const SSL_CONNECTION *s, const SSL_CIPHER *c, int op,
			int ecdhe)
{
     ...
     */
	if (minversion == TLS1_VERSION && ecdhe &&
...
int tls1_set_server_sigalgs(SSL_CONNECTION *s)
{
	...
	if (s->s3.tmp.valid_flags)
		memset(s->s3.tmp.valid_flags, 0,
		       s->ssl_pkey_num * sizeof(uint32_t));
	else
		s->s3.tmp.valid_flags =
			OPENSSL_calloc(s->ssl_pkey_num, sizeof(uint32_t));
	...
	if (s->s3.tmp.peer_cert_sigalgs == NULL &&
		...
		for (i = 0; i < s->ssl_pkey_num; i++) {
			const SIGALG_LOOKUP *lu =
				tls1_get_legacy_sigalg(s, (int)i);
			size_t j;
				     ...
				     size_t sesslen, SSL_SESSION **psess)
{
...
#endif
	{
		...
		if (tctx->ext.ticket_key_evp_cb != NULL)
			rv = tctx->ext.ticket_key_evp_cb(
				SSL_CONNECTION_GET_USER_SSL(s), nctick,
				nctick + TLSEXT_KEYNAME_LENGTH, ctx,
				ssl_hmac_get0_EVP_MAC_CTX(hctx), 0);
#ifndef OPENSSL_NO_DEPRECATED_3_0
		else if (tctx->ext.ticket_key_cb != NULL)
			/* if 0 is returned, write an empty ticket */
			rv = tctx->ext.ticket_key_cb(
				SSL_CONNECTION_GET_USER_SSL(s), nctick,
				nctick + TLSEXT_KEYNAME_LENGTH, ctx,
				ssl_hmac_get0_HMAC_CTX(hctx), 0);
#endif
		if (rv < 0) {
	...
	if (s->session_ctx->decrypt_ticket_cb != NULL &&
		...
		retcb = s->session_ctx->decrypt_ticket_cb(
			SSL_CONNECTION_GET_SSL(s), sess, etick, keyname_len,
			ret, s->session_ctx->ticket_cb_data);
		switch (retcb) {
				...
				const SIGALG_LOOKUP *lu)
{
	...
	if (lu->sig == NID_id_GostR3410_2012_256 ||
		...
		if (s->server && SSL_CONNECTION_IS_TLS13(s))
			return 0;
		if (!s->server &&
		    SSL_CONNECTION_GET_SSL(s)->method->version ==
			    TLS_ANY_VERSION &&
			...
			for (i = 0; i < num; i++) {
				...
				if (ssl_cipher_disabled(
					    s, c, SSL_SECOP_CIPHER_SUPPORTED,
					    0))
					continue;

				if ((c->algorithm_mkey &
				     (SSL_kGOST | SSL_kGOST18)) != 0)
					break;
			}
			if (i == num)
				return 0;
...
int tls12_copy_sigalgs(SSL_CONNECTION *s, WPACKET *pkt, const uint16_t *psig,
		       size_t psiglen)
{
	size_t i;
	int rv = 0;
...
static int tls1_set_shared_sigalgs(SSL_CONNECTION *s)
{
	...
	if (nmatch) {
		if ((salgs = OPENSSL_malloc_array(nmatch, sizeof(*salgs))) ==
		    NULL)
			return 0;
		nmatch = tls12_shared_sigalgs(s, salgs, pref, preflen, allow,
					      allowlen);
	} else {
		salgs = NULL;
	}
...
int SSL_get_shared_sigalgs(SSL *s, int idx, int *psign, int *phash,
			   int *psignhash, unsigned char *rsig,
			   unsigned char *rhash)
{
	...
	if (sc->shared_sigalgs == NULL || idx < 0 ||
	...
	if (p == NULL) {
		...
		*p = 0;
		...
		if (sig_alg != NID_undef && hash_alg != NID_undef) {
			if (sarg->ctx != NULL) {
				for (i = 0;
				     i < sarg->ctx->sigalg_lookup_cache_len;
				     i++) {
				...
				for (i = 0; i < OSSL_NELEM(sigalg_lookup_tbl);
				     i++) {
...
int tls1_set_sigalgs(CERT *c, const int *psig_nids, size_t salglen, int client)
{
	...
	if (salglen & 1)
		return 0;
	if ((sigalgs = OPENSSL_malloc_array(salglen / 2, sizeof(*sigalgs))) ==
	    NULL)
		return 0;
	for (i = 0, sptr = sigalgs; i < salglen; i += 2) {
		...
		for (j = 0, curr = sigalg_lookup_tbl;
		     j < OSSL_NELEM(sigalg_lookup_tbl); j++, curr++) {
	...
	if (TLS1_get_version(SSL_CONNECTION_GET_SSL(s)) >= TLS1_2_VERSION &&
		...
		if (default_nid > 0 && c->conf_sigalgs) {
			...
			for (j = 0; j < c->conf_sigalgslen; j++, p++) {
				const SIGALG_LOOKUP *lu = tls1_lookup_sigalg(
					SSL_CONNECTION_GET_CTX(s), *p);

				if (lu != NULL && lu->hash == NID_sha1 &&
				    lu->sig == rsign)
					break;
			}
		...
		for (i = 0; i < sk_X509_num(chain); i++) {
			if (!tls1_check_sig_alg(s, sk_X509_value(chain, i),
						default_nid)) {
				if (check_flags) {
					rv &= ~CERT_PKEY_CA_SIGNATURE;
					break;
...
skip_sigs:
	...
	if (!s->server && strict_mode) {
		...
		if (ca_dn == NULL || sk_X509_NAME_num(ca_dn) == 0 ||
	...
	if (s->cert->dh_tmp_auto != 2) {
		if (s->s3.tmp.new_cipher->algorithm_auth &
			...
			if (s->s3.tmp.cert == NULL)
				return NULL;
			dh_secbits = EVP_PKEY_get_security_bits(
...
int ssl_security_cert_chain(SSL_CONNECTION *s, STACK_OF(X509) *sk, X509 *x,
			    int vfy)
{
	int rv, start_idx, i;
				     ...
				     const SIGALG_LOOKUP *lu)
{
	...
	const SSL_CERT_LOOKUP *clu =
		ssl_cert_lookup_by_idx(sig_idx, SSL_CONNECTION_GET_CTX(s));
	...
	if (tls12_rpk_and_privkey(s, sig_idx))
		return s->s3.tmp.valid_flags[sig_idx] & CERT_PKEY_RPK ?
			       sig_idx :
			       -1;
...
int tls_choose_sigalg(SSL_CONNECTION *s, int fatalerrs)
{
	...
	if (SSL_CONNECTION_IS_TLS13(s)) {
		...
		if (!(s->s3.tmp.new_cipher->algorithm_auth & SSL_aCERT))
			return 1;
		if (!s->server &&
		    !ssl_has_cert(s, (int)(s->cert->key - s->cert->pkeys)))
			return 1;
			...
			if (s->s3.tmp.peer_sigalgs != NULL) {
				...
				for (i = 0; i < s->shared_sigalgslen; i++) {
					...
					if (lu->sig == EVP_PKEY_RSA_PSS) {
						/* validate that key is large enough for the signature algorithm */
						EVP_PKEY *pkey =
							s->cert->pkeys[sig_idx]
								.privatekey;

						if (!rsa_pss_check_min_key_size(
							    sctx, pkey, lu))
							continue;
					}
				...
				if ((lu = tls1_get_legacy_sigalg(s, -1)) ==
				    NULL) {
					if (!fatalerrs)
						return 1;
					SSLfatal(
...
{
	if (session->ext.max_fragment_len_mode ==
