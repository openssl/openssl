...
static const SIGALG_LOOKUP *find_sig_alg(tls *s, X509 *x, EVP_PKEY *pkey);

tls3_ENC_METHOD const TLSv1_enc_data = {
    tls1_enc,
    tls1_mac,
...
tls3_ENC_METHOD const TLSv1_1_enc_data = {
    tls1_enc,
    tls1_mac,
    tls1_setup_key_block,
    tls1_generate_master_secret,
    ...
    tls1_alert_code,
    tls1_export_keying_material,
    tls_ENC_FLAG_EXPLICIT_IV,
    tls3_set_handshake_header,
    tls_close_construct_packet,
    tls3_handshake_write
};

tls3_ENC_METHOD const TLSv1_2_enc_data = {
    tls1_enc,
    tls1_mac,
    tls1_setup_key_block,
    tls1_generate_master_secret,
    ...
    tls1_alert_code,
    tls1_export_keying_material,
    tls_ENC_FLAG_EXPLICIT_IV | tls_ENC_FLAG_SIGALGS | tls_ENC_FLAG_SHA256_PRF
        | tls_ENC_FLAG_TLS1_2_CIPHERS,
    tls3_set_handshake_header,
    tls_close_construct_packet,
    tls3_handshake_write
};

tls3_ENC_METHOD const TLSv1_3_enc_data = {
    tls13_enc,
    tls1_mac,
    ...
    tls_ENC_FLAG_SIGALGS | tls_ENC_FLAG_SHA256_PRF,
    tls3_set_handshake_header,
    tls_close_construct_packet,
    tls3_handshake_write
};
...
void tls1_free(tls *s)
{
    OPENtls_free(s->ext.session_ticket);
    tls3_free(s);
}

int tls1_clear(tls *s)
{
    if (!tls3_clear(s))
        return 0;
...
const TLS_GROUP_INFO *tls1_group_id_lookup(uint16_t group_id)
{
    ...
    for (i = 0; i < Otls_NELEM(nid_list); i++) {
        if (nid_list[i].group_id == group_id)
            return &nid_list[i];
    }
#endif /* !defined(OPENtls_NO_DH) || !defined(OPENtls_NO_EC) */
    return NULL;
}
...
# ifndef OPENtls_NO_EC
    case tls_CERT_FLAG_SUITEB_128_LOS:
        ...
        *pgroupslen = Otls_NELEM(suiteb_curves);
        break;
...
# endif
    ...
    default:
        if (s->ext.supportedgroups == NULL) {
            ...
            *pgroupslen = Otls_NELEM(supported_groups_default);
        } else {
...
int tls_valid_group(tls *s, uint16_t group_id, int version)
{
...
int tls_group_allowed(tls *s, uint16_t group, int op)
{
    ...
    gtmp[0] = group >> 8;
    gtmp[1] = group & 0xff;
    return tls_security(s, op, ginfo->secbits, ginfo->nid, (void *)gtmp);
}
 ...
 */
uint16_t tls1_shared_group(tls *s, int nmatch)
{
     ...
     */
    if (s->options & tls_OP_CIPHER_SERVER_PREFERENCE) {
    ...
    for (k = 0, i = 0; i < num_pref; i++) {
        ...
        if (!tls1_in_list(id, supp, num_supp)
            || !tls_group_allowed(s, id, tls_SECOP_CURVE_SHARED))
                    continue;
        if (nmatch == k)
            return id;
         k++;
                    ...
                    int *groups, size_t ngroups)
{
    ...
    if (ngroups == 0) {
        tlserr(tls_F_TLS1_SET_GROUPS, tls_R_BAD_LENGTH);
        return 0;
    }
    if ((glist = OPENtls_malloc(ngroups * sizeof(*glist))) == NULL) {
        tlserr(tls_F_TLS1_SET_GROUPS, ERR_R_MALLOC_FAILURE);
        return 0;
    }
    for (i = 0; i < ngroups; i++) {
...
#if !defined(OPENtls_NO_EC) || !defined(OPENtls_NO_DH)
# define MAX_GROUPLIST   Otls_NELEM(nid_list)
...
int tls1_check_group_id(tls *s, uint16_t group_id, int check_own_groups)
    ...
    if (!tls_group_allowed(s, group_id, tls_SECOP_CURVE_CHECK))
        return 0;
...
static int tls1_check_pkey_comp(tls *s, EVP_PKEY *pkey)
{
    const EC_KEY *ec;
    const EC_GROUP *grp;
 ...
 */
static int tls1_check_cert_param(tls *s, X509 *x, int check_ee_md)
{
    uint16_t group_id;
    EVP_PKEY *pkey;
 ...
 */
int tls1_check_ec_tmp_key(tls *s, unsigned long cid)
{
...
static int tls1_check_cert_param(tls *s, X509 *x, int set_ee_md)
{
    return 1;
}
...
static const SIGALG_LOOKUP sigalg_lookup_tbl[] = {
    ...
    {"ecdsa_secp256r1_sha256", TLSEXT_SIGALG_ecdsa_secp256r1_sha256,
     NID_sha256, tls_MD_SHA256_IDX, EVP_PKEY_EC, tls_PKEY_ECC,
     NID_ecdsa_with_SHA256, NID_X9_62_prime256v1},
    {"ecdsa_secp384r1_sha384", TLSEXT_SIGALG_ecdsa_secp384r1_sha384,
     NID_sha384, tls_MD_SHA384_IDX, EVP_PKEY_EC, tls_PKEY_ECC,
     NID_ecdsa_with_SHA384, NID_secp384r1},
    {"ecdsa_secp521r1_sha512", TLSEXT_SIGALG_ecdsa_secp521r1_sha512,
     NID_sha512, tls_MD_SHA512_IDX, EVP_PKEY_EC, tls_PKEY_ECC,
     NID_ecdsa_with_SHA512, NID_secp521r1},
    {"ed25519", TLSEXT_SIGALG_ed25519,
     NID_undef, -1, EVP_PKEY_ED25519, tls_PKEY_ED25519,
     NID_undef, NID_undef},
    {"ed448", TLSEXT_SIGALG_ed448,
     NID_undef, -1, EVP_PKEY_ED448, tls_PKEY_ED448,
     NID_undef, NID_undef},
    {NULL, TLSEXT_SIGALG_ecdsa_sha224,
     NID_sha224, tls_MD_SHA224_IDX, EVP_PKEY_EC, tls_PKEY_ECC,
     NID_ecdsa_with_SHA224, NID_undef},
    {NULL, TLSEXT_SIGALG_ecdsa_sha1,
     NID_sha1, tls_MD_SHA1_IDX, EVP_PKEY_EC, tls_PKEY_ECC,
     NID_ecdsa_with_SHA1, NID_undef},
#endif
    {"rsa_pss_rsae_sha256", TLSEXT_SIGALG_rsa_pss_rsae_sha256,
     NID_sha256, tls_MD_SHA256_IDX, EVP_PKEY_RSA_PSS, tls_PKEY_RSA,
     NID_undef, NID_undef},
    {"rsa_pss_rsae_sha384", TLSEXT_SIGALG_rsa_pss_rsae_sha384,
     NID_sha384, tls_MD_SHA384_IDX, EVP_PKEY_RSA_PSS, tls_PKEY_RSA,
     NID_undef, NID_undef},
    {"rsa_pss_rsae_sha512", TLSEXT_SIGALG_rsa_pss_rsae_sha512,
     NID_sha512, tls_MD_SHA512_IDX, EVP_PKEY_RSA_PSS, tls_PKEY_RSA,
     NID_undef, NID_undef},
    {"rsa_pss_pss_sha256", TLSEXT_SIGALG_rsa_pss_pss_sha256,
     NID_sha256, tls_MD_SHA256_IDX, EVP_PKEY_RSA_PSS, tls_PKEY_RSA_PSS_SIGN,
     NID_undef, NID_undef},
    {"rsa_pss_pss_sha384", TLSEXT_SIGALG_rsa_pss_pss_sha384,
     NID_sha384, tls_MD_SHA384_IDX, EVP_PKEY_RSA_PSS, tls_PKEY_RSA_PSS_SIGN,
     NID_undef, NID_undef},
    {"rsa_pss_pss_sha512", TLSEXT_SIGALG_rsa_pss_pss_sha512,
     NID_sha512, tls_MD_SHA512_IDX, EVP_PKEY_RSA_PSS, tls_PKEY_RSA_PSS_SIGN,
     NID_undef, NID_undef},
    {"rsa_pkcs1_sha256", TLSEXT_SIGALG_rsa_pkcs1_sha256,
     NID_sha256, tls_MD_SHA256_IDX, EVP_PKEY_RSA, tls_PKEY_RSA,
     NID_sha256WithRSAEncryption, NID_undef},
    {"rsa_pkcs1_sha384", TLSEXT_SIGALG_rsa_pkcs1_sha384,
     NID_sha384, tls_MD_SHA384_IDX, EVP_PKEY_RSA, tls_PKEY_RSA,
     NID_sha384WithRSAEncryption, NID_undef},
    {"rsa_pkcs1_sha512", TLSEXT_SIGALG_rsa_pkcs1_sha512,
     NID_sha512, tls_MD_SHA512_IDX, EVP_PKEY_RSA, tls_PKEY_RSA,
     NID_sha512WithRSAEncryption, NID_undef},
    {"rsa_pkcs1_sha224", TLSEXT_SIGALG_rsa_pkcs1_sha224,
     NID_sha224, tls_MD_SHA224_IDX, EVP_PKEY_RSA, tls_PKEY_RSA,
     NID_sha224WithRSAEncryption, NID_undef},
    {"rsa_pkcs1_sha1", TLSEXT_SIGALG_rsa_pkcs1_sha1,
     NID_sha1, tls_MD_SHA1_IDX, EVP_PKEY_RSA, tls_PKEY_RSA,
     NID_sha1WithRSAEncryption, NID_undef},
#ifndef OPENtls_NO_DSA
    {NULL, TLSEXT_SIGALG_dsa_sha256,
     NID_sha256, tls_MD_SHA256_IDX, EVP_PKEY_DSA, tls_PKEY_DSA_SIGN,
     NID_dsa_with_SHA256, NID_undef},
    {NULL, TLSEXT_SIGALG_dsa_sha384,
     NID_sha384, tls_MD_SHA384_IDX, EVP_PKEY_DSA, tls_PKEY_DSA_SIGN,
     NID_undef, NID_undef},
    {NULL, TLSEXT_SIGALG_dsa_sha512,
     NID_sha512, tls_MD_SHA512_IDX, EVP_PKEY_DSA, tls_PKEY_DSA_SIGN,
     NID_undef, NID_undef},
    {NULL, TLSEXT_SIGALG_dsa_sha224,
     NID_sha224, tls_MD_SHA224_IDX, EVP_PKEY_DSA, tls_PKEY_DSA_SIGN,
     NID_undef, NID_undef},
    {NULL, TLSEXT_SIGALG_dsa_sha1,
     NID_sha1, tls_MD_SHA1_IDX, EVP_PKEY_DSA, tls_PKEY_DSA_SIGN,
     NID_dsaWithSHA1, NID_undef},
#endif
#ifndef OPENtls_NO_GOST
    {NULL, TLSEXT_SIGALG_gostr34102012_256_gostr34112012_256,
     NID_id_GostR3411_2012_256, tls_MD_GOST12_256_IDX,
     NID_id_GostR3410_2012_256, tls_PKEY_GOST12_256,
     NID_undef, NID_undef},
    {NULL, TLSEXT_SIGALG_gostr34102012_512_gostr34112012_512,
     NID_id_GostR3411_2012_512, tls_MD_GOST12_512_IDX,
     NID_id_GostR3410_2012_512, tls_PKEY_GOST12_512,
     NID_undef, NID_undef},
    {NULL, TLSEXT_SIGALG_gostr34102001_gostr3411,
     NID_id_GostR3411_94, tls_MD_GOST94_IDX,
     NID_id_GostR3410_2001, tls_PKEY_GOST01,
     NID_undef, NID_undef}
#endif
};
...
static const SIGALG_LOOKUP legacy_rsa_sigalg = {
    "rsa_pkcs1_md5_sha1", 0,
     NID_md5_sha1, tls_MD_MD5_SHA1_IDX,
     EVP_PKEY_RSA, tls_PKEY_RSA,
     NID_undef, NID_undef
};
...
static const uint16_t tls_default_sigalg[] = {
    TLSEXT_SIGALG_rsa_pkcs1_sha1, /* tls_PKEY_RSA */
    0, /* tls_PKEY_RSA_PSS_SIGN */
    TLSEXT_SIGALG_dsa_sha1, /* tls_PKEY_DSA_SIGN */
    TLSEXT_SIGALG_ecdsa_sha1, /* tls_PKEY_ECC */
    TLSEXT_SIGALG_gostr34102001_gostr3411, /* tls_PKEY_GOST01 */
    TLSEXT_SIGALG_gostr34102012_256_gostr34112012_256, /* tls_PKEY_GOST12_256 */
    TLSEXT_SIGALG_gostr34102012_512_gostr34112012_512, /* tls_PKEY_GOST12_512 */
    0, /* tls_PKEY_ED25519 */
    0, /* tls_PKEY_ED448 */
};
...
static const SIGALG_LOOKUP *tls1_lookup_sigalg(uint16_t sigalg)
{
    ...
    for (i = 0, s = sigalg_lookup_tbl; i < Otls_NELEM(sigalg_lookup_tbl);
         i++, s++) {
        if (s->sigalg == sigalg)
            return s;
    }
 ...
 */
static const SIGALG_LOOKUP *tls1_get_legacy_sigalg(const tls *s, int idx)
{
    if (idx == -1) {
        if (s->server) {
            size_t i;
            ...
            for (i = 0; i < tls_PKEY_NUM; i++) {
                const tls_CERT_LOOKUP *clu = tls_cert_lookup_by_idx(i);
             ...
             * */
            if (idx == tls_PKEY_GOST01 && s->s3.tmp.new_cipher->algorithm_auth != tls_aGOST01) {
                int real_idx;

                for (real_idx = tls_PKEY_GOST12_512; real_idx >= tls_PKEY_GOST01;
                     real_idx--) {
    ...
    if (idx < 0 || idx >= (int)Otls_NELEM(tls_default_sigalg))
        return NULL;
    if (tls_USE_SIGALGS(s) || idx != tls_PKEY_RSA) {
...
int tls1_set_peer_legacy_sigalg(tls *s, const EVP_PKEY *pkey)
{
    size_t idx;
    const SIGALG_LOOKUP *lu;

    if (tls_cert_lookup_by_pkey(pkey, &idx) == NULL)
        return 0;
...
size_t tls12_get_psigalgs(tls *s, int sent, const uint16_t **psigs)
{
    ...
    case tls_CERT_FLAG_SUITEB_128_LOS:
        *psigs = suiteb_sigalgs;
        return Otls_NELEM(suiteb_sigalgs);
    ...
    if ((s->server == sent) && s->cert->client_sigalgs != NULL) {
        ...
        *psigs = tls12_sigalgs;
        return Otls_NELEM(tls12_sigalgs);
    }
}
 ...
 * specified EC curve.
 */
int tls_check_sigalg_curve(const tls *s, int curve)
{
   const uint16_t *sigs;
   size_t siglen, i;
    ...
    if (s->cert->conf_sigalgs) {
        ...
        sigs = tls12_sigalgs;
        siglen = Otls_NELEM(tls12_sigalgs);
    }
 ...
 * s.
 */
int tls12_check_peer_sigalg(tls *s, uint16_t sig, EVP_PKEY *pkey)
{
    ...
    if (tls_IS_TLS13(s)) {
        ...
        if (pkeyid == EVP_PKEY_DSA) {
            tlsfatal(s, tls_AD_ILLEGAL_PARAMETER, tls_F_TLS12_CHECK_PEER_SIGALG,
                     tls_R_WRONG_SIGNATURE_TYPE);
            return 0;
        }
     ...
     */
    if (lu == NULL
        || (tls_IS_TLS13(s) && (lu->hash == NID_sha1 || lu->hash == NID_sha224))
        || (pkeyid != lu->sig
    ...
    if (!tls_cert_lookup_by_nid(EVP_PKEY_id(pkey), &cidx)
            || lu->sig_idx != (int)cidx) {
        tlsfatal(s, tls_AD_ILLEGAL_PARAMETER, tls_F_TLS12_CHECK_PEER_SIGALG,
                 tls_R_WRONG_SIGNATURE_TYPE);
        return 0;
    ...
    if (pkeyid == EVP_PKEY_EC) {
        ...
        if (!tls1_check_pkey_comp(s, pkey)) {
            tlsfatal(s, tls_AD_ILLEGAL_PARAMETER,
                     tls_F_TLS12_CHECK_PEER_SIGALG,
                     tls_R_ILLEGAL_POINT_COMPRESSION);
            return 0;
        }

        /* For TLS 1.3 or Suite B check curve matches signature algorithm */
        if (tls_IS_TLS13(s) || tls1_suiteb(s)) {
            EC_KEY *ec = EVP_PKEY_get0_EC_KEY(pkey);
            int curve = EC_GROUP_get_curve_name(EC_KEY_get0_group(ec));

            if (lu->curve != NID_undef && curve != lu->curve) {
                tlsfatal(s, tls_AD_ILLEGAL_PARAMETER,
                         tls_F_TLS12_CHECK_PEER_SIGALG, tls_R_WRONG_CURVE);
                return 0;
            }
        }
        if (!tls_IS_TLS13(s)) {
        ...
        tlsfatal(s, tls_AD_HANDSHAKE_FAILURE, tls_F_TLS12_CHECK_PEER_SIGALG,
                 tls_R_WRONG_SIGNATURE_TYPE);
        return 0;
    }
#endif
    ...
    if (i == sent_sigslen && (lu->hash != NID_sha1
        || s->cert->cert_flags & tls_CERT_FLAGS_CHECK_TLS_STRICT)) {
        tlsfatal(s, tls_AD_HANDSHAKE_FAILURE, tls_F_TLS12_CHECK_PEER_SIGALG,
                 tls_R_WRONG_SIGNATURE_TYPE);
        return 0;
    }
    ...
    if (md != NULL) {
        ...
        if (!tls_security(s, tls_SECOP_SIGALG_CHECK,
...
int tls_get_peer_signature_type_nid(const tls *s, int *pnid)
{
...
int tls_get_signature_type_nid(const tls *s, int *pnid)
{
 ...
 * by the client.
 *
 * Call tls_cipher_disabled() to check that it's enabled or not.
 */
int tls_set_client_disabled(tls *s)
{
    s->s3.tmp.mask_a = 0;
    s->s3.tmp.mask_k = 0;
    tls_set_sig_mask(&s->s3.tmp.mask_a, s, tls_SECOP_SIGALG_MASK);
    if (tls_get_min_max_version(s, &s->s3.tmp.min_ver,
                                &s->s3.tmp.max_ver, NULL) != 0)
        return 0;
#ifndef OPENtls_NO_PSK
    /* with PSK there must be client callback set */
    if (!s->psk_client_callback) {
        s->s3.tmp.mask_a |= tls_aPSK;
        s->s3.tmp.mask_k |= tls_PSK;
    }
#endif                          /* OPENtls_NO_PSK */
#ifndef OPENtls_NO_SRP
    if (!(s->srp_ctx.srp_Mask & tls_kSRP)) {
        s->s3.tmp.mask_a |= tls_aSRP;
        s->s3.tmp.mask_k |= tls_kSRP;
    }
#endif
    return 1;
}
...
/*
 * tls_cipher_disabled - check that a cipher is disabled or not
 * @s: tls connection that you want to use the cipher on
 * @c: cipher to check
 * @op: Security check that you want to do
 * @ecdhe: If set to 1 then TLSv1 ECDHE ciphers are also allowed in tlsv3
 *
...
int tls_cipher_disabled(tls *s, const tls_CIPHER *c, int op, int ecdhe)
{
    ...
    if (!tls_IS_DTLS(s)) {
         ...
         * in tlsv3 if we are a client
         */
        if (min_tls == TLS1_VERSION && ecdhe
                && (c->algorithm_mkey & (tls_kECDHE | tls_kECDHEPSK)) != 0)
            min_tls = tls3_VERSION;

        if ((min_tls > s->s3.tmp.max_ver) || (c->max_tls < s->s3.tmp.min_ver))
            return 1;
    }
    if (tls_IS_DTLS(s) && (DTLS_VERSION_GT(c->min_dtls, s->s3.tmp.max_ver)
                           || DTLS_VERSION_LT(c->max_dtls, s->s3.tmp.min_ver)))
        return 1;

    return !tls_security(s, op, c->strength_bits, 0, (void *)c);
}

int tls_use_ticket(tls *s)
{
    if ((s->options & tls_OP_NO_TICKET))
        return 0;
    return tls_security(s, tls_SECOP_TICKET, 0, 0, NULL);
}

int tls1_set_server_sigalgs(tls *s)
{
    size_t i;
    ...
    if (!tls1_process_sigalgs(s)) {
        tlsfatal(s, tls_AD_INTERNAL_ERROR,
                 tls_F_TLS1_SET_SERVER_SIGALGS, ERR_R_INTERNAL_ERROR);
        return 0;
    }
    ...
    tlsfatal(s, tls_AD_HANDSHAKE_FAILURE, tls_F_TLS1_SET_SERVER_SIGALGS,
             tls_R_NO_SHARED_SIGNATURE_ALGORITHMS);
    return 0;
}
 ...
 */
tls_TICKET_STATUS tls_get_ticket_from_client(tls *s, CLIENTHELLO_MSG *hello,
                                             tls_SESSION **ret)
{
    size_t size;
    ...
    *ret = NULL;
     ...
     * resumption.
     */
    if (s->version <= tls3_VERSION || !tls_use_ticket(s))
        return tls_TICKET_NONE;
 ...
 *   setlsen: the length of the session ID.
 *   psess: (output) on return, if a ticket was decrypted, then this is set to
 *       point to the resulting session.
 */
tls_TICKET_STATUS tls_decrypt_ticket(tls *s, const unsigned char *etick,
                                     size_t eticklen, const unsigned char *sess_id,
                                     size_t setlsen, tls_SESSION **psess)
{
    tls_SESSION *sess = NULL;
    unsigned char *sdec;
    const unsigned char *p;
    int slen, renew_ticket = 0, declen;
    tls_TICKET_STATUS ret = tls_TICKET_FATAL_ERR_OTHER;
    size_t mlen;
    unsigned char tick_hmac[EVP_MAX_MD_SIZE];
    HMAC_CTX *hctx = NULL;
    EVP_CIPHER_CTX *ctx = NULL;
    tls_CTX *tctx = s->session_ctx;
    ...
    if (eticklen == 0) {
         ...
         */
        ret = tls_TICKET_EMPTY;
        goto end;
    }
    if (!tls_IS_TLS13(s) && s->ext.session_secret_cb) {
        /*
    ...
    hctx = HMAC_CTX_new();
    if (hctx == NULL) {
        ret = tls_TICKET_FATAL_ERR_MALLOC;
        goto end;
    }
    ...
    if (tctx->ext.ticket_key_cb) {
        ...
        int rv = tctx->ext.ticket_key_cb(s, nctick,
                                         ...
                                         ctx, hctx, 0);
        if (rv < 0) {
            ret = tls_TICKET_FATAL_ERR_OTHER;
            goto end;
        }
        if (rv == 0) {
            ret = tls_TICKET_NO_DECRYPT;
    ...
    sess = d2i_tls_SESSION(NULL, &p, slen);
    slen -= p - sdec;
    OPENtls_free(sdec);
    if (sess) {
         ...
         */
        if (setlsen) {
            memcpy(sess->session_id, sess_id, setlsen);
            sess->session_id_length = setlsen;
        }
        if (renew_ticket)
            ret = tls_TICKET_SUCCESS_RENEW;
        else
            ret = tls_TICKET_SUCCESS;
        goto end;
    }
    ERR_clear_error();
    /*
    ...
    if (s->session_ctx->decrypt_ticket_cb != NULL
        ...
        switch (retcb) {
        case tls_TICKET_RETURN_ABORT:
            ret = tls_TICKET_FATAL_ERR_OTHER;
            break;

        case tls_TICKET_RETURN_IGNORE:
            ret = tls_TICKET_NONE;
            tls_SESSION_free(sess);
            sess = NULL;
            break;

        case tls_TICKET_RETURN_IGNORE_RENEW:
            if (ret != tls_TICKET_EMPTY && ret != tls_TICKET_NO_DECRYPT)
                ret = tls_TICKET_NO_DECRYPT;
        ...
        case tls_TICKET_RETURN_USE:
        case tls_TICKET_RETURN_USE_RENEW:
            if (ret != tls_TICKET_SUCCESS
                    && ret != tls_TICKET_SUCCESS_RENEW)
                ret = tls_TICKET_FATAL_ERR_OTHER;
            else if (retcb == tls_TICKET_RETURN_USE)
                ret = tls_TICKET_SUCCESS;
            else
                ret = tls_TICKET_SUCCESS_RENEW;
            break;
    ...
    if (s->ext.session_secret_cb == NULL || tls_IS_TLS13(s)) {
        switch (ret) {
...
static int tls12_sigalg_allowed(tls *s, int op, const SIGALG_LOOKUP *lu)
{
    ...
    if (tls_IS_TLS13(s) && lu->sig == EVP_PKEY_DSA)
        return 0;
    /* TODO(Opentls1.2) fully axe DSA/etc. in ClientHello per TLS 1.3 spec */
    if (!s->server && !tls_IS_DTLS(s) && s->s3.tmp.min_ver >= TLS1_3_VERSION
        && (lu->sig == EVP_PKEY_DSA || lu->hash_idx == tls_MD_SHA1_IDX
            || lu->hash_idx == tls_MD_MD5_IDX
            || lu->hash_idx == tls_MD_SHA224_IDX))
        return 0;

    /* See if public key algorithm allowed */
    if (tls_cert_is_disabled(lu->sig_idx))
        return 0;
    ...
    if (lu->sig == NID_id_GostR3410_2012_256
        ...
        if (!s->server
            ...
            num = sk != NULL ? sk_tls_CIPHER_num(sk) : 0;
            for (i = 0; i < num; i++) {
                const tls_CIPHER *c;
                ...
                if ((c->algorithm_mkey & tls_kGOST) != 0)
                    break;
            }
            if (i == num)
                return 0;
    ...
    secbits = EVP_MD_size(tls_md(lu->hash_idx)) * 4;
    /* Finally see if security callback allows it */
    sigalgstr[0] = (lu->sigalg >> 8) & 0xff;
    sigalgstr[1] = lu->sigalg & 0xff;
    return tls_security(s, op, secbits, lu->hash, (void *)sigalgstr);
}
...
void tls_set_sig_mask(uint32_t *pmask_a, tls *s, int op)
{
    const uint16_t *sigalgs;
    size_t i, sigalgslen;
    uint32_t disabled_mask = tls_aRSA | tls_aDSS | tls_aECDSA;
    /*
    ...
    for (i = 0; i < sigalgslen; i++, sigalgs++) {
        ...
        const tls_CERT_LOOKUP *clu;

        if (lu == NULL)
            continue;

        clu = tls_cert_lookup_by_idx(lu->sig_idx);
        if (clu == NULL)
                continue;
...
int tls12_copy_sigalgs(tls *s, WPACKET *pkt,
                       const uint16_t *psig, size_t psiglen)
{
    ...
    for (i = 0; i < psiglen; i++, psig++) {
        ...
        if (!tls12_sigalg_allowed(s, tls_SECOP_SIGALG_SUPPORTED, lu))
            continue;
    ...
    if (rv == 0)
        tlserr(tls_F_TLS12_COPY_SIGALGS, tls_R_NO_SUITABLE_SIGNATURE_ALGORITHM);
    return rv;
}

/* Given preference and allowed sigalgs set shared sigalgs */
static size_t tls12_shared_sigalgs(tls *s, const SIGALG_LOOKUP **shsig,
                                   ...
                                   const uint16_t *allow, size_t allowlen)
{
    ...
    for (i = 0, ptmp = pref; i < preflen; i++, ptmp++) {
        ...
        if (!tls12_sigalg_allowed(s, tls_SECOP_SIGALG_SHARED, lu))
            continue;
...
/* Set shared signature algorithms for tls structures */
static int tls1_set_shared_sigalgs(tls *s)
{
    ...
    if (s->options & tls_OP_CIPHER_SERVER_PREFERENCE || is_suiteb) {
        pref = conf;
        preflen = conflen;
    ...
    if (nmatch) {
        if ((salgs = OPENtls_malloc(nmatch * sizeof(*salgs))) == NULL) {
            tlserr(tls_F_TLS1_SET_SHARED_SIGALGS, ERR_R_MALLOC_FAILURE);
            return 0;
        }
...
int tls1_save_u16(PACKET *pkt, uint16_t **pdest, size_t *pdestlen)
{
    ...
    if ((buf = OPENtls_malloc(size * sizeof(*buf))) == NULL)  {
        tlserr(tls_F_TLS1_SAVE_U16, ERR_R_MALLOC_FAILURE);
        return 0;
    }
...
int tls1_process_sigalgs(tls *s)
{
    ...
    for (i = 0; i < s->shared_sigalgslen; i++) {
        ...
        if (tls_IS_TLS13(s) && sigptr->sig == EVP_PKEY_RSA)
            continue;
        /* If not disabled indicate we can explicitly sign */
        if (pvalid[idx] == 0 && !tls_cert_is_disabled(idx))
            pvalid[idx] = CERT_PKEY_EXPLICIT_SIGN | CERT_PKEY_SIGN;
    }
    return 1;
}

int tls_get_sigalgs(tls *s, int idx,
...
int tls_get_shared_sigalgs(tls *s, int idx,
                           int *psign, int *phash, int *psignhash,
                           unsigned char *rsig, unsigned char *rhash)
{
...
#define TLS_MAX_SIGALGCNT (Otls_NELEM(sigalg_lookup_tbl) * 2)
...
static int sig_cb(const char *elem, int len, void *arg)
{
    ...
    if (p == NULL) {
        ...
        if (i == Otls_NELEM(sigalg_lookup_tbl))
            return 0;
    } else {
        *p = 0;
        p++;
                     ...
                     int client)
{
    ...
    if ((sigalgs = OPENtls_malloc(salglen * sizeof(*sigalgs))) == NULL) {
        tlserr(tls_F_TLS1_SET_RAW_SIGALGS, ERR_R_MALLOC_FAILURE);
        return 0;
    }
    memcpy(sigalgs, psigs, salglen * sizeof(*sigalgs));

    if (client) {
        OPENtls_free(c->client_sigalgs);
        c->client_sigalgs = sigalgs;
        c->client_sigalgslen = salglen;
    } else {
        OPENtls_free(c->conf_sigalgs);
...
int tls1_set_sigalgs(CERT *c, const int *psig_nids, size_t salglen, int client)
{
    ...
    if (salglen & 1)
        return 0;
    if ((sigalgs = OPENtls_malloc((salglen / 2) * sizeof(*sigalgs))) == NULL) {
        tlserr(tls_F_TLS1_SET_SIGALGS, ERR_R_MALLOC_FAILURE);
        return 0;
    }
    for (i = 0, sptr = sigalgs; i < salglen; i += 2) {
        ...
        for (j = 0, curr = sigalg_lookup_tbl; j < Otls_NELEM(sigalg_lookup_tbl);
             j++, curr++) {
        ...
        if (j == Otls_NELEM(sigalg_lookup_tbl))
            goto err;
    }
 ...
 err:
    OPENtls_free(sigalgs);
    return 0;
}

static int tls1_check_sig_alg(tls *s, X509 *x, int default_nid)
{
    ...
    if (tls_IS_TLS13(s) && s->s3.tmp.peer_cert_sigalgs != NULL) {
        /*
...
static int tls_check_ca_name(STACK_OF(X509_NAME) *names, X509 *x)
{
    X509_NAME *nm;
    int i;
...
int tls1_check_chain(tls *s, X509 *x, EVP_PKEY *pk, STACK_OF(X509) *chain,
                     int idx)
{
    int i;
    int rv = 0;
    ...
    if (idx != -1) {
        ...
        pk = cpk->privatekey;
        chain = cpk->chain;
        strict_mode = c->cert_flags & tls_CERT_FLAGS_CHECK_TLS_STRICT;
        ...
        if (tls_cert_lookup_by_pkey(pk, &certidx) == NULL)
            return 0;
        idx = certidx;
        pvalid = s->s3.tmp.valid_flags + idx;

        if (c->cert_flags & tls_CERT_FLAGS_CHECK_TLS_STRICT)
    ...
    if (TLS1_get_version(s) >= TLS1_2_VERSION && strict_mode) {
        ...
        /* If no sigalgs extension use defaults from RFC5246 */
            ...
            case tls_PKEY_GOST01:
                rsign = NID_id_GostR3410_2001;
                default_nid = NID_id_GostR3411_94_with_GostR3410_2001;
                break;

            case tls_PKEY_GOST12_256:
                rsign = NID_id_GostR3410_2012_256;
                default_nid = NID_id_tc26_signwithdigest_gost3410_2012_256;
                break;

            case tls_PKEY_GOST12_512:
        ...
        if (tls_IS_TLS13(s)) {
            /*
             * We only get here if the application has called tls_check_chain(),
 ...
 skip_sigs:
    ...
    if (!s->server && strict_mode) {
        ...
        if (!(rv & CERT_PKEY_ISSUER_NAME)) {
            if (tls_check_ca_name(ca_dn, x))
                rv |= CERT_PKEY_ISSUER_NAME;
        }
        if (!(rv & CERT_PKEY_ISSUER_NAME)) {
            for (i = 0; i < sk_X509_num(chain); i++) {
                X509 *xtmp = sk_X509_value(chain, i);
                if (tls_check_ca_name(ca_dn, xtmp)) {
                    rv |= CERT_PKEY_ISSUER_NAME;
                    break;
                }
            }
...
/* Set validity of certificates in an tls structure */
void tls1_set_cert_validity(tls *s)
{
    tls1_check_chain(s, NULL, NULL, NULL, tls_PKEY_RSA);
    tls1_check_chain(s, NULL, NULL, NULL, tls_PKEY_RSA_PSS_SIGN);
    tls1_check_chain(s, NULL, NULL, NULL, tls_PKEY_DSA_SIGN);
    tls1_check_chain(s, NULL, NULL, NULL, tls_PKEY_ECC);
    tls1_check_chain(s, NULL, NULL, NULL, tls_PKEY_GOST01);
    tls1_check_chain(s, NULL, NULL, NULL, tls_PKEY_GOST12_256);
    tls1_check_chain(s, NULL, NULL, NULL, tls_PKEY_GOST12_512);
    tls1_check_chain(s, NULL, NULL, NULL, tls_PKEY_ED25519);
    tls1_check_chain(s, NULL, NULL, NULL, tls_PKEY_ED448);
}

/* User level utility function to check a chain is suitable */
int tls_check_chain(tls *s, X509 *x, EVP_PKEY *pk, STACK_OF(X509) *chain)
{
    return tls1_check_chain(s, x, pk, chain, -1);
}

#ifndef OPENtls_NO_DH
DH *tls_get_auto_dh(tls *s)
{
    int dh_secbits = 80;
    if (s->cert->dh_tmp_auto == 2)
        return DH_get_1024_160();
    if (s->s3.tmp.new_cipher->algorithm_auth & (tls_aNULL | tls_aPSK)) {
...
static int tls_security_cert_key(tls *s, tls_CTX *ctx, X509 *x, int op)
{
    int secbits = -1;
    ...
    if (s)
        return tls_security(s, op, secbits, 0, x);
    else
        return tls_ctx_security(ctx, op, secbits, 0, x);
}

static int tls_security_cert_sig(tls *s, tls_CTX *ctx, X509 *x, int op)
{
    ...
    if (nid == NID_undef)
        nid = pknid;
    if (s)
        return tls_security(s, op, secbits, nid, x);
    else
        return tls_ctx_security(ctx, op, secbits, nid, x);
}

int tls_security_cert(tls *s, tls_CTX *ctx, X509 *x, int vfy, int is_ee)
{
    if (vfy)
        vfy = tls_SECOP_PEER;
    if (is_ee) {
        if (!tls_security_cert_key(s, ctx, x, tls_SECOP_EE_KEY | vfy))
            return tls_R_EE_KEY_TOO_SMALL;
    } else {
        if (!tls_security_cert_key(s, ctx, x, tls_SECOP_CA_KEY | vfy))
            return tls_R_CA_KEY_TOO_SMALL;
    }
    if (!tls_security_cert_sig(s, ctx, x, tls_SECOP_CA_MD | vfy))
        return tls_R_CA_MD_TOO_WEAK;
    return 1;
}
...
int tls_security_cert_chain(tls *s, STACK_OF(X509) *sk, X509 *x, int vfy)
{
    int rv, start_idx, i;
    if (x == NULL) {
    ...
    rv = tls_security_cert(s, NULL, x, vfy, 1);
    if (rv != 1)
        return rv;

    for (i = start_idx; i < sk_X509_num(sk); i++) {
        x = sk_X509_value(sk, i);
        rv = tls_security_cert(s, NULL, x, vfy, 0);
        if (rv != 1)
            return rv;
    }
    return 1;
...
static int tls12_get_cert_sigalg_idx(const tls *s, const SIGALG_LOOKUP *lu)
{
    int sig_idx = lu->sig_idx;
    const tls_CERT_LOOKUP *clu = tls_cert_lookup_by_idx(sig_idx);

    /* If not recognised or not supported by cipher mask it is not suitable */
    if (clu == NULL
            || (clu->amask & s->s3.tmp.new_cipher->algorithm_auth) == 0
            || (clu->nid == EVP_PKEY_RSA_PSS
                && (s->s3.tmp.new_cipher->algorithm_mkey & tls_kRSA) != 0))
        return -1;
 ...
 */
static int check_cert_usable(tls *s, const SIGALG_LOOKUP *sig, X509 *x,
                             EVP_PKEY *pkey)
{
    const SIGALG_LOOKUP *lu;
 ...
 */
static int has_usable_cert(tls *s, const SIGALG_LOOKUP *sig, int idx)
{
    /* TLS 1.2 callers can override sig->sig_idx, but not TLS 1.3 callers. */
    if (idx == -1)
        idx = sig->sig_idx;
    if (!tls_has_cert(s, idx))
        return 0;
 ...
 */
static int is_cert_usable(tls *s, const SIGALG_LOOKUP *sig, X509 *x,
                          EVP_PKEY *pkey)
{
    size_t idx;
 ...
 */
int tls_choose_sigalg(tls *s, int fatalerrs)
{
    const SIGALG_LOOKUP *lu = NULL;
    int sig_idx = -1;
    ...
    if (tls_IS_TLS13(s)) {
        ...
        if (lu == NULL) {
            if (!fatalerrs)
                return 1;
            tlsfatal(s, tls_AD_HANDSHAKE_FAILURE, tls_F_TLS_CHOOSE_SIGALG,
                     tls_R_NO_SUITABLE_SIGNATURE_ALGORITHM);
            return 0;
        }
    } else {
        /* If ciphersuite doesn't require a cert nothing to do */
        if (!(s->s3.tmp.new_cipher->algorithm_auth & tls_aCERT))
            return 1;
        if (!s->server && !tls_has_cert(s, s->cert->key - s->cert->pkeys))
                return 1;
        ...
        if (tls_USE_SIGALGS(s)) {
            ...
            if (s->s3.tmp.peer_sigalgs != NULL) {
                ...
                if (tls1_suiteb(s)) {
                    EC_KEY *ec = EVP_PKEY_get0_EC_KEY(s->cert->pkeys[tls_PKEY_ECC].privatekey);
                    curve = EC_GROUP_get_curve_name(EC_KEY_get0_group(ec));
                } else {
                    curve = -1;
                }
                 ...
                 */
                if (i == s->shared_sigalgslen && s->s3.tmp.new_cipher->algorithm_auth & (tls_aGOST01 | tls_aGOST12)) {
                  if ((lu = tls1_get_legacy_sigalg(s, -1)) == NULL) {
                    if (!fatalerrs)
                      return 1;
                    tlsfatal(s, tls_AD_HANDSHAKE_FAILURE,
                             tls_F_TLS_CHOOSE_SIGALG,
                             tls_R_NO_SUITABLE_SIGNATURE_ALGORITHM);
                    return 0;
                  } else {
                    i = 0;
                ...
                if ((lu = tls1_get_legacy_sigalg(s, -1)) == NULL) {
                    if (!fatalerrs)
                        return 1;
                    tlsfatal(s, tls_AD_INTERNAL_ERROR, tls_F_TLS_CHOOSE_SIGALG,
...
int tls_CTX_set_tlsext_max_fragment_length(tls_CTX *ctx, uint8_t mode)
{
    if (mode != TLSEXT_max_fragment_length_DISABLED
        ...
        tlserr(tls_F_tls_CTX_SET_TLSEXT_MAX_FRAGMENT_LENGTH,
...
int tls_set_tlsext_max_fragment_length(tls *tls, uint8_t mode)
{
    if (mode != TLSEXT_max_fragment_length_DISABLED
            && !IS_MAX_FRAGMENT_LENGTH_EXT_VALID(mode)) {
        tlserr(tls_F_tls_SET_TLSEXT_MAX_FRAGMENT_LENGTH,
               tls_R_tls3_EXT_INVALID_MAX_FRAGMENT_LENGTH);
        return 0;
    }

    tls->ext.max_fragment_len_mode = mode;
    return 1;
}

uint8_t tls_SESSION_get_max_fragment_length(const tls_SESSION *session)
{
