                                 ...
                                 const SSL_METHOD *meth)
{
    ...
    if ((RAND_bytes_with_libctx(libctx, ret->ext.tick_key_name,
                                sizeof(ret->ext.tick_key_name)) <= 0)
        || (RAND_priv_bytes_with_libctx(libctx, ret->ext.secure->tick_hmac_key,
                                        sizeof(ret->ext.secure->tick_hmac_key)) <= 0)
        || (RAND_priv_bytes_with_libctx(libctx, ret->ext.secure->tick_aes_key,
                                        sizeof(ret->ext.secure->tick_aes_key)) <= 0))
        ret->options |= SSL_OP_NO_TICKET;

    if (RAND_priv_bytes_with_libctx(libctx, ret->ext.cookie_hmac_key,
