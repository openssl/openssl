...
int SSL_new_session_ticket(SSL *s)
{
    ...
    if ((SSL_in_init(s) && sc->ext.extra_tickets_expected == 0)
            ...
            || !(SSL_CONNECTION_IS_TLS13(sc) || SSL_CONNECTION_IS_DTLS13(sc)))
        return 0;
...
const char *SSL_get_servername(const SSL *s, const int type)
{
    ...
    if (server) {
         ...
         */
         if (sc->hit && !(SSL_CONNECTION_IS_TLS13(sc) || SSL_CONNECTION_IS_DTLS13(sc)))
        ...
        if (SSL_in_before(s)) {
            if (sc->ext.hostname == NULL
                    ...
                    && sc->session->ssl_version != DTLS1_3_VERSION)
                return sc->session->ext.hostname;
        } else {
            if (!(SSL_CONNECTION_IS_TLS13(sc) || SSL_CONNECTION_IS_DTLS13(sc)) && sc->hit
                                     ...
                                     size_t contextlen)
{
    ...
    if (sc->version != TLS1_3_VERSION && sc->version != DTLS1_3_VERSION)
        return 0;
...
void ssl_update_cache(SSL_CONNECTION *s, int mode)
{
    ...
    if ((i & mode) != 0
        && (!s->hit || SSL_CONNECTION_IS_TLS13(s) || SSL_CONNECTION_IS_DTLS13(s))) {
        /*
