...
static int ssl_dane_dup(SSL_CONNECTION *to, SSL_CONNECTION *from)
{
	...
	for (i = 0; i < num; ++i) {
		danetls_record *t =
			sk_danetls_record_value(from->dane.trecs, i);

		if (SSL_dane_tlsa_add(SSL_CONNECTION_GET_SSL(to), t->usage,
				      t->selector, t->mtype, t->data,
				      t->dlen) <= 0)
			return 0;
	}
	return 1;
...
static int dane_mtype_set(struct dane_ctx_st *dctx, const EVP_MD *md,
...
static int dane_tlsa_add(SSL_DANE *dane, uint8_t usage, uint8_t selector,
...
#ifdef OPENSSL_NO_TLS1_1
		    || (min_version <= TLS1_1_VERSION &&
			TLS1_1_VERSION <= max_version)
#endif
#ifdef OPENSSL_NO_TLS1_2
		    || (min_version <= TLS1_2_VERSION &&
			TLS1_2_VERSION <= max_version)
#endif
#ifdef OPENSSL_NO_TLS1_3
		    || (min_version <= TLS1_3_VERSION &&
			TLS1_3_VERSION <= max_version)
#endif
		)
			return 0;
	}
				 ...
				 const SSL_METHOD *method)
{
	...
	if (ctx->ext.ecpointformats != NULL) {
		s->ext.ecpointformats = OPENSSL_memdup(
			ctx->ext.ecpointformats, ctx->ext.ecpointformats_len);
	...
	if (ctx->ext.supportedgroups != NULL) {
		...
		if (ctx->ext.supportedgroups_len == 0)
			...
			add = 1;
		s->ext.supportedgroups = OPENSSL_memdup(
	...
	if (ctx->ext.tuples != NULL) {
		...
		if (ctx->ext.tuples_len == 0)
			/* Add 1 so allocation won't fail */
			add = 1;
		s->ext.tuples = OPENSSL_memdup(
			ctx->ext.tuples,
			(ctx->ext.tuples_len + add) * sizeof(*ctx->ext.tuples));
	...
	memcpy(s->cert_comp_prefs, ctx->cert_comp_prefs,
	       sizeof(s->cert_comp_prefs));
#endif
...
int SSL_get0_dane_authority(SSL *s, X509 **mcert, EVP_PKEY **mspki)
{
	...
	if (dane->mtlsa) {
		if (mcert)
			*mcert = dane->mcert;
		if (mspki)
			*mspki = (dane->mcert == NULL) ? dane->mtlsa->spki :
							 NULL;
	}
	return dane->mdpth;
}
...
int SSL_copy_session_id(SSL *t, const SSL *f)
{
	...
	tsc->cert = fsc->cert;
	if (!SSL_set_session_id_context(t, fsc->sid_ctx,
					(int)fsc->sid_ctx_length)) {
		return 0;
	}
...
int ssl_write_internal(SSL *s, const void *buf, size_t num, uint64_t flags,
		       size_t *written)
{
	SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL(s);
...
long ossl_ctrl_internal(SSL *s, int cmd, long larg, void *parg, int no_quic)
{
	...
	case SSL_CTRL_MODE: {
		...
		*opts++ = OSSL_PARAM_construct_uint32(
			OSSL_LIBSSL_RECORD_LAYER_PARAM_MODE, &sc->mode);
		*opts = OSSL_PARAM_construct_end();
	...
	case SSL_CTRL_SET_MAX_PIPELINES:
		...
		if (sc->rlayer.rrlmethod->set_max_pipelines != NULL)
			sc->rlayer.rrlmethod->set_max_pipelines(sc->rlayer.rrl,
								(size_t)larg);
		return 1;
	...
	case SSL_CTRL_SET_MIN_PROTO_VERSION:
		return ssl_check_allowed_versions(larg,
	...
	case SSL_CTRL_SET_MAX_PROTO_VERSION:
		return ssl_check_allowed_versions(sc->min_proto_version,
						  larg) &&
		       ssl_set_version_bound(s->defltmeth->version, (int)larg,
					     &sc->max_proto_version);
...
long SSL_CTX_ctrl(SSL_CTX *ctx, int cmd, long larg, void *parg)
{
	...
	if (cmd == SSL_CTRL_SET_GROUPS_LIST && larg == 1)
		return tls1_set_groups_list(ctx, NULL, NULL, NULL, NULL, NULL,
					    NULL, parg);
	if (ctx == NULL) {
		switch (cmd) {
	...
	case SSL_CTRL_SET_MAX_PROTO_VERSION:
		return ssl_check_allowed_versions(ctx->min_proto_version,
						  larg) &&
		       ssl_set_version_bound(ctx->method->version, (int)larg,
					     &ctx->max_proto_version);
	case SSL_CTRL_GET_MAX_PROTO_VERSION:
...
int SSL_set_cipher_list(SSL *s, const char *str)
{
	...
	sk = ssl_create_cipher_list(ctx, sc->tls13_ciphersuites,
				    &sc->cipher_list, &sc->cipher_list_by_id,
				    str, sc->cert);
...
char *SSL_get_shared_ciphers(const SSL *s, char *buf, int size)
{
	...
	if (!sc->server || sc->peer_ciphers == NULL || size < 2)
		return NULL;
...
const char *SSL_get_servername(const SSL *s, const int type)
{
	...
	if (server) {
         ...
         */
		if (SSL_in_before(s)) {
			if (sc->ext.hostname == NULL && sc->session != NULL &&
	...
	*outlen = (unsigned char)PACKET_remaining(&csubpkt);
	...
	if (PACKET_buf_init(&spkt, server, server_len)) {
		while (PACKET_get_length_prefixed_1(&spkt, &ssubpkt)) {
			...
			if (PACKET_buf_init(&cpkt, client, client_len)) {
				while (PACKET_get_length_prefixed_1(&cpkt,
								    &csubpkt)) {
					if (PACKET_equal(&csubpkt,
							 PACKET_data(&ssubpkt),
							 PACKET_remaining(
								 &ssubpkt))) {
						/* We found a match */
...
void SSL_CTX_set_npn_advertised_cb(SSL_CTX *ctx,
				   SSL_CTX_npn_advertised_cb_func cb, void *arg)
{
	if (IS_QUIC_CTX(ctx))
 ...
 */
void SSL_CTX_set_npn_select_cb(SSL_CTX *ctx, SSL_CTX_npn_select_cb_func cb,
			       void *arg)
{
	if (IS_QUIC_CTX(ctx))
		/* NPN not allowed for QUIC */
			       ...
			       int use_context)
{
	...
	return sc->ssl.method->ssl3_enc->export_keying_material(
		sc, out, olen, label, llen, context, contextlen, use_context);
}
...
static unsigned long ssl_session_hash(const SSL_SESSION *a)
{
	...
	l = (unsigned long)((unsigned long)session_id[0]) |
			...
			const SSL_METHOD *meth)
{
	...
	if ((ret->cert = ssl_cert_new(SSL_PKEY_NUM + ret->sigalg_list_len)) ==
	    NULL) {
		ERR_raise(ERR_LIB_SSL, ERR_R_SSL_LIB);
		goto err;
	}

	if (!ssl_create_cipher_list(ret, ret->tls13_ciphersuites,
	...
	if ((ret->ext.secure =
		     OPENSSL_secure_zalloc(sizeof(*ret->ext.secure))) == NULL)
		goto err;
...
#define eng_str(x) eng_strx(x)
	/* Use specific client engine automatically... ignore errors */
	{
		...
		if (!eng) {
			...
			eng = ENGINE_by_id(
				eng_str(OPENSSL_SSL_CLIENT_ENGINE_AUTO));
		}
	...
	if (IS_QUIC_METHOD(meth)) {
		...
		if (meth == OSSL_QUIC_client_thread_method())
			ret->domain_flags = SSL_DOMAIN_FLAG_MULTI_THREAD |
...
#else
		ret->domain_flags = SSL_DOMAIN_FLAG_SINGLE_THREAD |
				    SSL_DOMAIN_FLAG_LEGACY_BLOCKING;
#endif
	}
#endif
	...
	if (keylogfile != NULL && strlen(keylogfile) != 0) {
		...
		if (!CRYPTO_THREAD_write_lock(keylog_lock)) {
			...
			if (keylog_bio == NULL) {
				...
				BIO_set_callback_ex(keylog_bio,
...
void ssl_update_cache(SSL_CONNECTION *s, int mode)
{
	...
	if ((i & mode) != 0 && (!s->hit || SSL_CONNECTION_IS_TLS13(s))) {
		/*
		...
		if ((i & SSL_SESS_CACHE_NO_INTERNAL_STORE) == 0 &&
		    (!SSL_CONNECTION_IS_TLS13(s) || !s->server ||
		     (s->max_early_data > 0 &&
		...
		if (s->session_ctx->new_session_cb != NULL &&
			...
			if (!s->session_ctx->new_session_cb(
				    SSL_CONNECTION_GET_USER_SSL(s), s->session))
				SSL_SESSION_free(s->session);
		}
	}
...
int SSL_do_handshake(SSL *s)
{
	...
	if (SSL_in_init(s) || SSL_in_before(s)) {
		if ((sc->mode & SSL_MODE_ASYNC) &&
			...
			ret = ssl_start_async_job(s, &args,
...
__owur int SSL_get_handshake_rtt(const SSL *s, uint64_t *rtt)
{
	...
	*rtt = ossl_time2us(
		ossl_time_subtract(sc->ts_msg_read, sc->ts_msg_write));
	return 1;
}
...
SSL *SSL_dup(SSL *s)
{
	...
	if (!CRYPTO_dup_ex_data(CRYPTO_EX_INDEX_SSL, &ret->ex_data,
				&s->ex_data))
		goto err;
	...
	retsc->default_passwd_callback_userdata =
	...
	if (sc->cipher_list != NULL) {
		if ((retsc->cipher_list = sk_SSL_CIPHER_dup(sc->cipher_list)) ==
		    NULL)
			goto err;
	}
	if (sc->cipher_list_by_id != NULL)
		if ((retsc->cipher_list_by_id =
			     sk_SSL_CIPHER_dup(sc->cipher_list_by_id)) == NULL)
			goto err;
...
int SSL_set_record_padding_callback(SSL *ssl,
				    size_t (*cb)(SSL *ssl, int type, size_t len,
						 void *arg))
{
	BIO *b;
	SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL_ONLY(ssl);
			     ...
			     size_t hs_block_size)
{
	...
	if (sc == NULL ||
	    (IS_QUIC(ssl) && (app_block_size > 1 || hs_block_size > 1)))
		return 0;
...
int (*SSL_get_security_callback(const SSL *s))(const SSL *s, const SSL_CTX *ctx,
...
uint64_t SSL_set_options(SSL *s, uint64_t op)
{
	...
	*opts++ = OSSL_PARAM_construct_uint64(
		OSSL_LIBSSL_RECORD_LAYER_PARAM_OPTIONS, &sc->options);
	*opts = OSSL_PARAM_construct_end();
...
static int ct_extract_tls_extension_scts(SSL_CONNECTION *s)
{
	...
	if (s->ext.scts != NULL) {
		...
		scts_extracted =
			ct_move_scts(&s->scts, scts, SCT_SOURCE_TLS_EXTENSION);
...
static int ct_extract_ocsp_response_scts(SSL_CONNECTION *s)
{
	...
	for (j = 0; j < sk_OCSP_RESPONSE_num(s->ext.ocsp.resp_ex); j++) {
		...
		for (i = 0; i < OCSP_resp_count(br); ++i) {
			...
			scts = OCSP_SINGLERESP_get1_ext_d2i(
				single, NID_ct_cert_scts, NULL, NULL);
			if (scts == NULL) {
				scts_extracted = -1;
				goto err;
			}
...
static int ct_strict(const CT_POLICY_EVAL_CTX *ctx, const STACK_OF(SCT) *scts,
		     void *unused_arg)
{
...
int ssl_validate_ct(SSL_CONNECTION *s)
{
	...
	if (s->ct_validation_callback == NULL || cert == NULL ||
	    s->verify_result != X509_V_OK || s->verified_chain == NULL ||
	...
	CT_POLICY_EVAL_CTX_set_shared_CTLOG_STORE(
		ctx, SSL_CONNECTION_GET_CTX(s)->ctlog_store);
	...
	ret = s->ct_validation_callback(ctx, scts,
					s->ct_validation_callback_arg);
	if (ret < 0)
...
int SSL_CTX_enable_ct(SSL_CTX *ctx, int validation_mode)
{
	...
	case SSL_CT_VALIDATION_PERMISSIVE:
		return SSL_CTX_set_ct_validation_callback(ctx, ct_permissive,
							  NULL);
...
static int nss_keylog_int(const char *prefix, SSL_CONNECTION *sc,
			  const uint8_t *parameter_1, size_t parameter_1_len,
			  const uint8_t *parameter_2, size_t parameter_2_len)
{
	char *out = NULL;
	char *cursor = NULL;
	...
	out_len =
		prefix_len + (2 * parameter_1_len) + (2 * parameter_2_len) + 3;
...
#endif
	if (sctx->keylog_callback != NULL)
		sctx->keylog_callback(SSL_CONNECTION_GET_USER_SSL(sc),
				      (const char *)out);
				    ...
				    size_t premaster_len)
{
	...
	return nss_keylog_int("RSA", sc, encrypted_premaster, 8, premaster,
			      premaster_len);
}
...
int ssl_log_secret(SSL_CONNECTION *sc, const char *label, const uint8_t *secret,
		   size_t secret_len)
{
	return nss_keylog_int(label, sc, sc->s3.client_random, SSL3_RANDOM_SIZE,
			      secret, secret_len);
}
...
int ossl_bytes_to_cipher_list(SSL_CONNECTION *s, PACKET *cipher_suites,
			      ...
			      STACK_OF(SSL_CIPHER) **scsvs_out, int sslv2format,
			      int fatal)
{
	const SSL_CIPHER *c;
...
int SSL_verify_client_post_handshake(SSL *ssl)
{
	...
	if (!send_certificate_request(sc)) {
		sc->post_handshake_auth =
			SSL_PHA_EXT_RECEIVED; /* restore on error */
...
void SSL_CTX_set_allow_early_data_cb(SSL_CTX *ctx,
				     SSL_allow_early_data_cb_fn cb, void *arg)
{
	ctx->allow_early_data_cb = cb;
	ctx->allow_early_data_cb_data = arg;
}

void SSL_set_allow_early_data_cb(SSL *s, SSL_allow_early_data_cb_fn cb,
				 void *arg)
{
	SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL_ONLY(s);
...
const EVP_CIPHER *ssl_evp_cipher_fetch(OSSL_LIB_CTX *libctx, int nid,
				       const char *properties)
{
	...
	if (ciph != NULL) {
		...
		params[0] = OSSL_PARAM_construct_int(
			OSSL_CIPHER_PARAM_DECRYPT_ONLY, &decrypt_only);
...
const EVP_MD *ssl_evp_md_fetch(OSSL_LIB_CTX *libctx, int nid,
			       const char *properties)
{
	const EVP_MD *md;
...
int SSL_shutdown_ex(SSL *ssl, uint64_t flags, const SSL_SHUTDOWN_EX_ARGS *args,
		    size_t args_len)
{
#ifndef OPENSSL_NO_QUIC
	if (!IS_QUIC(ssl))
...
int SSL_add_expected_rpk(SSL *s, EVP_PKEY *rpk)
{
	...
	ret = SSL_dane_tlsa_add(s, DANETLS_USAGE_DANE_EE, DANETLS_SELECTOR_SPKI,
				DANETLS_MATCHING_FULL, data, (size_t)ret) > 0;
	OPENSSL_free(data);
	return ret;
}
...
static int set_cert_type(unsigned char **cert_type, size_t *cert_type_len,
...
int SSL_CTX_set1_client_cert_type(SSL_CTX *ctx, const unsigned char *val,
				  size_t len)
{
...
int SSL_CTX_set1_server_cert_type(SSL_CTX *ctx, const unsigned char *val,
				  size_t len)
{
...
int SSL_CTX_get0_client_cert_type(const SSL_CTX *ctx, unsigned char **t,
				  size_t *len)
{
	if (t == NULL || len == NULL)
		return 0;
...
int SSL_CTX_get0_server_cert_type(const SSL_CTX *ctx, unsigned char **t,
				  size_t *len)
{
	if (t == NULL || len == NULL)
		return 0;
