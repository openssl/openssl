...
struct ssl_method_st {
	...
	int (*ssl_read_bytes)(SSL *s, uint8_t type, uint8_t *recvd_type,
			      ...
			      size_t *readbytes);
	int (*ssl_write_bytes)(SSL *s, uint8_t type, const void *buf_,
...
struct ssl_ctx_st {
	...
	struct {
		...
		TSAN_QUALIFIER int
			sess_connect_renegotiate; /* SSL reneg - requested */
		TSAN_QUALIFIER int
			sess_connect_good; /* SSL new conne/reneg - finished */
		TSAN_QUALIFIER int sess_accept; /* SSL new accept - started */
		TSAN_QUALIFIER int
			sess_accept_renegotiate; /* SSL reneg - requested */
		TSAN_QUALIFIER int
	...
	struct {
		...
		int (*ticket_key_cb)(SSL *ssl, unsigned char *name,
...
typedef struct cert_st {
	...
	/* Security callback */
	int (*sec_cb)(const SSL *s, const SSL_CTX *ctx, int op, int bits,
...
typedef struct ssl3_enc_method {
	...
	int (*close_construct_packet)(SSL_CONNECTION *s, WPACKET *pkt,
				      int htype);
...
static ossl_inline int tls12_rpk_and_privkey(const SSL_CONNECTION *sc, int idx)
{
     ...
     */
	return ((sc->server &&
		 sc->ext.server_cert_type == TLSEXT_cert_type_rpk) ||
		(!sc->server &&
...
#define OSSL_TLS1_2_OPTIONS                                                 \
	(SSL_OP_CRYPTOPRO_TLSEXT_BUG | SSL_OP_DONT_INSERT_EMPTY_FRAGMENTS | \
	 ...
	 SSL_OP_CISCO_ANYCONNECT | SSL_OP_NO_RENEGOTIATION |                \
	 SSL_OP_NO_EXTENDED_MASTER_SECRET | SSL_OP_NO_ENCRYPT_THEN_MAC |    \
	 SSL_OP_COOKIE_EXCHANGE | SSL_OP_LEGACY_SERVER_CONNECT |            \
...
#define OSSL_QUIC_PERMITTED_OPTIONS_CONN                                   \
	(OSSL_LEGACY_SSL_OPTIONS | OSSL_TLS1_2_OPTIONS |                   \
	 SSL_OP_SERVER_PREFERENCE | SSL_OP_DISABLE_TLSEXT_CA_NAMES |       \
