...
long SSL_SESSION_set_timeout_update_cache(const SSL *s, time_t t)
{
    SSL_SESSION *ss, *ret;
    SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL(s);
    long r = 1;
    ...
    if (!SSL_SESSION_set_timeout(ss, (long)t))
        return 0;

    /* update cache in case it's a copy */
    if (!CRYPTO_THREAD_read_lock(sc->session_ctx->lock))
        return 0;
    ret = lh_SSL_SESSION_retrieve(sc->session_ctx->sessions, ss);
    if (ret != NULL && ret != ss)
        r = SSL_SESSION_set_timeout(ss, (long)t);
    CRYPTO_THREAD_unlock(sc->session_ctx->lock);
