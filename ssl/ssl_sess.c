...
SSL_SESSION *SSL_SESSION_new(void)
{
	...
	if (!CRYPTO_new_ex_data(CRYPTO_EX_INDEX_SSL_SESSION, ss,
				&ss->ex_data)) {
...
static SSL_SESSION *ssl_session_dup_intern(const SSL_SESSION *src, int ticket)
{
	...
	if (src->psk_identity_hint) {
		dest->psk_identity_hint =
			OPENSSL_strdup(src->psk_identity_hint);
	...
	if (src->ext.alpn_selected != NULL) {
		dest->ext.alpn_selected = OPENSSL_memdup(
			src->ext.alpn_selected, src->ext.alpn_selected_len);
	...
	if (src->ticket_appdata != NULL) {
		dest->ticket_appdata = OPENSSL_memdup(src->ticket_appdata,
						      src->ticket_appdata_len);
				  ...
				  size_t sess_id_len)
{
	...
	if (ret == NULL && s->session_ctx->get_session_cb != NULL) {
		...
		ret = s->session_ctx->get_session_cb(
			SSL_CONNECTION_GET_USER_SSL(s), sess_id,
			(int)sess_id_len, &copy);
...
int ssl_get_prev_session(SSL_CONNECTION *s, CLIENTHELLO_MSG *hello)
{
	...
	if (SSL_CONNECTION_IS_TLS13(s)) {
		...
		if (!tls_parse_extension(s, TLSEXT_IDX_psk_kex_modes,
		    ...
		    !tls_parse_extension(s, TLSEXT_IDX_psk,
					 SSL_EXT_CLIENT_HELLO,
					 hello->pre_proc_exts, NULL, 0))
			return -1;
	...
	if (s == NULL) {
		...
		if (SSL_CTX_sess_get_cache_size(ctx) > 0) {
			while (SSL_CTX_sess_number(ctx) >=
				...
				if (!remove_session_lock(
					    ctx, ctx->session_cache_tail, 0))
					break;
				else
					ssl_tsan_counter(
						ctx,
...
int SSL_set_session_ticket_ext(SSL *s, void *ext_data, int ext_len)
{
	...
	if (sc->version >= TLS1_VERSION) {
		...
		sc->ext.session_ticket = OPENSSL_malloc(
