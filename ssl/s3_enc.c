				   ...
				   int num)
{
	...
	for (i = 0; (int)i < num; i += MD5_DIGEST_LENGTH) {
		...
		if (!EVP_DigestInit_ex(s1, sha1, NULL) ||
		    ...
		    !EVP_DigestUpdate(s1, s->s3.server_random,
				      SSL3_RANDOM_SIZE) ||
		    !EVP_DigestUpdate(s1, s->s3.client_random,
				      SSL3_RANDOM_SIZE) ||
...
int ssl3_change_cipher_state(SSL_CONNECTION *s, int which)
{
	...
	int direction = (which & SSL3_CC_READ) != 0 ?
	...
	if (!ssl_set_new_record_layer(s, SSL3_VERSION, direction,
				      ...
				      NULL, 0, key, key_len, iv, iv_len,
				      mac_secret, md_len, ciph, 0, NID_undef,
				      md, comp, NULL)) {
				       ...
				       OSSL_PARAM params[])
{
	...
	params[n++] = OSSL_PARAM_construct_octet_string(
		OSSL_DIGEST_PARAM_SSL3_MS, (void *)session->master_key,
			     ...
			     unsigned char *p)
{
	...
	if (sender != NULL) {
		...
		ssl3_digest_master_key_set_params(s->session,
						  digest_cmd_params);
				...
				size_t *secret_size)
{
	...
	for (i = 0; i < 3; i++) {
		if (EVP_DigestInit_ex(ctx, SSL_CONNECTION_GET_CTX(s)->sha1,
				      NULL) <= 0 ||
		    ...
		    EVP_DigestInit_ex(ctx, SSL_CONNECTION_GET_CTX(s)->md5,
				      NULL) <= 0 ||
		    EVP_DigestUpdate(ctx, p, len) <= 0 ||
