			       ...
			       const unsigned char *ext, size_t extlen)
{
	...
	case TLSEXT_TYPE_max_fragment_length:
		if (extlen < 1)
			return 0;
		xlen = extlen;
		return ssl_trace_list(bio, indent + 2, ext, xlen, 1,
				      ssl_mfl_tbl);
	...
	case TLSEXT_TYPE_key_share:
		...
		for (; xlen > 0; ext += share_len, xlen -= share_len) {
			...
			BIO_printf(bio, "NamedGroup: %s (%d)\n",
				   ssl_trace_str(group_id, ssl_groups_tbl),
				   group_id);
			ssl_print_hex(bio, indent + 4, "key_exchange: ", ext,
				      share_len);
		}
		break;
	...
	default:
		BIO_dump_indent(bio, (const char *)ext, (int)extlen,
				indent + 2);
	}
	return 1;
}
	...
	while (extslen > 0) {
		...
		if (extslen < extlen + 4) {
			BIO_printf(bio,
				   ...
				   extype, (int)extlen);
			BIO_dump_indent(bio, (const char *)msg, (int)extslen,
					indent + 2);
			return 0;
		}
		msg += 4;
		if (!ssl_print_extension(bio, indent + 2, server, mt, extype,
					 msg, extlen))
			return 0;
		msg += extlen;
		extslen -= extlen + 4;
				  ...
				  const unsigned char *msg, size_t msglen)
{
	...
	case SSL_kRSA:
	case SSL_kRSAPSK:
		if (TLS1_get_version(SSL_CONNECTION_GET_SSL(sc)) ==
		    SSL3_VERSION) {
			ssl_print_hex(bio, indent + 2,
				      "EncryptedPreMasterSecret", msg, msglen);
		} else {
			if (!ssl_print_hexbuf(bio, indent + 2,
					      "EncryptedPreMasterSecret", 2,
					      &msg, &msglen))
				return 0;
		}
		break;
	...
	case SSL_kDHE:
	case SSL_kDHEPSK:
		if (!ssl_print_hexbuf(bio, indent + 2, "dh_Yc", 2, &msg,
				      &msglen))
			return 0;
		break;
	...
	case SSL_kECDHE:
	case SSL_kECDHEPSK:
		if (!ssl_print_hexbuf(bio, indent + 2, "ecdh_Yc", 1, &msg,
				      &msglen))
			return 0;
		break;
	case SSL_kGOST:
		ssl_print_hex(bio, indent + 2, "GostKeyTransportBlob", msg,
			      msglen);
		msglen = 0;
		break;
	case SSL_kGOST18:
		ssl_print_hex(bio, indent + 2, "GOST-wrapped PreMasterSecret",
			      msg, msglen);
		msglen = 0;
		break;
	}
				  ...
				  const unsigned char *msg, size_t msglen)
{
	...
	BIO_printf(bio, "KeyExchangeAlgorithm=%s\n", algname);
	if (id & SSL_PSK) {
		if (!ssl_print_hexbuf(bio, indent + 2, "psk_identity_hint", 2,
				      &msg, &msglen))
			return 0;
	}
	switch (id) {
	...
	case SSL_kDHE:
	case SSL_kDHEPSK:
		if (!ssl_print_hexbuf(bio, indent + 2, "dh_p", 2, &msg,
				      &msglen))
			return 0;
		if (!ssl_print_hexbuf(bio, indent + 2, "dh_g", 2, &msg,
				      &msglen))
			return 0;
		if (!ssl_print_hexbuf(bio, indent + 2, "dh_Ys", 2, &msg,
				      &msglen))
			return 0;
		break;
				  ...
				  const unsigned char *msg, size_t msglen)
{
	...
	if ((server && sc->ext.server_cert_type == TLSEXT_cert_type_rpk) ||
		...
		if (!ssl_print_raw_public_key(bio, &sc->ssl, server, indent,
					      &msg, &clen))
			return 0;
		if (SSL_CONNECTION_IS_TLS13(sc) &&
				  ...
				  const unsigned char *msg, size_t msglen)
{
	...
	if (SSL_CONNECTION_IS_TLS13(sc)) {
		if (!ssl_print_hexbuf(bio, indent, "request_context", 1, &msg,
				      &msglen))
			return 0;
		if (!ssl_print_extensions(bio, indent, 1,
					  SSL3_MT_CERTIFICATE_REQUEST, &msg,
					  &msglen))
			return 0;
		return 1;
	} else {
		...
		if (!ssl_trace_list(bio, indent + 2, msg, xlen, 1,
				    ssl_ctype_tbl))
			return 0;
		msg += xlen;
		msglen -= xlen + 1;
			       ...
			       int indent)
{
	...
	case SSL3_MT_CERTIFICATE:
		if (!ssl_print_certificates(bio, sc, server, indent + 2, msg,
					    msglen))
			return 0;
		break;

	case SSL3_MT_COMPRESSED_CERTIFICATE:
		if (!ssl_print_compressed_certificates(bio, sc, server,
						       indent + 2, msg, msglen))
			return 0;
		break;
	...
	case SSL3_MT_SERVER_DONE:
		if (msglen != 0)
			ssl_print_hex(bio, indent + 2, "unexpected value", msg,
				      msglen);
		break;
	...
	case SSL3_MT_ENCRYPTED_EXTENSIONS:
		if (!ssl_print_extensions(bio, indent + 2, 1,
					  SSL3_MT_ENCRYPTED_EXTENSIONS, &msg,
					  &msglen))
			return 0;
		break;
	...
	default:
		...
		BIO_dump_indent(bio, (const char *)msg, (int)msglen,
				indent + 4);
	}
	return 1;
}
	       ...
	       size_t msglen, SSL *ssl, void *arg)
{
	...
	if (qc != NULL) {
		if (ossl_quic_trace(write_p, version, content_type, buf, msglen,
				    ssl, arg))
			return;
		/*
	...
	case SSL3_RT_HANDSHAKE:
		if (!ssl_print_handshake(bio, sc,
					 sc->server ? write_p : !write_p, msg,
					 msglen, 4))
	...
	case SSL3_RT_ALERT:
		if (msglen != 2)
			...
			BIO_printf(bio,
				   ...
				   msg[0], SSL_alert_desc_string_long(msg[1]),
				   msg[1]);
		}
	}
