                               ...
                               const unsigned char *ext, size_t extlen)
{
    ...
    case TLSEXT_TYPE_early_data:
        ...
        if (extlen != 4)
            return 0;
        max_early_data = ((ext[0] + 0U) << 24) | (ext[1] << 16) | (ext[2] << 8)
                         | ext[3];
        BIO_indent(bio, indent + 2, 80);
                            ...
                            const unsigned char *msg, size_t msglen)
{
    ...
    if (msglen < 4)
        return 0;
    tick_life = ((msg[0] + 0U) << 24) | (msg[1] << 16) | (msg[2] << 8) | msg[3];
    msglen -= 4;
    msg += 4;
    BIO_indent(bio, indent + 2, 80);
    ...
    if (SSL_IS_TLS13(ssl)) {
        ...
        if (msglen < 4)
            return 0;
        ticket_age_add =
            ((msg[0] + 0U) << 24) | (msg[1] << 16) | (msg[2] << 8) | msg[3];
        msglen -= 4;
        msg += 4;
        BIO_indent(bio, indent + 2, 80);
