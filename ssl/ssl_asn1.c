...
int i2d_SSL_SESSION(const SSL_SESSION *in, unsigned char **pp)
{
    ...
    as.time = ossl_time_to_sec(in->time);
    as.timeout = ossl_time_to_sec(in->timeout);
                             ...
                             long length)
{
    ...
    if (as->time != 0)
        ret->time = ossl_time_from_sec(as->time);
    else
        ret->time = ossl_time_now();

    if (as->timeout != 0)
        ret->timeout = ossl_time_from_sec(as->timeout);
    else
        ret->timeout = ossl_time_from_sec(3);
