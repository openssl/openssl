...
static const ssl_cipher_table ssl_cipher_table_cipher[SSL_ENC_NUM_IDX] = {
	...
	{ SSL_CAMELLIA128,
	  NID_camellia_128_cbc }, /* SSL_ENC_CAMELLIA128_IDX 8 */
	{ SSL_CAMELLIA256,
	  NID_camellia_256_cbc }, /* SSL_ENC_CAMELLIA256_IDX 9 */
	...
	{ SSL_eGOST2814789CNT12,
	  NID_gost89_cnt_12 }, /* SSL_ENC_GOST8912_IDX 18 */
	{ SSL_CHACHA20POLY1305,
	  NID_chacha20_poly1305 }, /* SSL_ENC_CHACHA_IDX 19 */
	{ SSL_ARIA128GCM, NID_aria_128_gcm }, /* SSL_ENC_ARIA128GCM_IDX 20 */
	{ SSL_ARIA256GCM, NID_aria_256_gcm }, /* SSL_ENC_ARIA256GCM_IDX 21 */
	{ SSL_MAGMA, NID_magma_ctr_acpkm }, /* SSL_ENC_MAGMA_IDX */
	{ SSL_KUZNYECHIK,
	  NID_kuznyechik_ctr_acpkm }, /* SSL_ENC_KUZNYECHIK_IDX */
};
...
static const ssl_cipher_table ssl_cipher_table_mac[SSL_MD_NUM_IDX] = {
	...
	{ SSL_GOST12_256,
	  NID_id_GostR3411_2012_256 }, /* SSL_MD_GOST12_256_IDX 6 */
	{ SSL_GOST89MAC12, NID_gost_mac_12 }, /* SSL_MD_GOST89MAC12_IDX 7 */
	{ SSL_GOST12_512,
	  NID_id_GostR3411_2012_512 }, /* SSL_MD_GOST12_512_IDX 8 */
	{ 0, NID_md5_sha1 }, /* SSL_MD_MD5_SHA1_IDX 9 */
	{ 0, NID_sha224 }, /* SSL_MD_SHA224_IDX 10 */
	{ 0, NID_sha512 }, /* SSL_MD_SHA512_IDX 11 */
	{ SSL_MAGMAOMAC, NID_magma_mac }, /* sSL_MD_MAGMAOMAC_IDX */
	{ SSL_KUZNYECHIKOMAC,
	  NID_kuznyechik_mac } /* SSL_MD_KUZNYECHIKOMAC_IDX */
};
...
static const SSL_CIPHER cipher_aliases[] = {
	...
	{ 0, SSL_TXT_GOST, NULL, 0, 0, 0,
	  SSL_eGOST2814789CNT | SSL_eGOST2814789CNT12 | SSL_MAGMA |
		  SSL_KUZNYECHIK },
	...
	{ 0, SSL_TXT_AES_CCM_8, NULL, 0, 0, 0,
	...
	{ 0, SSL_TXT_GOST89MAC, NULL, 0, 0, 0, 0,
	  SSL_GOST89MAC | SSL_GOST89MAC12 },
...
int ssl_load_ciphers(SSL_CTX *ctx)
{
	...
	for (i = 0, t = ssl_cipher_table_cipher; i < SSL_ENC_NUM_IDX;
	     i++, t++) {
		if (t->nid != NID_undef) {
			const EVP_CIPHER *cipher = ssl_evp_cipher_fetch(
				ctx->libctx, t->nid, ctx->propq);
	...
	if (i == -1) {
		*enc = NULL;
		...
		if (i == SSL_ENC_NULL_IDX) {
             ...
             */
			*enc = EVP_CIPHER_fetch(ctx->libctx, "NULL",
						ctx->propq);
			if (*enc == NULL)
				return 0;
		} else {
		       ...
		       SSL_COMP **comp, int use_etm)
{
	...
	if ((*enc != NULL) &&
		...
		if (c->algorithm_enc == SSL_RC4 && c->algorithm_mac == SSL_MD5)
			evp = ssl_evp_cipher_fetch(
				ctx->libctx, NID_rc4_hmac_md5, ctx->propq);
			   ...
			   CIPHER_ORDER **tail_p)
{
	...
	for (i = 0; i < num_of_ciphers; i++) {
		...
		if ((c->algorithm_mkey & disabled_mkey) ||
			...
			continue;
		if (((ssl_method->ssl3_enc->enc_flags & SSL_ENC_FLAG_DTLS) ==
		     0) &&
		    c->min_tls == 0)
			continue;
		if (((ssl_method->ssl3_enc->enc_flags & SSL_ENC_FLAG_DTLS) !=
	...
	for (;;) {
		...
		if (strength_bits >= 0) {
			...
			if ((algo_strength & SSL_STRONG_MASK) &&
			    !(algo_strength & SSL_STRONG_MASK &
			      cp->algo_strength))
				continue;
			if ((algo_strength & SSL_DEFAULT_MASK) &&
			    !(algo_strength & SSL_DEFAULT_MASK &
			      cp->algo_strength))
				continue;
		}
				    ...
				    CIPHER_ORDER **tail_p)
{
	...
	for (i = max_strength_bits; i >= 0; i--)
		if (number_uses[i] > 0)
			ssl_cipher_apply_rule(0, 0, 0, 0, 0, 0, 0, CIPHER_ORD,
					      i, head_p, tail_p);
			...
			while (ca_list[j]) {
				if (strncmp(buf, ca_list[j]->name, buflen) ==
					...
					break;
					   ...
					   strncmp(buf, ca_list[j]->stdname,
						   buflen) == 0 &&
					   ca_list[j]->stdname[buflen] ==
						   '\0') {
					found = 1;
					break;
				} else
		       ...
		       const char *rule_str, CERT *c)
{
	...
	if (num_of_ciphers > 0) {
		co_list =
			OPENSSL_malloc_array(num_of_ciphers, sizeof(*co_list));
		if (co_list == NULL)
			return NULL; /* Failure */
	}
	...
	ssl_cipher_apply_rule(0, SSL_kECDHE, SSL_aECDSA, 0, 0, 0, 0, CIPHER_ADD,
			      -1, &head, &tail);
	ssl_cipher_apply_rule(0, SSL_kECDHE, 0, 0, 0, 0, 0, CIPHER_ADD, -1,
			      &head, &tail);
	ssl_cipher_apply_rule(0, SSL_kECDHE, 0, 0, 0, 0, 0, CIPHER_DEL, -1,
			      &head, &tail);
     ...
     * strength.
     */
	ssl_cipher_apply_rule(0, 0, 0, SSL_AES ^ SSL_AESGCM, 0, 0, 0,
...
{
	int i = ssl_cipher_info_lookup(ssl_cipher_table_auth,
				       c->algorithm_auth);
