				   ...
				   int *abort_blocking)
{
	...
	if (fd_nfy != -1) {
		...
		if (!ossl_rio_poll_builder_add_fd(rpb, fd_nfy, /*r = */ 1,
						  /*w = */ 0))
			return 0;
			  ...
			  int *abort_blocking, size_t *p_result_count)
{
	...
	for (i = 0; i < num_items; ++i) {
		...
		case BIO_POLL_DESCRIPTOR_TYPE_SSL:
			...
			case SSL_TYPE_QUIC_CONNECTION:
			case SSL_TYPE_QUIC_XSO:
				if (!poll_translate_ssl_quic(ssl, wctx, rpb,
							     item->events,
							     abort_blocking))
					FAIL_ITEM(i);
				...
				if (!SSL_get_event_timeout(ssl, &timeout,
							   &is_infinite))
					FAIL_ITEM(
						i++); /* need to clean up this item too */
			...
			int do_tick, size_t *p_result_count)
{
	...
	for (i = 0; i < num_items; ++i) {
		...
		case BIO_POLL_DESCRIPTOR_TYPE_SSL:
			...
			case SSL_TYPE_QUIC_CONNECTION:
			case SSL_TYPE_QUIC_XSO:
				if (!ossl_quic_conn_poll_events(
					    ssl, events, do_tick, &revents))
	     ...
	     size_t *p_result_count)
{
	int ok = 1;
	size_t result_count = 0;
	ossl_unused int do_tick =
		((flags & SSL_POLL_FLAG_NO_HANDLE_EVENTS) == 0);
	OSSL_TIME deadline;
	...
	for (;;) {
		...
		if (!poll_readout(items, num_items, stride, do_tick,
				  &result_count)) {
			ok = 0;
			goto out;
		}
