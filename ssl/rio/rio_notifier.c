...
static int create_socket(int domain, int socktype, int protocol)
{
	...
	if (fd == INVALID_SOCKET) {
		...
		ERR_raise_data(ERR_LIB_SYS, err, "calling WSASocketA() = %d",
			       err);
		return INVALID_SOCKET;
	}

	/* Prevent interference with the socket from other processes on Windows. */
	if (setsockopt(fd, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, (void *)&on,
		       sizeof(on)) < 0) {
...
int ossl_rio_notifier_init(RIO_NOTIFIER *nfy)
{
     ...
     */
	if (accept_sa.sin_family != AF_INET ||
	    accept_sa.sin_port != sa.sin_port) {
		ERR_raise_data(
			ERR_LIB_SSL, ERR_R_INTERNAL_ERROR,
