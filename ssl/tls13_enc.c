		      ...
		      int fatal)
{
	...
	ret = tls13_hkdf_expand_ex(sctx->libctx, sctx->propq, md, secret, label,
				   labellen, data, datalen, out, outlen,
				   !fatal);
	if (ret == 0 && fatal)
			     ...
			     size_t finlen)
{
	...
	return tls13_hkdf_expand(s, md, secret, finishedlabel,
				 sizeof(finishedlabel) - 1, NULL, 0, fin,
				 finlen, 1);
}
			  ...
			  unsigned char *outsecret)
{
	...
	kdf = EVP_KDF_fetch(sctx->libctx, OSSL_KDF_NAME_TLS1_3_KDF,
			    sctx->propq);
	kctx = EVP_KDF_CTX_new(kdf);
	EVP_KDF_free(kdf);
	if (kctx == NULL) {
	...
	*p++ = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_DIGEST,
	...
	if (insecret != NULL)
		*p++ = OSSL_PARAM_construct_octet_string(
			OSSL_KDF_PARAM_KEY, (unsigned char *)insecret,
			insecretlen);
	if (prevsecret != NULL)
		*p++ = OSSL_PARAM_construct_octet_string(
			OSSL_KDF_PARAM_SALT, (unsigned char *)prevsecret,
			mdlen);
	*p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_PREFIX,
						 (unsigned char *)label_prefix,
						 sizeof(label_prefix) - 1);
...
static int derive_secret_key_and_iv(
	SSL_CONNECTION *s, const EVP_MD *md, const EVP_CIPHER *ciph,
	int mac_type, const EVP_MD *mac_md, const unsigned char *insecret,
		...
		*keylen = EVP_CIPHER_get_key_length(ciph);
		...
		if (mode == EVP_CIPH_CCM_MODE) {
			...
			*ivlen = EVP_CCM_TLS_IV_LEN;
			...
			} else if (s->psksession != NULL &&
				   s->psksession->cipher != NULL) {
...
int tls13_change_cipher_state(SSL_CONNECTION *s, int which)
{
	...
	if (((which & SSL3_CC_CLIENT) && (which & SSL3_CC_WRITE)) ||
		...
		if ((which & SSL3_CC_EARLY) != 0) {
			...
			handlen = BIO_get_mem_data(s->s3.handshake_buffer,
						   &hdata);
			if (handlen <= 0) {
				SSLfatal(s, SSL_AD_INTERNAL_ERROR,
             ...
             * it again
             */
			if (!ssl_cipher_get_evp_cipher(sctx, sslcipher,
						       &cipher)) {
				/* Error is already recorded */
				SSLfatal_alert(s, SSL_AD_INTERNAL_ERROR);
				goto err;
			...
			if (((EVP_CIPHER_flags(cipher) &
			    ...
			    (!ssl_cipher_get_evp_md_mac(sctx, sslcipher,
							&mac_md, &mac_pkey_type,
							NULL))) {
				SSLfatal_alert(s, SSL_AD_INTERNAL_ERROR);
				goto err;
			}
	...
	if ((which & SSL3_CC_EARLY) == 0) {
		...
		if (!ssl3_digest_cached_records(s, 1) ||
		    !ssl_handshake_hash(s, hashval, sizeof(hashval),
					&hashlen)) {
			/* SSLfatal() already called */;
			goto err;
		}
	...
	if (label == server_application_traffic) {
		...
		if (!tls13_hkdf_expand(s, ssl_handshake_md(s), insecret,
				       ...
				       hashlen, s->exporter_master_secret,
				       hashlen, 1)) {
			/* SSLfatal() already called */
			goto err;
		}

		if (!ssl_log_secret(s, EXPORTER_SECRET_LABEL,
	...
	if (!ssl_set_new_record_layer(s, s->version, direction, level, secret,
				      hashlen, key, keylen, iv, ivlen, NULL, 0,
				      cipher, taglen, mac_pkey_type, mac_md,
				      NULL, md)) {
		/* SSLfatal already called */
		goto err;
	}
...
int tls13_update_key(SSL_CONNECTION *s, int sending)
{
	...
	if (!derive_secret_key_and_iv(
		    ...
		    sizeof(application_traffic) - 1, secret, key, &keylen, &iv,
		    &ivlen, &taglen)) {
		/* SSLfatal() already called */
		goto err;
	}
	...
	if (!ssl_set_new_record_layer(s, s->version, direction,
				      ...
				      NULL, 0, s->s3.tmp.new_sym_enc, taglen,
				      NID_undef, NULL, NULL, md)) {
		/* SSLfatal already called */
		goto err;
	}
				 ...
				 size_t contextlen, int use_context)
{
	...
	if (EVP_DigestInit_ex(ctx, md, NULL) <= 0 ||
	    ...
	    !tls13_hkdf_expand(s, md, s->exporter_master_secret,
			       (const unsigned char *)label, llen, data,
			       datalen, exportsecret, hashsize, 0) ||
