...
static int tls1_PRF(SSL_CONNECTION *s, const void *seed1, size_t seed1_len,
		    ...
		    size_t olen, int fatal)
{
	...
	*p++ = OSSL_PARAM_construct_octet_string(
		OSSL_KDF_PARAM_SECRET, (unsigned char *)sec, (size_t)slen);
	*p++ = OSSL_PARAM_construct_octet_string(
		OSSL_KDF_PARAM_SEED, (void *)seed1, (size_t)seed1_len);
	*p++ = OSSL_PARAM_construct_octet_string(
		OSSL_KDF_PARAM_SEED, (void *)seed2, (size_t)seed2_len);
	*p++ = OSSL_PARAM_construct_octet_string(
		OSSL_KDF_PARAM_SEED, (void *)seed3, (size_t)seed3_len);
	*p++ = OSSL_PARAM_construct_octet_string(
		OSSL_KDF_PARAM_SEED, (void *)seed4, (size_t)seed4_len);
	*p++ = OSSL_PARAM_construct_octet_string(
		OSSL_KDF_PARAM_SEED, (void *)seed5, (size_t)seed5_len);
...
int tls1_change_cipher_state(SSL_CONNECTION *s, int which)
{
	...
	if (!ssl_set_new_record_layer(s, s->version, direction,
				      ...
				      NULL, 0, key, cl, iv, (size_t)k,
				      mac_secret, mac_secret_size, c, taglen,
...
int tls1_setup_key_block(SSL_CONNECTION *s)
{
	...
	if (!ssl_cipher_get_evp(SSL_CONNECTION_GET_CTX(s), s->session, &c,
	...
	OSSL_TRACE_BEGIN(TLS)
	{
		...
		BIO_dump_indent(trc_out, s->s3.client_random, SSL3_RANDOM_SIZE,
				4);
		BIO_printf(trc_out, "server random\n");
		BIO_dump_indent(trc_out, s->s3.server_random, SSL3_RANDOM_SIZE,
				4);
		BIO_printf(trc_out, "master key\n");
		BIO_dump_indent(trc_out, s->session->master_key,
...
size_t tls1_final_finish_mac(SSL_CONNECTION *s, const char *str, size_t slen,
			     unsigned char *out)
{
	size_t hashlen;
				...
				size_t *secret_size)
{
	if (s->session->flags & SSL_SESS_FLAG_EXTMS) {
		...
		if (!tls1_PRF(s, TLS_MD_EXTENDED_MASTER_SECRET_CONST,
			      TLS_MD_EXTENDED_MASTER_SECRET_CONST_SIZE, hash,
			      hashlen, NULL, 0, NULL, 0, NULL, 0, p, len, out,
		...
		if (!tls1_PRF(s, TLS_MD_MASTER_SECRET_CONST,
			      ...
			      s->s3.server_random, SSL3_RANDOM_SIZE, NULL, 0, p,
			      len, out, SSL3_MASTER_SECRET_SIZE, 1)) {
			/* SSLfatal() already called */
			return 0;
		}
	}
