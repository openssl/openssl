             ...
             */
{
	...
	}
	{
		...
		if (pass_str != NULL) {
			...
			res = OSSL_CMP_CTX_set1_secretValue(
				ctx, (unsigned char *)pass_str,
				strlen(pass_str));
			clear_free(pass_str);
		}
	}
...
lab:; /* should not trigger: space before ';' */
	...
	if (1)
		...
		do
			f(c,
			  c); /*@ (non-brace) code after 'do' just to construct case */
		while (2);
...
#define CB_ERR_IF(cond, ctx, cert, depth, err)                           \
	if ((cond) &&                                                    \
	    ((depth) < 0 || verify_cb_cert(ctx, cert, depth, err) == 0)) \
	return err
...
#define DEFINE_SET_GET_BASE_TEST(PREFIX, SETN, GETN, DUP, FIELD, TYPE, ERR, \
	...
	{                                                                   \
		CTX *ctx = fixture->ctx;                                    \
		int (*set_fn)(CTX * ctx, TYPE) = (int (*)(CTX * ctx, TYPE)) \
...
ASN1_ITEM_TEMPLATE(OSSL_CRMF_MSGS) = ASN1_EX_TEMPLATE_TYPE(
	ASN1_TFLG_SEQUENCE_OF, 0, OSSL_CRMF_MSGS, OSSL_CRMF_MSG)
