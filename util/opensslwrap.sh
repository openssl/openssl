...
if [ -x "${OPENSSL}.exe" ]; then
	# The original reason for this script's existence is to work around
	# certain caveats in run-time linker behaviour. On Windows platforms
	# adjusting $PATH used to be sufficient, but with the introduction of
	...
	# for backward compatibility (and nostalgic reasons :3).
