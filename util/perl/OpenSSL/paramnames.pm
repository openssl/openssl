...
# KEM_PARAM_OPERATION values
    'KEM_PARAM_OPERATION_RSASVE' =>     "RSASVE",
    'KEM_PARAM_OPERATION_DHKEM' =>      "DHKEM",
...
sub generate_internal_macros {
    ...
    # Sort the names to improve the change of cache coherency
...
sub generate_code_from_trie {
    ...
    print "static ossl_inline int ossl_param_find_idx(const char *s)\n{\n" if $n == 0;

    if ($trieref->{'suffix'}) {
        my $suf = $trieref->{'suffix'};

        printf "%*s if ($strcmp(\"$suf\", s + $n) == 0", $indent, "";
        if ($case_sensitive) {
            $suf =~ tr/_/-/;
    ...
    for my $l (sort keys %$trieref) {
        if ($l eq 'val') {
            printf "%*s break;\n", $indent, "";
            printf "%*scase '\\0':\n", $indent, "";
            printf "%*s return PIDX_%s;\n", $indent, "", $trieref->{'val'};
        } else {
            printf "%*s break;\n", $indent, "";
            printf "%*scase '%s':", $indent, "", $l;
            if (not $case_sensitive) {
