...
my $config       = "crypto/err/opentls.ec";
my $debug        = 0;
my $internal     = 0;
my $nowrite      = 0;
my $rebuild      = 0;
...
if ( ! $reindex && $statefile ) {
    ...
    while ( <STATE> ) {
        ...
        if ( $name =~ /^(?:Otls_|OPENtls_)?[A-Z0-9]{2,}_R_/ ) {
            ...
            $rcodes{$name} = $code;
        } elsif ( $name =~ /^(?:Otls_|OPENtls_)?[A-Z0-9]{2,}_F_/ ) {
            $fassigned{$lib} .= "$code:";
...
while ( ( my $hdr, my $lib ) = each %libinc ) {
    ...
    if ( $lib eq "tls" && $rmax{$lib} >= 1000 ) {
        print STDERR "tls error codes 1000+ are reserved for alerts.\n";
...
foreach my $file ( @source ) {
    ...
    while ( <IN> ) {
        ...
        if ( /(((?:Otls_|OPENtls_)?[A-Z0-9]{2,})_F_([A-Z0-9_]+))/ ) {
        ...
        if ( /(((?:Otls_|OPENtls_)?[A-Z0-9]{2,})_R_[A-Z0-9_]+)/ ) {
            next unless exists $errorfile{$2};
            $usedreasons{$1} = 1;
            if ( !exists $rcodes{$1} ) {
...
#ifndef OPENtls_${lib}ERR_H
# define OPENtls_${lib}ERR_H
...
EOF
    if ( $internal ) {
        ...
        if ($lib ne "tls" && $lib ne "ASYNC"
                && (grep { $lib eq uc $_ } @disablables, @disablables_int)) {
            print OUT <<"EOF";
# include <opentls/opentlsconf.h>

# ifndef OPENtls_NO_${lib}
...
EOF
        ...
        print OUT <<"EOF";
# define ${lib}err(f, r) ERR_${lib}_error(0, (r), OPENtls_FILE, OPENtls_LINE)
...
EOF
    ...
    print OUT "\n/*\n * $lib function codes.\n */\n";
    print OUT "# ifndef OPENtls_NO_DEPRECATED_3_0\n";
...
if ( $newstate )  {
    ...
    print OUT <<"EOF";
# Copyright 1999-$YEAR The Opentls Project Authors. All Rights Reserved.
#
# Licensed under the Apache License 2.0 (the "License").  You may not use
# this file except in compliance with the License.  You can obtain a copy
# in the file LICENSE in the source distribution or at
