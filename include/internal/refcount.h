...
{
	*ret = atomic_fetch_add_explicit(&refcnt->val, 1,
...
static inline int CRYPTO_DOWN_REF(CRYPTO_REF_COUNT *refcnt, int *ret)
{
     ...
     */
	*ret = atomic_fetch_sub_explicit(&refcnt->val, 1,
...
#if (defined(_M_ARM) && _M_ARM >= 7 && !defined(_WIN32_WCE)) || \
	defined(_M_ARM64)
#include <intrin.h>
...
static ossl_unused ossl_inline int CRYPTO_NEW_REF(CRYPTO_REF_COUNT *refcnt,
						  int n)
{
	refcnt->val = n;
...
#define REF_ASSERT_ISNT(test)                                               \
	(void)((test) ? (OPENSSL_die("refcount error", __FILE__, __LINE__), \
			 1) :                                               \
			0)
#else
