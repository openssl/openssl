...
#define evp_pkey_ctx_is_legacy(ctx) ((ctx)->keymgmt == NULL)
#define evp_pkey_ctx_is_provided(ctx) (!evp_pkey_ctx_is_legacy(ctx))
...
void *evp_skeymgmt_generate(const EVP_SKEYMGMT *skeymgmt,
			    const OSSL_PARAM params[]);
...
size_t evp_rand_get_seed(EVP_RAND_CTX *ctx, unsigned char **buffer, int entropy,
			 size_t min_len, size_t max_len,
			 int prediction_resistance, const unsigned char *adin,
			 size_t adin_len);
void evp_rand_clear_seed(EVP_RAND_CTX *ctx, unsigned char *buffer,
			 size_t b_len);
