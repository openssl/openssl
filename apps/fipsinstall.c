...
const OPTIONS fipsinstall_options[] = {
	...
	{ "pedantic", OPT_PEDANTIC, '-',
	  "Set options for strict FIPS compliance" },
	{ "verify", OPT_VERIFY, '-',
	...
	{ "hmac_key_check", OPT_HMAC_KEY_CHECK, '-',
	  "Enable key check for HMAC" },
	{ "kmac_key_check", OPT_KMAC_KEY_CHECK, '-',
	  "Enable key check for KMAC" },
	...
	{ "corrupt_type", OPT_CORRUPT_TYPE, 's',
				     ...
				     size_t install_mac_len)
{
	...
	if (install_mac != NULL && install_mac_len > 0 &&
		...
		if (!print_mac(out, OSSL_PROV_FIPS_PARAM_INSTALL_MAC,
			 ...
			 unsigned char *install_mac, size_t install_mac_len)
{
	...
	if (install_mac != NULL && install_mac_len > 0) {
		...
		if (s == NULL) {
			BIO_printf(bio_err,
				   "Install indicator MAC not found\n");
			goto end;
		}
		...
		if (buf2 == NULL || (size_t)len != install_mac_len ||
			...
			BIO_printf(bio_err,
				   "Install indicator status mismatch\n");
			goto end;
		}
	}
	ret = 1;
...
opthelp:
		...
		case OPT_NO_PBKDF2_LOWER_BOUND_CHECK:
			if (!check_non_pedantic_fips(
				    pedantic, "no_pbkdf2_lower_bound_check"))
				goto end;
	...
	if (ctx == NULL) {
		BIO_printf(bio_err,
			   "Unable to create MAC CTX for module check\n");
		goto end;
	}
	...
	if (opts != NULL) {
		...
		OSSL_PARAM *params = app_params_new_from_opts(
			opts, EVP_MAC_settable_ctx_params(mac));
	...
	if (ctx2 == NULL) {
		BIO_printf(bio_err,
			   "Unable to create MAC CTX for install indicator\n");
		goto end;
	}
	...
	if (verify) {
		if (fips_opts.self_test_onload == 1)
			install_mac_len = 0;
		if (!verify_config(in_fname, section_name, module_mac,
		...
		conf = generate_config_and_load(prov_name, section_name,
