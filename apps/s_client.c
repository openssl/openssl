...
#endif
	{ "dane_tlsa_domain", OPT_DANE_TLSA_DOMAIN, 's',
...
int s_client_main(int argc, char **argv)
{
	...
	char *chCApath = NULL, *chCAfile = NULL, *chCAstore = NULL,
	     *host = NULL;
	...
	int ret = 1, in_init = 1, i, nbio_test = 0, sock = -1, k, width,
	    state = 0;
	int sbuf_len, sbuf_off, cmdmode = USER_DATA_MODE_BASIC;
	int socket_family = AF_UNSPEC, socket_type = SOCK_STREAM, protocol = 0;
	int starttls_proto = PROTO_OFF, crl_format = FORMAT_UNDEF,
	    crl_download = 0;
...
/* Known false-positive of MemorySanitizer. */
	...
	SSL_CONF_CTX_set_flags(cctx,
			       SSL_CONF_FLAG_CLIENT | SSL_CONF_FLAG_CMDLINE);
...
opthelp:
		...
		case OPT_STATUS_OCSP_CHECK_LEAF:
			...
			X509_VERIFY_PARAM_set_flags(
				vpm, X509_V_FLAG_OCSP_RESP_CHECK);
			vpmtouched++;
			break;
		...
		case OPT_DANE_TLSA_RRDATA:
			...
			if (dane_tlsa_rrset == NULL ||
			    !sk_OPENSSL_STRING_push(dane_tlsa_rrset,
						    opt_arg())) {
				BIO_printf(bio_err,
					   "%s: Memory allocation failure\n",
					   prog);
		...
		case OPT_SERVERINFO:
			...
			for (start = 0, i = 0; i <= len; ++i) {
				if (i == len || p[i] == ',') {
					serverinfo_types[serverinfo_count] =
						atoi(p + start);
	...
	if (proxystr != NULL) {
		...
		res = BIO_parse_hostserv(proxystr, &host, &port,
					 BIO_PARSE_PRIO_HOST);
		if (tmp_host != host)
			OPENSSL_free(tmp_host);
		if (tmp_port != port)
	...
	if (ReqCAfile != NULL) {
		...
		if (nm == NULL ||
		    !SSL_add_file_cert_subjects_to_stack(nm, ReqCAfile)) {
	...
	if (alpn_in) {
		...
		/* Returns 0 on success! */
		if (SSL_CTX_set_alpn_protos(ctx, alpn,
	...
	for (i = 0; i < serverinfo_count; i++) {
		if (!SSL_CTX_add_client_custom_ext(
			    ctx, serverinfo_types[i], NULL, NULL, NULL,
	...
	if (!noservername && (servername != NULL || dane_tlsa_domain == NULL)) {
		if (servername == NULL) {
			if (host == NULL || is_dNS_name(host))
				servername = (host == NULL) ? "localhost" :
							      host;
		}
		if (servername != NULL &&
...
re_start:
	...
	if (proxystr != NULL) {
		...
		if (!OSSL_HTTP_proxy_connect(sbio, thost, tport, proxyuser,
					     proxypass, 0 /* no timeout */,
					     bio_err, prog))
			goto shut;
	}
#endif
	...
	case PROTO_MYSQL: {
		...
		if (bytes < 0) {
			...
			goto shut;
		} else if (bytes != (4 + packet[0] + (packet[1] << 8) +
				     (packet[2] << 16))) {
			BIO_printf(bio_err,
	...
	if (early_data_file != NULL &&
		...
		while (!finish) {
			...
			while (!SSL_write_early_data(con, cbuf, readbytes,
						     &writtenbytes)) {
	...
	for (;;) {
		...
		if (!SSL_is_init_finished(con) &&
		    SSL_total_renegotiations(con) == 0 &&
		...
		if (!write_ssl) {
			do {
				switch (user_data_process(&user_data, &cbuf_len,
							  &cbuf_off)) {
				default:
		...
		if (!ssl_pending) {
			...
			if ((!isquic && write_ssl) ||
			    (isquic &&
			     (first_loop || SSL_net_write_desired(con))))
				openssl_fdset(SSL_get_fd(con), &writefds);
#else
			if (!tty_on || !write_tty) {
				if ((!isquic && read_ssl) ||
				    (isquic && SSL_net_read_desired(con)))
					openssl_fdset(SSL_get_fd(con),
						      &readfds);
				if ((!isquic && write_ssl) ||
				    (isquic && (first_loop ||
	...
	defined(OPENSSL_SYS_VMS)
...
#endif
		{
			...
			ascii2ebcdic(&(sbuf[sbuf_off]), &(sbuf[sbuf_off]),
				     sbuf_len);
#endif
...
shut:
	if (ret > 0)
		ERR_print_errors(
			bio_err); /* show any errors accumulated so far */
	if (in_init)
		print_stuff(bio_c_out, con, full_log);
	do_ssl_shutdown(con);
	...
	if (ret > 0)
		ERR_print_errors(
			bio_err); /* show any new or remaining errors */
	if (con != NULL) {
		if (prexit != 0)
...
static void print_cert_key_info(BIO *bio, X509 *cert)
{
	...
	if (curve != NULL)
		BIO_printf(bio, "   a:PKEY: %s, (%s); sigalg: %s\n", keyalg,
			   curve, OBJ_nid2ln(X509_get_signature_nid(cert)));
	else
...
static void print_stuff(BIO *bio, SSL *s, int full)
{
	...
	if (full) {
		...
		if (sk != NULL) {
			...
			for (i = 0; i < sk_X509_num(sk); i++) {
				...
				X509_NAME_print_ex(
					bio, X509_get_subject_name(chain_cert),
					0, get_nameopt());
				BIO_puts(bio, "\n");
				BIO_printf(bio, "   i:");
				X509_NAME_print_ex(
					bio, X509_get_issuer_name(chain_cert),
					0, get_nameopt());
				BIO_puts(bio, "\n");
				print_cert_key_info(bio, chain_cert);
				BIO_printf(bio, "   v:NotBefore: ");
				ASN1_TIME_print(
					bio, X509_get0_notBefore(chain_cert));
		...
		if (SSL_get_negotiated_client_cert_type(s) ==
		    TLSEXT_cert_type_rpk)
			BIO_printf(
				bio,
				"Client-to-server raw public key negotiated\n");
		if (SSL_get_negotiated_server_cert_type(s) ==
		    TLSEXT_cert_type_rpk)
			BIO_printf(
				bio,
         ...
         */
		if (peer != NULL && !SSL_session_reused(s) &&
		    SSL_ct_is_enabled(s)) {
	...
	if (SSL_get_session(s) != NULL && keymatexportlabel != NULL) {
		...
		if (SSL_export_keying_material(
			    s, exportedkeymat, keymatexportlen,
...
static int ocsp_resp_cb(SSL *s, void *arg)
{
	...
	if (SSL_version(s) >= TLS1_3_VERSION) {
		...
		for (i = 0; i < num; i++)
			print_ocsp_response(arg,
					    sk_OCSP_RESPONSE_value(sk_resp, i));
	} else {
			     ...
			     size_t *off)
{
	...
	if (user_data->mode == USER_DATA_MODE_BASIC) {
		...
		case 'Q':
			...
			return user_data_execute(user_data, USER_COMMAND_QUIT,
						 NULL);

		case 'C':
			user_data->buflen = user_data->bufoff = *len = *off = 0;
			return user_data_execute(user_data,
						 USER_COMMAND_RECONNECT, NULL);

		case 'R':
			user_data->buflen = user_data->bufoff = *len = *off = 0;
			return user_data_execute(
				user_data, USER_COMMAND_RENEGOTIATE, NULL);

		case 'K':
		case 'k':
			user_data->buflen = user_data->bufoff = *len = *off = 0;
			return user_data_execute(
				user_data, USER_COMMAND_KEY_UPDATE,
		...
		if (cmd_start == buf_start) {
			...
			if (cmd_end == NULL) {
				...
				BIO_printf(
					...
					cmd_start);
				user_data->buflen = user_data->bufoff = *len =
					*off = 0;
				return USER_DATA_PROCESS_NO_DATA;
			}
			*cmd_end = '\0';
			...
			} else if (OPENSSL_strcasecmp(cmd_start, "reconnect") ==
				   0) {
			...
			if (SSL_version(user_data->con) == TLS1_3_VERSION) {
				if (OPENSSL_strcasecmp(cmd_start, "keyup") ==
				    0) {
			...
			if (cmd == -1) {
				...
				ret = user_data_execute(user_data, cmd,
							arg_start);
