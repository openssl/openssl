...
static void warn_deprecated(const FUNCTION *fp)
{
	...
	if (strcmp(fp->deprecated_alternative, DEPRECATED_NO_ALTERNATIVE) != 0)
		BIO_printf(bio_err, " Use '%s' instead.",
			   fp->deprecated_alternative);
	BIO_printf(bio_err, "\n");
}
				...
				int cmd, void *vdata)
{
	...
	case OSSL_TRACE_CTRL_BEGIN:
		...
		hex = OPENSSL_buf2hexstr((const unsigned char *)&tid,
					 sizeof(tid));
...
static void setup_trace_category(int category)
{
	...
	if (trace_data == NULL || bio == NULL ||
	    ...
	    OSSL_trace_set_callback(category, internal_trace_cb, trace_data) ==
		    0 ||
...
static void setup_trace(const char *str)
{
	...
	if (val != NULL) {
		...
		for (valp = val; (item = strtok(valp, ",")) != NULL;
		     valp = NULL) {
...
int main(int argc, char *argv[])
{
	...
	if (fp == NULL) {
		...
		global_help = argc > 1 && (strcmp(argv[1], "-help") == 0 ||
