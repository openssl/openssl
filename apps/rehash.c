		     ...
		     unsigned short old_id)
{
	...
	for (ep = bp->first_entry; ep; ep = ep->next) {
		if (digest &&
		    memcmp(digest, ep->digest, (size_t)evpmdsize) == 0) {
			BIO_printf(bio_err,
				   "%s: warning: skipping duplicate %s in %s\n",
				   opt_getprog(),
				   type == TYPE_CERT ? "certificate" : "CRL",
				   filename);
			return 0;
		}
...
static int do_file(const char *filename, const char *fullpath, enum Hash h)
{
	...
	if (name != NULL) {
		if (h == HASH_NEW || h == HASH_BOTH) {
			...
			if (ok) {
				errs += add_entry(type, hash_value, filename,
						  digest, 1, ~0);
			} else {
				BIO_printf(
					bio_err,
...
static int do_dir(const char *dirname, enum Hash h)
{
	...
	for (n = 0; n < numfiles; ++n) {
		...
		if (BIO_snprintf(buf, buflen, "%s%s%s", dirname, pathsep,
				 filename) >= buflen)
			continue;
		if (lstat(buf, &st) < 0)
			continue;
		...
		for (bp = hash_table[i]; bp; bp = nextbp) {
			...
			for (ep = bp->first_entry; ep; ep = nextep) {
				...
				if (ep->old_id < bp->num_needed) {
					...
					BIO_snprintf(buf, buflen, "%08x.%s%d",
						     bp->hash,
						     suffixes[bp->type],
						     ep->old_id);
					if (verbose)
