			     ...
			     const char *propq)
{
	...
	for (i = 0; params[i].key != NULL; ++i) {
		const char *name =
			param_datatype_2name(params[i].data_type, &ishex);
...
opthelp:
	...
	/* Fetch cipher, etc. */
	if (paramfile != NULL) {
		if (!init_keygen_file(&ctx, paramfile, e, libctx,
				      app_get0_propq()))
			goto end;
	}
	if (algname != NULL) {
		if (!init_gen_str(&ctx, algname, e, do_param, libctx,
				  app_get0_propq()))
			goto end;
	}
	if (ctx == NULL)
	...
	if (do_param) {
		...
		assert(private);
		rv = PEM_write_bio_PrivateKey(mem_out, pkey, cipher, NULL, 0,
					      NULL, pass);
	...
	if (ret != 0) {
		...
		if (mem_outpubkey != NULL) {
			rv = mem_bio_to_file(mem_outpubkey, outpubkeyfile,
					     outformat, private);
			if (!rv)
				BIO_printf(
					bio_err,
					"Error writing to outpubkey: '%s'. Error: %s\n",
					outpubkeyfile, strerror(errno));
		}
		if (mem_out != NULL) {
			rv = mem_bio_to_file(mem_out, outfile, outformat,
					     private);
			if (!rv)
				BIO_printf(
					bio_err,
					"Error writing to outfile: '%s'. Error: %s\n",
...
int init_gen_str(EVP_PKEY_CTX **pctx, const char *algname, ENGINE *e,
		 int do_param, OSSL_LIB_CTX *libctx, const char *propq)
{
	EVP_PKEY_CTX *ctx = NULL;
