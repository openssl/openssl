...
#define CMP_print(bio, level, prefix, msg, a1, a2, a3)                       \
	((void)(level > opt_verbosity ?                                      \
			...
			(BIO_printf(bio, "%s:%s:%d:CMP %s: " msg "\n", FUNC, \
				    OPENSSL_FILE, OPENSSL_LINE, prefix, a1,  \
				    a2, a3))))
...
static int set_verbosity(int level)
{
	if (level < OSSL_CMP_LOG_EMERG || level > OSSL_CMP_LOG_MAX) {
		CMP_err1("Logging verbosity level %d out of range (0 .. 8)",
			 level);
		return 0;
	}
	opt_verbosity = level;
					 ...
					 const OSSL_CMP_MSG *req)
{
	...
	if (opt_reqin != NULL && opt_rspin == NULL) {
		if ((req_new = read_PKIMESSAGE("actually sending",
					       &opt_reqin)) == NULL)
			goto err;
		/*-
	...
	if (opt_rspin != NULL) {
		...
		const OSSL_CMP_MSG *actual_req = req_new != NULL ? req_new :
								   req;
...
static OSSL_CMP_SRV_CTX *setup_srv_ctx(ENGINE *engine)
{
	...
	if (opt_srv_ref == NULL) {
		...
		if (!OSSL_CMP_CTX_set1_referenceValue(
			    ctx, (unsigned char *)opt_srv_ref,
	...
	if (opt_srv_secret != NULL) {
		int res;
		char *pass_str = get_passwd(opt_srv_secret,
	...
	if (!setup_cert(ctx, opt_srv_cert, opt_srv_keypass,
		...
		goto err;
	if (opt_srv_key != NULL) {
		EVP_PKEY *pkey = load_key_pwd(
			opt_srv_key, opt_keyform, opt_srv_keypass, engine,
	...
	if (opt_srv_trusted != NULL) {
		X509_STORE *ts = load_trusted(opt_srv_trusted, 0,
					      "certs trusted by mock server");
	...
	if (opt_rsp_cert == NULL) {
		...
		if (!setup_cert(
			    srv_ctx, opt_rsp_cert, opt_rsp_keypass,
	...
	if (opt_rsp_key != NULL) {
		EVP_PKEY *pkey =
			load_key_pwd(opt_rsp_key, opt_keyform, opt_rsp_keypass,
	...
	if (opt_failure != INT_MIN) { /* option has been set explicitly */
		if (opt_failure < 0 ||
		    OSSL_CMP_PKIFAILUREINFO_MAX < opt_failure) {
			CMP_err1(
	...
	if (!ossl_cmp_mock_srv_set_statusInfo(
		    srv_ctx, opt_pkistatus, opt_failurebits, opt_statusstring))
		goto err;
...
static int setup_verification_ctx(OSSL_CMP_CTX *ctx)
{
	...
	if (opt_srvcert != NULL || opt_trusted != NULL) {
		...
		if (opt_trusted != NULL) {
             ...
             */
			ts = load_trusted(opt_trusted, 0,
					  "certs trusted by client");
	...
	if (opt_out_trusted != NULL) { /* for use in OSSL_CMP_certConf_cb() */
		...
		X509_VERIFY_PARAM_clear_flags(out_vpm,
	...
	if (opt_disable_confirm)
		(void)OSSL_CMP_CTX_set_option(ctx, OSSL_CMP_OPT_DISABLE_CONFIRM,
					      1);
			      ...
			      ENGINE *engine)
{
	...
	if (opt_tls_cert != NULL && opt_tls_key != NULL) {
		...
		if (!load_cert_certs(
			    opt_tls_cert, &cert, &certs, 0, opt_tls_keypass,
		...
		if (!ok || !SSL_CTX_set0_chain(ssl_ctx, certs)) {
			CMP_err1(
				"unable to use client TLS certificate file '%s'",
				opt_tls_cert);
			OSSL_STACK_OF_X509_free(certs);
			goto err;
		}
...

		{
			...
			if (trust_store != NULL) {
				...
				X509_VERIFY_PARAM_clear_flags(
					tls_vpm, ~(X509_V_FLAG_USE_CHECK_TIME |
			...
			if (SSL_CTX_build_cert_chain(
				...
				CMP_warn(
					"could not build cert chain for own TLS cert");
			}
			if (trust_store != NULL)
		...
		if (opt_tls_extra != NULL) {
			...
			for (i = 0; i < sk_X509_num(tls_extra); i++) {
				...
				if (res != 0)
					res = SSL_CTX_add_extra_chain_cert(
						ssl_ctx, cert);
				if (res == 0)
					X509_free(cert);
			}
	...
	if (trust_store != NULL) {
         ...
         */
		if (!truststore_set_host_etc(trust_store, opt_tls_host != NULL ?
								  opt_tls_host :
								  host))
			goto err;
	}
...
{
	if (!opt_unprotected_requests && opt_secret == NULL &&
	    opt_key == NULL) {
	...
	if (opt_key != NULL) {
		EVP_PKEY *pkey =
			load_key_pwd(opt_key, opt_keyform, opt_keypass, engine,
	...
	if (opt_cert != NULL) {
		...
		if (!load_cert_certs(
			    opt_cert, &cert, &certs, 0, opt_keypass,
		...
		if (!ok) {
			...
			ok = ok && OSSL_CMP_CTX_build_cert_chain(
					   ctx, own_trusted, certs);
		}
	...
	if (opt_digest != NULL) {
		...
		if (!OSSL_CMP_CTX_set_option(ctx, OSSL_CMP_OPT_DIGEST_ALGNID,
					     digest) ||
		    !OSSL_CMP_CTX_set_option(ctx, OSSL_CMP_OPT_OWF_ALGNID,
					     digest)) {
	...
	if (opt_mac != NULL) {
		...
		if (mac == NID_undef) {
			CMP_err1("MAC algorithm name not recognized: '%s'",
				 opt_mac);
			return 0;
		}
		(void)OSSL_CMP_CTX_set_option(ctx, OSSL_CMP_OPT_MAC_ALGNID,
					      mac);
	}
	return 1;
}
...
static int set_fallback_pubkey(OSSL_CMP_CTX *ctx)
{
	...
	*end = bak;
	...
	if ((pubkey = OSSL_CMP_MSG_get0_certreq_publickey(req)) == NULL ||
		...
		CMP_err1(
			"failed to get fallback public key from ir/cr/kur file '%s'",
			file);
		goto err;
	}
	pkey1 = EVP_PKEY_dup(pkey);
	if (pkey == NULL ||
	    !OSSL_CMP_CTX_set0_newPkey(ctx, 0 /* priv */, pkey1)) {
		EVP_PKEY_free(pkey1);
		CMP_err1(
...
static int setup_request_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine)
{
	...
	if (opt_cmd == CMP_IR || opt_cmd == CMP_CR || opt_cmd == CMP_KUR) {
		if (opt_reqin == NULL && opt_newkey == NULL &&
		    !opt_centralkeygen && opt_key == NULL && opt_csr == NULL &&
		    opt_oldcert == NULL) {
		...
		if (opt_popo == OSSL_CRMF_POPO_NONE && !opt_centralkeygen) {
			CMP_info(
				"POPO is disabled, which implies -centralkeygen");
			opt_centralkeygen = 1;
		}
		if (opt_centralkeygen) {
			if (opt_popo > OSSL_CRMF_POPO_NONE) {
				CMP_err1(
					"-popo value %d is inconsistent with -centralkeygen",
					opt_popo);
				return 0;
			}
		...
		if (opt_cmd != CMP_P10CR) {
			if (opt_implicit_confirm)
				CMP_warn1("-implicit_confirm %s, and 'p10cr'",
					  msg);
			if (opt_disable_confirm)
				CMP_warn1("-disable_confirm %s, and 'p10cr'",
					  msg);
			if (opt_certout != NULL)
...
#define OSSL_CMP_RR_MSG "since -issuer and -serial is given for command 'rr'"
		...
		if (opt_serial != NULL) {
			...
			if ((sno = s2i_ASN1_INTEGER(NULL, opt_serial)) ==
			    NULL) {
		...
		if (opt_revreason > CRL_REASON_NONE)
			(void)OSSL_CMP_CTX_set_option(
				ctx, OSSL_CMP_OPT_REVOCATION_REASON,
				opt_revreason);
	} else {
		if (opt_serial != NULL)
			CMP_warn(
				"Ignoring -serial for command other than 'rr'");
	}
	...
	if (opt_recipient == NULL && opt_srvcert == NULL &&
	    opt_issuer == NULL && opt_oldcert == NULL && opt_cert == NULL)
		CMP_warn(
	...
	if (opt_cmd == CMP_P10CR || opt_cmd == CMP_RR || opt_cmd == CMP_GENM) {
		...
		bio_err =
			NULL; /* suppress diagnostics on first try loading key */
		...
		if (pkey == NULL ||
		    !OSSL_CMP_CTX_set0_newPkey(ctx, priv, pkey)) {
			EVP_PKEY_free(pkey);
			return 0;
		}
	...
	if (opt_csr != NULL) {
		if (opt_cmd == CMP_GENM) {
			...
			csr = load_csr_autofmt(opt_csr, FORMAT_UNDEF, NULL,
					       "PKCS#10 CSR");
			if (csr == NULL)
				return 0;
	...
	if (opt_reqexts != NULL || opt_policies != NULL) {
		...
		if (opt_reqexts != NULL &&
		    !X509V3_EXT_add_nconf_sk(conf, &ext_ctx, opt_reqexts,
					     &exts)) {
			CMP_err1(
				"cannot load certificate request extension section '%s'",
				opt_reqexts);
			goto exts_err;
		}
		if (opt_policies != NULL &&
		    !X509V3_EXT_add_nconf_sk(conf, &ext_ctx, opt_policies,
					     &exts)) {
			CMP_err1(
				"cannot load policy cert request extension section '%s'",
				opt_policies);
			goto exts_err;
		}
	...
	if (opt_san_nodefault) {
		...
		(void)OSSL_CMP_CTX_set_option(
			ctx, OSSL_CMP_OPT_SUBJECTALTNAME_NODEFAULT, 1);
	}
	...
	if (opt_policy_oids_critical) {
		...
		(void)OSSL_CMP_CTX_set_option(
			ctx, OSSL_CMP_OPT_POLICIES_CRITICAL, 1);
	}
	...
	while (opt_policy_oids != NULL) {
		...
		if ((policy = OBJ_txt2obj(opt_policy_oids, 1)) == 0) {
			CMP_err1("Invalid -policy_oids arg '%s'",
				 opt_policy_oids);
			return 0;
		}
		if (OBJ_obj2nid(policy) == NID_undef)
			CMP_warn1("Unknown -policy_oids arg: %.40s",
				  opt_policy_oids);
		...
		if (!OSSL_CMP_CTX_push0_policy(ctx, pinfo)) {
			CMP_err1("cannot add policy with OID '%s'",
				 opt_policy_oids);
	...
	if (opt_oldcert != NULL) {
		if (opt_cmd == CMP_GENM) {
			CMP_warn(
				"-oldcert option is ignored for 'genm' command");
		} else {
			if (!setup_cert(
				    ctx, opt_oldcert, opt_keypass,
	...
	do {
		...
		*ptr++ = '\0';
		...
		if (CHECK_AND_SKIP_CASE_PREFIX(ptr, "int:")) {
			...
			if (end == ptr) {
				CMP_err1(
					"Cannot parse int in -geninfo arg %.40s",
					ptr);
				goto err;
			}
			ptr = end;
			...
			CMP_err1(
				"Missing 'int:' or 'str:' in -geninfo arg %.40s",
				ptr);
			goto err;
		}
...
static int setup_client_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine)
{
	...
	if (!opt_use_mock_srv &&
	    opt_rspin == NULL) { /* note: -port is not given */
#if !defined(OPENSSL_NO_SOCK) && !defined(OPENSSL_NO_HTTP)
	...
	if (opt_server == NULL) {
		if (opt_proxy != NULL)
			CMP_warn(
				"ignoring -proxy option since -server is not given");
		if (opt_no_proxy != NULL)
			CMP_warn(
				"ignoring -no_proxy option since -server is not given");
		goto set_path;
	}
	...
	if (use_ssl && !opt_tls_used) {
		CMP_warn(
			"assuming -tls_used since -server URL indicates HTTPS");
		opt_tls_used = 1;
	}
	...
	if (opt_proxy != NULL && !OSSL_CMP_CTX_set1_proxy(ctx, opt_proxy))
		goto oom;
	if (opt_no_proxy != NULL &&
	    !OSSL_CMP_CTX_set1_no_proxy(ctx, opt_no_proxy))
		goto oom;
	...
	proxy_host =
		OSSL_HTTP_adapt_proxy(opt_proxy, opt_no_proxy, host, use_ssl);
	if (proxy_host != NULL)
		(void)BIO_snprintf(proxy_buf, sizeof(proxy_buf), " via %s",
				   proxy_host);
...
set_path:
	...
	if (opt_infotype_s == NULL) {
		...
		CMP_warn(
			"-infotype option is ignored for commands other than 'genm'");
	} else {
		char id_buf[100] = "id-it-";

		strncat(id_buf, opt_infotype_s,
			sizeof(id_buf) - strlen(id_buf) - 1);
	...
	if (opt_total_timeout > 0 && opt_msg_timeout > 0 &&
		...
		CMP_err2(
			"-total_timeout argument = %d must not be < %d (-msg_timeout)",
			opt_total_timeout, opt_msg_timeout);
		goto err;
	}
	...
	if (opt_reqin_new_tid && opt_reqin == NULL)
		CMP_warn(
			"-reqin_new_tid is ignored since -reqin is not present");
	...
	if (opt_tls_used) {
		...
		info->timeout =
			OSSL_CMP_CTX_get_option(ctx, OSSL_CMP_OPT_MSG_TIMEOUT);
	...
	if (opt_reqout_only == NULL)
		CMP_info3(
			"will contact %s%s%s ", server_buf, proxy_buf,
			opt_rspin == NULL ?
				"" :
			   ...
			   const char *desc)
{
	...
	for (i = 0; i < n; i++) {
		if (!write_cert(bio, sk_X509_value(certs, i))) {
			CMP_err2("cannot write %s certificate to file '%s'",
				 desc, file);
			n = -1;
			goto end;
		}
...
static int save_cert_or_delete(X509 *cert, const char *file, const char *desc)
{
	...
	if (cert == NULL) {
		...
		BIO_snprintf(desc_cert, sizeof(desc_cert), "%s certificate",
			     desc);
...
static int save_crl_or_delete(X509_CRL *crl, const char *file, const char *desc)
{
	if (file == NULL)
		return 1;
	return (crl == NULL) ? delete_file(file, desc) :
			       save_crl(crl, file, desc);
}
...
static int save_template(const char *file, const OSSL_CRMF_CERTTEMPLATE *tmpl)
{
	...
	if (bio == NULL) {
		CMP_err1(
			"error saving certTemplate from genp: cannot open file %s",
			file);
		return 0;
	}
	if (!ASN1_i2d_bio_of(OSSL_CRMF_CERTTEMPLATE, i2d_OSSL_CRMF_CERTTEMPLATE,
			     bio, tmpl)) {
		CMP_err1(
			"error saving certTemplate from genp: cannot write file %s",
			file);
		BIO_free(bio);
		return 0;
	} else {
...
static int save_keyspec(const char *file, const OSSL_CMP_ATAVS *keyspec)
{
	...
	if (bio == NULL) {
		CMP_err1("error saving keySpec from genp: cannot open file %s",
			 file);
		return 0;
	}

	if (!ASN1_i2d_bio_of(OSSL_CMP_ATAVS, i2d_OSSL_CMP_ATAVS, bio,
			     keyspec)) {
		CMP_err1("error saving keySpec from genp: cannot write file %s",
			 file);
		BIO_free(bio);
		return 0;
	} else {
			     ...
			     const char *name)
{
	...
	while ((end = prev_item(groups, end)) != NULL) {
		if ((res = app_conf_try_string(src_conf, opt_item, name)) !=
		    NULL)
			return res;
	}
	return res;
...
static int read_config(void)
{
	...
	OPENSSL_assert(OSSL_NELEM(cmp_vars) ==
		       n_options + OPT_PROV__FIRST + 1 - OPT_PROV__LAST +
			       OPT_R__FIRST + 1 - OPT_R__LAST + OPT_V__FIRST +
			       1 - OPT_V__LAST);
	for (opt = &cmp_options[start_opt], i = start_idx; opt->name != NULL;
	     i++, opt++) {
		...
		case 'N':
		case 'l':
			if (!conf_get_number_e(conf, opt_section, opt->name,
					       &num)) {
				ERR_clear_error();
		...
		default:
			CMP_err2(
				"internal: unsupported type '%c' for option '%s'",
		...
		if (provider_option || verification_option) {
			...
			if (opt->valtype == '-') {
				...
				conf_argc = 3;
				conf_argv[2] = conf_get_string(
					conf, opt_section, opt->name);
				/* not NULL */
			}
			if (conf_argc > 1) {
				...
				if (provider_option ?
					...
					CMP_err2(
						"for option '%s' in config file section '%s'",
...
static char *opt_str(void)
{
	...
	if (arg[0] == '\0') {
		CMP_warn1(
			"%s option argument is empty string, resetting option",
			opt_flag());
		arg = NULL;
	} else if (arg[0] == '-') {
...
static int cmp_server(OSSL_CMP_CTX *srv_cmp_ctx)
{
	...
	while (opt_max_msgs <= 0 || msgs < opt_max_msgs) {
		...
		if (req != NULL) {
			if (strcmp(path, "") != 0 &&
			    strcmp(path, "pkix/") != 0) {
				(void)http_server_send_status(prog, cbio, 404,
							      "Not Found");
				CMP_err1(
					"expecting empty path or 'pkix/' but got '%s'",
					path);
				OPENSSL_free(path);
				OSSL_CMP_MSG_free(req);
				goto next;
...
next:
		if (!ret) { /* on transmission error, cancel CMP transaction */
			(void)OSSL_CMP_CTX_set1_transactionID(srv_cmp_ctx,
							      NULL);
			(void)OSSL_CMP_CTX_set1_senderNonce(srv_cmp_ctx, NULL);
		}
		if (!ret || !keep_alive ||
		    OSSL_CMP_CTX_get_status(srv_cmp_ctx) !=
...
static void print_keyspec(OSSL_CMP_ATAVS *keySpec)
{
	...
	for (i = 0; i < sk_OSSL_CMP_ATAV_num(keySpec); i++) {
		...
		ASN1_OBJECT *type =
			OSSL_CMP_ATAV_get0_type(atav /* may be NULL */);
...
static void print_status(void)
{
	...
	const char *string = OSSL_CMP_CTX_snprint_PKIStatus(
		cmp_ctx, buf, OSSL_CMP_PKISI_BUFLEN);
...
static int do_genm(OSSL_CMP_CTX *ctx)
{
	if (opt_infotype == NID_id_it_caCerts) {
		...
		if (cacerts == NULL) {
			...
			CMP_err1(
				"Failed to store CA certificates from genp in %s",
				opt_cacertsout);
			return 0;
		}
		return 1;
...
end_upd:
		...
		if (opt_crlcert != NULL) {
			crlcert = load_cert_pwd(
				opt_crlcert, opt_otherpass,
		...
		if (opt_oldcrl != NULL && opt_crlcert != NULL) {
			if (X509_NAME_cmp(X509_CRL_get_issuer(oldcrl),
				...
				CMP_warn(
					"-oldcrl and -crlcert have different issuer");
		}
...
end_crlupd:
		...
		if (!OSSL_CMP_get1_certReqTemplate(ctx, &certTemplate,
						   &keySpec))
			return 0;

		if (certTemplate == NULL) {
			CMP_warn("no certificate request template available");
			if (!delete_file(opt_template,
					 "certTemplate from genp"))
				return 0;
			if (opt_keyspec != NULL &&
...
tmpl_end:
		...
		if (opt_infotype != NID_undef) {
			CMP_warn1(
				"No specific support for -infotype %s available",
				opt_infotype_s);

			req = OSSL_CMP_ITAV_create(OBJ_nid2obj(opt_infotype),
						   NULL);
			if (req == NULL ||
			    !OSSL_CMP_CTX_push0_genm_ITAV(ctx, req)) {
				CMP_err1(
					"Failed to create genm for -infotype %s",
					opt_infotype_s);
				return 0;
			}
		}
...
static int handle_opts_upfront(int argc, char **argv)
{
	...
	for (i = 1; i < argc - 1; i++) {
		if (*argv[i] == '-') {
			if (!strcmp(argv[i] + 1,
				    cmp_options[OPT_CONFIG - OPT_HELP].name))
				opt_config = argv[++i];
			else if (!strcmp(argv[i] + 1,
					 cmp_options[OPT_SECTION - OPT_HELP]
						 .name))
				opt_section = argv[++i];
			else if (strcmp(argv[i] + 1,
					cmp_options[OPT_VERBOSITY - OPT_HELP]
						.name) == 0 &&
...
int cmp_main(int argc, char **argv)
{
	...
	if (configfile != NULL && configfile[0] != '\0' /* non-empty string */
		...
		CMP_info2(
			"using section(s) '%s' of OpenSSL configuration file '%s'",
			opt_section, configfile);
		conf = app_load_config(configfile);
		if (conf == NULL) {
			goto err;
		} else {
			if (strcmp(opt_section, CMP_SECTION) ==
			    0) { /* default */
				...
				const char *end =
					opt_section + strlen(opt_section);

				while ((end = prev_item(opt_section, end)) !=
				       NULL) {
			...
			ret = read_config();
			if (!set_verbosity(
				    opt_verbosity)) /* just for checking range */
				ret = -1;
			if (ret <= 0) {
				if (ret == -1)
					BIO_printf(bio_err,
	...
	if (opt_engine != NULL) {
		engine = setup_engine_methods(
			opt_engine, 0 /* not: ENGINE_METHOD_ALL */, 0);
		if (engine == NULL) {
	...
	if (!OSSL_CMP_CTX_set_log_cb(cmp_ctx, print_to_bio_out)) {
		CMP_err1("cannot set up error reporting and logging for %s",
			 prog);
		goto err;
	}
	...
	if (opt_tls_cert == NULL && opt_tls_key == NULL &&
		...
		if (opt_tls_used)
			CMP_warn(
				"-tls_used given without any other TLS options");
	} else if (!opt_tls_used) {
		CMP_warn(
			"ignoring TLS options(s) since -tls_used is not given");
	}
	if (opt_port != NULL) {
		if (opt_tls_used) {
	...
	if (opt_ignore_keyusage)
		(void)OSSL_CMP_CTX_set_option(cmp_ctx,
					      OSSL_CMP_OPT_IGNORE_KEYUSAGE, 1);
	if (opt_no_cache_extracerts)
		(void)OSSL_CMP_CTX_set_option(
			cmp_ctx, OSSL_CMP_OPT_NO_CACHE_EXTRACERTS, 1);
	...
	if (opt_reqout_only == NULL && (opt_use_mock_srv
		...
		if (!OSSL_CMP_CTX_set_log_cb(srv_cmp_ctx, print_to_bio_err)) {
			CMP_err1(
				"cannot set up error reporting and logging for %s",
				prog);
			goto err;
		}
	...
	for (i = 0; i < opt_repeat; i++) {
		...
		if (OSSL_CMP_CTX_get_status(cmp_ctx) <
		...
		print_status();
		if (!save_cert_or_delete(
			    OSSL_CMP_CTX_get0_validatedSrvCert(cmp_ctx),
		...
		if (newcert != NULL &&
			...
			if (opt_centralkeygen) {
				...
				BIO *out;
				int result = 1;
				EVP_PKEY *new_key = OSSL_CMP_CTX_get0_newPkey(
					cmp_ctx, 1 /* priv */);

				if (new_key == NULL)
					goto err;
				if ((out = bio_open_owner(opt_newkeyout,
							  FORMAT_PEM, 1)) ==
				    NULL)
					goto err;
				if (opt_newkeypass != NULL) {
				...
				CMP_info1(
					"saving centrally generated key to file '%s'",
					opt_newkeyout);
				if (PEM_write_bio_PrivateKey(
					    out, new_key, cipher, NULL, 0, NULL,
					    (void *)pass_string) <= 0)
					result = 0;
...
err:
	...
	if (cmp_ctx != NULL) {
		...
		ossl_cmp_mock_srv_free(
			OSSL_CMP_CTX_get_transfer_cb_arg(cmp_ctx));
	...
	NCONF_free(
		conf); /* must not do as long as opt_... variables are used */
	OSSL_CMP_log_close();
