        ...
        if ((flags & CMS_VERIFY_PARTIAL) != 0) {
            ...
            for (i = 0; i < sk_CMS_SignerInfo_num(sinfos); i++) {
                ...
                BIO_printf(bio_err, "\n  Verification %s (cert: %s, attrs: %s, content: %s)\n",
                    ...
                    CMS_SignerInfo_get_verification_result(si, CMS_VERIFY_CERT) ? "success" : "failure",
                    CMS_SignerInfo_get_verification_result(si, CMS_VERIFY_ATTR) ? "success" : "failure",
                    CMS_SignerInfo_get_verification_result(si, CMS_VERIFY_CONTENT) ? "success" : "failure");
            }
        }
        if (signerfile != NULL) {
