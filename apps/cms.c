...
const OPTIONS cms_options[] = {
	...
	{ "digest", OPT_DIGEST, 's',
	  "Sign a pre-computed digest in hex notation" },
	...
	{ "recip_ukm", OPT_RECIP_UKM, 's',
	  "KEMRecipientInfo user keying material for current recipient, in hex notation" },
...
int cms_main(int argc, char **argv)
{
	...
	int verify_retcode = 0, rctformat = FORMAT_SMIME,
...
opthelp:
		...
		case OPT_RECIP_UKM:
			...
			if (o == OPT_RECIP_KDF) {
				...
				if (recip_opt->ukm_data == NULL) {
					BIO_printf(
						bio_err,
						"Invalid hex value after -recip_ukm\n");
					goto end;
				}
			}
			break;
		...
		case OPT_KEYOPT:
			...
			if (key_param == NULL || key_param->idx != keyidx) {
				...
				nparam = app_malloc(sizeof(*nparam),
						    "key param buffer");
				if ((nparam->param =
					     sk_OPENSSL_STRING_new_null()) ==
				    NULL) {
			...
			if (sk_OPENSSL_STRING_push(key_param->param,
						   opt_arg()) <= 0)
				goto end;
			break;
		case OPT_V_CASES:
	...
	if ((flags & CMS_CADES) != 0) {
		...
		if (operation == SMIME_VERIFY &&
		    (flags &
		     (CMS_NO_SIGNER_CERT_VERIFY | CMS_NO_ATTR_VERIFY)) != 0) {
			BIO_puts(
				bio_err,
	...
	if (operation == SMIME_DATA_CREATE) {
		...
		for (i = 0; i < sk_X509_num(encerts); i++) {
			...
			ri = CMS_add1_recipient(cms, x, key, originator,
						tflags);
			if (ri == NULL)
				goto end;
			...
			if (wrap_cipher != NULL) {
				...
				if (ri_type == CMS_RECIPINFO_AGREE)
					wctx = CMS_RecipientInfo_kari_get0_ctx(
						ri);
				else if (ri_type == CMS_RECIPINFO_KEM)
					wctx = CMS_RecipientInfo_kemri_get0_ctx(
						ri);
				if (wctx != NULL) {
			...
			if (ropt != NULL && ri_type == CMS_RECIPINFO_KEM) {
				...
				if (ropt->kdf != NULL) {
					...
					if (!X509_ALGOR_set0(kdf_algo, kdf_obj,
							     V_ASN1_UNDEF,
							     NULL))
						goto end;
				}
		...
		if (pwri_pass != NULL) {
			pwri_tmp = (unsigned char *)OPENSSL_strdup(
				(char *)pwri_pass);
			if (pwri_tmp == NULL)
				goto end;
			if (CMS_add0_recipient_password(cms, -1, NID_undef,
							NID_undef, pwri_tmp, -1,
							kekcipher) == NULL)
				goto end;
			pwri_tmp = NULL;
		}
		...
		for (i = 0; i < sk_OPENSSL_STRING_num(sksigners); i++) {
			...
			if (rr != NULL && !CMS_add1_ReceiptRequest(si, rr)) {
				BIO_puts(bio_err,
					 "Error adding CMS ReceiptRequest\n");
				goto end;
			}
			X509_free(signer);
			signer = NULL;
		...
		if (operation == SMIME_SIGN && (flags & CMS_STREAM) == 0) {
			...
			if (digestbin !=
				...
				res = CMS_final_digest(cms, digestbin,
	...
	if (operation == SMIME_DECRYPT) {
		...
		if (key != NULL) {
			if (!CMS_decrypt_set1_pkey_and_peer(cms, key, recip,
							    originator)) {
				BIO_puts(
					bio_err,
		...
		if (noout) {
			if (print) {
				...
				if (get_nameopt() != XN_FLAG_ONELINE) {
					pctx = ASN1_PCTX_new();
					if (pctx !=
					    NULL) { /* Print anyway if malloc failed */
						ASN1_PCTX_set_flags(
							pctx,
			...
			if (to != NULL &&
			    BIO_printf(out, "To: %s%s", to, mime_eol) < 0)
				goto end;
			if (from != NULL &&
			    BIO_printf(out, "From: %s%s", from, mime_eol) < 0)
				goto end;
			if (subject != NULL &&
			    BIO_printf(out, "Subject: %s%s", subject,
				       mime_eol) < 0)
				goto end;
			      ...
			      STACK_OF(OPENSSL_STRING) *param)
{
	...
	for (i = 0; i < sk_OPENSSL_STRING_num(param); i++) {
		...
		if (pkey_ctrl_string(pctx, keyopt) <= 0) {
			BIO_printf(bio_err,
				   "Error setting CMS key parameter \"%s\"\n",
				   keyopt);
			return 0;
		}
	}
