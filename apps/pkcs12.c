...
int dump_certs_pkeys_bag(BIO *out, const PKCS12_SAFEBAG *bags, const char *pass,
			 int passlen, int options, char *pempass,
			 const EVP_CIPHER *enc);
...
const OPTIONS pkcs12_options[] = {
	...
	{ "pbmac1_pbkdf2", OPT_PBMAC1_PBKDF2, '-',
	  "Use PBMAC1 with PBKDF2 instead of MAC" },
	{ "pbmac1_pbkdf2_md", OPT_PBMAC1_PBKDF2_MD, 's',
	  "Digest to use for PBMAC1 KDF (default SHA256)" },
	{ "iter", OPT_ITER, 'p',
	...
	{ "jdktrust", OPT_JDKTRUST, 's',
	  "Mark certificate in PKCS#12 store as trusted for JDK compatibility" },
	{ NULL }
};
...
opthelp:
	...
	if (export_pkcs12) {
		...
		if (!(options & NOKEYS)) {
			key = load_key(keyname ? keyname : infile, FORMAT_PEM,
				       1, passin, e,
				       keyname ?
			...
			if (key != NULL) {
				...
				for (i = 0; i < sk_X509_num(certs); i++) {
					...
					if (cert_matches_key(x, key)) {
						...
						X509_keyid_set1(ee_cert, NULL,
								0);
						X509_alias_set1(ee_cert, NULL,
								0);
						/* Remove from list */
		...
		if (untrusted != NULL) {
			if (!load_certs(untrusted, 0, &untrusted_certs,
					passcerts, "untrusted certificates"))
				goto export_end;
		}
		...
		if (chain) {
			...
			if ((store = setup_verify(CAfile, noCAfile, CApath,
						  noCApath, CAstore,
						  noCAstore)) == NULL)
				goto export_end;

			vret = get_cert_chain(ee_cert_tmp, store,
					      untrusted_certs, &chain2);
		...
		if (maciter != -1) {
			if (pbmac1_pbkdf2 == 1) {
				if (!PKCS12_set_pbmac1_pbkdf2(
					    p12, mpass, -1, NULL, macsaltlen,
					    maciter, macmd, pbmac1_pbkdf2_md)) {
					BIO_printf(bio_err,
						   "Error creating PBMAC1\n");
					goto export_end;
				}
			} else {
				if (!PKCS12_set_mac(p12, mpass, -1, NULL,
						    macsaltlen, maciter,
						    macmd)) {
					BIO_printf(
						bio_err,
...
export_end:
	...
	p12 = PKCS12_init_ex(NID_pkcs7_data, app_get0_libctx(),
			     app_get0_propq());
	if (p12 == NULL) {
		ERR_print_errors(bio_err);
		goto end;
	...
	if ((options & INFO) && PKCS12_mac_present(p12)) {
		...
		if (OBJ_obj2nid(macobj) == NID_pbmac1) {
			PBKDF2PARAM *pbkdf2_param =
				PBMAC1_get1_pbkdf2_param(macalgid);

			if (pbkdf2_param == NULL) {
				BIO_printf(
					bio_err,
					", Unsupported KDF or params for PBMAC1\n");
			} else {
				...
				BIO_printf(
					bio_err,
					" using PBKDF2, Iteration %ld\n",
					ASN1_INTEGER_get(pbkdf2_param->iter));
				BIO_printf(bio_err,
					   "Key length: %ld, Salt length: %d\n",
					   ASN1_INTEGER_get(
						   pbkdf2_param->keylength),
					   ASN1_STRING_length(
						   pbkdf2_param->salt->value
							   .octet_string));
				if (pbkdf2_param->prf == NULL) {
					prfnid = NID_hmacWithSHA1;
				} else {
					X509_ALGOR_get0(&prfobj, NULL, NULL,
							pbkdf2_param->prf);
			...
			BIO_printf(bio_err, ", Iteration %ld\n",
				   tmaciter != NULL ?
					   ...
					   1L);
			BIO_printf(
				bio_err, "MAC length: %ld, salt length: %ld\n",
	...
	if (macver) {
		...
		if (OBJ_obj2nid(macobj) != NID_pbmac1) {
			...
			pkcs12kdf = EVP_KDF_fetch(app_get0_libctx(),
						  "PKCS12KDF",
						  app_get0_propq());
			if (pkcs12kdf == NULL) {
				BIO_printf(
...
static int jdk_trust(PKCS12_SAFEBAG *bag, void *cbarg)
{
	...
	attr = X509_ATTRIBUTE_create(NID_oracle_jdk_trustedkeyusage,
				     V_ASN1_OBJECT, (ASN1_OBJECT *)cbarg);
			...
			const EVP_CIPHER *enc)
{
	...
	for (i = 0; i < sk_PKCS7_num(asafes); i++) {
		...
		if (bagnid == NID_pkcs7_data) {
			...
			if (options & INFO) {
				...
				if (p7->d.encrypted == NULL) {
					...
					alg_print(p7->d.encrypted->enc_data
							  ->algorithm);
				}
			}
		...
		if (bags == NULL)
			goto err;
		if (!dump_certs_pkeys_bags(out, bags, pass, passlen, options,
					   pempass, enc)) {
			  ...
			  char *pempass, const EVP_CIPHER *enc)
{
	...
	for (i = 0; i < sk_PKCS12_SAFEBAG_num(bags); i++) {
		if (!dump_certs_pkeys_bag(out, sk_PKCS12_SAFEBAG_value(bags, i),
...
int dump_certs_pkeys_bag(BIO *out, const PKCS12_SAFEBAG *bag, const char *pass,
			 int passlen, int options, char *pempass,
			 const EVP_CIPHER *enc)
{
	EVP_PKEY *pkey;
	...
	case NID_keyBag:
		...
		if ((pkey = EVP_PKCS82PKEY(p8c)) == NULL)
			return 0;
		print_attribs(out, PKCS8_pkey_get0_attrs(p8c),
			      "Key Attributes");
		ret = PEM_write_bio_PrivateKey(out, pkey, enc, NULL, 0, NULL,
					       pempass);
		EVP_PKEY_free(pkey);
		break;
	...
	case NID_safeContentsBag:
		...
		print_attribs(out, attrs, "Bag Attributes");
		return dump_certs_pkeys_bags(out,
...
static int alg_print(const X509_ALGOR *alg)
{
	...
	if (pbenid == NID_pbes2) {
		...
		if (pbenid == NID_id_pbkdf2) {
			...
			int prfnid;
			if (aparamtype == V_ASN1_SEQUENCE)
				kdf = ASN1_item_unpack(
					aparam, ASN1_ITEM_rptr(PBKDF2PARAM));
			if (kdf == NULL) {
				BIO_puts(bio_err, ", <unsupported parameters>");
				goto done;
			}
			...
			if (aparamtype == V_ASN1_SEQUENCE)
				kdf = ASN1_item_unpack(
					aparam, ASN1_ITEM_rptr(SCRYPT_PARAMS));
			if (kdf == NULL) {
				BIO_puts(bio_err, ", <unsupported parameters>");
				goto done;
			}
