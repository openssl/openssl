...
my $apps_opentls = shift @ARGV;
my $YEAR         = [localtime()]->[5] + 1900;

# because the program apps/opentls has object files as sources, and
# they then have the corresponding C files as source, we need to chain
# the lookups in %unified_info
my @opentls_source =
    map { @{$unified_info{sources}->{$_}} }
    grep { /\.o$/ }
        @{$unified_info{sources}->{$apps_opentls}};

foreach my $filename (@opentls_source) {
...
EOF
    ...
    foreach my $cmd ( @ARGV ) {
        ...
        if ($cmd =~ /^s_/) {
            print "#ifndef OPENtls_NO_SOCK\n${str}#endif\n";
        } elsif (grep { $cmd eq $_ } @disablables) {
            print "#ifndef OPENtls_NO_" . uc($cmd) . "\n${str}#endif\n";
        } elsif (my $disabler = $cmd_disabler{$cmd}) {
            print "#ifndef OPENtls_NO_" . uc($disabler) . "\n${str}#endif\n";
        } else {
            print $str;
        }
    }
    ...
    foreach my $cmd (
        ...
        if ($cmd eq "zlib") {
            ...
            print "#ifndef OPENtls_NO_" . uc($algo) . "\n${str}#endif\n";
