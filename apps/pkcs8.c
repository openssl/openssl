...
opthelp:
		...
		case OPT_V2PRF:
			pbe_nid = OBJ_txt2nid(opt_arg());
			if (!EVP_PBE_find(EVP_PBE_TYPE_PRF, pbe_nid, NULL, NULL,
					  0)) {
				BIO_printf(bio_err,
		...
		if (nocrypt) {
			...
			if (cipher) {
				...
				if (scrypt_N && scrypt_r && scrypt_p)
					pbe = PKCS5_pbe2_set_scrypt(
						cipher, NULL, saltlen, NULL,
						scrypt_N, scrypt_r, scrypt_p);
				else
#endif
					pbe = PKCS5_pbe2_set_iv(cipher, iter,
								NULL, saltlen,
								NULL, pbe_nid);
			} else {
				pbe = PKCS5_pbe_set(pbe_nid, iter, NULL,
						    saltlen);
			}
			if (pbe == NULL) {
				BIO_printf(bio_err,
			...
			p8 = PKCS8_set0_pbe(p8pass, (int)strlen(p8pass), p8inf,
					    pbe);
			if (p8 == NULL) {
				X509_ALGOR_free(pbe);
	...
	if (outformat == FORMAT_PEM) {
		if (traditional)
			PEM_write_bio_PrivateKey_traditional(
				out, pkey, NULL, NULL, 0, NULL, passout);
		else
