...
static int query_command(const char *data, const char *digest, const EVP_MD *md,
			 ...
			 const char *in, const char *out, int text);
...
static int reply_command(CONF *conf, const char *section, const char *engine,
			 ...
			 const char *signer, const char *chain,
...
static TS_RESP *create_response(CONF *conf, const char *section,
				const char *engine, const char *queryfile,
				const char *passin, const char *inkey,
...
static int verify_command(const char *data, const char *digest,
			  const char *queryfile, const char *in, int token_in,
...
opthelp:
	...
	if (mode == OPT_ERR) {
		BIO_printf(bio_err,
			   "%s: Must give one of -query, -reply, or -verify\n",
			   prog);
		goto opthelp;
	}
	...
	if (mode == OPT_QUERY) {
		...
		ret = !reply_command(conf, section, engine, queryfile, password,
				     inkey, md, signer, chain, policy, in,
				     token_in, out, token_out, text);
	...
	if (in != NULL) {
		...
		if (digest == NULL &&
		    (data_bio = bio_open_default(data, 'r', FORMAT_ASN1)) ==
			    NULL)
			goto end;
		query = create_query(data_bio, digest, md, policy, no_nonce,
				     cert);
	}
	if (query == NULL)
		goto end;
			  ...
			  X509_VERIFY_PARAM *vpm)
{
	...
	if ((verify_ctx = create_verify_ctx(data, digest, queryfile, CApath,
					    CAfile, CAstore, untrusted, vpm)) ==
	    NULL)
		goto end;

	ret = token_in ? TS_RESP_verify_token(verify_ctx, token) :
					...
					X509_VERIFY_PARAM *vpm)
{
	...
	if (data != NULL || digest != NULL) {
		...
		if (data != NULL) {
			...
			unsigned char *hexstr =
				OPENSSL_hexstr2buf(digest, &imprint_len);
			f |= TS_VFY_IMPRINT;
			if (!TS_VERIFY_CTX_set0_imprint(ctx, hexstr,
							imprint_len)) {
	...
	if (untrusted != NULL) {
		certs = load_certs_multifile(untrusted, NULL,
					     "extra untrusted certs", vpm);
