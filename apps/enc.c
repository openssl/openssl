...
int enc_main(int argc, char **argv)
{
 ...
 opthelp:
    ...
    if (cipher != NULL) {
        ...
        /* if decrypting, try to extract or skip any heading salt */
        if(enc == 0) {
            inl = BIO_read(rbio, mbuf, sizeof(mbuf));
            if(inl < 1) {
                BIO_printf(bio_err, "error reading input file\n");
                goto end;
            }
            if (inl == sizeof(mbuf) /* Is file salted ? */
                && memcmp(mbuf, magic, sizeof(mbuf)) == 0) {
                ...
                if (hsalt == NULL && str != NULL) { /* NO salt at all */
            ...
            if (nosalt) {
                ...
                if (enc) {  /* encryption */
                    if (hsalt == NULL) {
                         ...
                         * (and only write the salt when it was generated, i.e.
