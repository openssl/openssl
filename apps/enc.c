...
# endif
    ...
    if (cipher != NULL) {
         ...
         * (probably password-derived) or an opaque symmetric key.
         * We do not allow both options simultaneously.
         */
        if (rawkey_set > 0 && skeyopts != NULL) {
            BIO_printf(bio_err, "You should use either a raw key (probably password-derived) "
                       "or the 'skeyopt' args.\n");
            goto end;
        }
