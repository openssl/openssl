...
const OPTIONS enc_options[] = {
	...
	{ "saltlen", OPT_SALTLEN, 'p',
	  "Specify the PBKDF2 salt length (in bytes)" },
...
opthelp:
		...
		case OPT_SKEYOPT:
			if ((skeyopts == NULL &&
			     (skeyopts = sk_OPENSSL_STRING_new_null()) ==
				     NULL) ||
			...
			for (;;) {
				...
				BIO_snprintf(prompt, sizeof(prompt),
					     ...
					     (enc) ? "encryption" :
						     "decryption");
				strbuf[0] = '\0';
				i = EVP_read_pw_string((char *)strbuf, SIZE,
						       prompt, enc);
				if (i == 0) {
					if (strbuf[0] == '\0') {
						ret = 1;
	...
	if (cipher != NULL) {
		if (str != NULL) { /* a passphrase is available */
			...
			if (pbkdf2 == 1) {
                ...
                */
				unsigned char tmpkeyiv[EVP_MAX_KEY_LENGTH +
						       EVP_MAX_IV_LENGTH];
				...
				if (!PKCS5_PBKDF2_HMAC(str, (int)str_len, sptr,
						       islen, iter, dgst,
						       iklen + ivlen,
						       tmpkeyiv)) {
					BIO_printf(
		...
		if (rawkey_set) {
			...
			if (!EVP_CipherInit_SKEY(
				    ctx, cipher, skey,
				    (hiv == NULL && wrap == 1 ? NULL : iv),
				    EVP_CIPHER_get_iv_length(cipher), enc,
				    NULL)) {
				BIO_printf(
					bio_err,
		...
		if (printkey) {
			...
			if (EVP_CIPHER_get_key_length(cipher) > 0) {
				printf("key=");
				for (i = 0;
				     i < EVP_CIPHER_get_key_length(cipher); i++)
					printf("%02X", key[i]);
				printf("\n");
			}
			if (EVP_CIPHER_get_iv_length(cipher) > 0) {
				printf("iv =");
				for (i = 0;
				     i < EVP_CIPHER_get_iv_length(cipher); i++)
					printf("%02X", iv[i]);
				printf("\n");
			}
			if (printkey == 2) {
