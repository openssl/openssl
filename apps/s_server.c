...
static int
bring_ocsp_resp_in_correct_order(SSL *s, tlsextstatusctx *srctx,
				 ...
				 STACK_OF(OCSP_RESPONSE) **sk_resp)
{
     ...
     */
	if (server_certs != NULL && srctx->status_all && !SSL_is_dtls(s) &&
	...
	*sk_resp = sk_OCSP_RESPONSE_new_reserve(NULL, num);
	...
	for (i = 0; i < num; i++) {
		if (i !=
		    0) /* for each certificate in chain (except root) get the OCSP response */
		...
		if (issuer == NULL ||
		    (cert_id = OCSP_cert_to_id(NULL, ssl_cert, issuer)) ==
			    NULL) {
		...
		for (j = 0; j < sk_OCSP_RESPONSE_num(sk_resp_unordered); j++) {
			if ((resp = sk_OCSP_RESPONSE_value(sk_resp_unordered,
							   j)) == NULL)
				continue;
				    ...
				    STACK_OF(OCSP_RESPONSE) **sk_resp)
{
	...
	if (sk_resp_unordered == NULL) {
		BIO_puts(
			bio_err,
			"cert_status: Cannot reserve memory for OCSP responses\n");
		return SSL_TLSEXT_ERR_ALERT_FATAL;
	}
	...
	for (i = 0; i < num; i++) {
		...
		if (derbio == NULL) {
			BIO_printf(
				bio_err,
				"cert_status: Cannot open OCSP response file %s\n",
				respfile);
			ret = SSL_TLSEXT_ERR_ALERT_FATAL;
			goto err;
		}
		...
		if (resp == NULL) {
			BIO_printf(
				bio_err,
				"cert_status: Error reading OCSP response from file %s\n",
				respfile);
			ret = SSL_TLSEXT_ERR_ALERT_FATAL;
			goto err;
		}
	...
	ret = bring_ocsp_resp_in_correct_order(s, srctx, sk_resp_unordered,
					       sk_resp);
					...
					STACK_OF(OCSP_RESPONSE) **sk_resp)
{
	...
	*sk_resp = sk_OCSP_RESPONSE_new_reserve(NULL, num);
	...
	for (i = 0; i < num; i++) {
		...
		resp = NULL;
		if (get_ocsp_resp_from_responder_single(
			    s, ssl_cert, srctx, &resp) != SSL_TLSEXT_ERR_OK)
			resp = NULL;
...
const OPTIONS s_server_options[] = {
	...
	{ "max_send_frag", OPT_MAX_SEND_FRAG, 'p',
	...
	{ "http_server_binmode", OPT_HTTP_SERVER_BINMODE, '-',
	  "opening files in binary mode when acting as http server (-WWW and -HTTP)" },
	{ "no_ca_names", OPT_NOCANAMES, '-', "Disable TLS Extension CA Names" },
	...
	{ "sctp_label_bug", OPT_SCTP_LABEL_BUG, '-',
	...
	{ "sendfile", OPT_SENDFILE, '-',
	  "Use sendfile to response file with -WWW" },
	{ "zerocopy_sendfile", OPT_USE_ZC_SENDFILE, '-',
	  "Use zerocopy mode of KTLS sendfile" },
#endif
	{ "enable_server_rpk", OPT_ENABLE_SERVER_RPK, '-',
	  "Enable raw public keys (RFC7250) from the server" },
	{ "enable_client_rpk", OPT_ENABLE_CLIENT_RPK, '-',
...
#define IS_PROT_FLAG(o)                                         \
	(o == OPT_SSL3 || o == OPT_TLS1 || o == OPT_TLS1_1 ||   \
	 o == OPT_TLS1_2 || o == OPT_TLS1_3 || o == OPT_DTLS || \
	 o == OPT_DTLS1 || o == OPT_DTLS1_2)
...
opthelp:
...
#ifndef OPENSSL_NO_OCSP
			s_tlsextstatus = 1;
			if (!OSSL_HTTP_parse_url(opt_arg(), &tlscstatp.use_ssl,
						 NULL, &tlscstatp.host,
						 &tlscstatp.port, NULL,
	...
	if (nocert == 0) {
		...
		if (tlsextcbp.servername != NULL) {
			s_key2 = load_key(
				s_key_file2, s_key_format, 0, pass, engine,
				"second server certificate private key");
			if (s_key2 == NULL)
				goto end;

			s_cert2 = load_cert_pass(s_cert_file2, s_cert_format, 1,
						 pass,
	...
	if (session_id_prefix) {
		...
		if (!SSL_CTX_set_generate_session_id(ctx,
						     generate_session_id)) {
	...
	if (!ssl_load_stores(ctx, vfyCApath, vfyCAfile, vfyCAstore, chCApath,
			     chCAfile, chCAstore, crls, crl_download)) {
	...
	if (s_cert2) {
		ctx2 = SSL_CTX_new_ex(app_get0_libctx(), app_get0_propq(),
				      meth);
		if (ctx2 == NULL) {
			ERR_print_errors(bio_err);
			goto end;
	...
	if (ctx2 != NULL) {
		...
		if (no_cache)
			SSL_CTX_set_session_cache_mode(ctx2,
						       SSL_SESS_CACHE_OFF);
		else if (ext_cache)
	...
	if (!no_dhe) {
		...
		if (dhfile != NULL)
			dhpkey = load_keyparams(dhfile, FORMAT_UNDEF, 0, "DH",
						"DH parameters");
		else if (s_cert_file != NULL)
			dhpkey = load_keyparams_suppress(s_cert_file,
							 FORMAT_UNDEF, 0, "DH",
							 "DH parameters", 1);
		...
		if (ctx2 != NULL) {
			if (dhfile != NULL) {
				EVP_PKEY *dhpkey2 = load_keyparams_suppress(
					s_cert_file2, FORMAT_UNDEF, 0, "DH",
					"DH parameters", 1);
	...
	if (no_resume_ephemeral) {
		SSL_CTX_set_not_resumable_session_callback(
			ctx, not_resumable_sess_cb);

		if (ctx2 != NULL)
			SSL_CTX_set_not_resumable_session_callback(
				ctx2, not_resumable_sess_cb);
	}
#ifndef OPENSSL_NO_PSK
	if (psk_key != NULL) {
		if (s_debug)
	...
	if (psk_identity_hint != NULL) {
		if (min_version == TLS1_3_VERSION) {
			...
			if (!SSL_CTX_use_psk_identity_hint(ctx,
							   psk_identity_hint)) {
				BIO_printf(
					bio_err,
	...
	if (!SSL_CTX_set_session_id_context(
		    ctx, (void *)&s_server_session_id_context,
	...
	if (srp_verifier_file != NULL) {
		if (!set_up_srp_verifier_file(ctx, &srp_callback_parm,
					      srpuserseed, srp_verifier_file))
			goto end;
	} else
#endif
		if (CAfile != NULL) {
		...
		if (ctx2)
			SSL_CTX_set_client_CA_list(
				ctx2, SSL_load_client_CA_file(CAfile));
	}
#ifndef OPENSSL_NO_OCSP
	if (s_tlsextstatus) {
	...
	if (enable_server_rpk)
		if (!SSL_CTX_set1_server_cert_type(ctx, cert_type_rpk,
	...
	if (enable_client_rpk)
		if (!SSL_CTX_set1_client_cert_type(ctx, cert_type_rpk,
						   sizeof(cert_type_rpk))) {
			BIO_printf(bio_s_out,
				   "Error setting server certificate types\n");
			goto end;
	...
	do_server(&accept_socket, host, port, socket_family, socket_type,
		  protocol, server_cb, context, naccept, bio_s_out, tfo);
				     ...
				     size_t *processed)
{
	...
	if (s_debug) {
		...
		ret = (int)bio_dump_callback(bio, cmd, argp, len, argi, argl,
					     ret, processed);
...
static int sv_body(int s, int stype, int prot, unsigned char *context)
{
	...
	if (context != NULL &&
	    !SSL_set_session_id_context(
		    con, context, (unsigned int)strlen((char *)context))) {
		BIO_printf(bio_err, "Error setting session id context\n");
		ret = -1;
		goto err;
	}
	...
	if (early_data) {
		...
		while (edret != SSL_READ_EARLY_DATA_FINISH) {
			for (;;) {
				edret = SSL_read_early_data(con, buf, bufsize,
							    &readbytes);
		...
		if (write_header) {
			if (SSL_get_early_data_status(con) ==
	...
	for (;;) {
		...
		if (!read_from_sslcon) {
			...
			timeout.tv_sec = 1;
			timeout.tv_usec = 0;
			i = select(width, (void *)&readfds, NULL, NULL,
				   &timeout);
		...
		if (read_from_terminal) {
			...
			if (!s_quiet && !s_brief) {
				...
				if (buf[0] == 'c' &&
				    ((buf[1] == '\n') || (buf[1] == '\r'))) {
					SSL_set_verify(con, SSL_VERIFY_PEER,
						       NULL);
					i = SSL_verify_client_post_handshake(
						con);
					if (i == 0) {
		...
		if (read_from_sslcon) {
			...
			if ((!async || !SSL_waiting_for_async(con)) &&
				...
				BIO_set_callback_arg(SSL_get_rbio(con),
						     (char *)&read_counter);
...
static void print_connection_info(SSL *con)
{
...
#if !defined(OPENSSL_NO_NEXTPROTONEG)
	SSL_get0_next_proto_negotiated(con, &next_proto_neg,
				       &next_proto_neg_len);
	if (next_proto_neg) {
	...
	if (keymatexportlabel != NULL) {
		...
		if (SSL_export_keying_material(
			    con, exportedkeymat, keymatexportlen,
			    keymatexportlabel, strlen(keymatexportlabel), NULL,
			    0, 0) <= 0) {
			BIO_printf(bio_s_out, "    Error\n");
		} else {
...
static int add_session(SSL *ssl, SSL_SESSION *session)
{
	...
	sess->id =
		OPENSSL_memdup(SSL_SESSION_get_id(session, NULL), sess->idlen);
	...
	*do_copy = 0;
	for (sess = first; sess; sess = sess->next) {
		if (idlen == (int)sess->idlen && !memcmp(sess->id, id, idlen)) {
			...
			return d2i_SSL_SESSION_ex(NULL, &p, sess->derlen,
						  app_get0_libctx(),
						  app_get0_propq());
		}
	}
