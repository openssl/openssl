    ...
    STACK_OF(OCSP_RESPONSE) **sk_resp)
{
     ...
     */
    if (server_certs != NULL && srctx->status_all && !SSL_is_dtls(s) && SSL_version(s) >= TLS1_3_VERSION) {
...
int s_server_main(int argc, char *argv[])
{
    ...
    else if (record_size_limit > 0 && !SSL_CTX_set_record_size_limit(ctx, record_size_limit)) {
        BIO_printf(bio_err,
