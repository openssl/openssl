...
opthelp:
		...
		case OPT_URL:
			...
			if (!OSSL_HTTP_parse_url(
				    opt_arg(), &use_ssl, NULL /* userinfo */,
				    &host, &port, NULL /* port_num */, &path,
				    NULL /* qry */, NULL /* frag */)) {
				BIO_printf(bio_err,
					   "%s Error parsing -url argument\n",
					   prog);
				goto end;
			}
			thost = host;
		...
		case OPT_SERIAL:
			...
			if (cert_id_md == NULL)
				cert_id_md = (EVP_MD *)EVP_sha1();
			if (!add_ocsp_serial(&req, opt_arg(), cert_id_md,
					     issuer, ids))
				goto end;
			if (!sk_OPENSSL_STRING_push(reqnames, opt_arg()))
				goto end;
...
#else
		BIO_printf(bio_err,
			   "Cannot act as server - sockets not supported\n");
		goto end;
#endif
	}
	...
	if (rsignfile != NULL) {
		...
		if (!load_certs(rca_filename, 0, &rca_certs, NULL,
				"CA certificates"))
			goto end;
		if (rcertfile != NULL) {
...
redo_accept:
	...
	if (signfile != NULL) {
		if (keyfile == NULL)
			keyfile = signfile;
		signer =
			load_cert(signfile, FORMAT_UNDEF, "signer certificate");
		if (signer == NULL) {
			BIO_printf(bio_err,
	...
	if (rdb != NULL) {
		make_ocsp_response(bio_err, &resp, req, rdb, rca_certs, rsigner,
				   rkey, rsign_md, rsign_sigopts, rother,
				   rflags, nmin, ndays, badsig, resp_certid_md);
		if (resp == NULL)
			goto end;
		if (cbio != NULL)
			send_ocsp_response(cbio, resp);
...
#ifndef OPENSSL_NO_SOCK
		resp = process_responder(req, host, port, path, opt_proxy,
					 opt_no_proxy, use_ssl, headers,
					 req_timeout);
		if (resp == NULL)
			goto end;
#else
...
done_resp:
	...
	if (!noverify) {
		...
		if (i <= 0 && issuers) {
			i = OCSP_basic_verify(bs, issuers, store,
					      OCSP_TRUSTOTHER);
			if (i > 0)
				ERR_clear_error();
		}
			      ...
			      long maxage)
{
	...
	for (i = 0; i < sk_OCSP_CERTID_num(ids); i++) {
		...
		if (!OCSP_resp_find_status(bs, id, &status, &reason, &rev,
					   &thisupd, &nextupd)) {
			       ...
			       const EVP_MD *resp_md)
{
	...
	for (i = 0; i < id_count; i++) {
		...
		for (jj = 0; jj < sk_X509_num(ca) && !found; jj++) {
			...
			OCSP_CERTID *ca_id =
				OCSP_cert_to_id(cert_id_md, NULL, ca_cert);

			if (OCSP_id_issuer_cmp(ca_id, cid) == 0) {
				found = 1;
				if (resp_md != NULL)
					cid_resp_md = OCSP_cert_to_id(
		...
		if (inf == NULL) {
			...
			OCSP_basic_add1_status(bs, cid, V_OCSP_CERTSTATUS_GOOD,
			...
			single = OCSP_basic_add1_status(
				bs, cid, V_OCSP_CERTSTATUS_REVOKED, reason,
			...
			if (invtm != NULL)
				OCSP_SINGLERESP_add1_ext_i2d(
					single, NID_invalidity_date, invtm, 0,
					0);
			else if (inst != NULL)
				OCSP_SINGLERESP_add1_ext_i2d(
					single, NID_hold_instruction_code, inst,
					0, 0);
	...
	mctx = EVP_MD_CTX_new();
	if (mctx == NULL ||
	    !EVP_DigestSignInit(mctx, &pkctx, rmd, NULL, rkey)) {
		*resp = OCSP_response_create(OCSP_RESPONSE_STATUS_INTERNALERROR,
					     NULL);
		goto end;
	}
			...
			int timeout)
{
	...
	return http_server_get_asn1_req(ASN1_ITEM_rptr(OCSP_REQUEST),
					...
					NULL /* found_keep_alive */, prog,
					1 /* accept_get */, timeout);
#else
	BIO_printf(bio_err,
...
{
#ifndef OPENSSL_NO_SOCK
	return http_server_send_asn1_resp(prog, cbio, 0 /* no keep-alive */,
				 ...
				 int req_timeout)
{
	...
	resp = (OCSP_RESPONSE *)app_http_post_asn1(
		...
		ASN1_ITEM_rptr(OCSP_REQUEST), "application/ocsp-response",
