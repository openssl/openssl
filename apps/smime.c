...
#define SET_OPERATION(op)                                                 \
	((operation != 0 && (operation != (op))) ?                        \
		 0 * BIO_printf(bio_err,                                  \
				"%s: Cannot use -%s together with -%s\n", \
				prog, operation_name(op),                 \
...
int smime_main(int argc, char **argv)
{
	...
	char *infile = NULL, *outfile = NULL, *signerfile = NULL,
	     *recipfile = NULL;
...
opthelp:
	...
	if (operation == SMIME_ENCRYPT) {
		...
		for (i = 0; i < sk_OPENSSL_STRING_num(sksigners); i++) {
			...
			if (signer == NULL)
				goto end;
			key = load_key(keyfile, keyform, 0, passin, e,
				       "signing key");
			if (key == NULL)
				goto end;

			if (!PKCS7_sign_add_signer(p7, signer, key, sign_md,
						   flags))
				goto end;
			X509_free(signer);
			signer = NULL;
