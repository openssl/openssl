...
static void print_user(CA_DB *db, int userindex, int verbose)
{
	if (verbose > 0) {
		...
		if (pp[DB_srptype][0] != 'I') {
			...
			print_entry(db, get_index(db, pp[DB_srpgN], 'I'),
				    verbose, "g N entry");
		}
	}
}
...
opthelp:
	...
	if (srpvfile == NULL) {
		...
		if (section == NULL) {
			...
			section = lookup_conf(conf, BASE_SECTION,
					      ENV_DEFAULT_SRP);
			if (section == NULL)
				goto end;
		}
	...
	for (i = 0; i < sk_OPENSSL_PSTRING_num(db->db->data); i++) {
		...
		if (pp[DB_srptype][0] == DB_SRP_INDEX) {
			maxgN = i;
			if ((gNindex < 0) && (gN != NULL) &&
			    strcmp(gN, pp[DB_srpid]) == 0)
				gNindex = i;
	...
	while (mode == OPT_LIST || user != NULL) {
		...
		if (mode == OPT_LIST) {
			if (user == NULL) {
				...
				for (i = 0;
				     i < sk_OPENSSL_PSTRING_num(db->db->data);
				     i++)
					print_user(db, i, 1);
			} else if (userindex < 0) {
				BIO_printf(
			...
			if (userindex >= 0) {
				...
				char **row = sk_OPENSSL_PSTRING_value(
					db->db->data, userindex);
				BIO_printf(bio_err,
					   "user \"%s\" reactivated.\n", user);
				row[DB_srptype][0] = 'V';
					...
					if (row[DB_srptype][0] == 'V') {
						...
						if (verbose)
							BIO_printf(
								...
								user);
						if ((user_gN = get_index(
							     db, row[DB_srpgN],
							     DB_SRP_INDEX)) >=
						    0)
							irow = sk_OPENSSL_PSTRING_value(
								db->db->data,
								userindex);
						...
						if (!srp_verify_user(
							    ...
							    row[DB_srpverifier],
							    row[DB_srpsalt],
							    irow ? irow[DB_srpsalt] :
								   row[DB_srpgN],
							    irow ? irow[DB_srpverifier] :
								   NULL,
							    passin, verbose)) {
							BIO_printf(
								bio_err,
			...
			if (userindex < 0) {
				...
				char **xpp = sk_OPENSSL_PSTRING_value(
					db->db->data, userindex);
