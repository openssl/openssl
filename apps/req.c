...
static int join(char buf[], size_t buf_size, const char *name, const char *tail,
		const char *desc);
static EVP_PKEY_CTX *set_keygen_ctx(const char *gstr, char **pkeytype,
				    long *pkeylen, ENGINE *keygen_engine);
...
const OPTIONS req_options[] = {
	...
	{ "cipher", OPT_CIPHER, 's',
	  "Specify the cipher for private key encryption" },
	...
	{ "x509v1", OPT_X509V1, '-',
	  "Request cert generation with X.509 version 1" },
	{ "CA", OPT_CA, '<',
...
int req_main(int argc, char **argv)
{
	...
	char *keyalgstr = NULL, *p, *prog, *passargin = NULL,
	     *passargout = NULL;
...
opthelp:
		...
		case OPT_ADDEXT:
			p = opt_arg();
			if (addexts == NULL) {
				addexts = lh_OPENSSL_STRING_new(ext_name_hash,
								ext_name_cmp);
			...
			if (i == -1)
				BIO_printf(
					bio_err,
					"Internal error handling -addext %s\n",
					p);
	...
	if (!gen_x509) {
		...
		if (ext_copy == EXT_COPY_NONE)
			BIO_printf(
				bio_err,
				"Warning: Ignoring -copy_extensions 'none' when -x509 is not given\n");
	}
	if (infile == NULL) {
		if (gen_x509)
			newreq = 1;
	...
	if (addext_bio != NULL) {
		...
		if ((addext_conf = app_load_config_bio(addext_bio, NULL)) ==
		    NULL)
			goto end;
	}
	...
	if (passin == NULL)
		passin = nofree_passin = app_conf_try_string(req_conf, section,
							     "input_password");

	if (passout == NULL)
		passout = nofree_passout = app_conf_try_string(
			req_conf, section, "output_password");
...
loop:
	...
	if (CAkeyfile != NULL) {
		if (CAfile == NULL) {
			...
			if ((CAkey = load_key(
				     CAkeyfile, FORMAT_UNDEF, 0, passin, e,
				     CAkeyfile != CAfile ?
	...
	if (newreq || gen_x509) {
		if (CAcert == NULL && pkey == NULL) {
			BIO_printf(bio_err,
				   "Must provide a signature key using -key or"
				   " provide -CA / -CAkey\n");
			goto end;
		}

		if (req == NULL) {
			req = X509_REQ_new_ex(app_get0_libctx(),
					      app_get0_propq());
			if (req == NULL) {
				goto end;
			}

			if (!make_REQ(req, pkey, fsubj, multirdn, !gen_x509,
				      chtype)) {
				BIO_printf(
					bio_err,
		...
		if (gen_x509) {
			...
			X509V3_CTX ext_ctx;
			X509_NAME *issuer =
				CAcert != NULL ? X509_get_subject_name(CAcert) :
			...
			if (serial != NULL) {
				...
				if (!rand_serial(NULL, X509_get_serialNumber(
							       new_x509)))
					goto end;
			}
			...
			if (days == UNSET_DAYS)
				...
				BIO_printf(
					bio_err,
					"Warning: -not_after option overriding -days option\n");
			if (!set_cert_times(new_x509, not_before, not_after,
					    days, 1))
				goto end;
			...
			if (ext_copy == EXT_COPY_UNSET) {
				...
				BIO_printf(
					bio_err,
					"Error copying extensions from request\n");
				goto end;
			}

			/* Set up V3 context struct */
			X509V3_set_ctx(&ext_ctx,
				       CAcert != NULL ? CAcert : new_x509,
				       new_x509, NULL, NULL,
				       X509V3_CTX_REPLACE);
			/* prepare fallback for AKID, but only if issuer cert == new_x509 */
			if (CAcert == NULL) {
				if (!X509V3_set_issuer_pkey(&ext_ctx,
							    issuer_key))
					goto end;
			...
			i = do_X509_sign(new_x509, x509v1, issuer_key, digest,
					 sigopts, &ext_ctx);
			if (!i)
				goto end;
		} else {
	...
	if (text) {
		if (gen_x509)
			ret = X509_print_ex(out, new_x509, get_nameopt(),
					    reqflag);
		else
			ret = X509_REQ_print_ex(out, req, get_nameopt(),
						reqflag);
	...
	if (subject) {
		print_name(out, "subject=",
			   gen_x509 ? X509_get_subject_name(new_x509) :
				      X509_REQ_get_subject_name(req));
	}
		    ...
		    int multirdn, int attribs, unsigned long chtype)
{
	...
	if (fsubj != NULL)
		...
		i = prompt_info(req, dn_sk, dn_sect, attr_sk, attr_sect,
				attribs, chtype);
	if (!i)
		goto err;
...
start:
		for (;;) {
			...
			if ((nid = OBJ_txt2nid(type)) == NID_undef)
				goto start;
			if (!join(buf, sizeof(buf), v->name, "_default",
				  "Name"))
				return 0;
			if ((def = app_conf_try_string(req_conf, dn_sect,
						       buf)) == NULL)
				def = "";

			if (!join(buf, sizeof(buf), v->name, "_value", "Name"))
				return 0;
			if ((value = app_conf_try_string(req_conf, dn_sect,
							 buf)) == NULL)
				value = NULL;

			if (!join(buf, sizeof(buf), v->name, "_min", "Name"))
				return 0;
			if (!app_conf_try_number(req_conf, dn_sect, buf,
						 &n_min))
				n_min = -1;
		...
		if (attribs) {
			if ((attr_sk != NULL) &&
			    (sk_CONF_VALUE_num(attr_sk) > 0) && (!batch)) {
				BIO_printf(
					bio_err,
...
start2:
			for (;;) {
				...
				if (!join(buf, sizeof(buf), type, "_default",
					  "Name"))
					return 0;
				def = app_conf_try_string(req_conf, attr_sect,
							  buf);
				if (def == NULL)
					def = "";

				if (!join(buf, sizeof(buf), type, "_value",
					  "Name"))
					return 0;
				value = app_conf_try_string(req_conf, attr_sect,
							    buf);

				if (!join(buf, sizeof(buf), type, "_min",
					  "Name"))
					return 0;
				if (!app_conf_try_number(req_conf, attr_sect,
							 buf, &n_min))
					n_min = -1;

				if (!join(buf, sizeof(buf), type, "_max",
					  "Name"))
					return 0;
				if (!app_conf_try_number(req_conf, attr_sect,
							 buf, &n_max))
				...
				if (!add_attribute_object(req, v->value, def,
							  value, nid, n_min,
							  n_max, chtype))
					return 0;
			}
			 ...
			 int mval)
{
	...
	if (!X509_NAME_add_entry_by_NID(n, nid, chtype, (unsigned char *)buf,
					-1, -1, mval))
		ret = 0;
				...
				unsigned long chtype)
{
	...
	if (!X509_REQ_add1_attr_by_NID(req, nid, chtype, (unsigned char *)buf,
				       -1)) {
				    ...
				    long *pkeylen, ENGINE *keygen_engine)
{
	...
	if (param != NULL) {
		...
		*pkeylen = EVP_PKEY_get_bits(param);
		...
		if (keygen_engine != NULL) {
			int pkey_id = get_legacy_pkey_id(
				app_get0_libctx(), *pkeytype, keygen_engine);

			if (pkey_id != NID_undef)
				gctx = EVP_PKEY_CTX_new_id(pkey_id,
							   keygen_engine);
		} else {
#endif
			gctx = EVP_PKEY_CTX_new_from_name(
				app_get0_libctx(), *pkeytype, app_get0_propq());
#ifndef OPENSSL_NO_DEPRECATED_3_6
		}
#endif
	}
