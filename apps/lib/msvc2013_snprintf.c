 ...
 * crypto/msvc2013_snprintf.c to serve the libcrypto build.  Because
 * each definition lives in its own .obj, the linker's archive-search
 * rule ensures only one copy is pulled into any final binary.  Defining
 * any additional symbol here would risk pulling both copies and
 * producing a duplicate-symbol link error.  Keep both files in sync;
 * any change here MUST be mirrored in crypto/msvc2013_snprintf.c.
 */
