...
int chopup_args(ARGS *arg, char *buf)
{
	...
	if (arg->size == 0) {
		arg->size = 20;
		arg->argv = app_malloc(sizeof(*arg->argv) * arg->size,
				       "argv space");
	}
...
CONF *app_load_config_internal(const char *filename, int quiet)
{
	...
	if (filename == NULL || *filename != '\0') {
		if ((in = bio_open_default_(filename, 'r', FORMAT_TEXT,
					    quiet)) == NULL)
			return NULL;
		   ...
		   const char *desc)
{
	...
	if (IS_HTTPS(uri)) {
		...
		(void)load_key_certs_crls(uri, format, maybe_stdin, NULL, desc,
					  0, NULL, NULL, NULL, NULL, NULL, &crl,
					  NULL);
	}
	return crl;
}
				  ...
				  int suppress_decode_errors)
{
	...
	if (desc == NULL)
		desc = "key parameters";
	(void)load_key_certs_crls(uri, format, maybe_stdin, NULL, desc,
				  suppress_decode_errors, NULL, NULL, &params,
				  NULL, NULL, NULL, NULL);
	if (params != NULL && keytype != NULL &&
	    !EVP_PKEY_is_a(params, keytype)) {
		ERR_print_errors(bio_err);
		BIO_printf(
			bio_err,
...
EVP_PKEY *load_keyparams(const char *uri, int format, int maybe_stdin,
			 const char *keytype, const char *desc)
{
	return load_keyparams_suppress(uri, format, maybe_stdin, keytype, desc,
				       0);
}
		      ...
		      X509_VERIFY_PARAM *vpm)
{
	...
	if (res != 0)
		warn_cert_msg(uri, cert,
			      res > 0 ? "has expired" : "not yet valid");
	if (warn_EE && (ex_flags & EXFLAG_V1) == 0 &&
	    (ex_flags & EXFLAG_CA) == 0)
	      ...
	      const char *desc)
{
	...
	if (desc == NULL)
		desc = "CRLs";
	ret = load_key_certs_crls(uri, FORMAT_UNDEF, 0, pass, desc, 0, NULL,
				  NULL, NULL, NULL, NULL, NULL, crls);
			...
			STACK_OF(X509_CRL) **pcrls)
{
	...
	while ((ppkey != NULL || ppubkey != NULL || pparams != NULL ||
		pcert != NULL || pcerts != NULL || pcrl != NULL ||
		pcrls != NULL) &&
	       !OSSL_STORE_eof(ctx)) {
		...
		case OSSL_STORE_INFO_PKEY:
			if (ppkey != NULL) {
				ok = (*ppkey = OSSL_STORE_INFO_get1_PKEY(
					      info)) != NULL;
				if (ok)
					ppkey = NULL;
				break;
		...
		case OSSL_STORE_INFO_PUBKEY:
			if (ppubkey != NULL) {
				ok = (*ppubkey = OSSL_STORE_INFO_get1_PUBKEY(
					      info)) != NULL;
				if (ok)
					ppubkey = NULL;
			}
		...
		case OSSL_STORE_INFO_PARAMS:
			if (pparams != NULL) {
				ok = (*pparams = OSSL_STORE_INFO_get1_PARAMS(
					      info)) != NULL;
				if (ok)
					pparams = NULL;
			}
		...
		case OSSL_STORE_INFO_CERT:
			if (pcert != NULL) {
				ok = (*pcert = OSSL_STORE_INFO_get1_CERT(
					      info)) != NULL;
				if (ok)
					pcert = NULL;
			} else if (pcerts != NULL) {
		...
		case OSSL_STORE_INFO_CRL:
			if (pcrl != NULL) {
				ok = (*pcrl = OSSL_STORE_INFO_get1_CRL(info)) !=
				     NULL;
				if (ok)
					pcrl = NULL;
			} else if (pcrls != NULL) {
				ok = sk_X509_CRL_push(
					*pcrls, OSSL_STORE_INFO_get1_CRL(info));
			}
			ncrls += ok;
			break;
		default:
...
int set_name_ex(unsigned long *flags, const char *arg)
{
	static const NAME_EX_TBL ex_tbl[] = {
		...
		{ "sep_comma_plus_space", XN_FLAG_SEP_CPLUS_SPC,
		  XN_FLAG_SEP_MASK },
			 ...
			 int noCApath, const char *CAstore, int noCAstore)
{
	...
	if (CAfile != NULL || !noCAfile) {
		...
		if (lookup == NULL)
			goto end;
		if (CAfile != NULL) {
			if (X509_LOOKUP_load_file_ex(lookup, CAfile,
						     X509_FILETYPE_PEM, libctx,
						     propq) <= 0) {
				ERR_clear_error();
				if (X509_LOOKUP_load_file_ex(
					    lookup, CAfile, X509_FILETYPE_ASN1,
					    libctx, propq) <= 0) {
					BIO_printf(bio_err,
			...
			X509_LOOKUP_load_file_ex(lookup, NULL,
						 X509_FILETYPE_DEFAULT, libctx,
						 propq);
		}
	}
	...
	if (CApath != NULL || !noCApath) {
		...
		if (lookup == NULL)
			goto end;
		if (CApath != NULL) {
			if (X509_LOOKUP_add_dir(lookup, CApath,
						X509_FILETYPE_PEM) <= 0) {
				BIO_printf(bio_err,
		...
		const BIGNUM *serial, ASN1_INTEGER **retai)
{
	...
	if (suffix == NULL) {
		...
		BIO_snprintf(buf[0], sizeof(buf[0]), "%s.%s", serialfile,
			     suffix);
#else
		BIO_snprintf(buf[0], sizeof(buf[0]), "%s-%s", serialfile,
			     suffix);
...
CA_DB *load_index(const char *dbfile, DB_ATTR *db_attr)
{
	...
	if (dbattr_conf != NULL) {
		char *p = app_conf_try_string(dbattr_conf, NULL,
					      "unique_subject");
			...
			STACK_OF(OPENSSL_STRING) *sigopts)
{
     ...
     * for this algorithm.
     */
	if (EVP_PKEY_get_default_digest_name(pkey, def_md, sizeof(def_md)) ==
		    2 &&
		 ...
		 STACK_OF(OPENSSL_STRING) *sigopts, X509V3_CTX *ext_ctx)
{
	...
	if (!force_v1) {
		...
		if (!adapt_keyid_ext(cert, ext_ctx, "subjectKeyIdentifier",
				     "hash", 1))
			goto end;
		       ...
		       STACK_OF(OPENSSL_STRING) *vfyopts)
{
	...
	if (do_x509_req_init(x, vfyopts) > 0)
		rv = X509_REQ_verify_ex(x, pkey, app_get0_libctx(),
					app_get0_propq());
	else
		rv = -1;
	return rv;
	...
	if (connect) {
		...
		if ((info->use_proxy &&
		     !OSSL_HTTP_proxy_connect(
			     bio, info->server, info->port, NULL,
			      ...
			      const ASN1_ITEM *it)
{
	...
	if (!OSSL_HTTP_parse_url(url, &use_ssl, NULL /* userinfo */, &server,
				 &port, NULL /* port_num, */, NULL, NULL, NULL))
		return NULL;
	...
	mem = OSSL_HTTP_get(url, proxy, no_proxy, NULL /* bio */,
			    NULL /* rbio */, app_http_tls_cb, &info,
			    0 /* buf_size */, headers, expected_content_type,
			    1 /* expect_asn1 */, OSSL_HTTP_DEFAULT_MAX_RESP_LEN,
			    timeout);
			       ...
			       const ASN1_ITEM *rsp_it)
{
	...
	info.timeout = timeout;
	info.ssl_ctx = ssl_ctx;
	rsp = OSSL_HTTP_transfer(NULL, host, port, path, use_ssl, proxy,
...
static int WIN32_rename(const char *from, const char *to)
{
	...
	if (sizeof(TCHAR) == 1) {
		...
		if (!MultiByteToWideChar(CP_ACP, 0, from, (int)flen,
					 (WCHAR *)tfrom, (int)flen))
#endif
			for (i = 0; i < flen; i++)
...
{
	BIO *b = BIO_new_fp(
		stdout, BIO_NOCLOSE | (FMT_istext(format) ? BIO_FP_TEXT : 0));
...
{
	BIO *b = BIO_new_fp(
		stderr, BIO_NOCLOSE | (FMT_istext(format) ? BIO_FP_TEXT : 0));
		   ...
		   int days, int strict_compare_times)
{
	...
	if (startdate == NULL || strcmp(startdate, "today") == 0) {
		...
		if (!ASN1_TIME_set_string_X509(X509_getm_notBefore(x),
					       startdate)) {
			BIO_printf(
				bio_err,
				"Error setting notBefore certificate field\n");
	...
	if (ASN1_TIME_compare(X509_get0_notAfter(x), X509_get0_notBefore(x)) <
	    0) {
		       ...
		       long hours, long secs)
{
	...
	if (nextupdate == NULL) {
		if (X509_time_adj_ex(tm, days, hours * 60 * 60 + secs, NULL) ==
		    NULL)
			goto end;
	} else {
