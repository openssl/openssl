...
void spawn_loop(const char *prog)
{
	...
	kidpids =
		app_malloc(n_responders * sizeof(*kidpids), "child PID array");
	for (i = 0; i < n_responders; ++i)
		kidpids[i] = 0;
...
BIO *http_server_init(const char *prog, const char *port, int verb)
{
	...
	if (BIO_do_accept(acbio) <= 0) {
		log_HTTP1(prog, LOG_ERR, "error setting accept on port %s",
			  port);
		goto err;
	}
	...
	if ((accept_get && CHECK_AND_SKIP_PREFIX(url, "GET ")) ||
		...
		if (*url != '/') {
			log_HTTP2(
				...
				meth, url);
			(void)http_server_send_status(prog, cbio, 400,
						      "Bad Request");
			goto out;
		}
		url++;
		...
		*end = '\0';
		...
		if (found_keep_alive != NULL)
			*found_keep_alive = end[sizeof(HTTP_VERSION_STR) - 1] >
					    '0';
	...
	*end = '\0';
	...
	for (;;) {
		...
		log_HTTP1(prog, LOG_TRACE, "%s",
			  *inbuf == '\0' ?
				  " " /* workaround for "" getting ignored */ :
				  inbuf);
		if (end == NULL) {
			log_HTTP(
				prog, LOG_WARNING,
