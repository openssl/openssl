...
#pragma __nomember_alignment
	__union
	{
		...
		{
			unsigned short int
				iosb$w_status; /* Final I/O status           */
			__union
			{
				__struct
				{ /* 16-bit byte count variant        */
					unsigned short int
						iosb$w_bcnt; /* 16-bit byte count    */
					__union
					{
						unsigned int
							iosb$l_dev_depend; /* 32-bit device dependent info */
						unsigned int
							iosb$l_pid; /* 32-bit pid              */
					}
					iosb$r_l;
				}
				iosb$r_bcnt_16;
		...
		__struct
		{
			...
			{
				unsigned int
					iosb$l_getxxi_status; /* Final GETxxI status   */
				unsigned int
					iosb$l_reg_status; /* Final $Registry status   */
			}
			iosb$r_l_status;
			unsigned int
				iosb$l_reserved; /* Reserved field                 */
		}
		iosb$r_get_64;
	}
	iosb$r_io_get;
...
int TerminalSocket(int FunctionCode, int *ReturnSocket)
{
    ...
    */
	...
	case TERM_SOCK_CREATE:
        ...
        */
		status = sys$assign(&TerminalDeviceDesc, &TerminalDeviceChan, 0,
				    0, 0);
		if (!(status & 1)) {
			LogMessage("TerminalSocket: SYS$ASSIGN () - %08X",
				   status);
			close(TerminalSocketPair[0]);
			close(TerminalSocketPair[1]);
			return TERM_SOCK_FAILURE;
	...
	case TERM_SOCK_DELETE:
        ...
        */
		...
		if (!(status & 1)) {
			LogMessage("TerminalSocket: SYS$CANCEL () - %08X",
				   status);
			close(TerminalSocketPair[0]);
			close(TerminalSocketPair[1]);
			return TERM_SOCK_FAILURE;
        ...
        */
		...
		if (!(status & 1)) {
			LogMessage("TerminalSocket: SYS$DASSGN () - %08X",
				   status);
			close(TerminalSocketPair[0]);
			close(TerminalSocketPair[1]);
			return TERM_SOCK_FAILURE;
...
{
	struct dsc$descriptor AscTimeDesc = { 0, DSC$K_DTYPE_T, DSC$K_CLASS_S,
					      NULL };
    ...
    */
	status = sys$qio(EFN$C_ENF, TcpAcceptChan, IO$_ACCESS | IO$M_ACCEPT,
			 &iosb, 0, 0, 0, 0, 0, &TcpDeviceChan, 0, 0);
	if (!(status & 1)) {
    ...
    */
	status = sys$setimr(EFN$C_ENF, BinTimeBuff, SocketPairTimeoutAst, &sptb,
			    0);
	if (!(status & 1)) {
...
static int TerminalDeviceAst(int astparm)
{
    ...
    */
	status = sys$qio(EFN$C_ENF, TerminalDeviceChan, IO$_READVBLK,
			 &TerminalDeviceIosb, TerminalDeviceAst, 0,
			 TerminalDeviceBuff, sizeof(TerminalDeviceBuff) - 1, 0,
			 0, 0, 0);
