...
typedef struct {
	...
	STACK_OF(X509)
	*chainOut; /* chain of certOut to add to extraCerts field */
	STACK_OF(X509)
	*caPubsOut; /* used in caPubs of ip and in caCerts of genp */
	...
	OSSL_CMP_PKISI
	*statusOut; /* status for ip/cp/kup/rp msg unless polling */
...
static int delayed_delivery(OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *req)
{
     ...
     */
	if (req_type == OSSL_CMP_IR || req_type == OSSL_CMP_CR ||
...
static int refcert_cmp(const X509 *refcert, const X509_NAME *issuer,
		       const ASN1_INTEGER *serial)
{
	...
	return (ref_issuer == NULL || X509_NAME_cmp(issuer, ref_issuer) == 0) &&
	       (ref_serial == NULL ||
		ASN1_INTEGER_cmp(serial, ref_serial) == 0);
}
...
process_cert_request(OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *cert_req,
		     ossl_unused int certReqId, const OSSL_CRMF_MSG *crm,
		     ...
		     STACK_OF(X509) **chainOut, STACK_OF(X509) **caPubs)
{
	...
	if (ctx == NULL || cert_req == NULL || certOut == NULL ||
	    chainOut == NULL || caPubs == NULL) {
		ERR_raise(ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
		return NULL;
	}
	...
	*caPubs = NULL;
	...
	if (ctx->pollCount > 0 && ctx->curr_pollCount == 0) {
		...
		if ((ctx->req = OSSL_CMP_MSG_dup(cert_req)) == NULL)
			return NULL;
		return OSSL_CMP_STATUSINFO_new(OSSL_CMP_PKISTATUS_waiting, 0,
					       NULL);
	}
	...
	if (OSSL_CMP_MSG_get_bodytype(cert_req) == OSSL_CMP_CR) {
		STACK_OF(OSSL_CMP_ITAV) *itavs =
			OSSL_CMP_HDR_get0_geninfo_ITAVs(
				OSSL_CMP_MSG_get0_header(cert_req));
		int i;
		...
		for (i = 0; i < sk_OSSL_CMP_ITAV_num(itavs); i++) {
			...
			if (OBJ_obj2nid(obj) == NID_id_it_certProfile) {
				if (!OSSL_CMP_ITAV_get0_certProfile(itav,
								    &strs))
					return NULL;
				...
				if (str == NULL ||
				    (data = (const char *)ASN1_STRING_get0_data(
					     str)) == NULL) {
					ERR_raise(
						ERR_LIB_CMP,
	...
	if (bodytype == OSSL_CMP_KUR && crm != NULL /* thus not p10cr */ &&
	    ctx->refCert != NULL) {
			    ...
			    const X509_CRL *crl)
{
	...
	if (issuer != NULL) {
		...
		if (gn != NULL && gn->type == GEN_DIRNAME) {
			...
			if (X509_NAME_cmp(gen_name, X509_CRL_get_issuer(crl)) !=
			    0) {
				ERR_raise(ERR_LIB_CMP,
					  CMP_R_UNKNOWN_CRL_ISSUER);
				return -1;
			}
		} else {
			ERR_raise(ERR_LIB_CMP,
					...
					const OSSL_CMP_ITAV *req)
{
	...
	case NID_id_it_rootCaCert: {
		...
		if (rootcacert != NULL &&
			...
			rsp = OSSL_CMP_ITAV_new_rootCaKeyUpdate(NULL, NULL,
								NULL);
		else
			rsp = OSSL_CMP_ITAV_new_rootCaKeyUpdate(
				ctx->newWithNew, ctx->newWithOld,
				ctx->oldWithNew);
	} break;
	case NID_id_it_crlStatusList: {
		...
		if (res < 0)
			...
			rsp = OSSL_CMP_ITAV_new_crls(res == 0 ? NULL :
								ctx->crlOut);
	} break;
	case NID_id_it_certReqTemplate: {
		...
		if (!OSSL_CRMF_CERTTEMPLATE_fill(
			    reqtemp, NULL, NULL,
...
static int process_pollReq(OSSL_CMP_SRV_CTX *srv_ctx,
			   ...
			   ossl_unused int certReqId, OSSL_CMP_MSG **req,
			   int64_t *check_after)
{
	mock_srv_ctx *ctx = OSSL_CMP_SRV_CTX_get0_custom_ctx(srv_ctx);
	...
	if (req != NULL)
		*req = NULL;
	if (ctx == NULL || pollReq == NULL || req == NULL ||
	    check_after == NULL) {
		ERR_raise(ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
		return 0;
	}
