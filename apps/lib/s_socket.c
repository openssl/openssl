		...
		int type, int protocol, int tfo, int doconn, BIO_ADDR **ba_ret)
{
	...
	ret = BIO_lookup_ex(host, port, BIO_LOOKUP_CLIENT, family, type,
			    protocol, &res);
	if (ret == 0) {
	...
	for (ai = res; ai != NULL; ai = BIO_ADDRINFO_next(ai)) {
		...
		if (bindaddr != NULL) {
			for (bi = bindaddr; bi != NULL;
			     bi = BIO_ADDRINFO_next(bi)) {
				if (BIO_ADDRINFO_family(bi) ==
		...
		*sock = BIO_socket(BIO_ADDRINFO_family(ai),
		...
		if (doconn &&
		    !BIO_connect(*sock, BIO_ADDRINFO_address(ai), options)) {
			BIO_closesocket(*sock);
			*sock = INVALID_SOCKET;
			continue;
		}
	      ...
	      int naccept, BIO *bio_s_out, int tfo)
{
	...
	if (!BIO_lookup_ex(host, port, BIO_LOOKUP_SERVER, family, type,
			   protocol, &res)) {
		ERR_print_errors(bio_err);
		return 0;
	}
	...
	if (asock == INVALID_SOCKET && sock_family_fallback != AF_UNSPEC) {
		asock = BIO_socket(sock_family_fallback, sock_type,
				   sock_protocol, 0);
	...
	for (;;) {
		...
		if (type == SOCK_STREAM) {
			...
			} while (select(sock + 1, &readfds, NULL, NULL,
					&timeout) > 0 &&
