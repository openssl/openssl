...
char *opt_init(int ac, char **av, const OPTIONS *o)
{
	...
	for (; o->name != NULL && o->name != OPT_PARAM_STR; ++o) {
		...
		if (o->valtype == '.')
			...
			OPENSSL_assert(o->retval == OPT_DUP ||
				       o->retval > OPT_PARAM);
		switch (i) {
		case 0:
		case '-':
		...
		for (next = o + 1; next->name; ++next) {
			...
			if (duplicated) {
				opt_printf_stderr(
					"%s: Internal error: duplicate option %s\n",
					prog, o->name);
		...
		if (o->name[0] == '\0') {
			...
			unknown = o;
			OPENSSL_assert(unknown->valtype == 0 ||
				       unknown->valtype == '-');
		}
	}
	return prog;
...
static OPT_PAIR formats[] = {
	{ "pem", OPT_FMT_PEM },	      { "der", OPT_FMT_DER },
	{ "b64", OPT_FMT_B64 },	      { "pkcs12", OPT_FMT_PKCS12 },
	{ "smime", OPT_FMT_SMIME },   { "engine", OPT_FMT_ENGINE },
	{ "msblob", OPT_FMT_MSBLOB }, { "nss", OPT_FMT_NSS },
	{ "text", OPT_FMT_TEXT },     { "http", OPT_FMT_HTTP },
	{ "pvk", OPT_FMT_PVK },	      { NULL }
};
...
int opt_format(const char *s, unsigned long flags, int *result)
{
	...
	case 'B':
	case 'b':
		if (s[1] == '\0' || strcmp(s, "B64") == 0 ||
		    strcmp(s, "b64") == 0 || strcmp(s, "BASE64") == 0 ||
		    strcmp(s, "base64") == 0) {
...
int opt_cipher_silent(const char *name, EVP_CIPHER **cipherp)
{
	...
	ERR_set_mark();
	if ((c = EVP_CIPHER_fetch(app_get0_libctx(), name, app_get0_propq())) !=
		    NULL ||
	    (opt_legacy_okay() &&
...
int opt_cipher(const char *name, EVP_CIPHER **cipherp)
{
	...
	if (opt_cipher_any(name, &c)) {
		...
		if (mode == EVP_CIPH_XTS_MODE) {
			...
			opt_printf_stderr(
				"%s: ENC-then-MAC cipher not supported\n",
				prog);
		} else {
			ret = 1;
			if (cipherp != NULL)
...
int opt_md_silent(const char *name, EVP_MD **mdp)
{
	...
	ERR_set_mark();
	if ((md = EVP_MD_fetch(app_get0_libctx(), name, app_get0_propq())) !=
		    NULL ||
	    (opt_legacy_okay() &&
...
int opt_uintmax(const char *value, ossl_uintmax_t *result)
{
	...
	/* Ensure that the value in |m| is never too big for |*result| */
	if (sizeof(m) > sizeof(*result) && m > OSSL_UINTMAX_MAX) {
		opt_number_error(value);
		return 0;
	}
...
int opt_ulong(const char *value, unsigned long *result)
{
	...
	if (*endptr || endptr == value ||
	    ((l == ULONG_MAX) && errno == ERANGE) || (l == 0 && errno != 0)) {
		opt_number_error(value);
		errno = oerrno;
		return 0;
	}
...
int opt_verify(int opt, X509_VERIFY_PARAM *vpm)
{
	...
	case OPT_V_POLICY:
		...
		if (!X509_VERIFY_PARAM_add0_policy(vpm, otmp)) {
			...
			opt_printf_stderr(
				"%s: Internal error adding Policy %s\n", prog,
				opt_arg());
			return 0;
		}
		break;
...
int opt_isdir(const char *name)
{
	...
	if (!MultiByteToWideChar(CP_ACP, 0, name, (int)len_0, tempname,
				 MAX_PATH))
#endif
		for (i = 0; i < len_0; i++)
