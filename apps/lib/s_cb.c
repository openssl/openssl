...
int verify_callback(int ok, X509_STORE_CTX *ctx)
{
	...
	case X509_V_ERR_OCSP_NO_RESPONSE:
		if (!verify_args.quiet)
			BIO_printf(
				bio_err,
				"no OCSP response(s) for certificate(s) found.\n");
		break;
	}
...
static void ssl_print_client_cert_types(BIO *bio, SSL *s)
{
	...
	for (i = 0; i < cert_type_num; i++) {
		...
		const char *cname =
			lookup((int)cert_type, cert_type_list, NULL);
...
static int do_print_sigalgs(BIO *out, SSL *s, int shared)
{
	...
	if (shared)
		nsig = SSL_get_shared_sigalgs(s, 0, NULL, NULL, NULL, NULL,
					      NULL);
	else
...
int ssl_print_tmp_key(BIO *out, SSL *s)
{
	...
	case EVP_PKEY_EC: {
		...
		if (!EVP_PKEY_get_utf8_string_param(
			    key, OSSL_PKEY_PARAM_GROUP_NAME, name, sizeof(name),
			    &name_len))
			strcpy(name, "?");
...
static STRINT_PAIR signature_tls13_scheme_list[] = {
	...
	{ "rsa_pss_rsae_sha256",
	  0x0804 /* TLSEXT_SIGALG_rsa_pss_rsae_sha256 */ },
	{ "rsa_pss_rsae_sha384",
	  0x0805 /* TLSEXT_SIGALG_rsa_pss_rsae_sha384 */ },
	{ "rsa_pss_rsae_sha512",
	  0x0806 /* TLSEXT_SIGALG_rsa_pss_rsae_sha512 */ },
...
int load_excert(SSL_EXCERT **pexc)
{
	...
	for (; exc; exc = exc->next) {
		...
		if (exc->cert == NULL)
			return 0;
		if (exc->keyfile != NULL) {
			exc->key = load_key(exc->keyfile, exc->keyform, 0, NULL,
					    NULL, "server key");
		} else {
			exc->key = load_key(exc->certfile, exc->certform, 0,
					    NULL, NULL, "server key");
		}
		if (exc->key == NULL)
			return 0;
...
void print_verify_detail(SSL *s, BIO *bio)
{
	...
	if ((mdpth = SSL_get0_dane_authority(s, NULL, &mspki)) >= 0) {
		...
		mdpth = SSL_get0_dane_tlsa(s, &usage, &selector, &mtype, &data,
					   &dlen);
...
#define TLSA_TAIL_SIZE 12
		if (dlen > TLSA_TAIL_SIZE)
			hexdata = hexencode(data + dlen - TLSA_TAIL_SIZE,
					    TLSA_TAIL_SIZE);
		else
			hexdata = hexencode(data, dlen);
		BIO_printf(bio, "DANE TLSA %d %d %d %s%s ", usage, selector,
			   mtype, (dlen > TLSA_TAIL_SIZE) ? "..." : "",
			   hexdata);
...
typedef struct {
	BIO *out;
	int verbose;
	int (*old_cb)(const SSL *s, const SSL_CTX *ctx, int op, int bits,
				   ...
				   int bits, int nid, void *other, void *ex)
{
	...
	case SSL_SECOP_OTHER_SIGALG: {
		...
		int raw_sig_code = (salg[0] << 8) +
				   salg[1]; /* always big endian (msb, lsb) */
		...
		if (sname != NULL) {
			...
			const char *alg_str = lookup(
				alg_code, signature_tls12_alg_list, NULL);
			const char *hash_str = lookup(
				hash_code, signature_tls12_hash_list, NULL);

			if (alg_str != NULL && hash_str != NULL)
				BIO_printf(sdb->out, " digest=%s, algorithm=%s",
					   hash_str, alg_str);
			else
...
void print_ca_names(BIO *bio, SSL *s)
{
	...
	for (i = 0; i < sk_X509_NAME_num(sk); i++) {
		X509_NAME_print_ex(bio, sk_X509_NAME_value(sk, i), 0,
				   get_nameopt());
		BIO_write(bio, "\n", 1);
	}
}
