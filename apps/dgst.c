...
opthelp:
		...
		case OPT_MACOPT:
			...
			if (!macopts ||
			    !sk_OPENSSL_STRING_push(macopts, opt_arg()))
				goto opthelp;
			break;
		case OPT_DIGEST:
	...
	if ((!(mac_name == NULL) + !(keyfile == NULL) + !(hmac_key == NULL)) >
	    1) {
		BIO_printf(bio_err,
	...
	if (keyfile != NULL) {
		if (want_pub)
			sigkey = load_pubkey(keyfile, keyform, 0, NULL, e,
					     "public key");
		else
			sigkey = load_key(keyfile, keyform, 0, passin, e,
					  "private key");
		if (sigkey == NULL) {
			/*
		...
		}
		{
			...
			if (EVP_PKEY_get_default_digest_name(
				    sigkey, def_md, sizeof(def_md)) == 2 &&
	...
	if (mac_name != NULL) {
		...
		if (macopts != NULL) {
			for (i = 0; i < sk_OPENSSL_STRING_num(macopts); i++) {
				char *macopt =
					sk_OPENSSL_STRING_value(macopts, i);
	...
	if (sigkey != NULL) {
		...
		if (do_verify)
			if (impl == NULL)
				res = EVP_DigestVerifyInit_ex(mctx, &pctx,
							      digestname,
							      app_get0_libctx(),
							      app_get0_propq(),
							      sigkey, NULL);
			else
				res = EVP_DigestVerifyInit(mctx, &pctx, md,
							   impl, sigkey);
		else if (impl == NULL)
			res = EVP_DigestSignInit_ex(mctx, &pctx, digestname,
						    app_get0_libctx(),
						    app_get0_propq(), sigkey,
						    NULL);
		else
		...
		if (sigopts != NULL) {
			for (i = 0; i < sk_OPENSSL_STRING_num(sigopts); i++) {
				char *sigopt =
					sk_OPENSSL_STRING_value(sigopts, i);
	...
	/* we use md as a filter, reading from 'in' */
		...
		if (oneshot_sign) {
			BIO_printf(bio_err,
				   "Oneshot algorithms don't use a digest\n");
			goto end;
		}
	...
	if (argc == 0) {
		...
		if (oneshot_sign)
			ret = do_fp_oneshot_sign(out, signctx, in, separator,
						 out_bin, sigkey, sigbuf,
						 siglen, NULL, "stdin");
		else
			ret = do_fp(out, buf, inp, separator, out_bin, xoflen,
				    sigkey, sigbuf, siglen, NULL, md_name,
				    "stdin");
	} else {
		...
		for (i = 0; i < argc; i++) {
			if (BIO_read_filename(in, argv[i]) <= 0) {
				...
				if (oneshot_sign) {
					if (do_fp_oneshot_sign(
						    out, signctx, in, separator,
						    out_bin, sigkey, sigbuf,
						    siglen, sig_name, argv[i]))
						ret = EXIT_FAILURE;
				} else {
					if (do_fp(out, buf, inp, separator,
						  out_bin, xoflen, sigkey,
						  sigbuf, siglen, sig_name,
						  md_name, argv[i]))
						ret = EXIT_FAILURE;
				}
			}
			      ...
			      const char *file)
{
	...
	if (key != NULL) {
		...
		BIO_printf(bio_err,
			   "key must be set for one-shot algorithms\n");
		goto end;
	}
