...
static const int aead_lengths_list[] = { 2, 31, 136, 1024, 8 * 1024, 16 * 1024 };
...
const OPTIONS speed_options[] = {
	...
	{ "kem-algorithms", OPT_KEM, '-', "Benchmark KEM algorithms" },
...
static const char *names[ALGOR_NUM] = { "md2",
					"mdc2",
					"md4",
					"md5",
					"sha1",
...
static const OPT_PAIR dsa_choices[DSA_NUM] = { { "dsa1024", R_DSA_1024 },
					       { "dsa2048", R_DSA_2048 } };
...
static const OPT_PAIR rsa_choices[RSA_NUM] = {
	{ "rsa512", R_RSA_512 },    { "rsa1024", R_RSA_1024 },
	{ "rsa2048", R_RSA_2048 },  { "rsa3072", R_RSA_3072 },
	{ "rsa4096", R_RSA_4096 },  { "rsa7680", R_RSA_7680 },
...
static const OPT_PAIR ffdh_choices[FFDH_NUM] = {
	{ "ffdh2048", R_FFDH_2048 }, { "ffdh3072", R_FFDH_3072 },
	{ "ffdh4096", R_FFDH_4096 }, { "ffdh6144", R_FFDH_6144 },
...
static const OPT_PAIR ecdsa_choices[ECDSA_NUM] = {
	{ "ecdsap160", R_EC_P160 },	    { "ecdsap192", R_EC_P192 },
	{ "ecdsap224", R_EC_P224 },	    { "ecdsap256", R_EC_P256 },
	{ "ecdsap384", R_EC_P384 },	    { "ecdsap521", R_EC_P521 },
#ifndef OPENSSL_NO_EC2M
	{ "ecdsak163", R_EC_K163 },	    { "ecdsak233", R_EC_K233 },
	{ "ecdsak283", R_EC_K283 },	    { "ecdsak409", R_EC_K409 },
	{ "ecdsak571", R_EC_K571 },	    { "ecdsab163", R_EC_B163 },
	{ "ecdsab233", R_EC_B233 },	    { "ecdsab283", R_EC_B283 },
	{ "ecdsab409", R_EC_B409 },	    { "ecdsab571", R_EC_B571 },
#endif
	{ "ecdsabrp256r1", R_EC_BRP256R1 }, { "ecdsabrp256t1", R_EC_BRP256T1 },
	{ "ecdsabrp384r1", R_EC_BRP384R1 }, { "ecdsabrp384t1", R_EC_BRP384T1 },
	{ "ecdsabrp512r1", R_EC_BRP512R1 }, { "ecdsabrp512t1", R_EC_BRP512T1 }
};
enum {
...
static const OPT_PAIR ecdh_choices[EC_NUM] = {
	{ "ecdhp160", R_EC_P160 },	   { "ecdhp192", R_EC_P192 },
	{ "ecdhp224", R_EC_P224 },	   { "ecdhp256", R_EC_P256 },
	{ "ecdhp384", R_EC_P384 },	   { "ecdhp521", R_EC_P521 },
#ifndef OPENSSL_NO_EC2M
	{ "ecdhk163", R_EC_K163 },	   { "ecdhk233", R_EC_K233 },
	{ "ecdhk283", R_EC_K283 },	   { "ecdhk409", R_EC_K409 },
	{ "ecdhk571", R_EC_K571 },	   { "ecdhb163", R_EC_B163 },
	{ "ecdhb233", R_EC_B233 },	   { "ecdhb283", R_EC_B283 },
	{ "ecdhb409", R_EC_B409 },	   { "ecdhb571", R_EC_B571 },
#endif
	{ "ecdhbrp256r1", R_EC_BRP256R1 }, { "ecdhbrp256t1", R_EC_BRP256T1 },
	{ "ecdhbrp384r1", R_EC_BRP384R1 }, { "ecdhbrp384t1", R_EC_BRP384T1 },
	{ "ecdhbrp512r1", R_EC_BRP512R1 }, { "ecdhbrp512t1", R_EC_BRP512T1 },
#ifndef OPENSSL_NO_ECX
	{ "ecdhx25519", R_EC_X25519 },	   { "ecdhx448", R_EC_X448 }
#endif
};
...
static const OPT_PAIR eddsa_choices[EdDSA_NUM] = { { "ed25519", R_EC_Ed25519 },
						   { "ed448", R_EC_Ed448 }
...
static const OPT_PAIR sm2_choices[SM2_NUM] = { { "curveSM2", R_EC_CURVESM2 } };
...
static int EVP_Digest_loop(const char *mdname, ossl_unused int algindex,
			   void *args)
{
	...
	if (EVP_MD_xof(md)) {
		...
		for (count = 0; COND(c[algindex][testnum]); count++) {
			if (!EVP_Digest(buf, (size_t)lengths[testnum], digest,
					NULL, md, NULL)) {
				count = -1;
				break;
			}
...
static int mac_setup(const char *name, EVP_MAC **mac, OSSL_PARAM params[],
...
static void mac_teardown(EVP_MAC **mac, loopargs_t *loopargs,
			 unsigned int loopargs_len)
{
	unsigned int i;
...
static int EVP_Cipher_loop(void *args)
{
	...
	if (tempargs->ctx == NULL)
		return -1;
	for (count = 0; COND(c[algindex][testnum]); count++)
		if (EVP_Cipher(tempargs->ctx, buf, buf,
			       (size_t)lengths[testnum]) <= 0)
			return -1;
	return count;
}
...
static int EVP_Update_loop(void *args)
{
	...
	if (decrypt) {
		for (count = 0; COND(c[D_EVP][testnum]); count++) {
			rc = EVP_DecryptUpdate(ctx, buf, &outl, buf,
					       lengths[testnum]);
			if (rc != 1) {
				/* reset iv in case of counter overflow */
				rc = EVP_CipherInit_ex(ctx, NULL, NULL, NULL,
						       iv, -1);
			}
		}
	} else {
		for (count = 0; COND(c[D_EVP][testnum]); count++) {
			rc = EVP_EncryptUpdate(ctx, buf, &outl, buf,
					       lengths[testnum]);
			if (rc != 1) {
				/* reset iv in case of counter overflow */
				rc = EVP_CipherInit_ex(ctx, NULL, NULL, NULL,
...
static int EVP_Update_loop_aead_enc(void *args)
{
	...
	for (count = 0; COND(c[D_EVP][testnum]); count++) {
		...
		if (mode_op == EVP_CIPH_CCM_MODE) {
			if (!EVP_EncryptUpdate(ctx, NULL, &outl, NULL,
					       lengths[testnum])) {
				BIO_printf(
					bio_err,
		...
		if (aead) {
			if (!EVP_EncryptUpdate(ctx, NULL, &outl, aad,
					       sizeof(aad))) {
				BIO_printf(
					bio_err,
		...
		if (!EVP_EncryptUpdate(ctx, buf, &outl, buf,
				       lengths[testnum])) {
...
static int EVP_Update_loop_aead_dec(void *args)
{
	...
	for (count = 0; COND(c[D_EVP][testnum]); count++) {
		...
		if (mode_op != EVP_CIPH_SIV_MODE) {
			if (!EVP_DecryptInit_ex(ctx, NULL, NULL, NULL,
						aead_iv)) {
				BIO_printf(bio_err, "\nFailed to set iv\n");
				dofail();
				exit(1);
		...
		if (mode_op == EVP_CIPH_CCM_MODE) {
			if (!EVP_DecryptUpdate(ctx, NULL, &outl, NULL,
					       lengths[testnum])) {
				BIO_printf(
					bio_err,
		...
		if (aead) {
			if (!EVP_DecryptUpdate(ctx, NULL, &outl, aad,
					       sizeof(aad))) {
				BIO_printf(
					bio_err,
		...
		if (!EVP_DecryptUpdate(ctx, outbuf, &outl, buf,
				       lengths[testnum])) {
...
static int RSA_sign_loop(void *args)
{
	...
	for (count = 0; COND(rsa_c[testnum][0]); count++) {
		*rsa_num = tempargs->buflen;
		ret = EVP_PKEY_sign(rsa_sign_ctx[testnum], buf2, rsa_num, buf,
				    36);
		if (ret <= 0) {
...
static int RSA_verify_loop(void *args)
{
	...
	for (count = 0; COND(rsa_c[testnum][1]); count++) {
		ret = EVP_PKEY_verify(rsa_verify_ctx[testnum], buf2, rsa_num,
				      buf, 36);
		if (ret <= 0) {
...
static int RSA_encrypt_loop(void *args)
{
	...
	for (count = 0; COND(rsa_c[testnum][2]); count++) {
		*rsa_num = tempargs->buflen;
		ret = EVP_PKEY_encrypt(rsa_encrypt_ctx[testnum], buf2, rsa_num,
				       buf, 36);
		if (ret <= 0) {
...
static int RSA_decrypt_loop(void *args)
{
	...
	for (count = 0; COND(rsa_c[testnum][3]); count++) {
		rsa_num = tempargs->buflen;
		ret = EVP_PKEY_decrypt(rsa_decrypt_ctx[testnum], buf, &rsa_num,
				       buf2, tempargs->encsize);
		if (ret <= 0) {
...
static int DSA_sign_loop(void *args)
{
	...
	for (count = 0; COND(dsa_c[testnum][0]); count++) {
		*dsa_num = tempargs->buflen;
		ret = EVP_PKEY_sign(dsa_sign_ctx[testnum], buf2, dsa_num, buf,
				    20);
		if (ret <= 0) {
...
static int DSA_verify_loop(void *args)
{
	...
	for (count = 0; COND(dsa_c[testnum][1]); count++) {
		ret = EVP_PKEY_verify(dsa_verify_ctx[testnum], buf2, dsa_num,
				      buf, 20);
		if (ret <= 0) {
...
static int ECDSA_verify_loop(void *args)
{
	...
	for (count = 0; COND(ecdsa_c[testnum][1]); count++) {
		ret = EVP_PKEY_verify(ecdsa_verify_ctx[testnum], buf2,
				      ecdsa_num, buf, 20);
		if (ret <= 0) {
...
static int EdDSA_sign_loop(void *args)
{
	...
	for (count = 0; COND(eddsa_c[testnum][0]); count++) {
		ret = EVP_DigestSignInit(edctx[testnum], NULL, NULL, NULL,
					 NULL);
		if (ret == 0) {
		...
		ret = EVP_DigestSign(edctx[testnum], eddsasig, eddsasigsize,
				     buf, 20);
		if (ret == 0) {
...
static int EdDSA_verify_loop(void *args)
{
	...
	for (count = 0; COND(eddsa_c[testnum][1]); count++) {
		ret = EVP_DigestVerifyInit(edctx[testnum], NULL, NULL, NULL,
					   NULL);
		if (ret == 0) {
		...
		ret = EVP_DigestVerify(edctx[testnum], eddsasig, eddsasigsize,
				       buf, 20);
		if (ret != 1) {
...
static int SM2_sign_loop(void *args)
{
	...
	for (count = 0; COND(sm2_c[testnum][0]); count++) {
		...
		if (!EVP_DigestSignInit(sm2ctx[testnum], NULL, EVP_sm3(), NULL,
					sm2_pkey[testnum])) {
		...
		ret = EVP_DigestSign(sm2ctx[testnum], sm2sig, &sm2sigsize, buf,
				     20);
		if (ret == 0) {
...
static int SM2_verify_loop(void *args)
{
	...
	for (count = 0; COND(sm2_c[testnum][1]); count++) {
		...
		ret = EVP_DigestVerify(sm2ctx[testnum], sm2sig, sm2sigsize, buf,
				       20);
		if (ret != 1) {
...
static int KEM_encaps_loop(void *args)
{
	...
	for (count = 0; COND(kems_c[testnum][1]); count++) {
		if (EVP_PKEY_encapsulate(ctx, out, &out_len, secret,
					 &secret_len) <= 0)
			return -1;
	}
	return count;
...
static int KEM_decaps_loop(void *args)
{
	...
	for (count = 0; COND(kems_c[testnum][2]); count++) {
		if (EVP_PKEY_decapsulate(ctx, secret, &secret_len, out,
					 out_len) <= 0)
			return -1;
	}
	return count;
...
static int SIG_sign_loop(void *args)
{
	...
	unsigned char *sig =
		app_malloc(tempargs->sig_max_sig_len[testnum], "sig sign loop");
			 ...
			 loopargs_t *loopargs)
{
	...
	for (i = 0; i < async_jobs && !error; i++) {
		...
		ret = ASYNC_start_job(&loopargs[i].inprogress_job,
				      ...
				      loop_function, (void *)&looparg_item,
				      sizeof(looparg_item));
		switch (ret) {
		case ASYNC_PAUSE:
			++num_inprogress;
	...
	while (num_inprogress > 0) {
		...
		for (i = 0; i < async_jobs && num_inprogress > 0; i++) {
			...
			if (!ASYNC_WAIT_CTX_get_all_fds(loopargs[i].wait_ctx,
							NULL, &num_job_fds) ||
			    num_job_fds > 1) {
				BIO_printf(bio_err,
			...
			ASYNC_WAIT_CTX_get_all_fds(loopargs[i].wait_ctx,
						   &job_fd, &num_job_fds);
		...
		select_result =
			select(max_fd + 1, &waitfdset, NULL, NULL, NULL);
		...
		for (i = 0; i < async_jobs; i++) {
			...
			if (num_job_fds == 1 &&
			    !PeekNamedPipe(job_fd, NULL, 0, NULL, &avail,
					   NULL) &&
			    avail > 0)
				continue;
#endif
...
static EVP_PKEY *get_ecdsa(const EC_CURVE *curve)
{
	...
	if (kctx == NULL) {
		...
		if ((pctx = EVP_PKEY_CTX_new_from_name(NULL, "EC", NULL)) ==
		    ...
		    EVP_PKEY_CTX_set_ec_paramgen_curve_nid(pctx, curve->nid) <=
			    0 ||
...
static int kems_cmp(const EVP_KEM *const *a, const EVP_KEM *const *b)
{
...
static void collect_kem(EVP_KEM *kem, void *stack)
{
	...
	if (is_kem_fetchable(kem) && EVP_KEM_up_ref(kem) &&
	    sk_EVP_KEM_push(kem_stack, kem) <= 0)
		EVP_KEM_free(
			kem); /* up-ref successful but push to stack failed */
}
...
static void collect_signatures(EVP_SIGNATURE *sig, void *stack)
{
	...
	if (is_signature_fetchable(sig) && EVP_SIGNATURE_up_ref(sig) &&
	    sk_EVP_SIGNATURE_push(sig_stack, sig) <= 0)
		EVP_SIGNATURE_free(
			sig); /* up-ref successful but push to stack failed */
}
...
int speed_main(int argc, char **argv)
{
...
#endif
	long op_count = 1;
	openssl_speed_sec_t seconds = { SECONDS,      RSA_SECONDS,
					DSA_SECONDS,  ECDSA_SECONDS,
					ECDH_SECONDS, EdDSA_SECONDS,
					SM2_SECONDS,  FFDH_SECONDS,
					KEM_SECONDS,  SIG_SECONDS };
	...
	static const struct {
		...
		unsigned int length;
		unsigned int bits;
	} rsa_keys[] = { { test512, sizeof(test512), 512 },
	...
	OPENSSL_assert(strcmp(ecdsa_choices[ECDSA_NUM - 1].name,
			      "ecdsabrp512t1") == 0);
#endif /* OPENSSL_NO_ECX */
...
opterr:
		...
		case OPT_PRIMES:
			...
			break;
		case OPT_SECONDS:
			seconds.sym = seconds.rsa = seconds.dsa =
				seconds.ecdsa = seconds.ecdh = seconds.eddsa =
	...
	for (idx = 0; idx < (unsigned int)sk_EVP_KEM_num(kem_stack); idx++) {
		...
		if (strcmp(EVP_KEM_get0_name(kem), "RSA") == 0) {
			if (kems_algs_len + OSSL_NELEM(rsa_choices) >=
			    MAX_KEM_NUM) {
				BIO_printf(
					bio_err,
			...
			for (i = 0; i < OSSL_NELEM(rsa_choices); i++) {
				...
				kems_algname[kems_algs_len++] =
					OPENSSL_strdup(rsa_choices[i].name);
			}
	...
	for (idx = 0; idx < (unsigned int)sk_EVP_SIGNATURE_num(sig_stack);
	     idx++) {
		...
		if (strcmp(sig_name, "RSA") == 0) {
			if (sigs_algs_len + OSSL_NELEM(rsa_choices) >=
			    MAX_SIG_NUM) {
				BIO_printf(
					bio_err,
			...
			for (i = 0; i < DSA_NUM; i++) {
				sigs_doit[sigs_algs_len] = 1;
				sigs_algname[sigs_algs_len++] =
					OPENSSL_strdup(dsa_choices[i].name);
			}
		}
#endif /* OPENSSL_NO_DSA */
	...
	loopargs = app_malloc(loopargs_len * sizeof(loopargs_t),
			      "array of loopargs");
	...
	for (i = 0; i < loopargs_len; i++) {
		...
		loopargs[i].secret_a =
			app_malloc(MAX_ECDH_SIZE, "ECDH secret a");
		loopargs[i].secret_b =
			app_malloc(MAX_ECDH_SIZE, "ECDH secret b");
#ifndef OPENSSL_NO_DH
		loopargs[i].secret_ff_a =
			app_malloc(MAX_FFDH_SIZE, "FFDH secret a");
		loopargs[i].secret_ff_b =
			app_malloc(MAX_FFDH_SIZE, "FFDH secret b");
#endif
	}
	...
	if (doit[D_MD2]) {
		for (testnum = 0; testnum < size_num; testnum++) {
			print_message(names[D_MD2], lengths[testnum],
				      seconds.sym);
			Time_F(START);
			count = run_benchmark(async_jobs, EVP_Digest_MD2_loop,
					      loopargs);
			d = Time_F(STOP);
	...
	if (doit[D_MDC2]) {
		for (testnum = 0; testnum < size_num; testnum++) {
			print_message(names[D_MDC2], lengths[testnum],
				      seconds.sym);
			Time_F(START);
			count = run_benchmark(async_jobs, EVP_Digest_MDC2_loop,
					      loopargs);
			d = Time_F(STOP);
	...
	if (doit[D_SHA256]) {
		for (testnum = 0; testnum < size_num; testnum++) {
			print_message(names[D_SHA256], lengths[testnum],
				      seconds.sym);
			Time_F(START);
			count = run_benchmark(async_jobs, SHA256_loop,
					      loopargs);
			d = Time_F(STOP);
	...
	if (doit[D_WHIRLPOOL]) {
		for (testnum = 0; testnum < size_num; testnum++) {
			print_message(names[D_WHIRLPOOL], lengths[testnum],
				      seconds.sym);
			Time_F(START);
			count = run_benchmark(async_jobs, WHIRLPOOL_loop,
					      loopargs);
			d = Time_F(STOP);
	...
	if (doit[D_RMD160]) {
		for (testnum = 0; testnum < size_num; testnum++) {
			print_message(names[D_RMD160], lengths[testnum],
				      seconds.sym);
			Time_F(START);
			count = run_benchmark(async_jobs,
					      EVP_Digest_RMD160_loop, loopargs);
			d = Time_F(STOP);
	...
	if (doit[D_HMAC]) {
		...
		BIO_snprintf(evp_hmac_name, hmac_name_len, "hmac(%s)",
			     evp_mac_mdname);
		names[D_HMAC] = evp_hmac_name;

		params[0] = OSSL_PARAM_construct_utf8_string(
			OSSL_MAC_PARAM_DIGEST, evp_mac_mdname, 0);
		params[1] = OSSL_PARAM_construct_octet_string(
			OSSL_MAC_PARAM_KEY, (char *)hmac_key, len);
	...
	if (doit[D_CBC_DES]) {
		...
		for (i = 0; st && i < loopargs_len; i++) {
			loopargs[i].ctx = init_evp_cipher_ctx(
				"des-cbc", deskey, sizeof(deskey) / 3);
			st = loopargs[i].ctx != NULL;
		}
		algindex = D_CBC_DES;
		for (testnum = 0; st && testnum < size_num; testnum++) {
			if (!check_block_size(loopargs[0].ctx,
					      lengths[testnum]))
				break;
			print_message(names[D_CBC_DES], lengths[testnum],
				      seconds.sym);
			Time_F(START);
			count = run_benchmark(async_jobs, EVP_Cipher_loop,
					      loopargs);
			d = Time_F(STOP);
	...
	if (doit[D_EDE3_DES]) {
		...
		for (i = 0; st && i < loopargs_len; i++) {
			loopargs[i].ctx = init_evp_cipher_ctx(
				"des-ede3-cbc", deskey, sizeof(deskey));
			st = loopargs[i].ctx != NULL;
		}
		algindex = D_EDE3_DES;
		for (testnum = 0; st && testnum < size_num; testnum++) {
			if (!check_block_size(loopargs[0].ctx,
					      lengths[testnum]))
				break;
			print_message(names[D_EDE3_DES], lengths[testnum],
				      seconds.sym);
			Time_F(START);
			count = run_benchmark(async_jobs, EVP_Cipher_loop,
					      loopargs);
	...
	for (k = 0; k < 3; k++) {
		...
		if (doit[algindex]) {
			...
			for (testnum = 0; st && testnum < size_num; testnum++) {
				...
				Time_F(START);
				count = run_benchmark(
					async_jobs, EVP_Cipher_loop, loopargs);
				d = Time_F(STOP);
	...
	if (doit[D_GHASH]) {
		...
		params[0] = OSSL_PARAM_construct_utf8_string(
			OSSL_ALG_PARAM_CIPHER, "aes-128-gcm", 0);
		params[1] = OSSL_PARAM_construct_octet_string(
			OSSL_MAC_PARAM_IV, (char *)gmac_iv,
			sizeof(gmac_iv) - 1);
		params[2] = OSSL_PARAM_construct_octet_string(
			OSSL_MAC_PARAM_KEY, (void *)key32, 16);
		...
		for (testnum = 0; testnum < size_num; testnum++) {
			print_message(names[D_GHASH], lengths[testnum],
				      seconds.sym);
			Time_F(START);
	...
	if (doit[D_RAND]) {
		for (testnum = 0; testnum < size_num; testnum++) {
			print_message(names[D_RAND], lengths[testnum],
				      seconds.sym);
			Time_F(START);
			count = run_benchmark(async_jobs, RAND_bytes_loop,
					      loopargs);
			d = Time_F(STOP);
	...
	if (doit[D_EVP]) {
		if (evp_cipher != NULL) {
			...
			if (multiblock && (EVP_CIPHER_get_flags(evp_cipher) &
					   EVP_CIPH_FLAG_TLS1_1_MULTIBLOCK)) {
				multiblock_speed(evp_cipher, lengths_single,
						 &seconds);
				ret = 0;
				goto end;
			}
			...
			for (testnum = 0; testnum < size_num; testnum++) {
				...
				for (k = 0; k < loopargs_len; k++) {
					...
					keylen = EVP_CIPHER_CTX_get_key_length(
						loopargs[k].ctx);
					loopargs[k].key = app_malloc(
						keylen, "evp_cipher key");
				...
				Time_F(START);
				count = run_benchmark(async_jobs, loopfunc,
						      loopargs);
				d = Time_F(STOP);
				for (k = 0; k < loopargs_len; k++) {
					OPENSSL_clear_free(loopargs[k].key,
							   keylen);
	...
	if (doit[D_EVP_CMAC]) {
		...
		BIO_snprintf(evp_cmac_name, len, "cmac(%s)",
			     evp_mac_ciphername);
		names[D_EVP_CMAC] = evp_cmac_name;

		params[0] = OSSL_PARAM_construct_utf8_string(
			OSSL_ALG_PARAM_CIPHER, evp_mac_ciphername, 0);
		params[1] = OSSL_PARAM_construct_octet_string(
			OSSL_MAC_PARAM_KEY, (char *)key32, keylen);
		params[2] = OSSL_PARAM_construct_end();
	...
	if (doit[D_KMAC128]) {
		...
		if (mac_setup("KMAC-128", &mac, params, loopargs,
			      loopargs_len) < 1)
			goto end;
		for (testnum = 0; testnum < size_num; testnum++) {
			print_message(names[D_KMAC128], lengths[testnum],
				      seconds.sym);
			Time_F(START);
			count = run_benchmark(async_jobs, KMAC128_loop,
					      loopargs);
			d = Time_F(STOP);
	...
	if (doit[D_KMAC256]) {
		...
		params[0] = OSSL_PARAM_construct_octet_string(
			OSSL_MAC_PARAM_KEY, (void *)key32, 32);
		params[1] = OSSL_PARAM_construct_end();

		if (mac_setup("KMAC-256", &mac, params, loopargs,
			      loopargs_len) < 1)
			goto end;
		for (testnum = 0; testnum < size_num; testnum++) {
			print_message(names[D_KMAC256], lengths[testnum],
				      seconds.sym);
			Time_F(START);
			count = run_benchmark(async_jobs, KMAC256_loop,
					      loopargs);
			d = Time_F(STOP);
	...
	for (testnum = 0; testnum < RSA_NUM; testnum++) {
		...
		if (primes > RSA_DEFAULT_PRIME_NUM) {
			...
			bn = BN_new();
			st = bn != NULL && BN_set_word(bn, RSA_F4) &&
			     init_gen_str(&genctx, "RSA", NULL, 0, NULL,
					  NULL) &&
			     EVP_PKEY_CTX_set_rsa_keygen_bits(
				     genctx, rsa_keys[testnum].bits) > 0 &&
			     EVP_PKEY_CTX_set1_rsa_keygen_pubexp(genctx, bn) >
				     0 &&
			     EVP_PKEY_CTX_set_rsa_keygen_primes(genctx,
								primes) > 0 &&
			...
			st = (rsa_key = d2i_PrivateKey(
				      EVP_PKEY_RSA, NULL, &p,
				      rsa_keys[testnum].length)) != NULL;
		}

		for (i = 0; st && i < loopargs_len; i++) {
			loopargs[i].rsa_sign_ctx[testnum] =
				EVP_PKEY_CTX_new(rsa_key, NULL);
			loopargs[i].sigsize = loopargs[i].buflen;
			if (loopargs[i].rsa_sign_ctx[testnum] == NULL ||
			    EVP_PKEY_sign_init(
				    loopargs[i].rsa_sign_ctx[testnum]) <= 0 ||
			    EVP_PKEY_sign(loopargs[i].rsa_sign_ctx[testnum],
					  loopargs[i].buf2,
					  &loopargs[i].sigsize, loopargs[i].buf,
					  36) <= 0)
				st = 0;
		}
		if (!st) {
			...
			Time_F(START);
			count = run_benchmark(async_jobs, RSA_sign_loop,
					      loopargs);
			d = Time_F(STOP);
			BIO_printf(
				bio_err,
		...
		for (i = 0; st && i < loopargs_len; i++) {
			loopargs[i].rsa_verify_ctx[testnum] =
				EVP_PKEY_CTX_new(rsa_key, NULL);
			if (loopargs[i].rsa_verify_ctx[testnum] == NULL ||
			    EVP_PKEY_verify_init(
				    loopargs[i].rsa_verify_ctx[testnum]) <= 0 ||
		...
		if (!st) {
			...
			pkey_print_message("public", "rsa verify",
					   rsa_keys[testnum].bits, seconds.rsa);
			Time_F(START);
			count = run_benchmark(async_jobs, RSA_verify_loop,
					      loopargs);
			d = Time_F(STOP);
			BIO_printf(
				bio_err,
		...
		for (i = 0; st && i < loopargs_len; i++) {
			loopargs[i].rsa_encrypt_ctx[testnum] =
				EVP_PKEY_CTX_new(rsa_key, NULL);
			...
			if (loopargs[i].rsa_encrypt_ctx[testnum] == NULL ||
			    ...
			    EVP_PKEY_encrypt(
				    loopargs[i].rsa_encrypt_ctx[testnum],
				    loopargs[i].buf2, &loopargs[i].encsize,
				    loopargs[i].buf, 36) <= 0)
				st = 0;
		}
		if (!st) {
			...
			/* RSA_blinding_on(rsa_key[testnum],NULL); */
			Time_F(START);
			count = run_benchmark(async_jobs, RSA_encrypt_loop,
					      loopargs);
			d = Time_F(STOP);
			BIO_printf(
				bio_err,
		...
		for (i = 0; st && i < loopargs_len; i++) {
			loopargs[i].rsa_decrypt_ctx[testnum] =
				EVP_PKEY_CTX_new(rsa_key, NULL);
			...
			if (loopargs[i].rsa_decrypt_ctx[testnum] == NULL ||
			    ...
			    EVP_PKEY_decrypt(
				    loopargs[i].rsa_decrypt_ctx[testnum],
				    loopargs[i].buf, &declen, loopargs[i].buf2,
		...
		if (!st) {
			...
			/* RSA_blinding_on(rsa_key[testnum],NULL); */
			Time_F(START);
			count = run_benchmark(async_jobs, RSA_decrypt_loop,
					      loopargs);
			d = Time_F(STOP);
			BIO_printf(
				bio_err,
	...
	for (testnum = 0; testnum < DSA_NUM; testnum++) {
		...
		for (i = 0; st && i < loopargs_len; i++) {
			loopargs[i].dsa_sign_ctx[testnum] =
				EVP_PKEY_CTX_new(dsa_key, NULL);
			loopargs[i].sigsize = loopargs[i].buflen;
			if (loopargs[i].dsa_sign_ctx[testnum] == NULL ||
			    EVP_PKEY_sign_init(
				    loopargs[i].dsa_sign_ctx[testnum]) <= 0 ||
		...
		if (!st) {
			...
			pkey_print_message("sign", "dsa", dsa_bits[testnum],
					   seconds.dsa);
			Time_F(START);
			count = run_benchmark(async_jobs, DSA_sign_loop,
					      loopargs);
			d = Time_F(STOP);
			BIO_printf(bio_err,
				   mr ? "+R5:%ld:%u:%.2f\n" :
		...
		for (i = 0; st && i < loopargs_len; i++) {
			loopargs[i].dsa_verify_ctx[testnum] =
				EVP_PKEY_CTX_new(dsa_key, NULL);
			if (loopargs[i].dsa_verify_ctx[testnum] == NULL ||
			    EVP_PKEY_verify_init(
				    loopargs[i].dsa_verify_ctx[testnum]) <= 0 ||
		...
		if (!st) {
			...
			pkey_print_message("verify", "dsa", dsa_bits[testnum],
					   seconds.dsa);
			Time_F(START);
			count = run_benchmark(async_jobs, DSA_verify_loop,
					      loopargs);
			d = Time_F(STOP);
			BIO_printf(bio_err,
				   mr ? "+R6:%ld:%u:%.2f\n" :
	...
	for (testnum = 0; testnum < ECDSA_NUM; testnum++) {
		...
		for (i = 0; st && i < loopargs_len; i++) {
			loopargs[i].ecdsa_sign_ctx[testnum] =
				EVP_PKEY_CTX_new(ecdsa_key, NULL);
			loopargs[i].sigsize = loopargs[i].buflen;
			if (loopargs[i].ecdsa_sign_ctx[testnum] == NULL ||
			    EVP_PKEY_sign_init(
				    loopargs[i].ecdsa_sign_ctx[testnum]) <= 0 ||
		...
		if (!st) {
			...
			pkey_print_message("sign", "ecdsa",
					   ec_curves[testnum].bits,
					   seconds.ecdsa);
			Time_F(START);
			count = run_benchmark(async_jobs, ECDSA_sign_loop,
					      loopargs);
			d = Time_F(STOP);
			BIO_printf(bio_err,
				   mr ? "+R7:%ld:%u:%.2f\n" :
		...
		for (i = 0; st && i < loopargs_len; i++) {
			loopargs[i].ecdsa_verify_ctx[testnum] =
				EVP_PKEY_CTX_new(ecdsa_key, NULL);
		...
		if (!st) {
			...
			pkey_print_message("verify", "ecdsa",
					   ...
					   seconds.ecdsa);
			Time_F(START);
			count = run_benchmark(async_jobs, ECDSA_verify_loop,
					      loopargs);
			d = Time_F(STOP);
			BIO_printf(
				bio_err,
	...
	for (testnum = 0; testnum < EC_NUM; testnum++) {
		...
		for (i = 0; i < loopargs_len; i++) {
			...
			if ((key_A = get_ecdsa(&ec_curves[testnum])) ==
				    NULL /* generate secret key A */
			    || (key_B = get_ecdsa(&ec_curves[testnum])) ==
				       NULL /* generate secret key B */
			    || (ctx = EVP_PKEY_CTX_new(key_A, NULL)) ==
				       NULL /* derivation ctx from skeyA */
			    ...
			    ||
			    outlen >
				    MAX_ECDH_SIZE /* avoid buffer overflow */) {
				ecdh_checks = 0;
				BIO_printf(bio_err,
			...
			if ((test_ctx = EVP_PKEY_CTX_new(key_B, NULL)) ==
			    ...
			    || EVP_PKEY_derive(test_ctx, NULL, &test_outlen) <=
				       0 /* determine max length */
			    || EVP_PKEY_derive(ctx, loopargs[i].secret_a,
					       &outlen) <= 0 /* compute a*B */
			    ||
			    EVP_PKEY_derive(test_ctx, loopargs[i].secret_b,
					    &test_outlen) <= 0 /* compute b*A */
			    ||
		...
		if (ecdh_checks != 0) {
			pkey_print_message("", "ecdh", ec_curves[testnum].bits,
					   seconds.ecdh);
			Time_F(START);
			count = run_benchmark(
				async_jobs, ECDH_EVP_derive_key_loop, loopargs);
			d = Time_F(STOP);
			BIO_printf(bio_err,
				   mr ? "+R9:%ld:%d:%.2f\n" :
	...
	for (testnum = 0; testnum < EdDSA_NUM; testnum++) {
		...
		for (i = 0; i < loopargs_len; i++) {
			...
			if ((ed_pctx = EVP_PKEY_CTX_new_id(
				     ed_curves[testnum].nid, NULL)) == NULL ||
			...
			if (!EVP_DigestSignInit(loopargs[i].eddsa_ctx[testnum],
			...
			if (!EVP_DigestVerifyInit(
				    loopargs[i].eddsa_ctx2[testnum], NULL, NULL,
				    NULL, ed_pkey)) {
				st = 0;
				EVP_PKEY_free(ed_pkey);
				break;
		...
		if (st == 0) {
			...
			for (i = 0; i < loopargs_len; i++) {
				...
				loopargs[i].sigsize =
					ed_curves[testnum].sigsize;
				st = EVP_DigestSign(
					loopargs[i].eddsa_ctx[testnum],
					loopargs[i].buf2, &loopargs[i].sigsize,
					loopargs[i].buf, 20);
				if (st == 0)
					break;
			...
			if (st == 0) {
				...
				pkey_print_message("sign",
						   ed_curves[testnum].name,
						   ed_curves[testnum].bits,
						   seconds.eddsa);
				Time_F(START);
				count = run_benchmark(
					async_jobs, EdDSA_sign_loop, loopargs);
				d = Time_F(STOP);
	...
	for (testnum = 0; testnum < SM2_NUM; testnum++) {
		...
		for (i = 0; i < loopargs_len; i++) {
			...
			st = !((pctx = EVP_PKEY_CTX_new_id(EVP_PKEY_SM2,
							   NULL)) == NULL ||
			       EVP_PKEY_keygen_init(pctx) <= 0 ||
			       EVP_PKEY_CTX_set_ec_paramgen_curve_nid(
				       pctx, sm2_curves[testnum].nid) <= 0 ||
			...
			EVP_MD_CTX_set_pkey_ctx(
				loopargs[i].sm2_vfy_ctx[testnum], sm2_vfy_pctx);

			/*
             * No need to allow user to set an explicit ID here, just use
             * the one defined in the 'draft-yang-tls-tl13-sm-suites' I-D.
             */
			if (EVP_PKEY_CTX_set1_id(sm2_pctx, SM2_ID,
						 SM2_ID_LEN) != 1 ||
			    EVP_PKEY_CTX_set1_id(sm2_vfy_pctx, SM2_ID,
						 SM2_ID_LEN) != 1)
				break;

			if (!EVP_DigestSignInit(loopargs[i].sm2_ctx[testnum],
						NULL, EVP_sm3(), NULL,
						sm2_pkey))
				break;
			if (!EVP_DigestVerifyInit(
		...
		if (st == 0) {
			...
			if (st == 0) {
				...
				pkey_print_message("sign",
						   sm2_curves[testnum].name,
						   sm2_curves[testnum].bits,
						   seconds.sm2);
				Time_F(START);
				count = run_benchmark(async_jobs, SM2_sign_loop,
						      loopargs);
				d = Time_F(STOP);
			...
			for (i = 0; i < loopargs_len; i++) {
				st = EVP_DigestVerify(
					loopargs[i].sm2_vfy_ctx[testnum],
					loopargs[i].buf2, loopargs[i].sigsize,
					loopargs[i].buf, 20);
				if (st != 1)
					break;
			...
			if (st != 1) {
				...
				Time_F(START);
				count = run_benchmark(
					async_jobs, SM2_verify_loop, loopargs);
				d = Time_F(STOP);
				BIO_printf(
					bio_err,
					mr ? "+R13:%ld:%u:%s:%.2f\n" :
	...
	for (testnum = 0; testnum < FFDH_NUM; testnum++) {
		...
		for (i = 0; i < loopargs_len; i++) {
			...
			if (EVP_PKEY_CTX_set_dh_nid(
				    ffdh_ctx, ffdh_params[testnum].nid) <= 0) {
				BIO_printf(
					bio_err,
			...
			if (EVP_PKEY_derive(ffdh_ctx, NULL, &secret_size) <=
			    0) {
				BIO_printf(
					bio_err,
			...
			if (EVP_PKEY_derive(ffdh_ctx, loopargs[i].secret_ff_a,
					    &secret_size) <= 0) {
				BIO_printf(bio_err,
			...
			if (EVP_PKEY_derive_init(test_ctx) <= 0 ||
			    ...
			    EVP_PKEY_derive(test_ctx, loopargs[i].secret_ff_b,
					    &test_out) <= 0 ||
		...
		if (ffdh_checks != 0) {
			pkey_print_message("", "ffdh",
					   ffdh_params[testnum].bits,
					   seconds.ffdh);
			Time_F(START);
			count = run_benchmark(async_jobs, FFDH_derive_key_loop,
					      loopargs);
			d = Time_F(STOP);
			BIO_printf(bio_err,
				   mr ? "+R14:%ld:%d:%.2f\n" :
					"%ld %u-bits FFDH ops in %.2fs\n",
				   count, ffdh_params[testnum].bits, d);
	...
	for (testnum = 0; testnum < kems_algs_len; testnum++) {
		...
		for (i = 0; i < loopargs_len; i++) {
			...
			unsigned char *out = NULL, *send_secret = NULL,
				      *rcv_secret;
			unsigned int bits;
			char *name;
			...
			if (strlen(kem_name) <
				    MAX_ALGNAME_SUFFIX + 4 /* rsa+digit */
			...
			kem_gen_ctx = EVP_PKEY_CTX_new_from_name(
				app_get0_libctx(),
			...
			if ((!kem_gen_ctx ||
			     EVP_PKEY_keygen_init(kem_gen_ctx) <= 0) ||
			    (use_params && EVP_PKEY_CTX_set_params(
						   kem_gen_ctx, params) <= 0)) {
				BIO_printf(
					bio_err,
			...
			kem_encaps_ctx = EVP_PKEY_CTX_new_from_pkey(
				app_get0_libctx(), pkey, app_get0_propq());
			if (kem_encaps_ctx == NULL ||
			    EVP_PKEY_encapsulate_init(kem_encaps_ctx, NULL) <=
				    0 ||
			    (kem_type == KEM_RSA &&
			     EVP_PKEY_CTX_set_kem_op(kem_encaps_ctx,
						     "RSASVE") <= 0) ||
			    ((kem_type == KEM_EC || kem_type == KEM_X25519 ||
			      kem_type == KEM_X448) &&
			     EVP_PKEY_CTX_set_kem_op(kem_encaps_ctx, "DHKEM") <=
				     0) ||
			...
			send_secret =
				app_malloc(send_secret_len, "encaps secret");
			...
			if (kem_decaps_ctx == NULL ||
			    EVP_PKEY_decapsulate_init(kem_decaps_ctx, NULL) <=
				    0 ||
			    (kem_type == KEM_RSA &&
			     EVP_PKEY_CTX_set_kem_op(kem_decaps_ctx,
						     "RSASVE") <= 0) ||
			    ((kem_type == KEM_EC || kem_type == KEM_X25519 ||
			      kem_type == KEM_X448) &&
			     EVP_PKEY_CTX_set_kem_op(kem_decaps_ctx, "DHKEM") <=
				     0) ||
			    EVP_PKEY_decapsulate(kem_decaps_ctx, NULL,
						 &rcv_secret_len, out,
						 out_len) <= 0) {
				BIO_printf(
					bio_err,
			...
			rcv_secret =
				app_malloc(rcv_secret_len, "KEM decaps secret");
			if (rcv_secret == NULL) {
				BIO_printf(
					bio_err,
...
kem_err_break:
		...
		if (kem_checks != 0) {
			...
			Time_F(START);
			count = run_benchmark(async_jobs, KEM_keygen_loop,
					      loopargs);
			d = Time_F(STOP);
			BIO_printf(bio_err,
				   mr ? "+R15:%ld:%s:%.2f\n" :
			...
			Time_F(START);
			count = run_benchmark(async_jobs, KEM_encaps_loop,
					      loopargs);
			d = Time_F(STOP);
			BIO_printf(bio_err,
				   mr ? "+R16:%ld:%s:%.2f\n" :
			...
			Time_F(START);
			count = run_benchmark(async_jobs, KEM_decaps_loop,
					      loopargs);
			d = Time_F(STOP);
			BIO_printf(bio_err,
				   mr ? "+R17:%ld:%s:%.2f\n" :
	...
	for (testnum = 0; testnum < sigs_algs_len; testnum++) {
		...
		for (i = 0; i < loopargs_len; i++) {
			...
			if (sig_gen_ctx == NULL)
				sig_gen_ctx = EVP_PKEY_CTX_new_from_name(
					app_get0_libctx(),
             ...
             */
			ERR_set_mark();
			alg = EVP_SIGNATURE_fetch(app_get0_libctx(), sig_name,
						  app_get0_propq());
			ERR_pop_to_mark();
			...
			if (EVP_PKEY_sign_init(sig_sign_ctx) <= 0 &&
			    (alg == NULL ||
			     EVP_PKEY_sign_message_init(sig_sign_ctx, alg,
							NULL) <= 0)) {
				ERR_clear_last_mark();
				BIO_printf(
					bio_err,
			...
			if (use_params == 1 &&
			    EVP_PKEY_CTX_set_rsa_padding(
				    sig_sign_ctx, RSA_PKCS1_PADDING) <= 0) {
				BIO_printf(
					bio_err,
			...
			if (EVP_PKEY_sign(sig_sign_ctx, NULL, &max_sig_len, md,
					  md_len) <= 0) {
				BIO_printf(
					bio_err,
			...
			sig = app_malloc(sig_len = max_sig_len,
					 "signature buffer");
			if (sig == NULL) {
				BIO_printf(bio_err,
			...
			if (EVP_PKEY_sign(sig_sign_ctx, sig, &sig_len, md,
					  md_len) <= 0) {
			...
			memset(md, 0, SHA256_DIGEST_LENGTH);
			sig_verify_ctx = EVP_PKEY_CTX_new_from_pkey(
				app_get0_libctx(), pkey, app_get0_propq());
			...
			if (EVP_PKEY_verify_init(sig_verify_ctx) <= 0 &&
			    (alg == NULL ||
			     EVP_PKEY_verify_message_init(sig_verify_ctx, alg,
							  NULL) <= 0)) {
				ERR_clear_last_mark();
				BIO_printf(
					bio_err,
			...
			ERR_pop_to_mark();
			if (EVP_PKEY_verify(sig_verify_ctx, sig, sig_len, md,
					    md_len) <= 0) {
...
sig_err_break:
		...
		if (sig_checks != 0) {
			...
			Time_F(START);
			count = run_benchmark(async_jobs, SIG_keygen_loop,
					      loopargs);
			d = Time_F(STOP);
			BIO_printf(
				bio_err,
			...
			Time_F(START);
			count = run_benchmark(async_jobs, SIG_sign_loop,
					      loopargs);
			d = Time_F(STOP);
			BIO_printf(bio_err,
				   mr ? "+R19:%ld:%s:%.2f\n" :
			...
			Time_F(START);
			count = run_benchmark(async_jobs, SIG_verify_loop,
					      loopargs);
			d = Time_F(STOP);
			BIO_printf(
				bio_err,
...
show_res:
	...
	for (k = 0; k < ALGOR_NUM; k++) {
		...
		if (k == D_EVP) {
			...
			else if ((alg_name = EVP_CIPHER_get0_name(
					  evp_cipher)) == NULL)
				app_bail_out(
					"failed to get name of cipher '%s'\n",
					evp_cipher);
		}
	...
	for (k = 0; k < RSA_NUM; k++) {
		...
		if (mr)
			...
			printf("rsa %5u bits %8.6fs %8.6fs %8.6fs %8.6fs %8.1f %8.1f %8.1f %8.1f\n",
			       ...
			       1.0 / rsa_results[k][3], rsa_results[k][0],
			       rsa_results[k][1], rsa_results[k][2],
			       rsa_results[k][3]);
	}
	testnum = 1;
#ifndef OPENSSL_NO_DSA
	for (k = 0; k < DSA_NUM; k++) {
		...
		if (mr)
			...
			printf("dsa %4u bits %8.6fs %8.6fs %8.1f %8.1f\n",
			       dsa_bits[k], 1.0 / dsa_results[k][0],
			       1.0 / dsa_results[k][1], dsa_results[k][0],
			       dsa_results[k][1]);
	}
#endif /* OPENSSL_NO_DSA */
	testnum = 1;
	for (k = 0; k < OSSL_NELEM(ecdsa_doit); k++) {
		...
		if (mr)
			...
			printf("%4u bits ecdsa (%s) %8.4fs %8.4fs %8.1f %8.1f\n",
			       ...
			       1.0 / ecdsa_results[k][0],
			       1.0 / ecdsa_results[k][1], ecdsa_results[k][0],
			       ecdsa_results[k][1]);
	}
	...
	for (k = 0; k < OSSL_NELEM(eddsa_doit); k++) {
		...
		if (mr)
			printf("+F6:%u:%u:%s:%f:%f\n", k, ed_curves[k].bits,
			       ed_curves[k].name, eddsa_results[k][0],
			       eddsa_results[k][1]);
		else
			printf("%4u bits EdDSA (%s) %8.4fs %8.4fs %8.1f %8.1f\n",
			       ed_curves[k].bits, ed_curves[k].name,
			       1.0 / eddsa_results[k][0],
			       1.0 / eddsa_results[k][1], eddsa_results[k][0],
			       eddsa_results[k][1]);
	}
#endif /* OPENSSL_NO_ECX */
	...
	for (k = 0; k < OSSL_NELEM(sm2_doit); k++) {
		...
		if (mr)
			printf("+F7:%u:%u:%s:%f:%f\n", k, sm2_curves[k].bits,
			       sm2_curves[k].name, sm2_results[k][0],
			       sm2_results[k][1]);
		else
	...
	for (k = 0; k < FFDH_NUM; k++) {
		...
		if (mr)
			...
			printf("%4u bits ffdh %8.4fs %8.1f\n",
			       ffdh_params[k].bits, 1.0 / ffdh_results[k][0],
			       ffdh_results[k][0]);
	}
#endif /* OPENSSL_NO_DH */
	...
	for (k = 0; k < kems_algs_len; k++) {
		...
		if (mr)
			printf("+F9:%u:%f:%f:%f\n", k, kems_results[k][0],
			       kems_results[k][1], kems_results[k][2]);
		else
			printf("%27s %8.6fs %8.6fs %8.6fs %9.1f %9.1f %9.1f\n",
			       kem_name, 1.0 / kems_results[k][0],
			       1.0 / kems_results[k][1],
			       1.0 / kems_results[k][2], kems_results[k][0],
			       kems_results[k][1], kems_results[k][2]);
	}
	ret = 0;
	...
	for (k = 0; k < sigs_algs_len; k++) {
		...
		if (mr)
			...
			printf("%27s %8.6fs %8.6fs %8.6fs %9.1f %9.1f %9.1f\n",
			       sig_name, 1.0 / sigs_results[k][0],
			       1.0 / sigs_results[k][1],
		    ...
		    int *res)
{
	...
	if (errno == 0 && end != str && *end == 0 && min_val <= val &&
	    val < upper_val) {
		*res = (int)val;
		return 1;
	} else {
	...
	for (n = 0; n < multi; ++n) {
		...
		while (fgets(buf, sizeof(buf), f)) {
			...
			if (CHECK_AND_SKIP_PREFIX(p, "+F:")) {
				...
				if (strtoint(sstrsep(&p, sep), 0, ALGOR_NUM,
					     &alg)) {
					sstrsep(&p, sep);
				...
				tk = sstrsep(&p, sep);
				if (strtoint(tk, 0, OSSL_NELEM(rsa_results),
					     &k)) {
					sstrsep(&p, sep);
				...
				tk = sstrsep(&p, sep);
				if (strtoint(tk, 0, OSSL_NELEM(dsa_results),
					     &k)) {
					sstrsep(&p, sep);
				...
				tk = sstrsep(&p, sep);
				if (strtoint(tk, 0, OSSL_NELEM(ecdsa_results),
					     &k)) {
					sstrsep(&p, sep);
				...
				tk = sstrsep(&p, sep);
				if (strtoint(tk, 0, OSSL_NELEM(ecdh_results),
					     &k)) {
					sstrsep(&p, sep);
				...
				tk = sstrsep(&p, sep);
				if (strtoint(tk, 0, OSSL_NELEM(eddsa_results),
					     &k)) {
					sstrsep(&p, sep);
					sstrsep(&p, sep);
				...
				tk = sstrsep(&p, sep);
				if (strtoint(tk, 0, OSSL_NELEM(sm2_results),
					     &k)) {
					sstrsep(&p, sep);
					sstrsep(&p, sep);
				...
				tk = sstrsep(&p, sep);
				if (strtoint(tk, 0, OSSL_NELEM(ffdh_results),
					     &k)) {
					sstrsep(&p, sep);
				...
				tk = sstrsep(&p, sep);
				if (strtoint(tk, 0, OSSL_NELEM(kems_results),
					     &k)) {
					d = atof(sstrsep(&p, sep));
					kems_results[k][0] += d;
				...
				tk = sstrsep(&p, sep);
				if (strtoint(tk, 0, OSSL_NELEM(sigs_results),
					     &k)) {
					d = atof(sstrsep(&p, sep));
					sigs_results[k][0] += d;
...
static void multiblock_speed(const EVP_CIPHER *evp_cipher, int lengths_single,
			     const openssl_speed_sec_t *seconds)
{
	static const int mblengths_list[] = { 8 * 1024, 2 * 8 * 1024,
					      4 * 8 * 1024, 8 * 8 * 1024,
					      8 * 16 * 1024 };
	...
	if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_MAC_KEY, sizeof(no_key),
				no_key) <= 0)
		...
		for (count = 0; run && COND(count); count++) {
			...
			if (packlen > 0) {
				...
				(void)EVP_CIPHER_CTX_ctrl(
					ctx, EVP_CTRL_TLS1_1_MULTIBLOCK_ENCRYPT,
					sizeof(mb_param), &mb_param);
			} else {
				int pad;
				...
				if (RAND_bytes(inp, 16) <= 0)
					app_bail_out(
						"error setting random bytes\n");
				len += 16;
				aad[11] = (unsigned char)(len >> 8);
				aad[12] = (unsigned char)(len);
				pad = EVP_CIPHER_CTX_ctrl(
					ctx, EVP_CTRL_AEAD_TLS1_AAD,
					EVP_AEAD_TLS1_AAD_LEN, aad);
				ciph_success =
					EVP_Cipher(ctx, out, inp,
