...
opthelp:
	...
	if (pubin)
		pkey = load_pubkey(infile, informat, 1, passin, e,
				   "Public Key");
	else
	...
	if (asn1_encoding != NULL || point_format != NULL) {
		...
		if (asn1_encoding != NULL)
			*p++ = OSSL_PARAM_construct_utf8_string(
				OSSL_PKEY_PARAM_EC_ENCODING, asn1_encoding, 0);
