...
#define MADE_UP_MAX_MD_NAME_LEN 100
	...
	deftype = EVP_PKEY_get_default_digest_name(pkey, defname,
						   sizeof(defname));
...
const OPTIONS pkeyutl_options[] = {
	...
	{ "kemop", OPT_KEMOP, 's',
	  "KEM operation specific to the key algorithm" },
...
int pkeyutl_main(int argc, char **argv)
{
	...
	EVP_PKEY *pkey = NULL;
	char *infile = NULL, *outfile = NULL, *secoutfile = NULL,
	     *sigfile = NULL, *passinarg = NULL;
...
opthelp:
		...
		case OPT_PKEYOPT_PASSIN:
			if ((pkeyopts_passin == NULL &&
			     (pkeyopts_passin = sk_OPENSSL_STRING_new_null()) ==
				     NULL) ||
			    sk_OPENSSL_STRING_push(pkeyopts_passin,
						   opt_arg()) == 0) {
				BIO_puts(bio_err, "out of memory\n");
				goto end;
			}
	...
	if (pkey_op == EVP_PKEY_OP_VERIFYRECOVER &&
	    !EVP_PKEY_is_a(pkey, "RSA")) {
		BIO_printf(bio_err,
			   "%s: -verifyrecover can be used only with RSA\n",
			   prog);
		goto end;
	}
	...
	if (pkeyopts_passin != NULL) {
		...
		for (i = 0; i < num; i++) {
			...
			if (passin == NULL) {
				...
				BIO_snprintf(passwd_buf, sizeof(passwd_buf),
					     "Enter %s: ", opt);
				r = EVP_read_pw_string(passwd_buf,
						       sizeof(passwd_buf) - 1,
						       passwd_buf, 0);
				if (r < 0) {
					if (r == -2)
				...
				*passin = 0;
				passin++;
				if (app_passwd(passin, NULL, &passwd, NULL) ==
				    0) {
					BIO_printf(bio_err,
						   "failed to get '%s'\n", opt);
					goto end;
	...
	if (pkey_op == EVP_PKEY_OP_VERIFY) {
		if (rawin) {
			rv = do_raw_keyop(pkey_op, mctx, pkey, in, filesize,
					  sig, siglen, NULL, 0);
		} else {
	...
	if (rawin) {
		...
		if (kdflen != 0) {
			...
			rv = do_keyop(ctx, pkey_op, NULL, &buf_outlen, buf_in,
				      (size_t)buf_inlen, NULL, &secretlen);
		}
		if (rv > 0 &&
			...
			if (buf_outlen > 0)
				buf_out =
					app_malloc(buf_outlen, "buffer output");
			if (secretlen > 0)
				secret = app_malloc(secretlen, "secret output");
			rv = do_keyop(ctx, pkey_op, buf_out, &buf_outlen,
				      buf_in, (size_t)buf_inlen, secret,
				      &secretlen);
		}
	}
	if (rv <= 0) {
	...
	if (asn1parse) {
		if (!ASN1_parse_dump(out, buf_out, (long)buf_outlen, 1, -1))
			ERR_print_errors(
				bio_err); /* but still return success */
	} else if (hexdump) {
			      ...
			      const char *propq)
{
	...
	if (rawin) {
		...
		case EVP_PKEY_OP_SIGN:
			rv = EVP_DigestSignInit_ex(mctx, NULL, digestname,
						   libctx, propq, pkey, NULL);
			break;

		case EVP_PKEY_OP_VERIFY:
			rv = EVP_DigestVerifyInit_ex(mctx, NULL, digestname,
						     libctx, propq, pkey, NULL);
			break;
		}
		      ...
		      ENGINE *e)
{
	...
	if (strcmp(EVP_PKEY_get0_type_name(peer),
		   EVP_PKEY_get0_type_name(pkey)) != 0) {
		BIO_printf(
			bio_err,
			"Type of peer public key: %s does not match type of private key: %s\n",
			EVP_PKEY_get0_type_name(peer),
			...
			unsigned char **out, size_t *poutlen)
{
	...
	if (only_nomd(pkey)) {
		...
		case EVP_PKEY_OP_VERIFY:
			...
			rv = EVP_DigestVerify(mctx, sig, (size_t)siglen, mbuf,
					      buf_len);
			break;
		case EVP_PKEY_OP_SIGN:
			buf_len = BIO_read(in, mbuf, filesize);
			...
			if (rv == 1 && out != NULL) {
				*out = app_malloc(*poutlen, "buffer output");
				rv = EVP_DigestSign(mctx, *out, poutlen, mbuf,
						    buf_len);
			}
			break;
		}
	...
	case EVP_PKEY_OP_VERIFY:
		for (;;) {
			...
			rv = EVP_DigestVerifyUpdate(mctx, tbuf,
						    (size_t)buf_len);
			if (rv != 1) {
				BIO_printf(bio_err,
