...
int x509_main(int argc, char **argv)
{
	...
	X509_STORE *ctx = NULL;
	char *CAkeyfile = NULL, *CAserial = NULL, *pubkeyfile = NULL,
	     *alias = NULL;
	...
	char *extsect = NULL, *extfile = NULL, *passin = NULL,
	     *passinarg = NULL;
	char *infile = NULL, *outfile = NULL, *privkeyfile = NULL,
	     *CAfile = NULL;
	...
	int ocsp_uri = 0, trustout = 0, clrtrust = 0, clrreject = 0,
	    aliasout = 0;
...
opthelp:
	...
	if (preserve_dates && days != UNSET_DAYS) {
		BIO_printf(bio_err,
			   "Cannot use -preserve_dates with -days option\n");
		goto err;
	}
	if (days == UNSET_DAYS)
		days = DEFAULT_DAYS;
	...
	if (newcert && infile != NULL) {
		BIO_printf(bio_err,
			   "The -in option cannot be used with -new\n");
		goto err;
	}
	if (newcert && reqfile) {
		BIO_printf(bio_err,
	...
	if (privkeyfile != NULL) {
		privkey = load_key(privkeyfile, keyformat, 0, passin, e,
				   "private key");
		if (privkey == NULL)
			goto err;
	}
...
#define WARN_NO_CA(opt)                              \
	...
	if (extfile == NULL) {
		...
		if ((extconf = app_load_config(extfile)) == NULL)
			goto err;
		if (extsect == NULL) {
			extsect = app_conf_try_string(extconf, "default",
						      "extensions");
			if (extsect == NULL)
				extsect = "default";
		}
	...
	if (multi && (reqfile || newcert)) {
		BIO_printf(bio_err,
			   "Error: -multi cannot be used with -req or -new\n");
		goto err;
	}
	...
	if (reqfile) {
		...
		if ((pkey = X509_REQ_get0_pubkey(req)) == NULL) {
			BIO_printf(bio_err,
				   "Error unpacking public key from CSR\n");
			goto err;
		}
	...
	if (reqfile || newcert) {
		...
		if (multi) {
			...
			x = load_cert_pass(infile, informat, 1, passin,
					   "certificate");
			if (x == NULL)
				goto err;
		}
...
cert_loop:
	...
	if (CAfile != NULL) {
		xca = load_cert_pass(CAfile, CAformat, 1, passin,
				     "CA certificate");
		if (xca == NULL)
			goto err;
	}
	...
	if (trust != NULL) {
		for (i = 0; i < sk_ASN1_OBJECT_num(trust); i++)
			X509_add1_trust_object(x,
					       sk_ASN1_OBJECT_value(trust, i));
	}

	if (reject != NULL) {
		for (i = 0; i < sk_ASN1_OBJECT_num(reject); i++)
			X509_add1_reject_object(x, sk_ASN1_OBJECT_value(reject,
									i));
	}

	if (clrext && ext_names != NULL)
		BIO_printf(bio_err,
			   "Warning: Ignoring -ext since -clrext is given\n");
	for (i = X509_get_ext_count(x) - 1; i >= 0; i--) {
		...
		if (clrext ||
		    (ext_names != NULL && strstr(ext_names, sn) == NULL))
	...
	if (CAfile != NULL) {
		issuer_cert = xca;
		if (sno == NULL)
			sno = x509_load_serial(CAfile, CAserial,
					       CA_createserial);
		if (sno == NULL)
			goto err;
	...
	if (reqfile || newcert || privkey != NULL || CAfile != NULL) {
		...
		if (fissu != NULL) {
			...
			if (!X509_set_issuer_name(
				    x, X509_get_subject_name(issuer_cert)))
				goto err;
		}
	}

	X509V3_set_ctx(&ext_ctx, issuer_cert, x, NULL, NULL,
		       X509V3_CTX_REPLACE);
	...
	if (x509toreq) { /* also works in conjunction with -req */
		...
		if (extconf != NULL) {
			X509V3_set_nconf(&ext_ctx, extconf);
			if (!X509V3_EXT_REQ_add_nconf(extconf, &ext_ctx,
						      extsect, rq)) {
				BIO_printf(
					bio_err,
		...
		if (!noout) {
			if (outformat == FORMAT_ASN1) {
				X509_REQ_print_ex(out, rq, get_nameopt(),
						  X509_FLAG_COMPAT);
	...
	for (i = 1; i <= num; i++) {
		if (i == issuer) {
			...
			ASN1_INTEGER *ser;
			BIGNUM *bnser = ASN1_INTEGER_to_BN(
				X509_get0_serialNumber(x), NULL);
			...
			if (EVP_PKEY_is_a(pkey, "RSA") ||
				...
				BIO_printf(
					out,
					"No modulus for this public key type");
			}
			BIO_printf(out, "\n");
			...
			ASN1_TIME_print_ex(out, X509_get0_notBefore(x),
					   dateopt);
			BIO_puts(out, "\n");
		} else if (i == enddate) {
