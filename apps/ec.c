...
int ec_main(int argc, char **argv)
{
	...
	char *passin = NULL, *passout = NULL, *passinarg = NULL,
	     *passoutarg = NULL;
	OPTION_CHOICE o;
	int informat = FORMAT_UNDEF, outformat = FORMAT_PEM, text = 0,
	    noout = 0;
...
opthelp:
	...
	if (pubin)
		eckey = load_pubkey(infile, informat, 1, passin, e,
				    "public key");
	else
	...
	if (no_public) {
		if (!EVP_PKEY_set_int_param(
			    eckey, OSSL_PKEY_PARAM_EC_INCLUDE_PUBLIC, 0)) {
			BIO_printf(bio_err,
		...
		if (!EVP_PKEY_set_int_param(
			    eckey, OSSL_PKEY_PARAM_EC_INCLUDE_PUBLIC, 1)) {
			BIO_printf(bio_err,
	...
	if (text) {
		...
		if ((pubin &&
		     EVP_PKEY_print_public(out, eckey, 0, NULL) <= 0) ||
		    (!pubin &&
		     EVP_PKEY_print_private(out, eckey, 0, NULL) <= 0)) {
	...
	if (!noout) {
		int selection;
		const char *output_type = outformat == FORMAT_ASN1 ? "DER" :
								     "PEM";
