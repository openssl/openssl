...
static void collect_digests(EVP_MD *digest, void *stack)
{
	...
	if (is_digest_fetchable(digest) && EVP_MD_up_ref(digest) &&
		...
		EVP_MD_free(
			digest); /* up-ref successful but push to stack failed */
}
...
static void collect_kdfs(EVP_KDF *kdf, void *stack)
{
	...
	if (is_kdf_fetchable(kdf) && EVP_KDF_up_ref(kdf) &&
		...
		EVP_KDF_free(
			kdf); /* up-ref successful but push to stack failed */
}
...
{
	int ret = OPENSSL_strcasecmp(EVP_RAND_get0_name(*a),
				     EVP_RAND_get0_name(*b));

	if (ret == 0)
		ret = strcmp(
			OSSL_PROVIDER_get0_name(EVP_RAND_get0_provider(*a)),
...
static void collect_rands(EVP_RAND *rand, void *stack)
{
	...
	if (is_rand_fetchable(rand) && EVP_RAND_up_ref(rand) &&
		...
		EVP_RAND_free(
			rand); /* up-ref successful but push to stack failed */
}
...
static void collect_keymanagers(EVP_KEYMGMT *km, void *stack)
{
	...
	if (is_keymgmt_fetchable(km) && EVP_KEYMGMT_up_ref(km) &&
		...
		EVP_KEYMGMT_free(
			km); /* up-ref successful but push to stack failed */
}
	...
	for (i = 0; i < sk_EVP_KEYMGMT_num(km_stack); i++) {
		...
		if (names != NULL &&
			...
			if (desc != NULL)
				...
				BIO_printf(bio_out, "%s",
					   sk_OPENSSL_CSTRING_value(names, 0));
			BIO_printf(bio_out, "\n");
...
{
	int i, count = 0;
	STACK_OF(EVP_SIGNATURE) *sig_stack =
	...
	for (i = 0; i < sk_EVP_SIGNATURE_num(sig_stack); i++) {
		...
		if (names != NULL &&
			...
			if (verbose) {
				...
				print_param_types(
					"settable operation parameters",
					EVP_SIGNATURE_settable_ctx_params(k),
					4);
				print_param_types(
					"retrievable operation parameters",
					EVP_SIGNATURE_gettable_ctx_params(k),
					4);
			}
		}
		sk_OPENSSL_CSTRING_free(names);
...
static void list_tls_groups(int version, int all)
{
	...
	if (!SSL_CTX_get0_implemented_groups(ctx, all, groups)) {
		BIO_printf(bio_err,
			   "ERROR: getting implemented TLS group list\n");
		goto err;
	}
...
static void list_tls_signatures(void)
{
	...
	if (!OSSL_PROVIDER_do_all(NULL, list_tls_sigalg_caps,
				  &tls_sigalg_listed))
		BIO_printf(
			bio_err,
...
{
	return strcmp(
		OSSL_PROVIDER_get0_name(EVP_ASYM_CIPHER_get0_provider(*a)),
...
static void list_asymciphers(void)
{
	...
	for (i = 0; i < sk_EVP_ASYM_CIPHER_num(asymciph_stack); i++) {
		EVP_ASYM_CIPHER *k =
			sk_EVP_ASYM_CIPHER_value(asymciph_stack, i);
		STACK_OF(OPENSSL_CSTRING) *names = NULL;

		if (select_name != NULL &&
		    !EVP_ASYM_CIPHER_is_a(k, select_name))
			continue;
		...
		if (names != NULL &&
			...
			BIO_printf(bio_out, " @ %s\n",
				   OSSL_PROVIDER_get0_name(
					   EVP_ASYM_CIPHER_get0_provider(k)));
			...
			if (verbose) {
				...
				print_param_types(
					"settable operation parameters",
					EVP_ASYM_CIPHER_settable_ctx_params(k),
					4);
				print_param_types(
					"retrievable operation parameters",
					EVP_ASYM_CIPHER_gettable_ctx_params(k),
					4);
			}
		}
		sk_OPENSSL_CSTRING_free(names);
...
static void collect_kex(EVP_KEYEXCH *kex, void *stack)
{
	...
	if (is_keyexch_fetchable(kex) && EVP_KEYEXCH_up_ref(kex) &&
		...
		EVP_KEYEXCH_free(
			kex); /* up-ref successful but push to stack failed */
}
...
static void list_pkey(void)
{
	...
	if (select_name == NULL && include_legacy()) {
		...
		for (i = 0; i < EVP_PKEY_asn1_get_count(); i++) {
			...
			EVP_PKEY_asn1_get0_info(&pkey_id, &pkey_base_id,
						&pkey_flags, &pinfo, &pem_str,
						ameth);
...
{
	return strcmp(
		OSSL_PROVIDER_get0_name(OSSL_STORE_LOADER_get0_provider(*a)),
...
static void list_store_loaders(void)
{
	...
	OSSL_STORE_LOADER_do_all_provided(app_get0_libctx(),
					  collect_store_loaders, stores);
	sk_OSSL_STORE_LOADER_sort(stores);
	for (i = 0; i < sk_OSSL_STORE_LOADER_num(stores); i++) {
		const OSSL_STORE_LOADER *l =
			sk_OSSL_STORE_LOADER_value(stores, i);
		STACK_OF(OPENSSL_CSTRING) *names = NULL;

		if (select_name != NULL &&
		    !OSSL_STORE_LOADER_is_a(l, select_name))
			continue;
		...
		if (names != NULL &&
			...
			BIO_printf(bio_out, " @ %s\n",
				   OSSL_PROVIDER_get0_name(
					   OSSL_STORE_LOADER_get0_provider(l)));

			if (verbose) {
				const char *desc =
					OSSL_STORE_LOADER_get0_description(l);
...
static void list_provider_info(void)
{
	...
	for (i = 0; i < sk_OSSL_PROVIDER_num(providers); i++) {
		const OSSL_PROVIDER *prov =
			sk_OSSL_PROVIDER_value(providers, i);
		...
		params[2] = OSSL_PARAM_construct_int(OSSL_PROV_PARAM_STATUS,
						     &status);
		params[3] = OSSL_PARAM_construct_utf8_ptr(
			OSSL_PROV_PARAM_BUILDINFO, &buildinfo, 0);
...
const OPTIONS list_options[] = {
	...
	{ "skey-managers", OPT_SKEYMANAGERS, '-',
	  "List of symmetric key managers" },
