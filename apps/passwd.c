		...
		do {
			...
			if (r > 0) {
				...
				if (!do_passwd(passed_salt, &salt, &salt_malloc,
					       passwd, bio_out, quiet, table,
					       reverse, pw_maxlen, mode))
					goto end;
			}
			done = (r <= 0);
		} while (!done);
...
static char *md5crypt(const char *passwd, const char *magic, const char *salt)
{
	...
	for (i = 0; i < 1000; i++) {
		if (!EVP_DigestInit_ex(md2, EVP_md5(), NULL))
			goto err;
		if (!EVP_DigestUpdate(
			    md2, (i & 1) ? (const unsigned char *)passwd : buf,
		...
		if (!EVP_DigestUpdate(
			    md2, (i & 1) ? buf : (const unsigned char *)passwd,
...
#define ROUNDS_MAX 999999999
	...
	while (n) {
		if (!EVP_DigestUpdate(
			    md, (n & 1) ? buf : (const unsigned char *)passwd,
	...
	for (n = 0; n < rounds; n++) {
		if (!EVP_DigestInit_ex(md2, sha, NULL))
			goto err;
		if (!EVP_DigestUpdate(
			    md2, (n & 1) ? (const unsigned char *)p_bytes : buf,
		...
		if (!EVP_DigestUpdate(
			    md2, (n & 1) ? buf : (const unsigned char *)p_bytes,
			    (n & 1) ? buf_size : passwd_len))
			goto err;
		if (!EVP_DigestFinal_ex(md2, buf, NULL))
			goto err;
		     ...
		     size_t pw_maxlen, passwd_modes mode)
{
	...
	if (!passed_salt) {
		...
		if (*salt_malloc_p == NULL)
			*salt_p = *salt_malloc_p =
				app_malloc(saltlen + 1, "salt buffer");
		if (RAND_bytes((unsigned char *)*salt_p, (int)saltlen) <= 0)
			goto end;

		for (i = 0; i < saltlen; i++)
			(*salt_p)[i] =
				cov_2char[(*salt_p)[i] & 0x3f]; /* 6 bits */
		(*salt_p)[i] = 0;
#ifdef CHARSET_EBCDIC
	...
	if (mode == passwd_md5 || mode == passwd_apr1)
		hash = md5crypt(passwd, (mode == passwd_md5 ? "1" : "apr1"),
				*salt_p);
	if (mode == passwd_aixmd5)
		hash = md5crypt(passwd, "", *salt_p);
	if (mode == passwd_sha256 || mode == passwd_sha512)
		hash = shacrypt(passwd, (mode == passwd_sha256 ? "5" : "6"),
				*salt_p);
	assert(hash != NULL);
