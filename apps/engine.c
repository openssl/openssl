...
static void util_store_cap(const STORE_LOADER *loader, void *arg)
{
    struct util_store_cap_data *ctx = arg;

    if (STORE_LOADER_get0_engine(loader) == ctx->engine) {
        char buf[256];
        BIO_snprintf(buf, sizeof(buf), "STORE(%s)",
                     STORE_LOADER_get0_scheme(loader));
 ...
 skip_pmeths:
                {
                    ...
                    STORE_do_all_loaders(util_store_cap, &store_ctx);
                    if (!store_ctx.ok)
                        goto end;
                }
