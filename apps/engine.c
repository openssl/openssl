...
static int util_flags(BIO *out, unsigned int flags, const char *indent)
{
	...
	flags = flags & ~ENGINE_CMD_FLAG_NUMERIC & ~ENGINE_CMD_FLAG_STRING &
...
static int util_verbose(ENGINE *e, int verbose, BIO *out, const char *indent)
{
	...
	if (!ENGINE_ctrl(e, ENGINE_CTRL_HAS_CTRL_FUNCTION, 0, NULL, NULL) ||
	    ((num = ENGINE_ctrl(e, ENGINE_CTRL_GET_FIRST_CMD_TYPE, 0, NULL,
				NULL)) <= 0)) {
		return 1;
	}
	...
	do {
		...
		if (!(flags & ENGINE_CMD_FLAG_INTERNAL) || verbose >= 4) {
			...
			if ((len = ENGINE_ctrl(
				     e, ENGINE_CTRL_GET_NAME_LEN_FROM_CMD, num,
				     NULL, NULL)) <= 0)
				goto err;
			name = app_malloc(len + 1, "name buffer");
			if (ENGINE_ctrl(e, ENGINE_CTRL_GET_NAME_FROM_CMD, num,
					name, NULL) <= 0)
				goto err;
			/* Get the command description */
			if ((len = ENGINE_ctrl(
				     e, ENGINE_CTRL_GET_DESC_LEN_FROM_CMD, num,
				     NULL, NULL)) < 0)
				goto err;
			if (len > 0) {
				desc = app_malloc(len + 1,
						  "description buffer");
				if (ENGINE_ctrl(e,
		...
		num = ENGINE_ctrl(e, ENGINE_CTRL_GET_NEXT_CMD_TYPE, num, NULL,
				  NULL);
	} while (num > 0);
	if (xpos > 0)
		BIO_printf(out, "\n");
...
static void util_do_cmds(ENGINE *e, STACK_OF(OPENSSL_STRING) *cmds, BIO *out,
			 const char *indent)
{
	...
	for (i = 0; i < sk_OPENSSL_CSTRING_num(engines); i++) {
		...
		if ((e = ENGINE_by_id(id)) != NULL) {
			...
			if (list_cap) {
				...
				for (k = 0; k < n; ++k)
					if (!append_buf(&cap_buf, &cap_size,
							OBJ_nid2sn(nids[k])))
						goto end;
...
skip_pmeths: {
	...
	if (!store_ctx.ok)
		goto end;
			...
			if ((verbose > 0) &&
			    !util_verbose(e, verbose, out, indent))
				goto end;
			ENGINE_free(e);
		} else {
