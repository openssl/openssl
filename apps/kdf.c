...
opthelp:
		...
		case OPT_KDFOPT:
			...
			if (opts == NULL ||
			    !sk_OPENSSL_STRING_push(opts, opt_arg()))
				goto opthelp;
			break;
		case OPT_CIPHER:
			OPENSSL_free(cipher);
			cipher = alloc_kdf_algorithm_name(&opts, "cipher",
							  opt_arg());
			if (cipher == NULL)
				goto opthelp;
			break;
		case OPT_DIGEST:
			OPENSSL_free(digest);
			digest = alloc_kdf_algorithm_name(&opts, "digest",
							  opt_arg());
			if (digest == NULL)
				goto opthelp;
			break;
	...
	if (opts != NULL) {
		int ok = 1;
		OSSL_PARAM *params = app_params_new_from_opts(
			opts, EVP_KDF_settable_ctx_params(kdf));
