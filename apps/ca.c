    ...
    int default_op, int ext_copy, int selfsign, unsigned long dateopt)
{
    ...
    for (i = 0; i < X509_NAME_entry_count(name); i++) {
        ...
        if (msie_hack) {
            /*
             * Can this "hack" just die, we are well past accommodating msie.
             */
            /* assume all type should be strings */
            const unsigned char *bytes = ASN1_STRING_get0_data(str);
            int type = ASN1_STRING_type(str);
            int length = ASN1_STRING_length(str);

            if (type == V_ASN1_UNIVERSALSTRING) {
                if (!ASN1_UNIVERSALSTRING_to_string(str)) {
                    BIO_printf(bio_err,
                        "\nmsie hack failed\n");
                    goto end;
                }
                bytes = ASN1_STRING_get0_data(str);
                type = ASN1_STRING_type(str);
                length = ASN1_STRING_length(str);
            }

            if (ASN1_STRING_type(str) == V_ASN1_IA5STRING && nid != NID_pkcs9_emailAddress)
            ...
            /* XXX Casts away const, because it is mutating the object! */
            if (!X509_NAME_ENTRY_set_data((X509_NAME_ENTRY *)name, type, bytes, length)) {
                BIO_printf(bio_err,
                    "\nmsie hack failed\n");
                goto end;
            }
