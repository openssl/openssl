...
static int
certify(X509 **xret, const char *infile, int informat, EVP_PKEY *pkey,
	X509 *x509, const char *dgst, STACK_OF(OPENSSL_STRING) *sigopts,
	STACK_OF(OPENSSL_STRING) *vfyopts, STACK_OF(CONF_VALUE) *policy,
	CA_DB *db, BIGNUM *serial, const char *subj, unsigned long chtype,
	int multirdn, int email_dn, const char *startdate, const char *enddate,
	long days, int batch, const char *ext_sect, CONF *conf, int verbose,
	unsigned long certopt, unsigned long nameopt, int default_op,
	int ext_copy, int selfsign, unsigned long dateopt);
static int certify_cert(X509 **xret, const char *infile, int certformat,
			...
			int email_dn, const char *startdate,
			...
			int default_op, int ext_copy, unsigned long dateopt);
...
static int do_body(X509 **xret, EVP_PKEY *pkey, X509 *x509, const char *dgst,
		   STACK_OF(OPENSSL_STRING) *sigopts,
		   STACK_OF(CONF_VALUE) *policy, CA_DB *db, BIGNUM *serial,
		   const char *subj, unsigned long chtype, int multirdn,
		   ...
		   unsigned long nameopt, int default_op, int ext_copy,
		   int selfsign, unsigned long dateopt);
...
const OPTIONS ca_options[] = {
	...
	{ "days", OPT_DAYS, 'p',
	  "Number of days from today to certify the cert for" },
...
int ca_main(int argc, char **argv)
{
	...
	char *passin = NULL;
	char *outdir = NULL, *outfile = NULL, *rev_arg = NULL,
	     *ser_status = NULL;
	...
	int ret = 1, email_dn = 1, req = 0, verbose = 0, gencrl = 0,
	    dorevoke = 0;
...
end_of_options:
	...
	if (!selfsign || spkac_file || ss_cert_file || gencrl) {
		...
		x509 = load_cert_pass(certfile, certformat, 1, passin,
				      "CA certificate");
		if (x509 == NULL)
			goto end;
	...
	if (extfile) {
		...
		if (extensions == NULL) {
			extensions = app_conf_try_string(
				extfile_conf, "default", "extensions");
			if (extensions == NULL)
				extensions = "default";
		}
	}
	...
	def_ret = EVP_PKEY_get_default_digest_name(pkey, def_dgst,
						   sizeof(def_dgst));
	/*
	...
	if (def_ret == 2 && strcmp(def_dgst, "UNDEF") == 0) {
		dgst = NULL;
		   ...
		   (dgst = lookup_conf(conf, section, ENV_DEFAULT_MD)) ==
			   NULL &&
	...
	if (req) {
		if (email_dn == 1) {
			...
			tmp_email_dn = app_conf_try_string(
				conf, section, ENV_DEFAULT_EMAIL_DN);
			if (tmp_email_dn != NULL &&
			    strcmp(tmp_email_dn, "no") == 0)
				email_dn = 0;
		}
		if (verbose)
		...
		if (app_conf_try_string(conf, section, ENV_RAND_SERIAL) !=
		    NULL) {
			rand_ser = 1;
		} else {
		...
		if (extfile_conf != NULL) {
			...
			if (!X509V3_EXT_add_nconf(extfile_conf, &ctx,
						  extensions, NULL)) {
				BIO_printf(
					bio_err,
		...
		if (startdate == NULL)
			startdate = app_conf_try_string(conf, section,
							ENV_DEFAULT_STARTDATE);
		if (enddate == NULL)
			enddate = app_conf_try_string(conf, section,
						      ENV_DEFAULT_ENDDATE);
		if (days == 0) {
		...
		if (rand_ser) {
			...
			serial =
				load_serial(serialfile, NULL, create_ser, NULL);
			if (serial == NULL) {
				BIO_printf(
					bio_err,
		...
		if (spkac_file != NULL) {
			...
			j = certify_spkac(&x, spkac_file, pkey, x509, dgst,
					  sigopts, attribs, db, serial, subj,
					  chtype, multirdn, email_dn, startdate,
					  enddate, days, extensions, conf,
					  verbose, certopt, get_nameopt(),
					  default_op, ext_copy, dateopt);
			if (j < 0)
				goto end;
			if (j > 0) {
				total_done++;
		...
		if (ss_cert_file != NULL) {
			...
			j = certify_cert(&x, ss_cert_file, certformat, passin,
					 pkey, x509, dgst, sigopts, vfyopts,
					 attribs, db, serial, subj, chtype,
					 multirdn, email_dn, startdate, enddate,
					 days, batch, extensions, conf, verbose,
					 certopt, get_nameopt(), default_op,
					 ext_copy, dateopt);
			if (j < 0)
				goto end;
			if (j > 0) {
		...
		if (infile != NULL) {
			...
			j = certify(&x, infile, informat, pkey, x509p, dgst,
				    sigopts, vfyopts, attribs, db, serial, subj,
				    chtype, multirdn, email_dn, startdate,
				    enddate, days, batch, extensions, conf,
	...
	if (gencrl) {
		...
		if (crl_ext == NULL)
			crl_ext =
				app_conf_try_string(conf, section, ENV_CRLEXT);
		if (crl_ext != NULL) {
		...
		if (crlnumberfile != NULL) {
			if ((crlnumber = load_serial(crlnumberfile, NULL, 0,
						     NULL)) == NULL) {
				BIO_printf(bio_err,
		...
		if ((crl_nextupdate == NULL) && (crldays == 0) &&
		    (crlhours == 0) && (crlsec == 0)) {
			BIO_printf(
				bio_err,
		...
		if ((crl = X509_CRL_new_ex(app_get0_libctx(),
					   app_get0_propq())) == NULL)
			goto end;
		...
		if (!set_crl_nextupdate(crl, crl_nextupdate, crldays, crlhours,
					crlsec)) {
...
certify(X509 **xret, const char *infile, int informat, EVP_PKEY *pkey,
	...
	CA_DB *db, BIGNUM *serial, const char *subj, unsigned long chtype,
	int multirdn, int email_dn, const char *startdate, const char *enddate,
	long days, int batch, const char *ext_sect, CONF *lconf, int verbose,
	unsigned long certopt, unsigned long nameopt, int default_op,
	int ext_copy, int selfsign, unsigned long dateopt)
{
	X509_REQ *req = NULL;
	...
	ok = do_body(xret, pkey, x509, dgst, sigopts, policy, db, serial, subj,
		     ...
		     batch, verbose, req, ext_sect, lconf, certopt, nameopt,
		     default_op, ext_copy, selfsign, dateopt);
...
again2:
			...
			if (push != NULL) {
				if (!X509_NAME_add_entry(subject, push, -1,
							 0)) {
					BIO_printf(
						bio_err,
						"Memory allocation failure\n");
	...
	X509V3_set_ctx(&ext_ctx, selfsign ? ret : x509, ret,
	...
	if (ext_sect) {
		if (extfile_conf != NULL) {
			...
			if (!X509V3_EXT_add_nconf(extfile_conf, &ext_ctx,
						  ext_sect, ret)) {
				BIO_printf(
					bio_err,
			...
			if (!X509V3_EXT_add_nconf(lconf, &ext_ctx, ext_sect,
						  ret)) {
				BIO_printf(
					bio_err,
	...
	if (!email_dn) {
		...
		i = -1;
		while ((i = X509_NAME_get_index_by_NID(
				dn_subject, NID_pkcs9_emailAddress, i)) >= 0) {
			 ...
			 unsigned long dateopt)
{
	...
	for (i = 0;; i++) {
		...
		if (!X509_NAME_add_entry_by_NID(
			    n, nid, chtype, (unsigned char *)buf, -1, -1, 0))
			goto end;
	}
	if (spki == NULL) {
		BIO_printf(bio_err,
	...
	ok = do_body(xret, pkey, x509, dgst, sigopts, policy, db, serial, subj,
		     ...
		     verbose, req, ext_sect, lconf, certopt, nameopt,
		     default_op, ext_copy, 0, dateopt);
end:
	X509_REQ_free(req);
	CONF_free(parms);
...
int do_updatedb(CA_DB *db, time_t *now)
{
	...
	for (i = 0; i < sk_OPENSSL_PSTRING_num(db->db->data); i++) {
		...
		if (rrow[DB_type][0] == DB_TYPE_VAL) {
			...
			if (!ASN1_TIME_set_string(exp_date,
						  rrow[DB_exp_date])) {
				ASN1_TIME_free(a_tm);
...
static int make_revoked(X509_REVOKED *rev, const char *str)
{
	...
	if (rev && (reason_code != OCSP_REVOKED_STATUS_NOSTATUS)) {
		...
		if (rtmp == NULL || !ASN1_ENUMERATED_set(rtmp, reason_code))
			goto end;
		if (X509_REVOKED_add1_ext_i2d(rev, NID_crl_reason, rtmp, 0,
					      0) <= 0)
			goto end;
	}
		   ...
		   ASN1_GENERALIZEDTIME **pinvtm, const char *str)
{
	...
	if (reason_str) {
		for (i = 0; i < NUM_REASONS; i++) {
			if (OPENSSL_strcasecmp(reason_str, crl_reasons[i]) ==
			    0) {
				reason_code = i;
				break;
			}
