...
opthelp:
		...
		case OPT_CRITERION_SUBJECT:
			...
			subject = parse_name(opt_arg(), MBSTRING_UTF8, 1,
					     "subject");
			if (subject == NULL)
				goto end;
			break;
		case OPT_CRITERION_ISSUER:
			...
			issuer = parse_name(opt_arg(), MBSTRING_UTF8, 1,
					    "issuer");
			if (issuer == NULL)
				goto end;
			break;
		case OPT_CRITERION_SERIAL:
			...
			if ((serial = s2i_ASN1_INTEGER(NULL, opt_arg())) ==
			    NULL) {
				BIO_printf(
					bio_err,
		...
		case OPT_CRITERION_FINGERPRINT:
			...
			}
			{
				...
				if ((fingerprint = OPENSSL_hexstr2buf(
					     opt_arg(), &tmplen)) == NULL) {
					BIO_printf(
						bio_err,
	...
	if (criterion != 0) {
		...
		case OSSL_STORE_SEARCH_BY_NAME:
			if ((search = OSSL_STORE_SEARCH_by_name(subject)) ==
			    NULL) {
		...
		case OSSL_STORE_SEARCH_BY_ALIAS:
			if ((search = OSSL_STORE_SEARCH_by_alias(alias)) ==
			    NULL) {
				ERR_print_errors(bio_err);
				goto end;
			}
	...
	ret = process(argv[0], get_ui_method(), &pw_cb_data, expected,
		      criterion, search, text, noout, recursive, 0, outfile,
		      prog, libctx);
		   ...
		   const char *outfile, const char *prog, OSSL_LIB_CTX *libctx)
{
	...
	if ((store_ctx = OSSL_STORE_open_ex(uri, libctx, app_get0_propq(),
	...
	for (;;) {
		...
		if (out == NULL) {
			if ((out = bio_open_default(outfile, 'w',
						    FORMAT_TEXT)) == NULL) {
				ret++;
				goto end2;
			}
		...
		case OSSL_STORE_INFO_NAME:
			if (recursive) {
				...
				ret += process(suburi, uimeth, uidata, expected,
					       criterion, search, text, noout,
					       recursive, indent + 2, outfile,
					       prog, libctx);
			}
			break;
		case OSSL_STORE_INFO_PARAMS:
			if (text)
				EVP_PKEY_print_params(
					out, OSSL_STORE_INFO_get0_PARAMS(info),
					0, NULL);
			if (!noout)
		...
		case OSSL_STORE_INFO_PUBKEY:
			if (text)
				EVP_PKEY_print_public(
					out, OSSL_STORE_INFO_get0_PUBKEY(info),
					0, NULL);
			if (!noout)
		...
		case OSSL_STORE_INFO_PKEY:
			if (text)
				EVP_PKEY_print_private(
					out, OSSL_STORE_INFO_get0_PKEY(info), 0,
					NULL);
			if (!noout)
				PEM_write_bio_PrivateKey(
					out, OSSL_STORE_INFO_get0_PKEY(info),
		...
		case OSSL_STORE_INFO_CERT:
			...
			if (!noout)
				PEM_write_bio_X509(
					out, OSSL_STORE_INFO_get0_CERT(info));
			break;
		case OSSL_STORE_INFO_CRL:
			...
			if (!noout)
				PEM_write_bio_X509_CRL(
					out, OSSL_STORE_INFO_get0_CRL(info));
			break;
		default:
