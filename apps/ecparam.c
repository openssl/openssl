...
opthelp:
	...
	if (curve_name != NULL) {
		...
		*p = OSSL_PARAM_construct_end();
		...
		if (OPENSSL_strcasecmp(curve_name, "SM2") == 0)
			gctx_params = EVP_PKEY_CTX_new_from_name(
				app_get0_libctx(), "sm2", app_get0_propq());
		else
			gctx_params = EVP_PKEY_CTX_new_from_name(
				app_get0_libctx(), "ec", app_get0_propq());
		if (gctx_params == NULL ||
		...
		params_key = load_keyparams_suppress(infile, informat, 1, "EC",
						     "EC parameters", 1);
		if (params_key == NULL)
			params_key = load_keyparams_suppress(infile, informat,
							     1, "SM2",
							     "SM2 parameters",
							     1);
		...
		if (params_key == NULL) {
			BIO_printf(bio_err,
				   "Unable to load parameters from %s\n",
				   infile);
			goto end;
		}
	...
	if (genkey) {
         ...
         */
		gctx_key = EVP_PKEY_CTX_new_from_pkey(
			app_get0_libctx(), params_key, app_get0_propq());
