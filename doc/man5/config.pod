...
containing ASN.1 string table entries. This allows customization of the
minimum and maximum sizes, as well as the permitted character types, for
various ASN.1 string types used in X.509 certificates.

Each entry in the section has the form:

 oid_name = min:N, max:N, mask:XXX, flags:YYY

where I<oid_name> is the short or long name of an ASN.1 object identifier.
The parameters are:
...
The minimum length of the string (default: -1, meaning no minimum).

=item B<max>

The maximum length of the string (default: -1, meaning no maximum).
...
Optional flags. The value B<nomask> means no mask is enforced, and B<none>
clears all flags.

=back

For example, to allow longer common names:
