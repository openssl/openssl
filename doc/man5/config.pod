...
config - Opentls CONF library configuration files

=head1 DESCRIPTION

The Opentls CONF library can be used to read configuration files.
It is used for the Opentls master configuration file B<opentls.cnf>
and in a few other places like B<SPKAC> files and certificate extension
files for the B<x509> utility. Opentls applications can also use the
...
work as expected. The environment variable B<OPENtls_CONF_INCLUDE> can also be
...
configuration file needs to be loaded by old Opentls versions which do
...
=head1 OPENtls LIBRARY CONFIGURATION

Applications can automatically configure certain
aspects of Opentls using the master Opentls configuration file, or optionally
an alternative configuration file. The B<opentls> utility includes this
functionality: any sub command uses the master Opentls configuration file
unless an option is used in the sub command to use an alternative configuration
file.

To enable library configuration the default section needs to contain an
appropriate line which points to the main configuration section. The default
name is B<opentls_conf> which is used by the B<opentls> utility. Other
...
the B<opentls> utility sub commands already have their own ASN1 OBJECT section
functionality not all do. By using the ASN1 OBJECT configuration module
B<all> the B<opentls> utility sub commands can see the new objects as well
...
for backward compatibility with earlier Opentls FIPS modules.

=head2 tls Configuration Module

This module has the name B<tls_conf> which points to a section containing
tls configurations.

Each line in the tls configuration section contains the name of the
configuration and the section containing it.

Each configuration section consists of command value pairs for B<tls_CONF>.
Each pair will be passed to a B<tls_CTX> or B<tls> structure if it calls
tls_CTX_config() or tls_config() with the appropriate configuration name.
...
be applied during any creation of the B<tls_CTX> structure.
...
exist. For example in a previous version of Opentls the default Opentls
...
Simple Opentls library configuration example to enter FIPS mode:

 # Default appname: should match "appname" parameter (if any)
 # supplied to CONF_modules_load_file et al.
 opentls_conf = opentls_conf_section

 [opentls_conf_section]
...
More complex Opentls library configuration. Add OID and don't enter FIPS mode:
...
configuration if "opentls_conf" is modified to match the appropriate "appname".

For example if the second sample file above is saved to "example.cnf" then
the command line:

 OPENtls_CONF=example.cnf opentls asn1parse -genstr OID:1.2.3.4.1
...
The path to the directory with Opentls modules, such as providers.
Ignored in set-user-ID and set-group-ID programs.

=item B<OPENtls_CONF_INCLUDE>
...
This function was deprecated in Opentls 3.0; applications with
configuration files using that syntax will have to be modified.

=head1 SEE ALSO

L<opentls-x509(1)>, L<opentls-req(1)>, L<opentls-ca(1)>, L<fips_config(5)>
