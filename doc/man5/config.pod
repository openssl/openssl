...
is he first non-space character in a line, the entire line is ignored.
...
Doing this is dubious, and is unlikely to result in compatible behavior.
...
available on systems with POSIX IO support.)  Multiple levels of directory
inclusion are not supported.
...
looked up it is first looked up in the current or named named section,
and then the default section if necesssary.
...
DNs, the same field may occur multiple times.
...
substitutes the value of the named variable from the current section.
To substitute a value from another section use C<$section::name>
or C<${section::name}>.
...
=head2 ASN1 Object Identifier Configuration

The name B<oid_section> in the initialization section names the section
containing name/value pairs of OID's.
The name is the short name, and the value is the optional long name,
and numeric value.
...
containing cryptographic provider configuration. That section has
a list of provider names the section for each provider. The
provider-specific section is used to specify how to load the module,
...
If present, the module is ativated. The value assigned to this name is not
significant.
...
Each configuration section consists of name/value pairs as parsed
...
active it, and set other parameters.
...
This example shows how to using quoting and escaping.
...
to a temporary file, and the environment variable B<TMP> or
B<TEMP>, if present, specify the directory where the file
should be put.
Since the default section is checked if a variable does not
exist, it is possible to set B<TMP> default to F</tmp>, and
B<TEMP> defaulting to B<TMP>.
