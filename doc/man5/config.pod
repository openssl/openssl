...
dynamic, perform initialisation and send ctrls. The actual operation performed
...
The command B<init> determines whether to initialise the ENGINE. If the value
is B<0> the ENGINE will not be initialised, if B<1> and attempt it made to
initialised the ENGINE immediately. If the B<init> command is not present
then an attempt will be made to initialise the ENGINE after all commands in
