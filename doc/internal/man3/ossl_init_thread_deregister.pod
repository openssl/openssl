...
Otls_thread_stop_handler_fn,
otls_init_thread_start,
otls_init_thread_deregister
...
Thread stop events may be detected by Opentls either automatically (using the
capabilities of the underlying threading library) where possible or explicitly
by the application calling OPENtls_thread_stop() or OPENtls_thread_stop_ex().
...
A new stop handler is registered using the function otls_init_thread_start().
...
otls_init_thread_deregister. If no later call to otls_init_thread_deregister is
...
then this can be done using the function otls_init_thread_deregister().
This will deregister all stop handlers (no matter which thread they were
registered for) which the same I<index> value.

=head1 RETURN VALUES

otls_init_thread_start() and otls_init_thread_deregister() return 1 for success
or 0 on error.
