...
B<OPENSSL_DEFAULT_SEED_SRC>.  If a seed source was explicitly configured
by one of these means, it is instantiated on first use and an error is
returned when it cannot be used; the operating system entropy sources
are only used as a fallback when no seed source was configured.
...
way as for ossl_rand_get_user_entropy() and an explicitly configured
seed source is likewise never silently substituted by the operating
system sources.
