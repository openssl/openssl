...
openssl_ctx_init_index, openssl_ctx_get_data, openssl_ctx_run_once,
openssl_ctx_onfree - internal OPENSSL_CTX routines
...
OPENSSL_CTX_METHOD must be defined and a reference to it included in the methods
table in context.c along with a index for it in cryptlib.h. The functions
through the method are used to create or free items that are stored at that
index whenever a library context is created or freed, meaning that the code that
use a data item of that index doesn't have to worry about that, just use the
data available.

openssl_ctx_init_index() initialises a given library context index, and
associates it with the functions given through C<meth>. This is not normally
needed except in test code where there is no pre-registered method available in
the context.c methods table.
...
openssl_ctx_init_index() returns 0 on error or 1 on success.
...
To get and use the data stored in the library context, simply do this:
  ...
  */
 FOO *data = openssl_ctx_get_data(ctx, OPENSSL_CTX_FOO_INDEX);
