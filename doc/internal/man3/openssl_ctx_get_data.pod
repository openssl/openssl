...
ossl_ctx_get_data, ossl_ctx_run_once, ossl_ctx_onfree
- internal OSSL_CTX routines
...
=head1 SYNOPSIS
 ...
 typedef struct ossl_ctx_method {
     void *(*new_func)(OSSL_CTX *ctx);
     void (*free_func)(void *);
 } OSSL_CTX_METHOD;

 void *ossl_ctx_get_data(OSSL_CTX *ctx, int index,
                            const OSSL_CTX_METHOD *meth);

 int ossl_ctx_run_once(OSSL_CTX *ctx, unsigned int idx,
                          ossl_ctx_run_once_fn run_once_fn);
 int ossl_ctx_onfree(OSSL_CTX *ctx, ossl_ctx_onfree_fn onfreefn);

=head1 DESCRIPTION

Internally, the OpenSSL library context B<OSSL_CTX> is implemented
...
ossl_ctx_get_data() is used to retrieve a pointer to the data in
the library context I<ctx> associated with the given I<index>. An
OSSL_CTX_METHOD must be defined and given in the I<meth> parameter. The index
...
ossl_ctx_run_once is used to run some initialisation routine I<run_once_fn>
...
using ossl_ctx_onfree. This associates an "on free" routine I<onfreefn> with
the library context I<ctx>. When I<ctx> is freed all associated "on free"
routines are called.

=head1 RETURN VALUES

ossl_ctx_get_data() returns a pointer on success, or NULL on
failure.
...
and a destructor to an index.
 ...
 static void *foo_new(OSSL_CTX *ctx)
 {
  ...
  * OSSL_CTX_FOO_INDEX should be added to internal/cryptlib.h
  */
 const OSSL_CTX_METHOD foo_method = {
     foo_new,
     foo_free
 };
...
To get and use the data stored in the library context, simply do this:
  ...
  */
 FOO *data = ossl_ctx_get_data(ctx, OSSL_CTX_FOO_INDEX, &foo_method);

=head2 Run Once

 void foo_cleanup(OSSL_CTX *ctx)
 {
     /* Free foo resources associated with ctx */
 }

 static ossl_ctx_run_once_fn do_foo_init;
 static int do_foo_init(OSSL_CTX *ctx)
 {
     /* Allocate and initialise some foo resources and associated with ctx */
     return ossl_ctx_onfree(ctx, &foo_cleanup)
 }

 int foo_some_function(OSSL_CTX *ctx)
 {
    if (!ossl_ctx_run_once(ctx,
                              OSSL_CTX_FOO_RUN_ONCE_INDEX,
                              do_foo_init))
        return 0;
