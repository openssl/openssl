...
openssl_ctx_run_once is used to run some initialization routine I<run_once_fn>
exactly once per library context I<ctx> object. Each initialization routine
should be allocate a unique run once index in cryptlib.h.

Any resources allocated via a run once initialization routine can be cleaned up
...
=head2 Run Once
 ...
 {
     /* Allocate and initialize some foo resources and associated with ctx */
