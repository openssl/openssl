...
and other utilities functions are also provided.
...
The B<OPTIONS> C<typedef> specifies an options, what type of argument
...
The B<retval> is a the value to return if the option is found. It should be
...
be chosen from the following set:
    ...
    'u' Aunsigned number that fits in the C<unsigned long> type
...
Each subsequent line will be indent the correct amount.
