...
It computes the SHA256 digest of the certificate using the default library context
and property query string and stores the result in x->sha256_hash,
or on failure sets B<EXFLAG_NO_FINGERPRINT> and B<EXFLAG_INVALID> in x->flags.
