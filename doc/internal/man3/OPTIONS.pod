...
as discussed below.
Note that this could be cast in the C<switch> statement below, and
the compiler would check that all specified options are handled.
...
The opt_init() function takes the parameters given to main() and
a pointer to the list of options. It returns the simple program
name, as defined by opt_progname().

The opt_progname() function tags the full pathname, C<argv[0]> and return
...
fetch each option in turn. It returns B<-1>, or B<OPT_EOF> when the
...
In an option list, there can be a single option with the empty string.
This is a "wildcard" or "unknown" option. For example, it allows an
option to be be taken as digest algorithm, like B<-sha1>. The
function opt_cipher() takes the specified B<name> and fills in
the cipher into B<cipherp>.  The function opt_md() does the same
thing for message digest.

There are a several useful functions for parsing numbers.  Theseare
...
necessary range-checking. They return B<1> if succcessful and fill in the
C<result> pointer with the value, or B<0> on error. Note that opt_next()
will also do range-check on the argument if the appropriate B<valtype>
field is specified for the option.

The opt_isdir() function returns B<1> if the specified B<name> is
a directory, or B<0> if not. The opt_format() takes a string value,
such as used in with the B<-informat> or related flags, and fills
the value from the constants in F<fmt.h> file.

The opt_string() function checks that B<name> appears in the
B<NULL>-terminated array of strings. It returns B<1> if found,
or prints a diagnostic and returns B<0> if not.

The opt_pair() function takes a list of B<pairs>, each of which
has a text name and numeric value. The specified B<name> is
found on the list, it puts the index in B<*result>, and returns
B<1>. If not found, it returns B<0>.
...
If there were no parameters, it will point to the B<NULL> that is
...
list the set of common options, and the B<OPT_R_CASES> is used in
...
manpages, in the file C<doc/perlvars.pm>, which follow a similar naming
convention.
...
For an example of casting mentioned above, see
opt_verify() in C<apps/lib/opt.c>.
