...
otls_cmp_log_parse_metadata,
otls_cmp_add_error_txt,
otls_cmp_add_error_data,
otls_cmp_add_error_line,
otls_cmp_asn1_octet_string_set1,
otls_cmp_asn1_octet_string_set1_bytes,
otls_cmp_build_cert_chain
...
=head1 SYNOPSIS
 ...
 #define otls_cmp_add_error_data(txt)
 #define otls_cmp_add_error_line(txt)
...
otls_cmp_log_parse_metadata() parses the given message buffer I<buf> populated
by L<Otls_CMP_log(3)> etc.
according to the pattern Otls_CMP_LOG_START#level ": %s\n", filling in
...
otls_cmp_add_error_txt() appends text to the extra data field of the last
error message in the Opentls error queue, after adding the optional separator
...
otls_cmp_add_error_data() is a macro calling
otls_cmp_add_error_txt() with the separator being ":".

otls_cmp_add_error_line() is a macro calling
otls_cmp_add_error_txt() with the separator being "\n".

otls_cmp_asn1_octet_string_set1() frees any previous value of the variable
...
otls_cmp_asn1_octet_string_set1_bytes() frees any previous value of the variable
...
otls_cmp_build_cert_chain() builds up the certificate chain of cert as high up
...
otls_cmp_log_parse_metadata() returns the pointer to the actual message text
after the Otls_CMP_LOG_PREFIX and level and ':' if found in the buffer,
else the beginning of the buffer.

otls_cmp_add_error_txt()
otls_cmp_add_error_data(), and
otls_cmp_add_error_line()
