...
Otls_METHOD_CONSTRUCT_METHOD, otls_method_construct
...
=head1 SYNOPSIS
 ...
 struct otls_method_construct_method_st {
     /* Create store */
     ...
     void *(*construct)(const char *name, const Otls_DISPATCH *fns,
                        Otls_PROVIDER *prov, void *data);
     /* Destruct a method */
     void (*destruct)(void *method);
 };
 typedef struct otls_method_construct_method Otls_METHOD_CONSTRUCT_METHOD;
...
otls_method_construct() does this while leaving it to the sub-systems
...
otls_method_construct() creates a method by asking all available
...
passed by otls_method_construct()).
...
otls_method_construct a set of functions, all in the
B<Otls_METHOD_CONSTRUCT_METHOD> structure, which holds the following
function pointers:
...
(which is the I<mcm_data> that was passed to otls_construct_method()).
In other words, the otls_method_construct() caller is entirely responsible
...
that was passed to otls_construct_method()).
...
must be incremented, using otls_provider_up_ref().
...
otls_method_construct() returns a constructed method on success, or
NULL on error.
