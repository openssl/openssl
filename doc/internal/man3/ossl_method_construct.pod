...
All libcrypto sub-systems that wants to create their own methods based
...
C<mcm> and the data in C<mcm_data> (which is just passed by
...
This store is used to temporarly store methods for easier lookup, for
