...
B<ossl_list_I<TYPE>_join<() joins list B<lt> with list <lh>. List B<lt>
is appended to list B<lh>. The list B<lt> becomes empty becomes empty
as all its members are part of B<lh> after function returns.
