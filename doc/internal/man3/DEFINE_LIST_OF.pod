...
ossl_list_TYPE_insert_before, ossl_list_TYPE_after, ossl_list_TYPE_append
- doubly linked list
...
=for openssl generic
 ...
 void ossl_list_TYPE_append(OSSL_LIST(name) *lh, OSSL_LIST(name) *lt);
...
B<ossl_list_I<TYPE>_append<() appends list B<lt> to list <lh>. List B<lt>
becomes empty as all B<lt> members are part of B<lh> after function returns.
