...
C<OSSL_print_attribute_value> prints an X.500 directory value, which is an
...
This function takes a C<BIO> pointer for printing the output, the NID of the
attribute type object identifier, the ASN.1 value itself, and the level of
desired indentation in terms of the number of spaces.
...
print_attribute_value() was added in OpenSSL 3.4.
