...
=head1 SYNOPSIS
 ...
 int ossl_provider_get_capabilties(const OSSL_PROVIDER *prov,
...
ossl_provider_get_capabilties() calls the provider's I<get_capabilties> function,
if the provider has one. It provides the name of the I<capability> and a callback
in th I<cb> parameter to call for each capability that has that name in the
provider. The callback gets passed OSSL_PARAM details about the capability as
...
ossl_provider_get_capabilties() returns 1 on success, or 0 on error.
