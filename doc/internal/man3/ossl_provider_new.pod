...
module.
Instances of this type are commonly refered to as "provider objects".
...
The provider object it finds gets its reference count incremented.
...
The reference counter of a newly created provider object will always
be 2; one for being added to the store, and one for the returned
reference.
To indicate a built-in provider, the I<init_function> argument must
...
reference counter; if it drops below 2, the provider object is assumed
to have fallen out of use and will be inactivated (its I<teardown>
...
This will be used in preference of automatically trying to figure out
...
provider will use the I<name> to find the value it wants.
...
Use the provider object's name as module path, with platform specific
standard extentions added.
...
If the environment variable B<OPENSSL_MODULES> isn't defined, merge
the value of the OpenSSL built in macro B<MODULESDIR> with the module
path.
...
provider object (I<OSSL_PROVIDER>) on success, or B<NULL> on error.
