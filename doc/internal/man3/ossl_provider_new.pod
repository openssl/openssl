...
They are activated with a the first call to ossl_provider_load(), and
are inactivated when ossl_provider_free() has been called enough
times.
...
Look in each directory given by ossl_provider_add_load_location().
