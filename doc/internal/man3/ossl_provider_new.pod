...
ossl_provider_random() queries the provider, I<prov>, randomness source for I<n>
bytes of entropy which are returned in the buffer I<buf>, the returned entropy will
have a number of bits of I<strength>.
...
ossl_provider_random() returns 1 on success or 0 or -1 on error as per
L<RAND_bytes(3)>.
