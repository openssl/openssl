...
ossl_provider_free, ossl_provider_add_load_location,
ossl_provider_load, ossl_provider_name, ossl_provider_dso,
...
C<OSSL_PROVIDER> is an type that holds all the necessary information
to handle a provider, regardless of if it's a built in to the
...
They get activated with a the first call to ossl_provider_load(), and
get inactivated when ossl_provider_free() has been called enough
times.
...
ossl_provider_add_load_location() adds a location to look for a
provider module.

ossl_provider_load() "activates" the provider for the given
I<provider object>.
What "activates" actually means depends on what type of
I<provider object> it is:

=over 4

=item *

If a initialization function was given with ossl_provider_new(), that
...
that come in form of loadable modules.

ossl_provider_module_name() returns the file name of the module, for
providers that come in form of loadable modules.

ossl_provider_module_path() returns the full path of module file, for
providers that come in form of loadable modules.
...
It should returns an array of C<OSSL_ITEM> to describe all the
...
Look in each directory given with ossl_provider_add_load_location().

=item 2.

Look in the directory given with the environment variable
B<OPENSSL_MODULES>.
...
I<provider object> (C<OSSL_PROVIDER>) on succes, or B<NULL> on error.
...
ossl_provider_add_load_location() and ossl_provider_load() return 1 on
