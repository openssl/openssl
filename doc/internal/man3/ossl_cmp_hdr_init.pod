...
ossl_cmp_hdr_set1_senderKID() sets the B<senderKID> field in the given
PKIHeader I<hdr> to I<senderKID>. If I<senderKID> is NULL the field will be omitted.
...
With MAC-based protection, as required by the CMP profile RFC 9483 section 3.1,
the sender field is set to the reference value string provided via
OSSL_CMP_CTX_set1_referenceValue() placed as the commonName in the directoryName
choice of the GeneralName as far as possible, otherwise we use as a fallback
the strategy desribed next for signature-based protection.

With signature-based protection or when no message protection is used,
the sender name is copied from the subject of the protection cert, if any,
otherwise from the subject of the cert provided via OSSL_CMP_CTX_set1_oldCert(),
from the subject of any CSR provided via OSSL_CMP_CTX_set1_p10CSR(),
from the subject name provided for CRMF-based certification requests,
or else from the string provided via OSSL_CMP_CTX_set1_referenceValue().

As required by RFC 9810 section 5.1.1., if the sender name is not known
to the sending entity, the NULL-DN is used as the last resort.
In this case, for identification at least the senderKID must be given,
which we take from any referenceValue provided.
