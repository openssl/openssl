...
OSSL_DEPRECATED, OSSL_DEPRECATED_FOR, OSSL_DEPRECATED_PRAGMA - General deprecation macros
...
=head1 SYNOPSIS
 ...
 #define OSSL_DEPRECATED_PRAGMA(msg)
...
OSSL_DEPRECATED_PRAGMA() implements a warning via a compiler specific
pragma, to be used when e.g. deprecating a macro.
It takes a single argument I<msg>, which must be a "single string literal".
