...
ossl_evp_keymgmt_util_export,
ossl_evp_keymgmt_util_export_to_provider,
ossl_evp_keymgmt_util_find_operation_cache_index,
ossl_evp_keymgmt_util_clear_operation_cache,
ossl_evp_keymgmt_util_cache_keydata,
ossl_evp_keymgmt_util_cache_keyinfo,
ossl_evp_keymgmt_util_fromdata
...
=head1 SYNOPSIS
 ...
 void ossl_evp_keymgmt_util_cache_keydata(EVP_PKEY *pk, size_t index,
...
ossl_evp_keymgmt_util_export() calls L<ossl_evp_keymgmt_export(3)> with the
I<keymgmt> and I<keydata> from I<pk>.  This is a convenience function.

ossl_evp_keymgmt_util_export_to_provider() exports cached key material
...
To export a legacy key, use L<ossl_evp_pkey_export_to_provider(3)> instead,
as this function ignores any legacy key data.

ossl_evp_keymgmt_util_find_operation_cache_index() finds the location if
...
ossl_evp_keymgmt_util_clear_operation_cache() can be used to explicitly
clear the cache of operation key references.

ossl_evp_keymgmt_util_cache_keydata() can be used to assign a provider key
object to a specific cache slot in the given I<target>.
I<Use extreme care>.

ossl_evp_keymgmt_util_cache_keyinfo() can be used to get all kinds of
information from the provvider "origin" and save it in I<pk>'s
information cache.

ossl_evp_keymgmt_util_fromdata() can be used to add key object data to a
given key I<target> via a B<EVP_KEYMGMT> interface.  This is used as a
helper for L<EVP_PKEY_fromdata(3)>.

=head1 RETURN VALUES

evp_keymgmt_export_to_provider() and ossl_evp_keymgmt_util_fromdata()
return a pointer to the appropriate provider side key (created or
found again), or NULL on error.

ossl_evp_keymgmt_util_find_operation_cache_index() returns the index of the
