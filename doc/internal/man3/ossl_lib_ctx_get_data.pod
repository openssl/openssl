...
I<index> are specified in cryptlib.h. The I<meth> parameter is ignored for most
objects and can be set to NULL; historically it was used to allow objects to be
lazily constructed. The I<meth> argument is still required on first retrieval of
object types which are implemented in separate loadable modules, such as the
FIPS module; currently the only such I<index> value for which this applies is
OSSL_LIB_CTX_FIPS_PROV_INDEX. In this case, the function pointers in the
structure pointed to by I<meth> are used to create and free the object.
...
this:
  ...
  */
 FOO *data = ossl_lib_ctx_get_data(ctx, OSSL_LIB_CTX_FOO_INDEX, NULL);
