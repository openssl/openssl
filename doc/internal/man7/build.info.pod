...
like a tag than as the actual file name that's going to be produced.
...
However, the program file name may end up being just C<foo> (typical
...
condition value in B<IF> and B<ELSIF> can really literal value that
perl can interpret as true or false.
...
Perl nuggest are anything between C<{-> and C<-}>, and whatever the
...
=head2 B<IF[>0|1B<]>, B<ELSIF[>0|1B<]>, B<ELSE>, B<ENDIF>

If the B<IF> or B<ELSIF> condition is true (represented as C<1> here),
everything between this B<IF> / B<ELSIF> and the next corresponding
B<ELSIF>, B<ELSE> or B<ENDIF> apply and the rest is skipped until the
corresponding B<ENDIF>, otherwise the following corresponding B<ELSIF>
or B<ELSE> portion applies.

=head2 B<SUBDIRS=> I<dir> ...
...
=head2 B<DEPEND[>I<item>B<]> B<=> I<file> ...
...
=head2 B<GENERATE[>I<item>B<]> B<=> I<generator> I<generator-arg> ...
...
=head2 B<SOURCE[>I<item>B<]> B<=> I<file> ...
...
=head2 B<SHARED_SOURCE[>I<item>B<]> B<=> I<file> ...
...
=head2 B<DEFINE[>I<item>B<]> B<=> I<name>[B<=>I<value>] ...
...
=head2 B<INCLUDE[>I<item>B<]> B<=> I<dir> ...
