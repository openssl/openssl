...
This is used with B<LIBS>, to give an installed shared library a
shared library version of its own instead of the configured default
(B<SHLIB_VERSION> from F<VERSION.dat>):

    LIBS{shlib_version=1}=libwhatever
