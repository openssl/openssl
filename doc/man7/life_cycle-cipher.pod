...
This state represents an AEAD CIPHER after it has been given additional
authenticated data with EVP_CipherUpdate and a NULL output buffer.  AAD must
be supplied before any plaintext or ciphertext; further AAD updates stay in
this state.
...
=begin man
                                                    ...
                                                    +----------------------------+
                                                      ...
                                                      v
+ - - - - - - - - - - - - - +                       + - - - - - - - - - - - - - - - - - - - - - - - - - - - - -+                     + - - - - - - - - - - - - - -+
'  any of the initialised   '                       '                                                          '                     '      EVP_DecryptInit       ' --------------------+
' updated or finaled states '                       '                   EVP_CIPHER_CTX_reset                   '  EVP_DecryptInit    ' (not required but allowed) '                     |
'                           ' --------------------> '                                                          ' ------------------> '                            ' <-------------------+
+ - - - - - - - - - - - - - +                       + - - - - - - - - - - - - - - - - - - - - - - - - - - - - -+                     + - - - - - - - - - - - - - -+
                                                      |                                                      |                         |
                                                      | EVP_CipherInit                                       |                         | EVP_DecryptUpdate
                                                      v                                                      |                         v
+---------------------------+   EVP_CipherUpdate    + - - - - - - - - - - - - - -+                           |                       +----------------------------+   EVP_DecryptUpdate
|                           | -------------------+  '       EVP_CipherInit       ' ---------------------+    |                       |          updated           | --------------------+
|          updated          |                    |  ' (not required but allowed) '                      |    |                       |        (decryption)        |                     |
|                           | <------------------+  '                            ' <--------------------+    |                       |                            | <-------------------+
+---------------------------+                       + - - - - - - - - - - - - - -+                           |                       +----------------------------+
  |                                                   |                                                      |                         |
  |                                                   | EVP_CipherUpdate                                     |                         |
  ...
  |                            |                    +----------------------------+                                                     |
  |                            |                      |                                                          EVP_DecryptFinal      |
  |                            | EVP_CipherFinal      | EVP_EncryptFinal                                     +-------------------------+
  |                            |                      v                                                      v
  |                            |                    + - - - - - - - - - - - - - - - - - - - - - - - - - - - - -+
  ...
  |                                                 + - - - - - - - - - - - - - - - - - - - - - - - - - - - - -+
  |                                                   |                             ^
  |                                                   | EVP_CIPHER_CTX_free         | EVP_CipherFinal
  |                                                   v                             |
  |                                                 +----------------------------+  |
  |                                                 |           freed            |  |
  |                                                 +----------------------------+  |
...
=begin man
 ...
 Function Call                ---------------------------------------------------- Current State -----------------------------------------------------
                              start   newed    initialised AAD updated   updated     finaled   initialised   updated    initialised   updated    freed
                                                                                                decryption  decryption   encryption  encryption
 EVP_CIPHER_CTX_new           newed
 EVP_CipherInit                    initialised initialised initialised initialised initialised initialised initialised  initialised initialised
 EVP_DecryptInit                   initialised initialised initialised initialised initialised initialised initialised  initialised initialised
                                    decryption  decryption decryption   decryption  decryption  decryption  decryption  decryption  decryption
 EVP_EncryptInit                   initialised initialised initialised initialised initialised initialised initialised  initialised initialised
                                    encryption  encryption encryption   encryption  encryption  encryption  encryption  encryption  encryption
 EVP_CipherUpdate                                updated     updated     updated
 EVP_CipherUpdate (AAD)                                AAD updated AAD updated
 EVP_DecryptUpdate                                                                               updated     updated
                                                                                                decryption  decryption
 EVP_EncryptUpdate                                                                                                        updated     updated
                                                                                                                         encryption  encryption
 EVP_CipherFinal                                             finaled     finaled
 EVP_DecryptFinal                                                                                            finaled
 EVP_EncryptFinal                                                                                                                     finaled
 EVP_CIPHER_CTX_free          freed   freed       freed       freed       freed       freed       freed       freed        freed       freed
 EVP_CIPHER_CTX_reset                 newed       newed       newed       newed       newed       newed       newed        newed       newed
 EVP_CIPHER_CTX_get_params            newed    initialised AAD updated   updated               initialised   updated    initialised   updated
                                                                                                decryption  decryption   encryption  encryption
 EVP_CIPHER_CTX_set_params            newed    initialised AAD updated   updated               initialised   updated    initialised   updated
                                                                                                decryption  decryption   encryption  encryption
 EVP_CIPHER_CTX_gettable_params       newed    initialised AAD updated   updated               initialised   updated    initialised   updated
                                                                                                decryption  decryption   encryption  encryption
 EVP_CIPHER_CTX_settable_params       newed    initialised AAD updated   updated               initialised   updated    initialised   updated
                                                                                                decryption  decryption   encryption  encryption
...
<tr><th style="border:1px solid" align="left">Function Call</th>
    <th style="border:1px solid" colspan="11">Current State</th></tr>
<tr><th style="border:1px solid"></th>
    ...
    <th style="border:1px solid" align="center">AAD<br>updated</th>
...
<tr><th style="border:1px solid" align="left">EVP_CipherUpdate</th>
    ...
    <td style="border:1px solid" align="center"></td>
    <td style="border:1px solid" align="center"></td></tr>
<tr><th style="border:1px solid" align="left">EVP_CipherUpdate (AAD)</th>
    <td style="border:1px solid" align="center"></td>
    <td style="border:1px solid" align="center"></td>
    <td style="border:1px solid" align="center">AAD<br>updated</td>
    <td style="border:1px solid" align="center">AAD<br>updated</td>
    <td style="border:1px solid" align="center"></td>
    <td style="border:1px solid" align="center"></td>
    <td style="border:1px solid" align="center"></td>
