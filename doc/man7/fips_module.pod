...
If those applications attempt to use any algorithms that are not present,
then they will fail.
...
FIPS module, but it is not actually automatically loaded when the application
starts. The FIPS provider can then be loaded programmatically like this:
...
Finally, use L<strcmp(3)> or L<printf(3)> on the name.
