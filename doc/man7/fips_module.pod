...
For backwards compatability individual algorithms may override the checks by
using either an option in the FIPS configuration (See
L<fips_config(5)/FIPS indicator options>) OR in code using an algorithm context setter.
...
can be queried using an algorithm context getter to retrieve the "fips-indicator".
...
=item DSA Signatures L<EVP_SIGNATURE-DSA(7)/Signature Parameters>

=item ECDSA Signatures L<EVP_SIGNATURE-ECDSA(7)/ECDSA Signature Parameters>

=item EC Key Generation L<EVP_PKEY-EC(7)/Common EC parameters>
...
AES-GCM also has a indicator called "iv-generated".
See L<EVP_EncryptInit(3)/PARAMETERS)> for further information. This indicator
does not trigger the indicator callback.
