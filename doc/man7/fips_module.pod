...
Every algorithm implementation advertised with the property C<fips=yes> by
the OpenSSL FIPS provider exposes "fips-indicator" as a gettable context
parameter.  The getter returns 1 when the operation is approved.  An absent
indicator must not be interpreted as approval.

Algorithms advertised with C<fips=no> are unapproved.  When such an
implementation exposes "fips-indicator", its getter returns 0.  Providers
other than the OpenSSL FIPS provider are not required to expose this parameter;
an absent indicator, or an indicator value of 0, does not assert FIPS approval.

Some algorithms can change from approved to unapproved as a result of their
parameters or the operation performed.  The following algorithms perform such
additional checks:
...
=item ML-KEM Key Encapsulation

Supplying the test-only "ikme" parameter makes the operation unapproved.
See L<EVP_KEM-ML-KEM(7)/ML-KEM KEM parameters>.

=item ML-DSA and SLH-DSA Signatures

Supplying test entropy makes the operation unapproved.  ML-DSA operations
that disable the standard message encoding are also unapproved.
See L<EVP_SIGNATURE-ML-DSA(7)/FIPS indicator> and
L<EVP_SIGNATURE-SLH-DSA(7)/FIPS indicator>.

=item RSA Key Generation

Supplying the parameters reserved for FIPS module testing makes the operation
unapproved.  See L<EVP_PKEY-RSA(7)/RSA key generation FIPS indicator>.
...
=item X448MLKEM1024 Key Generation and Key Encapsulation

The other hybrid ML-KEM algorithms supplied by the FIPS provider are approved,
including X25519MLKEM768, SecP256r1MLKEM768 and SecP384r1MLKEM1024.
...
The AES-GCM "fips-indicator" also accounts for the IV source and returns 0
after encryption with an externally supplied IV.  It also returns 0 for GCM
tag lengths other than 4, 8, or 12 through 16 bytes.
...
The signature "fips-indicator" also accounts for this value and returns 0
when verification is performed directly on a caller-supplied digest.
