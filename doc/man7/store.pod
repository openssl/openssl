...
=head2 A generic call
 ...
 while (!STORE_eof(ctx)) {
     ...
     /* Do whatever is necessary with the STORE_INFO, here just one example */
     switch (STORE_INFO_get_type(info)) {
     case STORE_INFO_X509:
         /* Print the X.509 certificate text */
         X509_print_fp(stdout, STORE_INFO_get0_CERT(info));
         /* Print the X.509 certificate PEM output */
         PEM_write_X509(stdout, STORE_INFO_get0_CERT(info));
         break;
     }
 }
