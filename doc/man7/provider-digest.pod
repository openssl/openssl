...
OSSL_FUNC_digest_init() initializes a digest operation given a newly created
...
previously initialized digest operation.
The I<dctx> parameter contains a pointer to a previously initialized provider
side context.
...
Instead the provider context that was created during provider initialization is
...
Parameters currently recognized by built-in digests with this function
...
Diverse flags that describe exceptional behavior for the digest:
