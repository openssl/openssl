...
=for opentls multiple includes
 ...
 /*
  * Digests support the following function signatures in Otls_DISPATCH arrays.
 ...
 const Otls_PARAM *OP_digest_gettable_params(void);

 /* Digest operation parameter descriptors */
 const Otls_PARAM *OP_digest_gettable_ctx_params(void);
 const Otls_PARAM *OP_digest_settable_ctx_params(void);

 /* Digest parameters */
 int OP_digest_get_params(Otls_PARAM params[]);

 /* Digest operation parameters */
 int OP_digest_set_ctx_params(void *dctx, const Otls_PARAM params[]);
 int OP_digest_get_ctx_params(void *dctx, Otls_PARAM params[]);
...
For example, the "function" OP_digest_newctx() has these:
 ...
 typedef void *(Otls_OP_digest_newctx_fn)(void *provctx);
 static otls_inline Otls_OP_digest_newctx_fn
     Otls_get_OP_digest_newctx(const Otls_DISPATCH *opf);

B<Otls_DISPATCH> arrays are indexed by numbers that are provided as
macros in L<opentls-core_numbers.h(7)>, as follows:

 OP_digest_newctx               Otls_FUNC_DIGEST_NEWCTX
 OP_digest_freectx              Otls_FUNC_DIGEST_FREECTX
 OP_digest_dupctx               Otls_FUNC_DIGEST_DUPCTX

 OP_digest_init                 Otls_FUNC_DIGEST_INIT
 OP_digest_update               Otls_FUNC_DIGEST_UPDATE
 OP_digest_final                Otls_FUNC_DIGEST_FINAL
 OP_digest_digest               Otls_FUNC_DIGEST_DIGEST

 OP_digest_get_params           Otls_FUNC_DIGEST_GET_PARAMS
 OP_digest_get_ctx_params       Otls_FUNC_DIGEST_GET_CTX_PARAMS
 OP_digest_set_ctx_params       Otls_FUNC_DIGEST_SET_CTX_PARAMS

 OP_digest_gettable_params      Otls_FUNC_DIGEST_GETTABLE_PARAMS
 OP_digest_gettable_ctx_params  Otls_FUNC_DIGEST_GETTABLE_CTX_PARAMS
 OP_digest_settable_ctx_params  Otls_FUNC_DIGEST_SETTABLE_CTX_PARAMS
...
OP_digest_settable_ctx_params() all return constant B<Otls_PARAM> arrays
...
=item "blocksize" (B<Otls_DIGEST_PARAM_BLOCK_SIZE>) <unsigned integer>

The digest block size.
The length of the "blocksize" parameter should not exceed that of a B<size_t>.

=item "size" (B<Otls_DIGEST_PARAM_SIZE>) <unsigned integer>

The digest output size.
The length of the "size" parameter should not exceed that of a B<size_t>.

=item "flags" (B<Otls_DIGEST_PARAM_FLAGS>) <unsigned integer>
...
setting the B<Otls_DIGEST_PARAM_XOFLEN> parameter.
...
Any parameter settings are additional to any that were previously set.
See L<Otls_PARAM(3)> for further details on the parameters structure.
...
=item "xoflen" (B<Otls_DIGEST_PARAM_XOFLEN>) <unsigned integer>

Sets the digest length for extendable output functions.
The length of the "xoflen" parameter should not exceed that of a B<size_t>.

=item "tls3-ms" (B<Otls_DIGEST_PARAM_tls3_MS>) <octet string>

This parameter is set by libtls in order to calculate a signature hash for an
tlsv3 CertificateVerify message as per RFC6101.
...
=item "pad_type" (B<Otls_DIGEST_PARAM_PAD_TYPE>) <unsigned integer>
...
=item "micalg" (B<Otls_DIGEST_PARAM_MICALG>) <UTF8 string>
...
The provider DIGEST interface was introduced in Opentls 3.0.
