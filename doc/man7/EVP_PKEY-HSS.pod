...
The B<HSS> keytype is implemented in OpenSSL's default provider.
Due to the stateful nature of B<HSS> keys only signature verification is
supported in OpenSSL. This means that only B<HSS> public keys are supported.
...
Note that for each level there is only one 'active' LMS tree and we only need to
generate a new active tree if the current 'active' tree is exhausted.
Once the top level tree is exhausted no more signatures can be produced.
The nodes of the leaf tree are used for signing and they are only allowed to be
used one time (OTS). Reusing a signature is a security issue. To implement this
the node index 'state' must be stored in the private key and updated each time
...
Generated signature data consists of encoded public keys for intermediate
