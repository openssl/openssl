...
If another provider has already been loaded then the default provider
won't be loaded automatically. Therefore if it is to be used in
conjunction with other providers then it must be loaded explicitly.
Automatic loading of the default provider only occurs once; if the default
provider is explicitly unloaded or the explicit provider load function
L<OSSL_PROVIDER_load(3)> is called successfully or unsuccessfully,
the default provider will not be automatically loaded again.
