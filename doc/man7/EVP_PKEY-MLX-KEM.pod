...
The algorithms supported here are specifically for use in either TLS 1.3 or HPKE,
and are not suitable for other purposes. There is no encoder/decoder support, and
...
The following names are supported only in OpenSSL's default provider.
...
=item B<HPKE-XWING> or B<HPKE-MLKEM768-X25519>

This is a HPKE hybrid ML-KEM algorithm that combines MLKEM768 and X25519.

=item B<HPKE-MLKEM768P256>

This is a HPKE hybrid ML-KEM algorithm that combines MLKEM768 and EC P-256.

=item B<HPKE-MLKEM1024P384>

This is a HPKE hybrid ML-KEM algorithm that combines MLKEM1024 and EC P-384.
...
=item "dhkem-ikm" (B<OSSL_PKEY_PARAM_IKM>) <octet string>

Used for testing purposes purposes to deterministically set the input key material
that will be used to derive a keypair.  If this is set B<OSSL_PKEY_PARAM_SEED>
must not be set. This parameter is only used by HPKE algorithms.

=item "seed" (B<OSSL_PKEY_PARAM_SEED> <octet string>

Used for testing purposes purposes to deterministically set the raw seed used
generate a keypair. If this is set B<OSSL_PKEY_PARAM_IKM> must not be set.
This parameter is only used by HPKE algorithms.
...
A 1184 byte ML-KEM-768 public key followed by a 65 byte EC public key.

=item B<HPKE-MLKEM1024P384>

A 1568 byte ML-KEM-1024 public key followed by a 97 byte EC public key.
...
2400 byte ML-KEM-768 private key followed by a 32 byte EC private key.

=item B<HPKE-MLKEM1024P384>

3168 byte ML-KEM-1024 private key followed by a 48 byte EC private key.
...
=item https://datatracker.ietf.org/doc/html/draft-irtf-cfrg-hybrid-kems-11

=item https://datatracker.ietf.org/doc/draft-ietf-hpke-pq/04/

=item https://datatracker.ietf.org/doc/draft-connolly-cfrg-xwing-kem/

=item https://datatracker.ietf.org/doc/html/draft-irtf-cfrg-concrete-hybrid-kems-03
...
Support for B<HPKE-XWING>, B<HPKE-MLKEM768P256> and B<HPKE-MLKEM1024P384> was
added in openSSL 4.1.
