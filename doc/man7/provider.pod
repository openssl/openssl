...
I<NOTE: This section is mostly interesting for those writing APIs that
are directly dealing with providers, or provider authors.>

A I<provider> offers an initialization function, as set of base
functions in form of a B<OSSL_DISPATCH> array, and by extension, a set
of B<OSSL_ALGORITHM>s (see L<openssl-core.h(7)>).
...
Instead, the diverse OpenSSL APIs oftemn have fetching functions that
do the work, and that returns an appropriate method object back to the
user.
...
See L<OPENSSL_CTX(3)> for a closer description.
...
See L<property(7)> for a closer description.
...
When they are use with functions like L<EVP_DigestInit_ex(3)> or
...
implementations the same agorithms), the property "default=yes" can be
...
BLAKE2b512, BLAKE2s256 and MD5-SHA1.
...
AES-128-CTR, id-aes256-GCM, id-aes192-GCM and id-aes128-GCM.
...
(see L<config(5)>).
Should it be needed (if other providers are loaded and offer
implementations the same agorithms), the property "fips=yes" can be
used as a search criterion for these implementations.
...
(see L<config(5)>).
Should it be needed (if other providers are loaded and offer
implementations the same agorithms), the property "legacy=yes" can be
used as a search criterion for these implementations.
...
RIPEMD160, MD2, MD4, MD5, MDC2 and whirlpool.
