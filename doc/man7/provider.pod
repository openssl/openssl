...
A I<provider> offers an initialisation function, as a set of base
...
If it's a dynamically loadable module, the initialisation function
must be named C<OSSL_provider_init> and must be exported.
If it's built-in, the initialisation function may have any name.

The initialisation function must have the following signature:
