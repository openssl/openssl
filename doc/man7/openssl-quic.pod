...
This man page describes how to let applications use QUIC protocol using the
libssl API.

QUIC protocol maps to the standard SSL API. A QUIC connection is represented
...
in. QUIC clients can use L<OSSL_QUIC_client_method(3)> or
...
Returns 1 if a SSL object is a QUIC domain SSL object.
