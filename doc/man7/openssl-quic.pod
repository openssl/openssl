...
Initially, only client connectivity was supported. Now, server connectivity is
also supported. This man page describes the usage of QUIC functionality for both
...
QUIC functionality. To make use of QUIC for clients, use the SSL method
L<OSSL_QUIC_client_method(3)> or L<OSSL_QUIC_client_thread_method(3)> with
L<SSL_CTX_new(3)>. For servers, use the SSL method L<OSSL_QUIC_server_method(3)>
with L<SSL_CTX_new(3)>.
...
An client application wishing to use QUIC must use L<OSSL_QUIC_client_method(3)>
...
Creates a listener SSL object. A listener SSL object differs from an ordinary
SSL object in that it is used to provide an abstraction for the acceptance of
network connections in a protocol-agnostic manner.

Currently, listener SSL objects are only supported for QUIC server usage or
client-only usage. It is expected that the listener interface will be expanded
...
Returns 1 if and only if a SSL object is a listener SSL object.
...
L<SSL_get_accept_connection_queue_len(3)>
