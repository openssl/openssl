...
with WSAECONNRESET error causing OpenSSL QUIC stack to enter permanent
...
OpenSSL attempts to always dissable SIO_UDP_CONNRESET and SIO_UDP_NETRESET
on UDP sockets it receives from application, but no error is reported back
when it fails. Robust application should set those options itself so
it can handle errors from WSAIoctl() conveniently.
