...
I<We use the wide term "decode" in this manual.  This includes but is
not limited to deserialization.>
...
a memory data, it should provide a L<BIO_s_mem(3)> B<BIO>. The decoded
...
that decodes DER to an provider-native object.

As the last decoding step in the decoding chain is supposed to create
a provider-native object referenced by an object reference the
OSSL_FUNC_decoder_export_object() allows importing that object into
a different provider directly as the final step of the decoding
process.
...
can decode.  Currently known input types are:
