...
L<OSSL_DESERIALIZER(3)> allows multiple encodings, depending on existing
deserializers.  If central OpenSSL library functionality is to handle the
data directly, it B<must> be encoded in DER.
...
The exact target implementation is determined from the I<object type> and
possibly the I<object data type>.  For example, an object abstraction with

the I<object type> B<OSSL_OBJECT_PKEY> will have code to fetch a
L<provider-keymgmt(7)> using the I<object data type> as its key type (the
second argument in L<EVP_KEYMGMT_fetch(3)>).
