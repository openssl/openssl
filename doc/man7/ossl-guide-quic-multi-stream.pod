...
stream at all and instead should use a spearate stream B<SSL> object for each
...
this is only meaningful for connection level B<SSL> objects.
...
a simple multi-stream QUIC client application which connects to a server, sends
some HTTP/1.0 requests to it, and reads back the responses. Note that HTTP/1.0
...
waiting for more data to become availabe and return it when it is, or it will
