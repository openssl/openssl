...
=item "keep-private" (B<OSSL_PKEY_PARAM_KEEP_PRIVATE>) <int>

A parameter settable on key generation. If set to nonzero it makes the key
resulting from the public key generation from a supplied private key to keep
