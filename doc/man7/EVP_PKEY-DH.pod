...
of the problems associated with FIPS186-4 style parameters as discussed in
L</DH key validation>.
...
(but the I<gindex> or I<hindex> can not be stored). It is recommended to use a
...
value for backwards compatability.
...
SP800-56Ar3 'FFC Full Public-Key Validation'.

For DH keys L<EVP_PKEY_public_check_quick(3)> conforms to
SP800-56Ar3 'FFC Partial Public-Key Validation' when the
...
For backwards compatability the OpenSSL default provider only requires I<p> to
be set.

For DH keys L<EVP_PKEY_pairwise_check(3)> conforms to the
SP800-56Ar3 'Owner Assurance of Pair-wise Consistency'.
