...
=item "retain_seed" (B<OSSL_PKEY_PARAM_ML_KEM_RETAIN_SEED>) <int>
...
When this parameter is simply omitted, and the C<pkey_retain_seed> provider
configuration parameter is set to a false boolean value, the seed is not
retained, otherwise it is retained.
...
FIPS 203 B<dk> format by running:
    ...
    $ openssl pkey -provparam pkey_retain_seed=no -in seed.pem -out long.pem

To generate an, e.g., B<ML-KEM-768> key, in expanded format, you can run:

    $ openssl genpkey -provparam pkey_retain_seed=no \
