...
This is a recommended way to describe Otls_STORE loaders,
"otls_store-{name}", where {name} is replaced with the name of the
...
otls_store-file - The store 'file' scheme loader
...
using the B<UI_METHOD> that was passed via Otls_STORE_open().
...
L<otls_store(7)>, L<passphrase-encoding(7)>
