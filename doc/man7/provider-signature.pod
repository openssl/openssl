...
These correspond to L<EVP_SignInit(3)> and L<EVP_SignFinal(3)>, respectively. They typically operate on an already-computed digest value (i.e. the signature implementation is not responsible for hashing arbitrary message data).

=item OSSL_FUNC_signature_sign_message_init and OSSL_FUNC_signature_sign
These correspond to L<EVP_PKEY_sign_init(3)> and L<EVP_PKEY_sign(3)>, respectively. They are also used to handle message data directly, but typically in a one-shot fashion.
...
These correspond to L<EVP_PKEY_sign_init(3)> and L<EVP_PKEY_sign(3)>. They allow for streaming of message data (via the sign_message_update function) before producing the signature (via the sign_message_final function).

=item OSSL_FUNC_signature_verify_init and OSSL_FUNC_signature_verify
These correspond to L<EVP_VerifyInit(3)> and L<EVP_VerifyFinal(3)>, respectively. They typically operate on an already-computed digest value to verify a signature.

=item OSSL_FUNC_signature_verify_message_init and OSSL_FUNC_signature_verify
These correspond to L<EVP_PKEY_verify_init(3)> and L<EVP_PKEY_verify(3)>, respectively, for one-shot message verification.
...
These correspond to L<EVP_PKEY_verify_init(3)> and L<EVP_PKEY_verify(3)> for the streaming variant.

=item OSSL_FUNC_signature_verify_recover_init and OSSL_FUNC_signature_verify_recover
These correspond to L<EVP_PKEY_verify_recover_init(3)> and L<EVP_PKEY_verify_recover(3)>. They are used for signature algorithms that support message recovery (i.e. extracting the message or a portion of it from the signature).
...
These correspond to L<EVP_DigestSignInit(3)>, L<EVP_DigestSignUpdate(3)>, and L<EVP_DigestSignFinal(3)>, respectively. This full streaming API (which includes the digest computation) is particularly important for use within libssl (such as TLS 1.3), where providers are expected to implement the digest sign variant.
...
These correspond to L<EVP_DigestVerifyInit(3)>, L<EVP_DigestVerifyUpdate(3)>, and L<EVP_DigestVerifyFinal(3)>, respectively. Similarly, for protocols that require combined digest and verify operations (e.g. TLS 1.3 in libssl), this streaming API is often required.

=item OSSL_FUNC_signature_digest_sign_init and OSSL_FUNC_signature_digest_sign
These correspond to L<EVP_DigestSignInit(3)> and L<EVP_DigestSign(3)>, respectively, providing a one-shot digest sign operation.

=item OSSL_FUNC_signature_digest_verify_init and OSSL_FUNC_signature_digest_verify
These correspond to L<EVP_DigestVerifyInit(3)> and L<EVP_DigestVerify(3)>, respectively, providing a one-shot digest verify operation.
