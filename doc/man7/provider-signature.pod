...
OSSL_FUNC_signature_set_ctx_params and OSSL_FUNC_signature_settable_ctx_params
function are optional,
but if one of them is present then the other one must also be present.
The same applies to the
OSSL_FUNC_signature_get_ctx_params and OSSL_FUNC_signature_gettable_ctx_params functions,
as well as the "md_params" functions.

The OSSL_FUNC_signature_dupctx function is optional.
So far it is not used by OpenSSL.
