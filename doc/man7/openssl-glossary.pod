...
L<OSSL_DECODER_CTX_new_for_pkey.pod>
...
L<OSSL_ENCODER_CTX_new_for_pkey.pod>
...
finally choosing the implementation that will be used. Fetches can either be
explicit where a function call is made to obtain an algorithm object
representing an implementation such as L<EVP_MD_fetch(3)> or
L<EVP_CIPHER_fetch(3)>, or implicit where an algorithm object with no associated
implementation is used such as the return value from L<EVP_sha256(3)> or
L<EVP_aes_128_cbc(3)>. With implicit fetching an implementation is fetched
automatically using default selection criteria the first time the algorithm is
used.
...
An operation in OpenSSL is a group of algorithms with a common purpose such as
encryption, or digesting. Each operation has a set of OpenSSL functions
associated with it for using that operation.
