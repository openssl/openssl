...
=for openssl multiple includes
 ...
 const OSSL_PARAM *OSSL_FUNC_kdf_get_params(void *provctx);
 const OSSL_PARAM *OSSL_FUNC_kdf_get_ctx_params(void *provctx);
 const OSSL_PARAM *OSSL_FUNC_kdf_set_ctx_params(void *provctx);
...
OSSL_FUNC_kdf_derive() performs the KDF operatin.
The I<kctx> parameter contains a pointer to the provider side context.
The resulting key should be written to I<key> and the amount of data written
to I<*keylen>.
...
OSSL_FUNC_kdf_get_ctx_params() gets details of currently set parameter values
associated with the given provider side KDF context I<kctx> and stores them
in I<params>.
