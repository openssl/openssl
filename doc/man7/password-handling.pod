...
encoding again. This will ensure that the exact same byte sequence is used in
both cases.  In case the character encoding is UTF-8, the exact locale is not
important, as long as it's still UTF-8.
...
PKCS#12, this should open up any object that was created according to spec.
...
Try the password (i.e. the byte sequence) that you have as it is, without
conversion.  It's possible that it's the exact right sequence.  This probably
only works with objects created with OpenSSL older than 1.1.0.
