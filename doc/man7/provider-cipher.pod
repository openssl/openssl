...
The IV to be used is given in B<iv> which B<ivlen> bytes long.
...
B<out> may be the same location as B<in> but it will not partially overlap.
...
OP_cipher_encrypt_init() or OP_cipher_decrypto_init(), and OP_cipher_update()
calls.
