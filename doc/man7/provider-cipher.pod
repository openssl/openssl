...
Gets 1 if this is an AEAD cipher algorithm, otherwise it returns 0.

=item "custom-iv" (B<OSSL_CIPHER_PARAM_CUSTOM_IV>) <integer>

Gets 1 if the cipher algorithm has a custom IV, otherwise it returns 0.
Storing and initializing the IV is left entirely to the implementation, if a
custom IV is used.

=item "cts" (B<OSSL_CIPHER_PARAM_CTS>) <integer>

Gets 1 if the cipher algorithm uses ciphertext stealing, otherwise it returns 0.
...
it returns 0. The interleaving is an optimization only applicable to certain
TLS ciphers.
