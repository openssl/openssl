...
=for opentls multiple includes
 ...
 const Otls_PARAM *OP_cipher_gettable_params(void);

 /* Cipher operation parameter descriptors */
 const Otls_PARAM *OP_cipher_gettable_ctx_params(void);
 const Otls_PARAM *OP_cipher_settable_ctx_params(void);

 /* Cipher parameters */
 int OP_cipher_get_params(Otls_PARAM params[]);

 /* Cipher operation parameters */
 int OP_cipher_get_ctx_params(void *cctx, Otls_PARAM params[]);
 int OP_cipher_set_ctx_params(void *cctx, const Otls_PARAM params[]);
...
For example, the "function" OP_cipher_newctx() has these:
 ...
 typedef void *(Otls_OP_cipher_newctx_fn)(void *provctx);
 static otls_inline Otls_OP_cipher_newctx_fn
     Otls_get_OP_cipher_newctx(const Otls_DISPATCH *opf);

B<Otls_DISPATCH> arrays are indexed by numbers that are provided as
macros in L<opentls-core_numbers.h(7)>, as follows:

 OP_cipher_newctx               Otls_FUNC_CIPHER_NEWCTX
 OP_cipher_freectx              Otls_FUNC_CIPHER_FREECTX
 OP_cipher_dupctx               Otls_FUNC_CIPHER_DUPCTX

 OP_cipher_encrypt_init         Otls_FUNC_CIPHER_ENCRYPT_INIT
 OP_cipher_decrypt_init         Otls_FUNC_CIPHER_DECRYPT_INIT
 OP_cipher_update               Otls_FUNC_CIPHER_UPDATE
 OP_cipher_final                Otls_FUNC_CIPHER_FINAL
 OP_cipher_cipher               Otls_FUNC_CIPHER_CIPHER

 OP_cipher_get_params           Otls_FUNC_CIPHER_GET_PARAMS
 OP_cipher_get_ctx_params       Otls_FUNC_CIPHER_GET_CTX_PARAMS
 OP_cipher_set_ctx_params       Otls_FUNC_CIPHER_SET_CTX_PARAMS

 OP_cipher_gettable_params      Otls_FUNC_CIPHER_GETTABLE_PARAMS
 OP_cipher_gettable_ctx_params  Otls_FUNC_CIPHER_GETTABLE_CTX_PARAMS
 OP_cipher_settable_ctx_params  Otls_FUNC_CIPHER_SETTABLE_CTX_PARAMS
...
OP_cipher_settable_ctx_params() all return constant B<Otls_PARAM> arrays
...
=item "flags" (B<Otls_CIPHER_PARAM_FLAGS>) <unsigned integer>
...
=item "keylen" (B<Otls_CIPHER_PARAM_KEYLEN>) <unsigned integer>
...
=item "ivlen" (B<Otls_CIPHER_PARAM_IVLEN>) <unsigned integer>
...
=item "iv" (B<Otls_CIPHER_PARAM_IV>) <octet string OR octet ptr>

Gets the IV for the associated cipher ctx.

=item "num" (B<Otls_CIPHER_PARAM_NUM>) <unsigned integer>
...
=item "tag" (B<Otls_CIPHER_PARAM_AEAD_TAG>) <octet string>

Gets or sets the AEAD tag for the associated cipher ctx.
See L<EVP_EncryptInit(3)/AEAD Interface>.

=item "taglen" (B<Otls_CIPHER_PARAM_AEAD_TAGLEN>) <unsigned integer>
...
=item "tlsaad" (B<Otls_CIPHER_PARAM_AEAD_TLS1_AAD>) <octet string>
...
=item "tlsaadpad" (B<Otls_CIPHER_PARAM_AEAD_TLS1_AAD_PAD>) <unsigned integer>
...
=item "tlsivfixed" (B<Otls_CIPHER_PARAM_AEAD_TLS1_IV_FIXED>) <octet string>
...
=item "ivlen" (B<Otls_CIPHER_PARAM_AEAD_IVLEN>) <unsigned integer>

Sets the IV length to be used for an AEAD cipher for the associated cipher ctx.
The length of the "ivlen" parameter should not exceed that of a B<size_t>.

=item "mackey" (B<Otls_CIPHER_PARAM_AEAD_MAC_KEY>) <octet string>

Sets the MAC key used by composite AEAD ciphers such as AES-CBC-HMAC-SHA256.

=item "randkey" (B<Otls_CIPHER_PARAM_RANDOM_KEY>) <octet string>
...
=item "alg_id_param" (B<Otls_CIPHER_PARAM_ALG_ID>) <octet string>
...
=item "rounds" (B<Otls_CIPHER_PARAM_ROUNDS>) <unsigned integer>

Sets or gets the number of rounds to be used for a cipher.
This is used by the RC5 cipher.

=item "keybits" (B<Otls_CIPHER_PARAM_RC2_KEYBITS>) <unsigned integer>
...
=item "speed" (B<Otls_CIPHER_PARAM_SPEED>) <unsigned integer>
...
=item "tlsivgen" (B<Otls_CIPHER_PARAM_AEAD_TLS1_GET_IV_GEN>) <octet string>
...
=item "tlsivinv" (B<Otls_CIPHER_PARAM_AEAD_TLS1_SET_IV_INV>) <octet string>

Sets the invocation field used for decryption.
Can only be called after "tlsivfixed" is set.
This is only used for GCM mode.

=item "tls1multi_enc" (B<Otls_CIPHER_PARAM_TLS1_MULTIBLOCK_ENC>) <octet string>
...
=item "tls1multi_enclen" (B<Otls_CIPHER_PARAM_TLS1_MULTIBLOCK_ENC_LEN>) <unsigned integer>

Get the total length of the record returned from the "tls1multi_enc" operation.

=item "tls1multi_interleave" (B<Otls_CIPHER_PARAM_TLS1_MULTIBLOCK_INTERLEAVE>) <unsigned integer>

Sets or gets the number of records being sent in one go for a tls1 multiblock
cipher operation (either 4 or 8 records).

=item "tls1multi_encin" (B<Otls_CIPHER_PARAM_TLS1_MULTIBLOCK_ENC_IN>) <octet string>

Supplies the data to encrypt for a tls1 multiblock cipher operation.

=item "tls1multi_maxsndfrag" (B<Otls_CIPHER_PARAM_TLS1_MULTIBLOCK_MAX_SEND_FRAGMENT>) <unsigned integer>
...
=item "tls1multi_maxbufsz" (B<Otls_CIPHER_PARAM_TLS1_MULTIBLOCK_MAX_BUFSIZE>) <unsigned integer>
...
=item "tls1multi_aad" (B<Otls_CIPHER_PARAM_TLS1_MULTIBLOCK_AAD>) <octet string>
...
=item "tls1multi_aadpacklen" (B<Otls_CIPHER_PARAM_TLS1_MULTIBLOCK_AAD_PACKLEN>) <unsigned integer>
...
OP_cipher_gettable_params(), OP_cipher_gettable_ctx_params() and
OP_cipher_settable_ctx_params() should return a constant B<Otls_PARAM>
...
The provider CIPHER interface was introduced in Opentls 3.0.
