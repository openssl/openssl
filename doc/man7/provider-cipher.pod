...
provider-cipher - The CIPHER provider dispatch interface
...
=for openssl multiple includes
 ...
 /* Context management */
 void *{cipher_newctx}(void *provctx);
 void {cipher_freectx}(void *cctx);
 void *{cipher_dupctx}(void *cctx);

 /* Encryption/decryption */
 int {cipher_encrypt_init}(void *cctx, const unsigned char *key,
                           size_t keylen, const unsigned char *iv,
                           size_t ivlen);
 int {cipher_decrypt_init}(void *cctx, const unsigned char *key,
                           size_t keylen, const unsigned char *iv,
                           size_t ivlen);
 int {cipher_update}(void *cctx, unsigned char *out, size_t *outl,
                     size_t outsize, const unsigned char *in, size_t inl);
 int {cipher_final}(void *cctx, unsigned char *out, size_t *outl,
                    size_t outsize);
 int {cipher_cipher}(void *cctx, unsigned char *out, size_t *outl,
                     size_t outsize, const unsigned char *in, size_t inl);

 /* Cipher parameter descriptors */
 const OSSL_PARAM *{cipher_gettable_params}(void);

 /* Cipher operation parameter descriptors */
 const OSSL_PARAM *{cipher_gettable_ctx_params}(void);
 const OSSL_PARAM *{cipher_settable_ctx_params}(void);

 /* Cipher parameters */
 int {cipher_get_params}(OSSL_PARAM params[]);

 /* Cipher operation parameters */
 int {cipher_get_ctx_params}(void *cctx, OSSL_PARAM params[]);
 int {cipher_set_ctx_params}(void *cctx, const OSSL_PARAM params[]);
...
macros in L<openssl-core_dispatch.h(7)>, as follows:
 ...
 {cipher_newctx}                  OSSL_FUNC_CIPHER_NEWCTX
 {cipher_freectx}                 OSSL_FUNC_CIPHER_FREECTX
 {cipher_dupctx}                  OSSL_FUNC_CIPHER_DUPCTX

 {cipher_encrypt_init}            OSSL_FUNC_CIPHER_ENCRYPT_INIT
 {cipher_decrypt_init}            OSSL_FUNC_CIPHER_DECRYPT_INIT
 {cipher_update}                  OSSL_FUNC_CIPHER_UPDATE
 {cipher_final}                   OSSL_FUNC_CIPHER_FINAL
 {cipher_cipher}                  OSSL_FUNC_CIPHER_CIPHER

 {cipher_get_params}              OSSL_FUNC_CIPHER_GET_PARAMS
 {cipher_get_ctx_params}          OSSL_FUNC_CIPHER_GET_CTX_PARAMS
 {cipher_set_ctx_params}          OSSL_FUNC_CIPHER_SET_CTX_PARAMS

 {cipher_gettable_params}         OSSL_FUNC_CIPHER_GETTABLE_PARAMS
 {cipher_gettable_ctx_params}     OSSL_FUNC_CIPHER_GETTABLE_CTX_PARAMS
 {cipher_settable_ctx_params}     OSSL_FUNC_CIPHER_SETTABLE_CTX_PARAMS
...
In all cases both the {cipher_newctx} and {cipher_freectx} functions must be
present.
All other functions are optional.

=head2 Context Management Functions

{cipher_newctx}() should create and return a pointer to a provider side
...
{cipher_freectx}() is passed a pointer to the provider side cipher context in
the I<cctx> parameter.
This function should free any resources associated with that context.

{cipher_dupctx}() should duplicate the provider side cipher context in the
I<cctx> parameter and return the duplicate copy.

=head2 Encryption/Decryption Functions

{cipher_encrypt_init}() initialises a cipher operation for encryption given a
...
{cipher_decrypt_init}() is the same as {cipher_encrypt_init}() except that it
initialises the context for a decryption operation.

{cipher_update}() is called to supply data to be encrypted/decrypted as part of
a previously initialised cipher operation.
The I<cctx> parameter contains a pointer to a previously initialised provider
side context.
{cipher_update}() should encrypt/decrypt I<inl> bytes of data at the location
pointed to by I<in>.
...
{cipher_update}() may be called multiple times for a single cipher operation.
...
{cipher_final}() completes an encryption or decryption started through previous
{cipher_encrypt_init}() or {cipher_decrypt_init}(), and {cipher_update}()
calls.
...
{cipher_cipher}() performs encryption/decryption using the provider side cipher
context in the I<cctx> parameter that should have been previously initialised via
a call to {cipher_encrypt_init}() or {cipher_decrypt_init}().
...
{cipher_get_params}() gets details of the algorithm implementation
and stores them in I<params>.

{cipher_set_ctx_params}() sets cipher operation parameters for the
provider side cipher context I<cctx> to I<params>.
Any parameter settings are additional to any that were previously set.

{cipher_get_ctx_params}() gets cipher operation details details from
the given provider side cipher context I<cctx> and stores them in I<params>.

{cipher_gettable_params}(), {cipher_gettable_ctx_params}(), and
{cipher_settable_ctx_params}() all return constant B<OSSL_PARAM> arrays
as descriptors of the parameters that {cipher_get_params}(),
{cipher_get_ctx_params}(), and {cipher_set_ctx_params}() can handle,
respectively.
...
TLS records are encrypted/decrypted using a single {cipher_cipher} call per
record.
...
{cipher_cipher} call should be the length of the payload excluding the explicit
...
{cipher_newctx}() and {cipher_dupctx}() should return the newly created
provider side cipher context, or NULL on failure.

{cipher_encrypt_init}(), {cipher_decrypt_init}(), {cipher_update}(),
{cipher_final}(), {cipher_cipher}(), {cipher_get_params}(),
{cipher_get_ctx_params}() and {cipher_set_ctx_params}() should return 1 for
success or 0 on error.

{cipher_gettable_params}(), {cipher_gettable_ctx_params}() and
{cipher_settable_ctx_params}() should return a constant B<OSSL_PARAM>
