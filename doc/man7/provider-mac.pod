...
=for openssl multiple includes
 ...
 int OSSL_FUNC_mac_squeeze(void *mctx, unsigned char *out, size_t outsize);
...
macros in L<openssl-core_dispatch.h(7)>, as follows:
 ...
 OSSL_FUNC_mac_squeeze              OSSL_FUNC_MAC_SQUEEZE
...
OSSL_FUNC_mac_squeeze() allows a MAC to produce an infinite-length output and
can be called multiple times after the OSSL_FUNC_mac_update() call.
OSSL_FUNC_mac_final() and OSSL_FUNC_mac_squeeze() should NOT be used during the
same MAC operation. Note that most MAC algorithms do not support XOF operations.
The I<mctx> parameter contains a pointer to the provider side context.
The resulting MAC should write I<outsize> bytes to I<out>.
...
OSSL_FUNC_mac_update(), OSSL_FUNC_mac_final(), OSSL_FUNC_mac_squeeze(),
...
The function OSSL_FUNC_mac_squeeze() was introduced in OpenSSL 4.0.
