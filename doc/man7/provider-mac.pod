...
=for openssl multiple includes
 ...
 void OSSL_FUNC_mac_cleanse(void *mctx);
...
macros in L<openssl-core_dispatch.h(7)>, as follows:
 ...
 OSSL_FUNC_mac_cleanse              OSSL_FUNC_MAC_CLEANSE
...
OSSL_FUNC_mac_cleanse() clears any sensitive data for an existing provider side
mac context in the I<mctx> parameter. This can be used if the user intends to
maintain a pool of existing provider side mac contexts that can be used for
multiple operations by calling OSSL_FUNC_mac_init() with a new MAC I<key>.
If there is a delay before reuse, OSSL_FUNC_mac_cleanse() should been called to
clear out any sensitive data related to the last operation.
OSSL_FUNC_mac_cleanse() is not needed when OSSL_FUNC_mac_freectx() is called,
...
The function OSSL_FUNC_mac_cleanse() was added in OpenSSL 4.1.
