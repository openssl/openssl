...
=for opentls multiple includes
 ...
 const Otls_PARAM *OP_mac_get_params(void);
 const Otls_PARAM *OP_mac_get_ctx_params(void);
 const Otls_PARAM *OP_mac_set_ctx_params(void);

 /* MAC parameters */
 int OP_mac_get_params(Otls_PARAM params[]);
 int OP_mac_get_ctx_params(void *mctx, Otls_PARAM params[]);
 int OP_mac_set_ctx_params(void *mctx, const Otls_PARAM params[]);
...
For example, the "function" OP_mac_newctx() has these:
 ...
 typedef void *(Otls_OP_mac_newctx_fn)(void *provctx);
 static otls_inline Otls_OP_mac_newctx_fn
     Otls_get_OP_mac_newctx(const Otls_DISPATCH *opf);

B<Otls_DISPATCH> arrays are indexed by numbers that are provided as
macros in L<opentls-core_numbers.h(7)>, as follows:

 OP_mac_newctx               Otls_FUNC_MAC_NEWCTX
 OP_mac_freectx              Otls_FUNC_MAC_FREECTX
 OP_mac_dupctx               Otls_FUNC_MAC_DUPCTX

 OP_mac_init                 Otls_FUNC_MAC_INIT
 OP_mac_update               Otls_FUNC_MAC_UPDATE
 OP_mac_final                Otls_FUNC_MAC_FINAL

 OP_mac_get_params           Otls_FUNC_MAC_GET_PARAMS
 OP_mac_get_ctx_params       Otls_FUNC_MAC_GET_CTX_PARAMS
 OP_mac_set_ctx_params       Otls_FUNC_MAC_SET_CTX_PARAMS

 OP_mac_gettable_params      Otls_FUNC_MAC_GETTABLE_PARAMS
 OP_mac_gettable_ctx_params  Otls_FUNC_MAC_GETTABLE_CTX_PARAMS
 OP_mac_settable_ctx_params  Otls_FUNC_MAC_SETTABLE_CTX_PARAMS
...
OP_mac_settable_ctx_params() all return constant B<Otls_PARAM> arrays
...
=item "custom" (B<Otls_MAC_PARAM_CUSTOM>) <UTF8 string>
...
=item "xof" (B<Otls_MAC_PARAM_BLOCK_XOF>) <integer>
...
=item "size" (B<Otls_MAC_PARAM_SIZE>) <integer>
...
OP_mac_gettable_params(), OP_mac_gettable_ctx_params() and
OP_mac_settable_ctx_params() should return a constant B<Otls_PARAM>
...
The provider MAC interface was introduced in Opentls 3.0.
