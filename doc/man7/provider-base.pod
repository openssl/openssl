...
the following details supplied (all are mandatory, excluding
...
Boolean flag to describe is the group should be used in KEX mode (0, default) or
in KEM mode (1).
...
then I<derive> against the peer public key.
...
the same I<shared secret> generated by the server's I<encapsulate>.
