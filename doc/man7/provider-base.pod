...
provider-base
- The basic OpenSSL library E<lt>-E<gt> provider dispatch interface
...
=head1 SYNOPSIS
  ...
  * the function names.
  *
  * See the L<Dispatched Functions> section for an explanation how to
  * interpret those prototypes.
  */
...
All functions listed here are so called I<dispatched functions>, which
are passed as function pointers between
...
=head2 Dispatched Functions

We indicate a dispatched function by placing the {function_name} in
curly brackets.
A dispatched function {function_name} represents a function pointer
which is associated with a numeric ID B<OSSL_FUNC_FUNCTION_NAME>
and a function signature, given in form of a function type definition
B<OSSL_FUNC_function_name_fn>. There is a helper function named
B<OSSL_FUNC_function_name> to retrieve the function pointer from the
B<OSSL_DISPATCH> element.

For example, the dispatched function {core_gettable_params} has the
following signature

 const OSSL_ITEM * {core_gettable_params}(const OSSL_CORE_HANDLE *handle);

Roughly speaking, this is simply the signature of the function stored
in the dispatch table, if you forget about the curly braces.

In more detail, it means that a numeric ID,
...
and helper function to retrieve the function pointer

 static ossl_inline OSSL_NAME_core_gettable_params_fn
     OSSL_FUNC_core_gettable_params(const OSSL_DISPATCH *opf);

are associated with the dispatched function {core_gettable_params}.


The numeric IDs serve as an index into the B<OSSL_DISPATCH> and are provided as
...
{core_gettable_params}() returns a constant array of descriptors
