...
must support the TLS 1.3 record operations used by libssl: setting the IV length
and authentication tag, obtaining the authentication tag, AAD updates, and
...
SSL_set_SSL_CTX() keeps its existing behaviour: it does not replace the SSL
...
TLS 1.3 selects its ciphersuite before the servername callback runs. Applications
with server-name-specific ciphersuite policies should apply them before selection,
using the ClientHello callback described in L<SSL_CTX_set_client_hello_cb(3)> and
SSL_set_ciphersuites(). Changing the list in the servername callback does not
retroactively change the selected ciphersuite.

Provider-defined ciphersuites are restricted to TLS 1.3 and are not available
for DTLS or QUIC.
...
Provider-defined ciphersuites have no built-in cipher-family classification,
so family-specific preferences do not apply. They do not use kTLS.
...
The application must trust the providers supplying the capability and the
fetched algorithms. The declared security bits are not an independent assessment
of the algorithms or their implementations.
