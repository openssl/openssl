...
The core_obj_create() function registers a new OID and associated short and
long names for the given I<handle>. It corresponds to the OpenSSL function
L<OBJ_create(3)> except that it returns 1 on success or 0 on failure. A common
failure reason is that the OID is already registered.
...
to identify the object. It returns 1 on success or 0 on failure.
