...
Engine-based hardware acceleration for asymmetric algorithms will be available
if you use the legacy API (e.g. B<RSA_METHOD>) and create the RSA object from
parameters.  However, if you get the B<RSA> object with
L<PEM_read_bio_PrivateKey(3)> and L<EVP_PKEY_get1_RSA(3)>, the hardware offload will
not work.

To ensure the future compatibility, the engines shouldbe turned to providers.
