...
widely used file formats, an application code that checks for particular error
reason codes on key loading failures might need an update.

The password-protected keys may deserve a special attention. If only some errors
are treated as an indicator that the user should be asked about the password again,
it's worth adding processing the error lib B<ERR_LIB_OSSL_DECODER> and error
reason B<ERR_R_UNSUPPORTED> as a new case.
