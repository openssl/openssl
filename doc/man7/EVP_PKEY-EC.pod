...
This parameter has no effect at key generation time: the value
supplied via B<EVP_PKEY_CTX_set_params>(3) or
B<-pkeyopt point-format:E<lt>formE<gt>> is silently ignored, and
generated keys are always in uncompressed form.
The keygen codepath for this option had been ineffective for
several releases without complaint, and compressed and hybrid
encodings of EC public keys are largely deprecated in modern
protocols (TLS 1.3 mandates the uncompressed point format for key
exchange, and places no restriction on the key form in
certificates), so the parameter is now formally a no-op for new
keys.
The parameter is still honoured on existing keys, so imported keys
-- via B<EVP_PKEY_set_params>(3), B<EVP_PKEY_fromdata>(3) or
PEM/DER decode -- retain and round-trip their original encoding.
