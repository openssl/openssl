...
digital certificates (also known as X.509 certifictes). Finally it also supplies
various additional supporting APIs that are not directly cryptography related
but are nonetheless useful and dependended upon by other APIs. For example the
"BIO" functions provide capabilities for abstracting I/O, e.g. via a file or
over a network.
...
examples of how use the various API functions. To look at them download the
