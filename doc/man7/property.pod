...
and underscores.
Property names are case-insensitive, but OpenSSL to only use lowercase
letters.
...
name, which is prefixed prefixed by some sort of 'namespace'.
...
A property is a I<name=value> pair.
A property definition is a sequence of comma separated properties.
...
"fips=yes", "default!=yes" or "?iteration.count!=3".
The first two represent mandatory clauses, such clauses B<must> match
for any algorithm to even be in consideration.
...
A query is a sequence of comma separated query clauses.
...
chosen among those in an indeterminate way.
