...
and L<OSSL_ENCODER_CTX_set_params(3)> to perform a just-in-time encoding
of a private key to a seed-only B<PKCS#8> form.
When its value is B<1>, the encoded B<PKCS#8> object will be in C<seed-only>
form, overriding any provider output formats and seed retention parameters.
When its value is B<0>, the parameter is effectively ignored.
Any other value is presently unsupported and results in an error.
...
keys are encoded to B<PKCS#8> form, unless the C<seed-only> encoder
parameter is set to B<1> as described above.
...
The C<seed-only> parameter was added in OpenSSL 4.0.
