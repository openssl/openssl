...
decapsulation key B<sk> is present in the above sequence.
...
If the C<oqs-kpair> format is not included in the list, this format will not be
recognised on input.
...
The legacy C<oqs-kpair>, C<bare-seed> and C<bare-priv> formats can also be
