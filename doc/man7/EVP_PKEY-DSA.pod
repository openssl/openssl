...
The following values can be used by OpenSSL's default and FIPS providers:
...
provider will instead chose a named safe prime group based on the size of I<q>.
...
is not set or -1 then unverifiable generation of generator I<g> will be used.
...
For unverifiable generation of generator I<g> this value is output during 
generation of I<g>. It value is the first integer larger than one that satisfies
g = h^j mod p (where g != 1 and "j" is the cofactor).
...
If specified then a named safe prime named will be chosen using the "pbits" type.
...
For "fips186_4" this can be either 2048 (DSA also allows 3072).
