...
As a normal application developer, you don't have to worry about most details,
just use L<RAND_bytes(3)> to obtain random data.
Having said that, there are a few important rules to obey: Always check the error
return value of L<RAND_bytes(3)>, don't take randomness for granted and make
sure system entropy sources can be accessed at all times (especially after chroot).
