...
Although (re-)seeding is done automatically, it can happen that it fails,
because no trusted random source is available or it (temporarily) fails
to provide the requested random seed.
In this case the CSPRNG enters an error state and refuses to provide output,
