...
derived if not provided.  Setting a nonzero value will cause any of n, e, d, and all
needed exponents and coefficients to be derived if not available.  Setting this
option requires at least OSSL_PARAM_RSA_FACTOR1 and OSSL_PARAM_RSA_FACTOR2 to be
set.  Additionally in the multiprime case, in which more than
OSSL_PARAM_RSA_FACTOR3 or above is provided, OSSL_PARAM_RSA_N and
OSSL_PARAM_RSA_D must also be provided in the parameter list.
If any of the above potentially derived values are already set in the
parameter list, they will not be overwritten with separately derived values.
