...
=head2 RSA key generation FIPS indicator

The following key generation parameter can be retrieved with
L<EVP_PKEY_CTX_get_params(3)> from the OpenSSL FIPS provider:

=over 4

=item "fips-indicator" (B<OSSL_PKEY_PARAM_FIPS_APPROVED_INDICATOR>) <integer>

Returns 1 if the operation is FIPS approved, or 0 otherwise.
See L<fips_module(7)/FIPS indicators> for more information.
Using any of the FIPS module testing parameters described below causes this
