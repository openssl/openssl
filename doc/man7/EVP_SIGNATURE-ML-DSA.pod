...
The following parameter can be retrieved using
L<EVP_PKEY_CTX_get_params(3)>:

=over 4

=item "fips-indicator" (B<OSSL_SIGNATURE_PARAM_FIPS_APPROVED_INDICATOR>) <integer>

The OpenSSL FIPS provider returns 1 if the operation is FIPS approved, or 0
otherwise.  See L<fips_module(7)/FIPS indicators> for more information.
Supplying "test-entropy", or setting the test-only "message-encoding"
parameter to 0, causes this indicator to return 0.
