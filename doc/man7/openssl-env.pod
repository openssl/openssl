...
This variable is considered as a security-sensitive environment variable.

=item B<HARNESS_OSSL_PREFIX>

Allows specification of a prefix prepended to each line sent to the I<stdout>
by L<openssl(1)>, used by the test harness to avoid comingling the command
under test output with the output for the TAP consumer.

This variable is not considered as a security-sensitive.
...
These variables are considered as security-sensitive environment variables.
...
=item B<OPENSSL_MALLOC_FD>, B<OPENSSL_MALLOC_FAILURES>, B<OPENSSL_MALLOC_SEED>

If built with debugging, this allows memory allocation to fail.
See L<OPENSSL_malloc(3)>.

These variables are not considered as security-sensitive.
