...
is set-user-ID, set-group-ID or has capabilities.

L<OSSL_LIB_CTX_get_safe_getenv_function(3)> can be used to provide an
application callback to control the security-sensitivity or other aspects
of retrieving an environment variable's value.
