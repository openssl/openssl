...
provider-serializer - The SERIALIZER provider dispatch interface
...
=for openssl multiple includes
 ...
 void *{serializer_newctx}(void *provctx);
 void {serializer_freectx}(void *ctx);
 int {serializer_set_ctx_params}(void *ctx, const OSSL_PARAM params[]);
 const OSSL_PARAM *{serializer_settable_ctx_params}(void)

 /* Functions to serialize object data */
 int {serializer_serialize_data}(void *ctx, const OSSL_PARAM *data,
                                 OSSL_CORE_BIO *out,
                                 OSSL_PASSPHRASE_CALLBACK *cb,
                                 void *cbarg);
 int {serializer_serialize_object}(void *ctx, void *obj, OSSL_CORE_BIO *out,
                                   OSSL_PASSPHRASE_CALLBACK *cb,
                                   void *cbarg);
...
B<OSSL_FUNC_{name}>.
For example, the "function" {serializer_serialize_data}() has these:

 typedef int
     ({serializer_serialize_data_fn})(void *provctx,
                                            const OSSL_PARAM params[],
                                            OSSL_CORE_BIO *out);
 static ossl_inline {serializer_serialize_data_fn}
     {serializer_serialize_data}(const OSSL_DISPATCH *opf);

B<OSSL_DISPATCH> arrays are indexed by numbers that are provided as
macros in L<openssl-core_dispatch.h(7)>, as follows:

 {serializer_newctx}              OSSL_FUNC_SERIALIZER_NEWCTX
 {serializer_freectx}             OSSL_FUNC_SERIALIZER_FREECTX
 {serializer_set_ctx_params}      OSSL_FUNC_SERIALIZER_SET_CTX_PARAMS
 {serializer_settable_ctx_params} OSSL_FUNC_SERIALIZER_SETTABLE_CTX_PARAMS

 {serializer_serialize_data}      OSSL_FUNC_SERIALIZER_SERIALIZE_DATA
 {serializer_serialize_object}    OSSL_FUNC_SERIALIZER_SERIALIZE_OBJECT
...
{serializer_newctx}() returns a context to be used with the rest of
the functions.

{serializer_freectx}() frees the given I<ctx>, if it was created by
{serializer_newctx}().

{serializer_set_ctx_params}() sets context data according to
parameters from I<params> that it recognises.  Unrecognised parameters
should be ignored.

{serializer_settable_ctx_params}() returns a constant B<OSSL_PARAM>
array describing the parameters that {serializer_set_ctx_params}()
can handle.

See L<OSSL_PARAM(3)> for further details on the parameters structure used
by {serializer_set_ctx_params}() and {serializer_settable_ctx_params}().

=head2 Serializing functions

=for comment There will be a "Deserializing functions" title as well

{serializer_serialize_data}() should take an array of B<OSSL_PARAM>,
...
{serializer_serialize_object}() should take a pointer to an object
...
{serializer_serialize_data}() and {serializer_serialize_object}()
...
{serializer_newctx}() returns a pointer to a context, or NULL on
failure.

{serializer_set_ctx_params}() returns 1, unless a recognised
parameters was invalid or caused an error, for which 0 is returned.

{serializer_settable_ctx_params}() returns a pointer to an array of
