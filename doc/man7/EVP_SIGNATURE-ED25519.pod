...
generation, one-shot signature generation and verification using the EdDSA
...
EdDSA can be accessed using both the EVP_PKEY_sign()/EVP_PKEY_verify()
and EVP_DigestSign()/EVP_DigestVerify() sets of API functions. Both
APIs function identically. However, when calling EVP_DigestSignInit()
or EVP_DigestVerifyInit(), the digest I<type> parameter B<MUST> be
set to NULL.

The PureEdDSA instances do not support the streaming mechanism of
other signature algorithms using, for example, EVP_DigestSignUpdate().
The message to sign or verify must be passed using the one-shot
EVP_PKEY_sign(), EVP_PKEY_verify(), EVP_DigestSign(), or EVP_DigestVerify()
functions.
...
To sign a message using an ED25519 EVP_PKEY structure and the EVP_PKEY_sign API:

    void do_sign(EVP_PKEY *ed_key, unsigned char *msg, size_t msg_len)
    {
        size_t sig_len;
        unsigned char *sig = NULL;
        EVP_PKEY_CTX *pkey_ctx = EVP_PKEY_CTX_new_from_pkey(NULL, ed_key, NULL);
        ...
        /* The input "params" is not needed if default options are acceptable.
           Use EVP_PKEY_sign_init in that case. */
        EVP_PKEY_sign_init_ex(pkey_ctx, params);
        /* Calculate the required size for the signature by passing a NULL buffer. */
        EVP_PKEY_sign(pkey_ctx, NULL, &sig_len, msg, msg_len);
        sig = OPENSSL_zalloc(sig_len);

        EVP_PKEY_sign(pkey_ctx, sig, &sig_len, msg, msg_len);
        ...
        OPENSSL_free(sig);
        EVP_PKEY_CTX_free(pkey_ctx);
    }

To verify a message using an ED448 EVP_PKEY structure and the EVP_PKEY_verify API:

    void do_verify(EVP_PKEY *ed_key, unsigned char *msg, size_t msg_len,
                   unsigned char *sig, size_t sig_len)
    {
        EVP_PKEY_CTX *pkey_ctx = EVP_PKEY_CTX_new_from_pkey(NULL, ed_key, NULL);

        const OSSL_PARAM params[] = {
            OSSL_PARAM_utf8_string ("instance", "Ed448", 5),
            OSSL_PARAM_octet_string("context-string", (unsigned char *)"A protocol defined context string", 33),
            OSSL_PARAM_END
        };

        /* The input "params" is not needed if default options are acceptable.
           Use EVP_PKEY_verify_init in that case. */
        EVP_PKEY_verify_init_ex(pkey_ctx, params);

        int ret = EVP_PKEY_verify(pkey_ctx, sig, sig_len, msg, msg_len);
        /*
...
To sign a message using an ED25519 EVP_PKEY structure and the EVP_DigestSign API:
...
To verify a message using an ED448 EVP_PKEY structure and the EVP_DigestVerify API:
                   ...
                   unsigned char *sig, size_t sig_len)
    {
        ...
        /* The input "params" is not needed if default options are acceptable.
           Use NULL in place of "params" in that case. */
        EVP_DigestVerifyInit_ex(md_ctx, NULL, NULL, NULL, NULL, ed_key, params);
...
EdDSA support was added to EVP_PKEY_sign() and EVP_PKEY_verify() in
OpenSSL 3.3.0.
