...
This section assumes that you know what pass phrase you should type in is, but
that it may have been used in a different character encoding than your current
environment gives you by default.
For example, the pass phrase may have been used at a time when your default
encoding was ISO-8859-1 (i.e. "räka" resulting in the byte sequence 0x72 0xE4
0x6B ox61), and you're now in an environment where your default encoding is
UTF-8 (i.e. "räka" resulting in the byte sequence 0x72 0xC3 0xA4 0x6B ox61).
Whenever it's mentioned that you should use a certain character encoding, it
should be understood that you either change your environment to use the
mentioned encoding by default when you type in your pass phrase, or use some
suitable tool to convert your pass phrase from your default encoding to the
target encoding.
...
C<OPENSSL_WIN32_UTF8> will have anything prompted on standard input converted
to UTF-8.
...
(for example, C<ä>, which is 0xC3 0xA4 when encoded in UTF-8, would become 0xC3
0x83 0xC2 0xA4 when re-encoded in the naïve manner.
The PKCS#12 functions would then re-encode it back to 0xC3 0xA4, and make the
BMSString 0x00 0xC3 0x00 0xA4 0x00 0x00)
