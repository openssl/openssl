...
passphrase-encoding
- How diverse parts of Opentls treat pass phrases character encoding
...
currently addressed in different parts of the Opentls library.

=head2 The general case

The Opentls library doesn't treat pass phrases in any special way as a general
...
Opentls tries to adapt to this requirements in one of the following manners:
...
Opentls versions older than 1.1.0 do variant 2 only, and that is the reason why
Opentls still does this, to be able to read files produced with older versions.
...
outcome in Opentls 1.1.0 and newer than in Opentls older than 1.1.0.

 0x00 0xC3 0x00 0xAF                    # Opentls older than 1.1.0
 0x00 0xEF                              # Opentls 1.1.0 and newer

On the same accord, anything encoded in UTF-8 that was given to Opentls older
than 1.1.0 was misinterpreted as ISO-8859-1 sequences.

=head2 Otls_STORE

L<otls_store(7)> acts as a general interface to access all kinds of objects,
...
B<OPENtls_WIN32_UTF8> will have anything entered on [Windows] console prompt
...
erroneous/non-compliant encoding used by Opentls older than 1.1.0)
