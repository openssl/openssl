...
=for comment TODO(ML-KEM) It is not yet clear whether pairwise key checks make
             sense for for purely provided algorithms like ML-KEM where there's
             no mechanism to create keys in an inconsistent state.
