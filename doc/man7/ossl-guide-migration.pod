...
=head3 Semantic change of EVP_get_cipherby* and EVP_get_digestby* functions

Before 4.0 functions B<EVP_get_cipherby*> and B<EVP_get_digestby*> were often
used as fallback in case when EVP_CIPHER_fetch() and EVP_MD_fetch() implying
possibility to access an algorithm implementation provided by an engine. Now
these functions on success return the implementation from the default OpenSSL
library context with default properties. This can be probably not the desired
result for applications that use the nondefault library context.

The other difference between B<EVP_get_cipherby*> and B<EVP_get_digestby*> and
EVP_CIPHER_fetch() and EVP_MD_fetch() is that the former functions deal with
the algorithm aliases, and the newer ones don't (except the ones declared in
the particular providers). Applications wishing to preserve alias handling
behaviour can first check for an alias using B<OBJ_NAME_get()> (with
B<OBJ_NAME_TYPE_CIPHER_METH> or B<OBJ_NAME_TYPE_MD_METH>) and fall back to
EVP_CIPHER_fetch() or EVP_MD_fetch() if no match is found.
