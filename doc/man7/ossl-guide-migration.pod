...
=head3 Introduction of library reference counting

OpenSSL 3.x encountered various issues with automatic library cleanup via
atexit() registration.  To address these issues, OpenSSL is moving to a library
reference counting implementation.  Users of the OpenSSL 4.0 libcrypto library
should register their software via a call to L<OPENSSL_add_library_user(3)>,
which will increment the reference count on the library.  Correspondingly, when
a software component has ceased its use of OpenSSL, it should indicate such via
a call to L<OPENSSL_cleanup_ex(3)>.  Only after OPENSSL_cleanup_ex() has been called
an equal number of times to OPENSSL_add_library_user(), will library resources
be freed.  Failure to balance these calls will result in memory leaks or
undefined behavior (the latter occurring in the case where OPENSSL_cleanup() is
called without a prior corresponding call to OPENSSL_add_library_user()).

Note that OPENSSL_cleanup() has been deprecated in OpenSSL 4.0.0.  While this
API call may still be used, its behavior is altered in light of the new
reference counted API above.  If no other software component within a process
uses the OPENSSL_add_library_user() and OPENSSL_cleanup_ex() calls above,
OPENSSL_cleanup() functions exactly as it does in prior releases.  However, if
any other software component makes a call to OPENSSL_add_library_user() or
OPENSSL_cleanup_ex(), then any calls to OPENSSL_cleanup become no-ops.  This is
to ensure that the library does not free resources prematurely.  Note that the
result of this design change is that library memory resources may leak on
application exit.  Only when all software components of a process make use of
the new api, can proper leak-free cleanup be performed.

Note that libcrypto opportunistically uses the B<__attribute__((constructor))> and
B<__attribute__((destructor))> compiler extensions to automatically increase/decrease
the reference counts on libcrypto on platforms/compilers that support those features, and
so, on those platforms, manual addition of these calls are not necessecary.
