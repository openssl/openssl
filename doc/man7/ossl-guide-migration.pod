...
=head3 The ASN1_STRING type is now opaque

ASN1_STRING is the basis for many types in OpenSSL. Since OpenSSL 1.1.1
accessor functions have been available to access and create ASN1_STRING
values of various types. See ASN1_STRING_type_new(3),
ASN1_STRING_get0_data(3), ASN1_STRING_set(3) and ASN1_STRING_length(3).

If your code uses direct access to ASN1_STRING structure members you will
need to convert it to allocate the correct type ASN1_STRING, and use accessors.

If your code has in the past used an ASN1_STRING as a stack allocated
object, you will need to allocate it from the heap with
ASN1_STRING_new(3) or ASN1_STRING_type_new(3).

The flags member of ASN1_STRING has become inaccessible, and the definitions
...
The final flag was used to indicate that an ASN1_BIT_STRING has a
value of unused bits left - Most applications do not touch this.
You should not use ASN1_STRING_set() to set the value of an
ASN1_BIT_STRING that may have unused bits on the end.
To ensure the number of unused bits is correctly set in an
ASN1_BIT_STRING type, use the functions ASN1_BIT_STRING_set_bit(3) or
ASN1_BIT_STRING_set1(3) to set the value of a BIT_STRING, and ensure
that the number of unused bits is correctly set. The function
ASN1_BIT_STRING_get_length(3) may be used to retrieve the length in
bytes, and the number of unused bits of an ASN1_BIT_STRING.

=head3 Constification of X509 functions

A large number of public API functions that access an X509 * object have
been constified. This change has been made to allow for future improvements
to the X509 layer to reduce the number of memory allocations and copies
...
bearing in mind that your copy has no effect on the original X509* object itself.

The following functions have had arguments / return values related to X509* access
...
an explicitly mutable pointer from within an X509 object:
