...
library context with default properties. This is probably not the desired
result for applications that use a nondefault library context.
...
algorithm aliases, and the newer ones don't (except the ones declared in a
specific provider as explicit aliases). Applications wishing to preserve alias
handling behaviour can first check for an alias using B<OBJ_NAME_get()> (with
