...
The functionality includes symmetric encryption, public key cryptography and key
...
referred to in OpenSSL as "algorithms". An algorithm can have more than one
implementation of it that is available for use. For example the RSA algorithm
is available as a "default" implementation suitable for general use, and a
"fips" implementation which has been validated to FIPS standards for situations
where that is important. It is also possible that a third party could add
...
safe at all. Where this is the case this restriction will be noted in the
...
be used to explicitly fetch a digest algorithm implementation.
...
The algorithm implementation that is fetched can then be used with diverse other
...
provider can also be selected with the property "fips=yes". The FIPS provide
also contains a number of non-approved algorithm implementations and these can
be selected with the property "fips=no".
...
insecure, or are no longer in common use. Should it be needed (if other
providers are loaded and offer implementations of the same algorithms), the
...
Cryptographic algorithms are made available to applications through using the
...
them will a config file using the L<OSSL_LIB_CTX_load_config(3)> function.
