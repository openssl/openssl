...
If you perform the same operation many times then it is recommeded to use
L</Explicit fetching>. An algorithm may be prefetched once initially,
and then passed to any operations that are currently using L</Implicit fetching>.
See an example of Explicit fecthing in L</USING ALGORITHMS IN APPLICATIONS>.
...
and then build the method table. Internally OpenSSL caches similar algorithms
on the first fetch (so loading a digest caches all digests).
