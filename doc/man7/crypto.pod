...
Functions that revolve around B<EVP_PKEY_CTX> and B<EVP_PKEY>, such as
...
L<EVP_SIGNATURE(3)>) and an L<EVP_KEYMGMT(3)> for the B<EVP_PKEY>, they try
the following:
...
provider of the B<EVP_PKEY>'s L<EVP_KEYMGMT(3)> implementation, try to fetch a
L<EVP_KEYMGMT(3)> implementation in the same provider as the operation type
implementation and export the B<EVP_PKEY> to it (effectively making a
...
provider as the original B<EVP_PKEY>'s L<EVP_KEYMGMT(3)>, still using the
