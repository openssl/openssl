...
Given any input message (of any size), a Hash Function (or Message Digest)
produces a fixed size output (hash) value as a result.

A Hash function exhibits the following properties:

=over 4

=item * It is quick to create a hash value for any given message.

=item * A Hash function is deterministic.

=item * It is computationally infeasible to calculate a message from any given hash (i.e. the function is one-way)

=item * It is infeasible to modify a message without also modifying the hash value

=item * It is infeasible to find two messages that result in the same hash.

=item * Small changes to an input message result in significant changes to the out hash (avalanche effect).
...
See L<OSSL_PROVIDER-default(7)/Hashing Algorithms E<sol> Message Digests>
and L<OSSL_PROVIDER-FIPS(7)/Hashing Algorithms E<sol> Message Digests> for a
...
See also the L<digest demos|https://github.com/openssl/openssl/tree/master/demos/digest>.

Hash Functions are used by other algorithms such as L</Message Authentication Code (MAC)> and L</Digital Signature>.

=head2 Public key cryptography

Public-key cryptography (or Asymmetric Cryptography) use pairs of related keys (a key pair) to perform
...
L<EVP_PKEY-EC(7)> is used for L</Key Agreement> and L</Digital Signature>.
L<EVP_PKEY-SM2(7)>

The edwards curves L<EVP_PKEY-X25519(7)> and L<EVP_PKEY-X448(7)> are used for L</Key Agreement>.
The edwards curves L<EVP_PKEY-ED25519(7)>, L<EVP_PKEY-ED448(7)> are used for L</Digital Signature>.

=item * RSA

See L<EVP_PKEY-RSA(7)> which is used by 
L</Key Encapsulation Mechanism (KEM)>,
L</Digital Signature> and
...
Domain parameters are normally public values associated with key pairs,
that can be shared by multiple parties, and are used for key generation,
key agreement and digital signatures.

Domain parameters are used by DSA, DH and EC.

For more information see:

=over 4

=item * L<EVP_PKEY-DSA(7)/DSA parameters>

=item * L<EVP_PKEY-DH(7)/DH and DHX domain parameters>

=item * L<EVP_PKEY-EC(7)/Common EC parameters>

=back

=head3 Asymmetric Key Generation

Asymetric key generation is the process of generating key pairs.
In many cases keys are randomly generated using a pseudo-random number generator (PRNG).
Large integers (in the form of B<BIGNUM>'s) or byte arrays are used to store keys.

See L<Demos for keygen|https://github.com/openssl/openssl/tree/master/demos/pkey>
...
See L<Encrypt Demo|https://github.com/openssl/openssl/blob/master/demos/encrypt/rsa_encrypt.c>

=head3 Digital Signature
...
=item * L<EVP_DigestSignInit(3)>

=item * L<EVP_DigestVerifyInit(3)>

=item * L<Signature demos|https://github.com/openssl/openssl/tree/master/demos/signature>

=item * L<EVP_SIGNATURE-ECDSA(7)>

=item * L<EVP_SIGNATURE-ED25519(7)>

=item * L<EVP_SIGNATURE-ED448(7)>

=item * L<EVP_SIGNATURE-RSA(7)>

=item * L<EVP_SIGNATURE-DSA(7)>

=back

=head3 Public Key Certificates
...
The L</Digital signature> of an entity that has verified the certificate's contents (Issuer).
...
=head3 Asymmetric Key Validation

Prior to, or during a key-establishment transaction, the parties involved
must obtain assurances about the validity of the key(s) being used.
See L<SP800-56A|https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-56Ar3.pdf>
for more information.

The OpenSSL FIPS provider implements these assurances using:

=over 4

=item * L<EVP_PKEY_check(3)>

=item * L<EVP_PKEY_param_check(3)>

=item * L<EVP_PKEY_param_check_quick(3)>

=item * L<EVP_PKEY_public_check(3)>

=item * L<EVP_PKEY_public_check_quick(3)>

=item * L<EVP_PKEY_private_check(3)>

=item * L<EVP_PKEY_pairwise_check(3)>
...
=item * L<EVP_PKEY-DH(7)/DH key validation>

=item * L<EVP_PKEY-DSA(7)/DSA key validation>,

=item * L<EVP_PKEY-EC(7)/EC key validation>

=item * L<EVP_PKEY-RSA(7)/RSA key validation>
...
Each OpenSSL Key manager supports methods to handle Asymmetric Key generation,
Asymmetric Key validation, as well as the ability to import keys into a provider,
and export keys from a provider.

For a list of Key managers see:

=over 4

=item * L<OSSL_PROVIDER-default(7)/Asymmetric Key Management>

=item * L<OSSL_PROVIDER-FIPS(7)/Asymmetric Key Management>
...
in comparison to L</Public key cryptography>
...
Key Agreement Schemes involve the process of sharing of a secret key between two or more entities,
either by:
...
Key Agreement (Key exchange) is the process of generating a shared key for each entity,
based on information shared by the entities.

For more information see:

=over 4

=item * L<Key exchange demo|https://github.com/openssl/openssl/tree/master/demos/keyexch>

=item * L<EVP_KEYEXCH-ECDH(7)>

Refer to "ecdh-cofactor-mode" in order to use ECCDH as defined in SP800-56A.

=item * L<EVP_KEYEXCH-DH(7)>

=item * L<EVP_KEYEXCH-X25519(7)>

=back

=item * Key Transport

Key transportation is defined as the secure transportation of a key from one entity to another.
...
=item * Symmetric Key Wrapping

The Advanced Encryption Standard (AES) Key Wrap algorithm is defined in
L<SP800-38F|https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-38F.pdf>

See L<EVP_CIPHER-AES(7)> and L<AES Key Wrapping Demo|https://github.com/openssl/openssl/tree/master/demos/cipher/aeskeywrap.c>.

This consists of:

=over 4 

=item AES Key Wrap (KW) mode

The algorithm names are "AES-128-WRAP", "AES-192-WRAP" and "AES-256-WRAP".

=item AES Key Wrap With Padding (KWP) mode

The algorithm names are "AES-128-WRAP-PAD", "AES-192-WRAP-PAD" and "AES-256-WRAP-PAD".

=item inverse ciphers

The inverse ciphers use AES decryption for wrapping, and AES encryption for unwrapping.

The algorithms names are "AES-128-WRAP-INV", "AES-192-WRAP-INV", "AES-256-WRAP-INV",
"AES-128-WRAP-PAD-INV", "AES-192-WRAP-PAD-INV" and "AES-256-WRAP-PAD-INV".

=item * Asymmetric Key Wrapping

Key-Transport Using RSA-OAEP is defined in
L<SP800-56BR2 RSA-OAEP|https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-56Br2.pdf#%5B%7B%22num%22%3A290%2C%22gen%22%3A0%7D%2C%7B%22name%22%3A%22XYZ%22%7D%2C70%2C710%2C0%5D>

For further information see L<EVP_ASYM_CIPHER-RSA(7)> and L<RSA OEAP demo|https://github.com/openssl/openssl/tree/master/demos/encrypt/rsa_encrypt.c>

=back

=back

KAS protocols are defined by L<SP800-56A|https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-56Ar3.pdf> and
L<SP800-56B|https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-56Br2.pdf>.
Note that OpenSSL does not implement the high level protocols, but it does supply all the building blocks needed
to implement them including the KDF's defined in L<SP800-56C|https://csrc.nist.gov/publications/detail/sp/800-56c/rev-2/final>
and L</Asymmetric Key Validation>.
...
=item * L<EVP_PKEY_encapsulate(3)>

=item * L<EVP_PKEY_decapsulate(3)>

=item * L<EVP_KEM-RSA(7)>

This supports RSA Secret Value Encapsulation (RSASVE) as defined in
L<SP800-56B|https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-56Br2.pdf#%5B%7B%22num%22%3A213%2C%22gen%22%3A0%7D%2C%7B%22name%22%3A%22XYZ%22%7D%2C70%2C404%2C0%5D>

=item * L<EVP_KEM-EC(7)> and L<EVP_KEM-X25519(7)>

This support DHKEM which is used by L<HPKE|/Hybrid Public Key Encryption (HPKE)>
...
Hybrid Public Key Encryption is a modern standard for generic hybrid encryption (See L<RFC9810|https://www.rfc-editor.org/rfc/rfc9180.pdf>).
...
MAC's are a family of secret-key cryptographic algorithms acting on input data of arbitrary length to produce an output value of a specified length
...
MAC's are used by other algorithms such as L</Key Derivation Function (KDF)> and L</Deterministic Random Bit Generators (DRBG)>.

More information related to MAC's can be found at:

=over 4

=item * L<EVP_MAC(3)>

=item * L<Mac demos|https://github.com/openssl/openssl/tree/master/demos/mac>

=item * L<OSSL_PROVIDER-default(7)/Message Authentication Code (MAC)>

=item * L<OSSL_PROVIDER-FIPS(7)/Message Authentication Code (MAC)>

=item * L<migration_guide(7)/Message Authentication Code API (EVP_MAC)>

This contains information related to the old API used.
...
KDF's are L<deterministic|openssl-glossary(7)/Deterministic>.

For more information related to OpenSSL KDF's see:

=over 4

=item * L<EVP_KDF(3)>

=item * L<KDF demos|https://github.com/openssl/openssl/tree/master/demos/kdf>

=item * Password based KDF's

Examples are L<EVP_KDF-SCRYPT(7)> and L<EVP_KDF-PBKDF2(7)>.

=item * Non Password based KDF's.

Examples are L<EVP_KDF-HKDF(7)>, L<EVP_KDF-SS(7)>, L<EVP_KDF-KB(7)> and L<EVP_KDF-SSHKDF(7)>.

=item * L<OSSL_PROVIDER-default(7)/Key Derivation Function (KDF)>

=item * L<OSSL_PROVIDER-FIPS(7)/Key Derivation Function (KDF)>

=item * L<OSSL_PROVIDER-legacy(7)/Key Derivation Function (KDF)>
