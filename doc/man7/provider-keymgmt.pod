...
=item "ri-type" (B<OSSL_PKEY_PARAM_CMS_RI_TYPE>) <UTF8 string>

The value should be the CMS RecipientInfo type for the given key, for example
CMS_RECIPINFO_KEM or CMS_RECIPINFO_AGREE.
...
using CMS_RECIPINFO_TRANS.

=item "kemri-kdf-alg" (B<OSSL_PKEY_PARAM_CMS_KEMRI_KDF_ALGORITHM>) <UTF8 string>

The value should be the default KDF for this key it supports the KEMRecipientInfo
(CMS_RECIPINFO_KEM) type.

Any KDF that takes B<OSSL_KDF_PARAM_KEY> and B<OSSL_KDF_PARAM_INFO> parameters
and is otherwise fully defined by its name or OID can be used, for example
B<HKDF-SHA256>.
