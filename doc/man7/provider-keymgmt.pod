...
libraries, it's a rather internal one that's designed to work in
tandem with operations that use private/public key pairs.

Because the KEYMGMT operation share knowledge with the operations it
works in tandem with, they must belong to the same provider.
...
B<OSSL_ALGORITHM> arrays that are returned by the providers
...
B<OSSL_get_{name}>.
For example, the "function" OP_keymgmt_importdomparams() have these:
...
with the identity I<id> or size I<idlen>.
