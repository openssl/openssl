...
and initialize it with I<selection>, which will determine what kind
of contents the key object to be generated should get.
In particular if B<OSSL_KEYMGMT_SELECT_PUBLIC_KEY> is set and
B<OSSL_KEYMGMT_SELECT_PRIVATE_KEY> is not set the public key is to be generated
from a private key set as the template.
