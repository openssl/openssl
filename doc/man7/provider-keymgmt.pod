...
provider-keymgmt - The KEYMGMT provider dispatch interface
...
=for openssl multiple includes
 ...
 void *{keymgmt_new}(void *provctx);
 void {keymgmt_free}(void *keydata);

 void *{keymgmt_gen_init}(void *provctx, int selection);
 int {keymgmt_gen_set_template}(void *genctx, void *template);
 int {keymgmt_gen_set_params}(void *genctx, const OSSL_PARAM params[]);
 const OSSL_PARAM *{keymgmt_gen_settable_params}(void *provctx);
 void *{keymgmt_gen}(void *genctx, OSSL_CALLBACK *cb, void *cbarg);
 void {keymgmt_gen_cleanup}(void *genctx);

 /* Key object information */
 int {keymgmt_get_params}(void *keydata, OSSL_PARAM params[]);
 const OSSL_PARAM *{keymgmt_gettable_params}(void);
 int {keymgmt_set_params}(void *keydata, const OSSL_PARAM params[]);
 const OSSL_PARAM *{keymgmt_settable_params}(void);

 /* Key object content checks */
 int {keymgmt_has}(void *keydata, int selection);
 int {keymgmt_match}(const void *keydata1, const void *keydata2,
                             int selection);

 /* Discovery of supported operations */
 const char *{keymgmt_query_operation_name}(int operation_id);

 /* Key object import and export functions */
 int {keymgmt_import}(int selection, void *keydata, const OSSL_PARAM params[]);
 const OSSL_PARAM *{keymgmt_import_types}(int selection);
 int {keymgmt_export}(int selection, void *keydata,
                              OSSL_CALLBACK *param_cb, void *cbarg);
 const OSSL_PARAM *{keymgmt_export_types}(int selection);

 /* Key object copy */
 int {keymgmt_copy}(void *keydata_to, const void *keydata_from, int selection);

 /* Key object validation */
 int {keymgmt_validate}(void *keydata, int selection);
...
B<OSSL_FUNC_{name}>.
For example, the "function" {keymgmt_new}() has these:

 typedef void *({keymgmt_new_fn})(void *provctx);
 static ossl_inline {keymgmt_new_fn}
     {keymgmt_new}(const OSSL_DISPATCH *opf);

B<OSSL_DISPATCH> arrays are indexed by numbers that are provided as
macros in L<openssl-core_dispatch.h(7)>, as follows:

 {keymgmt_new}                  OSSL_FUNC_KEYMGMT_NEW
 {keymgmt_free}                 OSSL_FUNC_KEYMGMT_FREE

 {keymgmt_gen_init}             OSSL_FUNC_KEYMGMT_GEN_INIT
 {keymgmt_gen_set_template}     OSSL_FUNC_KEYMGMT_GEN_SET_TEMPLATE
 {keymgmt_gen_set_params}       OSSL_FUNC_KEYMGMT_GEN_SET_PARAMS
 {keymgmt_gen_settable_params}  OSSL_FUNC_KEYMGMT_GEN_SETTABLE_PARAMS
 {keymgmt_gen}                  OSSL_FUNC_KEYMGMT_GEN
 {keymgmt_gen_cleanup}          OSSL_FUNC_KEYMGMT_GEN_CLEANUP

 {keymgmt_get_params}           OSSL_FUNC_KEYMGMT_GET_PARAMS
 {keymgmt_gettable_params}      OSSL_FUNC_KEYMGMT_GETTABLE_PARAMS
 {keymgmt_set_params}           OSSL_FUNC_KEYMGMT_SET_PARAMS
 {keymgmt_settable_params}      OSSL_FUNC_KEYMGMT_SETTABLE_PARAMS

 {keymgmt_query_operation_name} OSSL_FUNC_KEYMGMT_QUERY_OPERATION_NAME

 {keymgmt_has}                  OSSL_FUNC_KEYMGMT_HAS
 {keymgmt_validate}             OSSL_FUNC_KEYMGMT_VALIDATE
 {keymgmt_match}                OSSL_FUNC_KEYMGMT_MATCH

 {keymgmt_import}               OSSL_FUNC_KEYMGMT_IMPORT
 {keymgmt_import_types}         OSSL_FUNC_KEYMGMT_IMPORT_TYPES
 {keymgmt_export}               OSSL_FUNC_KEYMGMT_EXPORT
 {keymgmt_export_types}         OSSL_FUNC_KEYMGMT_EXPORT_TYPES

 {keymgmt_copy}                 OSSL_FUNC_KEYMGMT_COPY
...
{keymgmt_new}() should create a provider side key object.  The
provider context I<provctx> is passed and may be incorporated in the
key object, but that is not mandatory.

{keymgmt_free}() should free the passed I<keydata>.

{keymgmt_gen_init}(), {keymgmt_gen_set_template}(),
{keymgmt_gen_set_params}(), {keymgmt_gen_settable_params}(),
{keymgmt_gen}() and {keymgmt_gen_cleanup}() work together as a more
elaborate context based key object constructor.

{keymgmt_gen_init}() should create the key object generation context
and initialize it with I<selections>, which will determine what kind
of contents the key object to be generated should get.

{keymgmt_gen_set_template}() should add I<template> to the context
...
{keymgmt_gen_set_params}() should set additional parameters from
I<params> in the key object generation context I<genctx>.

{keymgmt_gen_settable_params}() should return a constant array of
descriptor B<OSSL_PARAM>, for parameters that {keymgmt_gen_set_params}() 
can handle.

{keymgmt_gen}() should perform the key object generation itself, and
...
{keymgmt_gen_cleanup}() should clean up and free the key object
generation context I<genctx>

At least one of {keymgmt_new}() and {keymgmt_gen}() are mandatory,
as well as {keymgmt_free}().  Additionally, if {keymgmt_gen}() is
present, {keymgmt_gen_init}() and {keymgmt_gen_cleanup}() must be
present as well.

=head2 Key Object Information Functions

{keymgmt_get_params}() should extract information data associated
with the given I<keydata>, see L</Common Information Parameters>.

{keymgmt_gettable_params}() should return a constant array of
descriptor B<OSSL_PARAM>, for parameters that {keymgmt_get_params}()
can handle.

If {keymgmt_gettable_params}() is present, {keymgmt_get_params}()
must also be present, and vice versa.

{keymgmt_set_params}() should update information data associated
with the given I<keydata>, see L</Common Information Parameters>.

{keymgmt_settable_params}() should return a constant array of
descriptor B<OSSL_PARAM>, for parameters that {keymgmt_set_params}()
can handle.

If {keymgmt_settable_params}() is present, {keymgmt_set_params}()
must also be present, and vice versa.

=head2 Key Object Checking Functions

{keymgmt_query_operation_name}() should return the name of the
...
{keymgmt_has}() should check whether the given I<keydata> contains the subsets
...
{keymgmt_validate}() should check if the I<keydata> contains valid
...
{keymgmt_match}() should check if the data subset indicated by
...
{keymgmt_import}() should import data indicated by I<selection> into
I<keydata> with values taken from the B<OSSL_PARAM> array I<params>.

{keymgmt_export}() should extract values indicated by I<selection>
from I<keydata>, create an B<OSSL_PARAM> array with them and call
I<param_cb> with that array as well as the given I<cbarg>.

{keymgmt_import_types}() should return a constant array of descriptor
B<OSSL_PARAM> for data indicated by I<selection>, for parameters that
{keymgmt_import}() can handle.

{keymgmt_export_types}() should return a constant array of descriptor
B<OSSL_PARAM> for data indicated by I<selection>, that the
{keymgmt_export}() callback can expect to receive.

{keymgmt_copy}() should copy data subsets indicated by I<selection>
...
{keymgmt_new}() should return a valid reference to the newly created provider
side key object, or NULL on failure.

{keymgmt_import}(), {keymgmt_export}(), {keymgmt_get_params}() and
{keymgmt_set_params}() should return 1 for success or 0 on error.

{keymgmt_validate}() should return 1 on successful validation, or 0 on
failure.

{keymgmt_has}() should return 1 if all the selected data subsets are contained
in the given I<keydata> or 0 otherwise.

{keymgmt_query_operation_name}() should return a pointer to a string matching
the requested operation, or NULL if the same name used to fetch the keymgmt
applies.

{keymgmt_gettable_params}() and {keymgmt_settable_params}()
{keymgmt_import_types}(), {keymgmt_export_types}()
should always return a constant B<OSSL_PARAM> array.
...
The KEYMGMT provider interface was introduced in OpenSSL 3.0.
