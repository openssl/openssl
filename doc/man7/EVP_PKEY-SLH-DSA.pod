...
The public key sizes are relatively small values of 32, 48 or 64 bytes that
correspond to the algorithm names of 128, 192 and 256 respectively.

The algorithms ending with B<s> produce smaller signatures, but are much slower
that the faster B<f> variants.

The signature sizes for the B<s> algorithm variants are 7856, 16224 and 29792
which correspond to the algorithm names of 128s, 192s and 256s.
...
Internally there are 7 functions that are used for each algorithms.or the
...
The private key value of size 4 * B<n>, which includes the public key components.
