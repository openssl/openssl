...
Errors are not handled in the following example.

This example generates a MAC in 'out' of size 'out_len' using KMAC-128 using
inputs for the 'key', 'in' and 'custom' string. If 'out_len' is not known
before the final() is called 'xof_enabled' should be set to 1.
                     ...
                     int xof_enabled, unsigned char *out, int out_len)
  {
      ...
      *p++ = OSSL_PARAM_construct_octet_string(OSSL_MAC_PARAM_CUSTOM,
      ...
      /* Optionally set xof_enabled to 1 if the output length is not known */
      *p++ = OSSL_PARAM_construct_int(OSSL_MAC_PARAM_XOF, &xof_enabled);
      /* Set the out_len if it is not the default size, or use EVP_MAC_finalXOF */
      ...
      *p = OSSL_PARAM_construct_end();
      ...
      /*
       * At this point you can use either EVP_MAC_final(), EVP_MAC_finalXOF()
       * or one or more calls to EVP_MAC_squeeze().
       */
