...
- OpenSSL Guide: Writing a simple non-blocking QUIC server
...
Both the accepting socket and client connections are "non-blocking".  However,
...
The complete source code for this example non-blocking QUIC server is available
...
select an ALPN the server considers acceptable.  In this case, we only accept
"http/1.0" and "hq-interop".
...
To run the QUIC server, we create a B<SSL_LISTENER> to listen for incoming
...
L<ossl-guide-quic-server-block(7)>,
