...
initialization (see L<provider(7)>).
...
OSSL_FUNC_asym_cipher_encrypt_init() initializes a context for an asymmetric encryption
...
A previously initialized asymmetric cipher context is passed in the I<ctx>
parameter.
...
OSSL_FUNC_asym_cipher_decrypt_init() initializes a context for an asymmetric decryption
given a provider side asymmetric cipher context in the I<ctx> parameter, and a
pointer to a provider key object in the I<provkey> parameter.
The key object should have been previously generated, loaded or imported into
the provider using the key management (OSSL_OP_KEYMGMT) operation (see
...
Parameters currently recognized by built-in asymmetric cipher algorithms are as
follows.
