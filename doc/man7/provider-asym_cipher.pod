...
provider-asym_cipher - The ASYM_CIPHER provider dispatch interface
...
=for openssl multiple includes
 ...
 /* Context management */
 void *{asym_cipher_newctx}(void *provctx);
 void {asym_cipher_freectx}(void *ctx);
 void *{asym_cipher_dupctx}(void *ctx);

 /* Encryption */
 int {asym_cipher_encrypt_init}(void *ctx, void *provkey);
 int {asym_cipher_encrypt}(void *ctx, unsigned char *out, size_t *outlen,
                                   size_t outsize, const unsigned char *in,
                                   size_t inlen);

 /* Decryption */
 int {asym_cipher_decrypt_init}(void *ctx, void *provkey);
 int {asym_cipher_decrypt}(void *ctx, unsigned char *out, size_t *outlen,
                                   size_t outsize, const unsigned char *in,
                                   size_t inlen);

 /* Asymmetric Cipher parameters */
 int {asym_cipher_get_ctx_params}(void *ctx, OSSL_PARAM params[]);
 const OSSL_PARAM *{asym_cipher_gettable_ctx_params}(void);
 int {asym_cipher_set_ctx_params}(void *ctx, const OSSL_PARAM params[]);
 const OSSL_PARAM *{asym_cipher_settable_ctx_params}(void);
...
B<OSSL_FUNC_{name}>.
For example, the "function" {asym_cipher_newctx}() has these:

 typedef void *({asym_cipher_newctx_fn})(void *provctx);
 static ossl_inline {asym_cipher_newctx_fn}
     {asym_cipher_newctx}(const OSSL_DISPATCH *opf);

B<OSSL_DISPATCH> arrays are indexed by numbers that are provided as
macros in L<openssl-core_dispatch.h(7)>, as follows:

 {asym_cipher_newctx}               OSSL_FUNC_ASYM_CIPHER_NEWCTX
 {asym_cipher_freectx}              OSSL_FUNC_ASYM_CIPHER_FREECTX
 {asym_cipher_dupctx}               OSSL_FUNC_ASYM_CIPHER_DUPCTX

 {asym_cipher_encrypt_init}         OSSL_FUNC_ASYM_CIPHER_ENCRYPT_INIT
 {asym_cipher_encrypt}              OSSL_FUNC_ASYM_CIPHER_ENCRYPT

 {asym_cipher_decrypt_init}         OSSL_FUNC_ASYM_CIPHER_DECRYPT_INIT
 {asym_cipher_decrypt}              OSSL_FUNC_ASYM_CIPHER_DECRYPT

 {asym_cipher_get_ctx_params}       OSSL_FUNC_ASYM_CIPHER_GET_CTX_PARAMS
 {asym_cipher_gettable_ctx_params}  OSSL_FUNC_ASYM_CIPHER_GETTABLE_CTX_PARAMS
 {asym_cipher_set_ctx_params}       OSSL_FUNC_ASYM_CIPHER_SET_CTX_PARAMS
 {asym_cipher_settable_ctx_params}  OSSL_FUNC_ASYM_CIPHER_SETTABLE_CTX_PARAMS

An asymmetric cipher algorithm implementation may not implement all of these
functions.
In order to be a consistent set of functions a provider must implement
{asym_cipher_newctx} and {asym_cipher_freectx}.
It must also implement both of {asym_cipher_encrypt_init} and
{asym_cipher_encrypt}, or both of {asym_cipher_decrypt_init} and
{asym_cipher_decrypt}.
{asym_cipher_get_ctx_params} is optional but if it is present then so must
{asym_cipher_gettable_ctx_params}.
Similarly, {asym_cipher_set_ctx_params} is optional but if it is present then
so must {asym_cipher_settable_ctx_params}.
...
{asym_cipher_newctx}() should create and return a pointer to a provider side
...
{asym_cipher_freectx}() is passed a pointer to the provider side asymmetric
cipher context in the I<ctx> parameter.
This function should free any resources associated with that context.

{asym_cipher_dupctx}() should duplicate the provider side asymmetric cipher
context in the I<ctx> parameter and return the duplicate copy.

=head2 Encryption Functions

{asym_cipher_encrypt_init}() initialises a context for an asymmetric encryption
...
{asym_cipher_encrypt}() performs the actual encryption itself.
...
{asym_cipher_decrypt_init}() initialises a context for an asymmetric decryption
given a provider side asymmetric cipher context in the I<ctx> parameter, and a
pointer to a provider key object in the I<provkey> parameter.
The key object should have been previously generated, loaded or imported into
the provider using the key management (OSSL_OP_KEYMGMT) operation (see
...
{asym_cipher_decrypt}() performs the actual decryption itself.
A previously initialised asymmetric cipher context is passed in the I<ctx>
parameter.
...
the {asym_cipher_get_ctx_params}() and {asym_cipher_set_ctx_params}()
functions.

{asym_cipher_get_ctx_params}() gets asymmetric cipher parameters associated
with the given provider side asymmetric cipher context I<ctx> and stores them in
I<params>.
{asym_cipher_set_ctx_params}() sets the asymmetric cipher parameters associated
...
{asym_cipher_gettable_ctx_params}() and {asym_cipher_settable_ctx_params}()
get a constant B<OSSL_PARAM> array that describes the gettable and settable
parameters, i.e. parameters that can be used with {asym_cipherget_ctx_params}()
and {asym_cipher_set_ctx_params}() respectively.
See L<OSSL_PARAM(3)> for the use of B<OSSL_PARAM> as parameter descriptor.

=head1 RETURN VALUES

{asym_cipher_newctx}() and {asym_cipher_dupctx}() should return the newly
