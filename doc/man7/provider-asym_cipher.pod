...
=for opentls multiple includes
 ...
 int OP_asym_cipher_get_ctx_params(void *ctx, Otls_PARAM params[]);
 const Otls_PARAM *OP_asym_cipher_gettable_ctx_params(void);
 int OP_asym_cipher_set_ctx_params(void *ctx, const Otls_PARAM params[]);
 const Otls_PARAM *OP_asym_cipher_settable_ctx_params(void);
...
The asymmetric cipher (Otls_OP_ASYM_CIPHER) operation enables providers to
...
For example, the "function" OP_asym_cipher_newctx() has these:
 ...
 typedef void *(Otls_OP_asym_cipher_newctx_fn)(void *provctx);
 static otls_inline Otls_OP_asym_cipher_newctx_fn
     Otls_get_OP_asym_cipher_newctx(const Otls_DISPATCH *opf);

B<Otls_DISPATCH> arrays are indexed by numbers that are provided as
macros in L<opentls-core_numbers.h(7)>, as follows:

 OP_asym_cipher_newctx               Otls_FUNC_ASYM_CIPHER_NEWCTX
 OP_asym_cipher_freectx              Otls_FUNC_ASYM_CIPHER_FREECTX
 OP_asym_cipher_dupctx               Otls_FUNC_ASYM_CIPHER_DUPCTX

 OP_asym_cipher_encrypt_init         Otls_FUNC_ASYM_CIPHER_ENCRYPT_INIT
 OP_asym_cipher_encrypt              Otls_FUNC_ASYM_CIPHER_ENCRYPT

 OP_asym_cipher_decrypt_init         Otls_FUNC_ASYM_CIPHER_DECRYPT_INIT
 OP_asym_cipher_decrypt              Otls_FUNC_ASYM_CIPHER_DECRYPT

 OP_asym_cipher_get_ctx_params       Otls_FUNC_ASYM_CIPHER_GET_CTX_PARAMS
 OP_asym_cipher_gettable_ctx_params  Otls_FUNC_ASYM_CIPHER_GETTABLE_CTX_PARAMS
 OP_asym_cipher_set_ctx_params       Otls_FUNC_ASYM_CIPHER_SET_CTX_PARAMS
 OP_asym_cipher_settable_ctx_params  Otls_FUNC_ASYM_CIPHER_SETTABLE_CTX_PARAMS
...
=item "pad-mode" (B<Otls_ASYM_CIPHER_PARAM_PAD_MODE>) <integer>
...
modes: 1 (RSA_PKCS1_PADDING), 2 (RSA_tlsV23_PADDING), 3 (RSA_NO_PADDING),
...
=item "digest" (B<Otls_ASYM_CIPHER_PARAM_OAEP_DIGEST>) <UTF8 string>

Gets or sets the name of the OAEP digest algorithm used when OAEP padding is in
use.

=item "digest-props" (B<Otls_ASYM_CIPHER_PARAM_OAEP_DIGEST_PROPS>) <UTF8 string>

Gets or sets the properties to use when fetching the OAEP digest algorithm.

=item "mgf1-digest" (B<Otls_ASYM_CIPHER_PARAM_MGF1_DIGEST>) <UTF8 string>

Gets or sets the name of the MGF1 digest algorithm used when OAEP or PSS padding
is in use.

=item "mgf1-digest-props" (B<Otls_ASYM_CIPHER_PARAM_MGF1_DIGEST_PROPS>) <UTF8 string>

Gets or sets the properties to use when fetching the MGF1 digest algorithm.

=item "oaep-label" (B<Otls_ASYM_CIPHER_PARAM_OAEP_LABEL>) <octet string>

Gets or sets the OAEP label used when OAEP padding is in use.

=item "oaep-label-len" (B<Otls_ASYM_CIPHER_PARAM_OAEP_LABEL_LEN>) <size_t>

Gets the length of an OAEP label when OAEP padding is in use.

=item "tls-client-version" (B<Otls_ASYM_CIPHER_PARAM_TLS_CLIENT_VERSION>) <unsigned integer>
...
=item "tls-negotiated-version" (B<Otls_ASYM_CIPHER_PARAM_TLS_CLIENT_VERSION>) <unsigned integer>
...
get a constant B<Otls_PARAM> array that describes the gettable and settable
...
The provider ASYM_CIPHER interface was introduced in Opentls 3.0.
