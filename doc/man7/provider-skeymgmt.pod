...
=head1 SYNOPSIS
 ...
 void *OSSL_FUNC_skeymgmt_import(const OSSL_PARAM params[]);
 int OSSL_FUNC_skeymgmt_export(void *keydata, int selection,
                              OSSL_CALLBACK *param_cb, void *cbarg);

=head1 DESCRIPTION

The SKEYMGMT operation doesn't have much public visibility in OpenSSL
libraries, it's rather an internal operation that's designed to work
with operations that use opaque symmetric keys ojects.

Because the SKEYMGMT operation shares knowledge with the operations it
...
The SKEYMGMT management is inspired by KEYMGMT but is much simpler.
...
The selector bits have also been combined for easier use:
...
OSSL_FUNC_skeymgmt_import() returns a pointer to an allocated object on success
and NULL on error.  OSSL_FUNC_skeymgmt_export() returns 1 for success or 0 on
error.
