...
provider-keyexch - The KEYEXCH provider dispatch interface
...
=for openssl multiple includes
 ...
 int {keyexch_init}(void *ctx, void *provkey);
 int {keyexch_set_peer}(void *ctx, void *provkey);
 int {keyexch_derive}(void *ctx, unsigned char *secret, size_t *secretlen,
                      size_t outlen);

 /* Key Exchange parameters */
 int {keyexch_set_ctx_params}(void *ctx, const OSSL_PARAM params[]);
 const OSSL_PARAM *{keyexch_settable_ctx_params}(void);
 int {keyexch_get_ctx_params}(void *ctx, OSSL_PARAM params[]);
 const OSSL_PARAM *{keyexch_gettable_ctx_params}(void);
...
L<EVP_PKEY_derive(3)> (and other related functions).
...
macros in L<openssl-core_dispatch.h(7)>, as follows:
 ...
 {keyexch_newctx}                OSSL_FUNC_KEYEXCH_NEWCTX
 {keyexch_freectx}               OSSL_FUNC_KEYEXCH_FREECTX
 {keyexch_dupctx}                OSSL_FUNC_KEYEXCH_DUPCTX

 {keyexch_init}                  OSSL_FUNC_KEYEXCH_INIT
 {keyexch_set_peer}              OSSL_FUNC_KEYEXCH_SET_PEER
 {keyexch_derive}                OSSL_FUNC_KEYEXCH_DERIVE

 {keyexch_set_ctx_params}        OSSL_FUNC_KEYEXCH_SET_CTX_PARAMS
 {keyexch_settable_ctx_params}   OSSL_FUNC_KEYEXCH_SETTABLE_CTX_PARAMS
 {keyexch_get_ctx_params}        OSSL_FUNC_KEYEXCH_GET_CTX_PARAMS
 {keyexch_gettable_ctx_params}   OSSL_FUNC_KEYEXCH_GETTABLE_CTX_PARAMS

A key exchange algorithm implementation may not implement all of these functions.
In order to be a consistent set of functions a provider must implement
{keyexch_newctx}, {keyexch_freectx}, {keyexch_init} and {keyexch_derive}.
...
{keyexch_newctx}() should create and return a pointer to a provider side
...
{keyexch_freectx}() is passed a pointer to the provider side key exchange
context in the I<ctx> parameter.
This function should free any resources associated with that context.

{keyexch_dupctx}() should duplicate the provider side key exchange context in
the I<ctx> parameter and return the duplicate copy.

=head2 Shared Secret Derivation Functions

{keyexch_init}() initialises a key exchange operation given a provider side key
...
{keyexch_set_peer}() is called to supply the peer's public key (in the
...
{keyexch_derive}() performs the actual key exchange itself by deriving a shared
secret.
...
{keyexch_set_ctx_params}() sets key exchange parameters associated with the
given provider side key exchange context I<ctx> to I<params>,
see L</Common Key Exchange parameters>.
Any parameter settings are additional to any that were previously set.

{keyexch_get_ctx_params}() gets key exchange parameters associated with the
given provider side key exchange context I<ctx> into I<params>,
see L</Common Key Exchange parameters>.

{keyexch_settable_ctx_params}() yields a constant B<OSSL_PARAM> array that
describes the settable parameters, i.e. parameters that can be used with
OP_signature_set_ctx_params().
If {keyexch_settable_ctx_params}() is present, {keyexch_set_ctx_params}() must
also be present, and vice versa.
Similarly, {keyexch_gettable_ctx_params}() yields a constant B<OSSL_PARAM>
array that describes the gettable parameters, i.e. parameters that can be
handled by OP_signature_get_ctx_params().
If {keyexch_gettable_ctx_params}() is present, {keyexch_get_ctx_params}() must
also be present, and vice versa.
...
the {keyexch_set_ctx_params}() and {keyexch_get_ctx_params}() functions.
...
{keyexch_newctx}() and {keyexch_dupctx}() should return the newly created
provider side key exchange context, or NULL on failure.

{keyexch_init}(), {keyexch_set_peer}(), {keyexch_derive}(),
{keyexch_set_params}(), and {keyexch_get_params}() should return 1 for success
or 0 on error.

{keyexch_settable_ctx_params}() and {keyexch_gettable_ctx_params}() should
