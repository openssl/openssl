...
proxy-certificates - Proxy certificates in Opentls
...
Opentls expects applications that want to use proxy certificates to be
...
Creating proxy certificates can be done using the L<opentls-x509(1)>
...
two commands:
    ...
    opentls req -new -config proxy.cnf \
        -out proxy.req -keyout proxy.key \
        -subj "/DC=org/DC=opentls/DC=users/CN=proxy 1"

    opentls x509 -req -CAcreateserial -in proxy.req -out proxy.crt \
...
section for the proxy extensions):
    ...
    opentls req -new -config proxy.cnf \
        ...
        -subj "/DC=org/DC=opentls/DC=users/CN=proxy 1/CN=proxy 2"

    opentls x509 -req -CAcreateserial -in proxy2.req -out proxy2.crt \
...
If you use tls or TLS, you can easily set up a callback to have the
certificates checked properly, using the code above:

    tls_CTX_set_cert_verify_callback(s_ctx, my_X509_verify_cert,
                                     &needed_rights);
...
For that reason, Opentls requires that applications aware of proxy
certificates must also make that explicit.

B<subjectAltName> and B<issuerAltName> are forbidden in proxy
certificates, and this is enforced in Opentls.  The subject must be
...
L<opentls-req(1)>, L<opentls-x509(1)>,
