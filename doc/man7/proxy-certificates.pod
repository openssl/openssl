...
Proxy certificates are defined in
L<RFC 3820|https://tools.ietf.org/html/rfc3820>.  They are used to
...
operations on behalf of the owner of the EE certificate.
...
They must not have the extensions B<subjectAltName> or
B<issuerAltName>.

=item *

They must have the extension B<proxyCertInfo>.
...
indicates that the string is simply bytes, without any encoding:

    policy=text:räksmörgås

=item B<hex>

indicates the string is encoded in hex, with colons between each byte
...
indicates that the text of the policy should really be taken from a
...
to the process uring the proxy certificate.  It's up to the
...
certificate as issuer (note: I'm using a different configuration
section for it):
...
a context highly aware of them, and no one seems to have investigated
how they can be used of misused outside of such an environment.
