...
as an X509 certificate). The certificate contains various information about the
...
=head1 Important objects for a TLS client application
...
OpenSSL provides a helper wrapper function to do this which also integrates into
the OpenSSL error system to log error data, e.g.
...
See L<BIO_socket(3)> for further information on this function.
...
OpenSSL also provides wrapper functions that can be used to assist with this
too:
...
See L<BIO_lookup_ex(3)>, L<BIO_connect(3)>, L<BIO_closesocket(3)>,
L<BIO_ADDRINFO_address(3)> and L<BIO_ADDRINFO_free(3)> for further information
on the functions used here. In the above example code the B<hostname> and
B<port> variables are strings, e.g. "www.example.com" and "443".
...
We also need to tell OpenSSL what hostname we expect to see in the certificate
coming back from the server. This is almost always the same one that we asked
for in the original request. This is important because, without this, we do not
verify that the hostname in the certificate is what we expect it to be and any
certificate is acceptable unless your application explicitly checks this itself.
We do this via the L<SSL_set1_host(3)> function:
...
=head1 Sending and receiving data
...
=head1 Shuting down the connection
