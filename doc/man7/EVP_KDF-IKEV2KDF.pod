...
the shared secret established during the Diffie-Hellman exchange.
...
This parameter sets the shared secret (g^ir) used for generating SEEDKEY,
or the new shared secret (new g^ir) used for deriving DKM(Child_SA) or REKEY.
The shared secret is represented as a string of octets in big endian order,
padded with zeros if necessary to make it the length of the modulus.
If a value is already set, the contents are replaced.
...
It generates SKEYSEED used in deriving key material

=item EVP_KDF_IKEV2_MODE_DKM

It has 3 stages, based on the input parameter sets.

It first performs initial key derivation - derives the initial set of keys
