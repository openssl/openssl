...
EVP_KDF-SRTP - The SRTP EVP_KDF implementation
...
=item "kdr" (B<OSSL_KDF_PARAM_SRTPKDF_KDR>) <octet string>
...
rederivation is performed. The KDR value is typically 6 bytes.
...
=item "label" (B<OSSL_KDF_PARAM_SRTPKDF_LABEL>) <integer>
 ...
 *p++ = OSSL_PARAM_construct_int(OSSL_KDF_PARAM_SRTPKDF_LABEL, &label);
...
rate and index:
 ...
 unsigned char master_key[16] = { /* master key bytes */ };
 unsigned char master_salt[14] = { /* master salt bytes */ };
 unsigned char kdr[6] = { 0x00, 0x00, 0x00, 0x00, 0x10, 0x00 }; /* KDR */
 ...
 *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT,
                                          master_salt, sizeof(master_salt));
 *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SRTPKDF_KDR,
                                          kdr, sizeof(kdr));
...
The SRTP KDF was added in OpenSSL 3.6.
