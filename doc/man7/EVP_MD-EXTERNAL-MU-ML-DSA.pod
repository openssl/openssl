...
EVP_MD-EXTERNAL-MU-ML-DSA - The ExternalMu-ML-DSA.Prehash() EVP_MD implementation

=head1 DESCRIPTION

Support for computing ExternalMu-ML-DSA.Prehash() through the B<EVP_MD> API.

Normally the value of "mu" is calculated internally as part of a ML-DSA
sign or verify operation. Mu is defined as:
    mu = SHAKE256(tr || M', 64)

Where tr is the hash of the encoded public key, and for Pure (ML-DSA)
    M' = 00 || ctx_len || ctx || in (where in = message)

In cases where prehashing the message is required, FIPS 204 allows
the mu calculation to be done externally and then mu can be passed to
ML-DSA sign or verify operations.

PreHash (Hash-ML-DSA) is also supported and uses
    M' = 01 || ctx_len || ctx || OID || in (where in = hashed(msg))

The output 'mu' value can then be supplied as an input to L<EVP_SIGNATURE-ML-DSA(7)>
using the ML-DSA Signature Parameter "mu".
...
default provider, and is identified with the name "ExternalMu-ML-DSA".

=head2 Parameters

This implementation supports the following settable L<OSSL_PARAM(3)> entries:
...
This option MUST be set or an error will occur.
...
An optional parameter related to "Hash-ML-DSA" that is used to add an OID to M'.
By default it is not used.
The HASH-ML-DSA variant is only supplied for interoperability purposes, since it
is not used in protocols such as X509 & CMS (See RFC 9981 and 9982).

OpenSSL accepts the following options "SHAKE-256", "SHAKE-128", "SHA-224",
"SHA-256", "SHA-384", "SHA-512", "SHA3-224", "SHA3-256", "SHA3-384" and "SHA3-512".
For SHAKE-128 and SHAKE-256 the expected XOF lengths are 32 and 64 respectively.
...
To generate external 'mu' given an existing ML-DSA key and a large message:
    ...
    calculate_mu(EVP_PKEY *pkey, unsigned char *msg, size_t msglen,
                 unsigned char *ctx, size_t ctxlen)
    {
        unsigned char pub[2592], mu[64];
        ...
        *p = OSSL_PARAM_construct_end();
        ...
        md = EVP_MD_fetch(libctx, "ExternalMu-ML-DSA", NULL);
        ...
        EVP_DigestFinalXOF(mdctx, mu, mulen)
    }
...
EVP_MD-EXTERNAL-MU-ML-DSA was added in OpenSSL 4.0.0.
