...
An optional parameter related to "HashML-DSA". If used it determines the OID in
...
The HashML-DSA variant is available to enable specialised use-cases,
in which signing the full message with pure ML-DSA is not practical, and
the external-mu API is a viable alternative.
HashML-DSA is not used in protocols such as X509 & CMS (See RFC 9981 and 9982),
