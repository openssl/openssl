...
global DRBG instances, which are accessed concurrently by all threads.
The necessary locking is done automatically by the RAND methods, e.g.,
by L<RAND_bytes(3)> and L<RAND_priv_bytes(3)>.

Currently, there are three DRBG instances,
...
the RAND_DRBG interface is I<not> thread-safe.

All other DRBG instances don't support locking, because they are
intended to be used by a single thread. Instead of accessing a single
...
DRBG (preferably the public DRBG) as parent of DRBG instances on
...
together and are being used.
  ...
  RAND_add() ==> <master>        \   +-----------------------------+
            ...
            <public>  <private>  /   +-----------------------------+
                ...
                |               |
                |              +------------------------------------+
                |              | Used only for long-term secrets    |
                |              | like private keys for certificates |
                |              +------------------------------------+
                |
                + ==> RAND_bytes()
                |      ^
                ...
                |     +----------------------------------+
                v
              <ssl->drbg> ==> RAND_DRBG_bytes(ssl->drbg, ...)
                               ^
                               |
                              +---------------------------------+
                              | Every SSL context has its own   |
                              | private, unlocked DRBG instance |
                              +---------------------------------+
...
When configuring OpenSSL, the seed source can specified using the
following option.

   --with-rand-seed=seed1[,seed2,...]

     os:          Use a trusted operating system entropy source (default)
     getrandom:   Use the L<getrandom(2)> if available.
     devrandom:   Use the /dev/urandom device.
     egd:         Check for an entropy generating daemon.
     rdcpu:       Use the RDSEED or RDRAND command if provided by the CPU.
     librandom:   Use librandom (not implemented yet).

Multiple seed sources can be specified in form of a comma separated list.
Not all options are available on all platforms.
The default option is --with-rand-seed=os, which is in general the best
choice for the given operating system. So in general, it is not necessary
to specify this option explicitly.

Automatic seeding can be disabled entirely using the --with-rand-seed=none
option. Some platforms have it disabled by default, because no suitable
random seed source is available.

The following two sections describe the reseeding process of the master
DRBG, depending on whether automatic reseeding is configured or not.

For more details about the configuration process, see the INSTALLATION
instructions .
...
At least 32 bytes (256 bits) of randomness have to be provided, otherwise
