...
by the other two. The necessary locking is done internally.
...
The <public> and <private> DRBG are thread-local, so they can safely
be accessed via the RAND_DRBG interface.
