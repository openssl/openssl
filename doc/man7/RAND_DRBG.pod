...
If entropy sources are permanently unavailable, there is no way for DRBG
to recover. Special care has to be taken by applications when I<chroot()>ing
into an environment without I</dev/[urandom|srandom|random]> and when
I<getrandom()> or equivalent syscalls are not available: as the initial
seed may happen before I<chroot()>, the application will initially work, but
when either I<reseed_interval> or I<reseed_time_interval> trigger, DRBG
reseeding will permanently fail. An application may catch this scenario
early by checking the return code off I<RAND_poll()> after I<chroot()>ing.
