...
The DRBG instance which acts as entropy source is called the I<primary> DRBG,
the other instance is the I<child> DRBG.

This is called chaining. A chained DRBG instance is created by passing
a pointer to the primary DRBG as argument to the RAND_DRBG_new() call.
...
the I<primary>, I<public>, and I<private> DRBG.
While the I<primary> DRBG is a single global instance,
the I<public> and I<private>
...
the thread-local I<public> and I<private> DRBG instance, respectively.

=head2 The I<primary> DRBG instance

The I<primary> DRBG is not used directly by the application, only for reseeding
...
=head2 The I<public> DRBG instance

This instance is used per default by L<RAND_bytes(3)>.

=head2 The I<private> DRBG instance

This instance is used per default by L<RAND_priv_bytes(3)>

=head1 LOCKING

The I<primary> DRBG is intended to be accessed concurrently for reseeding
by its child DRBG instances. The necessary locking is done internally.
It is I<not> thread-safe to access the I<primary> DRBG directly via the
RAND_DRBG interface.
The I<public> and I<private> DRBG are thread-local, i.e. there is an
...
thread. Using the I<primary> DRBG as entropy source for multiple DRBG
instances on different threads is thread-safe, because the DRBG instance
will lock the I<primary> DRBG automatically for obtaining random input.
...
RAND_DRBG_bytes(I<public>, ...) and RAND_DRBG_bytes(I<private>, ...),
...
(or L<RAND_seed(3)>), then this will immediately reseed the I<primary> DRBG.
The I<public> and I<private> DRBG will detect this on their next generate
call and reseed, pulling randomness from I<primary>.
...
for automatically seeding and reseeding its I<primary> DRBG. In some cases
