...
This may be used after calling either EVP_PKEY_encapsulate() or
EVP_PKEY_decapsulate(). It may return 0 if "key-check" is set to 0.
This option is used by the OpenSSL FIPS provider.

=item "key-check" (B<OSSL_ASYM_CIPHER_PARAM_FIPS_KEY_CHECK>) <int>

If required this parameter should be set using either EVP_PKEY_encapsulate_init()
or EVP_PKEY_decapsulate_init().
