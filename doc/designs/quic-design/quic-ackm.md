...
lifetime of a `QUIC_ACKM_TX_PKT`, and only once.

Returns 1 on success.

```c
typedef struct  quic_ackm_tx_pkt_st {
...
int QUIC_ACKM_on_tx_ack_packet(QUIC_ACKM *ackm, const QUIC_ACKM_TX_PKT *pkt);
```
...
separately in the subsequent calls to `QUIC_ACKM_on_rx_packet` and
`QUIC_ACKM_on_rx_ack_frame`, respectively.

Returns 1 on success.

```c
int QUIC_ACKM_on_rx_datagram(QUIC_ACKM *ackm, size_t num_bytes);
```

### On RX Packet

This must be called whenever a packet is received. It should be called after
`QUIC_ACKM_on_rx_datagram` was called for the datagram containing the packet.
...
after any call to `QUIC_ACKM_on_rx_packet`.
...
`QUIC_ACKM_TX_PKT` structures have their `on_discarded` callback invoked,
...
```c
int QUIC_ACKM_on_handshake_confirmed(QUIC_ACKM *ackm);
```
...
```c
int QUIC_ACKM_on_timeout(QUIC_ACKM *ackm);
```
...
This returns a deadline after which `QUIC_ACKM_on_timeout` should be called.
...
```c
OSSL_TIME QUIC_ACKM_get_loss_detection_timeout(QUIC_ACKM *ackm);
```
...
The user calls `QUIC_ACKM_get_probe_request`. The structure pointed
to by `info` is filled and the function returns 1 on success.

The fields of `QUIC_ACKM_PROBE_INFO` record the number of probe requests
...
outstanding requests by calling `QUIC_ACKM_get_probe_request` with `clear` set
to 1. If `clear` is non-zero, the current values are returned and then zeroed,
so that the next call to `QUIC_ACKM_get_probe_request` (if made immediately)
...
```c
int QUIC_ACKM_get_largest_unacked(QUIC_ACKM *ackm, int pkt_space, QUIC_PN *pn);
```
