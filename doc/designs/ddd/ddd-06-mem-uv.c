...
APP_CONN *new_conn(SSL_CTX *ctx, const char *hostname, struct sockaddr *sa,
		   socklen_t sa_len, app_connect_cb *cb, void *arg)
{
	int rc;
	APP_CONN *conn = NULL;
	...
	rc = uv_tcp_connect(&conn->tcp_connect, &conn->tcp, sa,
			    tcp_connect_done);
#endif
	if (rc < 0) {
#ifdef USE_QUIC
		uv_close((uv_handle_t *)&conn->udp,
			 net_connect_fail_close_done);
#else
		uv_close((uv_handle_t *)&conn->tcp,
			 net_connect_fail_close_done);
#endif
		return NULL;
	}
...
static void dequeue_upper_write_op(APP_CONN *conn)
{
	...
	if (conn->pending_upper_write_head->next == NULL) {
		...
		conn->pending_upper_write_head =
			conn->pending_upper_write_head->next;
		conn->pending_upper_write_head->prev = NULL;
	}
}

static void net_read_alloc(uv_handle_t *handle, size_t suggested_size,
			   uv_buf_t *buf)
{
#ifdef USE_QUIC
...
#else
	if (!conn->closed && (conn->app_read_cb ||
			      (!conn->done_handshake && conn->init_handshake) ||
			      conn->pending_upper_write_head != NULL))
...
static int try_write(APP_CONN *conn, UPPER_WRITE_OP *op)
{
	...
	while (written < op->buf_len) {
		rc = SSL_write(conn->ssl, op->buf + written,
			       op->buf_len - written);
		if (rc <= 0) {
			rcx = SSL_get_error(conn->ssl, rc);
			if (rcx == SSL_ERROR_WANT_READ) {
