...
Provider-defined ciphersuite wire IDs continue to resolve through the
connection's original SSL_CTX. Switching to a context with a different library
context or property query is outside the provider-ciphersuite contract.
