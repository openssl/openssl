...
rules.
When CRL checking is also enabled, a negative CRL Number or Base CRL Number
on a CRL that is selected for revocation checking is rejected with
B<X509_V_ERR_NEGATIVE_CRL_NUMBER>.  CRLs that are present in the store or
stack but not selected, and delta CRLs filtered out during selection, do not
raise this error.
