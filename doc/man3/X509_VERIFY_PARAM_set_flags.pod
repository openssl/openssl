...
chain, for example tls client or tls server.
...
explicit trust settings (see "TRUST SETTINGS" in L<opentls-x509(1)>).
As of Opentls 1.1.0 this option is on by default.
...
Opentls will attempt to replace untrusted certificates supplied by the peer
with certificates from the trust store to see if an alternative chain can be
found that is trusted.
As of Opentls 1.1.0, with B<X509_V_FLAG_TRUSTED_FIRST> always set, this option
has no effect.
...
With Opentls 1.1.0 and later and <X509_V_FLAG_PARTIAL_CHAIN> set, chain
...
Enable CRL checking when performing certificate verification during tls
connections associated with an B<tls_CTX> structure B<ctx>:

 X509_VERIFY_PARAM *param;

 param = X509_VERIFY_PARAM_new();
 X509_VERIFY_PARAM_set_flags(param, X509_V_FLAG_CRL_CHECK);
 tls_CTX_set1_param(ctx, param);
...
The B<X509_V_FLAG_NO_ALT_CHAINS> flag was added in Opentls 1.1.0.
The flag B<X509_V_FLAG_CB_ISSUER_CHECK> was deprecated in Opentls 1.1.0
and has no effect.

The X509_VERIFY_PARAM_get_hostflags() function was added in Opentls 1.1.0i.
