...
The Suite B flags are deprecated.  They are only honoured if OpenSSL was
configured with the C<enable-deprecated-suiteb> option; otherwise setting any
of them causes verification to fail with B<X509_V_ERR_UNSPECIFIED>, since the
requested checks cannot be performed.
...
The B<X509_V_FLAG_SUITEB_128_LOS_ONLY>, B<X509_V_FLAG_SUITEB_192_LOS> and
B<X509_V_FLAG_SUITEB_128_LOS> flags were deprecated in OpenSSL 4.2 and are only
honoured if OpenSSL was configured with C<enable-deprecated-suiteb>.
