...
X509_VERIFY_PARAM_set1_cn() sets in I<param> the expected commonName
value to I<name>, clearing any previously specified commonName. The
list of expected commonName values is a separate set of reference
identifiers from the host list set via X509_VERIFY_PARAM_set1_host():
the host list is matched against the certificate's subject alternative
name extension, while the commonName list is matched against the value
of the commonName attribute of the certificate's subject distinguished
name.
If I<name> is NULL or the empty string the commonName list is cleared
and commonName checks are not performed on the peer certificate.
If I<namelen> is zero, I<name> must be NUL-terminated; otherwise
I<namelen> must be set to the length of I<name>.
...
and no change is made if I<name> is NULL or the empty string. When
multiple commonName values are configured, the peer is considered
verified when any one of them matches.
...
I<name> be no longer than 64 bytes, which is the X.509 maximum length
for the value of a commonName attribute. This default validation can be
...
The X509_VERIFY_PARAM_set1_cn(), X509_VERIFY_PARAM_add1_cn(), and
X509_VERIFY_PARAM_set1_cn_input_validation() functions were added in
OpenSSL 4.1.
