...
X509_VERIFY_PARAM_inherit, X509_VERIFY_PARAM_set1,
X509_VERIFY_PARAM_set1_name, X509_VERIFY_PARAM_get0_name,
...
X509_VERIFY_PARAM_get0_peername, X509_VERIFY_PARAM_move_peername,
...
=head1 SYNOPSIS
 ...
 int X509_VERIFY_PARAM_set1(X509_VERIFY_PARAM *dest,
                            const X509_VERIFY_PARAM *src);
 ...
 void X509_VERIFY_PARAM_move_peername(X509_VERIFY_PARAM *dest,
                                      X509_VERIFY_PARAM *src);
...
X509_VERIFY_PARAM_inherit() returns success if I<src> is NULL.  Otherwise,
it copies the parameters from the B<X509_VERIFY_PARAM> structure I<src> to
I<dest>, depending on the inheritance flags of I<src> and I<dest> ORed together.
See the L</INHERITANCE FLAGS> section for a description of these bits.
Yet if in the resulting flags <X509_VP_FLAG_LOCKED> is set, it just
returns success (after resetting the inheritance flags of I<dest>
if the B<X509_VP_FLAG_ONCE> flag is set).<br>
The flags in I<dest> get ORed by those in I<src>.

X509_VERIFY_PARAM_set1() acts like X509_VERIFY_PARAM_inherit() while in addition
B<X509_VP_FLAG_DEFAULT> is temporarily set in the inhertiance flags of I<dest>,
and finally restores the original inheritance flags of I<dest>.

X509_VERIFY_PARAM_set1_name() copies the given I<name> as the name of I<param>.

X509_VERIFY_PARAM_get0_name() returns the name set for I<param>.
...
to I<purpose>.
This determines the acceptable use of the certificate
chain, for example B<X509_PURPOSE_SSL_CLIENT>.
See L<openssl-verification-options(1)/Purposes> for details.
...
neither the target certificate nor the trust anchor count against this limit.
Thus a I<depth> limit of 0 only allows the target certificate to be signed
...
certificate.
The default value is -1, meaning "no maximum".
...
levels.
The default security level is -1, meaning "not set".
...
I<name> clearing any previously specified hostname.  If
I<name> is NULL, or empty the list of hostnames is cleared, and
name checks are not performed on the peer certificate.  If I<name>
is NUL-terminated, I<namelen> may be zero, otherwise I<namelen>
must be set to the length of I<name>.
...
are retained, no change is made if I<name> is NULL or empty.  When
...
X509_VERIFY_PARAM_move_peername() copies the peername from the
B<X509_VERIFY_PARAM> structure I<src> to the structure <dest> and removes it
from I<src>. If I<src> is NULL, it just removes the peername from <dest>.
...
level.
X509_VERIFY_PARAM_get_time() returns the verification time, NULL if unset.

X509_VERIFY_PARAM_set_time(), X509_VERIFY_PARAM_set_depth(),
X509_VERIFY_PARAM_set_auth_level(), X509_VERIFY_PARAM_set_hostflags(),
and X509_VERIFY_PARAM_move_peername() do not return values.
