...
The returned number is probably prime with the false positive rate
as documented for BN_is_prime_fasttest_ex() called with B<nchecks> set to
B<BN_prime_checks> below.
...
BN_is_prime_ex2() always does both test, and does at least 64 bit rounds
of the Miller-Rabin test giving a false positive rate of 2^-128.
If the size of B<p> is more than 2048 bits, it does 128 rounds giving
a false positive rate is 2^-256.

BN_is_prime_fasttest_ex() is a deprecated function.
It allows to skip the trial division by calling it with
B<do_trial_division> set to B<0>.
It performs B<nchecks> rounds of the Miller-Rabin test.

BN_is_prime_ex() is a deprecated function.
It performs B<nchecks> rounds of the Miller-Rabin test.
It does not do the trial division, making it on average
slower when B<p> is composite.

BN_is_prime_fasttest() and BN_is_prime() are deprecated functions
that behave just like BN_is_prime_fasttest_ex() and BN_is_prime_ex()
respectively, but with the old style call back.

If B<nchecks> == B<BN_prime_checks>, a number of iterations is used that
...
Miller-Rabin checks needs to be much higher to reach the same level of
assurance: It should equal half of the targeted security level in bits (rounded
up to the next integer if necessary).
...
BN_is_prime_ex2 was added in OpenSSL 3.0.
