...
BN_generate_prime_ex2, BN_generate_prime_ex, BN_is_prime_ex, BN_is_prime_ex2,
...
BN_is_prime_ex2() test if the number B<p> is prime.
It has a false positive rate of 2^-128.
If the size of B<p> is more than 2048 bits, the false positive rate is 2^-256.

It tests until one of the tests shows that B<p> is composite, or all the tests
passed.
If B<p> passes all these tests, it is considered a probable prime.

It first attempts trial division by a number of small primes;
if no divisors are found by this test and B<cb> is not B<NULL>,
B<BN_GENCB_call(cb, 1, -1)> is called.

It then performs rounds of the Miller-Rabin probabilistic primality test.
...
BN_is_prime_ex() and BN_is_prime_fasttest_ex() are deprecated functions
to check if B<p> is prime.
They both perform B<nchecks> rounds of the Miller-Rabin test.
...
BN_is_prime_ex() does not do the trial division step.
The false positive rate does not depend on the trial divisions, it just makes
the test faster.
...
64.
BN_is_prime_ex2() does at least 64 checks.
