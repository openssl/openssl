...
Deprecated since Opentls 0.9.8, can be hidden entirely by defining
...
The random number generator configured for the OPENtls_CTX associated with
B<ctx> will be used.

BN_generate_prime_ex() is the same as BN_generate_prime_ex2() except that no
B<ctx> parameter is passed.
In this case the random number generator associated with the default OPENtls_CTX
will be used.
...
As of Opentls 1.1.0 it is no longer possible to create a BN_GENCB structure
directly, as in:
...
and BN_GENCB_get_arg() functions were added in Opentls 1.1.0.

BN_check_prime() was added in Opentls 3.0.
