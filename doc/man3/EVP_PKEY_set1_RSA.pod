...
EVP_PKEY_get0_hmac,
EVP_PKEY_type, EVP_PKEY_id, EVP_PKEY_base_id, EVP_PKEY_check
- EVP_PKEY assignment and check functions
...
EVP_PKEY_check validates the key-pair given by B<pkey>.
...
Currently, the behaviors of EVP_PKEY_check are not consistent for different
public key algorithms. For instance, an RSA key will be checked for both
public and private components, but for EC keys they could be checked
separately for the public or the private components.
...
EVP_PKEY_check() returns 1 for success or other for failure. It
