...
EVP_PKEY_get0_engine() was used exclusively with ENGINE functionality, which has
been removed in OpenSSL 4.0. However to preserve a level of API compatibility,
this symbol is maintained and it only returns 0 without any changes made.
Applications should use providers instead of engines (see L<provider(7)> for
details).

EVP_PKEY_set1_engine() was used exclusively with ENGINE functionality, which has
been removed in OpenSSL 4.0. However to preserve a level of API compatibility,
this symbol is maintained and it only returns 0 without any changes made.
Applications should use providers instead of engines (see L<provider(7)> for
details).
...
EVP_PKEY_set1_engine() returns 0.
