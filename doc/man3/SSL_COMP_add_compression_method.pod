...
Some versions of the TLS specifications (and SSLv3) allow
the integration of compression methods
into the communication. Identifiers for the compression mechanism are
not defined, so there is currently no compatible
way to integrate compression with unknown peers.
There are also security issues that can be exploited if compression
is done at the TLS layer, rather than at the application layer.
Because of this, compression is not enabled by default and this
