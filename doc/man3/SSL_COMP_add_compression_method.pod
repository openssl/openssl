...
SSL_COMP_set0_compression_methods() replaces the list of available
compression methods with B<meths> and returns the previous list.

SSL_COMP_get_name() returns the name of the B<COMP_METHOD> B<comp>.
...
SSL_get_current_compression() returns the B<COMP_METHOD> for the compression
algorithm in use on the connection B<s>, or NULL if compression is not active.

SSL_get_current_expansion() returns the B<COMP_METHOD> for the expansion
(decompression) algorithm in use on the connection B<s>, or NULL if expansion
is not active.
...
SSL_COMP_get_id() returns the id of the compression method or -1 on error.

SSL_get_current_compression() and SSL_get_current_expansion() return a pointer
to the compression or expansion method in use, or NULL if none or on error.
...
SSL_COMP_add_compression_method(), SSL_COMP_get_compression_methods(),
SSL_COMP_set0_compression_methods(), SSL_COMP_get_name(), SSL_COMP_get0_name(),
SSL_COMP_get_id(), SSL_get_current_compression(), and SSL_get_current_expansion()
were deprecated in OpenSSL 4.1.
TLS record-level compression is obsolete and a security liability
(see the CRIME attack). These functions will be removed in a future version.
