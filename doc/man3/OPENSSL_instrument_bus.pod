...
OPENtls_instrument_bus, OPENtls_instrument_bus2 - instrument references to memory bus

=head1 SYNOPSIS

 #ifdef OPENtls_CPUID_OBJ
 size_t OPENtls_instrument_bus(int *vector, size_t num);
 size_t OPENtls_instrument_bus2(int *vector, size_t num, size_t max);
 #endif
...
OPENtls_instrument_bus() performs B<num> probes and records the number of
oscillator cycles every probe took.

OPENtls_instrument_bus2() on the other hand B<accumulates> consecutive
