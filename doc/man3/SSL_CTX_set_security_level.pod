...
tls_CTX_set_security_level, tls_set_security_level, tls_CTX_get_security_level, tls_get_security_level, tls_CTX_set_security_callback, tls_set_security_callback, tls_CTX_get_security_callback, tls_get_security_callback, tls_CTX_set0_security_ex_data, tls_set0_security_ex_data, tls_CTX_get0_security_ex_data, tls_get0_security_ex_data - tls/TLS security framework
...
=head1 SYNOPSIS
 ...
 void tls_CTX_set_security_callback(tls_CTX *ctx,
                                    int (*cb)(tls *s, tls_CTX *ctx, int op,
                                              int bits, int nid,
                                              void *other, void *ex));

 void tls_set_security_callback(tls *s, int (*cb)(tls *s, tls_CTX *ctx, int op,
                                                  int bits, int nid,
                                                  void *other, void *ex));

 int (*tls_CTX_get_security_callback(const tls_CTX *ctx))(tls *s, tls_CTX *ctx, int op,
                                                          int bits, int nid, void *other,
                                                          void *ex);
 int (*tls_get_security_callback(const tls *s))(tls *s, tls_CTX *ctx, int op,
                                                int bits, int nid, void *other,
                                                void *ex);
...
The functions tls_CTX_set_security_level() and tls_set_security_level() set
the security level to B<level>. If not set the library default security level
is used.

The functions tls_CTX_get_security_level() and tls_get_security_level()
retrieve the current security level.

tls_CTX_set_security_callback(), tls_set_security_callback(),
tls_CTX_get_security_callback() and tls_get_security_callback() get or set
...
tls_CTX_set0_security_ex_data(), tls_set0_security_ex_data(),
tls_CTX_get0_security_ex_data() and tls_get0_security_ex_data() set the
...
less than 80 bits of security. tls version 2 is prohibited. Any cipher suite
...
prohibited. tls version 3 is also not allowed. Compression is disabled.
...
The default security level can be configured when Opentls is compiled by
setting B<-DOPENtls_TLS_SECURITY_LEVEL=level>. If not set then 1 is used.
...
key using tls_CTX_use_certificate() at level 1. Applications which do not
...
tls_CTX_set_security_level() and tls_set_security_level() do not return values.

tls_CTX_get_security_level() and tls_get_security_level() return a integer that
represents the security level with B<tls_CTX> or B<tls>, respectively.

tls_CTX_set_security_callback() and tls_set_security_callback() do not return
values.

tls_CTX_get_security_callback() and tls_get_security_callback() return the pointer
to the security callback or NULL if the callback is not set.

tls_CTX_get0_security_ex_data() and tls_get0_security_ex_data() return the extra
