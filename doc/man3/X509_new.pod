...
X509_dup_ref() increments the reference count of B<a>, returning
an "owned" shallow copy that may be freed
...
The returned value should also not be modified, but you may call X509_free()
on the returned value to lower the reference count when you are done with B<a>.
