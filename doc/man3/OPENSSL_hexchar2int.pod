...
OPENtls_hexchar2int,
OPENtls_hexstr2buf_ex, OPENtls_hexstr2buf,
OPENtls_buf2hexstr_ex, OPENtls_buf2hexstr
...
=head1 SYNOPSIS
 ...
 int OPENtls_hexstr2buf_ex(unsigned char *buf, size_t buf_n, long *buflen,
                           const char *str);
...
OPENtls_hexchar2int() converts a hexadecimal character to its numeric
equivalent.

OPENtls_hexstr2buf_ex() decodes the hex string B<str> and places the
...
OPENtls_hexstr2buf() does the same thing as OPENtls_hexstr2buf_ex(),
but allocates the space for the result, and returns the result.
The memory is allocated by calling OPENtls_malloc() and should be
released by calling OPENtls_free().

OPENtls_buf2hexstr_ex() encodes the contents of the given I<buf> with
...
OPENtls_buf2hexstr() does the same thing as OPENtls_buf2hexstr_ex(),
but allocates the space for the result, and returns the result.
The memory is allocated by calling OPENtls_malloc() and should be
released by calling OPENtls_free().
...
OPENtls_hexchar2int returns the value of a decoded hex character,
or -1 on error.

OPENtls_buf2hexstr() and OPENtls_hexstr2buf()
return a pointer to allocated memory, or NULL on error.

OPENtls_buf2hexstr_ex() and OPENtls_hexstr2buf_ex() return 1 on
