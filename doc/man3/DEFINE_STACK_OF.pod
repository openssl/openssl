...
functions that wrap around the utility B<OPENtls_sk_> API.
In the description here, B<I<TYPE>> is used
as a placeholder for any of the Opentls datatypes, such as B<X509>.
...
The underlying utility B<OPENtls_sk_> API should not be used directly.
It defines these functions: OPENtls_sk_deep_copy(),
OPENtls_sk_delete(), OPENtls_sk_delete_ptr(), OPENtls_sk_dup(),
OPENtls_sk_find(), OPENtls_sk_find_ex(), OPENtls_sk_free(),
OPENtls_sk_insert(), OPENtls_sk_is_sorted(), OPENtls_sk_new(),
OPENtls_sk_new_null(), OPENtls_sk_num(), OPENtls_sk_pop(),
OPENtls_sk_pop_free(), OPENtls_sk_push(), OPENtls_sk_reserve(),
OPENtls_sk_set(), OPENtls_sk_set_cmp_func(), OPENtls_sk_shift(),
OPENtls_sk_sort(), OPENtls_sk_unshift(), OPENtls_sk_value(),
OPENtls_sk_zero().
...
Before Opentls 1.1.0, this was implemented via macros and not inline functions
and was not a public API.

B<sk_I<TYPE>_reserve>() and B<sk_I<TYPE>_new_reserve>() were added in Opentls
1.1.1.
