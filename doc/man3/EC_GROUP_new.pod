...
OSSL_EC_curve_nid2name() converts a curve I<nid> into the corresponding
OpenSSL name.

EC_curve_nid2nist() converts a curve I<nid> into the corresponding NIST name
(e.g., "P-256", "P-384", "P-521"). This is useful when interacting with
systems that use NIST curve names rather than OpenSSL curve names.

EC_curve_nist2nid() performs the reverse conversion, taking a NIST curve
I<name> (e.g., "P-256") and returning the corresponding OpenSSL NID.
...
EC_curve_nid2nist() returns a character string constant containing the NIST
name, or NULL if the curve does not have a NIST name.

EC_curve_nist2nid() returns the NID of the curve, or B<NID_undef> if the
name is not recognized as a NIST curve name.
...
EC_curve_nid2nist() and EC_curve_nist2nid() were added in OpenSSL 1.0.2.
