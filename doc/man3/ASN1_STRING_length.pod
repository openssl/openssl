...
I<x> B<MUST NOT> be NULL.

ASN1_STRING_get_length() returns the length of the content of I<x>.
I<x> B<MUST NOT> be NULL.  If the current length of the content of I<x>
cannot be represented by a value of type B<size_t> (for example, after
an ASN1_STRING_length_set() call with a negative I<n> argument), 0 is returned.
...
ASN1_STRING_copy() copies the string type and content (along with the relevant
internal attributes) from I<str> to I<dst>.
...
from the memory pointed to by I<data> to I<str>. If I<len> is -1,
then the length is determined by strlen(data).  As an implementation detail,
ASN1_STRING_set() appends a NUL byte after the data copied into I<str>.
...
Contrary to ASN1_STRING_set(), ASN1_STRING_set1_data() does not append a NUL
byte to the data copied into I<str>.
...
B<ASN1_BIT_STRING>.  ASN1_STRING_set1_string() does not append a NUL
byte to the data copied into I<str>.

ASN1_STRING_set0() sets the content of the string I<str> to I<data>
of I<len> bytes in size;  the ownership of I<data> is assumed
by the string (so, I<data> has to be allocated using OPENSSL_malloc()
or a similar function, and the caller should not attempt to free I<data>
after the call).  The previously held content (in case they were owned
by the string) is freed using OPENSSL_clear_free(), provided the length
of the content is positive.
...
ASN1_STRING_length_set() allows setting the length of I<x> to an arbitrary value
provided in I<n>.
...
by ASN1_STRING_get0_data() is NUL-terminated or does not contain
embedded NUL bytes.  The actual format of the data will depend
on the actual string type itself: for example, for an B<IA5String>, the data
will be ASCII; for a B<BMPString>, it will be two bytes per character
in big endian format without BOM; and for a B<UTF8String>, it will be encoded
in UTF-8.
...
ASN1_STRING_length_set() updates only the length attribute of a string
and does not affect the stored data, which may lead to buffer overruns
and invalid memory accesses, if an incorrect length value is supplied.
It also does not perform any sanity checks and stores any supplied value
as-is, including a negative one.
For these reasons (and due to limited usefulness), this function is deprecated.
...
ASN1_STRING_get_length() returns the length of the content of I<x> or 0,
if it cannot be represented as a value of type B<size_t>.

ASN1_STRING_get0_data() returns an internal pointer to
the data of I<x>.

ASN1_STRING_copy() returns 1 on success, or 0 on error or failure.
If the call was not successful, I<dst> may be left in an inconsistent state,
...
ASN1_STRING_set0() and ASN1_STRING_length_set() do not return a value.
...
ASN1_STRING_length_set() was deprecated in OpenSSL 3.0.

ASN1_STRING_set1_data(), ASN1_STRING_set1_string(), and ASN1_STRING_get_length()
were added in OpenSSL 4.1.

ASN1_STRING_length() and ASN1_STRING_set() were deprecated in OpenSSL 4.1.

Before OpenSSL 4.1, ASN1_STRING_set0() used OPENSSL_free()
instead of OPENSSL_clean_free() to free previously held string content.
