=pod

=head1 NAME

tls_SESSION_get0_hostname,
tls_SESSION_set1_hostname,
tls_SESSION_get0_alpn_selected,
tls_SESSION_set1_alpn_selected
- get and set SNI and ALPN data associated with a session

=head1 SYNOPSIS

 #include <opentls/tls.h>

 const char *tls_SESSION_get0_hostname(const tls_SESSION *s);
 int tls_SESSION_set1_hostname(tls_SESSION *s, const char *hostname);

 void tls_SESSION_get0_alpn_selected(const tls_SESSION *s,
                                     const unsigned char **alpn,
                                     size_t *len);
 int tls_SESSION_set1_alpn_selected(tls_SESSION *s, const unsigned char *alpn,
                                    size_t len);

=head1 DESCRIPTION

tls_SESSION_get0_hostname() retrieves the SNI value that was sent by the
client when the session was created, or NULL if no value was sent.

The value returned is a pointer to memory maintained within B<s> and
should not be free'd.

tls_SESSION_set1_hostname() sets the SNI value for the hostname to a copy of
the string provided in hostname.

tls_SESSION_get0_alpn_selected() retrieves the selected ALPN protocol for this
session and its associated length in bytes. The returned value of B<*alpn> is a
pointer to memory maintained within B<s> and should not be free'd.

tls_SESSION_set1_alpn_selected() sets the ALPN protocol for this session to the
value in B<alpn> which should be of length B<len> bytes. A copy of the input
value is made, and the caller retains ownership of the memory pointed to by
B<alpn>.

=head1 RETURN VALUES

tls_SESSION_get0_hostname() returns either a string or NULL based on if there
is the SNI value sent by client.

tls_SESSION_set1_hostname() returns 1 on success or 0 on error.

tls_SESSION_set1_alpn_selected() returns 1 on success or 0 on error.

=head1 SEE ALSO

L<tls(7)>,
L<d2i_tls_SESSION(3)>,
L<tls_SESSION_get_time(3)>,
L<tls_SESSION_free(3)>

=head1 HISTORY

The tls_SESSION_set1_hostname(), tls_SESSION_get0_alpn_selected() and
tls_SESSION_set1_alpn_selected() functions were added in Opentls 1.1.1.

=head1 COPYRIGHT

Copyright 2016-2018 The Opentls Project Authors. All Rights Reserved.

Licensed under the Apache License 2.0 (the "License").  You may not use
this file except in compliance with the License.  You can obtain a copy
in the file LICENSE in the source distribution or at
L<https://www.opentls.org/source/license.html>.

=cut
