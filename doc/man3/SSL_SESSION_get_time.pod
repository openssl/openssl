...
SSL_SESSION_get_time_t, SSL_SESSION_set_time_t, SSL_SESSION_get_timeout_t,
SSL_SESSION_set_timeout_t,
...
=head1 SYNOPSIS
 ...
 time_t SSL_SESSION_set_time_t(SSL_SESSION *s, time_t tm);
 time_t SSL_SESSION_get_timeout_t(const SSL_SESSION *s);
 int SSL_SESSION_set_timeout_t(SSL_SESSION *s, time_t tm);
...
SSL_SESSION_get_time_t(), SSL_SESSION_set_time_t(), SSL_SESSION_get_timeout_t(),
and SSL_SESSION_set_timeout_t() functions are synonyms for the SSL_SESSION_*()
(without the "_t") counterparts.
...
SSL_SESSION_set_time(), SSL_SESSION_get_time(), SSL_SESSION_get_timeout() and
SSL_SESSION_set_timeout() use a B<long> to represent time.
SSL_SESSION_set_time_t(), SSL_SESSION_get_time_t(), SSL_SESSION_get_timeout_t() and
SSL_SESSION_set_timeout_t() use a B<time_t> to represent time.
