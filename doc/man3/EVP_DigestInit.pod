...
provider offering it, withing the criteria given by the B<properties>.
...
If supplied by a function such as EVP_sha1(), ENGINE B<impl> may be
non-NULL and point at an engine that implements the type, and
otherwise, the default implementation of digest B<type> is used.

If supplied by an explicitly fetched value, B<impl> is ignored.
