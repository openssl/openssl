...
other similar function) when the EVP_MD_CTX was first initialized. Note that
...
This flag instructs EVP_DigestInit() and similar not to initialize the
implementation specific data.

=item EVP_MD_CTX_FLAG_FINALISE

Some functions such as EVP_DigestSign only finalise copies of internal
contexts so additional data can be included after the finalization call.
