...
ones. See the "flags" parameter as documented in L<EVP_MD-common(7)> for a
...
It may be used by the SHAKE, CSHAKE, TurboSHAKE and KangarooTwelve XOF
algorithms to set the output length used by EVP_DigestFinal_ex() and
EVP_DigestFinal().
...
EVP_DigestFinal_ex() and EVP_DigestFinal().  It may be used by the SHAKE,
CSHAKE, TurboSHAKE and KangarooTwelve XOF algorithms as an alias of "xoflen".
...
=item "customization" (B<OSSL_DIGEST_PARAM_CUSTOMIZATION>)<UTF8 string or octet string>

Sets a customisation string.
It is used by L<EVP_MD-CSHAKE(7)> as a UTF8 string with a length of at most
512 bytes, and by L<EVP_MD-KANGAROOTWELVE(7)> as an octet string with a length
of at most 512 bytes.

=item "domain-separator" (B<OSSL_DIGEST_PARAM_DOMAIN_SEPARATOR>) <unsigned integer>

Sets the TurboSHAKE domain separator.
It is used by L<EVP_MD-TURBOSHAKE(7)>.
