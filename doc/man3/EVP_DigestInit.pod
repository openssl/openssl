...
EVP_DigestUpdate, EVP_DigestFinal_ex, EVP_DigestFinalXOF,
EVP_DigestFinalVarLen, EVP_DigestFinal,
...
=head1 SYNOPSIS
 ...
 int EVP_DigestFinalVarLen(EVP_MD_CTX *ctx, unsigned char *md, size_t len);
...
=item EVP_DigestFinalVarLen()

Retrieves the digest value for variable length digest functions such as Blake2b
from I<ctx> and places it in I<len>-sized I<md>. After calling this function
no additional calls to EVP_DigestUpdate() can be made, but EVP_DigestInit_ex2()
...
EVP_DigestFinalVarLen() and EVP_MD_CTX_dup() were added in OpenSSL 3.2.
