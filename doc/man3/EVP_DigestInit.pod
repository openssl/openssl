...
EVP_DigestInit_ex() can be called to initialise a new digest operation.
...
made, but EVP_DigestInit_ex() can be called to initialise a new operation.
...
update function from the B<EVP_MD> type specified at initialisation is used.
