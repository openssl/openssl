...
=head1 SYNOPSIS
 ...
 const OSSL_PARAM *EVP_MD_CTX_settable_params(const EVP_MD *md);
 const OSSL_PARAM *EVP_MD_CTX_gettable_params(const EVP_MD *md);
...
EVP_MD_get_params().
EVP_MD_CTX_gettable_params() returns parameters that can be used with
EVP_MD_CTX_get_params().
EVP_MD_CTX_settable_params() returns parameters that can be used with
