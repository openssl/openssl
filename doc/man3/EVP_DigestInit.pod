...
=head1 SYNOPSIS
 ...
 EVP_MD *EVP_MD_fetch(OPENtls_CTX *ctx, const char *algorithm,
...
Get a B<Otls_PARAM> array that describes the retrievable and settable
...
Returns an B<Otls_PROVIDER> pointer to the provider that implements the given
B<EVP_MD>.
...
The space is allocated by Opentls and has the size originally set with
...
See L<Otls_PARAM(3)> for information about passing parameters.

EVP_MD_CTX_set_params() can be used with the following Otls_PARAM keys:

=over 4

=item "xoflen" (B<Otls_PARAM_DIGEST_KEY_XOFLEN>) <unsigned integer>

Sets the digest length for extendable output functions.
It is used by the SHAKE algorithm and should not exceed what can be given
using a B<size_t>.

=item "pad_type" (B<Otls_PARAM_DIGEST_KEY_PAD_TYPE>) <integer>
...
EVP_MD_CTX_get_params() can be used with the following Otls_PARAM keys:

=over 4

=item "micalg" (B<Otls_PARAM_DIGEST_KEY_MICALG>) <UTF8 string>.
...
Return an array of constant B<Otls_PARAM>s, or NULL if there is none
to get.
...
EVP_MD_CTX_new() and EVP_MD_CTX_free() in Opentls 1.1.0, respectively.

The link between digests and signing algorithms was fixed in Opentls 1.0 and
later, so now EVP_sha1() can be used with RSA and DSA.

The EVP_dss1() function was removed in Opentls 1.1.0.
