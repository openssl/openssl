...
Sets the fixed digest length for an extendable output function.
The value should not exceed what can be given using a B<size_t>.
It may be used by BLAKE2B-512
It is NOT used by SHAKE-128 and SHAKE-256.
...
Currently only used by legacy code for SHAKE.
