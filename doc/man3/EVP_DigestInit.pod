...
EVP_DigestFinal_ex, EVP_DigestFinalXOF, EVP_DigestSqueezeXOF, EVP_DigestFinal,
...
=head1 SYNOPSIS
 ...
 int EVP_DigestSqueezeXOF(EVP_MD_CTX *ctx, unsigned char *md, size_t len);
...
SHAKE256.  It retrieves the digest value from B<ctx> and places it in
B<len>-sized <B>md.  After calling this function no additional calls
to EVP_DigestUpdate(), EVP_DigestSqueezeXOF(), or EVP_DigestFinalXOF()
can be made. EVP_DigestInit_ex() can still be called to initialize a
new operation. If you need to extract data from the XOF more than once, use
EVP_DigestSqueezeXOF() instead.

=item EVP_DigestSqueezeXOF()

Similar to EVP_DigestFinalXOF(), but can be called more than once to
extract bytes from the XOF repeatedly.  Each call to
EVP_DigestSqueezeXOF() yields the next bytes in sequence after the
bytes that were previously extracted.
