...
Feature request value. Limits the number of pending
...
The default limit is 256. Setting the value to 0 is rejected: unlike
the pending timeout, this cap cannot be disabled, as doing so would
expose the listener to Denial of Service attacks from spoofed-source
UDP floods. Values larger than B<SIZE_MAX> are clamped to B<SIZE_MAX>.

=item B<SSL_VALUE_DTLS_LISTENER_PENDING_TIMEOUT> (DTLS listener object)

Feature request value. Sets the maximum time in milliseconds that a
...
The default is 30000 (30 seconds). Setting the value to B<UINT64_MAX>
disables the timeout (pending connections are never expired).

=item B<SSL_VALUE_DTLS_LISTENER_MAX_DGRAM_SIZE> (DTLS listener object)

Feature request value. Sets the maximum size in bytes of a datagram that a
