...
=head1 SYNOPSIS
 ...
 #define SSL_VALUE_DTLS_LISTENER_MAX_PENDING_CONNS
 #define SSL_VALUE_DTLS_LISTENER_PENDING_TIMEOUT
 #define SSL_VALUE_DTLS_LISTENER_MAX_DGRAM_SIZE
...
=head1 CONFIGURABLE VALUES FOR DTLS SSL LISTENER OBJECTS

The following configurable values are supported for DTLS SSL listener objects
created by L<SSL_new_listener(3)>. These are purely local configuration values
that do not participate in feature negotiation with a peer, and are therefore
accessed using the B<SSL_VALUE_CLASS_GENERIC> value class (for example via the
SSL_get_generic_value_uint() and SSL_set_generic_value_uint() convenience
macros). Using any other value class fails with an error.

=over 4

=item B<SSL_VALUE_DTLS_LISTENER_MAX_PENDING_CONNS> (DTLS listener object)

Generic value. Limits the number of pending
(handshake-in-progress) connections a DTLS listener created by
L<SSL_new_listener(3)> will track. When the limit is reached, datagrams
from novel peers are silently dropped; existing pending connections may
still complete their handshake and enter the accept queue, and stale
entries age out via B<SSL_VALUE_DTLS_LISTENER_PENDING_TIMEOUT>.

The default limit is 256. Setting the value to 0 is rejected: the limit is an
upper bound on tracked pending connections, so a cap of 0 would reject every
incoming connection and leave the listener unable to accept anything. Any
non-zero value is accepted (values larger than B<SIZE_MAX> are clamped to
B<SIZE_MAX>); a sufficiently large value therefore makes the limit effectively
unbounded, so keeping it at a sensible value is what protects the listener
against Denial of Service from spoofed-source UDP floods.

=item B<SSL_VALUE_DTLS_LISTENER_PENDING_TIMEOUT> (DTLS listener object)

Generic value. Sets the maximum time in milliseconds that a
pending (handshake-in-progress) DTLS connection may remain in the
listener's pending table before being reaped. Expired pending
connections are silently discarded, freeing a slot in the
B<SSL_VALUE_DTLS_LISTENER_MAX_PENDING_CONNS> quota.

The default is 30000 (30 seconds). Setting the value to 0 is rejected: a zero
timeout will remove all pending connections on the next tick, before the
handshake could complete. Setting the value to B<UINT64_MAX> disables the
timeout (pending connections are never expired).

=item B<SSL_VALUE_DTLS_LISTENER_MAX_DGRAM_SIZE> (DTLS listener object)

Generic value. Sets the maximum size in bytes of a datagram that a
DTLS listener created by L<SSL_new_listener(3)> will receive. Datagrams larger
than this are truncated by the operating system on receipt; for a handshake
datagram such as a large ClientHello this prevents the connection from being
established.

The default is 2000, chosen a little above the 1500-byte Ethernet MTU so that a
DTLS 1.3 ClientHello carrying large (for example post-quantum) key shares is
received whole rather than truncated. Deployments over a transport with a
larger MTU (for example jumbo frames) may raise it. Values are clamped to 65535
(the maximum UDP payload), and values below the minimum receive size (1200) are
rejected.
