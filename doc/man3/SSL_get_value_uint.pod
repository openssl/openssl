...
=item B<SSL_VALUE_QUIC_DATA_MAX> (connection/listener object)
...
=item B<SSL_VALUE_QUIC_STREAM_DATA_BIDI_LOCAL_MAX> (connection/listener object)
...
=item B<SSL_VALUE_QUIC_STREAM_DATA_BIDI_REMOTE_MAX> (connection/listener object)
...
=item B<SSL_VALUE_QUIC_STREAM_DATA_UNI_REMOTE_MAX> (connection/listener object)

As above, but provides the initial maximum amount of data in bytes that can be
received in terms of QUIC stream flow control on a unidirectional stream created
by the remote endpoint.

=item B<SSL_VALUE_QUIC_STREAM_BIDI_REMOTE_MAX> (connection/listener object)
...
=item B<SSL_VALUE_QUIC_STREAM_UNI_REMOTE_MAX> (connection/listener object)
...
=item B<SSL_VALUE_QUIC_DISABLE_ACTIVE_MIGRATION> (connection/listener object)
...
=item B<SSL_VALUE_QUIC_ACTIVE_CONN_ID_LIMIT> (connection/listener object)
