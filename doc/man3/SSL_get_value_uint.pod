...
SSL_VALUE_QUIC_UDP_PAYLOAD_SIZE_MAX, SSL_VALUE_QUIC_DATA_MAX,
SSL_VALUE_QUIC_STREAM_DATA_BIDI_LOCAL_MAX,
SSL_VALUE_QUIC_STREAM_DATA_BIDI_REMOTE_MAX,
SSL_VALUE_QUIC_STREAM_DATA_UNI_REMOTE_MAX,
SSL_VALUE_QUIC_STREAM_BIDI_REMOTE_MAX, SSL_VALUE_QUIC_STREAM_UNI_REMOTE_MAX,
SSL_VALUE_QUIC_ACK_DELAY_EXPONENT, SSL_VALUE_QUIC_ACK_DELAY_MAX,
SSL_VALUE_QUIC_DISABLE_ACTIVE_MIGRATION, SSL_VALUE_QUIC_ACTIVE_CONN_ID_LIMIT,
...
=head1 SYNOPSIS
 ...
 #define SSL_VALUE_QUIC_DATA_MAX
 #define SSL_VALUE_QUIC_STREAM_DATA_BIDI_LOCAL_MAX
 #define SSL_VALUE_QUIC_STREAM_DATA_BIDI_REMOTE_MAX
 #define SSL_VALUE_QUIC_STREAM_DATA_UNI_REMOTE_MAX
 #define SSL_VALUE_QUIC_STREAM_BIDI_REMOTE_MAX
 #define SSL_VALUE_QUIC_STREAM_UNI_REMOTE_MAX
 #define SSL_VALUE_QUIC_ACK_DELAY_EXPONENT
 #define SSL_VALUE_QUIC_ACK_DELAY_MAX
 #define SSL_VALUE_QUIC_DISABLE_ACTIVE_MIGRATION
 #define SSL_VALUE_QUIC_ACTIVE_CONN_ID_LIMIT
...
=item B<SSL_VALUE_QUIC_UDP_PAYLOAD_SIZE_MAX> (connection object)

Feature (peer) request value. This configures the maximum UDP payload size in
bytes. This feature can only be configured prior to connection establishment and
cannot be subsequently changed. This value will be sent to the peer in the
transport parameters.

This release of OpenSSL uses a default value of 1200 bytes. This default value
may change between releases of OpenSSL.

=item B<SSL_VALUE_QUIC_DATA_MAX> (connection object)

Feature (peer) request value. This configures the initial maximum amount of data
in bytes that can be sent on the connection. This feature can only be configured
prior to connection establishment and cannot be subsequently changed. This value
will be sent to the peer in the transport parameters.

This release of OpenSSL uses a default value of 768 * 1024 bytes. This default
value may change between releases of OpenSSL.

=item B<SSL_VALUE_QUIC_STREAM_DATA_BIDI_LOCAL_MAX> (connection object)

Feature (peer) request value. This configures the initial maximum amount of data
in bytes that can be received in terms of QUIC stream flow control on a
bidirectional stream created by the local endpoint. This feature can only be
configured prior to connection establishment and cannot be subsequently changed.
This value will be sent to the peer in the transport parameters.

This release of OpenSSL uses a default value of 512 * 1024 bytes. This default
value may change between releases of OpenSSL.

=item B<SSL_VALUE_QUIC_STREAM_DATA_BIDI_REMOTE_MAX> (connection object)

As above, but provides the initial maximum amount of data in bytes that can be
received in terms of QUIC stream flow control on a bidirectional stream created
by the remote endpoint.

=item B<SSL_VALUE_QUIC_STREAM_DATA_UNI_REMOTE_MAX> (connection object)

As above, but provides the initial maximum amount of data in bytes that can be
received in terms of QUIC stream flow control on a unidirectional stream created
by the remote endpoint.

=item B<SSL_VALUE_QUIC_STREAM_BIDI_REMOTE_MAX> (connection object)

Feature (peer) request value. This configures the initial maximum number of
bidirectional streams the peer is permitted to initiate. This feature can only
...
This release of OpenSSL uses a default value of 100 streams. This default
value may change between releases of OpenSSL.

=item B<SSL_VALUE_QUIC_STREAM_UNI_REMOTE_MAX> (connection object)

As above, but provides the initial maximum number of unidirectional streams the
peer is permitted to initiate.

=item B<SSL_VALUE_QUIC_ACK_DELAY_EXPONENT> (connection object)
...
=item B<SSL_VALUE_QUIC_ACK_DELAY_MAX> (connection object)
...
=item B<SSL_VALUE_QUIC_DISABLE_ACTIVE_MIGRATION> (connection object)

Feature (peer) request value. This configures whether to signal to the peer that
we do not support active migration. This feature can only be configured prior to
connection establishment and cannot be subsequently changed. This value will be
sent to the peer in the transport parameters.

This release of OpenSSL uses a default value of 1 (i.e., active migration is
disabled). This default value may change between releases of OpenSSL.

=item B<SSL_VALUE_QUIC_ACTIVE_CONN_ID_LIMIT> (connection object)

Feature (peer) request value. This configures the maximum number of active
connection IDs that can be used by the peer on the connection. This feature can
only be configured prior to connection establishment and cannot be subsequently
changed. This value will be sent to the peer in the transport parameters.

This release of OpenSSL uses a default value of 2. This default value may change
between releases of OpenSSL.
...
The values SSL_VALUE_QUIC_UDP_PAYLOAD_SIZE_MAX, SSL_VALUE_QUIC_DATA_MAX,
SSL_VALUE_QUIC_STREAM_DATA_BIDI_LOCAL_MAX,
SSL_VALUE_QUIC_STREAM_DATA_BIDI_REMOTE_MAX,
SSL_VALUE_QUIC_STREAM_DATA_UNI_REMOTE_MAX,
SSL_VALUE_QUIC_STREAM_BIDI_REMOTE_MAX, SSL_VALUE_QUIC_STREAM_UNI_REMOTE_MAX,
SSL_VALUE_QUIC_ACK_DELAY_EXPONENT, SSL_VALUE_QUIC_ACK_DELAY_MAX,
SSL_VALUE_QUIC_DISABLE_ACTIVE_MIGRATION, and SSL_VALUE_QUIC_ACTIVE_CONN_ID_LIMIT 
