...
OSSL_DECODER_new_by_EVP_PKEY,
OSSL_DECODER_set_passphrase,
OSSL_DECODER_set_pem_password_cb,
OSSL_DECODER_set_passphrase_ui,
OSSL_DECODER_set_passphrase_cb
...
=head1 SYNOPSIS
 ...
 OSSL_DECODER *
 OSSL_DECODER_new_by_EVP_PKEY(const EVP_PKEY **pkey,
...
OSSL_DECODER_new_by_EVP_PKEY() is a utility function that creates a
B<OSSL_DECODER>, finds all applicable decoder implementations and sets
...
Internally OSSL_DECODER_new_by_EVP_PKEY() searches for all available
...
They are set with L<OSSL_DECODER_set_input_type(3)> and
L<OSSL_DECODER_set_input_structure(3)>.
...
OSSL_DECODER_new_by_EVP_PKEY() still creates a B<OSSL_DECODER>, but
with no associated decoder (L<OSSL_DECODER_get_num_methods(3)> returns
zero).  This helps the caller to distinguish between an error when creating
the B<OSSL_ENCODER> and missing encoder implementation, and allows it to
act accordingly.

OSSL_DECODER_set_passphrase() gives the implementation a pass phrase to
...
OSSL_DECODER_set_pem_password_cb(), OSSL_DECODER_set_passphrase_ui()
and OSSL_DECODER_set_passphrase_cb() set up a callback method that the
...
OSSL_DECODER_new_by_EVP_PKEY() returns a pointer to a
B<OSSL_DECODER>, or NULL if it couldn't be created.

OSSL_DECODER_set_passphrase(), OSSL_DECODER_set_pem_password_cb(),
OSSL_DECODER_set_passphrase_ui() and
OSSL_DECODER_set_passphrase_cb() all return 1 on success, or 0 on
failure.
...
B<EVP_PKEY> in OSSL_DECODER_new_by_EVP_PKEY() matches the type name,
thus making for the naming pattern B<OSSL_DECODER_new_by_I<TYPE>>() when
new types are handled.

=head1 SEE ALSO

L<provider(7)>, L<OSSL_DECODER(3)>, L<OSSL_DECODER_METHOD(3)>
