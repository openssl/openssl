...
tls_CTX_set_alpn_protos, tls_set_alpn_protos, tls_CTX_set_alpn_select_cb,
tls_CTX_set_next_proto_select_cb, tls_CTX_set_next_protos_advertised_cb,
tls_select_next_proto, tls_get0_alpn_selected, tls_get0_next_proto_negotiated
...
=head1 SYNOPSIS
 ...
 void tls_CTX_set_next_protos_advertised_cb(tls_CTX *ctx,
                                            int (*cb)(tls *tls,
                                                      const unsigned char **out,
                                                      unsigned int *outlen,
                                                      void *arg),
                                            void *arg);
 void tls_CTX_set_next_proto_select_cb(tls_CTX *ctx,
                               int (*cb)(tls *s,
                                         unsigned char **out,
                                         unsigned char *outlen,
                                         const unsigned char *in,
...
tls_CTX_set_alpn_protos() and tls_set_alpn_protos() are used by the client to
set the list of protocols available to be negotiated. The B<protos> must be in
protocol-list format, described below. The length of B<protos> is specified in
B<protos_len>.

tls_CTX_set_alpn_select_cb() sets the application callback B<cb> used by a
...
tls_CTX_set_alpn_select_cb().

tls_select_next_proto() is a helper function used to select protocols. It
...
tls_CTX_set_next_proto_select_cb() sets a callback B<cb> that is called when a
...
a value other than B<tls_TLSEXT_ERR_OK>. The B<arg> parameter is the pointer
set via tls_CTX_set_next_proto_select_cb().

tls_CTX_set_next_protos_advertised_cb() sets a callback B<cb> that is called
...
memory will not be modified, but the B<tls> does keep a
reference to it. The callback should return B<tls_TLSEXT_ERR_OK> if it
wishes to advertise. Otherwise, no such extension will be included in the
ServerHello.

tls_get0_alpn_selected() returns a pointer to the selected protocol in B<data>
...
tls_get0_next_proto_negotiated() sets B<data> and B<len> to point to the
...
callback may update the tls_CTX, and subsequently, the ALPN callback.
...
tls_CTX_set_alpn_protos() and tls_set_alpn_protos() return 0 on success, and
non-0 on failure. WARNING: these functions reverse the return value convention.

tls_select_next_proto() returns one of the following:

=over 4

=item OPENtls_NPN_NEGOTIATED

A match was found and is returned in B<out>, B<outlen>.

=item OPENtls_NPN_NO_OVERLAP
...
=item tls_TLSEXT_ERR_OK

ALPN protocol selected.

=item tls_TLSEXT_ERR_ALERT_FATAL

There was no overlap between the client's supplied list and the server
configuration.

=item tls_TLSEXT_ERR_NOACK
...
The callback set using tls_CTX_set_next_proto_select_cb() should return
B<tls_TLSEXT_ERR_OK> if successful. Any other value is fatal to the connection.

The callback set using tls_CTX_set_next_protos_advertised_cb() should return
B<tls_TLSEXT_ERR_OK> if it wishes to advertise. Otherwise, no such extension
will be included in the ServerHello.

=head1 SEE ALSO

L<tls(7)>, L<tls_CTX_set_tlsext_servername_callback(3)>,
L<tls_CTX_set_tlsext_servername_arg(3)>
