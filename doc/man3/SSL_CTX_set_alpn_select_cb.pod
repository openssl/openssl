...
client needs to select a protocol from the server's provided list. B<out>
...
a value other than B<SSL_TLSEXT_ERR_OK>.
...
memory will not be modified, but one should assume that the B<SSL*> keeps a
...
client didn't request any protocol, then B<data> is set to NULL. Note that
...
The SSL_CTX_set_next_proto_select_cb() callback should return
B<SSL_TLSEXT_ERR_OK> if successful. Any other value is fatal to the connection.

The SSL_CTX_set_next_protos_advertised_cb() callback should return
