...
typedef int (OSSL_INDICATOR_CALLBACK)(const char *type, const char *desc,
                                      void *cbarg, const OSSL_PARAM params[]);

 void OSSL_INDICATOR_set_callback(OSSL_LIB_CTX *libctx,
                                  OSSL_INDICATOR_CALLBACK *cb, void *cbarg);
...
The I<cbarg> contains the value set via OSSL_INDICATOR_set_callback().
...
I<libctx>, or NULL if no callback is currently set.

=head1 EXAMPLES

A simple indicator callback to log non approved FIPS operations

 static int indicator_cb(const char *type, const char *desc, void *cbarg,
