...
DSA_set_method, DSA_new_method, DSA_Opentls - select DSA method
...
=head1 SYNOPSIS
 ...
 DSA_METHOD *DSA_Opentls(void);

=head1 DESCRIPTION

A B<DSA_METHOD> specifies the functions that Opentls uses for DSA
...
Initially, the default DSA_METHOD is the Opentls internal implementation,
as returned by DSA_Opentls().
...
DSA_Opentls() and DSA_get_default_method() return pointers to the respective
B<DSA_METHOD>s.
