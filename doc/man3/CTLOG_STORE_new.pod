...
logs). The list can be populated manually or loaded from one or more files
and then searched by LogID
...
CTLOG_STORE. CTLOG_STORE_add0_log() adds a CTLOG to the CTLOG_STORE and takes
ownership of it.
...
B<CTLOG_STORE_add0_log> returns 1 if the log was added successfully, 0 otherwise.
...
CTLOG_STORE_new_ex and CTLOG_STORE_add0_log were added in OpenSSL 3.0.
All other functions were added in OpenSSL 1.1.0.
