...
Since SSL_read() and SSL_write() can return B<SSL_ERROR_WANT_READ> for
non-application data records, and SSL_has_pending() can't tell the difference
between processed and unprocessed data, it's recommended that if read ahead is
turned on that you also call SSL_CTX_set_mode() and set B<SSL_MODE_AUTO_RETRY>.
That will prevent getting B<SSL_ERROR_WANT_READ> when it still has an
unprocessed record buffered.
