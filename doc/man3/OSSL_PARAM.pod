...
Otls_PARAM - a structure to pass or request object parameters
...
B<Otls_PARAM> is a type that allows passing arbitrary data for some
...
The caller sets up the B<Otls_PARAM> array and calls some function
(the I<setter>) that has intimate knowledge about the object that can
take the data from the B<Otls_PARAM> array and assign them in a
suitable form for the internal structure of the object.

=item * Request parameters of some object

The caller (the I<requestor>) sets up the B<Otls_PARAM> array and
...
I<requestor> and pointed at with the B<Otls_PARAM> I<data>.

=item * Request parameter descriptors

The caller gets an array of constant B<Otls_PARAM>, which describe
...
build up its own B<Otls_PARAM> array to pass down to a I<setter> or
I<responder>.

=back

Normally, the order of the an B<Otls_PARAM> array is not relevant.
...
When the B<Otls_PARAM> is used as a parameter descriptor, I<data>
should be ignored.
...
When an array of B<Otls_PARAM> is used to request data, the
...
When the B<Otls_PARAM> is used as a parameter descriptor,
...
Opentls libraries are defined by the libraries, and names for
...
entities should remain oblivious and just pass the B<Otls_PARAM> array
along.
...
=item B<Otls_PARAM_INTEGER>

=item B<Otls_PARAM_UNSIGNED_INTEGER>
...
=item B<Otls_PARAM_OCTET_PTR>

The parameter data is a pointer to an arbitrary string of bytes.

The difference between this and B<Otls_PARAM_OCTET_STRING> is that
...
B<Otls_PARAM_UNSIGNED_INTEGER>, is plausible to pass the value as an
B<Otls_PARAM_OCTET_STRING>), but this is in no way mandatory.

=item *

If a I<responder> finds that some data sizes are too small for the
requested data, it must set I<return_size> for each such
B<Otls_PARAM> item to the minimum required size, and eventually return
an error.

=item *

For the integer type parameters (B<Otls_PARAM_UNSIGNED_INTEGER> and
B<Otls_PARAM_INTEGER>), a I<responder> may choose to return an error
...
A couple of examples to just show how B<Otls_PARAM> arrays could be
set up.
...
This example is for setting parameters on some object:
    ...
    Otls_PARAM set[] = {
        { "foo", Otls_PARAM_UTF8_STRING_PTR, &foo, foo_l, 0 },
        { "bar", Otls_PARAM_UTF8_STRING, &bar, sizeof(bar), 0 },
        { NULL, 0, NULL, 0, NULL }
    };
...
This example is for requesting parameters on some object:
    ...
    size_t bar_l;
    Otls_PARAM request[] = {
        { "foo", Otls_PARAM_UTF8_STRING_PTR, &foo, 0 /*irrelevant*/, 0 },
        { "bar", Otls_PARAM_UTF8_STRING, &bar, sizeof(bar), 0 },
        { NULL, 0, NULL, 0, NULL }
    };
...
L<opentls-core.h(7)>, L<Otls_PARAM_get_int(3)>

=head1 HISTORY

B<Otls_PARAM> was added in Opentls 3.0.
