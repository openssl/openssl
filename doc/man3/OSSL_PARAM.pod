...
The C<flags> field contains flags as described in L</Flags> below.

=item C<reserved>

This field is reserved for future use and should be zero.
...
The C<flags> field contains the following flags:

=over 4

=item C<OSSL_PARAM_LOCATED>

This flag is used, by the receiver, to indicate if the parameter has been
looked up by name successfully or not.

=item C<OSSL_PARAM_READ>

This flag is used, by the receiver, to indicate if the value of the parameter
has been read.
