...
should be ignored.
If C<data_size> may be zero, it means that an arbitrary data size is
...
Functions returning a constant C<OSSL_PARAM> array as a parameter
descriptor may set C<data> and C<return_size> for their own purposes.
And example would be to have C<data> point at handler functions for
each parameter, which would be used by a I<setter> or I<responder> of
