...
When dealing with B<BIGNUM>s, OpenSSL currently processes paramaters
as the B<OSSL_PARAM_UNSIGNED_INTEGER> type.  The means that negative
values are not permitted.  Third party providers need not be thus
restricted.
