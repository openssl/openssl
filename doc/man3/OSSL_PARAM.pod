...
The difference between this and C<OSSL_PARAM_UTF8_STRING> is that C<buffer>
...
C<buffer_size> will be set to the size of the data, not the size of
the pointer to the data.
If this is used in a parameter request,
C<buffer_size> is not relevant.  However, the I<responder> will set
...
The difference between this and C<OSSL_PARAM_OCTET_STRING> is that C<buffer>
doesn't point directly at the data, but to a pointer that points to the data.
