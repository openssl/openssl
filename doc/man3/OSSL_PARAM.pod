...
The C<used> field contains a value indicating how the parameter was accessed
by thye receiver.  The possible values are described in L</Usages> below.
...
The organization of the data depends on the parameter type.
...
The C<used> field can contain one of the following values:

=over 4

=item C<OSSL_PARAM_UNTOUCHED>

The parameter was not accessed or searched for.

=item C<OSSL_PARAM_LOCATED>

The parameter was searched for but neither read from nor written to.

=item C<OSSL_PARAM_READ>

The parameter was read but not written to.

=item C<OSSL_PARAM_WRITTEN>

The parameter was written to, it could have been read in addition.
