...
ASN1_TIME_get, ASN1_UTCTIME_get, ASN1_GENERALIZEDTIME_get,
...
ASN1_TIME_set_string_gmt, ASN1_UTCTIME_set_string_gmt, ASN1_GENERALIZEDTIME_set_string_gmt,
ASN1_TIME_print, ASN1_UTCTIME_print, ASN1_GENERALIZEDTIME_print,
ASN1_TIME_print_gmt, ASN1_UTCTIME_print_gmt, ASN1_GENERALIZEDTIME_print_gmt,
ASN1_TIME_diff, ASN1_UTCTIME_diff, ASN1_GENERALIZEDTIME_diff
ASN1_TIME_cmp_time_t, ASN1_UTCTIME_cmp_time_t, ASN1_GENERLIZEDTIME_cmp_time_t,
ASN1_TIME_to_utctime, ASN1_TIME_to_generalizedtime - ASN.1 Time functions
...
=head1 SYNOPSIS
 ...
 int ASN1_TIME_get(const ASN1_TIME *s, time_t *t, struct tm *ptm);
 int ASN1_UTCTIME_get(const ASN1_UTCTIME *s, time_t *t, struct tm *ptm);
 int ASN1_GENERALIZEDTIME_get(const ASN1_GENERALIZEDTIME *s, time_t *t,
                              struct tm *ptm);
 ...
 int ASN1_GENERALIZEDTIME_set_string_gmt(ASN1_GENERALIZEDTIME *s,
                                         const char *str);
 ...
 int ASN1_TIME_print_gmt(BIO *b, const ASN1_TIME *s);
 int ASN1_UTCTIME_print_gmt(BIO *b, const ASN1_UTCTIME *s);
 int ASN1_GENERALIZEDTIME_print_gmt(BIO *b, const ASN1_GENERALIZEDTIME *s);

 int ASN1_TIME_diff(int *pday, int *psec, const ASN1_TIME *from,
                    const ASN1_TIME *to);
 int ASN1_UTCTIME_diff(int *pday, int *psec, const ASN1_UTCTIME *from,
                       const ASN1_TIME *to);
 int ASN1_GENERALIZEDTIME_diff(int *pday, int *psec,
                               const ASN1_GENERALIZEDTIME *from,
                               const ASN1_GENERLIZEDTIME *to);

 int ASN1_TIME_cmp_time_t(const ASN1_TIME *s, time_t t);
 int ASN1_UTCTIME_cmp_time_t(const ASN1_UTCTIME *s, time_t t);
 int ASN1_GENERLIZEDTIME_cmp_time_t(const ASN1_GENERALIZEDTIME *s, time_t t);
...
The ASN1_TIME_get(), ASN1_UTCTIME_get() and ASN1_GENERALIZEDTIME_get()
functions retrieve the time_t B<t> or struct tm B<tm> value from the
time structure B<s>. Either B<t> or B<tm> may be NULL. Only the
tm_year, tm_mon, tm_mday, tm_hour, tm_min and tm_sec fields are set
in B<tm>. The other fields are cleared.
...
time format (for example YYMMDDHHMMSSZ or YYYYMMDDHHMMSSZ). The string B<str>
is copied into B<s>.

The ASN1_TIME_set_string_gmt(), ASN1_UTCTIME_set_string_gmt() and
ASN1_GENERALIZEDTIME_set_stringgmt_() functions set the time structure B<s>
to the time represented by string B<str> which must be in appropriate ASN.1
time format (for example YYMMDDHHMMSSZ or YYYYMMDDHHMMSSZ). The string value
B<str> is converted to GMT before it is copined into B<s>

The ASN1_TIME_check(), ASN1_UTCTIME_check() and ASN1_GENERALIZEDTIME_check()
functions check the syntax of the tim structure B<s>.
...
The ASN1_TIME_print_gmt(), ASN1_UTCTIME_print_gmt() and ASN1_GENERALIZEDTIME_print_gmt()
functions print the time structure B<s> to BIO B<b> in human readable
format. It will be of the format MMM DD HH:MM:SS YYYY GMT, for example
"Feb  3 00:55:52 2015 GMT" it does not include a newline. If the time
structure has invalid format it prints out "Bad time value" and returns
...
The ASN1_TIME_diff(), ASN1_UTCTIME_diff() and ASN1_GENERALIZEDTIME_diff()
functions set B<*pday> and B<*psec> to the time difference between
...
The ASN1_TIME_cmp_time_t(), ASN1_UTCTIME_cmp_time_t() and ASN1_GENERALIZEDTIME_cmp_time_t()
functions compare the two times represented by the time structure B<s> and the time_t B<t>.

The ASN1_TIME_to_utctime() and ASN1_TIME_to_generalizedtime() functions convert
an ASN1_TIME to an ASN1_UTCTIME or ASN1_GENERALIZEDTIME, respectively. If either B<out> or
...
ASN1_TIME_print(), ASN1_UTCTIME_print() and ASN1_GENERALIZEDTIME_print() currently
do not print out the time zone: it either prints out "GMT" or nothing. But all
certificates complying with RFC5280 et al use GMT anyway. Use the ASN1_TIME_print_gmt(),
ASN1_UTCTIME_print_gmt() and ASN1_GENERALIZEDTIME_print_gmt() functions to convert
and print as GMT instead.
...
ASN1_TIME_get(), ASN1_UTCTIME_get() and ASN1_GENERALIZEDTIME_get() return 1 on
success, and 0 if an error occurred. The time is returned in the B<t> and/or
B<tm> parameters.

ASN1_TIME_set_string(), ASN1_UTCTIME_set_string() and ASN1_GENERALIZEDTIME_set_string()
return 1 if the time value is successfully set and 0 otherwise.

ASN1_TIME_set_string_gmt(), ASN1_UTCTIME_set_string_gmt() and ASN1_GENERALIZEDTIME_set_string_gmt()
return 1 if the time value is successfully set and 0 otherwise.
...
ASN1_TIME_print_gmt(), ASN1_UTCTIME_print_gmt() and ASN1_GENERALIZEDTIME_print_gmt() return 1
if the time is successfully printed out and 0 if an error occurred (I/O error or
invalid time format).

ASN1_TIME_diff(), ASN1_UTCTIME_diff() and ASN1_GENERALIZEDTIME_diff() return 1
for success and 0 for failure. It can fail if the passed-in time structure has
invalid syntax for example.

ASN1_TIME_cmp_time_t(), ASN1_UTCTIME_cmp_time_t() and ASN1_GENERALIZEDTIME_cmp_time_t()
return -1 if B<s> is before B<t>, 0 if B<s> equals B<t>, or 1 if B<s> is after B<t>.
-2 is returned on error.

ASN1_TIME_to_utctime() and ASN1_TIME_to_generalizedtime() return a pointer to
