...
I<attr> attributes NID is used to set an B<ANS1_STRING> using
...
attributes existing list of objects, which could be a issue if the attributes
...
the B<ANS1_TYPE> object at index I<idx> OR if I<atrtype> is either
B<V_ASN1_BOOLEAN> or B<V_ASN1_NULL> OR if the I<idx> is not in the
range 0 to X509_ATTRIBUTE_count() - 1.

X509at_get0_data_by_OBJ() finds the first attribute in a list I<x> that matches
the I<obj> starting at index I<lastpos> and returns the data retrieved from
the found attributes first B<ASN1_TYPE> object. An error will occur if the
attribute type I<type> does not match the type of the B<ANS1_TYPE> object OR
if I<type> is either B<V_ASN1_BOOLEAN> or B<V_ASN1_NULL> OR
the attribute is not found.
If I<lastpos> is smaller than -1 then an error will occur if there is multiple
objects in the list I<x> that match I<obj>.
If I<lastpos> is smaller than -2 then an error will occur if there is more than
...
X509at_get_attr() returns either a B<X509_ATTRIBUTE> or NULL if there is an error.
...
a B<X509_ATTRIBUTE> on success, or NULL if there is an error.
