...
X509_STORE_CTX_get1_objs_by_auth
...
X509_STORE_CTX_get1_objs_by_auth() finds all certificates in the store that
match the given subject I<name> and, if provided, the authority key ID I<akid>.
If both I<name> and I<akid> are supplied, the function requires both values
to match. The authority key ID is matched against the subject key ID of the
certificates in the store. Certificates are matched in the order they
are stored.
...
X509_STORE_CTX_get1_objs_by_auth() returns a new stack with pointers to all
matching certificates in the store. The caller must free the returned stack.
If no certificate matches the given subject, an empty stack is returned.
...
X509_STORE_CTX_get1_objs_by_auth() was added in OpenSSL 3.5.0.
