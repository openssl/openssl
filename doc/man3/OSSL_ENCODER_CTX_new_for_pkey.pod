...
Note that OSSL_ENCODER_CTX_new_for_pkey() finalizes the OSSL_ENCODER_CTX; after
that most of the B<OSSL_ENCODER_CTX_set_*()> and B<OSSL_ENCODER_CTX_add_*()>
functions described in L<OSSL_ENCODER_CTX(3)> are no longer available (return
an error if called) as described in L<OSSL_ENCODER_CTX_set_finalized(3)>.
L<OSSL_ENCODER_CTX_set_params(3)>, however, is still available to set encoder
parameters.
...
L<OSSL_ENCODER_CTX_set_finalized(3)>,
