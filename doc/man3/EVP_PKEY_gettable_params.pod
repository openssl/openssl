...
EVP_PKEY_get_int_param, EVP_PKEY_get_size_t_param, EVP_PKEY_get_uint64_param,
...
EVP_PKEY_get_size_t_param(), EVP_PKEY_get_uint64_param() or
EVP_PKEY_get_bn_param(). Attempting to obtain an integer value that does not fit
into a native C B<int> type will cause EVP_PKEY_get_int_param() to fail.
Similarly attempting to obtain an integer value that is negative or does not fit
into a native C B<size_t> type using EVP_PKEY_get_size_t_param() will also fail.
...
EVP_PKEY_get_uint64_param() retrieves a key I<pkey> uint64 value I<*out>
associated with a name of I<key_name> if it fits into C<uint64> type. For
parameters that do not fit into C<uint64> use EVP_PKEY_get_bn_param().
...
Support for EVP_PKEY_get related functions was added in OpenSSL 3.0.
The function EVP_PKEY_get_uint64_param() was added in OpenSSL 3.5.0.
