...
RAND_DRBG_reseed()
reseeds the given B<drbg>, obtaining entropy input from its parent drbg
(if it is chained) or from the trusted os entropy sources,
and mixing in the specified additional data provided
in the buffer B<adin> of length B<adinlen>.
The additional data can be omitted by setting B<adin> to NULL and B<adinlen> to 0.
...
The ensure that they are applied to the global DRBG instances
(<master>, <public>, and <private>), it is necessary to call
RAND_DRBG_set_reseed_defaults() before calling any
