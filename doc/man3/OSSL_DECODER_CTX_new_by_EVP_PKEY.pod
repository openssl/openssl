...
L<OSSL_DECODER_from_bio(3)>.  The caller may used the optional I<input_type>,
...
NULL and zero are valid and signifies that the decoder implementations will
...
The known input types that OpenSSL decoder implementations offer for
B<EVP_PKEY>s are:
...
PEM is the term used for encoding of binary content, first defined in IETF
RFC 1421.  The content is a series of base64-encoded lines, surrounded by
begin/end markers each on their own line.  For example:
...
The known input structures that OpenSSL decoder implementations offer for
B<EVP_PKEY>s are:
...
This is the structure used for keys encoded according to key type specific
specifications.  For example, RSA keys encoded according to PKCS#1.

=item "pkcs8"

For keys wrapped in a PKCS#8 structure.  Both encrypted and unencrypted
encodings are supported.  This only supports private keys.
