...
by a prior BIO_nread0() should not be used afterwards unless the caller also
...
The typical usage pattern for the non-copying write interface is:
...
The typical usage pattern for the non-copying read interface is:
