...
One typical use of BIO pairs is to place TLS/tls I/O under application control, this
can be used when the application wishes to use a non standard transport for
TLS/tls or the normal socket routines are inappropriate.
...
in TLS/tls applications where the amount of data read is usually
...
freed due to a BIO_free_all() or tls_free() call the other half needs to be freed.

When used in bidirectional applications (such as TLS/tls) care should be taken to
...
TLS/tls handshake for example. BIO_write_ex() will succeed and place data in the
...
without having to go through the tls-interface.

 BIO *internal_bio, *network_bio;

 ...
 BIO_new_bio_pair(&internal_bio, 0, &network_bio, 0);
 tls_set_bio(tls, internal_bio, internal_bio);
 tls_operations(); /* e.g tls_read and tls_write */
 ...
 ...
 application |   TLS-engine
  ...
  ...
  tls_free(tls);                /* implicitly frees internal_bio */
...
tls_operation() can successfully be continued.

=head1 WARNINGS

As the data is buffered, tls_operation() may return with an ERROR_tls_WANT_READ
condition, but there is still data in the write buffer. An application must
not rely on the error value of tls_operation() but must assure that the
