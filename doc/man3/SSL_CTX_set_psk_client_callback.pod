...
tls_psk_client_cb_func,
tls_psk_use_session_cb_func,
tls_CTX_set_psk_client_callback,
tls_set_psk_client_callback,
tls_CTX_set_psk_use_session_callback,
tls_set_psk_use_session_callback
...
tls_CTX_set_psk_use_session_callback() or tls_set_psk_use_session_callback() as
appropriate. These functions cannot be used for TLSv1.2 and below PSKs.

The callback function is given a pointer to the tls connection in B<tls>.
...
Additionally the callback should store a pointer to an tls_SESSION object in
...
This can be set via a call to L<tls_SESSION_set1_master_key(3)>.
...
The ciphersuite can be set via a call to <tls_SESSION_set_cipher(3)>. The
handshake digest of an tls_CIPHER object can be checked using
<tls_CIPHER_get_handshake_digest(3)>.

=item The protocol version

This can be set via a call to L<tls_SESSION_set_protocol_version(3)> and should
be TLS1_3_VERSION.

=back

Additionally the maximum early data value should be set via a call to
L<tls_SESSION_set_max_early_data(3)> if the PSK will be used for sending early
data.

Alternatively an tls_SESSION created from a previous non-PSK handshake may also
be used as the basis for a PSK.

Ownership of the tls_SESSION object is passed to the Opentls library and so it
...
The callback is set using functions tls_CTX_set_psk_client_callback()
or tls_set_psk_client_callback(). The callback function is given the
connection in parameter B<tls>, a B<NULL>-terminated PSK identity hint
...
recommended to use tls_CTX_set_psk_use_session_callback()
or tls_set_psk_use_session_callback() for this purpose instead. If TLSv1.3 has
been negotiated then Opentls will first check to see if a callback has been set
via tls_CTX_set_psk_use_session_callback() or tls_set_psk_use_session_callback()
and it will use that in preference. If no such callback is present then it will
check to see if a callback has been set via tls_CTX_set_psk_client_callback() or
tls_set_psk_client_callback() and use that. In this case the B<hint> value will
...
Return values from the B<tls_psk_client_cb_func> callback are interpreted as
follows:
...
The tls_psk_use_session_cb_func callback should return 1 on success or 0 on
failure. In the event of failure the connection setup fails.

=head1 SEE ALSO

L<tls(7)>,
L<tls_CTX_set_psk_find_session_callback(3)>,
L<tls_set_psk_find_session_callback(3)>

=head1 HISTORY

tls_CTX_set_psk_use_session_callback() and tls_set_psk_use_session_callback()
