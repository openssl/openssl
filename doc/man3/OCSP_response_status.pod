...
OCSP_basic_sign() signs OCSP response brsp using certificate signer, private key
key, digest dgst and additional certificates certs. If the flags option
OCSP_NOCERTS is set then no certificates will be included in the request. If the
flags option OCSP_RESPID_KEY is set then the responder is identified by key ID
rather than by name. OCSP_basic_sign_ctx() also signs OCSP response brsp but
