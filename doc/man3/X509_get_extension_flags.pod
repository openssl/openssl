...
certificate B<x>, overriding any path length from the certificate's
B<proxyCertInfo> extension.  This is for the users who need to mark
...
given certificate B<x> if it is a proxy certificate.  A value set with
X509_set_proxy_pathlen() takes precedence over one from the certificate's
B<proxyCertInfo> extension.
...
These functions read cached information that is populated only when the
certificate is finalized, that is parsed or signed; see L<x509(7)>. They are
meaningful only for a finalized certificate and do not reflect changes made to
a certificate after it was finalized.
