...
The certificate is finalized: its cached extension information has been
computed. See L<x509(7)>.
...
No longer set. Before OpenSSL 4.2 this bit was set on a certificate whose
policy extensions were found invalid or inconsistent during a certificate
verification with policy checking enabled; an invalid policy extension is
still reported by the verification, as B<X509_V_ERR_INVALID_POLICY_EXTENSION>.
...
in the B<X509> structure when the certificate is decoded or signed; see
L<x509(7)>. If the flags returned do not provide sufficient
...
certificate extension values. X509_get_key_usage() and
X509_get_extended_key_usage() return 0 if I<x> is not finalized (see
L<x509(7)>) or has an invalid extension; X509_get_extension_flags() then
returns flags without B<EXFLAG_SET> or with B<EXFLAG_INVALID> respectively.
