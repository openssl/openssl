...
X509_CRL_get0_authority_key_id - retrieve certificate and CRL extension data
...
X509_CRL_get0_authority_key_id() returns an internal pointer to the authority
key identifier of B<crl> as an B<ASN1_OCTET_STRING> or B<NULL> if the extension
is not present. The key is cached when the object is loaded from data using
ASN1 methods, like L<d2i_X509_CRL_bio(3)>.
...
X509_CRL_get0_authority_key_id() was added in OpenSSL 3.5.0.
