...
handshake to be completed (for example by using SSL_accept()).
...
L<SSL_new_listener(3)>, L<SSL_CTX_set_cookie_generate_cb(3)>,
L<SSL_CTX_set_cookie_verify_cb(3)>,
...
DTLSv1_listen() was restricted to DTLS 1.0 and DTLS 1.2 only in OpenSSL 4.1
DTLS 1.3 applications should use L<SSL_new_listener(3)> instead.
