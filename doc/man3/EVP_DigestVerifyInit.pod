...
EVP_DigestVerifyFinal, EVP_DigestVerify,
EVP_DigestVerifyPQInit, EVP_DigestVerifyPQUpdate, EVP_DigestVerifyPQFinal
...
=head1 SYNOPSIS
 ...
 #define EVP_DigestVerifyPQUpdate EVP_DigestVerifyUpdate
...
The digest is optional, If the value is set then it must mach the digest used
by the HSS public key and HSS signature, otherwise an error will occur.

Supports SHA256, SHA256-192, SHAKE-256
...
EVP_DigestVerifyPQInit() is similiar to EVP_DigestVerifyInit_ex() except that
the signature I<sig> of size I<siglen> is also supplied. This can be used for
streaming Hash-Based Signature Schemes such as L<EVP_PKEY-HSS(7)> that require
the signature to be set before calling EVP_DigestVerifyUpdate().

EVP_DigestVerifyPQUpdate() is an alias for EVP_DigestVerifyUpdate() that can be
called after EVP_DigestVerifyPQInit().

EVP_DigestVerifyPQFinal() is similar to EVP_DigestVerifyFinal() except that it
does not pass the signature (since it was alreay passed previously by calling
EVP_DigestVerifyPQInit()).

=head1 RETURN VALUES

EVP_DigestVerifyInit(), EVP_DigestVerifyPQInit() and EVP_DigestVerifyUpdate()
return 1 for success and 0 for failure.

EVP_DigestVerifyFinal(), EVP_DigestVerifyPQFinal() and EVP_DigestVerify()
return 1 for success; any other value indicates failure. A return value of zero
indicates that the signature did not verify successfully (that is, B<tbs> did
not match the original data or the signature had an invalid form), while other
...
EVP_DigestVerifyPQInit(), EVP_DigestVerifyPQUpdate() and
EVP_DigestVerifyPQFinal() were added in OpenSSL 3.1.
