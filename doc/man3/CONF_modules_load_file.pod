...
then the filename from the environment variable B<OPENSSL_CONF> is
used, or the standard OpenSSL configuration file is used if the
environment variable us unavailable. If B<appname> is NULL the
standard OpenSSL application name B<openssl_conf> is used. The
behaviour can be customized using B<flags>.
...
L<config(5)>, L<OPENSSL_config(3)>, L<openssl-environment(7)>
