...
B<CONF_MFLAGS_SILENT_ACTIVATION>.
Suppress reporting of provider activation failure as an initialisation failure,
even when B<config_diagnostics> are enabled or B<CONF_MFLAGS_IGNORE_RETURN_CODES>
is not set (normally sufficient to do the same unless diagnostics are enabled).
This is not a default flag.
