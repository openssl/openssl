...
CONF_modules_load_file, CONF_modules_load_ex, CONF_modules_load,
CONF_module_add_ex, CONF_module_add - OpenSSL configuration functions
...
=head1 SYNOPSIS
 ...
 int CONF_modules_load_file(const char *filename, const char *appname,
                            unsigned long flags);
 int CONF_modules_load_ex(OSSL_LIB_CTX *libctx, const CONF *cnf, const char *appname,
                       unsigned long flags);
 int CONF_modules_load(const CONF *cnf, const char *appname,
                       unsigned long flags);
 int CONF_module_add_ex(OSSL_LIB_CTX *libctx, const char *name,
...
CONF_modules_load_ex() is identical to CONF_modules_load_file() except it
reads configuration information from B<cnf>.

CONF_modules_load() is identical to CONF_modules_load_ex() except it
has a NULL library context.

CONF_module_add_ex() registers a module into a library context using the given
name, initialization and finish functions.

CONF_module_add() is identical to CONF_modules_add_Ex() except it
has a NULL library context.
...
Applications can use the CONF_modules_load_ex() function if they wish to load a
...
The CONF_modules_load_ex(), CONF_modules_load_file_ex() and CONF_module_add_ex()
functions were introduced in OpenSSL 3.1. Prior to this version, module state
was maintained independently of library contexts.
