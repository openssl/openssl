...
SSL_SESSION_CACHE_set_mode, SSL_SESSION_CACHE_get_mode - enable/disable session caching
...
=head1 SYNOPSIS
 ...
 long SSL_SESSION_CACHE_set_mode(SSL_SESSION_CACHE *c, long mode);
 long SSL_SESSION_CACHE_get_mode(const SSL_SESSION_CACHE *c);

=head1 DESCRIPTION

SSL_CTX_set_session_cache_mode() enables/disables session caching
by setting the operational mode for B<ctx> to B<mode>.

SSL_SESSION_CACHE_set_mode() enables/disables session caching
by setting the operational mode for B<cache> to B<mode>.

SSL_CTX_get_session_cache_mode() and SSL_SESSION_CACHE_get_mode() return
...
SSL_CTX_set_session_cache_mode() and SSL_SESSION_CACHE_set_mode() return
the previously set cache mode.

SSL_CTX_get_session_cache_mode() and SSL_SESION_CACHE_get_mode() return
the currently set cache mode.

=head1 HISTORY

The SSL_SESSION_CACHE_set_mode() and SSL__SESSION_CACHE_get_mode()
