...
The EVP_SKEY_generate() functions creates a new B<EVP_SKEY> object
initialized it according to the B<params> argument.
...
EVP_SKEY_generate(), EVP_SKEY_import() and EVP_SKEY_new_raw_key() return either
the newly allocated B<EVP_SKEY> structure or NULL if an error occurred.
