...
EVP_SKEY_export, EVP_SKEY_get_raw_key,
...
which is used by OpenSSL to store public and private keys, assigns the
B<EVP_KEYMGMT> object associated with the key, and initializes the object from
the B<params> argument.

The EVP_SKEY_import_raw_key() function s a helper that creates an B<EVP_SKEY> object
containing the raw byte representation ot the symmetric keys.
...
and sets the real key len. The operation can fail when the underlying key
...
EVP_SKEY_import() and EVP_SKEY_import_raw_key() return either the newly
...
EVP_SKEY_import_raw_key(), EVP_SKEY_up_ref(), EVP_SKEY_is_a(),
EVP_SKEY_to_provider() were introduced in OpenSSL 3.5.
