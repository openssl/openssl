...
of algorithms, and B<length> is size of the B<algs> array. Only those algorithms
enabled in the library will be accepted in B<algs>, unknown algorithms in B<algs>
are ignored. On an error, the preference order is left unmodified.
...
when compressing the certificate.

SSL_CTX_compress_certs() and SSL_compress_certs() are used to pre-compress all
the configured certificates on an SSL_CTX (or SSL) with algorithm B<alg>. If
B<alg> is 0, then the certificates are compressed with every configured
algorithm in the library. Calling these functions on a client SSL_CTX/SSL will
result in an error, as only server certificates may be pre-compressed.

SSL_CTX_get1_compressed_cert() and SSL_get1_compressed_cert() are used to 
get the pre-compressed certificate most recently set that may be stored for
later use. Calling these functions on a client
SSL_CTX/SSL will result in an error, as only server certificates may be
pre-compressed. The B<data> and B<orig_len> arguments are required.
...
(except SSL_CTX_set1_cert_comp_preferences()/SSL_set1_cert_comp_preferences())
on a client SSL_CTX/SSL will return an error. Client certificates are compressed
on-demand as unique context data from the server is compressed along with the
certificate.
...
SSL_get1_compressed_cert() is the last certificate set on the SSL_CTX/SSL.
The certificate memory is allocated by the function and the caller must
free B<*data> via OPENSSL_free().

The compressed certificate data set by SSL_CTX_set1_compressed_cert() and
SSL_set1_compressed_cert() is copied into the SSL_CTX/SSL.
