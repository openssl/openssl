...
used by EVP_get_cipherbyname() and an initialisation vector used by the
...
initialisation vector is passed to EVP_BytesToKey() as the I<salt>
parameter. Internally, B<PKCS5_SALT_LEN> bytes of the salt are used
(regardless of the size of the initialisation vector). The user's
password is passed to EVP_BytesToKey() using the I<data> and I<datal>
parameters. Finally, the library uses an iteration count of 1 for
EVP_BytesToKey().

The I<key> derived by EVP_BytesToKey() along with the original initialisation
vector is then used to decrypt the encrypted data. The I<iv> produced by
EVP_BytesToKey() is not utilised or needed, and NULL should be passed to
the function.
...
The pseudo code to derive the key would look similar to:
 ...
 /* On success, use key and iv to initialise the cipher */
