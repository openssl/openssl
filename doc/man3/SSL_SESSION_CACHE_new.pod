...
Adding the B<SSL_SESSION_CACHE> to a B<SSL_CTX> via SSL_CTX_new_ex() increments the
...
In the following example (error handling eliminated):
 ...
 SSL_CTX ctx2 = SSL_CTX_new_ex(TLS_method(), cache);
...
SSL_SESSION_CACHE_up_ref() returns 1 for success and 0 for failure.
...
and SSL_SESSION_CACHE_get1_session()
