...
tls_check_chain - check certificate chain suitability
...
tls_check_chain() checks whether certificate B<x>, private key B<pk> and
certificate chain B<chain> is suitable for use with the current session
B<s>.

=head1 RETURN VALUES

tls_check_chain() returns a bitmap of flags indicating the validity of the
chain.
...
tls_check_chain() must be called in servers after a client hello message or in
