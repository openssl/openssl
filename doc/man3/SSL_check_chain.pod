...
B<CERT_PKEY_SUITEB>: chain is suitable for Suite B use.  Suite B support is
deprecated and this flag is only set if OpenSSL was configured with
C<enable-deprecated-suiteb>.
