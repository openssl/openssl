...
OSSL_ENCODER_new_by_EVP_PKEY,
OSSL_ENCODER_set_cipher,
OSSL_ENCODER_set_passphrase,
OSSL_ENCODER_set_pem_password_cb,
OSSL_ENCODER_set_passphrase_cb,
OSSL_ENCODER_set_passphrase_ui
...
=head1 SYNOPSIS
 ...
 OSSL_ENCODER *
 OSSL_ENCODER_new_by_EVP_PKEY(const EVP_PKEY *pkey, int selection,
...
OSSL_ENCODER_new_by_EVP_PKEY() is a utility function that creates a
B<OSSL_ENCODER>, finds all applicable encoder implementations and sets
...
Internally, OSSL_ENCODER_new_by_EVP_PKEY() uses the names from the
...
OSSL_ENCODER_new_by_EVP_PKEY() still creates a B<OSSL_ENCODER>, but
with no associated methods (L<OSSL_ENCODER_get_num_methods(3)> returns
...
OSSL_ENCODER_set_cipher() tells the implementation what cipher
...
OSSL_ENCODER_set_passphrase() gives the implementation a
pass phrase to use when encrypting the encoded private key.
Alternatively, a pass phrase callback may be specified with the
following functions.

OSSL_ENCODER_set_pem_password_cb(), OSSL_ENCODER_set_passphrase_ui()
and OSSL_ENCODER_set_passphrase_cb() sets up a callback method that the
...
OSSL_ENCODER_new_by_EVP_PKEY() returns a pointer to a
B<OSSL_ENCODER>, or NULL if it couldn't be created.

OSSL_ENCODER_set_cipher(), OSSL_ENCODER_set_passphrase(),
OSSL_ENCODER_set_pem_password_cb(), OSSL_ENCODER_set_passphrase_ui()
and OSSL_ENCODER_set_passphrase_cb() all return 1 on success, or 0 on
failure.
...
B<EVP_PKEY> in OSSL_ENCODER_new_by_EVP_PKEY() matches the type name,
thus making for the naming pattern B<OSSL_ENCODER_new_by_I<TYPE>>() when
new types are handled.

=head1 SEE ALSO

L<provider(7)>, L<OSSL_ENCODER(3)>, L<OSSL_ENCODER_METHOD(3)>
