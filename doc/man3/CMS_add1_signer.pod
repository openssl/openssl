...
Note that in the case of no-signedAttributes that for some hash-less signing
schemes the given hash B<md> will be set to a hash required by its specification,
independent of what is provided. This is the case for ML-DSA and EdDSA (RFC 8419).
For SLH-DSA (RFC 9814), the suggested hash will only be used if none is
provided.
