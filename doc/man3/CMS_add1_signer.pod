...
structure is not complete and must be finalised either by streaming (if
...
The returned CMS_ContentInfo structure will be valid and finalised when this
flag is set.

If B<CMS_PARTIAL> is set in addition to B<CMS_REUSE_DIGEST> then the
CMS_SignerInfo structure will not be finalised so additional attributes
