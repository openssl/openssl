...
L<BIO_make_pair(3)> may be used to join two datagram pair BIOs into a pair. The
two BIOs must both use the method returned by BIO_s_dgram_pair() and neither of
the BIOs may currently be associated in a pair.

L<BIO_destroy_pair(3)> destroys the association between two connected BIOs.
...
by multiple threads simultaneously on the same BIO datagram pair. Making any
other BIO call, or any other L<BIO_ctrl(3)> operation, on either half of a BIO
datagram pair while any other BIO call is also in progress to either half of the
same BIO datagram pair results in undefined behaviour.
...
L<BIO_writemmsg(3)>) and the peer has not advertised the
