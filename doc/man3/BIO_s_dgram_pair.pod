...
Freeing either half of the pair will automatically destroy the association. Once
the association is destroyed, the remaining BIO reports EOF, fails any read or
write, and may be joined into a new pair.
...
As an exception, one half of a BIO datagram pair may be freed while the calls
listed above are in progress on the other half. Those calls then fail as if the
