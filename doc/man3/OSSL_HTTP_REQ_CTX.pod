...
OSSL_HTTP_REQ_CTX_get0_server returns the server hostname or IP address in I<rctx>,
or NULL if unset.
...
OSSL_HTTP_REQ_CTX_get0_server() returns string, which must not be freed, or NULL.
...
OSSL_HTTP_REQ_CTX_get0_server() was added in OpenSSL 4.0.
