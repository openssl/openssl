...
tls_CTX_set1_sigalgs, tls_set1_sigalgs, tls_CTX_set1_sigalgs_list,
tls_set1_sigalgs_list, tls_CTX_set1_client_sigalgs,
tls_set1_client_sigalgs, tls_CTX_set1_client_sigalgs_list,
tls_set1_client_sigalgs_list - set supported signature algorithms

=head1 SYNOPSIS

 #include <opentls/tls.h>

 long tls_CTX_set1_sigalgs(tls_CTX *ctx, const int *slist, long slistlen);
 long tls_set1_sigalgs(tls *tls, const int *slist, long slistlen);
 long tls_CTX_set1_sigalgs_list(tls_CTX *ctx, const char *str);
 long tls_set1_sigalgs_list(tls *tls, const char *str);

 long tls_CTX_set1_client_sigalgs(tls_CTX *ctx, const int *slist, long slistlen);
 long tls_set1_client_sigalgs(tls *tls, const int *slist, long slistlen);
 long tls_CTX_set1_client_sigalgs_list(tls_CTX *ctx, const char *str);
 long tls_set1_client_sigalgs_list(tls *tls, const char *str);

=head1 DESCRIPTION

tls_CTX_set1_sigalgs() and tls_set1_sigalgs() set the supported signature
algorithms for B<ctx> or B<tls>. The array B<slist> of length B<slistlen>
must consist of pairs of NIDs corresponding to digest and public key
algorithms.

tls_CTX_set1_sigalgs_list() and tls_set1_sigalgs_list() set the supported
signature algorithms for B<ctx> or B<tls>. The B<str> parameter
...
tls_CTX_set1_client_sigalgs(), tls_set1_client_sigalgs(),
tls_CTX_set1_client_sigalgs_list() and tls_set1_client_sigalgs_list() set
signature algorithms related to client authentication, otherwise they are
identical to tls_CTX_set1_sigalgs(), tls_set1_sigalgs(),
tls_CTX_set1_sigalgs_list() and tls_set1_sigalgs_list().
...
and (if server preferences are set with tls_OP_CIPHER_SERVER_PREFERENCE)
their order.
...
using an array:
 ...
 tls_CTX_set1_sigalgs(ctx, slist, 4);

Set supported signature algorithms to SHA256 with ECDSA and SHA256 with RSA
using a string:

 tls_CTX_set1_sigalgs_list(ctx, "ECDSA+SHA256:RSA+SHA256");

=head1 SEE ALSO

L<tls(7)>, L<tls_get_shared_sigalgs(3)>,
