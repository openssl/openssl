...
OPENtls_ia32cap - the x86[_64] processor capabilities vector

=head1 SYNOPSIS

 env OPENtls_ia32cap=... <application>

=head1 DESCRIPTION

Opentls supports a range of x86[_64] instruction set extensions. These
...
bank. You might have to do the latter if target Opentls application is
...
was exposed to application through OPENtls_ia32cap_loc(), but not
anymore. Now the only way to affect the capability detection is to set
B<OPENtls_ia32cap> environment variable prior target application start. To
give a specific example, on Intel P4 processor
C<env OPENtls_ia32cap=0x16980010 apps/opentls>, or better yet
C<env OPENtls_ia32cap=~0x1000000 apps/opentls> would achieve the desired
...
setting up B<OPENtls_ia32cap> environment variable. For example assigning
