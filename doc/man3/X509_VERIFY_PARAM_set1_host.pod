...
X509_VERIFY_PARAM_get1_ip_asc() returns the first previously configured expected
...
address previously configured on I<param> via X509_VERIFY_PARAM_set1_ip(),
X509_VERIFY_PARAM_add1_ip(), X509_VERIFY_PARAM_set1_ip_asc(), or
X509_VERIFY_PARAM_add1_ip_asc(), as a freshly allocated ASCII string, or
NULL if I<idx> is out of range.  To iterate over the configured IP addresses,
start with I<idx> = 0 and increment I<idx> until the function returns NULL.
...
L<OPENSSL_free(3)>, or NULL if no such configured value exists
(X509_VERIFY_PARAM_get1_ip_asc_ex() also returns NULL when its index
...
X509_VERIFY_PARAM_get1_ip_asc_ex() was added in OpenSSL 4.2.
