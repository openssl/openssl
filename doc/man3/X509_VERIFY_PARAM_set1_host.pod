...
X509_VERIFY_PARAM_get1_ip_asc, X509_VERIFY_PARAM_get1_ip_asc_ex,
...
=head1 SYNOPSIS
 ...
 char *X509_VERIFY_PARAM_get1_ip_asc_ex(X509_VERIFY_PARAM *param, int n);
...
X509_VERIFY_PARAM_get1_ip_asc() returns the first previously configured
expected IP address from I<param> as a freshly allocated ASCII string
(dotted decimal quad for IPv4, colon-separated hexadecimal for IPv6), or
NULL if no IP address has been set. The caller is responsible for freeing
the returned string with L<OPENSSL_free(3)>.

X509_VERIFY_PARAM_get1_ip_asc_ex() returns the I<n>th expected IP
address that has been set using X509_VERIFY_PARAM_set1_ip(),
X509_VERIFY_PARAM_add1_ip(), X509_VERIFY_PARAM_set1_ip_asc(), or
X509_VERIFY_PARAM_add1_ip_asc() as a freshly allocated ASCII string, or
NULL if no such address has been set.  To obtain all addresses start with
I<n> = 0 and increment I<n> as long as no NULL pointer is returned.
...
X509_VERIFY_PARAM_get1_ip_asc() and X509_VERIFY_PARAM_get1_ip_asc_ex()
return a freshly allocated string that the caller must free with
L<OPENSSL_free(3)>, or NULL if no matching IP address has been configured.
...
X509_VERIFY_PARAM_get1_ip_asc_ex() was added in Openssl 4.1.
