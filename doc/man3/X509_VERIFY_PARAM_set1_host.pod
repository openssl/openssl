...
X509_VERIFY_PARAM_get1_ip_asc_ex() returns a string containing the I<n>th
expected IP address that has been set using X509_VERIFY_PARAM_set1_ip(),
X509_VERIFY_PARAM_add1_ip(), X509_VERIFY_PARAM_set1_ip_asc(), or
X509_VERIFY_PARAM_add1_ip_asc().  To obtain all addresses start with
...
L<OPENSSL_free(3)>, or NULL if no IP address has been configured.
