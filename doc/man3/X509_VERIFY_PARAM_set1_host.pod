...
the hostname configuration and must be set explicitly when commonName
matching is desired.
...
certificate, clearing any previously specified hostname.
...
X509_VERIFY_PARAM_add1_host() are retained.
...
X509_VERIFY_PARAM_add1_host(), or NULL if I<n> is out of range. To iterate
over the configured hostnames, start with I<n> = 0 and increment I<n> until
the function returns NULL. The returned string is owned by the library
...
list is a separate set of reference identifiers from the hostname list set
via X509_VERIFY_PARAM_set1_host(): the hostname list is matched against
...
X509_VERIFY_PARAM_set1_rfc822_input_validation(),
X509_VERIFY_PARAM_set1_smtputf8_input_validation(),
X509_VERIFY_PARAM_set1_cn_input_validation(), and
...
The hostname list is matched only against the certificate's subject
alternative name extension. To verify identities against the certificate's
commonName attribute, use the commonName list configured via
X509_VERIFY_PARAM_set1_cn() and X509_VERIFY_PARAM_add1_cn().
...
X509_VERIFY_PARAM_set1_cn_input_validation() were added in OpenSSL 4.1.
