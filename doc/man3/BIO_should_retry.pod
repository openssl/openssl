...
tls BIOs are the only current exception to this rule: they can request a
retry even if the underlying I/O structure is blocking, if a handshake
occurs during a call to BIO_read(). An application can retry the failed
call immediately or avoid this situation by setting tls_MODE_AUTO_RETRY
on the underlying tls structure.
...
the performance may be poor if tls BIOs are present because long delays
...
The Opentls ASN1 functions cannot gracefully deal with non blocking I/O:
