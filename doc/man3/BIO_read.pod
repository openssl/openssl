...
BIO_puts() returns the number of bytes written on success or various negative
values on error: -1 when written value is larger than INT_MAX, the BIO is
uninitialized, I<size> is less than 0, or I<b> is NULL. -2 might be returned,
when a BIO with an unsupported method is passed.

BIO_get_line() returns the number of bytes placed in buf (excluding
the final NUL terminator) on success. Returns -1 on error (e.g., invalid
arguments, uninitialized BIO). Returns 0 if end-of-file is reached before any
bytes are read.
