...
OSSL_SERIALIZER_CTX_set_cipher_name,
...
OSSL_SERIALIZER_CTX_set_cipher_name() tells the implementation what
cipher should be used to encrypt serialized keys.  If no cipher is
specified, the implementation may still choose to serialize the key,
but with unencrypted result.
