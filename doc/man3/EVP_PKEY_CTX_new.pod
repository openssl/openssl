...
context using the library context I<libctx> (see L<OPENtls_CTX(3)>), the
...
context using the library context I<libctx> (see L<OPENtls_CTX(3)>) and the
...
by the Opentls high level public key API. Contexts B<MUST NOT> be shared between
...
EVP_PKEY_CTX_free() functions were added in Opentls 1.0.0.
