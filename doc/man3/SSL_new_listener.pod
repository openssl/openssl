...
For DTLS listeners, two per-listener tunables control pending-connection
tracking. Both are configured through L<SSL_get_value_uint(3)> /
L<SSL_set_value_uint(3)> using the values
B<SSL_VALUE_DTLS_LISTENER_MAX_PENDING_CONNS> (cap, default 1000, cannot be
disabled) and B<SSL_VALUE_DTLS_LISTENER_PENDING_TIMEOUT> (millisecond reap
timeout, default 30000, B<UINT64_MAX> disables). See L<SSL_get_value_uint(3)>
for details.
...
and B<SSL_VALUE_DTLS_LISTENER_PENDING_TIMEOUT>, accessed via
