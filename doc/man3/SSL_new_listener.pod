...
only be used from a single thread. Single-threaded mode avoids the overhead
of synchronization mechanisms and notifiers.
...
used from multiple threads concurrently. This includes a notifier mechanism
that enables efficient polling across threads - when data arrives for a
connection while another thread is blocked in L<SSL_poll(3)>, that thread
will be woken up.
