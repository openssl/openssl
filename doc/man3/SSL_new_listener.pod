...
in that it polls a listening SSL objects, and, if a new connection is available,
...
result in an error.  Likewise, if a listener has accepted a connection via
...
parameter to B<SSL_listen_ex()> must be created using L<OSSL_QUIC_method(3)>.
