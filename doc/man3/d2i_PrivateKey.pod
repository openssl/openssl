...
d2i_PrivateKey_with_libctx, d2i_PrivateKey,
d2i_PublicKey, d2i_KeyParams,
d2i_AutoPrivateKey_with_libctx,  d2i_AutoPrivateKey,
i2d_PrivateKey, i2d_PublicKey,
i2d_KeyParams, i2d_KeyParams_bio,
d2i_PrivateKey_bio_with_libctx, d2i_PrivateKey_bio,
d2i_PrivateKey_fp_with_libctx, d2i_PrivateKey_fp,
d2i_KeyParams_bio, i2d_PrivateKey_bio,
i2d_PrivateKey_fp
...
d2i_PrivateKey_with_libctx() decodes a private key using algorithm I<type>. It
attempts to use any key specific format or PKCS#8 unencrypted PrivateKeyInfo
format. The I<type> parameter should be a public key algorithm constant such as
...
d2i_PrivateKey() does the same as d2i_PrivateKey_with_libctx() except that the
...
The d2i_PrivateKey_bio_with_libctx() and d2i_PrivateKey_bio() functions are
similar to d2i_PrivateKey_with_libctx() and d2i_PrivateKey() respectively
except that they decode the data read from the given BIO.
The d2i_PrivateKey_fp_with_libctx() and d2i_PrivateKey_fp() functions are the
same except that they read the data from the given FILE.

d2i_AutoPrivateKey_with_libctx() and d2i_AutoPrivateKey() are similar to
d2i_PrivateKey_with_libctx() and d2i_PrivateKey() respectively except that they
...
The d2i_PrivateKey_with_libctx(), d2i_PrivateKey(),
d2i_AutoPrivateKey_with_libctx(), d2i_AutoPrivateKey(),
d2i_PrivateKey_bio_with_libctx(), d2i_PrivateKey_bio(),
d2i_PrivateKey_fp_with_libctx(), d2i_PrivateKey_fp(),
d2i_PublicKey(), d2i_KeyParams() and d2i_KeyParams_bio() functions return a
valid B<EVP_KEY> structure or B<NULL> if an error occurs. The error code can be
...
d2i_PrivateKey_with_libctx(), d2i_PrivateKey_bio_with_libctx(),
d2i_PrivateKey_fp_with_libctx(), and d2i_AutoPrivateKey_with_libctx() were added
in OpenSSL 3.0.
