...
X509_REQ_add1_attr() pushes a copy of the passed in B<X509_ATTRIBUTE> object
to the I<req> objects attribute list. An error will occur if the attribute list
is NULL.
...
to the I<req> objects attribute list. I<req> must be non NULL or an error
...
that is passes a name I<attrname> associated with the object.
...
X509_REQ_get_attr_by_OBJ() returns -1 if either the I<req> objects attribute
...
X509_REQ_get_attr() returns either a B<X509_ATTRIBUTE> or NULL on error.
...
to indicate the ANS.1 encoding has been modified.
