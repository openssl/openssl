...
that C<float> from a unsupported or deleted tuple element to the first
...
Best practice is to make sure you have the correct spelling of the group name
and also prefix it with a C<?> to handle configurations in which it might
nevertheless be unknown.
