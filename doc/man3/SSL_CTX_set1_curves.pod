...
NID_undef is returned. If the NID for the shared group is unknown then value is
set to the bitwise OR of TLSEXT_nid_unknown (0x1000000) and the id of the group.
...
the NID for the shared group is unknown then value is set to the bitwise OR of
