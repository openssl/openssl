...
string B<list>. In contrast to SSL_CTX_set1_groups(), the names of the
groups, rather than their NIDs, are used. The groups are used as in
...
(e.g. B<prime256v1>). Group names are case sensitive.

The B<list> can be used to define several group tuples of comparable security
...
In it's simplest form, the string B<list> is just a colon separated list
...
Group tuples of comparable security are defined by enclosing each tuple in B<{..}> braces.
Keyshares to be sent by a client are specified by prepending a C<*> to the group name,
for example:
{P-521:*P-256}:{*P-384}:{*X25519:P-384:ffdhe2048}.
The general syntax for the string B<list> is:
C<[{] [-][?][*]group1 rep[:[-][?][*]groupN] [}] rep[: {[-][?][*]groupM rep[:[-][?][*]groupP]} ]>
...
will removed from the list of groups if present, ignored otherwise. The "DEFAULT"
group name can be used to select the OpenSSL built-in default list of groups.

With TLS 1.3, on the client side, all the groups are added to the supported groups
extension, in the order in which they are listed, ignoring any braces designating
group tuples.
The extension's preference order, to be evaluated by the server,
is determined by the order of the elements in the array. If a group name is preceded
by C<*>, a key share will be sent for this group. A maximum of 4 key shares are supported.
If no C<*> is used anywhere in the list, a single key share for the leftmost group is sent.

For a TLS 1.3 server, the groups in string B<list> will be used to determine which group
...
a group tuple is determined by the order of the groups in the tuple. Any C<*> will be
ignored by a server. Server preference can be enforced by using
C<SSL_OP_CIPHER_SERVER_PREFERENCE> (default: client preference).
...
Hence, if a client supports a group in a server group tuple, a HRR is triggered if
the client has not also sent a key share for this group. For the example:
{P-521:*P-256}:{*P-384}:{*X25519:P-384:ffdhe2048}
if a client supports C<P-521> but does not send a key share, and it supports
C<P-384> including key share, a HRR will be triggered for C<P-521>
despite the availability of a key share.
