...
With server preference a HRR will be triggered for C<P-521> despite the
availability of a key share for P-384, which overlaps with a lower priority
server-side tuple.

Consider the same server and server preference but different client with I<list>
...
- "A:D:*F": Both prefer "A", but the server did not receive a key share for "A",
so an HRR is triggered for "A".
...
- "F": The server will trigger an HRR for "F" because it did not receive a key
share for "F".
...
- "*E:C:A"
  - Client preference: The server uses the key share for "E".
  - Server preference: The server prefers "A" but did not receive a key share
for "A", so an HRR is triggered for "A".

- "E:*C:A"
  - Client preference: The server uses the key share for "C". Within a tuple,
existing keyshares trump preference order.
  - Server preference: The server prefers "A" but did not receive a key share
for "A", so an HRR is triggered for "A".

- "*E:B:*A"
  - Client preference: The server uses the key share for "E".
  - Server preference: The server uses the key share for "A".
