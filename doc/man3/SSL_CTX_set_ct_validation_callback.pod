...
tls_ct_validation_cb,
tls_enable_ct, tls_CTX_enable_ct, tls_disable_ct, tls_CTX_disable_ct,
tls_set_ct_validation_callback, tls_CTX_set_ct_validation_callback,
tls_ct_is_enabled, tls_CTX_ct_is_enabled -
...
=head1 SYNOPSIS
 ...
 int tls_CTX_set_ct_validation_callback(tls_CTX *ctx,
                                        ...
                                        void *arg);
 void tls_disable_ct(tls *s);
 void tls_CTX_disable_ct(tls_CTX *ctx);
 int tls_ct_is_enabled(const tls *s);
 int tls_CTX_ct_is_enabled(const tls_CTX *ctx);

=head1 DESCRIPTION

tls_enable_ct() and tls_CTX_enable_ct() enable the processing of signed
certificate timestamps (SCTs) either for a given tls connection or for all
connections that share the given tls context, respectively.
...
which can be either of B<tls_CT_VALIDATION_PERMISSIVE> or
B<tls_CT_VALIDATION_STRICT> as described below.

If B<validation_mode> is equal to B<tls_CT_VALIDATION_STRICT>, then in a full
TLS handshake with the verification mode set to B<tls_VERIFY_PEER>, if the peer
presents no valid SCTs the handshake will be aborted.
If the verification mode is B<tls_VERIFY_NONE>, the handshake will continue
...
callback.
Applications can call L<tls_get_verify_result(3)> to check the status at
handshake completion, even after session resumption since the verification
status is part of the saved session state.
See L<tls_set_verify(3)>, <tls_get_verify_result(3)>, L<tls_session_reused(3)>.

If B<validation_mode> is equal to B<tls_CT_VALIDATION_PERMISSIVE>, then the
...
tls_set_ct_validation_callback() and tls_CTX_set_ct_validation_callback()
...
The TLS handshake is aborted if the verification mode is not B<tls_VERIFY_NONE>
...
tls_disable_ct() and tls_CTX_disable_ct() turn off CT processing, whether
enabled via the built-in or the custom callbacks, by setting a NULL callback.
These may be implemented as macros.

tls_ct_is_enabled() and tls_CTX_ct_is_enabled() return 1 if CT processing is
enabled via either tls_enable_ct() or a non-null custom callback, and 0
otherwise.
...
The time returned by tls_SESSION_get_time() will be used to evaluate whether any
...
tls_enable_ct(), tls_CTX_enable_ct(), tls_CTX_set_ct_validation_callback() and
tls_set_ct_validation_callback() return 1 if the B<callback> is successfully
set.
They return 0 if an error occurs, e.g. a custom client extension handler has
been setup to handle SCTs.

tls_disable_ct() and tls_CTX_disable_ct() do not return a result.

tls_CTX_ct_is_enabled() and tls_ct_is_enabled() return a 1 if a non-null CT
