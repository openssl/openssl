...
tls_get_shared_sigalgs, tls_get_sigalgs - get supported signature algorithms
...
tls_get_shared_sigalgs() returns information about the shared signature
...
tls_get_sigalgs() is similar to tls_get_shared_sigalgs() except it returns
information about all signature algorithms supported by B<s> in the order
they were sent by the peer.

=head1 RETURN VALUES

tls_get_shared_sigalgs() and tls_get_sigalgs() return the number of
...
The shared signature algorithms returned by tls_get_shared_sigalgs() are
...
The NIDs are Opentls equivalents. For example if the peer sent sha256(4) and
