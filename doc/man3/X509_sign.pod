...
X509_CRL_sign() and X509_CRL_sign_ctx() also refresh the cached CRL extension
values after signing. This includes the cached issuer and revocation reason
for each entry, so changes made before signing are reflected in subsequent
CRL lookups without an encoding and decoding step.
If cache initialization fails, the signing function returns zero.
