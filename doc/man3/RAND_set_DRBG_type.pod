...
Note that a provider (for example the FIPS provider) requesting entropy
or a nonce can instantiate the seed source before the application's first
use of it, if one was configured via the B<random> configuration section
or at build time.  RAND_set_seed_source_type() fails once this happens.
