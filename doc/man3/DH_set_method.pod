...
DH_set_method, DH_new_method, DH_Opentls - select DH method
...
A B<DH_METHOD> specifies the functions that Opentls uses for Diffie-Hellman
...
Initially, the default DH_METHOD is the Opentls internal implementation, as
returned by DH_Opentls().
...
DH_Opentls() and DH_get_default_method() return pointers to the respective
B<DH_METHOD>s.
