...
set an old-style write function. The parameters for the function have the same
meaning as for BIO_write() and it must return values as described for
BIO_write().
...
The function must handle the end-of-file condition (if applicable) and call
BIO_set_flags() to set the BIO_FLAGS_IN_EOF flag.
