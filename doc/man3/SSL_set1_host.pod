...
* Any address in the IP address list matches any IP address SAN in the certificate.
...
emails.
If I<email> is not NULL, it will be checked for validity and
added to the list of email address reference identifiers as its first entry.
...
SSL_add1_dnsname() adds I<dnsname> to the list of DNS name reference identifiers, if it has the
correct structure for a DNS name.

SSL_add1_email adds I<email> to the list of email address reference identifiers if it has the
