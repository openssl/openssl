...
a match for validation purposes if any of the following is true:

* Any name in the dnsname list matches any SAN dnsname in the
  certificate If verification flags allow it, these will also attempt
  to match against the CN in the subject.

* Any email address in the email list matches any SAN email in the
  certificate.  If verification flags allow it, these will also
  attempt to match against email addresses in the subject.

* Any ip address in the ip address list matches any SAN ipaddr in the certificate.
...
SAN dnsnames. if I<dnsname> is not NULL, it will be checked for
validity and added to the list of dnsnames as the first entry.

SSL_set1_email clears the list of emails to match certificate SAN
emails. if I<email> is not NULL, it will be checked for validity and
added to the list of dnsnames as the first entry.

SSL_set1_ipaddr clears the list of ip addresses to match
certificate SAN ipaddrs. if <ip_asc> is not NULL, it is parsed as an
ipv4 or ipv6 address and added to the list as the first entry.

SSL_add1_dnsname adds I<dnsname> to the list of dnsnames if it has the
correct structure for a dns name.

SSL_add1_email adds I<email> to the list of emails if it has the
correct structure for an email address.

SSL_add1_ipaddr adds I<ip> to the list of ip addresses if it parses as an IP address.

TLS clients are recommended to use SSL_set1_dnsname() for server
hostname validation. as well as L<SSL_set_tlsext_host_name(3)> for
Server Name Indication (SNI), which may be crucial also for correct
routing of the connection request. SSL_set1_ip, or SSL_set1_ip_asc
...
SSL_set1_dnsname, SSL_set1_email, SSL_set1_ip, SSL_set1_ip_asc,
SSL_add1_dnsname, SSL,add1_email, SSL_add1_ip, SSL_add1_ip_asc,
