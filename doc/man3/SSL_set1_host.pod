...
* Any email address in the email list, if not empty, matches any SAN
  email in the certificate.  If verification flags allow it, these
  will also attempt to match against email addresses in the subject.
...
IP address, if present, matches.  This function is required for DANE
TLSA in the presence of service name indirection via CNAME, MX or SRV
records as specified in RFCs 7671, 7672, and 7673.
