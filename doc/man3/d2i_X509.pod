...
the byte following the parsed data.

If I<a> is not NULL, then the contents of the pointer will be examined.

If I<*a> is not NULL, then it is assumed that caller has allocated a new I<TYPE>
structure, and it will be used.  This is the B<reuse> capability.
Please see BUGS below.

If an error occurs, then the structure will not be used or modified.

If I<a> is NULL, or I<*a> is NULL, then a new structure will be allocated.
(A NULL libctx and a NULL propq will be provided)
If I<*a> is not NULL, then it will be set to the newly allocated structure.

When an existing structure is B<reused>, the reused structure is not
completely cleared.  This may results in memory leaks, and reference
count leaks, particularly when errors occur. (See B<BUGS>).
From OpenSSL 3.x onwards, the only supported use for the reuse mechanism
is to allocate the I<TYPE> structure with one of the many _ex() routines,
such that the I<libctx> and I<propq> can be initialized.  (See B<EXAMPLES> section)
No other reuse is supported.
