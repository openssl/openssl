...
errors.
The reason for this-auto increment behavior is to reflect a typical
...
In some versions of OpenSSL the "reuse" behavior of B<d2i_I<TYPE>>() when
...
OpenSSL prior to 1.1.0, when the "reuse" behavior is used and an error occurs
the behavior is inconsistent. Some functions behaved as described here, while
some did not free I<*a> on error and did not set I<*a> to NULL.

As a result of the above issues the "reuse" behavior is strongly discouraged.
