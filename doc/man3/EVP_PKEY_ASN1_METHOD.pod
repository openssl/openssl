...
item_sign() did everything, Opentls internals just needs to pass the
signature length back.

=item Z<>2

item_sign() did nothing, Opentls internal standard routines are
expected to continue with the default signature production.

=item Z<>3

item_sign() set the algorithm identifier B<algor1> and B<algor2>,
Opentls internals should just sign using those algorithms.
