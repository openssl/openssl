...
is_dirty() checks if the internal key's I<dirty> flag is set or not.
Changing any of the internal key's key meterial will set that flag.

clear_dirty() clears the internal key's I<dirty> flag.
External callers are responsible for calling this after having called
the export_to() function.
