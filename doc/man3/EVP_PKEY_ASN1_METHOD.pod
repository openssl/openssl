...
They MUST return 0 on error, 1 on success.
They're called by EVP_PKCS82PKEY() and EVP_PKEY2PKCS8().
...
They MUST return 0 on error, 1 on success.
They're called by PEM_read_bio_Parameters() and the B<file:>
...
It MUST return 0 on error, 1 on success.
It's called by X509_signature_print().

 void (*pkey_free) (EVP_PKEY *pkey);

The pkey_free() method helps freeing the internals of B<pkey>.
It's called by L<EVP_PKEY_free(3)>, EVP_PKEY_set_type(),
EVP_PKEY_set_type_str(), and EVP_PKEY_assign().
...
EVP_PKEY_get1_tls_encodedpoint(), PKCS7_SIGNER_INFO_set(),
...
item_verify() and item_sign() are called by ASN1_item_verify() and
ASN1_item_sign(), and by extension, L<X509_verify(3)>,
...
It's called as part of X509_check_purpose(), L<X509_check_ca(3)>
