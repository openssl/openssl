...
OSSL_CTX, OSSL_CTX_new, OSSL_CTX_free, OSSL_CTX_load_config,
OSSL_CTX_set0_default
...
=head1 SYNOPSIS
 ...
 OSSL_CTX *OSSL_CTX_new(void);
 int OSSL_CTX_load_config(OSSL_CTX *ctx, const char *config_file);
 void OSSL_CTX_free(OSSL_CTX *ctx);
 OSSL_CTX *OSSL_CTX_set0_default(OSSL_CTX *ctx);

=head1 DESCRIPTION

B<OSSL_CTX> is an internal OpenSSL library context type.
Applications may allocate their own, but may also use NULL to use
a default context with functions that take an B<OSSL_CTX>
argument.
...
the OSSL_CTX is freed.
See L<OPENSSL_thread_stop_ex(3)> for more information.

OSSL_CTX_new() creates a new OpenSSL library context.

OSSL_CTX_load_config() loads a configuration file using the given C<ctx>.
This can be used to associate a library context with providers that are loaded
from a configuration.

OSSL_CTX_free() frees the given I<ctx>, unless it happens to be the
default OpenSSL library context.

OSSL_CTX_set0_default() sets the default OpenSSL library context to be
...
OSSL_CTX_new() and OSSL_CTX_set0_default() return a library context
pointer on success, or NULL on error.

OSSL_CTX_free() doesn't return any value.

=head1 HISTORY

OSSL_CTX, OSSL_CTX_new(), OSSL_CTX_load_config(), OSSL_CTX_free()
and OSSL_CTX_set0_default() were added in OpenSSL 3.0.
