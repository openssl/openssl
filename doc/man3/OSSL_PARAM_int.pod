...
OSSL_PARAM_located, OSSL_PARAM_read, OSSL_PARAM_written,
OSSL_PARAM_set_all_unused
- OSSL_PARAM helpers
...
=for comment generic
 ...
 OSSL_PARAM *OSSL_PARAM_locate_const(const OSSL_PARAM *array, const char *key);
 ...
 void OSSL_PARAM_set_all_unused(OSSL_PARAM *p);
...
OSSL_PARAM_located() queries the setting of the located flag for the parameter
referenced by B<p>.
This flag is set if the parameter has been the subject of a OSSL_PARAM_locate()
call.

OSSL_PARAM_read() queries the setting of the read flag for the parameter
referenced by B<p>.
This flag is set if the parameter has had its value read using one of the
OSSL_PARAM_get_TYPE calls.

OSSL_PARAM_written() queries the setting of the written flag for the parameter
referenced by B<p>.
This flag is set if the parameter has had its value written using one of the
OSSL_PARAM_set_TYPE calls.

OSSL_PARAM_set_all_unused() clears the used flag for all the parameters in
the array referenced by B<params>.
The array is terminated by an OSSL_PARAM_END record.
...
OSSL_PARAM_used() returns B<1> if the used flag is set and B<0> otherwise.
...
The OSSL_PARAM_locate_const() function does not set the located flag if a
paramter is returned.
