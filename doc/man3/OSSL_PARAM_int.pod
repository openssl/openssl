...
OSSL_PARAM_used, OSSL_PARAM_set_used, OSSL_PARAM_set_all_unused
- OSSL_PARAM helpers
...
=for comment generic
 ...
 int OSSL_PARAM_used(const OSSL_PARAM *p);
...
OSSL_PARAM_used() queries the setting of the used flag for the parameter
referenced by B<p>.

OSSL_PARAM_set_used() clears the used flag for the parameter referenced
by B<p> to B<set>.
