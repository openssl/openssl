...
fails with a negative return value and sets I<ctx>-E<gt>error to
B<X509_V_ERR_INVALID_CALL>.
...
walking from the trust anchor down to the target. Signatures on self-signed
certificates are checked only when B<X509_V_FLAG_CHECK_SS_SIGNATURE> is set.
...
B<RFC 3779 path validation> of AS-number and IP-address delegation extensions,
when the library is built with that support.
...
call, the verification machinery sets I<ctx>-E<gt>error to the
B<X509_V_ERR_*> code describing the failure and, for certificate-level errors,
sets I<ctx>-E<gt>error_depth to the depth at which the error was detected and
I<ctx>-E<gt>current_cert to the certificate in question. CRL- and OCSP-related
errors set I<ctx>-E<gt>error only; the depth and current certificate are
already set from a preceding context.
...
I<ctx>-E<gt>current_cert, I<ctx>-E<gt>current_issuer and
I<ctx>-E<gt>error_depth are set to describe the certificate that has just been
accepted. The callback may use this to log progress, but B<must> return a
nonzero value, otherwise verification is aborted.
...
treated as passed for control-flow purposes, but I<ctx>-E<gt>error is B<not>
reset to B<X509_V_OK>. A subsequent successful return from X509_verify_cert()
therefore does B<not> imply that L<X509_STORE_CTX_get_error(3)> will return
B<X509_V_OK>: it may still hold the last error code that was waived. This is
intentional. Only the callback itself is permitted to overwrite
I<ctx>-E<gt>error, and only at its own risk.
...
diagnostics. Inspect I<ctx>-E<gt>error and waive only the specific codes you
intend to.
...
B<Clearing I<ctx>-E<gt>error>. The sticky-error rule exists so that a waived
error remains visible to the caller after verification returns. A callback that
overwrites I<ctx>-E<gt>error with B<X509_V_OK> hides this information and can
also mask a later error if the callback is invoked again before verification
completes.
...
B<Trusting the depth alone>. I<ctx>-E<gt>error_depth records where an error
was detected during chain processing, not the position of the certificate in
the final chain. Always consult I<ctx>-E<gt>current_cert in addition to the
depth when deciding whether to waive.
...
silently across OpenSSL releases. Write callbacks defensively: inspect
I<ctx>-E<gt>error and I<ctx>-E<gt>current_cert afresh on each invocation, and
...
than one check, the specific B<X509_V_ERR_*> code left in I<ctx>-E<gt>error
reflects whichever check fired first; this ordering is an implementation
detail and is not stable across releases. Callers must therefore treat the
return value as the authoritative success/failure signal, and treat the
specific error code as diagnostic information that may shift over time.
...
invariant was violated. In these cases I<ctx>-E<gt>error is set to an
appropriate B<X509_V_ERR_*> value (B<X509_V_ERR_INVALID_CALL>,
