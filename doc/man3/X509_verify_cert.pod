...
All certificates involved in the verification -- the target, the untrusted
certificates, and the trusted certificates in the store -- must be finalized,
that is parsed or signed and not modified since; see L<x509(7)>. Verification
relies on cached extension information that is populated only when a certificate
is finalized. Using an unfinalized certificate can result in undefined behaviour.
