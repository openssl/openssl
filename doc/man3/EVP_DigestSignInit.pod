...
In some rare cases (including SM2 signatures) it is necessary to reinitialize
the digest computation after setting some parameters on the B<pctx>. This
is done by calling EVP_DigestSignInit() twice. The first time, B<pctx>
should be non NULL. Then any necesssary options can be set on B<pctx>
using for instance EVP_PKEY_CTX_ctrl_str(). Afterwards, EVP_SignVerifyInit()
is called again, using the same verification context B<ctx>.
