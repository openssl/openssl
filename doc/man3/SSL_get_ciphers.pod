...
SSL_get_flags,
SSL_get1_supported_flags
- get list of available SSL_CIPHERs or their flags
...
=head1 SYNOPSIS
 ...
 uint8_t *SSL_get_flags(const SSL *ssl);
 uint8_t *SSL_get1_supported_flags(SSL *ssl);
...
SSL_get_flags() returns array of flags for ciphers available for B<ssl>
If B<ssl> is NULL or no ciphers are available, NULL is returned. This array of
flags should be passed to function SSL_CIPHER_flags_description() for
interpret meaning of each flags.

SSL_get1_supported_flags() returns simillar array of flags like function
SSL_get_flags() but only for ciphers, which have not been disabled.
...
The array returned by SSL_get1_supported_flags() should be freed using
OPENSSL_free().
