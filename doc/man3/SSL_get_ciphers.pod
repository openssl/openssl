...
SSL_CTX_get_ciphers(), SSL_get_ciphers() and SSL_get_client_ciphers() return
borrowed internal stacks. The caller B<MUST NOT> free these stacks or their
elements. A stack cannot outlive its owning SSL or SSL_CTX and may be replaced
earlier by a cipher-list setter, connection reset or subsequent handshake;
obtain it again after such operations.

SSL_get_ciphers() borrows the current SSL_CTX's combined list until a per-SSL
setter creates a separate list. SSL_set_SSL_CTX() can therefore change the
returned stack and release its previous owner. Keeping B<ssl> alive does not
keep every previously selected SSL_CTX alive. Obtain the list again after a
context switch. Retaining its owner prevents destruction, not replacement
of that owner's list.

Built-in cipher descriptors have static lifetime. Provider-defined descriptors
belong to their originating SSL_CTX registry. Unlike the stack, such a
descriptor remains valid across list changes while that SSL_CTX is alive.

The stack returned by SSL_get1_supported_ciphers() should be freed using
sk_SSL_CIPHER_free(). Its elements are borrowed and must not be freed. A
provider-defined element remains valid only while B<ssl> remains alive.

The stacks returned by SSL_bytes_to_cipher_list() should be freed using
sk_SSL_CIPHER_free(). Their elements are borrowed and must not be freed. A
provider-defined element remains valid only while B<s> remains alive.
