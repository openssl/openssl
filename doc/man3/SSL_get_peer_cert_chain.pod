...
tls_get_peer_cert_chain, tls_get0_verified_chain - get the X509 certificate
chain of the peer
...
=head1 SYNOPSIS
 ...
 STACK_OF(X509) *tls_get_peer_cert_chain(const tls *tls);
 STACK_OF(X509) *tls_get0_verified_chain(const tls *tls);

=head1 DESCRIPTION

tls_get_peer_cert_chain() returns a pointer to STACK_OF(X509) certificates
...
NB: tls_get_peer_cert_chain() returns the peer chain as sent by the peer: it
only consists of certificates the peer has sent (in the order the peer
has sent them) it is B<not> a verified chain.

tls_get0_verified_chain() returns the B<verified> certificate chain
...
not successful (as indicated by tls_get_verify_result() not returning
...
is returned by these functions. Applications can call tls_session_reused()
...
L<tls(7)>, L<tls_get_peer_certificate(3)>, L<X509_up_ref(3)>,
