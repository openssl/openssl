...
be NULL and the return value from SSL_get0_tlsext_status_ocsp_resp_ex() will
be -1.
...
CA certificate).
Typically the server callback would obtain the certificates that are being sent
back to the client via a call to SSL_get_certificate() resp.
SSL_get0_chain_certs(); obtain the OCSP response to be sent back; and then set
that response data by calling SSL_set0_tlsext_status_ocsp_resp_ex(). A stack of
OCSP responses should be provided in the B<resp> argument.
...
SSL_set_tlsext_status_ocsp_resp() and SSL_set0_tlsext_status_ocsp_resp_ex()
...
macros were added in OpenSSL 3.4.
