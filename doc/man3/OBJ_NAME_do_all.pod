...
OBJ_NAME,
OBJ_NAME_TYPE_MD_METH, OBJ_NAME_TYPE_CIPHER_METH, OBJ_NAME_TYPE_PKEY_METH,
OBJ_NAME_TYPE_COMP_METH, OBJ_NAME_TYPE_MAC_METH, OBJ_NAME_TYPE_KDF_METH
- iterate over objects of a given type
...
in the internal globally defined name table. This table maps algorithm names
to their implementations and is used by OpenSSL to look up ciphers, digests,
and other cryptographic methods by name. The I<arg> value is passed as the
second argument to each call of I<fn>. The order in which objects are
iterated is undefined.

OBJ_NAME_do_all_sorted() is similar to OBJ_NAME_do_all(), except that the
objects being iterated over are sorted by name using strcmp(3).
