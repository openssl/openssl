...
the listener. The BIO should typically be configured once before calling
L<SSL_listen(3)> and not modified afterward while the listener is in use.
If the listener was created with the B<SSL_LISTENER_FLAG_SINGLE_THREAD> flag,
all operations including BIO changes must be performed from a single thread.
