...
When B<ssl> is a DTLS listener object, these functions are thread-safe and may
be called while other threads are using the listener. However, if the listener
was created with the B<SSL_LISTENER_FLAG_SINGLE_THREAD> flag, no mutex is
allocated for the listener and these functions should only be called from a
single thread.
