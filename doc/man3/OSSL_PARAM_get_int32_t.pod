...
=for comment generic
 ...
 OSSL_PARAM_TYPE(key, address)
 OSSL_PARAM_utf8_string(key, address, size)
 OSSL_PARAM_octet_string(key, address, size)
 OSSL_PARAM_utf8_ptr(key, address, size)
 OSSL_PARAM_octet_ptr(key, address, size)
 OSSL_PARAM_SIZED_TYPE(key, address, return_size)
 OSSL_PARAM_SIZED_BN(key, address, size, return_size)
 OSSL_PARAM_SIZED_utf8_string(key, address, size, return_size)
 OSSL_PARAM_SIZED_octet_string(key, address, size, return_size)
 OSSL_PARAM_SIZED_utf8_ptr(key, address, size, return_size)
 OSSL_PARAM_SIZED_octet_ptr(key, address, size, return_size)
 OSSL_PARAM_END
...
OSSL_PARAM_octet_ptr() are macros that provide supportfor defining UTF8 strings
and OCTET strings.
...
They are similar to the OSSL_PARAM_TYPE() but also include a B<return_size>
argument which contains the address of a size_t variable which will be
populated with the actual size of the parameter upon return from a
...
referenced by B<p> and stores it in B<*val>.
The legnth of the OCTET string is stored in B<*used_len>.
