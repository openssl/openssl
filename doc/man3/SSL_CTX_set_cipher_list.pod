...
order of preference.
As of OpenSSL 4.0, B<str> is processed case-insensitively.
Built-in TLSv1.3 ciphersuite names are:
...
Loaded providers can add TLSv1.3 ciphersuites through C<TLS-CIPHERSUITE>; see
L<provider-base(7)>. They are discovered when B<ctx> is created, require
explicit selection, and do not update when another provider is loaded.
...
configures the ciphersuites for B<ssl>. Provider-defined names resolve through
the original SSL_CTX; L<SSL_set_SSL_CTX(3)> does not replace that registry.
SSL_set_SSL_CTX() otherwise keeps its existing behaviour. Switching to a
context with a different library context or property query is outside the
provider-ciphersuite contract.
...
Passing a NULL context, SSL object, or list string returns 0 and reports
B<ERR_R_PASSED_NULL_PARAMETER> without changing the configured list.
An empty string is valid and configures an empty TLSv1.3 ciphersuite list.
...
Provider-defined TLSv1.3 ciphersuites were added in OpenSSL 4.1.
