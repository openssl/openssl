...
SSL_set_cipher_list,
SSL_CTX_set_cipher_list_and_mask,
SSL_set_cipher_list_and_mask,
...
=head1 SYNOPSIS
 ...
 int SSL_CTX_set_cipher_list_and_mask(SSL_CTX *ctx, const char *str, int version_mask);
 int SSL_set_cipher_list_and_mask(SSL *ssl, const char *str, int version_mask);
...
SSL_CTX_set_cipher_list_and_mask() sets the list of available ciphers
for B<ctx> using the control string B<str> and B<version_mask>. The format of
these strings is described in L<openssl-ciphers(1)>. The list of ciphers is
inherited by all B<ssl> objects created from B<ctx>. If you want to set only
TLSv1.3 ciphersuites. Use SSL_CTX_set_ciphersuites() to configure those.

SSL_CTX_set_cipher_list() works just like SSL_CTX_set_cipher_list_and_mask(),
but sets the version_mask to the default value.

SSL_set_cipher_list_and_mask() and SSL_set_cipher_list() set the
list of ciphers only for B<ssl>.

OPENSSL_version_list() is used to convert SSL/TLS version string from B<str>
into B<version_mask>. This is a simple colon vertical bar ("|") separated list of
version names. The format of this string is described in L<openssl-ciphers(1)>.
...
ciphersuite names in the order of preference. Valid TLSv1.3 ciphersuite names are:
...
The control string B<str> for SSL_CTX_set_cipher_list(),
SSL_CTX_set_cipher_list_and_mask(), SSL_set_cipher_list() and
SSL_set_cipher_list_and_mask() should be universally usable and not depend
...
RSA ciphers using DHE need a certificate, a key and an additional DH-parameters
...
SSL_CTX_set_cipher_list_and_mask() and SSL_set_cipher_list_and_mask()
...
OPENSSL_version_list() returns number of characters that were suscessfully
procesed and 0 when no versin name was recognized.
