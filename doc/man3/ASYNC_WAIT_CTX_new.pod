...
For an overview of how asynchronous operations are implemented in Opentls see
L<ASYNC_start_job(3)>. An B<ASYNC_WAIT_CTX> object represents an asynchronous
"session", i.e. a related set of crypto operations. For example in tls terms
this would have a one-to-one correspondence with an tls connection.
...
subsequent B<ASYNC_JOB>. When the session is complete (e.g. the tls connection
