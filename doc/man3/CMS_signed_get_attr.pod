...
CMS_signerInfo contains seperate attribute lists for signed and unsigned
attributes. Each CMS_signed_XXX() function is used for signed attributes, and
there is an equivalent CMS_unsigned_XXX() function for unsigned attributes.
Only the CMS_signed_XXX() functions are described below.
...
on the next call to X509_REQ_get_attr_by_OBJ() as the value of I<lastpos> in
...
the I<si> signed attribute list. An error occurs if I<si> attribute list is NULL.
...
required. An error occurs id I<attr> is NULL.
...
to the I<key> objects attribute list. If I<obj> already exists in the signed
attribute list then an error occurs.

CMS_signed_add1_attr_by_NID() is similar to CMS_signed_add1_attr_by_OBJ() except
that it passes the numerical identifier (NID) I<nid> associated with the object.
See <openssl/obj_mac.h> for a list of NID_*.

int CMS_signed_add1_attr_by_txt() is similar to CMS_signed_add1_attr_by_OBJ()
except that is passes a name I<attrname> associated with the object.
...
type of the B<ANS1_TYPE> object OR if I<type> is either B<V_ASN1_BOOLEAN> or
...
CMS_unsigned_XXX() functions are similiar to the return values of the matching
CMS_signed_XXX() functions.
...
CMS_signed_delete_attr()return either the removed signed B<X509_ATTRIBUTE> or
