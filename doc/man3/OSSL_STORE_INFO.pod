...
for the storage where the object (or collection of objects if that's what
each name represents) resides.
It is up to each loader to ensure this.
...
canonical doesn't mean that all other variants can't be used.
