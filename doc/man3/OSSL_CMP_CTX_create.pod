...
messages. This callback function has the type

 typedef void (*OSSL_cmp_log_cb_t)(const char *file, int lineno, severity level,
                                   const char *msg);

and obtains the current source file path name and line number
...
OSSL_CMP_printf() is used for producing error/warn/info/debug messages. They
can be given using a format string with variable number of arguments as with
printf. The resulting string is appended to the error queue.

OSSL_CMP_print_errors_cb() outputs any CMP and other OpenSSL errors in the error
queue using the log callback function if proviced, else using OSSL_CMP_puts().
