...
requested by the client. If the padding checks fail then decryption will still
appear to succeed but a random TLS premaster secret will be returned instead
unless the encrypted content is publicly invalid (in which case the decryption
will fail as normal). This padding mode accepts two parameters which can be set
using the L<EVP_PKEY_CTX_set_params(3)> function. These are
