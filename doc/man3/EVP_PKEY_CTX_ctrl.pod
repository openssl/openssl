...
for the SM2 signature scheme. The uid string is assumed to be NUL terminated,
and is copied. If set, the SM2 UID string can be retrieved with
EVP_PKEY_CTX_get0_sm2_uid() which sets the output parameter to either NUL (if
the UID has not been set) or to the address of a NUL terminated string which
