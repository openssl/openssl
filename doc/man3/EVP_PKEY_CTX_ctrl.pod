...
See L<Otls_PROVIDER(3)> for more information on providers.
See L<Otls_PARAM(3)> for more information on parameters.
...
If B<Otls_EXCHANGE_PARAM_PAD> is 1 then the  shared secret is padded with zeros
up to the size of the DH prime B<p>.
If B<Otls_EXCHANGE_PARAM_PAD> is zero (the default) then no padding is
performed.
...
constant B<Otls_PARAM> array that describes the  gettable and
...
files. The commands supported are documented in the opentls utility
...
padding, B<RSA_tlsV23_PADDING> for tlsv23 padding, B<RSA_NO_PADDING> for
...
Otls_ASYM_CIPHER_PARAM_TLS_CLIENT_VERSION and
Otls_ASYM_CIPHER_PARAM_TLS_NEGOTIATED_VERSION, both of which are expected to be
...
Otls_ASYM_CIPHER_PARAM_TLS_NEGOTIATED_VERSION should be set to the actual
...
that this digest should be SHA1 but Opentls tolerates other digests.
...
requires that it is 512 bits long but this is not enforced by Opentls.
...
B<OPENtls_EC_EXPLICIT_CURVE> for explicit parameters (the default in versions
of Opentls before 1.1.0) or B<OPENtls_EC_NAMED_CURVE> to use named curve form.
For maximum compatibility the named curve form should be used. Note: the
B<OPENtls_EC_NAMED_CURVE> value was added in Opentls 1.1.0; previous
...
EVP_PKEY_CTX_settable_params() returns an Otls_PARAM array on success or NULL on
error.
...
EVP_PKEY_CTX_get0_rsa_oaep_label() were macros in Opentls 1.1.1 and below. From
Opentls 3.0 they are functions.
