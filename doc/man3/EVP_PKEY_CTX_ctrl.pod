...
= for comment Is this an "operation" or an "operating mode".  I'd would have
              though the "operation" is always "decapsulate" or "encapsulate"
              and that options like "RSASVE" are "operating modes", is in
              the comment "Set the mode - only 'RSASVE' is currently supported",
              the code example.

=for comment TODO(ML-KEM): Probably nothing, there aren't presently any modes
             for ML-KEM.
