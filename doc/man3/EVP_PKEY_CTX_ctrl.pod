...
=for comment Is this an "operation" or an "operating mode".  I would have
             thought the "operation" is always "decapsulate" or "encapsulate"
             and that tweaks like "RSASVE" are "operating modes" (what kind of
             encapsulation or decapsulation to perform, not which of the two),
             as, for example, in the comment "Set the mode - only 'RSASVE' is
             currently supported".
