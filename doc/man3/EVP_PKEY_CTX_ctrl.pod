...
EVP_PKEY_CTX_set1_sm2_uid,
EVP_PKEY_CTX_get0_sm2_uid - algorithm specific control operations
...
=head1 SYNOPSIS
 ...
 int EVP_PKEY_CTX_set1_sm2_uid(EVP_PKEY_CTX *ctx, const char *uid);
 int EVP_PKEY_CTX_get0_sm2_uid(EVP_PKEY_CTX *ctx, char **uid);
...
The EVP_PKEY_CTX_set1_sm2_uid() sets the user identifier string, which is used
for the SM2 signature scheme. The uid string is assumed to be NULL terminated,
and is copied. If set, the SM2 UID string can be retrieved with
EVP_PKEY_CTX_get0_sm2_uid() which sets the output parameter to either NULL (if
the UID has not been set) or to the address of a NULL terminated string which
contains the UID. Ownership of this memory pointed to by the reuslt of
EVP_PKEY_CTX_get0_sm2_uid() is retained by the library.
...
The SM2 related functions were first added in OpenSSL 1.1.1
