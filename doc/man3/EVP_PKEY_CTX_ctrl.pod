...
EVP_PKEY_CTX_set_priv_key,
EVP_PKEY_CTX_set_pub_key,
...
=head1 SYNOPSIS
 ...
 int EVP_PKEY_CTX_set_priv_key(EVP_PKEY_CTX *ctx, unsigned char *key, int len);
 int EVP_PKEY_CTX_set_pub_key(EVP_PKEY_CTX *ctx, unsigned char *key, int len);
...
When B<cmd> is B<EVP_PKEY_CTRL_SET_PRIV_KEY>, B<p1> is the length of the MAC key,
...
allow private or public key data to be set explicitly using the
EVP_PKEY_CTX_set_priv_key() or EVP_PKEY_CTX_set_pub_key() macros. In this case
key generation is simply the process of setting up the parameters for the key
and then setting the raw key data to the values explicitly provided by those
macros. Unless an algorithm provides greater control over the process of
setting up parameters then, typically, applications would call the
L<EVP_PKEY_new_priv_key(3)> or L<EVP_PKEY_new_pub_key(3)> functions instead of
these macros.

The EVP_PKEY_CTX_set_priv_key() macro can be used with any of the algorithms
supported by the L<EVP_PKEY_new_priv_key(3)> function.

The EVP_PKEY_CTX_set_pub_key() macro can be used with any of the algorithms
supported by the L<EVP_PKEY_new_pub_key(3)> function.

The EVP_PKEY_CTX_set_mac_key() macro is synonymous with the
EVP_PKEY_CTX_set_priv_key() macro.
