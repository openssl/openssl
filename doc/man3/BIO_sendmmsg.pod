...
the nummber of messages successfully processed (which need not be nonzero) to
...
If BIO_sendmmsg() and BIO_recvmmsg() fail, they always raise an error using
L<ERR_raise(3)>. Any error may be raised, but the following in particular may be
noted:
