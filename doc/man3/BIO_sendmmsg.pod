...
=head1 SYNOPSIS
 ...
 size_t BIO_sendmmsg(BIO *b, BIO_MSG *msg,
                     ...
                     int *err);
 size_t BIO_recvmmsg(BIO *b, BIO_MSG *msg,
                     size_t stride, size_t num_msg, uint64_t flags,
                     int *err);
...
by the return value, which expresses the number of messages
processed.
...
I<err> must be non-NULL and points to an integer written with an error code; see
the RETURN VALUES section for further discussion.
...
BIO_sendmmsg() and BIO_recvmmsg() always write a value to I<err> regardless of
their outcome, and regardless of whether any messages were sent or received. The
possible values of I<err> after a call to BIO_sendmmsg() or BIO_recvmmsg()
...
Zero, indicating that no error occurred. This outcome typically occurs when
I<num_msg> is zero, or when all I<num_msg> B<BIO_MSG> structures were
successfully processed. It may also occur if the operating system chose to
return less than I<num_msg> messages, but did not indicate an error.

=item *

A system-specific error code (EWOULDBLOCK, etc.). If at least one message was
returned (that is, if the function returned a positive value), this error code
is the error code that occurred when trying to send or receive the next message
after that message. If the function did not successfully send or receive any
message (returned 0), this error code is the error code that occurred while
trying to send or receive the first message.

=item *

The value B<BIO_NO_LOCAL>, indicating that a local address was specified
or requested but local address support is not available or nott enabled.

=item *

The value B<BIO_NO_SUPPORT>, indicating that the BIO_sendmmsg() or
BIO_recvmmsg() method is not supported by the BIO.

=item *

The value B<BIO_UNSPEC_ERR>, indicating that BIO_sendmmsg() or BIO_recvmmsg()
failed for any other reason (for example, if the function was called with an
invalid argument).
