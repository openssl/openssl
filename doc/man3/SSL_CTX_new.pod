...
The L<X509_VERIFY_PARAM_set_purpose(3)> can be used, also in conjunction with
L<SSL_CTX_get0_param(3)>, to set the intended purpose of the session.
...
The methods exists in a generic type (for client and server use),
