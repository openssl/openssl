...
SSL_CTX_new_ex() creates a new B<SSL_CTX> object, which holds various configuration and data
relevant to TLS/SSL or DTLS session establishment. The library context
...
An B<SSL_CTX> object should not be changed after it is used to create any B<SSL> objects or from multiple
