...
tls_get_peer_signature_nid, tls_get_peer_signature_type_nid,
tls_get_signature_nid, tls_get_signature_type_nid - get TLS message signing
types
...
=head1 SYNOPSIS
 ...
 int tls_get_peer_signature_nid(tls *tls, int *psig_nid);
 int tls_get_peer_signature_type_nid(const tls *tls, int *psigtype_nid);
 int tls_get_signature_nid(tls *tls, int *psig_nid);
 int tls_get_signature_type_nid(const tls *tls, int *psigtype_nid);

=head1 DESCRIPTION

tls_get_peer_signature_nid() sets B<*psig_nid> to the NID of the digest used
by the peer to sign TLS messages. It is implemented as a macro.

tls_get_peer_signature_type_nid() sets B<*psigtype_nid> to the signature
...
tls_get_signature_nid() and tls_get_signature_type_nid() return the equivalent
...
L<tls(7)>, L<tls_get_peer_certificate(3)>,
