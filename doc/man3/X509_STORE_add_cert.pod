...
The certificate files in B<dir> are only looked up when required, e.g., when
...
directory, the function behaves generally like X509_STORE_load_path() if the
store is a directory, otherwise like X509_STORE_load_file_ex().
