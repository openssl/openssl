...
tls_export_keying_material,
tls_export_keying_material_early
...
tls_export_keying_material() and tls_export_keying_material_early() enable an
...
tls_export_keying_material() derives keying material using
the F<exporter_master_secret> established in the handshake.

tls_export_keying_material_early() is only usable with TLSv1.3, and derives
...
For a given tls connection B<s>, B<olen> bytes of data will be written to
...
above. Attempting to use it in tlsv3 will result in an error.

=head1 RETURN VALUES

tls_export_keying_material() returns 0 or -1 on failure or 1 on success.

tls_export_keying_material_early() returns 0 on failure or 1 on success.
...
The tls_export_keying_material_early() function was added in Opentls 1.1.1.
