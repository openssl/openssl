...
OSSL_CMP_print_to_bio() prints the string constant C<CMP routines>,
the function name I<func> followed by C<()> if I<func> is not NULL,
the filename I<file> and line number I<line> if I<file> is not NULL,
a string representing the severity level I<level> such as C<error>,
and the message I<msg> followed by a newline character if I<msg> is not NULL,
to the given I<bio> if provided.

OSSL_CMP_print_errors() outputs all entries in the OpenSSL error queue.
It is similar to L<ERR_print_errors_cb(3)> but uses the CMP log callback
function if set in the I<ctx> for uniformity with CMP logging.
Otherwise it prints to STDERR using OSSL_CMP_print_to_bio()
(unless B<OPENSSL_NO_STDIO> is defined).

=head1 RETURN VALUES

OSSL_CMP_log_close() and OSSL_CMP_print_errors() do not return anything.
