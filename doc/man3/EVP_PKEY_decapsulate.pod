...
indicates the operation is not supported by the private key algorithm. In addition, 
EVP_PKEY_decapsulate() may not return an explicit failure code if the decapsulation
itself failed.
