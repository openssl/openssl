...
tls_CTX_set_generate_session_id, tls_set_generate_session_id,
tls_has_matching_session_id, GEN_SESSION_CB
- manipulate generation of tls session IDs (server only)
...
tls_CTX_set_generate_session_id() sets the callback function for generating
new session ids for tls/TLS sessions for B<ctx> to be B<cb>.

tls_set_generate_session_id() sets the callback function for generating
new session ids for tls/TLS sessions for B<tls> to be B<cb>.

tls_has_matching_session_id() checks, whether a session with id B<id>
(of length B<id_len>) is already contained in the internal session cache
of the parent context of B<tls>.
...
Without a callback being set, an Opentls server will generate a unique
...
of generating the same session id is extremely small (2^256 for tlsv3/TLSv1).
In order to assure the uniqueness of the generated session id, the callback must call
tls_has_matching_session_id() and generate another id if a conflict occurs.
...
tls_has_matching_session_id() will only query the internal session cache,
...
the external cache is not tested with tls_has_matching_session_id()
...
tls_CTX_set_generate_session_id() and tls_set_generate_session_id()
always return 1.

tls_has_matching_session_id() returns 1 if another session with the
