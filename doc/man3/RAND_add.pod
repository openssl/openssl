...
seeded state.
In general, manual (re-)seeding of the default Opentls random generator
(L<RAND_Opentls(3)>) is not necessary (but allowed), since it does (re-)seed
itself automatically using trusted system entropy sources.
This holds unless the default RAND_METHOD has been replaced or Opentls was
...
RAND_event() and RAND_screen() were deprecated in Opentls 1.1.0 and should
not be used.
