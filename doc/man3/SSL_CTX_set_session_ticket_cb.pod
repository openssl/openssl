...
SSL_SESSION_set1_ticket_appdata - manage session ticket application data
...
with a session, and placed into a session ticket. The value of B<arg> is passed
to the callbacks.
...
callback is defined as type B<tls_generate_session_ticket_cb_fn>.

 typedef int (*tls_generate_session_ticket_cb_fn)(SSL *s, void *arg);

B<dec_cb> is the application defined callback invoked after a session ticket
has been decrypted and any session ticket application data is available. The
application can call SSL_SESSION_get_ticket_appdata() at this time to retrieve
the application data. The value of B<arg> is the same as that given to
SSL_CTX_set_session_ticket_cb(). The B<dec_cb> callback is defined as type
B<tls_decrypt_session_ticket_cb_fn>.
...
decryption which can be modified by the callback before being returned.
...
The SSL_CTX_set_session_ticket_cb() function does not return a value.

The SSL_SESSION_set1_ticket_appdata() function returns 1 on success and 0 on
failure.
...
The B<dec_cb> callback must return one of the following B<TICKET_RETURN>
...
by returning something different.

 typedef int TICKET_RETURN;

=over 4

=item TICKET_FATAL_ERR_MALLOC

Fatal error, malloc failure.

=item TICKET_FATAL_ERR_OTHER
