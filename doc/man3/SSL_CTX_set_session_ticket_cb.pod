...
tls_CTX_set_session_ticket_cb,
tls_SESSION_get0_ticket_appdata,
tls_SESSION_set1_ticket_appdata,
tls_CTX_generate_session_ticket_fn,
tls_CTX_decrypt_session_ticket_fn - manage session ticket application data
...
tls_CTX_set_set_session_ticket_cb() sets the application callbacks B<gen_cb>
...
about to be created. The application can call tls_SESSION_set1_ticket_appdata()
at this time to add application data to the session ticket. The value of B<arg>
is the same as that given to tls_CTX_set_session_ticket_cb(). The B<gen_cb>
callback is defined as type B<tls_CTX_generate_session_ticket_fn>.
...
of B<arg> is the same as that given to tls_CTX_set_session_ticket_cb(). The
B<dec_cb> callback is defined as type B<tls_CTX_decrypt_session_ticket_fn>.

tls_SESSION_set1_ticket_appdata() sets the application data specified by
...
tls_SESSION_get0_ticket_appdata() assigns B<data> to the session ticket
...
tls_SESSION_set1_ticket_appdata() or by a session ticket. NULL will be assigned
to B<data> and 0 will be assigned to B<len> if there is no session ticket
application data. tls_SESSION_get0_ticket_appdata() can be called any time
...
When the B<dec_cb> callback is invoked, the tls_SESSION B<ss> has not yet been
assigned to the tls B<s>. The B<status> indicates the result of the ticket
...
=item tls_TICKET_SUCCESS_RENEW

Same as B<tls_TICKET_SUCCESS>, but a new ticket should be sent to the client.
...
should return this value if B<status> is B<tls_TICKET_EMPTY> or
B<tls_TICKET_NO_DECRYPT>.

=item tls_TICKET_RETURN_USE
...
B<tls_TICKET_SUCCESS> or B<tls_TICKET_SUCCESS_RENEW>.

If the callback does not wish to change the default ticket behaviour then it
should return this value if B<status> is B<tls_TICKET_SUCCESS>.

=item tls_TICKET_RETURN_USE_RENEW
...
If the callback does not wish to change the default ticket behaviour then it
should return this value if B<status> is B<tls_TICKET_SUCCESS_RENEW>.

=back

If B<status> has the value B<tls_TICKET_EMPTY> or B<tls_TICKET_NO_DECRYPT> then
no session data will be available and the callback must not use the B<ss>
argument. If B<status> has the value B<tls_TICKET_SUCCESS> or
B<tls_TICKET_SUCCESS_RENEW> then the application can call
tls_SESSION_get0_ticket_appdata() using the session provided in the B<ss>
argument to retrieve the application data.

When the B<gen_cb> callback is invoked, the tls_get_session() function can be
used to retrieve the tls_SESSION for tls_SESSION_set1_ticket_appdata().
...
L<tls_CTX_set_tlsext_ticket_key_cb(3)>).

=head1 RETURN VALUES

The tls_CTX_set_session_ticket_cb(), tls_SESSION_set1_ticket_appdata() and
tls_SESSION_get0_ticket_appdata() functions return 1 on success and 0 on
failure.
...
The tls_CTX_set_session_ticket_cb(), Stls_SESSION_set1_ticket_appdata()
and tls_SESSION_get_ticket_appdata() functions were added in Opentls 1.1.1.
