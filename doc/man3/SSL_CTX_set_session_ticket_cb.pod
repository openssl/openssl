...
If the callback does not wish to change the default ticket behavior then it
...
default behavior is to always issue a new ticket on resumption. In both cases
this behavior can be changed if a ticket key callback is in use (see
