...
OSSL_CMP_SRV_CTX_centralKeygen_req() returns 1 if central key generation
is requested i.e., the public key in the certificate request (I<crm> if non-NULL, 
otherwise I<p10cr>) is NULL or has an empty key value (with length zero).
Otherwise or on error it returns 0.
...
OSSL_CMP_SRV_CTX_set_grant_implicit_confirm() and OSSL_CMP_SRV_CTX_centralKeygen_req()
