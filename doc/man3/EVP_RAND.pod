...
EVP_RAND_enable_locking() enables locking for the RAND I<rand> and all of
its parents.  After this I<rand> will operate in a thread safe manner, albeit
more slowly. This function is not itself thread safe if called with the same
I<rand> from multiple threads. Typically locking should be enabled before a
I<rand> is shared across multiple threads.
