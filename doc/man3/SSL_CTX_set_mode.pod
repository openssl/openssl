...
During normal operations, non-applications data records might need to be sent or
received that the application is not aware of.
If a non-application data record was processed, a
L<SSL_read_ex(3)>, L<SSL_read(3)>, L<SSL_write_ex(3)> or L<SSL_write(3)> can
return with a failure and indicate the need to retry with B<SSL_ERROR_WANT_READ>.
...
deal with read/write operations returning without success report,
and setting the B<SSL_MODE_AUTO_RETRY> flag will cause them to only
return after success or failure.
