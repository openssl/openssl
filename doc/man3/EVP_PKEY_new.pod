...
L<EVP_PKEY_set1_EC_KEY(3)> for legacy key types or for fully provider-managed
key types (see L<provider-keymgmt(7)>), functions such as
L<EVP_PKEY_set1_encoded_public_key(3)>.

Generally a user of an B<EVP_PKEY> structure should not rely on it being
valid across different library contexts. Under specific circumstances, though,
it is possible that an B<EVP_PKEY> structure can be used in a library context B
after having been created in a library context A, namely iff both library
contexts contain a provider implementing the key management interface for the
given key type, and the key manager in context A also implements the
OSSL_FUNC_keymgmt_export() function, and the key manager in context B does
implement the OSSL_FUNC_keymgmt_import() function.
