...
different library contexts. In many circumstances it is possible that an
B<EVP_PKEY> structure can be used in a library context B after having been
created in a library context A, namely iff both library contexts contain a
provider implementing the key management interface for the given key type,
and the key manager in context A also implements the OSSL_FUNC_keymgmt_export()
function, and the key manager in context B does implement the
OSSL_FUNC_keymgmt_import() function and both providers are able to parse and
output, respectively, the same key formats.
