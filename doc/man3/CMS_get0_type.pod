...
content type.
It differs from CMS_get0_type in that it points to the actual eContentType element contained in the ASN1 structure that was constructed or received.
...
While this function will always return a valid B<ASN1_OCTET_STRING> object, it may point to NULL when it is used on outgoing objects created with B<CMS_PARTIAL>.
Calling L<CMS_final> will finalize the object and make the pointers valid.
