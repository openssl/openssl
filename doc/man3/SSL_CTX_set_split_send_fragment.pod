...
a value set by SSL_CTX_set_record_size_limit() will be ignored and the limit will be the
same as the maximum fragment length value. Both extensions are gonna be sent, and a server will choose
record size limit over maximum fragment length if it supports the extension.
Both server and client can set this limit, but only client may initiate this extension use.
...
affected by this limit unlike maximum fragment length.
Record size limit is asymmetric : a server can advertise a limit that is different than the client.
