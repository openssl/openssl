...
If you configure both a maximum fragment length and a record size limit, they will both be sent during the handshake,
...
SSL_set_record_size_limit() has the same semantics of the above function. It only affects SSL connections created after
this function is called.

See the B<Notes> to understand the key differences between maximum fragment length and record size limit.

If SSL_CTX_set_max_send_fragment() is set to a value lower than the negotiated maximum fragment length or record size
limit, this value will be used instead for limiting sent fragment.
...
SSL_get_max_send_fragment() retrieve the maximum fragment size of plaintext bytes used for outbound records.
SSL_get_max_recv_fragment() retrieve the maximum fragment size of plaintext bytes acceptable for inbound records.

These functions cannot be used with QUIC SSL objects.
SSL_set_max_send_fragment(), SSL_set_max_pipelines(), SSL_set_record_size_limit(),
...
set in the handshake that produces the keys that are used to protect those records. This admits the possibility that the
extension might not be negotiated when a connection is renegotiated or resumed.
