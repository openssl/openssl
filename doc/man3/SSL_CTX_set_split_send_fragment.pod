...
SSL_CTX_set_record_size_limit, SSL_set_record_size_limit,
SSL_CTX_get_record_size_limit, SSL_get_record_size_limit,
SSL_get_usable_max_send_size - Control fragment size settings and pipelining operations

=head1 SYNOPSIS

 #include <openssl/ssl.h>

 long SSL_CTX_set_max_send_fragment(SSL_CTX *ctx, long m);
 ...
 long SSL_CTX_set_record_size_limit(SSL_CTX *ctx, long len);
 long SSL_set_record_size_limit(SSL *ssl, long len);

 long SSL_CTX_get_record_size_limit(SSL_CTX *ctx);
 long SSL_get_record_size_limit(SSL *ssl);

 long SSL_get_usable_max_send_size(SSL *ssl);
...
This value has also an influence on the size of the memory that will be
allocated for the output record buffer, and the initial size of the handshake
buffer (ssl->init_buf).
...
SSL_CTX_set_record_size_limit() sets the default record size limit
value to be negotiated via the value B<len> to B<ctx>.
This setting affects only SSL instances created after this function is called.
The allowed values are 0 to disable this option, which is the default,
and 64 to 16385 (SSL3_RT_MAX_PLAIN_LENGTH + 1) to enable it.  It is
recommended to set this value to 16385, to advertise the record size limit
extension, even if there is no need to limit the size of records.
In TLS1.3 the value includes an additional byte for the encrypted record type,
while in TLS1.2 the record type is unencrypted.
The value, if negotiated, will be used to limit the record sizes sent by the
peer in reception direction only.  However the record sizes sent to the peer
will be limited by the peer's record size limit value.  If a lower limit of
the send fragment size is set by the function
L<SSL_CTX_set_max_send_fragment(3)> that limit will not be exceeded even
if the peer advertises a higher record size limit.
In resource-constrained environments it is furthermore recommended to use
L<SSL_CTX_set_max_send_fragment(3)> or L<SSL_set_max_send_fragment(3)> to
limit the memory allocation size for output record buffers, and additionally
to use L<SSL_CTX_set_max_cert_list(3)> or L<SSL_set_max_cert_list(3)> to
reduce the maximum size of the certificate chain from the default of
100 KBytes to 16384.  Further reduction of this parameter is possible,
but does not have an effect on the maximum memory size per SSL connection.

SSL_set_record_size_limit() sets the default record size limit
value to be negotiated via the value B<len> to B<ssl>.
This setting will be used during a handshake when extensions are exchanged
between client and server.
The value may be changed before or after a connection was established, but
it may not be changed while the handshake is done.
When the connection is already established, the value will only be used
after the next renegotiation, so usually L<SSL_renegotiate(3)> or
L<SSL_renegotiate_abbreviated(3)> should be called in order to make the
new record size limit effective.
If this function is called while a handshake is done, the return value will
be -1, in that case the value is ignored.
If the value is outside the allowed range, the return value is 0.
Otherwise the return value is 1.

SSL_CTX_get_record_size_limit() returns the currently in B<ssl> configured
record size limit value.

SSL_get_record_size_limit() returns the currently in B<ctx> configured
record size limit value.

SSL_get_usable_max_send_size() returns the maximum data size that can be
sent in a single data record.  Since the record size limit extension,
unlike the max fragment length extension, can be renegotiated at any time,
the usable max send size may change over time.
While TLS will automatically split larger send requests into smaller
records, DTLS will fail any attempt to send larger datagrams.
...
Likewise if the Record Size Limit extension is supported by client and
server, the server must ignore the Maximum Fragment Length extension.
...
The SSL_CTX_set_record_size_limit(), SSL_set_record_size_limit(),
SSL_CTX_get_record_size_limit(), SSL_get_record_size_limit()
and SSL_get_usable_max_send_size() functions were added in OpenSSL 3.1.
