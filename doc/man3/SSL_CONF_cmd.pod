...
Disables support for SSL/TLS record compression, same as setting
B<SSL_OP_NO_COMPRESSION>.
As of OpenSSL 1.1.0, compression is off by default. In OpenSSL 5.0 and later,
TLS record compression was removed; this option has no effect.

=item B<-comp>

Enables support for SSL/TLS record compression, same as clearing
B<SSL_OP_NO_COMPRESSION>.
This command was introduced in OpenSSL 1.1.0.
As of OpenSSL 5.0, TLS record compression was removed; this option has no
effect. In earlier versions, compression was off by default and could only be
used at security level 1 or lower. See L<SSL_CTX_set_security_level(3)>.
...
B<Compression>: SSL/TLS record compression. As of OpenSSL 5.0, TLS record
compression was removed; this option has no effect. Inverse of
B<SSL_OP_NO_COMPRESSION>.
...
Enable compression (no effect in OpenSSL 5.0 and later; TLS record
