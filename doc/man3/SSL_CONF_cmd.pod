...
The B<value> argument is the expected DNS hostname to use for
certificate verification.  Any previous value is cleared.  This option
...
The B<value> argument is an expected DNS hostname to use for
certificate verification, in addition to any previously set value.  This option
is intended for configurations that are dedicated to connecting to a single
server or cluster of servers, and not appropriate for configurations that
are used to connect to a diverse set of destinations.
...
The B<value> argument is an expected IPv4 or IPv6 address to use for
certificate verification, in addition to any previously set value.  This option
is intended for configurations that are dedicated to connecting to a single
server or cluster of servers, and not appropriate for configurations that
are used to connect to a diverse set of destinations.
