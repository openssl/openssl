...
The deprecated Suite B modes (enabled via B<SSL_CERT_FLAG_SUITEB_128_LOS> or
B<SSL_CERT_FLAG_SUITEB_192_LOS>, and only available if OpenSSL was configured
with C<enable-deprecated-suiteb>) always imply strict mode.
...
The recommended groups for (D)TLS 1.3 are presently documented in the default
...
Controls use of (D)TLSv1.3 record layer padding.  B<padding> is a string of the
