...
=item B<CiphVersionMask>

Sets the list of versions. This list will be used as the mask for cipher strings
where no version was specified. The format for this list is a simple vertical bar
("|") separated list of version names. This option, if used, will set
B<Ciphersuites> to empty string. See L<openssl-ciphers(1)> for more information.

=item B<CipherString>

Sets the ciphersuite list to B<value>. If there are no TLSv1.3 ciphersuites and
B<CiphVersionMask> is not used, this list will be combined with any TLSv1.3
ciphersuites that have been configured using B<Ciphersuites>. Note: syntax
...
If at least one valid TLSv1.3 ciphersuite is in the B<CipherString>, or parameter
B<CiphVersionMask> is used, all ciphers entered here will be erased
