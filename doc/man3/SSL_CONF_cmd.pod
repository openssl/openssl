...
authorities for validation. These options are only supported
if certificate operations are permitted.
...
=item B<CertValidation>

The B<value> argument is a comma separated list of certificate validation
flags to set.  If a flag string is preceded B<-> it is disabled.
...
B<Strict> disables workarounds for for some broken certificates, and
...
B<SuiteB128Only> Equivalent to B<X509_V_FLAG_SUITEB_128_LOS_ONLY>.

B<SuiteB192> Equivalent to B<X509_V_FLAG_SUITEB_192_LOS>.

B<SuiteB128> Equivalent to B<X509_V_FLAG_SUITEB_128_LOS>.
...
=item B<SetValidFlags>

The B<value> argument is a comma separated list of host validation flags
to set.  If a flag string is preceded B<-> it is disabled.
...
B<NeverCheckSubject> causes vertificate hostname verification to
never consider the subject DN.  Equivalent to
B<X509_CHECK_FLAG_NEVER_CHECK_SUBJECT>.

=item B<SetValidHost>

The B<value> argument is the expected DNS hostname to use for certificate
verification.  Any previous value is cleared.

=item B<AddValidHost>

The B<value> argument is an expected DNS hostname to use for certificate
verification, in addition to any previously set value.

=item B<SetValidIP>

The B<value> argument is the expected IPv4 or IPv6 address to use for
certificate verification.  Any previous value is cleared.

=item B<SetValidHostOrIP>

The B<value> argument is the expected DNS hostname, IPv4 address, or IPv6
address to use for certificate verification.  If it is a hostname, any
previous hostname is cleared.  If it is an IP address, any previous IP
address is cleared.
