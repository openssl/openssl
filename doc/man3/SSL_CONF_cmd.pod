...
Note that, for QUIC objects, padding is always preformed at the
packet level, and so cannont be done at the record level.  Given that, when the
config file is created, there is no knoweldge of what kind of SSL objects are
