...
registry.  For some groups, OpenSSL supports additional aliases, such as their
B<NIST> name (e.g. B<P-256>), another commonly used name (e.g. B<X25519>,
B<ffdhe2048>) or an OpenSSL OID name (e.g. B<prime256v1>).  Group names are
case sensitive. The list should be in order of preference with the most
...
OpenSSL 3.5 introduces support for a few hybrid ECDH post-quantum (PQ) key
exchange TLS I<groups>.  Among these B<X25519MLKEM768> offers reasonable
performance, be at the cost of much larger key exchange messages (~1200 bytes,
in comparison with 32 bytes for B<X25519> and 65 bytes for B<secp256r1>, a.k.a.
NIST B<P-256>).
