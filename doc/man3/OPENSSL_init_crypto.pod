...
Enabling B<config_diagnotics> at the top level (default section) of the
configuration file will cause errors to be reported even when the
B<CONF_MFLAGS_IGNORE_RETURN_CODES> flag is set, this will also report
provider action errors unless the B<CONF_MFLAGS_SILENT_ACTIVATION> flag is set.
