...
OPENSSL_INIT_free, OPENSSL_init_crypto, OPENSSL_cleanup, OPENSSL_cleanup_ex, OPENSSL_atexit,
OPENSSL_add_library_user, OPENSSL_thread_stop_ex, OPENSSL_thread_stop - OpenSSL initialisation
...
The OPENSSL_add_library_user() call is used to increase the reference count on
libcrypto as a whole.  Each call to OPENSSL_add_library_user must be paired with
a call to OPENSSL_cleanup_ex(), which drops the library reference count.  Only
after reaching a reference count of 0 will library resources actually be
released.  Failure to balance these calls properly will result in memory leaks
(in the event that OPENSSL_add_library_user() is called more often than
OPENSSL_cleanup()), or in undefined behavior resulting from using the library
while it is uninitialized (in the event OPENSSL_cleanup_ex() is called more often
than OPENSSL_add_library_user()).

The OPENSSL_cleanup() function deinitialises OpenSSL (both libcrypto
and libssl). All resources allocated by OpenSSL are freed.  Note that
OPENSSL_cleanup() in favor of OPENSSL_cleanup_ex which must be paired with a
call to OPENSSL_add_library_user().  OPENSSL_cleanup() can still be used by
legacy applications, but will perform no operation if any other software
component has use the OPENSSL_add_library_user() call.  This ensures that,
should other software components use the new library reference counting
mechanism available via OPENSSL_add_library_user() and OPENSSL_cleanup_ex(),
that calls to OPENSSL_cleanup() do not free library resources prematurely.
However, this change in behavior may lead to memory leaks if all software
components in a process do not update to the new mechanism.  See
L<ossl-guide-migration(7)> for more details.

Once OPENSSL_cleanup() has been called, Or once OPENSSL_cleanup_ex() has been
called and the library reference count reaches 0, the library cannot be reinitialised.
...
The OPENSSL_add_library_user() and OPENSSL_cleanup_ex() calls was added in OpenSSL 4.0.0.

The OPENSSL_cleanup() function was deprecated in OpenSSL 4.0.0.
