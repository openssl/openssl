...
with the current thread for the given OSSL_CTX B<ctx>. The B<ctx> parameter
can be NULL in which case the default OSSL_CTX is used.

Typically, this function will be called automatically by the library when
the thread exits as long as the OSSL_CTX has not been freed before the thread
exits. If OSSL_CTX_free() is called OPENSSL_thread_stop_ex will be called
...
OSSL_CTX is freed.
Typically this is not necessary for the default OSSL_CTX (because all
...
default OSSL_CTX is always used.
