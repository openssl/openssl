...
OPENSSL_thread_stop_ex, OPENSSL_thread_stop - OpenSSL initialization
...
With this option the library with automatically initialize the libcrypto async
sub-library (see L<ASYNC_start_job(3)>). This is a default option.

=item OPENSSL_INIT_ENGINE_RDRAND

With this option the library will automatically load and initialize the
...
With this option the library will automatically load and initialize all the
...
"atexit" handler. Using this option suppresses that behavior. This means that
...
on auto-deinitialization. This is to avoid error conditions where both an
application and a library it depends on both use OpenSSL, and the library
deinitialises it before the application has finished using it.

Once OPENSSL_cleanup() has been called the library cannot be reinitialized.
Attempts to call OPENSSL_init_crypto() will fail and an ERR_R_INIT_FAIL error
will be added to the error stack. Note that because initialization has failed
...
called after deinitialization of resources local to a thread, but before other
