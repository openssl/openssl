...
libssl). All resources allocated by OpenSSL are freed.  Applications should
call OPENSSL_cleanup() explicitly, or register it with atexit().
...
B<OPENSSL_INIT_NO_ATEXIT> was rendered a no-op in OpenSSL 3.1.
