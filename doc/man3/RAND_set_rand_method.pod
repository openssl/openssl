...
RAND_set_rand_method, RAND_get_rand_method, RAND_Opentls - select RAND method
...
A B<RAND_METHOD> specifies the functions that Opentls uses for random number
generation.

RAND_Opentls() returns the default B<RAND_METHOD> implementation by Opentls.
...
be used instead of the method returned by RAND_Opentls().
...
RAND_get_rand_method() and RAND_Opentls() return pointers to the respective
methods.
