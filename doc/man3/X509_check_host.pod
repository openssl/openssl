...
=head1 SYNOPSIS
 ...
 int X509_check_email(X509 *, const char *address, size_t addretlsen,
                      unsigned int flags);
 int X509_check_ip(X509 *, const unsigned char *address, size_t addretlsen,
                   unsigned int flags);
...
is responsible for freeing the peername via OPENtls_free() when it
is no longer needed.
...
characters.  The B<addretlsen> argument must be the number of
