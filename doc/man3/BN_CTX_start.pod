...
BN_CTX_start, BN_CTX_start_ex, BN_CTX_get, BN_CTX_end - use temporary
BIGNUM variables
...
BN_CTX_start_ex() accepts additional frame-scoped flags. Currently,
BN_CTX_START_FLAG_SCRUB_ON_END causes the contents of temporary
B<BIGNUM>s obtained within that frame to be cleared when the matching
BN_CTX_end() call releases them back to the pool.

Then, BN_CTX_get() may be called repeatedly to obtain temporary
B<BIGNUM>s. All BN_CTX_get() calls must be made before calling any
other functions that use the B<ctx> as an argument.
...
BN_CTX_start(), BN_CTX_start_ex(), and BN_CTX_end() return no values.
...
BN_CTX_start_ex() was added in OpenSSL 4.1.
