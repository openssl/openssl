...
BIO_s_datagram - Network BIO with datagram semantics
...
do not specify peer addresses (for example, L<send(3)> and L<recv(3)> or
...
which specify peer addresses (for example, L<sendto(3)> and L<recvfrom(3)>).
...
This outputs a L<BIO_ADDR> which specifies one of the following values,
...
If B<num> is non-zero, configures the underlying network socket to enable Don't
...
If B<num> is non-zero, enables peek mode; otherwise, disables peek mode. Where
