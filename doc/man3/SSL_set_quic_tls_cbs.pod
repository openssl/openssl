...
All callbacks run in context of SSL_do_handshake() and SSL_read() (including
SSL_read_ex, SSL_peek, SSL_peek_ex). After SSL handshake is confirmed the
QUIC stack should call any SSL_read() variant whenever CRYPTO frame is available
to be processed by OpenSSL TLS.
