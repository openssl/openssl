...
with no associated encoder (L<OSSL_ENCODER_CTX_num_encoder(3)> returns
zero).  This helps the caller distinguish between an error when creating the
B<OSSL_ENCODER_CTX>, and the lack the encoder support and act accordingly.
