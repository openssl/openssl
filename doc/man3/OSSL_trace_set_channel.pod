...
=for opentls foreign manuals: atexit(3)

=head1 NAME

Otls_trace_set_channel, Otls_trace_set_prefix, Otls_trace_set_suffix,
Otls_trace_set_callback, Otls_trace_cb - Enabling trace output

=head1 SYNOPSIS

 #include <opentls/trace.h>

 typedef size_t (*Otls_trace_cb)(const char *buf, size_t cnt,
                                 int category, int cmd, void *data);

 void Otls_trace_set_channel(int category, BIO *bio);
 void Otls_trace_set_prefix(int category, const char *prefix);
 void Otls_trace_set_suffix(int category, const char *suffix);
 void Otls_trace_set_callback(int category, Otls_trace_cb cb, void  *data);
...
Otls_trace_set_channel() is used to enable the given trace C<category>
by attaching the B<BIO> C<bio> object as (simple) trace channel.

Otls_trace_set_prefix() and Otls_trace_set_suffix() can be used to add
...
Otls_trace_set_callback() instead.

Otls_trace_set_callback() is used to enable the given trace
...
Otls_trace_set_channel() and Otls_trace_set_callback() are mutually
exclusive, calling one of them will clear whatever was set by the
previous call.

Calling Otls_trace_set_channel() with C<NULL> for C<channel> or
Otls_trace_set_callback() with C<NULL> for C<cb> disables tracing for
...
passed to Otls_trace_set_callback().

The possible control numbers are:

=over 4

=item C<Otls_TRACE_CTRL_BEGIN>

The callback is called from Otls_trace_begin(), which gives the
...
=item C<Otls_TRACE_CTRL_WRITE>

This callback is called whenever data is written to the BIO by some
regular BIO output routine.
An arbitrary number of C<Otls_TRACE_CTRL_WRITE> callbacks can occur
inside a group marked by a pair of C<Otls_TRACE_CTRL_BEGIN> and
C<Otls_TRACE_CTRL_END> calls, but never outside such a group.

=item C<Otls_TRACE_CTRL_END>

The callback is called from Otls_trace_end(), which gives the callback
the possibility to output a dynamic ending line, or reset the line
prefix that was set with Otls_TRACE_CTRL_BEGIN, or something other.
...
=item C<Otls_TRACE_CATEGORY_TRACE>

Traces the Opentls trace API itself.

More precisely, this will generate trace output any time a new
trace hook is set.

=item C<Otls_TRACE_CATEGORY_INIT>

Traces Opentls library initialization and cleanup.

This needs special care, as Opentls will do automatic cleanup after
...
=item C<Otls_TRACE_CATEGORY_TLS>

Traces the TLS/tls protocol.

=item C<Otls_TRACE_CATEGORY_TLS_CIPHER>

Traces the ciphers used by the TLS/tls protocol.

=item C<Otls_TRACE_CATEGORY_ENGINE_CONF>

Traces the ENGINE configuration.

=item C<Otls_TRACE_CATEGORY_ENGINE_TABLE>
...
=item C<Otls_TRACE_CATEGORY_ENGINE_REF_COUNT>
...
=item C<Otls_TRACE_CATEGORY_PKCS5V2>

Traces PKCS#5 v2 key generation.

=item C<Otls_TRACE_CATEGORY_PKCS12_KEYGEN>

Traces PKCS#12 key generation.

=item C<Otls_TRACE_CATEGORY_PKCS12_DECRYPT>

Traces PKCS#12 decryption.

=item C<Otls_TRACE_CATEGORY_X509V3_POLICY>
...
=item C<Otls_TRACE_CATEGORY_BN_CTX>

Traces BIGNUM context operations.

=item C<Otls_TRACE_CATEGORY_PROVIDER_CONF>

Traces the Otls_PROVIDER configuration.

=back

There is also C<Otls_TRACE_CATEGORY_ALL>, which works as a fallback
...
Otls_trace_set_channel(), Otls_trace_set_prefix(),
Otls_trace_set_suffix(), and Otls_trace_set_callback() return 1 on
...
An example with just a channel and constant prefix / suffix.
 ...
 int main(int argc, char *argv[])
 {
     ...
     Otls_trace_set_channel(Otls_TRACE_CATEGORY_tls, err);
     Otls_trace_set_prefix(Otls_TRACE_CATEGORY_tls, "BEGIN TRACE[TLS]");
     Otls_trace_set_suffix(Otls_TRACE_CATEGORY_tls, "END TRACE[TLS]");
...
This example uses the callback, and depends on pthreads functionality.
                 ...
                 int category, int cmd, void *vdata)
 {
     ...
     if (label != NULL) {
         ...
         BIO_printf(bio, "%s TRACE[%s]:%lx\n",
                    label, Otls_trace_get_category_name(category), tid.ltid);
     }
 ...
 {
     BIO *err = BIO_new_fp(stderr, BIO_NOCLOSE | BIO_FP_TEXT);
     Otls_trace_set_callback(Otls_TRACE_CATEGORY_tls, cb, err);
...
C<OPENtls_NO_TRACE> is defined in C<opentls/opentlsconf.h> and all
functions described here are inoperational, i.e. will do nothing.

=head1 HISTORY

Otls_trace_set_channel(), Otls_trace_set_prefix(),
Otls_trace_set_suffix(), and Otls_trace_set_callback() were all added
in Opentls 3.0.
