...
If a non-application data record had been processed, the read function can
return and set the error to B<SSL_ERROR_WANT_READ>.
This behaviour can be controlled with the B<SSL_MODE_AUTO_RETRY> flag of the
L<SSL_CTX_set_mode(3)> call.

If the underlying BIO is B<blocking>, a read function will only return once the
read operation has been finished or an error occurred, except when a
non-application data record had been processed and B<SSL_MODE_AUTO_RETRY> is
not set.
...
As at any time it's possible that non-application data needs to be send,
