...
EVP_KDF_CTX_set_skey() allows to set an skey as the input key for the
derivation functions. It is equivalent to setting the parameter
B<OSSL_KDF_PARAM_KEY> when the input key is an B<EVP_SKEY> rather than raw
bytes.
