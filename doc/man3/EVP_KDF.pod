...
EVP_KDF_CTX_set_SKEY, EVP_KDF_derive_SKEY, EVP_KDF_derive_SKEYs,
EVP_KDF_CTX_get0_SKEY, EVP_KDF_CTX_get1_SKEY, EVP_KDF_CTX_get0_data,
...
=head1 SYNOPSIS
 ...
 EVP_SKEY *EVP_KDF_CTX_get0_SKEY(EVP_KDF_CTX *ctx, EVP_KDF_CTX *ctx,
                                 EVP_SKEYMGMT *skeymgmt, const char *key_type,
                                 const char *propquery, const char *purpose);
 EVP_SKEY *EVP_KDF_CTX_get1_SKEY(EVP_KDF_CTX *ctx, EVP_KDF_CTX *ctx,
                                 EVP_SKEYMGMT *skeymgmt, const char *key_type,
                                 const char *propquery, const char *purpose);
...
EVP_KDF_derive_SKEYs() is used to derive multiple B<EVP_SKEY> objects in
a single step. It can be used with KDFs that allow derivation of multiple
outputs while avoiding multiple calls to providers for each output (for
example L<EVP_KDF-TLS1_PRF(7)> may derive 2/4 keys and 2 IVs). This function
does not return any objects when it succeeds - they are stored in I<ctx>.
The parameters in I<params> are specific for each KDF and may contain
information about the requested key lengths, number of keys, etc. If no
parameters are specified, the KDF may use internal defaults. The
parameters may also be set with a call to EVP_KDF_CTX_set_params().

EVP_KDF_CTX_get0_SKEY() retrieves B<EVP_SKEY> objects that were derived
by a preceding call to EVP_KDF_derive_SKEYs(). It can be called multiple
times, for each desired output. If the I<skeymgmt> argument is explicitly
passed, it will be used for the generated key. If the I<skeymgmt> argument
is NULL, the I<key_type> and I<propquery> will be used for fetching the
B<EVP_SKEYMGMT> object linked to the key. I<purpose> is a name that
identifies which of the multiple derived keys is requested (e.g.
"client_write_MAC_key" or "server_write_key"), as specified by the
documentation of the specific KDF. EVP_KDF_CTX_get1_SKEY() does the same
and also increases the reference count of the returned B<EVP_SKEY>.

EVP_KDF_CTX_get0_data() can be used to retrieve data that may also have
been derived by the KDF. I<purpose> is a name that identifies which piece
of data is requested (e.g. "client_write_IV"), as specified by the
documentation of the specific KDF. I<datalen> can be used to get the
