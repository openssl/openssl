...
EVP_KDF_derive_SKEY() behaves similar except it accepts an B<EVP_SKEY> object
as an output argument. The B<EVP_SKEY> object must be the fresh one and have a
keymgmt from the same provider as the ctx has.
