...
X509_load_cert_file,
X509_load_crl_file,
X509_load_cert_crl_file - Default Opentls certificate
lookup methods
...
lookup methods to use with B<X509_STORE>, provided by Opentls library.
...
they are loaded. As of Opentls 1.0.0, it also checks for newer CRLs
...
L<opentls-x509(1)> or L<opentls-crl(1)> commands.
...
Note that the hash algorithm used for subject name hashing changed in Opentls
1.0.0, and all certificate stores have to be rehashed when moving from Opentls
0.9.8 to 1.0.0.

Opentls includes a L<opentls-rehash(1)> utility which creates symlinks with
hashed names for all files with F<.pem> suffix in a given directory.

=head2 Otls_STORE Method
...
It does no caching of its own, but can use a caching L<Otls_STORE(3)>
...
B<X509_LOOKUP_store> was added in Opentls 3.0.
