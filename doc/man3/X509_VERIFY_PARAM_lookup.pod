...
C<smime_sign>, C<pkcs7> (which is equivalent to C<smime_sign>), and C<default>.

These entries are immutable, but the list may be extended.

X509_VERIFY_PARAM_lookup() locates the B<X509_VERIFY_PARAM> structure
...
X509_VERIFY_PARAM_add0_table() and X509_VERIFY_PARAM_table_cleanup()
do not implement locking. For this reason,
they must not be used concurrently with any functions that access the table.
These include not only the functions X509_VERIFY_PARAM_get_count(),
X509_VERIFY_PARAM_get0(), and X509_VERIFY_PARAM_lookup() documented here,
but also any of their users like L<X509_STORE_CTX_set_default(3)>,
L<X509_verify_cert(3)>, and L<X509_STORE_CTX_verify(3)>,
as well as their numerous users.

=head1 SEE ALSO

L<X509_STORE_CTX_set_default(3)>,
