...
If, for example, the loaded configuration file contains the following:
 ...
 tls12 = tls12_settings
 tls13 = tls13_settings
...
An application that wants to only use "TLS 1.2" might call:
 ...
 /*
  * Futher tweaks per the "ssl" module "tls12" setting, i.e. the
  * configuration file's "tls12_settings" section.
  */
 if (SSL_CTX_config(ctx, "tls12") == 0) {
     fprintf(stderr, "Error applying 'tls12' context configuration.\n");
     goto err;
 }
...
application.
Similarly, using C<tls13> as the configuration B<name> applies the corresponding
settings from the "tls13_settings" section (per the C<tls13> SSL module parameter).
