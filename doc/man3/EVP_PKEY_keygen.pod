...
=head1 SYNOPSIS
 ...
 EVP_PKEY *EVP_PKEY_generate_public_key(OSSL_LIB_CTX *libctx,
                                        const char *propq,
...
EVP_PKEY_generate_public_key() generates an B<EVP_PKEY> with public key
components from I<private_key> that contains just private key components
if the provider and the algorithm of the key supports the operation.
If I<private_key> already contains the public key components the function
duplicates I<private_key>.
If I<keep_private> is set to nonzero the returned B<EVP_PKEY> will also
contain the private key components otherwise they will be dropped from
the resulting key.
...
EVP_PKEY_Q_keygen() and EVP_PKEY_generate_public_key() return an B<EVP_PKEY>,
or NULL on failure.
...
EVP_PKEY_generate_public_key() was added in OpenSSL 3.1.
