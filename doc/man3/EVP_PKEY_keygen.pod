...
In Opentls an EVP_PKEY structure containing a private key also contains the
public key components and parameters (if any). An Opentls private key is
...
Example of generation callback for Opentls public key implementations:
