...
functions and macros are used primarily by BIO implementations and by code
that builds BIO chains to manipulate those flags.

BIO_set_flags() sets the bits given in B<flags> in the BIO B<b>.  Any bits
already set in the B<BIO>'s flag word remain set.

BIO_clear_flags() clears the bits given in B<flags> from the BIO B<b>.  Any
other bits in the flag word are left unchanged.

BIO_test_flags() tests the bits given in B<flags> in the BIO B<b> and
returns a nonzero value if any of them are currently set and zero
otherwise.

BIO_get_flags() returns the current flag word from the BIO B<b>.  This is
...
BIO_set_retry_read() marks the BIO B<b> as being in a retryable state
...
BIO_set_retry_write() marks the BIO B<b> as being in a retryable state
by setting the B<BIO_FLAGS_SHOULD_RETRY> flag. In addition, it sets the
B<BIO_FLAGS_WRITE> flag to indicate that the retry condition is
associated with a write operation.

BIO_set_retry_special() marks the BIO B<b> as being in a retryable state
by setting the B<BIO_FLAGS_SHOULD_RETRY> flag. In addition, it sets the
...
The precise meaning of this condition depends on the BIO type.

BIO_clear_retry_flags() clears all retry-related bits from B<b>, i.e.
...
currently set in B<b>.  The result is a subset of
...
This flag is normally set by the BIO implementation via BIO_set_retry_read()
...
This flag is normally set by the BIO implementation via BIO_set_retry_write()
...
becomes true.  The precise meaning of this condition depends on the BIO
...
Set if the last I/O operation on the BIO should be retried at a later time.
If this bit is not set then the condition is treated as an error.

=item B<BIO_FLAGS_BASE64_NO_NL>

When set on a base64 filter BIO this flag disables the generation of
...
When set on a memory BIO this flag indicates that the underlying buffer is
read only.  Attempts to write to such a BIO will fail.

=item B<BIO_FLAGS_NONCLEAR_RST>

On a memory BIO this flag modifies the behaviour of BIO_reset().  When it
is set, resetting the BIO does not clear the underlying buffer but only
resets the current read position.

=item B<BIO_FLAGS_IN_EOF>

This flag is used internally in BIO implementations to record that
the data source has reached its end and no further data is expected
...
argument that are currently set in the BIO. Consequently, it returns a
...
B<BIO_FLAGS_READ>, B<BIO_FLAGS_WRITE>, B<BIO_FLAGS_IO_SPECIAL>, and
B<BIO_FLAGS_SHOULD_RETRY> that are currently set in the BIO.

=head1 NOTES

Ordinary application code will rarely need to call BIO_set_flags(),
BIO_clear_flags() or BIO_test_flags() directly.  They are intended for BIO
implementations and for code that forwards retry state from one BIO in a
chain to another.
...
These functions and macros are not thread-safe. If a single BIO
...
at least 1.1.0 (BIO_FLAGS_IN_EOF since 1.1.1).
