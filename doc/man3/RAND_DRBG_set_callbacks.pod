...
Setting the callbacks is allowed only if the DRBG has not been initialised yet.
...
The derivation function is disabled during initialisation by calling the
...
utilise for the nonce, following the recommendations of [NIST SP 800-90A Rev. 1],
section 8.6.7.
