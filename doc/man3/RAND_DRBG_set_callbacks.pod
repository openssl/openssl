...
Even if a nonce is required but the B<get_nonce>() and B<cleanup_nonce>()
callbacks can be omitted (by setting them to NULL), in which case the DRBG will
automatically request an extra amount of entropy
