...
The returned value should either be random and provide B<security_strength> bits
of entropy or not expected to repeat more often than a B<security_strength> bit
random string.
...
It is important that B<cleanup_entropy>() clear the buffer contents safely
before freeing it, in order not to leave sensitive information about the DRBG's
state in memory.
