...
I<a> and I<b>. Certificates for which L<X509_check_purpose(3)> fails while
caching internal fields are ordered after those for which it succeeds. Two
certificates that fail this check are compared using their complete canonical
(DER) encodings. Otherwise, the comparison is based on the B<memcmp> result of
the hash values of the two B<X509> objects and the canonical (DER) encoding
values.
