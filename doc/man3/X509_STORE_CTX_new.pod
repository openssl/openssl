...
X509_STORE_CTX_free, X509_STORE_CTX_init, X509_STORE_CTX_set0_trace_output,
...
X509_STORE_CTX_set0_trace_output() can be used to enable reporting certificate
verification activity, in particular the checks done on candidate certificates
during chain building.
Output is sent to the BIO given as the I<out> parameter.
If it is NULL, this reporting disabled, which is the default.
Note that essentially the same output can also be achieved by enabling
the trace API with B<OSSL_TRACE_CATEGORY_X509_VERIFY>.
...
X509_STORE_CTX_set0_trace_output, X509_STORE_CTX_set0_trusted_stack(),
