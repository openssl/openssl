...
X509_STORE_CTX_set0_ocsp_resp() sets the OCSP response(s) for the verification
of a certificate chain or for including in the TLS handshake, when the client
requests OCSP stapling. The stack of OCSP responses are not copied, it is just
stored to the context.
Ownership of the chain is transferred to I<ctx>, and so it should not be free'd
by the caller.
...
X509_STORE_CTX_set0_ocsp_resp() function was added in OpenSSL 3.5.
