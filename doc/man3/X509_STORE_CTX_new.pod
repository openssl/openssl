...
X509_STORE_CTX_set0_store, X509_STORE_CTX_set_cert,
...
X509_STORE_CTX_set0_store() sets the certificate store of B<ctx> to
B<store>.  This is similar to X509_STORE_CTX_set0_trusted_stack() in that
it allows to specify a different set of trusted certificates to be used,
but also has other effects since the certificate store is used for
operations other than looking up certificates and getting issuers, such
as holding CRLs.  The additional roles performed by the store object
are described in L<X509_STORE_set_verify_cb_func>.
