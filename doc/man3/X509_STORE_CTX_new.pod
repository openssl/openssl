...
Among others, the verification parameters with name C<default> are set.
...
in I<ctx> using the function L<X509_VERIFY_PARAM_inherit(3)>.
This uses the function L<X509_VERIFY_PARAM_lookup(3)> to find the set of
verification parameters associated with the given verification method I<name>.
Among others, the parameters determine the trust ID and verification purpose,
unless the purpose was already set before and B<X509_VP_FLAG_OVERWRITE> is not set.
More detail, including the list of currently predefined methods,
is described in L<openssl-verification-options(1)/Verification Methods>.
...
An X.509 certificate may include information about what the key contained in it
is allowed to be used for, for example, "TLS Server Authentication" or
"Email Protection". This extended key usage (EKU) information, as defined in
RFC 5280 section 4.2.1.12, is held internally to the certificate itself
in the form of an X.509v3 certificate extension.
In addition, as detailed in L<openssl-verification-options(1)/Trust Anchors>,
to-be-trusted certificates can be associated with an explicit designation
indicating for which uses of target certificates it may serve as a trust anchor.
This designation is not held within the certificate itself but is
"meta" information that can be attached to the certificate
after has been issued and could be determined by a system administrator.
For example, a certificate might declare in its EKU extension that it is
suitable for both "TLS Server Authentication" and "TLS Client Authentication",
but a system administrator might only trust it for the former.
This extended mechanism is arbitrarily extensible
...
L<X509_VERIFY_PARAM_lookup(3)>, L<X509_VERIFY_PARAM_inherit(3)>,
