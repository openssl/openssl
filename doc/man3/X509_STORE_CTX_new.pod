...
X509_STORE_CTX_set0_dane() associates DANE verification state with the
verification context. The I<dane> parameter is a pointer to an B<SSL_DANE>
structure obtained from L<SSL_get0_dane(3)>. This enables DANE TLSA (RFC6698 and
RFC7671) peer authentication during certificate verification. The I<dane>
pointer is stored internally and the caller must ensure the B<SSL_DANE>
structure remains valid for the duration of the verification operation. This
function is primarily used by applications that implement custom certificate
verification callbacks via L<SSL_CTX_set_cert_verify_callback(3)> and need to
support DANE authentication.
See L<SSL_CTX_dane_enable(3)> for more information about DANE support.
...
X509_STORE_CTX_set0_dane() was added in OpenSSL 1.1.0.
