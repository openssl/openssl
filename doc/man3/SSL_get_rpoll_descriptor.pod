...
BIO_get_rpoll_descriptor() and BIO_get_wpoll_descriptor() respectively on the
respective read and write BIOs configured on the SSL object.

On non-QUIC SSL objects, calls to SSL_net_read_desired() and
SSL_net_want_desired() function identically to calls to SSL_want_read() and
