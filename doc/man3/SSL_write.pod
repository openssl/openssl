...
SSL_write_ex, SSL_write, SSL_writev_ex, SSL_writev,
SSL_sendfile, OSSL_IOVEC - write bytes to a TLS/SSL connection
...
SSL_writev_ex() and SSL_writev() write B<iovcnt> buffers from the array B<iov>
into the specified B<ssl> connection. On success SSL_writev_ex() will store the
number of bytes written in B<*written>.
...
SSL_write_ex(), SSL_write(), SSL_writev_ex(), or SSL_writev().
...
You should not call SSL_write() or SSL_writev() with num=0, it will return an error.
SSL_write_ex() and SSL_writev_ex() can be called with num=0, but will not send application data to
the peer.

=head1 RETURN VALUES

SSL_write_ex() and SSL_writev_ex() will return 1 for success or 0 for failure. Success means that
...
For SSL_write() and SSL_writev() the following return values can occur:
