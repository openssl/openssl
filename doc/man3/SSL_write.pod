...
SSL_write_ex, SSL_write, SSL_writev_ex, SSL_sendfile - write bytes to a TLS/SSL connection
...
SSL_writev_ex() write B<iovcnt> buffers from the array B<iov> into the specified
B<ssl> connection. On success SSL_writev_ex() will store the number of bytes
written in B<*written>.
...
SSL_write_ex(), SSL_write(), or SSL_writev_ex().
