...
SSL_write_ex2, SSL_write_ex, SSL_write, SSL_writev, SSL_sendfile,
...
B<flags> argument of 0 is exactly equivalent to calling SSL_write_ex().

There is also a B<SSL_writev> counterpart to accept vectored IO.
Instead of accepting a single buffer, this method accept an array of
proprietary B<ossl_iovec> structures, which emulate the UNIX/Linux B<iovec>
structure since it is not universal on all platforms. More specifically,
the B<ossl_iovec> describes a region of memory, beginning at B<data> address
and with the size of B<data_len> bytes. B<_writev> calls use arrays of this
structure, where each element of the array represents a memory region, and
the whole array represents a vector of memory regions. The definition of
the B<ossl_iovec> structure is included above.

For the B<SSL_writev> method, aside from the B<flags> and B<*written>
arguments which mean the same as the non-vectored IO version, there is an
additional argument B<offset>. The B<offset> argument specifies how many
bytes starting from B<data> of the first B<OSSL_IOVEC> should be skipped
in the write process. The purpose of this is to enable correct
B<SSL_MODE_ENABLE_PARTIAL_WRITE> behaviour without needing the caller to
manually update the pointers of the B<OSSL_IOVEC>s.
