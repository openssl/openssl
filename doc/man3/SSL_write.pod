...
SSL_write_ex, SSL_write, SSL_writev, SSL_sendfile - write bytes to a TLS/SSL connection
...
SSL_writev() write B<iovcnt> buffers from the array B<iov> into the specified
B<ssl> connection. On success SSL_writev() will store the number of bytes
...
SSL_write_ex(), SSL_write(), or SSL_writev().
...
SSL_write_ex() and SSL_writev() can be called with num=0, but will not send application data to
the peer.

=head1 RETURN VALUES

SSL_write_ex() and SSL_writev() will return 1 for success or 0 for failure. Success means that
