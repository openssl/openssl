...
SSL_write_ex2, SSL_write_ex, SSL_write, SSL_writev_ex2, SSL_writev_ex, SSL_writev,
SSL_sendfile, SSL_WRITE_FLAG_CONCLUDE - write bytes to a TLS/SSL connection
...
=head1 SYNOPSIS
 ...
 int SSL_writev_ex2(SSL *s, const OSSL_IOVEC *iov, size_t iovcnt,
                   uint64_t flags,
                   size_t *written);
 int SSL_writev_ex(SSL *s, const OSSL_IOVEC *iov, size_t iovcnt,
                  size_t *written);
 int SSL_writev(SSL *ssl, const OSSL_IOVEC *iov, int iovcnt);
...
Each of these methods has a B<_writev> counterpart to accept vectored IO.
Instead of accepting a single buffer, these methods accept an array of
propietary B<ossl_iovec> structures, which emulate the UNIX/Linux B<iovec>
...
and with the size of B<data_len> bytes. B<_wwritev> calls use arrays of this
