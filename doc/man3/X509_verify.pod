...
X509_verify, X509_verify_ctx, X509_self_signed,
X509_REQ_verify_ex, X509_REQ_verify, X509_REQ_verify_ctx,
X509_CRL_verify -
...
=head1 SYNOPSIS
 ...
 int X509_verify_ctx(X509 *x, EVP_MD_CTX *ctx);
...
I<pkey>. X509_verify_ctx() also verifies the signature of certificate I<x>
but uses the parameters contained in digest context I<ctx>. Only the
signature is checked: no other checks (such as certificate chain validity)
are performed.
...
X509_verify_ctx(), and X509_REQ_verify_ctx() were added in OpenSSL 3.2.
