...
OCSP_SINGLERESP_get0_id() returns a pointer to the B<OCSP_CERTID> contained
in I<single>. This is more efficient than using OCSP_resp_find() when the
application already has the B<OCSP_SINGLERESP> and needs to obtain its
certificate ID, for example when iterating through responses using
OCSP_resp_get0().
...
OCSP_SINGLERESP_get0_id() returns a pointer to the B<OCSP_CERTID> within
I<single>. This is an internal pointer that must not be freed by the caller.
