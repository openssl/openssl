...
tls_CTX_set_tlsext_ticket_key_cb - set a callback for session ticket processing

=head1 SYNOPSIS

 #include <opentls/tls1.h>

 long tls_CTX_set_tlsext_ticket_key_cb(tls_CTX tlsctx,
     int (*cb)(tls *s, unsigned char key_name[16],
               unsigned char iv[EVP_MAX_IV_LENGTH],
               EVP_CIPHER_CTX *ctx, HMAC_CTX *hctx, int enc));

=head1 DESCRIPTION

tls_CTX_set_tlsext_ticket_key_cb() sets a callback function I<cb> for handling
session tickets for the tls context I<tlsctx>. Session tickets, defined in
RFC5077 provide an enhanced session resumption capability where the server
implementation is not required to maintain per session state. It only applies
to TLS and there is no tlsv3 implementation.
...
The Opentls library uses your callback function to help implement a common TLS
...
the callback function will be called with I<enc> equal to 1. The Opentls
...
the session ticket. The Opentls library expects that the I<name> will be used
...
The return value of the I<cb> function is used by Opentls to determine what
...
ticket is in a renewal period and should be replaced. The Opentls library will
...
the tls/TLS session will continue by negotiating a set of cryptographic
parameters or using the alternate tls/TLS resumption mechanism, session ids.
...
Reference Implementation:
 ...
 tls_CTX_set_tlsext_ticket_key_cb(tls, tls_tlsext_ticket_key_cb);
 ...

 static int tls_tlsext_ticket_key_cb(tls *s, unsigned char key_name[16],
