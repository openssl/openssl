...
As of OpenSSL 5.0, TLS record compression has been removed (CRIME hardening).
This option is retained for API compatibility but has no effect; compression
is never used. In earlier versions, this option disabled TLS record compression
(default on). Certificate compression (RFC 8879) is unaffected.
