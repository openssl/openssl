...
=item B<X509_V_ERR_MISSING_AUTHORITY_KEY_IDENTIFIER: AKID is missing>

Missing Authority Key Identifier.

=item B<X509_V_ERR_EMPTY_AUTHORITY_KEY_IDENTIFIER: AKID has no attributes>

Authority Key Identifier has no attributes.

=item B<X509_V_ERR_AKID_ISSUER_SERIAL_NOT_PAIRED: issuer and serial number must be paired>
...
The CRL Number or Base CRL Number extension of a CRL selected for revocation
checking contains a negative integer.
This is only reported when B<X509_V_FLAG_X509_STRICT> is set and CRL checking
is enabled.
Parsing and commands such as B<openssl crl -text> are unaffected.
Delta CRLs that are filtered out during CRL selection do not raise this error.
