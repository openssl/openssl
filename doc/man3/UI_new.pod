...
are encoded according to the current locale or (forWindows) code page.
For applications having different demands, these strings need to be
converted appropriately by the caller.
For Windows, it's possible to get an UTF8 encoded string by defining
the environment variable C<OPENSSL_WIN32_UTF8> prior to calling
UI_process() (one way is to define it before calling the application).
