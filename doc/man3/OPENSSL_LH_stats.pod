...
The information output by these functions may vary depending on how OpenSSL is
built. If the library is built with statistics enabled, statistical counters are
kept for each B<LHASH> and are output by these functions. Otherwise, these
values are not recorded or output. Building the library with statistics enabled
