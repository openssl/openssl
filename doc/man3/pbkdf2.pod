...
This example derives a 32-byte long test vector using PBKDF2-HMAC-SHA256 using
the password "password" and salt "salt". The iteration count (work factor) is
chosen to be 100000 iterations.
 ...
 if (EVP_PKEY_CTX_set_pbkdf2_md(pctx, EVP_sha256()) <= 0) {
 ...
 if (EVP_PKEY_CTX_set_pbkdf2_iter(pctx, 100000) <= 0) {
