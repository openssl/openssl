...
In OpenSSL versions before 3.0.0, when used in PKCS#1 v1.5 padding,
...
Since version 3.0.0, the EVP_PKEY_decrypt() method when used with PKCS#1
v1.5 padding doesn't return an error in case it detect an error in padding,
