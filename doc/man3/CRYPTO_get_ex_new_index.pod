...
Several Opentls structures can have application-specific data attached to them,
known as "exdata."
...
The API described here is used by Opentls to manipulate exdata for specific
...
When a structure is free'd (such as tls_CTX_free()) then the
...
for B<tls>, B<tls_SESSION>, B<EC_KEY> objects and B<BIO> chains via
...
CRYPTO_alloc_ex_data() was added in Opentls 3.0.
