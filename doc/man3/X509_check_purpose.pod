...
X509_check_eku,
X509_check_purpose - Check the EKU or purpose of a certificate
...
X509_check_eku() checks if certificate I<x> supports
the Extended Key Usage (EKU) represented by I<nid>.
...
X509_check_eku() returns 1 on allowed, 0 if not, and -1 on internal error.

X509_check_purpose() returns for non-CA checks
