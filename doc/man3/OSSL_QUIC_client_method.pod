...
OSSL_QUIC_server_method() functions provide methods for L<SSL_CTX_new_ex(3)>
function to provide QUIC protocol support.

The OSSL_QUIC_client_thread_method() uses threads to allow blocking
mode of operation. The OSSL_QUIC_client_method() does not use threads
and depends on nonblocking mode and application periodically calling
SSL functions.
...
implementation compliant with QUIC specification.

=head1 RETURN VALUES

The functions return pointers to the constant method objects.
