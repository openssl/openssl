...
lh_TYPE_new, lh_TYPE_new_ex, lh_TYPE_free,
...
=for comment generic
 ...
 LHASH *lh_TYPE_new_ex(OPENSSL_LH_HASHFUNC hash, OPENSSL_LH_COMPFUNC compare,
          unsigned long flags);
...
lh_TYPE_new_ex() is similar except that it contains an additional B<flags>
parameter.  Use B<OPENSSL_LH_FLAGS_STATS> to maintain statistics.
...
An alternative is to explictly use lh_TYPE_new_ex() with no
flags, which will not maintain any statistics and therefore avoid data-races.
...
The lh_TYPE_new_ex() function was added in OpenSSL 1.1.1.
