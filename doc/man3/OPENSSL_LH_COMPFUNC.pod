...
table entries can be arbitrary structures, often containing a key and
one or more related values.
In the description here, B<I<TYPE>> is used a placeholder
for any of the OpenSSL datatypes, such as I<SSL_SESSION>.  Applications can
also create hash tables of their own datatypes.

The macro LHASH_OF() declares the hash type, which is a pointer to an
opaque structure. It should be used in every file that references
the hashtable.
The macro DEFINE_LHASH_OF() defines static inline functions to actually
implement the functions described on this page.
This normally would appear in just one file, and the hash functions are
wrapped with application-specific functions.

B<lh_I<TYPE>_new>() creates a new B<LHASH_OF>(B<I<TYPE>>) structure to store
arbitrary data entries, and specifies the C<hash> and C<compare>
callbacks to be used in organising the table's entries.  The C<hash>
...
If your hash table
contains items of a particular type and the I<hash> and
