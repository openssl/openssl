...
For a provider-defined TLS 1.3 ciphersuite, SSL_CIPHER_get_name() returns the
capability name and SSL_CIPHER_standard_name() returns B<NULL> because the
capability does not supply a standard name. OPENSSL_cipher_name() searches
built-in ciphersuites only.
...
B<NID_undef> is returned. A provider-defined cipher implementation may also
return B<NID_undef> if it has no assigned NID.
...
for AEAD cipher suites), or B<c> is NULL, then B<NID_undef> is returned.
...
different to the digest used to calculate the MAC for encrypted records. It
returns NULL when B<c> is NULL.
...
two-byte TLS ciphersuite value in network byte order. Provider-defined suites
use values that do not collide with built-in suites. This parameter is usually
retrieved from a TLS packet by using functions like
L<SSL_client_hello_get0_ciphers(3)>.  SSL_CIPHER_find() returns NULL if an
error occurs or the indicated cipher is not found. It searches built-in and
provider-defined ciphersuites associated with B<ssl>. It returns NULL when
B<ssl> or B<ptr> is NULL.
The returned object is borrowed. For a provider-defined ciphersuite it remains
valid only while B<ssl> remains alive.
