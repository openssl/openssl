...
SSL_CTX_count_ciphers_by_version,
SSL_count_ciphers_by_version
...
=head1 SYNOPSIS
 ...
 char *SSL_CIPHER_flags_description(const uint8_t *flags, int i, char *buf, int size);
...
SSL_CIPHER_flags_description() returns a textual/graphical description of flags
of the B<i>-th flag from array B<flags> into the buffer B<buf> of length B<len>
provided. If B<buf> is provided, it must be at least 8 bytes, otherwise a buffer
will be allocated using OPENSSL_malloc().  If the provided buffer is too small,
or the allocation fails, B<NULL> is returned. Buffer B<buf> can contain unicode
characters. Index B<i> must be greater than or equal to zero and can not exceed
the number of ciphersuites in SSL.

SSL_CTX_count_ciphers_by_version() returns the number of ciphersuites from
B<SSL_CTX> matching B<version> and being specified using ciphersuites
interface (e.g.: SSL_CTX_set_ciphersuites)

SSL_count_ciphers_by_version() returns the number of ciphersuites from B<SSL>
matching B<version> and being specified using ciphersuites interface
(e.g.: SSL_set_ciphersuites)
...
SSL_CIPHER_flags_description() returns the corresponding value in a
null-terminated string for a specific index.
...
SSL_CTX_count_ciphers_by_version() and SSL_count_ciphers_by_version() return
number of ciphers matching specified version.
