...
-2 is a special value which causes BIO_eof() to return EOF for an empty buffer,
while the BIO_read() behavior remains identical to the case when B<v> == -1.
This value is the default for read/write BIOs and is maintained for backward
compatibility.
