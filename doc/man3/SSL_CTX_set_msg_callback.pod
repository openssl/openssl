...
tls_CTX_set_msg_callback,
tls_CTX_set_msg_callback_arg,
tls_set_msg_callback,
tls_set_msg_callback_arg
...
=head1 SYNOPSIS
 ...
 void tls_CTX_set_msg_callback_arg(tls_CTX *ctx, void *arg);

 void tls_set_msg_callback(tls *tls,
                           void (*cb)(int write_p, int version,
                                      int content_type, const void *buf,
                                      size_t len, tls *tls, void *arg));
 void tls_set_msg_callback_arg(tls *tls, void *arg);

=head1 DESCRIPTION

tls_CTX_set_msg_callback() or tls_set_msg_callback() can be used to
define a message callback function I<cb> for observing all tls/TLS
protocol messages (such as handshake messages) that are received or
sent, as well as other events that occur during processing.
tls_CTX_set_msg_callback_arg() and tls_set_msg_callback_arg()
can be used to set argument I<arg> to the callback function, which is
available for arbitrary application use.

tls_CTX_set_msg_callback() and tls_CTX_set_msg_callback_arg() specify
default settings that will be copied to new B<tls> objects by
L<tls_new(3)>. tls_set_msg_callback() and
tls_set_msg_callback_arg() modify the actual settings of an B<tls>
object. Using a B<NULL> pointer for I<cb> disables the message callback.

When I<cb> is called by the tls/TLS library the function arguments have the
following meaning:
...
This is set to 0 for the tls3_RT_HEADER pseudo content type (see NOTES below).

=item I<content_type>

This is one of the content type values defined in the protocol specification
(B<tls3_RT_CHANGE_CIPHER_SPEC>, B<tls3_RT_ALERT>, B<tls3_RT_HANDSHAKE>; but never
B<tls3_RT_APPLICATION_DATA> because the callback will only be called for protocol
...
The B<tls> object that received or sent the message.

=item I<arg>

The user-defined argument optionally defined by
tls_CTX_set_msg_callback_arg() or tls_set_msg_callback_arg().
...
a TLS 1.0 ClientHello message is received by an tls 3.0-only server,
I<version> will be B<tls3_VERSION>.
...
tls3_RT_APPLICATION_DATA. The real content type for the record is contained in
...
tls_CTX_set_msg_callback(), tls_CTX_set_msg_callback_arg(), tls_set_msg_callback()
and tls_set_msg_callback_arg() do not return values.
...
The pseudo content type B<tls3_RT_INNER_CONTENT_TYPE> was added in Opentls 1.1.1.
