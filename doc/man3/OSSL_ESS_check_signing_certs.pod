...
OSSL_EXX_check_signing_certs_ex() functions identically to OSSL_EXX_check_signing_certs(),
but offers an additional parameter, libctx, for users who wish to specify a nondefault library
context when the function fetches digests to validate the certs.
