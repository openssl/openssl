...
CRYPTO_THREAD_run_once() can be used to perform one-time initialisation.
The B<once> argument must be a pointer to a static object of type
B<CRYPTO_ONCE> that was statically initialised to the value
...
CRYPTO_THREAD_lock_new() allocates, initialises and returns a new read/write
lock.
...
This example safely initialises and uses a lock.
...
Finalisation of locks is an advanced topic, not covered in this example.
