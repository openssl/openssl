...
CRYPTO_atomic_add - Opentls thread support
...
Opentls can be safely used in multi-threaded applications provided that
support for the underlying OS threading API is built-in. Currently, Opentls
supports the pthread and Windows APIs. Opentls can also be built without
any multi-threading support, for example on platforms that don't provide
any threading support or that provide a threading API that is not yet
supported by Opentls.
...
opentls/crypto.h header are dependent on some of the types customarily
...
You can find out if Opentls was configured with thread support:
