...
CRYPTO_THREAD_try_read_lock, CRYPTO_THREAD_try_write_lock,
CRYPTO_THREAD_lock_dual,
...
=head1 SYNOPSIS
 ...
 int CRYPTO_THREAD_try_read_lock(CRYPTO_RWLOCK *lock);
 int CRYPTO_THREAD_write_lock(CRYPTO_RWLOCK *lock);
 int CRYPTO_THREAD_try_write_lock(CRYPTO_RWLOCK *lock);
 int CRYPTO_THREAD_lock_dual(CRYPTO_RWLOCK *lock1, int lock1_write,
                             CRYPTO_RWLOCK *lock2, int lock2_write);
...
CRYPTO_THREAD_try_read_lock() and CRYPTO_THREAD_try_write_lock() lock the
provided I<lock> for reading or writing respectively if the lock is not locked,
and otherwise returns 0.

=item *

CRYPTO_THREAD_lock_dual() locks two locks using a deadlock avoidance algorithm.
If B<lock1_write> or B<lock2_write> is nonzero, the respective lock is locked
for writing; otherwise, the respective lock is locked for reading.
