...
Before OpenSSL 3.6, the call to OPENSSL_aligned_alloc() may failed for reasons
...
was built with C<OPENSSL_SMALL_FOOTPRINT> macro defined.  Consequently, caller
may need to fall back to a non-aligned memory allocation (and open-code
the alignment routine if the alignment is a requirement).
