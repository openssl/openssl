...
SSL_CTX_use_cert_and_key() and SSL_use_cert_and_key() assign the B<x> X.509
certificate, B<pkey> private key, and B<chain> certificates onto the corresponding
B<ssl> or B<ctx>. The B<x> and B<pkey> arguments must match each
other. If the  B<override> argument is 0, then B<x>, B<pkey> and B<chain>
are set only if all were not previously set. If B<override> is non-0, then the
certificate, private key and chain certs are always set. If B<pkey> is NULL,
then the public key of B<x> is used as the private key. This
is intended to be used with hardware (via the ENGINE inteface) that stores
the private key securely, such that it cannot be accessed by OpenSSL. The
reference count of the public key is incremented; it is not copied nor
duplicated. This allows all private key validations checks to succeed
without an actual private key being assigned via SSL_CTX_use_PrivateKey(),
etc.
...
For SSL_use_cert_and_key() and SSL_CTX_use_cert_and_key(), the return values are:

=over 4

=item Z<>2

on success of replacing an existing certificate/private key (B<override> is non-0)

=item Z<>1

on success if no existing certificate

=item Z<>0

on failure to replace an existing certificate (B<override> = 0)
