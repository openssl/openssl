...
SSL_CTX_use_cert_and_key() and SSL_use_cert_and_key() install a complete
credential -- leaf certificate I<x>, private key I<pkey>, and issuer chain
I<chain> -- into I<ctx> or I<ssl> in a single call.  Loading the three
pieces together keeps the certificate, its key, and its chain consistent,
and is the preferred way to add a credential when all three are available
in memory.

The leaf certificate I<x> must not be NULL.
If the private key I<pkey> is non-NULL, the functions verify that it matches
the public key in I<x> and fail otherwise.
If I<pkey> is NULL, the public key extracted from I<x> is installed in its
place; this supports providers, or external accelerators that don't expose the
actual private key and perform the signing operation given just the public-key
handle.

The I<chain> argument is an ordered stack of issuer certificates, the issuer of
the leaf certificate first; the I<chain> may be NULL if no intermediates are
needed.
The functions take their own references on I<x>, on a non-NULL I<pkey>, and on
every certificate in a non-NULL I<chain>; the caller retains its own references
and must release them when no longer needed.

An B<SSL_CTX> or B<SSL> object can hold one credential per supported
public-key algorithm.  If I<override> is 0, these functions fail when a
credential of the same algorithm type is already installed (nothing is
replaced and no references are taken); if I<override> is nonzero, any
previously installed certificate, private key, and chain of the same
algorithm type are released and replaced.  To install credentials for
several algorithms, call the function once per algorithm with the
appropriate B<(x, pkey, chain)> triple.

A common use of SSL_use_cert_and_key() is from a certificate callback
registered with L<SSL_set_cert_cb(3)>, where it installs a per-connection
credential -- for example to provide certificates that match a Server Name
Indication (SNI) name recorded by the servername callback.
Calling L<SSL_certs_clear(3)> first discards the credentials inherited from the
parent B<SSL_CTX> so the connection-specific ones take their place cleanly.
See L<SSL_CTX_set_tlsext_servername_callback(3)> for a worked example.
...
types.
For example, SSL_CTX_use_cert_and_key() and SSL_use_cert_and_key() can be
called multiple times on the same B<SSL_CTX> or B<SSL> object, respectively,
with a different certificate type (and matching private key type) in each
call.
...
L<SSL_certs_clear(3)>,
L<SSL_CTX_set_tlsext_servername_callback(3)>
