...
Each SSL_CTX or SSL object can hold multiple certificate and private key pairs simultaneously, provided they are of different key types (for example RSA, ECDSA, or Ed25519). Certificates and private keys are stored internally by key type; loading a new certificate or private key replaces only an existing item of the same type and does not affect other configured types.
...
private key being assigned via SSL_CTX_use_PrivateKey(), etc. This function may be called multiple times to configure multiple certificate and private key pairs of different types on the same SSL_CTX or SSL object. This function may be called multiple times to configure multiple certificate and private key pairs of different types on the same SSL_CTX or SSL object.
...
SSL_CTX_check_priavte_key() checks the consistency of a priavte key with the corresponding certificate loaded into B<ctx>. If more than one key/certificate pair of different types is installed, the consistency of the most recently configured pair of the corresponding key type is checked. Applications that load multiple certificate types should call this function after configuring each certificate and private key pair. SSL_check_private_key() performs
...
If multiple key/certificate pairs of different types are installed, this function checks the consistency of the most recently configured pair of the corresponding key type. Applications that load multiple certificate types should call this function after configuring each key/certificate pair.
...
Each certificate and private key is stored internally by the key type (for example RSA, ECDSA, or Ed25519). Loading a new certificate or private key of a given type replaces any previously configured item of the same type, but does not affect other types.
