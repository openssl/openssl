...
The B<OSSL_ALGORITHM> type is a I<public structure> that describe an
...
Including this identity supports fetching names that are returned by
functions like L<OBJ_obj2txt(3)>.
...
Including this identity supports fetching OIDs that are returned by
functions like L<OBJ_obj2txt(3)>, which is a fallback in case the OID hasn't
been registered in F<libcrypto>'s internal object database.
