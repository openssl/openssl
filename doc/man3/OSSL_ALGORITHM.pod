...
All the names necessary for using an algorithm should be provided by the
OSSL_ALGORITHM structure. Symbolic names, ahort and long, are necessary for
looking up by name. OID is necessary for usage in ASN.1 structures etc. In
case when the names are well-known (e.g. present in OID database of OpenSSL),
the developers aren't obliged to provide anythings besides short name.

If the names provided by various providers are in contradiction (e.g. a shared
sinonym for two or more different algorithms), the behavior is not defined.
