...
desired, please use L<EVP_DigestSignInit(3)> and associated functions, or
L<EVP_SignInit(3)> and associated functions.

=head2 Performing multiple signatures

EVP_PKEY_sign_message_final() resets the context I<ctx> in such a way that
another sequence of (multiple) EVP_PKEY_sign_message_update() and
EVP_PKEY_sign_message_final() can be performed using the parameters that were
given by the initializing call.
Likewise, EVP_PKEY_sign() can be called more than once on the same context to
have several one-shot operations performed using the same parameters.
