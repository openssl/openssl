...
OSSL_PARAM_set_BN() stores the BIGNUM B<val> in the specified parameter
value. It also internally stores the size of the buffer used; this is why
...
B<NULL>.  If also internally stores the serialized size of the number; if
the value of B<val> changes, this function must be called again.
