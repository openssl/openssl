...
ERR_print_errors() prints the contents of the current
error stack to the BIO B<bp>, and empties the error queue.
No output is generated if B<bp> is NULL.

ERR_print_errors_fp() is the same, except that the output goes to the
specified B<FILE>.
No output is generated if B<fp> is NULL.
...
I<function name> and I<reason string> are ASCII text, as is
I<optional text message>
