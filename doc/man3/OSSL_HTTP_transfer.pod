...
OSSL_HTTP_parse_url() parses its input string B<url> as a URL
of the form C<[http[s]://]address[:port][/path]> and splits it up
into host, port and path components and a flag whether it begins with 'https'.
The host component may be a DNS name or an IPv4 address
or an IPv6 address enclosed in C<[> and C<]>.
The port component is optional and defaults to "443" for HTTPS, else "80".
If the B<pport_num> argument is NULL the port specification
...
As far as the result pointer arguments are not NULL the function assigns
via them the respective url components.
For string values these are non-NULL pointers to copies of the strings
where B<*ppath> is guaranteed to begin with a C</> character.
