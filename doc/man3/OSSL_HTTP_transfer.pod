...
A path string returned is guaranteed to begin with a C</> character.
On success the function assigns via each non-NULL result pointer argument
the respective url component. On error, string results are assigned to NULL,
else they are guaranteed to be non-NULL pointers to copies of the strings.
