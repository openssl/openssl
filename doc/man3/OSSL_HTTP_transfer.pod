...
I<bio_update_fn> is a BIO connect/disconnect callback function with prototype
 ...
 BIO *(*OSSL_HTTP_bio_cb_t)(BIO *bio, void *arg, int connect,
                            int detail, OSSL_HTTP_REQ_CTX *rctx)
...
the callback function is invoked with the I<connect> argument being 1.
Since OpenSSL 4.0 this is done at first with the I<detail> argument being 1,
which is for backward compatibility with OpenSSL 3.x.
If the callback function needs to use I<rctx>, it must return NULL;
in reaction it is called again with the I<detail> argument being 0,
which indicates that the I<rctx> argument may be accessed.
On disconnect I<connect> is 0 and I<detail> is 1 if no error occurred, else 0.
The I<rctx> argument may be accessed on disconnect only since OpenSSL 4.0.
...
Here is a simple example that supports TLS connections (but not via a proxy):
 ...
 BIO *http_tls_cb(BIO *bio, void *arg, int connect,
                  int detail, OSSL_HTTP_REQ_CTX *rctx)
 {
...
It does nothing if the new I<rctx> parameter indicates that no proxy is used.
...
In OpenSSL 4.0 the design of the connection callback function of type
B<OSSL_HTTP_bio_cb_t> was fixed and OSSL_HTTP_REQ_CTX_proxy_connect() added.
