...
SSL_SESSION_CACHE_set_timeout,
SSL_SESSION_CACHE_get_timeout - manipulate timeout values for session caching
...
B<ctx> to B<t>. SSL_SESSION_CACHE_set_timeout() sets the timeout for
newly created sessions for B<cache> to B<t>.
The timeout value B<t> must be given in seconds.

SSL_CTX_get_timeout() returns the currently set timeout value for B<ctx>.

SSL_SESSION_CACHE_get_timeout() returns the currently set timeout value for
B<cache>.
...
SSL_CTX_set_timeout() and SSL_SESSION_CACHE_set_timeout() return the
previously set timeout value.

SSL_CTX_get_timeout() and SSL_SESSION_CACHE_get_timeout() return the
currently set timeout value.

=head1 HISTORY

The SSL_SESSION_CACHE_set_timeout() and SSL__SESSION_CACHE_get_timeout()
