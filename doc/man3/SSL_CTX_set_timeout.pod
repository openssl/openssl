...
SSL_CTX_set_timeout_ex,
SSL_CTX_get_timeout_ex - manipulate timeout values for session caching
...
=head1 SYNOPSIS
 ...
 time_t SSL_CTX_get_timeout_ex(SSL_CTX *ctx);

=head1 DESCRIPTION

SSL_CTX_set_timeout() and SSL_CTX_set_timeout_ex() set the timeout for newly
created sessions for B<ctx> to B<t>. The timeout value B<t> must be given in
seconds.

SSL_CTX_get_timeout() and SSL_CTX_get_timeout_ex() return the currently set
timeout value for B<ctx>.
...
basis, see L<SSL_get_default_timeout_ex(3)>.
...
SSL_CTX_get_timeout_ex() uses a B<time_t> to represent time.
Otherwise, their behavior is identical.

SSL_CTX_set_timeout() uses a B<long> to represent time.
SSL_CTX_set_timeout_ex() uses a B<time_t> to represent time.
Otherwise, their behavior is identical.

=head1 RETURN VALUES

SSL_CTX_set_timeout() and SSL_CTX_set_timeout_ex() return the previously set
timeout value.
...
L<ssl(7)>,
L<SSL_CTX_set_session_cache_mode(3)>,
L<SSL_SESSION_get_time_ex(3)>,
L<SSL_CTX_flush_sessions(3)>,
L<SSL_get_default_timeout_ex(3)>

=head1 HISTORY

SSL_CTX_set_timeout_ex() SSL_CTX_get_timeout_ex() were added in OpenSSL 3.1.
