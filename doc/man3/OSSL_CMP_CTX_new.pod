...
OSSL_CMP_CTX_new() allocates and initialises an OSSL_CMP_CTX structure to
...
=item B<OSSL_CMP_OPT_TOTALTIMEOUT>
        ...
        Maximum total number of seconds an enrolment (including polling)
...
OSSL_CMP_CTX_get0_newPkey() gives the key to use for certificate enrolment
...
see B<OSSL_CMP_CTX_set1_oldCert()>. This default is used for Initialisation
...
The following code does an Initialisation Request:
...
The following code does an Initialisation Request using an
