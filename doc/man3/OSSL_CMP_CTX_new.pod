...
default values. If the trace API is enabled it calls OSSL_CMP_CTX_set_log_cb()
with the NULL callback pointer.
Transfer is set to HTTP, proof-of-possession method to POPOSigningKey, etc.
...
=item B<OSSL_CMP_OPT_LOG_VERBOSITY>
        ...
        The level of severity needed for actually outputting diagnostic
        information such as errors, warnings, general info, and debugging info.
...
OSSL_CMP_CTX_get_option() reads the current of the given option
(e.g., OSSL_CMP_OPT_IMPLICITCONFIRM) from the given OSSL_CMP_CTX structure.

OSSL_CMP_CTX_set_log_cb() sets in B<ctx> the log callback function C<cb>
for error, warning, info, debug, etc. messages.
It fails if the OpenSSL trace facility is disabled. When C<cb> is NULL the
output is sent to stdout (unless OPENSSL_NO_STDIO is not defined).
For details see L<OSSL_CMP_log_open(3)>.
...
Port defaults to OSSL_CMP_DEFAULT_PORT = 8080 if not set explicitly.
