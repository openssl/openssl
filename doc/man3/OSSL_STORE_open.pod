...
OSSL_STORE_ctrl, OSSL_STORE_load_with_libctx, OSSL_STORE_load, OSSL_STORE_eof,
...
information for OSSL_STORE_open(), OSSL_STORE_load(),
OSSL_STORE_load_with_libctx(), OSSL_STORE_eof() and OSSL_STORE_close() to work
together.
...
OSSL_STORE_load_with_libctx() takes a B<OSSL_STORE_CTX> with a library context
I<libctx> and property query <propq>, and tries to load the next
available object and return it wrapped with  B<OSSL_STORE_INFO>.

OSSL_STORE_load() is similar to OSSL_STORE_load_with_libctx() but uses NULL for
...
OSSL_STORE_load_with_libctx() and OSSL_STORE_load() return a pointer to a
B<OSSL_STORE_INFO> on success, or NULL on error or when end of data is reached.
...
OSSL_STORE_load_with_libctx() was added in OpenSSL 3.0.
