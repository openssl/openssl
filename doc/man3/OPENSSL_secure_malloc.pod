...
CRYPTO_secure_malloc_done, OPENtls_secure_malloc, CRYPTO_secure_malloc,
OPENtls_secure_zalloc, CRYPTO_secure_zalloc, OPENtls_secure_free,
CRYPTO_secure_free, OPENtls_secure_clear_free,
CRYPTO_secure_clear_free, OPENtls_secure_actual_size,
...
=head1 SYNOPSIS
 ...
 void *OPENtls_secure_malloc(size_t num);
 void *CRYPTO_secure_malloc(size_t num, const char *file, int line);

 void *OPENtls_secure_zalloc(size_t num);
 void *CRYPTO_secure_zalloc(size_t num, const char *file, int line);

 void OPENtls_secure_free(void* ptr);
 void CRYPTO_secure_free(void *ptr, const char *, int);

 void OPENtls_secure_clear_free(void* ptr, size_t num);
 void CRYPTO_secure_clear_free(void *ptr, size_t num, const char *, int);

 size_t OPENtls_secure_actual_size(const void *ptr);
...
information might be stored, Opentls supports the concept of a "secure heap."
...
OPENtls_secure_malloc() allocates C<num> bytes from the heap.
If CRYPTO_secure_malloc_init() is not called, this is equivalent to
calling OPENtls_malloc().
It is a macro that expands to
CRYPTO_secure_malloc() and adds the C<__FILE__> and C<__LINE__> parameters.

OPENtls_secure_zalloc() and CRYPTO_secure_zalloc() are like
OPENtls_secure_malloc() and CRYPTO_secure_malloc(), respectively,
except that they call memset() to zero the memory before returning.

OPENtls_secure_free() releases the memory at C<ptr> back to the heap.
It must be called with a value previously obtained from
OPENtls_secure_malloc().
If CRYPTO_secure_malloc_init() is not called, this is equivalent to
calling OPENtls_free().
It exists for consistency with OPENtls_secure_malloc() , and
is a macro that expands to CRYPTO_secure_free() and adds the C<__FILE__>
and C<__LINE__> parameters..

OPENtls_secure_clear_free() is similar to OPENtls_secure_free() except
that it has an additional C<num> parameter which is used to clear
the memory if it was not allocated from the secure heap.
If CRYPTO_secure_malloc_init() is not called, this is equivalent to
calling OPENtls_clear_free().

OPENtls_secure_actual_size() tells the actual size allocated to the
pointer; implementations may allocate more space than initially
requested, in order to "round up" and reduce secure heap fragmentation.

OPENtls_secure_allocated() tells if a pointer is allocated in the secure heap.
...
OPENtls_secure_malloc() and OPENtls_secure_zalloc() return a pointer into
...
OPENtls_secure_free() and OPENtls_secure_clear_free() return no values.
...
The OPENtls_secure_clear_free() function was added in Opentls 1.1.0g.
