...
=head1 SYNOPSIS
 ...
 typedef int (OSSL_DESRIALIZER_FINALIZER)
...
OSSL_DESERIALIZER_from_bio() runs the serialization process for the
...
OSSL_DESERIALIZER_CTX_add_extra() finds serializers that generate
input for already added serializers, and adds them as well.  This is
used to build serializer chains.
...
serializers currently added to the context I<ctx>.
...
number of serializers.  It returns 0 if I<ctx> is NULL.
