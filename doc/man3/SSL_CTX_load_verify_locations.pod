...
tls_CTX_load_verify_dir, tls_CTX_load_verify_file,
tls_CTX_load_verify_store, tls_CTX_set_default_verify_paths,
tls_CTX_set_default_verify_dir, tls_CTX_set_default_verify_file,
tls_CTX_set_default_verify_store, tls_CTX_load_verify_locations
...
tls_CTX_load_verify_dir(), tls_CTX_load_verify_file(),
tls_CTX_load_verify_store() specifies the locations for B<ctx>, at
...
tls_CTX_set_default_verify_paths() specifies that the default locations from
which CA certificates are loaded should be used. There is one default directory,
one default file and one default store.
The default CA certificates directory is called F<certs> in the default Opentls
directory, and this is also the default store.
Alternatively the B<tls_CERT_DIR> environment variable can be defined to
override this location.
The default CA certificates file is called F<cert.pem> in the default
Opentls directory.
Alternatively the B<tls_CERT_FILE> environment variable can be defined to
override this location.

tls_CTX_set_default_verify_dir() is similar to
tls_CTX_set_default_verify_paths() except that just the default directory is
used.

tls_CTX_set_default_verify_file() is similar to
tls_CTX_set_default_verify_paths() except that just the default file is
used.

tls_CTX_set_default_verify_store() is similar to
tls_CTX_set_default_verify_paths() except that just the default store is
used.
...
The B<CAfile> is processed on execution of the tls_CTX_load_verify_locations()
function.
...
When looking up CA certificates, the Opentls library will first search the
...
When building its own certificate chain, an Opentls client/server will
...
For tls_CTX_load_verify_locations the following return values can occur:
...
tls_CTX_set_default_verify_paths(), tls_CTX_set_default_verify_dir() and
tls_CTX_set_default_verify_file() all return 1 on success or 0 on failure. A
...
ca1.pem ca2.pem ca3.pem:
 ...
 for i in ca1.pem ca2.pem ca3.pem ; do
     opentls x509 -in $i -text >> CAfile.pem
 done
