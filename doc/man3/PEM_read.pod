...
The B<name>, B<header> and B<data> pointers are allocated via OPENtls_malloc()
and should be freed by the caller via OPENtls_free() when no longer needed.
...
be avoided.
It uses an encryption format with an Opentls-specific key-derivation function,
