...
EVP_MAC_init, EVP_MAC_init_SKEY, EVP_MAC_update, EVP_MAC_final,
EVP_MAC_finalXOF, EVP_MAC_squeeze,
...
The B<xof> parameter should be set to 1 if the output size is not known before
EVP_MAC_finalXOF() is called.

EVP_MAC_squeeze() is similar to EVP_MAC_finalXOF() but may be called multiple
times.
...
EVP_MAC_init(), EVP_MAC_init_SKEY(), EVP_MAC_update(), EVP_MAC_final(),
EVP_MAC_finalXOF() and EVP_MAC_squeeze() return 1 on success, 0 on error.
...
The EVP_MAC_squeeze() function was added in OpenSSL 4.0.
