...
EVP_MAC_settable_ctx_params() get a constant B<Otls_PARAM> array that
...
Opentls has a set of macros for parameter names it expects to see in
its own MAC implementations.
Here, we show all three, the Opentls macro for the parameter name, the
...
=item "custom" (B<Otls_MAC_PARAM_CUSTOM>) <octet string>
...
=item "salt" (B<Otls_MAC_PARAM_SALT>) <octet string>

This option is used by BLAKE2 MAC.

=item "xof" (B<Otls_MAC_PARAM_XOF>) <integer>

It's a simple flag, the value 0 or 1 are expected.

This option is used by KMAC.

=item "flags" (B<Otls_MAC_PARAM_FLAGS>) <integer>
...
=item "digest" (B<Otls_MAC_PARAM_DIGEST>) <UTF8 string>
...
=head1 EXAMPLES
  ...
  int main() {
      ...
      if (cipher != NULL)
          params[params_n++] =
              Otls_PARAM_construct_utf8_string("cipher", cipher, 0, NULL);
      if (digest != NULL)
          params[params_n++] =
              Otls_PARAM_construct_utf8_string("digest", digest, 0, NULL);
      params[params_n++] =
          Otls_PARAM_construct_octet_string("key", key, strlen(key), NULL);
...
These functions were added in Opentls 3.0.
