...
EVP_MAC_init, EVP_MAC_init_SKEY, EVP_MAC_update,
EVP_MAC_final, EVP_MAC_finalXOF, EVP_MAC_cleanse,
...
EVP_MAC_cleanse() can be called to remove any sensitive data from an existing
MAC context I<ctx>. It can be used if an existing context I<ctx> is reused and
there is a some period of time before EVP_MAC_CTX_init() is called with a
new key material. This function is not needed for normal use cases, since any
sensitive data will be cleansed when EVP_MAC_CTX_free() is called.
...
The EVP_MAC_cleanse() function was added in OpenSSL 4.1.
