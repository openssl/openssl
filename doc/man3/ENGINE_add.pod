...
initialized and able to use any of its cryptographic
...
ENGINE, you have a guarantee that the ENGINE has been initialized and
is ready to perform cryptographic operations, and will remain initialized
...
already operational and couldn't be successfully initialized (e.g. lack of
...
functional reference to an initialized ENGINE whose implementation should be
...
table trying to initialize each of them in turn, in case one of them is
...
same iteration unless the state table changes. This behavior can also be
changed; if the ENGINE_TABLE_FLAG_NOINIT flag is set (using
ENGINE_set_table_flags()), no attempted initializations will take place,
...
"registered" or called into use by OpenSSL automatically - that behavior
is something for the application to control. Some applications
will want to allow the user to specify exactly which ENGINE they want used
if any is to be used at all. Others may prefer to load all support and have
OpenSSL automatically use at run-time any ENGINE that is able to
successfully initialize - i.e. to assume that this corresponds to
...
illustrates how to approach this;
 ...
 if (!e)
     ...
     return;
 if (!ENGINE_init(e)) {
     /* the engine couldn't initialize, release 'e' */
     ENGINE_free(e);
     return;
 }
 if (!ENGINE_set_default_RSA(e))
     /*
      * This should only happen when 'e' can't initialize, but the previous
...
OpenSSL - if there is an ENGINE that implements it and can be initialized,
...
so that it can be initialized for use. This could include the path to any
driver or config files it needs to load, required network addresses,
smart-card identifiers, passwords to initialize protected devices,
logging information, etc etc. This class of commands typically needs to be
passed to an ENGINE B<before> attempting to initialize it, i.e. before
calling ENGINE_init(). The other class of commands consist of settings or
operations that tweak certain behavior or cause certain operations to take
...
global table flags which are used to control the registration behavior of
...
used to control various behaviors of an ENGINE.
