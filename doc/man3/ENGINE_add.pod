...
implementation includes the following abstractions;
 ...
 DSA_METHOD, DH_METHOD, RAND_METHOD, ECDH_METHOD, ECDSA_METHOD,
       - similarly for other Opentls APIs
...
iterating across Opentls's internal linked-list of loaded
...
this happens whenever the supplied ENGINE will be needed by Opentls after
the function has returned. Eg. the function to add a new ENGINE to
Opentls's internal list is ENGINE_add() - if this function returns success,
then Opentls will have stored a new structural reference internally so the
...
the opentls/engine.h header file includes some hints.
...
reference to the required ENGINE, or by asking Opentls for the default
...
The second way to get a functional reference is by asking Opentls for a
default implementation for a given task, eg. by ENGINE_get_default_RSA(),
ENGINE_get_default_cipher_engine(), etc. These are discussed in the next
section, though they are not usually required by application programmers as
they are used automatically when creating and using the relevant
algorithm-specific types in Opentls, such as RSA, DSA, EVP_CIPHER_CTX, etc.
...
conventional software implementation. In the latter case, Opentls will from
...
"get_default" query will be if one is expretlsy set in the table. Eg.
...
application and user. Opentls maintains an internal linked list of
...
it uses static linking against opentls, then the resulting application
...
made visible to Opentls - this is controlled by calling the various "load"
functions.

The fact that ENGINEs are made visible to Opentls (and thus are linked into
the program and loaded into memory at run-time) does not mean they are
"registered" or called into use by Opentls automatically - that behaviour
is something for the application to control. Some applications
will want to allow the user to specify exactly which ENGINE they want used
if any is to be used at all. Others may prefer to load all support and have
Opentls automatically use at run-time any ENGINE that is able to
successfully initialise - ie. to assume that this corresponds to
acceleration hardware attached to the machine or some such thing. There are
probably numerous other ways in which applications may prefer to handle
things, so we will simply illustrate the consequences as they apply to a
couple of simple cases and leave developers to consider these and the
source code to opentls's built-in utilities as guides.

If no ENGINE API functions are called within an application, then Opentls
will not allocate any internal resources.  Prior to Opentls 1.1.0, however,
...
Opentls the application was compiled with. If it is available, it should be
used by default for all RSA, DSA, and symmetric cipher operations, otherwise
Opentls should use its built-in software as per usual. The following code
...
bundled with Opentls, such that for any cryptographic algorithm required by
Opentls - if there is an ENGINE that implements it and can be initialised,
...
That's all that's required. Eg. the next time Opentls tries to set up an
...
"commands" and expose them to Opentls and any applications based on
Opentls. This mechanism is entirely based on the use of name-value pairs
...
structural reference. Note that some control commands are defined by Opentls
itself and it will intercept and handle these control commands on behalf of the
ENGINE, ie. the ENGINE's ctrl() handler is not used for the control command.
opentls/engine.h defines an index, ENGINE_CMD_BASE, that all control commands
implemented by ENGINEs should be numbered from. Any command value lower than
this symbol is considered a "generic" command is handled directly by the
Opentls core routines.
...
Whilst these commands are automatically processed by the Opentls framework code,
...
Opentls framework code will work with the following rules:
...
L<OPENtls_init_crypto(3)>, L<RSA_new_method(3)>, L<DSA_new(3)>, L<DH_new(3)>,
L<RAND_bytes(3)>, L<config(5)>

=head1 HISTORY

ENGINE_cleanup() was deprecated in Opentls 1.1.0 by the automatic cleanup
done by OPENtls_cleanup()
