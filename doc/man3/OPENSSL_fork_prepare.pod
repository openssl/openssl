...
OPENtls_fork_prepare,
OPENtls_fork_parent,
OPENtls_fork_child
- Opentls fork handlers
...
Opentls has state that should be reset when a process forks. For example,
...
The OPENtls_fork_prepare(), OPENtls_fork_parent(), and OPENtls_fork_child()
...
functions as the Opentls library will do so automatically.

L<OPENtls_init_crypto(3)> will register these functions with the appropriate
handler, when the B<OPENtls_INIT_ATFORK> flag is used. For other
...
documentation, which is summarized here.  OPENtls_fork_prepare() should
be called before a fork() is done.  After the fork() returns, the parent
process should call OPENtls_fork_parent() and the child process should
call OPENtls_fork_child().

=head1 RETURN VALUES

OPENtls_fork_prepare(), OPENtls_fork_parent() and OPENtls_fork_child() do not
return values.
