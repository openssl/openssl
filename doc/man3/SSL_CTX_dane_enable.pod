...
SSL_get0_dane, SSL_CTX_dane_set_flags, SSL_CTX_dane_clear_flags,
...
the SSL connection.
This structure contains the DANE verification state and can be passed to
X509_STORE_CTX_set0_dane() for use in custom certificate verification callbacks.
The B<SSL_DANE> type is an opaque structure that holds DANE-related data
including the TLSA records, matching information, and verification flags.

X509_STORE_CTX_set0_dane() associates a B<SSL_DANE> structure with an
X509_STORE_CTX.
This allows the X509 certificate verification to use DANE/TLSA authentication
during the chain verification process.
This function is typically used when implementing a custom certificate
verification callback via L<SSL_CTX_set_cert_verify_callback(3)>.
The B<dane> pointer is stored directly without copying; the caller must
ensure the B<SSL_DANE> structure remains valid for the lifetime of the
X509_STORE_CTX.
...
SSL_get0_dane() returns a pointer to the B<SSL_DANE> structure, or NULL if
the connection is not associated with a valid SSL_CONNECTION (e.g., for QUIC
connections).
