...
tls_CTX_dane_enable, tls_CTX_dane_mtype_set, tls_dane_enable,
tls_dane_tlsa_add, tls_get0_dane_authority, tls_get0_dane_tlsa,
tls_CTX_dane_set_flags, tls_CTX_dane_clear_flags,
tls_dane_set_flags, tls_dane_clear_flags
...
=head1 SYNOPSIS
 ...
 int tls_CTX_dane_mtype_set(tls_CTX *ctx, const EVP_MD *md,
...
tls_CTX_dane_enable() must be called first to initialize the shared state
...
L<tls_CTX_set_cert_verify_callback(3)> are responsible to authenticate the peer
chain in whatever manner they see fit.

tls_CTX_dane_mtype_set() may then be called zero or more times to adjust the
supported digest algorithms.
This must be done before any tls handles are created for the context.
...
tls_dane_enable() must be called before the tls handshake is initiated with
L<tls_connect(3)> if (and only if) you want to enable DANE for that connection.
(The connection must be associated with a DANE-enabled tls context).
...
name checks.
Additional server names can be specified via L<tls_add1_host(3)>.
The B<basedomain> is used as the default SNI hint if none has yet been
specified via L<tls_set_tlsext_host_name(3)>.

tls_dane_tlsa_add() may then be called one or more times, to load each of the
TLSA records that apply to the remote TLS peer.
(This too must be done prior to the beginning of the tls handshake).
...
The caller is expected to check the return value of each tls_dane_tlsa_add()
...
tls_get0_dane_authority() can be used to get more detailed information about
...
The complete verified chain can be retrieved via L<tls_get0_verified_chain(3)>.
The return value is an index into this verified chain, rather than the list of
certificates sent by the peer as returned by L<tls_get_peer_cert_chain(3)>.
...
tls_CTX_dane_set_flags() and tls_dane_set_flags() can be used to enable
optional DANE verification features.
tls_CTX_dane_clear_flags() and tls_dane_clear_flags() can be used to disable
the same features.
The B<flags> argument is a bit-mask of the features to enable or disable.
The B<flags> set for an B<tls_CTX> context are copied to each B<tls> handle
...
The functions tls_CTX_dane_enable(), tls_CTX_dane_mtype_set(),
tls_dane_enable() and tls_dane_tlsa_add() return a positive value on success.
Negative return values indicate resource problems (out of memory, etc.) in the
tls library, while a return value of B<0> indicates incorrect usage or invalid
...
The functions tls_get0_dane_authority() and tls_get0_dane_tlsa() return a
...
The functions tls_CTX_dane_set_flags(), tls_CTX_dane_clear_flags(),
tls_dane_set_flags() and tls_dane_clear_flags() return the B<flags> in effect
...
the lifetime of the tls connection.
 ...
 if ((ctx = tls_CTX_new(TLS_client_method())) == NULL)
     /* error */
 if (tls_CTX_dane_enable(ctx) <= 0)
     /* error */
 if ((tls = tls_new(ctx)) == NULL)
     /* error */
 if (tls_dane_enable(tls, dane_tlsa_domain) <= 0)
     /* error */
 ...
 if (!tls_add1_host(tls, nexthop_domain))
     /* error */
 ...
 for (... each TLSA record ...) {
     ...
     if (ret < 0)
         /* handle tls library internal error */
     else if (ret == 0)
 ...
 /*
  * At this point, the verification mode is still the default tls_VERIFY_NONE.
 ...
 if (num_usable == 0) {
     ...
     tls_set_verify(tls, tls_VERIFY_NONE, verify_cb);
 } else {
      ...
      * Alternatively, use the permissive tls_VERIFY_NONE verification mode,
      ...
      * failed.
      */
     tls_set_verify(tls, tls_VERIFY_PEER, verify_cb);
 }
 ...
 /* Perform tls_connect() handshake and handle errors here */

 if (tls_session_reused(tls)) {
 ...
 } else if (tls_get_verify_result(tls) == X509_V_OK) {
     const char *peername = tls_get0_peername(tls);
     EVP_PKEY *mspki = NULL;

     int depth = tls_get0_dane_authority(tls, NULL, &mspki);
     if (depth >= 0) {
         (void) tls_get0_dane_tlsa(tls, &usage, &selector, &mtype, NULL, NULL);
...
call to tls_dane_tlsa_add(), and if all return 0 (due to invalid
or unsupported parameters) disable peer authentication by calling
L<tls_set_verify(3)> with B<mode> equal to B<tls_VERIFY_NONE>.
...
L<tls_new(3)>,
L<tls_add1_host(3)>,
L<tls_set_hostflags(3)>,
L<tls_set_tlsext_host_name(3)>,
L<tls_set_verify(3)>,
L<tls_CTX_set_cert_verify_callback(3)>,
L<tls_get0_verified_chain(3)>,
L<tls_get_peer_cert_chain(3)>,
L<tls_get_verify_result(3)>,
L<tls_connect(3)>,
L<tls_get0_peername(3)>,
