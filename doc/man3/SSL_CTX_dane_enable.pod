...
B<SSL_DANE> is an opaque structure that holds DANE verification state
for an SSL connection. It is used internally by OpenSSL during DANE
authentication and can be passed to L<X509_STORE_CTX_set0_dane(3)> for
custom certificate verification callbacks.
...
the B<SSL> connection I<ssl>. This can be used to pass DANE verification state
to custom certificate verification callbacks via
L<X509_STORE_CTX_set0_dane(3)>. The returned pointer is an internal reference
and must not be freed by the caller. It remains valid for the lifetime of the
SSL connection. Returns NULL if I<ssl> is not a valid SSL connection (e.g., for
QUIC stream objects).
...
SSL_get0_dane() returns a pointer to the B<SSL_DANE> structure associated with
the SSL connection, or NULL if the connection is not a regular SSL connection
(e.g., for QUIC stream objects).
