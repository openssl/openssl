...
SSL_dane_tlsa_add, SSL_get0_dane, SSL_get0_dane_authority, SSL_get0_dane_tlsa,
...
SSL_get0_dane() returns a pointer to the B<SSL_DANE> structure associated with
the SSL connection I<ssl>. This structure contains the DANE verification state
and can be used with X509_STORE_CTX_set0_dane() when implementing custom
certificate verification callbacks.

X509_STORE_CTX_set0_dane() associates the B<SSL_DANE> structure I<dane> with
the certificate verification context I<ctx>. This function is declared in
B<openssl/x509_vfy.h>. It is used internally by the SSL library to enable
DANE verification during certificate chain validation. Applications that
override the default certificate verification via
L<SSL_CTX_set_cert_verify_callback(3)> may need to call this function to
enable DANE verification in their custom callback.
...
SSL_get0_dane() returns a pointer to the B<SSL_DANE> structure associated with
the SSL connection, or NULL if I<ssl> does not represent a connection (e.g., a
QUIC stream SSL object). The returned structure is always present for valid
connections regardless of whether DANE has been enabled; applications should
use other means (such as checking for TLSA records) to determine whether DANE
is active.
...
X509_STORE_CTX_set0_dane() does not return a value.
