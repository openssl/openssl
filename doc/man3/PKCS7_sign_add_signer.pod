...
is not complete and must be finalised either by streaming (if applicable) or
...
returned PKCS7 structure will be valid and finalised when this flag is set.

If B<PKCS7_PARTIAL> is set in addition to B<PKCS7_REUSE_DIGEST> then the
B<PKCS7_SIGNER_INO> structure will not be finalised so additional attributes
