...
The shutdown procedure consists of two steps: sending of the "close notify"
...
from the server.
In case you want to be able to resume the session, it's recommend to do a complete
...
If succesful, SSL_shutdown() will return 0.
...
event.
When it's done sending data, it will send the "close notify" alert.
...
flag.
If succesful, SSL_shutdown() will return 1.
...
After SSL_shutdown() returned 0, it's possible to call SSL_shutdown() again
to wait for the peer's "close notify" alert.
SSL_shudown() will return 1 in that case.
However, it's recommended to wait for it using SSL_read() instead.
