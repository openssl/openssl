...
The shutdown procedure consists of 2 steps: sending of the "close notify"
shutdown alert, and reception of the peer's "close notify" shutdown alert.
The order of those 2 steps depends on the application.
...
This should only be done in case you're sure that the other side will not
send more data, otherwise you risk a truncation attack.

When a client only writes, then calls SSL_shutdown() and closes the underlying
connection, it might not be able to resume the session because it did not
process the session ticket of the server.
In such cases it's recommend to do a complete shutdown procedure (bidirectional
"close notify" alerts).
...
SSL_shutdown() can return -1, in which case the SSL_shutdown() should be called
again.

If a unidirectional shutdown is enough (the underlying connection shall be
closed anyway), this first succesful call to SSL_shutdown() is sufficient.
...
event.
After it's done sending data, it will send the "close notify" alert.
...
flag.
SSL_shutdown() will then return with 1.
