...
An error of SSL_ERROR_SYSCALL can also happen in a bidirectional shutdown when
the peer sends more data instead of the expected "close notify" shutdown alert.
To avoid that issue one could call L<SSL_read(3)> after the first successful
SSL_shutdown() until L<SSL_read(3)> returns 0 and <SSL_get_error(3)> returns
