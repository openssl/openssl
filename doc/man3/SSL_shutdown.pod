...
SSL_shutdown() only closes the communication in 1 direction.
It is not possible to call SSL_write() after calling SSL_shutdown().
The peer is still allowed to send data after it has received the "close notify"
alert messages.
The peer will indicate that it is done sending data by sending the "close notify"
alert.
After receiving the "close notify" alert from the peer SSL_read() will return
<= 0 and SSL_get_error() will return SSL_ERROR_ZERO_RETURN.
...
It can occur if action is need to continue the operation for non-blocking BIOs.
The output of L<SSL_get_error(3)> may be misleading, as an
erroneous SSL_ERROR_SYSCALL may be flagged when not all data was read by
SSL_read() yet.
