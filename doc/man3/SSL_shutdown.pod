...
This should only be done in case you are sure that the other side will not
...
SSL_shutdown() will then return 0.

SSL_shutdown() can return -1, in which case SSL_shutdown() should be called
again.
...
flag.
SSL_shutdown() will then return 1.
