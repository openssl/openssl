...
continuing sending data for some period of time before honouring the local
application's shutdown request.
...
return B<SSL_ERROR_ZERO_RETURN>), after any application data sent by the peer
...
underlying BIO is blocking, a call to SSL_shutdown() will return either once
close_notify alert messages have both been sent to and received from the peer,
and the shutdown process is thereby completed (returning 1), or if an error
occurs.
...
received). However, the preferred method of waiting for the shutdown to complete
is to use L<SSL_read(3)> until L<SSL_get_error(3)> indicates EOF by returning
B<SSL_ERROR_ZERO_RETURN>. This ensures any data received immediately before the
peer's close_notify alert is still provided to the application.

SSL_shutdown() must not be called if a previous fatal error has occurred on a
connection; i.e., if L<SSL_get_error(3)> has returned B<SSL_ERROR_SYCALL> or
B<SSL_ERROR_SSL>.
...
SSL_shutdown_ex() may be used by an application to provide additional
information to the peer on the reason why a connection is being shut down. The
information which can be provided is as follows:
...
acknowledged by the peer. Only once this process is completed is thue shutdown
considered complete.
