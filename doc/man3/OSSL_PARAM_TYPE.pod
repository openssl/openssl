...
OSSL_PARAM_locate_get_TYPE, OSSL_PARAM_locate_set_TYPE,
OSSL_PARAM_locate_get_BN, OSSL_PARAM_locate_set_BN,
OSSL_PARAM_locate_get_utf8_string, OSSL_PARAM_locate_set_utf8_string,
OSSL_PARAM_locate_get_octet_string, OSSL_PARAM_locate_set_octet_string,
OSSL_PARAM_locate_get_utf8_ptr, OSSL_PARAM_locate_set_utf8_ptr,
OSSL_PARAM_locate_get_octet_ptr, OSSL_PARAM_locate_set_octet_ptr,
OSSL_PARAM_optional_set_TYPE, OSSL_PARAM_optional_set_BN,
OSSL_PARAM_optional_set_utf8_string, OSSL_PARAM_optional_set_octet_string,
OSSL_PARAM_optional_set_utf8_ptr, OSSL_PARAM_optional_set_octet_ptr
- OSSL_PARAM helpers
...
=head1 SYNOPSIS
 ...
 int OSSL_PARAM_locate_get_TYPE(const OSSL_PARAM *p, const char *key,
                                TYPE *val);
 int OSSL_PARAM_locate_set_TYPE(const OSSL_PARAM *p, const char *key, TYPE val);
 ...
 int OSSL_PARAM_locate_get_octet_ptr(const OSSL_PARAM *p, const char *key,
                                     void **val, size_t *used_len);
 int OSSL_PARAM_locate_set_octet_ptr(const OSSL_PARAM *p, const char *key,
                                     void *val, size_t used_len);

 int OSSL_PARAM_optional_set_TYPE(const OSSL_PARAM *p, const char *key,
                                  TYPE val);
...
OSSL_PARAM_locate_get_TYPE() is a shortcut for:
OSSL_PARAM_get_TYPE(OSSL_PARAM_locate(B<p>, B<key>), B<val>).

OSSL_PARAM_locate_set_TYPE() is a shortcut for:
OSSL_PARAM_set_TYPE(OSSL_PARAM_locate(B<p>, B<key>), B<val>).

OSSL_PARAM_locate_get_BN() is a shortcut for:
OSSL_PARAM_get_BN(OSSL_PARAM_locate(B<p>, B<key>), B<val>).

OSSL_PARAM_locate_set_BN() is a shortcut for:
OSSL_PARAM_set_BN(OSSL_PARAM_locate(B<p>, B<key>), B<val>).

OSSL_PARAM_locate_get_utf8_string() is a shortcut for:
OSSL_PARAM_get_utf8_string(OSSL_PARAM_locate(B<p>, B<key>), B<val>,
B<max_len>).

OSSL_PARAM_locate_set_utf8_string() is a shortcut for:
OSSL_PARAM_set_utf8_string(OSSL_PARAM_locate(B<p>, B<key>), B<val>).

OSSL_PARAM_locate_get_octet_string() is a shortcut for:
OSSL_PARAM_get_octet_string(OSSL_PARAM_locate(B<p>, B<key>), B<val>,
B<max_len>, B<used_len>).

OSSL_PARAM_locate_set_octet_string() is a shortcut for:
OSSL_PARAM_set_octet_string(OSSL_PARAM_locate(B<p>, B<key>), B<val>,
B<len>).

OSSL_PARAM_locate_get_utf8_ptr() is a shortcut for:
OSSL_PARAM_get_utf8_ptr(OSSL_PARAM_locate(B<p>, B<key>), B<val>).

OSSL_PARAM_locate_set_utf8_ptr() is a shortcut for:
OSSL_PARAM_set_utf8_ptr(OSSL_PARAM_locate(B<p>, B<key>), B<val>).

OSSL_PARAM_locate_get_octet_ptr() is a shortcut for:
OSSL_PARAM_get_octet_ptr(OSSL_PARAM_locate(B<p>, B<key>), B<val>,
B<used_len>).

OSSL_PARAM_locate_set_octet_ptr() is a shortcut for:
OSSL_PARAM_set_octet_ptr(OSSL_PARAM_locate(B<p>, B<key>), B<val>,
B<used_len>).

OSSL_PARAM_optional_get_TYPE() is the same as OSSL_PARAM_optional_get_TYPE()
except that it returns success if the parameter B<key> is not present in the
parameter array B<p>.

OSSL_PARAM_optional_set_TYPE() is the same as OSSL_PARAM_optional_set_TYPE()
except that it returns success if the parameter B<key> is not present in the
parameter array B<p>.

OSSL_PARAM_optional_get_BN() is the same as OSSL_PARAM_optional_get_BN()
except that it returns success if the parameter B<key> is not present in the
parameter array B<p>.

OSSL_PARAM_optional_set_BN() is the same as OSSL_PARAM_optional_set_BN()
except that it returns success if the parameter B<key> is not present in the
parameter array B<p>.

OSSL_PARAM_optional_get_utf8_string() is the same as
OSSL_PARAM_locate_get_utf8_string() except that it returns success if the
parameter B<key> is not present in the parameter array B<p>.

OSSL_PARAM_optional_set_utf8_string() is the same as
OSSL_PARAM_locate_set_utf8_string() except that it returns success if the
parameter B<key> is not present in the parameter array B<p>.

OSSL_PARAM_optional_get_octet_string() is the same as
OSSL_PARAM_locate_get_octet_string() except that it returns success if the
parameter B<key> is not present in the parameter array B<p>.

OSSL_PARAM_optional_set_octet_string() is the same as
OSSL_PARAM_locate_set_octet_string() except that it returns success if the
parameter B<key> is not present in the parameter array B<p>.

OSSL_PARAM_optional_get_utf8_ptr() is the same as
OSSL_PARAM_locate_get_utf8_ptr() except that it returns success if the
parameter B<key> is not present in the parameter array B<p>.

OSSL_PARAM_optional_set_utf8_ptr() is the same as
OSSL_PARAM_locate_set_utf8_ptr() except that it returns success if the
parameter B<key> is not present in the parameter array B<p>.

OSSL_PARAM_optional_get_octet_ptr() is the same as
OSSL_PARAM_locate_get_octet_ptr() except that it returns success if the
parameter B<key> is not present in the parameter array B<p>.

OSSL_PARAM_optional_set_octet_ptr() is the same as
OSSL_PARAM_locate_set_octet_ptr() except that it returns success if the
parameter B<key> is not present in the parameter array B<p>.
...
This example is of sending RSA public key parameters to a function.
...
To unpack RSA parameters:
    ...
    static int rsa_set_params(void *vrsa, const OSSL_PARAM *params)
    {
        ...
        return OSSL_PARAM_locate_get_BN(params, "n", rsa->n)
            && OSSL_PARAM_optional_get_BN(params, "e", rsa->e)
            && OSSL_PARAM_optional_get_BN(params, "d", rsa->d);
    }

The sending side is responsible for only using the parameters that were
transferred because there is no missing detection in the
OSSL_PARAM_optional_get_BN() function.
