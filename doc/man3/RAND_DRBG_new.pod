...
RAND_DRBG_set_crng_test_block_size()
sets the number of I<bytes> used for a I<FIPS 140-2 Continuous
random number generator test (section 4.9.2)> for the B<drbg>.
Specifying B<blocksize> to be zero, disables this testing, which is the
default setting.  This call is only valid when the B<drbg> is in the
