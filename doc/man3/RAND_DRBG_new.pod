...
the secure heap, for the given B<ctx>
(using OPENSSL_zalloc() resp. OPENSSL_secure_zalloc()). The B<ctx> parameter can
...
RAND_DRBG_set_defaults_ex() sets the default B<type> and B<flags> for new DRBG
instances in the given B<ctx>. The B<ctx> parameter be NULL in which cae the
default OPENSSL_CTX is used.
RAND_DRBG_set_defaults() is the same as RAND_DRBG_set_defaults_ex() with a
NULL B<ctx>.
...
RAND_DRBG_set_defaults_ex() was added in OpenSSL 3.0.
