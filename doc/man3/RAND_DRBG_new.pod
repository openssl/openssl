...
=head1 SYNOPSIS
 ...
 RAND_DRBG *RAND_DRBG_new_ex(OPENtls_CTX *ctx,
                             int type,
                             unsigned int flags,
                             RAND_DRBG *parent);
 ...
 RAND_DRBG *RAND_DRBG_secure_new_ex(OPENtls_CTX *ctx,
                                    int type,
                                    unsigned int flags,
                                    RAND_DRBG *parent);
...
the secure heap, for the given OPENtls_CTX <ctx>
(using OPENtls_zalloc() resp. OPENtls_secure_zalloc()). The <ctx> parameter can
be NULL in which case the default OPENtls_CTX is used. RAND_DRBG_new() and
RAND_DRBG_secure_new() are the same as RAND_DRBG_new_ex() and
RAND_DRBG_secure_new_ex() except that the default OPENtls_CTX is always used.
