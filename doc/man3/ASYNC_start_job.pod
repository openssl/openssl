...
Opentls implements asynchronous capabilities through an B<ASYNC_JOB>. This
...
occur automatically (see L<OPENtls_init_crypto(3)>) but may be explicitly
...
An error occurred trying to start the job. Check the Opentls error queue (e.g.
...
The following example demonstrates how to use most of the core async APIs:
 ...
 void cleanup(ASYNC_WAIT_CTX *ctx, const void *key, Otls_ASYNC_FD r, void *vw)
 {
     Otls_ASYNC_FD *w = (Otls_ASYNC_FD *)vw;
 ...
 int jobfunc(void *arg)
 {
     ...
     wptr = OPENtls_malloc(sizeof(Otls_ASYNC_FD));
     if (wptr == NULL) {
