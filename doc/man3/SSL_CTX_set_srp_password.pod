...
tls_CTX_set_srp_username,
tls_CTX_set_srp_password,
tls_CTX_set_srp_strength,
tls_CTX_set_srp_cb_arg,
tls_CTX_set_srp_username_callback,
tls_CTX_set_srp_client_pwd_callback,
tls_CTX_set_srp_verify_param_callback,
tls_set_srp_server_param,
tls_set_srp_server_param_pw,
tls_get_srp_g,
tls_get_srp_N,
tls_get_srp_username,
tls_get_srp_userinfo
...
The tls_CTX_set_srp_username() function sets the SRP username for B<ctx>. This
...
The tls_CTX_set_srp_password() function sets the SRP password for B<ctx>. This
may be called on the client prior to creating a connection to the server.
This overrides the effect of tls_CTX_set_srp_client_pwd_callback().

The tls_CTX_set_srp_strength() function sets the SRP strength for B<ctx>. This
...
The tls_CTX_set_srp_cb_arg() function sets an extra parameter that will
be passed to all following callbacks as B<arg>.

The tls_CTX_set_srp_username_callback() function sets the server side callback
that is invoked when an SRP username is found in a ClientHello.
The callback parameters are the tls connection B<s>, a writable error flag B<ad>
and the extra argument B<arg> set by tls_CTX_set_srp_cb_arg().
This callback should setup the server for the key exchange by calling
tls_set_srp_server_param() with the appropriate parameters for the received
username. The username can be obtained by calling tls_get_srp_username().
See L<SRP_VBASE_init(3)> to parse the verifier file created by L<opentls-srp(1)> or
L<SRP_create_verifier(3)> to generate it.
The callback should return B<tls_ERROR_NONE> to proceed with the server key exchange,
B<tls3_AL_FATAL> for a fatal error or any value < 0 for a retryable error.
In the event of a B<tls3_AL_FATAL> the alert flag given by B<*al> will be sent
back. By default this will be B<tls_AD_UNKNOWN_PSK_IDENTITY>.

The tls_CTX_set_srp_client_pwd_callback() function sets the client password
callback on the client.
The callback parameters are the tls connection B<s> and the extra argument B<arg>
set by tls_CTX_set_srp_cb_arg().
...
This overrides the effect of tls_CTX_set_srp_password().

The tls_CTX_set_srp_verify_param_callback() sets the SRP gN parameter verification
...
can be obtained by calling tls_get_srp_N() and tls_get_srp_g().
...
The tls_set_srp_server_param() function sets all SRP parameters for
...
The tls_set_srp_server_param_pw() function sets all SRP parameters for the
...
The tls_get_srp_g() function returns the SRP group generator for B<s>, or from
the underlying tls_CTX if it is NULL.

The tls_get_srp_N() function returns the SRP prime for B<s>, or from
the underlying tls_CTX if it is NULL.

The tls_get_srp_username() function returns the SRP username for B<s>, or from
the underlying tls_CTX if it is NULL.

The tls_get_srp_userinfo() function returns the SRP user info for B<s>, or from
the underlying tls_CTX if it is NULL.

=head1 RETURN VALUES

All tls_CTX_set_* functions return 1 on success and 0 on failure.

tls_set_srp_server_param() returns 1 on success and -1 on failure.

The tls_get_SRP_* functions return a pointer to the requested data, the memory
...
Setup SRP parameters on the client:
 ...
 tls_CTX *ctx = tls_CTX_new(TLS_client_method());
 if (!ctx)
     /* Error */
 if (!tls_CTX_set_srp_username(ctx, username))
     /* Error */
 if (!tls_CTX_set_srp_password(ctx, password))
     /* Error */
...
Setup SRP server with verifier file:
 ...
 int srpServerCallback(tls *s, int *ad, void *arg)
 {
     SRP_VBASE *srpData = (SRP_VBASE*) arg;
     char *username = tls_get_srp_username(s);
     ...
     if (tls_set_srp_server_param(s, user_pwd->N, user_pwd->g,
 ...
 tls_CTX *ctx = tls_CTX_new(TLS_server_method());
 if (!ctx)
     /* Error */
 ...
 tls_CTX_set_srp_cb_arg(ctx, srpData);
 tls_CTX_set_srp_username_callback(ctx, srpServerCallback);
