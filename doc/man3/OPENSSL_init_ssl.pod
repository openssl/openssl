...
OPENSSL_init_ssl - OpenSSL (libssl and libcrypto) initialization
...
needs so no explicit initialization is required. Similarly it will also
automatically deinitialise as required.

However, there may be situations when explicit initialization is desirable or
needed, for example when some non-default initialization is required. The
function OPENSSL_init_ssl() can be used for this purpose. Calling
this function will explicitly initialize BOTH libcrypto and libssl. To
explicitly initialize ONLY libcrypto see the
L<OPENSSL_init_crypto(3)> function.

Numerous internal OpenSSL functions call OPENSSL_init_ssl().
Therefore, in order to perform non-default initialization,
...
initialized. Valid options for libcrypto are described on the
