...
TS_VERIFY_CTX_free() frees up a B<TS_VERIFY_CTX> object. B<ctx> is the
verification context to be freed. If B<ctx> is NULL, the call is ignored.

TS_VERIFY_CTX_set_flags() sets the flags in the verification context. B<f> are
the flags to be set.

TS_VERIFY_CTX_add_flags() adds flags to the verification context. B<f> are the
flags to be added (OR'd).

TS_VERIFY_CTX_set0_data() sets the data to be verified. B<b> is the B<BIO> with
the data. A previously assigned B<BIO> is freed.

TS_VERIFY_CTX_set0_imprint() sets the message imprint. B<hexstr> is the
message imprint to be assigned. A previously assigned imprint is freed.

TS_VERIFY_CTX_set0_store() sets the store for the verification context. B<s> is
...
verifying a TST. B<certs> is a stack of B<X509> certificates.

TS_VERIFY_CTX_cleanup() frees all data associated with the given
B<TS_VERIFY_CTX> object and initializes it. B<ctx> is the verification context
created in advance. If B<ctx> is NULL, the call is ignored.
...
A previously assigned BIO is B<not freed> by this call. B<b> is the B<BIO>
with the data to assign.

TS_VERIFY_CTX_set_imprint() is used to set the message imprint. A previously
assigned imprint B<is freed> by this call. B<hexstr> is the string with the
message imprint to assign.

TS_VERIFY_CTX_set_store() is used to set the certificate store. A previously
assigned store is B<not freed> by this call. B<s> is the store to assign.

TS_VERIFY_CTX_set_certs() is used to set the server's certificate chain.
A previously assigned stack is B<not freed> by this call. B<certs> is a stack
...
TS_VERIFY_CTX_set_flags() returns the flags passed via parameter B<f>.

TS_VERIFY_CTX_add_flags() returns the flags of the context after the ones
passed via parameter B<f> are added to it.
...
the user passes via parameter B<bio>, B<hexstr>, B<s> or B<certs>.
