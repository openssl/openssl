...
OSSL_DECODER,
OSSL_DECODER_new,
OSSL_DECODER_set_params,
OSSL_DECODER_free,
OSSL_DECODER_set_selection,
OSSL_DECODER_set_input_type,
OSSL_DECODER_set_input_structure,
OSSL_DECODER_add_method,
OSSL_DECODER_add_extra_methods,
OSSL_DECODER_get_num_methods,
OSSL_DECODER_INSTANCE,
OSSL_DECODER_CONSTRUCT,
OSSL_DECODER_CLEANUP,
OSSL_DECODER_set_construct,
OSSL_DECODER_set_construct_data,
OSSL_DECODER_set_cleanup,
OSSL_DECODER_get_construct,
OSSL_DECODER_get_construct_data,
OSSL_DECODER_get_cleanup,
OSSL_DECODER_export,
OSSL_DECODER_INSTANCE_get_method,
OSSL_DECODER_INSTANCE_get_method_ctx,
...
The B<OSSL_DECODER> holds data about multiple decoders, as needed to
...
starting point.  This is both considered by OSSL_DECODER_add_extra(),
...
The constructor is set with OSSL_DECODER_set_construct().
...
OSSL_DECODER_new() creates a new empty B<OSSL_DECODER>.

OSSL_DECODER_settable_params() returns an L<OSSL_PARAM(3)> array of
parameter descriptors.

OSSL_DECODER_set_params() attempts to set parameters specified with an
L<OSSL_PARAM(3)> array I<params>.  These parameters are passed to all
decoders that have been added to the I<decoder> so far.  Parameters that an
implementation doesn't recognise should be ignored by it.

OSSL_DECODER_free() frees the given I<decoder>.

OSSL_DECODER_add_method() populates the I<decoder> with a decoder, to be
used to attempt to decode some encoded input.

OSSL_DECODER_add_extra_methods() finds decoders that generate input for already
added decoders, and adds them as well.  This is used to build decoder
chains.

OSSL_DECODER_set_input_type() sets the starting input type.  This limits
the decoder chains to be considered, as explained in the general description
above.

OSSL_DECODER_set_input_structure() sets the name of the structure that
...
OSSL_DECODER_get_num_methods() gets the number of decoders currently
added to the I<decoder>.

OSSL_DECODER_set_construct() sets the constructor I<construct>.

OSSL_DECODER_set_construct_data() sets the constructor data that is
passed to the constructor every time it's called.

OSSL_DECODER_set_cleanup() sets the constructor data I<cleanup>
function.  This is called by OSSL_DECODER_free().

OSSL_DECODER_get_construct(), OSSL_DECODER_get_construct_data() and
OSSL_DECODER_get_cleanup() return the values that have been set by
OSSL_DECODER_set_construct(), OSSL_DECODER_set_construct_data() and
OSSL_DECODER_set_cleanup() respectively.
...
The pointer that was set with OSSL_DECODE_set_construct_data().
...
OSSL_DECODER_INSTANCE_get_method be used to get the decoder
implementation from a decoder instance I<decoder_inst>.

OSSL_DECODER_INSTANCE_get_method_ctx() can be used to get the decoder
...
OSSL_DECODER_new() returns a pointer to a B<OSSL_DECODER>, or NULL if the
structure couldn't be allocated.

OSSL_DECODER_settable_params() returns an L<OSSL_PARAM(3)> array, or
NULL if none is available.

OSSL_DECODER_set_params() returns 1 if all recognised parameters were
valid, or 0 if one of them was invalid or caused some other failure in the
implementation.

OSSL_DECODER_add_method(), OSSL_DECODER_add_extra_methods(),
OSSL_DECODER_set_construct(), OSSL_DECODER_set_construct_data() and
OSSL_DECODER_set_cleanup() return 1 on success, or 0 on failure.

OSSL_DECODER_get_construct(), OSSL_DECODER_get_construct_data() and
OSSL_DECODER_get_cleanup() return the current pointers to the
constructor, the constructor data and the cleanup functions, respectively.

OSSL_DECODER_num_methods() returns the current number of decoders.  It
returns 0 if I<decoder> is NULL.

OSSL_DECODER_export() returns 1 on success, or 0 on failure.

OSSL_DECODER_INSTANCE_get_method() returns an B<OSSL_DECODER_METHOD> pointer
on success, or NULL on failure.

OSSL_DECODER_INSTANCE_get_methoid_ctx() returns a provider context pointer on
success, or NULL on failure.

=head1 SEE ALSO

L<provider(7)>, L<OSSL_DECODER_METHOD(3)>, L<OSSL_DECODER_from_bio(3)>
