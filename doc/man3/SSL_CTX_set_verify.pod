...
The behavior can be controlled by the additional
...
The B<verify_callback> function is used to control the behavior when the
...
lead to unexpected behavior if SSL_VERIFY_PEER and other flags are not used as
required.
