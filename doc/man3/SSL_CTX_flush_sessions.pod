...
SSL_SESSION_CACHE_flush - remove expired sessions
...
SSL_SESSION_CACHE_flush() causes a run through the B<cache> to
...
These functions will only check sessions stored in the internal
cache. When a session is found and removed, the remove_session_cb is however
called to synchronize with the external cache (see
L<SSL_CTX_sess_set_get_cb(3)>). The B<SSL_CTX> will be NULL when
SSL_SESSION_CACHE_flush() is used.

=head1 HISTORY

The SSL_SESSION_CACHE_flush() function was introduced in OpenSSL
1.1.1.
