...
X509_LOOKUP_add_store_ex() passes a URI or filename for a directory-like or file-like
structure from which containers with certificates and CRLs are loaded on demand
into the given B<X509_STORE>. The library context I<libctx> and property
query I<propq> are used when fetching algorithms from providers.
