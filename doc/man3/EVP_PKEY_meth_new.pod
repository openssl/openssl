...
There are two places where store the B<EVP_PKEY_METHOD> objects, one is
a built-in static array represneting the stadard methods for different
alogorithms, the other one is a stack of user defined application specified
methods, which can be manipulated by using L<EVP_PKEY_meth_add0(3)> function.

The B<EVP_PKEY_METHOD> objects are usally referenced by B<EVP_PKEY_CTX>
objects.

=head2 Functions

EVP_PKEY_meth_new() creates and returns a new B<EVP_PKEY_METHOD> object,
and associates the given B<id> and B<flags>.
...
EVP_PKEY_meth_add0() adds B<pemth> to the user defined stack of methods.
...
values. For the 'get' functions, function poitners are returned by
arguments.
