...
L<EVP_PKEY_CTX_free> and L<EVP_PKEY_CTX_dup>.

 int (*paramgen_init) (EVP_PKEY_CTX *ctx);
 int (*paramgen) (EVP_PKEY_CTX *ctx, EVP_PKEY *pkey);

The paramgen_init() and paramgen() methods deal with key parameters generation.
...
The verify_init() and verify() methods are used to verify a signature is valid
or not. They are called by L<EVP_PKEY_verify_init(3)> and L<EVP_PKEY_verify(3)>.
...
from a public key algorithm (for instance, DH algorithm). They are called by
L<EVP_PKEY_derive_init(3)> and L<EVP_PKEY_derive(3)>.

 int (*ctrl) (EVP_PKEY_CTX *ctx, int type, int p1, void *p2);
 int (*ctrl_str) (EVP_PKEY_CTX *ctx, const char *type, const char *value);

The ctrl() and ctrl_str() methods are used to adjust algorithm specified
