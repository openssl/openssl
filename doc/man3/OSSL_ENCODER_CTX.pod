...
OSSL_ENCODER_CTX_prune_encoders() goes through all the encoders that were
added with OSSL_ENCODER_CTX_add_encoder() and OSSL_ENCODER_CTX_add_extra(),
and removes those that don't match the criteria given with
OSSL_ENCODER_CTX_set_selection(), OSSL_ENCODER_CTX_set_output_type() and
OSSL_ENCODER_CTX_set_output_structure().
...
OSSL_ENCODER_INSTANCE_get_encoder() can be used to get the encoder method
from a encoder instance I<encoder_inst>.

OSSL_ENCODER_INSTANCE_get_encoder_ctx() can be used to get the encoder
method's provider context from a encoder instance I<encoder_inst>.
...
for encoder method from a encoder instance I<encoder_inst>.  This will never
be NULL.
...
structure for encoder method from a encoder instance I<encoder_inst>.  This
may be NULL.
