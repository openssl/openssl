...
OSSL_ENCODER_CTX_set_finalized() finalises the context.
The functions OSSL_ENCODER_CTX_add_encoder(), OSSL_ENCODER_CTX_add_extra(),
OSSL_ENCODER_CTX_set_cleanup() OSSL_ENCODER_CTX_set_construct(),
OSSL_ENCODER_CTX_set_construct_data() OSSL_ENCODER_CTX_set_output_structure(),
OSSL_ENCODER_CTX_set_output_type(), and OSSL_ENCODER_CTX_set_selection() return
an error if called after the context is finalised.
