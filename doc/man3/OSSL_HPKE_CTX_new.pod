...
OSSL_HPKE_seal_base, OSSL_HPKE_open_base
...
OSSL_HPKE_BOX_seal() calls OSSL_HPKE_encap and OSSL_HPKE_seal, and serializes the
outputs into a newly allocated buffer, whose address is returned in box and sets boxlen
accordingly. It is the caller's responsibility to free I<*box>.

OSSL_HPKE_BOX_open() calls OSSL_HPKE_decap and OSSL_HPKE_open, and returns a
newly allocated copy of the plaintext in ptx, and sets ptlen to the length. It is
the caller's responsibility to free I<*ptx>.
