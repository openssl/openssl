...
X509_check_issued,
X509_check_akid - check if certificate is apparently issued by another one
...
matches the subject field of I<issuer>. Moreover, it compares using
...
It assumes that the internal B<ossl_x509v3_cache_extensions()> function
(or equivalently, L<X509_check_purpose(3)> with its I<id> parameter being -1)
has already been called to fill in various internal fields of I<issuer>.
It checks if all sub-fields of the authority key identifier information I<akid>,
as far as present, match the respective B<subjectKeyIdentifier> extension (if
present in I<issuer>), B<serialNumber> field, and B<issuer> fields of I<issuer>.
