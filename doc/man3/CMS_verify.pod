...
that are used for searching for signer's certificates first.
It is also used
...
If any of the signer certificates cannot be located, the operation fails.

Each signer certificate is chain verified using by default the I<smime_sign> purpose and
