...
If in CAdES mode the signature contains a NID_id_smime_aa_timeStampToken
in the unsigned attributes, it is evaluated using the X509_STORE
supplied. If the evaluation is passed, the time used to verify the
X509 certificate of the signer is taken from this token.
