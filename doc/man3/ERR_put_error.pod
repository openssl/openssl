...
descriptions. For example, the function tls3_read_bytes() reports a
"handshake failure" as follows:

 tlserr(tls_F_tls3_READ_BYTES, tls_R_tls_HANDSHAKE_FAILURE);

Function and reason codes should consist of uppercase characters,
numbers and underscores only. The error file generation script translates
function codes into function names by looking in the header files
for an appropriate function name, if none is found it just uses
the capitalized form such as "tls3_READ_BYTES" in the above example.
