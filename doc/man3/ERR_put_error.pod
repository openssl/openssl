...
ERR_raise, ERR_raise_data, ERR_raise_malloc_failure,
ERR_raise_realloc_failure, ERR_raise_clear_realloc_failure,
...
ERR_raise_malloc_failure(), ERR_raise_realloc_failure(), and
ERR_raise_clear_realloc_failure() are meant to be used only internally.

ERR_raise_malloc_failure() calls I<(*alloc_fn)(num, file, line)>
and returns its result if I<num> is zero or the result is not NULL.
Otherwise it acts like I<ERR_raise(ERR_LIB_CRYPTO, ERR_R_MALLOC_FAILURE)>
with the I<file>, I<line>, and I<func> parameters taken as the error location.

ERR_raise_realloc_failure() is like ERR_raise_malloc_failure()
but calls I<CRYPTO_realloc(addr, num, file, line)>.

ERR_raise_clear_realloc_failure() is like ERR_raise_malloc_failure()
but calls I<CRYPTO_clear_realloc(addr, old_num, num, file, line)>.
...
ERR_raise_malloc_failure(), ERR_raise_realloc_failure(), and
ERR_raise_clear_realloc_failure() return whatever the underlying
memory (re-)allocation function returns.
...
ERR_raise_malloc_failure(), ERR_raise_realloc_failure(), and
ERR_raise_clear_realloc_failure() were added in OpenSSL 3.1.
