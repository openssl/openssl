...
provider name with a provider initialisation function.
This name can then be used with OSSL_PROVIDER_load().

OSSL_PROVIDER_load() loads and initialises a provider.
This may simply initialise a provider that was previously added with
OSSL_PROVIDER_add_builtin() and run its given initialisation function,
