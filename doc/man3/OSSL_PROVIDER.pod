...
interpret them.
The C<default> provider does not load any parameters from the configuration file
and this is the only way to set C<default> provider parameters.

OSSL_PROVIDER_get_parameters() retrieves global configuration parameters
