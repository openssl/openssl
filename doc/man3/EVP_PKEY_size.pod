...
contents of the target buffer may be smaller or larger than indicated.
It is therefore crucial to take note of the size given back by the
function that performs the operation, such as L<EVP_PKEY_sign(3)>
(the I<siglen> argument will receive that length), to avoid bugs.
