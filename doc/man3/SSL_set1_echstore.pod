...
On ECH rejection, the cover DNS name is applied to the temporary
B<X509_STORE_CTX> used for certificate verification. The application's
configured DNS names, accessible through L<SSL_get0_param(3)>, are unchanged.
Application certificate-verification callbacks should use the parameters of
their B<X509_STORE_CTX>, obtained with L<X509_STORE_CTX_get0_param(3)>, for the
effective DNS constraint, rather than the application's configured DNS names.
