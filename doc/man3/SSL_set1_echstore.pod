...
Those can be used (by servers) to enable ECH for an B<SSL_CTX> or B<SSL>
connection. In addition to loading those values, the application can also
indicate via I<for_retry> which ECHConfig values are to be included in the
I<retry_configs> fallback scheme defined by the ECH protocol.
...
allow clients and servers can use to associate an B<OSSL_ECHSTORE>
...
provide similar control over inner and outer ALPN values.

If a client attempts ECH but that fails, or sends an ECH-GREASEd CH, to an
