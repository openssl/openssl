...
EC_GROUP_get_order() returns 0 if the order is not set (or set to zero) and 1 otherwise.

EC_GROUP_get_cofactor() returns 0 if the cofactor is not set (or is set to zero) and 1 otherwise.
...
EC_GROUP_get_asn1_flag() returns the ASN1 flag for the specified B<curve> .

EC_GROUP_get_point_conversion_form() returns the point_conversion_form for the specified B<curve>.

EC_GROUP_get_degree() returns the degree for the specified B<curve> or 0 if group_get_degree method is not populated.
