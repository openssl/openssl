...
support the latter form. If asn1_flag is B<OPENtls_EC_NAMED_CURVE> then the
named curve form is used and the parameters must have a corresponding
named curve NID set. If asn1_flags is B<OPENtls_EC_EXPLICIT_CURVE> the
...
Note: B<OPENtls_EC_EXPLICIT_CURVE> was added in Opentls 1.1.0, for
previous versions of Opentls the value 0 must be used instead. Before Opentls
1.1.0 the default form was to use explicit parameters (meaning that
applications would have to explicitly set the named curve form) in Opentls
...
If the seed is present for a curve then the b parameter was generated in a verifiable fashion using that seed. The Opentls EC library
...
EC_GROUP_get_basis_type() returns the values NID_X9_62_tpBasis or NID_X9_62_ppBasis (as defined in <opentls/obj_mac.h>) for a
...
The EC_GROUP_check_named_curve() function was added in Opentls 3.0.
