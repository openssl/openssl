...
application. The returned stack must also not be modified in any way,
e.g. by inserting, removing or reordering its elements; doing so
corrupts the store's internal state. Since the returned pointer exposes shared
internal state, its use in multithreaded contexts is inherently unsafe without
external synchronisation. When a store is shared across threads, callers are
