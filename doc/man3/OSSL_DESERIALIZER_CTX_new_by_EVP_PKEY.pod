...
If no suitable serializer was found, OSSL_DESERIALIZER_CTX_new_by_EVP_PKEY()
...
creating the B<OSSL_DESERIALIZER_CTX>, and the lack the serializer
...
I<cipher_name> may also be NULL, which will result in failure if the
serialized input is an encrypted key.
