...
X509_EXTENSION_get_critical, X509_EXTENSION_get_data,
X509_EXTENSION_get0_value - extension utility functions
...
B<crit> is zero the extension in non-critical otherwise it is critical. The
outcome reported by X509_EXTENSION_get0_value() follows the criticality.
...
X509_EXTENSION_get0_value() gets the value of extension B<ex> as decoded,
into the extension specific structure for its type, when the extension was
parsed or set. If B<value> is not NULL, B<*value> is set to a borrowed pointer
to that structure, or to NULL if there is none. If B<outcome> is not NULL,
B<*outcome> is set to one of:

=over 4

=item B<X509_EXT_VALUE_DECODED>

B<*value> points to the decoded structure.

=item B<X509_EXT_VALUE_UNKNOWN>, B<X509_EXT_VALUE_UNKNOWN_CRITICAL>

No B<X509V3_EXT_METHOD> is registered for the type of B<ex>. The second form
is reported when B<ex> is critical. RFC 5280 requires a certificate with a
critical extension the user does not recognise to be rejected, and permits a
non-critical one to be ignored.

=item B<X509_EXT_VALUE_INVALID>, B<X509_EXT_VALUE_INVALID_CRITICAL>

The registered method rejected the data. The second form is reported when
B<ex> is critical.

=item B<X509_EXT_VALUE_MALLOC_FAILED>

Decoding the data ran out of memory.

=item B<X509_EXT_VALUE_ERROR>

Decoding the data failed for a reason the decoder did not report.
...
X509_EXTENSION_get0_value() returns 1 if B<*outcome> is
...
X509_EXTENSION_get0_value() was added in OpenSSL 4.1.
