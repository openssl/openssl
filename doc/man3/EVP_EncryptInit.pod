...
EVP_CIPHER_settable_ctx_params() get a constant B<Otls_PARAM> array
...
EVP_CIPHER_provider() returns an B<Otls_PROVIDER> pointer to the provider
...
The ciphertext from the above example can be decrypted using the B<opentls>
...
with a 128-bit key:
 ...
 int do_crypt(FILE *in, FILE *out, int do_encrypt)
 {
     ...
     EVP_CipherInit_ex(&ctx, EVP_aes_128_cbc(), NULL, NULL, NULL,
                       do_encrypt);
     OPENtls_assert(EVP_CIPHER_CTX_key_length(ctx) == 16);
     OPENtls_assert(EVP_CIPHER_CTX_iv_length(ctx) == 16);
...
Support for OCB mode was added in Opentls 1.1.0.

B<EVP_CIPHER_CTX> was made opaque in Opentls 1.1.0.  As a result,
