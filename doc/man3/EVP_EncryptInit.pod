...
It uses standard block padding (a.k.a. PKCS padding) as described in
...
Sets the CCM nonce (IV) length. This call can only be made before specifying a nonce value. The nonce length is given by B<15 - L> so it is 7 by default for
AES.
...
For SIV mode ciphers the behavior of the EVP interface is subtly
