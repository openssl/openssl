...
0 is returned to indicate failure. EVP_EncryptUpdate() is guaranteed to
be able to encrypt in place when the B<ctx> has not been used before.
Note: the above statement about the necessary output buffer size is not
true for WRAP mode ciphers. These ciphers may write up to (inl + 16)
bytes into B<out>.

Partially overlapping is any kind of overlapping that is known to be
not working in the evp cipher update function. It is dependent on the
cipher mode, and dependent on the internal state of the cipher context.
