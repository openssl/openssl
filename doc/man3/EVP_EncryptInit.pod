...
B<algorithm> from any provider offering it, withing the criteria given
by the B<properties>.
...
EVP_CIPHER_fetch(). If supplied by a function such as
EVP_aes_256_cbc(), the ENGINE B<impl> may be non-NULL and point at an
engine that implements the type, and otherwise, the default
implementation of digest B<type> is used. If supplied by an explicitly
fetched value, B<impl> is ignored. B<key> is the symmetric key to use
