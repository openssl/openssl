...
or O otherwise.
This may be used by AES GCM ciphers after calling a cipher final operation such
as EVP_EncryptFinal_ex().
AES GCM should generate an IV internally if the iv is not specified during a
cipher init call such as EVP_CipherInit_ex().
