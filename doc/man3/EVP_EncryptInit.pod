...
The supplied data starts with 13 bytes of record data containing:
...
For AES-CBC-HMAC ciphers, the optional "tls1multi_aadlen" parameter
specifies the total input length.

"tls1multi_interleave" must also be set for this operation.

=item "tls1multi_aadlen" (B<OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_AAD_LEN>) <unsigned integer>

Sets the total input length for an AES-CBC-HMAC TLS1 multiblock cipher operation.
It must be set with "tls1multi_aad". If it is not set, the cipher uses the
legacy "tls1multi_aad" length convention: when bytes 11-12 are zero, the
octet string size is the total input length. This parameter is only supported
by AES-CBC-HMAC ciphers.
...
"tls1multi_aad" (B<OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_AAD>),
"tls1multi_aadlen" (B<OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_AAD_LEN>) and
