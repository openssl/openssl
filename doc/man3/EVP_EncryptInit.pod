...
EVP_aes_128_siv, EVP_aes_192_siv, EVP_aes_256_siv,
...
the L</Authenticated Modes> section below for details.
...
=item EVP_aes_128_siv(), EVP_aes_192_siv(), EVP_aes_256_siv()

AES in Synthetic Initialization Vector (SIV) mode
for 128, 192 and 256 bit keys respectively.
These ciphers require additional control operations to function correctly: see
SIV mode section below for details.
...
L</Authenticated Modes> section for more information.

=back

=head1 Authenticated Modes

For GCM, OCB, and SIV mode ciphers the behaviour of the EVP interface is subtly
...
The following ctrls are supported in by all modes:
