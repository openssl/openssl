...
that can be used with EVP_CIPHER_get_params(), or NULL on error.

=item EVP_CIPHER_gettable_ctx_params() and EVP_CIPHER_CTX_gettable_params()

Get a constant L<OSSL_PARAM(3)> array that describes the retrievable parameters
that can be used with EVP_CIPHER_CTX_get_params(), or NULL on error.
...
that can be used with EVP_CIPHER_CTX_set_params(), or NULL on error.
