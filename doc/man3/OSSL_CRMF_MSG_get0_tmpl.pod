...
OSSL_CRMF_ASN1_get_int() converts an ASN1_INTEGER into an int type.
If successful (i.e., the ASN.1 encoded value is valid and fits in an int)
it sets B<*pr> to the value of B<a>.
...
OSSL_CRMF_ASN1_get_int() returns 1 on success and 0 on error.
