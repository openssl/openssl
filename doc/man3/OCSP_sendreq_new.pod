...
B<req>, which is the ASN.1 type B<it>.
The B<content_type>, if not NULL, it will be included in the HTTP request.
The function should be called after all other headers have already been added.

OCSP_REQ_CTX_set1_req() is equivalent to the following:
