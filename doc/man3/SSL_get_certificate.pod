...
object is available, for example on a client where no client certificate is
being used, or on a server where a specific certificate has not yet been chosen.
