...
This error can also be returned when the library generates an error.
For instance it's generated on the second call to SSL_shutdown() when not all
data was read using SSL_read().
