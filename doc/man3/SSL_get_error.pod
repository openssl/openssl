...
The read and write functions can return SSL_ERROR_WANT_READ when there is still
unprocessed data available at either the SSL or the BIO layer, see
L<SSL_read(3)> and L<SSL_write(3)> for more information.
