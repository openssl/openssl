...
The following functions can be called on either the server or the client side.
...
The following functions are very similar to the functions described above except
that they only have an effect when called on the server side. The list of CA
names managed are separate - so you cannot (for example) set CA names using the
functions above and then get them using the functions below. Where a mix of the
above functions and the functions below have been used on the server side the
settings set by the following functions take precedence. These functions are
present for backwards compatibility. L<SSL_CTX_set0_CA_list(3)> and similar
...
This list must explicitly be set using SSL_CTX_set0_CA_list() or
SSL_CTX_set_client_CA_list() for B<ctx> and SSL_set0_CA_list() or
SSL_set_client_CA_list() for the specific B<ssl>. The list specified
...
L<SSL_load_client_CA_file(3)> function can be used to help importing the
necessary data.

SSL_CTX_add1_to_CA_list(), SSL_CTX_add_client_CA(), SSL_add1_to_CA_list and
SSL_add_client_CA() can be used to add additional items the list of CAs. If no
list was specified before using SSL_CTX_set0_CA_list(),
SSL_CTX_set_client_CA_list(), SSL_set0_CA_list or SSL_set_client_CA_list(), a
new CA list for B<ctx> or B<ssl> (as appropriate) is opened.

=head1 RETURN VALUES

SSL_CTX_set0_CA_list(), SSL_set0_CA_list(), SSL_CTX_set_client_CA_list(),
SSL_set_client_CA_list(), SSL_CTX_set_client_CA_list() and
SSL_set_client_CA_list() do not return a value.

SSL_CTX_get0_CA_list() and SSL_get0_CA_list() return a stack of CA names
or B<NULL> is no CA names are set.

SSL_CTX_add1_to_CA_list(), SSL_add1_to_CA_list() SSL_CTX_add_client_CA() and
SSL_add_client_CA() return 1 for success and 0 for failure.
...
List of CA names explicitly set (for B<ctx> or in server mode) or sent
