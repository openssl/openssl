...
=head1 SYNOPSIS
 ...
 int OSSL_CMP_CTX_set1_secretValue(OSSL_CMP_CTX *ctx, const unsigned char *sec,
                                   const size_t len);
 int OSSL_CMP_CTX_set1_caCert(OSSL_CMP_CTX *ctx, const X509 *cert);
 ...
 typedef void (*OSSL_cmp_log_cb_t)(const char *file, int lineno, severity level,
                                   const char *msg);
 int OSSL_CMP_CTX_set_log_cb(OSSL_CMP_CTX *ctx, OSSL_cmp_log_cb_t cb);

 int OSSL_CMP_puts(const char *file, int lineno, severity level,
                   const char *msg);
 ...
 int OSSL_CMP_CTX_set_popoMethod(OSSL_CMP_CTX *ctx, int method);
...
OSSL_CMP_CTX_set1_caCert() sets the given CA Certificate in the given
OSSL_CMP_CTX structure.

OSSL_CMP_CTX_set1_srvCert() sets the server certificate to be directly trusted
for verifying response messages. Additionally using
OSSL_CMP_CTX_set0_trustedStore() is
recommended in order to be able to supply verification parameters and CRLs.
...
OSSL_CMP_CTX_set_log_cb() sets the log callback for error/warn/info/debug
messages.  It obtains the current source file path name and line number
...
OSSL_CMP_puts() prints errors and warnings to stderr, whereas info and debug
messages are printed to stdout. In debug mode, the message is preceded by a
source file path name and line number if available or else by "CMP". It is also
proceded by a severity indication such as "ERROR", "WARNING", "INFO", or
"DEBUG".

OSSL_CMP_printf() is used for outputting error/warn/info/debug messages. They
can be given using a format string with variable number of arguments as with
printf. The resulting string is handed over to the log callback.
...
must return a bit number <= OSSL_CMP_PKIFAILUREINFO_MAX indicating failure and
...
in the ctx by means of OSSL_CMP_CTX_set_certConf_cb_arg(), which may be
...
as returns bitstring in ulong on success, -1 on error.
