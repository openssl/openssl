...
During FIPS installation this utility is used to run the FIPS modules self tests
and then generate a FIPS related configuration file that contains:
...
This configuration file will then be used each time the FIPS module is loaded
...
Calculate the mac of the FIPS module 'fips.so' and run a FIPS self test
