...
When used, this must be the first at the start of the string and it cannot be
prefixed by any of the special characters described above.
...
If there are extra characters after the word B<DEFAULT> before the separating
colon, they are taken to be a cipher string.
This will often be a non-existent cipher, which has no effect, but the results
can be misleading if you are not careful.

There is no check to see if B<DEFAULT> appears other than as the first word.
