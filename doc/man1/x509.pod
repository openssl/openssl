...
If B<-req> is given, this specifies the input format of the infile, and has
the same meaning and default as the B<-outform> option.

If B<-req> is not given, this option is DEPRECATED AND IGNORED.  The format
of the input file is determined automatically or according to its scheme, see
L<openssl/Key, Certificate and CRL Options>.
...
=item B<-in filename|URI>

This specifies the input input to read a certificate or a CSR, depending on
of B<-req> is given or not.  When B<-req> is given, only the filename argument
is valid.
If this option is not given, standard input is used.
...
DEPRECATED AND GENERALLY IGNORED.  The format of the private key is
determined automatically or according to its scheme, except for the
ENGINE format, see L<openssl/Key, Certificate and CRL Options>.
...
=item B<-force_pubkey filename|URI>

When a certificate is created set its public key to the key given as argument
