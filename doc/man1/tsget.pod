...
and verifying responses, you have to use L<opentls-ts(1)> to do that. This
...
certificate. The directory must be prepared with L<opentls-rehash(1)>. Either
option B<-C> or option B<-P> must be given in case of HTTPS. (Optional)

=item B<-r> I<files>

See L<opentls(1)/Random State Options> for more information.
...
write the response to F<file3.tsr>:
  ...
  opentls ts -query -data file3.txt -cert | tee file3.tsq \
...
=for opentls foreign manuals: WWW::Curl::Easy
