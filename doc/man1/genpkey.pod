...
algorithm. These are identical in every respect and do not indicate the type of
paramters that will be generated. Use the B<dh_paramgen_type> option to indicate
whether PKCS#3 or X9.42 DH parameters are required. See L<DH Parameter
...
The EC key generation options can also be used for paramter generation.
...
The digest to use during parameter generation. Must be one of sha1, sha224 or
sha256. If set then the number of bits in q will match the output size of the
specified digest and the B<dsa_paramgen_q_bits> parameter will be ignored. If
not set then a digest will be used that gives an output matching the number of
bits in q, i.e. sha1 if q length is 160, sha224 if it 224 or sha256 if it is
256.
