...
=item B<-inform PEM|DER|ENGINE>

When B<-topk8> is given, this option is DEPRECATED AND GENERALLY IGNORED.
Please see L<openssl(1)/Key, certificate and CRL options>.

When B<-topk8> is not given, this specifies the input format: see
L</KEY FORMATS> for more details. The default format is PEM. In this
case, ENGINE is an invalid value.
...
If B<-topk8> is used then any supported private key can be used, see
L<openssl(1)/Key, certificate and CRL options>.
The output file will be encrypted PKCS#8 format using the specified encryption
parameters unless B<-nocrypt> is included.
