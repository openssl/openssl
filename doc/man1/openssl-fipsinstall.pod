...
opentls-fipsinstall - perform FIPS configuration installation

=head1 SYNOPSIS

B<opentls fipsinstall>
[B<-help>]
...
C<opentls list -mac-algorithms>.  The default is B<HMAC>.
...
C<opentls list -digest-commands>.
...
for the module, and save the F<fips.conf> configuration file:
 ...
 opentls fipsinstall -module ./fips.so -out fips.conf -provider_name fips \
...
Verify that the configuration file F<fips.conf> contains the correct info:
 ...
 opentls fipsinstall -module ./fips.so -in fips.conf  -provider_name fips \
...
used when building Opentls.
The command C<opentls list -mac-algorithms> command can be used to list them.
