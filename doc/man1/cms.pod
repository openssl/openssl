...
Set Windows 10 compatible wrapping algorithm for KARI
recipients. Windows 10 expects wrapping algorithm parameter to be NULL
on key agreement recipients. Set this option, when Windows recipients
with ECC certificates can't decrypt CMS messages. Use together with a
-*-wrap parameter, for example -aes128-wrap or -aes256-wrap.
