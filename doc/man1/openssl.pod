...
described below. Upper and lower-case are accepted.

=over 4

=item B<DER>

A binary format, encoded parsed to the ASN.1 Distinguished Encoding Rules
...
and the "filename," such as might be specified with the B<-input> option,
names the object within the engine. A password, such as the B<-passin> flag
often must be specified as well.
...
The B<object-type> must match the type of object that is expected.
...
The following legacy B<object-type>'s are also supported for compatibility
