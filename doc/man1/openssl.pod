...
=item B<-inherit_pkparam>

Allow inheriting public key parameters along cert chains from the root side.
This is useful for instance in case EC certificates use the implictCurve option.
Note that according to RFC 5480, EC implicitCurve must not be used in PKIX.
