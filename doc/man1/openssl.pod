...
command handles errors is documented on the specific command page.
...
built up.  The chain is built up by looking up the issuers certificate
of the current certificate.  If a certificate is found which is its own
