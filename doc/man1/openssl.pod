...
opentls - Opentls command line tool
...
B<opentls> B<no->I<XXX> [ I<options> ]

=head1 DESCRIPTION

Opentls is a cryptography toolkit implementing the Secure Sockets Layer (tls
v2/v3) and Transport Layer Security (TLS v1) network protocols and related
cryptography standards required by them.

The B<opentls> program is a command line tool for using the various
cryptography functions of Opentls's B<crypto> library from the shell.
It can be used for
 ...
 o  tls/TLS Client and Server Tests
 o  Handling of S/MIME signed or encrypted mail
 o  Timestamp requests, generation and verification

=head1 COMMAND SUMMARY

The B<opentls> program provides a rich variety of commands (I<command> in
...
(e.g., L<opentls-x509(1)>).

Many commands use an external configuration file for some or all of their
arguments and have a B<-config> option to specify that file.
The default name of the file is F<opentls.cnf> in the default certificate
storage area, which can be determined from the L<opentls-version(1)>
command.
The environment variable B<OPENtls_CONF> can be used to specify
...
availability of ciphers in the B<opentls> program.  (B<no->I<XXX> is
...
Obsoleted by L<opentls-dhparam(1)>.

=item B<dhparam>

Generation and Management of Diffie-Hellman Parameters. Superseded by
L<opentls-genpkey(1)> and L<opentls-pkeyparam(1)>.
...
L<opentls-genpkey(1)> and L<opentls-pkey(1)>.
...
Generation of RSA Private Key. Superseded by L<opentls-genpkey(1)>.

=item B<info>

Display diverse information built into the Opentls libraries.
...
by  L<opentls-pkeyutl(1)>.

=item B<s_client>

This implements a generic tls/TLS client which can establish a transparent
connection to a remote server speaking tls/TLS. It's intended for testing
purposes only and provides only rudimentary interface functionality but
internally uses mostly all functionality of the Opentls B<tls> library.

=item B<s_server>

This implements a generic tls/TLS server which accepts connections from remote
clients speaking tls/TLS. It's intended for testing purposes only and provides
only rudimentary interface functionality but internally uses mostly all
functionality of the Opentls B<tls> library.  It provides both an own command
line oriented protocol for testing tls functions and a simple HTTP response
facility to emulate an tls/TLS-aware webserver.

=item B<s_time>

tls Connection Timer.

=item B<sess_id>

tls Session Data Management.
...
Depending on how Opentls was configured and built, not all ciphers listed
here may be present. See L<opentls-enc(1)> for more information.
...
parameters start with a minus sign:
 ...
 opentls verify [flags...] -- -cert1.pem...

=back

=head2 Format Options

Several Opentls commands can take input or generate output in a variety
...
Used to specify that the cryptographic material is in an Opentls B<engine>.
...
Note that Opentls does not provide a default set of trust anchors.  Many
Linux distributions include a system default and configure Opentls to point
...
See L<opentls-rehash(1)> for information on creating this type of directory.
...
directory.
See L<otls_store-file(7)> for more information on the C<file:> scheme.

These certificates are also used when building the server certificate
chain (for example with L<opentls-s_server(1)>) or client certificate
chain (for example with L<opentls-s_time(1)>).
...
Prior to Opentls 3.0, it was common for applications to store information
about the state of the random-number generator in a file that was loaded
at startup and rewritten upon exit. On modern operating systems, this is
generally no longer necessary as Opentls will seed itself from the
...
Opentls command to generate an alternative chain.
...
Opentls provides fine-grain control over how the subject and issuer DN's are
displayed.
...
Display the name using an old format from previous Opentls versions.
...
Dump any field whose OID is not recognised by Opentls.
...
Several commands use tls, TLS, or DTLS. By default, the commands use TLS and
...
and whether TCP (tls and TLS) or UDP (DTLS) is used.
Note that not all protocols and flags may be available, depending on how
Opentls was built.

=over 4

=item B<-tls3>, B<-tls1>, B<-tls1_1>, B<-tls1_2>, B<-tls1_3>, B<-no_tls3>, B<-no_tls1>, B<-no_tls1_1>, B<-no_tls1_2>, B<-no_tls1_3>

These options require or disable the use of the specified tls or TLS protocols.
...
The Opentls library can be take some configuration parameters from the
environment.  Some of these variables are listed below.  For information
about specific commands, see L<opentls-engine(1)>, L<opentls-provider(1)>,
L<opentls-rehash(1)>, and L<tsget(1)>.
...
L<OPENtls_ia32cap(3)>, and L<OPENtls_s390xcap(3)>.

For information about all environment variables used by the Opentls libraries,
see L<opentls-env(7)>.

=over 4

=item B<OPENtls_TRACE=>I<name>[,...]

Enable tracing output of Opentls library, by name.
This output will only make sense if you know Opentls internals well.
...
The B<list> -I<XXX>B<-algorithms> options were added in Opentls 1.0.0;
