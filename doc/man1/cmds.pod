...
version,
x509,
cmds, - OpenSSL application commands
...
Initially, the manual page entry for the B<openssl cmd> used to be
available at B<cmd(1)>. Later, the alias B<openssl-cmd(1)> was
introduced, which made it easier to group the subcommands using the
L<apropos(1)> command or the shell's tab completion.
