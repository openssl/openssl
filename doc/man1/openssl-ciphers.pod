...
If at least one valid TLSv1.3 ciphersuite is in the cipherlist, or parameter
B<version_mask> is used, all ciphers entered here will be erased
(B<ciphersuites> will be set to empty string).

=item B<version_mask> I<val>

Sets the list of versions. This list will be used as mask for cipher strings
where no version is specified. The format for this list is a simple vertical bar
("|") separated list of version names. By default this value is:

 SSLv3|TLSv1|TLSv1.2

=item B<cipherlist>

A cipher list of ciphersuites will be converted to a cipher preference list. If there
are no TLSv1.3 ciphersuites and B<-version_mask> is not used, this list will be
combined with any TLSv1.3 ciphersuites that have been configured using
B<-ciphersuites>. If it is not included then the default cipher list will be
...
Each cipher string can optionally be preceded by the characters B<!>,
B<->, B<+>, B<^>, B<*> or can be closed by brackets B<[> B<]> into
...
If B<+> is used then the ciphers are moved to the bottom of the list. This
option doesn't add any new ciphers it just moves matching existing ones.
Can't be used within groups and doesn't affect ciphersuites within groups.

If B<^> is used then the ciphers are moved to the top of the list. This
option doesn't add any new ciphers it just moves matching existing ones.
Can't be used within groups and doesn't affect ciphersuites within groups.

If B<*> is used then the cipher preference flag is set. This option doesn't
add any new ciphers it just sets the flag. Can be used within or outside of the group.
If it is used within a group, it is only applied to the ciphersuites within the currrent group.
...
cipher list in order of encryption algorithm key length. Can't be used with
...
Can't be used with equal-preference groups.
...
this prefix must not be combined with other strings using B<+> character, but can
be used vith version strings. For example, B<DEFAULT+DES> is not valid,
but B<DEFAULT|ALL> or B<DEFAULT|TLSv1.2|TLSv1.3> is alloved.

The content of the default list is determined at compile time and normally
corresponds to B<ALL:!COMPLEMENTOFDEFAULT:!eNULL>.

=head1 CIPHER PREFERENCE

If cipher has priority flag in server list set and it's the first common cipher in
client list, this cipher will be selected.
...
client: DEFAULT|TLSv1.3:^CHACHA20|ALL
 TLS_CHACHA20_POLY1305_SHA256
 TLS_AES_256_GCM_SHA384
 TLS_AES_128_GCM_SHA256

This will result into selecting B<TLS_CHACHA20_POLY1305_SHA256> because it's at
the top of the client's list and has priority flag in the server list.



If some ciphersuites in server cipher list are in an equal-preference group,
these ciphersuites receive the same preference and are ordered according to
the client's preference.

e.g. (suppose only TLSv1.3 ciphersuites):

server: [TLSv1.3:-CHACHA20|ALL]:CHACHA20|ALL
 TLS_AES_256_GCM_SHA384
 TLS_AES_128_GCM_SHA256
 TLS_AES_128_CCM_8_SHA256
 TLS_AES_128_CCM_SHA256
...
client: DEFAULT|TLSv1.3:^AES128|ALL:^CHACHA20|ALL
 TLS_CHACHA20_POLY1305_SHA256
 TLS_AES_128_GCM_SHA256
 TLS_AES_256_GCM_SHA384

First four ciphersuites have equal preference, so the selection depends on the
client's order. In this case B<TLS_AES_128_GCM_SHA256> will be selected,
because B<TLS_CHACHA20_POLY1305_SHA256> is not in equal-preference group.

These preference options take effect only in the server cipher list when server
...
cipher suites using Authenticated Encryption with Associated Data (AEAD).
...
The following list gives the SSL and TLS their version names.
