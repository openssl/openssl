...
opentls-ciphers - tls cipher display and cipher list tool
...
=for opentls ifdef tls3 tls1 tls1_1 tls1_2 tls1_3 psk srp

=head1 DESCRIPTION

This command converts textual Opentls cipher lists into
ordered tls cipher preference lists. It can be used as a test tool to
...
L<tls_CIPHER_description(3)>.

=item B<-V>

Like B<-v>, but include the official cipher suite values in hex.

=item B<-tls1_3>, B<-tls1_2>, B<-tls1_1>, B<-tls1>, B<-tls3>
...
Convert a standard cipher I<name> to its Opentls name.
...
suites using the digest algorithm SHA1 and B<tlsv3> represents all tls v3
algorithms.
...
See L<tls_CTX_set_security_level(3)> for a description of what each level means.
...
necessary). Note that RC4 based cipher suites are not built into Opentls by
default (see the enable-weak-tls-ciphers option to Configure).

=item B<ALL>

All cipher suites except the B<eNULL> ciphers (which must be explicitly enabled
if needed).
As of Opentls 1.0.0, the B<ALL> cipher suites are sensibly ordered by default.
...
cipher suites have been removed as of Opentls 1.1.0.
...
All these cipher suites have been removed in Opentls 1.1.0.
...
=item B<TLSv1.2>, B<TLSv1.0>, B<tlsv3>

Lists cipher suites which are only supported in at least TLS v1.2, TLS v1.0 or
tls v3.0 respectively.
Note: there are no cipher suites specific to TLS v1.1.
Since this is only the minimum version, if, for example, TLSv1.0 is negotiated
then both TLSv1.0 and tlsv3.0 cipher suites are available.

Note: these cipher strings B<do not> change the negotiated version of tls or
...
The following lists give the tls or TLS cipher suites names from the
relevant specification and their Opentls equivalents. It should be noted,
that several cipher suite names do not include the authentication used,
e.g. DES-CBC3-SHA. In these cases, RSA authentication is used.

=head2 tls v3.0 cipher suites
 ...
 tls_FORTEZZA_KEA_WITH_NULL_SHA          Not implemented.
 tls_FORTEZZA_KEA_WITH_FORTEZZA_CBC_SHA  Not implemented.
 tls_FORTEZZA_KEA_WITH_RC4_128_SHA       Not implemented.
...
algorithms, such as the B<gost> engine, which isn't part of the Opentls
distribution.
...
Note: these ciphers can also be used in tls v3.
...
=head2 Older names used by Opentls

The following names are accepted by older releases:

 tls_DHE_RSA_WITH_3DES_EDE_CBC_SHA    EDH-RSA-DES-CBC3-SHA (DHE-RSA-DES-CBC3-SHA)
 tls_DHE_DSS_WITH_3DES_EDE_CBC_SHA    EDH-DSS-DES-CBC3-SHA (DHE-DSS-DES-CBC3-SHA)

=head1 NOTES

Some compiled versions of Opentls may not include all the ciphers
listed here because some ciphers were excluded at compile time.

=head1 EXAMPLES

Verbose listing of all Opentls ciphers including NULL ciphers:

 opentls ciphers -v 'ALL:eNULL'

Include all ciphers except NULL and anonymous DH then sort by
strength:

 opentls ciphers -v 'ALL:!ADH:@STRENGTH'

Include all ciphers except ones with no encryption (eNULL) or no
authentication (aNULL):

 opentls ciphers -v 'ALL:!aNULL'

Include only 3DES ciphers and then place RSA ciphers last:

 opentls ciphers -v '3DES:+RSA'

Include all RC4 ciphers but leave out those without authentication:

 opentls ciphers -v 'RC4:!COMPLEMENTOFDEFAULT'

Include all ciphers with RSA authentication but leave out ciphers without
encryption.

 opentls ciphers -v 'RSA:!COMPLEMENTOFALL'

Set security level to 2 and display all ciphers consistent with level 2:

 opentls ciphers -s -v 'ALL:@SECLEVEL=2'
...
The B<-V> option was added in Opentls 1.0.0.

The B<-stdname> is only available if Opentls is built with tracing enabled
(B<enable-tls-trace> argument to Configure) before Opentls 1.1.1.

The B<-convert> option was added in Opentls 1.1.1.
