...
message digest algorithm. The user can specifies a digest algorithm by using
the B<-digest> option.

=item B<-digest algorithm>

This specifies the digest algorithm which is used to hash the input data before
signing it with the input key. This option could be omitted if the signature
algorithm does not require one (for instance, EdDSA). If this option is present,
then the B<-raw> option must be also specified to B<pkeyutl>.
