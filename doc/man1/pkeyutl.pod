...
The input key source, by default it should be a private key.

=item B<-keyform PEM|DER|ENGINE>

DEPRECATED AND GENERALLY IGNORED.  The format of the input key file is
...
=item B<-peerkey file|URI>

The peer key source, used by key derivation (agreement) operations.

=item B<-peerform PEM|DER|ENGINE>

DEPRECATED AND GENERALLY IGNORED.  The format of the peer key file is
determined automatically or according to its scheme, except for the
ENGINE format, see L<openssl(1)/Key, Certificate and CRL Options>.
