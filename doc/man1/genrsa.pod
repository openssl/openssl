...
That is to say, the number of primes is fixed due to a security policy, see
L<RSA_generate_multi_prime_key(3)> for details. But sometimes users may need
to generate more primes in a private key, this option can switch off the
