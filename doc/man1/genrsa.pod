...
By default OpenSSL will force users to generate secure multi-prime keys.
That is to say, the number of primes is limited for a given length of modulus
based on the security policy, see L<RSA_generate_multi_prime_key(3)> for details
of that security policy. However, sometimes users may need to generate more primes
for a private key, this option disables the default security policy.
...
its security depends on both how many primes are used and how long the RSA
modulus is. The security is still related to the difficulty of factoring the
modulus -- in simple terms, the more primes a multi-prime RSA key has, the
easier it is for it to be factored. This link describs the security of multi-prime
RSA: https://www.imperialviolet.org/2011/04/09/multiprime.html. And if you want
to know more, this link provides the requisite details:
