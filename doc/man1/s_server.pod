...
[B<-CRL infile|URI>]
[B<-crl_download>]
[B<-cert_chain infile|URI>]
[B<-dcert_chain infile|URI>]
[B<-chainCApath dir>]
[B<-verifyCApath dir>]
[B<-no_cache>]
[B<-ext_cache>]
...
[B<-verify_quiet>]
[B<-build_chain>]
[B<-chainCAfile infile|URI>]
[B<-verifyCAfile infile|URI>]
[B<-ign_eof>]
[B<-no_ign_eof>]
[B<-status>]
[B<-status_verbose>]
...
DEPRECATED AND IGNORED.  The format of the private key is determined
automatically or according to its scheme, see
L<openssl/Key, Certificate and CRL Options>.

=item B<-key infile|URI>
...
=item B<-dcert infile|URI>, B<-dkey infile|URI>
...
=item B<-dcertform PEM|DER>, B<-dkeyform PEM|DER|ENGINE>
...
Additional private key passphrase.

=item B<-xkey infile|URI>, B<-xcert infile|URI>, B<-xchain infile|URI>
