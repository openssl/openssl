...
server certificate chain when certificate specified via B<-cert> option is in
use.
...
A file containing trusted certificates to use when attempting to build the
server certificate chain when certificate specified via B<-dcert> option is in
use.
...
Specify whether the application should build the certificate chain to be
provided to client for the extra certificates provided via B<-xkey infile>,
B<-xcert infile>, B<-xchain> options.

=item B<-xcertform PEM|DER>, B<-xkeyform PEM|DER>

Extra certificate and private key format and passphrase respectively.
...
The directory to use for building the chain provided to client. This directory
must be in "hash format", see B<verify> for more information.
