...
and typically ending in a trusted self-signed "root" CA certificate.
It is an error if the whole chain cannot be built up to a self-signed
certificate unless the B<-partial_chain> option is given.
...
Allow verification to succeed also if an incomplete> chain can be built,
i.e., to a trust anchor that is not self-signed.
