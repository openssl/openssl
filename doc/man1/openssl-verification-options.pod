...
The I<trust id> determines which trust anchors are acceptable
depending on their auxiliary trust or reject attributes (see L</Trust Anchors> below).
...
=item C<ssl_server> for SSL/(D)TLS servers with purpose C<sslservert> and trust C<serverAuth>
...
where their essential fields are the subjectPublicKeyInfo containing the public key,
...
from the OpenSSL perspective a trust anchor certificate
...
This certificate may be self-issued or belong to an subordinate CA.
