...
When CRL checking is also enabled (B<-crl_check> or B<-crl_check_all>),
a negative CRL Number or Base CRL Number on a CRL selected for revocation
checking is rejected.
This does not change CRL parsing or B<openssl crl -text> output.
