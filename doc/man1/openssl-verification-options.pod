...
for consistency with the given required purpose.
If the B<-purpose> option is not given
then no such checks are done except for SSL/TLS connection setup,
where by default the purpose C<sslserver> or C<sslclient> is checked.
...
The following two subsections on the B<basicConstraints> and <keyUsage>
...
B<codeSigning> OID and must not include the B<anyExtendedKeyUsage> or
