...
Certificate validation can be done relatively to a given
...
is generally optional but highly recommended to include in CA certificates.
...
The goal is that for each certificate in the chain, except for the final
element, the subject of the successor is the issuer of the former certificate.
Since it happens rather often that the subject and issuer Distinguished Names
qare not unique, the subject and issuer key identifier extensions, as far as
present in the certificates, are used for precise and efficient matching.
...
for consistency with any given purpose.
...
Each certificate must either not have an EKU extension,
or the EKU extension must include B<anyExtendedKeyUsage>
or the given required purpose.
...
It must be trusted for the given use (if specified).
...
The extented key usage (EKU) extension places additional restrictions on the
certificate uses. If this extension is present (whether critical or not)
the key must only be used for the purposes specified by the EKUs included there
(or for all purposes if B<anyExtendedKeyUsage> is included in the extension).
