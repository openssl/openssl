...
the given certificate, called I<target certificate>,
...
Although in the given context the term I<validation> would be more adequate,
for historical reasons we usually stick with the term I<verification>.

In a nutshell, for certificate verification a valid chain of certificates needs
...
CMS signature validation, which can be done via L<openssl-cms(1)>,
...
Purposes are strongly related to the extended key usages (EKUs)
that may be placed by the respective CA in X.509v3 certificate extensions.
For instance,
the purpose C<sslserver> corresponds to the EKU B<id-kp-serverAuth>.

While IETF RFC 5280 says that EKUs B<id-kp-serverAuth> and B<id-kp-clientAuth>
...
They mimic the combinations of purpose and trust settings used in SSL/(D)TLS,
CMS/PKCS7 (including S/MIME), and code signing.


=head2 Trust Anchors

In general, according to [RFC 4158 section 1.3](
https://datatracker.ietf.org/doc/html/rfc4158#section-1.3), a I<trust anchor>
is a public key that for some reason is considered trusted for some purpose,
plus the name of the entity to which the corresponding private key belongs.<br>
[RFC 5280 section 6.1.1](
https://datatracker.ietf.org/doc/html/rfc5280#section-6.1.1)
introduces the term I<trust anchor information>, which is a trust anchor
plus some more detail, namely the algorithm of the public key and,
optionally, any parameters of the public key.
...
where their essential fields are the public key
and the subject distinguished name (DN) and/or subject alternative names.
...
which means that its issuer is identical to its subject.
...
From the OpenSSL perspective, when a trust anchor is given as a certificate,
...
See L<openssl-x509(1)/Trust Settings> how to edit those trust attributes.
...
the list of untrusted ("subordinate" CA) certificates, if provided.
...
The third step is to check the trust settings on the trust anchor
(which typically is provided as a self-signed root CA certificate).
...
Parse I<file> as a set of one or more certificates.
All certificates (typically of subordinate CAs) are considered untrusted
and may be used to
