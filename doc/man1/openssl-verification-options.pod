...
adequate, but for historical reasons typically the term I<verification> is used.

Certificate verification is implemented by L<X509_verify_cert(3)>.
It is a complicated process consisting of several steps
and depending on numerous options.
The most important of them are detailed in the following sections.

In a nutshell, a valid chain of certificates needs to be built up and checked
...
Certificate validation can be done relatively to a
I<purpose> specifying the intended use of the target certificate,
such as SSL server, or (by default) for any purpose.
...
OpenSSL checks the validity period of all certificates including trust anchors
and makes use of further fields and extensions
such as subject key identifier and extended key usage.
The (regular) key usage extension is ignored for trust anchor certificates.

In the most simple and common case, trust anchors are by default all
self-signed certificates that have been placed in the I<trust store>,
...
in addition to any included extended key usage (EKU) X.509v3 extension,
should be augmented locally with an explicit designation for which
...
(EKUs) that may be included in X.509v3 certificate extensions.
...
if present in the certificates, are used for precise and efficient matching.
...
for consistency with any purpose required.
If the B<-purpose> option is not given
then no such checks are done except for SSL/TLS connection setup,
where by default C<sslserver> or C<sslclient> is checked.
...
and S/MIME.
The trust model is inferred from the purpose when not
...
Options like B<-purpose> lead to checking specific certificate extensions,
which determine what the target certificate and intermediate CA certificates
can be used for.

The follwing two subsections on the B<basicConstraints> and <keyUsage>
extensions and on X.509v1 certificates apply to B<all> CA certificates
...
The extented key usage (EKU) extension (whether critical or not)
places additional restrictions on certificate use.
If a certain purpose is required, the corresponding set of checks from the
following list is performed.
...
authentication" OID.  The keyUsage extension must be absent
or it must have the digitalSignature or keyAgreement bit set.
...
authentication" or one of the SGC OIDs.
The keyUsage extension must be absent or it must have
any of the digitalSignature, keyEncipherment, or keyAgreement bits set.
...
=item B<common S/MIME client checks>
...
In addition to the common S/MIME client checks the digitalSignature bit or
the nonRepudiation bit must be set for the target certificate
if the keyUsage extension is present.

=item B<S/MIME Encryption>

In addition to the common S/MIME client checks the keyEncipherment bit
must be set for the target certificate if the keyUsage extension is present.
...
The normal CA checks apply. Except in this case the basicConstraints extension
must be present.


=item B<OCSP helper>

For the target certificate special checks apply; see L<OCSP_basic_verify(3)>.
For all other certificates the normal CA checks apply.

=item B<Time Stamp signing>

For the target certificate the following checks apply.
If the keyUsage extension is present, it must have the digitalSignature
or nonRepudiation bit set, while all other bits must not be set.
If the extendedKeyUsage extension is present, it must be marked critical
and include the B<timeStamping> OID and must not include any other EKUs.
...
For the target certificate the following checks apply.
The keyUsage extension must be present and marked critical
and must have the digitalSignature bit set,
while the keyCertSign and crlSign bits must not be set.
Also the extendedKeyUsage extension must be present. It must include the
B<codeSigning> OID and must not include the B<anyExtendedKeyUsage> and
B<serverAuth> ("web server authentication") EKUs.
...
This B<-trusted_first> option is always enabled since since OpenSSL 1.1.0.
