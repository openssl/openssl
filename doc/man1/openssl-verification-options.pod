...
starting from the target certificate and ending in a trust anchor,
which usually is a certificate that due to some policy is trusted.
...
All certificates except the target must also be valid CA certificates.
...
in an end-entity certficiate, only the uses specified are accepted.

The extended key uses with a currently predefined meaning are
B<clientAuth> (SSL/TLS client), B<serverAuth> (SSL/TLS server),
B<emailProtection> (S/MIME email), B<codeSigning> (object signer),
B<OCSPSigning> (OCSP responder), B<OCSP> (OCSP responder),
B<timeStamping> (TSA server), and B<anyExtendedKeyUsage>.
The special EKU B<anyExtendedKeyUsage> allows for all uses.
