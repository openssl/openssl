...
By default all supposedly self-signed certificates in the trust store
are considered to be trust anchors, for all or for specified purposes.

For efficiency reasons, the self-signed status of certificates is
not verified cryptographically. Instead, an approximate heuristic is applied.
A certificate is supposedly self-signed
...
It is supposedly self-signed or the B<-partial_chain> option is given
...
Once a supposedly self-signed certificate has been added, chain construction stops.
...
For compatibility, a (supposedly) self-signed certificate
...
or it is supposedly self-signed or the B<-partial_chain> option is specified.
