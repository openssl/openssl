...
=head2 Certificate validation

I<Certificate validation> is the process of checking whether
the I<target certificate>, i.e., the certificate to be validated,
can be traced down to a so-called trust anchor (see L</Trust Anchors> below)
and is acceptable to use for the verification purpose (see L</Purposes>), if any.

In a nutshell, a valid chain of certificates needs
to be built up and verified starting from the target certificate
and ending in a certificate having a signature that can be verified
...
All certificates in the chain except the target certificate must be CA certificates.
...
are documented on the specific command pages.
...
A I<purpose> is a high-level specification of the intended use of the target certificate.
...
CMS signature validation, which can be done via L<openssl-cms(1)> and L<openssl-smime(1)>,
checks by default for consistency with S/MIME signing purpose (C<smimesign>).
Timestamp validation checks for the (C<timestampsign>) purpose,
and OCSP response validation checks for the (C<ocsphelper>) purpose.
Otherwise, if no purpose is given, no purpose check is performed,
which implies that all purposes are allowed.

Purposes are related to certificate trust attribute values,
which directly correspond to extended key usages (EKUs)
that may be placed by the respective CA in X.509v3 certificate extensions.
For instance,
the purpose C<sslserver> maps to SSL Server trust,
which corresponds to the EKU B<serverAuth>.
Both purposes C<smimesign> and C<smimeencrypt> map to S/MIME email trust,
which corresponds to the EKU B<emailProtection>.

While IETF RFC 5280 contains comments that EKUs B<serverAuth> and B<clientAuth>
are for TLS WWW server or client authentication, respectively,
in fact they are used for all kinds of TLS servers and clients.

For the list of recognized EKUs see the L</Extended Key Usage> section below.


=head2 Verification Methods

A verification method determines a set verification parameters,
such as the verification purpose and flags,
the trust ID, the maximum verification depth
(i.e., the number of subordinate CA certificates allowed in a chain),
and the authentication security level.

By default, no purpose and trust ID are set,
only the flag B<X509_V_FLAG_TRUSTED_FIRST> is set,
the maximum verification depth is 100,
the authentication security level is unset,
and certificate policy processing is not enabled.

The I<trust ID> determines which trust anchors are acceptable, depending
on any given trust or reject attributes (see L</Trust Anchors> below).
The trust attributes can be set using the B<-addtrust> and B<-addreject> options
for L<openssl-x509(1)>.

Verification methods can be given using the B<-verify_name> option of various OpenSSL commands.
The currently predefined verification methods are:

=over 4

=item C<ssl_client> for SSL/(D)TLS clients with purpose C<sslclient> and trust C<clientAuth>

=item C<ssl_server> for SSL/(D)TLS servers with purpose C<sslserver> and trust C<serverAuth>

=item C<smime_sign> with alias C<pkcs7> for CMS/PKCS7 (including S/MIME)
with purpose C<smimesign> and trust C<emailProtection>

=item C<smime_encrypt> for CMS/PKCS7 (including S/MIME)
with purpose C<smimeencrypt> and trust C<emailProtection>

=item C<code_sign> for code signing with purpose C<codesign> and trust C<codeSign>

=item C<default> for general validation, without specific purpose etc.
...
When validating a certificate, a chain of certificates is acceptable only if
the signature of its last certificate can be verified using a trust anchor.

In practice, trust anchors are typically given in the form of certificates,
where their essential fields are the C<subjectPublicKeyInfo> containing the public key,
the subject distinguished name (DN) and/or subject alternative names,
Usually, a I<trust anchor certificate> is self-signed but does not need to be.
A self-signed certificate must also be I<self-issued>,
which means that its issuer DN is identical to its subject DN.
The subjectKeyIdentifier extension is recommended to be present
also in trust anchor certificates because it is useful for chain building.
...
Regardless of any extended key usage (EKU) X.509v3 extension that
a trust anchor certificate may contain, OpenSSL supports
associating such a certificate with an explicit designation indicating
for which uses of target certificates it may serve as a trust anchor.
Without this designation, it is by default permitted for all uses.
...
See L<openssl-x509(1)/Trust Attributes> how to edit those trust attributes.
...
The trust attributes are held in an auxiliary structure and can be persisted in
the certificate PEM encoding, indicated by the C<TRUSTED CERTIFICATE> string.
...
If it does not find a match there it consults the list of untrusted
 ("intermediate" or "subordinate" CA) certificates, if provided.
...
The third step is to check the trust attributes on the trust anchor
(which typically is given as a self-signed root CA certificate).
...
The C<signatureAlgorithm> field and the cert signature must be consistent.
...
This certificate may be self-issued or belong to a subordinate CA.
...
Parse I<file> as a set of one or more certificates in PEM format.
All certificates (typically of intermediate/subordinate CAs) are considered untrusted
and may be used to
...
The intended use for the certificate to be verified.
See L</Purposes> for details.

For details on the checks performed see the L</Certificate Extensions> section.

=item B<-verify_depth> I<num>

Limit the certificate chain to I<num> subordinate CA certificates.
A maximal depth chain can have up to I<num>+2 certificates, since neither the
target certificate nor the trust anchor count against the
...
Use a predefined set of verification parameters,
also known as verification method, identified by I<name>.
See L</Verification Methods> for details.
...
This usually happens when a root or subordinate CA signs a certificate
for another a CA in other organization.
Another reason is when a CA might have subordinates that use two different
...
certificate extensions, such as extended key usage (EKU),
that determine what certificates can be used for.
...
=head3 Checks Implied by Specific Predefined Purposes
