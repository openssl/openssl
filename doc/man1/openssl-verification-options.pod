...
using a so-called trust anchor (see L</Trust Anchors> below).
...
which corresponds to the EKU B<id-kp-serverAuth>.
Both purposes C<smimesign> and C<smimeencrypt> map to S/MIME email trust,
which corresponds to the EKU B<id-emailProtection>.

While IETF RFC 5280 contains comments that EKUs B<id-kp-serverAuth> and B<id-kp-clientAuth>
...
Verification methods determine a set verification parameters,
such as the verification purpose (if any),
the maximum verification depth (i.e., the number of subordinate CA certificates
allowed in a chain),
the trust model, and any required certificate policies.

By default, no purpose is given, the maximum verification depth is 100,
and certificate policy processing is not enabled.

The trust model determines which auxiliary trust or reject attributes are applicable
to verifying the given certificate chain.
They can be given using the B<-addtrust> and B<-addreject> options
for L<openssl-x509(1)>.

The currently predefined verification methods are C<ssl_client>,
C<ssl_server>, C<smime_sign> with alias C<pkcs7>,  C<smime_encrypt>, C<code_sign>, and C<default>.
These can be given using the B<-verify_name> option of various OpenSSL commands.
They mimic the combinations of purpose and trust attributes used in SSL/(D)TLS,
