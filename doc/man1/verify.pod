...
=item B<-untrusted file|URI>

A B<file> or B<URI> with additional untrusted certificates (intermediate issuer
CAs) used to construct a certificate chain from the subject certificate to a
trust-anchor.
This option can be specified more than once to include untrusted certificates
from multiple sources.

=item B<-trusted file|URI>

A B<file> or B<URI> with trusted certificates, which must be self-signed,
unless the B<-partial_chain> option is specified.
With this option, no additional (e.g., default) certificate lists are
consulted.
That is, the only trust-anchors are those listed in these sources.
