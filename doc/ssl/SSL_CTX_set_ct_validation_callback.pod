...
=head1 SYNOPSIS
 ...
 int SSL_ct_enable(SSL *s, int vtype);
 int SSL_CTX_ct_enable(SSL_CTX *ctx, int vtype);
...
The behaviour of the callback is determined by the B<vtype> argument.

If B<vtype> is equal to B<SSL_CT_VALIDATION_PERMISSIVE>, then the
handshake continues regardless of the validation status of any SCTs.
The application can inspect the validation status of the SCTs at
...
If B<vtype> is equal to B<SSL_CT_VALIDATION_STRICT>, then in a full TLS
handshake with the verification mode set to B<SSL_VERIFY_PEER>, if the peer
...
enabled via a non-null validation callback, and 0 otherwise.
