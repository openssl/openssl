...
EVP_CIPHER_meth_set_ctrl, EVP_CIPHER_meth_get_ctx_size,
...
=head1 SYNOPSIS
 ...
 int EVP_CIPHER_meth_get_impl_ctx_size(EVP_CIPHER *cipher);
...
EVP_CIPHER_meth_get_impl_ctx_size(), EVP_CIPHER_meth_get_init(),
EVP_CIPHER_meth_get_update(), EVP_CIPHER_meth_get_final(),
EVP_CIPHER_meth_get_copy(), EVP_CIPHER_meth_get_cleanup() and
EVP_CIPHER_meth_get_ctrl() are all used to retrieve the method data given with
the EVP_CIPHER_meth_set_*() functions above.
