 ...
 * Added a library context that applications as well as other
   ...
   - Indirectly, by creating a new library context and then assigning
     it as the new default, with `OSSL_CTX_set0_default`.

   All public OpenSSL functions that take an `OSSL_CTX` pointer,
   apart from the functions directly related to `OSSL_CTX`, accept
   ...
   `OSSL_CTX_set0_default` should take care to restore it with a
