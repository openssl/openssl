			       ...
			       void *in2, void **out1, void **out2)
{
	...
	const OSSL_PARAM params[] = {
		OSSL_PARAM_octet_string("context-string",
					(unsigned char *)"A context string",
					16),
		OSSL_PARAM_END
	};
	...
	if ((tbs = OPENSSL_malloc(tbslen)) == NULL || ctx == NULL ||
	    alg == NULL || !RAND_bytes_ex(NULL, tbs, tbslen, 0)) {
				      ...
				      void *in2, void **out1, void **out2)
{
	...
	if ((ctx = EVP_MD_CTX_new()) == NULL ||
		...
		fprintf(stderr,
			"Failed to verify digest with EVP_DigestVerify\n");
		goto err;
	}
...
static struct op_table_entry ops[] = {
	...
	{ "Do a digest sign/verify operation on a key",
	  ...
	  keygen_ml_dsa_real_key, ml_dsa_digest_sign_verify,
	  cleanup_ml_dsa_keys },
