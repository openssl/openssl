                                   ...
                                   int only_valid)
{
    ...
    *len -= sizeof(uint16_t);
    ...
    case 4:
        ...
        *keylen %= ML_DSA_87_PRIV_LEN; /* size to our key buffer */
        break;
    default:
        *keytype = NULL;
        *keylen = 0;
 ...
 */
static void ml_dsa_sign_verify(uint8_t **buf, size_t *len, void *key1, void *in2,
                               void **out1, void **out2)
{
    ...
    if (ctx == NULL || alg == NULL || !RAND_bytes(tbs, TBS_LEN)) {
        fprintf(stderr, "Failed basic initialization\n");
        goto err;
    }

    if ((sig_alg = EVP_SIGNATURE_fetch(NULL, alg, NULL)) == NULL
        || !EVP_PKEY_sign_message_init(ctx, sig_alg, params)
        || !EVP_PKEY_sign(ctx, NULL, &siglen, tbs, TBS_LEN)
        || (sig = OPENSSL_zalloc(siglen)) == NULL
        || !EVP_PKEY_sign(ctx, sig, &siglen, tbs, TBS_LEN)) {
    ...
    if ((ctx = EVP_PKEY_CTX_new_from_pkey(NULL, key, NULL)) == NULL
        || !EVP_PKEY_verify_message_init(NULL, sig_alg, NULL)
        || !EVP_PKEY_verify(NULL, sig, siglen, tbs, TBS_LEN)) {
 ...
 * @param[out] out2  Unused output parameter (reserved for future use).
 */
static void ml_dsa_digest_sign_verify(uint8_t **buf, size_t *len, void *key1, void *in2,
                                      void **out1, void **out2)
{
    EVP_PKEY *key = (EVP_PKEY *)key1;
    ...
    if (ctx == NULL || !RAND_bytes(tbs, TBS_LEN)) {
        fprintf(stderr, "Failed basic initialization\n");
        goto err;
    }

    if (!EVP_DigestSignInit_ex(ctx, NULL, NULL, NULL, "?fips=true", key, params)
        || !EVP_DigestSign(ctx, NULL, &siglen, tbs, TBS_LEN)
        || (sig = OPENSSL_zalloc(siglen)) == NULL
        || !EVP_DigestSign(ctx, sig, &siglen, tbs, TBS_LEN)) {
    ...
    if ((ctx = EVP_MD_CTX_new()) == NULL
        || !EVP_DigestVerifyInit_ex(ctx, NULL, NULL, NULL, "?fips=true", key, params)
        || !EVP_DigestVerify(ctx, sig, siglen, tbs, TBS_LEN)) {
        fprintf(stderr, "Failed to verify digest\n");
        goto err;
    }
