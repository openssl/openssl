...
int FuzzerTestOneInput(const uint8_t *buf, size_t len)
{
	...
	if (SSL_set_alpn_protos(client, (const unsigned char *)"\x08ossltest",
				9) != 0)
		goto end;
	...
	if (!SSL_set_incoming_stream_policy(
		    client, SSL_INCOMING_STREAM_POLICY_ACCEPT, 0))
		goto end;
	...
	for (;;) {
		...
		if (len >= 2) {
			...
			nxtpkt =
				ossl_time_add(fake_now, ossl_ms2time(nxtpktms));
			len -= 2;
			buf += 2;
		}
		...
		for (;;) {
			...
			case ACCEPTING_STREAM:
				...
				if (numstreams == OSSL_NELEM(allstreams) ||
				    SSL_get_accept_stream_queue_len(client) ==
					    0)
					break;
			...
			if (isinf) {
				...
				nxttimeout = ossl_time_add(
					fake_now, ossl_time_from_timeval(tv));
				if (len > 3 && ossl_time_compare(nxttimeout,
								 nxtpkt) >= 0) {
					fake_now = nxtpkt;
					break;
				}
