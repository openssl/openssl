...
#define DEFINE_ALGORITHMS(name, evp)                                       \
	...
	{                                                                  \
		return strcmp(                                             \
			OSSL_PROVIDER_get0_name(evp##_get0_provider(*a)),  \
	...
	{                                                                  \
		...
		evp##_do_all_provided(libctx, collect_##evp,               \
				      name##_collection);                  \
	}                                                                  \
	static void cleanup_##name(void)                                   \
	{                                                                  \
			       ...
			       size_t *len)
{
	...
	for (; param != NULL && param->key != NULL; param++) {
		...
		case OSSL_PARAM_INTEGER:
			if (strcmp(param->key, OSSL_KDF_PARAM_ITER) == 0) {
				...
				*p_value_int = ITERS;
					  ...
					  OSSL_KDF_PARAM_SCRYPT_N) == 0) {
				p_value_int = OPENSSL_malloc(sizeof(ITERS));
				*p_value_int = ITERS;
			} else if (strcmp(param->key,
					  OSSL_KDF_PARAM_SCRYPT_R) == 0) {
				p_value_int = OPENSSL_malloc(sizeof(BLOCKSIZE));
				*p_value_int = BLOCKSIZE;
			} else if (strcmp(param->key,
					  OSSL_KDF_PARAM_SCRYPT_P) == 0) {
				p_value_int = OPENSSL_malloc(sizeof(BLOCKSIZE));
				*p_value_int = BLOCKSIZE;
			} else if (!*use_param ||
				   !read_int(buf, len, &p_value_int)) {
		...
		case OSSL_PARAM_UNSIGNED_INTEGER:
			if (strcmp(param->key, OSSL_KDF_PARAM_ITER) == 0) {
				...
				*p_value_uint = UITERS;
					  ...
					  OSSL_KDF_PARAM_SCRYPT_R) == 0) {
				p_value_uint =
					OPENSSL_malloc(sizeof(UBLOCKSIZE));
				...
				*p_value_uint = UBLOCKSIZE;
				   ...
				   !read_uint(buf, len, &p_value_uint)) {
		...
		case OSSL_PARAM_REAL:
			if (!*use_param ||
			    !read_double(buf, len, &p_value_double)) {
		...
		case OSSL_PARAM_UTF8_STRING:
			if (*use_param &&
			    (data_len = read_utf8_string(
				     buf, len, &p_value_utf8_str)) < 0)
				data_len = 0;
			*p = *param;
		...
		case OSSL_PARAM_UTF8_PTR:
			if (*use_param &&
			    (data_len = read_utf8_ptr(buf, len,
						      &p_value_utf8_ptr)) < 0)
				data_len = 0;
			*p = *param;
		...
		case OSSL_PARAM_OCTET_PTR:
			if (*use_param &&
			    (data_len = read_octet_ptr(buf, len,
						       &p_value_octet_ptr)) < 0)
				data_len = 0;
			*p = *param;
...
#define EVP_FUZZ(source, evp, f)                                            \
	do {                                                                \
		evp *alg = sk_##evp##_value(                                \
			source, *algorithm % sk_##evp##_num(source));       \
		OSSL_PARAM *fuzzed_params;                                  \
                                                                            \
		if (alg == NULL)                                            \
			break;                                              \
		fuzzed_params = fuzz_params(                                \
			(OSSL_PARAM *)evp##_settable_ctx_params(alg), &buf, \
			&len);                                              \
...
int FuzzerTestOneInput(const uint8_t *buf, size_t len)
{
	...
	case 5:
		...
		break;
	case 6:
		EVP_FUZZ(asym_ciphers_collection, EVP_ASYM_CIPHER,
			 do_evp_asym_cipher);
		break;
	case 7:
