...
int FuzzerTestOneInput(const uint8_t *buf, size_t len)
{
	...
	case OP_INSERT:
         ...
         */
		if (IS_REPLACE(op_flags))
			rc = ossl_ht_fz_FUZZER_VALUE_insert(
				fuzzer_table, TO_HT_KEY(&key), valptr, &lval);
		else
			rc = ossl_ht_fz_FUZZER_VALUE_insert(
				fuzzer_table, TO_HT_KEY(&key), valptr, NULL);
	...
	case OP_LOOKUP:
         ...
         * do the lookup
         */
		lval = ossl_ht_fz_FUZZER_VALUE_get(fuzzer_table,
						   TO_HT_KEY(&key), &v);
	...
	case OP_FLUSH:
		...
		for (i = 0; i < USHRT_MAX; i++)
			OPENSSL_assert((prediction_table[i].flags &
					FZ_FLAG_ALLOCATED) == 0);
	...
	case OP_FILTER:
		...
		htvlist = ossl_ht_filter(fuzzer_table, 1, filter_iterator,
					 &keyval);
