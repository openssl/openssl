...
For functions, OpenSSL names follow a `PREFIX_object_action()`
shape: an uppercase subsystem prefix, followed by an object or
context, followed by an action in lowercase with underscores.
Examples: `X509_get_subject_name`, `EVP_PKEY_sign`,
`OSSL_CMP_validate_msg`, `SSL_CTX_set_verify`.
...
Avoid bitfields in structs: their layout is implementation-defined
and varies across compilers and ABIs. Use explicit shifts and
masks on a regular integer where bit-level positioning matters.
