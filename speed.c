...
skip_hmac:
    ...
    for (testnum = 0; testnum < RSA_NUM; testnum++) {
        EVP_PKEY **rsa_pkeys = malloc(loopargs_len * sizeof(intptr_t)); 
        int st = 0;
        ...
        if (primes > RSA_DEFAULT_PRIME_NUM) {
            ...
            for (i = 0; i < loopargs_len; ++i)
            {
                ...
                st = bn != NULL
                    ...
                    && EVP_PKEY_keygen(genctx, &rsa_pkeys[i]);
                BN_free(bn);
                bn = NULL;
            ...
            for (i = 0; i < loopargs_len; ++i)
            {
                ...
                st = (rsa_pkeys[i] = d2i_PrivateKey(EVP_PKEY_RSA, NULL, &p,
                                           rsa_keys[testnum].length)) != NULL;
            }
        }

        for (i = 0; st && i < loopargs_len; i++) {
            loopargs[i].rsa_sign_ctx[testnum] = EVP_PKEY_CTX_new(rsa_pkeys[i], NULL);
        ...
        for (i = 0; st && i < loopargs_len; i++) {
            loopargs[i].rsa_verify_ctx[testnum] = EVP_PKEY_CTX_new(rsa_pkeys[i],
                                                                   NULL);
