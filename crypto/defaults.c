...
#define REGISTRY_KEY                                    \
	...
	"-" MAKESTR(OPENSSL_VERSION_MAJOR) "." MAKESTR( \
		OPENSSL_VERSION_MINOR) "-" MAKESTR(OSSL_WINCTX)
#endif
				 ...
				 LPCWSTR valuename)
{
	...
	ret = RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT(REGISTRY_KEY),
