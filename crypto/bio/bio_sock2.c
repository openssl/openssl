...
int BIO_connect(int sock, const BIO_ADDR *addr, int options)
{
	...
	if (options & BIO_SOCK_TFO) {
		...
		/* Later FreeBSD */
		if (sysctlbyname(OSSL_TFO_SYSCTL_CLIENT, &enabled, &enabledlen,
				 NULL, 0) < 0) {
		...
		/* macOS */
		if (sysctlbyname(OSSL_TFO_SYSCTL, &enabled, &enabledlen, NULL,
				 0) < 0) {
			ERR_raise(ERR_LIB_BIO, BIO_R_TFO_NO_KERNEL_SUPPORT);
			return 0;
		}
...
int BIO_bind(int sock, const BIO_ADDR *addr, int options)
{
	...
	if (bind(sock, BIO_ADDR_sockaddr(addr), BIO_ADDR_sockaddr_size(addr)) !=
	    0) {
		ERR_raise_data(ERR_LIB_SYS,
...
int BIO_listen(int sock, const BIO_ADDR *addr, int options)
{
	...
	if ((options & BIO_SOCK_TFO) && socktype != SOCK_DGRAM) {
		...
		/* Later FreeBSD */
		if (sysctlbyname(OSSL_TFO_SYSCTL_SERVER, &enabled, &enabledlen,
				 NULL, 0) < 0) {
			ERR_raise(ERR_LIB_BIO, BIO_R_TFO_NO_KERNEL_SUPPORT);
			return 0;
		}
...
int BIO_accept_ex(int accept_sock, BIO_ADDR *addr_, int options)
{
	...
	len = sizeof(*addr);
	accepted_sock =
		accept(accept_sock, BIO_ADDR_sockaddr_noconst(addr), &len);
	if (accepted_sock == -1) {
	...
	if (!BIO_socket_nbio(accepted_sock,
			     (options & BIO_SOCK_NONBLOCK) != 0)) {
