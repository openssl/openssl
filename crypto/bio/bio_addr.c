		     ...
		     size_t wherelen, unsigned short port)
{
	...
	if (family == AF_UNIX) {
		...
		strncpy(ap->s_un.sun_path, where,
			sizeof(ap->s_un.sun_path) - 1);
		return 1;
	}
#endif
	if (family == AF_INET) {
			...
			char **service)
{
	...
	if (1) {
		...
		if ((ret = getnameinfo(BIO_ADDR_sockaddr(ap),
				       BIO_ADDR_sockaddr_size(ap), host,
		...
		if (hostname != NULL)
			*hostname =
				OPENSSL_strdup(inet_ntoa(ap->s_in.sin_addr));
		if (service != NULL) {
			char serv[6]; /* port is 16 bits => max 5 decimal digits */
			BIO_snprintf(serv, sizeof(serv), "%d",
				     ntohs(ap->s_in.sin_port));
			*service = OPENSSL_strdup(serv);
		}
	}
...
int BIO_lookup(const char *host, const char *service,
	       ...
	       BIO_ADDRINFO **res)
{
	return BIO_lookup_ex(host, service, lookup_type, family, socktype, 0,
			     res);
}
...
#pragma pointer_size restore
		...
		if (host == NULL) {
			...
			default:
				...
				assert("We forgot to handle a lookup type!" ==
				       NULL);
