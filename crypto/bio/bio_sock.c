...
int BIO_get_host_ip(const char *str, unsigned char *ip)
{
	...
	if (BIO_lookup(str, NULL, BIO_LOOKUP_CLIENT, AF_INET, SOCK_STREAM,
		       &res)) {
		size_t l;
		...
		if (BIO_ADDRINFO_family(res) != AF_INET) {
		...
		} else if (BIO_ADDR_rawaddress(BIO_ADDRINFO_address(res), NULL,
					       &l)) {
			/*
             ...
             */
			if (ossl_assert(l == 4))
				ret = BIO_ADDR_rawaddress(
					BIO_ADDRINFO_address(res), ip, &l);
		}
...
int BIO_get_port(const char *str, unsigned short *port_ptr)
{
	...
	if (BIO_lookup(NULL, str, BIO_LOOKUP_CLIENT, AF_INET, SOCK_STREAM,
		       &res)) {
		if (BIO_ADDRINFO_family(res) != AF_INET) {
			ERR_raise(ERR_LIB_BIO,
			...
			*port_ptr = ntohs(
				BIO_ADDR_rawport(BIO_ADDRINFO_address(res)));
			ret = 1;
		}
		BIO_ADDRINFO_free(res);
	} else {
...
#ifdef BIO_HAVE_WSAMSG
		{
			...
			if (s != INVALID_SOCKET) {
				if (WSAIoctl(s,
					     ...
					     &id_WSARecvMsg,
					     sizeof(id_WSARecvMsg),
					     &bio_WSARecvMsg,
					     sizeof(bio_WSARecvMsg), &len_out,
					     NULL, NULL) != 0 ||
				...
				if (WSAIoctl(s,
					     SIO_GET_EXTENSION_FUNCTION_POINTER,
					     &id_WSASendMsg,
					     sizeof(id_WSASendMsg),
					     &bio_WSASendMsg,
					     sizeof(bio_WSASendMsg), &len_out,
					     NULL, NULL) != 0 ||
...
int BIO_get_accept_socket(char *host, int bind_mode)
{
	...
	if (BIO_lookup(h, p, BIO_LOOKUP_SERVER, AF_UNSPEC, SOCK_STREAM, &res) !=
	    0)
		goto err;

	if ((s = BIO_socket(BIO_ADDRINFO_family(res),
...
int BIO_accept(int sock, char **ip_port)
{
	...
	if (ip_port != NULL) {
		...
		if (host != NULL && port != NULL) {
			*ip_port =
				OPENSSL_zalloc(strlen(host) + strlen(port) + 2);
		} else {
			*ip_port = NULL;
