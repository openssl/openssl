...
static int _dopr(char **sbuffer, char **buffer, size_t *maxlen, size_t *retlen,
	...
	while (state != DP_S_DONE) {
		...
		case DP_S_CONV:
			...
			case 'f':
				...
				if (!fmtfp(&desc, fvalue, min, max, flags,
					   F_FORMAT))
					goto out;
				break;
			case 'E':
			...
			case 'p':
				...
				if (!fmtint(&desc, value, 16, min, max,
					    flags | DP_F_NUM))
					goto out;
				break;
			case 'n':
...
static int fmtstr(struct pr_desc *desc, const char *value, int flags, int min,
		  int max)
{
	int padlen;
	size_t strln;
...
static int fmtint(struct pr_desc *desc, int64_t value, int base, int min,
		  int max, int flags)
{
	...
	if (zpadlen < 0)
		zpadlen = 0;
	spadlen = min - OSSL_MAX(max, place + zpadlen + (signvalue ? 1 : 0) +
					      (int)strlen(prefix));
	if (spadlen < 0)
		spadlen = 0;
	if (flags & DP_F_MINUS) {
...
static int fmtfp(struct pr_desc *desc, LDOUBLE fvalue, int min, int max,
		 int flags, int style)
{
	int signvalue = 0;
	LDOUBLE ufvalue;
...
static int doapr_outch(struct pr_desc *desc, int c)
{
	...
	if (desc->buffer != NULL && desc->currlen == desc->maxlen) {
		...
		if (*(desc->buffer) == NULL) {
			if ((*(desc->buffer) = OPENSSL_malloc(desc->maxlen)) ==
			    NULL)
				return 0;
			if (desc->currlen > 0) {
				if (!ossl_assert(desc->sbuffer != NULL))
					return 0;
				memcpy(*(desc->buffer), desc->sbuffer,
				       desc->currlen);
			}
			desc->sbuffer = NULL;
		} else {
