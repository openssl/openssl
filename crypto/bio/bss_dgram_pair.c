...
static int ring_buf_resize(struct ring_buf *r, size_t nbytes)
{
	...
	if (r->count > 0) {
		if (r->idx[0] <= r->idx[1]) {
			...
			memmove(new_start + r->idx[1] + offset,
				new_start + r->idx[1], r->len - r->idx[1]);
			r->idx[1] += offset;
		}
	} else {
...
static long dgram_mem_ctrl(BIO *bio, int cmd, long num, void *ptr)
{
	...
	case BIO_C_SET_WRITE_BUF_SIZE: /* Non-threadsafe */
		ret = (long)dgram_pair_ctrl_set_write_buf_size(bio,
							       (size_t)num);
		break;
	...
	case BIO_CTRL_DGRAM_SET0_LOCAL_ADDR:
		ret = (long)dgram_pair_ctrl_set0_local_addr(bio,
							    (BIO_ADDR *)ptr);
		break;
...
int BIO_new_bio_dgram_pair(BIO **pbio1, size_t writebuf1, BIO **pbio2,
			   size_t writebuf2)
{
	int ret = 0;
	long r;
...
/* Threadsafe */
static int dgram_pair_recvmmsg(BIO *bio, BIO_MSG *msg, size_t stride,
			       size_t num_msg, uint64_t flags,
			       size_t *num_processed)
{
	int ret;
	...
	for (i = 0; i < num_msg; ++i) {
		m = &BIO_MSG_N(msg, i);
		l = dgram_pair_read_actual(bio, m->data, m->data_len, m->local,
					   m->peer, 1);
		if (l < 0) {
			*num_processed = i;
			if (i > 0) {
	...
	while (sz > 0) {
		...
		if (dst_len == 0) {
			...
			if (!b->grows_on_write) /* resize only if size not set explicitly */
				break;
			/* increase the size */
			new_len = compute_rbuf_growth(b->req_buf_len + sz,
						      b->req_buf_len);
...
static ossl_ssize_t dgram_pair_write_actual(BIO *bio, const char *buf,
					    size_t sz, const BIO_ADDR *local,
					    const BIO_ADDR *peer, int is_multi)
{
	...
	if (dgram_pair_write_inner(b, (const uint8_t *)&hdr, sizeof(hdr)) !=
		    sizeof(hdr) ||
...
/* Threadsafe */
static int dgram_pair_sendmmsg(BIO *bio, BIO_MSG *msg, size_t stride,
			       size_t num_msg, uint64_t flags,
			       size_t *num_processed)
{
	ossl_ssize_t l;
	...
	for (i = 0; i < num_msg; ++i) {
		m = &BIO_MSG_N(msg, i);
		l = dgram_pair_write_actual(bio, m->data, m->data_len, m->local,
					    m->peer, 1);
		if (l < 0) {
			*num_processed = i;
			if (i > 0) {
