...
int BIO_free(BIO *a)
{
	...
	if (HAS_CALLBACK(a)) {
		ret = (int)bio_call_callback(a, BIO_CB_FREE, NULL, 0, 0, 0L, 1L,
					     NULL);
		if (ret <= 0)
			return 0;
	}
...
static int bio_read_intern(BIO *b, void *data, size_t dlen, size_t *readbytes)
{
	...
	if (HAS_CALLBACK(b) &&
	    ((ret = (int)bio_call_callback(b, BIO_CB_READ, data, dlen, 0, 0L,
					   1L, NULL)) <= 0))
		return ret;
		 ...
		 uint64_t flags, size_t *msgs_processed)
{
	...
	if (HAS_CALLBACK(b)) {
		...
		ret = bio_call_callback(b, BIO_CB_RECVMMSG, (void *)&args, 0, 0,
					0, 1, NULL);
		if (ret <= 0)
			return 0;
	}
...
int BIO_puts(BIO *b, const char *buf)
{
	...
	if (HAS_CALLBACK(b)) {
		ret = (int)bio_call_callback(b, BIO_CB_PUTS, buf, 0, 0, 0L, 1L,
					     NULL);
		if (ret <= 0)
			return ret;
	}
...
int BIO_gets(BIO *b, char *buf, int size)
{
	...
	if (HAS_CALLBACK(b)) {
		ret = (int)bio_call_callback(b, BIO_CB_GETS, buf, size, 0, 0L,
					     1, NULL);
		if (ret <= 0)
			return ret;
	}
	...
	if (HAS_CALLBACK(b))
		ret = (int)bio_call_callback(b, BIO_CB_GETS | BIO_CB_RETURN,
					     buf, size, 0, 0L, ret, &readbytes);
...
long BIO_ctrl(BIO *b, int cmd, long larg, void *parg)
{
	...
	if (HAS_CALLBACK(b)) {
		ret = bio_call_callback(b, BIO_CB_CTRL, parg, 0, cmd, larg, 1L,
					NULL);
		if (ret <= 0)
			return ret;
	}
...
long BIO_callback_ctrl(BIO *b, int cmd, BIO_info_cb *fp)
{
	...
	if (HAS_CALLBACK(b)) {
		ret = bio_call_callback(b, BIO_CB_CTRL, (void *)&fp, 0, cmd, 0,
					1L, NULL);
		if (ret <= 0)
			return ret;
	}
...
int BIO_wait(BIO *bio, time_t max_time, unsigned int nap_milliseconds)
{
	...
	if (rv <= 0)
		ERR_raise(ERR_LIB_BIO, rv == 0 ? BIO_R_TRANSFER_TIMEOUT :
...
retry:
	...
	if (rv <= 0) { /* could be timeout or retryable error or fatal error */
		...
		int do_retry =
			BIO_should_retry(bio); /* may be 1 only if !blocking */
		...
		if (ERR_GET_LIB(err) == ERR_LIB_BIO) {
			...
			case BIO_R_NBIO_CONNECT_ERROR:
				...
				(void)BIO_reset(
					bio); /* often needed to avoid retry failure */
				do_retry = 1;
				break;
			default:
				break;
		...
		if (timeout >= 0 && do_retry) {
			...
			if (rv > 0)
				goto retry;
			ERR_raise(ERR_LIB_BIO, rv == 0 ? BIO_R_CONNECT_TIMEOUT :
							 BIO_R_CONNECT_ERROR);
		} else {
			ERR_clear_last_mark();
			rv = -1;
