...
static void dgram_adjust_rcv_timeout(BIO *b)
{
	...
	if (!ossl_time_is_zero(data->next_timeout)) {
		...
		timeleft =
			ossl_time_subtract(data->next_timeout, ossl_time_now());
		if (ossl_time_compare(timeleft, ossl_ticks2time(OSSL_TIME_US)) <
		    0)
...
static long dgram_ctrl(BIO *b, int cmd, long num, void *ptr)
{
	...
	case BIO_C_SET_FD:
		...
		if (getpeername(b->num, (struct sockaddr *)&ss, &ss_len) == 0) {
			BIO_ADDR_make(&data->peer,
				      BIO_ADDR_sockaddr((BIO_ADDR *)&ss));
			data->connected = 1;
		}
#if defined(SUPPORT_LOCAL_ADDR)
	...
	case BIO_CTRL_DGRAM_SET_CONNECTED:
		if (ptr != NULL) {
			data->connected = 1;
			BIO_ADDR_make(&data->peer,
				      BIO_ADDR_sockaddr((BIO_ADDR *)ptr));
		} else {
			data->connected = 0;
	...
	case BIO_CTRL_DGRAM_DETECT_PEER_ADDR: {
		...
		if (BIO_ADDR_family(p) == AF_UNSPEC) {
			if (getpeername(b->num, (void *)&xaddr.sa,
					&xaddr_len) == 0 &&
	...
	case BIO_CTRL_DGRAM_SET_NEXT_TIMEOUT:
		data->next_timeout =
			ossl_time_from_timeval(*(struct timeval *)ptr);
		break;
#if defined(SO_RCVTIMEO)
	...
	case BIO_CTRL_DGRAM_GET_RECV_TIMEOUT: {
		...
		if ((ret = getsockopt(b->num, SOL_SOCKET, SO_RCVTIMEO, ptr,
				      &sz)) < 0) {
			ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(),
				       "calling getsockopt()");
		} else {
	...
	case BIO_CTRL_DGRAM_GET_SEND_TIMEOUT: {
		...
		if ((ret = getsockopt(b->num, SOL_SOCKET, SO_SNDTIMEO, ptr,
				      &sz)) < 0) {
			ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(),
				       "calling getsockopt()");
		} else {
	...
	defined(IP_PMTUDISC_PROBE)
			sockopt_val = num ? IP_PMTUDISC_PROBE :
					    IP_PMTUDISC_DONT;
			if ((ret = setsockopt(b->num, IPPROTO_IP,
					      IP_MTU_DISCOVER, &sockopt_val,
					      sizeof(sockopt_val))) < 0)
...
#if OPENSSL_USE_IPV6
		else if (msg->peer != NULL &&
			 dgram_get_sock_family(b) == AF_INET6)
...
static int extract_local(BIO *b, MSGHDR_TYPE *mh, BIO_ADDR *local)
{
	...
	for (cmsg = BIO_CMSG_FIRSTHDR(mh); cmsg != NULL;
		...
		if (af == AF_INET) {
			...
			local->s_in.sin_addr =
				((struct in_pktinfo *)BIO_CMSG_DATA(cmsg))
					->ipi_addr;
			...
			local->s_in.sin_addr =
				*(struct in_addr *)BIO_CMSG_DATA(cmsg);
#endif
...

			{
				...
				local->s_in6.sin6_addr =
					((struct in6_pktinfo *)BIO_CMSG_DATA(
						 cmsg))
						->ipi6_addr;
				local->s_in6.sin6_family = AF_INET6;
				local->s_in6.sin6_port =
					data->local_addr.s_in6.sin6_port;
			  ...
			  uint64_t flags, size_t *num_processed)
{
	...
	*num_processed = 1;
	...
	if (bio_WSARecvMsg != NULL) {
		...
		ret = WSARecvMsg((SOCKET)b->num, &wmsg, &num_bytes_received,
				 NULL, NULL);
		if (ret < 0) {
			ERR_raise(ERR_LIB_SYS, get_last_socket_error());
			*num_processed = 0;
...
static int dgram_sctp_read(BIO *b, char *out, int outl)
{
	...
	if (out != NULL) {
		...
		do {
				...
				for (cmsg = CMSG_FIRSTHDR(&msg); cmsg;
					...
					if (cmsg->cmsg_type == SCTP_RCVINFO) {
						...
						rcvinfo =
							(struct sctp_rcvinfo *)
								CMSG_DATA(cmsg);
						data->rcvinfo.rcv_sid =
							rcvinfo->rcv_sid;
						data->rcvinfo.rcv_ssn =
							rcvinfo->rcv_ssn;
						data->rcvinfo.rcv_flags =
							rcvinfo->rcv_flags;
						data->rcvinfo.rcv_ppid =
							rcvinfo->rcv_ppid;
						data->rcvinfo.rcv_tsn =
							rcvinfo->rcv_tsn;
						data->rcvinfo.rcv_cumtsn =
							rcvinfo->rcv_cumtsn;
						data->rcvinfo.rcv_context =
							rcvinfo->rcv_context;
					}
#endif
#ifdef SCTP_SNDRCV
			...
			if (msg.msg_flags & MSG_NOTIFICATION) {
				...
				if (snp.sn_header.sn_type ==
					...
					i = getsockopt(b->num, IPPROTO_SCTP,
						       SCTP_EVENTS, &event,
						       &eventsize);
					if (i < 0) {
						ret = i;
		...
		if (ret > 0 && !(msg.msg_flags & MSG_EOR)) {
			...
			ret = getsockopt(b->num, SOL_SOCKET, SO_RCVBUF, &optval,
					 &optlen);
			if (ret >= 0)
			...
			ret = getsockopt(b->num, IPPROTO_SCTP,
					 SCTP_PARTIAL_DELIVERY_POINT, &optval,
					 &optlen);
			if (ret >= 0)
				OPENSSL_assert(optval >= 18445);
		...
		if (!data->peer_auth_tested) {
			...
			optlen = (socklen_t)(sizeof(sctp_assoc_t) +
					     256 * sizeof(uint8_t));
			authchunks = OPENSSL_malloc(optlen);
			if (authchunks == NULL)
				return -1;
			memset(authchunks, 0, optlen);
			ii = getsockopt(b->num, IPPROTO_SCTP,
					SCTP_PEER_AUTH_CHUNKS, authchunks,
					&optlen);

			if (ii >= 0)
				for (p = (unsigned char *)
						 authchunks->gauth_chunks;
...
static int dgram_sctp_write(BIO *b, const char *in, int inl)
{
	...
	cmsg = (struct cmsghdr
			*)&cmsgbuf[CMSG_SPACE(sizeof(struct sctp_sndinfo))];
	cmsg->cmsg_level = IPPROTO_SCTP;
...
static long dgram_sctp_ctrl(BIO *b, int cmd, long num, void *ptr)
{
	...
	case BIO_CTRL_DGRAM_SCTP_ADD_AUTH_KEY:
		...
		/* Add new key */
		sockopt_len =
			sizeof(struct sctp_authkey) + 64 * sizeof(uint8_t);
	...
	case BIO_CTRL_DGRAM_SCTP_AUTH_CCS_RCVD:
		...
		if (data->ccs_rcvd == 1 && data->ccs_sent == 1) {
			...
			sockopt_len = sizeof(struct sctp_authkeyid);
			ret = getsockopt(b->num, IPPROTO_SCTP,
					 SCTP_AUTH_ACTIVE_KEY, &authkeyid,
					 &sockopt_len);
			if (ret < 0)
				break;
			...
			ret = setsockopt(b->num, IPPROTO_SCTP,
					 SCTP_AUTH_DEACTIVATE_KEY, &authkeyid,
					 sockopt_len);
			if (ret < 0)
				break;
#endif
...
static int dgram_sctp_wait_for_dry(BIO *b)
{
	...
	while (msg.msg_flags & MSG_NOTIFICATION) {
		...
		if (snp.sn_header.sn_type == SCTP_SENDER_DRY_EVENT) {
			...
			event.se_type = SCTP_SENDER_DRY_EVENT;
			event.se_on = 0;
			ret = setsockopt(b->num, IPPROTO_SCTP, SCTP_EVENT,
					 &event, sizeof(struct sctp_event));
#else
			eventsize =
				(socklen_t)sizeof(struct sctp_event_subscribe);
			ret = getsockopt(b->num, IPPROTO_SCTP, SCTP_EVENTS,
					 &event, &eventsize);
			if (ret < 0)
				return -1;
			...
			ret = setsockopt(b->num, IPPROTO_SCTP, SCTP_EVENTS,
					 &event,
					 sizeof(struct sctp_event_subscribe));
#endif
			if (ret < 0)
		...
		if (data->handle_notifications != NULL)
			data->handle_notifications(
				b, data->notification_context, (void *)&snp);
