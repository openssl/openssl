...
static const BIO_METHOD
	methods_acceptp = {
		...
		bwrite_conv,	 acpt_write,
		bread_conv,	 acpt_read,
		acpt_puts,	 NULL, /* connect_gets,         */
		acpt_ctrl,	 acpt_new,
		acpt_free,	 NULL, /* connect_callback_ctrl */
	};
...
static int acpt_state(BIO *b, BIO_ACCEPT *c)
{
	...
	for (;;) {
		...
		case ACPT_S_GET_ADDR: {
			...
			if (BIO_lookup(c->param_addr, c->param_serv,
				       BIO_LOOKUP_SERVER, family, SOCK_STREAM,
				       &c->addr_first) == 0)
				goto exit_loop;
		}
		...
		case ACPT_S_LISTEN: {
...

			{
				...
				if (!BIO_sock_info(c->accept_sock,
			...
			c->cache_accepting_name = BIO_ADDR_hostname_string(
				&c->cache_accepting_addr, 1);
			c->cache_accepting_serv = BIO_ADDR_service_string(
				&c->cache_accepting_addr, 1);
		...
		case ACPT_S_ACCEPT:
             ...
             */
			if (c->bio_chain != NULL) {
				if ((dbio = BIO_dup_chain(c->bio_chain)) ==
				    NULL)
					goto exit_loop;
			...
			c->cache_peer_name = BIO_ADDR_hostname_string(
				&c->cache_peer_addr, 1);
			c->cache_peer_serv =
	...
	case BIO_C_SET_ACCEPT:
		if (ptr != NULL) {
			if (num == 0) {
				...
				ret = BIO_parse_hostserv(ptr, &data->param_addr,
							 &data->param_serv,
							 BIO_PARSE_PRIO_SERV);
				...
				if ((data->param_serv = OPENSSL_strdup(ptr)) ==
				    NULL)
					ret = 0;
				else
					b->init = 1;
