...
static int conn_write(BIO *b, const char *in, int inl)
{
	...
	if (BIO_should_ktls_ctrl_msg_flag(b)) {
		ret = ktls_send_ctrl_message(b->num, data->record_type, in,
					     inl);
		if (ret >= 0) {
			ret = inl;
