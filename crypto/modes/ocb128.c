			   ...
			   void *keyenc, void *keydec)
{
	...
	if (keydec)
		dest->keydec = keydec;
	if (src->l) {
		if ((dest->l = OPENSSL_malloc_array(src->max_l_index, 16)) ==
		    NULL)
			return 0;
			...
			size_t len, size_t taglen)
{
	...
	mask = 0xff;
	mask <<= 8 - shift;
	ctx->sess.offset.c[15] |= (*(stretch + (bottom / 8) + 16) & mask) >>
				  (8 - shift);
...
int CRYPTO_ocb128_aad(OCB128_CONTEXT *ctx, const unsigned char *aad, size_t len)
{
	...
	for (i = ctx->sess.blocks_hashed + 1; i <= all_num_blocks; i++) {
		...
		if (lookup == NULL)
			return 0;
		ocb_block16_xor(&ctx->sess.offset_aad, lookup,
				&ctx->sess.offset_aad);
			  ...
			  unsigned char *out, size_t len)
{
	...
	if (num_blocks && all_num_blocks == (size_t)all_num_blocks &&
		...
		for (i = ctx->sess.blocks_processed + 1; i <= all_num_blocks;
		     i++) {
			OCB_BLOCK *lookup;
			OCB_BLOCK tmp;
			...
			if (lookup == NULL)
				return 0;
			ocb_block16_xor(&ctx->sess.offset, lookup,
					&ctx->sess.offset);
			...
			ocb_block16_xor(&tmp, &ctx->sess.checksum,
					&ctx->sess.checksum);
	...
	if (last_len > 0) {
		...
		/* Offset_* = Offset_m xor L_* */
		ocb_block16_xor(&ctx->sess.offset, &ctx->l_star,
				&ctx->sess.offset);
