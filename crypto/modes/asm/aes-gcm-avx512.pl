...
#      leave H^9..H^16 zeroed (all-zero = "not yet computed" sentinel).
#   2. On the first wide-loop use, precompute_hkeys_on_stack() fills the stack:
#      a. Probe H^9..H^12 in the context (vptestmd). If still zero, lazily
#         compute H^9..H^16 = H^1..H^8 * H^8 and cache them back to the context
#         (so later calls skip this).                                ["first16"]
#      b. Copy H^1..H^16 from the context onto the stack.            ["first16"]
#      c. Extend on the stack in two 8-wide passes:          ["mid16"/"first32"]
#            H^17..H^24 = H^9..H^16  * H^8   (pass 1)
#            H^25..H^32 = H^17..H^24 * H^8   (pass 2, reuses pass 1's output)
#      A caller that GHASHes straight from the context rather than the stack --
#      the sub-16-block AAD/IV tail -- needs (a) alone, so it calls
#      ensure_ctx_hkeys_9_16() and leaves the frame and HKEYS_READY untouched.
#   3. The context only ever holds H^1..H^16; H^17..H^32 live solely on the
#      stack frame and are recomputed per call (never written back).
#
...
# ;; =============================================================================
# ;; probe_ctx_hkeys_9_16: jump to $READY_LABEL when the context already holds
# ;; H^9..H^16, otherwise fall through to compute them.
# ;;
# ;; H^9..H^16 are computed and cached together, never partially, and a valid
# ;; hash-key power is never all-zero, so all-zero H^9..H^12 is the sentinel for
# ;; "none of H^9..H^16 is computed yet" (ossl_aes_gcm_init_avx512 leaves them
# ;; zeroed). H^9..H^12 is left in $ZDST so the ready path need not reload it.
# ;;
# ;; (A readiness flag in the context would be cleaner than the zero sentinel,
# ;; but the layout has no spare field for one; what keeps the probe correct is
# ;; the all-or-nothing rule above.)
# ;;
# ;; Clobbers: $ZDST, the mask register %k1 and EFLAGS.
# ;; =============================================================================
sub probe_ctx_hkeys_9_16 {
  my $GCM128_CTX  = $_[0];
  my $ZDST        = $_[1];    # ; [out] H^9..H^12, or zero when uncomputed
  my $READY_LABEL = $_[2];

  $code .= <<___;
        vmovdqu64         @{[HashKeyByIdx(12,$GCM128_CTX)]},$ZDST
        vptestmd          $ZDST,$ZDST,%k1
        kortestw          %k1,%k1
        jnz               $READY_LABEL
___
}
...
# ;; =============================================================================
# ;; compute_ctx_hkeys_9_16: H^9..H^16 = H^1..H^8 * H^8, cached in the context so
# ;; that later calls on the same key take probe_ctx_hkeys_9_16's ready path.
# ;;
# ;; Inputs:  $ZH1_4 - H^1..H^4, $ZH5_8 - H^5..H^8.
# ;; Outputs: $Z9_12 and $Z13_16 hold the new powers, and so does the context.
# ;;          $ZH5_8 is left holding H^8 broadcast to all four lanes, which the
# ;;          callers that go on to extend the range beyond H^16 reuse.
# ;; Clobbers: $ZH1_4 and the three temporaries. $ZT2 may alias $ZH1_4, which is
# ;;           dead from the first multiply onwards.
# ;; =============================================================================
sub compute_ctx_hkeys_9_16 {
  my $GCM128_CTX = $_[0];
  my $ZH1_4      = $_[1];
  my $ZH5_8      = $_[2];
  my $Z9_12      = $_[3];     # ; [out] H^9..H^12
  my $Z13_16     = $_[4];     # ; [out] H^13..H^16
  my $ZT0        = $_[5];
  my $ZT1        = $_[6];
  my $ZT2        = $_[7];

  $code .= <<___;
        # ; keep H^5..H^8 before $ZH5_8 becomes the H^8 broadcast
        vmovdqa64         $ZH5_8,$Z13_16
        vshufi64x2        \$0x00,$ZH5_8,$ZH5_8,$ZH5_8
        # ; H^9..H^12 = H^1..H^4 * H^8
        vmovdqa64         $ZH1_4,$Z9_12
___

  &GHASH_MUL($Z9_12, $ZH5_8, $ZT0, $ZT1, $ZT2);

  # ; H^13..H^16 = H^5..H^8 * H^8
  &GHASH_MUL($Z13_16, $ZH5_8, $ZT0, $ZT1, $ZT2);

  $code .= <<___;
        vmovdqu64         $Z9_12,@{[HashKeyByIdx(12,$GCM128_CTX)]}
        vmovdqu64         $Z13_16,@{[HashKeyByIdx(16,$GCM128_CTX)]}
___
}

# ;; =============================================================================
# ;; precompute_hkeys_on_stack: materialise hash-key powers on the stack frame's
# ;; HKEYS storage for the wide loops, and lazily complete H^9..H^16 in the
# ;; context on first use.
# ;;
# ;; Contract:
# ;;   Precondition (context): H^1..H^8 are valid. H^9..H^16 are either ALL
# ;;     uncomputed or ALL valid, as probe_ctx_hkeys_9_16 requires.
# ;;   Inputs:
# ;;     HKEYS_READY [GP] - if non-zero the whole routine is skipped (the stack
# ;;                        keys were already materialised earlier in this call).
# ;;     HKEYS_RANGE      - "first16": write H^1..H^16 to the stack;
# ;;                        "first32": write H^1..H^32 to the stack;
# ;;                        "mid16"  : write H^17..H^32 to the stack (assumes
...
# ;;     - H^17..H^32 are only ever written to the stack, never to the context.
# ;; =============================================================================
...
___
    ...
    # ; Complete H^9..H^16 in the context if this is their first use. The probe
    # ; leaves H^9..H^12 in $ZTMP2, ready for the stack store either way.
    &probe_ctx_hkeys_9_16($GCM128_CTX, $ZTMP2, ".L_hkeys_9_12_ready_${label_suffix}");
    &compute_ctx_hkeys_9_16($GCM128_CTX, $ZTMP0, $ZTMP1, $ZTMP2, $ZTMP3, $ZTMP4, $ZTMP5, $ZTMP6);
...
# ;; =============================================================================
# ;; ensure_ctx_hkeys_9_16: complete H^9..H^16 in the context if this is their
# ;; first use.
# ;;
# ;; For callers that GHASH straight from the context, so they need the powers
# ;; valid but neither the stack table nor its folded mirror. Going through
# ;; precompute_hkeys_on_stack instead would copy 16 powers to the frame, fold
# ;; them, and set HKEYS_READY, which makes the epilogue wipe 1 KiB of key
# ;; storage the caller never wrote.
# ;;
# ;; Clobbers: the six ZMM arguments, %k1 and EFLAGS. HKEYS_READY is left alone.
# ;; =============================================================================
sub ensure_ctx_hkeys_9_16 {
  my $GCM128_CTX = $_[0];
  my $ZTMP0      = $_[1];    # ; [clobbered] H^1..H^4
  my $ZTMP1      = $_[2];    # ; [clobbered] H^5..H^8, then H^8 broadcast
  my $ZTMP2      = $_[3];    # ; [clobbered] H^9..H^12
  my $ZTMP3      = $_[4];    # ; [clobbered] H^13..H^16
  my $ZTMP4      = $_[5];    # ; [clobbered]
  my $ZTMP5      = $_[6];    # ; [clobbered]

  my $label_suffix = $label_count++;

  # ; Probe first, so the common already-computed case costs a load and a branch
  &probe_ctx_hkeys_9_16($GCM128_CTX, $ZTMP2, ".L_ctx_hkeys_9_16_ready_${label_suffix}");
...
___
  ...
  &compute_ctx_hkeys_9_16($GCM128_CTX, $ZTMP0, $ZTMP1, $ZTMP2, $ZTMP3, $ZTMP4, $ZTMP5, $ZTMP0);

  $code .= ".L_ctx_hkeys_9_16_ready_${label_suffix}:\n";
}
...
___
  ...
  # ;; precompute_hkeys_on_stack, or by ensure_ctx_hkeys_9_16 when only the
  # ;; context copies are needed, once a message first requires them.
...
.L_exit_AAD_loop32x16_${label_suffix}:
        # ; < 32x16 bytes remain here (the loop exits above only when T2 < 512),
        # ; so fall through to the < 32-block tail.
...
.L_less_than_16x16_${label_suffix}:
        ...
        # ;; The per-count bodies below GHASH straight from the context, so this
        # ;; tail needs no stack hash keys - only H^9..H^16 completed in the
        # ;; context, and only once the count reaches 9. A 1-block AAD (a TLS
        # ;; record header, say) therefore costs what it did before the wide
        # ;; loops were added. The AAD load mask is not in %k1 yet, so the probe
        # ;; below is free to use it.
        cmp               \$9,@{[DWORD($T2)]}
        jb                .L_AAD_ctx_hkeys_done_${label_suffix}
___

  &ensure_ctx_hkeys_9_16($GCM128_CTX, $ZT5, $ZT6, $ZT7, $ZT8, $ZT9, $ZT0);

  $code .= <<___;
.L_AAD_ctx_hkeys_done_${label_suffix}:
        cmp               \$2,@{[DWORD($T2)]}
...
sub GCM_ENC_DEC {
  ...
  # ;;; unroll width (8, 16 or 32 blocks, or a masked small-block handler for
  # ;;; the shortest messages) is selected at run time from the
  ...
  # ;;;
  # ;;;     LENGTH <  128 B        -> masked small-block handler (<= 16 blocks
  # ;;;                               in one pass; the < 64 B fast dispatch and
  # ;;;                               the 64-127 B range both land here)
  # ;;;     128 B <= LENGTH <  704  -> 8-block loop
  # ;;;     704 B <= LENGTH < 1792  -> 16-block loop   (uses H^1..H^16)
  # ;;;     LENGTH >= 1792 B        -> 32-block big loop (uses H^1..H^32,
  # ;;;                                reduction every 32 blocks)
  # ;;;
  # ;;; Block diagram (LENGTH = bytes remaining after any leftover partial
  # ;;; block; drawn in code branch order). Every wide loop's short tail falls
  # ;;; back to a narrower path or the small handler, so any residual length is
  # ;;; always consumed:
  # ;;;
  # ;;;   LENGTH
  # ;;;     | ==0 --------------------------------------------> done
  ...
  # ;;;     |                        |                      | masked pass;     |
  # ;;;     |                        | >=128 B              | shared fallback  |
  # ;;;     |                        v                      | tail for the     |
  # ;;;     |                +-----------------+ tail <128B | loops below.     |
  # ;;;     |                | 8-block loop    |----------> +------------------+
  # ;;;     |                | 128 B / iter    |                 ^         ^
  # ;;;     | >=704 B        +-----------------+                 |         |
  # ;;;     v  (INITIAL_BLOCKS_16 + precompute H^1..H^16)        |         |
  # ;;;   [cmp 1792] -- <1792 B --> +-----------------+  tail re-enters dispatch:
  # ;;;     |                       | 16-block loop   |  >=128 B -> 8-block loop,
  # ;;;     |                       | 256 B / iter    |  else -> small handler
  # ;;;     | >=1792 B              +-----------------+
  # ;;;     v
  # ;;;   +--------------------------+  reduces the 32 pending blocks, then the
  # ;;;   | 32-block big loop        |  <32-block tail via GCM_ENC_DEC_LAST
  # ;;;   | 512 B / iter, reduce /32 |  (16<tail<32 blks) or the small handler
  # ;;;   +--------------------------+  (<=16 blks)
  # ;;;
  ...
  # ;;; encoded below as block counts: 64 B, 128 B, 44*16 = 704 B and
...
___
  ...
  $code .= <<___;
        cmp               \$`(44 * 16)`,$LENGTH
        jb                .L_dispatch_below_704B_${label_suffix}
___
...
___
  ...
  $code .= <<___;
        ...
        # ;; Defensive guard, unreachable with the current thresholds: this path
        # ;; is entered only for 704..1791 B, so after the first 16 blocks LENGTH
        # ;; is >= 448 (>= one 16-block group) and this jb is never taken
        # ;; (verified: 0 taken over an exhaustive length sweep). Kept so that
        # ;; lowering the 8->16-block crossover below 512 B cannot fall into the
        # ;; loop body with fewer than 16 blocks present.
        cmp               \$`(16 * 16)`,$LENGTH
...
___
  ...
  # ;; Cascade: if >= 64 bytes remain, re-enter the early path (8-block loop for
  # ;; >= 128 B, else the masked small-block handler).
  # ;; CTR_BLOCKz is broadcast BE, ADDBE restored, SHUF_MASK valid
  $code .= <<___;
        cmp               \$64,$LENGTH
        jae               .L_dispatch_below_704B_${label_suffix}
___
...
___
  ...
  # ══════════════════════════════════════════════════════════════
  # Early path (64 <= msg < 704): 8-block loop for >= 128 B; < 128 B is routed
  # to the masked small-block handler. Entered directly, or via cascade from the
  # 16-block loop tail. SHUF_MASK, ADDBE, CTR_CHECK, CTR_BLOCKz already set up.
  # ══════════════════════════════════════════════════════════════
  $code .= ".L_dispatch_below_704B_${label_suffix}:\n";

  # ;; Dispatch: messages >= 128 B use the 8-block path; < 128 B (4-7 blocks +
  # ;; optional partial) go to the masked small-block handler, which processes up
  # ;; to 16 blocks in a single pass. GCM_ENC_DEC_SMALL expects the counter in
  # ;; LE, so convert it back from BE first.
  $code .= <<___;
        cmp               \$128,$LENGTH
        jae               .L_8block_path_${label_suffix}
        vpshufb           @{[XWORD($SHUF_MASK)]},$CTR_BLOCKx,$CTR_BLOCKx
        jmp               .L_message_below_equal_16_blocks_${label_suffix}
.L_8block_path_${label_suffix}:
___
...
___
  ...
  # ;; small-block handler in a single pass (one GHASH reduction), which also
  # ;; flattens the dip just below 8-block multiples (e.g. 480/496/511B).
...
___
  ...
  $code .= <<___;
        # ;; Defensive guard, unreachable with the current thresholds: this path
        # ;; is entered only for LENGTH >= 1792 B (> 512), so this jb is never
        # ;; taken (verified: 0 taken over an exhaustive length sweep). Kept so
        # ;; that lowering the 16->32-block crossover below 512 B cannot enter the
        # ;; 32-block preamble with fewer than 32 blocks present.
        cmp               \$`(32 * 16)`,$LENGTH
...
___
  ...
  $code .= <<___;
        ...
        # ;; Defensive guard, unreachable with the current thresholds: entry is
        # ;; LENGTH >= 1792 B, so after this -512 LENGTH is >= 1280 (>= one
        # ;; 32-block group) and this jb is never taken (verified: 0 taken over an
        # ;; exhaustive length sweep). Kept so that lowering the 16->32-block
        # ;; crossover below 1024 B cannot force a 32-block iteration with fewer
        # ;; than 32 blocks present.
...
___
  ...
  # ;; Store the reduced GHASH accumulator (returned in ZTMP4 by the
  # ;; final_reduction above) into the running hash value AAD_HASHz.
  $code .= <<___;
        vmovdqa64         $ZTMP4,$AAD_HASHz
...
$code .= ".text\n";
{
  ...
  # ; context -- by precompute_hkeys_on_stack(), or by ensure_ctx_hkeys_9_16()
  # ; where only the context copies are wanted. Callers must therefore NOT
...
# ; CALC_AAD_HASH's HKEYS_READY: define it here so the paths that never write the
# ; frame tell EPILOG there are no hash keys to wipe. That covers the 12-byte IV
# ; fast path, which skips CALC_AAD_HASH altogether, and every IV short enough to
# ; be hashed by its sub-16-block tail -- which is every IV EVP accepts.
$code .= "xor               %rbx,%rbx\n";
&GCM_INIT_IV(
...
# ; See the note in setiv: keep HKEYS_READY defined for EPILOG. AAD below 256
# ; bytes stays in the sub-16-block tail and never writes the frame, so the common
# ; case (a TLS record header) leaves this zero and skips the wipe.
$code .= "xor               %rbx,%rbx\n";
&GCM_UPDATE_AAD(
