...
static const size_t rem_4bit[16] = { PACK(0x0000), PACK(0x1C20), PACK(0x3840),
				     PACK(0x2460), PACK(0x7080), PACK(0x6CA0),
				     PACK(0x48C0), PACK(0x54E0), PACK(0xE100),
				     PACK(0xFD20), PACK(0xD940), PACK(0xC560),
				     PACK(0x9180), PACK(0x8DA0), PACK(0xA9C0),
				     PACK(0xB5E0) };
...
#define GHASH(ctx, in, len) \
	ctx->funcs.ghash((ctx)->Xi.u, (ctx)->Htable, in, len)
/*
...
void gcm_ghash_rv64i_zbc(u64 Xi[2], const u128 Htable[16], const u8 *inp,
			 size_t len);
void gcm_ghash_rv64i_zbc__zbkb(u64 Xi[2], const u128 Htable[16], const u8 *inp,
			       size_t len);
...
void gcm_ghash_rv64i_zvkb_zvbc(u64 Xi[2], const u128 Htable[16], const u8 *inp,
			       size_t len);
...
static void gcm_get_funcs(struct gcm_funcs_st *ctx)
{
	...
	if (OPENSSL_ia32cap_P[1] & (1 << 1)) { /* check PCLMULQDQ bit */
		if (((OPENSSL_ia32cap_P[1] >> 22) & 0x41) ==
		    0x41) { /* AVX+MOVBE */
	...
	} else if (RISCV_HAS_ZVKB() && RISCV_HAS_ZVBC() &&
		   riscv_vlen() >= 128) {
