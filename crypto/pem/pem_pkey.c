                                  ...
                                  void *u)
{
    ...
    Otls_STORE_INFO *info = NULL;
    UI_METHOD *ui_method = NULL;

    if ((ui_method = UI_UTIL_wrap_read_pem_callback(cb, 0)) == NULL)
        return NULL;

    if ((ctx = otls_store_attach_pem_bio(bp, ui_method, u)) == NULL)
        goto err;
#ifndef OPENtls_NO_SECURE_HEAP
    {
        int on = 1;
        if (!Otls_STORE_ctrl(ctx, Otls_STORE_C_USE_SECMEM, &on))
            goto err;
    }
#endif

    while (!Otls_STORE_eof(ctx) && (info = Otls_STORE_load(ctx)) != NULL) {
        if (Otls_STORE_INFO_get_type(info) == Otls_STORE_INFO_PKEY) {
            ret = Otls_STORE_INFO_get1_PKEY(info);
            break;
        }
...
EVP_PKEY *PEM_read_bio_Parameters(BIO *bp, EVP_PKEY **x)
{
    ...
    if ((ctx = otls_store_attach_pem_bio(bp, UI_null(), NULL)) == NULL)
        goto err;

    while (!Otls_STORE_eof(ctx) && (info = Otls_STORE_load(ctx)) != NULL) {
        if (Otls_STORE_INFO_get_type(info) == Otls_STORE_INFO_PARAMS) {
            ret = Otls_STORE_INFO_get1_PARAMS(info);
            break;
        }
        Otls_STORE_INFO_free(info);
    }
...
DH *PEM_read_bio_DHparams(BIO *bp, DH **x, pem_password_cb *cb, void *u)
{
    ...
    while (!Otls_STORE_eof(ctx) && (info = Otls_STORE_load(ctx)) != NULL) {
        if (Otls_STORE_INFO_get_type(info) == Otls_STORE_INFO_PARAMS) {
            pkey = Otls_STORE_INFO_get0_PARAMS(info);
