					  ...
					  const char *propq, int selection)
{
	...
	while (!OSSL_DECODER_from_bio(dctx, bp) || pkey == NULL)
		if (BIO_eof(bp) != 0 || (newpos = BIO_tell(bp)) < 0 ||
		    newpos <= pos) {
			ERR_clear_last_mark();
			goto err;
		} else {
					 ...
					 const char *propq, int selection)
{
	...
	if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
		...
		if (!PEM_bytes_read_bio(&data, &len, &nm, pem_string, bp, cb,
					u)) {
			ERR_pop_to_mark();
			return NULL;
		}
				  ...
				  const char *propq, int selection)
{
	...
	if (ret == NULL && (BIO_seek(bp, pos) < 0 ||
			    (ret = pem_read_bio_key_legacy(
				     bp, x, ossl_pw_pem_password, &pwdata,
...
legacy:
	if (x != NULL && (x->ameth == NULL || x->ameth->priv_encode != NULL))
		return PEM_write_bio_PKCS8PrivateKey(
			out, x, enc, (const char *)kstr, klen, cb, u);
	return PEM_write_bio_PrivateKey_traditional(out, x, enc, kstr, klen, cb,
						    u);
}
					 ...
					 pem_password_cb *cb, void *u)
{
	...
	ret = PEM_ASN1_write_bio((i2d_of_void *)i2d_PrivateKey, pem_str, bp, x,
				 enc, kstr, klen, cb, u);
...
{
	return PEM_write_PrivateKey_ex(out, x, enc, kstr, klen, cb, u, NULL,
				       NULL);
}
#endif
