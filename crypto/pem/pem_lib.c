		  ...
		  pem_password_cb *callback, void *u)
{
	...
	ok = EVP_DecryptInit_ex(ctx, cipher->cipher, NULL, key,
				&(cipher->iv[0]));
	if (ok)
			       ...
			       unsigned int flags)
{
	...
	while (1) {
		...
		partial_line_read = len == LINESIZE - 1 &&
				    linebuf[LINESIZE - 2] != '\n';
		    ...
		    unsigned char **data, long *len_out, unsigned int flags)
{
	...
	*data = PEM_MALLOC(len, flags);
	if (*header == NULL || *data == NULL)
		goto out_free;
	if (headerlen != 0 &&
	    BIO_read(headerB, *header, headerlen) != headerlen)
		goto out_free;
	(*header)[headerlen] = '\0';
