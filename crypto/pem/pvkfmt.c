...
static EVP_PKEY *evp_pkey_new0_key(void *key, int evp_type)
{
	...
	if (!ossl_assert(evp_type == EVP_PKEY_RSA ||
			 evp_type == EVP_PKEY_DSA)) {
			...
			int *isdss, int *ispub)
{
	...
	if (ossl_do_blob_header(&p, length, &magic, &bitlen, isdss, ispub) <=
	    0) {
...
static int do_i2b(unsigned char **out, const EVP_PKEY *pk, int ispub)
{
	...
	outlen = 16 + ossl_blob_length(bitlen,
				       keyalg == MS_KEYALG_DSS_SIGN ? 1 : 0,
				       ispub);
	if (out == NULL)
		goto end;
	if (*out)
	...
	if (ispub) {
		...
		*pmagic = MS_RSA2MAGIC;
		...
		if ((BN_num_bytes(iqmp) > hnbyte) ||
		    (BN_num_bytes(p) > hnbyte) || (BN_num_bytes(q) > hnbyte) ||
			     ...
			     const char *propq)
{
	...
	if (saltlen) {
		...
		if (!derive_pvk_key(keybuf, sizeof(keybuf), p, saltlen,
				    (unsigned char *)psbuf, inlen, libctx,
				    propq))
			goto err;
		p += saltlen;
