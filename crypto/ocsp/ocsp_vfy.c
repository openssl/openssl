		       ...
		       unsigned long flags)
{
	...
	if ((flags & OCSP_NOSIGS) == 0) {
		...
		if (req != NULL)
			ret = OCSP_REQUEST_verify(req, skey, signer->libctx,
						  signer->propq);
		else
			ret = OCSP_BASICRESP_verify(bs, skey, signer->libctx,
						    signer->propq);
		if (ret <= 0)
		      ...
		      unsigned long flags)
{
	...
	if ((flags & OCSP_NOVERIFY) == 0) {
		...
		ret = ocsp_verify_signer(signer, 1, st, flags, untrusted,
					 &chain);
		if (ret <= 0)
			goto end;
...
static X509 *ocsp_find_signer_sk(STACK_OF(X509) *certs, OCSP_RESPID *id)
{
	...
	for (i = 0; i < sk_X509_num(certs); i++) {
		if ((x = sk_X509_value(certs, i)) != NULL) {
			if ((md = EVP_MD_fetch(x->libctx, SN_sha1, x->propq)) ==
			    NULL)
				break;
			       ...
			       STACK_OF(OCSP_SINGLERESP) *sresp)
{
	...
	if (cid != NULL) {
		...
		OBJ_obj2txt(name, sizeof(name), cid->hashAlgorithm.algorithm,
			    0);
