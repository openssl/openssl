...
static int namemap_add_name(OSSL_NAMEMAP *namemap, int number, const char *name)
{
	...
	if (ret <= 0) {
         ...
         */
		ERR_raise(ERR_LIB_CRYPTO, (ret < 0) ? CRYPTO_R_TOO_MANY_NAMES :
...
static void get_legacy_cipher_names(const OBJ_NAME *on, void *arg)
{
	...
	if (cipher != NULL)
		get_legacy_evp_names(NID_undef, EVP_CIPHER_get_type(cipher),
				     NULL, arg);
}
				       ...
				       void *arg)
{
	...
	EVP_PKEY_asn1_get0_info(&nid, &base_nid, &flags, NULL, &pem_name,
				ameth);
	if (nid != NID_undef) {
		if ((flags & ASN1_PKEY_ALIAS) == 0) {
			...
			default:
				...
				get_legacy_evp_names(base_nid, nid, pem_name,
						     arg);
			}
		}
	}
...
OSSL_NAMEMAP *ossl_namemap_stored(OSSL_LIB_CTX *libctx)
{
	...
	if (nms == 1) {
		...
		OBJ_NAME_do_all(OBJ_NAME_TYPE_MD_METH, get_legacy_md_names,
				namemap);
		...
		if (num != 0) {
			...
			ossl_namemap_add_name(namemap, num,
...
#ifndef OPENSSL_NO_DEPRECATED_3_6
		{
			...
			for (i = 0, end = EVP_PKEY_asn1_get_count(); i < end;
			     i++)
