...
/* zero107 is 0 mod p */
static const felem zero107 = { two107m43m11, two107, two107m43p11,
			       two107m43p11 };
...
static void longfelem_diff(longfelem out, const longfelem in)
{
	...
	static const limb two70m40m38p6 = (((limb)1) << 70) -
					  (((limb)1) << 40) -
					  (((limb)1) << 38) + (((limb)1) << 6);
			       ...
			       const smallfelem y_in, const smallfelem z_in)
{
	...
	point_double(felem_x_out, felem_y_out, felem_z_out, felem_x_in,
		     felem_y_in, felem_z_in);
		      ...
		      const smallfelem g_pre_comp[2][16][3])
{
	...
	for (i = (num_points ? 255 : 31); i >= 0; --i) {
		...
		if (gen_mul && (i <= 31)) {
			...
			/* Arg 1 below is for "mixed" */
			point_add(nq[0], nq[1], nq[2], nq[0], nq[1], nq[2], 1,
				  tmp[0], tmp[1], tmp[2]);
		}
			...
			for (num = 0; num < num_points; ++num) {
				...
				smallfelem_neg(
					...
					tmp[1]); /* (X, -Y, Z) is the negative
                                               * point */
				copy_small_conditional(ftmp, tmp[1],
						       (((limb)sign) - 1));
				    ...
				    const BIGNUM *scalars[], BN_CTX *ctx)
{
	...
	if (scalar != NULL) {
		...
		if (pre)
			...
			g_pre_comp =
				(const smallfelem(*)[16][3])pre->g_pre_comp;
		else
	...
	if (num_points > 0) {
		...
		if (mixed)
			tmp_smallfelems = OPENSSL_malloc_array(
				num_points * 17 + 1, sizeof(*tmp_smallfelems));
	...
	if ((scalar != NULL) && (have_pre_comp)) {
		...
		batch_mul(x_out, y_out, z_out,
			  ...
			  g_secret, mixed, (const smallfelem(*)[17][3])pre_comp,
			  g_pre_comp);
	} else {
		/* do the multiplication without generator precomputation */
		batch_mul(x_out, y_out, z_out,
			  (const felem_bytearray(*))secrets, num_points, NULL,
			  mixed, (const smallfelem(*)[17][3])pre_comp, NULL);
	}
