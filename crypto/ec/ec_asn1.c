...
ASN1_SEQUENCE(
	X9_62_PENTANOMIAL) = { ASN1_EMBED(X9_62_PENTANOMIAL, k1, INT32),
			       ASN1_EMBED(X9_62_PENTANOMIAL, k2, INT32),
			       ASN1_EMBED(
				       X9_62_PENTANOMIAL, k3,
...
ASN1_ADB(X9_62_CHARACTERISTIC_TWO) = {
	...
	ADB_ENTRY(NID_X9_62_tpBasis, ASN1_SIMPLE(X9_62_CHARACTERISTIC_TWO,
						 p.tpBasis, ASN1_INTEGER)),
	ADB_ENTRY(NID_X9_62_ppBasis, ASN1_SIMPLE(X9_62_CHARACTERISTIC_TWO,
						 p.ppBasis, X9_62_PENTANOMIAL))
} ASN1_ADB_END(X9_62_CHARACTERISTIC_TWO, 0, type, 0, &char_two_def_tt, NULL);

ASN1_SEQUENCE(
	X9_62_CHARACTERISTIC_TWO) = { ASN1_EMBED(X9_62_CHARACTERISTIC_TWO, m,
						 INT32),
...
ASN1_SEQUENCE(
	EC_PRIVATEKEY) = { ASN1_EMBED(EC_PRIVATEKEY, version, INT32),
			   ...
			   ASN1_EXP_OPT(EC_PRIVATEKEY, parameters,
					ECPKPARAMETERS, 0),
			   ASN1_EXP_OPT(
				   EC_PRIVATEKEY, publicKey, ASN1_BIT_STRING,
...
static int ec_asn1_group2fieldid(const EC_GROUP *group, X9_62_FIELDID *field)
{
...
#else
	{
		...
		if (field_type == NID_X9_62_tpBasis) {
			...
			if (!EC_GROUP_get_pentanomial_basis(group, &k1, &k2,
							    &k3))
				goto err;
...
EC_GROUP *EC_GROUP_new_from_ecparameters(const ECPARAMETERS *params)
{
...
#else
	{
		...
		if (tmp == NID_X9_62_tpBasis) {
			...
			if (!(char_two->m > penta->k3 &&
			      penta->k3 > penta->k2 && penta->k2 > penta->k1 &&
			      penta->k1 > 0)) {
				ERR_raise(ERR_LIB_EC,
...
EC_KEY *d2i_ECPrivateKey(EC_KEY **a, const unsigned char **in, long len)
{
	...
	if (priv_key->parameters) {
		...
		ret->group =
			EC_GROUP_new_from_ecpkparameters(priv_key->parameters);
		if (ret->group != NULL &&
