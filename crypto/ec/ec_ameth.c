...
static int eckey_pub_encode(X509_PUBKEY *pk, const EVP_PKEY *pkey)
{
	...
	if (penclen <= 0)
		goto err;
	if (X509_PUBKEY_set0_param(pk, OBJ_nid2obj(EVP_PKEY_EC), ptype, pval,
				   penc, penclen))
		return 1;
err:
...
static int do_EC_KEY_print(BIO *bp, const EC_KEY *x, int off, ec_print_t ktype)
{
	...
	if (ktype == EC_KEY_PRINT_PRIVATE &&
	    EC_KEY_get0_private_key(x) != NULL) {
...
static int ec_pkey_ctrl(EVP_PKEY *pkey, int op, long arg1, void *arg2)
{
	...
	case ASN1_PKEY_CTRL_SET1_TLS_ENCPT:
		...
		if (!ossl_assert(evp_pkey_is_legacy(pkey)))
			return 0;
		return EC_KEY_oct2key(evp_pkey_get0_EC_KEY_int(pkey), arg2,
				      arg1, NULL);
			     ...
			     OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if (!ossl_ec_group_todata(ecg, tmpl, NULL, libctx, propq, bnctx,
				  &gen_buf))
		goto err;
	...
	if (pub_point != NULL) {
		...
		if ((pub_key_buflen = EC_POINT_point2buf(ecg, pub_point, format,
		    ...
		    !OSSL_PARAM_BLD_push_octet_string(
			    tmpl, OSSL_PKEY_PARAM_PUB_KEY, pub_key_buf,
			    pub_key_buflen))
			goto err;
	...
	if (priv_key != NULL) {
         ...
         */
		ecdh_cofactor_mode =
			(EC_KEY_get_flags(eckey) & EC_FLAG_COFACTOR_ECDH) ? 1 :
									    0;
