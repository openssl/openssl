    ...
    const BIGNUM *b, BN_CTX *ctx)
{
    ...
    /*
     * The field Montgomery context (group->fn_mont_ctx) is (re)built inside the
     * GF(p) group_set_curve sink, ossl_ec_GFp_simple_group_set_curve(), so that
     * the direct meth->group_set_curve() call on the named-curve path gets it
     * too.
     */
    ...
    const EC_POINT *point, OSSL_FN_CTX *ctx)
{
     ...
     * OSSL_FN_CTX_SIZE_NONE means the implementation needs no context (leave it
     * NULL).
     */
    if (ctx == NULL) {
