		  ...
		  BN_CTX *ctx)
{
	...
	if (group->meth->mul != NULL)
		ret = group->meth->mul(group, r, scalar, num, points, scalars,
				       ctx);
	else
		/* use default */
		ret = ossl_ec_wNAF_mul(group, r, scalar, num, points, scalars,
				       ctx);
		 ...
		 const EC_POINT *point, const BIGNUM *p_scalar, BN_CTX *ctx)
{
	...
	if (group->meth->mul != NULL)
		ret = group->meth->mul(group, r, g_scalar, num, &point,
				       &p_scalar, ctx);
	else
		/* use default */
		ret = ossl_ec_wNAF_mul(group, r, g_scalar, num, &point,
				       &p_scalar, ctx);
...
int EC_GROUP_get_basis_type(const EC_GROUP *group)
{
	...
	for (i = 0; i < (int)OSSL_NELEM(group->poly) && group->poly[i] != 0;
	     i++)
		continue;
					    ...
					    const char *propq, BN_CTX *ctx)
{
	...
	if ((curve_name_nid = ossl_ec_curve_nid_from_params(dup, ctx)) !=
		...
		ret_group = EC_GROUP_new_by_curve_name_ex(libctx, propq,
							  curve_name_nid);
		if (ret_group == NULL)
			goto err;
				     ...
				     const char *propq)
{
	...
	if (ok) {
		...
		if (nid == NID_undef) {
			...
			return EC_GROUP_new_by_curve_name_ex(libctx, propq,
							     nid);
		}
	}
	return NULL;
				   ...
				   OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	EC_GROUP_set_point_conversion_form(
		group, (point_conversion_form_t)buf[0] & ~0x01);
