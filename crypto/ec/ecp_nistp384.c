...
static void felem_diff128(widefelem out, const widefelem in)
{
	...
	static const widelimb two127p111m79m71 =
		(((widelimb)1) << 127) + (((widelimb)1) << 111) -
		(((widelimb)1) << 79) - (((widelimb)1) << 71);
...
static void felem_inv(felem out, const felem in)
{
	...
	felem_mul_reduce(ftmp, ftmp3,
			 ftmp); /* 2^5 + 2^4 + 2^3 + 2^2 + 2^1 + 2^0 */
...
static void felem_contract(felem out, const felem in)
{
	...
	cond[2] =
		((tmp[2] | 0xffffffffff0000) & (tmp[1] | 0x0000ffffffffff)) + 1;
	/* 0 iff (2^32..2^95) bits are all zero */
	cond[3] = (tmp[1] & ~0xffff0000000000) |
		  (tmp[0] & ~((int64_t)0x000000ffffffff));
		      ...
		      const felem x2, const felem y2, const felem z2)
{
	...
	if (!mixed) {
		...
		felem_diff_128_64(tmp,
				  ftmp2); /* tmp[i] < 2^117 + 2^65 + 2^49 */
	...
	for (i = (num_points ? 380 : 98); i >= 0; --i) {
		...
		if (gen_mul && (i <= 98)) {
			...
			if (!skip) {
				...
				point_add(nq[0], nq[1], nq[2], nq[0], nq[1],
					  nq[2], 1, tmp[0], tmp[1], tmp[2]);
			} else {
		...
		if (num_points && (i % 5 == 0)) {
			...
			for (num = 0; num < num_points; ++num) {
				...
				felem_neg(tmp[3],
					  tmp[1]); /* (X, -Y, Z) is the negative
                                            * point */
				copy_conditional(tmp[1], tmp[3], (-(limb)sign));

				if (!skip) {
					point_add(nq[0], nq[1], nq[2], nq[0],
						  nq[1], nq[2], mixed, tmp[0],
						  tmp[1], tmp[2]);
				} else {
					memcpy(nq, tmp, 3 * sizeof(felem));
					skip = 0;
			       ...
			       felem tmp_felems[])
{
	...
	ossl_ec_GFp_nistp_points_make_affine_internal(
		num, points, sizeof(felem), tmp_felems,
		(void (*)(void *))felem_one, felem_is_zero_int,
				    ...
				    const BIGNUM *scalars[], BN_CTX *ctx)
{
	...
	if (num_points > 0) {
		...
		if (mixed)
			tmp_felems = OPENSSL_malloc_array(num_points * 17 + 1,
							  sizeof(*tmp_felems));
		...
		for (i = 0; i < num_points; ++i) {
			...
			if (p_scalar != NULL && p != NULL) {
				...
				if ((BN_num_bits(p_scalar) > 384) ||
                     ...
                     * constant-timeness
                     */
					if (!BN_nnmod(tmp_scalar, p_scalar,
						      group->order, ctx)) {
						ERR_raise(ERR_LIB_EC,
							  ERR_R_BN_LIB);
						goto err;
					}
					num_bytes = BN_bn2lebinpad(
						tmp_scalar, secrets[i],
						sizeof(secrets[i]));
				} else {
					num_bytes = BN_bn2lebinpad(
						p_scalar, secrets[i],
		...
		if (mixed)
			make_points_affine(num_points * 17, pre_comp[0],
					   tmp_felems);
	}
	...
	if (scalar != NULL && have_pre_comp) {
		...
		if ((BN_num_bits(scalar) > 384) || (BN_is_negative(scalar))) {
			...
			num_bytes = BN_bn2lebinpad(tmp_scalar, g_secret,
						   sizeof(g_secret));
		} else {
			num_bytes = BN_bn2lebinpad(scalar, g_secret,
						   sizeof(g_secret));
		}
		/* do the multiplication with generator precomputation */
		batch_mul(x_out, y_out, z_out,
			  (const felem_bytearray(*))secrets, num_points,
			  g_secret, mixed, (const felem(*)[17][3])pre_comp,
			  (const felem(*)[3])g_pre_comp);
	} else {
		/* do the multiplication without generator precomputation */
		batch_mul(x_out, y_out, z_out,
			  (const felem_bytearray(*))secrets, num_points, NULL,
			  mixed, (const felem(*)[17][3])pre_comp, NULL);
	}
	...
	ret = ossl_ec_GFp_simple_set_Jprojective_coordinates_GFp(group, r, x, y,
								 z, ctx);
...
int ossl_ec_GFp_nistp384_precompute_mult(EC_GROUP *group, BN_CTX *ctx)
{
	...
	point_add(pre->g_pre_comp[6][0], pre->g_pre_comp[6][1],
		  ...
		  pre->g_pre_comp[4][1], pre->g_pre_comp[4][2], 0,
		  pre->g_pre_comp[2][0], pre->g_pre_comp[2][1],
		  pre->g_pre_comp[2][2]);
	/* 2^95*G + 2^285*G */
	point_add(pre->g_pre_comp[10][0], pre->g_pre_comp[10][1],
		  pre->g_pre_comp[10][2], pre->g_pre_comp[8][0],
		  pre->g_pre_comp[8][1], pre->g_pre_comp[8][2], 0,
		  pre->g_pre_comp[2][0], pre->g_pre_comp[2][1],
		  pre->g_pre_comp[2][2]);
	/* 2^190*G + 2^285*G */
	...
	point_add(pre->g_pre_comp[14][0], pre->g_pre_comp[14][1],
		  ...
		  pre->g_pre_comp[12][1], pre->g_pre_comp[12][2], 0,
		  pre->g_pre_comp[2][0], pre->g_pre_comp[2][1],
		  pre->g_pre_comp[2][2]);
	for (i = 1; i < 8; ++i) {
		/* odd multiples: add G */
		point_add(pre->g_pre_comp[2 * i + 1][0],
			  ...
			  pre->g_pre_comp[2 * i][0], pre->g_pre_comp[2 * i][1],
			  pre->g_pre_comp[2 * i][2], 0, pre->g_pre_comp[1][0],
			  pre->g_pre_comp[1][1], pre->g_pre_comp[1][2]);
	}
