...
static int ossl_ec_GFp_nistp224_points_mul_fn(const EC_GROUP *group,
    EC_POINT *r, const OSSL_FN *scalar, const EC_POINT *point,
    OSSL_FN_CTX *fnctx);
static size_t ossl_ec_GFp_nistp224_points_mul_fn_ctx_size(const EC_GROUP *group,
    EC_POINT *r, const OSSL_FN *scalar, const EC_POINT *point);
...
const EC_METHOD *EC_GFp_nistp224_method(void)
{
    static const EC_METHOD ret = {
        ...
        0, /* ladder_post */
        0, /* group_full_init */
        ossl_ec_GFp_nistp224_points_mul_fn, /* mul_fn */
        ossl_ec_GFp_nistp224_points_mul_fn_ctx_size, /* mul_fn_ctx_size */
 ...
 */
static int nistp224_scalar_to_bytes(felem_bytearray out, const BIGNUM *scalar,
...
static int nistp224_points_mul_bytes(const EC_GROUP *group, EC_POINT *r,
    ...
    const BIGNUM *scalars[], BN_CTX *ctx)
{
    ...
    if (num > 0) {
        ...
        for (i = 0; i < num; i++) {
            if (scalars[i] != NULL && points[i] != NULL) {
                if (!nistp224_scalar_to_bytes(bufs[i], scalars[i], group, ctx))
                    goto err;
                buf_ptrs[i] = &bufs[i];
            }
        }
    ...
    if (scalar != NULL) {
        if (!nistp224_scalar_to_bytes(buf, scalar, group, ctx))
            goto err;
        pbuf = &buf;
    }

    ret = nistp224_points_mul_bytes(group, r, pbuf, num, points, buf_ptrs, ctx);
...
/*
 * OSSL_FN counterpart of ossl_ec_GFp_nistp224_points_mul() and this method's
 ...
 */
static int ossl_ec_GFp_nistp224_points_mul_fn(const EC_GROUP *group, EC_POINT *r,
    const OSSL_FN *scalar, const EC_POINT *point, OSSL_FN_CTX *fnctx)
{
    ...
    if (point == NULL)
        ret = nistp224_points_mul_bytes(group, r, &b, 0, NULL, NULL, ctx);
    else
        ret = nistp224_points_mul_bytes(group, r, NULL, 1, &point, &pb, ctx);
