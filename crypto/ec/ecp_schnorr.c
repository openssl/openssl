/* Schnorr Σ-Protocol (RFC 8235) over secp256k1
 * Fiat-Shamir transform: c = H(R || Y || msg)
 * s = k + c·x (mod order)
                      ...
                      unsigned char *sig, size_t *sig_len)
{
    ...
    BIGNUM *priv = BN_new();
    BN_bin2bn(priv_key, priv_key_len, priv);
    ...
    *sig_len = 33 + order_bytes;
    ...
    BN_free(c_priv); BN_free(s); BN_free(c);
    EC_POINT_free(R); BN_free(k); BN_free(priv);
    EC_POINT_free(pub); EC_KEY_free(ec); BN_CTX_free(bn_ctx);
    return 1;
}
                        ...
                        const unsigned char *sig, size_t sig_len)
{
    ...
    if (pub_key_len > 0) {
        EC_POINT_oct2point(group, Y, pub_key, pub_key_len, bn_ctx);
    }
    EC_KEY_set_public_key(ec, Y);

    int order_bytes = BN_num_bytes(order);
    if (sig_len < (size_t)(33 + order_bytes)) {
        EC_POINT_free(Y); EC_KEY_free(ec); BN_CTX_free(bn_ctx);
        return 0;
    }
    ...
    BN_free(c); BN_free(s); EC_POINT_free(R); EC_POINT_free(Y);
    EC_KEY_free(ec); BN_CTX_free(bn_ctx);
...
int schnorr_sign(const unsigned char *msg, size_t msg_len,
                  ...
                  unsigned char *sig, size_t *sig_len)
{
    return schnorr_sign_raw(msg, msg_len, priv_key, 32, sig, sig_len);
}
...
int schnorr_verify(const unsigned char *msg, size_t msg_len,
                    ...
                    const unsigned char *sig, size_t sig_len)
{
    return schnorr_verify_raw(msg, msg_len, pub_key, 33, sig, sig_len);
}
