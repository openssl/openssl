					    ...
					    BN_CTX *ctx)
{
	...
	if ((num * 16 + 6) > OPENSSL_MALLOC_MAX_NELEMS(P256_POINT) ||
	    (table_storage = OPENSSL_malloc(
		     (num * 16 + 5) * sizeof(P256_POINT) + 64)) == NULL ||
	...
	for (i = 0; i < num; i++) {
		...
		for (j = 0; j < bn_get_top(scalars[i]) * BN_BYTES;
		     j += BN_BYTES) {
		...
		if (!ecp_nistz256_bignum_to_field_elem(temp[0].X,
						       point[i]->X) ||
		    !ecp_nistz256_bignum_to_field_elem(temp[0].Y,
						       point[i]->Y) ||
		    !ecp_nistz256_bignum_to_field_elem(temp[0].Z,
						       point[i]->Z)) {
		...
		ecp_nistz256_point_add(&temp[2], &temp[1],
				       &temp[0]); /*2+1=3  */
		...
		ecp_nistz256_point_add(&temp[3], &temp[1],
				       &temp[0]); /*4+1=5  */
		ecp_nistz256_scatter_w5(row, &temp[3], 5);
		ecp_nistz256_point_add(&temp[4], &temp[2],
				       &temp[0]); /*6+1=7  */
		...
		ecp_nistz256_point_add(&temp[2], &temp[2],
				       &temp[0]); /*12+1=13*/
		ecp_nistz256_scatter_w5(row, &temp[2], 13);
		ecp_nistz256_point_add(&temp[3], &temp[3],
				       &temp[0]); /*10+1=11*/
		ecp_nistz256_scatter_w5(row, &temp[3], 11);
		ecp_nistz256_point_add(&temp[4], &temp[4],
				       &temp[0]); /*14+1=15*/
     ...
     * to table
     */
	ecp_nistz256_gather_w5(&temp[0], table[0],
			       _booth_recode_w5(wvalue) >> 1);
...
__owur static int ecp_nistz256_mult_precompute(EC_GROUP *group, BN_CTX *ctx)
{
	...
	if ((precomp_storage = OPENSSL_malloc(
		     37 * 64 * sizeof(P256_POINT_AFFINE) + 64)) == NULL)
		goto err;
	...
	if (scalar) {
		...
		if (pre_comp) {
			...
			if (0 == EC_POINT_cmp(group, generator,
				...
				preComputedTable = (const PRECOMP256_ROW *)
							   pre_comp->precomp;
		...
		if (preComputedTable) {
			...
			for (i = 0; i < bn_get_top(scalar) * BN_BYTES;
			     i += BN_BYTES) {
			...
			if (P256_LIMBS == 8)
				infty |= (p.p.X[4] | p.p.X[5] | p.p.X[6] |
					  p.p.X[7] | p.p.Y[4] | p.p.Y[5] |
					  p.p.Y[6] | p.p.Y[7]);
			...
			for (i = 1; i < 37; i++) {
				...
				ecp_nistz256_gather_w7(
					&t.a, preComputedTable[i], wvalue >> 1);
				    ...
				    const BIGNUM *x, BN_CTX *ctx)
{
	...
	ecp_nistz256_ord_mul_mont(table[i_101111], table[i_101010],
				  table[i_101]);
				       ...
				       const unsigned char *params)
{
	...
	if (!ossl_bn_mont_ctx_set(mont, p, 256, params + 6 * param_len,
				  param_len, 1, 0))
		goto err;
