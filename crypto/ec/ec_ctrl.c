...
int EVP_PKEY_CTX_set1_dist_id(EVP_PKEY_CTX *ctx, const void *id, int len)
{
    int ret;
    OSSL_PARAM params[2], *p = params;
...
int EVP_PKEY_CTX_set1_id(EVP_PKEY_CTX *ctx, const void *id, int len)
{
    ...
    if (ctx->keytype == NULL) {
        ...
        keytype = OBJ_sn2nid(ctx->keytype);
        if (keytype == NID_undef)
            keytype = OBJ_ln2nid(ctx->keytype);
    }

    return EVP_PKEY_CTX_ctrl(ctx, keytype, -1, \
