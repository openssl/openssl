...
ret:
	...
	if (rc == -1)
		rc = ossl_ec_wNAF_mul(group, r, scalar, num, points, scalars,
				      ctx);
					...
					unsigned int fc, int len)
{
	...
	if (group->meth->point_get_affine_coordinates(group, pubkey, x, y,
						      ctx) != 1 ||
...
#define EC_GFP_S390X_NISTP_METHOD(bits)                                        \
	...
	static ECDSA_SIG *ecdsa_s390x_nistp##bits##_sign_sig(                  \
		...
		const BIGNUM *r, EC_KEY *eckey)                                \
	{                                                                      \
		return ecdsa_s390x_nistp_sign_sig(dgst, dgstlen, kinv, r,      \
						  eckey,                       \
	...
	static int ecdsa_s390x_nistp##bits##_verify_sig(                       \
		...
		EC_KEY *eckey)                                                 \
	{                                                                      \
		return ecdsa_s390x_nistp_verify_sig(                           \
			dgst, dgstlen, sig, eckey, S390X_ECDSA_VERIFY_P##bits, \
			S390X_SIZE_P##bits);                                   \
	}                                                                      \
                                                                               \
