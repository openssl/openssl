			       ...
			       EC_KEY *eckey)
{
	...
	return eckey->group->meth->ecdsa_sign_sig(dgst, dgst_len, in_kinv, in_r,
						  eckey);
}
				  ...
				  const char *propq)
{
	...
	*siglen = 0;
	if (!ecdsa_sign_setup(eckey, NULL, &kinv, &r, dgst, dlen, nonce_type,
			    ...
			    OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if (order_bits < MIN_ECDSA_SIGN_ORDERBITS ||
	    !BN_set_bit(k, order_bits) || !BN_set_bit(r, order_bits) ||
	...
	do {
		...
		do {
			...
			if (dgst != NULL) {
				if (nonce_type == 1) {
					res = ossl_gen_deterministic_nonce_rfc6979(
						k, order, priv_key, dgst, dlen,
						digestname, libctx, propq);
				} else {
				...
				res = ossl_bn_priv_rand_range_fixed_top(
					k, order, 0, ctx);
			}
			if (!res) {
		...
		if (!EC_POINT_get_affine_coordinates(group, tmp_point, X, NULL,
						     ctx)) {
			ERR_raise(ERR_LIB_EC, ERR_R_EC_LIB);
			goto err;
		}
...
int ossl_ecdsa_simple_sign_setup(EC_KEY *eckey, BN_CTX *ctx_in, BIGNUM **kinvp,
				 BIGNUM **rp)
{
	return ecdsa_sign_setup(eckey, ctx_in, kinvp, rp, NULL, 0, 0, NULL,
				NULL, NULL);
}
				      ...
				      EC_KEY *eckey)
{
	...
	do {
		if (in_kinv == NULL || in_r == NULL) {
			if (!ecdsa_sign_setup(eckey, ctx, &kinv, &ret->r, dgst,
					      dgst_len, 0, NULL, NULL, NULL)) {
				ERR_raise(ERR_LIB_EC, ERR_R_ECDSA_LIB);
				goto err;
			}
			ckinv = kinv;
		...
		if (!bn_to_mont_fixed_top(s, ret->r, group->mont_data, ctx) ||
		    !bn_mul_mont_fixed_top(s, s, priv_key, group->mont_data,
					   ctx)) {
			ERR_raise(ERR_LIB_EC, ERR_R_BN_LIB);
			goto err;
		}
		...
		if (!bn_to_mont_fixed_top(s, s, group->mont_data, ctx) ||
		    !BN_mod_mul_montgomery(s, s, ckinv, group->mont_data,
					   ctx)) {
			ERR_raise(ERR_LIB_EC, ERR_R_BN_LIB);
			goto err;
		}
