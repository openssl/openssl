					 ...
					 OSSL_CALLBACK *cb, void *cbarg)
{
	...
	if (!EC_POINT_mul(eckey->group, pub_key2, eckey->priv_key, NULL, NULL,
			  ctx))
		goto err;
			       ...
			       size_t ikmlen)
{
	...
	if (ossl_ec_dhkem_derive_private(eckey, eckey->priv_key, ikm, ikmlen) <=
	    0)
		goto err;
	if (eckey->pub_key == NULL) {
...
static int ec_key_public_range_check(BN_CTX *ctx, const EC_KEY *key)
{
	...
	if (!EC_POINT_get_affine_coordinates(key->group, key->pub_key, x, y,
					     ctx))
		goto err;

	if (EC_GROUP_get_field_type(key->group) == NID_X9_62_prime_field) {
		if (BN_is_negative(x) || BN_cmp(x, key->group->field) >= 0 ||
		    BN_is_negative(y) || BN_cmp(y, key->group->field) >= 0) {
			goto err;
		}
	} else {
...
int EC_KEY_set_group(EC_KEY *key, const EC_GROUP *group)
{
	...
	if (key->group != NULL &&
	    EC_GROUP_get_curve_name(key->group) == NID_sm2)
...
size_t ossl_ec_key_simple_priv2oct(const EC_KEY *eckey, unsigned char *buf,
				   size_t len)
{
	int buf_len;
