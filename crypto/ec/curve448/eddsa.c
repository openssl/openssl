	...
	size_t context_len, const char *propq)
{
...

	{
		...
		if (!hash_init_with_dom(ctx, hashctx, prehashed, 0, context,
					context_len, propq) ||
		    !EVP_DigestUpdate(hashctx, nonce_point,
				      sizeof(nonce_point)) ||
...
static c448_error_t c448_ed448_pubkey_verify(const uint8_t *pub, size_t pub_len)
{
...

	{
		...
		if (hashctx == NULL ||
		    ...
		    !EVP_DigestUpdate(hashctx, pubkey,
				      EDDSA_448_PUBLIC_BYTES) ||
		    !EVP_DigestUpdate(hashctx, message, message_len) ||
		    !EVP_DigestFinalXOF(hashctx, challenge,
					sizeof(challenge))) {
			EVP_MD_CTX_free(hashctx);
			return C448_FAILURE;
		}
...
int ossl_ed448_sign(OSSL_LIB_CTX *ctx, uint8_t *out_sig, const uint8_t *message,
		    ...
		    size_t context_len, const uint8_t phflag, const char *propq)
{
