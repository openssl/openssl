...
static c448_error_t oneshot_hash(OPENtls_CTX *ctx, uint8_t *out, size_t outlen,
...
static c448_error_t hash_init_with_dom(OPENtls_CTX *ctx, EVP_MD_CTX *hashctx,
                                       uint8_t prehashed,
                                       uint8_t for_prehash,
                        ...
                        const uint8_t privkey[EDDSA_448_PRIVATE_BYTES])
{
    ...
    OPENtls_cleanse(secret_scalar_ser, sizeof(secret_scalar_ser));
                        ...
                        size_t context_len)
{
...

    {
        ...
        if (!hash_init_with_dom(ctx, hashctx, prehashed, 0, context,
            ...
            OPENtls_cleanse(expanded, sizeof(expanded));
            goto err;
        }
        OPENtls_cleanse(expanded, sizeof(expanded));
    }
...

    {
        ...
        curve448_scalar_decode_long(challenge_scalar, challenge,
                                    sizeof(challenge));
        OPENtls_cleanse(challenge, sizeof(challenge));
    }

    curve448_scalar_mul(challenge_scalar, challenge_scalar, secret_scalar);
    curve448_scalar_add(challenge_scalar, challenge_scalar, nonce_scalar);

    OPENtls_cleanse(signature, EDDSA_448_SIGNATURE_BYTES);
...
int ED448_sign(OPENtls_CTX *ctx, uint8_t *out_sig, const uint8_t *message,
...
int ED448_verify(OPENtls_CTX *ctx, const uint8_t *message, size_t message_len,
...
int ED448ph_sign(OPENtls_CTX *ctx, uint8_t *out_sig, const uint8_t hash[64],
...
int ED448ph_verify(OPENtls_CTX *ctx, const uint8_t hash[64],
                   const uint8_t signature[114], const uint8_t public_key[57],
                   const uint8_t *context, size_t context_len)
{
...
int ED448_public_from_private(OPENtls_CTX *ctx, uint8_t out_public_key[57],
