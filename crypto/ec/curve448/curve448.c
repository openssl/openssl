...
c448_bool_t ossl_curve448_point_eq(const curve448_point_t p,
...
c448_error_t ossl_curve448_point_decode_like_eddsa_and_mul_by_ratio(
...
c448_error_t ossl_x448_int(uint8_t out[X_PUBLIC_BYTES],
				 ...
				 const uint8_t scalar[X_PRIVATE_BYTES])
{
	...
	scalar2[X_PRIVATE_BYTES - 1] &=
		~((0u - 1u) << ((X_PRIVATE_BITS + 7) % 8));
...
void ossl_curve448_base_double_scalarmul_non_secret(
	curve448_point_t combo, const curve448_scalar_t scalar1,
	const curve448_point_t base2, const curve448_scalar_t scalar2)
{
	...
	if (i > control_pre[0].power) {
		...
		add_niels_to_pt(
			combo,
			ossl_curve448_wnaf_base[control_pre[0].addend >> 1], i);
		contv++;
		contp++;
	} else {
