...
static ossl_inline void ecp_sm2p256_mod_ord_inverse(BN_ULONG *out,
						    const BN_ULONG *in)
{
	BN_MOD_INV(out, in, ecp_sm2p256_div_by_2_mod_ord,
		   ecp_sm2p256_sub_mod_ord, def_ord);
}
...
static void ecp_sm2p256_point_G_mul_by_scalar(P256_POINT *R, const BN_ULONG *k)
{
	...
	for (i = 1; i < 32; ++i) {
		...
		if (index) {
			...
			memcpy(Q.Y,
			       ecp_sm2p256_precomputed + index + P256_LIMBS,
			       32);
					      ...
					      P256_POINT_AFFINE P)
{
	...
	for (i = 3; i < 16; ++i)
		ecp_sm2p256_point_add_affine(&precomputed[i],
					     &precomputed[i - 1], &P);
				    ...
				    BN_CTX *ctx)
{
	...
	for (i = 0; i < num; i++) {
		...
		if ((BN_num_bits(scalar[i]) > 256) ||
		    BN_is_negative(scalar[i])) {
			BIGNUM *tmp;
				  ...
				  const BIGNUM *scalars[], BN_CTX *ctx)
{
	...
	if (num) {
		...
		if (!ecp_sm2p256_windowed_mul(group, out, scalars, points, num,
					      ctx))
			goto err;
