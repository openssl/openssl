...
static int ecx_ctrl(EVP_PKEY *pkey, int op, long arg1, void *arg2)
{
	...
	case ASN1_PKEY_CTRL_SET1_TLS_ENCPT: {
		ECX_KEY *ecx = ossl_ecx_key_op(NULL, arg2, arg1,
					       pkey->ameth->pkey_id,
					       KEY_OP_PUBLIC, NULL, NULL);
	...
	case ASN1_PKEY_CTRL_GET1_TLS_ENCPT:
		if (pkey->pkey.ecx != NULL) {
			...
			*ppt = OPENSSL_memdup(pkey->pkey.ecx->pubkey,
					      KEYLEN(pkey));
			if (*ppt != NULL)
				return KEYLEN(pkey);
		}
			      ...
			      OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if (key->privkey != NULL) {
		if (!OSSL_PARAM_BLD_push_octet_string(
			    tmpl, OSSL_PKEY_PARAM_PRIV_KEY, key->privkey,
			    key->keylen))
			goto err;
...
static int ecd_item_sign(X509_ALGOR *alg1, X509_ALGOR *alg2, int nid)
{
	...
	if (alg2 != NULL)
		(void)X509_ALGOR_set0(alg2, OBJ_nid2obj(nid), V_ASN1_UNDEF,
				      NULL);
				      ...
				      size_t tbslen)
{
	...
	return ossl_ed25519_verify(tbs, tbslen, sig, edkey->pubkey, 0, 0, 0,
				   NULL, 0, edkey->libctx, edkey->propq);
}
...
static int s390x_pkey_ecd_keygen25519(EVP_PKEY_CTX *ctx, EVP_PKEY *pkey)
{
	...
	if (s390x_ed25519_mul(x_dst, pubkey, generator_x, generator_y, buff) !=
	    1)
		goto err;
...
{
	static const unsigned char generator_x[] = {
		0x5e, 0xc0, 0x0c, 0xc7, 0x2b, 0xa8, 0x26, 0x26, 0x8e, 0x93,
		0x00, 0x8b, 0xe1, 0x80, 0x3b, 0x43, 0x11, 0x65, 0xb6, 0x2a,
		0xf7, 0x1a, 0xae, 0x12, 0x64, 0xa4, 0xd3, 0xa3, 0x24, 0xe3,
		0x6d, 0xea, 0x67, 0x17, 0x0f, 0x47, 0x70, 0x65, 0x14, 0x9e,
		0xda, 0x36, 0xbf, 0x22, 0xa6, 0x15, 0x1d, 0x22, 0xed, 0x0d,
		0xed, 0x6b, 0xc6, 0x70, 0x19, 0x4f, 0x00
	};
	static const unsigned char generator_y[] = {
		0x14, 0xfa, 0x30, 0xf2, 0x5b, 0x79, 0x08, 0x98, 0xad, 0xc8,
		0xd7, 0x4e, 0x2c, 0x13, 0xbd, 0xfd, 0xc4, 0x39, 0x7c, 0xe6,
		0x1c, 0xff, 0xd3, 0x3a, 0xd7, 0xc2, 0xa0, 0x05, 0x1e, 0x9c,
		0x78, 0x87, 0x40, 0x98, 0xa3, 0x6c, 0x73, 0x73, 0xea, 0x4b,
		0x62, 0xc7, 0xc9, 0x56, 0x37, 0x20, 0x76, 0x88, 0x24, 0xbc,
		0xb6, 0x6e, 0x71, 0x46, 0x3f, 0x69, 0x00
	};
					  ...
					  size_t tbslen)
{
	...
	return s390x_kdsa(S390X_EDDSA_VERIFY_ED448, &param.ed448, tbs,
			  tbslen) == 0 ?
		       1 :
		       0;
}
