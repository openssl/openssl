...
static ossl_inline void getprivkey(unsigned char **priv, const EVP_PKEY *pk)
{
...
static ossl_inline void getpubkey(unsigned char **pub, const EVP_PKEY *pk)
{
    if (IS25519(pk->ameth->pkey_id)) {
        if (pk->pkey.curve25519 == NULL)
            return;
                      ...
                      ecx_key_op_t op)
{
    ...
    if (IS25519(id)) {
        key25519 = OPENSSL_zalloc(sizeof(*key25519));
        key = key25519;
    } else if(ISX448(id)) {
        keyx448 = OPENSSL_zalloc(sizeof(*keyx448));
        key = keyx448;
    } else {
        keyed448 = OPENSSL_zalloc(sizeof(*keyed448));
        key = keyed448;
    }
    if (key == NULL) {
    ...
    if (IS25519(id)) {
        privkey = &key25519->privkey;
        pubkey = key25519->pubkey;
    } else if(ISX448(id)) {
        privkey = &keyx448->privkey;
        pubkey = keyx448->pubkey;
    } else {
        privkey = &keyed448->privkey;
                                ...
                                size_t *keylen)
{
    ...
    if (peerkey == NULL) {
        ECerr(EC_F_PKEY_ECX_DERIVE25519, EC_R_INVALID_PEER_KEY);
        return 0;
    }
                              ...
                              size_t *keylen)
{
    ...
    if (peerkey == NULL) {
        ECerr(EC_F_PKEY_ECX_DERIVE448, EC_R_INVALID_PEER_KEY);
        return 0;
    }
    *keylen = X448_KEYLEN;
    if (key != NULL && X448(key, pkey->privkey, peerkey->pubkey) == 0)
        return 0;
    return 1;
}
