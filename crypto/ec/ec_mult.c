		     ...
		     const BIGNUM *scalars[], BN_CTX *ctx)
{
	...
	if (!BN_is_zero(group->order) && !BN_is_zero(group->cofactor)) {
         ...
         */
		if ((scalar != group->order) && (scalar != NULL) &&
		    (num == 0)) {
			/*-
             ...
             */
			return ossl_ec_scalar_mul_ladder(group, r, scalar, NULL,
							 ctx);
		}
		if ((scalar == NULL) && (num == 1) &&
		    (scalars[0] != group->order)) {
			/*-
             ...
             */
			return ossl_ec_scalar_mul_ladder(group, r, scalars[0],
							 points[0], ctx);
		}
	}
	...
	if (scalar != NULL) {
		...
		if (pre_comp && pre_comp->numblocks &&
			...
			if (pre_comp->num !=
			    (pre_comp->numblocks * pre_points_per_block)) {
	...
	if (wsize == NULL || wNAF_len == NULL || wNAF == NULL ||
	    val_sub == NULL)
		goto err;
	...
	if (numblocks) {
		...
		if (pre_comp == NULL) {
			...
			tmp_wNAF = bn_compute_wNAF(scalar, (int)wsize[num],
						   &tmp_len);
			if (!tmp_wNAF)
				goto err;
			...
			if (tmp_len <= max_len) {
				...
				numblocks = 1;
				totalnum =
					num + 1; /* don't use wNAF splitting */
				wNAF[num] = tmp_wNAF;
				wNAF[num + 1] = NULL;
	...
	for (i = 0; i < num + num_scalar; i++) {
		...
		if (wsize[i] > 1) {
			...
			for (j = 1; j < ((size_t)1 << (wsize[i] - 1)); j++) {
				if (!EC_POINT_add(group, val_sub[i][j],
						  val_sub[i][j - 1], tmp, ctx))
					goto err;
			}
		}
	}
			...
			if (wNAF_len[i] > (size_t)k) {
				...
				if (digit) {
					...
					if (r_is_at_infinity) {
                         ...
                         * group.
                         */
						if (!ossl_ec_point_blind_coordinates(
							    group, r, ctx)) {
							ERR_raise(
								ERR_LIB_EC,
...
int ossl_ec_wNAF_precompute_mult(EC_GROUP *group, BN_CTX *ctx)
{
	...
	for (i = 0; i < numblocks; i++) {
		...
		for (j = 1; j < pre_points_per_block; j++, var++) {
             ...
             */
			if (!EC_POINT_add(group, *var, tmp_point, *(var - 1),
					  ctx))
				goto err;
		}
