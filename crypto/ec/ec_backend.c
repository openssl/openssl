...
static int ec_check_group_type_name2id(const char *name)
{
	...
	for (i = 0, sz = OSSL_NELEM(check_group_type_nameid_map); i < sz; i++) {
		if (OPENSSL_strcasecmp(name,
				       check_group_type_nameid_map[i].ptr) == 0)
				    ...
				    unsigned char **genbuf)
{
	...
	if (tmpl != NULL || param_p != NULL || param_a != NULL ||
	    param_b != NULL) {
		...
		if (!ossl_param_build_set_bn(tmpl, params, OSSL_PKEY_PARAM_EC_P,
					     p) ||
		    !ossl_param_build_set_bn(tmpl, params, OSSL_PKEY_PARAM_EC_A,
					     a) ||
		    !ossl_param_build_set_bn(tmpl, params, OSSL_PKEY_PARAM_EC_B,
					     b)) {
	...
	if (tmpl != NULL || param != NULL) {
		...
		if (!ossl_param_build_set_bn(tmpl, params,
	...
	if (tmpl != NULL || param != NULL) {
		if (!ossl_param_build_set_utf8_string(
			    tmpl, params, OSSL_PKEY_PARAM_EC_FIELD_TYPE,
			    field_type)) {
			ERR_raise(ERR_LIB_EC, ERR_R_CRYPTO_LIB);
			goto err;
		}
	...
	if (tmpl != NULL || param != NULL) {
		...
		genbuf_len = EC_POINT_point2buf(group, genpt, genform, genbuf,
						bnctx);
		if (genbuf_len == 0) {
			ERR_raise(ERR_LIB_EC, EC_R_INVALID_GENERATOR);
			goto err;
		...
		if (!ossl_param_build_set_octet_string(
			    tmpl, params, OSSL_PKEY_PARAM_EC_GENERATOR, *genbuf,
			    genbuf_len)) {
			ERR_raise(ERR_LIB_EC, ERR_R_CRYPTO_LIB);
			goto err;
		}
			 ...
			 unsigned char **genbuf)
{
	...
	if (encoding_name == NULL ||
	    !ossl_param_build_set_utf8_string(
		    tmpl, params, OSSL_PKEY_PARAM_EC_ENCODING, encoding_name)) {
	...
	if (tmpl == NULL || curve_nid == NID_undef)
		if (!ec_group_explicit_todata(group, tmpl, params, bnctx,
					      genbuf))
			goto err;
	...
	if (curve_nid != NID_undef) {
		...
		if (curve_name == NULL ||
		    !ossl_param_build_set_utf8_string(
			    tmpl, params, OSSL_PKEY_PARAM_GROUP_NAME,
			    curve_name)) {
			ERR_raise(ERR_LIB_EC, EC_R_INVALID_CURVE);
			goto err;
		}
			 ...
			 int include_private)
{
	...
	if (include_private)
		param_priv_key = OSSL_PARAM_locate_const(
			params, OSSL_PKEY_PARAM_PRIV_KEY);
	...
	if (param_pub_key != NULL)
		if (!OSSL_PARAM_get_octet_string(param_pub_key,
		    ...
		    !EC_POINT_oct2point(ecg, pub_point, pub_key, pub_key_len,
					ctx))
			goto err;
...
EC_KEY *ossl_ec_key_dup(const EC_KEY *src, int selection)
{
	...
	if (src->priv_key != NULL &&
		...
		if (ret->priv_key == NULL ||
		    !BN_copy(ret->priv_key, src->priv_key))
			goto err;
...
#ifndef FIPS_MODULE
	if (!CRYPTO_dup_ex_data(CRYPTO_EX_INDEX_EC_KEY, &ret->ex_data,
				&src->ex_data))
		goto err;
#endif
					  ...
					  const char *propq)
{
	...
	if (ptype == V_ASN1_SEQUENCE) {
		...
		group = EC_GROUP_new_by_curve_name_ex(libctx, propq,
						      OBJ_obj2nid(poid));
		if (group == NULL)
			goto ecerr;
