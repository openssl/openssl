    ...
    const BIGNUM *b, BN_CTX *ctx)
{
    ...
    /*
     * Build the generic field Montgomery context used by the constant-time
     * OSSL_FN point arithmetic.  Done here, the universal sink for GF(p) field
     * setup (every GF(p) method's group_set_curve delegates to this one), so
     * that named-curve construction - which calls meth->group_set_curve
