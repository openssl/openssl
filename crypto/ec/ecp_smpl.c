...
int ossl_ec_GFp_simple_group_set_curve(EC_GROUP *group, const BIGNUM *p,
				       ...
				       BIGNUM *a, BIGNUM *b, BN_CTX *ctx)
{
	...
	if (a != NULL || b != NULL) {
		if (group->meth->field_decode != NULL) {
			...
			if (a != NULL) {
				if (!group->meth->field_decode(group, a,
							       group->a, ctx))
					goto err;
			}
			if (b != NULL) {
				if (!group->meth->field_decode(group, b,
							       group->b, ctx))
					goto err;
			}
		} else {
...
int ossl_ec_GFp_simple_set_Jprojective_coordinates_GFp(
	const EC_GROUP *group, EC_POINT *point, const BIGNUM *x,
	const BIGNUM *y, const BIGNUM *z, BN_CTX *ctx)
{
	...
	if (x != NULL) {
		...
		if (group->meth->field_encode) {
			if (!group->meth->field_encode(group, point->X,
						       point->X, ctx))
				goto err;
		}
	}
	...
	if (y != NULL) {
		if (!BN_nnmod(point->Y, y, group->field, ctx))
			goto err;
		if (group->meth->field_encode) {
			if (!group->meth->field_encode(group, point->Y,
						       point->Y, ctx))
				goto err;
		}
	}
	...
	if (z != NULL) {
		...
		if (group->meth->field_encode) {
			if (Z_is_one && (group->meth->field_set_to_one != 0)) {
				if (!group->meth->field_set_to_one(
					    group, point->Z, ctx))
					goto err;
			} else {
				if (!group->meth->field_encode(group, point->Z,
							       point->Z, ctx))
					goto err;
			}
		}
						    ...
						    BN_CTX *ctx)
{
	...
	if (BN_is_one(Z_)) {
		if (group->meth->field_decode != NULL) {
			if (x != NULL) {
				if (!group->meth->field_decode(group, x,
							       point->X, ctx))
					goto err;
			}
			if (y != NULL) {
				if (!group->meth->field_decode(group, y,
							       point->Y, ctx))
					goto err;
			}
		} else {
		...
		if (x != NULL) {
             ...
             * factor in X:
             */
			if (!group->meth->field_mul(group, x, point->X, Z_2,
						    ctx))
				goto err;
		}
		...
		if (y != NULL) {
			if (group->meth->field_encode == NULL) {
                 ...
                 */
				if (!group->meth->field_mul(group, Z_3, Z_2,
							    Z_1, ctx))
					goto err;
			} else {
				if (!BN_mod_mul(Z_3, Z_2, Z_1, group->field,
						ctx))
					goto err;
			}
             ...
             * factor in Y:
             */
			if (!group->meth->field_mul(group, y, point->Y, Z_3,
						    ctx))
				goto err;
		}
	}
...
int ossl_ec_GFp_simple_add(const EC_GROUP *group, EC_POINT *r,
...
int ossl_ec_GFp_simple_dbl(const EC_GROUP *group, EC_POINT *r,
					  ...
					  EC_POINT *points[], BN_CTX *ctx)
{
	...
	for (i = num - 1; i > 0; --i) {
		...
		if (!BN_is_zero(points[i]->Z)) {
             ...
             */
			if (!group->meth->field_mul(group, tmp_Z, prod_Z[i - 1],
						    tmp, ctx))
				goto err;
			/*
	...
	for (i = 0; i < num; i++) {
		...
		if (!BN_is_zero(p->Z)) {
			...
			if (!group->meth->field_sqr(group, tmp, p->Z, ctx))
				goto err;
			if (!group->meth->field_mul(group, p->X, p->X, tmp,
						    ctx))
				goto err;

			if (!group->meth->field_mul(group, tmp, tmp, p->Z, ctx))
				goto err;
			if (!group->meth->field_mul(group, p->Y, p->Y, tmp,
						    ctx))
				goto err;
...
int ossl_ec_GFp_simple_field_mul(const EC_GROUP *group, BIGNUM *r,
				 const BIGNUM *a, const BIGNUM *b, BN_CTX *ctx)
{
	return BN_mod_mul(r, a, b, group->field, ctx);
}

int ossl_ec_GFp_simple_field_sqr(const EC_GROUP *group, BIGNUM *r,
 ...
 */
int ossl_ec_GFp_simple_ladder_pre(const EC_GROUP *group, EC_POINT *r,
 ...
 */
int ossl_ec_GFp_simple_ladder_step(const EC_GROUP *group, EC_POINT *r,
				   EC_POINT *s, EC_POINT *p, BN_CTX *ctx)
{
	int ret = 0;
	...
	if (t6 == NULL || !group->meth->field_mul(group, t6, r->X, s->X, ctx) ||
 ...
 */
int ossl_ec_GFp_simple_ladder_post(const EC_GROUP *group, EC_POINT *r,
				   EC_POINT *s, EC_POINT *p, BN_CTX *ctx)
{
	int ret = 0;
	BIGNUM *t0, *t1, *t2, *t3, *t4, *t5, *t6 = NULL;
	...
	if (t6 == NULL || !BN_mod_lshift1_quick(t4, p->Y, group->field) ||
