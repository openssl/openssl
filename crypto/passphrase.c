...
{
	OPENSSL_clear_free(data->cached_passphrase,
			   ...
			   struct ossl_passphrase_data_st *data)
{
	...
	} else if (data->flag_cache_passphrase &&
		   data->cached_passphrase != NULL) {
	...
	if (data->type == is_ossl_passphrase) {
		OSSL_PASSPHRASE_CALLBACK *cb =
			data->_.ossl_passphrase.passphrase_cb;
				...
				const char *info)
{
	...
	OSSL_PARAM params[] = { OSSL_PARAM_utf8_string(
					OSSL_PASSPHRASE_PARAM_INFO, NULL, 0),
				OSSL_PARAM_END };
...
int ossl_pw_passphrase_callback_enc(char *pass, size_t pass_size,
				    ...
				    void *arg)
{
	return ossl_pw_get_passphrase(pass, pass_size, pass_len, params, 1,
				      arg);
}
...
int ossl_pw_passphrase_callback_dec(char *pass, size_t pass_size,
				    ...
				    void *arg)
{
	return ossl_pw_get_passphrase(pass, pass_size, pass_len, params, 0,
				      arg);
}
