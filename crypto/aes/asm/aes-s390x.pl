...
$code.=<<___ if (!$softonly);
	...
	llihh	%r0,0x8000
	srlg	%r0,%r0,0(%r5)
	ng	%r0,56(%r1)	# check availability of both km...
	ng	%r0,72(%r1)	# ...and kmc support for given key length
	jz	.Lekey_internal
...
$code.=<<___ if (!$softonly && 0);# kmctr code was measured to be ~12% slower
	...
	srlg	%r0,%r0,0($s0)
	ng	%r0,104(%r1)	# check kmctr capability vector
	lgr	%r0,$s0
	lgr	%r1,$s1
	jz	.Lctr32_km_loop
...
$code.=<<___ if(1);
	...
	ng	%r0,56(%r1)		# check km capability vector
