				...
				size_t rnd_len, uint8_t *out_sig)
{
     ...
     */
	w1_encoded_len =
		k * (gamma2 == ML_DSA_GAMMA2_Q_MINUS1_DIV88 ? 192 : 128);
	...
	if (!shake_xof_3(md_ctx, priv->shake256_md, priv->K, sizeof(priv->K),
			 rnd, rnd_len, mu, mu_len, rho_prime,
			 sizeof(rho_prime)))
		goto err;
	...
	for (kappa = 0;; kappa += l) {
		...
		vector_expand_mask(&y, rho_prime, sizeof(rho_prime),
				   (uint32_t)kappa, gamma1, md_ctx,
				   priv->shake256_md);
		vector_copy(y_ntt, &y);
		vector_ntt(y_ntt);
		...
		if (!shake_xof_2(md_ctx, priv->shake256_md, mu, mu_len,
				 w1_encoded, w1_encoded_len, c_tilde,
				 c_tilde_len))
			break;
		     ...
		     unsigned char *sig, size_t *sig_len, size_t sig_size)
{
	...
	if (msg_is_mu) {
		...
		md_ctx =
			ossl_ml_dsa_mu_init(priv, encode, context, context_len);
		if (md_ctx == NULL)
			return 0;
