...
int ossl_ml_dsa_pk_decode(ML_DSA_KEY *key, const uint8_t *in, size_t in_len)
{
	...
	if (!shake_xof(ctx, key->shake256_md, in, in_len, key->tr,
		       sizeof(key->tr)))
		goto err;
...
int ossl_ml_dsa_poly_decode_expand_mask(POLY *out, const uint8_t *in,
					size_t in_len, uint32_t gamma1)
{
	PACKET pkt;
 ...
 * @returns 1 if the signature was encoded successfully or 0 otherwise.
 */
int ossl_ml_dsa_w1_encode(const VECTOR *w1, uint32_t gamma2, uint8_t *out,
			  size_t out_len)
{
	WPACKET pkt;
	ENCODE_FN *encode_fn;
