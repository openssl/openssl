...
static ossl_inline ossl_unused int vector_secure_alloc(VECTOR *v,
						       size_t num_polys)
{
		   ...
		   const EVP_MD *md)
{
	...
	for (i = 0; i < out->num_poly; i++) {
		...
		poly_expand_mask(out->poly + i, derived_seed,
				 sizeof(derived_seed), gamma1, h_ctx, md);
	}
}
		 ...
		 uint32_t gamma2, VECTOR *out)
{
	...
	for (i = 0; i < out->num_poly; i++)
		poly_make_hint(ct0->poly + i, cs2->poly + i, w->poly + i,
			       gamma2, out->poly + i);
}
