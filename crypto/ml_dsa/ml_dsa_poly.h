...
{
	if (!ossl_ml_dsa_poly_sample_in_ball(out, seed, seed_len, h_ctx, md,
					     tau))
		return 0;
	poly_ntt(out);
	return 1;
						      ...
						      POLY *t0)
{
	...
	for (i = 0; i < ML_DSA_NUM_POLY_COEFFICIENTS; i++)
		ossl_ml_dsa_key_compress_power2_round(
			t->coeff[i], t1->coeff + i, t0->coeff + i);
}
						   ...
						   uint32_t gamma2, POLY *out)
{
	...
	for (i = 0; i < ML_DSA_NUM_POLY_COEFFICIENTS; i++)
		out->coeff[i] = ossl_ml_dsa_key_compress_high_bits(in->coeff[i],
								   gamma2);
}
						  ...
						  uint32_t gamma2, POLY *out)
{
	...
	for (i = 0; i < ML_DSA_NUM_POLY_COEFFICIENTS; i++)
		out->coeff[i] =
			ossl_ml_dsa_key_compress_low_bits(in->coeff[i], gamma2);
}
						  ...
						  uint32_t gamma2, POLY *out)
{
	...
	for (i = 0; i < ML_DSA_NUM_POLY_COEFFICIENTS; i++)
		out->coeff[i] = ossl_ml_dsa_key_compress_use_hint(
			h->coeff[i], r->coeff[i], gamma2);
}
