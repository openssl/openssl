			   ...
			   const uint8_t *sk, size_t sk_len)
{
	...
	if (key == NULL || key->pub_encoding != NULL ||
...
void ossl_ml_dsa_key_reset(ML_DSA_KEY *key)
{
	...
	if (key->priv_encoding != NULL)
		OPENSSL_secure_clear_free(key->priv_encoding,
					  key->params->sk_len);
	key->priv_encoding = NULL;
	if (key->seed != NULL)
			...
			if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) !=
				...
				if (src->priv_encoding != NULL) {
					...
					if (ret->priv_encoding == NULL)
						goto err;
					memcpy(ret->priv_encoding,
					       src->priv_encoding,
					       src->params->sk_len);
				}
				if (src->seed != NULL) {
					ret->seed = OPENSSL_secure_malloc(
						ML_DSA_SEED_BYTES);
					if (ret->seed == NULL)
						goto err;
					memcpy(ret->seed, src->seed,
					       ML_DSA_SEED_BYTES);
				}
			}
		}
	...
	if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
		if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) {
			if (key1->pub_encoding != NULL &&
			    key2->pub_encoding != NULL) {
				if (memcmp(key1->pub_encoding,
					   key2->pub_encoding,
		...
		if (!key_checked &&
			...
			if (key1->priv_encoding != NULL &&
			    key2->priv_encoding != NULL) {
				if (memcmp(key1->priv_encoding,
					   key2->priv_encoding,
...
int ossl_ml_dsa_key_public_from_private(ML_DSA_KEY *key)
{
	...
	if (!vector_alloc(
		    &t0, key->params->k)) /* t0 is already in the private key */
		return 0;
	ret = ((md_ctx = EVP_MD_CTX_new()) != NULL) &&
	      ...
	      vector_equal(
		      &t0,
		      &key->t0) /* compare the generated t0 to the expected */
	      && ossl_ml_dsa_pk_encode(key) &&
	      shake_xof(md_ctx, key->shake256_md, key->pub_encoding,
			key->params->pk_len, key->tr, sizeof(key->tr));
	vector_free(&t0);
...
static int keygen_internal(ML_DSA_KEY *out)
{
	...
	uint8_t expanded_seed[ML_DSA_RHO_BYTES + ML_DSA_PRIV_SEED_BYTES +
			      ML_DSA_K_BYTES];
	...
	if (!shake_xof(md_ctx, out->shake256_md, augmented_seed,
		       sizeof(augmented_seed), expanded_seed,
		       sizeof(expanded_seed)))
		goto err;

	memcpy(out->rho, rho, sizeof(out->rho));
	memcpy(out->K, K, sizeof(out->K));

	ret = vector_expand_S(md_ctx, out->shake256_md, params->eta, priv_seed,
			      &out->s1, &out->s2) &&
	      public_from_private(out, md_ctx, &out->t1, &out->t0) &&
	      ossl_ml_dsa_pk_encode(out) &&
	      shake_xof(md_ctx, out->shake256_md, out->pub_encoding,
			out->params->pk_len, out->tr, sizeof(out->tr)) &&
	      ossl_ml_dsa_sk_encode(out);

err:
	if (out->seed != NULL &&
	    (out->prov_flags & ML_DSA_KEY_RETAIN_SEED) == 0) {
...
int ossl_ml_dsa_generate_key(ML_DSA_KEY *out)
{
	...
	if (out->seed == NULL) {
		if ((out->seed = OPENSSL_secure_malloc(seed_len)) == NULL)
			return 0;
		if (RAND_priv_bytes_ex(out->libctx, out->seed, seed_len, 0) <=
		    0) {
	...
	if (sk == NULL) {
		...
		if ((ret = keygen_internal(out)) != 0 &&
			...
			ERR_raise_data(
				ERR_LIB_PROV, PROV_R_INVALID_KEY,
