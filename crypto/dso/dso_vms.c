			  ...
			  unsigned long flags)
{
	...
	if (ptr->imagename_dsc.dsc$w_length)
		return lib$find_image_symbol(&ptr->filename_dsc, symname_dsc,
					     sym, &ptr->imagename_dsc, flags);
	else
		return lib$find_image_symbol(&ptr->filename_dsc, symname_dsc,
					     sym, 0, flags);
}
...
#define SYMNAME ((char *)symname)
	...
	*sym = NULL;
	...
	ptr = (DSO_VMS_INTERNAL *)sk_void_value(
		dso->meth_data, sk_void_num(dso->meth_data) - 1);
	if (ptr == NULL) {
		ERR_raise(ERR_LIB_DSO, DSO_R_NULL_HANDLE);
		return;
	}
		...
		if (!$VMS_STATUS_SUCCESS(status))
			...
			if (ptr->imagename_dsc.dsc$w_length)
				ERR_raise_data(ERR_LIB_DSO, DSO_R_SYM_FAILURE,
					       "Symbol %s in %s (%s): %s",
					       symname, ptr->filename,
					       ptr->imagename, errstring);
			else
				ERR_raise_data(ERR_LIB_DSO, DSO_R_SYM_FAILURE,
					       "Symbol %s in %s: %s", symname,
					       ptr->filename, errstring);
		}
		return;
	}
	return;
