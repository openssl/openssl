...
static DSO_METHOD dso_meth_win32 = { "OpenSSL 'win32' shared library method",
				     win32_load,
				     win32_unload,
				     win32_bind_func,
				     NULL, /* ctrl */
...
static int win32_load(DSO *dso)
{
	...
	if (h == NULL) {
		ERR_raise_data(ERR_LIB_DSO, DSO_R_LOAD_FAILED, "filename(%s)",
			       filename);
		goto err;
	}
...
static DSO_FUNC_TYPE win32_bind_func(DSO *dso, const char *symname)
{
	...
	if (sym.p == NULL) {
		ERR_raise_data(ERR_LIB_DSO, DSO_R_SYM_FAILURE, "symname(%s)",
			       symname);
		return NULL;
	}
...
static char *win32_joiner(DSO *dso, const struct file_st *file_split)
{
	...
	while (file_split->predirlen > (start - file_split->predir)) {
		const char *end = openssl_strnchr(
			start, '/',
...
static char *win32_name_converter(DSO *dso, const char *filename)
{
	...
	BIO_snprintf(translated, len, "%s%s", filename,
		     transform ? ".dll" : "");
	return translated;
}
...
static int win32_pathbyaddr(void *addr, char *path, int sz)
{
	...
	create_snap = (CREATETOOLHELP32SNAPSHOT)GetProcAddress(
	...
	do {
		if ((size_t)addr >= (size_t)me32.modBaseAddr &&
		    (size_t)addr <
			    (size_t)(me32.modBaseAddr + me32.modBaseSize)) {
			(*close_snap)(hModuleSnap);
			FreeLibrary(dll);
#ifdef _WIN32_WCE
#if _WIN32_WCE >= 101
			return WideCharToMultiByte(CP_ACP, 0, me32.szExePath,
						   -1, path, sz, NULL, NULL);
#else
			{
