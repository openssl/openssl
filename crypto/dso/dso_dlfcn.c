...
static char *dlfcn_name_converter(DSO *dso, const char *filename)
{
	...
	if (transform) {
		if ((DSO_flags(dso) & DSO_FLAG_NAME_TRANSLATION_EXT_ONLY) == 0)
			BIO_snprintf(translated, rsize, "lib%s" DSO_EXTENSION,
				     filename);
		else
			BIO_snprintf(translated, rsize, "%s" DSO_EXTENSION,
				     filename);
	} else {
	...
	do {
		...
		if (((addr >= (uintptr_t)this_ldi->ldinfo_textorg) &&
			...
			if ((buffer = OPENSSL_malloc(buffer_sz)) != NULL) {
				OPENSSL_strlcpy(buffer,
						this_ldi->ldinfo_filename,
						buffer_sz);
				if (member_len > 0) {
					/*
