		   ...
		   const OSSL_PARAM param[])
{
	...
	if (cipher != NULL) {
		...
		ctx->nlast_block = -1;
		if (impl != NULL) {
			if (!EVP_EncryptInit_ex(ctx->cctx, cipher, impl, NULL,
						NULL))
				return 0;
		} else {
			if (!EVP_EncryptInit_ex2(ctx->cctx, cipher, NULL, NULL,
						 param))
				return 0;
		}
	}
...
int CMAC_Update(CMAC_CTX *ctx, const void *in, size_t dlen)
{
	...
	if (max_burst_blocks == 0) {
		...
		while (cipher_blocks > max_burst_blocks) {
			if (EVP_Cipher(ctx->cctx, buf, data,
				       (int)(max_burst_blocks * bl)) <= 0)
				return 0;
