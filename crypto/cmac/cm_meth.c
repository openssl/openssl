...
static int cmac_reset(EVP_MAC_IMPL *cctx)
{
    return CMAC_resume(cctx->ctx);
}

static EVP_MAC_IMPL *cmac_copy(EVP_MAC_IMPL *csrc)
{
    EVP_MAC_IMPL *cdst = NULL;

    if ((cdst = cmac_new()) == NULL
        || !CMAC_CTX_copy(cdst->ctx, csrc->ctx)) {
        cmac_free(cdst);
        return NULL;
    }
