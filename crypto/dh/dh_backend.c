...
static int dh_ffc_params_fromdata(DH *dh, const OSSL_PARAM params[])
{
	...
	if (ret)
		ossl_dh_cache_named_group(
			dh); /* This increments dh->dirty_cnt */
	return ret;
}
...
int ossl_dh_params_todata(DH *dh, OSSL_PARAM_BLD *bld, OSSL_PARAM params[])
{
	...
	if (!ossl_ffc_params_todata(ossl_dh_get0_params(dh), bld, params))
		return 0;
	if (l > 0 && !ossl_param_build_set_long(bld, params,
						OSSL_PKEY_PARAM_DH_PRIV_LEN, l))
		return 0;
	return 1;
}
...
#ifndef FIPS_MODULE
	if (!CRYPTO_dup_ex_data(CRYPTO_EX_INDEX_DH, &dupkey->ex_data,
				&dh->ex_data))
		goto err;
#endif
