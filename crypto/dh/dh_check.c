	...
	if (q_good) {
		if (BN_cmp(dh->params.g, BN_value_one()) <= 0)
			...
			*ret |= DH_NOT_SUITABLE_GENERATOR;
			...
			/* Check g^q == 1 mod p */
			if (!BN_mod_exp(t1, dh->params.g, dh->params.q,
					dh->params.p, ctx))
				goto err;
			if (!BN_is_one(t1))
				*ret |= DH_NOT_SUITABLE_GENERATOR;
...
{
	return ossl_ffc_validate_public_key_partial(&dh->params, pub_key,
						    ret) &&
	       *ret == 0;
}
...
int ossl_dh_check_pairwise(const DH *dh, int return_on_null_numbers)
{
	...
	if (dh->params.p == NULL || dh->params.g == NULL ||
	    dh->priv_key == NULL || dh->pub_key == NULL)
