				    ...
				    BN_GENCB *cb)
{
	...
	if (type == DH_PARAMGEN_TYPE_FIPS_186_2)
		ret = ossl_ffc_params_FIPS186_2_generate(
			dh->libctx, &dh->params, FFC_PARAM_TYPE_DH, pbits,
			qbits, &res, cb);
	else
#endif
			      ...
			      BN_GENCB *cb)
{
	...
	if (ret->meth->generate_params)
		return ret->meth->generate_params(ret, prime_len, generator,
						  cb);
				...
				BN_GENCB *cb)
{
	...
	if (!BN_generate_prime_ex2(ret->params.p, prime_len, 1, t1, t2, cb,
				   ctx))
		goto err;
	...
	ret->length = (2 * ossl_ifc_ffc_compute_security_bits(prime_len) + 24) /
		      25 * 25;
	ret->dirty_cnt++;
	ok = 1;
err:
