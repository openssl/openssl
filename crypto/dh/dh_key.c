...
int ossl_dh_compute_key(unsigned char *key, const BIGNUM *pub_key, DH *dh)
{
	...
	if (!dh->meth->bn_mod_exp(dh, z, pub_key, dh->priv_key, dh->params.p,
				  ctx, mont)) {
	...
	if (BN_copy(pminus1, dh->params.p) == NULL ||
	    !BN_sub_word(pminus1, 1) || BN_cmp(z, BN_value_one()) <= 0 ||
	    BN_cmp(z, pminus1) == 0) {
				...
				const BIGNUM *priv_key, BIGNUM *pub_key)
{
	...
	if (dh->flags & DH_FLAG_CACHE_MONT_P) {
		...
		mont = BN_MONT_CTX_set_locked(pmont, dh->lock, dh->params.p,
					      ctx);
		if (mont == NULL)
			goto err;
	}
	...
	if (generate_new_key) {
		...
		if (DH_get_nid(dh) != NID_undef) {
			...
			if (!ossl_ffc_generate_private_key(
				    ctx, &dh->params, dh->length, max_strength,
				    priv_key))
				goto err;
		} else {
#ifdef FIPS_MODULE
