				    ...
				    const char *buf, size_t bsize)
{
	...
	pd = param_push(bld, key, bsize, bsize + 1, OSSL_PARAM_UTF8_STRING,
			secure);
	if (pd == NULL)
		return 0;
	pd->string = buf;
				     ...
				     const void *buf, size_t bsize)
{
	...
	secure = CRYPTO_secure_allocated(buf);
	pd = param_push(bld, key, bsize, bsize, OSSL_PARAM_OCTET_STRING,
			secure);
	if (pd == NULL)
		return 0;
	pd->string = buf;
	...
	for (i = 0; i < num; i++) {
		...
		if (pd->bn != NULL) {
			...
			if (pd->type == OSSL_PARAM_UNSIGNED_INTEGER)
				BN_bn2nativepad(pd->bn, (unsigned char *)p,
						(int)pd->size);
			else
				BN_signed_bn2native(pd->bn, (unsigned char *)p,
						    (int)pd->size);
...
OSSL_PARAM *OSSL_PARAM_BLD_to_param(OSSL_PARAM_BLD *bld)
{
	...
	const size_t total =
		OSSL_PARAM_ALIGN_SIZE * (p_blks + bld->total_blocks);
