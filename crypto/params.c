...
static int general_get_int(const OSSL_PARAM *p, void *val, size_t val_size)
{
	...
	if (p->data_type == OSSL_PARAM_UNSIGNED_INTEGER)
		return signed_from_unsigned(val, val_size, p->data,
					    p->data_size);
	err_not_integer;
	return 0;
}
...
static int general_get_uint(const OSSL_PARAM *p, void *val, size_t val_size)
{
	...
	if (p->data_type == OSSL_PARAM_INTEGER)
		return unsigned_from_signed(val, val_size, p->data,
					    p->data_size);
	if (p->data_type == OSSL_PARAM_UNSIGNED_INTEGER)
		return unsigned_from_unsigned(val, val_size, p->data,
					      p->data_size);
	err_not_integer;
	return 0;
}
...
static int general_set_uint(OSSL_PARAM *p, void *val, size_t val_size)
{
	...
	if (p->data_type == OSSL_PARAM_INTEGER)
		...
		r = unsigned_from_unsigned(p->data, p->data_size, val,
					   val_size);
	else
		err_not_integer;
	p->return_size = r ? p->data_size : val_size;
...
int OSSL_PARAM_set_BN(OSSL_PARAM *p, const BIGNUM *val)
{
	...
	if (p->data_size >= bytes) {
		switch (p->data_type) {
		case OSSL_PARAM_UNSIGNED_INTEGER:
			if (BN_bn2nativepad(val, p->data, (int)p->data_size) <
			    0) {
				ERR_raise(ERR_LIB_CRYPTO,
			...
			break;
		case OSSL_PARAM_INTEGER:
			if (BN_signed_bn2native(val, p->data,
						(int)p->data_size) < 0) {
				ERR_raise(ERR_LIB_CRYPTO,
					  CRYPTO_R_INTEGER_OVERFLOW);
				return 0;
...
{
	return get_ptr_internal(p, (const void **)val, NULL,
				OSSL_PARAM_UTF8_PTR);
}
			     ...
			     size_t *outlen)
{
	...
	for (i = 0; i < n; i++) {
		if (p[i]->data_type != OSSL_PARAM_OCTET_STRING)
			goto err;
		if (p[i]->data != NULL && p[i]->data_size != 0 &&
				   ...
				   unsigned int type)
{
	...
	if (p->data_type == ref_type)
		return get_ptr_internal_skip_checks(p, (const void **)val,
						    used_len);
