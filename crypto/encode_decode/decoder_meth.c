				  ...
				  OSSL_PROVIDER *prov)
{
	...
	if ((decoder->base.parsed_propdef = ossl_parse_property(
					      ...
					      const char *properties)
{
	...
	if (id == 0 ||
		...
		if ((method = ossl_method_construct(
			     methdata->libctx, OSSL_OP_DECODER, &prov,
			...
			if (id != 0)
				ossl_method_store_cache_set(store, prov, id,
							    propq, method,
							    up_ref_decoder,
							    free_decoder);
		}
			      ...
			      void *data)
{
	...
	if (decoder->base.prov != NULL) {
		...
		return ossl_namemap_doall_names(namemap, decoder->base.id, fn,
						data);
	}
