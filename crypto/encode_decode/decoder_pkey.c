				  ...
				  void *construct_data)
{
	...
	if (keymgmt != NULL) {
         ...
         */
		if (keymgmt_prov == decoder_prov) {
			keydata = evp_keymgmt_load(keymgmt, object_ref,
						   object_ref_sz);
		} else {
             ...
             */
			(void)decoder->export_object(
				decoderctx, object_ref, object_ref_sz,
				   ...
				   void *provctx, struct collect_data_st *data)
{
	...
	if (data->ctx->start_input_type != NULL && di->input_type != NULL &&
	    OPENSSL_strcasecmp(di->input_type, data->ctx->start_input_type) !=
		    0 &&
...
static int check_keymgmt(EVP_KEYMGMT *keymgmt, struct collect_data_st *data)
{
	...
	if (!data->keytype_resolved) {
		...
		data->keytype_id =
			ossl_namemap_name2num(namemap, data->keytype);
			      ...
			      const char *input_structure)
{
	...
	struct decoder_pkey_data_st *process_data_src,
	...
	if (!OSSL_DECODER_CTX_set_construct(
		    dest, OSSL_DECODER_CTX_get_construct(src))) {
		ERR_raise(ERR_LIB_OSSL_DECODER, ERR_R_OSSL_DECODER_LIB);
		goto err;
	}
	...
	if (process_data_src != NULL) {
		...
		if (process_data_src->propq != NULL) {
			process_data_dest->propq =
				OPENSSL_strdup(process_data_src->propq);
		...
		if (process_data_src->keymgmts != NULL) {
			process_data_dest->keymgmts = sk_EVP_KEYMGMT_deep_copy(
				process_data_src->keymgmts, keymgmt_dup,
				EVP_KEYMGMT_free);
		...
		if (!OSSL_DECODER_CTX_set_construct_data(dest,
							 process_data_dest)) {
			ERR_raise(ERR_LIB_OSSL_DECODER, ERR_R_OSSL_DECODER_LIB);
			goto err;
		}
...
static unsigned long decoder_cache_entry_hash(const DECODER_CACHE_ENTRY *cache)
{
	...
	hash = (hash * 23) + (cache->propquery == NULL ?
				      0 :
	...
	hash = (hash * 23) + (cache->input_type == NULL ?
				      0 :
				      ossl_lh_strcasehash(cache->input_type));
	hash = (hash * 23) + (cache->keytype == NULL ?
				      0 :
...
void ossl_decoder_cache_free(void *vcache)
{
	...
	lh_DECODER_CACHE_ENTRY_doall(cache->hashtable,
			      ...
			      const char *propquery)
{
	...
	if (res == NULL) {
		...
		if (OSSL_DECODER_CTX_set_input_type(ctx, input_type) &&
		    OSSL_DECODER_CTX_set_input_structure(ctx,
							 input_structure) &&
		    OSSL_DECODER_CTX_set_selection(ctx, selection) &&
		    ossl_decoder_ctx_setup_for_pkey(ctx, keytype, libctx,
						    propquery) &&
		...
		res = lh_DECODER_CACHE_ENTRY_retrieve(cache->hashtable,
						      &cacheent);
		if (res == NULL) {
			(void)lh_DECODER_CACHE_ENTRY_insert(cache->hashtable,
							    newcache);
	...
	ctx = ossl_decoder_ctx_for_pkey_dup(ctx, pkey, input_type,
					    input_structure);
	CRYPTO_THREAD_unlock(cache->lock);
