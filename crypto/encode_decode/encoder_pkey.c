				...
				const char *propquery)
{
	...
	params[1] = OSSL_PARAM_construct_utf8_string(
		OSSL_ENCODER_PARAM_PROPERTIES, (void *)propquery, 0);
		...
		for (i = 0; i < end_i; i++) {
			if (data->flag_find_same_provider)
				...
				match = OSSL_ENCODER_is_a(
					encoder, sk_OPENSSL_CSTRING_value(
							 data->names, i));
			if (!match ||
			    (encoder->does_selection != NULL &&
			     !encoder->does_selection(provctx,
						      data->ctx->selection)) ||
...
static int encoder_import_cb(const OSSL_PARAM params[], void *arg)
{
	...
	construct_data->constructed_obj = encoder->import_object(
		encoderctx, construct_data->selection, params);

	return (construct_data->constructed_obj != NULL);
}

static const void *encoder_construct_pkey(OSSL_ENCODER_INSTANCE *encoder_inst,
					  void *arg)
{
	...
	if (data->obj == NULL) {
		...
		const OSSL_PROVIDER *k_prov =
			EVP_KEYMGMT_get0_provider(pk->keymgmt);
		...
		if (k_prov != e_prov) {
			...
			if (!evp_keymgmt_export(pk->keymgmt, pk->keydata,
						selection, &encoder_import_cb,
						data))
				return NULL;
					   ...
					   const char *propquery)
{
	...
	if (pkey->keymgmt != NULL) {
		...
		EVP_KEYMGMT_names_do_all(pkey->keymgmt, collect_name,
					 &keymgmt_data);
		...
		if (end > 0) {
			encoder_data.id_names =
				OPENSSL_malloc_array(end, sizeof(int));
			if (encoder_data.id_names == NULL) {
				sk_OPENSSL_CSTRING_free(keymgmt_data.names);
				goto err;
			}
			for (i = 0; i < end; ++i) {
				const char *name = sk_OPENSSL_CSTRING_value(
					keymgmt_data.names, i);

				encoder_data.id_names[i] =
					ossl_namemap_name2num(namemap, name);
			}
		}
		/*
		...
		OSSL_ENCODER_do_all_provided(libctx, collect_encoder,
					     &encoder_data);
	...
	if (data != NULL && OSSL_ENCODER_CTX_get_num_encoders(ctx) != 0) {
		if (!OSSL_ENCODER_CTX_set_construct(ctx,
						...
						const char *propquery)
{
	...
	if (!evp_pkey_is_assigned(pkey)) {
		ERR_raise_data(ERR_LIB_OSSL_ENCODER,
	...
	if (evp_pkey_is_provided(pkey)) {
		const OSSL_PROVIDER *prov =
			EVP_KEYMGMT_get0_provider(pkey->keymgmt);
	...
	if (OSSL_ENCODER_CTX_set_output_type(ctx, output_type) &&
		...
		params[0] = OSSL_PARAM_construct_int(
			OSSL_ENCODER_PARAM_SAVE_PARAMETERS, &save_parameters);
