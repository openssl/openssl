				    ...
				    OSSL_PROVIDER *prov)
{
	...
	if ((encoder->base.parsed_propdef = ossl_parse_property(
					      ...
					      const char *properties)
{
	...
	if (id == 0 ||
		...
		if ((method = ossl_method_construct(
			     methdata->libctx, OSSL_OP_ENCODER, &prov,
			      ...
			      void *data)
{
	...
	if (encoder->base.prov != NULL) {
		...
		return ossl_namemap_doall_names(namemap, encoder->base.id, fn,
						data);
	}
