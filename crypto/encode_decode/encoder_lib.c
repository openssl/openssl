...
int OSSL_ENCODER_to_bio(OSSL_ENCODER_CTX *ctx, BIO *out)
{
	...
	if (data.current_encoder_inst_index == 0) {
		ERR_raise_data(
			ERR_LIB_OSSL_ENCODER, OSSL_ENCODER_R_ENCODER_NOT_FOUND,
...
static int encoder_process(struct encoder_process_data_st *data)
{
	...
	for (i = data->current_encoder_inst_index; i-- > 0;) {
		...
		if (!top)
			next_encoder = OSSL_ENCODER_INSTANCE_get_encoder(
				data->next_encoder_inst);

		current_encoder_inst = sk_OSSL_ENCODER_INSTANCE_value(
			data->ctx->encoder_insts, i);
		current_encoder =
			OSSL_ENCODER_INSTANCE_get_encoder(current_encoder_inst);
		current_encoder_ctx = OSSL_ENCODER_INSTANCE_get_encoder_ctx(
			current_encoder_inst);
		current_output_type = OSSL_ENCODER_INSTANCE_get_output_type(
			current_encoder_inst);
		...
		if (top) {
			...
			if (!OSSL_ENCODER_is_a(next_encoder,
					       current_output_type)) {
				OSSL_TRACE_BEGIN(ENCODER)
				{
					BIO_printf(
	...
	if (i < 0) {
		...
		if (ok) {
			...
			else if ((current_out = allocated_out =
					  BIO_new(BIO_s_mem())) == NULL)
				ok = 0; /* Assume BIO_new() recorded an error */

			if (ok)
				ok = (cbio = ossl_core_bio_new_from_bio(
					      current_out)) != NULL;
			if (ok) {
				ok = current_encoder->encode(
					current_encoder_ctx, cbio,
...
int ossl_bio_print_ffc_params(BIO *out, const FFC_PARAMS *ffc)
{
	...
	if (ffc->seed != NULL) {
		if (!ossl_bio_print_labeled_buf(out, "SEED:", ffc->seed,
						ffc->seedlen))
			goto err;
	}
	if (ffc->gindex != -1) {
