...
int OSSL_DECODER_from_bio(OSSL_DECODER_CTX *ctx, BIO *in)
{
	...
	if (OSSL_DECODER_CTX_get_num_decoders(ctx) == 0) {
		ERR_raise_data(
			ERR_LIB_OSSL_DECODER, OSSL_DECODER_R_DECODER_NOT_FOUND,
	...
	if (!data.flag_construct_called) {
		const char *spaces = ctx->start_input_type != NULL &&
						     ctx->input_structure !=
							     NULL ?
					     " " :
					     "";
		...
		const char *comma = ctx->start_input_type != NULL &&
						    ctx->input_structure !=
							    NULL ?
					    ", " :
					    "";
		const char *input_type = ctx->start_input_type != NULL ?
						 ctx->start_input_type :
						 "";
		const char *input_structure = ctx->input_structure != NULL ?
						      ctx->input_structure :
						      "";
		...
		if (ERR_peek_last_error() == lasterr || ERR_peek_error() == 0)
			...
			ERR_raise_data(
				ERR_LIB_OSSL_DECODER, ERR_R_UNSUPPORTED,
						 ...
						 void *decoderctx)
{
	...
	if (props == NULL) {
		ERR_raise_data(
			ERR_LIB_OSSL_DECODER, ERR_R_INVALID_PROPERTY_DEFINITION,
...
void ossl_decoder_instance_free(OSSL_DECODER_INSTANCE *decoder_inst)
{
	if (decoder_inst != NULL) {
		if (decoder_inst->decoder != NULL)
			decoder_inst->decoder->freectx(
				decoder_inst->decoderctx);
...
static void collect_extra_decoder(OSSL_DECODER *decoder, void *arg)
{
	...
	if (ossl_decoder_fast_is_a(decoder, data->output_type,
		...
		for (j = data->w_prev_start; j < data->w_new_end; j++) {
			OSSL_DECODER_INSTANCE *check_inst =
				sk_OSSL_DECODER_INSTANCE_value(
					data->ctx->decoder_insts, j);

			if (decoder->base.algodef ==
			    check_inst->decoder->base.algodef) {
		...
		if (decoder->set_ctx_params != NULL &&
			...
			params[0] = OSSL_PARAM_construct_utf8_string(
				OSSL_OBJECT_PARAM_DATA_STRUCTURE, (char *)str,
				0);
			if (!decoder->set_ctx_params(decoderctx, params)) {
				decoder->freectx(decoderctx);
				return;
		...
		if ((di = ossl_decoder_instance_new(decoder, decoderctx)) ==
		    NULL) {
			decoder->freectx(decoderctx);
			return;
		}
			       ...
			       const char *propq)
{
	...
	if (propq != NULL || ossl_ctx_global_properties(libctx, 0) != NULL) {
		int num_decoder_insts =
			sk_OSSL_DECODER_INSTANCE_num(ctx->decoder_insts);
		int i;
		OSSL_DECODER_INSTANCE *di;
		sk_OSSL_DECODER_INSTANCE_compfunc old_cmp =
			sk_OSSL_DECODER_INSTANCE_set_cmp_func(
				ctx->decoder_insts, decoder_sk_cmp);

		for (i = 0; i < num_decoder_insts; i++) {
			di = sk_OSSL_DECODER_INSTANCE_value(ctx->decoder_insts,
							    i);
			di->order = i;
		}
...
#define LEVEL_STR ">>>>>>>>>>>>>>>>"
#define LEVEL                                                             \
	((size_t)new_data.recursion < sizeof(LEVEL_STR) ?                 \
	...
	if (params == NULL) {
		...
		if (ctx->construct != NULL) {
			...
			rv = ctx->construct(decoder_inst, params,
					    ctx->construct_data);
		...
		p = OSSL_PARAM_locate_const(params,
					    OSSL_OBJECT_PARAM_DATA_STRUCTURE);
		if (p != NULL &&
		    !OSSL_PARAM_get_utf8_string_ptr(p, &data_structure))
			goto end;
		...
		if (p != NULL) {
			if (!OSSL_PARAM_get_utf8_string_ptr(
				    p, &ctx->start_input_type))
				goto end;
			/*
	...
	for (i = data->current_decoder_inst_index; i-- > 0;) {
		...
		const char *new_input_structure =
			OSSL_DECODER_INSTANCE_get_input_structure(
				new_decoder_inst, &n_i_s_was_set);
		...
		if (decoder == NULL && ctx->start_input_type != NULL &&
		    OPENSSL_strcasecmp(ctx->start_input_type, new_input_type) !=
			    0) {
			OSSL_TRACE_BEGIN(DECODER)
			{
				BIO_printf(
					...
					(void *)new_data.ctx, LEVEL,
					(unsigned int)i, ctx->start_input_type);
			}
			OSSL_TRACE_END(DECODER);
			continue;
		}
         ...
         */
		if (data_type != NULL &&
		    !OSSL_DECODER_is_a(new_decoder, data_type)) {
			OSSL_TRACE_BEGIN(DECODER)
			{
				BIO_printf(
					trc_out,
