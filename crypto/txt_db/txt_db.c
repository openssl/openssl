...
TXT_DB *TXT_DB_read(BIO *in, int num)
{
	...
	if ((ret->data = sk_OPENSSL_PSTRING_new_null()) == NULL)
		goto err;
	if ((ret->index = OPENSSL_malloc_array(num, sizeof(*ret->index))) ==
	    NULL)
		goto err;
	if ((ret->qual = OPENSSL_malloc_array(num, sizeof(*(ret->qual)))) ==
	    NULL)
		goto err;
	for (i = 0; i < num; i++) {
		ret->index[i] = NULL;
			...
			OPENSSL_LH_HASHFUNC hash, OPENSSL_LH_COMPFUNC cmp)
{
	...
	if ((idx = (LHASH_OF(OPENSSL_STRING) *)OPENSSL_LH_new(hash, cmp)) ==
	    NULL) {
...
int TXT_DB_insert(TXT_DB *db, OPENSSL_STRING *row)
{
	...
	for (i = 0; i < db->num_fields; i++) {
		if (db->index[i] != NULL) {
			...
			if (lh_OPENSSL_STRING_retrieve(db->index[i], row) ==
			    NULL)
				goto err1;
		}
	}
