...
#define IS_ALIGNED_64(ptr) (((uintptr_t)(ptr) & 7) == 0)
...
size_t SHA3_absorb_zbb(uint64_t A[5][5], const unsigned char *inp, size_t len,
    size_t r);
...
size_t SHA3_absorb(uint64_t A[5][5], const unsigned char *inp, size_t len,
    size_t r)
{
    if (RISCV_HAS_ZBB() && IS_ALIGNED_64(inp)) {
        return SHA3_absorb_zbb(A, inp, len, r);
    } else {
...
void SHA3_squeeze(uint64_t A[5][5], unsigned char *out, size_t len, size_t r, int next)
{
    if (RISCV_HAS_ZBB() && IS_ALIGNED_64(out)) {
        SHA3_squeeze_zbb(A, out, len, r, next);
    } else {
