...
KeccakF1600_int:
    ...
    # Using $V0 register as temporary storage
    @{[vmv_v_x $V0, $T0]}
    sd $T2, 32+8(sp)
    j .Loop
...
.Loop_absorb:
    ...
    # --- A[0]
    @{[vmv_v_x $V7, $T0]}
    @{[vmv_x_s $T1, $V9 ]}      # reload inp pointer
...
.p2align    4
.Lprocess_block:
    @{[vmv_v_x $V9, $T1]}    # save inp pointer

    jal $T2, KeccakF1600_int

    @{[vmv_x_s $T1, $V7]}    # restore len
    @{[vmv_x_s $T2, $V11]}   # restore bsz
    j .Loop_absorb

.Labsorbed:
    # Write back the status to memory
    @{[vmv_x_s $T0, $V8]}    # restore state pointer
    sd $A0, 0($T0)
    sd $A1, 0+8($T0)
    sd $A2, 16($T0)
    sd $A3, 16+8($T0)
