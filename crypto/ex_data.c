...
int do_ex_data_init(OSSL_CTX *ctx)
{
    OSSL_EX_DATA_GLOBAL *global = ossl_ctx_get_ex_data_global(ctx);
 ...
 * after all).
 */
void crypto_cleanup_all_ex_data_int(OSSL_CTX *ctx)
{
    int i;
    OSSL_EX_DATA_GLOBAL *global = ossl_ctx_get_ex_data_global(ctx);
...
int crypto_free_ex_index_ex(OSSL_CTX *ctx, int class_index, int idx)
{
    EX_CALLBACKS *ip;
    EX_CALLBACK *a;
    int toret = 0;
 ...
 */
int crypto_get_ex_new_index_ex(OSSL_CTX *ctx, int class_index, long argl,
 ...
 */
int crypto_new_ex_data_ex(OSSL_CTX *ctx, int class_index, void *obj,
                          CRYPTO_EX_DATA *ad)
{
    int mx, i;
    void *ptr;
                       ...
                       const CRYPTO_EX_DATA *from)
{
    ...
    global = ossl_ctx_get_ex_data_global(from->ctx);
    if (global == NULL)
        return 0;
...
void CRYPTO_free_ex_data(int class_index, void *obj, CRYPTO_EX_DATA *ad)
{
    ...
    EX_CALLBACK *stack[10];
    EX_CALLBACK **storage = NULL;
    OSSL_EX_DATA_GLOBAL *global = ossl_ctx_get_ex_data_global(ad->ctx);
                         ...
                         int idx)
{
    ...
    global = ossl_ctx_get_ex_data_global(ad->ctx);
    if (global == NULL)
        return 0;
...
OSSL_CTX *crypto_ex_data_get_ossl_ctx(const CRYPTO_EX_DATA *ad)
{
    return ad->ctx;
}
