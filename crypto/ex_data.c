...
{
	const struct ex_callback_entry *ap =
		(const struct ex_callback_entry *)a;
	const struct ex_callback_entry *bp =
		(const struct ex_callback_entry *)b;
...
void CRYPTO_free_ex_data(int class_index, void *obj, CRYPTO_EX_DATA *ad)
{
	...
	if (mx > 0) {
		...
		if (storage != NULL)
			for (i = 0; i < mx; i++) {
				storage[i].excb =
					sk_EX_CALLBACK_value(ip->meth, i);
				storage[i].index = i;
			}
	}
	CRYPTO_THREAD_unlock(global->ex_data_lock);
	...
	if (storage != NULL) {
		...
		for (i = 0; i < mx; i++) {
			...
			if (f != NULL && f->free_func != NULL) {
				...
				f->free_func(obj, ptr, ad, storage[i].index,
					     f->argl, f->argp);
			}
		}
	}
