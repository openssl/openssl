...
void OPENSSL_showfatal(const char *fmta, ...)
{
	...
	if ((h = GetStdHandle(STD_ERROR_HANDLE)) != NULL &&
		...
		WriteFile(h, buf, len < 0 ? sizeof(buf) : (DWORD)len, &out,
			  NULL);
		va_end(ap);
		return;
	}
	...
	if (sizeof(TCHAR) == sizeof(char))
		...
		do {
			...
			if (!MultiByteToWideChar(CP_ACP, 0, fmta, (int)len_0,
						 fmtw, (int)len_0))
				for (i = 0; i < len_0; i++)
					fmtw[i] = (WCHAR)fmta[i];
			for (i = 0; i < len_0; i++) {
	...
	if (check_winnt() && OPENSSL_isservice() > 0) {
		...
		if (hEventLog != NULL) {
			...
			if (!ReportEvent(hEventLog, EVENTLOG_ERROR_TYPE, 0, 0,
					 NULL, 1, 0, &pmsg, NULL)) {
#if !defined(NDEBUG)
				/*
                 * We are in a situation where we tried to report a critical
		...
		MessageBox(NULL, buf, _T("OpenSSL: FATAL"),
			   MB_OK | MB_ICONERROR);
	}
#endif
#else
