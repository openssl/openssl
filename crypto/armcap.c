...
void OPENSSL_cpuid_setup(void)
{
...
#else
	{
         ...
         * beginning (the A7).
         */
		OPENSSL_armcap_P |= ARMV7_NEON | ARMV8_PMULL | ARMV8_AES |
		...
		if (OPENSSL_armcap_P & ARMV8_SHA3) {
			...
			if ((sysctlbyname("machdep.cpu.brand_string", uarch,
					  &len, NULL, 0) == 0) &&
	...
	if (OPENSSL_armcap_P & ARMV7_NEON) {
		...
		if (!(OPENSSL_armcap_P & ARMV8_AES)) {
			OPENSSL_armcap_P |=
				arm_probe_for(_armv8_aes_probe, ARMV8_AES);
		}

		OPENSSL_armcap_P |=
			arm_probe_for(_armv8_sha1_probe, ARMV8_SHA1);
		OPENSSL_armcap_P |=
			arm_probe_for(_armv8_sha256_probe, ARMV8_SHA256);

#if defined(__aarch64__)
		OPENSSL_armcap_P |= arm_probe_for(_armv8_sm3_probe, ARMV8_SM3);
		OPENSSL_armcap_P |= arm_probe_for(_armv8_sm4_probe, ARMV8_SM4);
		OPENSSL_armcap_P |=
			arm_probe_for(_armv8_sha512_probe, ARMV8_SHA512);
		OPENSSL_armcap_P |=
			arm_probe_for(_armv8_eor3_probe, ARMV8_SHA3);
#endif
	}
#ifdef __aarch64__
