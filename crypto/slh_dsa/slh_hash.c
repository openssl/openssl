...
static ossl_inline int xof_digest_3(EVP_MD_CTX *ctx, const uint8_t *in1,
				    size_t in1_len, const uint8_t *in2,
				    size_t in2_len, const uint8_t *in3,
				    size_t in3_len, uint8_t *out,
				    size_t out_len)
{
...
static ossl_inline int xof_digest_4(EVP_MD_CTX *ctx, const uint8_t *in1,
				    size_t in1_len, const uint8_t *in2,
				    size_t in2_len, const uint8_t *in3,
				    size_t in3_len, const uint8_t *in4,
				    size_t in4_len, uint8_t *out,
				    size_t out_len)
{
	return (EVP_DigestInit_ex2(ctx, NULL, NULL) == 1 &&
		EVP_DigestUpdate(ctx, in1, in1_len) == 1 &&
			  ...
			  size_t out_len)
{
	...
	return xof_digest_4(ctx->md_ctx, r, n, pk_seed, n, pk_root, n, msg,
			    msg_len, out, m);
}
			     ...
			     size_t msg_len, WPACKET *pkt)
{
	...
	return xof_digest_3(ctx->md_ctx, sk_prf, n, opt_rand, n, msg, msg_len,
			    out, n) &&
		       ...
		       uint8_t *out, size_t out_len)
{
	...
	return xof_digest_3(ctx->md_ctx, pk_seed, n, adrs, SLH_ADRS_SIZE, m1,
			    m1_len, out, n);
}

static int slh_h_shake(SLH_DSA_HASH_CTX *ctx, const uint8_t *pk_seed,
		       const uint8_t *adrs, const uint8_t *m1,
		       const uint8_t *m2, uint8_t *out, size_t out_len)
{
	const SLH_DSA_PARAMS *params = ctx->key->params;
	size_t n = params->n;

	return xof_digest_4(ctx->md_ctx, pk_seed, n, adrs, SLH_ADRS_SIZE, m1, n,
			    m2, n, out, n);
}
		       ...
		       uint8_t *out, size_t out_len)
{
	...
	return xof_digest_3(ctx->md_ctx, pk_seed, n, adrs, SLH_ADRS_SIZE, ml,
			    ml_len, out, n);
}

static ossl_inline int digest_4(EVP_MD_CTX *ctx, const uint8_t *in1,
				size_t in1_len, const uint8_t *in2,
				size_t in2_len, const uint8_t *in3,
				size_t in3_len, const uint8_t *in4,
				size_t in4_len, uint8_t *out)
...
static int slh_hmsg_sha2(SLH_DSA_HASH_CTX *hctx, const uint8_t *r,
			 const uint8_t *pk_seed, const uint8_t *pk_root,
			 const uint8_t *msg, size_t msg_len, uint8_t *out,
			 size_t out_len)
{
	...
	return digest_4(hctx->md_big_ctx, r, n, pk_seed, n, pk_root, n, msg,
			msg_len, seed + 2 * n) &&
	       (PKCS1_MGF1(out, (long)m, seed, (long)seed_len,
			   hctx->key->md_big) == 0);
}

static int slh_prf_msg_sha2(SLH_DSA_HASH_CTX *hctx, const uint8_t *sk_prf,
			    const uint8_t *opt_rand, const uint8_t *msg,
			    size_t msg_len, WPACKET *pkt)
{
	int ret;
	...
	if (hctx->hmac_digest_used == 0) {
		...
		*p++ = OSSL_PARAM_construct_utf8_string(
		...
		if (key->propq != NULL)
			*p++ = OSSL_PARAM_construct_utf8_string(
				OSSL_MAC_PARAM_PROPERTIES, (char *)key->propq,
				0);
...
static ossl_inline int do_hash(EVP_MD_CTX *ctx, size_t n,
			       const uint8_t *pk_seed, const uint8_t *adrs,
			       const uint8_t *m, size_t m_len, size_t b,
			       uint8_t *out, size_t out_len)
{
	int ret;
	uint8_t zeros[128] = { 0 };
	...
	ret = digest_4(ctx, pk_seed, n, zeros, b - n, adrs, SLH_ADRSC_SIZE, m,
		       m_len, digest);
...
static int slh_prf_sha2(SLH_DSA_HASH_CTX *hctx, const uint8_t *pk_seed,
			const uint8_t *sk_seed, const uint8_t *adrs,
			uint8_t *out, size_t out_len)
{
	...
	return do_hash(hctx->md_ctx, n, pk_seed, adrs, sk_seed, n,
		       OSSL_SLH_DSA_SHA2_NUM_ZEROS_H_AND_T_BOUND1, out,
		       out_len);
}

static int slh_f_sha2(SLH_DSA_HASH_CTX *hctx, const uint8_t *pk_seed,
		      const uint8_t *adrs, const uint8_t *m1, size_t m1_len,
		      uint8_t *out, size_t out_len)
{
	return do_hash(hctx->md_ctx, hctx->key->params->n, pk_seed, adrs, m1,
		       m1_len, OSSL_SLH_DSA_SHA2_NUM_ZEROS_H_AND_T_BOUND1, out,
		       out_len);
}

static int slh_h_sha2(SLH_DSA_HASH_CTX *hctx, const uint8_t *pk_seed,
...
static int slh_t_sha2(SLH_DSA_HASH_CTX *hctx, const uint8_t *pk_seed,
		      const uint8_t *adrs, const uint8_t *ml, size_t ml_len,
		      uint8_t *out, size_t out_len)
{
	const SLH_DSA_PARAMS *prms = hctx->key->params;
...
const SLH_HASH_FUNC *ossl_slh_get_hash_fn(int is_shake)
{
	static const SLH_HASH_FUNC methods[] = {
		{ slh_hmsg_shake, slh_prf_shake, slh_prf_msg_shake, slh_f_shake,
		  slh_h_shake, slh_t_shake },
		{ slh_hmsg_sha2, slh_prf_sha2, slh_prf_msg_sha2, slh_f_sha2,
		  slh_h_sha2, slh_t_sha2 }
	};
