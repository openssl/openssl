...
static int slh_dsa_key_hash_init(SLH_DSA_KEY *key)
{
	...
	if (is_shake == 0) {
		if (security_category == 1) {
			...
			key->md_big = EVP_MD_fetch(key->libctx, "SHA2-512",
						   key->propq);
			if (key->md_big == NULL)
				goto err;
		}
...
int ossl_slh_dsa_key_fromdata(SLH_DSA_KEY *key, const OSSL_PARAM *param_pub,
			      const OSSL_PARAM *param_priv, int include_private)
{
	...
	if (include_private) {
		if (param_priv != NULL) {
			p = key->priv;
			if (!OSSL_PARAM_get_octet_string(param_priv, &p,
							 priv_len, &data_len))
				return 0;
...
int ossl_slh_dsa_generate_key(SLH_DSA_HASH_CTX *ctx, SLH_DSA_KEY *out,
			      OSSL_LIB_CTX *lib_ctx, const uint8_t *entropy,
			      size_t entropy_len)
{
...
int ossl_slh_dsa_set_priv(SLH_DSA_KEY *key, const uint8_t *priv,
			  size_t priv_len)
{
...
int ossl_slh_dsa_key_to_text(BIO *out, const SLH_DSA_KEY *key, int selection)
{
	...
	if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
		...
		if (!ossl_bio_print_labeled_buf(
			    out, "priv:", ossl_slh_dsa_key_get_priv(key),
	...
	if (!ossl_bio_print_labeled_buf(out,
