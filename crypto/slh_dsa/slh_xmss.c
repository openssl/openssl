...
int ossl_slh_xmss_node(SLH_DSA_HASH_CTX *ctx, const uint8_t *sk_seed,
		       uint32_t node_id, uint32_t h, const uint8_t *pk_seed,
		       uint8_t *adrs, uint8_t *pk_out, size_t pk_out_len)
{
	...
	if (h == 0) {
		...
		if (!ossl_slh_xmss_node(ctx, sk_seed, 2 * node_id, h - 1,
					pk_seed, adrs, lnode, sizeof(lnode)) ||
		...
		if (!key->hash_func->H(ctx, pk_seed, adrs, lnode, rnode, pk_out,
				       pk_out_len))
			return 0;
	}
	return 1;
		       ...
		       const uint8_t *pk_seed, uint8_t *adrs, WPACKET *sig_wpkt)
{
	...
	for (h = 0; h < hm; ++h) {
		if (!WPACKET_allocate_bytes(sig_wpkt, auth_path_len,
					    &auth_path) ||
			      ...
			      uint8_t *pk_out, size_t pk_out_len)
{
	...
	adrsf->set_type_and_clear(adrs, SLH_ADRS_TYPE_WOTS_HASH);
	adrsf->set_keypair_address(adrs, node_id);
	if (!ossl_slh_wots_pk_from_sig(ctx, sig_rpkt, msg, pk_seed, adrs, node,
				       pk_out_len))
		return 0;
	...
	for (k = 0; k < hm; ++k) {
		...
		if ((node_id & 1) == 0) { /* even */
			...
			if (!H(ctx, pk_seed, adrs, node, auth_path, node,
			       pk_out_len))
				return 0;
		} else { /* odd */
			node_id = (node_id - 1) >> 1;
			set_tree_index(adrs, node_id);
			if (!H(ctx, pk_seed, adrs, auth_path, node, node,
			       pk_out_len))
				return 0;
		}
	}
