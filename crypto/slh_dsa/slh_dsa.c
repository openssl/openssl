			     ...
			     size_t sig_size, const uint8_t *opt_rand)
{
	...
	const uint8_t *pk_seed,
		*sk_seed; /* pointers to elements within |priv| */
	...
	if (!hashf->PRF_MSG(hctx, SLH_DSA_SK_PRF(priv), opt_rand, msg, msg_len,
	    ...
	    || !hashf->H_MSG(hctx, r, pk_seed, SLH_DSA_PK_ROOT(priv), msg,
			     msg_len, m_digest, sizeof(m_digest))
	...
	ret = ossl_slh_fors_sign(hctx, md, sk_seed, pk_seed, adrs, wpkt)
	      ...
	      && ossl_slh_ht_sign(hctx, pk_fors, sk_seed, pk_seed, tree_id,
				  leaf_id, wpkt);
	*sig_len = sig_len_expected;
	ret = 1;
err:
			       ...
			       size_t sig_len)
{
	...
	uint8_t m_digest[SLH_MAX_M];
	const uint8_t
		*md; /* This is a pointer into the buffer in m_digest_rpkt */
		      ...
		      size_t *siglen, size_t sigsize)
{
	...
	ret = slh_sign_internal(slh_ctx, m, m_len, sig, siglen, sigsize,
				add_rand);
	if (m != msg && m != m_tmp)
		OPENSSL_free(m);
	return ret;
			...
			uint64_t *tree_id, uint32_t *leaf_id)
{
	...
	leaf_id_mask = ((uint64_t)1 << params->hm) -
		       1; /* max value is 0x1FF when hm = 9 */
