...
typedef void(OSSL_SLH_ADRS_FUNC_copy_keypair_address)(uint8_t *dst,
						      const uint8_t *src);
/*
...
typedef void(OSSL_SLH_ADRS_FUNC_set_layer_address)(uint8_t *adrs,
						   uint32_t layer);
typedef void(OSSL_SLH_ADRS_FUNC_set_type_and_clear)(uint8_t *adrs,
						    uint32_t type);
typedef void(OSSL_SLH_ADRS_FUNC_set_keypair_address)(uint8_t *adrs,
						     uint32_t in);
