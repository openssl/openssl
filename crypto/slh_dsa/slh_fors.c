			 ...
			 size_t node_len)
{
	...
	if (height == 0) {
		...
		ret = key->hash_func->F(ctx, pk_seed, adrs, sk, n, node,
					node_len);
		OPENSSL_cleanse(sk, n);
		return ret;
	} else {
		if (!slh_fors_node(ctx, sk_seed, pk_seed, adrs, 2 * node_id,
				   height - 1, lnode, sizeof(rnode)) ||
	...
	for (tree_id = 0; tree_id < k; ++tree_id) {
		...
		for (layer = 0; layer < a; ++layer) {
			...
			if (!slh_fors_node(ctx, sk_seed, pk_seed, adrs,
					   s + tree_offset, layer, out,
					   sizeof(out)))
				return 0;
			node_id >>= 1; /* Get the parent node id */
			tree_offset >>=
				1; /* Each layer up has half as many nodes */
			      ...
			      uint8_t *adrs, uint8_t *pk_out, size_t pk_out_len)
{
	...
	for (i = 0; i < k; ++i) {
		...
		for (j = 0; j < a; ++j) {
			...
			if ((id & 1) == 0) {
				node_id >>= 1;
				set_tree_index(adrs, node_id);
				if (!H(ctx, pk_seed, adrs, node0, authj, node1,
				       n))
					goto err;
			} else {
				node_id = (node_id - 1) >> 1;
				set_tree_index(adrs, node_id);
				if (!H(ctx, pk_seed, adrs, authj, node0, node1,
				       n))
					goto err;
			}
			id >>= 1;
	...
	ret = hashf->T(ctx, pk_seed, pk_adrs, roots, roots_len, pk_out,
		       pk_out_len);
err:
