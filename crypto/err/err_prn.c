			 ...
			 void *u)
{
	...
	while ((l = ERR_get_error_all(&file, &line, &func, &data, &flags)) !=
	       0) {
		...
		hex = ossl_buf2hexstr_sep((const unsigned char *)&tid,
					  sizeof(tid), '\0');
		BIO_snprintf(buf, sizeof(buf),
			     "%s:", hex == NULL ? "<null>" : hex);
		offset = (int)strlen(buf);
		ossl_err_string_int(l, func, buf + offset,
				    sizeof(buf) - offset);
...
void ERR_add_error_txt(const char *separator, const char *txt)
{
	...
	do {
		...
		if (data_len >= MAX_DATA_LEN ||
			...
			available_len =
				MAX_DATA_LEN - data_len - strlen(separator) - 1;
