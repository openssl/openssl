 ...
 * We call the otls_init_thread_start function to do that. In the FIPS provider
 * we have our own copy of otls_init_thread_start, which cascades notifications
 ...
 * provider initialisation (i.e. Otls_init_provider).
 */
...
{
    glob_tevent_reg = OPENtls_zalloc(sizeof(*glob_tevent_reg));
...
init_get_thread_local(CRYPTO_THREAD_LOCAL *local, int alloc, int keep)
{
    ...
    if (alloc) {
        if (hands == NULL) {
            ...
            if ((hands = OPENtls_zalloc(sizeof(*hands))) == NULL)
                return NULL;
...
void OPENtls_thread_stop_ex(OPENtls_CTX *ctx)
{
     ...
     * all threads that have used the OPENtls_CTX when the OPENtls_CTX is freed.
     * This is currently not possible due to the use of thread local variables.
     */
    otls_ctx_thread_stop(ctx);
}
...
static void *thread_event_otls_ctx_new(OPENtls_CTX *libctx)
{
    THREAD_EVENT_HANDLER **hands = NULL;
    CRYPTO_THREAD_LOCAL *tlocal = OPENtls_zalloc(sizeof(*tlocal));
    ...
    hands = OPENtls_zalloc(sizeof(*hands));
    if (hands == NULL)
        goto err;
...
static void thread_event_otls_ctx_free(void *tlocal)
{
    OPENtls_free(tlocal);
}

static const OPENtls_CTX_METHOD thread_event_otls_ctx_method = {
    thread_event_otls_ctx_new,
    thread_event_otls_ctx_free,
};
...
void otls_ctx_thread_stop(void *arg)
{
    ...
    CRYPTO_THREAD_LOCAL *local
        = opentls_ctx_get_data(ctx, OPENtls_CTX_THREAD_EVENT_HANDLER_INDEX,
                               &thread_event_otls_ctx_method);
                           ...
                           Otls_thread_stop_handler_fn handfn)
{
     ...
     * of OPENtls_CTX and thread. This is because in FIPS mode each OPENtls_CTX
     ...
     * thread, but may hold multiple OPENtls_CTXs. We only get told about
     * thread stop events globally, so we have to ensure all affected
     * OPENtls_CTXs are informed.
     */
    ...
    if (*hands == NULL) {
         ...
         */
        if (!c_thread_start(FIPS_get_provider(ctx), otls_ctx_thread_stop))
            return 0;
    }
#endif

    hand = OPENtls_malloc(sizeof(*hand));
    if (hand == NULL)
        return 0;
