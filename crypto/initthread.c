...
void OPENSSL_thread_stop_ex(OSSL_CTX *ctx)
{
    ctx = ossl_ctx_get_concrete(ctx);
    /*
     * TODO(3.0). It would be nice if we could figure out a way to do this on
     * all threads that have used the OSSL_CTX when the OSSL_CTX is freed.
...
static void *thread_event_ossl_ctx_new(OSSL_CTX *libctx)
{
...
static const OSSL_CTX_METHOD thread_event_ossl_ctx_method = {
...
void ossl_ctx_thread_stop(void *arg)
{
    ...
    CRYPTO_THREAD_LOCAL *local
        = ossl_ctx_get_data(ctx, OSSL_CTX_THREAD_EVENT_HANDLER_INDEX,
                           ...
                           OSSL_thread_stop_handler_fn handfn)
{
     ...
     * of OSSL_CTX and thread. This is because in FIPS mode each OSSL_CTX
     ...
     * thread, but may hold multiple OSSL_CTXs. We only get told about
     * thread stop events globally, so we have to ensure all affected
     * OSSL_CTXs are informed.
     */
