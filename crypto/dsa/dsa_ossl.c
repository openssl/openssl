...
redo:
	...
	do {
		if (!BN_priv_rand_ex(blind, BN_num_bits(dsa->params.q) - 1,
				     BN_RAND_TOP_ANY, BN_RAND_BOTTOM_ANY, 0,
				     ctx))
			goto err;
...
static int dsa_sign_setup_no_digest(DSA *dsa, BN_CTX *ctx_in, BIGNUM **kinvp,
				    BIGNUM **rp)
{
	return dsa_sign_setup(dsa, ctx_in, kinvp, rp, NULL, 0, 0, NULL, NULL,
			      NULL);
}
			  ...
			  OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	do {
		if (dgst != NULL) {
			if (nonce_type == 1) {
				...
				if (!ossl_gen_deterministic_nonce_rfc6979(
					    k, dsa->params.q, dsa->priv_key,
					    dgst, dlen, digestname, libctx,
					    propq))
#endif
					goto err;
			} else {
		...
		} else if (!ossl_bn_priv_rand_range_fixed_top(k, dsa->params.q,
							      0, ctx))
			goto err;
	...
	if ((dsa)->meth->bn_mod_exp != NULL) {
		if (!dsa->meth->bn_mod_exp(dsa, r, dsa->params.g, k,
					   dsa->params.p, ctx,
					   dsa->method_mont_p))
			goto err;
	} else {
			 ...
			 DSA *dsa)
{
	...
	if (dsa->meth->dsa_mod_exp != NULL) {
		if (!dsa->meth->dsa_mod_exp(dsa, t1, dsa->params.g, u1,
					    dsa->pub_key, u2, dsa->params.p,
					    ctx, mont))
			goto err;
	} else {
				      ...
				      BN_CTX *ctx)
{
	...
	BN_CTX_start(ctx);
	if ((e = BN_CTX_get(ctx)) != NULL && BN_set_word(r, 2) &&
	    BN_sub(e, q, r) && BN_mod_exp_mont(r, k, e, q, ctx, NULL))
		res = r;
	else
		BN_free(r);
	BN_CTX_end(ctx);
