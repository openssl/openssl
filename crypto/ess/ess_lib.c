					 ...
					 int set_issuer_serial)
{
	...
	if ((name->d.dirn = X509_NAME_dup(X509_get_issuer_name(cert))) ==
	    NULL) {
		ERR_raise(ERR_LIB_ESS, ERR_R_X509_LIB);
		goto err;
	}
				  ...
				  int set_issuer_serial)
{
	...
	for (i = 0; i < sk_X509_num(certs); ++i) {
		...
		if ((cid = ESS_CERT_ID_V2_new_init(hash_alg, cert, 1)) ==
		    NULL) {
			ERR_raise(ERR_LIB_ESS, ERR_R_ESS_LIB);
			goto err;
		}
		...
		const STACK_OF(X509) *certs)
{
	...
	for (i = 0; i < sk_X509_num(certs); ++i) {
		...
		cid_hash_len = cid != NULL ? cid->hash->length :
					     cid_v2->hash->length;
		...
		if (memcmp(cid != NULL ? cid->hash->data : cid_v2->hash->data,
			   cert_digest, len) == 0) {
			is = cid != NULL ? cid->issuer_serial :
					   cid_v2->issuer_serial;
			/* Well, it's not really required to match the serial numbers. */
			if (is == NULL ||
			    ess_issuer_serial_cmp(is, cert) == 0) {
				if ((i == 0) == (index == 0)) {
					ret = i + 1;
					goto end;
				}
				ERR_raise(ERR_LIB_ESS,
				 ...
				 int require_signing_cert)
{
	...
	for (i = 0; i < n_v1; i++) {
		ret = find(sk_ESS_CERT_ID_value(ss->cert_ids, i), NULL, i,
			   chain);
		if (ret <= 0)
			return ret;
	}
	for (i = 0; i < n_v2; i++) {
		ret = find(NULL, sk_ESS_CERT_ID_V2_value(ssv2->cert_ids, i), i,
			   chain);
		if (ret <= 0)
			return ret;
	}
