			       ...
			       size_t fixlen)
{
	...
	if (EVP_PKEY_is_a(pkey, "RSA"))
		...
		EVP_PKEY_CTX_ctrl_str(pctx, "rsa_pkcs1_implicit_rejection",
				      "0");
...
BIO *PKCS7_dataInit(PKCS7 *p7, BIO *bio)
{
	...
	if (evp_cipher != NULL) {
		...
		(void)ERR_set_mark();
		fetched_cipher = EVP_CIPHER_fetch(
			libctx, EVP_CIPHER_get0_name(evp_cipher), propq);
		(void)ERR_pop_to_mark();
	...
	if (bio == NULL) {
		if (PKCS7_is_detached(p7)) {
			...
			if (bio != NULL) {
				...
				if (BIO_write(bio, os->data, os->length) !=
				    os->length) {
					BIO_free_all(bio);
					bio = NULL;
				}
...
int PKCS7_dataFinal(PKCS7 *p7, BIO *bio)
{
	...
	if (si_sk != NULL) {
		for (i = 0; i < sk_PKCS7_SIGNER_INFO_num(si_sk); i++) {
			...
			if (sk_X509_ATTRIBUTE_num(sk) > 0) {
				...
				unsigned int abuflen =
					EVP_PKEY_get_size(si->pkey);

				if (abuflen == 0 ||
				    (abuf = OPENSSL_malloc(abuflen)) == NULL)
					goto err;

				if (!EVP_SignFinal_ex(
					    ctx_tmp, abuf, &abuflen, si->pkey,
		...
		if (!EVP_DigestFinal_ex(mdc, md_data, &md_len))
			goto err;
		if (!ASN1_OCTET_STRING_set(p7->d.digest->digest, md_data,
					   md_len))
			goto err;
	}
		     ...
		     PKCS7 *p7, PKCS7_SIGNER_INFO *si)
{
	...
	signer = X509_find_by_issuer_and_serial(untrusted, ias->issuer,
						ias->serial);
			  ...
			  X509 *signer)
{
	...
	i = EVP_VerifyFinal_ex(mdc_tmp, os->data, os->length, pkey, libctx,
			       propq);
	if (i <= 0) {
				...
				STACK_OF(X509_ATTRIBUTE) *sk)
{
	...
	p7si->auth_attr = sk_X509_ATTRIBUTE_deep_copy(sk, X509_ATTRIBUTE_dup,
						      X509_ATTRIBUTE_free);
...
int PKCS7_set_attributes(PKCS7_SIGNER_INFO *p7si, STACK_OF(X509_ATTRIBUTE) *sk)
{
	...
	p7si->unauth_attr = sk_X509_ATTRIBUTE_deep_copy(sk, X509_ATTRIBUTE_dup,
							X509_ATTRIBUTE_free);
