			  ...
			  const EVP_MD *dgst)
{
	...
	if (!X509_ALGOR_set0(p7i->digest_alg,
			     OBJ_nid2obj(EVP_MD_get_type(dgst)), V_ASN1_NULL,
			     NULL))
		return 0;
	...
	if (pkey->ameth != NULL && pkey->ameth->pkey_ctrl != NULL) {
		ret = pkey->ameth->pkey_ctrl(pkey, ASN1_PKEY_CTRL_PKCS7_SIGN, 0,
					     p7i);
		if (ret > 0)
			return 1;
		if (ret != -2) {
...
int PKCS7_RECIP_INFO_set(PKCS7_RECIP_INFO *p7i, X509 *x509)
{
	...
	ret = pkey->ameth->pkey_ctrl(pkey, ASN1_PKEY_CTRL_PKCS7_ENCRYPT, 0,
				     p7i);
	if (ret == -2) {
		ERR_raise(ERR_LIB_PKCS7,
			  PKCS7_R_ENCRYPTION_NOT_SUPPORTED_FOR_THIS_KEY_TYPE);
...
{
	if (PKCS7_type_is_signed(p7))
		return (X509_find_by_issuer_and_serial(
