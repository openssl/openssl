...
static int pkcs7_copy_existing_digest(PKCS7 *p7, PKCS7_SIGNER_INFO *si) {
  ...
  for (i = 0; i < sk_PKCS7_SIGNER_INFO_num(sinfos); i++) {
    ...
    if (!OBJ_cmp(si->digest_alg->algorithm, sitmp->digest_alg->algorithm)) {
      os = PKCS7_digest_from_attributes(sitmp->auth_attr);
      break;
    }
  }

  if (os != NULL)
    return PKCS7_add1_attrib_digest(si, os->data, os->length);
...
int PKCS7_verify(PKCS7 *p7, STACK_OF(X509) * certs, X509_STORE *store,
  ...
  char *buf = NULL;
  int i, j = 0, k, ret = 0;
  int total_signers = 0; /* Total signers checked */
  int valid_cnt = 0;     /* Count of successfully verified signatures */
#if defined(NID_ML_DSA_44) || defined(NID_SLH_DSA_SHA2_128s)
  int valid_pq_cnt = 0; /* Count of valid PQ signatures */
#endif
  BIO *p7bio = NULL;
  int sig_count = 0; /* Number of signatures */
  BIO *tmpout = NULL;
  ...
  sinfos = PKCS7_get_signer_info(p7);
  sig_count = sk_PKCS7_SIGNER_INFO_num(sinfos);
  ...
  if (!(flags & PKCS7_NOSIGS)) {
    if (sig_count > 0) {
      for (i = 0; i < sig_count; i++) {
        ...
        total_signers++; /* Count every signer we attempt to verify */
        j = PKCS7_signatureVerify(p7bio, p7, si, signer);
        if (j > 0) {
          /* Signature verification succeeded for this signer */
          valid_cnt++;
#if defined(NID_ML_DSA_44) || defined(NID_SLH_DSA_SHA2_128s)
          /* Check if the signature algorithm is Post-Quantum */
          int pkey_nid = OBJ_obj2nid(si->digest_enc_alg->algorithm);
          /* Add NIDs for ML-DSA and SLH-DSA variants */
#if defined(NID_ML_DSA_44)
          if (pkey_nid >= NID_ML_DSA_44 && pkey_nid <= NID_ML_DSA_87) {
            valid_pq_cnt++;
          }
#endif
#if defined(NID_SLH_DSA_SHA2_128s)
          /* Check SLH-DSA range if ML-DSA didn't match or isn't defined */
          if (valid_pq_cnt == 0 && /* Avoid double counting if both defined */
              pkey_nid >= NID_SLH_DSA_SHA2_128s &&
              pkey_nid <= NID_SLH_DSA_SHAKE_256f) {
            valid_pq_cnt++;
          }
#endif
#endif /* defined(NID_ML_DSA_44) || defined(NID_SLH_DSA_SHA2_128s) */
        }
        /* If j <= 0, the signature is invalid, counters are not incremented */
      }

      /* Check verification policy based on flags and counters */
#if defined(NID_ML_DSA_44) || defined(NID_SLH_DSA_SHA2_128s)
      if (flags & PKCS7_VERIFY_POLICY_REQUIRE_PQ) {
        /* Policy requires at least one valid PQ signature */
        if (valid_pq_cnt == 0) {
          ERR_raise(ERR_LIB_PKCS7, PKCS7_R_PQ_SIGNATURE_REQUIRED_BUT_NOT_FOUND);
          goto err;
        }
        /* If AND policy also active, all signatures must be valid */
        if ((flags & PKCS7_VERIFY_POLICY_AND) && (valid_cnt != total_signers)) {
          ERR_raise(ERR_LIB_PKCS7, PKCS7_R_SIGNATURE_FAILURE);
          goto err;
        }
        /* If only REQUIRE_PQ, at least one overall signature must be valid
         * (implicit check via valid_pq_cnt > 0) */
        /* This check is slightly redundant as valid_pq_cnt > 0 implies
         * valid_cnt > 0, but keep for clarity */
        if (!(flags & PKCS7_VERIFY_POLICY_AND) && valid_cnt == 0) {
          ERR_raise(ERR_LIB_PKCS7, PKCS7_R_SIGNATURE_FAILURE);
          goto err;
        }
      } else if (flags & PKCS7_VERIFY_POLICY_AND) {
        /* AND policy without REQUIRE_PQ: all signatures must be valid */
        if (valid_cnt != total_signers) {
          ERR_raise(ERR_LIB_PKCS7, PKCS7_R_SIGNATURE_FAILURE);
          goto err;
        }
      } else {
        /* Default policy (implicit OR) without REQUIRE_PQ: at least one
...
#else        /* No PQ algorithms defined */
      /* Only AND or default (OR) policies are possible */
      if (flags & PKCS7_VERIFY_POLICY_AND) {
        /* AND policy: all signatures must be valid */
        if (valid_cnt != total_signers) {
          ERR_raise(ERR_LIB_PKCS7, PKCS7_R_SIGNATURE_FAILURE);
          goto err;
        }
      } else {
        /* Default policy (implicit OR): at least one signature must be valid */
        if (valid_cnt == 0) {
          ERR_raise(ERR_LIB_PKCS7, PKCS7_R_SIGNATURE_FAILURE);
          goto err;
        }
      }
      /* Raise error if PKCS7_VERIFY_POLICY_REQUIRE_PQ is used but PQ algs are
       * not defined */
      if (flags & PKCS7_VERIFY_POLICY_REQUIRE_PQ) {
        ERR_raise(ERR_LIB_PKCS7, PKCS7_R_PQ_SIGNATURE_REQUIRED_BUT_NOT_FOUND);
        goto err;
  ...
  } /* End of if (!(flags & PKCS7_NOSIGS)) */

  /* If PKCS7_NOSIGS is set, or if policy checks passed, we reach here */
  ret = 1;
