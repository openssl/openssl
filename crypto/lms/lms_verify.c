...
static int lms_sig_compute_tc_from_path(const unsigned char *paths, uint32_t n,
					uint32_t node_num, EVP_MD_CTX *ctx,
					EVP_MD_CTX *ctxI, unsigned char *Tc)
{
	int ret = 0;
...
int ossl_lms_sig_verify(const LMS_SIG *lms_sig, const LMS_KEY *pub,
			const EVP_MD *md, const unsigned char *msg,
			size_t msglen)
{
	int ret = 0;
	...
	if (!ossl_lm_ots_compute_pubkey(ctx, ctxIq, &lms_sig->sig,
					pub->ots_params, pub->Id, lms_sig->q,
					msg, msglen, Kc))
		goto err;
	...
	if (!EVP_DigestInit_ex2(ctx, NULL, NULL) ||
	    ...
	    !lms_sig_compute_tc_from_path(lms_sig->paths, n, node_num, ctx,
					  ctxI, Tc))
		goto err;
	/* Algorithm 6: Step 4 */
