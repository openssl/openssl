		      ...
		      OSSL_PROPERTY_IDX *idx)
{
	...
	name[i] = '\0';
	if (err) {
		ERR_raise_data(ERR_LIB_PROP, PROP_R_NAME_TOO_LONG, "HERE-->%s",
			       *t);
		return 0;
	}
	*t = skip_space(s);
...
static int parse_hex(const char *t[], OSSL_PROPERTY_DEFINITION *res)
{
	...
	do {
		if (ossl_isdigit(*s)) {
			...
			ERR_raise_data(ERR_LIB_PROP,
				       PROP_R_NOT_AN_HEXADECIMAL_DIGIT, "%s",
				       *t);
			return 0;
		}
			...
			OSSL_PROPERTY_DEFINITION *res, const int create)
{
	...
	v[i] = '\0';
	if (err) {
		ERR_raise_data(ERR_LIB_PROP, PROP_R_STRING_TOO_LONG,
			       "HERE-->%s", *t);
	} else {
		       ...
		       STACK_OF(OSSL_PROPERTY_DEFINITION) *sk)
{
	...
	if (r != NULL) {
		...
		for (i = 0; i < n; i++) {
			r->properties[i] =
				*sk_OSSL_PROPERTY_DEFINITION_value(sk, i);
			r->has_optional |= r->properties[i].optional;

			/* Check for duplicated names */
			if (i > 0 &&
			    r->properties[i].name_idx == prev_name_idx) {
				OPENSSL_free(r);
				ERR_raise_data(ERR_LIB_PROP,
					       PROP_R_PARSE_FAILED,
					       "Duplicated name `%s'",
			      ...
			      const OSSL_PROPERTY_LIST *defn)
{
	...
	while (i < query->num_properties) {
		...
		if (j < defn->num_properties) {
			if (q[i].name_idx >
			    d[j].name_idx) { /* skip defn, not in query */
				j++;
				continue;
			}
			if (q[i].name_idx ==
			    d[j].name_idx) { /* both in defn and query */
					...
					const OSSL_PROPERTY_LIST *b)
{
	...
	r->has_optional = 0;
	for (i = j = n = 0; i < a->num_properties || j < b->num_properties;
	     n++) {
	...
	for (i = 0; i < list->num_properties; i++, prop--) {
		...
		case OSSL_PROPERTY_OPER_EQ:
			...
			case OSSL_PROPERTY_TYPE_STRING:
				val = ossl_property_value_str(ctx,
							      prop->v.str_val);
				if (val == NULL)
					return 0;
				put_str(val, &buf, &bufsize, &needed);
			...
			case OSSL_PROPERTY_TYPE_NUMBER:
				put_num(prop->v.int_val, &buf, &bufsize,
					&needed);
				break;
