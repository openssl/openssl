 ...
 *
 * This implementation uses Opentls's standard hash table.  There are more
...
typedef struct {
    ...
    Otls_PROPERTY_IDX prop_name_idx;
    Otls_PROPERTY_IDX prop_value_idx;
} PROPERTY_STRING_DATA;

static unsigned long property_hash(const PROPERTY_STRING *a)
{
    return OPENtls_LH_strhash(a->s);
}
...
static void property_string_data_free(void *vpropdata)
{
    ...
    OPENtls_free(propdata);
}

static void *property_string_data_new(OPENtls_CTX *ctx) {
    PROPERTY_STRING_DATA *propdata = OPENtls_zalloc(sizeof(*propdata));
...
static const OPENtls_CTX_METHOD property_string_data_method = {
    property_string_data_new,
    property_string_data_free,
};

static PROPERTY_STRING *new_property_string(const char *s,
                                            Otls_PROPERTY_IDX *pidx)
{
    const size_t l = strlen(s);
    PROPERTY_STRING *ps = OPENtls_malloc(sizeof(*ps) + l);
...
static Otls_PROPERTY_IDX otls_property_string(PROP_TABLE *t,
                                              Otls_PROPERTY_IDX *pidx,
                                              const char *s)
{
                                     ...
                                     int create)
{
    PROPERTY_STRING_DATA *propdata
        = opentls_ctx_get_data(ctx, OPENtls_CTX_PROPERTY_STRING_INDEX,
                               &property_string_data_method);

    if (propdata == NULL)
        return 0;
    return otls_property_string(propdata->prop_names,
                                      ...
                                      int create)
{
    ...
    if (propdata == NULL)
        return 0;
    return otls_property_string(propdata->prop_values,
