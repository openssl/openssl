...
HT_DEF_KEY_FIELD_CHAR_ARRAY(name, NAMEMAP_NAME_MAX_LENGTH)
...
int ossl_frozen_method_store_cache_get(OSSL_METHOD_STORE *store, OSSL_PROVIDER *prov,
    const char *name, const char *prop_query, void **method)
{
    int res = 0;
...
static void alg_cache_freeze(QUERY *elem, void *arg)
{
    ...
    if (elem->method.dup == NULL
        || !evp_method_id2name_id_op_id(af->nid, &name_id, &op_id))
        return;

    name = ossl_namemap_num2name(af->namemap, name_id, 0);
    if (name == NULL)
        return;
    ...
    HT_SET_KEY_STRING(&key, propq, elem->query);
    ...
    /*
     *  TODO: add reference counting to METHOD so that we can avoid
     * duplicating the method here.
     */
    val.value = (void *)ossl_method_dup(&elem->method);
    if (val.value == NULL)
        goto err;
...
int ossl_method_store_freeze_cache(OSSL_METHOD_STORE *store, OSSL_NAMEMAP *namemap)
{
    ...
    if (store == NULL || ossl_method_store_is_frozen(store))
        return 0;
