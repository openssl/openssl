...
void ossl_method_store_free(OSSL_METHOD_STORE *store)
{
	if (store != NULL) {
		if (store->algs != NULL)
			ossl_sa_ALGORITHM_doall_arg(store->algs, &alg_cleanup,
						    store);
			  ...
			  void (*method_destruct)(void *))
{
	...
	if ((impl->properties = ossl_prop_defn_get(store->ctx, properties)) ==
		...
		if (impl->properties == NULL)
			goto err;
		if (!ossl_prop_defn_set(store->ctx, properties,
					&impl->properties)) {
	...
	for (i = 0; i < sk_IMPLEMENTATION_num(alg->impls); i++) {
		const IMPLEMENTATION *tmpimpl =
			sk_IMPLEMENTATION_value(alg->impls, i);
			      ...
			      void *fnarg)
{
	...
	if (store != NULL) {
		...
		for (i = 0; i < numalgs; i++) {
			...
			for (j = 0; j < numimps; j++)
				alg_do_one(alg,
					   sk_IMPLEMENTATION_value(alg->impls,
								   j),
					   fn, fnarg);
		}
			    ...
			    const OSSL_PROVIDER **prov_rw, void **method)
{
	...
	if (alg == NULL) {
		ossl_property_unlock(store);
		OSSL_TRACE2(QUERY,
			    "Failed to retrieve by nid %d from store %p\n", nid,
			    (void *)store);
		return 0;
	}
...
static void ossl_method_cache_flush_some(OSSL_METHOD_STORE *store)
{
	...
	ossl_sa_ALGORITHM_doall_arg(store->algs, &impl_cache_flush_one_alg,
				    &state);
