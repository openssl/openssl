			 ...
			 int input_flags, char *result_buf)
{
	...
	if (ok_chars == NULL) {
		...
		s = general_allocate_prompt(ui, prompt, prompt_freeable, type,
					    input_flags, result_buf);
			...
			int minsize, int maxsize)
{
	...
	ret = general_allocate_string(ui, prompt_copy, 1, UIT_PROMPT, flags,
				      result_buf, minsize, maxsize, NULL);
	if (ret <= 0)
			 ...
			 const char *test_buf)
{
	...
	ret = general_allocate_string(ui, prompt_copy, 1, UIT_VERIFY, flags,
				      result_buf, minsize, maxsize, test_buf);
	if (ret <= 0)
		OPENSSL_free(prompt_copy);
	return ret;
			 ...
			 int flags, char *result_buf)
{
	return general_allocate_boolean(ui, prompt, action_desc, ok_chars,
					cancel_chars, 0, UIT_BOOLEAN, flags,
					result_buf);
}
...
int UI_dup_info_string(UI *ui, const char *text)
{
	...
	ret = general_allocate_string(ui, text_copy, 1, UIT_INFO, 0, NULL, 0, 0,
				      NULL);
	if (ret <= 0)
...
int UI_dup_error_string(UI *ui, const char *text)
{
	...
	ret = general_allocate_string(ui, text_copy, 1, UIT_ERROR, 0, NULL, 0,
				      0, NULL);
	if (ret <= 0)
		OPENSSL_free(text_copy);
	return ret;
			  ...
			  const char *object_name)
{
	...
	if (ui != NULL && ui->meth != NULL &&
	    ui->meth->ui_construct_prompt != NULL)
		prompt = ui->meth->ui_construct_prompt(ui, phrase_desc,
						       object_name);
	else {
		char prompt1[] = "Enter ";
		char prompt2[] = " for ";
...
int UI_process(UI *ui)
{
	...
	for (i = 0; i < sk_UI_STRING_num(ui->strings); i++) {
		if (ui->meth->ui_write_string != NULL &&
		    (ui->meth->ui_write_string(
			     ui, sk_UI_STRING_value(ui->strings, i)) <= 0)) {
	...
	for (i = 0; i < sk_UI_STRING_num(ui->strings); i++) {
		if (ui->meth->ui_read_string != NULL) {
			switch (ui->meth->ui_read_string(
				ui, sk_UI_STRING_value(ui->strings, i))) {
			case -1: /* Interrupt/Cancel/something... */
				ui->flags &= ~UI_FLAG_REDOABLE;
				ok = -2;
				goto err;
...
err:
	...
	if (ok == -1)
		ERR_raise_data(ERR_LIB_UI, UI_R_PROCESSING_ERROR, "while %s",
			       state);
	return ok;
}
