...
static int read_string_inner(UI *ui, UI_STRING *uis, int echo, int strip_nl)
{
	...
	if (is_a_tty) {
		DWORD numread;
#if defined(CP_UTF8)
		if (GetEnvironmentVariableW(L"OPENSSL_WIN32_UTF8", NULL, 0) !=
		    0) {
...
#endif
			if (ReadConsoleA(GetStdHandle(STD_INPUT_HANDLE), result,
					 maxsize, &numread, NULL)) {
...
static int noecho_console(UI *ui)
{
	...
	if (is_a_tty) {
		...
		status = sys$qiow(0, channel, IO$_SETMODE, &iosb, 0, 0, tty_new,
				  12, 0, 0, 0, 0);
		if ((status != SS$_NORMAL) ||
		    (iosb.iosb$w_value != SS$_NORMAL)) {
			ERR_raise_data(
				ERR_LIB_UI, UI_R_SYSQIOW_ERROR,
