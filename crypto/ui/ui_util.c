...
{
	if (*pptr != NULL) {
		*pptr = OPENSSL_memdup(*pptr,
				       sizeof(struct pem_password_cb_data));
		if (*pptr != NULL)
			return 1;
	}
	return 0;
...
static int ui_read(UI *ui, UI_STRING *uis)
{
	...
	case UIT_PROMPT: {
		...
		const struct pem_password_cb_data *data = UI_method_get_ex_data(
			UI_get_method(ui), ui_method_data_index);
