...
#define NEIGHBORHOOD_LEN \
	(CACHE_LINE_BYTES / sizeof(struct ht_neighborhood_entry_st))
/*
							      ...
							      void **freeptr)
{
	...
	ret = OPENSSL_aligned_alloc_array(len,
					  sizeof(struct ht_neighborhood_st),
					  CACHE_LINE_BYTES, freeptr);

	/* fall back to regular malloc */
	if (ret == NULL) {
		ret = *freeptr = OPENSSL_malloc_array(
			len, sizeof(struct ht_neighborhood_st));
		if (ret == NULL)
			return NULL;
	}
...
HT *ossl_ht_new(const HT_CONFIG *conf)
{
	...
	new->md->neighborhoods = alloc_new_neighborhood_list(
		new->wpd.neighborhood_len, &new->md->neighborhood_ptr_to_free);
...
static int ossl_ht_flush_internal(HT *h)
{
	...
	newmd->neighborhoods = alloc_new_neighborhood_list(
		DEFAULT_NEIGH_LEN, &newmd->neighborhood_ptr_to_free);
	...
	for (i = 0; i < md->neighborhood_mask + 1; i++) {
		...
		for (j = 0; j < NEIGHBORHOOD_LEN; j++) {
			if (md->neighborhoods[i].entries[j].value != NULL) {
				if (!cb((HT_VALUE *)md->neighborhoods[i]
						.entries[j]
						.value,
					arg))
					goto out;
...
static int grow_hashtable(HT *h, size_t oldsize)
{
	...
	newmd->neighborhoods = alloc_new_neighborhood_list(
		oldsize * 2, &newmd->neighborhood_ptr_to_free);
	if (newmd->neighborhoods == NULL)
		goto out_free;
	...
	for (oldi = 0; oldi < h->wpd.neighborhood_len; oldi++) {
		...
		for (oldj = 0; oldj < NEIGHBORHOOD_LEN; oldj++) {
			...
			for (newj = 0; newj < NEIGHBORHOOD_LEN; newj++) {
				if (newmd->neighborhoods[newi]
					    .entries[newj]
					    .value == NULL) {
					newmd->neighborhoods[newi]
						.entries[newj]
						.value = oldv;
					newmd->neighborhoods[newi]
						.entries[newj]
				 ...
				 HT_VALUE **olddata)
{
	...
	do {
		...
		for (j = 0; j < NEIGHBORHOOD_LEN; j++) {
			ival = ossl_rcu_deref(
				&md->neighborhoods[neigh_idx].entries[j].value);
			if (ival == NULL) {
				empty_idx = j;
			...
			if (!CRYPTO_atomic_load(&md->neighborhoods[neigh_idx]
							 .entries[j]
							 .hash,
						&ihash, h->atomic_lock))
				return 0;
			if (compare_hash(hash, ihash) &&
			    match_key(&newval->value.key, &ival->key)) {
				if (olddata == NULL) {
					/* This would insert a duplicate -> fail */
					return 0;
				}
				/* Do a replacement */
				if (!CRYPTO_atomic_store(
					    &md->neighborhoods[neigh_idx]
						     .entries[j]
						     .hash,
					    hash, h->atomic_lock))
					return 0;
				*olddata =
					(HT_VALUE *)md->neighborhoods[neigh_idx]
						.entries[j]
						.value;
				ossl_rcu_assign_ptr(
					&md->neighborhoods[neigh_idx]
						 ...
						 .value,
					&newval);
				ossl_rcu_call(h->lock, free_old_ht_value,
					      *olddata);
				h->wpd.need_sync = 1;
				return 1;
			}
...
not_found:
	...
	h->wpd.value_count++;
	ossl_rcu_assign_ptr(
		&md->neighborhoods[neigh_idx].entries[empty_idx].value,
		&newval);
	return 1;
}

static struct ht_internal_value_st *alloc_new_value(HT *h, HT_KEY *key,
						    void *data, uintptr_t *type)
{
...
int ossl_ht_delete(HT *h, HT_KEY *key)
{
	...
	for (j = 0; j < NEIGHBORHOOD_LEN; j++) {
		v = (struct ht_internal_value_st *)h->md
			    ->neighborhoods[neigh_idx]
			    .entries[j]
			    .value;
		if (v == NULL)
			continue;
		if (compare_hash(
			    hash,
			    h->md->neighborhoods[neigh_idx].entries[j].hash) &&
		    match_key(key, &v->value.key)) {
			if (!CRYPTO_atomic_store(&h->md->neighborhoods[neigh_idx]
							  .entries[j]
							  .hash,
						 0, h->atomic_lock))
				break;
			h->wpd.value_count--;
			ossl_rcu_assign_ptr(&h->md->neighborhoods[neigh_idx]
						     .entries[j]
						     .value,
					    &nv);
			rc = 1;
