						       ...
						       int nid)
{
	...
	return (STACK_OF(X509_EXTENSION) *)ASN1_item_d2i(
		NULL, &p, ext->value.sequence->length,
			      ...
			      const unsigned char *bytes, int len)
{
	...
	if (!X509at_add1_attr_by_OBJ(&req->req_info.attributes, obj, type,
				     bytes, len))
		return 0;
	req->req_info.enc.modified = 1;
	return 1;
			      ...
			      const unsigned char *bytes, int len)
{
	...
	if (!X509at_add1_attr_by_NID(&req->req_info.attributes, nid, type,
				     bytes, len))
		return 0;
	req->req_info.enc.modified = 1;
	return 1;
