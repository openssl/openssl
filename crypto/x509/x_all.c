...
int X509_verify(X509 *a, EVP_PKEY *r)
{
	...
	return ASN1_item_verify_ex(ASN1_ITEM_rptr(X509_CINF), &a->sig_alg,
				   ...
				   a->distinguishing_id, r, a->libctx,
				   a->propq);
}
...
int X509_ACERT_verify(X509_ACERT *a, EVP_PKEY *r)
{
	...
	return ASN1_item_verify_ex(ASN1_ITEM_rptr(X509_ACERT_INFO), &a->sig_alg,
				   &a->signature, a->acinfo, NULL, r, NULL,
				   NULL);
}
...
int X509_sign(X509 *x, EVP_PKEY *pkey, const EVP_MD *md)
{
	...
	return ASN1_item_sign_ex(ASN1_ITEM_rptr(X509_CINF),
				 ...
				 &x->signature, &x->cert_info, NULL, pkey, md,
				 x->libctx, x->propq);
}
...
int X509_REQ_sign(X509_REQ *x, EVP_PKEY *pkey, const EVP_MD *md)
{
	...
	return ASN1_item_sign_ex(ASN1_ITEM_rptr(X509_REQ_INFO), &x->sig_alg,
				 NULL, x->signature, &x->req_info, NULL, pkey,
				 md, x->libctx, x->propq);
}
...
int X509_REQ_sign_ctx(X509_REQ *x, EVP_MD_CTX *ctx)
{
	...
	x->req_info.enc.modified = 1;
	return ASN1_item_sign_ctx(ASN1_ITEM_rptr(X509_REQ_INFO), &x->sig_alg,
				  NULL, x->signature, &x->req_info, ctx);
}
...
int X509_ACERT_sign(X509_ACERT *x, EVP_PKEY *pkey, const EVP_MD *md)
{
	return ASN1_item_sign_ex(ASN1_ITEM_rptr(X509_ACERT_INFO), &x->sig_alg,
				 &x->acinfo->signature, &x->signature,
...
int NETSCAPE_SPKI_sign(NETSCAPE_SPKI *x, EVP_PKEY *pkey, const EVP_MD *md)
{
	return ASN1_item_sign_ex(ASN1_ITEM_rptr(NETSCAPE_SPKAC), &x->sig_algor,
				 NULL, x->signature, x->spkac, NULL, pkey, md,
				 NULL, NULL);
}
...
PKCS7 *d2i_PKCS7_bio(BIO *bp, PKCS7 **p7)
{
	...
	ret = ASN1_item_d2i_bio_ex(ASN1_ITEM_rptr(PKCS7), bp, p7, libctx,
				   propq);
	if (ret != NULL)
		ossl_pkcs7_resolve_libctx(ret);
	return ret;
				   ...
				   int *md_is_fallback)
{
	...
	if (!OBJ_find_sigid_algs(X509_get_signature_nid(cert), &mdnid,
				 &pknid)) {
	...
	if (mdnid == NID_undef) {
		if (pknid == EVP_PKEY_RSA_PSS) {
			...
			if ((md = EVP_MD_fetch(cert->libctx,
...
{
	return ASN1_d2i_fp_of(EVP_PKEY, EVP_PKEY_new, d2i_AutoPrivateKey, fp,
			      a);
}
