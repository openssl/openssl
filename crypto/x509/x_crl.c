...
static X509_CRL_METHOD int_crl_meth = { 0, 0, 0, def_crl_lookup,
					def_crl_verify };
		      ...
		      void *exarg)
{
	...
	case ASN1_OP_D2I_POST:
		(void)sk_X509_REVOKED_set_cmp_func(a->revoked,
						   X509_REVOKED_cmp);
		break;
	}
	return 1;
...
static int crl_set_issuers(X509_CRL *crl)
{
	...
	for (i = 0; i < sk_X509_REVOKED_num(revoked); i++) {
		...
		gtmp = X509_REVOKED_get_ext_d2i(rev, NID_certificate_issuer, &j,
						NULL);
		if (gtmp == NULL && j != -1) {
		...
		reason =
			X509_REVOKED_get_ext_d2i(rev, NID_crl_reason, &j, NULL);
		if (reason == NULL && j != -1) {
			crl->flags |= EXFLAG_INVALID;
			return 1;
		}
		...
		for (j = 0; j < sk_X509_EXTENSION_num(exts); j++) {
			...
			if (X509_EXTENSION_get_critical(ext)) {
				if (OBJ_obj2nid(X509_EXTENSION_get_object(
					    ext)) == NID_certificate_issuer)
					continue;
		  ...
		  void *exarg)
{
	...
	case ASN1_OP_D2I_POST:
		...
		crl->idp = X509_CRL_get_ext_d2i(
			crl, NID_issuing_distribution_point, &i, NULL);
		if (crl->idp != NULL) {
		...
		crl->akid = X509_CRL_get_ext_d2i(
			crl, NID_authority_key_identifier, &i, NULL);
		if (crl->akid == NULL && i != -1)
			crl->flags |= EXFLAG_INVALID;

		crl->crl_number =
			X509_CRL_get_ext_d2i(crl, NID_crl_number, &i, NULL);
