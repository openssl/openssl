		      ...
		      STACK_OF(CONF_VALUE) *nval)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (cnf->value == NULL) {
			...
			sk_DIST_POINT_push(
				crld,
				point); /* no failure as it was reserved */
		} else {
		   ...
		   int indent)
{
	...
	if (idp->onlysomereasons)
		print_reasons(out, "Only Some Reasons", idp->onlysomereasons,
			      indent);
	if (idp->onlyattr > 0)
	...
	for (i = 0; i < sk_CONF_VALUE_num(nval); i++) {
		...
		if (strcmp(cnf->name, "reasons") == 0) {
			...
			if (!X509V3_get_value_bool(cnf, &point->indirectCRL))
				goto err;
		} else if (strcmp(cnf->name, "containsUserAttributeCerts") ==
			   0) {
			if (!X509V3_get_value_bool(
				    cnf, &point->containsUserAttributeCerts))
				goto err;
		} else if (strcmp(cnf->name, "containsAACerts") == 0) {
			if (!X509V3_get_value_bool(cnf,
						   &point->containsAACerts))
				goto err;
		} else if (strcmp(cnf->name, "containsSOAPublicKeyCerts") ==
			   0) {
			if (!X509V3_get_value_bool(
		     ...
		     int indent)
{
	...
	if (pdp->containsUserAttributeCerts) {
		if (BIO_printf(out, "%*sContains User Attribute Certificates: ",
			       indent, "") <= 0)
			return 0;
