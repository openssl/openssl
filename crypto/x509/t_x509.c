...
int X509_print_ex(BIO *bp, X509 *x, unsigned long nmflags, unsigned long cflag)
{
	...
	if (!(cflag & X509_FLAG_NO_ISSUER)) {
		if (BIO_printf(bp, "        Issuer:%c", mlch) <= 0)
			goto err;
		if (X509_NAME_print_ex(bp, X509_get_issuer_name(x), nmindent,
				       nmflags) < printok)
			goto err;
		if (BIO_write(bp, "\n", 1) <= 0)
			goto err;
	...
	if (!(cflag & X509_FLAG_NO_SUBJECT)) {
		if (BIO_printf(bp, "        Subject:%c", mlch) <= 0)
			goto err;
		if (X509_NAME_print_ex(bp, X509_get_subject_name(x), nmindent,
				       nmflags) < printok)
			goto err;
		if (BIO_write(bp, "\n", 1) <= 0)
			goto err;
...
int X509_signature_dump(BIO *bp, const ASN1_STRING *sig, int indent)
{
	...
	for (i = 0; i < n; i++) {
		...
		if (BIO_printf(bp, "%02x%s", s[i], ((i + 1) == n) ? "" : ":") <=
		    0)
			return 0;
	}
	if (BIO_write(bp, "\n", 1) != 1)
	...
	if (sig_nid != NID_undef) {
		...
		if (OBJ_find_sigid_algs(sig_nid, &dig_nid, &pkey_nid)) {
			...
			if (ameth && ameth->sig_print)
				return ameth->sig_print(bp, sigalg, sig,
							indent + 4, 0);
		}
	}
#endif
	...
	for (i = 0; i < sk_X509_num(certs); i++) {
		...
		if (cert != NULL) {
			...
			if (!X509V3_extensions_print(
				    bio, NULL, X509_get0_extensions(cert),
...
int X509_STORE_CTX_print_verify_cb(int ok, X509_STORE_CTX *ctx)
{
	if (ok == 0 && ctx != NULL) {
		...
		BIO_printf(bio, "%s at depth = %d error = %d (%s)\n",
			   ...
			   X509_STORE_CTX_get_error_depth(ctx), cert_error,
			   X509_verify_cert_error_string(cert_error));
		{
			...
			case X509_V_ERR_HOSTNAME_MISMATCH:
				...
				while ((str = X509_VERIFY_PARAM_get0_host(
						vpm, idx++)) != NULL)
					BIO_printf(bio, "%s%s",
						   idx == 1 ? "" : ", ", str);
				BIO_printf(bio, "\n");
