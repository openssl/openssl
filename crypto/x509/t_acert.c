...
static int print_attribute(BIO *bp, X509_ATTRIBUTE *a)
{
	...
	for (i = 0; i < count; i++) {
		...
		default:
			if (BIO_printf(
				    bp,
				    "unable to print attribute of type 0x%X\n",
				    type) < 0)
				goto err;
			break;
		}
			...
			unsigned long cflag)
{
	...
	if ((cflag & X509_FLAG_NO_SUBJECT) == 0) {
		...
		if (holderEntities != NULL) {
			for (i = 0; i < sk_GENERAL_NAME_num(holderEntities);
			     i++) {
				GENERAL_NAME *entity;

				entity = sk_GENERAL_NAME_value(holderEntities,
							       i);

				if (BIO_printf(bp, "%12sName:%c", "", mlch) <=
				    0)
					goto err;
		...
		if ((holder_bcid = X509_ACERT_get0_holder_baseCertId(x)) !=
		    NULL)
			holderIssuer =
		...
		if (holderIssuer != NULL) {
			...
			if (X509_NAME_print_ex(bp, holderIssuer, 0, nmflags) <=
			    0)
				goto err;
			...
			holder_serial =
				OSSL_ISSUER_SERIAL_get0_serial(holder_bcid);
	...
	if ((cflag & X509_FLAG_NO_VALIDITY) == 0) {
		...
		if (ASN1_GENERALIZEDTIME_print(
			    bp, X509_ACERT_get0_notBefore(x)) == 0)
			goto err;
		if (BIO_printf(bp, "\n%12sNot After : ", "") <= 0)
			goto err;
		if (ASN1_GENERALIZEDTIME_print(
			    bp, X509_ACERT_get0_notAfter(x)) == 0)
			goto err;
		if (BIO_write(bp, "\n", 1) <= 0)
			goto err;
	}
