...
ASN1_ITEM_TEMPLATE(OSSL_ALLOWED_ATTRIBUTES_SYNTAX) = ASN1_EX_TEMPLATE_TYPE(
	ASN1_TFLG_SET_OF, 0, OSSL_ALLOWED_ATTRIBUTES_SYNTAX,
					 ...
					 BIO *out, int indent)
{
	...
	case (OSSL_AAA_ATTRIBUTE_VALUES):
		...
		for (j = 0; j < X509_ATTRIBUTE_count(attr); j++) {
			...
			if (ossl_print_attribute_value(out, attr_nid, av,
						       indent + 4) <= 0)
				return 0;
			if (BIO_puts(out, "\n") <= 0)
				return 0;
				       ...
				       BIO *out, int indent)
{
	...
	for (i = 0; i < sk_OSSL_ALLOWED_ATTRIBUTES_CHOICE_num(aai->attributes);
		...
		if (i2r_ALLOWED_ATTRIBUTES_CHOICE(method, a, out, indent + 4) <=
		    0)
			return 0;
	}
					 ...
					 BIO *out, int indent)
{
	...
	for (i = 0; i < sk_OSSL_ALLOWED_ATTRIBUTES_ITEM_num(aaa); i++) {
		...
		if (i2r_ALLOWED_ATTRIBUTES_ITEM(method, aai, out, indent + 4) <=
		    0)
			return 0;
	}
	return 1;
