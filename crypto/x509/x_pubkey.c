					...
					OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	return (X509_PUBKEY *)ASN1_item_d2i_ex(
		...
		ASN1_ITEM_rptr(X509_PUBKEY_INTERNAL), libctx, propq);
}
	...
	if (ret <= 0 && !pubkey->flag_force_legacy) {
        ...
        */
		...
		if ((dctx = OSSL_DECODER_CTX_new_for_pkey(
			     &pubkey->pkey, "DER", "SubjectPublicKeyInfo",
			     txtoidname, EVP_PKEY_PUBLIC_KEY, pubkey->libctx,
			     pubkey->propq)) != NULL)
			/*
...
X509_PUBKEY *X509_PUBKEY_new_ex(OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	pubkey = (X509_PUBKEY *)ASN1_item_new_ex(X509_PUBKEY_it(), libctx,
						 propq);
...
int i2d_PUBKEY(const EVP_PKEY *a, unsigned char **pp)
{
	...
	if (a->ameth != NULL) {
		...
		OSSL_ENCODER_CTX *ctx = OSSL_ENCODER_CTX_new_for_pkey(
			a, EVP_PKEY_PUBLIC_KEY, "DER", "SubjectPublicKeyInfo",
			NULL);
		BIO *out = BIO_new(BIO_s_mem());
		BUF_MEM *buf = NULL;

		if (OSSL_ENCODER_CTX_get_num_encoders(ctx) != 0 &&
		    out != NULL && OSSL_ENCODER_to_bio(ctx, out) &&
