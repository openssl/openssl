		     ...
		     unsigned int flags)
{
	...
	*ptree = NULL;
     ...
     */
	for (i = n - 1; i >= 0 && (explicit_policy > 0 ||
				   (ret & X509_PCY_TREE_EMPTY) == 0);
	     i--) {
		X509 *x = sk_X509_value(certs, i);
     ...
     */
	if ((tree->levels = OPENSSL_calloc(n + 1, sizeof(*tree->levels))) ==
	    NULL) {
		OPENSSL_free(tree);
		return X509_PCY_TREE_INTERNAL;
	}
	tree->nlevel = n + 1;
	level = tree->levels;
	if ((data = ossl_policy_data_new(NULL, OBJ_nid2obj(NID_any_policy),
					 0)) == NULL)
		goto bad_tree;
				    ...
				    X509_POLICY_TREE *tree)
{
	...
	for (i = 0; i < sk_X509_POLICY_NODE_num(last->nodes); i++) {
		X509_POLICY_NODE *node =
			sk_X509_POLICY_NODE_value(last->nodes, i);

		if (ossl_policy_node_match(last, node, data->valid_policy)) {
			if (ossl_policy_level_add_node(curr, data, node, tree,
						       0) == NULL)
				return 0;
			matched = 1;
		}
			   ...
			   X509_POLICY_TREE *tree)
{
	...
	for (i = 0; i < sk_X509_POLICY_DATA_num(cache->data); i++) {
		X509_POLICY_DATA *data =
			sk_X509_POLICY_DATA_value(cache->data, i);
			      ...
			      X509_POLICY_TREE *tree)
{
     ...
     * node.
     */
	if ((data = ossl_policy_data_new(NULL, id, node_critical(node))) ==
	    NULL)
		return 0;
				   ...
				   STACK_OF(X509_POLICY_NODE) *auth_nodes)
{
	...
	for (i = 0; i < sk_ASN1_OBJECT_num(policy_oids); i++) {
		...
		if (!node) {
             ...
             * from anyPolicy.
             */
			extra = ossl_policy_data_new(NULL, oid,
			...
			node = ossl_policy_level_add_node(
				NULL, extra, anyPolicy->parent, tree, 1);
			if (node == NULL) {
