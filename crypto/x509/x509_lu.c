			...
			char **ret, OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if (ctx->method->ctrl_ex != NULL)
		return ctx->method->ctrl_ex(ctx, cmd, argc, argl, ret, libctx,
					    propq);
			      ...
			      OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if (ctx->method->get_by_subject_ex != NULL)
		return ctx->method->get_by_subject_ex(ctx, type, name, ret,
						      libctx, propq);
	else
...
X509_STORE *X509_STORE_new(void)
{
	...
	if (!CRYPTO_new_ex_data(CRYPTO_EX_INDEX_X509_STORE, ret,
				&ret->ex_data)) {
		ERR_raise(ERR_LIB_X509, ERR_R_CRYPTO_LIB);
		goto err;
	}
				       ...
				       const X509_NAME *name, X509_OBJECT *ret)
{
	...
	if (tmp == NULL || type == X509_LU_CRL) {
		for (i = 0; i < sk_X509_LOOKUP_num(store->get_cert_methods);
		     i++) {
			...
			if (j !=
			    0) { /* non-zero value is considered success here */
				tmp = &stmp;
				break;
			}
		}
...
STACK_OF(X509) *X509_STORE_get1_all_certs(X509_STORE *store)
{
	...
	for (i = 0; i < sk_X509_OBJECT_num(objs); i++) {
		X509 *cert =
			X509_OBJECT_get0_X509(sk_X509_OBJECT_value(objs, i));
					  ...
					  const X509_NAME *nm)
{
	...
	if (idx < 0) {
         ...
         */
		X509_STORE_unlock(store);
		i = ossl_x509_store_ctx_get_by_subject(ctx, X509_LU_X509, nm,
						       NULL);
		if (i <= 0)
