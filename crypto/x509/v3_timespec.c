			   ...
			   BIO *out, int indent)
{
	...
	if (p->timesOfDay) {
		...
		for (i = 0; i < sk_OSSL_DAY_TIME_BAND_num(p->timesOfDay); i++) {
			...
			if (BIO_printf(out, "%*s", indent + 8, "") <= 0)
				return 0;
			if (!i2r_OSSL_DAY_TIME_BAND(method, band, out,
						    indent + 8))
				return 0;
			if (!BIO_puts(out, "\n"))
				return 0;
	...
	if (p->days) {
		if (p->days->type == OSSL_TIME_SPEC_DAY_TYPE_INT) {
			if (p->weeks != NULL) {
				if (BIO_printf(out, "%*sDays of the week: ",
					       indent + 4, "") <= 0)
					return 0;
			} else if (p->months != NULL) {
				if (BIO_printf(out, "%*sDays of the month: ",
					       indent + 4, "") <= 0)
					return 0;
			} else if (p->years != NULL) {
				if (BIO_printf(out, "%*sDays of the year: ",
					       indent + 4, "") <= 0)
					return 0;
			}
		} else {
		...
		case (OSSL_TIME_SPEC_DAY_TYPE_INT):
			for (i = 0;
			     i < sk_ASN1_INTEGER_num(p->days->choice.intDay);
			     i++) {
				big_val = sk_ASN1_INTEGER_value(
					p->days->choice.intDay, i);
				...
				if (p->weeks != NULL) {
					if (!print_int_day_of_week(out,
								   small_val))
						return 0;
				} else if (BIO_printf(out, "%lld",
						      (long long int)
							      small_val) <= 0) {
					return 0;
				}
		...
		case (OSSL_TIME_SPEC_DAY_TYPE_DAY_OF):
			...
			case (OSSL_NAMED_DAY_TYPE_BIT):
				if (!print_bit_named_day(
					    out, nd->choice.bitNamedDays))
					return 0;
				break;
			default:
				return 0;
	...
	if (p->weeks) {
		if (p->weeks->type == OSSL_TIME_SPEC_WEEKS_TYPE_INT) {
			if (p->months != NULL) {
				if (BIO_printf(out, "%*sWeeks of the month: ",
					       indent + 4, "") <= 0)
					return 0;
			} else if (p->years != NULL) {
				if (BIO_printf(out, "%*sWeeks of the year: ",
					       indent + 4, "") <= 0)
					return 0;
			}
		} else {
		...
		case (OSSL_TIME_SPEC_WEEKS_TYPE_INT):
			for (i = 0;
			     i < sk_ASN1_INTEGER_num(p->weeks->choice.intWeek);
			     i++) {
				big_val = sk_ASN1_INTEGER_value(
					p->weeks->choice.intWeek, i);
				if (!ASN1_INTEGER_get_int64(&small_val,
							    big_val))
					return 0;
				if (i > 0 && !BIO_puts(out, ", "))
					return 0;
				if (!BIO_printf(out, "%lld",
						(long long int)small_val))
					return 0;
			}
			break;
	...
	if (p->months) {
		...
		case (OSSL_TIME_SPEC_MONTH_TYPE_INT):
			for (i = 0; i < sk_ASN1_INTEGER_num(
						p->months->choice.intMonth);
			     i++) {
				big_val = sk_ASN1_INTEGER_value(
					p->months->choice.intMonth, i);
				if (!ASN1_INTEGER_get_int64(&small_val,
							    big_val))
					return 0;
				if (i > 0 && !BIO_puts(out, ", "))
	...
	if (p->years) {
		...
		for (i = 0; i < sk_ASN1_INTEGER_num(p->years); i++) {
			...
			if (i > 0 && !BIO_puts(out, ", "))
				return 0;
			if (BIO_printf(out, "%04lld",
				       (long long int)small_val) <= 0)
				return 0;
		}
	}
	return 1;
				   ...
				   int indent)
{
	...
	case (OSSL_TIME_SPEC_TIME_TYPE_ABSOLUTE):
		if (BIO_printf(out, "%*sAbsolute: ", indent, "") <= 0)
			return 0;
		if (i2r_OSSL_TIME_SPEC_ABSOLUTE(method, time->choice.absolute,
						out, indent + 4) <= 0)
			return 0;
		return BIO_puts(out, "\n");
	case (OSSL_TIME_SPEC_TIME_TYPE_PERIODIC):
		...
		for (i = 0; i < sk_OSSL_TIME_PERIOD_num(time->choice.periodic);
			...
			if (i > 0 && !BIO_puts(out, "\n"))
				return 0;
			tp = sk_OSSL_TIME_PERIOD_value(time->choice.periodic,
						       i);
