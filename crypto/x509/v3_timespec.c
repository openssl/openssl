...
static int print_int_month (BIO *out, int64_t month) {
    switch (month) {
        case (OSSL_TIME_SPEC_INT_MONTH_JAN): return BIO_puts(out, "JAN");
        case (OSSL_TIME_SPEC_INT_MONTH_FEB): return BIO_puts(out, "FEB");
        case (OSSL_TIME_SPEC_INT_MONTH_MAR): return BIO_puts(out, "MAR");
        case (OSSL_TIME_SPEC_INT_MONTH_APR): return BIO_puts(out, "APR");
        case (OSSL_TIME_SPEC_INT_MONTH_MAY): return BIO_puts(out, "MAY");
        case (OSSL_TIME_SPEC_INT_MONTH_JUN): return BIO_puts(out, "JUN");
        case (OSSL_TIME_SPEC_INT_MONTH_JUL): return BIO_puts(out, "JUL");
        case (OSSL_TIME_SPEC_INT_MONTH_AUG): return BIO_puts(out, "AUG");
        case (OSSL_TIME_SPEC_INT_MONTH_SEP): return BIO_puts(out, "SEP");
        case (OSSL_TIME_SPEC_INT_MONTH_OCT): return BIO_puts(out, "OCT");
        case (OSSL_TIME_SPEC_INT_MONTH_NOV): return BIO_puts(out, "NOV");
        case (OSSL_TIME_SPEC_INT_MONTH_DEC): return BIO_puts(out, "DEC");
        default: return 0;
    }
    return 0;
}
...
static int print_bit_month (BIO *out, ASN1_BIT_STRING *bs) {
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_BIT_MONTH_JAN)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "JAN"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_BIT_MONTH_FEB)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "FEB"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_BIT_MONTH_MAR)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "MAR"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_BIT_MONTH_APR)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "APR"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_BIT_MONTH_MAY)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "MAY"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_BIT_MONTH_JUN)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "JUN"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_BIT_MONTH_JUL)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "JUL"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_BIT_MONTH_AUG)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "AUG"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_BIT_MONTH_SEP)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "SEP"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_BIT_MONTH_OCT)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "OCT"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_BIT_MONTH_NOV)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "NOV"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_BIT_MONTH_DEC)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "DEC"))
...
static int print_bit_week (BIO *out, ASN1_BIT_STRING *bs) {
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_BIT_WEEKS_2)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_BIT_WEEKS_3)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_BIT_WEEKS_4)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_BIT_WEEKS_5)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
...
static int print_day_of_week (BIO *out, ASN1_BIT_STRING *bs) {
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_DAY_BIT_MON)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "MON"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_DAY_BIT_TUE)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "TUE"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_DAY_BIT_WED)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "WED"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_DAY_BIT_THU)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "THU"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_DAY_BIT_FRI)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "FRI"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_TIME_SPEC_DAY_BIT_SAT)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "SAT"))
...
static int print_bit_named_day (BIO *out, ASN1_BIT_STRING *bs) {
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_NAMED_DAY_BIT_MON)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "MON"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_NAMED_DAY_BIT_TUE)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "TUE"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_NAMED_DAY_BIT_WED)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "WED"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_NAMED_DAY_BIT_THU)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "THU"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_NAMED_DAY_BIT_FRI)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "FRI"))
    ...
    if (ASN1_BIT_STRING_get_bit(bs, OSSL_NAMED_DAY_BIT_SAT)) {
        if (i > 0 && !BIO_puts(out, ", "))
            return 0;
        i++;
        if (!BIO_puts(out, "SAT"))
                ...
                for (i = 0; i < sk_ASN1_INTEGER_num(p->days->choice.intDay); i++) {
                    ...
                    if (p->weeks != NULL) {
                        if (!print_int_day_of_week(out, day))
                            return 0;
                    } else if (BIO_printf(out, "%ld", day) <= 0)
                        return 0;
                }
                break;
            }
