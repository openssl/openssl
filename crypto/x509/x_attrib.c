			       ...
			       int indent)
{
	...
	case V_ASN1_ENUMERATED:
		if (BIO_printf(out, "%*s", indent, "") < 0)
			return 0;
		if (ASN1_ENUMERATED_get_int64(&int_val, av->value.integer) >
		    0) {
	...
	case V_ASN1_SEQUENCE:
		...
		case NID_owner:
			...
			if (xn == NULL) {
				BIO_puts(
					out,
					"(COULD NOT DECODE DISTINGUISHED NAME)\n");
				return 0;
			}
			if (X509_NAME_print_ex(out, xn, indent,
					       XN_FLAG_SEP_CPLUS_SPC) <= 0)
				ret = 0;
			X509_NAME_free(xn);
			return ret;
		...
		return ASN1_parse_dump(out, av->value.sequence->data,
				       av->value.sequence->length, indent,
				       1) > 0;
