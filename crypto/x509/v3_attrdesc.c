		    ...
		    int indent)
{
	...
	if (hash->algorithmIdentifier->parameter) {
		...
		if (ossl_print_attribute_value(
			    out, 0, hash->algorithmIdentifier->parameter,
			    indent + 4) <= 0)
			return 0;
		if (BIO_puts(out, "\n") <= 0)
			return 0;
				...
				OSSL_INFO_SYNTAX *info, BIO *out, int indent)
{
	...
	case OSSL_INFO_SYNTAX_TYPE_POINTER:
		if (BIO_printf(out, "%*sPointer:\n", indent, "") <= 0)
			return 0;
		return i2r_INFO_SYNTAX_POINTER(method, info->choice.pointer,
					       out, indent + 4);
	default:
		return 0;
	}
					...
					BIO *out, int indent)
{
	...
	if (BIO_printf(out, "%*sPrivilege Policy Syntax:\n", indent, "") <= 0)
		return 0;
	return i2r_OSSL_INFO_SYNTAX(method, ppid->privPolSyntax, out,
				    indent + 4);
}
