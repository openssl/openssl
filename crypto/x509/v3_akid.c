...
const X509V3_EXT_METHOD ossl_v3_akey_id = { NID_authority_key_identifier,
					    ...
					    STACK_OF(CONF_VALUE) *values)
{
	...
	if (n == 1 &&
	    strcmp(sk_CONF_VALUE_value(values, 0)->name, "none") == 0) {
		return akeyid;
	}
	...
	if (ctx->issuer_pkey != NULL)
		ss = X509_check_private_key(ctx->subject_cert,
					    ctx->issuer_pkey);
	else
		ss = same_issuer;
	ERR_pop_to_mark();
	...
	if (keyid == 2 || (keyid == 1 && !ss)) {
         ...
         */
		i = X509_get_ext_by_NID(issuer_cert, NID_subject_key_identifier,
					-1);
		if (i >= 0 && (ext = X509_get_ext(issuer_cert, i)) != NULL &&
		    !(same_issuer && !ss)) {
			ikeyid = X509V3_EXT_d2i(ext);
			if (ASN1_STRING_length(ikeyid) ==
			    0) /* indicating "none" */ {
