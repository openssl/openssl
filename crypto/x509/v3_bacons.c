...
static STACK_OF(CONF_VALUE) *i2v_OSSL_BASIC_ATTR_CONSTRAINTS(
    ...
    OSSL_BASIC_ATTR_CONSTRAINTS *bcons,
    STACK_OF(CONF_VALUE)
    *extlist);
...
static OSSL_BASIC_ATTR_CONSTRAINTS *v2i_OSSL_BASIC_ATTR_CONSTRAINTS(
    ...
    STACK_OF(CONF_VALUE) *values)
{
    OSSL_BASIC_ATTR_CONSTRAINTS *bcons = NULL;
    CONF_VALUE *val;
    int i;

    if ((bcons = OSSL_BASIC_ATTR_CONSTRAINTS_new()) == NULL) {
    ...
    return bcons;
 err:
    OSSL_BASIC_ATTR_CONSTRAINTS_free(bcons);
    return NULL;
}
