		       ...
		       const ASN1_BIT_STRING *bs)
{
	...
	case IANA_AFI_IPV6:
		if (!addr_expand(addr, bs, 16, fill))
			return 0;
		for (n = 16;
		     n > 1 && addr[n - 1] == 0x00 && addr[n - 2] == 0x00;
		     n -= 2)
			;
		for (i = 0; i < n; i += 2)
			   ...
			   unsigned char *max, int length)
{
	...
	case IPAddressOrRange_addressRange:
		return (addr_expand(min, aor->u.addressRange->min, length,
				    0x00) &&
			addr_expand(max, aor->u.addressRange->max, length,
				    0xFF));
	}
	return 0;
}
...
int X509v3_addr_is_canonical(IPAddrBlocks *addr)
{
	...
	for (i = 0; i < sk_IPAddressFamily_num(addr) - 1; i++) {
		...
		if (!IPAddressFamily_check_len(a) ||
		    !IPAddressFamily_check_len(b))
			return 0;
	...
	for (i = 0; i < sk_CONF_VALUE_num(values); i++) {
		...
		if (!ossl_v3_name_cmp(val->name, "IPv4")) {
			...
			ERR_raise_data(ERR_LIB_X509V3,
				       X509V3_R_EXTENSION_NAME_ERROR, "%s",
				       val->name);
			goto err;
		}
		...
		case '/':
			...
			if (t == s + i2 || *t != '\0' ||
			    prefixlen > (length * 8) || prefixlen < 0) {
				ERR_raise(ERR_LIB_X509V3,
		...
		for (j = 0; j < sk_IPAddressFamily_num(child); j++) {
			...
			if (fp->ipAddressChoice->type ==
				...
				if (fc->ipAddressChoice->type ==
					    IPAddressChoice_inherit ||
				    addr_contains(
					    fp->ipAddressChoice->u
						    .addressesOrRanges,
					    fc->ipAddressChoice->u
						    .addressesOrRanges,
					    length_from_afi(
						    X509v3_addr_get_afi(fc))))
					(void)sk_IPAddressFamily_set(child, j,
								     fp);
				else
					validation_err(
						X509_V_ERR_UNNESTED_RESOURCE);
