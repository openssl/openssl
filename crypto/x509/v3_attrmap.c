...
ASN1_ITEM_TEMPLATE(OSSL_ATTRIBUTE_MAPPINGS) = ASN1_EX_TEMPLATE_TYPE(
	ASN1_TFLG_SET_OF, 0, OSSL_ATTRIBUTE_MAPPINGS, OSSL_ATTRIBUTE_MAPPING)
				 ...
				 int indent)
{
	...
	case (OSSL_ATTR_MAP_VALUE):
		...
		if (BIO_puts(out, ":") <= 0)
			return 0;
		if (ossl_print_attribute_value(out, local_attr_nid, local_val,
					       0) <= 0)
			return 0;
		if (BIO_puts(out, " == ") <= 0)
			return 0;
		...
		if (BIO_puts(out, ":") <= 0)
			return 0;
		return ossl_print_attribute_value(out, remote_attr_nid,
						  remote_val, 0);
	default:
		return 0;
	}
