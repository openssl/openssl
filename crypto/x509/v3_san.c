...
const X509V3_EXT_METHOD ossl_v3_alt[3] = {
	{ NID_subject_alt_name, 0, ASN1_ITEM_ref(GENERAL_NAMES), 0, 0, 0, 0, 0,
	  0, (X509V3_EXT_I2V)i2v_GENERAL_NAMES, (X509V3_EXT_V2I)v2i_subject_alt,
	  NULL, NULL, NULL },

	{ NID_issuer_alt_name, 0, ASN1_ITEM_ref(GENERAL_NAMES), 0, 0, 0, 0, 0,
	  0, (X509V3_EXT_I2V)i2v_GENERAL_NAMES, (X509V3_EXT_V2I)v2i_issuer_alt,
	  NULL, NULL, NULL },

	{ NID_certificate_issuer, 0, ASN1_ITEM_ref(GENERAL_NAMES), 0, 0, 0, 0,
	  0, 0, (X509V3_EXT_I2V)i2v_GENERAL_NAMES, NULL, NULL, NULL, NULL },
};
				       ...
				       STACK_OF(CONF_VALUE) *ret)
{
	...
	case GEN_OTHERNAME:
		...
		case NID_NAIRealm:
			...
			break;
		default:
			if (OBJ_obj2txt(oline, sizeof(oline),
					gen->d.otherName->type_id, 0) > 0)
				BIO_snprintf(othername, sizeof(othername),
					     "othername: %s", oline);
			else
				OPENSSL_strlcpy(othername, "othername",
						sizeof(othername));
	...
	case GEN_DIRNAME:
		if (X509_NAME_oneline(gen->d.dirn, oline, sizeof(oline)) ==
			    NULL ||
...
int GENERAL_NAME_print(BIO *out, GENERAL_NAME *gen)
{
	...
	case GEN_OTHERNAME:
		...
		case NID_id_on_SmtpUTF8Mailbox:
			BIO_printf(
				out, "othername:SmtpUTF8Mailbox:%.*s",
				gen->d.otherName->value->value.utf8string
					->length,
				     ...
				     STACK_OF(CONF_VALUE) *nval)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (!ossl_v3_name_cmp(cnf->name, "issuer") && cnf->value &&
			...
			sk_GENERAL_NAME_push(
				gens, gen); /* no failure as it was reserved */
		}
	}
	return gens;
err:
...
static int copy_issuer(X509V3_CTX *ctx, GENERAL_NAMES *gens)
{
	...
	for (i = 0; i < num; i++) {
		...
		sk_GENERAL_NAME_push(gens,
				     gen); /* no failure as it was reserved */
	}
				      ...
				      STACK_OF(CONF_VALUE) *nval)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (ossl_v3_name_cmp(cnf->name, "email") == 0 && cnf->value &&
		    strcmp(cnf->value, "copy") == 0) {
...
static int copy_email(X509V3_CTX *ctx, GENERAL_NAMES *gens, int move_p)
{
	...
	while ((i = X509_NAME_get_index_by_NID(nm, NID_pkcs9_emailAddress,
					       i)) >= 0) {
