				  ...
				  int indent, const char *msg)
{
	...
	case ASIdentifierChoice_asIdsOrRanges:
		for (i = 0; i < sk_ASIdOrRange_num(choice->u.asIdsOrRanges);
		     i++) {
			ASIdOrRange *aor = sk_ASIdOrRange_value(
				choice->u.asIdsOrRanges, i);
			switch (aor->type) {
			case ASIdOrRange_id:
			...
			case ASIdOrRange_range:
				if ((s = i2s_ASN1_INTEGER(
					     NULL, aor->u.range->min)) == NULL)
					return 0;
				BIO_printf(out, "%*s%s-", indent + 2, "", s);
				OPENSSL_free(s);
				if ((s = i2s_ASN1_INTEGER(
					     NULL, aor->u.range->max)) == NULL)
					return 0;
				BIO_printf(out, "%s\n", s);
				OPENSSL_free(s);
				break;
...
static int ASIdentifierChoice_is_canonical(ASIdentifierChoice *choice)
{
	...
	for (i = 0; i < sk_ASIdOrRange_num(choice->u.asIdsOrRanges) - 1; i++) {
		...
		ASN1_INTEGER *a_min = NULL, *a_max = NULL, *b_min = NULL,
			     *b_max = NULL;
		...
		if ((a_max_plus_one = BN_to_ASN1_INTEGER(
			     bn, orig = a_max_plus_one)) == NULL) {
			a_max_plus_one = orig;
...
int X509v3_asid_subset(ASIdentifiers *a, ASIdentifiers *b)
{
	...
	return a->rdi == NULL ||
	       (b->rdi != NULL && asid_contains(b->rdi->u.asIdsOrRanges,
