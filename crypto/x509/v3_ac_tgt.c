	...
	if (sig_nid != NID_undef) {
		...
		if (OBJ_find_sigid_algs(sig_nid, &dig_nid, &pkey_nid)) {
			...
			if (ameth && ameth->sig_print)
				return ameth->sig_print(out, digalg, sig,
							indent + 4, 0);
		}
	}
#endif
			   ...
			   BIO *out, int indent)
{
	...
	if (tc->targetCertificate != NULL) {
		...
		i2r_ISSUER_SERIAL(method, tc->targetCertificate, out,
				  indent + 2);
	}
	...
	if (tc->certDigestInfo != NULL) {
		...
		i2r_OBJECT_DIGEST_INFO(method, tc->certDigestInfo, out,
				       indent + 2);
	}
	BIO_puts(out, "\n");
	return 1;
		      ...
		      int indent)
{
	...
	case OSSL_TGT_TARGET_CERT:
		...
		i2r_TARGET_CERT(method, target->choice.targetCert, out,
				indent + 2);
		break;
	}
	return 1;
