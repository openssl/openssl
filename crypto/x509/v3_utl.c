...
static int check_subject(int flags, int def)
{
    ...
    unsigned int flags, char **peername)
{
    if (do_x509_check(x, chk, chklen, flags, GEN_DNS, 0, NID_undef, peername)
        ...
        return 1;
    /*
     * If we have the legacy flag to force the check, check the subject
     * This should be removed in 5.0
     */
    if (check_subject(flags, 0)
...
int ossl_x509_check_rfc822_subject(const X509 *x, const char *chk, size_t chklen, unsigned int flags)
{
...
int X509_check_ip_asc(const X509 *x, const char *ipasc, unsigned int flags)
{
    ...
    return ossl_x509_check_ip(x, ipout, iplen, flags);
}
#endif /* !defined(OPENSSL_NO_DEPRECATED_4_1) */
