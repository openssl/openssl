		     ...
		     STACK_OF(CONF_VALUE) **extlist)
{
	return x509v3_add_len_value(
		name, value, value != NULL ? strlen((const char *)value) : 0,
		extlist);
}
					   ...
					   GENERAL_NAMES *gens)
{
	...
	while ((i = X509_NAME_get_index_by_NID(name, NID_pkcs9_emailAddress,
					       i)) >= 0) {
		       ...
		       unsigned int unused_flags)
{
	...
	while (i > 0) {
		--i;
		if (a[i] == '@' || b[i] == '@') {
			if (!equal_nocase(a + i, a_len - i, b + i, a_len - i,
					  0))
				return 0;
			break;
		}
			   ...
			   char **peername)
{
	...
	if (cmp_type > 0) {
		...
		if (cmp_type == V_ASN1_IA5STRING)
			rv = equal(a->data, a->length, (unsigned char *)b, blen,
				   flags);
		...
		for (i = 0; i < sk_GENERAL_NAME_num(gens); i++) {
			...
			case GEN_OTHERNAME:
				...
				case NID_id_on_SmtpUTF8Mailbox:
                     ...
                     */
					if (check_type != GEN_EMAIL ||
					    gen->d.otherName->value->type !=
						    V_ASN1_UTF8STRING)
						continue;
					alt_type = 0;
					cstr = gen->d.otherName->value->value
						       .utf8string;
					break;
				}
				break;
...
char *ossl_ipaddr_to_asc(unsigned char *p, int len)
{
	...
	case 16: /* IPv6 */
		for (out = buf, i = 8, remain = sizeof(buf);
		     i-- > 0 && bytes >= 0; remain -= bytes, out += bytes) {
			const char *template = (i > 0 ? "%X:" : "%X");

			bytes = BIO_snprintf(out, remain, template,
					     p[0] << 8 | p[1]);
			p += 2;
		}
		break;
