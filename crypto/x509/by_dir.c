		    ...
		    char **retp)
{
	...
	case X509_L_ADD_DIR:
		if (argl == X509_FILETYPE_DEFAULT) {
			const char *dir = ossl_safe_getenv(
	...
	for (i = 0; i < sk_BY_DIR_ENTRY_num(ctx->dirs); i++) {
		...
		for (;;) {
			...
			if (c == '\0') {
                 ...
                 * should be added.
                 */
				BIO_snprintf(b->data, b->max, "%s%08lx.%s%d",
					     ent->dir, h, postfix, k);
			} else
#endif
			{
				BIO_snprintf(b->data, b->max, "%s%c%08lx.%s%d",
					     ent->dir, c, h, postfix, k);
			}
#ifndef OPENSSL_NO_POSIX_IO
#ifdef _WIN32
#define stat _stat
			...
			/* found one. */
			if (type == X509_LU_X509) {
				if ((X509_load_cert_file_ex(
					    xl, b->data, ent->dir_type, libctx,
					    propq)) == 0)
					break;
			} else if (type == X509_LU_CRL) {
