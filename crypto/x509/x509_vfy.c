...
static int check_auth_level(X509_STORE_CTX *ctx)
{
	...
	for (i = 0; i < num; ++i) {
         ...
         */
		CB_FAIL_IF(i > 0 && !check_cert_key_level(ctx, cert), ctx, cert,
			   i, X509_V_ERR_CA_KEY_TOO_SMALL);
		/*
         * We also check the signature algorithm security of all certificates
         * except those of the trust anchor at index num-1.
         */
		CB_FAIL_IF(i < num - 1 && !check_sig_level(ctx, cert), ctx,
			   cert, i, X509_V_ERR_CA_MD_TOO_WEAK);
	}
	return 1;
}
...
int X509_verify_cert(X509_STORE_CTX *ctx)
{
	...
	return (ctx->rpk != NULL) ? x509_verify_rpk(ctx) :
...
static int x509_verify_rpk(X509_STORE_CTX *ctx)
{
	...
	ret = DANETLS_ENABLED(ctx->dane) ? dane_verify_rpk(ctx) :
					   verify_rpk(ctx);
...
static int x509_verify_x509(X509_STORE_CTX *ctx)
{
	...
	if (!ossl_x509_add_cert_new(&ctx->chain, ctx->cert,
	...
	for (i = 0; i < sk_X509_num(sk); i++) {
		...
		if (ctx->check_issued(ctx, x, candidate)) {
			if (check_signing_allowed
			    ...
			    && ossl_x509_signing_allowed(candidate, x) !=
				       X509_V_OK)
				continue;
...
int X509_STORE_CTX_get1_issuer(X509 **issuer, X509_STORE_CTX *ctx, X509 *x)
{
	...
	*issuer = NULL;
	...
	if ((certs = X509_STORE_CTX_get1_certs(ctx, xn)) == NULL)
		goto end;
	*issuer =
		get0_best_issuer_sk(ctx, 0, 0 /* allow duplicates */, certs, x);
	ret = 0;
	if (*issuer != NULL)
...
{
	*issuer =
		get0_best_issuer_sk(ctx, 0, 1 /* no_dup */, ctx->other_ctx, x);
	if (*issuer == NULL)
		return 0;
			 ...
			 int must_be_ca)
{
	...
	if (depth >= ctx->num_untrusted && purpose == ctx->param->purpose)
		tr_ok = X509_check_trust(x, ctx->param->trust,
...
static int check_extensions(X509_STORE_CTX *ctx)
{
	...
	if (ctx->parent != NULL) {
		...
		allow_proxy_certs = (ctx->param->flags &
	...
	for (i = 0; i < num; i++) {
		...
		CB_FAIL_IF((ctx->param->flags & X509_V_FLAG_IGNORE_CRITICAL) ==
					   0 &&
				   (x->ex_flags & EXFLAG_CRITICAL) != 0,
			   ctx, x, i, X509_V_ERR_UNHANDLED_CRITICAL_EXTENSION);
		CB_FAIL_IF(
			!allow_proxy_certs && (x->ex_flags & EXFLAG_PROXY) != 0,
			ctx, x, i, X509_V_ERR_PROXY_CERTIFICATES_NOT_ALLOWED);
		ret = X509_check_ca(x);
		switch (must_be_ca) {
		case -1:
			CB_FAIL_IF((ctx->param->flags &
				    X509_V_FLAG_X509_STRICT) != 0 &&
					   ret != 1 && ret != 0,
				   ctx, x, i, X509_V_ERR_INVALID_CA);
			break;
		case 0:
			CB_FAIL_IF(ret != 0, ctx, x, i,
		...
		if ((ctx->param->flags & X509_V_FLAG_X509_STRICT) != 0 &&
			...
			CB_FAIL_IF(X509_NAME_entry_count(
			...
			CB_FAIL_IF(
				X509_ALGOR_cmp(&x->sig_alg,
					       &x->cert_info.signature) != 0,
				ctx, x, i,
				X509_V_ERR_SIGNATURE_ALGORITHM_INCONSISTENCY);
			CB_FAIL_IF(
				x->akid != NULL && (x->ex_flags &
						    EXFLAG_AKID_CRITICAL) != 0,
				ctx, x, i,
				X509_V_ERR_AUTHORITY_KEY_IDENTIFIER_CRITICAL);
			CB_FAIL_IF(x->skid != NULL &&
					   (x->ex_flags &
					    EXFLAG_SKID_CRITICAL) != 0,
				   ctx, x, i,
...
static int check_name_constraints(X509_STORE_CTX *ctx)
{
	...
	for (i = sk_X509_num(ctx->chain) - 1; i >= 0; i--) {
		...
		if ((x->ex_flags & EXFLAG_PROXY) != 0) {
             ...
             * multi-valued RDN
             */
			if (X509_NAME_ENTRY_set(X509_NAME_get_entry(
				    tmpsubject, last_loc)) ==
			    X509_NAME_ENTRY_set(X509_NAME_get_entry(
				    tmpsubject, last_loc - 1))) {
				err = X509_V_ERR_PROXY_SUBJECT_NAME_VIOLATION;
				goto proxy_name_done;
			}
			...
			last_nid = OBJ_obj2nid(
				X509_NAME_ENTRY_get_object(tmpentry));
...
static int check_hosts(X509 *x, X509_VERIFY_PARAM *vpm)
{
	...
	for (i = 0; i < n; ++i) {
		...
		if (X509_check_host(x, name, 0, vpm->hostflags,
				    &vpm->peername) > 0)
			return 1;
	}
	return n == 0;
...
static int check_revocation(X509_STORE_CTX *ctx)
{
	...
	int ocsp_check_enabled =
		(ctx->param->flags & (X509_V_FLAG_OCSP_RESP_CHECK |
				      X509_V_FLAG_OCSP_RESP_CHECK_ALL)) != 0;
	...
	if (ocsp_check_enabled) {
		...
		for (i = 0; i <= last; i++) {
			...
			if (crl_check_all_enabled ||
			    (crl_check_enabled && i == 0)) {
				ok = check_cert_crl(ctx);
				if (!ok)
					return ok;
			} else {
				ok = verify_cb_ocsp(
					ctx, X509_V_ERR_OCSP_VERIFY_FAILED);
				if (!ok)
					return ok;
			}
		}
...
static int check_cert_ocsp_resp(X509_STORE_CTX *ctx)
{
	...
	if (OCSP_basic_verify(bs, ctx->chain, ctx->store, OCSP_TRUSTOTHER) <=
	    0) {
	...
	for (i = 0; i < num; i++) {
		...
		if (cert_id_md_oid != NULL)
			cert_id_md =
				(EVP_MD *)EVP_get_digestbyobj(cert_id_md_oid);
		else
			cert_id_md = NULL;
...
static int check_crl_time(X509_STORE_CTX *ctx, X509_CRL *crl, int notify)
{
	...
	if (X509_CRL_get0_nextUpdate(crl)) {
		...
		if (i < 0 &&
			...
			if (!notify ||
			    !verify_cb_crl(ctx, X509_V_ERR_CRL_HAS_EXPIRED))
				return 0;
		}
	}
...
static int internal_verify(X509_STORE_CTX *ctx)
{
	...
	while (n >= 0) {
		...
		if (xi != NULL &&
			...
			CB_FAIL_IF(ret != X509_V_OK, ctx, xi, issuer_depth,
				   ret);
			if ((pkey = X509_get0_pubkey(xi)) == NULL) {
				CB_FAIL_IF(
					1, ctx, xi, issuer_depth,
					X509_V_ERR_UNABLE_TO_DECODE_ISSUER_PUBLIC_KEY);
			} else {
				CB_FAIL_IF(X509_verify(xs, pkey) <= 0, ctx, xs,
					   n,
...
int X509_cmp_time(const ASN1_TIME *ctm, time_t *cmp_time)
{
	...
	static const size_t generalizedtime_length =
			...
			const EVP_MD *md, unsigned int flags)
{
	...
	for (i = 0; i < sk_X509_REVOKED_num(revs); i++) {
         ...
         * multiple CAs.
         */
		if (!X509_CRL_get0_by_serial(base, &rvtmp,
					     &rvn->serialNumber)) {
			...
			STACK_OF(X509) *chain)
{
	...
	if (store == NULL)
		ctx->param->inh_flags |= X509_VP_FLAG_DEFAULT |
					 X509_VP_FLAG_ONCE;
...
int X509_STORE_CTX_set_default(X509_STORE_CTX *ctx, const char *name)
{
	...
	if (param == NULL) {
		ERR_raise_data(ERR_LIB_X509, X509_R_UNKNOWN_PURPOSE_ID,
			       "name=%s", name);
		return 0;
	}
...
static int dane_match_cert(X509_STORE_CTX *ctx, X509 *cert, int depth)
{
     ...
     */
	recnum = (dane->umask & mask) != 0 ?
			 sk_danetls_record_num(dane->trecs) :
			 0;
	for (i = 0; matched == 0 && i < recnum; ++i) {
		...
		if (t->mtype != mtype) {
			...
			if (md != NULL) {
				cmpbuf = mdbuf;
				if (!EVP_Digest(i2dbuf, i2dlen, cmpbuf, &cmplen,
						md, 0)) {
					matched = -1;
					break;
				}
...
#define S_DOALTERNATE (1 << 2) /* Retry with pruned alternate chain */
     ...
     */
	if (!X509_add_certs(sk_untrusted, ctx->untrusted,
	...
	while (search != 0) {
		...
		if ((search & S_DOTRUSTED) != 0) {
			...
			ok = num > max_depth ?
				     0 :
				     get1_trusted_issuer(&issuer, ctx, curr);
			...
			if (ok > 0) {
                 ...
                 */
				if (ok) {
					if (!ossl_assert(ctx->num_untrusted <=
							 num))
						goto int_err;
		...
		if ((search & S_DOUNTRUSTED) != 0) {
			...
			issuer = (X509_self_signed(curr, 0) > 0 ||
				  num > max_depth) ?
					 NULL :
					 get0_best_issuer_sk(ctx, 0,
							     1 /* no_dup */,
							     sk_untrusted,
							     curr);
			if (issuer == NULL) {
			...
			if (!X509_add_cert(ctx->chain, issuer,
					   X509_ADD_FLAG_UP_REF))
				goto int_err;
	...
	default:
		...
		CB_FAIL_IF(DANETLS_ENABLED(dane) && (!DANETLS_HAS_PKIX(dane) ||
						     dane->pdpth >= 0),
				 ...
				 OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if ((ctx = X509_STORE_CTX_new_ex(libctx, propq)) == NULL)
		return NULL;
	if (!X509_STORE_CTX_init(ctx, store, target,
				 finish_chain ? certs : NULL))
		goto err;
	if (!finish_chain)
		X509_STORE_CTX_set0_trusted_stack(ctx, certs);
	if (!ossl_x509_add_cert_new(&ctx->chain, target,
				    X509_ADD_FLAG_UP_REF)) {
		ctx->error = X509_V_ERR_OUT_OF_MEM;
		goto err;
	}
