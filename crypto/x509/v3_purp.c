...
static X509_PURPOSE xstandard[] = {
	...
	{ X509_PURPOSE_NS_SSL_SERVER, X509_TRUST_SSL_SERVER, 0,
	  check_purpose_ns_ssl_server, "Netscape SSL server", "nssslserver",
	  NULL },
	{ X509_PURPOSE_SMIME_SIGN, X509_TRUST_EMAIL, 0,
	  check_purpose_smime_sign, "S/MIME signing", "smimesign", NULL },
	{ X509_PURPOSE_SMIME_ENCRYPT, X509_TRUST_EMAIL, 0,
	  check_purpose_smime_encrypt, "S/MIME encryption", "smimeencrypt",
	  NULL },
...
static int check_sig_alg_match(const EVP_PKEY *issuer_key, const X509 *subject)
{
	...
	if (OBJ_find_sigid_algs(
		    OBJ_obj2nid(subject->cert_info.signature.algorithm), NULL,
		    &subj_sig_nid) == 0)
...
int ossl_x509v3_cache_extensions(X509 *x)
{
	...
	if ((x->ex_flags & EXFLAG_SET) !=
	    0) { /* Cert has already been processed */
	...
	if (x->rfc3779_addr == NULL && i != -1)
		x->ex_flags |= EXFLAG_INVALID;
	x->rfc3779_asid =
		X509_get_ext_d2i(x, NID_sbgp_autonomousSysNum, &i, NULL);
				    ...
				    int non_leaf)
{
	...
	if (!ret || non_leaf)
		return ret;
	return ku_reject(x, KU_DIGITAL_SIGNATURE | KU_NON_REPUDIATION) ? 0 :
									 ret;
}
...
int X509_check_akid(const X509 *issuer, const AUTHORITY_KEYID *akid)
{
	...
	if (akid->issuer) {
		...
		if (nm != NULL &&
		    X509_NAME_cmp(nm, X509_get_issuer_name(issuer)) != 0)
			return X509_V_ERR_AKID_ISSUER_SERIAL_MISMATCH;
	}
	return X509_V_OK;
}
