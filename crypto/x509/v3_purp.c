 ...
 * x->sha256_hash is filled in, or else EXFLAG_INVALID | EXFLAG_NO_FINGERPRINT
 * is set in x->flags.
...
int ossl_x509v3_cache_extensions(const X509 *const_x)
{
    ...
    unsigned char tmp_sha256_hash[SHA256_DIGEST_LENGTH];
    long tmp_ex_pathlen;
    ...
    /* Cache the SHA256 digest of the cert */
    if (!X509_digest(const_x, EVP_sha256(), tmp_sha256_hash, NULL))
        tmp_ex_flags |= EXFLAG_INVALID | EXFLAG_NO_FINGERPRINT;
    ...
    if (!(tmp_ex_flags & EXFLAG_NO_FINGERPRINT))
        memcpy(((X509 *)const_x)->sha256_hash, tmp_sha256_hash, SHA256_DIGEST_LENGTH);
