	...
	for (;;) {
		...
		if (infotype == OSSL_STORE_INFO_NAME) {
			...
			if (depth > 0) {
				...
				substore.uri =
					(char *)OSSL_STORE_INFO_get0_NAME(info);
				substore.libctx = store->libctx;
				substore.propq = store->propq;
				ok = cache_objects(lctx, &substore, criterion,
						   depth - 1);
			}
		} else {
			/*
			...
			case OSSL_STORE_INFO_CERT:
				ok = X509_STORE_add_cert(
					xstore,
					OSSL_STORE_INFO_get0_CERT(info));
				break;
			case OSSL_STORE_INFO_CRL:
				ok = X509_STORE_add_crl(
					xstore, OSSL_STORE_INFO_get0_CRL(info));
				break;
			}
		}
			    ...
			    const char *propq)
{
	...
	case X509_L_ADD_STORE:
		if (argp != NULL) {
             ...
             * errors early.
             */
			sctx = OSSL_STORE_open_ex(argp, libctx, propq, NULL,
						  NULL, NULL, NULL, NULL);
			if (sctx == NULL ||
			...
			if (stores == NULL) {
				...
				if (stores != NULL)
					X509_LOOKUP_set_method_data(ctx,
								    stores);
			}
			if (stores == NULL ||
		    ...
		    const OSSL_STORE_SEARCH *criterion, X509_OBJECT *ret)
{
	...
	for (i = 0; i < sk_CACHED_STORE_num(stores); i++) {
		ok = cache_objects(ctx, sk_CACHED_STORE_value(stores, i),
				   criterion, 1 /* depth */);
...
{
	OSSL_STORE_SEARCH *criterion = OSSL_STORE_SEARCH_by_name(
	...
	if (ok) {
		...
		if (!ossl_x509_store_read_lock(store))
			return 0;
		tmp = X509_OBJECT_retrieve_by_subject(store_objects, type,
						      name);
