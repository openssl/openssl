...
ASN1_SEQUENCE(OSSL_ROLE_SPEC_CERT_ID) = {
	...
	ASN1_IMP_OPT(OSSL_ROLE_SPEC_CERT_ID, roleCertSerialNumber, ASN1_INTEGER,
		     2),
	ASN1_IMP_SEQUENCE_OF_OPT(OSSL_ROLE_SPEC_CERT_ID, roleCertLocator,
				 GENERAL_NAME, 3),
...
ASN1_ITEM_TEMPLATE(OSSL_ROLE_SPEC_CERT_ID_SYNTAX) = ASN1_EX_TEMPLATE_TYPE(
	ASN1_TFLG_SEQUENCE_OF, 0, OSSL_ROLE_SPEC_CERT_ID_SYNTAX,
				      ...
				      int indent)
{
	...
	if (rscid->roleCertSerialNumber != NULL) {
		if (BIO_puts(out, "\n") <= 0)
			return 0;
		if (BIO_printf(out, "%*sRole Certificate Serial Number:",
			       indent, "") <= 0)
			return 0;
	...
	if (rscid->roleCertLocator != NULL) {
		...
		if (BIO_printf(out, "%*sRole Certificate Locator:\n", indent,
			       "") <= 0)
			return 0;
		if (OSSL_GENERAL_NAMES_print(out, rscid->roleCertLocator,
					     indent) <= 0)
			return 0;
	}
	return BIO_puts(out, "\n");
				  ...
				  BIO *out, int indent)
{
	...
	for (i = 0; i < sk_OSSL_ROLE_SPEC_CERT_ID_num(rscids); i++) {
		...
		if (i2r_OSSL_ROLE_SPEC_CERT_ID(method, rscid, out,
					       indent + 4) != 1)
			return 0;
	}
	return 1;
