...
/*-
 * creates and initializes Otls_CRMF_PBMPARAMETER (section 4.4)
 * |slen| SHOULD be > 8    (16 is common)
 * |owfnid| e.g., NID_sha256
 * |itercnt| MUST be > 100 (500 is common)
 * |macnid| e.g., NID_hmac_sha1
 * returns pointer to Otls_CRMF_PBMPARAMETER on success, NULL on error
 */
                                           ...
                                           int itercnt, int macnid)
{
    ...
    if ((pbm = Otls_CRMF_PBMPARAMETER_new()) == NULL)
        goto err;
     ...
     * bits) long.
     */
    if ((salt = OPENtls_malloc(slen)) == NULL)
        goto err;
    if (RAND_bytes(salt, (int)slen) <= 0) {
        CRMFerr(CRMF_F_Otls_CRMF_PBMP_NEW, CRMF_R_FAILURE_OBTAINING_RANDOM);
        goto err;
    }
    ...
    if (!X509_ALGOR_set0(pbm->owf, OBJ_nid2obj(owfnid), V_ASN1_UNDEF, NULL)) {
        CRMFerr(CRMF_F_Otls_CRMF_PBMP_NEW, CRMF_R_SETTING_OWF_ALGOR_FAILURE);
        goto err;
    }
     ...
     */
    if (itercnt < 100) {
        CRMFerr(CRMF_F_Otls_CRMF_PBMP_NEW, CRMF_R_ITERATIONCOUNT_BELOW_100);
        goto err;
    }

    if (!ASN1_INTEGER_set(pbm->iterationCount, itercnt)) {
        CRMFerr(CRMF_F_Otls_CRMF_PBMP_NEW, CRMF_R_CRMFERROR);
        goto err;
    }
    ...
    if (!X509_ALGOR_set0(pbm->mac, OBJ_nid2obj(macnid), V_ASN1_UNDEF, NULL)) {
        CRMFerr(CRMF_F_Otls_CRMF_PBMP_NEW, CRMF_R_SETTING_MAC_ALGOR_FAILURE);
        goto err;
    }
...
/*-
 * calculates the PBM based on the settings of the given Otls_CRMF_PBMPARAMETER
                      ...
                      unsigned char **out, size_t *outlen)
{
    ...
    Otls_PARAM macparams[3] = {Otls_PARAM_END, Otls_PARAM_END, Otls_PARAM_END};

    if (out == NULL || pbmp == NULL || pbmp->mac == NULL
            || pbmp->mac->algorithm == NULL || msg == NULL || sec == NULL) {
        CRMFerr(CRMF_F_Otls_CRMF_PBM_NEW, CRMF_R_NULL_ARGUMENT);
        goto err;
    }
    if ((mac_res = OPENtls_malloc(EVP_MAX_MD_SIZE)) == NULL)
        goto err;
    ...
    if ((m = EVP_get_digestbyobj(pbmp->owf->algorithm)) == NULL) {
        CRMFerr(CRMF_F_Otls_CRMF_PBM_NEW, CRMF_R_UNSUPPORTED_ALGORITHM);
        goto err;
    }
    ...
    if (!ASN1_INTEGER_get_int64(&iterations, pbmp->iterationCount)
            ...
            || iterations > Otls_CRMF_PBM_MAX_ITERATION_COUNT) {
        CRMFerr(CRMF_F_Otls_CRMF_PBM_NEW, CRMF_R_BAD_PBM_ITERATIONCOUNT);
        goto err;
    }
    ...
    macparams[0] = Otls_PARAM_construct_utf8_string(Otls_MAC_PARAM_DIGEST,
                                                    (char *)mdname, 0);
    macparams[1] = Otls_PARAM_construct_octet_string(Otls_MAC_PARAM_KEY,
                                                     basekey, bklen);
