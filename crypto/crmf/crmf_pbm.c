					   ...
					   int macnid)
{
     ...
     * support SHA-1.
     */
	if (!X509_ALGOR_set0(pbm->owf, OBJ_nid2obj(owfnid), V_ASN1_UNDEF,
			     NULL)) {
     ...
     */
	if (!X509_ALGOR_set0(pbm->mac, OBJ_nid2obj(macnid), V_ASN1_UNDEF,
			     NULL)) {
		      ...
		      unsigned char **out, size_t *outlen)
{
	...
	if (!EVP_PBE_find(EVP_PBE_TYPE_PRF, mac_nid, NULL, &hmac_md_nid,
			  NULL) ||
