    ...
    *outlen = 0;
    ...
    if (pkctx != NULL && EVP_PKEY_decrypt_init(pkctx) > 0) {
        ...
        if (EVP_PKEY_decrypt(pkctx, NULL, &eksize,
                ASN1_STRING_get0_data(encKey), ASN1_STRING_get_length(encKey))
                <= 0
    ...
    if ((out = OPENSSL_malloc(ASN1_STRING_get_length(enc->encValue)
    ...
    if (!EVP_DecryptInit(evp_ctx, cipher, ek, iv)
        || !EVP_DecryptUpdate(evp_ctx, out, outlen,
            ...
            (int)ASN1_STRING_get_length(enc->encValue))
