...
#define IMPLEMENT_CRMF_CTRL_FUNC(atyp, valt, ctrlinf)                         \
	...
	{                                                                     \
		...
		for (i = 0;                                                   \
		     i < sk_OSSL_CRMF_ATTRIBUTETYPEANDVALUE_num(controls);    \
		     i++) {                                                   \
			atav = sk_OSSL_CRMF_ATTRIBUTETYPEANDVALUE_value(      \
				controls, i);                                 \
			if (OBJ_obj2nid(atav->type) ==                        \
			    NID_id_##ctrlinf##_##atyp)                        \
	...
	{                                                                     \
		...
		if ((atav = OSSL_CRMF_ATTRIBUTETYPEANDVALUE_new()) == NULL)   \
			goto err;                                             \
		if ((atav->type = OBJ_nid2obj(NID_id_##ctrlinf##_##atyp)) ==  \
		    NULL)                                                     \
			goto err;                                             \
				       ...
				       OSSL_CRMF_ATTRIBUTETYPEANDVALUE *ctrl)
{
	...
	if (crm->certReq->controls == NULL) {
		crm->certReq->controls =
			sk_OSSL_CRMF_ATTRIBUTETYPEANDVALUE_new_null();
		if (crm->certReq->controls == NULL)
			goto err;
		new = 1;
	}
	if (!sk_OSSL_CRMF_ATTRIBUTETYPEANDVALUE_push(crm->certReq->controls,
						     ctrl))
		goto err;
...
int OSSL_CRMF_MSG_set0_SinglePubInfo(OSSL_CRMF_SINGLEPUBINFO *spi, int method,
				     GENERAL_NAME *nm)
{
	if (spi == NULL || method < OSSL_CRMF_PUB_METHOD_DONTCARE ||
...
int OSSL_CRMF_MSG_PKIPublicationInfo_push0_SinglePubInfo(
	OSSL_CRMF_PKIPUBLICATIONINFO *pi, OSSL_CRMF_SINGLEPUBINFO *spi)
{
	if (pi == NULL || spi == NULL) {
		ERR_raise(ERR_LIB_CRMF, CRMF_R_NULL_ARGUMENT);
		return 0;
...
{
	if (pi == NULL || action < OSSL_CRMF_PUB_ACTION_DONTPUBLISH ||
...
int OSSL_CRMF_MSG_set0_validity(OSSL_CRMF_MSG *crm, ASN1_TIME *notBefore,
				ASN1_TIME *notAfter)
{
...
int OSSL_CRMF_MSG_create_popo(int meth, OSSL_CRMF_MSG *crm, EVP_PKEY *pkey,
			      const EVP_MD *digest, OSSL_LIB_CTX *libctx,
			      const char *propq)
{
	...
	case OSSL_CRMF_POPO_KEYENC:
		if ((pp->value.keyEncipherment = OSSL_CRMF_POPOPRIVKEY_new()) ==
		    NULL)
			goto err;
...
int OSSL_CRMF_MSGS_verify_popo(const OSSL_CRMF_MSGS *reqs, int rid,
			       int acceptRAVerified, OSSL_LIB_CTX *libctx,
			       const char *propq)
{
	...
	case OSSL_CRMF_POPO_SIGNATURE:
		...
		if (sig->poposkInput != NULL) {
			...
			if (X509_PUBKEY_eq(pubkey,
					   sig->poposkInput->publicKey) != 1) {
				ERR_raise(ERR_LIB_CRMF,
		...
		if (ASN1_item_verify_ex(
			    it, sig->algorithmIdentifier, sig->signature, asn,
			    NULL, X509_PUBKEY_get0(pubkey), libctx, propq) < 1)
			return 0;
		break;
	case OSSL_CRMF_POPO_KEYENC:
		/*
					  ...
					  const X509_REQ *p10cr)
{
	...
	if (crm != NULL)
		pubkey = OSSL_CRMF_CERTTEMPLATE_get0_publicKey(
 ...
 */
int OSSL_CRMF_CERTTEMPLATE_fill(OSSL_CRMF_CERTTEMPLATE *tmpl, EVP_PKEY *pubkey,
				...
				const ASN1_INTEGER *serial)
{
	...
	if (subject != NULL &&
	    !X509_NAME_set((X509_NAME **)&tmpl->subject, subject))
		return 0;
	if (issuer != NULL &&
	    !X509_NAME_set((X509_NAME **)&tmpl->issuer, issuer))
		return 0;
	if (serial != NULL) {
...
static int check_cmKGA(ossl_unused const X509_PURPOSE *purpose, const X509 *x,
		       int ca)
{
...
EVP_PKEY *OSSL_CRMF_ENCRYPTEDKEY_get1_pkey(
	const OSSL_CRMF_ENCRYPTEDKEY *encryptedKey, X509_STORE *ts,
	STACK_OF(X509) *extra, EVP_PKEY *pkey, X509 *cert,
	ASN1_OCTET_STRING *secret, OSSL_LIB_CTX *libctx, const char *propq)
{
#ifndef OPENSSL_NO_CMS
	BIO *bio = NULL;
	...
	if (encryptedKey->type != OSSL_CRMF_ENCRYPTEDKEY_ENVELOPEDDATA) {
		...
		p = OSSL_CRMF_ENCRYPTEDVALUE_decrypt(
			encryptedKey->value.encryptedValue, libctx, propq, pkey,
			&len);
		if ((p_copy = p) != NULL)
			ret = d2i_AutoPrivateKey_ex(NULL, &p_copy, len, libctx,
						    propq);
		OPENSSL_free(p);
		return ret;
	}
	...
	if ((purpose_id = X509_PURPOSE_get_by_sname(SN_cmKGA)) < 0) {
		...
		if (!X509_PURPOSE_add(purpose_id, X509_TRUST_COMPAT, 0,
				      check_cmKGA, LN_cmKGA, SN_cmKGA, NULL))
			goto end;
	}
	...
	pkey_bio = CMS_SignedData_verify(sd, NULL, NULL /* scerts */, ts, extra,
					 NULL, 0, libctx, propq);
	...
	if ((ret = d2i_PrivateKey_ex_bio(pkey_bio, NULL, libctx, propq)) ==
	    NULL)
				 ...
				 EVP_PKEY *pkey, int *outlen)
{
	...
	*outlen = 0;
	...
	if (cipher == NULL)
		cipher = (EVP_CIPHER *)EVP_get_cipherbyobj(
			enc->symmAlg->algorithm);
	if (cipher == NULL) {
	...
	if (pkctx != NULL && EVP_PKEY_decrypt_init(pkctx) > 0) {
		...
		if (EVP_PKEY_decrypt(pkctx, NULL, &eksize, encKey->data,
			...
			goto end;
		retval = EVP_PKEY_decrypt(pkctx, ek, &eksize, encKey->data,
					  encKey->length);
		failure =
	...
	if (!EVP_DecryptInit(evp_ctx, cipher, ek, iv) ||
	    !EVP_DecryptUpdate(evp_ctx, out, outlen, enc->encValue->data,
			       enc->encValue->length) ||
				      ...
				      EVP_PKEY *pkey)
{
	...
	buf = OSSL_CRMF_ENCRYPTEDVALUE_decrypt(ecert, libctx, propq, pkey,
					       &len);
					  ...
					  unsigned int flags)
{
	...
	if (ecert->type != OSSL_CRMF_ENCRYPTEDKEY_ENVELOPEDDATA)
		return OSSL_CRMF_ENCRYPTEDVALUE_get1_encCert(
			ecert->value.encryptedValue, libctx, propq, pkey);
#ifndef OPENSSL_NO_CMS
	bio = CMS_EnvelopedData_decrypt(ecert->value.envelopedData, NULL, pkey,
					NULL /* cert */, NULL, flags, libctx,
					propq);
	if (bio == NULL)
		return NULL;
