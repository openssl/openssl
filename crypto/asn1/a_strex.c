...
#define CHARTYPE_BS_ESC                                    \
	(ASN1_STRFLGS_ESC_2253 | CHARTYPE_FIRST_ESC_2253 | \
	 CHARTYPE_LAST_ESC_2253)

#define ESC_FLAGS                                         \
	(ASN1_STRFLGS_ESC_2253 | ASN1_STRFLGS_ESC_2254 |  \
	 ASN1_STRFLGS_ESC_QUOTE | ASN1_STRFLGS_ESC_CTRL | \
	...
	while (p != q) {
		...
		if (type & BUF_TYPE_CONVUTF8) {
			...
			for (i = 0; i < utflen; i++) {
                 ...
                 */
				len = do_esc_char(utfbuf[i], flags | orflags,
						  quotes, io_ch, arg);
				if (len < 0)
					return -1;
				outlen += len;
			...
			len = do_esc_char(c, flags | orflags, quotes, io_ch,
					  arg);
			if (len < 0)
				return -1;
			outlen += len;
