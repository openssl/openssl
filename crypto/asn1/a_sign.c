...
int ASN1_item_sign(const ASN1_ITEM *it, X509_ALGOR *algor1, X509_ALGOR *algor2,
		   ...
		   const EVP_MD *md)
{
	return ASN1_item_sign_ex(it, algor1, algor2, signature, data, NULL,
				 pkey, md, NULL, NULL);
}
		       ...
		       const void *data, EVP_MD_CTX *ctx)
{
	...
	if (pkey->ameth == NULL) {
		...
		if (algor1 != NULL) {
			...
			if (d2i_X509_ALGOR(&algor1, &pp, (long)aid_len) ==
			    NULL) {
				ERR_raise(ERR_LIB_ASN1, ERR_R_INTERNAL_ERROR);
				goto err;
			}
		...
		if (algor2 != NULL) {
			...
			if (d2i_X509_ALGOR(&algor2, &pp, (long)aid_len) ==
			    NULL) {
				ERR_raise(ERR_LIB_ASN1, ERR_R_INTERNAL_ERROR);
				goto err;
			}
		...
		rv = pkey->ameth->item_sign(ctx, it, data, algor1, algor2,
					    signature);
		if (rv == 1)
