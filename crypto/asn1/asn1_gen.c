...
static int asn1_cb(const char *elem, int len, void *bitstr)
{
	...
	if (utype == -1) {
		ERR_raise_data(ERR_LIB_ASN1, ASN1_R_UNKNOWN_TAG, "tag=%s",
			       elem);
		return -1;
	}
	...
	case ASN1_GEN_FLAG_SEQWRAP:
		if (!append_exp(arg, V_ASN1_SEQUENCE, V_ASN1_UNIVERSAL, 1, 0,
				1))
			return -1;
		break;
	...
	case ASN1_GEN_FLAG_BITWRAP:
		if (!append_exp(arg, V_ASN1_BIT_STRING, V_ASN1_UNIVERSAL, 0, 1,
				1))
			return -1;
		break;

	case ASN1_GEN_FLAG_OCTWRAP:
		if (!append_exp(arg, V_ASN1_OCTET_STRING, V_ASN1_UNIVERSAL, 0,
				0, 1))
			return -1;
		break;
			     ...
			     int depth, int *perr)
{
	...
	if (section) {
		...
		for (i = 0; i < sk_CONF_VALUE_num(sect); i++) {
			ASN1_TYPE *typ =
				generate_v3(sk_CONF_VALUE_value(sect, i)->value,
					    cnf, depth + 1, perr);
			if (!typ)
				goto bad;
...
static ASN1_TYPE *asn1_str2type(const char *str, int format, int utype)
{
	...
	case V_ASN1_ENUMERATED:
		...
		if ((atmp->value.integer = s2i_ASN1_INTEGER(NULL, str)) ==
		    NULL) {
