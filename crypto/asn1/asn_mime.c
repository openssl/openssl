			...
			OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if ((flags & SMIME_DETACHED) && data) {
		...
		if (BIO_printf(bio,
			       ...
			       "------%s%s",
			       bound, mime_eol, mime_eol, mime_eol, mime_eol,
			       bound, mime_eol) < 0)
			return 0;
		if (!asn1_output_data(bio, data, val, flags, it))
			return 0;
		if (BIO_printf(bio, "%s------%s%s", mime_eol, bound, mime_eol) <
		    0)
			return 0;
		...
		if (BIO_printf(
			    bio,
			    "Content-Type: %ssignature; name=\"smime.p7s\"%s"
			    "Content-Transfer-Encoding: base64%s"
			    "Content-Disposition: attachment; filename=\"smime.p7s\"%s%s",
			    mime_prefix, mime_eol, mime_eol, mime_eol,
			    mime_eol) < 0)
			return 0;
	...
	if (BIO_printf(bio,
		       ...
		       " filename=\"%s\"%s",
		       mime_eol, cname, mime_eol) < 0)
		return 0;
	if (BIO_printf(bio, "Content-Type: %smime;", mime_prefix) < 0)
		return 0;
	if (msg_type != NULL &&
	    BIO_printf(bio, " smime-type=%s;", msg_type) < 0)
		return 0;
	if (BIO_printf(bio,
		       " name=\"%s\"%s"
	...
	if (strcmp(hdr->value, "multipart/signed") == 0) {
		...
		if (strcmp(hdr->value, "application/x-pkcs7-signature") &&
			...
			ERR_raise_data(ERR_LIB_ASN1,
				       ASN1_R_SIG_INVALID_MIME_TYPE, "type: %s",
				       hdr->value);
			sk_MIME_HEADER_pop_free(headers, mime_hdr_free);
			sk_BIO_pop_free(parts, BIO_vfree);
			return NULL;
		...
		sk_MIME_HEADER_pop_free(headers, mime_hdr_free);
		/* Read in ASN1 */
		if ((val = b64_read_asn1(asnin, it, x, libctx, propq)) ==
		    NULL) {
			    ...
			    int blen)
{
	...
	if ((CHECK_AND_SKIP_PREFIX(line, "--")) &&
	    strncmp(line, bound, blen) == 0)
...
static int strip_eol(char *linebuf, int *plen, int flags)
{
	...
	for (p = linebuf + len - 1; len > 0; len--, p--) {
		...
		if (c == '\n') {
			is_eol = 1;
		} else if (is_eol && (flags & SMIME_ASCIICRLF) != 0 &&
			   c == 32) {
