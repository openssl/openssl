...
{
	static const int ydays[12] = { 0,   31,	 59,  90,  120, 151,
...
int ossl_asn1_time_print_ex(BIO *bp, const ASN1_TIME *tm, unsigned long flags)
{
	...
	if (tm->type == V_ASN1_GENERALIZEDTIME) {
		...
		if (f_len > 0) {
			if ((flags & ASN1_DTFLGS_TYPE_MASK) ==
			    ASN1_DTFLGS_ISO8601) {
				return BIO_printf(
					       bp,
					       "%4d-%02d-%02d %02d:%02d:%02d.%.*sZ",
					       stm.tm_year + 1900,
					       stm.tm_mon + 1, stm.tm_mday,
					       stm.tm_hour, stm.tm_min,
					       stm.tm_sec, f_len, f) > 0;
			} else {
				return BIO_printf(
					       bp,
					       "%s %2d %02d:%02d:%02d.%.*s %d GMT",
					       _asn1_mon[stm.tm_mon],
	...
	if ((flags & ASN1_DTFLGS_TYPE_MASK) == ASN1_DTFLGS_ISO8601) {
		...
		return BIO_printf(bp, "%s %2d %02d:%02d:%02d %d GMT",
				  _asn1_mon[stm.tm_mon], stm.tm_mday,
