		     ...
		     const ASN1_ITEM *it, int tag, int aclass)
{
	...
	case ASN1_ITYPE_CHOICE:
		...
		if ((i >= 0) && (i < it->tcount)) {
			...
			return asn1_template_ex_i2d(pchval, out, chtt, -1,
						    aclass);
		}
				...
				const ASN1_TEMPLATE *tt, int tag, int iclass)
{
	...
	if (flags & ASN1_TFLG_SK_MASK) {
		/* SET OF, SEQUENCE OF */
		STACK_OF(const_ASN1_VALUE) *sk =
			(STACK_OF(const_ASN1_VALUE) *)*pval;
		...
		for (i = 0; i < sk_const_ASN1_VALUE_num(sk); i++) {
			...
			len = ASN1_item_ex_i2d(&skitem, NULL,
					       ASN1_ITEM_ptr(tt->item), -1,
					       iclass);
			    ...
			    int iclass)
{
	...
	for (i = 0, tder = derlst; i < sk_const_ASN1_VALUE_num(sk);
	     i++, tder++) {
		skitem = sk_const_ASN1_VALUE_value(sk, i);
		tder->data = p;
		       ...
		       int *putype, const ASN1_ITEM *it)
{
	...
	if (it->itype == ASN1_ITYPE_MSTRING) {
		...
		*putype = utype;
		...
		utype = typ->type;
		*putype = utype;
		pval = (const ASN1_VALUE **)&typ->value
