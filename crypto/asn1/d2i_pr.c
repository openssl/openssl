			    ...
			    const char *propq)
{
	...
	if (ret == NULL)
		ret = ossl_d2i_PrivateKey_legacy(keytype, a, pp, length, libctx,
						 propq);
	return ret;
}
					   ...
					   const char *propq)
{
	...
	if (sk_ASN1_TYPE_num(inkey) == 6) {
		...
		keytype = EVP_PKEY_EC;
	} else if (sk_ASN1_TYPE_num(inkey) ==
		   3) { /* This seems to be PKCS8, not
                                              * traditional format */
		PKCS8_PRIV_KEY_INFO *p8 =
			d2i_PKCS8_PRIV_KEY_INFO(NULL, &p, length);
		EVP_PKEY *ret;
	...
	sk_ASN1_TYPE_pop_free(inkey, ASN1_TYPE_free);
	return ossl_d2i_PrivateKey_legacy(keytype, a, pp, length, libctx,
					  propq);
}
				...
				const char *propq)
{
	...
	ret = d2i_PrivateKey_decoder(EVP_PKEY_NONE, a, pp, length, libctx,
				     propq);
	/* try the legacy path if the decoder failed */
	if (ret == NULL)
