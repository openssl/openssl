			       ...
			       OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	case ASN1_ITYPE_EXTERN:
		...
		if (ef->asn1_ex_d2i_ex != NULL)
			return ef->asn1_ex_d2i_ex(pval, in, len, it, tag,
						  aclass, opt, ctx, libctx,
						  propq);
	...
	case ASN1_ITYPE_CHOICE:
		...
		} else if (!ossl_asn1_item_ex_new_intern(pval, it, libctx,
							 propq)) {
			ERR_raise(ERR_LIB_ASN1, ERR_R_NESTED_ASN1_ERROR);
			goto err;
		}
	...
	case ASN1_ITYPE_SEQUENCE:
		...
		for (i = 0, tt = it->templates; i < it->tcount; i++, tt++) {
			...
			ret = asn1_template_ex_d2i(pseqval, &p, len, seqtt,
						   isopt, ctx, depth, libctx,
						   propq);
			if (!ret) {
				errtt = seqtt;
				goto err;
				...
				const char *propq)
{
	...
	if (flags & ASN1_TFLG_EXPTAG) {
         ...
         */
		ret = asn1_check_tlen(&len, NULL, NULL, &exp_eoc, &cst, &p,
				      inlen, tt->tag, aclass, opt, ctx);
		q = p;
		if (!ret) {
			ERR_raise(ERR_LIB_ASN1, ERR_R_NESTED_ASN1_ERROR);
			return 0;
				   ...
				   OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if (flags & ASN1_TFLG_SK_MASK) {
		...
		while (len > 0) {
			...
			skfield = NULL;
			if (asn1_item_embed_d2i(
				    &skfield, &p, len, ASN1_ITEM_ptr(tt->item),
				    -1, 0, 0, ctx, depth, libctx, propq) <= 0) {
				ERR_raise(ERR_LIB_ASN1,
					  ERR_R_NESTED_ASN1_ERROR);
				/* |skfield| may be partially allocated despite failure. */
				ASN1_item_free(skfield,
					       ASN1_ITEM_ptr(tt->item));
				goto err;
			}
			len -= (long)(p - q);
		       ...
		       int utype, char *free_cont, const ASN1_ITEM *it)
{
	...
	case V_ASN1_BIT_STRING:
		if (!ossl_c2i_ASN1_BIT_STRING((ASN1_BIT_STRING **)pval, &cont,
					      len))
			goto err;
		break;
	...
	default:
		...
		if (utype == V_ASN1_GENERALIZEDTIME && (len < 15)) {
			ERR_raise(ERR_LIB_ASN1,
				  ASN1_R_GENERALIZEDTIME_IS_TOO_SHORT);
			goto err;
		}
		...
		if (*free_cont) {
			ASN1_STRING_set0(stmp,
					 (unsigned char *)cont /* UGLY CAST! */,
					 len);
			*free_cont = 0;
		} else {
...
static int asn1_find_end(const unsigned char **in, long len, char inf)
{
	...
	while (len > 0) {
		...
		if (!asn1_check_tlen(&plen, NULL, NULL, &inf, NULL, &p, len, -1,
				     0, 0, NULL)) {
			ERR_raise(ERR_LIB_ASN1, ERR_R_NESTED_ASN1_ERROR);
			return 0;
		}
