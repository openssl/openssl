...
static int asn1_item_print_ctx(BIO *out, const ASN1_VALUE **fld, int indent,
			       const ASN1_ITEM *it, const char *fname,
			       const char *sname, int nohdr,
		    ...
		    const ASN1_ITEM *it, const ASN1_PCTX *pctx)
{
	...
	if (pctx->flags & ASN1_PCTX_FLAGS_NO_STRUCT_NAME)
		...
		sname = it->sname;
	return asn1_item_print_ctx(out, &ifld, indent, it, NULL, sname, 0,
				   pctx);
}
			       ...
			       const ASN1_PCTX *pctx)
{
	...
	if (aux != NULL) {
		...
		parg.pctx = pctx;
		asn1_cb =
			((aux->flags & ASN1_AFLG_CONST_CB) != 0) ?
				aux->asn1_const_cb :
				(ASN1_aux_const_cb *)aux
					->asn1_cb; /* backward compatibility */
	}
	...
	case ASN1_ITYPE_MSTRING:
		if (!asn1_primitive_print(out, fld, it, indent, fname, sname,
					  pctx))
			return 0;
		break;
...
static int asn1_print_obstring(BIO *out, const ASN1_STRING *str, int indent)
{
	if (str->type == V_ASN1_BIT_STRING) {
		...
		return 0;
	if ((str->length > 0) && BIO_dump_indent(out, (const char *)str->data,
						 str->length, indent + 2) <= 0)
		return 0;
	return 1;
}
				...
				const ASN1_PCTX *pctx)
{
	...
	if (utype == V_ASN1_ANY) {
		...
		utype = atype->type;
		fld = (const ASN1_VALUE **)&atype->value
			      .asn1_value; /* actually is const */
		str = (ASN1_STRING *)*fld;
	...
	case V_ASN1_OTHER:
		if (BIO_puts(out, "\n") <= 0)
			return 0;
		if (ASN1_parse_dump(out, str->data, str->length, indent, 0) <=
		    0)
			ret = 0;
		needlf = 0;
		break;
