				...
				const char *propq)
{
	...
	sparam =
		ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(SCRYPT_PARAMS), param);
	...
	if (sparam->keyLength) {
		uint64_t spkeylen;
		if ((ASN1_INTEGER_get_uint64(&spkeylen, sparam->keyLength) ==
		     0) ||
		    (spkeylen != keylen)) {
	...
	if (ASN1_INTEGER_get_uint64(&N, sparam->costParameter) == 0 ||
	    ...
	    ASN1_INTEGER_get_uint64(&p, sparam->parallelizationParameter) ==
		    0 ||
	    EVP_PBE_scrypt_ex(NULL, 0, NULL, 0, N, r, p, 0, NULL, 0, libctx,
			      propq) == 0) {
...
{
	return PKCS5_v2_scrypt_keyivgen_ex(ctx, pass, passlen, param, c, md,
					   en_de, NULL, NULL);
}
