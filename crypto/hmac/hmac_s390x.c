...
int s390x_HMAC_init(HMAC_CTX *ctx, const void *key, int key_len, ENGINE *impl)
{
	...
	if (key != NULL) {
		...
		case S390X_HMAC_SHA_256:
			OPENSSL_cleanse(
				&ctx->plat.s390x.param.hmac_224_256.key,
		...
		case S390X_HMAC_SHA_512:
			OPENSSL_cleanse(
				&ctx->plat.s390x.param.hmac_384_512.key,
		...
		if (!ossl_assert(ctx->plat.s390x.blk_size <=
				 (int)key_param_len))
			return 0;
...
int s390x_HMAC_update(HMAC_CTX *ctx, const unsigned char *data, size_t len)
{
	...
	if (len > remain) {
		...
		if (ctx->plat.s390x.num > 0) {
			...
			memcpy(&ctx->plat.s390x.buf[ctx->plat.s390x.num], data,
			       remain);
			ctx->plat.s390x.num += remain;

			s390x_call_kmac(ctx, ctx->plat.s390x.buf,
					ctx->plat.s390x.num);
