...
static int hmac_reset(EVP_MAC_IMPL *hctx)
{
    if (!HMAC_CTX_reset(hctx->ctx))
        return 0;
    return 1;
}

static EVP_MAC_IMPL *hmac_copy(EVP_MAC_IMPL *hsrc)
{
    EVP_MAC_IMPL *hdst = NULL;

    if ((hdst = hmac_new()) == NULL
...
static int hmac_update(EVP_MAC_IMPL *hctx, const unsigned char *data,
                       size_t datalen)
{
    if (!HMAC_Update(hctx->ctx, data, datalen))
        return 0;
    return 1;
}
...
static int hmac_final(EVP_MAC_IMPL *hctx, unsigned char *out)
{
    ...
    if (!HMAC_Final(hctx->ctx, out, &hlen))
        return 0;
    return 1;
}
...
static const EVP_MAC hmac_meth = {
    EVP_MAC_HMAC,
    hmac_new,
    hmac_reset,
    hmac_copy,
