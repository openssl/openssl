...
struct opentls_ctx_onfree_list_st {
    opentls_ctx_onfree_fn *fn;
    struct opentls_ctx_onfree_list_st *next;
};

struct opentls_ctx_st {
    CRYPTO_RWLOCK *lock;
    CRYPTO_EX_DATA data;

    /*
     * For most data in the OPENtls_CTX we just use ex_data to store it. But
     * that doesn't work for ex_data itself - so we store that directly.
     */
    Otls_EX_DATA_GLOBAL global;

    /* Map internal static indexes to dynamically created indexes */
    int dyn_indexes[OPENtls_CTX_MAX_INDEXES];

    /* Keep a separate lock for each index */
    CRYPTO_RWLOCK *index_locks[OPENtls_CTX_MAX_INDEXES];

    CRYPTO_RWLOCK *oncelock;
    int run_once_done[OPENtls_CTX_MAX_RUN_ONCE];
    int run_once_ret[OPENtls_CTX_MAX_RUN_ONCE];
    struct opentls_ctx_onfree_list_st *onfreelist;
};

#ifndef FIPS_MODE
static OPENtls_CTX default_context_int;

/* Always points at default_context_int if it has been initialised */
static OPENtls_CTX *default_context = NULL;
#endif

static int context_init(OPENtls_CTX *ctx)
{
    size_t i;
    int exdata_done = 0;
    ...
    for (i = 0; i < OPENtls_CTX_MAX_INDEXES; i++) {
    ...
    /* OPENtls_CTX is built on top of ex_data so we initialise that directly */
    if (!do_ex_data_init(ctx))
        goto err;
    exdata_done = 1;

    if (!crypto_new_ex_data_ex(ctx, CRYPTO_EX_INDEX_OPENtls_CTX, NULL,
                               &ctx->data)) {
    ...
    if (!otls_property_parse_init(ctx))
        goto err;
...
static int context_deinit(OPENtls_CTX *ctx)
{
    struct opentls_ctx_onfree_list_st *tmp, *onfree;
    int i;
    ...
    CRYPTO_free_ex_data(CRYPTO_EX_INDEX_OPENtls_CTX, NULL, &ctx->data);
    crypto_cleanup_all_ex_data_int(ctx);
    for (i = 0; i < OPENtls_CTX_MAX_INDEXES; i++)
...
{
    OPENtls_CTX *ctx = OPENtls_zalloc(sizeof(*ctx));
...
static void opentls_ctx_generic_new(void *parent_ign, void *ptr_ign,
                                    CRYPTO_EX_DATA *ad, int index,
                                    long argl_ign, void *argp)
{
    const OPENtls_CTX_METHOD *meth = argp;
    void *ptr = meth->new_func(crypto_ex_data_get_opentls_ctx(ad));

    if (ptr != NULL)
        CRYPTO_set_ex_data(ad, index, ptr);
}
static void opentls_ctx_generic_free(void *parent_ign, void *ptr,
                                     CRYPTO_EX_DATA *ad, int index,
                                     long argl_ign, void *argp)
{
    const OPENtls_CTX_METHOD *meth = argp;
...
static int opentls_ctx_init_index(OPENtls_CTX *ctx, int static_index,
                                  const OPENtls_CTX_METHOD *meth)
{
    ...
    idx = crypto_get_ex_new_index_ex(ctx, CRYPTO_EX_INDEX_OPENtls_CTX, 0,
                                     (void *)meth,
                                     opentls_ctx_generic_new,
                                     NULL, opentls_ctx_generic_free);
    if (idx < 0)
        return 0;
                           ...
                           const OPENtls_CTX_METHOD *meth)
{
    ...
    if (!opentls_ctx_init_index(ctx, index, meth)) {
        CRYPTO_THREAD_unlock(ctx->lock);
        CRYPTO_THREAD_unlock(ctx->index_locks[index]);
        return NULL;
    }
    ...
    if (CRYPTO_alloc_ex_data(CRYPTO_EX_INDEX_OPENtls_CTX, NULL,
...
{
    struct opentls_ctx_onfree_list_st *newonfree
        = OPENtls_malloc(sizeof(*newonfree));
