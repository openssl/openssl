...
OSSL_HTTP_REQ_CTX *OSSL_HTTP_REQ_CTX_new(BIO *wbio, BIO *rbio, int buf_size)
{
	...
	rctx->buf_size = buf_size > 0 ? buf_size :
					       ...
					       unsigned long len)
{
	...
	rctx->max_resp_len = len != 0 ? (size_t)len :
				       ...
				       const char *path)
{
	...
	if (path == NULL) {
		path = "/";
			      ...
			      "http://")) { /* absoluteURI for proxy use */
		if (server != NULL) {
...
static int parse_http_line1(char *line, int *found_keep_alive)
{
	...
	*reason++ = '\0';
	...
	if (*reason != '\0') {
		...
		for (end = reason + strlen(reason) - 1; ossl_isspace(*end);
		     end--)
			*end = '\0';
	}
...
next_io:
	...
	if ((rctx->state & OHS_NOREAD) == 0) {
		if (rctx->expect_asn1 && (rctx->state == OHS_ASN1_HEADER ||
			...
			if (n ==
			    -2) { /* some BIOs, such as SSL, do not support "gets" */
				(void)ERR_pop_to_mark();
				n = BIO_get_line(rctx->rbio, buf,
						 rctx->buf_size);
			} else {
		...
		if (n <= 0) {
			if (rctx->state == OHS_ERROR_CONTENT) {
				...
				(void)check_set_resp_len(
					"error response content", rctx,
					resp_len);
				return 0;
			}
	...
	case OHS_WRITE_REQ:
		...
		if (rctx->len_to_send > 0) {
			...
			if (!BIO_write_ex(rctx->wbio, rctx->pos,
					  rctx->len_to_send, &sz)) {
			...
			if (OSSL_TRACE_ENABLED(HTTP)) {
				...
				OSSL_TRACE_STRING(
					...
					rctx->state != OHS_WRITE_REQ ||
						rctx->text,
					rctx->state != OHS_WRITE_REQ, rctx->pos,
					sz);
				OSSL_TRACE(
					HTTP,
...
next_line:
		...
		if (rctx->state == OHS_ERROR_CONTENT) {
			resp_len += n;
			if (!check_max_len("error response content",
					   rctx->max_resp_len, resp_len))
				return 0;
			if (OSSL_TRACE_ENABLED(
				    HTTP)) /* dump response content line */
				OSSL_TRACE_STRING(HTTP, got_text, 1,
						  (unsigned char *)buf, n);
			goto next_line;
		}

		resp_hdr_lines++;
		if (rctx->max_hdr_lines != 0 &&
		    rctx->max_hdr_lines < resp_hdr_lines) {
			ERR_raise(ERR_LIB_HTTP,
		...
		if (rctx->state == OHS_FIRSTLINE) {
			...
			default:
				...
				rctx->state =
					i < HTTP_STATUS_CODES_NONFATAL_ERROR ?
						OHS_HEADERS_ERROR :
						OHS_HEADERS;
		...
		if (value != NULL && line_end != NULL) {
			...
			if (OPENSSL_strcasecmp(key, "Connection") == 0) {
				if (OPENSSL_strcasecmp(value, "keep-alive") ==
				    0)
					found_keep_alive = 1;
				else if (OPENSSL_strcasecmp(value, "close") ==
					 0)
					found_keep_alive = 0;
			} else if (OPENSSL_strcasecmp(key, "Content-Length") ==
				   0) {
				size_t content_len =
					(size_t)strtoul(value, &line_end, 10);
		...
		if (rctx->keep_alive !=
			    0 /* do not let server initiate keep_alive */
		...
		if (!rctx->expect_asn1) {
			if (OSSL_TRACE_ENABLED(HTTP))
				OSSL_TRACE(HTTP,
					   "Receiving response text content\n");
	...
	case OHS_ASN1_CONTENT:
		...
		if (OSSL_TRACE_ENABLED(HTTP))
			OSSL_TRACE(
				HTTP,
				"Finished receiving response ASN.1 content\n");
					   ...
					   const char *port, int use_ssl)
{
	if (port == NULL) {
		...
		if (!BIO_parse_hostserv(hostserv, NULL, &service,
					BIO_PARSE_PRIO_HOST))
			return NULL;
...
BIO *OSSL_HTTP_REQ_CTX_exchange(OSSL_HTTP_REQ_CTX *rctx)
{
	...
	for (;;) {
		...
		if (BIO_wait(rctx->rbio, rctx->max_time,
			     100 /* milliseconds */) <= 0)
			return NULL;
	}
				  ...
				  int buf_size, int overall_timeout)
{
	...
	if (bio != NULL) {
		...
		if (proxy != NULL &&
		    !OSSL_HTTP_parse_url(proxy, NULL /* use_ssl */,
					 NULL /* user */, &proxy_host,
					 &proxy_port, NULL /* num */,
					 NULL /* path */, NULL, NULL))
			return NULL;
		cbio = http_new_bio(server, port, use_ssl, proxy_host,
				    proxy_port);
	...
	if (rbio == NULL &&
	    BIO_do_connect_retry(cbio, overall_timeout, -1) <= 0) {
	...
	if (bio_update_fn != NULL) {
		...
		cbio = (*bio_update_fn)(cbio, arg, 1 /* connect */,
					use_ssl != 0);
		if (cbio == NULL) {
			if (bio == NULL) /* cbio was not provided by caller */
	...
	rctx = http_req_ctx_new(bio == NULL, cbio, rbio != NULL ? rbio : cbio,
				bio_update_fn, arg, use_ssl, proxy, server,
	...
	if (resp == NULL) {
		if (rctx->redirection_url != NULL) {
			if (redirection_url == NULL)
				...
				*redirection_url =
					OPENSSL_strdup(rctx->redirection_url);
		} else {
			char buf[200];
			...
			if (lib == ERR_LIB_SSL || lib == ERR_LIB_HTTP ||
			    (lib == ERR_LIB_BIO &&
			     reason == BIO_R_CONNECT_TIMEOUT) ||
			    (lib == ERR_LIB_BIO &&
			     reason == BIO_R_CONNECT_ERROR)
		   ...
		   size_t max_resp_len, int timeout)
{
	...
	for (;;) {
		...
		if (!OSSL_HTTP_parse_url(current_url, &use_ssl, NULL /* user */,
					 &host, &port, NULL /* port_num */,
					 &path, NULL, NULL))
			break;

		rctx = OSSL_HTTP_open(host, port, proxy, no_proxy, use_ssl, bio,
				      rbio, bio_update_fn, arg, buf_size,
				      timeout);
new_rpath:
		...
		if (rctx != NULL) {
			if (!OSSL_HTTP_set1_request(
				...
				resp = OSSL_HTTP_exchange(rctx,
							  &redirection_url);
			}
		}
		OPENSSL_free(path);
		if (resp == NULL && redirection_url != NULL) {
			if (redirection_ok(++n_redirs, current_url,
					   redirection_url) &&
			...
			size_t max_resp_len, int timeout, int keep_alive)
{
	...
	if (rctx == NULL) {
		rctx = OSSL_HTTP_open(server, port, proxy, no_proxy, use_ssl,
				      bio, rbio, bio_update_fn, arg, buf_size,
				      timeout);
		timeout = -1; /* Already set during opening the connection */
	}
	if (rctx != NULL) {
		if (OSSL_HTTP_set1_request(rctx, path, headers, content_type,
					   req, expected_ct, expect_asn1,
...
#define BUF_SIZE (8 * 1024)
	...
	if (mbuf == NULL || fbio == NULL) {
		BIO_printf(bio_err /* may be NULL */, "%s: out of memory",
			   prog);
		goto end;
	}
	BIO_push(fbio, bio);
