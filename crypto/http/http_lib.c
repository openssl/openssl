		   ...
		   char **pfrag)
{
	...
	if (host[0] == '[') {
		...
		if (host_end ==
		    NULL) /* the remaining string is just the hostname */
	...
	if (sscanf(port, "%u", &portnum) <= 0 || portnum > 65535) {
		ERR_raise_data(ERR_LIB_HTTP, HTTP_R_INVALID_PORT_NUMBER, "%s",
			       port);
		goto err;
	}
...
int OSSL_HTTP_parse_url(const char *url, int *pssl, char **puser, char **phost,
			char **pport, int *pport_num, char **ppath,
			char **pquery, char **pfrag)
{
	...
	if (pssl != NULL)
		*pssl = 0;
	if (!OSSL_parse_url(url, &scheme, puser, phost, &port, pport_num, ppath,
			    pquery, pfrag))
		return 0;
...
static int use_proxy(const char *no_proxy, const char *server)
{
	...
	if (!ossl_assert(server != NULL))
		return 0;
	sl = strlen(server);
	if (sl >= 2 && sl < sizeof(host) + 2 && server[0] == '[' &&
	    server[sl - 1] == ']') {
	...
	while (found != NULL &&
	       ((found != no_proxy && !ossl_isspace(found[-1]) &&
		 found[-1] != ',') ||
		(found[sl] != '\0' && !ossl_isspace(found[sl]) &&
		 found[sl] != ',')))
				  ...
				  const char *server, int use_ssl)
{
     ...
     */
	if (proxy == NULL)
		proxy = ossl_safe_getenv(use_ssl ? "https_proxy" :
						   "http_proxy");
	if (proxy == NULL)
		proxy = ossl_safe_getenv(use_ssl ? OPENSSL_HTTPS_PROXY :
						   OPENSSL_HTTP_PROXY);
