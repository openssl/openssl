...
static int use_proxy(const char *no_proxy, const char *server)
{
     ...
     */
    if (no_proxy == NULL)
        no_proxy = ossl_getenv("no_proxy");
    if (no_proxy == NULL)
        no_proxy = ossl_getenv(OPENSSL_NO_PROXY);
                                  ...
                                  const char *server, int use_ssl)
{
     ...
     */
    if (proxy == NULL)
        proxy = ossl_getenv(use_ssl ? "https_proxy" : "http_proxy");
    if (proxy == NULL)
        proxy = ossl_getenv(use_ssl ? OPENSSL_HTTP_PROXY :
                       OPENSSL_HTTPS_PROXY);
