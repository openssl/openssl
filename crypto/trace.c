		       ...
		       size_t *written)
{
	...
	size_t cnt = ctx->callback(buf, num, ctx->category,
				   OSSL_TRACE_CTRL_WRITE, ctx->data);
...
int OSSL_trace_set_channel(int category, BIO *channel)
{
	...
	if (category >= 0 && category < OSSL_TRACE_CATEGORY_NUM)
		return set_trace_data(category, SIMPLE_CHANNEL, &channel, NULL,
				      NULL, trace_attach_cb, trace_detach_cb);
#endif
	return 0;
}
...
int OSSL_trace_set_callback(int category, OSSL_trace_cb callback, void *data)
{
	...
	if (callback != NULL) {
		if ((channel = BIO_new(&trace_method)) == NULL ||
		    (trace_data = OPENSSL_zalloc(
			     sizeof(struct trace_data_st))) == NULL)
			goto err;
