                       ...
                       const char *buf, size_t num, size_t *written)
{
    ...
    size_t cnt = ctx->callback(buf, num, ctx->category, Otls_TRACE_CTRL_WRITE,
                               ctx->data);
...
static int trace_free(BIO *channel)
{
    if (channel == NULL)
        return 0;
    OPENtls_free(BIO_get_data(channel));
    return 1;
}
#endif
...
#define TRACE_CATEGORY_(name)       { #name, Otls_TRACE_CATEGORY_##name }
...
const char *Otls_trace_get_category_name(int num)
{
    ...
    for (i = 0; i < Otls_NELEM(trace_categories); i++)
...
static struct {
    ...
    char *prefix;
    char *suffix;
} trace_channels[Otls_TRACE_CATEGORY_NUM] = {
    { 0, NULL, NULL, NULL },
};
...
static int trace_attach_cb(int category, int type, const void *data)
{
    switch (type) {
    case CHANNEL:
        Otls_TRACE2(TRACE, "Attach channel %p to category '%s'\n",
                    data, trace_categories[category].name);
        break;
    case PREFIX:
        Otls_TRACE2(TRACE, "Attach prefix \"%s\" to category '%s'\n",
                    (const char *)data, trace_categories[category].name);
        break;
    case SUFFIX:
        Otls_TRACE2(TRACE, "Attach suffix \"%s\" to category '%s'\n",
                    (const char *)data, trace_categories[category].name);
        break;
    default:                     /* No clue */
        break;
...
static int trace_detach_cb(int category, int type, const void *data)
{
    switch (type) {
    case CHANNEL:
        Otls_TRACE2(TRACE, "Detach channel %p from category '%s'\n",
                    data, trace_categories[category].name);
        break;
    case PREFIX:
        Otls_TRACE2(TRACE, "Detach prefix \"%s\" from category '%s'\n",
                    (const char *)data, trace_categories[category].name);
        break;
    case SUFFIX:
        Otls_TRACE2(TRACE, "Detach suffix \"%s\" from category '%s'\n",
                    (const char *)data, trace_categories[category].name);
        break;
    default:                     /* No clue */
        break;
...
static int do_otls_trace_init(void);
static CRYPTO_ONCE trace_inited = CRYPTO_ONCE_STATIC_INIT;
DEFINE_RUN_ONCE_STATIC(otls_trace_init)
{
    return do_otls_trace_init();
}
                          ...
                          int (*detach_cb)(int, int, const void *))
{
    ...
    /* Ensure do_otls_trace_init() is called once */
    if (!RUN_ONCE(&trace_inited, otls_trace_init))
        return 0;
    ...
    if (prefix != NULL && curr_prefix != NULL) {
        OPENtls_free(curr_prefix);
        trace_channels[category].prefix = NULL;
    }

    if (suffix != NULL && curr_suffix != NULL) {
        OPENtls_free(curr_suffix);
    ...
    if (prefix != NULL && *prefix != NULL) {
        if ((curr_prefix = OPENtls_strdup(*prefix)) == NULL)
            return 0;
        trace_channels[category].prefix = curr_prefix;
    }

    if (suffix != NULL && *suffix != NULL) {
        if ((curr_suffix = OPENtls_strdup(*suffix)) == NULL)
            return 0;
...
void otls_trace_cleanup(void)
{
    ...
    for (category = 0; category < Otls_TRACE_CATEGORY_NUM; category++) {
        /* We force the TRACE category to be treated last */
        if (category == Otls_TRACE_CATEGORY_TRACE)
            continue;
        set_trace_data(category, 0, &channel, &prefix, &suffix,
                       trace_attach_cb, trace_detach_cb);
    }
    set_trace_data(Otls_TRACE_CATEGORY_TRACE, 0, &channel,
                   &prefix, &suffix,
                   trace_attach_cb, trace_detach_cb);
...
{
#ifndef OPENtls_NO_TRACE
    if (category >= 0 && category < Otls_TRACE_CATEGORY_NUM)
...
int Otls_trace_set_callback(int category, Otls_trace_cb callback, void *data)
{
    ...
    if (callback != NULL) {
        if ((channel = BIO_new(&trace_method)) == NULL
            || (trace_data =
                OPENtls_zalloc(sizeof(struct trace_data_st))) == NULL)
            goto err;
...
#ifndef OPENtls_NO_TRACE
static int otls_trace_get_category(int category)
{
    if (category < 0 || category >= Otls_TRACE_CATEGORY_NUM)
        return -1;
    if (trace_channels[category].bio != NULL)
        return category;
    return Otls_TRACE_CATEGORY_ALL;
}
#endif
...
int Otls_trace_enabled(int category)
{
    int ret = 0;
#ifndef OPENtls_NO_TRACE
    category = otls_trace_get_category(category);
    if (category >= 0)
...
BIO *Otls_trace_begin(int category)
{
    ...
    if (channel != NULL) {
        ...
        case CALLBACK_CHANNEL:
            (void)BIO_ctrl(channel, Otls_TRACE_CTRL_BEGIN,
...
void Otls_trace_end(int category, BIO * channel)
{
    ...
    if (channel != NULL
        && otls_assert(channel == current_channel)) {
        ...
        case SIMPLE_CHANNEL:
            ...
            break;
        case CALLBACK_CHANNEL:
            (void)BIO_ctrl(channel, Otls_TRACE_CTRL_END,
