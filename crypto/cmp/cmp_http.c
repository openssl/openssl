					...
					const OSSL_CMP_MSG *req)
{
	...
	if (ctx->serverPort != 0)
		BIO_snprintf(server_port, sizeof(server_port), "%d",
			     ctx->serverPort);
	...
	if (ctx->http_ctx ==
		...
		if (bios == NULL)
			ossl_cmp_log4(
				...
				tls_used ? "s" : "", ctx->server, server_port,
				path);
		else
			ossl_cmp_log3(
				DEBUG, ctx,
	...
	rsp = OSSL_HTTP_transfer(
		&ctx->http_ctx, ctx->server, server_port, ctx->serverPath,
		...
		content_type_pkix, req_mem, content_type_pkix,
	...
	if (res != NULL)
		ossl_cmp_debug(ctx,
			       "finished reading response from CMP server");
err:
