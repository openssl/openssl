				...
				X509_STORE *trusted_store, X509 *cert)
{
	...
	if ((csc = X509_STORE_CTX_new_ex(ctx->libctx, ctx->propq)) == NULL ||
	    !X509_STORE_CTX_init(csc, trusted_store, cert, ctx->untrusted))
		goto err;
	...
	if (!valid &&
	    ERR_GET_REASON(err) != CMP_R_POTENTIALLY_INVALID_CERTIFICATE)
		      ...
		      const char *expect_desc, const X509_NAME *expect_name)
{
	...
	if (str != NULL)
		ossl_cmp_log2(INFO, ctx, " actual name in %s = %s", actual_desc,
			      str);
	OPENSSL_free(str);
	if ((str = X509_NAME_oneline(expect_name, NULL, 0)) != NULL)
		ossl_cmp_log2(INFO, ctx, " does not match %s = %s", expect_desc,
			      str);
	OPENSSL_free(str);
	return 0;
}
...
static int check_kid(const OSSL_CMP_CTX *ctx, const ASN1_OCTET_STRING *ckid,
		     const ASN1_OCTET_STRING *skid)
{
	...
	if (ckid == NULL) {
		ossl_cmp_warn(ctx,
			      "missing Subject Key Identifier in certificate");
		return 0;
	}
	...
	if (ASN1_OCTET_STRING_cmp(ckid, skid) == 0) {
		if (str != NULL)
			ossl_cmp_log1(INFO, ctx,
				      " subjectKID matches senderKID: %s", str);
		OPENSSL_free(str);
		return 1;
	}

	if (str != NULL)
		ossl_cmp_log1(INFO, ctx, " cert Subject Key Identifier = %s",
			      str);
	OPENSSL_free(str);
	if ((str = i2s_ASN1_OCTET_STRING(NULL, skid)) != NULL)
		ossl_cmp_log1(INFO, ctx, " does not match senderKID    = %s",
			      str);
	OPENSSL_free(str);
	return 0;
}
 ...
 */
static int cert_acceptable(const OSSL_CMP_CTX *ctx, const char *desc1,
			   const char *desc2, X509 *cert,
			   ...
			   const OSSL_CMP_MSG *msg)
{
	...
	if ((str = X509_NAME_oneline(X509_get_subject_name(cert), NULL, 0)) !=
	    NULL)
	...
	if (!check_name(ctx, 1, "cert subject", X509_get_subject_name(cert),
			"sender field", msg->header->sender->d.directoryName))
		return 0;

	if (!check_kid(ctx, X509_get0_subject_key_id(cert),
		       msg->header->senderKID))
		return 0;
			   ...
			   X509 *scrt)
{
	...
	ossl_cmp_warn(
		ctx, "msg signature validates but cert path validation failed");
	return 0;
}
	...
	if (!valid) {
		...
		OSSL_CMP_CERTRESPONSE *crep =
			ossl_cmp_certrepmessage_get0_certresponse(
				msg->body->value.ip, OSSL_CMP_CERTREQID);
...
static int check_msg_given_cert(const OSSL_CMP_CTX *ctx, X509 *cert,
				const OSSL_CMP_MSG *msg)
{
	return cert_acceptable(ctx, "previously validated", "sender cert", cert,
			       NULL, NULL, msg) &&
			       ...
			       int mode_3gpp)
{
	...
	if (check_msg_with_certs(ctx, msg->extraCerts, "extraCerts", NULL, NULL,
				 msg, mode_3gpp))
		return 1;
	...
	if (ctx->trusted == NULL) {
		...
		STACK_OF(X509) *trusted =
			X509_STORE_get1_all_certs(ctx->trusted);

		ret = check_msg_with_certs(ctx, trusted,
					   mode_3gpp ?
						   "self-issued extraCerts" :
						   "certs in trusted store",
					   msg->extraCerts, ctx->untrusted, msg,
					   mode_3gpp);
...
{
	X509 *scrt =
		ctx->validatedSrvCert; /* previous successful sender cert */
	...
	if (ctx->log_cb != NULL) {
		...
		if (sname != NULL)
			ossl_cmp_log1(INFO, ctx, "matches msg sender    = %s",
				      sname);
		if (skid_str != NULL)
			ossl_cmp_log1(INFO, ctx, "matches msg senderKID = %s",
				      skid_str);
		else
			ossl_cmp_info(
				ctx,
				"while msg header does not contain senderKID");
		...
		if (verify_PBMAC(ctx, msg)) {
			...
			case OSSL_CMP_PKIBODY_CCP:
				if (ctx->trusted != NULL) {
					...
					if (!ossl_cmp_X509_STORE_add1_certs(
						    ctx->trusted, certs, 0))
	...
	default:
		...
		if (scrt == NULL) {
			...
			if (verify_signature(ctx, msg, scrt)) {
				...
				return ossl_cmp_ctx_set1_validatedSrvCert(ctx,
									  scrt);
			}
			ossl_cmp_warn(
				ctx,
				"CMP message signature verification failed");
			ERR_raise(ERR_LIB_CMP,
			      ...
			      ossl_cmp_allow_unprotected_cb_t cb, int cb_arg)
{
	...
	if (num_added > 10)
		ossl_cmp_log1(WARN, ctx,
			      "received CMP message contains %d extraCerts",
			      num_added);
	/*
     ...
     */
	num_untrusted = ctx->untrusted == NULL ? 0 :
						 sk_X509_num(ctx->untrusted);
	...
	if (!check_transactionID_or_nonce(ctx->transactionID,
					  hdr->transactionID,
	...
	if (!check_transactionID_or_nonce(ctx->senderNonce, hdr->recipNonce,
		...
		if (ctx->first_senderNonce == NULL ||
		    ...
		    || !check_transactionID_or_nonce(
			       ctx->first_senderNonce, hdr->recipNonce,
			       CMP_R_RECIPNONCE_UNMATCHED)) {
			(void)ERR_clear_last_mark();
			return 0;
		}
...
int ossl_cmp_verify_popo(const OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg,
			 int acceptRAVerified)
{
	...
	case OSSL_CMP_PKIBODY_P10CR: {
		...
		if (X509_REQ_verify_ex(req, X509_REQ_get0_pubkey(req),
	...
	case OSSL_CMP_PKIBODY_KUR:
		if (!OSSL_CRMF_MSGS_verify_popo(
			    msg->body->value.ir, OSSL_CMP_CERTREQID,
			    acceptRAVerified, ctx->libctx, ctx->propq)) {
#ifndef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
			return 0;
#endif
		}
