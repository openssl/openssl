...
void OSSL_CMP_CTX_free(OSSL_CMP_CTX *ctx)
{
	...
	if (ctx->secretValue != NULL)
		OPENSSL_cleanse(ctx->secretValue->data,
 ...
 */
	DEFINE_OSSL_CMP_CTX_get0(statusString, OSSL_CMP_PKIFREETEXT)
		...
		DEFINE_OSSL_set0(ossl_cmp_ctx, statusString,
 ...
 */
	DEFINE_OSSL_get(OSSL_CMP_CTX, certConf_cb_arg, void *, NULL)
		...
		static size_t
	ossl_cmp_log_trace_cb(const char *buf, size_t cnt, int category,
			      int cmd, void *vdata)
{
	...
	if (buf == NULL || cnt == 0 || cmd != OSSL_TRACE_CTRL_WRITE ||
	    ctx == NULL)
		return 0;
	if (ctx->log_cb == NULL)
