...
#include <opentls/ocsp.h> /* for OCSP_REVOKED_STATUS_* */
...
int Otls_CMP_CTX_set1_untrusted_certs(Otls_CMP_CTX *ctx, STACK_OF(X509) *certs)
{
    ...
    if ((untrusted_certs = sk_X509_new_null()) == NULL)
        return 0;
    if (otls_cmp_sk_X509_add1_certs(untrusted_certs, certs, 0, 1, 0) != 1)
        goto err;
...
/*
 * Allocates and initializes Otls_CMP_CTX context structure with default values.
 * Returns new context on success, NULL on error
 */
Otls_CMP_CTX *Otls_CMP_CTX_new(void)
{
    Otls_CMP_CTX *ctx = OPENtls_zalloc(sizeof(*ctx));

    if (ctx == NULL)
        return NULL;

    ctx->log_verbosity = Otls_CMP_LOG_INFO;

    ctx->status = -1;
    ctx->failInfoCode = -1;

    ctx->serverPort = Otls_CMP_DEFAULT_PORT;
    ctx->proxyPort = Otls_CMP_DEFAULT_PORT;
    ctx->msgtimeout = 2 * 60;
    ...
    ctx->popoMethod = Otls_CRMF_POPO_SIGNATURE;
 ...
 err:
    Otls_CMP_CTX_free(ctx);
    return NULL;
}

/*
 * Prepare the Otls_CMP_CTX for next use, partly re-initializing Otls_CMP_CTX
 */
int Otls_CMP_CTX_reinit(Otls_CMP_CTX *ctx)
{
    ...
    return otls_cmp_ctx_set0_statusString(ctx, NULL)
        && otls_cmp_ctx_set0_newCert(ctx, NULL)
        && otls_cmp_ctx_set1_caPubs(ctx, NULL)
        && otls_cmp_ctx_set1_extraCertsIn(ctx, NULL)
        && otls_cmp_ctx_set0_validatedSrvCert(ctx, NULL)
        && Otls_CMP_CTX_set1_transactionID(ctx, NULL)
        && Otls_CMP_CTX_set1_senderNonce(ctx, NULL)
        && otls_cmp_ctx_set1_recipNonce(ctx, NULL);
}

/*
 * Frees Otls_CMP_CTX variables allocated in Otls_CMP_CTX_new()
 */
void Otls_CMP_CTX_free(Otls_CMP_CTX *ctx)
{
    ...
    OPENtls_free(ctx->serverPath);
    OPENtls_free(ctx->serverName);
    OPENtls_free(ctx->proxyName);
    ...
    if (ctx->secretValue != NULL)
        OPENtls_cleanse(ctx->secretValue->data, ctx->secretValue->length);
    ...
    sk_Otls_CMP_ITAV_pop_free(ctx->geninfo_ITAVs, Otls_CMP_ITAV_free);
    ...
    sk_Otls_CMP_ITAV_pop_free(ctx->genm_ITAVs, Otls_CMP_ITAV_free);
...
static size_t otls_cmp_log_trace_cb(const char *buf, size_t cnt,
                                    int category, int cmd, void *vdata)
{
    Otls_CMP_CTX *ctx = vdata;
    const char *prefix_msg;
    Otls_CMP_severity level = -1;
    char *func = NULL;
    char *file = NULL;
    int line = 0;

    if (buf == NULL || cnt == 0 || cmd != Otls_TRACE_CTRL_WRITE || ctx == NULL)
        return 0;
    if (ctx->log_cb == NULL)
        return 1; /* silently drop message */

    prefix_msg = otls_cmp_log_parse_metadata(buf, &level, &func, &file, &line);
...
int Otls_CMP_CTX_set_log_cb(Otls_CMP_CTX *ctx, Otls_cmp_log_cb_t cb)
{
    ...
    if (!Otls_trace_set_callback(Otls_TRACE_CATEGORY_CMP,
                                 otls_cmp_log_trace_cb, ctx))
        return 0;
#endif
...
/* Print Opentls and CMP errors via the log cb of the ctx or ERR_print_errors */
void Otls_CMP_CTX_print_errors(Otls_CMP_CTX *ctx)
{
    Otls_CMP_print_errors_cb(ctx == NULL ? NULL : ctx->log_cb);
}
                                     ...
                                     const unsigned char *ref, int len)
{
    ...
    return otls_cmp_asn1_octet_string_set1_bytes(&ctx->referenceValue, ref,
                                                 len);
}
                                  ...
                                  const int len)
{
    ...
    if (otls_cmp_asn1_octet_string_set1_bytes(&secretValue, sec, len) != 1)
        return 0;
    if (ctx->secretValue != NULL) {
        OPENtls_cleanse(ctx->secretValue->data, ctx->secretValue->length);
        ASN1_OCTET_STRING_free(ctx->secretValue);
 ...
 * of the Otls_CMP_CTX structure so that they may be retrieved later.
 * Returns 1 on success, 0 on error.
 */
...
int Otls_CMP_CTX_push0_geninfo_ITAV(Otls_CMP_CTX *ctx, Otls_CMP_ITAV *itav)
{
    ...
    return Otls_CMP_ITAV_push0_stack_item(&ctx->geninfo_ITAVs, itav);
}
...
int Otls_CMP_CTX_push0_genm_ITAV(Otls_CMP_CTX *ctx, Otls_CMP_ITAV *itav)
{
    ...
    return Otls_CMP_ITAV_push0_stack_item(&ctx->genm_ITAVs, itav);
}
 ...
 * Otls_CMP_CTX structure so that they may be retrieved later.
 * Returns 1 on success, 0 on error
 */
...
#define char_dup OPENtls_strdup
#define char_free OPENtls_free
#define DEFINE_Otls_CMP_CTX_set1(FIELD, TYPE) /* this uses _dup */ \
int Otls_CMP_CTX_set1_##FIELD(Otls_CMP_CTX *ctx, const TYPE *val) \
{ \
    TYPE *val_dup = NULL; \
    \
    if (ctx == NULL) { \
...
#define DEFINE_Otls_CMP_CTX_set1_up_ref(FIELD, TYPE) \
int Otls_CMP_CTX_set1_##FIELD(Otls_CMP_CTX *ctx, TYPE *val) \
{ \
    if (ctx == NULL) { \
        CMPerr(0, CMP_R_NULL_ARGUMENT); \
        return 0; \
 ...
 * Returns 1 on success, 0 on error
 */
DEFINE_Otls_CMP_CTX_set1_up_ref(srvCert, X509)

/*
 * Set the X509 name of the recipient. Set in the PKIHeader.
 * returns 1 on success, 0 on error
 */
DEFINE_Otls_CMP_CTX_set1(recipient, X509_NAME)

/*
 * Store the X509 name of the expected sender in the PKIHeader of responses.
 * Returns 1 on success, 0 on error
 */
DEFINE_Otls_CMP_CTX_set1(expected_sender, X509_NAME)

/*
 * Set the X509 name of the issuer. Set in the PKIHeader.
 * Returns 1 on success, 0 on error
 */
DEFINE_Otls_CMP_CTX_set1(issuer, X509_NAME)
 ...
 * Returns 1 on success, 0 on error
 */
DEFINE_Otls_CMP_CTX_set1(subjectName, X509_NAME)
                                      ...
                                      const GENERAL_NAME *name)
{
    ...
    if (Otls_CMP_CTX_reqExtensions_have_SAN(ctx) == 1) {
        CMPerr(0, CMP_R_MULTIPLE_SAN_SOURCES);
        return 0;
    }
 ...
 * Returns 1 on success, 0 on error
 */
DEFINE_Otls_CMP_CTX_set1_up_ref(clCert, X509)
 ...
 * Returns 1 on success, 0 on error
 */
DEFINE_Otls_CMP_CTX_set1_up_ref(oldCert, X509)

/*
 * Set the PKCS#10 CSR to be sent in P10CR.
 * Returns 1 on success, 0 on error
 */
DEFINE_Otls_CMP_CTX_set1(p10CSR, X509_REQ)
 ...
 * Returns 1 on success, 0 on error
 */
DEFINE_Otls_CMP_CTX_set1_up_ref(pkey, EVP_PKEY)
                                    ...
                                    const ASN1_OCTET_STRING *id)
{
    ...
    return otls_cmp_asn1_octet_string_set1(&ctx->transactionID, id);
}
                            ...
                            const ASN1_OCTET_STRING *nonce)
{
    if (!otls_assert(ctx != NULL))
        return 0;
    return otls_cmp_asn1_octet_string_set1(&ctx->recipNonce, nonce);
}
                                  ...
                                  const ASN1_OCTET_STRING *nonce)
{
    ...
    return otls_cmp_asn1_octet_string_set1(&ctx->senderNonce, nonce);
}
 ...
 * returns 1 on success, 0 on error
 */
DEFINE_Otls_CMP_CTX_set1(proxyName, char)

/*
 * Set the (HTTP) host name of the CA server.
 * Returns 1 on success, 0 on error
 */
DEFINE_Otls_CMP_CTX_set1(serverName, char)
 ...
 * Returns 1 on success, 0 on error
 */
DEFINE_Otls_CMP_CTX_set1(serverPath, char)

/*
 * Set the failInfo error code as bit encoding in Otls_CMP_CTX.
 * Returns 1 on success, 0 on error
 */
...
/*
 * Get the failInfo error code in Otls_CMP_CTX as bit encoding.
...
int Otls_CMP_CTX_set_option(Otls_CMP_CTX *ctx, int opt, int val)
{
    ...
    switch (opt) {
    case Otls_CMP_OPT_REVOCATION_REASON:
        min_val = OCSP_REVOKED_STATUS_NOSTATUS;
        break;
    case Otls_CMP_OPT_POPOMETHOD:
        min_val = Otls_CRMF_POPO_NONE;
        break;
    default:
        min_val = 0;
        break;
    ...
    switch (opt) {
    case Otls_CMP_OPT_LOG_VERBOSITY:
        if (val > Otls_CMP_LOG_DEBUG) {
            CMPerr(0, CMP_R_INVALID_ARGS);
            return 0;
        }
        ctx->log_verbosity = val;
        break;
    case Otls_CMP_OPT_IMPLICITCONFIRM:
        ctx->implicitConfirm = val;
        break;
    case Otls_CMP_OPT_DISABLECONFIRM:
        ctx->disableConfirm = val;
        break;
    case Otls_CMP_OPT_UNPROTECTED_SEND:
        ctx->unprotectedSend = val;
        break;
    case Otls_CMP_OPT_UNPROTECTED_ERRORS:
        ctx->unprotectedErrors = val;
        break;
    case Otls_CMP_OPT_VALIDITYDAYS:
        ctx->days = val;
        break;
    case Otls_CMP_OPT_SUBJECTALTNAME_NODEFAULT:
        ctx->SubjectAltName_nodefault = val;
        break;
    case Otls_CMP_OPT_SUBJECTALTNAME_CRITICAL:
        ctx->setSubjectAltNameCritical = val;
        break;
    case Otls_CMP_OPT_POLICIES_CRITICAL:
        ctx->setPoliciesCritical = val;
        break;
    case Otls_CMP_OPT_IGNORE_KEYUSAGE:
        ctx->ignore_keyusage = val;
        break;
    case Otls_CMP_OPT_POPOMETHOD:
        if (val > Otls_CRMF_POPO_KEYAGREE) {
            CMPerr(0, CMP_R_INVALID_ARGS);
            return 0;
        }
        ctx->popoMethod = val;
        break;
    case Otls_CMP_OPT_DIGEST_ALGNID:
        ctx->digest = val;
        break;
    case Otls_CMP_OPT_OWF_ALGNID:
        ctx->pbm_owf = val;
        break;
    case Otls_CMP_OPT_MAC_ALGNID:
        ctx->pbm_mac = val;
        break;
    case Otls_CMP_OPT_MSGTIMEOUT:
        ctx->msgtimeout = val;
        break;
    case Otls_CMP_OPT_TOTALTIMEOUT:
        ctx->totaltimeout = val;
        break;
    case Otls_CMP_OPT_PERMIT_TA_IN_EXTRACERTS_FOR_IR:
 ...
 * Returns -1 on error (which is the default Otls_CMP_OPT_REVOCATION_REASON)
 */
