...
ASN1_ITEM_TEMPLATE(OSSL_CMP_POPODECKEYCHALLCONTENT) = ASN1_EX_TEMPLATE_TYPE(
	ASN1_TFLG_SEQUENCE_OF, 0, OSSL_CMP_POPODECKEYCHALLCONTENT,
	OSSL_CMP_CHALLENGE)
					      ...
					      const X509 *cert, int only_DN)
{
	...
	const ASN1_TIME *last = crl == NULL ? NULL :
     ...
     */
	if (cert != NULL) {
		crldps = X509_get_ext_d2i(cert, NID_crl_distribution_points,
					  NULL, NULL);
		...
		idp = X509_CRL_get_ext_d2i(crl, NID_issuing_distribution_point,
					   NULL, NULL);
	...
	if (dpn == NULL && CRLissuer == NULL) {
		if (cert != NULL) {
			...
			if (akid != NULL &&
			    gennames_allowed(akid->issuer, only_DN))
				CRLissuer = akid->issuer;
			else
				CRLissuer = issuers = gennames_new(
