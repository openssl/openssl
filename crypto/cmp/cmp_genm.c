			   ...
			   int type_CA, const X509_VERIFY_PARAM *vpm)
{
	...
	int ret = res == 0;
	OSSL_CMP_severity level = vpm == NULL ? OSSL_CMP_LOG_WARNING :
						OSSL_CMP_LOG_ERR;
	...
	if (type_CA >= 0 && (ex_flags & EXFLAG_V1) == 0) {
		...
		if ((type_CA != 0) != is_CA) {
			cert_msg(OPENSSL_FUNC, OPENSSL_FILE, OPENSSL_LINE,
				 level, ctx, source, cert,
				 is_CA ? "is not an EE cert" :
					 "is not a CA cert");
			ret = 0;
		}
	}
			       ...
			       const X509_VERIFY_PARAM *vpm)
{
	...
	for (i = 0; i < sk_X509_num(certs /* may be NULL */); i++)
		ret = ossl_X509_check(ctx, source, sk_X509_value(certs, i),
				      type_CA, vpm) &&
				    ...
				    int expected, const char *desc)
{
	...
	for (i = 0; i < n; i++) {
		...
		if (OBJ_obj2nid(obj) == expected) {
			for (i++; i < n; i++)
				OSSL_CMP_ITAV_free(
					sk_OSSL_CMP_ITAV_shift(itavs));
			sk_OSSL_CMP_ITAV_free(itavs);
			return itav;
		}

		if (OBJ_obj2txt(name + offset, (int)(sizeof(name) - offset),
				obj, 0) < 0)
			strcat(name, "<unknown>");
		ossl_cmp_log2(WARN, ctx, "%s' while expecting 'id-it-%s'", name,
			      desc);
...
int OSSL_CMP_get1_caCerts(OSSL_CMP_CTX *ctx, STACK_OF(X509) **out)
{
	...
	*out = NULL;
	...
	if ((req = OSSL_CMP_ITAV_new_caCerts(NULL)) == NULL)
		return 0;
	if ((itav = get_genm_itav(ctx, req, NID_id_it_caCerts, "caCerts")) ==
	    NULL)
		return 0;
		     ...
		     X509 *target, const char *desc)
{
	...
	res = verify_ss_cert(OSSL_CMP_CTX_get0_libctx(ctx),
			     OSSL_CMP_CTX_get0_propq(ctx), ts, untrusted,
			     target);
	if (!res)
		ERR_raise_data(
				  ...
				  X509 **oldWithNew)
{
	...
	*newWithNew = NULL;
	...
	if ((req = OSSL_CMP_ITAV_new_rootCaCert(oldWithOld)) == NULL)
		return 0;
	itav = get_genm_itav(ctx, req, NID_id_it_rootCaKeyUpdate,
			     "rootCaKeyUpdate");
	if (itav == NULL)
		return 0;
	...
	if ((oldWithOld_copy = X509_dup(oldWithOld)) == NULL &&
	    oldWithOld != NULL)
		goto end;
			    ...
			    const X509_CRL *last_crl, X509_CRL **crl)
{
	...
	*crl = NULL;
	...
	if ((status = OSSL_CMP_CRLSTATUS_create(last_crl, crlcert, 1)) ==
	    NULL) {
	...
	if ((*crl = sk_X509_CRL_value(crls, 0)) == NULL ||
	    !X509_CRL_up_ref(*crl)) {
		*crl = NULL;
		goto end;
	}
