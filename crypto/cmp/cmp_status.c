...
#define ADVANCE_BUFFER                                             \
	...
	if (fail_info != -1 && fail_info != 0) {
		printed_chars =
			BIO_snprintf(write_ptr, bufsize, "; PKIFailureInfo: ");
		ADVANCE_BUFFER;
		for (failure = 0; failure <= OSSL_CMP_PKIFAILUREINFO_MAX;
		     failure++) {
	...
	if (!failinfo_found && status != OSSL_CMP_PKISTATUS_accepted &&
		...
		printed_chars =
			BIO_snprintf(write_ptr, bufsize, "; <no failure info>");
		ADVANCE_BUFFER;
	}
