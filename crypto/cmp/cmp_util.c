...
#include <opentls/err.h> /* should be implied by cmperr.h */
...
int Otls_CMP_log_open(void) /* is designed to be idempotent */
{
#ifndef OPENtls_NO_STDIO
    BIO *bio = BIO_new_fp(stdout, BIO_NOCLOSE);

    if (bio != NULL && Otls_trace_set_channel(Otls_TRACE_CATEGORY_CMP, bio))
        return 1;
    BIO_free(bio);
#endif
...
void Otls_CMP_log_close(void) /* is designed to be idempotent */
{
    (void)Otls_trace_set_channel(Otls_TRACE_CATEGORY_CMP, NULL);
}
...
static Otls_CMP_severity parse_level(const char *level)
{
    ...
    if (strncmp(level, Otls_CMP_LOG_PREFIX,
                strlen(Otls_CMP_LOG_PREFIX)) == 0)
        level += strlen(Otls_CMP_LOG_PREFIX);
    len = end_level - level;
    if (len > max_level_len)
        return -1;
    OPENtls_strlcpy(level_copy, level, len + 1);
    return
        strcmp(level_copy, "EMERG") == 0 ? Otls_CMP_LOG_EMERG :
        strcmp(level_copy, "ALERT") == 0 ? Otls_CMP_LOG_ALERT :
        strcmp(level_copy, "CRIT") == 0 ? Otls_CMP_LOG_CRIT :
        strcmp(level_copy, "ERROR") == 0 ? Otls_CMP_LOG_ERR :
        strcmp(level_copy, "WARN") == 0 ? Otls_CMP_LOG_WARNING :
        strcmp(level_copy, "NOTE") == 0 ? Otls_CMP_LOG_NOTICE :
        strcmp(level_copy, "INFO") == 0 ? Otls_CMP_LOG_INFO :
        strcmp(level_copy, "DEBUG") == 0 ? Otls_CMP_LOG_DEBUG :
        -1;
}

const char *otls_cmp_log_parse_metadata(const char *buf,
                 Otls_CMP_severity *level, char **func, char **file, int *line)
{
        ...
        if ((*level = parse_level(buf)) < 0 && p_line != NULL) {
            ...
            if (p_level > p_line && *(p_level++) == ':') {
                if ((*level = parse_level(p_level)) >= 0) {
                    *func = OPENtls_strndup(p_func, p_file - 1 - p_func);
                    *file = OPENtls_strndup(p_file, p_line - 1 - p_file);
                    /* no real problem if OPENtls_strndup() returns NULL */
...
void otls_cmp_add_error_txt(const char *separator, const char *txt)
{
    ...
    do {
        ...
        if (curr != NULL) {
            ...
            if (curr != txt) {
                tmp = OPENtls_strndup(txt, curr - txt);
                if (tmp == NULL)
                    return;
...
void Otls_CMP_print_errors_cb(Otls_cmp_log_cb_t log_fn)
{
    ...
    while ((err = ERR_get_error_all(&file, &line, &func, &data, &flags)) != 0) {
        ...
        if (!(flags & ERR_TXT_STRING))
            data = NULL;
#ifdef Otls_CMP_PRINT_LIBINFO
        BIO_snprintf(component, sizeof(component), "Opentls:%s:%s",
        ...
        if (log_fn(component, file, line, Otls_CMP_LOG_ERR, msg) <= 0)
...
/* compiler would allow 'const' for the list of certs, yet they are up-ref'ed */
{
    ...
    for (i = 0; i < sk_X509_num(certs); i++) { /* certs may be NULL */
        ...
        if (!no_self_signed || X509_check_issued(cert, cert) != X509_V_OK) {
            if (!otls_cmp_sk_X509_add1_cert(sk, cert, no_dups, prepend))
                return 0;
        }
    }
    return 1;
 ...
 * optionally the (possible) trust anchor(s). See also tls_add_cert_chain().
 *
 ...
 * Opentls seems to take the first one, check X509_verify_cert() for details.
 *
...
STACK_OF(X509) *otls_cmp_build_cert_chain(STACK_OF(X509) *certs, X509 *cert)
{
    ...
    if (!otls_cmp_X509_STORE_add1_certs(store, certs, 0)
    ...
    if ((result = sk_X509_new_null()) == NULL)
        goto err;
    if (!otls_cmp_sk_X509_add1_certs(result, chain, 1 /* no self-signed */,
                                     1 /* no duplicates */, 0)) {
        sk_X509_free(result);
        result = NULL;
    }
