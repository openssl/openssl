	...
	if (p_file != NULL) {
		...
		if ((*level = parse_level(buf)) < 0 && p_line != NULL) {
			...
			const long line_number =
				strtol(++p_line, &p_level_tmp, 10);

			p_level = p_level_tmp;
			if (p_level > p_line && *(p_level++) == ':') {
				if ((*level = parse_level(p_level)) >= 0) {
					*func = OPENSSL_strndup(
						p_func, p_file - 1 - p_func);
					*file = OPENSSL_strndup(
						p_file, p_line - 1 - p_file);
...
static const char *improve_location_name(const char *func, const char *fallback)
{
	...
	return func == NULL || *func == '\0' ||
			       strcmp(func, UNKNOWN_FUNC) == 0 ?
		       fallback :
		       func;
}
...
#ifndef NDEBUG
	if (BIO_printf(bio, "%s:%s:%d:",
		       improve_location_name(component, "CMP"), file, line) < 0)
		return 0;
#endif
...
void OSSL_CMP_print_errors_cb(OSSL_CMP_log_cb_t log_fn)
{
	...
	while ((err = ERR_get_error_all(&file, &line, &func, &data, &flags)) !=
		...
		if (log_fn == NULL) {
			...
			if (bio != NULL) {
				OSSL_CMP_print_to_bio(bio, component, file,
						      line, OSSL_CMP_LOG_ERR,
						      msg);
				BIO_free(bio);
			}
#else
			...
			if (log_fn(component, file, line, OSSL_CMP_LOG_ERR,
				   msg) <= 0)
	...
	for (i = 0; i < sk_X509_num(certs); i++) {
		...
		if (!only_self_signed || X509_self_signed(cert, 0) == 1)
			if (!X509_STORE_add_cert(
				    store, cert)) /* ups cert ref counter */
				return 0;
	}
	return 1;
}
