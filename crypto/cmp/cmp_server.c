				      ...
				      const OSSL_CMP_MSG *req)
{
	...
	if (!ossl_assert(srv_ctx != NULL && srv_ctx->ctx != NULL &&
			 req != NULL && srv_ctx->delayed_delivery != NULL))
		return NULL;
					  ...
					  const OSSL_CMP_MSG *req)
{
	...
	if (OSSL_CMP_MSG_get_bodytype(req) == OSSL_CMP_PKIBODY_P10CR) {
		certReqId =
			OSSL_CMP_CERTREQID_NONE; /* p10cr does not include an Id */
		p10cr = req->body->value.p10cr;
	} else {
		OSSL_CRMF_MSGS *reqs =
			req->body->value.ir; /* same for cr and kur */
		...
		if (certReqId !=
		    OSSL_CMP_CERTREQID) { /* so far, only possible value */
	...
	if (central_keygen == 0 &&
		...
		si = srv_ctx->process_cert_request(srv_ctx, req, certReqId, crm,
						   p10cr, &certOut, &chainOut,
						   &caPubs);
		if (si == NULL)
			goto err;
		...
		if (!OSSL_CMP_CTX_set_option(
			    srv_ctx->ctx, OSSL_CMP_OPT_IMPLICIT_CONFIRM,
	...
	msg = ossl_cmp_certrep_new(srv_ctx->ctx, bodytype, certReqId, si,
				   certOut, keyOut, NULL /* enc */, chainOut,
				   caPubs, srv_ctx->sendUnprotectedErrors);
				 ...
				 int accept_unprotected_requests)
{
	...
	if (OSSL_CMP_MSG_get_bodytype(req) == OSSL_CMP_PKIBODY_ERROR &&
	    OSSL_CMP_CTX_get_option(ctx, OSSL_CMP_OPT_UNPROTECTED_ERRORS) ==
		    1) {
		ossl_cmp_warn(ctx,
					   ...
					   const OSSL_CMP_MSG *req)
{
	...
	case OSSL_CMP_PKIBODY_ERROR:
		if (ctx->transactionID != NULL) {
			char *tid =
				i2s_ASN1_OCTET_STRING(NULL, ctx->transactionID);
	...
	if (ctx->secretValue != NULL && ctx->pkey != NULL &&
		...
		ctx->secretValue =
			NULL; /* use MSG_SIG_ALG when protecting rsp */
	if (!req_verified)
		goto err;
...
err:
	if (rsp == NULL) {
		...
		if (!req_verified) {
             ...
             */
			if (ctx->transactionID == NULL)
				(void)OSSL_CMP_CTX_set1_transactionID(
					ctx, hdr->transactionID);
			(void)ossl_cmp_ctx_set1_recipNonce(ctx,
							   hdr->senderNonce);
		}
	...
	rsp_type = rsp != NULL ? OSSL_CMP_MSG_get_bodytype(rsp) :
	...
	case OSSL_CMP_PKIBODY_KUP:
		if (OSSL_CMP_CTX_get_option(ctx,
					    OSSL_CMP_OPT_IMPLICIT_CONFIRM) == 0)
			break;
		/* fall through */
	...
	case OSSL_CMP_PKIBODY_GENP:
		...
		if (srv_ctx->clean_transaction != NULL)
			(void)srv_ctx->clean_transaction(srv_ctx,
							 ctx->transactionID);
		(void)OSSL_CMP_CTX_set1_transactionID(ctx, NULL);
		(void)OSSL_CMP_CTX_set1_senderNonce(ctx, NULL);
		ctx->status =
			OSSL_CMP_PKISTATUS_unspecified; /* transaction closed */
