...
const char *ossl_cmp_bodytype_to_string(int type)
{
	static const char *type_names[] = {
		"IR",	    "IP",      "CR",	  "CP",	    "P10CR", "POPDECC",
		"POPDECR",  "KUR",     "KUP",	  "KRR",    "KRP",   "RR",
		"RP",	    "CCR",     "CCP",	  "CKUANN", "CANN",  "RANN",
		"CRLANN",   "PKICONF", "NESTED",  "GENM",   "GENP",  "ERROR",
		"CERTCONF", "POLLREQ", "POLLREP",
	};
...
static int add_crl_reason_extension(X509_EXTENSIONS **pexts, int reason_code)
{
	...
	if (val != NULL && ASN1_ENUMERATED_set(val, reason_code))
		res = add1_extension(pexts, NID_crl_reason,
				     0 /* non-critical */, val);
...
OSSL_CMP_MSG *ossl_cmp_msg_create(OSSL_CMP_CTX *ctx, int bodytype)
{
	...
	case OSSL_CMP_PKIBODY_POLLREQ:
		if ((msg->body->value.pollReq =
			     sk_OSSL_CMP_POLLREQ_new_null()) == NULL)
			goto err;
		return msg;
	case OSSL_CMP_PKIBODY_POLLREP:
		if ((msg->body->value.pollRep =
	...
	case OSSL_CMP_PKIBODY_GENP:
		if ((msg->body->value.genm = sk_OSSL_CMP_ITAV_new_null()) ==
		    NULL)
			goto err;
		return msg;
...
OSSL_CRMF_MSG *OSSL_CMP_CTX_setup_CRM(OSSL_CMP_CTX *ctx, int for_KUR, int rid)
{
	...
	if (ctx->p10CSR != NULL &&
		...
		goto err;
	if (!ctx->SubjectAltName_nodefault && !HAS_SAN(ctx) &&
	    refcert != NULL &&
				   ...
				   const OSSL_CRMF_MSG *crm)
{
	...
	if (ctx->implicitConfirm &&
	    !ossl_cmp_hdr_set_implicitConfirm(msg->header))
		goto err;
	...
	if (type != OSSL_CMP_PKIBODY_P10CR) {
		...
		if (ctx->popoMethod >= OSSL_CRMF_POPO_SIGNATURE &&
		    privkey == NULL) {
					   ...
					   const EVP_PKEY *pkey)
{
	...
	envData = ossl_cms_sign_encrypt(privbio, ctx->cert, ctx->chain,
					ctx->pkey, CMS_BINARY,
		     ...
		     STACK_OF(X509) *caPubs, int unprotectedErrors)
{
	...
	if (status != OSSL_CMP_PKISTATUS_rejection &&
		...
		if (pkey != NULL) {
#ifndef OPENSSL_NO_CMS
			if ((resp->certifiedKeyPair->privateKey =
				     enc_privkey(ctx, pkey)) == NULL)
				goto err;
#else
			ERR_raise(ERR_LIB_CMP, ERR_R_UNSUPPORTED);
			goto err;
				 ...
				 int unprotected)
{
	...
	if ((msg->body->value.error->errorCode = ASN1_INTEGER_new()) == NULL)
		goto err;
	if (!ASN1_INTEGER_set_int64(msg->body->value.error->errorCode,
				    errorCode))
		goto err;
	if (errorCode > 0 &&
				    ...
				    int fail_info, const char *text)
{
	...
	sinfo = fail_info != 0 ?
			OSSL_CMP_STATUSINFO_new(OSSL_CMP_PKISTATUS_rejection,
						fail_info, text) :
			OSSL_CMP_STATUSINFO_new(OSSL_CMP_PKISTATUS_accepted, 0,
						text);
	if (sinfo == NULL)
		goto err;
				      ...
				      const OSSL_CMP_CERTRESPONSE *crep)
{
	...
	if (encr_key != NULL) {
		...
		pkey = OSSL_CRMF_ENCRYPTEDKEY_get1_pkey(
			encr_key, ctx->trusted, ctx->untrusted, ctx->pkey,
			ctx->cert, ctx->secretValue, ctx->libctx, ctx->propq);
		if (pkey == NULL) {
			ERR_raise(ERR_LIB_CMP,
	...
	if ((coec = crep->certifiedKeyPair->certOrEncCert) != NULL) {
		...
		case OSSL_CMP_CERTORENCCERT_ENCRYPTEDCERT:
			...
			crt = OSSL_CRMF_ENCRYPTEDKEY_get1_encCert(
				coec->value.encryptedCert, ctx->libctx,
				ctx->propq, pkey, 0);
			break;
		default:
...
int OSSL_CMP_MSG_update_recipNonce(OSSL_CMP_CTX *ctx, OSSL_CMP_MSG *msg)
{
	...
	if (ctx->recipNonce ==
	    NULL) /* nothing to do for 1st msg in transaction */
		return 1;
...
OSSL_CMP_MSG *d2i_OSSL_CMP_MSG_bio(BIO *bio, OSSL_CMP_MSG **msg)
{
	...
	return ASN1_item_d2i_bio_ex(ASN1_ITEM_rptr(OSSL_CMP_MSG), bio, msg,
				    libctx, propq);
}
...
int ossl_cmp_is_error_with_waiting(const OSSL_CMP_MSG *msg)
{
	...
	return (OSSL_CMP_MSG_get_bodytype(msg) == OSSL_CMP_PKIBODY_ERROR &&
		ossl_cmp_pkisi_get_status(
			msg->body->value.error->pKIStatusInfo) ==
