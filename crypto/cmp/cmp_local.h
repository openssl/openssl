...
struct ossl_cmp_ctx_st {
	...
	OSSL_CMP_transfer_cb_t
		transfer_cb; /* default: OSSL_CMP_MSG_http_perform */
	...
	X509_STORE *
		trusted; /* trust store maybe w CRLs and cert verify callback */
	...
	*secretValue; /* password/shared secret for MSG_MAC_ALG */
	...
	ASN1_OCTET_STRING
	*first_senderNonce; /* sender nonce when starting to poll */
	...
	STACK_OF(X509)
	*caPubs; /* CA certs received from server (in IP message) */
	STACK_OF(X509) *extraCertsIn; /* extraCerts received from server */

	/* certificate confirmation */
	OSSL_CMP_certConf_cb_t
		certConf_cb; /* callback for app checking new cert */
...
#define ossl_cmp_log(level, ctx, msg)                               \
	ossl_cmp_print_log(OSSL_CMP_LOG_##level, ctx, OPENSSL_FUNC, \
			   OPENSSL_FILE, OPENSSL_LINE, #level, "%s", msg)
#define ossl_cmp_log1(level, ctx, fmt, arg1)                        \
	ossl_cmp_print_log(OSSL_CMP_LOG_##level, ctx, OPENSSL_FUNC, \
			   OPENSSL_FILE, OPENSSL_LINE, #level, fmt, arg1)
...
OSSL_CMP_MSG *
ossl_cmp_certrep_new(OSSL_CMP_CTX *ctx, int bodytype, int certReqId,
		     const OSSL_CMP_PKISI *si, X509 *cert, const EVP_PKEY *pkey,
		     const X509 *encryption_recip, STACK_OF(X509) *chain,
		     STACK_OF(X509) *caPubs, int unprotectedErrors);
...
OSSL_CMP_MSG *ossl_cmp_pollRep_new(OSSL_CMP_CTX *ctx, int crid,
				   int64_t poll_after);
OSSL_CMP_PKISI *ossl_cmp_revrepcontent_get_pkisi(OSSL_CMP_REVREPCONTENT *rrep,
						 int rsid);
