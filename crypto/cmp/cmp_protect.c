    ...
    const OSSL_CMP_MSG *msg)
{
    ...
    if (OBJ_obj2nid(algorOID) == NID_id_PasswordBasedMAC) {
        ...
        pbm = d2i_OSSL_CRMF_PBMPARAMETER(NULL, &pbm_str_uc, (int)ASN1_STRING_length_ex(pbm_str));
        if (pbm == NULL) {
        ...
        if (!OSSL_CRMF_pbm_new(ctx->libctx, ctx->propq,
                ...
                ASN1_STRING_get0_data(ctx->secretValue), (int)ASN1_STRING_length_ex(ctx->secretValue),
                &protection, &sig_len))
            goto end;
