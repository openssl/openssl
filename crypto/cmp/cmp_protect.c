					  ...
					  const OSSL_CMP_MSG *msg)
{
	...
	if (OBJ_obj2nid(algorOID) == NID_id_PasswordBasedMAC) {
		...
		if (!OSSL_CRMF_pbm_new(
			    ctx->libctx, ctx->propq, pbm, prot_part_der,
		...
		if (EVP_PKEY_get_default_digest_name(ctx->pkey, name,
						     sizeof(name)) > 0 &&
		    strcmp(name, "UNDEF") ==
			    0) /* at least for Ed25519, Ed448 */
			md = NULL;

		if ((prot = ASN1_BIT_STRING_new()) == NULL)
			return NULL;
		if (ASN1_item_sign_ex(
			    ASN1_ITEM_rptr(OSSL_CMP_PROTECTEDPART),
...
void ossl_cmp_set_own_chain(OSSL_CMP_CTX *ctx)
{
	...
	if (ctx->chain == NULL) {
		...
		ctx->chain = X509_build_chain(ctx->cert, ctx->untrusted, NULL,
...
int ossl_cmp_msg_add_extraCerts(OSSL_CMP_CTX *ctx, OSSL_CMP_MSG *msg)
{
	...
	if (!ctx->unprotectedSend && ctx->secretValue == NULL &&
		...
		if (ctx->chain != NULL) {
			...
			if (!ossl_x509_add_cert_new(&msg->extraCerts, ctx->cert,
						    prepend))
				return 0;
			ossl_cmp_debug(
				ctx,
...
int ossl_cmp_msg_protect(OSSL_CMP_CTX *ctx, OSSL_CMP_MSG *msg)
{
	...
	if (ctx->unprotectedSend) {
		...
		if (!set_senderKID(ctx, msg,
				   X509_get0_subject_key_id(ctx->cert)))
			goto err;
