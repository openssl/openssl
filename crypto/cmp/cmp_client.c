...
#define IS_CREP(t)                                                   \
	((t) == OSSL_CMP_PKIBODY_IP || (t) == OSSL_CMP_PKIBODY_CP || \
				 ...
				 ossl_unused int expected_type)
{
	...
	case OSSL_CMP_PKIBODY_RP: {
		OSSL_CMP_PKISI *si = ossl_cmp_revrepcontent_get_pkisi(
			rep->body->value.rp, OSSL_CMP_REVREQSID);

		if (si == NULL)
			return -1;
		if (ossl_cmp_pkisi_get_status(si) ==
		    OSSL_CMP_PKISTATUS_rejection)
			msg_type =
				"revocation response message with rejection status";
		break;
	}
...
static int save_statusInfo(OSSL_CMP_CTX *ctx, OSSL_CMP_PKISI *si)
{
	...
	for (i = 0; i < sk_ASN1_UTF8STRING_num(ss); i++) {
		...
		if (dup == NULL ||
		    !sk_ASN1_UTF8STRING_push(ctx->statusString, dup)) {
...
static int is_crep_with_waiting(const OSSL_CMP_MSG *resp, int rid)
{
	...
	return (crep != NULL && ossl_cmp_pkisi_get_status(crep->status) ==
			      ...
			      OSSL_CMP_MSG **rep, int expected_type)
{
	...
	const char *expected_type_str =
		ossl_cmp_bodytype_to_string(expected_type);
	...
	*rep = (*transfer_cb)(ctx, req);
	...
	if (bt != OSSL_CMP_PKIBODY_ERROR) {
		...
		if (save_statusInfo(ctx, si) &&
		    OSSL_CMP_CTX_snprint_PKIStatus(ctx, buf, sizeof(buf)) !=
			    NULL)
		...
		if (emc->errorDetails != NULL) {
			char *text = ossl_sk_ASN1_UTF8STRING2text(
				emc->errorDetails, ", ",
			     ...
			     OSSL_CMP_MSG **rep, int *checkAfter)
{
	...
	*rep = NULL;
	for (;;) {
		...
		if (!send_receive_check(ctx, preq, &prep,
					OSSL_CMP_PKIBODY_POLLREP))
			goto err;

		/* handle potential pollRep */
		if (OSSL_CMP_MSG_get_bodytype(prep) ==
		    OSSL_CMP_PKIBODY_POLLREP) {
			OSSL_CMP_POLLREPCONTENT *prc =
			...
			pollRep =
				ossl_cmp_pollrepcontent_get0_pollrep(prc, rid);
			if (pollRep == NULL)
				goto err;

			if (!ASN1_INTEGER_get_int64(&check_after,
						    pollRep->checkAfter)) {
				ERR_raise(ERR_LIB_CMP,
					  CMP_R_BAD_CHECKAFTER_IN_POLLREP);
				goto err;
			}
			if (check_after < 0 ||
				...
				ERR_raise(ERR_LIB_CMP,
					  CMP_R_CHECKAFTER_OUT_OF_RANGE);
				if (BIO_snprintf(str, OSSL_CMP_PKISI_BUFLEN,
						 "value = %jd",
						 check_after) >= 0)
			...
			if (ctx->total_timeout !=
			    0) { /* timeout is not infinite */
				const int exp = OSSL_CMP_EXPECTED_RESP_TIME;
				int64_t time_left = (int64_t)(ctx->end_time -
							      exp - time(NULL));
			...
			(void)ossl_cmp_exchange_error(
				ctx, OSSL_CMP_PKISTATUS_rejection,
				     ...
				     OSSL_CMP_MSG **rep, int expected_type)
{
	...
	if (ossl_cmp_is_error_with_waiting(*rep)) {
		if (!save_senderNonce_if_waiting(ctx, *rep,
		...
		*rep = NULL;
		...
		if (poll_for_response(ctx, 1 /* can sleep */,
				      OSSL_CMP_CERTREQID_NONE, rep,
				      NULL /* checkAfter */) <= 0) {
 ...
 */
int ossl_cmp_exchange_certConf(OSSL_CMP_CTX *ctx, int certReqId, int fail_info,
			       const char *txt)
{
			    ...
			    const char *txt, int errorCode, const char *details)
{
	...
	if ((error = ossl_cmp_error_new(ctx, si, errorCode, details, 0)) ==
	    NULL)
		goto err;

	res = send_receive_also_delayed(ctx, error, &PKIconf,
					OSSL_CMP_PKIBODY_PKICONF);
			      ...
			      OSSL_CMP_CERTRESPONSE *crep)
{
	...
	case OSSL_CMP_PKISTATUS_grantedWithMods:
		ossl_cmp_warn(ctx,
			      "received \"grantedWithMods\" for certificate");
		break;
			 ...
			 const char **text)
{
	...
	if (out_trusted == NULL) {
		ossl_cmp_debug(ctx,
			       "trying to build chain for newly enrolled cert");
		chain = X509_build_chain(cert, ctx->untrusted, out_trusted, 0,
					 ctx->libctx, ctx->propq);
	} else {
		X509_STORE_CTX *csc =
			X509_STORE_CTX_new_ex(ctx->libctx, ctx->propq);

		ossl_cmp_debug(ctx, "validating newly enrolled cert");
		if (csc == NULL)
			goto err;
		if (!X509_STORE_CTX_init(csc, out_trusted, cert,
					 ctx->untrusted))
			goto err;
...
err:
	...
	if (out_trusted != NULL) {
		if (chain == NULL) {
			ossl_cmp_err(ctx,
				     "failed to validate newly enrolled cert");
			fail_info = 1 << OSSL_CMP_PKIFAILUREINFO_incorrectData;
		} else {
			ossl_cmp_debug(
				ctx, "success validating newly enrolled cert");
		}
	} else if (chain == NULL) {
		ossl_cmp_warn(
			ctx,
...
retry:
	...
	if (ossl_cmp_pkisi_get_status(si) == OSSL_CMP_PKISTATUS_waiting) {
		...
		*resp = NULL;
		if ((ret = poll_for_response(ctx, sleep, rid, resp,
					     checkAfter)) != 0) {
	...
	if (cert == NULL) {
		ERR_add_error_data(
			1, "; cannot extract certificate from response");
		return 0;
	}
	...
	if (fail_info !=
	    0) /* immediately log error before any certConf exchange */
	...
	if (fail_info != 0) {
		ERR_raise_data(
			ERR_LIB_CMP, CMP_R_CERTIFICATE_NOT_ACCEPTED,
...
static int initial_certreq(OSSL_CMP_CTX *ctx, int req_type,
			   const OSSL_CRMF_MSG *crm, OSSL_CMP_MSG **p_rep,
			   int rep_type)
{
	OSSL_CMP_MSG *req;
	int res;
			 ...
			 const OSSL_CRMF_MSG *crm, int *checkAfter)
{
	...
	if (ctx->status !=
	    OSSL_CMP_PKISTATUS_waiting) { /* not polling already */
		...
		if (req_type < 0)
			return ossl_cmp_exchange_error(
				ctx, OSSL_CMP_PKISTATUS_rejection, 0,
			    ...
			    const OSSL_CRMF_MSG *crm)
{
	...
	if (cert_response(ctx, 1 /* sleep */, rid, &rep, NULL, req_type,
			  rep_type) <= 0)
		goto err;
...
int OSSL_CMP_exec_RR_ses(OSSL_CMP_CTX *ctx)
{
	...
	case OSSL_CMP_PKISTATUS_grantedWithMods:
		ossl_cmp_info(
			ctx, "revocation accepted (PKIStatus=grantedWithMods)");
		ret = 1;
		break;
	case OSSL_CMP_PKISTATUS_rejection:
		ERR_raise(ERR_LIB_CMP, CMP_R_REQUEST_REJECTED_BY_SERVER);
	...
	if (sk_OSSL_CRMF_CERTID_num(rrep->revCerts) >= 1) {
		...
		OSSL_CRMF_CERTTEMPLATE *tmpl =
			sk_OSSL_CMP_REVDETAILS_value(rr->body->value.rr, rsid)
				->certDetails;
		...
		if ((cid = ossl_cmp_revrepcontent_get_CertId(rrep, rsid)) ==
		    NULL) {
			ERR_raise(ERR_LIB_CMP, CMP_R_MISSING_CERTID);
			ret = 0;
			goto err;
		}
		if (X509_NAME_cmp(issuer, OSSL_CRMF_CERTID_get0_issuer(cid)) !=
		    0) {
#ifndef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
		...
		if (ASN1_INTEGER_cmp(serial, OSSL_CRMF_CERTID_get0_serialNumber(
						     cid)) != 0) {
#ifndef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
	...
	if (rrep->crls != NULL &&
	    sk_X509_CRL_num(rrep->crls) != num_RevDetails) {
		ERR_raise(ERR_LIB_CMP, CMP_R_WRONG_RP_COMPONENT_COUNT);
		ret = 0;
		goto err;
	}
