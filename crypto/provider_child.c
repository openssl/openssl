...
static int provider_create_child_cb(const OSSL_CORE_HANDLE *prov, void *cbdata)
{
	...
	if ((cprov = ossl_provider_find(ctx, provname, 1)) != NULL) {
         ...
         * init children
         */
		if ((cprov = ossl_provider_new(ctx, provname,
					       ossl_child_provider_init, NULL,
					       1)) == NULL)
			goto err;
				...
				const OSSL_DISPATCH *in)
{
	...
	if (!gbl->c_provider_register_child_cb(
		    gbl->handle, provider_create_child_cb,
		    provider_remove_child_cb, provider_global_props_cb, ctx))
		return 0;
