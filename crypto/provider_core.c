...
static struct provider_store_st *get_provider_store(OSSL_LIB_CTX *libctx)
{
	...
	store = ossl_lib_ctx_get_data(libctx,
				    ...
				    OSSL_PROVIDER_INFO *entry)
{
	...
	if (store->provinfosz == 0) {
		...
		tmpbuiltins = OPENSSL_realloc_array(store->provinfo, newsz,
						    sizeof(*store->provinfo));
		if (tmpbuiltins == NULL)
			goto err;
				   ...
				   STACK_OF(INFOPAIR) *parameters)
{
	...
	if ((prov->opbits_lock = CRYPTO_THREAD_lock_new()) == NULL ||
	    ...
	    (prov->parameters = sk_INFOPAIR_deep_copy(parameters, infopair_copy,
						      infopair_free)) == NULL) {
		ossl_provider_free(prov);
		ERR_raise(ERR_LIB_CRYPTO, ERR_R_CRYPTO_LIB);
		return NULL;
				 ...
				 OSSL_PARAM *params, int noconfig)
{
	...
	if (init_function == NULL) {
		...
		if (!CRYPTO_THREAD_read_lock(store->lock))
			return NULL;
		for (i = 0, p = store->provinfo; i < store->numprovinfo;
		     p++, i++) {
			if (strcmp(p->name, name) != 0)
				continue;
			...
			template.parameters = sk_INFOPAIR_deep_copy(
				p->parameters, infopair_copy, infopair_free);
	...
	if (params != NULL) {
		...
		for (i = 0; params[i].key != NULL; i++) {
			if (params[i].data_type != OSSL_PARAM_UTF8_STRING)
				continue;
			if (ossl_provider_info_add_parameter(
				    &template, params[i].key,
				    (char *)params[i].data) <= 0) {
				sk_INFOPAIR_pop_free(template.parameters,
						     infopair_free);
				return NULL;
			}
		}
		...
		if (ref == 0) {
			if (prov->flag_initialized) {
				...
				if (prov->error_strings != NULL) {
					ERR_unload_strings(prov->error_lib,
							   prov->error_strings);
...
int OSSL_PROVIDER_conf_get_bool(const OSSL_PROVIDER *prov, const char *name,
				int defval)
{
	char *val = NULL;
	...
	if (OSSL_PROVIDER_get_conf_parameters(prov, param) &&
	    OSSL_PARAM_modified(param) && val != NULL) {
		if ((strcmp(val, "1") == 0) ||
...
static int provider_init(OSSL_PROVIDER *prov)
{
	...
	if (prov->init_function == NULL) {
		...
		if (prov->module == NULL) {
			...
			if ((store = get_provider_store(prov->libctx)) ==
				    NULL ||
			    !CRYPTO_THREAD_read_lock(store->default_path_lock))
				goto end;
			...
			if (store->default_path != NULL) {
				allocated_load_dir =
					OPENSSL_strdup(store->default_path);
				CRYPTO_THREAD_unlock(store->default_path_lock);
			...
			if (module_path == NULL)
				module_path = allocated_path =
					DSO_convert_filename(prov->module,
							     prov->name);
			if (module_path != NULL)
				merged_path = DSO_merge(prov->module,
							module_path, load_dir);

			if (merged_path == NULL ||
			    (DSO_load(prov->module, merged_path, NULL, 0)) ==
				    NULL) {
				DSO_free(prov->module);
				prov->module = NULL;
			}
		...
		if (prov->module == NULL) {
			...
			ERR_raise_data(ERR_LIB_CRYPTO, ERR_R_DSO_LIB, "name=%s",
				       prov->name);
			goto end;
		}

		prov->init_function = (OSSL_provider_init_fn *)DSO_bind_func(
			prov->module, "OSSL_provider_init");
#endif
	}
	...
	if (prov->init_function == NULL) {
		ERR_raise_data(
			ERR_LIB_CRYPTO, ERR_R_UNSUPPORTED,
	...
	{
		BIO_printf(trc_out, "(provider %s) initalizing\n", prov->name);
	}
	...
	if (!prov->init_function((OSSL_CORE_HANDLE *)prov, core_dispatch,
		...
		ERR_raise_data(ERR_LIB_CRYPTO, ERR_R_INIT_FAIL, "name=%s",
			       prov->name);
		goto end;
	}
	...
	if (provider_dispatch != NULL) {
		for (; provider_dispatch->function_id != 0;
		     provider_dispatch++) {
			switch (provider_dispatch->function_id) {
			case OSSL_FUNC_PROVIDER_TEARDOWN:
				prov->teardown = OSSL_FUNC_provider_teardown(
					provider_dispatch);
				break;
			...
			case OSSL_FUNC_PROVIDER_SELF_TEST:
				prov->self_test = OSSL_FUNC_provider_self_test(
					provider_dispatch);
				break;
	...
	if (p_get_reason_strings != NULL) {
		...
		for (cnt2 = 1; cnt2 <= cnt; cnt2++) {
			prov->error_strings[cnt2].error =
				(int)reasonstrings[cnt2 - 1].id;
			prov->error_strings[cnt2].string =
				reasonstrings[cnt2 - 1].ptr;
		}
			       ...
			       int removechildren)
{
	...
	if (!CRYPTO_atomic_add(&prov->activatecnt, -1, &count,
	...
	if (removechildren && store != NULL) {
		...
		for (i = 0; i < max; i++) {
			child_cb = sk_OSSL_PROVIDER_CHILD_CB_value(
				store->child_cbs, i);
			child_cb->remove_cb((OSSL_CORE_HANDLE *)prov,
					    child_cb->cbdata);
		}
	}
#endif
	...
	OSSL_TRACE_BEGIN(PROVIDER)
	{
		...
		if (prov->gettable_params != NULL) {
			if (!OSSL_PARAM_print_to_bio(ret, trc_out, 0))
				BIO_printf(trc_out,
					   "Failed to parse param values\n");
			OPENSSL_free(buf);
		} else {
			BIO_printf(
				trc_out,
				"Provider doesn't implement gettable_params\n");
		}
	}
	OSSL_TRACE_END(PROVIDER);
#endif
...
{
	return prov->random_bytes == NULL ?
		       0 :
		       prov->random_bytes(prov->provctx, which, buf, n,
					  strength);
}
				   ...
				   void *arg)
{
	if (prov->get_capabilities != NULL) {
		...
		OSSL_TRACE_END(PROVIDER);
#endif
		return prov->get_capabilities(prov->provctx, capability, cb,
					      arg);
	}
	return 1;
}
						    ...
						    int *no_cache)
{
	...
	if (prov->query_operation == NULL) {
		...
		{
			BIO_printf(trc_out,
				   "provider %s lacks query operation!\n",
				   prov->name);
		}
		OSSL_TRACE_END(PROVIDER);
#endif
		...
		if (res != NULL) {
			...
			for (idx = res; idx->algorithm_names != NULL; idx++) {
				BIO_printf(
					...
					prov->name,
					idx->algorithm_names == NULL ?
						"none" :
						idx->algorithm_names,
					idx->property_definition == NULL ?
						"none" :
						idx->property_definition,
					idx->algorithm_description == NULL ?
						"none" :
						idx->algorithm_description);
			}
		} else {
			BIO_printf(trc_out,
				   "(provider %s) query_operation failed\n",
				   prov->name);
		}
	}
	OSSL_TRACE_END(PROVIDER);
...
int ossl_provider_set_operation_bit(OSSL_PROVIDER *provider, size_t bitnum)
{
	...
	if (provider->operation_bits_sz <= byte) {
		unsigned char *tmp =
			OPENSSL_realloc(provider->operation_bits, byte + 1);
...
static int ossl_provider_register_child_cb(
	const OSSL_CORE_HANDLE *handle,
	int (*create_cb)(const OSSL_CORE_HANDLE *provider, void *cbdata),
	int (*remove_cb)(const OSSL_CORE_HANDLE *provider, void *cbdata),
	int (*global_props_cb)(const char *props, void *cbdata), void *cbdata)
{
	...
	if (i == max) {
		/* Success */
		ret = sk_OSSL_PROVIDER_CHILD_CB_push(store->child_cbs,
						     child_cb);
	}
	if (i != max || ret <= 0) {
...
static const OSSL_PARAM param_types[] = {
	OSSL_PARAM_DEFN(OSSL_PROV_PARAM_CORE_VERSION, OSSL_PARAM_UTF8_PTR, NULL,
			0),
	OSSL_PARAM_DEFN(OSSL_PROV_PARAM_CORE_PROV_NAME, OSSL_PARAM_UTF8_PTR,
			NULL, 0),
#ifndef FIPS_MODULE
	OSSL_PARAM_DEFN(OSSL_PROV_PARAM_CORE_MODULE_FILENAME,
...
static int core_get_params(const OSSL_CORE_HANDLE *handle, OSSL_PARAM params[])
{
	...
	if ((p = OSSL_PARAM_locate(params, OSSL_PROV_PARAM_CORE_VERSION)) !=
	    NULL)
		OSSL_PARAM_set_utf8_ptr(p, OPENSSL_VERSION_STR);
	if ((p = OSSL_PARAM_locate(params, OSSL_PROV_PARAM_CORE_PROV_NAME)) !=
	    NULL)
		OSSL_PARAM_set_utf8_ptr(p, prov->name);

#ifndef FIPS_MODULE
	if ((p = OSSL_PARAM_locate(
		     params, OSSL_PROV_PARAM_CORE_MODULE_FILENAME)) != NULL)
...
static size_t rand_get_user_entropy(const OSSL_CORE_HANDLE *handle,
				    ...
				    size_t min_len, size_t max_len)
{
	return ossl_rand_get_user_entropy(
		(OSSL_LIB_CTX *)core_get_libctx(handle), pout, entropy, min_len,
		max_len);
}
...
{
	ossl_rand_cleanup_entropy((OSSL_LIB_CTX *)core_get_libctx(handle), buf,
				  len);
}
...
static void rand_cleanup_nonce(const OSSL_CORE_HANDLE *handle,
			       unsigned char *buf, size_t len)
{
	ossl_rand_cleanup_nonce((OSSL_LIB_CTX *)core_get_libctx(handle), buf,
				len);
}
...
static const OSSL_DISPATCH core_dispatch_[] = {
	...
	{ OSSL_FUNC_CLEANUP_USER_ENTROPY,
	  (void (*)(void))rand_cleanup_user_entropy },
