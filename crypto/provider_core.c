...
# ifndef OPENSSL_NO_THREAD_POOL
/*
 * Note that we need wrappers around ossl_get_avail_threads() and
 * ossl_crypto_thread_start() since these functions must obtain the
 * libctx from a |handle|. This is not required for the join() and clean()
 * functions as these pass a |vhandle|.
 */
