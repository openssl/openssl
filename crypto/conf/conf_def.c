...
static CONF_METHOD default_method = { "OpenSSL default", def_create,
				      def_init_default,	 def_destroy,
				      def_destroy_data,	 def_load_bio,
				      def_dump,		 def_is_number,
				      def_to_int,	 def_load };
...
read_retry:
		...
		if (i == 0 && !again) {
			...
			if (dirctx != NULL) {
				...
				if ((next = get_next_file(dirpath, &dirctx)) !=
				    NULL) {
					BIO_vfree(in);
					in = next;
					goto read_retry;
			...
			if (CHECK_AND_SKIP_PREFIX(pname, ".pragma") &&
				...
				*pval++ = '\0';
				...
				if (strcmp(p, "dollarid") == 0) {
					...
					if ((conf->includedir = OPENSSL_strdup(
						     pval)) == NULL)
						goto err;
				}
				...
				if (include_dir != NULL &&
				    !ossl_is_absolute_path(include)) {
					size_t newlen = strlen(include_dir) +
							strlen(include) + 2;
					...
					OPENSSL_strlcpy(include_path,
							include_dir, newlen);
					if (!ossl_ends_with_dirsep(
						    include_path))
						OPENSSL_strlcat(include_path,
								"/", newlen);
					OPENSSL_strlcat(include_path, include,
							newlen);
					OPENSSL_free(include);
				} else {
...
#ifndef OPENSSL_NO_POSIX_IO
				next = process_include(include_path, &dirctx,
						       &dirpath);
...
static int str_copy(CONF *conf, char *section, char **pto, char *from)
{
	...
	for (;;) {
		if (IS_QUOTE(conf, *from)) {
			...
			break;
		else if (*from == '$' && (!conf->flag_dollarid ||
					  from[1] == '{' || from[1] == '(')) {
			size_t newsize;
			    ...
			    char **dirpath)
{
	...
	if (S_ISDIR(st.st_mode)) {
		if (*dirctx != NULL) {
			ERR_raise_data(ERR_LIB_CONF,
				       CONF_R_RECURSIVE_DIRECTORY_INCLUDE, "%s",
				       include);
			return NULL;
		}
...
static BIO *get_next_file(const char *path, OPENSSL_DIR_CTX **dirctx)
{
	...
	while ((filename = OPENSSL_DIR_read(dirctx, path)) != NULL) {
		...
		if ((namelen > 5 && OPENSSL_strcasecmp(filename + namelen - 5,
						       ".conf") == 0) ||
		    (namelen > 4 &&
...
static int is_keytype(const CONF *conf, char c, unsigned short type)
{
	const unsigned short *keytypes =
		(const unsigned short *)conf->meth_data;
