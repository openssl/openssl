...
static const ML_KEM_VINFO vinfo_map[3] = {
	{ "ML-KEM-512", PRVKEY_BYTES(512), sizeof(struct prvkey_512_alloc),
	  PUBKEY_BYTES(512), sizeof(struct pubkey_512_alloc), CTEXT_BYTES(512),
	  VECTOR_BYTES(512), U_VECTOR_BYTES(512), EVP_PKEY_ML_KEM_512,
	  ML_KEM_512_BITS, ML_KEM_512_RANK, ML_KEM_512_DU, ML_KEM_512_DV,
	  ML_KEM_512_SECBITS, ML_KEM_512_SECURITY_CATEGORY },
	{ "ML-KEM-768", PRVKEY_BYTES(768), sizeof(struct prvkey_768_alloc),
	  PUBKEY_BYTES(768), sizeof(struct pubkey_768_alloc), CTEXT_BYTES(768),
	  VECTOR_BYTES(768), U_VECTOR_BYTES(768), EVP_PKEY_ML_KEM_768,
	  ML_KEM_768_BITS, ML_KEM_768_RANK, ML_KEM_768_DU, ML_KEM_768_DV,
	  ML_KEM_768_SECBITS, ML_KEM_768_SECURITY_CATEGORY },
	{ "ML-KEM-1024", PRVKEY_BYTES(1024), sizeof(struct prvkey_1024_alloc),
	  PUBKEY_BYTES(1024), sizeof(struct pubkey_1024_alloc),
	  CTEXT_BYTES(1024), VECTOR_BYTES(1024), U_VECTOR_BYTES(1024),
	  EVP_PKEY_ML_KEM_1024, ML_KEM_1024_BITS, ML_KEM_1024_RANK,
	  ML_KEM_1024_DU, ML_KEM_1024_DV, ML_KEM_1024_SECBITS,
...
static const uint16_t kInverseNTTRoots[128] = {
	1,    1175, 2444, 394,	1219, 2300, 1455, 2117, 1607, 2443, 554,  1179,
	2186, 2303, 2926, 2237, 525,  735,  863,  2768, 1230, 2572, 556,  3010,
	2266, 1684, 1239, 780,	2954, 109,  1292, 1031, 1745, 2688, 3061, 992,
	2596, 941,  892,  1021, 2390, 642,  1868, 2377, 1482, 1540, 540,  1678,
	1626, 279,  314,  1173, 2573, 3096, 48,	  667,	1920, 2229, 1041, 2606,
	1692, 680,  2746, 568,	3312, 2419, 2102, 219,	855,  2681, 1848, 712,
	682,  927,  1795, 461,	1891, 2877, 2522, 1894, 1010, 1414, 2009, 3296,
	464,  2697, 816,  1352, 2679, 1274, 1052, 1025, 2132, 1573, 76,	  2998,
	3040, 2508, 1355, 450,	936,  447,  2794, 1235, 1903, 1996, 1089, 3273,
	283,  1853, 1990, 882,	3033, 1583, 2760, 69,	543,  2532, 3136, 1410,
 ...
 */
static __owur int single_keccak(uint8_t *out, size_t outlen, const uint8_t *in,
				size_t inlen, EVP_MD_CTX *mdctx)
{
 ...
 */
static __owur int prf(uint8_t *out, size_t len,
		      const uint8_t in[ML_KEM_RANDOM_BYTES + 1],
		      EVP_MD_CTX *mdctx, const ML_KEM_KEY *key)
{
 ...
 */
static __owur int hash_h(uint8_t out[ML_KEM_PKHASH_BYTES], const uint8_t *in,
			 size_t len, EVP_MD_CTX *mdctx, const ML_KEM_KEY *key)
{
...
static int hash_h_pubkey(uint8_t pkhash[ML_KEM_PKHASH_BYTES], EVP_MD_CTX *mdctx,
			 ML_KEM_KEY *key)
{
 ...
 */
static __owur int hash_g(uint8_t out[ML_KEM_SEED_BYTES], const uint8_t *in,
			 size_t len, EVP_MD_CTX *mdctx, const ML_KEM_KEY *key)
{
...
static __owur int kdf(uint8_t out[ML_KEM_SHARED_SECRET_BYTES],
		      ...
		      const uint8_t *ctext, size_t len, EVP_MD_CTX *mdctx,
...
static __owur int scalar_decode_12(scalar *out,
				   const uint8_t in[3 * DEGREE / 2])
{
	int i;
	uint16_t *c = out->c;
 ...
 */
static void scalar_decode_decompress_add(scalar *out,
					 const uint8_t in[DEGREE / 8])
{
 ...
 */
static void vector_decode_decompress_ntt(scalar *out, const uint8_t *in,
					 int bits, int rank)
{
	int stride = bits * DEGREE / 8;
...
static __owur int vector_decode_12(scalar *out,
				   const uint8_t in[3 * DEGREE / 2], int rank)
{
 ...
 */
static void matrix_mult_intt(scalar *out, const scalar *m, const scalar *a,
			     int rank)
{
	const scalar *ar;
	int i, j;
...
static void matrix_mult_transpose_add(scalar *out, const scalar *m,
 ...
 */
static void decrypt_cpa(uint8_t out[ML_KEM_SHARED_SECRET_BYTES],
			 ...
			 EVP_MD_CTX *mdctx, uint8_t *pubenc, ML_KEM_KEY *key)
{
	...
	if (!matrix_expand(mdctx, key) ||
	    !gencbd_vector_ntt(key->s, cbd_1, &counter, sigma, rank, mdctx,
			       key) ||
	    !gencbd_vector_ntt(key->t, cbd_1, &counter, sigma, rank, mdctx,
			       key))
		goto end;
	...
	if (pubenc == NULL) {
		...
		if (!hash_h(key->pkhash, pubenc, vinfo->pubkey_bytes, mdctx,
			    key))
			goto end;
	}
...
static int encap(uint8_t *ctext, uint8_t secret[ML_KEM_SHARED_SECRET_BYTES],
		 const uint8_t entropy[ML_KEM_RANDOM_BYTES], scalar *tmp,
		 EVP_MD_CTX *mdctx, const ML_KEM_KEY *key)
{
	...
	if (ret)
		...
		ERR_raise_data(
			ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR,
		 ...
		 EVP_MD_CTX *mdctx, const ML_KEM_KEY *key)
{
	...
	memcpy(decrypted + ML_KEM_SHARED_SECRET_BYTES, pkhash,
	       ML_KEM_PKHASH_BYTES);
	...
	mask = constant_time_eq_int_8(0, CRYPTO_memcmp(ctext, tmp_ctext,
						       vinfo->ctext_bytes));
 ...
 */
static __owur int add_storage(scalar *pub, scalar *priv, int private,
			      ML_KEM_KEY *key)
{
	int rank = key->vinfo->rank;
...
void ossl_ml_kem_key_reset(ML_KEM_KEY *key)
{
	...
	if (ossl_ml_kem_have_dkenc(key))
		OPENSSL_secure_clear_free(key->encoded_dk,
				...
				int evp_type)
{
	...
	key->d = key->z = key->rho = key->pkhash = key->encoded_dk =
		key->seedbuf = NULL;
	key->s = key->m = key->t = NULL;

	if (key->shake128_md != NULL && key->shake256_md != NULL &&
...
int ossl_ml_kem_encode_seed(uint8_t *out, size_t len, const ML_KEM_KEY *key)
{
				 ...
				 ML_KEM_KEY *key)
{
	if (key == NULL || ossl_ml_kem_have_pubkey(key) ||
	    ossl_ml_kem_have_seed(key) || seedlen != ML_KEM_SEED_BYTES)
		return NULL;
 ...
 * This is the deterministic version with randomness supplied externally.
 */
int ossl_ml_kem_encap_seed(uint8_t *ctext, size_t clen, uint8_t *shared_secret,
			   size_t slen, const uint8_t *entropy, size_t elen,
			   const ML_KEM_KEY *key)
{
	const ML_KEM_VINFO *vinfo;
	EVP_MD_CTX *mdctx;
...
int ossl_ml_kem_encap_rand(uint8_t *ctext, size_t clen, uint8_t *shared_secret,
			   size_t slen, const ML_KEM_KEY *key)
{
	...
	return ossl_ml_kem_encap_seed(ctext, clen, shared_secret, slen, r,
				      sizeof(r), key);
}

int ossl_ml_kem_decap(uint8_t *shared_secret, size_t slen, const uint8_t *ctext,
...
int ossl_ml_kem_pubkey_cmp(const ML_KEM_KEY *key1, const ML_KEM_KEY *key2)
{
	...
	if (ossl_ml_kem_have_pubkey(key1) && ossl_ml_kem_have_pubkey(key2))
		return memcmp(key1->pkhash, key2->pkhash,
