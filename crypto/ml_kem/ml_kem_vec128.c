...
/*
 * multiply_512_montgomery: a specialization of multiply_montgomery for
 * Montgomery multiplication by the constant 512.  The unreduced step accepts
 * the same input range as multiply_512_montgomery_unreduced (|b[i]| < 2q);
 * the added vec_q bias and reduce_once bring the result into [0, q).
 *
 * Pre:  Every lane of b satisfies |b[i]| < 2q.
 ...
 * = R_M mod q; unused padding).  Root consumption starts at index
 ...
 *   result_even ≡ (l0*r0 + l1*r1 * root_std) * 169  (mod q)
 *   result_odd  ≡ (l0*r1 + l1*r0) * 169             (mod q)
 *
 * Each Montgomery multiplication contributes one factor of R_M^{-1}.
 * Both outputs are therefore scaled by R_M^{-1} modulo q.
 * A subsequent demontgomerize_vec128 call converts them back to
 ...
 * leaving the raw output of multiply_Fq2_montgomery_unreduced() in out.
 * Each coefficient represents the logical product scaled by R_M^{-1} mod q.
 * The stored unreduced representatives satisfy the bounds documented below
 * (e.g. [-6007, 6007]) and are not necessarily in (-q, q).
 *
 * Pre:  Every coefficient of lhs and rhs is in [0, q)  (standard NTT-domain form).
 ...
 * Post: Every coefficient of out satisfies |out[i]| <= rank * 6007
 ...
 *
 * Pre:  Every coefficient s->c[i] satisfies |s->c[i]| <= 4*6007 < 8*q and represents a
    ...
    int rank)
{
    ...
    for (i = rank; i-- > 0; ++out) {
        ...
        /* Finish lazy reduction by demontgomerizing the accumulated sum. */
