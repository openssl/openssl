			  ...
			  PKCS12_create_cb *cb, void *cbarg)
{
	...
	if (pkey) {
		bag = PKCS12_add_key_ex(&bags, pkey, keytype, iter, nid_key,
					pass, ctx, propq);
			 ...
			 OSSL_LIB_CTX *ctx, const char *propq)
{
	return PKCS12_create_ex2(pass, name, pkey, cert, ca, nid_key, nid_cert,
				 iter, mac_iter, keytype, ctx, propq, NULL,
				 NULL);
}
		       ...
		       OSSL_LIB_CTX *ctx, const char *propq)
{
	...
	if (nid_safe == -1)
		...
		p7 = PKCS12_pack_p7encdata_ex(nid_safe, pass, -1, NULL, 0, iter,
					      bags, ctx, propq);
	if (p7 == NULL)
		goto err;
...
int PKCS12_add_safe(STACK_OF(PKCS7) **psafes, STACK_OF(PKCS12_SAFEBAG) *bags,
		    int nid_safe, int iter, const char *pass)
{
	return PKCS12_add_safe_ex(psafes, bags, nid_safe, iter, pass, NULL,
				  NULL);
}
