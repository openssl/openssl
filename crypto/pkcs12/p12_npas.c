	...
	for (i = 0; i < sk_PKCS7_num(asafes); i++) {
		...
		if (bagnid == NID_pkcs7_data)
			...
			p7new = PKCS12_pack_p7encdata_ex(
				pbe_nid, newpass, -1, NULL, pbe_saltlen,
			...
			const char *propq)
{
	...
	for (i = 0; i < sk_PKCS12_SAFEBAG_num(bags); i++) {
		if (!newpass_bag(sk_PKCS12_SAFEBAG_value(bags, i), oldpass,
				 newpass, libctx, propq))
			return 0;
	}
	return 1;
}
		   ...
		   int *cipherid)
{
	...
	case NID_pbes2:
		...
		*cipherid = encnid;
		break;
	default:
		pbe = ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(PBEPARAM),
						alg->parameter);
		if (pbe == NULL)
			goto done;
