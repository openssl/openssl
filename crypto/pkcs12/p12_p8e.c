			   ...
			   OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if (pbe_nid == -1) {
		...
		if (EVP_PBE_find(EVP_PBE_TYPE_PRF, pbe_nid, NULL, NULL, 0)) {
			...
			pbe = PKCS5_pbe2_set_iv_ex(cipher, iter, salt, saltlen,
						   NULL, pbe_nid, libctx);
		} else {
			ERR_pop_to_mark();
			pbe = PKCS5_pbe_set_ex(pbe_nid, iter, salt, saltlen,
					       libctx);
		}
	}
	if (pbe == NULL) {
...
{
	return PKCS8_encrypt_ex(pbe_nid, cipher, pass, passlen, salt, saltlen,
				iter, p8inf, NULL, NULL);
}
			    ...
			    OSSL_LIB_CTX *ctx, const char *propq)
{
	...
	enckey = PKCS12_item_i2d_encrypt_ex(pbe,
					    ASN1_ITEM_rptr(PKCS8_PRIV_KEY_INFO),
					    pass, passlen, p8inf, 1, ctx,
					    propq);
	if (!enckey) {
