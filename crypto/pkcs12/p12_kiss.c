		 ...
		 STACK_OF(X509) **ca)
{
	...
	while ((x = sk_X509_shift(ocerts)) != NULL) {
		if (pkey != NULL && *pkey != NULL && cert != NULL &&
		    *cert == NULL) {
			int match;
		...
		if (ca != NULL) {
			if (!ossl_x509_add_cert_new(ca, x,
		     ...
		     OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	case NID_pkcs8ShroudedKeyBag:
		if (pkey == NULL || *pkey != NULL)
			return 1;
		if ((p8 = PKCS12_decrypt_skey_ex(bag, pass, passlen, libctx,
						 propq)) == NULL)
			return 0;
	...
	case NID_certBag:
		if (ocerts == NULL ||
			...
			return 1;
		if ((x509 = PKCS12_SAFEBAG_get1_cert_ex(bag, libctx, propq)) ==
		    NULL)
			return 0;
	...
	case NID_safeContentsBag:
		return parse_bags(PKCS12_SAFEBAG_get0_safes(bag), pass, passlen,
				  pkey, ocerts, libctx, propq);
