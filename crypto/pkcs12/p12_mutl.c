			      ...
			      const X509_ALGOR *macalg, unsigned char *key)
{
	...
	kdf_md = EVP_MD_fetch(ctx, OBJ_nid2sn(ossl_hmac2mdnid(kdf_hmac_nid)),
			      propq);
	if (kdf_md == NULL) {
	...
	if (PKCS5_PBKDF2_HMAC(pass, passlen, pbkdf2_salt->data,
			      pbkdf2_salt->length,
			      ASN1_INTEGER_get(pbkdf2_param->iter), kdf_md,
			      keylen, key) <= 0) {
...
static int pkcs12_gen_mac(
	...
	int (*pkcs12_key_gen)(const char *pass, int passlen,
			      unsigned char *salt, int slen, int id, int iter,
			      int n, unsigned char *out, const EVP_MD *md_type))
{
	int ret = 0;
	const EVP_MD *md;
	EVP_MD *md_fetch;
	...
	if (pbmac1_md_nid != NID_undef && pkcs12_key_gen == NULL) {
		...
		if (pkcs12_key_gen != NULL) {
			int res = (*pkcs12_key_gen)(pass, passlen, salt,
						    saltlen, PKCS12_MAC_ID,
			...
			if (!PKCS12_key_gen_utf8_ex(
				    pass, passlen, salt, saltlen, PKCS12_MAC_ID,
				    iter, keylen, key, md,
...
{
	return pkcs12_gen_mac(p12, pass, passlen, mac, maclen, NID_undef,
			      NID_undef, NULL);
}
...
int PKCS12_verify_mac(PKCS12 *p12, const char *pass, int passlen)
{
	...
	if (OBJ_obj2nid(macoid) == NID_pbmac1) {
		...
		X509_ALGOR_get0(&hmac_oid, NULL, NULL,
				param->messageAuthScheme);
		md_nid = ossl_hmac2mdnid(OBJ_obj2nid(hmac_oid));

		if (!pkcs12_gen_mac(p12, pass, passlen, mac, &maclen, md_nid,
				    NID_undef, NULL)) {
			ERR_raise(ERR_LIB_PKCS12,
			    ...
			    int saltlen, int nid)
{
	...
	if (salt == NULL) {
		if (RAND_bytes_ex(p12->authsafes->ctx.libctx,
				  p12->mac->salt->data, (size_t)saltlen,
				  0) <= 0)
			return 0;
	} else {
			     ...
			     const EVP_MD *md_type, const char *prf_md_name)
{
	...
	if (!X509_ALGOR_set0(hmac_alg, OBJ_nid2obj(hmac_nid), V_ASN1_NULL,
			     NULL)) {
		ERR_raise(ERR_LIB_PKCS12, PKCS12_R_MAC_SETUP_ERROR);
		goto err;
	}
