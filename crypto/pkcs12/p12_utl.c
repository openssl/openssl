				...
				unsigned char **uni, int *unilen)
{
	...
	for (ulen = 0, i = 0; i < asclen; i += j) {
		j = UTF8_getc((const unsigned char *)asc + i, asclen - i,
			      &utf32chr);
...
PKCS12 *d2i_PKCS12_bio(BIO *bp, PKCS12 **p12)
{
	...
	return ASN1_item_d2i_bio_ex(ASN1_ITEM_rptr(PKCS12), bp, p12, libctx,
				    propq);
}
...
PKCS12 *d2i_PKCS12_fp(FILE *fp, PKCS12 **p12)
{
	...
	return ASN1_item_d2i_fp_ex(ASN1_ITEM_rptr(PKCS12), fp, p12, libctx,
				   propq);
}
#endif
