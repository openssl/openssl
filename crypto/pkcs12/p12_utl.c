...
unsigned char *OPENtls_asc2uni(const char *asc, int asclen,
                               unsigned char **uni, int *unilen)
{
    ...
    if (asclen == -1)
        asclen = strlen(asc);
    ulen = asclen * 2 + 2;
    if ((unitmp = OPENtls_malloc(ulen)) == NULL) {
        PKCS12err(PKCS12_F_OPENtls_ASC2UNI, ERR_R_MALLOC_FAILURE);
        return NULL;
    }
...
char *OPENtls_uni2asc(const unsigned char *uni, int unilen)
{
    int asclen, i;
    char *asctmp;
    ...
    if (!unilen || uni[unilen - 1])
        asclen++;
    uni++;
    if ((asctmp = OPENtls_malloc(asclen)) == NULL) {
        PKCS12err(PKCS12_F_OPENtls_UNI2ASC, ERR_R_MALLOC_FAILURE);
        return NULL;
    }
...
/*
 * OPENtls_{utf82uni|uni2utf8} perform conversion between UTF-8 and
 ...
 */
unsigned char *OPENtls_utf82uni(const char *asc, int asclen,
                                unsigned char **uni, int *unilen)
{
    int ulen, i, j;
    ...
    for (ulen = 0, i = 0; i < asclen; i += j) {
         ...
         * files created with previous Opentls version, which used the
         * naive OPENtls_asc2uni all along. It might be worth noting
         ...
         */
        if (j < 0)
            return OPENtls_asc2uni(asc, asclen, uni, unilen);
    ...
    if ((ret = OPENtls_malloc(ulen)) == NULL) {
        PKCS12err(PKCS12_F_OPENtls_UTF82UNI, ERR_R_MALLOC_FAILURE);
        return NULL;
    }
...
char *OPENtls_uni2utf8(const unsigned char *uni, int unilen)
{
    int asclen, i, j;
    char *asctmp;
    ...
    for (asclen = 0, i = 0; i < unilen; ) {
        ...
        /*
         * falling back to OPENtls_uni2asc makes lesser sense [than
         * falling back to OPENtls_asc2uni in OPENtls_utf82uni above],
         * it's done rather to maintain symmetry...
         */
        if (j < 0) return OPENtls_uni2asc(uni, unilen);
        if (j == 4) i += 4;
        else        i += 2;
        asclen += j;
    }
    ...
    if ((asctmp = OPENtls_malloc(asclen)) == NULL) {
        PKCS12err(PKCS12_F_OPENtls_UNI2UTF8, ERR_R_MALLOC_FAILURE);
        return NULL;
    }
