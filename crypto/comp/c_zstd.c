...
static ZSTD_customMem zstd_mem_funcs = { zstd_alloc, zstd_free, NULL };
#endif
...
typedef size_t (*compressStream2_ft)(ZSTD_CCtx *, ZSTD_outBuffer *,
...
typedef size_t (*decompressStream_ft)(ZSTD_DStream *, ZSTD_outBuffer *,
				      ZSTD_inBuffer *);
...
static ossl_ssize_t zstd_stateful_compress_block(COMP_CTX *ctx,
						 unsigned char *out,
						 ...
						 size_t ilen)
{
     ...
     */
	do {
		ret = ZSTD_compressStream2(state->compressor, &outbuf, &inbuf,
					   ZSTD_e_continue);
		if (ZSTD_isError(ret))
			return -1;
...
static ossl_ssize_t zstd_stateful_expand_block(COMP_CTX *ctx,
					       ...
					       unsigned char *in, size_t ilen)
{
	...
	do {
		ret = ZSTD_decompressStream(state->decompressor, &outbuf,
					    &inbuf);
		if (ZSTD_isError(ret))
			return -1;
...
static COMP_METHOD zstd_stateful_method = { NID_zstd,
					    LN_zstd,
					    zstd_stateful_init,
...
static ossl_ssize_t zstd_oneshot_compress_block(COMP_CTX *ctx,
						unsigned char *out, size_t olen,
						unsigned char *in, size_t ilen)
{
	size_t out_size;
...
static COMP_METHOD zstd_oneshot_method = { NID_zstd,
					   LN_zstd,
					   zstd_oneshot_init,
					   zstd_oneshot_finish,
...
#define LIBZSTD "zstd"
	...
	if (zstd_dso != NULL) {
		p_createCStream = (createCStream_ft)DSO_bind_func(
			zstd_dso, "ZSTD_createCStream");
		p_initCStream = (initCStream_ft)DSO_bind_func(
			zstd_dso, "ZSTD_initCStream");
		p_freeCStream = (freeCStream_ft)DSO_bind_func(
			zstd_dso, "ZSTD_freeCStream");
		p_compressStream2 = (compressStream2_ft)DSO_bind_func(
			zstd_dso, "ZSTD_compressStream2");
		p_flushStream = (flushStream_ft)DSO_bind_func(
			zstd_dso, "ZSTD_flushStream");
		p_endStream =
			(endStream_ft)DSO_bind_func(zstd_dso, "ZSTD_endStream");
		p_compress =
			(compress_ft)DSO_bind_func(zstd_dso, "ZSTD_compress");
		p_createDStream = (createDStream_ft)DSO_bind_func(
			zstd_dso, "ZSTD_createDStream");
		p_initDStream = (initDStream_ft)DSO_bind_func(
			zstd_dso, "ZSTD_initDStream");
		p_freeDStream = (freeDStream_ft)DSO_bind_func(
			zstd_dso, "ZSTD_freeDStream");
		p_decompressStream = (decompressStream_ft)DSO_bind_func(
			zstd_dso, "ZSTD_decompressStream");
		p_decompress = (decompress_ft)DSO_bind_func(zstd_dso,
							    "ZSTD_decompress");
		p_isError = (isError_ft)DSO_bind_func(zstd_dso, "ZSTD_isError");
		p_getErrorName = (getErrorName_ft)DSO_bind_func(
			zstd_dso, "ZSTD_getErrorName");
		p_DStreamInSize = (DStreamInSize_ft)DSO_bind_func(
			zstd_dso, "ZSTD_DStreamInSize");
		p_CStreamInSize = (CStreamInSize_ft)DSO_bind_func(
			zstd_dso, "ZSTD_CStreamInSize");
	}

	if (p_createCStream == NULL || p_initCStream == NULL ||
	    p_freeCStream == NULL || p_compressStream2 == NULL ||
	    p_flushStream == NULL || p_endStream == NULL ||
	    p_compress == NULL || p_createDStream == NULL ||
	    p_initDStream == NULL || p_freeDStream == NULL ||
	    p_decompressStream == NULL || p_decompress == NULL ||
	    p_isError == NULL || p_getErrorName == NULL ||
	    p_DStreamInSize == NULL || p_CStreamInSize == NULL) {
...
static const BIO_METHOD bio_meth_zstd = {
	...
	bwrite_conv, bio_zstd_write,
	/* TODO: Convert to new style read function */
	bread_conv, bio_zstd_read, NULL, /* bio_zstd_puts, */
	NULL, /* bio_zstd_gets, */
	bio_zstd_ctrl, bio_zstd_new, bio_zstd_free, bio_zstd_callback_ctrl
};
#endif
...
static int bio_zstd_read(BIO *b, char *out, int outl)
{
	...
	if (ctx->decompress.buffer == NULL) {
		ctx->decompress.buffer =
			OPENSSL_malloc(ctx->decompress.bufsize);
		if (ctx->decompress.buffer == NULL) {
			ERR_raise(ERR_LIB_COMP, ERR_R_MALLOC_FAILURE);
			return 0;
		}
	...
	for (;;) {
		...
		do {
			zret = ZSTD_decompressStream(ctx->decompress.state,
						     &outBuf,
						     &ctx->decompress.inbuf);
			if (ZSTD_isError(zret)) {
				ERR_raise(ERR_LIB_COMP,
		...
		} while (ctx->decompress.inbuf.pos <
			 ctx->decompress.inbuf.size);

		/*
         * No data in input buffer try to read some in, if an error then
         * return the total data read.
         */
		ret = BIO_read(next, ctx->decompress.buffer,
			       (int)ctx->decompress.bufsize);
		if (ret <= 0) {
...
static int bio_zstd_write(BIO *b, const char *in, int inl)
{
	...
	if (ctx->compress.outbuf.dst == NULL) {
		ctx->compress.outbuf.dst =
			OPENSSL_malloc(ctx->compress.bufsize);
		if (ctx->compress.outbuf.dst == NULL) {
			ERR_raise(ERR_LIB_COMP, ERR_R_MALLOC_FAILURE);
			return 0;
		}
	...
	for (;;) {
		...
		while (ctx->compress.write_pos < ctx->compress.outbuf.pos) {
			ret = BIO_write(
				next,
				(unsigned char *)ctx->compress.outbuf.dst +
					ctx->compress.write_pos,
				(int)(ctx->compress.outbuf.pos -
				      ctx->compress.write_pos));
			if (ret <= 0) {
				BIO_copy_next_retry(b);
		...
		ctx->compress.write_pos = 0;
		/* Compress some more */
		zret = ZSTD_compressStream2(ctx->compress.state,
					    &ctx->compress.outbuf, &inBuf,
					    ZSTD_e_end);
		if (ZSTD_isError(zret)) {
...
static int bio_zstd_flush(BIO *b)
{
	...
	for (;;) {
		...
		ctx->compress.write_pos = 0;
		/* Compress some more */
		zret = ZSTD_flushStream(ctx->compress.state,
					&ctx->compress.outbuf);
		if (ZSTD_isError(zret)) {
...
static long bio_zstd_ctrl(BIO *b, int cmd, long num, void *ptr)
{
	...
	case BIO_C_SET_BUFF_SIZE:
		...
		if (ibs > 0 && ibs != ctx->decompress.bufsize) {
			if (ctx->decompress.buffer != NULL) {
				tmp = OPENSSL_realloc(ctx->decompress.buffer,
						      ibs);
				if (tmp == NULL)
					return 0;
				if (ctx->decompress.inbuf.src ==
				    ctx->decompress.buffer)
		...
		if (obs > 0 && obs != ctx->compress.bufsize) {
			if (ctx->compress.outbuf.dst != NULL) {
				tmp = OPENSSL_realloc(ctx->compress.outbuf.dst,
						      obs);
				if (tmp == NULL)
					return 0;
