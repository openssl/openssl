			       ...
			       unsigned char *in, size_t ilen)
{
     ...
     * output buffer space
     */
	done = BrotliEncoderCompressStream(state->encoder,
					   BROTLI_OPERATION_FLUSH, &in_avail,
...
static const BIO_METHOD bio_meth_brotli = {
	...
	bio_brotli_ctrl, bio_brotli_new, bio_brotli_free,
	...
	for (;;) {
		...
		while (ctx->decode.avail_in > 0 ||
			...
			bret = BrotliDecoderDecompressStream(
				...
				(const uint8_t **)&ctx->decode.next_in,
				&ctx->decode.avail_out, &ctx->decode.next_out,
				NULL);
...
static int bio_brotli_write(BIO *b, const char *in, int inl)
{
	...
	for (;;) {
		...
		while (ctx->encode.count > 0) {
			ret = BIO_write(next, ctx->encode.ptr,
					(int)ctx->encode.count);
			if (ret <= 0) {
				/* Total data written */
		...
		brret = BrotliEncoderCompressStream(
			ctx->encode.state, BROTLI_OPERATION_FLUSH,
			&ctx->encode.avail_in,
			(const uint8_t **)&ctx->encode.next_in,
...
static int bio_brotli_flush(BIO *b)
{
	...
	if (ctx->encode.buf == NULL ||
	    (ctx->encode.done && ctx->encode.count == 0))
		return 1;
	...
	for (;;) {
		...
		/* Compress some more */
		brret = BrotliEncoderCompressStream(
			ctx->encode.state, BROTLI_OPERATION_FINISH,
			&ctx->encode.avail_in,
			(const uint8_t **)&ctx->encode.next_in,
			&ctx->encode.avail_out, &ctx->encode.next_out, NULL);
		if (brret != BROTLI_TRUE) {
