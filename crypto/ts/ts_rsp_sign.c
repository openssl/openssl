...
TS_RESP *TS_RESP_create_response(TS_RESP_CTX *ctx, BIO *req_bio)
{
	...
	if (!result) {
		...
		if (ctx->response != NULL) {
			if (TS_RESP_CTX_set_status_info_cond(
				    ctx, TS_STATUS_REJECTION,
...
static int ts_RESP_sign(TS_RESP_CTX *ctx)
{
	...
	if (ctx->signer_md == NULL)
		...
		signer_md = EVP_MD_fetch(ctx->libctx,
	...
	if (ctx->ess_cert_id_digest == NULL ||
		...
		sc2 = OSSL_ESS_signing_cert_v2_new_init(
			ctx->ess_cert_id_digest, ctx->signer_cert, certs, 0);
		if (sc2 == NULL)
			goto err;
