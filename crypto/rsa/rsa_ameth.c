	...
	if (priv) {
		...
		for (i = 0; i < sk_RSA_PRIME_INFO_num(x->prime_infos); i++) {
			...
			for (j = 0; j < 3; j++) {
				...
				switch (j) {
				case 0:
					if (BIO_printf(bp, "prime%d:", i + 3) <=
					    0)
						goto err;
					bn = pinfo->r;
					break;
				case 1:
					if (BIO_printf(bp, "exponent%d:",
						       i + 3) <= 0)
						goto err;
					bn = pinfo->d;
					break;
				case 2:
					if (BIO_printf(bp, "coefficient%d:",
						       i + 3) <= 0)
						goto err;
					bn = pinfo->t;
					break;
...
static int rsa_pkey_ctrl(EVP_PKEY *pkey, int op, long arg1, void *arg2)
{
	...
	case ASN1_PKEY_CTRL_DEFAULT_MD_NID:
		if (pkey->pkey.rsa->pss != NULL) {
			if (!ossl_rsa_pss_get_param(pkey->pkey.rsa->pss, &md,
						    &mgf1md, &min_saltlen)) {
	...
	if (pad_mode == RSA_PKCS1_PSS_PADDING) {
		...
		if (evp_pkey_ctx_is_legacy(pkctx)) {
			...
			if (alg2 != NULL) {
				...
				if (!X509_ALGOR_set0(
					    alg2, OBJ_nid2obj(EVP_PKEY_RSA_PSS),
					    V_ASN1_SEQUENCE, os2)) {
					ASN1_STRING_free(os1);
			    ...
			    const ASN1_STRING *sig)
{
     ...
     */
	if ((mdnid == NID_sha256 || mdnid == NID_sha384 ||
	     mdnid == NID_sha512) &&
			       ...
			       int rsa_type)
{
	...
	case RSA_FLAG_TYPE_RSASSAPSS:
		...
		if (!ossl_rsa_pss_params_30_is_unrestricted(&rsa_pss_params)) {
			...
			int mgf1mdnid = ossl_rsa_pss_params_30_maskgenhashalg(
				&rsa_pss_params);
			int saltlen =
			...
			if ((rsa->pss = ossl_rsa_pss_params_create(
				     md, mgf1md, saltlen)) == NULL)
				goto err;
		}
		break;
	default:
