 ...
 * arithmetic directly.
 *
 * TODO(FIXNUM): the default ossl_fn_mod_exp implementation is a windowed
 * engine that is not constant-time with respect to the exponent.  A
 * constant-time engine is in separate development and is designed to drop
 * in under the same function name, so no change here will be needed when
 * it lands.
 *
...
static int rsa_ossl_mod_exp(BIGNUM *r0, const BIGNUM *I, RSA *rsa, BN_CTX *ctx)
{
    ...
    OSSL_FN *fn_r0 = NULL;
    const OSSL_FN *fn_I = NULL, *fn_n = NULL;
    size_t fn_size, nl;
    int fn_bits, ret = 0;
    ...
    fn_n = bn_get_ossl_fn(rsa->n);
    if (fn_I == NULL || !ossl_assert(fn_n != NULL))
        return 0;
    nl = ossl_fn_get_dsize((OSSL_FN *)fn_n);
     ...
     * budget below is dominated by the smooth path, whose two prime-width
     * Montgomery exponentiations accumulate more arena than the generic
     * path; it is validated against OSSL_FN_CTX_peak_usage() during
     * development: at 2048-bit keys the observed peak is 3 frames,
     * 42 numbers, ~770 limbs, so the budgets 8 frames, 48 numbers,
