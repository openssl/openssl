				   ...
				   unsigned char *to, RSA *rsa, int padding)
{
	...
	case RSA_PKCS1_OAEP_PADDING:
		i = ossl_rsa_padding_add_PKCS1_OAEP_mgf1_ex(
			rsa->libctx, buf, num, from, flen, NULL, 0, NULL, NULL);
		break;
	case RSA_NO_PADDING:
	...
	if (padding == RSA_NO_PADDING) {
		...
		if (nminus1 == NULL || BN_copy(nminus1, rsa->n) == NULL ||
				    ...
				    unsigned char *to, RSA *rsa, int padding)
{
	...
	case RSA_PKCS1_NO_IMPLICIT_REJECT_PADDING:
		...
		break;
	case RSA_PKCS1_PADDING:
		r = ossl_rsa_padding_check_PKCS1_type_2(rsa->libctx, to, num,
							buf, j, num, kdk);
		break;
	case RSA_PKCS1_OAEP_PADDING:
...
static int rsa_ossl_mod_exp(BIGNUM *r0, const BIGNUM *I, RSA *rsa, BN_CTX *ctx)
{
	...
	if (rsa->flags & RSA_FLAG_CACHE_PRIVATE) {
		...
		for (i = 0; i < ex_primes; i++) {
			...
			if (!BN_MONT_CTX_set_locked(&pinfo->m, rsa->lock,
						    factor, ctx)) {
				BN_free(factor);
				goto err;
			}
	...
	if (smooth) {
		...
		if (/* m1 = I moq q */
		    ...
		    || !BN_mod_exp_mont_consttime_x2(m1, m1, rsa->dmq1, rsa->q,
						     rsa->_method_mod_q, r1, r1,
						     rsa->dmp1, rsa->p,
						     rsa->_method_mod_p, ctx)
		    /* r1 = (r1 - m1) mod p */
		    /*
		    ...
		    /* r1 = r1 * iqmp mod p */
		    || !bn_to_mont_fixed_top(r1, r1, rsa->_method_mod_p, ctx) ||
		    !bn_mul_mont_fixed_top(r1, r1, rsa->iqmp,
					   rsa->_method_mod_p, ctx)
		    /* r0 = r1 * q + m1 */
	...
	if (ex_primes > 0) {
		...
		for (i = 0; i < ex_primes; i++) {
			...
			if (!rsa->meth->bn_mod_exp(m[i], r1, di, pinfo->r, ctx,
						   pinfo->m)) {
				BN_free(cc);
				BN_free(di);
				goto err;
...
tail:
	if (rsa->e && rsa->n) {
		if (rsa->meth->bn_mod_exp == BN_mod_exp_mont) {
			...
			bn_correct_top(r0);
			if (!rsa->meth->bn_mod_exp(vrfy, r0, rsa->e, rsa->n,
						   ctx, rsa->_method_mod_n))
				goto err;
		}
		/*
...
{
	if (rsa->version != RSA_ASN1_VERSION_MULTI) {
		if (s390x_crt(r0, i, rsa->p, rsa->q, rsa->dmp1, rsa->dmq1,
			      rsa->iqmp) == 1)
			return 1;
	}
