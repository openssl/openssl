...
{
	return ossl_rsa_padding_add_PKCS1_OAEP_mgf1_ex(
		NULL, to, tlen, from, flen, param, plen, NULL, NULL);
}
...
{
	return ossl_rsa_padding_add_PKCS1_OAEP_mgf1_ex(
		NULL, to, tlen, from, flen, param, plen, md, mgf1md);
}
				      ...
				      const EVP_MD *mgf1md)
{
     ...
     */
	tlen = constant_time_select_int(constant_time_lt(dblen - mdlen - 1,
							 tlen),
					dblen - mdlen - 1, tlen);
	for (msg_index = 1; msg_index < dblen - mdlen - 1; msg_index <<= 1) {
		...
		for (i = mdlen + 1; i < dblen - msg_index; i++)
			db[i] = constant_time_select_8(mask, db[i + msg_index],
						       db[i]);
	}
	for (i = 0; i < tlen; i++) {
