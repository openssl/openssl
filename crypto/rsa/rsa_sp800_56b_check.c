...
int ossl_rsa_check_prime_factor(BIGNUM *p, BIGNUM *e, int nbits, BN_CTX *ctx)
{
	...
	ret = ret
	      /* (Step 5d) GCD(p-1, e) = 1 */
	      && (BN_copy(p1, p) != NULL) && BN_sub_word(p1, 1) &&
	      BN_gcd(gcd, p1, e, ctx) && BN_is_one(gcd);
...
int ossl_rsa_sp800_56b_check_private(const RSA *rsa)
{
	if (rsa->d == NULL || rsa->n == NULL)
		return 0;
	return BN_cmp(rsa->d, BN_value_one()) >= 0 &&
	       BN_cmp(rsa->d, rsa->n) < 0;
}
				     ...
				     int strength, int nbits)
{
	...
	if (rsa->p == NULL || rsa->q == NULL || rsa->e == NULL ||
	    rsa->d == NULL || rsa->n == NULL) {
