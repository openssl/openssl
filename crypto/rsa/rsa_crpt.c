    ...
    const BIGNUM *q)
{
    ...
    if (d == NULL || p == NULL || q == NULL)
        return NULL;
    /* OSSL_FN views are unsigned; negative inputs have no inverse here */
    if (BN_is_negative(d) || BN_is_negative(p) || BN_is_negative(q))
        return NULL;
    ...
    /*
     * e = d^-1 mod (p-1)(q-1) is less than (p-1)(q-1), which fits in
     * pl + ql limbs.
     */
