...
static int setup_tbuf(RSA_PKEY_CTX *ctx, EVP_PKEY_CTX *pk)
{
	if (ctx->tbuf != NULL)
		return 1;
	if ((ctx->tbuf = OPENSSL_malloc(
		     RSA_size(EVP_PKEY_get0_RSA(pk->pkey)))) == NULL)
		return 0;
	return 1;
}
	...
	if (rctx->md) {
		...
		if (EVP_MD_get_type(rctx->md) == NID_mdc2) {
			...
			if (rctx->pad_mode != RSA_PKCS1_PADDING)
				return -1;
			ret = RSA_sign_ASN1_OCTET_STRING(0, tbs, (int)tbslen,
							 sig, &sltmp, rsa);
			...
			rctx->tbuf[tbslen] =
				RSA_X931_hash_id(EVP_MD_get_type(rctx->md));
			ret = RSA_private_encrypt((int)(tbslen + 1), rctx->tbuf,
						  sig, rsa, RSA_X931_PADDING);
		} else if (rctx->pad_mode == RSA_PKCS1_PADDING) {
			unsigned int sltmp;
			ret = RSA_sign(EVP_MD_get_type(rctx->md), tbs,
				       (unsigned int)tbslen, sig, &sltmp, rsa);
			if (ret <= 0)
				return ret;
			ret = sltmp;
		...
		ret = RSA_private_encrypt((int)tbslen, tbs, sig, rsa,
					  rctx->pad_mode);
	}
	if (ret < 0)
		return ret;
				  ...
				  size_t siglen)
{
	...
	if (rctx->md) {
		if (rctx->pad_mode == RSA_X931_PADDING) {
			...
			size_t sltmp;
			ret = ossl_rsa_verify(EVP_MD_get_type(rctx->md), NULL,
					      0, rout, &sltmp, sig, siglen,
					      rsa);
			if (ret <= 0)
				return 0;
			ret = (int)sltmp;
		...
		ret = RSA_public_decrypt((int)siglen, sig, rout, rsa,
					 rctx->pad_mode);
	}
	if (ret <= 0)
		return ret;
			   ...
			   size_t tbslen)
{
	...
	if (rctx->md) {
		if (rctx->pad_mode == RSA_PKCS1_PADDING)
			return RSA_verify(EVP_MD_get_type(rctx->md), tbs,
					  (unsigned int)tbslen, sig,
		...
		if (!setup_tbuf(rctx, ctx))
			return -1;
		rslen = RSA_public_decrypt((int)siglen, sig, rctx->tbuf, rsa,
					   rctx->pad_mode);
		if (rslen <= 0)
			return 0;
	}
			    ...
			    size_t inlen)
{
	...
	if (rctx->pad_mode == RSA_PKCS1_OAEP_PADDING) {
		...
		if (!RSA_padding_add_PKCS1_OAEP_mgf1(
			    rctx->tbuf, klen, in, (int)inlen, rctx->oaep_label,
			    (int)rctx->oaep_labellen, rctx->md, rctx->mgf1md))
			return -1;
		ret = RSA_public_encrypt(klen, rctx->tbuf, out, rsa,
					 RSA_NO_PADDING);
	} else {
		ret = RSA_public_encrypt((int)inlen, in, out, rsa,
					 rctx->pad_mode);
	}
	if (ret < 0)
		return ret;
			    ...
			    size_t inlen)
{
	...
	if (rctx->pad_mode == RSA_PKCS1_OAEP_PADDING) {
		if (!setup_tbuf(rctx, ctx))
			return -1;
		ret = RSA_private_decrypt((int)inlen, in, rctx->tbuf, rsa,
					  RSA_NO_PADDING);
		if (ret <= 0)
			return ret;
	...
	*outlen =
		constant_time_select_s(constant_time_msb_s(ret), *outlen, ret);
...
bad_pad:
	...
	case EVP_PKEY_CTRL_GET_RSA_MGF1_MD:
		...
		if (type == EVP_PKEY_CTRL_GET_RSA_MGF1_MD) {
			...
			if (rsa_pss_restricted(rctx)) {
				if (EVP_MD_get_type(rctx->mgf1md) ==
				    EVP_MD_get_type(p2))
					return 1;
				ERR_raise(ERR_LIB_RSA,
			     ...
			     const char *value)
{
	...
	if (pkey_ctx_is_pss(ctx)) {
		...
		if (strcmp(type, "rsa_pss_keygen_saltlen") == 0) {
			...
			return EVP_PKEY_CTX_set_rsa_pss_keygen_saltlen(ctx,
								       saltlen);
		}
	}
...
static int rsa_set_pss_param(RSA *rsa, EVP_PKEY_CTX *ctx)
{
	...
	if (rctx->md == NULL && rctx->mgf1md == NULL && rctx->saltlen == -2)
		return 1;
	rsa->pss = ossl_rsa_pss_params_create(
		rctx->md, rctx->mgf1md,
...
static int pkey_rsa_keygen(EVP_PKEY_CTX *ctx, EVP_PKEY *pkey)
{
	...
	if (rctx->pub_exp == NULL) {
		rctx->pub_exp = BN_new();
		if (rctx->pub_exp == NULL ||
		    !BN_set_word(rctx->pub_exp, RSA_F4))
			return 0;
	}
	rsa = RSA_new();
	if (rsa == NULL)
...
static const EVP_PKEY_METHOD rsa_pkey_meth = { EVP_PKEY_RSA,
...
static const EVP_PKEY_METHOD rsa_pss_pkey_meth = { EVP_PKEY_RSA_PSS,
						   EVP_PKEY_FLAG_AUTOARGLEN,
						   pkey_rsa_init,
						   pkey_rsa_copy,
						   pkey_rsa_cleanup,
