				   ...
				   const unsigned char *from, int flen, int num)
{
	...
	for (i = 2; i < num; i++) {
		...
		zero_index = constant_time_select_int(
			~found_zero_byte & equals0, i, zero_index);
	...
	for (msg_index = 1; msg_index < num - RSA_PKCS1_PADDING_SIZE;
		...
		for (i = RSA_PKCS1_PADDING_SIZE; i < num - msg_index; i++)
			em[i] = constant_time_select_8(mask, em[i + msg_index],
						       em[i]);
	}
	for (i = 0; i < tlen; i++) {
		mask = good & constant_time_lt(i, mlen);
		to[i] = constant_time_select_8(
			mask, em[i + RSA_PKCS1_PADDING_SIZE], to[i]);
	}
...
#define MAX_LEN_GEN_TRIES 128
	...
	unsigned char
		candidate_lengths[MAX_LEN_GEN_TRIES * sizeof(len_candidate)];
	uint16_t len_mask;
					    ...
					    int alt_version)
{
	...
	good &= constant_time_is_zero_8(
		from[flen - SSL_MAX_MASTER_KEY_LENGTH - 1]);
