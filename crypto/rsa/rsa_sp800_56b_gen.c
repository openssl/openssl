...
/*
 * FIPS 186-4 Section B.3.6 "Generation of Probable Primes with Conditions
 *    Based on Auxiliary Probable Primes"
 * Notes:
 *     rsa is used to store p, q.
                                  ...
                                  const BIGNUM *e, BN_CTX *ctx, BN_GENCB *cb)
{
    ...
    if (!rsa_strength_check_prng(nbits))
        goto err;
#endif
    ...
    for (;;) {
        ...
        for(;;) {
            ...
            if (ok == 0)
                continue;
            goto end; /* successfully finished */
        }
    }
end:;
    ret = 1;
...
#if 0
/* TODO - How do we implement strength for fips??  - it should be seperated
 * from the RNG
 */

/* Check PRNG has sufficient security level to handle an RSA operation */
int rsa_strength_check_prng(int bits)
{
...
/* SP800-56Br1 6.3.1.1 (Steos 1a-1b) */
int rsa_sp800_56b_validate_strength(int nbits, int strength)
{
    if (strength != -1) {
        if (strength == 112) {
            if (nbits != 2048) {
                RSAerr(RSA_F_RSA_SP800_56B_VALIDATE_STRENGTH,
                                        ...
                                        const BIGNUM *e, BN_CTX *ctx)
{
    ...
    rsa->d = BN_secure_new();
    if (rsa->d == NULL || BN_mod_inverse(rsa->d, e, lcm, ctx2) == NULL)
        goto err;
    ...
    if (rsa->iqmp == NULL
            || BN_mod_inverse(rsa->iqmp, rsa->q, rsa->p, ctx2) == NULL)
        goto err;
...
/*
 * SP800-56Br1 6.3.1 "RSA Key-Pair Generation with a Fixed Public Exponent"
                               ...
                               BN_GENCB *cb)
{
    ...
    /*
     * TODO - how do we determine strength ?? - is this a global concept???
     * for now just ignore it by setting to -1
     */
    int strength = -1;
    /* (Steps 1a-1b) */
...
/*
 * SP800-56Br1 6.3.1.3 (Step 6) Perform a pair-wise consistency test by
