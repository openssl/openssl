...
static int collect_numbers(STACK_OF(BIGNUM) *numbers, const OSSL_PARAM params[],
			   const char *names[])
{
...
int ossl_rsa_fromdata(RSA *rsa, const OSSL_PARAM params[], int include_private)
{
	...
	if (include_private) {
		param_derive = OSSL_PARAM_locate_const(
			params, OSSL_PKEY_PARAM_RSA_DERIVE_FROM_PQ);
		if ((param_derive != NULL) &&
		    !OSSL_PARAM_get_int(param_derive, &derive_from_pq))
			goto err;

		param_d =
			OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_RSA_D);
		...
		if (derive_from_pq) {
			...
			param_p = OSSL_PARAM_locate_const(
				params, OSSL_PKEY_PARAM_RSA_FACTOR1);
			param_q = OSSL_PARAM_locate_const(
				params, OSSL_PKEY_PARAM_RSA_FACTOR2);
			if ((param_p == NULL ||
			     !OSSL_PARAM_get_BN(param_p, &p)) ||
			    (param_q == NULL ||
	...
	if (is_private) {
		...
		if (derive_from_pq && sk_BIGNUM_num(exps) == 0 &&
			...
			if (sk_BIGNUM_num(factors) == 2) {
                 ...
                 * coefficients
                 */
				if (!ossl_rsa_set0_all_params(rsa, factors,
							      exps, coeffs)) {
					ERR_raise(ERR_LIB_RSA,
						  ERR_R_INTERNAL_ERROR);
					goto err;
			...
			if (sk_BIGNUM_num(factors) != 0 &&
			    !ossl_rsa_set0_all_params(rsa, factors, exps,
						      coeffs))
				goto err;
		}
		/* sanity check to ensure we used everything in our stacks */
		if (sk_BIGNUM_num(factors) != 0 || sk_BIGNUM_num(exps) != 0 ||
		    sk_BIGNUM_num(coeffs) != 0) {
			ERR_raise_data(
				ERR_LIB_RSA, ERR_R_INTERNAL_ERROR,
	...
	if (!ossl_rsa_check_factors(rsa)) {
		ERR_raise_data(
			ERR_LIB_RSA, RSA_R_INVALID_KEYPAIR,
		    ...
		    int include_private)
{
	...
	if (!ossl_param_build_set_bn(bld, params, OSSL_PKEY_PARAM_RSA_N,
				     rsa_n) ||
	...
	if (include_private && rsa_d != NULL) {
		if (!ossl_param_build_set_bn(bld, params, OSSL_PKEY_PARAM_RSA_D,
		    ...
		    !ossl_param_build_set_multi_key_bn(
			    bld, params, ossl_rsa_mp_factor_names, factors) ||
		    !ossl_param_build_set_multi_key_bn(
			    bld, params, ossl_rsa_mp_exp_names, exps) ||
		    !ossl_param_build_set_multi_key_bn(
			    bld, params, ossl_rsa_mp_coeff_names, coeffs))
			goto err;
	}
				  ...
				  OSSL_PARAM_BLD *bld, OSSL_PARAM params[])
{
	if (!ossl_rsa_pss_params_30_is_unrestricted(pss)) {
         ...
         */
		if ((mdname != NULL && !ossl_param_build_set_utf8_string(
					       bld, params, key_md, mdname)) ||
		    (mgfname != NULL &&
		     !ossl_param_build_set_utf8_string(bld, params, key_mgf,
						       mgfname)) ||
		    (mgf1mdname != NULL &&
		     !ossl_param_build_set_utf8_string(bld, params, key_mgf1_md,
						       mgf1mdname)) ||
		    (!ossl_param_build_set_int(bld, params, key_saltlen,
					       saltlen)))
			return 0;
	}
	return 1;
				    ...
				    OSSL_LIB_CTX *libctx)
{
	...
	if (pss_params == NULL)
		return 0;
	param_propq = OSSL_PARAM_locate_const(params,
					      OSSL_PKEY_PARAM_RSA_DIGEST_PROPS);
	param_md = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_RSA_DIGEST);
	param_mgf = OSSL_PARAM_locate_const(params,
					    OSSL_PKEY_PARAM_RSA_MASKGENFUNC);
	param_mgf1md = OSSL_PARAM_locate_const(params,
					       OSSL_PKEY_PARAM_RSA_MGF1_DIGEST);
	param_saltlen = OSSL_PARAM_locate_const(
     ...
     */
	if (!*defaults_set && (param_md != NULL || param_mgf != NULL ||
	...
	if (param_md != NULL) {
		...
		if ((md = EVP_MD_fetch(libctx, mdname, propq)) == NULL ||
		    !ossl_rsa_pss_params_30_set_hashalg(
			    pss_params, ossl_rsa_oaeppss_md2nid(md)))
			goto err;
	}
	...
	if (param_mgf1md != NULL) {
		...
		if ((mgf1md = EVP_MD_fetch(libctx, mgf1mdname, propq)) ==
			    NULL ||
...
RSA *ossl_rsa_dup(const RSA *rsa, int selection)
{
	...
	if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0 &&
		...
		for (i = 0; i < pnum; i++) {
			...
			if ((duppinfo = OPENSSL_zalloc(sizeof(*duppinfo))) ==
			    NULL)
				goto err;
			/* push first so cleanup in error case works */
			(void)sk_RSA_PRIME_INFO_push(dupkey->prime_infos,
						     duppinfo);
	...
	if (!CRYPTO_dup_ex_data(CRYPTO_EX_INDEX_RSA, &dupkey->ex_data,
				&rsa->ex_data))
		goto err;
#endif
...
static int ossl_rsa_sync_to_pss_params_30(RSA *rsa)
{
	...
	if (rsa != NULL && (legacy_pss = RSA_get0_pss_params(rsa)) != NULL &&
