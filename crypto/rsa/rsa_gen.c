...
{
    OSSL_FN_CTX *fn_ctx = NULL;
    OSSL_FN *fn_L = NULL, *fn_d = NULL, *acc = NULL, *pm1 = NULL;
    ...
    for (i = 2; i < primes; i++) {
        ...
        if (fn_ri == NULL)
            goto err;
        if (!OSSL_FN_copy_truncate(pm1, fn_ri)
            || !OSSL_FN_sub_word(pm1, 1))
            goto err;
        /* stash r_i - 1 in pinfo->d (as a BIGNUM) for later use */
        {
            ...
            if (fn_pd == NULL)
                goto err;
            if (!OSSL_FN_copy_truncate(fn_pd, pm1))
                goto err;
            pd_bits = (int)OSSL_FN_num_bits(fn_pd);
            bn_release(pinfo->d,
                pd_bits > 0 ? (pd_bits + BN_BITS2 - 1) / BN_BITS2 : 1);
        }
        /* acc = fn_L; fn_L = acc * pm1 */
        if (!OSSL_FN_copy_truncate(acc, fn_L)
            || !OSSL_FN_mul(fn_L, acc, pm1, fn_ctx))
            goto err;
    }
