			       ...
			       STACK_OF(BIGNUM) *exps, STACK_OF(BIGNUM) *coeffs)
{
	...
	for (i = 0; i < sk_BIGNUM_num(factors); i++) {
		...
		case 1:
			...
			if (tmp == NULL)
				goto err;
			if (!sk_BIGNUM_insert(pplist, tmp,
				 ...
				 BIGNUM *e_value, BN_GENCB *cb)
{
	...
	if (primes < RSA_DEFAULT_PRIME_NUM ||
	    primes > ossl_rsa_multip_cap(bits)) {
...
redo:
		...
		if (i == 1) {
			...
			if (tmp == NULL)
				goto err;
			if (!sk_BIGNUM_insert(factors, tmp,
	...
	if (!ossl_rsa_multiprime_derive(rsa, bits, primes, e_value, factors,
					exps, coeffs))
		goto err;
...
static int rsa_keygen_pairwise_test(RSA *rsa, OSSL_CALLBACK *cb, void *cbarg)
{
	...
	ciphertxt_len = RSA_public_encrypt(plaintxt_len, plaintxt, ciphertxt,
					   rsa, padding);
	if (ciphertxt_len <= 0)
		goto err;
	...
	decoded_len = RSA_private_decrypt(ciphertxt_len, ciphertxt, decoded,
					  rsa, padding);
