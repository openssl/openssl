...
int RSA_security_bits(const RSA *rsa)
{
	...
	if (rsa->version == RSA_ASN1_VERSION_MULTI) {
		...
		if (ex_primes <= 0 ||
		    (ex_primes + 2) > ossl_rsa_multip_cap(bits))
			return 0;
	}
#endif
			     ...
			     STACK_OF(BIGNUM) *exps, STACK_OF(BIGNUM) *coeffs)
{
	...
	if (pnum > 2) {
		...
		for (i = 2; i < pnum; i++) {
			...
			if (!ossl_assert(prime != NULL && exp != NULL &&
					 coeff != NULL))
				goto err;
	...
	if (old_infos != NULL) {
         ...
         * with other *set0* functions: just free it...
         */
		sk_RSA_PRIME_INFO_pop_free(old_infos,
					   ossl_rsa_multip_info_free);
	}
#endif

	r->version = pnum > 2 ? RSA_ASN1_VERSION_MULTI :
			       ...
			       const char *propkey, const char *mdprops)
{
	...
	*p++ = OSSL_PARAM_construct_utf8_string(mdkey, (char *)mdname, 0);
	if (evp_pkey_ctx_is_provided(ctx) && mdprops != NULL) {
		/* Cast away the const. This is read only so should be safe */
		*p++ = OSSL_PARAM_construct_utf8_string(propkey,
							(char *)mdprops, 0);
	}
			       ...
			       size_t mdnamesize)
{
	...
	/* Cast away the const. This is read only so should be safe */
	*p++ = OSSL_PARAM_construct_utf8_string(mdkey, (char *)mdname,
						mdnamesize);
	*p++ = OSSL_PARAM_construct_end();
				      ...
				      const char *mdprops)
{
	return int_set_rsa_md_name(
		ctx, -1, EVP_PKEY_OP_TYPE_CRYPT | EVP_PKEY_OP_TYPE_SIG,
...
{
	return int_set_rsa_md_name(ctx, EVP_PKEY_RSA_PSS, EVP_PKEY_OP_KEYGEN,
				   OSSL_PKEY_PARAM_MGF1_DIGEST, mdname, NULL,
				   NULL);
}
...
int EVP_PKEY_CTX_get_rsa_mgf1_md(EVP_PKEY_CTX *ctx, const EVP_MD **md)
{
	return RSA_pkey_ctx_ctrl(ctx,
				 EVP_PKEY_OP_TYPE_SIG | EVP_PKEY_OP_TYPE_CRYPT,
				 EVP_PKEY_CTRL_GET_RSA_MGF1_MD, 0,
				 (void *)(md));
}
...
int EVP_PKEY_CTX_set0_rsa_oaep_label(EVP_PKEY_CTX *ctx, void *label, int llen)
{
	...
	*p++ = OSSL_PARAM_construct_octet_string(
		OSSL_ASYM_CIPHER_PARAM_OAEP_LABEL, (void *)plabel,
		(size_t)llen);
	*p++ = OSSL_PARAM_construct_end();
...
int EVP_PKEY_CTX_set_rsa_keygen_primes(EVP_PKEY_CTX *ctx, int primes)
{
	...
	*p++ = OSSL_PARAM_construct_size_t(OSSL_PKEY_PARAM_RSA_PRIMES,
					   &primes2);
	*p++ = OSSL_PARAM_construct_end();
