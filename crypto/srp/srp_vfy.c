...
static int t_tob64(char *dst, const unsigned char *src, int size)
{
	...
	leadz = 3 - (size % 3);
	if (leadz != 3 &&
	    !EVP_EncodeUpdate(ctx, (unsigned char *)dst, &outl, pad, leadz)) {
		EVP_ENCODE_CTX_free(ctx);
		return 0;
	}
	...
	for (i = 0; i < sk_OPENSSL_PSTRING_num(tmpdb->data); i++) {
		...
		if (pp[DB_srptype][0] == DB_SRP_INDEX) {
			...
			if ((gN->id = OPENSSL_strdup(pp[DB_srpid])) == NULL ||
			    (gN->N = SRP_gN_place_bn(vb->gN_cache,
						     pp[DB_srpverifier])) ==
				    NULL ||
			    (gN->g = SRP_gN_place_bn(vb->gN_cache,
						     pp[DB_srpsalt])) == NULL ||
			...
			if ((lgN = SRP_get_gN_by_id(pp[DB_srpgN],
						    SRP_gN_tab)) != NULL) {
				error_code = SRP_ERR_MEMORY;
				...
				if (!SRP_user_pwd_set1_ids(user_pwd,
							   pp[DB_srpid],
							   pp[DB_srpinfo]))
					goto err;

				error_code = SRP_ERR_VBASE_BN_LIB;
				if (!SRP_user_pwd_set_sv(user_pwd,
							 pp[DB_srpsalt],
							 pp[DB_srpverifier]))
					goto err;

				if (sk_SRP_user_pwd_insert(vb->users_pwd,
							   user_pwd, 0) == 0)
					goto err;
...
SRP_user_pwd *SRP_VBASE_get1_by_user(SRP_VBASE *vb, char *username)
{
	...
	if ((vb->seed_key == NULL) || (vb->default_g == NULL) ||
	    (vb->default_N == NULL))
		return NULL;
	...
	if (md == NULL)
		goto err;
	ctxt = EVP_MD_CTX_new();
	if (ctxt == NULL || !EVP_DigestInit_ex(ctxt, md, NULL) ||
	...
	EVP_MD_free(md);
	md = NULL;
	if (SRP_user_pwd_set0_sv(user, BN_bin2bn(digs, SHA_DIGEST_LENGTH, NULL),
			     ...
			     OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if (*salt == NULL) {
		...
		if ((tmp_salt = OPENSSL_malloc_array(SRP_RANDOM_SALT_LEN, 2)) ==
		    NULL)
			goto err;
...
char *SRP_create_verifier(const char *user, const char *pass, char **salt,
			  char **verifier, const char *N, const char *g)
{
	return SRP_create_verifier_ex(user, pass, salt, verifier, N, g, NULL,
				      NULL);
}
