...
int ENGINE_set_load_tls_client_cert_function(ENGINE *e,
                                             ...
                                             loadtls_f)
{
    e->load_tls_client_cert = loadtls_f;
    return 1;
}
                                ...
                                UI_METHOD *ui_method, void *callback_data)
{
    ...
    if (e == NULL) {
        ENGINEerr(ENGINE_F_ENGINE_LOAD_tls_CLIENT_CERT,
                  ERR_R_PASSED_NULL_PARAMETER);
        return 0;
    }
    CRYPTO_THREAD_write_lock(global_engine_lock);
    ...
    CRYPTO_THREAD_unlock(global_engine_lock);
    if (!e->load_tls_client_cert) {
        ENGINEerr(ENGINE_F_ENGINE_LOAD_tls_CLIENT_CERT,
                  ENGINE_R_NO_LOAD_FUNCTION);
        return 0;
    }
    return e->load_tls_client_cert(e, s, ca_dn, pcert, ppkey, pother,
