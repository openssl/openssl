...
static int int_table_check(ENGINE_TABLE **t, int create)
{
	...
	if (!create)
		return 0;
	if ((lh = lh_ENGINE_PILE_new(engine_pile_hash, engine_pile_cmp)) ==
	    NULL)
		return 0;
	*t = (ENGINE_TABLE *)lh;
	return 1;
			  ...
			  int setdefault)
{
	...
	while (num_nids--) {
		...
		if (!fnd) {
			...
			if (lh_ENGINE_PILE_retrieve(&(*table)->piles,
						    &tmplate) != fnd) {
...
void engine_table_unregister(ENGINE_TABLE **table, ENGINE *e)
{
	...
	if (int_table_check(table, 0))
		lh_ENGINE_PILE_doall_ENGINE(&(*table)->piles, int_unregister_cb,
					    e);
	CRYPTO_THREAD_unlock(global_engine_lock);
}
			...
			void *arg)
{
	...
	dall.arg = arg;
	if (table)
		lh_ENGINE_PILE_doall_ENGINE_PILE_DOALL(&table->piles, int_dall,
						       &dall);
}
