...
static dynamic_data_ctx *dynamic_get_data_ctx(ENGINE *e)
{
	...
	if (dynamic_ex_data_idx < 0) {
         ...
         */
		int new_idx = ENGINE_get_ex_new_index(
			0, NULL, NULL, NULL, dynamic_data_ctx_free_func);
		if (new_idx == -1) {
...
static int int_load(dynamic_data_ctx *ctx)
{
	...
	for (loop = 0; loop < num; loop++) {
		...
		char *merge =
			DSO_merge(ctx->dynamic_dso, ctx->DYNAMIC_LIBNAME, s);
		if (!merge)
			return 0;
