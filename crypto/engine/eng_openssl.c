...
static const EVP_CIPHER *test_r4_cipher(void)
{
	if (r4_cipher == NULL) {
		...
		if ((cipher = EVP_CIPHER_meth_new(NID_rc4, 1,
						  TEST_RC4_KEY_SIZE)) == NULL ||
...
static const EVP_MD *test_sha_md(void)
{
	if (sha1_md == NULL) {
		...
		if ((md = EVP_MD_meth_new(NID_sha1,
					  NID_sha1WithRSAEncryption)) == NULL ||
		    !EVP_MD_meth_set_result_size(md, SHA_DIGEST_LENGTH) ||
		    !EVP_MD_meth_set_input_blocksize(md, SHA_CBLOCK) ||
		    !EVP_MD_meth_set_app_datasize(
			    md, sizeof(EVP_MD *) + sizeof(SHA_CTX)) ||
...
{
	OSSL_HMAC_PKEY_CTX *hctx =
		EVP_PKEY_CTX_get_data(EVP_MD_CTX_get_pkey_ctx(ctx));
