				       ...
				       int *res, BN_GENCB *cb)
{
	...
	return ossl_ffc_params_FIPS186_4_gen_verify(libctx,
						    (FFC_PARAMS *)params,
				       ...
				       int *res, BN_GENCB *cb)
{
	...
	L = BN_num_bits(params->p);
	N = BN_num_bits(params->q);
	return ossl_ffc_params_FIPS186_2_gen_verify(libctx,
						    (FFC_PARAMS *)params,
						    FFC_PARAM_MODE_VERIFY, type,
						    L, N, res, cb);
}
				    ...
				    int *res)
{
	...
	if (params->flags & FFC_PARAM_FLAG_VALIDATE_LEGACY)
		ret = ossl_ffc_params_FIPS186_2_validate(libctx, &tmpparams,
							 paramstype, res, NULL);
	else
#endif
		ret = ossl_ffc_params_FIPS186_4_validate(libctx, &tmpparams,
							 paramstype, res, NULL);
	...
	if (params->seed != NULL) {
		if (params->flags & FFC_PARAM_FLAG_VALIDATE_LEGACY)
			return ossl_ffc_params_FIPS186_2_validate(
				libctx, params, paramstype, res, NULL);
		else
		...
		ret = ossl_ffc_params_simple_validate(libctx, params,
						      paramstype, res);
		if (ret) {
			BN_CTX *ctx;
