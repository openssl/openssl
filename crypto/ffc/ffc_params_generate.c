					 ...
					 BN_GENCB *cb)
{
	...
	*res = 0;
	...
	for (;;) {
		...
		r = generate_p(ctx, md, counter, n, seed_tmp, seedlen, q, p,
			       (int)L, cb, &pcounter, res);
		if (r > 0)
			break; /* found p */
		if (r < 0)
			goto err;
...
g_only:
	...
	if ((seed != NULL) && (params->gindex != FFC_UNVERIFIABLE_GINDEX)) {
		...
		if (!generate_unverifiable_g(ctx, mont, g, tmp, p, e, pm1,
					     &hret))
			goto err;
	}
					 ...
					 BN_GENCB *cb)
{
	...
	*res = 0;
	...
	for (;;) {
		...
		rv = generate_p(ctx, md, counter, n, buf, qsize, q, p, (int)L,
				cb, &pcounter, res);
		if (rv > 0)
			break; /* found it */
		if (rv == -1)
...
g_only:
	...
	if (!verify) {
		...
		if (!BN_div(r0, NULL, test, q, ctx))
			goto err;
		if (!generate_unverifiable_g(ctx, mont, g, tmp, p, r0, test,
					     &hret))
			goto err;
	...
	if (!verify) {
		...
		if (params->p == NULL || params->q == NULL || params->g == NULL)
			goto err;
		if (!ossl_ffc_params_set_validate_params(params, seed, qsize,
							 pcounter))
			goto err;
		params->h = hret;
	}
