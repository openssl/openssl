						  ...
						  size_t buflen)
{
	...
	if (cctx == NULL || EVP_PKEY_paramgen_init(cctx) <= 0 ||
			 ...
			 unsigned char *pt, size_t *ptlen)
{
	...
	if (ctlen <= taglen || *ptlen < ctlen - taglen || aadlen > INT_MAX ||
	    ctlen > INT_MAX) {
			 ...
			 unsigned char *ct, size_t *ctlen)
{
	...
	*ctlen += len;
	...
	if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, (int)taglen, tag) !=
	    1) {
		ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
		goto err;
	}
 ...
 * @return 1 for success, 0 otherwise
 */
static int hpke_random_suite(OSSL_LIB_CTX *libctx, const char *propq,
 ...
 * @return 1 for success, 0 otherwise
 */
static int hpke_expansion(OSSL_HPKE_SUITE suite, size_t *enclen,
 ...
 */
static size_t hpke_seqnonce2buf(OSSL_HPKE_CTX *ctx, unsigned char *buf,
				size_t blen)
{
	size_t i;
	uint64_t seq_copy;
		      ...
		      const unsigned char *pub, size_t publen)
{
	...
	*p++ = OSSL_PARAM_construct_utf8_string(
		OSSL_KEM_PARAM_OPERATION, OSSL_KEM_PARAM_OPERATION_DHKEM, 0);
	if (ctx->ikme != NULL) {
		*p++ = OSSL_PARAM_construct_octet_string(
			OSSL_KEM_PARAM_IKME, ctx->ikme, ctx->ikmelen);
	}
 ...
 * @return 1 for success, 0 for error
 */
static int hpke_decap(OSSL_HPKE_CTX *ctx, const unsigned char *enc,
		      size_t enclen, EVP_PKEY *priv)
{
	int erv = 0;
	EVP_PKEY_CTX *pctx = NULL;
	EVP_PKEY *spub = NULL;
	...
	*p = OSSL_PARAM_construct_end();
	if (ctx->mode == OSSL_HPKE_MODE_AUTH ||
		...
		if (hpke_kem_id_nist_curve(ctx->suite.kem_id) == 1) {
			spub = evp_pkey_new_raw_nist_public_key(
				ctx->libctx, ctx->propq, kem_info->groupname,
				ctx->authpub, ctx->authpublen);
		} else {
			spub = EVP_PKEY_new_raw_public_key_ex(
				ctx->libctx, kem_info->keytype, ctx->propq,
				ctx->authpub, ctx->authpublen);
		}
		if (spub == NULL) {
			ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
	...
	if (ctx->shared_secret == NULL)
		goto err;
	if (EVP_PKEY_decapsulate(pctx, ctx->shared_secret, &lsslen, enc,
				 enclen) != 1) {
		ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
		goto err;
	}
 ...
 */
static int hpke_do_middle(OSSL_HPKE_CTX *ctx, const unsigned char *info,
			  size_t infolen)
{
	int erv = 0;
	...
	if (ossl_hpke_labeled_extract(
		    kctx, ks_context + 1, halflen, NULL, 0,
		    OSSL_HPKE_SEC51LABEL, suitebuf, sizeof(suitebuf),
		    OSSL_HPKE_PSKIDHASH_LABEL, (unsigned char *)ctx->pskid,
		    pskidlen) != 1) {
		ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
		goto err;
	}
	if (ossl_hpke_labeled_extract(kctx, ks_context + 1 + halflen, halflen,
				      NULL, 0, OSSL_HPKE_SEC51LABEL, suitebuf,
				      sizeof(suitebuf),
	...
	if (ossl_hpke_labeled_extract(kctx, secret, secretlen,
				      ...
				      OSSL_HPKE_SEC51LABEL, suitebuf,
				      sizeof(suitebuf), OSSL_HPKE_SECRET_LABEL,
	...
	if (ctx->suite.aead_id != OSSL_HPKE_AEAD_ID_EXPORTONLY) {
		...
		if (ctx->nonce == NULL)
			goto err;
		if (ossl_hpke_labeled_expand(
			    kctx, ctx->nonce, ctx->noncelen, secret, secretlen,
			    OSSL_HPKE_SEC51LABEL, suitebuf, sizeof(suitebuf),
			    OSSL_HPKE_NONCE_LABEL, ks_context,
			    ks_contextlen) != 1) {
			ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
			goto err;
		}
		...
		if (ctx->key == NULL)
			goto err;
		if (ossl_hpke_labeled_expand(
			    kctx, ctx->key, ctx->keylen, secret, secretlen,
			    OSSL_HPKE_SEC51LABEL, suitebuf, sizeof(suitebuf),
			    OSSL_HPKE_KEY_LABEL, ks_context,
			    ks_contextlen) != 1) {
			ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
			goto err;
		}
	...
	if (ctx->exportersec == NULL)
		goto err;
	if (ossl_hpke_labeled_expand(
		    kctx, ctx->exportersec, ctx->exporterseclen, secret,
		    secretlen, OSSL_HPKE_SEC51LABEL, suitebuf, sizeof(suitebuf),
		    OSSL_HPKE_EXP_LABEL, ks_context, ks_contextlen) != 1) {
		ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
		goto err;
	}
	erv = 1;
				 ...
				 OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if (suite.aead_id != OSSL_HPKE_AEAD_ID_EXPORTONLY) {
		ctx->aead_ciph =
			EVP_CIPHER_fetch(libctx, aead_info->name, propq);
			       ...
			       size_t publen)
{
	...
	if (EVP_PKEY_get_octet_string_param(
		    pubp, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, lpub,
		    OSSL_HPKE_MAXSIZE, &lpublen) != 1) {
		OPENSSL_free(lpub);
		ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
		goto err;
	}
...
int OSSL_HPKE_decap(OSSL_HPKE_CTX *ctx, const unsigned char *enc, size_t enclen,
		    EVP_PKEY *recippriv, const unsigned char *info,
		    size_t infolen)
{
	int erv = 1;
	size_t minenc = 0;
		   ...
		   const unsigned char *pt, size_t ptlen)
{
	...
	if (hpke_aead_enc(ctx, seqbuf, aad, aadlen, pt, ptlen, ct, ctlen) !=
	    1) {
		ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
		   ...
		   const unsigned char *ct, size_t ctlen)
{
	...
	if (hpke_aead_dec(ctx, seqbuf, aad, aadlen, ct, ctlen, pt, ptlen) !=
	    1) {
		ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
		OPENSSL_cleanse(seqbuf, sizeof(seqbuf));
		return 0;
...
int OSSL_HPKE_export(OSSL_HPKE_CTX *ctx, unsigned char *secret,
		     size_t secretlen, const unsigned char *label,
		     size_t labellen)
{
	int erv = 0;
	EVP_KDF_CTX *kctx = NULL;
	...
	suitebuf[4] = ctx->suite.aead_id / 256;
	suitebuf[5] = ctx->suite.aead_id % 256;
	erv = ossl_hpke_labeled_expand(
		kctx, secret, secretlen, ctx->exportersec, ctx->exporterseclen,
		OSSL_HPKE_SEC51LABEL, suitebuf, sizeof(suitebuf),
		OSSL_HPKE_EXP_SEC_LABEL, label, labellen);
	EVP_KDF_CTX_free(kctx);
	if (erv != 1)
		ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
	return erv;
...
int OSSL_HPKE_keygen(OSSL_HPKE_SUITE suite, unsigned char *pub, size_t *publen,
		     EVP_PKEY **priv, const unsigned char *ikm, size_t ikmlen,
		     OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if ((ikmlen > 0 && ikm == NULL) || (ikmlen == 0 && ikm != NULL) ||
	...
	if (hpke_kem_id_nist_curve(suite.kem_id) == 1) {
		*p++ = OSSL_PARAM_construct_utf8_string(
			OSSL_PKEY_PARAM_GROUP_NAME, (char *)kem_info->groupname,
			0);
	...
	*p = OSSL_PARAM_construct_end();
	...
	EVP_PKEY_CTX_free(pctx);
	pctx = NULL;
	if (EVP_PKEY_get_octet_string_param(skR,
...
int OSSL_HPKE_get_grease_value(const OSSL_HPKE_SUITE *suite_in,
			       OSSL_HPKE_SUITE *suite, unsigned char *enc,
			       size_t *enclen, unsigned char *ct, size_t ctlen,
			       OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if (enc == NULL || enclen == 0 || ct == NULL || ctlen == 0 ||
	    suite == NULL) {
		ERR_raise(ERR_LIB_CRYPTO, ERR_R_PASSED_INVALID_ARGUMENT);
		return 0;
	}
	...
	*suite = chosen;
     ...
     * the relevant curve.
     */
	if (OSSL_HPKE_keygen(chosen, enc, enclen, &fakepriv, NULL, 0, libctx,
			     propq) != 1) {
		ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
		goto err;
	}
