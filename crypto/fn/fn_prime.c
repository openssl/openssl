...
size_t ossl_fn_miller_rabin_is_prime_ctx_size(const OSSL_FN *w)
{
    ...
    /*
     * The nested sizing functions inspect widths only, so a width-only
     * model would serve as well; |w| itself is simply at hand.
     */
    exp = OSSL_FN_mod_exp_mont_ctx_size(w, w, w, w, NULL);
    mul = OSSL_FN_mod_mul_ctx_size(w, w, w, w);
    gcd = OSSL_FN_gcd_ctx_size(w, w);
    inner = ossl_fn_ctx_max_size(exp, ossl_fn_ctx_max_size(mul, gcd));

    return ossl_fn_ctx_add_size(scratch, inner);
}
...
{
    int i, j, a, ret = 0;
    OSSL_FN *g = NULL, *w1 = NULL, *w3 = NULL, *x = NULL, *m = NULL, *z = NULL,
            *b = NULL;
     ...
     * w == 1 makes w3 underflow to a huge positive, so test w1 (w - 1) as
     * well -- w is odd, so w == 1 is the only underflow case
     */
    if (OSSL_FN_is_zero(w1) || OSSL_FN_is_zero(w3))
        goto err;
 ...
 * past |bits| bits, and the headroom lets the significance check below
 * detect that and redraw rather than silently truncate.
 *
 * The probably prime is saved in |rnd|.
 *
 * Returns 1 on success and 0 on error.
    ...
    OSSL_FN_CTX *ctx, OSSL_LIB_CTX *libctx)
{
    ...
    int trial_divisions = ossl_fn_calc_trial_divisions(bits);
    /*
     * Saturating low-limb read of |add|: when its value doesn't fit in one
     * limb, treat it as OSSL_FN_MASK, which makes maxdelta zero so that
     * every sieve hit forces a redraw -- slow, but never wrong.  Stepping
     * the sieve by the low limb alone would silently break the invariant
     * rnd % add == rem.
     */
    const OSSL_FN_ULONG addw = OSSL_FN_num_bits(add) > OSSL_FN_BITS
        ? OSSL_FN_MASK
        : OSSL_FN_get_word(add);
    OSSL_FN_ULONG maxdelta = OSSL_FN_MASK - primes[trial_divisions - 1];
    const void *token = NULL;

    if (maxdelta > OSSL_FN_MASK - addw)
        maxdelta = OSSL_FN_MASK - addw;
...
loop:
    ...
    if (!OSSL_FN_add_word(rnd, delta))
        goto err;
    /*
     * The residue adjustment and the sieve delta can push the candidate
     * past |bits| bits; the carry lands in the headroom limb and is caught
     * here, and the candidate is redrawn in that case.
     */
    if (OSSL_FN_num_bits(rnd) != bits)
        goto again;
    ret = 1;
    ...
    OSSL_FN_CTX *ctx, OSSL_LIB_CTX *libctx)
{
    ...
    /*
     * With |add| of |bits| bits or more, the progression rem + k*add may
     * contain no value of exactly |bits| bits, and the sieve's redraw
     * loop would then never terminate.  Anything narrower always has one.
     */
    if (add != NULL && OSSL_FN_num_bits(add) >= bits) {
...
err:
    if (found)
        /*
         * The candidate has exactly |bits| bits; the copy is exact when
         * |ret| is sized per the contract above.
         */
    ...
    int safe, const OSSL_FN *add, const OSSL_FN *rem)
{
    ...
    /*
     * Candidates carry one limb of headroom over |bits|.  A width-only
     * model suffices: the Miller-Rabin and div sizing it feeds inspects
     * widths only.
     */
    OSSL_FN cand = { .dsize = 0 };

    if (bits < 2 || limbs == 0 || (add == NULL && rem != NULL)
        || (ret != NULL && (size_t)ret->dsize < limbs))
        return 0;

    cand.dsize = (int)(limbs + 1);
