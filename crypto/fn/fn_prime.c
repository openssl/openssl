...
size_t ossl_fn_miller_rabin_is_prime_ctx_size(const OSSL_FN *w)
{
    ...
    /*
     * Some of the nested sizing functions inspect operand values (the
     * Montgomery family sizes reductions conditionally), so they are
     * given |w| itself; a width-only model would be read out of bounds.
     */
    ...
    int safe, const OSSL_FN *add, const OSSL_FN *rem)
{
     ...
     * Miller-Rabin sizing it feeds may inspect operand values; a zero
     * value takes the same comparison branches as the reduced operands
     * seen at runtime.
     */
