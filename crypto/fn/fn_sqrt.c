    ...
    OSSL_FN_CTX *ctx)
{
    ...
    size_t L = (size_t)p->dsize; /* all temporaries are p-wide */
    ...
    if (e == 2) {
        ...
        if (!OSSL_FN_mod_mul(t, t, y, p, ctx))
            goto end;
        /*
         * t is in [0, p) here; keep the subtraction meaningful modulo p
         * when t == 0 rather than letting it wrap past the width.
         */
        if (OSSL_FN_is_zero(t))
            if (OSSL_FN_copy(t, p) == NULL)
                goto end;
 ...
 * OSSL_FN_mod, and OSSL_FN_kronecker.  The largest nested requirement
 * dominates; mod_exp is typically the heaviest.
 */
    ...
    const OSSL_FN *p)
{
    ...
    size_t own_size = OSSL_FN_CTX_size(1, 7, 7 * L);
     ...
     *   - OSSL_FN_mod(A, a, p)           (initial A = a mod p)
     *
     * |p| stands in for the L-wide temporaries (sizing reads only dsize),
     * except for the initial reduction, where the numerator is |a| itself
     * and may be wider than p.
     */
