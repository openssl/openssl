    ...
    const OSSL_FN *e)
{
    ...
    nested = ossl_fn_ctx_max_size(inv1,
 ...
 *
 * The primality check sizing inspects operand values (the Montgomery
 * family sizes reductions conditionally), so it must be given real
 * numbers; caller-absent auxiliary primes are modelled with freshly
 * allocated zeroed numbers of the auxiliary prime width.
 */
...
{
    int bitlen;
    size_t bitlimbs, n_temps, own, nested, hunt1, hunt2, derive, ret = 0;
    OSSL_FN *m_p1 = NULL, *m_p2 = NULL;
    const OSSL_FN *w1, *w2;
    ...
    if (p1 == NULL && (m_p1 = OSSL_FN_new_limbs(bitlimbs)) == NULL)
        goto err;
    if (p2 == NULL && (m_p2 = OSSL_FN_new_limbs(bitlimbs)) == NULL)
        goto err;
    w1 = p1 != NULL ? p1 : m_p1;
    w2 = p2 != NULL ? p2 : m_p2;
    ...
    hunt1 = ossl_fn_check_generated_prime_ctx_size(w1);
    hunt2 = ossl_fn_check_generated_prime_ctx_size(w2);
    derive = ossl_fn_rsa_fips186_5_derive_prime_ctx_size(p, Xpout, w1, w2,
        nlen, e);
    nested = ossl_fn_ctx_max_size(hunt1,
        ossl_fn_ctx_max_size(hunt2, derive));
    ret = ossl_fn_ctx_add_size(own, nested);
err:
    OSSL_FN_free(m_p1);
    OSSL_FN_free(m_p2);
    return ret;
