			     ...
			     const OSSL_PARAM params[], const char *propq)
{
	...
	if (propq != NULL) {
		...
		if (OSSL_PARAM_locate_const(
			    params, OSSL_STORE_PARAM_PROPERTIES) != NULL)
	...
	for (i = 0; loader_ctx == NULL && i < schemes_n; i++) {
		...
		if ((loader = ossl_store_get0_loader_int(scheme)) != NULL) {
			...
			if (loader->open_ex != NULL)
				loader_ctx = loader->open_ex(loader, uri,
							     libctx, propq,
							     ui_method,
							     ui_data);
			else
				loader_ctx = loader->open(loader, uri,
							  ui_method, ui_data);
		} else {
			ERR_pop_to_mark();
		}
#endif
		if (loader == NULL &&
		    (fetched_loader = OSSL_STORE_LOADER_fetch(libctx, scheme,
							      propq)) != NULL) {
			...
			if (fetched_loader->p_open_ex != NULL) {
				...
				if (fetched_loader->p_open != NULL &&
				    (loader_ctx = fetched_loader->p_open(
					     provctx, uri)) != NULL &&
				    !loader_set_params(fetched_loader,
						       loader_ctx, params,
						       propq)) {
					(void)fetched_loader->p_close(
						loader_ctx);
					loader_ctx = NULL;
				}
			}
...
int OSSL_STORE_vctrl(OSSL_STORE_CTX *ctx, int cmd, va_list args)
{
	if (ctx->fetched_loader != NULL) {
		if (ctx->fetched_loader->p_set_ctx_params != NULL) {
			...
			return ctx->fetched_loader->p_set_ctx_params(
				ctx->loader_ctx, params);
		}
...
int OSSL_STORE_expect(OSSL_STORE_CTX *ctx, int expected_type)
{
	...
	if (ctx->fetched_loader != NULL &&
		...
		params[0] = OSSL_PARAM_construct_int(OSSL_STORE_PARAM_EXPECT,
						     &expected_type);
	...
	if (ctx->fetched_loader != NULL) {
		...
		case OSSL_STORE_SEARCH_BY_NAME:
			if ((name_der_sz = i2d_X509_NAME(
			    ...
			    OSSL_PARAM_BLD_push_octet_string(
				    bld, OSSL_STORE_PARAM_SUBJECT, name_der,
				    name_der_sz))
				ret = 1;
			break;
		case OSSL_STORE_SEARCH_BY_ISSUER_SERIAL:
			if ((name_der_sz = i2d_X509_NAME(
			    ...
			    (number = ASN1_INTEGER_to_BN(search->serial,
							 NULL)) != NULL &&
			    OSSL_PARAM_BLD_push_octet_string(
				    bld, OSSL_STORE_PARAM_ISSUER, name_der,
				    name_der_sz) &&
		...
		case OSSL_STORE_SEARCH_BY_KEY_FINGERPRINT:
			if (OSSL_PARAM_BLD_push_utf8_string(
			    ...
			    OSSL_PARAM_BLD_push_octet_string(
				    bld, OSSL_STORE_PARAM_FINGERPRINT,
				    search->string, search->stringlength))
				ret = 1;
			break;
		case OSSL_STORE_SEARCH_BY_ALIAS:
			if (OSSL_PARAM_BLD_push_utf8_string(
				    bld, OSSL_STORE_PARAM_ALIAS,
				    (char *)search->string,
				    search->stringlength))
				ret = 1;
			break;
		...
		if (ret) {
			...
			ret = ctx->fetched_loader->p_set_ctx_params(
				ctx->loader_ctx, params);
...
again:
	...
	if (ctx->cached_info != NULL) {
		...
		if (ctx->fetched_loader != NULL) {
			...
			if (!ctx->fetched_loader->p_load(
				    ctx->loader_ctx,
				    ossl_store_handle_load_result, &load_data,
				    ossl_pw_passphrase_callback_dec,
				    &ctx->pwdata)) {
				ctx->error_flag = 1;
				return NULL;
	...
	if (v != NULL && ctx->expected_type != 0) {
		...
		if (returned_type != OSSL_STORE_INFO_NAME &&
		    returned_type != 0) {
...
OSSL_STORE_INFO *OSSL_STORE_INFO_new_PARAMS(EVP_PKEY *params)
{
	OSSL_STORE_INFO *info =
		OSSL_STORE_INFO_new(OSSL_STORE_INFO_PARAMS, params);
...
{
	OSSL_STORE_INFO *info =
		OSSL_STORE_INFO_new(OSSL_STORE_INFO_PUBKEY, pkey);
...
int OSSL_STORE_supports_search(OSSL_STORE_CTX *ctx, int search_type)
{
	...
	if (ctx->fetched_loader != NULL) {
		...
		p_subject = OSSL_PARAM_locate_const(params,
						    OSSL_STORE_PARAM_SUBJECT);
		p_issuer = OSSL_PARAM_locate_const(params,
						   OSSL_STORE_PARAM_ISSUER);
		p_serial = OSSL_PARAM_locate_const(params,
						   OSSL_STORE_PARAM_SERIAL);
		p_fingerprint = OSSL_PARAM_locate_const(
			params, OSSL_STORE_PARAM_FINGERPRINT);
		p_alias =
			OSSL_PARAM_locate_const(params, OSSL_STORE_PARAM_ALIAS);
