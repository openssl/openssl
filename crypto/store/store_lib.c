...
const char *OSSL_file_stat(const char *uri, struct stat *st)
{
    ...
    if (CHECK_AND_SKIP_CASE_PREFIX(q, "//")) {
        if (CHECK_AND_SKIP_CASE_PREFIX(q, "localhost/")
            ...
            path = q - 1;
#ifdef _WIN32
        } else if (ossl_isalpha(path[2]) && path[3] == ':' && path[4] == '/') {
            /* Windows "file://" URIs with a drive letter followed by a '/' */
            path = q;
#endif
        } else {
...
#ifdef _WIN32
    /* Windows "file:" URIs with a drive letter may start with a '/' */
    if (path[0] == '/' && ossl_isalpha(path[1]) && path[2] == ':' && path[3] == '/')
