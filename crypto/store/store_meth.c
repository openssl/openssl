				   ...
				   void *data)
{
	...
	if (!ossl_method_store_fetch(store, id, methdata->propquery, prov,
				     &method))
		return NULL;
	return method;
}
...
static int put_loader_in_store(void *store, void *method,
			       const OSSL_PROVIDER *prov, const char *scheme,
	...
	for (; fns->function_id != 0; fns++) {
		...
		case OSSL_FUNC_STORE_OPEN_EX:
			if (loader->p_open_ex == NULL)
				loader->p_open_ex =
					OSSL_FUNC_store_open_ex(fns);
			break;
		}
	}

	if ((loader->p_open == NULL && loader->p_attach == NULL) ||
	    loader->p_load == NULL || loader->p_eof == NULL ||
	    loader->p_close == NULL) {
					     ...
					     const char *properties)
{
	...
	if (id == 0 ||
		...
		OSSL_METHOD_CONSTRUCT_METHOD mcm = {
			get_tmp_loader_store,	reserve_loader_store,
			unreserve_loader_store, get_loader_from_store,
			put_loader_in_store,	construct_loader,
			destruct_loader
		};
		...
		if ((method = ossl_method_construct(
			     methdata->libctx, OSSL_OP_STORE, &prov,
...
const OSSL_PROVIDER *
OSSL_STORE_LOADER_get0_provider(const OSSL_STORE_LOADER *loader)
{
...
void OSSL_STORE_LOADER_do_all_provided(
	OSSL_LIB_CTX *libctx,
				   ...
				   void *data)
{
	...
	if (loader->prov != NULL) {
		...
		return ossl_namemap_doall_names(namemap, loader->scheme_id, fn,
						data);
	}
