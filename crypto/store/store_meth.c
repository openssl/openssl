...
{
    OSSL_STORE_LOADER *loader = OPENSSL_zalloc(sizeof(*loader));

    loader->lock = CRYPTO_THREAD_lock_new();
    if (loader->lock == NULL) {
        OPENSSL_free(loader);
        return NULL;
    }
                                  ...
                                  OSSL_PROVIDER *prov)
{
    ...
    for (; fns->function_id != 0; fns++) {
        ...
        case OSSL_FUNC_STORE_OPEN:
            if (loader->o_open != NULL)
                break;
            loader->o_open = OSSL_get_OP_store_open(fns);
            break;
        case OSSL_FUNC_STORE_SET_PARAMS:
            if (loader->set_params != NULL)
                break;
            loader->set_params = OSSL_get_OP_store_set_params(fns);
            break;
        case OSSL_FUNC_STORE_LOAD:
            if (loader->o_load != NULL)
                break;
            loader->o_load = OSSL_get_OP_store_load(fns);
            break;
        case OSSL_FUNC_STORE_EOF:
            if (loader->o_eof != NULL)
                break;
            loader->o_eof = OSSL_get_OP_store_eof(fns);
            break;
        case OSSL_FUNC_STORE_CLOSE:
            if (loader->o_close != NULL)
                break;
            loader->o_close = OSSL_get_OP_store_close(fns);
            break;
        }
    }
    ...
    if (loader->o_open == NULL
        ...
        || loader->o_eof == NULL
        || loader->o_close == NULL) {
        /* Only ctx_set_params is optionaal */
                                       ...
                                       void *arg)
{
    ...
    data.user_arg = arg;
    ossl_algorithm_do_all(libctx, OSSL_OP_STORE, NULL, loader_do_one, &data);
}
