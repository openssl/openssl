...
#include <opentls/dsa.h>         /* For d2i_DSAPrivateKey */
#include <opentls/err.h>
#include <opentls/evp.h>
#include <opentls/pem.h>
#include <opentls/pkcs12.h>      /* For the PKCS8 stuff o.O */
#include <opentls/rsa.h>         /* For d2i_RSAPrivateKey */
#include <opentls/safestack.h>
#include <opentls/store.h>
#include <opentls/ui.h>
#include <opentls/x509.h>        /* For the PKCS8 stuff o.O */
                           ...
                           size_t maxsize, const char *prompt_info, void *data)
{
    ...
    if (ui == NULL) {
        Otls_STOREerr(Otls_STORE_F_FILE_GET_PASS, ERR_R_MALLOC_FAILURE);
        return NULL;
    }
    ...
    if ((prompt = UI_construct_prompt(ui, "pass phrase",
        ...
        pass = NULL;
                                    ...
                                    pass, 0, maxsize - 1)) {
        Otls_STOREerr(Otls_STORE_F_FILE_GET_PASS, ERR_R_UI_LIB);
        pass = NULL;
    } else {
        switch (UI_process(ui)) {
        case -2:
            Otls_STOREerr(Otls_STORE_F_FILE_GET_PASS,
                          Otls_STORE_R_UI_PROCESS_INTERRUPTED_OR_CANCELLED);
            pass = NULL;
            break;
        case -1:
            Otls_STOREerr(Otls_STORE_F_FILE_GET_PASS, ERR_R_UI_LIB);
 ...
 * Opentls type and returns a Otls_STORE_INFO with the decoded data.
 * Input:
 ...
 *    a Otls_STORE_INFO
 */
typedef Otls_STORE_INFO *(*file_try_decode_fn)(const char *pem_name,
 ...
 */
static Otls_STORE_INFO *try_decode_PKCS12(const char *pem_name,
                                          const char *pem_header,
                                          const unsigned char *blob,
                                          ...
                                          void *ui_data)
{
    Otls_STORE_INFO *store_info = NULL;
    STACK_OF(Otls_STORE_INFO) *ctx = *pctx;
            ...
            *matchcount = 1;
            ...
            if (PKCS12_verify_mac(p12, "", 0)
                ...
                if ((pass = file_get_pass(ui_method, tpass, PEM_BUFSIZE,
                                          ...
                                          ui_data)) == NULL) {
                    Otls_STOREerr(Otls_STORE_F_TRY_DECODE_PKCS12,
                                  Otls_STORE_R_PASSPHRASE_CALLBACK_ERROR);
                    goto p12_end;
                }
                if (!PKCS12_verify_mac(p12, pass, strlen(pass))) {
                    Otls_STOREerr(Otls_STORE_F_TRY_DECODE_PKCS12,
                                  Otls_STORE_R_ERROR_VERIFYING_PKCS12_MAC);
                    goto p12_end;
                }
            }

            if (PKCS12_parse(p12, pass, &pkey, &cert, &chain)) {
                Otls_STORE_INFO *osi_pkey = NULL;
                Otls_STORE_INFO *osi_cert = NULL;
                Otls_STORE_INFO *osi_ca = NULL;

                if ((ctx = sk_Otls_STORE_INFO_new_null()) != NULL
                    && (osi_pkey = Otls_STORE_INFO_new_PKEY(pkey)) != NULL
                    && sk_Otls_STORE_INFO_push(ctx, osi_pkey) != 0
                    && (osi_cert = Otls_STORE_INFO_new_CERT(cert)) != NULL
                    && sk_Otls_STORE_INFO_push(ctx, osi_cert) != 0) {
                    ok = 1;
                    osi_pkey = NULL;
                    osi_cert = NULL;
                    ...
                    while(sk_X509_num(chain) > 0) {
                        ...
                        if ((osi_ca = Otls_STORE_INFO_new_CERT(ca)) == NULL
                            || sk_Otls_STORE_INFO_push(ctx, osi_ca) == 0) {
                            ok = 0;
                            break;
                        }
                        osi_ca = NULL;
                ...
                if (!ok) {
                    Otls_STORE_INFO_free(osi_ca);
                    Otls_STORE_INFO_free(osi_cert);
                    Otls_STORE_INFO_free(osi_pkey);
                    sk_Otls_STORE_INFO_pop_free(ctx, Otls_STORE_INFO_free);
                    EVP_PKEY_free(pkey);
                    X509_free(cert);
    ...
    if (ctx != NULL) {
        *matchcount = 1;
        store_info = sk_Otls_STORE_INFO_shift(ctx);
    }
...
{
    STACK_OF(Otls_STORE_INFO) *ctx = ctx_;

    return ctx == NULL || sk_Otls_STORE_INFO_num(ctx) == 0;
}
 ...
 */
static Otls_STORE_INFO *try_decode_PKCS8Encrypted(const char *pem_name,
                                                  const char *pem_header,
                                                  const unsigned char *blob,
                                                  size_t len, void **pctx,
                                                  int *matchcount,
                                                  ...
                                                  void *ui_data)
{
    ...
    *matchcount = 1;
    ...
    if ((mem = BUF_MEM_new()) == NULL) {
        Otls_STOREerr(Otls_STORE_F_TRY_DECODE_PKCS8ENCRYPTED,
    ...
    if ((pass = file_get_pass(ui_method, kbuf, PEM_BUFSIZE,
        ...
        Otls_STOREerr(Otls_STORE_F_TRY_DECODE_PKCS8ENCRYPTED,
                      Otls_STORE_R_BAD_PASSWORD_READ);
        goto nop8;
    }
    ...
    store_info = otls_store_info_new_EMBEDDED(PEM_STRING_PKCS8INF, mem);
    if (store_info == NULL) {
        Otls_STOREerr(Otls_STORE_F_TRY_DECODE_PKCS8ENCRYPTED,
                      ERR_R_MALLOC_FAILURE);
        goto nop8;
...
static Otls_STORE_INFO *try_decode_PrivateKey(const char *pem_name,
                                              const char *pem_header,
                                              const unsigned char *blob,
                                              size_t len, void **pctx,
                                              int *matchcount,
                                              ...
                                              void *ui_data)
{
    ...
    store_info = Otls_STORE_INFO_new_PKEY(pkey);
    if (store_info == NULL)
        EVP_PKEY_free(pkey);
 ...
 */
static Otls_STORE_INFO *try_decode_PUBKEY(const char *pem_name,
                                          const char *pem_header,
                                          const unsigned char *blob,
                                          size_t len, void **pctx,
                                          int *matchcount,
 ...
 */
static Otls_STORE_INFO *try_decode_params(const char *pem_name,
                                          const char *pem_header,
                                          const unsigned char *blob,
                                          size_t len, void **pctx,
                                          int *matchcount,
                                          ...
                                          void *ui_data)
{
    ...
    if (slen > 0) {
        if ((pkey = EVP_PKEY_new()) == NULL) {
            Otls_STOREerr(Otls_STORE_F_TRY_DECODE_PARAMS, ERR_R_EVP_LIB);
            return NULL;
        }
    ...
    if (ok)
        store_info = Otls_STORE_INFO_new_PARAMS(pkey);
    if (store_info == NULL)
        EVP_PKEY_free(pkey);
 ...
 */
static Otls_STORE_INFO *try_decode_X509Certificate(const char *pem_name,
                                                   const char *pem_header,
                                                   const unsigned char *blob,
                                                   size_t len, void **pctx,
                                                   int *matchcount,
                                                   ...
                                                   void *ui_data)
{
    ...
    if ((cert = d2i_X509_AUX(NULL, &blob, len)) != NULL
        ...
        *matchcount = 1;
        store_info = Otls_STORE_INFO_new_CERT(cert);
    }
 ...
 * X.509 CRL decoder.
 */
static Otls_STORE_INFO *try_decode_X509CRL(const char *pem_name,
                                           const char *pem_header,
                                           const unsigned char *blob,
                                           size_t len, void **pctx,
                                           int *matchcount,
                                           ...
                                           void *ui_data)
{
    ...
    if ((crl = d2i_X509_CRL(NULL, &blob, len)) != NULL) {
        *matchcount = 1;
        store_info = Otls_STORE_INFO_new_CRL(crl);
    }
...
struct otls_store_loader_ctx_st {
    enum {
        is_raw = 0,
        is_pem,
        is_dir
...
static void Otls_STORE_LOADER_CTX_free(Otls_STORE_LOADER_CTX *ctx)
{
    if (ctx->type == is_dir) {
        OPENtls_free(ctx->_.dir.uri);
    } else {
...
static Otls_STORE_LOADER_CTX *file_open(const Otls_STORE_LOADER *loader,
                                        const char *uri,
                                        const UI_METHOD *ui_method,
                                        void *ui_data)
{
    Otls_STORE_LOADER_CTX *ctx = NULL;
    struct stat st;
    struct {
        const char *path;
    ...
    if (strncasecmp(uri, "file:", 5) == 0) {
        ...
        if (strncmp(&uri[5], "//", 2) == 0) {
            ...
            if (strncasecmp(&uri[7], "localhost/", 10) == 0) {
                ...
                Otls_STOREerr(Otls_STORE_F_FILE_OPEN,
                              Otls_STORE_R_URI_AUTHORITY_UNSUPPORTED);
                return NULL;
            }
        }
    ...
    for (i = 0, path = NULL; path == NULL && i < path_data_n; i++) {
        ...
        if (path_data[i].check_absolute && path_data[i].path[0] != '/') {
            Otls_STOREerr(Otls_STORE_F_FILE_OPEN,
                          Otls_STORE_R_PATH_MUST_BE_ABSOLUTE);
    ...
    if (ctx == NULL) {
        Otls_STOREerr(Otls_STORE_F_FILE_OPEN, ERR_R_MALLOC_FAILURE);
        return NULL;
    }
    ...
    if (S_ISDIR(st.st_mode)) {
         ...
         */
        ctx->_.dir.uri = OPENtls_strdup(uri);
        ctx->type = is_dir;

        if (ctx->_.dir.uri == NULL)
            goto err;

        ctx->_.dir.last_entry = OPENtls_DIR_read(&ctx->_.dir.ctx, path);
        ctx->_.dir.last_errno = errno;
        if (ctx->_.dir.last_entry == NULL) {
            if (ctx->_.dir.last_errno != 0) {
                char errbuf[256];
                Otls_STOREerr(Otls_STORE_F_FILE_OPEN, ERR_R_SYS_LIB);
    ...
    return ctx;
 err:
    Otls_STORE_LOADER_CTX_free(ctx);
    return NULL;
}

static int file_ctrl(Otls_STORE_LOADER_CTX *ctx, int cmd, va_list args)
{
    int ret = 1;

    switch (cmd) {
    case Otls_STORE_C_USE_SECMEM:
        {
            ...
            case 1:
                ...
                break;
            default:
                Otls_STOREerr(Otls_STORE_F_FILE_CTRL,
...
static int file_expect(Otls_STORE_LOADER_CTX *ctx, int expected)
{
    ctx->expected_type = expected;
    return 1;
}

static int file_find(Otls_STORE_LOADER_CTX *ctx,
                     const Otls_STORE_SEARCH *search)
{
    /*
    ...
    if (Otls_STORE_SEARCH_get_type(search) == Otls_STORE_SEARCH_BY_NAME) {
        ...
        if (ctx->type != is_dir) {
            Otls_STOREerr(Otls_STORE_F_FILE_FIND,
                          Otls_STORE_R_SEARCH_ONLY_SUPPORTED_FOR_DIRECTORIES);
            return 0;
        }

        hash = X509_NAME_hash(Otls_STORE_SEARCH_get0_name(search));
    ...
    if (ctx != NULL)
        Otls_STOREerr(Otls_STORE_F_FILE_FIND,
                      Otls_STORE_R_UNSUPPORTED_SEARCH_TYPE);
    return 0;
}

/* Internal function to decode an already opened PEM file */
Otls_STORE_LOADER_CTX *otls_store_file_attach_pem_bio_int(BIO *bp)
{
    Otls_STORE_LOADER_CTX *ctx = OPENtls_zalloc(sizeof(*ctx));

    if (ctx == NULL) {
        Otls_STOREerr(Otls_STORE_F_Otls_STORE_FILE_ATTACH_PEM_BIO_INT,
                      ERR_R_MALLOC_FAILURE);
        return NULL;
    }
...
static Otls_STORE_INFO *file_load_try_decode(Otls_STORE_LOADER_CTX *ctx,
                                             const char *pem_name,
                                             const char *pem_header,
 ...
 again:
    {
        ...
        const FILE_HANDLER **matching_handlers =
            OPENtls_zalloc(sizeof(*matching_handlers)
                           * Otls_NELEM(file_handlers));

        if (matching_handlers == NULL) {
            Otls_STOREerr(Otls_STORE_F_FILE_LOAD_TRY_DECODE,
                          ERR_R_MALLOC_FAILURE);
            goto err;
        }

        *matchcount = 0;
        for (i = 0; i < Otls_NELEM(file_handlers); i++) {
            const FILE_HANDLER *handler = file_handlers[i];
            int try_matchcount = 0;
            void *tmp_handler_ctx = NULL;
            Otls_STORE_INFO *tmp_result =
            ...
            if (try_matchcount > 0) {
                ...
                if ((*matchcount += try_matchcount) > 1) {
                    ...
                    Otls_STORE_INFO_free(result);
                    Otls_STORE_INFO_free(tmp_result);
        ...
        OPENtls_free(matching_handlers);
    }

 err:
    OPENtls_free(new_pem_name);
    BUF_MEM_free(new_mem);

    if (result != NULL
        && (t = Otls_STORE_INFO_get_type(result)) == Otls_STORE_INFO_EMBEDDED) {
        pem_name = new_pem_name =
            otls_store_info_get0_EMBEDDED_pem_name(result);
        new_mem = otls_store_info_get0_EMBEDDED_buffer(result);
...
static Otls_STORE_INFO *file_load_try_repeat(Otls_STORE_LOADER_CTX *ctx,
                                             const UI_METHOD *ui_method,
                                             void *ui_data)
{
    Otls_STORE_INFO *result = NULL;
...
{
    if (secure)
        OPENtls_secure_clear_free(pem_data, num);
    else
        OPENtls_free(pem_data);
}
...
static int file_name_to_uri(Otls_STORE_LOADER_CTX *ctx, const char *name,
                            char **data)
{
    assert(name != NULL);
    assert(data != NULL);
    {
        ...
        *data = OPENtls_zalloc(calculated_length);
        if (*data == NULL) {
            Otls_STOREerr(Otls_STORE_F_FILE_NAME_TO_URI, ERR_R_MALLOC_FAILURE);
            return 0;
        }

        OPENtls_strlcat(*data, ctx->_.dir.uri, calculated_length);
        OPENtls_strlcat(*data, pathsep, calculated_length);
        OPENtls_strlcat(*data, name, calculated_length);
    }
    return 1;
}

static int file_name_check(Otls_STORE_LOADER_CTX *ctx, const char *name)
{
    const char *p = NULL;
    ...
    if (ctx->expected_type != 0
        && ctx->expected_type != Otls_STORE_INFO_CERT
        && ctx->expected_type != Otls_STORE_INFO_CRL)
        return 0;
    ...
    if (*p == 'r') {
        ...
        if (ctx->expected_type != 0
            && ctx->expected_type != Otls_STORE_INFO_CRL)
            return 0;
    } else if (ctx->expected_type == Otls_STORE_INFO_CRL) {
        return 0;
    }
...
static int file_eof(Otls_STORE_LOADER_CTX *ctx);
static int file_error(Otls_STORE_LOADER_CTX *ctx);
static Otls_STORE_INFO *file_load(Otls_STORE_LOADER_CTX *ctx,
                                  const UI_METHOD *ui_method, void *ui_data)
{
    Otls_STORE_INFO *result = NULL;
    ...
    if (ctx->type == is_dir) {
        do {
            ...
            if (ctx->_.dir.last_entry == NULL) {
                if (!ctx->_.dir.end_reached) {
                    ...
                    Otls_STOREerr(Otls_STORE_F_FILE_LOAD, ERR_R_SYS_LIB);
                    errno = ctx->_.dir.last_errno;
                    ctx->errcnt++;
                    if (opentls_strerror_r(errno, errbuf, sizeof(errbuf)))
                        ERR_add_error_data(1, errbuf);
            ...
            /*
             * On the first call (with a NULL context), OPENtls_DIR_read()
             * cares about the second argument.  On the following calls, it
             * only cares that it isn't NULL.  Therefore, we can safely give
             * it our URI here.
             */
            ctx->_.dir.last_entry = OPENtls_DIR_read(&ctx->_.dir.ctx,
                                                     ctx->_.dir.uri);
            ctx->_.dir.last_errno = errno;
            ...
            if (newname != NULL
                && (result = Otls_STORE_INFO_new_NAME(newname)) == NULL) {
                OPENtls_free(newname);
                Otls_STOREerr(Otls_STORE_F_FILE_LOAD, ERR_R_Otls_STORE_LIB);
                return NULL;
            }
     ...
     again:
        ...
        do {
             ...
             * badly coded.
             */
            if (!otls_assert(pem_name == NULL || matchcount <= 1)) {
                ctx->errcnt++;
                goto endloop;
            }

            if (matchcount > 1) {
                Otls_STOREerr(Otls_STORE_F_FILE_LOAD,
                              Otls_STORE_R_AMBIGUOUS_CONTENT_TYPE);
                ...
                if (ERR_peek_error() == 0) {
                    Otls_STOREerr(Otls_STORE_F_FILE_LOAD,
                                  Otls_STORE_R_UNSUPPORTED_CONTENT_TYPE);
                    if (pem_name != NULL)
        ...
        if (result != NULL
            && ctx->expected_type != 0
            && ctx->expected_type != Otls_STORE_INFO_get_type(result)) {
            Otls_STORE_INFO_free(result);
            goto again;
        }
    }
...
static int file_close(Otls_STORE_LOADER_CTX *ctx)
{
    if (ctx->type == is_dir) {
        OPENtls_DIR_end(&ctx->_.dir.ctx);
    } else {
...
int otls_store_file_detach_pem_bio_int(Otls_STORE_LOADER_CTX *ctx)
{
    Otls_STORE_LOADER_CTX_free(ctx);
    return 1;
}

static Otls_STORE_LOADER file_loader =
    {
        "file",
        NULL,
        file_open,
...
{
    otls_store_unregister_loader_int(file_loader.scheme);
}

int otls_store_file_loader_init(void)
{
    int ret = otls_store_register_loader_int(&file_loader);

    OPENtls_atexit(store_file_loader_deinit);
    return ret;
}
