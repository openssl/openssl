...
static int try_key(struct extracted_param_data_st *, OSSL_STORE_INFO **,
		   OSSL_STORE_CTX *, const OSSL_PROVIDER *, OSSL_LIB_CTX *,
		   const char *);
...
int ossl_store_handle_load_result(const OSSL_PARAM params[], void *arg)
{
	...
	if ((p = OSSL_PARAM_locate_const(params, OSSL_OBJECT_PARAM_TYPE)) !=
		    NULL &&
	...
	if (p != NULL && !OSSL_PARAM_get_octet_string_ptr(
				 p, &helper_data.ref, &helper_data.ref_size))
		return 0;
	...
	if (*v == NULL) {
		...
		if (provider != NULL)
			ERR_raise_data(ERR_LIB_OSSL_STORE, ERR_R_UNSUPPORTED,
				       "provider=%s%s",
			     ...
			     OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	while (keymgmt != NULL && keydata == NULL && try_fallback-- > 0) {
		...
		if (EVP_KEYMGMT_get0_provider(keymgmt) == provider) {
			...
			try_fallback = 0;
			keydata = evp_keymgmt_load(keymgmt, data->ref,
						   data->ref_size);
		} else {
		...
		if (keydata == NULL && try_fallback > 0) {
			...
			keymgmt = evp_keymgmt_fetch_from_prov(
				(OSSL_PROVIDER *)provider, data->data_type,
				propq);
			if (keymgmt != NULL) {
				ERR_pop_to_mark();
				ERR_set_mark();
			       ...
			       int *harderr)
{
	...
	decoderctx = OSSL_DECODER_CTX_new_for_pkey(&pk, data->input_type,
						   data->data_structure,
						   data->data_type, selection,
						   libctx, propq);
				      ...
				      OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if (pk == NULL && (ctx->expected_type == 0 ||
		   ...
		   OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	if (data->object_type == OSSL_OBJECT_UNKNOWN ||
		...
		if (data->object_type == OSSL_OBJECT_PKEY &&
		    data->ref != NULL) {
			...
			if (pk == NULL && harderr == 0)
				pk = try_key_value_legacy(data, &store_info_new,
							  ctx, cb, cbarg,
							  libctx, propq);
		}
		...
		if (pk != NULL) {
			...
			if (store_info_new == NULL) {
                 ...
                 */
				if (evp_keymgmt_util_has(
					    pk,
					    OSSL_KEYMGMT_SELECT_PRIVATE_KEY))
					store_info_new =
						OSSL_STORE_INFO_new_PKEY;
				else if (evp_keymgmt_util_has(
						 pk,
		   ...
		   OSSL_LIB_CTX *libctx, const char *propq)
{
	if (data->object_type == OSSL_OBJECT_UNKNOWN ||
		...
		if (crl != NULL &&
		    !ossl_x509_crl_set0_libctx(crl, libctx, propq)) {
			X509_CRL_free(crl);
			crl = NULL;
		}
...
static int try_pkcs12(struct extracted_param_data_st *data, OSSL_STORE_INFO **v,
		      OSSL_STORE_CTX *ctx, OSSL_LIB_CTX *libctx,
		      const char *propq)
{
	int ok = 1;
	...
	if (data->object_type == OSSL_OBJECT_UNKNOWN) {
		...
		if (p12 != NULL) {
			...
			if (!PKCS12_mac_present(p12) ||
				...
				if (!ossl_pw_get_passphrase(
					    tpass, sizeof(tpass) - 1,
					    &tpass_len, pw_params, 0,
					    &ctx->pwdata)) {
					ERR_raise(
						ERR_LIB_OSSL_STORE,
			...
			if (PKCS12_parse(p12, pass, &pkey, &cert, &chain)) {
				...
				if ((infos = sk_OSSL_STORE_INFO_new_null()) !=
				    NULL) {
					if (pkey != NULL) {
						if ((osi_pkey =
							     OSSL_STORE_INFO_new_PKEY(
