	...
	if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
		...
		if (plen != NO_PAYLOAD_LENGTH) { /* "TLS" mode of operation */
			...
			if ((key->aux.tls_aad[plen - 4] << 8 |
				...
				if (len <
				    (AES_BLOCK_SIZE + SHA_DIGEST_LENGTH + 1))
					return 0;
...
#if defined(STITCHED_DECRYPT_CALL)
			if (stitch) {
				blocks = (len - (256 + 32 + SHA_CBLOCK)) /
					 SHA_CBLOCK;
				aes_off = len - AES_BLOCK_SIZE -
					  blocks * SHA_CBLOCK;
				sha_off = SHA_CBLOCK - plen;

				aesni_cbc_encrypt(in, out, aes_off, &key->ks,
						  ctx->iv, 0);
			...
			if (len >= 1024 && keylen == 32) {
				if (sha_off %= SHA_CBLOCK)
					blocks = (len - 3 * SHA_CBLOCK) /
						 SHA_CBLOCK;
				else
					blocks = (len - 2 * SHA_CBLOCK) /
						 SHA_CBLOCK;
