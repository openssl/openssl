				...
				const OSSL_PARAM params[], ENGINE *impl)
{
	...
	if (ctx->pctx != NULL && EVP_PKEY_CTX_IS_SIGNATURE_OP(ctx->pctx) &&
		...
		if (ctx->pctx->operation == EVP_PKEY_OP_VERIFYCTX)
			return EVP_DigestVerifyInit(ctx, NULL, type, impl,
						    NULL);
	...
	if (ctx->algctx == NULL) {
		ctx->algctx =
			ctx->digest->newctx(ossl_provider_ctx(type->prov));
		if (ctx->algctx == NULL) {
			ERR_raise(ERR_LIB_EVP, EVP_R_INITIALIZATION_ERROR);
			return 0;
		}
...
int EVP_DigestFinalXOF(EVP_MD_CTX *ctx, unsigned char *md, size_t size)
{
     ...
     */
	params[i++] =
		OSSL_PARAM_construct_size_t(OSSL_DIGEST_PARAM_XOFLEN, &size);
...
int EVP_MD_CTX_set_params(EVP_MD_CTX *ctx, const OSSL_PARAM params[])
{
	...
	if (ossl_unlikely(pctx != NULL) &&
		...
		return pctx->op.sig.signature->set_ctx_md_params(
			pctx->op.sig.algctx, params);

	if (ossl_likely(ctx->digest != NULL &&
			ctx->digest->set_ctx_params != NULL))
...
int EVP_MD_CTX_get_params(EVP_MD_CTX *ctx, OSSL_PARAM params[])
{
	...
	if (pctx != NULL &&
		...
		return pctx->op.sig.signature->get_ctx_md_params(
			pctx->op.sig.algctx, params);
...
int EVP_MD_CTX_ctrl(EVP_MD_CTX *ctx, int cmd, int p1, void *p2)
{
	...
	case EVP_CTRL_SSL3_MASTER_SECRET:
		params[0] = OSSL_PARAM_construct_octet_string(
			OSSL_DIGEST_PARAM_SSL3_MS, p2, p1);
		break;
	default:
		goto conclude;
	}
...
static int evp_md_cache_constants(EVP_MD *md)
{
	...
	params[0] = OSSL_PARAM_construct_size_t(OSSL_DIGEST_PARAM_BLOCK_SIZE,
						&blksz);
	params[1] =
		OSSL_PARAM_construct_size_t(OSSL_DIGEST_PARAM_SIZE, &mdsize);
...
{
	EVP_MD *md = evp_generic_fetch(ctx, OSSL_OP_DIGEST, algorithm,
			    ...
			    void (*fn)(EVP_MD *mac, void *arg), void *arg)
{
	evp_generic_do_all(libctx, OSSL_OP_DIGEST, (void (*)(void *, void *))fn,
			   arg, evp_md_from_algorithm, evp_md_up_ref,
			   evp_md_free);
}
