...
int EVP_PKEY_get_security_bits(const EVP_PKEY *pkey)
{
	...
	if (pkey != NULL) {
		...
		if (pkey->ameth != NULL &&
		    pkey->ameth->pkey_security_bits != NULL)
...
int EVP_PKEY_copy_parameters(EVP_PKEY *to, const EVP_PKEY *from)
{
...
#endif /* !FIPS_MODULE */
		{
			if (EVP_PKEY_set_type_by_keymgmt(to, from->keymgmt) ==
			    0)
				goto end;
		}
	}
	...
	if (to->keymgmt != NULL && from->keymgmt != NULL) {
		ok = evp_keymgmt_util_copy(to, (EVP_PKEY *)from,
					   SELECT_PARAMETERS);
		goto end;
	}
	...
	if (to->keymgmt != NULL && to->keydata == NULL) {
		...
		void *from_keydata = evp_pkey_export_to_provider(
			(EVP_PKEY *)from, NULL, &to_keymgmt, NULL);
		...
		if (from_keydata == NULL)
			...
			ok = (to->keydata = evp_keymgmt_dup(
				      to->keymgmt, from_keydata,
...
static int evp_pkey_cmp_any(const EVP_PKEY *a, const EVP_PKEY *b, int selection)
{
	...
	if (keymgmt2 != NULL && keymgmt2->match != NULL) {
		tmp_keydata = evp_pkey_export_to_provider((EVP_PKEY *)a, NULL,
							  &keymgmt2, NULL);
		if (tmp_keydata != NULL) {
			keymgmt1 = keymgmt2;
	...
	if (tmp_keydata == NULL && keymgmt1 != NULL &&
	    keymgmt1->match != NULL) {
		tmp_keydata = evp_pkey_export_to_provider((EVP_PKEY *)b, NULL,
							  &keymgmt1, NULL);
		if (tmp_keydata != NULL) {
			keymgmt2 = keymgmt1;
...
int EVP_PKEY_eq(const EVP_PKEY *a, const EVP_PKEY *b)
{
...
#endif /* !FIPS_MODULE */
	{
		...
		if (evp_keymgmt_util_has((EVP_PKEY *)a,
					 OSSL_KEYMGMT_SELECT_PUBLIC_KEY) &&
		    evp_keymgmt_util_has((EVP_PKEY *)b,
					 OSSL_KEYMGMT_SELECT_PUBLIC_KEY))
...
#ifndef FIPS_MODULE
static EVP_PKEY *new_raw_key_int(OSSL_LIB_CTX *libctx, const char *strtype,
				 const char *propq, int nidtype, ENGINE *e,
				 ...
				 int key_is_priv)
{
	...
	if (e == NULL && ameth == NULL) {
		...
		ctx = EVP_PKEY_CTX_new_from_name(
			libctx, strtype != NULL ? strtype : OBJ_nid2sn(nidtype),
			propq);
		if (ctx == NULL)
			goto err;
		...
		if (EVP_PKEY_fromdata_init(ctx) == 1) {
			...
			if (EVP_PKEY_fromdata(ctx, &pkey, EVP_PKEY_KEYPAIR,
					      params) != 1) {
...
{
	return new_raw_key_int(libctx, keytype, propq, EVP_PKEY_NONE, NULL,
			       priv, len, 1);
}
...
static int get_raw_key_details(const OSSL_PARAM params[], void *arg)
{
	...
	if (raw_key->selection == OSSL_KEYMGMT_SELECT_PRIVATE_KEY) {
		if ((p = OSSL_PARAM_locate_const(
			     params, OSSL_PKEY_PARAM_PRIV_KEY)) != NULL)
			return OSSL_PARAM_get_octet_string(
				p, (void **)raw_key->key,
				raw_key->key == NULL ? 0 : *raw_key->len,
				raw_key->len);
	} else if (raw_key->selection == OSSL_KEYMGMT_SELECT_PUBLIC_KEY) {
		if ((p = OSSL_PARAM_locate_const(
			     params, OSSL_PKEY_PARAM_PUB_KEY)) != NULL)
			return OSSL_PARAM_get_octet_string(
				p, (void **)raw_key->key,
				raw_key->key == NULL ? 0 : *raw_key->len,
				raw_key->len);
				 ...
				 size_t *len)
{
	if (pkey->keymgmt != NULL) {
		...
		return evp_keymgmt_util_export(pkey,
	...
	*p++ = OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_CIPHER,
	...
	if (propq != NULL)
		*p++ = OSSL_PARAM_construct_utf8_string(
			OSSL_PKEY_PARAM_PROPERTIES, (char *)propq, 0);
#ifndef OPENSSL_NO_ENGINE
	if (engine_id != NULL)
...
int EVP_PKEY_assign(EVP_PKEY *pkey, int type, void *key)
{
	...
	if ((key != NULL) &&
	    (pktype == EVP_PKEY_EC || pktype == EVP_PKEY_SM2)) {
...
static const OSSL_ITEM standard_name2type[] = {
	{ EVP_PKEY_RSA, "RSA" },     { EVP_PKEY_RSA_PSS, "RSA-PSS" },
	{ EVP_PKEY_EC, "EC" },	     { EVP_PKEY_ED25519, "ED25519" },
	{ EVP_PKEY_ED448, "ED448" }, { EVP_PKEY_X25519, "X25519" },
	{ EVP_PKEY_X448, "X448" },   { EVP_PKEY_SM2, "SM2" },
	{ EVP_PKEY_DH, "DH" },	     { EVP_PKEY_DHX, "X9.42 DH" },
	{ EVP_PKEY_DHX, "DHX" },     { EVP_PKEY_DSA, "DSA" },
};
	...
	if (pkey->keymgmt == NULL) {
		...
		const char *supported_sig =
			pkey->keymgmt->query_operation_name != NULL ?
				pkey->keymgmt->query_operation_name(
					OSSL_OP_SIGNATURE) :
...
{
	return print_pkey(
		pkey, out, indent, EVP_PKEY_PRIVATE_KEY, NULL,
		(pkey->ameth != NULL ? pkey->ameth->priv_print : NULL), pctx);
}
...
{
	return print_pkey(
		pkey, out, indent, EVP_PKEY_KEY_PARAMETERS, NULL,
		(pkey->ameth != NULL ? pkey->ameth->param_print : NULL), pctx);
}
...
static int legacy_asn1_ctrl_to_param(EVP_PKEY *pkey, int op, int arg1,
				     void *arg2)
{
	if (pkey->keymgmt == NULL)
		return 0;
	...
	case ASN1_PKEY_CTRL_DEFAULT_MD_NID: {
		...
		if (rv > 0) {
			...
			/*
                 ...
                 * associated nid
                 */
			if (!ossl_namemap_doall_names(namemap, mdnum,
						      mdname2nid, &nid))
				return 0;
			*(int *)arg2 = nid;
		}
...
{
	if (pkey->ameth == NULL)
		return evp_keymgmt_util_get_deflt_digest_name(
			pkey->keymgmt, pkey->keydata, mdname, mdname_sz);
					...
					const char *name, const char *propq)
{
	...
	ERR_set_mark();
	rv = EVP_DigestSignInit_ex(ctx, NULL, name, libctx, propq, pkey, NULL);
	ERR_pop_to_mark();
				     ...
				     size_t publen)
{
	...
	if (evp_pkey_is_provided(pkey))
#endif /* !FIPS_MODULE */
		return EVP_PKEY_set_octet_string_param(
...
size_t EVP_PKEY_get1_encoded_public_key(EVP_PKEY *pkey, unsigned char **ppub)
{
...
#endif
	{
		...
		EVP_PKEY_get_octet_string_param(
			pkey, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, NULL, 0,
			&return_size);
		...
		*ppub = NULL;
		...
		if (!EVP_PKEY_get_octet_string_param(
			    pkey, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, buf,
			    return_size, NULL)) {
			OPENSSL_free(buf);
			return 0;
		}
...
#ifndef FIPS_MODULE
	{
		int rv = evp_pkey_asn1_ctrl(pkey, ASN1_PKEY_CTRL_GET1_TLS_ENCPT,
					    0, ppub);
		if (rv <= 0)
			return 0;
		return rv;
			 ...
			 int len, EVP_KEYMGMT *keymgmt)
{
	...
	if (pkey != NULL) {
         ...
         */
		if (pkey->type != EVP_PKEY_NONE && type == pkey->save_type &&
		    pkey->ameth != NULL)
			return 1;
#ifndef OPENSSL_NO_ENGINE
...
#endif
	return pkey_set_type(pkey, NULL, EVP_PKEY_NONE, EVP_PKEY_TYPE_STR,
			     EVP_PKEY_TYPE_STRLEN, keymgmt);
...
#ifndef FIPS_MODULE
	/* copy auxiliary data */
	if (!CRYPTO_dup_ex_data(CRYPTO_EX_INDEX_EVP_PKEY, &dup_pk->ex_data,
				&pkey->ex_data))
		goto err;

	if (pkey->attributes != NULL) {
		if ((dup_pk->attributes = ossl_x509at_dup(pkey->attributes)) ==
		    NULL)
			goto err;
	}
#endif /* !FIPS_MODULE */
				  ...
				  EVP_KEYMGMT **keymgmt, const char *propquery)
{
	...
	keydata =
		evp_keymgmt_util_export_to_provider(pk, tmp_keymgmt, selection);
...
int evp_pkey_copy_downgraded(EVP_PKEY **dest, const EVP_PKEY *src)
{
	...
	if (evp_pkey_is_assigned(src) && evp_pkey_is_provided(src)) {
		...
		if (!ossl_assert(type != EVP_PKEY_NONE)) {
			ERR_raise_data(
				ERR_LIB_EVP, ERR_R_INTERNAL_ERROR,
				    ...
				    size_t *out_len)
{
	...
	params[0] =
		OSSL_PARAM_construct_octet_string(key_name, buf, max_buf_sz);
	params[1] = OSSL_PARAM_construct_end();
			  ...
			  const BIGNUM *bn)
{
	...
	if (key_name == NULL || bn == NULL || pkey == NULL ||
				    ...
				    const unsigned char *buf, size_t bsize)
{
	...
	params[0] = OSSL_PARAM_construct_octet_string(
		key_name, (unsigned char *)buf, bsize);
	params[1] = OSSL_PARAM_construct_end();
	return EVP_PKEY_set_params(pkey, params);
}
...
int EVP_PKEY_set_params(EVP_PKEY *pkey, OSSL_PARAM params[])
{
	if (pkey != NULL) {
		if (evp_pkey_is_provided(pkey)) {
			pkey->dirty_cnt++;
			return evp_keymgmt_set_params(pkey->keymgmt,
						      pkey->keydata, params);
		}
#ifndef FIPS_MODULE
		/*
...
int EVP_PKEY_get_params(const EVP_PKEY *pkey, OSSL_PARAM params[])
{
	if (pkey != NULL) {
		if (evp_pkey_is_provided(pkey))
			return evp_keymgmt_get_params(pkey->keymgmt,
						      pkey->keydata,
						      params) > 0;
#ifndef FIPS_MODULE
		else if (evp_pkey_is_legacy(pkey))
...
int EVP_PKEY_get_ec_point_conv_form(const EVP_PKEY *pkey)
{
	...
	if (!EVP_PKEY_get_utf8_string_param(
		    pkey, OSSL_PKEY_PARAM_EC_POINT_CONVERSION_FORMAT, name,
