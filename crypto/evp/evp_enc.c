...
nonlegacy:
	...
	if (ctx->algctx == NULL) {
		ctx->algctx =
			ctx->cipher->newctx(ossl_provider_ctx(cipher->prov));
		if (ctx->algctx == NULL) {
			ERR_raise(ERR_LIB_EVP, EVP_R_INITIALIZATION_ERROR);
			return 0;
		}
	...
	if (enc) {
		...
		return ctx->cipher->einit(
			...
			iv, iv == NULL ? 0 : EVP_CIPHER_CTX_get_iv_length(ctx),
			params);
	}
...
legacy:
	...
	if (cipher != NULL) {
		...
		if (ctx->cipher->ctx_size) {
			ctx->cipher_data =
				OPENSSL_zalloc(ctx->cipher->ctx_size);
	...
	if (enc) {
		if (ctx->cipher->einit_skey == NULL) {
			...
			ret = ctx->cipher->einit_skey(
				ctx->algctx,
				skey == NULL ? NULL : skey->keydata, iv, iv_len,
				params);
		}
	} else {
...
int EVP_CipherInit_ex2(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher,
		       ...
		       int enc, const OSSL_PARAM params[])
{
	return evp_cipher_init_internal(ctx, cipher, NULL, key, iv, enc, 0,
					params);
}
...
int EVP_CipherInit_ex(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher,
		      ...
		      const unsigned char *iv, int enc)
{
	return evp_cipher_init_internal(ctx, cipher, impl, key, iv, enc, 0,
					NULL);
}
				  ...
				  size_t ivlen)
{
	...
	return ctx->cipher->p_einit(ctx->algctx, key, keylen, numpipes, iv,
				    ivlen, NULL);
}
				  ...
				  size_t ivlen)
{
	...
	return ctx->cipher->p_dinit(ctx->algctx, key, keylen, numpipes, iv,
				    ivlen, NULL);
}
		      ...
		      const unsigned char *in, int inl)
{
	...
	ret = ctx->cipher->cupdate(
		ctx->algctx, out, &soutl,
		inl_ + (size_t)(blocksize == 1 ? 0 : blocksize), in, inl_);
...
int EVP_CIPHER_CTX_set_key_length(EVP_CIPHER_CTX *c, int keylen)
{
	if (c->cipher->prov != NULL) {
		...
		if (OSSL_PARAM_locate_const(
			    EVP_CIPHER_settable_ctx_params(c->cipher),
	...
	if (c->cipher->flags & EVP_CIPH_CUSTOM_KEY_LENGTH)
		return EVP_CIPHER_CTX_ctrl(c, EVP_CTRL_SET_KEY_LENGTH, keylen,
					   NULL);
	if (EVP_CIPHER_CTX_get_key_length(c) == keylen)
		return 1;
...
int EVP_CIPHER_CTX_ctrl(EVP_CIPHER_CTX *ctx, int type, int arg, void *ptr)
{
	...
	case EVP_CTRL_SET_RC5_ROUNDS:
		...
		i = (unsigned int)arg;
		params[0] =
			OSSL_PARAM_construct_uint(OSSL_CIPHER_PARAM_ROUNDS, &i);
		break;
	case EVP_CTRL_SET_SPEED:
		...
		i = (unsigned int)arg;
		params[0] =
			OSSL_PARAM_construct_uint(OSSL_CIPHER_PARAM_SPEED, &i);
		break;
	...
	case EVP_CTRL_AEAD_TLS1_AAD:
		...
		ret = evp_do_ciph_ctx_setparams(ctx->cipher, ctx->algctx,
						params);
		if (ret <= 0)
			goto end;
		params[0] = OSSL_PARAM_construct_size_t(
			OSSL_CIPHER_PARAM_AEAD_TLS1_AAD_PAD, &sz);
		ret = evp_do_ciph_ctx_getparams(ctx->cipher, ctx->algctx,
						params);
		if (ret <= 0)
			goto end;
		if (sz > INT_MAX)
	...
	case EVP_CTRL_SET_RC2_KEY_BITS:
		params[0] = OSSL_PARAM_construct_size_t(
			OSSL_CIPHER_PARAM_RC2_KEYBITS, &sz);
		break;
...
const OSSL_PARAM *EVP_CIPHER_CTX_gettable_params(EVP_CIPHER_CTX *cctx)
{
	...
	if (cctx != NULL && cctx->cipher->gettable_ctx_params != NULL) {
		provctx = ossl_provider_ctx(
			EVP_CIPHER_get0_provider(cctx->cipher));
...
legacy:
	...
	if (in->cipher->flags & EVP_CIPH_CUSTOM_COPY)
		if (!in->cipher->ctrl((EVP_CIPHER_CTX *)in, EVP_CTRL_COPY, 0,
				      out)) {
			out->cipher = NULL;
			ERR_raise(ERR_LIB_EVP, EVP_R_INITIALIZATION_ERROR);
			return 0;
...
static void set_legacy_nid(const char *name, void *vlegacy_nid)
{
     ...
     */
	const void *legacy_method =
		OBJ_NAME_get(name, OBJ_NAME_TYPE_CIPHER_METH);
				       ...
				       OSSL_PROVIDER *prov)
{
	...
	int fnciphcnt = 0, encinit = 0, decinit = 0, fnpipecnt = 0,
	    fnctxcnt = 0;
	...
	for (; fns->function_id != 0; fns++) {
		...
		case OSSL_FUNC_CIPHER_ENCRYPT_SKEY_INIT:
			if (cipher->einit_skey != NULL)
				break;
			cipher->einit_skey =
				OSSL_FUNC_cipher_encrypt_skey_init(fns);
			encinit = 1;
			break;
		...
		case OSSL_FUNC_CIPHER_PIPELINE_ENCRYPT_INIT:
			if (cipher->p_einit != NULL)
				break;
			cipher->p_einit =
				OSSL_FUNC_cipher_pipeline_encrypt_init(fns);
			fnpipecnt++;
			break;
		case OSSL_FUNC_CIPHER_PIPELINE_DECRYPT_INIT:
			if (cipher->p_dinit != NULL)
				break;
			cipher->p_dinit =
				OSSL_FUNC_cipher_pipeline_decrypt_init(fns);
			fnpipecnt++;
			break;
		case OSSL_FUNC_CIPHER_PIPELINE_UPDATE:
			if (cipher->p_cupdate != NULL)
				break;
			cipher->p_cupdate =
				OSSL_FUNC_cipher_pipeline_update(fns);
			fnpipecnt++;
			break;
			     ...
			     const char *properties)
{
	EVP_CIPHER *cipher = evp_generic_fetch(
		ctx, OSSL_OP_CIPHER, algorithm, properties,
