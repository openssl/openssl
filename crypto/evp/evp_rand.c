				     ...
				     OSSL_PROVIDER *prov)
{
	...
	if ((rand->type_name = ossl_algorithm_get1_first_name(algodef)) ==
	    NULL) {
		evp_rand_free(rand);
		return NULL;
	}
...
EVP_RAND_CTX *EVP_RAND_CTX_new(EVP_RAND *rand, EVP_RAND_CTX *parent)
{
	...
	if ((ctx->algctx = rand->newctx(ossl_provider_ctx(rand->prov),
...
{
	evp_generic_do_all(libctx, OSSL_OP_RAND, (void (*)(void *, void *))fn,
			   arg, evp_rand_from_algorithm, evp_rand_up_ref,
			   evp_rand_free);
}
...
static int evp_rand_instantiate_locked(EVP_RAND_CTX *ctx, unsigned int strength,
				       ...
				       const OSSL_PARAM params[])
{
	return ctx->meth->instantiate(ctx->algctx, strength,
				      prediction_resistance, pstr, pstr_len,
				      params);
}
				 ...
				 size_t outlen)
{
	...
	if (ctx->meth->nonce != NULL)
		return ctx->meth->nonce(ctx->algctx, out, str, outlen, outlen) >
		       0;
				       ...
				       size_t adin_len)
{
	if (ctx->meth->get_seed != NULL)
		return ctx->meth->get_seed(ctx->algctx, buffer, entropy,
					   min_len, max_len,
