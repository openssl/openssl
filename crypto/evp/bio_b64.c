	...
	while (outl > 0) {
		...
		if (i <= 0) {
			...
			if (!BIO_should_retry(next)) {
				...
				if (ctx->tmp_len == 0) {
					if (EVP_DecodeFinal(ctx->base64, NULL,
							    &num) < 0)
						ret_code = -1;
					EVP_DecodeInit(ctx->base64);
				}
         ...
         */
		if (ctx->start &&
		    (BIO_get_flags(b) & BIO_FLAGS_BASE64_NO_NL) != 0) {
			ctx->tmp_len = 0;
		} else if (ctx->start) {
			q = p = ctx->tmp;
			num = 0;
			for (j = 0; j < i; j++) {
				...
				k = EVP_DecodeUpdate(ctx->base64, ctx->buf,
						     &num, p, (int)(q - p));
				EVP_DecodeInit(ctx->base64);
				if (k <= 0 && num == 0) {
					p = q;
		...
		if ((BIO_get_flags(b) & BIO_FLAGS_BASE64_NO_NL) != 0) {
			if (ctx->tmp_len > 0) {
				...
				ctx->buf_len = EVP_EncodeBlock(
					ctx->buf, ctx->tmp, ctx->tmp_len);
				if (!ossl_assert(ctx->buf_len <=
						 (int)sizeof(ctx->buf))) {
					ERR_raise(ERR_LIB_BIO,
						  ERR_R_INTERNAL_ERROR);
					return ret == 0 ? -1 : ret;
			...
			if (!EVP_EncodeUpdate(ctx->base64, ctx->buf,
					      &ctx->buf_len,
					      (unsigned char *)in, n))
				return ret == 0 ? -1 : ret;
			if (!ossl_assert(ctx->buf_len <=
