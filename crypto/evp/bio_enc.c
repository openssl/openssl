...
static int enc_read(BIO *b, char *out, int outl)
{
	...
	while (outl > 0) {
		...
		if (ctx->read_start ==
		    ctx->read_end) { /* time to read more data */
		...
		if (i <= 0) {
			...
			if (!BIO_should_retry(next)) {
				ctx->cont = i;
				ctx->finished = 1;
				i = EVP_CipherFinal_ex(ctx->cipher, ctx->buf,
						       &(ctx->buf_len));
				ctx->ok = i;
				ctx->buf_off = 0;
			} else {
			...
			if (i > ENC_MIN_CHUNK)
				i = ENC_MIN_CHUNK;
			if (!EVP_CipherUpdate(ctx->cipher, ctx->buf,
					      &ctx->buf_len, ctx->read_start,
					      i)) {
				BIO_clear_retry_flags(b);
				ctx->ok = 0;
				return 0;
...
static long enc_ctrl(BIO *b, int cmd, long num, void *ptr)
{
	...
	case BIO_CTRL_RESET:
		...
		if (!EVP_CipherInit_ex(
			    ctx->cipher, NULL, NULL, NULL, NULL,
		   ...
		   const unsigned char *i, int e)
{
	...
	if ((callback_ex = BIO_get_callback_ex(b)) != NULL) {
		if (callback_ex(b, BIO_CB_CTRL, (const char *)c, 0,
				BIO_CTRL_SET, e, 1, NULL) <= 0)
			return 0;
	}
#ifndef OPENSSL_NO_DEPRECATED_3_0
	else {
	...
	if (callback_ex != NULL)
		return callback_ex(b, BIO_CB_CTRL | BIO_CB_RETURN,
				   (const char *)c, 0, BIO_CTRL_SET, e, 1,
				   NULL);
#ifndef OPENSSL_NO_DEPRECATED_3_0
	else if (callback != NULL)
		return callback(b, BIO_CB_CTRL, (const char *)c, BIO_CTRL_SET,
				e, 1L);
#endif
	return 1;
}
