...
static void ctr64_inc(unsigned char *counter)
{
	...
	defined(_M_X64)
#define AES_GCM_ASM2(gctx)                               \
	(gctx->gcm.block == (block128_f)aesni_encrypt && \
			      ...
			      const unsigned char *iv, int enc)
{
	...
	if (key != NULL) {
		...
		do {
			...
			if (!CRYPTO_ocb128_init(&octx->ocb, &octx->ksenc.ks,
						&octx->ksdec.ks,
						(block128_f)aesni_encrypt,
		...
		if (iv == NULL && octx->iv_set)
			iv = octx->iv;
		if (iv) {
			if (CRYPTO_ocb128_setiv(&octx->ocb, iv, octx->ivlen,
						octx->taglen) != 1)
				return 0;
			octx->iv_set = 1;
		}
		...
		/* If key set use IV, otherwise copy */
		if (octx->key_set)
			CRYPTO_ocb128_setiv(&octx->ocb, iv, octx->ivlen,
					    octx->taglen);
		else
...
#define BLOCK_CIPHER_generic(nid, keylen, blocksize, ivlen, nmode, mode, MODE, \
			     flags)                                            \
	...
	{                                                                      \
		return AESNI_CAPABLE ? &aesni_##keylen##_##mode :              \
				       &aes_##keylen##_##mode;                 \
	}
...
#define BLOCK_CIPHER_custom(nid, keylen, blocksize, ivlen, mode, MODE, flags)  \
	static const EVP_CIPHER aesni_##keylen##_##mode = {                    \
		...
		(EVP_CIPH_##MODE##_MODE == EVP_CIPH_XTS_MODE ||                \
				 EVP_CIPH_##MODE##_MODE == EVP_CIPH_SIV_MODE ? \
			 2 :                                                   \
			 1) *                                                  \
			keylen / 8,                                            \
		ivlen,                                                         \
			   ...
			   const unsigned char *iv, int enc)
{
	...
	if ((mode == EVP_CIPH_ECB_MODE || mode == EVP_CIPH_CBC_MODE) && !enc) {
		...
		case 128:
			if (mode == EVP_CIPH_CBC_MODE)
				dat->stream.cbc =
					(cbc128_f)aes128_t4_cbc_encrypt;
			else if (mode == EVP_CIPH_CTR_MODE)
				dat->stream.ctr =
		...
		case 192:
			if (mode == EVP_CIPH_CBC_MODE)
				dat->stream.cbc =
					(cbc128_f)aes192_t4_cbc_encrypt;
			else if (mode == EVP_CIPH_CTR_MODE)
				dat->stream.ctr =
		...
		case 256:
			if (mode == EVP_CIPH_CBC_MODE)
				dat->stream.cbc =
					(cbc128_f)aes256_t4_cbc_encrypt;
			else if (mode == EVP_CIPH_CTR_MODE)
				dat->stream.ctr =
	...
	if (key != NULL) {
		...
		do {
			...
			if (!CRYPTO_ocb128_init(
				    &octx->ocb, &octx->ksenc.ks,
				    &octx->ksdec.ks, (block128_f)aes_t4_encrypt,
				    (block128_f)aes_t4_decrypt, NULL))
				return 0;
		} while (0);
...
#define BLOCK_CIPHER_generic(nid, keylen, blocksize, ivlen, nmode, mode, MODE, \
	...
	const EVP_CIPHER *EVP_aes_##keylen##_##mode(void)                      \
	{                                                                      \
		return SPARC_AES_CAPABLE ? &aes_t4_##keylen##_##mode :         \
					   &aes_##keylen##_##mode;             \
	}
...
static int s390x_aes_ecb_init_key(EVP_CIPHER_CTX *ctx, const unsigned char *key,
				  const unsigned char *iv, int enc)
{
...
static int s390x_aes_ofb_init_key(EVP_CIPHER_CTX *ctx, const unsigned char *key,
...
static int s390x_aes_cfb_init_key(EVP_CIPHER_CTX *ctx, const unsigned char *key,
				  const unsigned char *ivec, int enc)
{
			     ...
			     size_t len)
{
	...
	if (n) {
		...
		if (!n) {
			s390x_kma(ctx->ares, 16, NULL, 0, NULL, ctx->fc,
				  &ctx->kma.param);
			ctx->fc |= S390X_KMA_HS;
		}
		ctx->areslen = n;
			 ...
			 unsigned char *out, size_t len)
{
	...
	if (rem) {
		if (!ctx->mreslen) {
			...
			s390x_km(buf.b, 16, ctx->kres, ctx->fc & 0x1f,
				 &ctx->kma.param.k);
		}
 ...
 */
static void s390x_aes_gcm_setiv(S390X_AES_GCM_CTX *ctx, const unsigned char *iv)
{
	...
	if (ctx->ivlen == 12) {
		...
		s390x_kma(ctx->iv, S390X_gcm_ivpadlen(ctx->ivlen), NULL, 0,
			  NULL, ctx->fc, &ctx->kma.param);
		ctx->fc |= S390X_KMA_HS;
...
static int s390x_aes_gcm_ctrl(EVP_CIPHER_CTX *c, int type, int arg, void *ptr)
{
	...
	case EVP_CTRL_AEAD_SET_IVLEN:
		...
		if (arg != 12) {
			...
			*((unsigned long long *)(gctx->iv + len - 8)) = arg
									<< 3;
		}
		gctx->ivlen = arg;
		return 1;
 ...
 */
static int s390x_aes_gcm_init_key(EVP_CIPHER_CTX *ctx, const unsigned char *key,
				  const unsigned char *iv, int enc)
{
				...
				const unsigned char *in, size_t len)
{
	...
	if (in != NULL) {
		...
		s390x_kma(gctx->ares, gctx->areslen, gctx->mres, gctx->mreslen,
			  tmp, gctx->fc | S390X_KMA_LAAD | S390X_KMA_LPC,
			  &gctx->kma.param);
		...
		if (enc) {
			...
			if (CRYPTO_memcmp(buf, gctx->kma.param.t.b,
					  gctx->taglen))
				return -1;
		}
		return 0;
...
static int s390x_aes_xts_init_key(EVP_CIPHER_CTX *ctx, const unsigned char *key,
				  const unsigned char *iv, int enc);
...
static inline void s390x_aes_ccm_setiv(S390X_AES_CCM_CTX *ctx,
				       const unsigned char *nonce, size_t mlen)
{
			      ...
			      size_t alen)
{
	...
	alen &= ~(size_t)0xf;
	if (alen) {
		s390x_kmac(aad, alen, ctx->aes.ccm.fc,
			   &ctx->aes.ccm.kmac_param);
			 ...
			 unsigned char *out, size_t len, int enc)
{
	...
	if (!(flags & S390X_CCM_AAD_FLAG)) {
		s390x_km(ctx->aes.ccm.nonce.b, 16,
			 ctx->aes.ccm.kmac_param.icv.b, ctx->aes.ccm.fc,
			 ctx->aes.ccm.kmac_param.k);
		ctx->aes.ccm.blocks++;
	}
	...
	if (enc) {
		/* mac-then-encrypt */
		if (len)
			s390x_kmac(in, len, ctx->aes.ccm.fc,
				   &ctx->aes.ccm.kmac_param);
		if (rem) {
			for (i = 0; i < rem; i++)
		...
		CRYPTO_ctr128_encrypt_ctr32(in, out, len + rem, &ctx->aes.key.k,
					    ctx->aes.ccm.nonce.b,
					    ctx->aes.ccm.buf.b, &num,
					    (ctr128_f)AES_ctr32_encrypt);
	} else {
		/* decrypt-then-mac */
		CRYPTO_ctr128_encrypt_ctr32(in, out, len + rem, &ctx->aes.key.k,
					    ctx->aes.ccm.nonce.b,
		...
		if (len)
			s390x_kmac(out, len, ctx->aes.ccm.fc,
				   &ctx->aes.ccm.kmac_param);
		if (rem) {
			for (i = 0; i < rem; i++)
				ctx->aes.ccm.kmac_param.icv.b[i] ^=
					out[len + i];
				    ...
				    const unsigned char *in, size_t len)
{
     ...
     */
	memcpy(ivec + EVP_CCM_TLS_FIXED_IV_LEN, in,
	...
	if (enc) {
		...
		memcpy(out + len, cctx->aes.ccm.kmac_param.icv.b,
		       cctx->aes.ccm.m);
		return len + EVP_CCM_TLS_EXPLICIT_IV_LEN + cctx->aes.ccm.m;
	} else {
		if (!s390x_aes_ccm(cctx, in, out, len, enc)) {
			if (!CRYPTO_memcmp(cctx->aes.ccm.kmac_param.icv.b,
					   in + len, cctx->aes.ccm.m))
				return len;
		}
 ...
 * returned.
 */
static int s390x_aes_ccm_init_key(EVP_CIPHER_CTX *ctx, const unsigned char *key,
				  const unsigned char *iv, int enc)
{
	S390X_AES_CCM_CTX *cctx = EVP_C_DATA(S390X_AES_CCM_CTX, ctx);
	int keylen;
	...
	BLOCK_CIPHER_generic(nid, keylen, 16, 16, cbc, cbc, CBC,              \
		...
		BLOCK_CIPHER_generic(nid, keylen, 16, 0, ecb, ecb, ECB,       \
			...
			BLOCK_CIPHER_generic(                                 \
				nid, keylen, 1, 16, ofb128, ofb, OFB,         \
				flags | EVP_CIPH_FLAG_DEFAULT_ASN1)           \
				BLOCK_CIPHER_generic(                         \
					nid, keylen, 1, 16, cfb128, cfb, CFB, \
					flags | EVP_CIPH_FLAG_DEFAULT_ASN1)   \
					BLOCK_CIPHER_generic(nid, keylen, 1,  \
							     16, cfb1, cfb1,  \
							     CFB, flags)      \
						BLOCK_CIPHER_generic(         \
							nid, keylen, 1, 16,   \
			  ...
			  const unsigned char *in, size_t len)
{
	...
	CRYPTO_ofb128_encrypt(in, out, len, &dat->ks, ctx->iv, &num,
			      dat->block);
			   ...
			   const unsigned char *in, size_t len)
{
	...
	if (len) {
		int num = EVP_CIPHER_CTX_get_num(ctx);
		CRYPTO_cfb128_1_encrypt(in, out, len * 8, &dat->ks, ctx->iv,
...
static int aes_gcm_ctrl(EVP_CIPHER_CTX *c, int type, int arg, void *ptr)
{
	...
	case EVP_CTRL_COPY: {
		...
		if (gctx->iv == c->iv)
			...
			if ((gctx_out->iv = OPENSSL_malloc(gctx->ivlen)) ==
			    NULL)
				return 0;
...
#ifdef HWAES_CAPABLE
			if (HWAES_CAPABLE) {
				HWAES_set_encrypt_key(key, keylen,
						      &gctx->ks.ks);
				CRYPTO_gcm128_init(&gctx->gcm, &gctx->ks,
						   (block128_f)HWAES_encrypt);
#ifdef HWAES_ctr32_encrypt_blocks
				gctx->ctr =
					(ctr128_f)HWAES_ctr32_encrypt_blocks;
#else
				gctx->ctr = NULL;
#endif
				break;
...
#ifdef VPAES_CAPABLE
				if (VPAES_CAPABLE) {
				vpaes_set_encrypt_key(key, keylen,
						      &gctx->ks.ks);
				CRYPTO_gcm128_init(&gctx->gcm, &gctx->ks,
	...
	if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
		...
		if (gctx->ctr) {
			...
			if (len >= 32 && AES_GCM_ASM(gctx)) {
				if (CRYPTO_gcm128_encrypt(&gctx->gcm, NULL,
							  NULL, 0))
					return -1;
...
#endif
			if (CRYPTO_gcm128_encrypt_ctr32(&gctx->gcm, in + bulk,
							out + bulk, len - bulk,
							gctx->ctr))
				goto err;
		} else {
...
#endif
			if (CRYPTO_gcm128_encrypt(&gctx->gcm, in + bulk,
						  out + bulk, len - bulk))
				goto err;
		}
		out += len;
		...
		rv = (int)(len + EVP_GCM_TLS_EXPLICIT_IV_LEN +
			   EVP_GCM_TLS_TAG_LEN);
	} else {
		/* Decrypt */
		if (gctx->ctr) {
			...
			if (len >= 16 && AES_GCM_ASM(gctx)) {
				if (CRYPTO_gcm128_decrypt(&gctx->gcm, NULL,
							  NULL, 0))
					return -1;
...
#endif
			if (CRYPTO_gcm128_decrypt_ctr32(&gctx->gcm, in + bulk,
							out + bulk, len - bulk,
							gctx->ctr))
				goto err;
		} else {
...
#endif
			if (CRYPTO_gcm128_decrypt(&gctx->gcm, in + bulk,
						  out + bulk, len - bulk))
				goto err;
		}
		/* Retrieve tag */
	...
	if (in) {
		if (out == NULL) {
			...
			if (gctx->ctr) {
				...
				if (len >= 32 && AES_GCM_ASM(gctx)) {
					...
					bulk = AES_gcm_encrypt(
						in + res, out + res, len - res,
		...
		if (!EVP_CIPHER_CTX_is_encrypting(ctx)) {
			...
			if (CRYPTO_gcm128_finish(
				    &gctx->gcm, EVP_CIPHER_CTX_buf_noconst(ctx),
				    gctx->taglen) != 0)
				return -1;
			gctx->iv_set = 0;
			return 0;
...
BLOCK_CIPHER_custom(NID_aes, 128, 1, 12, gcm, GCM,
	...
	BLOCK_CIPHER_custom(NID_aes, 192, 1, 12, gcm, GCM,
		...
		BLOCK_CIPHER_custom(NID_aes, 256, 1, 12, gcm, GCM,
			...
			static int aes_xts_ctrl(EVP_CIPHER_CTX *c, int type,
						int arg, void *ptr)
{
			    ...
			    const unsigned char *iv, int enc)
{
	...
	if (key != NULL) {
		do {
			...
			if (HWAES_CAPABLE) {
				if (enc) {
					HWAES_set_encrypt_key(key, bits,
							      &xctx->ks1.ks);
					xctx->xts.block1 =
						(block128_f)HWAES_encrypt;
					...
					HWAES_set_decrypt_key(key, bits,
							      &xctx->ks1.ks);
					xctx->xts.block1 =
				...
				HWAES_set_encrypt_key(key + bytes, bits,
						      &xctx->ks2.ks);
...
#ifdef BSAES_CAPABLE
				if (BSAES_CAPABLE)
				xctx->stream = enc ? ossl_bsaes_xts_encrypt :
				...
				if (VPAES_CAPABLE) {
				if (enc) {
					vpaes_set_encrypt_key(key, bits,
							      &xctx->ks1.ks);
					xctx->xts.block1 =
						(block128_f)vpaes_encrypt;
				} else {
					vpaes_set_decrypt_key(key, bits,
							      &xctx->ks1.ks);
					xctx->xts.block1 =
						(block128_f)vpaes_decrypt;
				}

				vpaes_set_encrypt_key(key + bytes, bits,
						      &xctx->ks2.ks);
			  ...
			  const unsigned char *in, size_t len)
{
	...
	if (xctx->xts.key1 == NULL || xctx->xts.key2 == NULL || out == NULL ||
	    in == NULL || len < AES_BLOCK_SIZE)
		return 0;
...
BLOCK_CIPHER_custom(NID_aes, 128, 1, 16, xts, XTS, XTS_FLAGS)
	BLOCK_CIPHER_custom(NID_aes, 256, 1, 16, xts, XTS, XTS_FLAGS)
		...
		static int aes_ccm_ctrl(EVP_CIPHER_CTX *c, int type, int arg,
					void *ptr)
{
	...
	case EVP_CTRL_AEAD_TLS1_AAD:
		...
		{
			uint16_t len = EVP_CIPHER_CTX_buf_noconst(c)[arg - 2]
					       << 8 |
			   ...
			   const unsigned char *in, size_t inlen)
{
	...
	if (pad) {
		if (EVP_CIPHER_CTX_is_encrypting(ctx))
			rv = CRYPTO_128_wrap_pad(&wctx->ks.ks, wctx->iv, out,
						 in, inlen,
						 (block128_f)AES_encrypt);
		else
			rv = CRYPTO_128_unwrap_pad(&wctx->ks.ks, wctx->iv, out,
						   in, inlen,
						   (block128_f)AES_decrypt);
	} else {
		if (EVP_CIPHER_CTX_is_encrypting(ctx))
			rv = CRYPTO_128_wrap(&wctx->ks.ks, wctx->iv, out, in,
					     inlen, (block128_f)AES_encrypt);
		else
			rv = CRYPTO_128_unwrap(&wctx->ks.ks, wctx->iv, out, in,
					       inlen, (block128_f)AES_decrypt);
	}
	return rv ? (int)rv : -1;
}
...
static const EVP_CIPHER aes_128_wrap = { NID_id_aes128_wrap,
					 8,
					 16,
					 8,
					 WRAP_FLAGS,
...
static const EVP_CIPHER aes_192_wrap = { NID_id_aes192_wrap,
					 8,
					 24,
					 8,
					 WRAP_FLAGS,
...
static const EVP_CIPHER aes_256_wrap = { NID_id_aes256_wrap,
					 8,
					 32,
					 8,
					 WRAP_FLAGS,
...
static const EVP_CIPHER aes_192_wrap_pad = { NID_id_aes192_wrap_pad,
					     8,
					     24,
					     4,
					     WRAP_FLAGS,
...
static const EVP_CIPHER aes_256_wrap_pad = { NID_id_aes256_wrap_pad,
					     8,
					     32,
					     4,
					     WRAP_FLAGS,
	...
	if (key != NULL) {
		...
		do {
			...
			if (HWAES_CAPABLE) {
				HWAES_set_encrypt_key(key, keylen,
						      &octx->ksenc.ks);
				HWAES_set_decrypt_key(key, keylen,
						      &octx->ksdec.ks);
				if (!CRYPTO_ocb128_init(
					    &octx->ocb, &octx->ksenc.ks,
					    &octx->ksdec.ks,
			...
			if (VPAES_CAPABLE) {
				vpaes_set_encrypt_key(key, keylen,
						      &octx->ksenc.ks);
				vpaes_set_decrypt_key(key, keylen,
						      &octx->ksdec.ks);
				if (!CRYPTO_ocb128_init(
					    &octx->ocb, &octx->ksenc.ks,
					    &octx->ksdec.ks,
					    (block128_f)vpaes_encrypt,
					    (block128_f)vpaes_decrypt, NULL))
					return 0;
				break;
			}
#endif
			  ...
			  const unsigned char *in, size_t len)
{
	...
	if (in != NULL) {
		...
		if (out == NULL) {
			...
			if (ossl_is_partially_overlapping(out + *buf_len, in,
							  (int)len)) {
				ERR_raise(ERR_LIB_EVP,
		...
		if (*buf_len > 0) {
			...
			in += remaining;
			if (out == NULL) {
				if (!CRYPTO_ocb128_aad(&octx->ocb, buf,
						       AES_BLOCK_SIZE))
					return -1;
			} else if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
		...
		if (len != trailing_len) {
			if (out == NULL) {
				if (!CRYPTO_ocb128_aad(&octx->ocb, in,
						       len - trailing_len))
					return -1;
			} else if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
				if (!CRYPTO_ocb128_encrypt(&octx->ocb, in, out,
							   len - trailing_len))
					return -1;
			} else {
				if (!CRYPTO_ocb128_decrypt(&octx->ocb, in, out,
							   len - trailing_len))
					return -1;
			}
		...
		if (octx->data_buf_len > 0) {
			if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
				if (!CRYPTO_ocb128_encrypt(&octx->ocb,
							   octx->data_buf, out,
							   octx->data_buf_len))
					return -1;
			} else {
				if (!CRYPTO_ocb128_decrypt(&octx->ocb,
							   octx->data_buf, out,
							   octx->data_buf_len))
					return -1;
			}
		...
		if (octx->aad_buf_len > 0) {
			if (!CRYPTO_ocb128_aad(&octx->ocb, octx->aad_buf,
					       octx->aad_buf_len))
				return -1;
			octx->aad_buf_len = 0;
		}
		...
		if (!EVP_CIPHER_CTX_is_encrypting(ctx)) {
			if (octx->taglen < 0)
				return -1;
			if (CRYPTO_ocb128_finish(&octx->ocb, octx->tag,
						 octx->taglen) != 0)
				return -1;
			octx->iv_set = 0;
			return written_len;
