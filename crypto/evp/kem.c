			...
			const OSSL_PARAM params[], EVP_PKEY *authkey)
{
	...
	for (iter = 1, provkey = NULL; iter < 3 && provkey == NULL; iter++) {
		...
		switch (iter) {
		case 1:
			kem = EVP_KEM_fetch(ctx->libctx, supported_kem,
					    ctx->propquery);
			if (kem != NULL)
         ...
         */
		tmp_keymgmt_tofree = tmp_keymgmt = evp_keymgmt_fetch_from_prov(
			(OSSL_PROVIDER *)tmp_prov,
			EVP_KEYMGMT_get0_name(ctx->keymgmt), ctx->propquery);
		if (tmp_keymgmt != NULL) {
			provkey = evp_pkey_export_to_provider(ctx->pkey,
							      ctx->libctx,
							      &tmp_keymgmt,
							      ctx->propquery);
			if (provkey != NULL && authkey != NULL) {
				provauthkey = evp_pkey_export_to_provider(
					authkey, ctx->libctx, &tmp_keymgmt,
					ctx->propquery);
				if (provauthkey == NULL) {
					EVP_KEM_free(kem);
	...
	case EVP_PKEY_OP_ENCAPSULATE:
		if (provauthkey != NULL && kem->auth_encapsulate_init != NULL) {
			ret = kem->auth_encapsulate_init(ctx->op.encap.algctx,
							 provkey, provauthkey,
							 params);
		} else if (provauthkey == NULL &&
			   kem->encapsulate_init != NULL) {
			ret = kem->encapsulate_init(ctx->op.encap.algctx,
						    provkey, params);
		} else {
			ERR_raise(
				ERR_LIB_EVP,
	...
	case EVP_PKEY_OP_DECAPSULATE:
		if (provauthkey != NULL && kem->auth_decapsulate_init != NULL) {
			ret = kem->auth_decapsulate_init(ctx->op.encap.algctx,
							 provkey, provauthkey,
							 params);
		} else if (provauthkey == NULL &&
			   kem->encapsulate_init != NULL) {
			ret = kem->decapsulate_init(ctx->op.encap.algctx,
						    provkey, params);
		} else {
			ERR_raise(
				ERR_LIB_EVP,
			 ...
			 unsigned char *secret, size_t *secretlen)
{
	...
	return ctx->op.encap.kem->encapsulate(ctx->op.encap.algctx, out, outlen,
					      secret, secretlen);
}
...
int EVP_PKEY_decapsulate(EVP_PKEY_CTX *ctx, unsigned char *secret,
			 size_t *secretlen, const unsigned char *in,
			 size_t inlen)
{
	...
	return ctx->op.encap.kem->decapsulate(ctx->op.encap.algctx, secret,
					      secretlen, in, inlen);
}
...
static EVP_KEM *evp_kem_new(OSSL_PROVIDER *prov)
{
	...
	if (!CRYPTO_NEW_REF(&kem->refcnt, 1) || !ossl_provider_up_ref(prov)) {
				    ...
				    OSSL_PROVIDER *prov)
{
	...
	for (; fns->function_id != 0; fns++) {
		...
		case OSSL_FUNC_KEM_GET_CTX_PARAMS:
			if (kem->get_ctx_params != NULL)
				break;
			kem->get_ctx_params = OSSL_FUNC_kem_get_ctx_params(fns);
			gparamfncnt++;
			break;
		...
		case OSSL_FUNC_KEM_SET_CTX_PARAMS:
			if (kem->set_ctx_params != NULL)
				break;
			kem->set_ctx_params = OSSL_FUNC_kem_set_ctx_params(fns);
			sparamfncnt++;
			break;
	...
	if (ctxfncnt != 2 ||
	    ...
	    (encfncnt != decfncnt) || (gparamfncnt != 0 && gparamfncnt != 2) ||
		       ...
		       const char *properties)
{
	return evp_generic_fetch(ctx, OSSL_OP_KEM, algorithm, properties,
				 evp_kem_from_algorithm, evp_kem_up_ref,
				 evp_kem_free);
}
...
EVP_KEM *evp_kem_fetch_from_prov(OSSL_PROVIDER *prov, const char *algorithm,
				 const char *properties)
{
	return evp_generic_fetch_from_prov(prov, OSSL_OP_KEM, algorithm,
					   properties, evp_kem_from_algorithm,
					   evp_kem_up_ref, evp_kem_free);
}
...
{
	evp_generic_do_all(libctx, OSSL_OP_KEM, (void (*)(void *, void *))fn,
			   arg, evp_kem_from_algorithm, evp_kem_up_ref,
			   evp_kem_free);
}
