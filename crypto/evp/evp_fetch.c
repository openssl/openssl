...
{
	return ossl_lib_ctx_get_data(libctx,
...
static void *inner_evp_generic_fetch(
	struct evp_method_data_st *methdata, OSSL_PROVIDER *prov,
	int operation_id, const char *name, ossl_unused const char *properties,
	...
	int (*up_ref_method)(void *), void (*free_method)(void *))
{
	...
	if (meth_id == 0 || !ossl_method_store_cache_get(store, prov, meth_id,
							 propq, &method)) {
		...
		if ((method = ossl_method_construct(
			     methdata->libctx, operation_id, &prov,
			     0 /* !force_cache */, &mcm, methdata)) != NULL) {
			/*
			...
			void (*free_method)(void *))
{
	...
	method = inner_evp_generic_fetch(&methdata, NULL, operation_id, name,
	...
	int (*up_ref_method)(void *), void (*free_method)(void *))
{
	...
	methdata.tmp_store = NULL;
	method = inner_evp_generic_fetch(&methdata, prov, operation_id, name,
					 properties, new_method, up_ref_method,
					 free_method);
	dealloc_tmp_evp_method_store(methdata.tmp_store);
	return method;
					     ...
					     int loadconfig, int mirrored)
{
	...
	OSSL_PROPERTY_LIST **plp =
		ossl_ctx_global_properties(libctx, loadconfig);
				   ...
				   int loadconfig, int mirrored)
{
	...
	if (propq != NULL &&
	    (pl = ossl_parse_query(libctx, propq, 1)) == NULL) {
			...
			void (*free_method)(void *))
{
	...
	if (methdata.tmp_store != NULL)
		ossl_method_store_do_all(methdata.tmp_store,
