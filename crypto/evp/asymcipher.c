				     ...
				     const OSSL_PARAM params[])
{
	...
	supported_ciph = evp_keymgmt_util_query_operation_name(
		ctx->keymgmt, OSSL_OP_ASYM_CIPHER);
	...
	for (iter = 1, provkey = NULL; iter < 3 && provkey == NULL; iter++) {
		...
		switch (iter) {
		case 1:
			cipher = EVP_ASYM_CIPHER_fetch(
				ctx->libctx, supported_ciph, ctx->propquery);
			if (cipher != NULL)
				tmp_prov =
		...
		case 2:
			...
			cipher = evp_asym_cipher_fetch_from_prov(
				(OSSL_PROVIDER *)tmp_prov, supported_ciph,
				ctx->propquery);
			if (cipher == NULL)
				goto legacy;
			break;
	...
	case EVP_PKEY_OP_ENCRYPT:
		if (cipher->encrypt_init == NULL) {
			ERR_raise_data(ERR_LIB_EVP,
				       ...
				       "%s encrypt_init:%s", cipher->type_name,
				       desc);
			ret = -2;
			goto err;
		}
		ret = cipher->encrypt_init(ctx->op.ciph.algctx, provkey,
					   params);
		break;
	case EVP_PKEY_OP_DECRYPT:
		if (cipher->decrypt_init == NULL) {
			ERR_raise_data(ERR_LIB_EVP,
				       EVP_R_PROVIDER_ASYM_CIPHER_NOT_SUPPORTED,
				       "%s decrypt_init:%s", cipher->type_name,
				       desc);
			ret = -2;
			goto err;
		}
		ret = cipher->decrypt_init(ctx->op.ciph.algctx, provkey,
					   params);
		break;
	default:
		ERR_raise(ERR_LIB_EVP, EVP_R_INITIALIZATION_ERROR);
...
legacy:
	...
	M_check_autoarg(ctx, out, outlen, EVP_F_EVP_PKEY_ENCRYPT) return ctx
		->pmeth->encrypt(ctx, out, outlen, in, inlen);
}
...
legacy:
	...
	M_check_autoarg(ctx, out, outlen, EVP_F_EVP_PKEY_DECRYPT) return ctx
		->pmeth->decrypt(ctx, out, outlen, in, inlen);
}
...
{
	return evp_generic_fetch(ctx, OSSL_OP_ASYM_CIPHER, algorithm,
