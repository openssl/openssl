				...
				evp_cipher_aead_asn1_params *asn1_params)
{
	...
	} else if ((EVP_CIPHER_get_flags(cipher) & EVP_CIPH_FLAG_CUSTOM_ASN1) ==
		   0) {
		switch (EVP_CIPHER_get_mode(cipher)) {
		case EVP_CIPH_WRAP_MODE:
			if (EVP_CIPHER_is_a(cipher,
				    ...
				    evp_cipher_aead_asn1_params *asn1_params)
{
	...
	i = ossl_asn1_type_get_octetstring_int(type, &tl, NULL,
					       EVP_MAX_IV_LENGTH);
	if (i <= 0)
		return -1;
...
int evp_cipher_cache_constants(EVP_CIPHER *cipher)
{
	...
	params[0] = OSSL_PARAM_construct_size_t(OSSL_CIPHER_PARAM_BLOCK_SIZE,
						&blksz);
	params[1] =
	...
	if (ctx->iv_len < 0) {
		...
		if (ctx->cipher->get_ctx_params != NULL) {
			params[0] = OSSL_PARAM_construct_size_t(
				OSSL_CIPHER_PARAM_IVLEN, &v);
			rv = evp_do_ciph_ctx_getparams(ctx->cipher, ctx->algctx,
						       params);
			if (rv > 0) {
		...
		/* Code below to be removed when legacy support is dropped. */
			...
			rv = EVP_CIPHER_CTX_ctrl((EVP_CIPHER_CTX *)ctx,
						 EVP_CTRL_GET_IVLEN, 0, &len);
			if (rv <= 0)
				return -1;
		}
		/*-
...
int EVP_CIPHER_CTX_get_tag_length(const EVP_CIPHER_CTX *ctx)
{
	...
	params[0] =
		OSSL_PARAM_construct_size_t(OSSL_CIPHER_PARAM_AEAD_TAGLEN, &v);
...
const unsigned char *EVP_CIPHER_CTX_original_iv(const EVP_CIPHER_CTX *ctx)
{
	...
	params[0] = OSSL_PARAM_construct_octet_ptr(
		OSSL_CIPHER_PARAM_IV, (void **)&v, sizeof(ctx->oiv));
...
const unsigned char *EVP_CIPHER_CTX_iv(const EVP_CIPHER_CTX *ctx)
{
	...
	params[0] = OSSL_PARAM_construct_octet_ptr(
		OSSL_CIPHER_PARAM_UPDATED_IV, (void **)&v, sizeof(ctx->iv));
	ok = evp_do_ciph_ctx_getparams(ctx->cipher, ctx->algctx, params);
...
int EVP_CIPHER_CTX_get_updated_iv(EVP_CIPHER_CTX *ctx, void *buf, size_t len)
{
	...
	params[0] = OSSL_PARAM_construct_octet_string(
		OSSL_CIPHER_PARAM_UPDATED_IV, buf, len);
...
int EVP_CIPHER_CTX_get_original_iv(EVP_CIPHER_CTX *ctx, void *buf, size_t len)
{
	...
	params[0] = OSSL_PARAM_construct_octet_string(OSSL_CIPHER_PARAM_IV, buf,
						      len);
	return evp_do_ciph_ctx_getparams(ctx->cipher, ctx->algctx, params) > 0;
}
...
int EVP_CIPHER_CTX_get_key_length(const EVP_CIPHER_CTX *ctx)
{
	...
	if (ctx->key_len <= 0 && ctx->cipher->prov != NULL) {
		...
		params[0] = OSSL_PARAM_construct_size_t(
			OSSL_CIPHER_PARAM_KEYLEN, &len);
		ok = evp_do_ciph_ctx_getparams(ctx->cipher, ctx->algctx,
					       params);
		if (ok <= 0)
         ...
         * cache the result for performance reasons.
         */
		if (!OSSL_PARAM_get_int(params,
					&((EVP_CIPHER_CTX *)ctx)->key_len))
			return -1;
...
{
	if (cipher->prov != NULL)
		return evp_names_do_all(cipher->prov, cipher->name_id, fn,
					data);
...
int EVP_MD_CTX_get_size_ex(const EVP_MD_CTX *ctx)
{
	...
	if (gettables != NULL &&
	    OSSL_PARAM_locate_const(gettables, OSSL_DIGEST_PARAM_SIZE) !=
		    NULL) {
		OSSL_PARAM params[2] = { OSSL_PARAM_END, OSSL_PARAM_END };
		size_t sz = 0;
		...
		params[0] = OSSL_PARAM_construct_size_t(OSSL_DIGEST_PARAM_SIZE,
							&sz);
		if (EVP_MD_CTX_get_params(c, params) != 1 || sz > INT_MAX ||
		    sz == 0)
			return -1;
		return (int)sz;
	}
					  ...
					  unsigned int enable)
{
	...
	params[0] =
		OSSL_PARAM_construct_uint(OSSL_CIPHER_PARAM_USE_BITS, &enable);
...
int EVP_CIPHER_CTX_set_algor_params(EVP_CIPHER_CTX *ctx, const X509_ALGOR *alg)
{
	...
	if ((derl = i2d_ASN1_TYPE(alg->parameter, &der)) >= 0) {
         ...
         */
		params[0] = OSSL_PARAM_construct_octet_string(k_old, der,
							      (size_t)derl);
		params[1] = OSSL_PARAM_construct_octet_string(k_new, der,
							      (size_t)derl);
...
int EVP_CIPHER_CTX_get_algor(EVP_CIPHER_CTX *ctx, X509_ALGOR **alg)
{
	...
	if (aid_len == 0) {
		ERR_raise(ERR_LIB_EVP,
			  EVP_R_GETTING_ALGORITHMIDENTIFIER_NOT_SUPPORTED);
		ret = -2;
		goto err;
	}
