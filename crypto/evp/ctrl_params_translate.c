			 ...
			 const struct translation_ctx_st *ctx)
{
	...
	case PRE_CTRL_STR_TO_PARAMS:
         ...
         */
		if (translation != NULL) {
			if (!ossl_assert(translation->action_type !=
					 OSSL_ACTION_GET)) {
			      ...
			      struct translation_ctx_st *ctx)
{
	...
	default:
		...
		ERR_raise_data(ERR_LIB_EVP, ERR_R_SHOULD_NOT_HAVE_BEEN_CALLED,
			       "[action:%d, state:%d]", ctx->action_type,
			       state);
		return 0;
	...
	case PRE_CTRL_TO_PARAMS:
		...
		case OSSL_PARAM_INTEGER:
			*ctx->params = OSSL_PARAM_construct_int(
				translation->param_key, &ctx->p1);
			break;
		case OSSL_PARAM_UNSIGNED_INTEGER:
			...
			if (ctx->p2 != NULL) {
				if (ctx->action_type == OSSL_ACTION_SET) {
					...
					if ((ctx->allocated_buf = OPENSSL_malloc(
						     ctx->buflen)) == NULL)
						return 0;
					if (BN_bn2nativepad(
						    ctx->p2, ctx->allocated_buf,
						    (int)ctx->buflen) < 0) {
						OPENSSL_free(
							ctx->allocated_buf);
	...
	case PRE_CTRL_STR_TO_PARAMS: {
		...
		if (ctx->action_type != OSSL_ACTION_SET) {
			ERR_raise_data(
				ERR_LIB_EVP, EVP_R_COMMAND_NOT_SUPPORTED,
	...
	case PRE_PARAMS_TO_CTRL: {
		...
		if (state == PRE_PARAMS_TO_CTRL &&
			...
			switch (translation->param_data_type) {
			case OSSL_PARAM_INTEGER:
				return OSSL_PARAM_get_int(ctx->params,
							  &ctx->p1);
			case OSSL_PARAM_UNSIGNED_INTEGER:
				if (ctx->p2 != NULL) {
					/* BIGNUM passed down with p2 */
					if (!OSSL_PARAM_get_BN(ctx->params,
							       ctx->p2))
						return 0;
				} else {
				...
				return 1;
			case OSSL_PARAM_UTF8_STRING:
				return OSSL_PARAM_get_utf8_string(
					ctx->params, ctx->p2, ctx->sz);
			case OSSL_PARAM_OCTET_STRING:
				return OSSL_PARAM_get_octet_string(
					ctx->params, &ctx->p2, ctx->sz,
					(size_t *)&ctx->p1);
			case OSSL_PARAM_OCTET_PTR:
				return OSSL_PARAM_get_octet_ptr(
					ctx->params, ctx->p2, &ctx->sz);
			default:
				ERR_raise_data(
					ERR_LIB_EVP, ERR_R_UNSUPPORTED,
			...
			if (param_data_type == 0) {
				...
				if (!ossl_assert(translation->fixup_args !=
						 NULL)) {
					ERR_raise(ERR_LIB_EVP,
						  ERR_R_INTERNAL_ERROR);
					return 0;
			...
			case OSSL_PARAM_UNSIGNED_INTEGER:
				if (ctx->p2 != NULL) {
					/* BIGNUM passed back */
					return OSSL_PARAM_set_BN(ctx->params,
								 ctx->p2);
				} else {
					/* Normal C unsigned int passed back */
					return OSSL_PARAM_set_uint(
						ctx->params,
						(unsigned int)ctx->p1);
				}
				return 0;
			case OSSL_PARAM_UTF8_STRING:
				return OSSL_PARAM_set_utf8_string(ctx->params,
								  ctx->p2);
			case OSSL_PARAM_OCTET_STRING:
				return OSSL_PARAM_set_octet_string(
					ctx->params, ctx->p2, size);
			case OSSL_PARAM_OCTET_PTR:
			...
			default:
			   ...
			   ctx->action_type == OSSL_ACTION_GET) {
			if (translation->param_data_type ==
...
static int fix_md(enum state state, const struct translation_st *translation,
		  struct translation_ctx_st *ctx)
{
	return fix_cipher_md(state, translation, ctx, get_md_name,
			     get_md_by_name);
}
	...
	if ((state == POST_CTRL_TO_PARAMS &&
		...
		for (; kdf_type_map->kdf_type_str != NULL; kdf_type_map++)
			if (OPENSSL_strcasecmp(
				    ctx->p2, kdf_type_map->kdf_type_str) == 0) {
				...
				struct translation_ctx_st *ctx)
{
	...
	if (state == PRE_CTRL_STR_TO_PARAMS) {
		if ((ctx->p2 = (char *)ossl_dh_gen_type_id2name(
			     atoi(ctx->p2))) == NULL) {
			ERR_raise(ERR_LIB_EVP, EVP_R_INVALID_VALUE);
			return 0;
		}
				...
				struct translation_ctx_st *ctx)
{
	...
	if (state == PRE_CTRL_TO_PARAMS &&
         ...
         */
		ctx->params[0] = OSSL_PARAM_construct_int(
			translation->param_key, &ctx->p1);
		return 1;
	} else if (state == POST_PARAMS_TO_CTRL &&
		   ctx->action_type == OSSL_ACTION_GET) {
		...
		case OSSL_PARAM_UNSIGNED_INTEGER:
			return OSSL_PARAM_get_uint(ctx->params,
						   (unsigned int *)&ctx->p1);
		default:
			break;
		}
			       ...
			       struct translation_ctx_st *ctx)
{
	...
	if (state == PRE_CTRL_TO_PARAMS &&
		...
		if (i == OSSL_NELEM(str_value_map)) {
			BIO_snprintf(ctx->name_buf, sizeof(ctx->name_buf), "%d",
				     ctx->p1);
		} else {
				  ...
				  struct translation_ctx_st *ctx)
{
	...
	case EVP_PKEY_DH:
		switch (ctx->params->data_type) {
		case OSSL_PARAM_OCTET_STRING:
			ctx->sz = ossl_dh_key2buf(EVP_PKEY_get0_DH(pkey), &buf,
						  0, 1);
			ctx->p2 = buf;
			break;
		case OSSL_PARAM_UNSIGNED_INTEGER:
	...
	case EVP_PKEY_EC:
		if (ctx->params->data_type == OSSL_PARAM_OCTET_STRING) {
			...
			ctx->sz = EC_POINT_point2buf(
				ecg, point, POINT_CONVERSION_COMPRESSED, &buf,
				bnctx);
			ctx->p2 = buf;
			BN_CTX_free(bnctx);
			break;
...
#ifndef OPENSSL_NO_EC
	case EVP_PKEY_EC:
		val = EC_KEY_decoded_from_explicit_params(
...
#define IMPL_GET_RSA_PAYLOAD_FACTOR(n)                                         \
	static int get_rsa_payload_f##n(                                       \
		enum state state, const struct translation_st *translation,    \
		struct translation_ctx_st *ctx)                                \
	{                                                                      \
		if (EVP_PKEY_get_base_id(ctx->p2) != EVP_PKEY_RSA &&           \
...
#define IMPL_GET_RSA_PAYLOAD_EXPONENT(n)                                    \
	static int get_rsa_payload_e##n(                                    \
		enum state state, const struct translation_st *translation, \
		struct translation_ctx_st *ctx)                             \
	{                                                                   \
		if (EVP_PKEY_get_base_id(ctx->p2) != EVP_PKEY_RSA &&        \
...
#define IMPL_GET_RSA_PAYLOAD_COEFFICIENT(n)                                 \
	static int get_rsa_payload_c##n(                                    \
		enum state state, const struct translation_st *translation, \
		struct translation_ctx_st *ctx)                             \
	{                                                                   \
		if (EVP_PKEY_get_base_id(ctx->p2) != EVP_PKEY_RSA &&        \
...
static const struct translation_st evp_pkey_ctx_translations[] = {
	...
	{ OSSL_ACTION_SET, -1, -1, EVP_PKEY_OP_TYPE_SIG, EVP_PKEY_CTRL_SET1_ID,
	  "distid", "hexdistid", OSSL_PKEY_PARAM_DIST_ID,
	  OSSL_PARAM_OCTET_STRING, NULL },
	{ OSSL_ACTION_GET, -1, -1, -1, EVP_PKEY_CTRL_GET1_ID, "distid",
	  "hexdistid", OSSL_PKEY_PARAM_DIST_ID, OSSL_PARAM_OCTET_PTR, NULL },
	...
	{ OSSL_ACTION_SET, EVP_PKEY_DHX, 0, EVP_PKEY_OP_PARAMGEN,
	  EVP_PKEY_CTRL_DH_PARAMGEN_SUBPRIME_LEN, "dh_paramgen_subprime_len",
	  NULL, OSSL_PKEY_PARAM_FFC_QBITS, OSSL_PARAM_UNSIGNED_INTEGER, NULL },

	{ OSSL_ACTION_SET, EVP_PKEY_DH, 0, EVP_PKEY_OP_DERIVE,
	  EVP_PKEY_CTRL_DH_PAD, "dh_pad", NULL, OSSL_EXCHANGE_PARAM_PAD,
     ...
     * ==
     */
	{ OSSL_ACTION_SET, EVP_PKEY_EC, 0,
	  EVP_PKEY_OP_PARAMGEN | EVP_PKEY_OP_KEYGEN, EVP_PKEY_CTRL_EC_PARAM_ENC,
	  "ec_param_enc", NULL, OSSL_PKEY_PARAM_EC_ENCODING,
	...
	{ OSSL_ACTION_SET, EVP_PKEY_RSA, EVP_PKEY_RSA_PSS,
	  ...
	  OSSL_PKEY_PARAM_PAD_MODE, OSSL_PARAM_UTF8_STRING,
     ...
     * ========
     */
	{ OSSL_ACTION_SET, -1, -1, EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_TLS_MD,
	  "md", NULL, OSSL_KDF_PARAM_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
	...
	{ OSSL_ACTION_SET, -1, -1, EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_TLS_SEED,
	  "seed", "hexseed", OSSL_KDF_PARAM_SEED, OSSL_PARAM_OCTET_STRING,
	  NULL },
     ...
     * ====
     */
	{ OSSL_ACTION_SET, -1, -1, EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_HKDF_MD,
	  "md", NULL, OSSL_KDF_PARAM_DIGEST, OSSL_PARAM_UTF8_STRING, fix_md },
	{ OSSL_ACTION_SET, -1, -1, EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_HKDF_SALT,
	  "salt", "hexsalt", OSSL_KDF_PARAM_SALT, OSSL_PARAM_OCTET_STRING,
	  NULL },
	{ OSSL_ACTION_SET, -1, -1, EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_HKDF_KEY,
	  "key", "hexkey", OSSL_KDF_PARAM_KEY, OSSL_PARAM_OCTET_STRING, NULL },
	{ OSSL_ACTION_SET, -1, -1, EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_HKDF_INFO,
	  "info", "hexinfo", OSSL_KDF_PARAM_INFO, OSSL_PARAM_OCTET_STRING,
	  NULL },
	{ OSSL_ACTION_SET, -1, -1, EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_HKDF_MODE,
	  "mode", NULL, OSSL_KDF_PARAM_MODE, OSSL_PARAM_INTEGER,
	  fix_hkdf_mode },
     ...
     * ======
     */
	{ OSSL_ACTION_SET, -1, -1, EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_PASS,
	  "pass", "hexpass", OSSL_KDF_PARAM_PASSWORD, OSSL_PARAM_OCTET_STRING,
	  NULL },
	{ OSSL_ACTION_SET, -1, -1, EVP_PKEY_OP_DERIVE,
	  EVP_PKEY_CTRL_SCRYPT_SALT, "salt", "hexsalt", OSSL_KDF_PARAM_SALT,
	  OSSL_PARAM_OCTET_STRING, NULL },
	{ OSSL_ACTION_SET, -1, -1, EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_SCRYPT_N,
	  "N", NULL, OSSL_KDF_PARAM_SCRYPT_N, OSSL_PARAM_UNSIGNED_INTEGER,
	  NULL },
	{ OSSL_ACTION_SET, -1, -1, EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_SCRYPT_R,
	  "r", NULL, OSSL_KDF_PARAM_SCRYPT_R, OSSL_PARAM_UNSIGNED_INTEGER,
	  NULL },
	{ OSSL_ACTION_SET, -1, -1, EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_SCRYPT_P,
	  "p", NULL, OSSL_KDF_PARAM_SCRYPT_P, OSSL_PARAM_UNSIGNED_INTEGER,
	  NULL },
	{ OSSL_ACTION_SET, -1, -1, EVP_PKEY_OP_DERIVE,
	...
	{ OSSL_ACTION_SET, -1, -1, EVP_PKEY_OP_KEYGEN,
	  EVP_PKEY_CTRL_SET_MAC_KEY, "key", "hexkey", OSSL_PKEY_PARAM_PRIV_KEY,
	  OSSL_PARAM_OCTET_STRING, NULL },
	...
	{ OSSL_ACTION_GET, -1, -1, EVP_PKEY_OP_TYPE_SIG, EVP_PKEY_CTRL_GET_MD,
	  NULL, NULL, OSSL_SIGNATURE_PARAM_DIGEST, OSSL_PARAM_UTF8_STRING,
	  fix_md },
     ...
     * ===
     */
	{ OSSL_ACTION_SET, EVP_PKEY_X25519, EVP_PKEY_X25519, EVP_PKEY_OP_KEYGEN,
	  -1, NULL, NULL, OSSL_PKEY_PARAM_GROUP_NAME, OSSL_PARAM_UTF8_STRING,
	  fix_group_ecx },
	{ OSSL_ACTION_SET, EVP_PKEY_X25519, EVP_PKEY_X25519,
	...
	{ OSSL_ACTION_SET, EVP_PKEY_X448, EVP_PKEY_X448, EVP_PKEY_OP_KEYGEN, -1,
	  ...
	  fix_group_ecx },
	{ OSSL_ACTION_SET, EVP_PKEY_X448, EVP_PKEY_X448, EVP_PKEY_OP_PARAMGEN,
	  -1, NULL, NULL, OSSL_PKEY_PARAM_GROUP_NAME, OSSL_PARAM_UTF8_STRING,
	  fix_group_ecx },
};
		   ...
		   size_t translations_num)
{
	...
	for (i = 0; i < translations_num; i++) {
         ...
         * 2.  TBA...
         */
		if (!ossl_assert((item->keytype1 == -1) ==
				 (item->keytype2 == -1)))
			continue;
		...
		if (tmpl->ctrl_num != 0) {
			...
			if (item->ctrl_str != NULL &&
			    OPENSSL_strcasecmp(tmpl->ctrl_str,
					       item->ctrl_str) == 0)
					      ...
					      OSSL_PARAM *params)
{
	...
	for (; params != NULL && params->key != NULL; params++) {
		...
		cleanup_translation_ctx(CLEANUP_PARAMS_TO_CTRL, translation,
					&ctx);
