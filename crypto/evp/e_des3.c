			      ...
			      const unsigned char *in, size_t inl)
{
	...
	while (inl >= EVP_MAXCHUNK) {
		DES_ede3_cbc_encrypt(in, out, (long)EVP_MAXCHUNK, &dat->ks1,
				     &dat->ks2, &dat->ks3,
				     (DES_cblock *)ctx->iv,
				     EVP_CIPHER_CTX_is_encrypting(ctx));
		inl -= EVP_MAXCHUNK;
				...
				const unsigned char *in, size_t inl)
{
	...
	for (n = 0; n < inl; ++n) {
		...
		DES_ede3_cfb_encrypt(c, d, 1, 1, &data(ctx)->ks1,
				     &data(ctx)->ks2, &data(ctx)->ks3,
				     (DES_cblock *)ctx->iv,
				     EVP_CIPHER_CTX_is_encrypting(ctx));
...
#define des_ede3_ecb_cipher des_ede_ecb_cipher
	BLOCK_CIPHER_defs(des_ede3, DES_EDE_KEY, NID_des_ede3, 8, 24, 8, 64,
		...
		BLOCK_CIPHER_def_cfb(des_ede3, DES_EDE_KEY, NID_des_ede3, 24, 8,
			...
			BLOCK_CIPHER_def_cfb(des_ede3, DES_EDE_KEY,
					     NID_des_ede3, 24, 8, 8,
					     EVP_CIPH_RAND_KEY |
						     EVP_CIPH_FLAG_DEFAULT_ASN1,
					     des_ede3_init_key, NULL, NULL,
					     NULL, des3_ctrl)

				static int des_ede_init_key(
					EVP_CIPHER_CTX *ctx,
			   ...
			   const unsigned char *in, size_t inl)
{
	...
	if (ossl_sha1(out, inl - 16,
		      sha1tmp) /* Work out hash of first portion */
