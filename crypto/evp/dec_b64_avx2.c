...
static inline __m256i decode_std(__m256i input, __m256i *err_acc)
{
    ...
    *err_acc = _mm256_or_si256(*err_acc, _mm256_cmpeq_epi8(check,
...
/*
 * Static lookup tables for SIMD whitespace compression.
 */

/*
 * pshufb gather indices: maps an 8-bit bitmask to shuffle indices that
 * keep the bytes whose corresponding mask bits are clear.
 */
...
/*
 * Combine table: merges two 8-byte halves produced by thintable_epi8
 * pshufb into a single contiguous result. Indexed by pop1 * 8 where
 * pop1 = BitsSetTable256mul2[low_mask_byte].
 */
...
/* popcount(x) * 2 for all 8-bit values */
...
{
    return (BitsSetTable256mul2[x & 0xFF] +
            BitsSetTable256mul2[(x >> 8) & 0xFF] +
            BitsSetTable256mul2[(x >> 16) & 0xFF] +
            BitsSetTable256mul2[x >> 24]) >> 1;
}
                                      ...
                                      unsigned char *safe_end)
{
    ...
    if (use_srp) {
        d0 = decode_srp(bv0, &err); p0 = pack(d0);
        d1 = decode_srp(bv1, &err); p1 = pack(d1);
    } else {
        d0 = decode_std(bv0, &err); p0 = pack(d0);
        d1 = decode_std(bv1, &err); p1 = pack(d1);
    }
                          ...
                          int *consumed_out)
{
    ...
    while (bufferptr == buffer && consumed < srclen) {
        ...
        if (use_srp) {
            d0 = decode_srp(v0, &err); p0 = pack(d0);
            d1 = decode_srp(v1, &err); p1 = pack(d1);
        } else {
            d0 = decode_std(v0, &err); p0 = pack(d0);
            d1 = decode_std(v1, &err); p1 = pack(d1);
        }
