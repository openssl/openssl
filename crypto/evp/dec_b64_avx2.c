...
/*
 * Lanes set to all-ones for bytes that are not standard base64 characters.
 * A byte is valid when its hi-nibble and lo-nibble class bits intersect.
 */
OPENSSL_TARGET_AVX2
static ossl_inline __m256i invalid_std(__m256i input)
{
    ...
    const __m256i check = _mm256_and_si256(_mm256_shuffle_epi8(hi_lut, hi_nib),
        _mm256_shuffle_epi8(lo_lut, lo_nib));

    return _mm256_cmpeq_epi8(check, _mm256_setzero_si256());
}
 ...
 */
OPENSSL_TARGET_AVX2
static ossl_inline __m256i decode_std(__m256i input, __m256i *err_acc)
{
    const __m256i mask_0f = _mm256_set1_epi8(0x0F);
    const __m256i hi_nib = _mm256_and_si256(_mm256_srli_epi32(input, 4),
        mask_0f);

    *err_acc = _mm256_or_si256(*err_acc, invalid_std(input));
...
/* Lanes set to all-ones for bytes that are not SRP base64 characters. */
OPENSSL_TARGET_AVX2
static ossl_inline __m256i invalid_srp(__m256i input)
{
    const __m256i mask_0f = _mm256_set1_epi8(0x0F);
    const __m256i lo_nib = _mm256_and_si256(input, mask_0f);
    const __m256i hi_nib = _mm256_and_si256(_mm256_srli_epi32(input, 4),
 ...
 */
OPENSSL_TARGET_AVX2
static ossl_inline __m256i decode_srp(__m256i input, __m256i *err_acc)
{
    const __m256i mask_0f = _mm256_set1_epi8(0x0F);
    const __m256i hi_nib = _mm256_and_si256(_mm256_srli_epi32(input, 4),
        mask_0f);

    *err_acc = _mm256_or_si256(*err_acc, invalid_srp(input));
...
/* use_srp is loop-invariant at every call site, so this folds away. */
OPENSSL_TARGET_AVX2
static ossl_inline __m256i invalid_b64(__m256i input, int use_srp)
{
    return use_srp ? invalid_srp(input) : invalid_std(input);
}
OPENSSL_UNTARGET_AVX2
 ...
 */
OPENSSL_TARGET_AVX2
static ossl_inline __m256i pack(__m256i in)
{
 ...
 */
OPENSSL_TARGET_AVX2
static ossl_inline void store_48(unsigned char *out, __m256i p0, __m256i p1)
{
...
static ossl_inline int popcount32(uint32_t x)
{
 ...
 */
OPENSSL_TARGET_AVX2
static ossl_inline void compress128(__m128i data, uint16_t mask,
 ...
 */
OPENSSL_TARGET_AVX2
static ossl_inline void compress256(__m256i data, uint32_t mask,
    unsigned char *output)
{
    if (mask == 0) {
 ...
 */
OPENSSL_TARGET_AVX2
static ossl_inline void compress_block_single(__m256i v0, __m256i v1, uint64_t mask,
    unsigned char *output)
{
    int pos64, pos, lane;
...
/*
 * Decode 64 buffer bytes into 48 output bytes. The general loop only
 * buffers alphabet characters, so there is nothing to validate here and
 * the dead error accumulator folds away.
 */
OPENSSL_TARGET_AVX2
static ossl_inline void decode_buffer_block(const unsigned char *buf,
    unsigned char *out,
    int use_srp)
{
    ...
    if (use_srp) {
        p0 = pack(decode_srp(bv0, &err));
        p1 = pack(decode_srp(bv1, &err));
    } else {
        p0 = pack(decode_std(bv0, &err));
        p1 = pack(decode_std(bv1, &err));
    }
 ...
 *
 * dst may alias src (PEM_read_bio_ex decodes in place). Each loop loads
 * a chunk before storing, and at most 48 bytes are written per 64
 * consumed, so writes stay behind src + consumed and input from
 * *consumed_out on is intact for the caller's scalar loop.
 */
OPENSSL_TARGET_AVX2
    ...
    int *consumed_out)
{
     ...
     *
     * The compressors store up to 64 bytes at bufferptr even when they
     * report fewer. With buf_rpos <= 255 and buf_avail <= 63 on entry,
     * bufferptr <= buffer+318, so the highest byte touched is +381.
     */
    ...
    /*
     * General loop: handles whitespace and buffer management. Entered
     * when WS appears mid-block or after the skip-WS loop encounters
     ...
     */
    {
        ...
        while (swallow_ws && consumed + 64 <= srclen) {
            ...
            /*
             * Reject the chunk before consuming it, as compression drops
             * the offsets needed to report an error later and src cannot
             * be replayed once the output has overwritten it.
             */
            bad0 = _mm256_andnot_si256(ws0, invalid_b64(v0, use_srp));
            bad1 = _mm256_andnot_si256(ws1, invalid_b64(v1, use_srp));
            if ((_mm256_movemask_epi8(bad0) | _mm256_movemask_epi8(bad1)) != 0)
                break;
             ...
             */
            {
                ...
                while (buf_avail >= 64) {
                    decode_buffer_block(buffer + buf_rpos, out, use_srp);
                    out += 48;
                    buf_rpos += 64;
                    buf_avail -= 64;
                }
