				      ...
				      const char *propquery)
{
	...
	if (skeymgmt == NULL) {
         ...
         * key management
         */
		skeymgmt = EVP_SKEYMGMT_fetch(libctx, OSSL_SKEY_TYPE_GENERIC,
					      propquery);
		if (skeymgmt == NULL) {
...
static int get_secret_key(const OSSL_PARAM params[], void *arg)
{
	...
	if ((p = OSSL_PARAM_locate_const(params, OSSL_SKEY_PARAM_RAW_BYTES)) !=
	    NULL)
		return OSSL_PARAM_get_octet_string_ptr(p, raw_key->key,
						       raw_key->len);
...
static int transfer_cb(const OSSL_PARAM params[], void *arg)
{
	...
	ctx->keydata =
		evp_skeymgmt_import(ctx->skeymgmt, ctx->selection, params);
	return 1;
}
