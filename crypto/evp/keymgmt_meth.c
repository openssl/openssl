				    ...
				    OSSL_PROVIDER *prov)
{
	...
	if ((keymgmt->type_name = ossl_algorithm_get1_first_name(algodef)) ==
	    NULL) {
		EVP_KEYMGMT_free(keymgmt);
		return NULL;
	}
		...
		case OSSL_FUNC_KEYMGMT_IMPORT_TYPES_EX:
			if (keymgmt->import_types_ex == NULL) {
				...
				keymgmt->import_types_ex =
					OSSL_FUNC_keymgmt_import_types_ex(fns);
			}
			break;
		...
		case OSSL_FUNC_KEYMGMT_EXPORT_TYPES_EX:
			if (keymgmt->export_types_ex == NULL) {
				...
				keymgmt->export_types_ex =
					OSSL_FUNC_keymgmt_export_types_ex(fns);
			}
			break;
		}
	}
	...
	if (keymgmt->free == NULL ||
	    ...
	    (keymgmt->gen != NULL &&
	     (keymgmt->gen_init == NULL || keymgmt->gen_cleanup == NULL))) {
		EVP_KEYMGMT_free(keymgmt);
...
{
	return evp_generic_fetch_from_prov(prov, OSSL_OP_KEYMGMT, name,
					   properties, keymgmt_from_algorithm,
					   evp_keymgmt_up_ref,
					   evp_keymgmt_free);
}
			     ...
			     void *data)
{
	if (keymgmt->prov != NULL)
		return evp_names_do_all(keymgmt->prov, keymgmt->name_id, fn,
					data);
		      ...
		      OSSL_CALLBACK *cb, void *cbarg)
{
	...
	if (keymgmt->gen == NULL) {
		ERR_raise_data(ERR_LIB_EVP,
			       ...
			       "%s key generation:%s", keymgmt->type_name,
			       desc);
		return NULL;
	}
