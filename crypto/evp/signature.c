					  ...
					  OSSL_PROVIDER *prov)
{
	...
	int gparamfncnt = 0, sparamfncnt = 0, gmdparamfncnt = 0,
	    smdparamfncnt = 0;
	int valid = 0;
	...
	if ((signature->type_name = ossl_algorithm_get1_first_name(algodef)) ==
	    NULL)
		goto err;
	...
	if (ctxfncnt != 2) {
		ERR_raise_data(ERR_LIB_EVP, EVP_R_INVALID_PROVIDER_FUNCTIONS,
			       "missing %s newctx or freectx:%s",
			       signature->type_name, desc);
		valid = 0;
	}
	if (valid && ((gparamfncnt != 0 && gparamfncnt != 2) ||
         ...
         */
		ERR_raise_data(ERR_LIB_EVP, EVP_R_INVALID_PROVIDER_FUNCTIONS,
			       "missing %s params getter or setter:%s",
			       signature->type_name, desc);
		valid = 0;
	}
	if (valid && initfncnt == 0) {
		ERR_raise_data(ERR_LIB_EVP, EVP_R_INVALID_PROVIDER_FUNCTIONS,
			       "missing %s init:%s", signature->type_name,
			       desc);
		valid = 0;
	}
	...
	if (valid &&
	    ((signature->sign_init != NULL && signature->sign == NULL) ||
	     (signature->sign_message_init != NULL && signature->sign == NULL &&
	...
	if (valid &&
	    ((signature->verify_init != NULL && signature->verify == NULL) ||
		...
		ERR_raise_data(ERR_LIB_EVP, EVP_R_INVALID_PROVIDER_FUNCTIONS,
			       "missing %s verification function:%s",
			       signature->type_name, desc);
		valid = 0;
	}
	if (valid &&
	    ...
	    signature->verify_message_init == NULL) {
		ERR_raise_data(
			ERR_LIB_EVP, EVP_R_INVALID_PROVIDER_FUNCTIONS,
	...
	if (valid && (signature->verify_recover_init != NULL) &&
	...
	if (valid && (signature->digest_sign_init != NULL &&
	...
	if (valid && ((signature->digest_verify_init != NULL &&
...
EVP_SIGNATURE *evp_signature_fetch_from_prov(OSSL_PROVIDER *prov,
					     ...
					     const char *properties)
{
	return evp_generic_fetch_from_prov(prov, OSSL_OP_SIGNATURE, algorithm,
					   properties,
					   evp_signature_from_algorithm,
				   ...
				   void *arg)
{
	evp_generic_do_all(libctx, OSSL_OP_SIGNATURE,
			   ...
			   evp_signature_from_algorithm, evp_signature_up_ref,
			   evp_signature_free);
}
			       ...
			       void *data)
{
	if (signature->prov != NULL)
		return evp_names_do_all(signature->prov, signature->name_id, fn,
					data);
				   ...
				   int operation, const OSSL_PARAM params[])
{
	...
	if (signature != NULL) {
		...
		supported_sig = evp_keymgmt_util_query_operation_name(
			ctx->keymgmt, OSSL_OP_SIGNATURE);
		...
		for (iter = 1; iter < 3 && provkey == NULL; iter++) {
			...
			switch (iter) {
			case 1:
				signature = EVP_SIGNATURE_fetch(ctx->libctx,
								supported_sig,
								ctx->propquery);
				if (signature != NULL)
			...
			case 2:
				...
				signature = evp_signature_fetch_from_prov(
					(OSSL_PROVIDER *)tmp_prov,
			...
			tmp_keymgmt_tofree = tmp_keymgmt =
				evp_keymgmt_fetch_from_prov(
					...
					ctx->propquery);
			if (tmp_keymgmt != NULL)
				provkey = evp_pkey_export_to_provider(
					ctx->pkey, ctx->libctx, &tmp_keymgmt,
					ctx->propquery);
			if (tmp_keymgmt == NULL)
				EVP_KEYMGMT_free(tmp_keymgmt_tofree);
		}
	...
	ctx->op.sig.algctx = signature->newctx(
		ossl_provider_ctx(signature->prov), ctx->propquery);
	...
	case EVP_PKEY_OP_SIGN:
		if (signature->sign_init == NULL) {
			ERR_raise_data(ERR_LIB_EVP,
				       ...
				       "%s sign_init:%s", signature->type_name,
				       desc);
			ret = -2;
			goto err;
		}
	...
	case EVP_PKEY_OP_SIGNMSG:
		...
		ret = signature->sign_message_init(ctx->op.sig.algctx, provkey,
						   params);
		break;
	case EVP_PKEY_OP_VERIFY:
		...
		ret = signature->verify_init(ctx->op.sig.algctx, provkey,
					     params);
		break;
	case EVP_PKEY_OP_VERIFYMSG:
		...
		ret = signature->verify_message_init(ctx->op.sig.algctx,
						     provkey, params);
		break;
	case EVP_PKEY_OP_VERIFYRECOVER:
		...
		ret = signature->verify_recover_init(ctx->op.sig.algctx,
						     provkey, params);
		break;
	default:
		ERR_raise(ERR_LIB_EVP, EVP_R_INITIALIZATION_ERROR);
				...
				size_t *siglen)
{
	...
	if (signature->sign_message_final == NULL) {
		ERR_raise_data(ERR_LIB_EVP,
			       EVP_R_PROVIDER_SIGNATURE_NOT_SUPPORTED,
			       "%s sign_message_final:%s", signature->type_name,
			       desc);
		return -2;
	}
...
legacy:
	...
	M_check_autoarg(ctx, sig, siglen, EVP_F_EVP_PKEY_SIGN) return ctx->pmeth
...
int EVP_PKEY_verify_message_init(EVP_PKEY_CTX *ctx, EVP_SIGNATURE *algo,
				 const OSSL_PARAM params[])
{
	return evp_pkey_signature_init(ctx, algo, EVP_PKEY_OP_VERIFYMSG,
				       params);
}
...
int EVP_PKEY_verify_recover_init_ex2(EVP_PKEY_CTX *ctx, EVP_SIGNATURE *algo,
				     const OSSL_PARAM params[])
{
	return evp_pkey_signature_init(ctx, algo, EVP_PKEY_OP_VERIFYRECOVER,
				       params);
}
			    ...
			    size_t siglen)
{
	...
	if (signature->verify_recover == NULL) {
		ERR_raise_data(ERR_LIB_EVP,
			       EVP_R_PROVIDER_SIGNATURE_NOT_SUPPORTED,
			       "%s verify_recover:%s", signature->type_name,
			       desc);
		return -2;
	}

	ret = signature->verify_recover(ctx->op.sig.algctx, rout, routlen,
					(rout == NULL ? 0 : *routlen), sig,
					siglen);
	if (ret <= 0)
		ERR_raise_data(ERR_LIB_EVP, EVP_R_PROVIDER_SIGNATURE_FAILURE,
			       "%s verify_recover:%s", signature->type_name,
