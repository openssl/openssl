...
int PKCS5_PBKDF2_HMAC(const char *pass, int passlen, const unsigned char *salt,
		      ...
		      unsigned char *out)
{
	return ossl_pkcs5_pbkdf2_hmac_ex(pass, passlen, salt, saltlen, iter,
			   ...
			   int keylen, unsigned char *out)
{
	...
	if ((digest = EVP_MD_fetch(NULL, SN_sha1, NULL)) != NULL)
		r = ossl_pkcs5_pbkdf2_hmac_ex(pass, passlen, salt, saltlen,
					      iter, digest, keylen, out, NULL,
					      NULL);
	EVP_MD_free(digest);
	return r;
}
			     ...
			     const char *propq)
{
     ...
     */
	if (OBJ_obj2txt(ciph_name, sizeof(ciph_name),
			pbe2->encryption->algorithm, 0) <= 0) {
	...
	rv = kdf(ctx, pass, passlen, pbe2->keyfunc->parameter, NULL, NULL,
		 en_de, libctx, propq);
err:
				...
				const char *propq)
{
	...
	(void)ERR_set_mark();
	prfmd = prfmd_fetch =
		EVP_MD_fetch(libctx, OBJ_nid2sn(hmac_md_nid), propq);
	if (prfmd == NULL)
	...
	if (!ossl_pkcs5_pbkdf2_hmac_ex(pass, passlen, salt, saltlen, iter,
				       prfmd, keylen, key, libctx, propq))
		goto err;
...
int PKCS5_v2_PBKDF2_keyivgen(EVP_CIPHER_CTX *ctx, const char *pass, int passlen,
			     ...
			     const EVP_MD *md, int en_de)
{
	return PKCS5_v2_PBKDF2_keyivgen_ex(ctx, pass, passlen, param, c, md,
					   en_de, NULL, NULL);
}
