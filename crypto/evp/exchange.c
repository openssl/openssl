					...
					OSSL_PROVIDER *prov)
{
	...
	if ((exchange->type_name = ossl_algorithm_get1_first_name(algodef)) ==
	    NULL)
		goto err;
	...
	if (fncnt != 4 || (gparamfncnt != 0 && gparamfncnt != 2) ||
			       ...
			       const char *properties)
{
	return evp_generic_fetch(ctx, OSSL_OP_KEYEXCH, algorithm, properties,
				 evp_keyexch_from_algorithm, evp_keyexch_up_ref,
				 evp_keyexch_free);
}
...
EVP_KEYEXCH *evp_keyexch_fetch_from_prov(OSSL_PROVIDER *prov,
					 ...
					 const char *properties)
{
	return evp_generic_fetch_from_prov(prov, OSSL_OP_KEYEXCH, algorithm,
					   properties,
...
int EVP_PKEY_derive_init_ex(EVP_PKEY_CTX *ctx, const OSSL_PARAM params[])
{
	...
	for (iter = 1, provkey = NULL; iter < 3 && provkey == NULL; iter++) {
		...
		case 2:
			...
			exchange = evp_keyexch_fetch_from_prov(
				(OSSL_PROVIDER *)tmp_prov, supported_exch,
				ctx->propquery);
			if (exchange == NULL)
				goto legacy;
			break;
	...
	/* coverity[deref_arg] */
	ctx->op.kex.algctx =
		exchange->newctx(ossl_provider_ctx(exchange->prov));
				...
				int validate_peer)
{
	...
	if (validate_peer) {
		check_ctx = EVP_PKEY_CTX_new_from_pkey(ctx->libctx, peer,
						       ctx->propquery);
		if (check_ctx == NULL)
			return -1;
     ...
     */
	tmp_keymgmt_tofree = tmp_keymgmt = evp_keymgmt_fetch_from_prov(
		(OSSL_PROVIDER *)EVP_KEYEXCH_get0_provider(
			ctx->op.kex.exchange),
		EVP_KEYMGMT_get0_name(ctx->keymgmt), ctx->propquery);
	if (tmp_keymgmt != NULL)
		...
		provkey = evp_pkey_export_to_provider(
			peer, ctx->libctx, &tmp_keymgmt, ctx->propquery);
	EVP_KEYMGMT_free(tmp_keymgmt_tofree);
...
#else
	if (ctx->pmeth == NULL ||
	    !(ctx->pmeth->derive != NULL || ctx->pmeth->encrypt != NULL ||
...
legacy:
	...
	M_check_autoarg(ctx, key, pkeylen, EVP_F_EVP_PKEY_DERIVE) return ctx
		->pmeth->derive(ctx, key, pkeylen);
}
			     ...
			     void *data)
{
	if (keyexch->prov != NULL)
		return evp_names_do_all(keyexch->prov, keyexch->name_id, fn,
					data);
