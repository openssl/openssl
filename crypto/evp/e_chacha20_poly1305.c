					...
					const unsigned char *in, size_t len)
{
	...
	if (plen <= CHACHA_BLK_SIZE) {
		...
		if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
			ChaCha20_ctr32(out, in, plen, actx->key.key.d,
				       actx->key.counter);
...

	{
		...
		if (IS_LITTLE_ENDIAN) {
			memcpy(ctr, (unsigned char *)&actx->len,
			       POLY1305_BLOCK_SIZE);
		} else {
				    ...
				    const unsigned char *in, size_t len)
{
	...
	if (in) { /* aad or text */
		if (out == NULL) { /* aad */
			...
			if (actx->aad) { /* wrap up aad */
				if ((rem = (size_t)actx->len.aad %
					   POLY1305_BLOCK_SIZE))
					Poly1305_Update(
						POLY1305_ctx(actx), zero,
	...
	if (in == NULL /* explicit final */
		...
		if (IS_LITTLE_ENDIAN) {
			...
			Poly1305_Update(POLY1305_ctx(actx), temp,
					POLY1305_BLOCK_SIZE);
		}
		Poly1305_Final(POLY1305_ctx(actx),
			       EVP_CIPHER_CTX_is_encrypting(ctx) ? actx->tag :
								   temp);
		actx->mac_inited = 0;
				  ...
				  void *ptr)
{
	...
	case EVP_CTRL_INIT:
		if (actx == NULL)
			actx = ctx->cipher_data = OPENSSL_zalloc(
				sizeof(*actx) + Poly1305_ctx_size());
		if (actx == NULL) {
	...
	case EVP_CTRL_COPY:
		if (actx) {
			...
			dst->cipher_data = OPENSSL_memdup(
				actx, sizeof(*actx) + Poly1305_ctx_size());
