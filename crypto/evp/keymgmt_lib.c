					  ...
					  int selection)
{
	...
	import_data.keydata =
		NULL; /* evp_keymgmt_util_try_import will create it */
...
void evp_keymgmt_util_cache_keyinfo(EVP_PKEY *pk)
{
	...
	if (pk->keydata != NULL) {
		...
		params[1] = OSSL_PARAM_construct_int(
			OSSL_PKEY_PARAM_SECURITY_BITS, &security_bits);
		params[2] = OSSL_PARAM_construct_int(
			OSSL_PKEY_PARAM_SECURITY_CATEGORY, &security_category);
		params[3] = OSSL_PARAM_construct_int(OSSL_PKEY_PARAM_MAX_SIZE,
						     &size);
					   ...
					   char *mdname, size_t mdname_sz)
{
	...
	params[1] = OSSL_PARAM_construct_utf8_string(
		OSSL_PKEY_PARAM_MANDATORY_DIGEST, mdmandatory,
		sizeof(mdmandatory));
