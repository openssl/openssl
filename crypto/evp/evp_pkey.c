			    ...
			    const char *propq)
{
	...
	if (dctx != NULL && OSSL_DECODER_CTX_get_num_decoders(dctx) == 0) {
         ...
         * keytype
         */
		dctx = OSSL_DECODER_CTX_new_for_pkey(&pkey, "DER",
						     "PrivateKeyInfo", NULL,
...
PKCS8_PRIV_KEY_INFO *EVP_PKEY2PKCS8(const EVP_PKEY *pkey)
{
	...
	if (evp_pkey_is_provided(pkey)) {
		...
		if ((ctx = OSSL_ENCODER_CTX_new_for_pkey(pkey, selection, "DER",
							 "PrivateKeyInfo",
							 NULL)) == NULL ||
...
int EVP_PKEY_add1_attr_by_txt(EVP_PKEY *key, const char *attrname, int type,
			      const unsigned char *bytes, int len)
{
	if (X509at_add1_attr_by_txt(&key->attributes, attrname, type, bytes,
				    len))
		return 1;
	return 0;
}
