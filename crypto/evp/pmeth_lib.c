...
int EVP_PKEY_CTX_get_signature_md(EVP_PKEY_CTX *ctx, const EVP_MD **md)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if ((EVP_PKEY_CTX_get_params(ctx, sig_md_params) <= 0) || !OSSL_PARAM_modified(sig_md_params))
        return 0;
                                          ...
                                          int datalen)
{
    ...
    if ((EVP_PKEY_CTX_get_params(ctx, os_params) <= 0) || !OSSL_PARAM_modified(os_params))
        return 0;
