...
static int evp_pkey_ctx_store_cached_data(EVP_PKEY_CTX *ctx, int keytype,
					  int optype, int cmd, const char *name,
					  const void *data, size_t data_len);
static void evp_pkey_ctx_free_cached_data(EVP_PKEY_CTX *ctx, int cmd,
					  const char *name);
...
static pmeth_fn standard_methods[] = {
	...
	ossl_ecx25519_pkey_method, ossl_ecx448_pkey_method,
	ossl_ed25519_pkey_method,  ossl_ed448_pkey_method,
#endif
};
...
int evp_pkey_ctx_state(const EVP_PKEY_CTX *ctx)
{
	...
	if ((EVP_PKEY_CTX_IS_DERIVE_OP(ctx) && ctx->op.kex.algctx != NULL) ||
	    (EVP_PKEY_CTX_IS_SIGNATURE_OP(ctx) && ctx->op.sig.algctx != NULL) ||
	    (EVP_PKEY_CTX_IS_ASYM_CIPHER_OP(ctx) &&
	     ctx->op.ciph.algctx != NULL) ||
	    (EVP_PKEY_CTX_IS_GEN_OP(ctx) && ctx->op.keymgmt.genctx != NULL) ||
	    (EVP_PKEY_CTX_IS_KEM_OP(ctx) && ctx->op.encap.algctx != NULL))
...

{
	...
	if (e == NULL && pkey != NULL)
		e = pkey->pmeth_engine != NULL ? pkey->pmeth_engine :
						 pkey->engine;
...
EVP_PKEY_CTX *EVP_PKEY_CTX_dup(const EVP_PKEY_CTX *pctx)
{
	...
	if (pctx->pmeth == NULL) {
		if (rctx->operation == EVP_PKEY_OP_UNDEFINED) {
			...
			provkey = evp_pkey_export_to_provider(pctx->pkey,
							      pctx->libctx,
							      &tmp_keymgmt,
							      pctx->propquery);
			if (provkey == NULL)
...
{
	if (app_pkey_methods != NULL)
		sk_EVP_PKEY_METHOD_pop_free(app_pkey_methods,
					    EVP_PKEY_meth_free);
}
...
int EVP_PKEY_CTX_set_params(EVP_PKEY_CTX *ctx, const OSSL_PARAM *params)
{
	...
	case EVP_PKEY_STATE_PROVIDER:
		if (EVP_PKEY_CTX_IS_DERIVE_OP(ctx) &&
			...
			return ctx->op.kex.exchange->set_ctx_params(
				ctx->op.kex.algctx, params);
		if (EVP_PKEY_CTX_IS_SIGNATURE_OP(ctx) &&
		    ctx->op.sig.signature != NULL &&
		    ctx->op.sig.signature->set_ctx_params != NULL)
			return ctx->op.sig.signature->set_ctx_params(
				ctx->op.sig.algctx, params);
		if (EVP_PKEY_CTX_IS_ASYM_CIPHER_OP(ctx) &&
		    ctx->op.ciph.cipher != NULL &&
		    ctx->op.ciph.cipher->set_ctx_params != NULL)
			return ctx->op.ciph.cipher->set_ctx_params(
				ctx->op.ciph.algctx, params);
		if (EVP_PKEY_CTX_IS_GEN_OP(ctx) && ctx->keymgmt != NULL &&
		    ctx->keymgmt->gen_set_params != NULL)
			return evp_keymgmt_gen_set_params(
				ctx->keymgmt, ctx->op.keymgmt.genctx, params);
		if (EVP_PKEY_CTX_IS_KEM_OP(ctx) && ctx->op.encap.kem != NULL &&
		    ctx->op.encap.kem->set_ctx_params != NULL)
			return ctx->op.encap.kem->set_ctx_params(
				ctx->op.encap.algctx, params);
		break;
...
int EVP_PKEY_CTX_get_params(EVP_PKEY_CTX *ctx, OSSL_PARAM *params)
{
	...
	case EVP_PKEY_STATE_PROVIDER:
		if (EVP_PKEY_CTX_IS_DERIVE_OP(ctx) &&
			...
			return ctx->op.kex.exchange->get_ctx_params(
				ctx->op.kex.algctx, params);
		if (EVP_PKEY_CTX_IS_SIGNATURE_OP(ctx) &&
		    ctx->op.sig.signature != NULL &&
		    ctx->op.sig.signature->get_ctx_params != NULL)
			return ctx->op.sig.signature->get_ctx_params(
				ctx->op.sig.algctx, params);
		if (EVP_PKEY_CTX_IS_ASYM_CIPHER_OP(ctx) &&
		    ctx->op.ciph.cipher != NULL &&
		    ctx->op.ciph.cipher->get_ctx_params != NULL)
			return ctx->op.ciph.cipher->get_ctx_params(
				ctx->op.ciph.algctx, params);
		if (EVP_PKEY_CTX_IS_KEM_OP(ctx) && ctx->op.encap.kem != NULL &&
		    ctx->op.encap.kem->get_ctx_params != NULL)
			return ctx->op.encap.kem->get_ctx_params(
				ctx->op.encap.algctx, params);
		if (EVP_PKEY_CTX_IS_GEN_OP(ctx) && ctx->keymgmt != NULL &&
		    ctx->keymgmt->gen_get_params != NULL)
			return evp_keymgmt_gen_get_params(
				ctx->keymgmt, ctx->op.keymgmt.genctx, params);
		break;
	case EVP_PKEY_STATE_UNKNOWN:
		break;
...
const OSSL_PARAM *EVP_PKEY_CTX_gettable_params(const EVP_PKEY_CTX *ctx)
{
	...
	if (EVP_PKEY_CTX_IS_DERIVE_OP(ctx) && ctx->op.kex.exchange != NULL &&
	...
	if (EVP_PKEY_CTX_IS_ASYM_CIPHER_OP(ctx) &&
		...
		return ctx->op.ciph.cipher->gettable_ctx_params(
			ctx->op.ciph.algctx, provctx);
	}
	if (EVP_PKEY_CTX_IS_KEM_OP(ctx) && ctx->op.encap.kem != NULL &&
	    ctx->op.encap.kem->gettable_ctx_params != NULL) {
		provctx = ossl_provider_ctx(
			EVP_KEM_get0_provider(ctx->op.encap.kem));
		return ctx->op.encap.kem->gettable_ctx_params(
			ctx->op.encap.algctx, provctx);
	}
	if (EVP_PKEY_CTX_IS_GEN_OP(ctx) && ctx->keymgmt != NULL &&
...
const OSSL_PARAM *EVP_PKEY_CTX_settable_params(const EVP_PKEY_CTX *ctx)
{
	...
	if (EVP_PKEY_CTX_IS_KEM_OP(ctx) && ctx->op.encap.kem != NULL &&
		...
		provctx = ossl_provider_ctx(
			EVP_KEM_get0_provider(ctx->op.encap.kem));
		return ctx->op.encap.kem->settable_ctx_params(
			ctx->op.encap.algctx, provctx);
	}
	return NULL;
}
...
int EVP_PKEY_CTX_set_tls1_prf_md(EVP_PKEY_CTX *ctx, const EVP_MD *md)
{
	return evp_pkey_ctx_set_md(ctx, md, ctx->op.kex.algctx == NULL,
				   OSSL_KDF_PARAM_DIGEST, EVP_PKEY_OP_DERIVE,
					  ...
					  int datalen)
{
	...
	/* Code below to be removed when legacy support is dropped. */
	if (fallback)
		return EVP_PKEY_CTX_ctrl(ctx, -1, op, ctrl, datalen,
					 (void *)(data));
					  ...
					  int datalen)
{
	...
	if (gettables == NULL ||
	    OSSL_PARAM_locate_const(gettables, param) == NULL)
		return evp_pkey_ctx_set1_octet_string(ctx, fallback, param, op,
						      ctrl, data, datalen);
	...
	os_params[0] =
		OSSL_PARAM_construct_octet_string(param, info, info_alloc);
...
{
	return evp_pkey_ctx_set1_octet_string(
		ctx, ctx->op.kex.algctx == NULL, OSSL_KDF_PARAM_SECRET,
...
{
	return evp_pkey_ctx_set1_octet_string(
		ctx, ctx->op.kex.algctx == NULL, OSSL_KDF_PARAM_SEED,
		EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_TLS_SEED, seed, seedlen);
}
...
{
	return evp_pkey_ctx_set1_octet_string(
		ctx, ctx->op.kex.algctx == NULL, OSSL_KDF_PARAM_SALT,
		EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_HKDF_SALT, salt, saltlen);
}
...
{
	return evp_pkey_ctx_set1_octet_string(
		ctx, ctx->op.kex.algctx == NULL, OSSL_KDF_PARAM_KEY,
		EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_HKDF_KEY, key, keylen);
}

int EVP_PKEY_CTX_add1_hkdf_info(EVP_PKEY_CTX *ctx, const unsigned char *info,
				int infolen)
{
	return evp_pkey_ctx_add1_octet_string(
		ctx, ctx->op.kex.algctx == NULL, OSSL_KDF_PARAM_INFO,
		EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_HKDF_INFO, info, infolen);
}
...
{
	return evp_pkey_ctx_set1_octet_string(
		ctx, ctx->op.kex.algctx == NULL, OSSL_KDF_PARAM_PASSWORD,
				  ...
				  int saltlen)
{
	return evp_pkey_ctx_set1_octet_string(
		ctx, ctx->op.kex.algctx == NULL, OSSL_KDF_PARAM_SALT,
		EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_SCRYPT_SALT, salt, saltlen);
}

static int evp_pkey_ctx_set_uint64(EVP_PKEY_CTX *ctx, const char *param, int op,
				   int ctrl, uint64_t val)
{
...
{
	return evp_pkey_ctx_set1_octet_string(
		ctx, ctx->op.keymgmt.genctx == NULL, OSSL_PKEY_PARAM_PRIV_KEY,
		EVP_PKEY_OP_KEYGEN, EVP_PKEY_CTRL_SET_MAC_KEY, key, keylen);
}
...
{
	return EVP_PKEY_CTX_ctrl(ctx, -1, -1, EVP_PKEY_CTRL_SET1_ID, (int)len,
				 (void *)(id));
}

int EVP_PKEY_CTX_get1_id(EVP_PKEY_CTX *ctx, void *id)
{
	return EVP_PKEY_CTX_ctrl(ctx, -1, -1, EVP_PKEY_CTRL_GET1_ID, 0,
				 (void *)id);
}
				 ...
				 int cmd, int p1, void *p2)
{
	...
	switch (evp_pkey_ctx_state(ctx)) {
	case EVP_PKEY_STATE_PROVIDER:
		return evp_pkey_ctx_ctrl_to_param(ctx, keytype, optype, cmd, p1,
						  p2);
	case EVP_PKEY_STATE_UNKNOWN:
	case EVP_PKEY_STATE_LEGACY:
		      ...
		      int p1, void *p2)
{
	...
	ERR_set_mark();
	ret = evp_pkey_ctx_store_cached_data(ctx, keytype, optype, cmd, NULL,
					     p2, p1);
	if (ret == -2) {
		ERR_pop_to_mark();
	} else {
...
static int evp_pkey_ctx_ctrl_str_int(EVP_PKEY_CTX *ctx, const char *name,
				     const char *value)
{
	int ret = 0;
			  ...
			  const char *value)
{
	...
	/* If unsupported, we don't want that reported here */
	ERR_set_mark();
	ret = evp_pkey_ctx_store_cached_data(ctx, -1, -1, -1, name, value,
					     strlen(value) + 1);
	if (ret == -2) {
		ERR_pop_to_mark();
	} else {
...
static int decode_cmd(int cmd, const char *name)
{
	if (cmd == -1) {
		...
		if (ossl_assert(name != NULL))
			if (strcmp(name, "distid") == 0 ||
			    strcmp(name, "hexdistid") == 0)
					  ...
					  const void *data, size_t data_len)
{
	...
	if (keytype != -1) {
		...
		case EVP_PKEY_STATE_LEGACY:
			...
			if (EVP_PKEY_type(ctx->pmeth->pkey_id) !=
	...
	case EVP_PKEY_CTRL_SET1_ID:
		...
		if (name != NULL) {
			ctx->cached_parameters.dist_id_name =
				OPENSSL_strdup(name);
			if (ctx->cached_parameters.dist_id_name == NULL)
				return 0;
		}
		if (data_len > 0) {
			ctx->cached_parameters.dist_id =
...
const OSSL_PROVIDER *EVP_PKEY_CTX_get0_provider(const EVP_PKEY_CTX *ctx)
{
	if (EVP_PKEY_CTX_IS_SIGNATURE_OP(ctx)) {
		...
		if (ctx->op.ciph.cipher != NULL)
			return EVP_ASYM_CIPHER_get0_provider(
				ctx->op.ciph.cipher);
	} else if (EVP_PKEY_CTX_IS_GEN_OP(ctx)) {
		if (ctx->keymgmt != NULL)
