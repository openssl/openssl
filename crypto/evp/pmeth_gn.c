...
static int ossl_callback_to_pkey_gencb(const OSSL_PARAM params[], void *arg)
{
	...
	if ((param = OSSL_PARAM_locate_const(
		     params, OSSL_GEN_PARAM_POTENTIAL)) == NULL ||
	    !OSSL_PARAM_get_int(param, &p))
		return 0;
	if ((param = OSSL_PARAM_locate_const(
		     params, OSSL_GEN_PARAM_ITERATION)) == NULL ||
...
int EVP_PKEY_generate(EVP_PKEY_CTX *ctx, EVP_PKEY **ppkey)
{
     ...
     */
	ret = ret &&
	      (evp_keymgmt_util_gen(*ppkey, ctx->keymgmt,
...
#endif
	return evp_keymgmt_util_export(pkey, selection, export_cb,
				       export_cbarg);
}
