...
static int do_sigver_init(EVP_MD_CTX *ctx, EVP_PKEY_CTX **pctx,
			  ...
			  OSSL_LIB_CTX *libctx, const char *props, ENGINE *e,
			  EVP_PKEY *pkey, int ver, const OSSL_PARAM params[])
{
	...
	if (ctx->pctx == NULL) {
		...
		if (e == NULL)
			ctx->pctx =
				EVP_PKEY_CTX_new_from_pkey(libctx, pkey, props);
		else
	...
	supported_sig = evp_keymgmt_util_query_operation_name(
		locpctx->keymgmt, OSSL_OP_SIGNATURE);
	...
	for (iter = 1, provkey = NULL; iter < 3 && provkey == NULL; iter++) {
		...
		switch (iter) {
		case 1:
			signature = EVP_SIGNATURE_fetch(locpctx->libctx,
							supported_sig,
							locpctx->propquery);
			if (signature != NULL)
				tmp_prov =
		...
		case 2:
			...
			signature = evp_signature_fetch_from_prov(
				(OSSL_PROVIDER *)tmp_prov, supported_sig,
				locpctx->propquery);
			if (signature == NULL)
				goto legacy;
			break;
		...
		if (tmp_keymgmt != NULL)
			provkey = evp_pkey_export_to_provider(
				locpctx->pkey, locpctx->libctx, &tmp_keymgmt,
				locpctx->propquery);
		if (tmp_keymgmt == NULL)
...
reinitialize:
	...
	if (type != NULL) {
		...
		if (mdname == NULL && !reinit) {
			if (evp_keymgmt_util_get_deflt_digest_name(
				    tmp_keymgmt, provkey, locmdname,
				    sizeof(locmdname)) > 0) {
...
{
	return do_sigver_init(ctx, pctx, NULL, mdname, libctx, props, NULL,
			      pkey, 0, params);
}
...
int EVP_DigestSignUpdate(EVP_MD_CTX *ctx, const void *data, size_t dsize)
{
	...
	if (pctx == NULL || pctx->operation != EVP_PKEY_OP_SIGNCTX ||
	...
	if (signature->digest_sign_update == NULL) {
		ERR_raise_data(ERR_LIB_EVP,
			       EVP_R_PROVIDER_SIGNATURE_NOT_SUPPORTED,
			       "%s digest_sign_update:%s", signature->type_name,
			       desc);
		return 0;
	}
...
int EVP_DigestVerifyUpdate(EVP_MD_CTX *ctx, const void *data, size_t dsize)
{
	...
	if (pctx == NULL || pctx->operation != EVP_PKEY_OP_VERIFYCTX ||
	    pctx->op.sig.algctx == NULL || pctx->op.sig.signature == NULL)
		goto legacy;
...
int EVP_DigestSignFinal(EVP_MD_CTX *ctx, unsigned char *sigret, size_t *siglen)
{
	...
	if (signature->digest_sign_final == NULL) {
		ERR_raise_data(ERR_LIB_EVP,
			       EVP_R_PROVIDER_SIGNATURE_NOT_SUPPORTED,
			       "%s digest_sign_final:%s", signature->type_name,
			       desc);
		return 0;
	}
...
legacy:
	...
	if (sigret != NULL) {
		...
		if (ctx->flags & EVP_MD_CTX_FLAG_FINALISE) {
			if (sctx)
				r = pctx->pmeth->signctx(pctx, sigret, siglen,
							 ctx);
			else
			...
			if (sctx)
				r = tmp_ctx->pctx->pmeth->signctx(
					tmp_ctx->pctx, sigret, siglen, tmp_ctx);
			else
		...
		if (sctx) {
			if (pctx->pmeth->signctx(pctx, sigret, siglen, ctx) <=
			    0)
				return 0;
		} else {
			...
			if (s <= 0 ||
			    EVP_PKEY_sign(pctx, sigret, siglen, NULL, s) <= 0)
				return 0;
		}
	}
	return 1;
	...
	if (pctx->operation == EVP_PKEY_OP_SIGNCTX &&
		...
		if (signature->digest_sign != NULL) {
			...
			if (sigret != NULL)
				ctx->flags |= EVP_MD_CTX_FLAG_FINALISED;
			ERR_set_mark();
			ret = signature->digest_sign(
				pctx->op.sig.algctx, sigret, siglen,
		...
		if (pctx->pmeth != NULL && pctx->pmeth->digestsign != NULL)
			return pctx->pmeth->digestsign(ctx, sigret, siglen, tbs,
						       tbslen);
	}
...
legacy:
	...
	if (ctx->flags & EVP_MD_CTX_FLAG_FINALISE) {
		...
		if (vctx)
			r = tmp_ctx->pctx->pmeth->verifyctx(
				tmp_ctx->pctx, sig, (int)siglen, tmp_ctx);
		else
			r = EVP_DigestFinal_ex(tmp_ctx, md, &mdlen);
		EVP_MD_CTX_free(tmp_ctx);
	}
		     ...
		     size_t siglen, const unsigned char *tbs, size_t tbslen)
{
	...
	if (pctx->operation == EVP_PKEY_OP_VERIFYCTX &&
		...
		if (pctx->op.sig.signature->digest_verify != NULL) {
			...
			ctx->flags |= EVP_MD_CTX_FLAG_FINALISED;
			ERR_set_mark();
			ret = signature->digest_verify(pctx->op.sig.algctx,
						       sigret, siglen, tbs,
						       tbslen);
			if (ret <= 0 && ERR_count_to_mark() == 0)
				ERR_raise_data(ERR_LIB_EVP,
		...
		if (pctx->pmeth != NULL && pctx->pmeth->digestverify != NULL)
			return pctx->pmeth->digestverify(ctx, sigret, siglen,
							 tbs, tbslen);
	}
