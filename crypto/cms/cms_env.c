					     ...
					     const char *propq)
{
	...
	if (!ossl_cms_EncryptedContent_init(env->encryptedContentInfo, cipher,
			       ...
			       OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	res = CMS_decrypt(ci, secret == NULL ? pkey : NULL,
			  secret == NULL ? cert : NULL, detached_data, bio,
			  flags);
				       ...
				       const CMS_CTX *ctx)
{
	...
	if (flags & CMS_KEY_PARAM) {
		ktri->pctx = EVP_PKEY_CTX_new_from_pkey(
			ossl_cms_ctx_get0_libctx(ctx), ktri->pkey,
			ossl_cms_ctx_get0_propq(ctx));
		if (ktri->pctx == NULL)
			return 0;
				      ...
				      X509 *originator, unsigned int flags)
{
	...
	case CMS_RECIPINFO_KEM:
		if (!ossl_cms_RecipientInfo_kemri_init(ri, recip, pk, flags,
						       ctx))
			goto err;
		break;
					  ...
					  ASN1_INTEGER **sno)
{
	...
	return ossl_cms_SignerIdentifier_get0_signer_id(ktri->rid, keyid,
							issuer, sno);
}
					  ...
					  CMS_RecipientInfo *ri)
{
	...
	if (EVP_PKEY_is_a(pkey, "RSA"))
		...
		EVP_PKEY_CTX_ctrl_str(ktri->pctx,
...
static BIO *cms_EnvelopedData_Decryption_init_bio(CMS_ContentInfo *cms)
{
	CMS_EncryptedContentInfo *ec =
		cms->d.envelopedData->encryptedContentInfo;
	BIO *contentBio = ossl_cms_EncryptedContent_init_bio(
		ec, ossl_cms_get0_cmsctx(cms));
	EVP_CIPHER_CTX *ctx = NULL;
...
int ossl_cms_pkey_is_ri_type_supported(EVP_PKEY *pk, int ri_type)
{
	...
	if (pk->ameth != NULL && pk->ameth->pkey_ctrl != NULL) {
		...
		i = pk->ameth->pkey_ctrl(
			pk, ASN1_PKEY_CTRL_CMS_IS_RI_TYPE_SUPPORTED, ri_type,
			&r);
		if (i > 0)
			return r;
	}
				     ...
				     const EVP_CIPHER *cipher)
{
	...
	if ((EVP_CIPHER_get_flags(cipher) & EVP_CIPH_FLAG_GET_WRAP_CIPHER) !=
	    0) {
		ret = EVP_CIPHER_meth_get_ctrl(
			cipher)(NULL, EVP_CTRL_GET_WRAP_CIPHER, 0, &kekcipher);
		if (ret <= 0)
			return 0;

		if (kekcipher != NULL) {
			if (EVP_CIPHER_get_mode(kekcipher) !=
			    EVP_CIPH_WRAP_MODE)
				return 0;
