...
static int cms_copy_messageDigest(CMS_ContentInfo *cms, CMS_SignerInfo *si)
{
	...
	for (i = 0; i < sk_CMS_SignerInfo_num(sinfos); i++) {
		...
		if (OBJ_cmp(si->digestAlgorithm->algorithm,
			...
			continue;
		messageDigest = CMS_signed_get0_data_by_OBJ(
			sitmp, OBJ_nid2obj(NID_pkcs9_messageDigest), -3,
			V_ASN1_OCTET_STRING);
		if (!messageDigest) {
...
{
	if (sid->type == CMS_SIGNERINFO_ISSUER_SERIAL)
		return ossl_cms_ias_cert_cmp(sid->d.issuerAndSerialNumber,
					     cert);
	else if (sid->type == CMS_SIGNERINFO_KEYIDENTIFIER)
		return ossl_cms_keyid_cert_cmp(sid->d.subjectKeyIdentifier,
					       cert);
	else
		return -1;
}
...
static int cms_signature_nomd(EVP_PKEY *pkey)
{
	...
	return EVP_PKEY_get_default_digest_name(pkey, def_md, sizeof(def_md)) ==
				       2 &&
...
static int cms_generic_sign(CMS_SignerInfo *si, int verify)
{
	...
	if (!verify) {
		...
		if (hnid == NID_undef)
			return -1;
		if (pknid <=
		    0) { /* check whether a provider registered a NID */
		...
		if (pknid > 0 && cms_signature_nomd(pkey))
			...
			return -1;
		return X509_ALGOR_set0(alg2, OBJ_nid2obj(snid), V_ASN1_UNDEF,
				       NULL);
	}
	return 1;
}
				      ...
				      const ESS_SIGNING_CERT *sc)
{
	...
	OPENSSL_free(pp);
	ret = CMS_signed_add1_attr_by_NID(si,
				...
				unsigned int flags)
{
	...
	if (!(flags & CMS_KEY_PARAM) && !cms_sd_asn1_ctrl(si, 0)) {
		ERR_raise_data(ERR_LIB_CMS,
			       CMS_R_UNSUPPORTED_SIGNATURE_ALGORITHM,
			       "pkey nid=%d", EVP_PKEY_get_id(pk));
		goto err;
	}
	if (!(flags & CMS_NOATTR)) {
		...
		if (flags & CMS_CADES) {
			...
			if (md == NULL || EVP_MD_is_a(md, SN_sha1)) {
				if ((sc = OSSL_ESS_signing_cert_new_init(
					     signer, NULL, 1)) == NULL)
					goto err;
				add_sc = ossl_cms_add1_signing_cert(si, sc);
				ESS_SIGNING_CERT_free(sc);
			} else {
				if ((sc2 = OSSL_ESS_signing_cert_v2_new_init(
					     md, signer, NULL, 1)) == NULL)
					goto err;
	...
	if (flags & CMS_KEY_PARAM) {
		if (flags & CMS_NOATTR) {
			si->pctx = EVP_PKEY_CTX_new_from_pkey(
				ossl_cms_ctx_get0_libctx(ctx), si->pkey,
...
static int cms_add1_signingTime(CMS_SignerInfo *si, ASN1_TIME *t)
{
	...
	if (CMS_signed_add1_attr_by_NID(si, NID_pkcs9_signingTime, tt->type, tt,
					-1) <= 0) {
		ERR_raise(ERR_LIB_CMS, ERR_R_CMS_LIB);
		goto err;
	}
...
static int cms_SignerInfo_content_sign(CMS_ContentInfo *cms, CMS_SignerInfo *si,
				       BIO *chain, const unsigned char *md,
				       unsigned int mdlen)
{
	...
	if (!ossl_cms_DigestAlgorithm_find_ctx(mctx, chain,
					       si->digestAlgorithm))
		goto err;
			      ...
			      unsigned int precomp_mdlen)
{
	...
	for (i = 0; i < sk_CMS_SignerInfo_num(sinfos); i++) {
		si = sk_CMS_SignerInfo_value(sinfos, i);
		if (!cms_SignerInfo_content_sign(cms, si, chain, precomp_md,
						 precomp_mdlen))
			return 0;
	}
...
int CMS_SignerInfo_sign(CMS_SignerInfo *si)
{
	...
	if (si->pctx) {
		...
		if (EVP_DigestSignInit_ex(
			    mctx, &pctx, md_name, ossl_cms_ctx_get0_libctx(ctx),
			    ossl_cms_ctx_get0_propq(ctx), si->pkey, NULL) <= 0)
			goto err;
	...
	if (md_name == NULL) {
		if (ASN1_item_sign_ctx(ASN1_ITEM_rptr(CMS_Attributes_Sign),
				       NULL, NULL, si->signature,
				       si->signedAttrs, mctx) <= 0)
			goto err;
		return 1;
	}
...
int CMS_SignerInfo_verify(CMS_SignerInfo *si)
{
	...
	if (cms_signature_nomd(si->pkey)) {
		r = ASN1_item_verify_ex(ASN1_ITEM_rptr(CMS_Attributes_Sign),
					...
					si->signedAttrs, NULL, si->pkey, libctx,
					propq);
		if (r <= 0)
	...
	if (EVP_DigestVerifyInit_ex(mctx, &si->pctx, EVP_MD_get0_name(md),
...
BIO *ossl_cms_SignedData_init_bio(CMS_ContentInfo *cms)
{
	...
	for (i = 0; i < sk_X509_ALGOR_num(sd->digestAlgorithms); i++) {
		...
		mdbio = ossl_cms_DigestAlgorithm_init_bio(
			digestAlgorithm, ossl_cms_get0_cmsctx(cms));
		if (mdbio == NULL)
			goto err;
		if (chain != NULL)
...
int CMS_SignerInfo_verify_content(CMS_SignerInfo *si, BIO *chain)
{
	...
	if (CMS_signed_get_attr_count(si) >= 0) {
		os = CMS_signed_get0_data_by_OBJ(
			si, OBJ_nid2obj(NID_pkcs9_messageDigest), -3,
			V_ASN1_OCTET_STRING);
		if (os == NULL) {
			ERR_raise(ERR_LIB_CMS,
				  CMS_R_ERROR_READING_MESSAGEDIGEST_ATTRIBUTE);
			    ...
			    int keysize)
{
	...
	alg = ossl_X509_ALGOR_from_nid(
		algnid, key != NULL ? V_ASN1_INTEGER : V_ASN1_UNDEF, key);
	if (alg == NULL) {
		ASN1_INTEGER_free(key);
		return 0;
	}
