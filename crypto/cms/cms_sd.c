...
int CMS_SignerInfo_sign(CMS_SignerInfo *si)
{
    ...
    char md_name_buf[CMS_MAX_NAME_SIZE], *md_name;
...
int CMS_SignerInfo_verify(CMS_SignerInfo *si)
{
    ...
    r = EVP_DigestVerifyFinal(mctx, ASN1_STRING_get0_data(si->signature),
        ASN1_STRING_get_length(si->signature));
    if (r <= 0)
        ERR_raise(ERR_LIB_CMS, CMS_R_VERIFICATION_FAILURE);
err:
...
int CMS_SignerInfo_verify_ex(CMS_SignerInfo *si, BIO *chain, BIO *data)
{
    ...
    if (os != NULL) {
        if (mlen != ASN1_STRING_get_length(os)) {
        ...
        if ((algorithm = cms_mdless_signing(si->pkey)) != NULL) {
            ...
            r = cms_EVP_PKEY_verify(pkctx, data,
                ...
                ASN1_STRING_get_length(si->signature), has_msg_update);
        } else {
            ...
            r = EVP_PKEY_verify(pkctx, ASN1_STRING_get0_data(si->signature),
                ASN1_STRING_get_length(si->signature), mval, mlen);
        }
        if (r <= 0) {
            ERR_raise(ERR_LIB_CMS, CMS_R_VERIFICATION_FAILURE);
            r = 0;
