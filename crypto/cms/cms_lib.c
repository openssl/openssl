	...
	if (pcerts != NULL) {
		for (i = 0; i < sk_CMS_CertificateChoices_num(*pcerts); i++) {
			...
			if (cch->type == CMS_CERTCHOICE_CERT)
				ossl_x509_set0_libctx(cch->d.certificate,
						      libctx, propq);
		}
	}
}
	...
	if (EVP_MD_xof(digest)) {
		...
		if (xof_len > 0) {
			...
			params[0] = OSSL_PARAM_construct_size_t(
				OSSL_DIGEST_PARAM_XOFLEN, &xof_len);
...
int ossl_cms_ias_cert_cmp(CMS_IssuerAndSerialNumber *ias, X509 *cert)
{
	...
	if (ret)
		return ret;
	return ASN1_INTEGER_cmp(ias->serialNumber,
...
int ossl_cms_set1_ias(CMS_IssuerAndSerialNumber **pias, X509 *cert)
{
	...
	if (!ASN1_STRING_copy(ias->serialNumber,
			      X509_get0_serialNumber(cert))) {
		ERR_raise(ERR_LIB_CMS, ERR_R_ASN1_LIB);
		goto err;
	}
		      ...
		      const char *propq)
{
	...
	evpcms = CMS_encrypt_ex(enc_recip, signbio, cipher, enc_flags, libctx,
				propq);
	if (evpcms == NULL)
		goto err;
