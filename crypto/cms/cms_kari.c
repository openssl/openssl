...
int CMS_RecipientInfo_kari_orig_id_cmp(CMS_RecipientInfo *ri, X509 *cert)
{
	...
	oik = ri->d.kari->originator;
	if (oik->type == CMS_OIK_ISSUER_SERIAL)
		return ossl_cms_ias_cert_cmp(oik->d.issuerAndSerialNumber,
					     cert);
	else if (oik->type == CMS_OIK_KEYIDENTIFIER)
		return ossl_cms_keyid_cert_cmp(oik->d.subjectKeyIdentifier,
					       cert);
	return -1;
}
				       ...
				       X509 *cert)
{
	...
	if (rid->type == CMS_REK_ISSUER_SERIAL)
		return ossl_cms_ias_cert_cmp(rid->d.issuerAndSerialNumber,
					     cert);
	else if (rid->type == CMS_REK_KEYIDENTIFIER)
		return ossl_cms_keyid_cert_cmp(
			rid->d.rKeyId->subjectKeyIdentifier, cert);
	else
		return -1;
}
					      ...
					      EVP_PKEY *pk, X509 *peer)
{
	...
	pctx = EVP_PKEY_CTX_new_from_pkey(
		ossl_cms_ctx_get0_libctx(kari->cms_ctx), pk,
				     ...
				     unsigned int flags, const CMS_CTX *ctx)
{
	...
	if (!sk_CMS_RecipientEncryptedKey_push(kari->recipientEncryptedKeys,
					       rek)) {
	...
	if (flags & CMS_USE_KEYID) {
		...
		if (rek->rid->d.rKeyId == NULL)
			return 0;
		if (!ossl_cms_set1_keyid(
			    &rek->rid->d.rKeyId->subjectKeyIdentifier, recip))
			return 0;
	} else {
		rek->rid->type = CMS_REK_ISSUER_SERIAL;
		if (!ossl_cms_set1_ias(&rek->rid->d.issuerAndSerialNumber,
				       recip))
			return 0;
	}
	...
	if (originatorPrivKey == NULL && originator == NULL) {
		...
		if (flags & CMS_USE_ORIGINATOR_KEYID) {
			...
			if (oik->d.subjectKeyIdentifier == NULL)
				return 0;
			if (!ossl_cms_set1_keyid(&oik->d.subjectKeyIdentifier,
						 originator))
				return 0;
		} else {
			oik->type = CMS_REK_ISSUER_SERIAL;
			if (!ossl_cms_set1_ias(&oik->d.issuerAndSerialNumber,
					       originator))
				return 0;
		}

		if (!cms_kari_set_originator_private_key(kari,
							 originatorPrivKey))
			return 0;
	}
