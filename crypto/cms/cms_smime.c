...
CMS_ContentInfo *CMS_EncryptedData_encrypt(BIO *in, const EVP_CIPHER *cipher,
					   ...
					   size_t keylen, unsigned int flags)
{
	return CMS_EncryptedData_encrypt_ex(in, cipher, key, keylen, flags,
					    NULL, NULL);
}
	       ...
	       BIO *dcont, BIO *out, unsigned int flags)
{
	...
	if ((flags & CMS_NO_SIGNER_CERT_VERIFY) == 0 || cadesVerify) {
		if (cadesVerify) {
			...
			si_chains =
				OPENSSL_calloc(scount, sizeof(si_chains[0]));
			if (si_chains == NULL)
				goto err;
		}
		...
		for (i = 0; i < scount; i++) {
			...
			if (!cms_signerinfo_verify_cert(
				    si, store, untrusted, crls,
				    si_chains ? &si_chains[i] : NULL, ctx))
				goto err;
		}
	}
	...
	if ((flags & CMS_NO_ATTR_VERIFY) == 0 || cadesVerify) {
		for (i = 0; i < scount; i++) {
			...
			if (CMS_SignerInfo_verify(si) <= 0)
				goto err;
			if (cadesVerify) {
				STACK_OF(X509) *si_chain =
					si_chains ? si_chains[i] : NULL;

				if (ossl_cms_check_signing_certs(si,
								 si_chain) <= 0)
					goto err;
			}
		}
			     ...
			     const char *propq)
{
	...
	if (pkey != NULL &&
	    !CMS_add1_signer(cms, signcert, pkey, NULL, flags)) {
...
static int cms_kari_set1_pkey_and_peer(CMS_ContentInfo *cms,
				       CMS_RecipientInfo *ri, EVP_PKEY *pk,
				       X509 *cert, X509 *peer)
{
	int i;
	...
	for (i = 0; i < sk_CMS_RecipientEncryptedKey_num(reks); i++) {
		...
		if (cert != NULL &&
		    CMS_RecipientEncryptedKey_cert_cmp(rek, cert))
			continue;
				   ...
				   X509 *cert, X509 *peer)
{
	...
	for (i = 0; i < sk_CMS_RecipientInfo_num(ris); i++) {
		...
		if (ri_type == CMS_RECIPINFO_AGREE) {
			r = cms_kari_set1_pkey_and_peer(cms, ri, pk, cert,
							peer);
			if (r > 0)
				return 1;
			if (r < 0)
			...
			if (cert == NULL ||
			    !CMS_RecipientInfo_kemri_cert_cmp(ri, cert)) {
	...
	if (cert == NULL && cms_pkey_ri_type == CMS_RECIPINFO_TRANS &&
	    match_ri && !debug) {
		ERR_clear_error();
		return 1;
	}
...
int CMS_decrypt(CMS_ContentInfo *cms, EVP_PKEY *pk, X509 *cert, BIO *dcont,
