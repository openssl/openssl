...
static int cms_signerinfos_remainig(const STACK_OF(CMS_SignerInfo) *sinfos)
{
    int i, remaining = 0;
    ...
    X509_STORE *store, BIO *dcont, BIO *out, unsigned int flags)
{
    ...
    for (i = 0; i < scount; i++) {
        ...
        if (si->verify_failure)
            continue;
        if ((flags & CMS_NO_SIGNER_CERT_VERIFY) == 0 && !si->cert_verified)
            continue;
        if ((flags & CMS_NO_ATTR_VERIFY) == 0 && !si->attr_verified)
            continue;
        if ((flags & CMS_NO_CONTENT_VERIFY) == 0 && !si->content_verified)
            continue;
    ...
    if ((flags & CMS_VERIFY_PARTIAL) != 0)
        ret = cms_signerinfos_remainig(sinfos) > 0; /* One success is enough */
    else
        ret = cms_signerinfos_remainig(sinfos) == scount; /* All must be successful */
err:
    if (!(flags & SMIME_BINARY) && dcont) {
