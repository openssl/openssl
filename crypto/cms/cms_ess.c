...
int ossl_cms_Receipt_verify(CMS_ContentInfo *cms, CMS_ContentInfo *req_cms)
{
	...
	if (OBJ_obj2nid(CMS_get0_eContentType(cms)) !=
	...
	msig = CMS_signed_get0_data_by_OBJ(
		si, OBJ_nid2obj(NID_id_smime_aa_msgSigDigest), -3,
		V_ASN1_OCTET_STRING);
...
ASN1_OCTET_STRING *ossl_cms_encode_Receipt(CMS_SignerInfo *si)
{
	...
	ctype = CMS_signed_get0_data_by_OBJ(
		si, OBJ_nid2obj(NID_pkcs9_contentType), -3, V_ASN1_OBJECT);
	if (!ctype) {
		ERR_raise(ERR_LIB_CMS, CMS_R_NO_CONTENT_TYPE);
		goto err;
	}
