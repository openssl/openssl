			  ...
			  EVP_CIPHER_CTX *ctx)
{
	...
	if (!EVP_DecryptUpdate(ctx, tmp + inlen - 2 * blocklen, &outl,
         ...
         */
	    || !EVP_DecryptUpdate(ctx, tmp, &outl, tmp + inlen - blocklen,
				  blocklen)
		...
		goto err;
	/* Check check bytes */
	if (((tmp[1] ^ tmp[4]) & (tmp[2] ^ tmp[5]) & (tmp[3] ^ tmp[6])) !=
	    0xff) {
		/* Check byte failure */
		goto err;
	}
			...
			EVP_CIPHER_CTX *ctx, const CMS_CTX *cms_ctx)
{
	...
	if (out) {
		...
		if (olen > inlen + 4 &&
		    RAND_bytes_ex(ossl_cms_ctx_get0_libctx(cms_ctx),
				  out + 4 + inlen, olen - 4 - inlen, 0) <= 0)
			return 0;
		/* Encrypt twice */
				      ...
				      CMS_RecipientInfo *ri, int en_de)
{
	...
	if (EVP_PBE_CipherInit_ex(algtmp->algorithm, (char *)pwri->pass,
				  (int)pwri->passlen, algtmp->parameter, kekctx,
				  en_de, cms_ctx->libctx, cms_ctx->propq) < 0) {
		ERR_raise(ERR_LIB_CMS, ERR_R_EVP_LIB);
		goto err;
	}
