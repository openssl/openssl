...
int BN_X931_generate_Xpq(BIGNUM *Xp, BIGNUM *Xq, int nbits, BN_CTX *ctx)
{
	...
	for (i = 0; i < 1000; i++) {
		if (!BN_priv_rand_ex(Xq, nbits, BN_RAND_TOP_TWO,
			      ...
			      BN_CTX *ctx, BN_GENCB *cb)
{
	...
	if (!BN_priv_rand_ex(Xp1, 101, BN_RAND_TOP_ONE, BN_RAND_BOTTOM_ANY, 0,
			     ctx))
		goto error;
	if (!BN_priv_rand_ex(Xp2, 101, BN_RAND_TOP_ONE, BN_RAND_BOTTOM_ANY, 0,
			     ctx))
		goto error;
