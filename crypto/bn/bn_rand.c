                  ...
                  BN_CTX *ctx)
{
    ...
    b = flag == NORMAL ? RAND_bytes_with_libctx(libctx, buf, bytes)
                       : RAND_priv_bytes_with_libctx(libctx, buf, bytes);
    if (b <= 0)
        goto err;
    ...
    if (flag == TESTING) {
        ...
        for (i = 0; i < bytes; i++) {
            if (RAND_bytes_with_libctx(libctx, &c, 1) <= 0)
                goto err;
            if (c >= 128 && i > 0)
                buf[i] = buf[i - 1];
            else if (c < 42)
                          ...
                          size_t message_len, BN_CTX *ctx)
{
    ...
    for (done = 0; done < num_k_bytes;) {
        if (!RAND_priv_bytes_with_libctx(libctx, random_bytes,
                                         sizeof(random_bytes)))
            goto err;
