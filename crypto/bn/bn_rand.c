			...
			unsigned int strength, BN_CTX *ctx)
{
	...
	if (n == 1)
         ...
         */
		do {
			if (!bnrand(flag, r, n + 1, BN_RAND_TOP_ANY,
				    BN_RAND_BOTTOM_ANY, strength, ctx))
				return 0;
		...
		do {
			...
			if (!bnrand(flag, r, n, BN_RAND_TOP_ANY,
				    BN_RAND_BOTTOM_ANY, strength, ctx))
				return 0;
				      ...
				      unsigned int strength, BN_CTX *ctx)
{
	...
	if (n == 1) {
		...
		do {
			if (!bnrand(PRIVATE, r, n + 1, BN_RAND_TOP_ONE,
				    BN_RAND_BOTTOM_ANY, strength, ctx))
				return 0;
				    ...
				    size_t message_len, BN_CTX *ctx)
{
	...
	for (n = 0; n < max_n; n++) {
		...
		for (done = 1; done < num_k_bytes;) {
			if (RAND_priv_bytes_ex(libctx, random_bytes,
					       sizeof(random_bytes), 0) <= 0)
				goto end;
