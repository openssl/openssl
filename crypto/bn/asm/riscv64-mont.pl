...
.L1st:
    ...
    add $lo0, $alo, $hi0    # {ap[j]*bp[0],low}+{ap[j-1]*bp[0],high}
    sltu $carry1, $lo0, $alo
    add $hi0, $ahi, $carry1    # {ap[j]*bp[0],high}+C_lo0
    ...
    add $lo1, $nlo, $hi1    # {np[j]*m1,low}+{np[j-1]*m1,high}
    sltu $carry1, $lo1, $nlo
    add $hi1, $nhi, $carry1    # {np[j]*m1,high}+C_lo1
...
.L1st_last_entry:
    ...
    add $lo0, $alo, $hi0    # {ap[num-1]*bp[0],low}+{ap[num-2]*bp[0],high}
    sltu $carry1, $lo0, $alo
    add $hi0, $ahi, $carry1    # {ap[num-1]*bp[0],high}+C_lo0

    sub $ap, $ap, $num    # rewind $ap
    sub $np, $np, $num    # rewind $np

    # last reduction term
    add $lo1, $nlo, $hi1    # {np[num-1]*m1,low}+{np[num-1]*m1,high}
    sltu $carry1, $lo1, $nlo
    add $hi1, $nhi, $carry1    # {np[num-1]*m1,high}+C_lo1
...
.Linner:
    ...
    # compute the sum of partial products of a and b
    add $lo0, $alo, $hi0    # {ap[j]*bp[i],low}+{ap[j-1]*bp[i],high}
    sltu $carry1, $lo0, $alo
    add $hi0, $ahi, $carry1    # {ap[j]*bp[i],high}+C_lo0
    ...
    add $lo1, $nlo, $hi1    # {np[j]*m1,low}+{np[j-1]*m1,high}
    sltu $carry1, $lo1, $nlo
    add $hi1, $nhi, $carry1    # {np[j]*m1}+C_lo1
...
.Linner_last_entry:
    ...
    # last partial product
    add $lo0, $alo, $hi0    # {ap[num-1]*bp[i],low}+{ap[num-2]*bp[i],high}
    sltu $carry1, $lo0, $alo
    add $hi0, $ahi, $carry1    # {ap[num-1]*bp[i],high}+C_lo0
    ...
    add $hi1, $temp, $carry1    # {np[num-1]*m1,high}+C_lo1
    sltu $ovf, $hi1, $temp
    or $carry1, $carry2, $ovf
    mv $ovf, $carry1
    ...
    ld $nj, 0($np)    # np[0]
    addi $np, $np, 8
    addi $j, $num, -8    # j=num-1 and clear borrow
    sltu $carry1, $num, 8
    xori $carry1, $carry1, 1
    mv $ap, $rp

.Lsub:
    # tp[j]-np[j],set carry flag
    xori $carry1, $carry1,1
    sub $temp, $tj, $nj
    sltu $carry2, $tj, $temp
    sub $aj, $temp, $carry1
    ...
    sd $aj, 0($ap)    # rp[j]=tp[j]-np[j]
    addi $ap, $ap, 8
    bnez $j, .Lsub

    # process the last word, tp[j]-np[j]
    xori $carry1, $carry1,1
    sub $temp, $tj, $nj
    sltu $carry2, $tj, $temp
    sub $aj, $temp, $carry1
