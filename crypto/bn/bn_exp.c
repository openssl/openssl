			      ...
			      BN_MONT_CTX *in_mont)
{
	...
	if (a->neg || BN_ucmp(a, m) >= 0) {
		...
		if (reduced == NULL || !BN_nnmod(reduced, a, m, ctx)) {
			goto err;
		}
		a = reduced;
	}
	...
	if (t4) {
		...
		int stride =
			5 * (6 - (top / 16 - 1)); /* multiple of 5, but less
                                                * than 32 */
		...
		if (top & 7) {
			while (bits > 0) {
				...
				bn_mul_mont(tmp.d, tmp.d, tmp.d, np, n0, top);
				bn_mul_mont_gather5(
					tmp.d, tmp.d, powerbuf, np, n0, top,
...
fallback:
		...
		if (window > 1) {
			...
			for (i = 3; i < numPowers; i++) {
				/* Calculate a^i = a^(i-1) * a */
				if (!bn_mul_mont_fixed_top(&tmp, &am, &tmp,
							   mont, ctx))
					goto err;
		...
		while (bits > 0) {
			...
			for (i = 0; i < window; i++)
				if (!bn_mul_mont_fixed_top(&tmp, &tmp, &tmp,
							   mont, ctx))
					goto err;
