					 ...
					 STACK_OF(OPENSSL_CSTRING) *visited)
{
	...
	if (sect != NULL) {
		...
		for (i = 0; i < sk_CONF_VALUE_num(sect); i++) {
			...
			if (buffer_len + strlen(sectconf->name) >=
			    sizeof(buffer)) {
			...
			rc = provider_conf_params_internal(prov, provinfo,
							   buffer,
							   sectconf->value, cnf,
							   visited);
			if (rc < 0) {
		...
		if (prov != NULL)
			ok = OSSL_PROVIDER_add_conf_parameter(prov, name,
							      value);
		else
			ok = ossl_provider_info_add_parameter(provinfo, name,
							      value);
	}
        ...
        */
		...
		if (ok == 1) {
			if (!ossl_provider_activate(prov, 1, 0)) {
				ok = 0;
			} else if (!ossl_provider_add_to_store(prov, &actual,
							       0)) {
					    ...
					    const char *confvalue, int *val)
{
	...
	if ((strcmp(confvalue, "1") == 0) || (strcmp(confvalue, "yes") == 0) ||
	    ...
	    (strcmp(confvalue, "TRUE") == 0) ||
	    (strcmp(confvalue, "on") == 0) || (strcmp(confvalue, "ON") == 0)) {
		*val = 1;
			      ...
			      const char *value, const CONF *cnf)
{
	...
	for (i = 0; i < sk_CONF_VALUE_num(ecmds); i++) {
		...
		if (strcmp(confname, "identity") == 0) {
			...
			if (!provider_conf_parse_bool_setting(
				    confname, confvalue, &activate))
				return 0;
		}
	}

	if (activate) {
		ok = provider_conf_activate(libctx, name, value, path, soft,
					    cnf);
	} else {
		...
		if (ok)
			ok = provider_conf_params(NULL, &entry, NULL, value,
						  cnf);
		if (ok >= 1 &&
		    (entry.path != NULL || entry.parameters != NULL)) {
