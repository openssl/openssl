...
{
    size_t len;
    LMS_KEY *lms = hsskey->public;
    uint8_t *buf = NULL;
    WPACKET pkt;

    if (out == NULL)
        return 0;
    len = HSS_SIZE_L + lms->pub.encodedlen;

    if ((buf = OPENSSL_malloc(len)) == NULL)
        return 0;
    if (!WPACKET_init_static_len(&pkt, buf, len, 0)
            || !WPACKET_put_bytes_u32(&pkt, hsskey->L)
            || !WPACKET_memcpy(&pkt, lms->pub.encoded, lms->pub.encodedlen)) {
        OPENSSL_free(buf);
        buf = NULL;
        len = 0;
    }
