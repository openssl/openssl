	...
	*ishex = CHECK_AND_SKIP_PREFIX(key, "hex");
	...
	case OSSL_PARAM_UNSIGNED_INTEGER:
		...
		if (p->data_type == OSSL_PARAM_INTEGER &&
		    BN_is_negative(*tmpbn) && !BN_add_word(*tmpbn, 1)) {
			return 0;
		}
	...
	if (buf_n > 0) {
		...
		case OSSL_PARAM_OCTET_STRING:
			if (ishex) {
				...
				if (!OPENSSL_hexstr2buf_ex(buf, buf_n, &l,
							   value, ':'))
					return 0;
			} else {
				  ...
				  size_t value_n, int *found)
{
	...
	if (!prepare_from_text(paramdefs, key, value, value_n, &paramdef,
			       &ishex, &buf_n, &tmpbn, found))
		goto err;

	if ((buf = OPENSSL_zalloc(buf_n > 0 ? buf_n : 1)) == NULL)
		goto err;

	ok = construct_from_text(to, paramdef, value, value_n, ishex, buf,
				 buf_n, tmpbn);
	BN_free(tmpbn);
	if (!ok)
		OPENSSL_free(buf);
