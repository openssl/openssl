...
{
    RAND_DRBG *drbg = RAND_DRBG_get_default();
    int ret = 0;
    ...
    do {
        ...
        if (count > (int)drbg->max_request)
            rcnt = drbg->max_request;
        else
            rcnt = count;
        ret = RAND_DRBG_generate(drbg, out, rcnt, 0, NULL, 0);
        if (!ret)
            goto err;
        out += rcnt;
        count -= rcnt;
...
static int drbg_status(void)
{
    ...
    CRYPTO_THREAD_write_lock(drbg->lock);
    ret = drbg->status == DRBG_STATUS_READY ? 1 : 0;
    CRYPTO_THREAD_unlock(drbg->lock);
    return ret;
}
