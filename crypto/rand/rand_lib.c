...
int RAND_poll(void)
{
	...
	if (!ret) {
		...
		RAND_POOL *pool = ossl_rand_pool_new(
			RAND_DRBG_STRENGTH, 1, (RAND_DRBG_STRENGTH + 7) / 8,
...
static void rand_delete_thread_state(void *arg)
{
	...
	rand = CRYPTO_THREAD_get_local_ex(CRYPTO_THREAD_LOCAL_DRBG_PUB_KEY,
					  ctx);
	CRYPTO_THREAD_set_local_ex(CRYPTO_THREAD_LOCAL_DRBG_PUB_KEY, ctx, NULL);
	EVP_RAND_CTX_free(rand);

	rand = CRYPTO_THREAD_get_local_ex(CRYPTO_THREAD_LOCAL_DRBG_PRIV_KEY,
					  ctx);
				   ...
				   time_t reseed_time_interval)
{
	...
	if (OSSL_PARAM_locate_const(settables, OSSL_DRBG_PARAM_CIPHER)) {
		cipher = dgbl->rng_cipher != NULL ? dgbl->rng_cipher :
						    "AES-256-CTR";
	...
	if (prov_name != NULL)
		*p++ = OSSL_PARAM_construct_utf8_string(
			OSSL_PROV_PARAM_CORE_PROV_NAME, (char *)prov_name, 0);
	if (dgbl->rng_propq != NULL)
		*p++ = OSSL_PARAM_construct_utf8_string(
			OSSL_DRBG_PARAM_PROPERTIES, dgbl->rng_propq, 0);
	if (OSSL_PARAM_locate_const(settables, OSSL_ALG_PARAM_MAC))
		*p++ = OSSL_PARAM_construct_utf8_string(OSSL_ALG_PARAM_MAC,
							"HMAC", 0);
...
static EVP_RAND_CTX *rand_get0_public(OSSL_LIB_CTX *ctx, RAND_GLOBAL *dgbl)
{
	...
	if (rand == NULL) {
         ...
         * used this thread.
         */
		if (CRYPTO_THREAD_get_local_ex(
			    CRYPTO_THREAD_LOCAL_DRBG_PRIV_KEY, ctx) == NULL &&
		    !ossl_init_thread_start(NULL, ctx,
					    rand_delete_thread_state))
			return NULL;
		rand = rand_new_drbg(ctx, primary, SECONDARY_RESEED_INTERVAL,
				     SECONDARY_RESEED_TIME_INTERVAL);
		if (!CRYPTO_THREAD_set_local_ex(
			    CRYPTO_THREAD_LOCAL_DRBG_PUB_KEY, ctx, rand)) {
			EVP_RAND_CTX_free(rand);
			rand = NULL;
		}
	}
...
static EVP_RAND_CTX *rand_get0_private(OSSL_LIB_CTX *ctx, RAND_GLOBAL *dgbl)
{
	...
	if (rand == NULL) {
         ...
         * used this thread.
         */
		if (CRYPTO_THREAD_get_local_ex(CRYPTO_THREAD_LOCAL_DRBG_PUB_KEY,
					       ctx) == NULL &&
		    !ossl_init_thread_start(NULL, ctx,
					    rand_delete_thread_state))
			return NULL;
		rand = rand_new_drbg(ctx, primary, SECONDARY_RESEED_INTERVAL,
				     SECONDARY_RESEED_TIME_INTERVAL);
		if (!CRYPTO_THREAD_set_local_ex(
			    CRYPTO_THREAD_LOCAL_DRBG_PRIV_KEY, ctx, rand)) {
			EVP_RAND_CTX_free(rand);
			rand = NULL;
		}
	}
...
EVP_RAND_CTX *ossl_rand_get0_private_noncreating(OSSL_LIB_CTX *ctx)
{
	...
	return CRYPTO_THREAD_get_local_ex(CRYPTO_THREAD_LOCAL_DRBG_PRIV_KEY,
					  ctx);
}
#endif
...
int RAND_set0_public(OSSL_LIB_CTX *ctx, EVP_RAND_CTX *rand)
{
	...
	if ((r = CRYPTO_THREAD_set_local_ex(CRYPTO_THREAD_LOCAL_DRBG_PUB_KEY,
					    ctx, rand)) > 0)
...
int RAND_set0_private(OSSL_LIB_CTX *ctx, EVP_RAND_CTX *rand)
{
	...
	if (dgbl == NULL)
		return 0;
	old = CRYPTO_THREAD_get_local_ex(CRYPTO_THREAD_LOCAL_DRBG_PRIV_KEY,
					 ctx);
	if ((r = CRYPTO_THREAD_set_local_ex(CRYPTO_THREAD_LOCAL_DRBG_PRIV_KEY,
					    ctx, rand)) > 0)
		EVP_RAND_CTX_free(old);
	return r;
}
	...
	for (i = 0; i < sk_CONF_VALUE_num(elist); i++) {
		...
		if (OPENSSL_strcasecmp(cval->name, "random") == 0) {
			...
			if (!random_set_string(&dgbl->seed_name, cval->value))
				return 0;
		} else if (OPENSSL_strcasecmp(cval->name, "seed_properties") ==
			   0) {
			if (!random_set_string(&dgbl->seed_propq, cval->value))
				return 0;
		} else if (OPENSSL_strcasecmp(cval->name, "random_provider") ==
			   0) {
#ifndef FIPS_MODULE
			OSSL_PROVIDER *prov =
				ossl_provider_find(libctx, cval->value, 0);
					    ...
					    OSSL_PROVIDER *prov)
{
	...
	if (strcmp(dgbl->random_provider_name, OSSL_PROVIDER_get0_name(prov)) !=
	    0)
		return 1;
