...
#pragma environment restore
	...
	for (;;) {
		if (bytes > 0)
			n = (bytes <= RAND_LOAD_BUF_SIZE) ? (int)bytes :
							    RAND_BUF_SIZE;
		else
	...
	fclose(in);
	if (!RAND_status()) {
		ERR_raise_data(ERR_LIB_RAND, RAND_R_RESEED_ERROR, "Filename=%s",
			       file);
		return -1;
	}
...
const char *RAND_file_name(char *buf, size_t size)
{
	...
	if ((envlen = GetEnvironmentVariableW(var = L"RANDFILE", NULL, 0)) ==
	    0) {
		use_randfile = 0;
		if ((envlen = GetEnvironmentVariableW(var = L"HOME", NULL,
						      0)) == 0 &&
		    (envlen = GetEnvironmentVariableW(var = L"USERPROFILE",
						      NULL, 0)) == 0)
			envlen = GetEnvironmentVariableW(var = L"SYSTEMROOT",
							 NULL, 0);
	}
