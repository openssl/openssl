	...
	*p++ = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_DIGEST,
	...
	if (propq != NULL)
		*p++ = OSSL_PARAM_construct_utf8_string(
			OSSL_KDF_PARAM_PROPERTIES, (char *)propq, 0);
	*p++ = OSSL_PARAM_construct_octet_string(
		OSSL_KDF_PARAM_HMACDRBG_ENTROPY, (void *)entropy, entropylen);
					 ...
					 const char *propq)
{
	...
	unsigned char *entropyx = NULL, *nonceh = NULL, *rbits = NULL,
		      *T = NULL;
	size_t allocsz = 0;
	...
	kdfctx = kdf_setup(digestname, entropyx, rlen, nonceh, rlen, libctx,
			   propq);
	if (kdfctx == NULL)
		goto end;
