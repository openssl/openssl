...
#define set_func(c, f)            \
	do {                      \
		...
		case OSSL_FUNC_GET_USER_ENTROPY:
			set_func(c_get_user_entropy,
				 OSSL_FUNC_get_user_entropy(fns));
			break;
		case OSSL_FUNC_CLEANUP_ENTROPY:
			set_func(c_cleanup_entropy,
		...
		case OSSL_FUNC_GET_USER_NONCE:
			set_func(c_get_user_nonce,
				 OSSL_FUNC_get_user_nonce(fns));
			break;
			     ...
			     int entropy, size_t min_len, size_t max_len)
{
	...
	if (c_get_user_entropy != NULL)
		return c_get_user_entropy(handle, pout, entropy, min_len,
					  max_len);
			   ...
			   size_t salt_len)
{
	...
	if (c_get_user_nonce != NULL)
		return c_get_user_nonce(handle, pout, min_len, max_len, salt,
					salt_len);
	if (c_get_nonce != NULL)
		return c_get_nonce(handle, pout, min_len, max_len, salt,
				   salt_len);
	return 0;
}
