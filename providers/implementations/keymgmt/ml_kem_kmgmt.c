...
static void *ml_kem_new(void *vctx, int evp_type)
{
    ...
    if (!ossl_prov_is_running())
        return NULL;
    if (vctx != NULL) {
        libctx = PROV_LIBCTX_OF(vctx);
        prov = (OSSL_PROVIDER *)ossl_prov_ctx_get0_handle(vctx);
        retain_seed =
            OSSL_PROVIDER_conf_get_bool(prov, "pkey_retain_seed", 1);
    }
                             ...
                             const OSSL_PARAM params[], int evp_type)
{
    ...
    prov = (OSSL_PROVIDER *)ossl_prov_ctx_get0_handle(provctx);
    gctx->retain_seed =
        OSSL_PROVIDER_conf_get_bool(prov, "pkey_retain_seed", 1);
    if (provctx != NULL)
