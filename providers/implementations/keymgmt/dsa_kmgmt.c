		      ...
		      void *cbarg)
{
	...
	if ((selection & OSSL_KEYMGMT_SELECT_ALL_PARAMETERS) != 0)
		ok = ok && ossl_ffc_params_todata(ossl_dsa_get0_params(dsa),
						  tmpl, NULL);
	if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
		int include_private =
			selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY ? 1 : 0;
...
static const OSSL_PARAM dsa_all_types[] = { DSA_IMEXPORTABLE_PARAMETERS,
					    DSA_IMEXPORTABLE_PUBLIC_KEY,
					    DSA_IMEXPORTABLE_PRIVATE_KEY,
					    OSSL_PARAM_END };
static const OSSL_PARAM dsa_parameter_types[] = { DSA_IMEXPORTABLE_PARAMETERS,
						  OSSL_PARAM_END };
static const OSSL_PARAM dsa_key_types[] = { DSA_IMEXPORTABLE_PUBLIC_KEY,
					    DSA_IMEXPORTABLE_PRIVATE_KEY,
					    OSSL_PARAM_END };
...
static ossl_inline int dsa_get_params(void *key, OSSL_PARAM params[])
{
	...
	if ((p = OSSL_PARAM_locate(
		     params, OSSL_PKEY_PARAM_SECURITY_CATEGORY)) != NULL &&
	    !OSSL_PARAM_set_int(p, 0))
		return 0;
	return ossl_ffc_params_todata(ossl_dsa_get0_params(dsa), NULL,
				      params) &&
			  ...
			  const OSSL_PARAM params[])
{
	...
	if (!ossl_prov_is_running() ||
	    (selection & DSA_POSSIBLE_SELECTIONS) == 0)
		return NULL;
...
static int dsa_gen_set_params(void *genctx, const OSSL_PARAM params[])
{
	...
	if (p != NULL && (p->data_type != OSSL_PARAM_OCTET_STRING ||
		...
		return 0;
	if ((p = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_FFC_PBITS)) !=
		    NULL &&
						 ...
						 ossl_unused void *provctx)
{
	static OSSL_PARAM settable[] = {
		...
		OSSL_PARAM_utf8_string(OSSL_PKEY_PARAM_FFC_DIGEST_PROPS, NULL,
				       0),
		OSSL_PARAM_int(OSSL_PKEY_PARAM_FFC_GINDEX, NULL),
		OSSL_PARAM_octet_string(OSSL_PKEY_PARAM_FFC_SEED, NULL, 0),
		OSSL_PARAM_int(OSSL_PKEY_PARAM_FFC_PCOUNTER, NULL),
		...
		OSSL_FIPS_IND_SETTABLE_CTX_PARAM(
			OSSL_PKEY_PARAM_FIPS_SIGN_CHECK) OSSL_PARAM_END
	};
	return settable;
}
...
static void *dsa_gen(void *genctx, OSSL_CALLBACK *osslcb, void *cbarg)
{
	...
	if (!ossl_assert((gctx->gen_type >= DSA_PARAMGEN_TYPE_FIPS_186_4) &&
		...
		ERR_raise_data(ERR_LIB_PROV, ERR_R_INTERNAL_ERROR,
			       "gen_type set to unsupported value %d",
			       gctx->gen_type);
		goto end;
	}
	...
	if ((gctx->selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0) {
		if (ossl_dsa_generate_ffc_parameters(
			    dsa, gctx->gen_type, (int)gctx->pbits,
	...
	if ((gctx->selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
		if (ffc->p == NULL || ffc->q == NULL || ffc->g == NULL)
			goto end;
