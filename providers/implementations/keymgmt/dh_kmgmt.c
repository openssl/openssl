...
static int dh_gen_common_set_params(void *genctx, const OSSL_PARAM params[])
{
	...
	if (p != NULL) {
		if (p->data_type != OSSL_PARAM_UTF8_STRING ||
		    ((gen_type = dh_gen_type_name2id_w_default(
			      p->data, gctx->dh_type)) == -1)) {
...
static int dh_gen_set_params(void *genctx, const OSSL_PARAM params[])
{
	...
	if (OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_FFC_GINDEX) !=
		    NULL ||
	    OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_FFC_PCOUNTER) !=
		    NULL ||
	    OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_FFC_H) != NULL ||
	    OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_FFC_SEED) != NULL ||
	    OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_FFC_QBITS) !=
		    NULL ||
	    OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_FFC_DIGEST) !=
		    NULL ||
...
static void *dh_gen(void *genctx, OSSL_CALLBACK *osslcb, void *cbarg)
{
	...
	if (gctx->gen_type == DH_PARAMGEN_TYPE_GROUP &&
		...
		if (gctx->group_nid == NID_undef)
			gctx->group_nid = ossl_dh_get_named_group_uid_from_size(
				(int)gctx->pbits);
		if (gctx->group_nid == NID_undef)
			return NULL;
		...
		if (gctx->gindex != -1) {
			...
			if (gctx->pcounter != -1)
				ossl_ffc_params_set_pcounter(ffc,
							     gctx->pcounter);
		...
		if ((gctx->selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) !=
		    0) {
			/*
			...
			if (gctx->gen_type == DH_PARAMGEN_TYPE_GENERATOR)
				ret = DH_generate_parameters_ex(
					dh, (int)gctx->pbits, gctx->generator,
					gencb);
			else
				ret = ossl_dh_generate_ffc_parameters(
					dh, gctx->gen_type, (int)gctx->pbits,
