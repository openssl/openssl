				     ...
				     unsigned char **pub_key)
{
	...
	if (pub_point != NULL) {
		...
		if (tmpl == NULL) {
			...
			px = OSSL_PARAM_locate(params,
					       OSSL_PKEY_PARAM_EC_PUB_X);
			py = OSSL_PARAM_locate(params,
		...
		if (p != NULL || tmpl != NULL) {
			...
			if ((pub_key_len = EC_POINT_point2buf(ecg, pub_point,
			    ...
			    !ossl_param_build_set_octet_string(
				    tmpl, p, OSSL_PKEY_PARAM_PUB_KEY, *pub_key,
				    pub_key_len))
				goto err;
		}
		if (px != NULL || py != NULL) {
			...
			if (!EC_POINT_get_affine_coordinates(ecg, pub_point, x,
							     y, bnctx))
				goto err;
			if (px != NULL &&
			    !ossl_param_build_set_bn(
				    tmpl, px, OSSL_PKEY_PARAM_EC_PUB_X, x))
				goto err;
			if (py != NULL &&
			    !ossl_param_build_set_bn(
				    tmpl, py, OSSL_PKEY_PARAM_EC_PUB_Y, y))
				goto err;
		}
	}
...
static ossl_inline int otherparams_to_params(const EC_KEY *ec,
					     OSSL_PARAM_BLD *tmpl,
					     OSSL_PARAM params[])
{
	...
	if (name != NULL &&
	    !ossl_param_build_set_utf8_string(
		    tmpl, params, OSSL_PKEY_PARAM_EC_POINT_CONVERSION_FORMAT,
		    name))
		return 0;
	...
	if (name != NULL &&
	    !ossl_param_build_set_utf8_string(
		    tmpl, params, OSSL_PKEY_PARAM_EC_GROUP_CHECK_TYPE, name))
		return 0;
...
static void *sm2_newdata(void *provctx)
{
	if (!ossl_prov_is_running())
		return NULL;
	return EC_KEY_new_by_curve_name_ex(PROV_LIBCTX_OF(provctx), NULL,
					   NID_sm2);
}
#endif
#endif
...
static int ec_match(const void *keydata1, const void *keydata2, int selection)
{
	...
	if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
		...
		if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) {
			...
			if (pa != NULL && pb != NULL) {
				ok = ok &&
				     EC_POINT_cmp(group_b, pa, pb, ctx) == 0;
				key_checked = 1;
			}
		}
		if (!key_checked &&
...
static int common_import(void *keydata, int selection,
			 const OSSL_PARAM params[], int sm2_wanted)
{
	EC_KEY *ec = keydata;
	int ok = 1;
		     ...
		     void *cbarg)
{
	...
	if ((selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0) {
		...
		BN_CTX_start(bnctx);
		ok = ok &&
		     ossl_ec_group_todata(EC_KEY_get0_group(ec), tmpl, NULL,
					  ossl_ec_key_get_libctx(ec),
					  ossl_ec_key_get0_propq(ec), bnctx,
					  &genbuf);
	}
	...
	if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
		...
		ok = ok &&
		     key_to_params(ec, tmpl, NULL, include_private, &pub_key);
	}
	if ((selection & OSSL_KEYMGMT_SELECT_OTHER_PARAMETERS) != 0)
...
#define EC_IMEXPORTABLE_DOM_PARAMETERS                                        \
	OSSL_PARAM_utf8_string(OSSL_PKEY_PARAM_GROUP_NAME, NULL, 0),          \
		...
		OSSL_PARAM_utf8_string(OSSL_PKEY_PARAM_EC_FIELD_TYPE, NULL,   \
				       0),                                    \
		...
		OSSL_PARAM_octet_string(OSSL_PKEY_PARAM_EC_GENERATOR, NULL,   \
					0),                                   \
		OSSL_PARAM_BN(OSSL_PKEY_PARAM_EC_ORDER, NULL, 0),             \
		OSSL_PARAM_BN(OSSL_PKEY_PARAM_EC_COFACTOR, NULL, 0),          \
		OSSL_PARAM_octet_string(OSSL_PKEY_PARAM_EC_SEED, NULL, 0),    \
		OSSL_PARAM_int(                                               \
			OSSL_PKEY_PARAM_EC_DECODED_FROM_EXPLICIT_PARAMS, NULL)
...
static int ec_get_ecm_params(const EC_GROUP *group, OSSL_PARAM params[])
{
	...
	if (!ossl_param_build_set_int(NULL, params, OSSL_PKEY_PARAM_EC_CHAR2_M,
				      m) ||
	    !ossl_param_build_set_utf8_string(
		    NULL, params, OSSL_PKEY_PARAM_EC_CHAR2_TYPE, basis_name))
		goto err;
...
static int common_get_params(void *key, OSSL_PARAM params[], int sm2)
{
	...
	if (!sm2) {
		if ((p = OSSL_PARAM_locate(
			     params, OSSL_PKEY_PARAM_DEFAULT_DIGEST)) != NULL &&
	...
	if (!sm2) {
		...
		if (p != NULL) {
			...
			ecdh_cofactor_mode = (EC_KEY_get_flags(eck) &
	...
	if ((p = OSSL_PARAM_locate(
		...
		p->return_size = EC_POINT_point2oct(
			ecg, ecp, POINT_CONVERSION_UNCOMPRESSED, p->data,
			p->data_size, bnctx);
		if (p->return_size == 0)
			goto err;
	}
...
#define EC2M_GETTABLE_DOM_PARAMS                                            \
	OSSL_PARAM_int(OSSL_PKEY_PARAM_EC_CHAR2_M, NULL),                   \
		OSSL_PARAM_utf8_string(OSSL_PKEY_PARAM_EC_CHAR2_TYPE, NULL, \
				       0),                                  \
...
static const OSSL_PARAM ec_known_gettable_params[] = {
	...
	EC2M_GETTABLE_DOM_PARAMS EC_IMEXPORTABLE_PUBLIC_KEY,
...
static int ec_set_params(void *key, const OSSL_PARAM params[])
{
	...
	if (!ossl_ec_group_set_params((EC_GROUP *)EC_KEY_get0_group(key),
				      params))
		return 0;
	...
	if (p != NULL) {
		...
		if (ctx == NULL || p->data_type != OSSL_PARAM_OCTET_STRING ||
		...
		if ((flags & EC_FLAG_CHECK_NAMED_GROUP) != 0)
			ok = ok &&
			     EC_GROUP_check_named_curve(
				     ...
				     (flags & EC_FLAG_CHECK_NAMED_GROUP_NIST) !=
					     0,
				     ctx) > 0;
		else
			ok = ok && EC_GROUP_check(EC_KEY_get0_group(eck), ctx);
			 ...
			 const OSSL_PARAM params[])
{
	...
	if (!ossl_prov_is_running() ||
	    (selection & (EC_POSSIBLE_SELECTIONS)) == 0)
		return NULL;
...
static int ec_gen_set_params(void *genctx, const OSSL_PARAM params[])
{
	...
	COPY_INT_PARAM(params, OSSL_PKEY_PARAM_USE_COFACTOR_ECDH,
		       gctx->ecdh_mode);

	COPY_UTF8_PARAM(params, OSSL_PKEY_PARAM_GROUP_NAME, gctx->group_name);
	COPY_UTF8_PARAM(params, OSSL_PKEY_PARAM_EC_FIELD_TYPE,
			gctx->field_type);
...
static int ec_gen_set_group_from_params(struct ec_gen_ctx *gctx)
{
	...
	if (gctx->pt_format != NULL &&
	    !OSSL_PARAM_BLD_push_utf8_string(
		    bld, OSSL_PKEY_PARAM_EC_POINT_CONVERSION_FORMAT,
		    gctx->pt_format, 0))
		goto err;
	...
	if (gctx->group_name != NULL) {
		...
		if (!OSSL_PARAM_BLD_push_utf8_string(
			    bld, OSSL_PKEY_PARAM_EC_FIELD_TYPE,
			    gctx->field_type, 0))
			goto err;
	} else {
		goto err;
	}
	if (gctx->p == NULL || gctx->a == NULL || gctx->b == NULL ||
	    gctx->order == NULL ||
...
static void *ec_gen(void *genctx, OSSL_CALLBACK *osslcb, void *cbarg)
{
	...
	if ((gctx->selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
		...
		if (gctx->dhkem_ikm != NULL && gctx->dhkem_ikmlen != 0)
			ret = ret &&
			      ossl_ec_generate_key_dhkem(ec, gctx->dhkem_ikm,
							 gctx->dhkem_ikmlen);
		else
#endif
	...
	if (gctx->ecdh_mode != -1)
		ret = ret &&
		      ossl_ec_set_ecdh_cofactor_mode(ec, gctx->ecdh_mode);
...
static void *sm2_gen(void *genctx, OSSL_CALLBACK *osslcb, void *cbarg)
{
	...
	if (gctx == NULL || (ec = EC_KEY_new_ex(gctx->libctx, NULL)) == NULL)
		return NULL;
