...
#define rsa_gen_pss                                                            \
	OSSL_PARAM_utf8_string(OSSL_PKEY_PARAM_RSA_DIGEST, NULL, 0),           \
		OSSL_PARAM_utf8_string(OSSL_PKEY_PARAM_RSA_DIGEST_PROPS, NULL, \
				       0),                                     \
		OSSL_PARAM_utf8_string(OSSL_PKEY_PARAM_RSA_MASKGENFUNC, NULL,  \
				       0),                                     \
		OSSL_PARAM_utf8_string(OSSL_PKEY_PARAM_RSA_MGF1_DIGEST, NULL,  \
				       0),                                     \
...
static const OSSL_PARAM *rsapss_gen_settable_params(ossl_unused void *genctx,
						    ossl_unused void *provctx)
{
	static OSSL_PARAM settable[] = { rsa_gen_basic, rsa_gen_pss,
					 OSSL_PARAM_END };
...
static void *rsa_gen(void *genctx, OSSL_CALLBACK *osslcb, void *cbarg)
{
	...
	if (gctx->acvp_test_params != NULL) {
		if (!ossl_rsa_acvp_test_set_params(rsa_tmp,
						   gctx->acvp_test_params))
			goto err;
	}
#endif

	if (!RSA_generate_multi_prime_key(rsa_tmp, (int)gctx->nbits,
					  (int)gctx->primes, gctx->pub_exp,
					  gencb))
		goto err;
