...
void ossl_hybrid_pkey_free(HYBRID_PKEY *key)
{
    ...
    CRYPTO_DOWN_REF(&key->references, &r);
    REF_PRINT_COUNT("HYBRID", key);
    if (r > 0)
        return;
    REF_ASSERT_ISNT(r < 0);
...
#define GET_SUBALG_PARAM(lname, type)                                       \
    ...
    {                                                                       \
        OSSL_PARAM prm[2] = { OSSL_PARAM_END, OSSL_PARAM_END };             \
        unsigned int i;                                                     \
        int got = 0;                                                        \
                                                                            \
        for (i = 0; i < n; i++) {                                           \
            *prm = *param;                                                  \
            OSSL_PARAM_set_all_unmodified(prm);                             \
            if (!type ## _get_params(s[i], prm))                            \
                return 0;                                                   \
            if (OSSL_PARAM_modified(prm)) {                                 \
                if (got) {                                                  \
...
int ossl_hybrid_kmgmt_get_params(void *vkey, OSSL_PARAM params[])
{
    ...
    for (p = params; params->key != NULL; params++) {
        ...
        case PIDX_PKEY_PARAM_MAX_SIZE:
            if (!OSSL_PARAM_set_size_t(p, key->key_length))
                return 0;
            break;
    ...
    for (p = params; params->key != NULL; params++) {
        ...
        case PIDX_PKEY_PARAM_ENCODED_PUBLIC_KEY:
            ...
            *prms = *p;
            for (i = 0; i < key->info->num_algs; i++) {
                prms->data = q;
                prms->data_size = key->info->key_lengths[i];
                q += key->info->key_lengths[i];
...
int ossl_hybrid_kmgmt_validate(const void *vctx, int selection, int checktype)
{
    ...
    if (!ossl_prov_is_running())
        return 0;
    if ((key_length = ctx->key_length) == 0)
        return 0;
                                            ...
                                            const HYBRID_ALG_INFO *info)
{
    ...
    for (i = 0; i < info->num_algs; i++) {
        EVP_PKEY_CTX_free(ctx->ctxs[i]);
        ctx->ctxs[i] = EVP_PKEY_CTX_new_from_name(libctx, info->names[i],
                                                  ctx->propq);
        if (ctx->ctxs[i] == NULL)
            return 0;
...
int ossl_hybrid_get_ctx_params(HYBRID_PKEY_CTX *ctx, OSSL_PARAM params[])
{
    ...
    for (p = params; params->key != NULL; params++) {
        ...
        case PIDX_PKEY_PARAM_MAX_SIZE:
            if (!OSSL_PARAM_set_size_t(p, ctx->key_length))
                return 0;
            break;
        ...
        case PIDX_PKEY_PARAM_ENCODED_PUBLIC_KEY:
            ...
            *prms = *p;
            for (i = 0; i < ctx->info->num_algs; i++) {
                prms->data = q;
                prms->data_size = ctx->info->key_lengths[i];
                /* Confirm there is space in the param's data buffer */
                if (!ossl_assert(bytes_written < ds - ctx->info->key_lengths[i]))
                    return 0;
                q += ctx->info->key_lengths[i];
                bytes_written += ctx->info->key_lengths[i];
                             ...
                             const OSSL_PARAM params[])
{
    ...
    for (i = 0; i < key->info->num_algs; i++) {
        for (k = j = 0; j < nparams; j++) {
             ...
             * Each byte contains seven bits of the integer and there are
             * more bytes if the top bit is set.  I.e. process up to and
             * including until a byte that doesn't have its top bit set.
             * The storage format being big endian.
             */
            if (plen[j] == 0)
                goto err;
            t = (*pptr[j] & 0300) >> 6;
        ...
        if (k > 0) {
            ...
            ctx = EVP_PKEY_CTX_new_from_name(key->libctx, key->info->names[i],
                                             key->propq);
            if (ctx == NULL
