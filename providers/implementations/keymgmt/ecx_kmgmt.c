...
static void *x25519_new_key(void *provctx)
{
    return ecx_key_new(ECX_KEY_TYPE_X25519, 0,
                       PROV_LIBRARY_CONTEXT_OF(provctx), NULL);
}
...
static void *ecx_gen(struct ecx_gen_ctx *gctx)
{
    ...
    if (gctx == NULL)
        return NULL;
    if ((key = ecx_key_new(gctx->type, 0, gctx->libctx, gctx->propq)) == NULL) {
        ERR_raise(ERR_LIB_PROV, ERR_R_MALLOC_FAILURE);
        return NULL;
    }
    ...
    case ECX_KEY_TYPE_ED25519:
        if (!ED25519_public_from_private(key->pubkey, privkey, gctx->libctx,
                                         gctx->propq))
            goto err;
        break;
    case ECX_KEY_TYPE_ED448:
        if (!ED448_public_from_private(key->pubkey, privkey, gctx->libctx,
                                       gctx->propq))
            goto err;
        break;
    }
...
static void *s390x_ecx_keygen25519(struct ecx_gen_ctx *gctx)
{
    ...
    ECX_KEY *key = ecx_key_new(ECX_KEY_TYPE_X25519, 1, gctx->libctx, gctx->propq);
...
static void *s390x_ecx_keygen448(struct ecx_gen_ctx *gctx)
{
    ...
    ECX_KEY *key = ecx_key_new(ECX_KEY_TYPE_X448, 1, gctx->libctx, gctx->propq);
    unsigned char *privkey = NULL, *pubkey;
...
static void *s390x_ecd_keygen25519(struct ecx_gen_ctx *gctx)
{
    ...
    ECX_KEY *key = ecx_key_new(ECX_KEY_TYPE_ED25519, 1, gctx->libctx, gctx->propq);
    unsigned char *privkey = NULL, *pubkey;
    unsigned int sz;
    EVP_MD *sha = NULL;
    int j;
    ...
    sha = EVP_MD_fetch("SHA512", gctx->libctx, gctx->propq);
    if (sha == NULL)
        goto err;
...
static void *s390x_ecd_keygen448(struct ecx_gen_ctx *gctx)
{
    ...
    ECX_KEY *key = ecx_key_new(ECX_KEY_TYPE_ED448, 1, gctx->libctx, gctx->propq);
    unsigned char *privkey = NULL, *pubkey;
    ...
    shake = EVP_MD_fetch("SHAKE256", gctx->libctx, gctx->propq);
    if (shake == NULL)
        goto err;
