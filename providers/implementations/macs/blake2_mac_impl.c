			   ...
			   size_t keylen, const OSSL_PARAM params[])
{
	...
	if (!ossl_prov_is_running() ||
	    !blake2_mac_set_ctx_params(macctx, params))
		return 0;
	if (key != NULL) {
...
static int blake2_get_ctx_params(void *vmacctx, OSSL_PARAM params[])
{
	...
	if (p.bsize != NULL &&
	    !OSSL_PARAM_set_size_t(p.bsize, BLAKE2_BLOCKBYTES))
		return 0;
...
static int blake2_mac_set_ctx_params(void *vmacctx, const OSSL_PARAM params[])
{
	...
	if (p.size != NULL) {
		...
		if (!OSSL_PARAM_get_size_t(p.size, &size) || size < 1 ||
			...
			ERR_raise(ERR_LIB_PROV,
				  PROV_R_NOT_XOF_OR_INVALID_LENGTH);
			return 0;
		}
	...
	if (p.salt != NULL) {
		...
		BLAKE2_PARAM_SET_SALT(&macctx->params, p.salt->data,
				      p.salt->data_size);
	}
	return 1;
}
...
const OSSL_DISPATCH BLAKE2_FUNCTIONS[] = {
	...
	{ OSSL_FUNC_MAC_SET_CTX_PARAMS,
	  (void (*)(void))blake2_mac_set_ctx_params },
	OSSL_DISPATCH_END
};
