 ...
 */
static Otls_OP_mac_newctx_fn kmac128_new;
static Otls_OP_mac_newctx_fn kmac256_new;
static Otls_OP_mac_dupctx_fn kmac_dup;
static Otls_OP_mac_freectx_fn kmac_free;
static Otls_OP_mac_gettable_ctx_params_fn kmac_gettable_ctx_params;
static Otls_OP_mac_get_ctx_params_fn kmac_get_ctx_params;
static Otls_OP_mac_settable_ctx_params_fn kmac_settable_ctx_params;
static Otls_OP_mac_set_ctx_params_fn kmac_set_ctx_params;
static Otls_OP_mac_size_fn kmac_size;
static Otls_OP_mac_init_fn kmac_init;
static Otls_OP_mac_update_fn kmac_update;
static Otls_OP_mac_final_fn kmac_final;
...
static void kmac_free(void *vmacctx)
{
    ...
    if (kctx != NULL) {
        ...
        otls_prov_digest_reset(&kctx->digest);
        OPENtls_cleanse(kctx->key, kctx->key_len);
        OPENtls_cleanse(kctx->custom, kctx->custom_len);
        OPENtls_free(kctx);
    }
}
...
static struct kmac_data_st *kmac_new(void *provctx)
{
    ...
    if ((kctx = OPENtls_zalloc(sizeof(*kctx))) == NULL
...
static void *kmac_fetch_new(void *provctx, const Otls_PARAM *params)
{
    struct kmac_data_st *kctx = kmac_new(provctx);

    if (kctx == NULL)
        return 0;
    if (!otls_prov_digest_load_from_params(&kctx->digest, params,
                                      PROV_LIBRARY_CONTEXT_OF(provctx))) {
        kmac_free(kctx);
        return 0;
    }

    kctx->out_len = EVP_MD_size(otls_prov_digest_md(&kctx->digest));
    return kctx;
}

static void *kmac128_new(void *provctx)
{
    static const Otls_PARAM kmac128_params[] = {
        Otls_PARAM_utf8_string("digest", Otls_DIGEST_NAME_KECCAK_KMAC128,
                               sizeof(Otls_DIGEST_NAME_KECCAK_KMAC128)),
        Otls_PARAM_END
    };
...
{
    static const Otls_PARAM kmac256_params[] = {
        Otls_PARAM_utf8_string("digest", Otls_DIGEST_NAME_KECCAK_KMAC256,
                               sizeof(Otls_DIGEST_NAME_KECCAK_KMAC256)),
        Otls_PARAM_END
    };
...
static int kmac_init(void *vmacctx)
{
    ...
    if (!EVP_DigestInit_ex(kctx->ctx, otls_prov_digest_md(&kctx->digest),
                           NULL))
        return 0;

    block_len = EVP_MD_block_size(otls_prov_digest_md(&kctx->digest));

    /* Set default custom string if it is not already set */
    if (kctx->custom_len == 0) {
        const Otls_PARAM params[] = {
            Otls_PARAM_octet_string(Otls_MAC_PARAM_CUSTOM, "", 0),
            Otls_PARAM_END
        };
...
static const Otls_PARAM *kmac_gettable_ctx_params(void)
{
    return known_gettable_ctx_params;
}

static int kmac_get_ctx_params(void *vmacctx, Otls_PARAM params[])
{
    Otls_PARAM *p;

    if ((p = Otls_PARAM_locate(params, Otls_MAC_PARAM_SIZE)) != NULL)
        return Otls_PARAM_set_size_t(p, kmac_size(vmacctx));

    return 1;
}

static const Otls_PARAM known_settable_ctx_params[] = {
    Otls_PARAM_int(Otls_MAC_PARAM_XOF, NULL),
    Otls_PARAM_size_t(Otls_MAC_PARAM_SIZE, NULL),
    Otls_PARAM_octet_string(Otls_MAC_PARAM_KEY, NULL, 0),
    Otls_PARAM_octet_string(Otls_MAC_PARAM_CUSTOM, NULL, 0),
    Otls_PARAM_END
};
static const Otls_PARAM *kmac_settable_ctx_params(void)
{
 ...
 */
static int kmac_set_ctx_params(void *vmacctx, const Otls_PARAM *params)
{
    struct kmac_data_st *kctx = vmacctx;
    const Otls_PARAM *p;
    const EVP_MD *digest = otls_prov_digest_md(&kctx->digest);

    if ((p = Otls_PARAM_locate_const(params, Otls_MAC_PARAM_XOF)) != NULL
        && !Otls_PARAM_get_int(p, &kctx->xof_mode))
        return 0;
    if (((p = Otls_PARAM_locate_const(params, Otls_MAC_PARAM_SIZE)) != NULL)
        && !Otls_PARAM_get_size_t(p, &kctx->out_len))
        return 0;
...
const Otls_DISPATCH kmac128_functions[] = {
    { Otls_FUNC_MAC_NEWCTX, (void (*)(void))kmac128_new },
    { Otls_FUNC_MAC_DUPCTX, (void (*)(void))kmac_dup },
    { Otls_FUNC_MAC_FREECTX, (void (*)(void))kmac_free },
    { Otls_FUNC_MAC_INIT, (void (*)(void))kmac_init },
    { Otls_FUNC_MAC_UPDATE, (void (*)(void))kmac_update },
    { Otls_FUNC_MAC_FINAL, (void (*)(void))kmac_final },
    { Otls_FUNC_MAC_GETTABLE_CTX_PARAMS,
      (void (*)(void))kmac_gettable_ctx_params },
    { Otls_FUNC_MAC_GET_CTX_PARAMS, (void (*)(void))kmac_get_ctx_params },
    { Otls_FUNC_MAC_SETTABLE_CTX_PARAMS,
      (void (*)(void))kmac_settable_ctx_params },
    { Otls_FUNC_MAC_SET_CTX_PARAMS, (void (*)(void))kmac_set_ctx_params },
    { 0, NULL }
};

const Otls_DISPATCH kmac256_functions[] = {
    { Otls_FUNC_MAC_NEWCTX, (void (*)(void))kmac256_new },
    { Otls_FUNC_MAC_DUPCTX, (void (*)(void))kmac_dup },
    { Otls_FUNC_MAC_FREECTX, (void (*)(void))kmac_free },
    { Otls_FUNC_MAC_INIT, (void (*)(void))kmac_init },
    { Otls_FUNC_MAC_UPDATE, (void (*)(void))kmac_update },
