                                ...
                                unsigned char *secret, size_t *secretlen)
{
    ...
    /* Encapsulate */
    for (i = 0; i < ctx->info->num_algs; i++) {
        a = ctx->info->alg[i].ciphertext_bytes;
        b = ctx->info->alg[i].shared_secret_bytes;
        if (!EVP_PKEY_encapsulate(ctx->ctxs[i], enc, &a, secret, &b))
            return 0;
        if (a != ctx->info->alg[i].ciphertext_bytes
                || b != ctx->info->alg[i].shared_secret_bytes) {
            ERR_raise_data(ERR_LIB_PROV, PROV_R_BAD_LENGTH,
                           "secretlen or enclen changed");
            return 0;
        }
        secret += ctx->info->alg[i].shared_secret_bytes;
                                ...
                                const unsigned char *enc, size_t enclen)
{
    ...
    /* Decapsulate */
    for (i = 0; i < ctx->info->num_algs; i++) {
        a = ctx->info->alg[i].shared_secret_bytes;
        if (!EVP_PKEY_decapsulate(ctx->ctxs[i], secret, &a, enc,
                                  ctx->info->alg[i].ciphertext_bytes))
            return 0;
        if (a != ctx->info->alg[i].shared_secret_bytes) {
            ERR_raise_data(ERR_LIB_PROV, PROV_R_BAD_LENGTH,
                           "secretlen changed");
            return 0;
        }
