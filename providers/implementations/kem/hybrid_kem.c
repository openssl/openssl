...
static int hybrid_kem_init(HYBRID_PKEY_CTX *ctx, HYBRID_PKEY *auth,
                                        ...
                                        const OSSL_PARAM params[]))
{
    ...
    for (i = 0; i < ctx->info->num_algs; i++) {
        ...
        ctx->ctxs[i] = EVP_PKEY_CTX_new_from_pkey(ctx->libctx, auth->keys[i],
                                                  ctx->propq);
        if (ctx->ctxs[i] == NULL)
            return 0;
...
int ossl_hybrid_kem_encapsulate_init(void *vctx, void *vauth,
                                     const OSSL_PARAM params[])
{
    HYBRID_PKEY_CTX *ctx = (HYBRID_PKEY_CTX *)vctx;
    HYBRID_PKEY *auth = (HYBRID_PKEY *)vauth;

    return hybrid_kem_init(ctx, auth, params, &EVP_PKEY_encapsulate_init);
}

int ossl_hybrid_kem_decapsulate_init(void *vctx, void *vauth,
                                     const OSSL_PARAM params[])
{
    HYBRID_PKEY_CTX *ctx = (HYBRID_PKEY_CTX *)vctx;
    HYBRID_PKEY *auth = (HYBRID_PKEY *)vauth;

    return hybrid_kem_init(ctx, auth, params, &EVP_PKEY_decapsulate_init);
}
                                ...
                                unsigned char *secret, size_t *secretlen)
{
    ...
    if (enc == NULL) {
        ...
        return enclen != NULL || secretlen != NULL;
    }
    ...
    *secretlen = ctx->shared_secret_bytes;
    ...
    /* Encapsulate */
    for (i = 0; i < ctx->info->num_algs; i++) {
        a = ctx->info->ciphertext_bytes[i];
        b = ctx->info->shared_secret_bytes[i];
        if (!EVP_PKEY_encapsulate(ctx->ctxs[i], enc, &a, secret, &b))
            return 0;
        if (a != ctx->info->ciphertext_bytes[i]
                || b != ctx->info->shared_secret_bytes[i]) {
            ERR_raise_data(ERR_LIB_PROV, PROV_R_BAD_LENGTH,
                           "secretlen or enclen changed");
            return 0;
        }
        secret += ctx->info->shared_secret_bytes[i];
        enc += ctx->info->ciphertext_bytes[i];
    }
    return 1;
}
                                ...
                                const unsigned char *enc, size_t enclen)
{
    ...
    /* Decapsulate */
    for (i = 0; i < ctx->info->num_algs; i++) {
        a = ctx->info->shared_secret_bytes[i];
        if (!EVP_PKEY_decapsulate(ctx->ctxs[i], secret, &a, enc,
                                  ctx->info->ciphertext_bytes[i]))
            return 0;
        if (a != ctx->info->shared_secret_bytes[i]) {
            ERR_raise_data(ERR_LIB_PROV, PROV_R_BAD_LENGTH,
                           "secretlen changed");
            return 0;
        }
