				...
				const OSSL_PARAM params[])
{
	...
	if (pmacctx->key->cipher.cipher != NULL)
		ciphername = (char *)EVP_CIPHER_get0_name(
			pmacctx->key->cipher.cipher);
#if !defined(OPENSSL_NO_ENGINE) && !defined(FIPS_MODULE)
	if (pmacctx->key->cipher.engine != NULL)
		engine = (char *)ENGINE_get_id(pmacctx->key->cipher.engine);
#endif

	if (!ossl_prov_set_macctx(pmacctx->macctx, (char *)ciphername,
...
#define MAC_SETTABLE_CTX_PARAMS(funcname, macname)                     \
	static const OSSL_PARAM *mac_##funcname##_settable_ctx_params( \
...
#define MAC_SIGNATURE_FUNCTIONS(funcname)                                 \
	const OSSL_DISPATCH                                               \
		ossl_mac_legacy_##funcname##_signature_functions[] = {    \
			{ OSSL_FUNC_SIGNATURE_NEWCTX,                     \
			  (void (*)(void))mac_##funcname##_newctx },      \
			...
			{ OSSL_FUNC_SIGNATURE_FREECTX,                    \
			  (void (*)(void))mac_freectx },                  \
			{ OSSL_FUNC_SIGNATURE_DUPCTX,                     \
			  (void (*)(void))mac_dupctx },                   \
			{ OSSL_FUNC_SIGNATURE_SET_CTX_PARAMS,             \
			  (void (*)(void))mac_set_ctx_params },           \
			{ OSSL_FUNC_SIGNATURE_SETTABLE_CTX_PARAMS,        \
			  (void (*)(void))                                \
				  mac_##funcname##_settable_ctx_params }, \
			OSSL_DISPATCH_END                                 \
		};
