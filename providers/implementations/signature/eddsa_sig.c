...
static void *eddsa_newctx(OSSL_LIB_CTX *libctx)
{
...
#ifdef FIPS_MODULE
    if (!FIPS_deferred_self_test(libctx, &ed448_sig_deferred_test))
        return NULL;
#endif

    return eddsa_newctx(libctx);
}
...
#ifdef FIPS_MODULE
    if (!FIPS_deferred_self_test(libctx, &ed25519_sig_deferred_test))
        return NULL;
#endif
