...
static OSSL_FUNC_signature_digest_sign_update_fn dsa_digest_sign_update;
...
static OSSL_FUNC_signature_digest_verify_update_fn dsa_digest_sign_update;
...
static int dsa_setup_md(PROV_DSA_CTX *ctx,
                        ...
                        const char *desc)
{
    /* Template for what's going into ctx->md */
    struct md_wrapper_st wrp = {0, };
    WPACKET pkt;

    if (mdprops == NULL)
        mdprops = ctx->propq;

    if (!wrap_md_setup(&wrp, ctx->libctx, mdname, mdprops, mddispatch, mdnid))
        return 0;

    /*
     * If wrap_md_setup() fetched a digest implementation, wrp.md is non-NULL.
     * In that case, some extra checks are needed.
     */
    if (wrp.md != NULL)
        wrp.mdnid = ossl_digest_get_approved_nid(wrp.md);
    ...
    if (!ctx->flag_allow_md) {
        if (ctx->md.mdname[0] != '\0'
            && !wrap_md_is_a(&wrp, ctx->md.mdname, ctx->md.mdnid)) {
            ERR_raise_data(ERR_LIB_PROV, PROV_R_DIGEST_NOT_ALLOWED,
                             ...
                             const unsigned char *tbs, size_t tbslen)
{
    ...
    size_t mdsize = wrap_md_get_size(&pdsactx->md);
    ...
    ret = ossl_dsa_sign_int(0, tbs, tbslen, sig, &sltmp, pdsactx->dsa,
                            pdsactx->nonce_type, pdsactx->md.mdname,
                                         ...
                                         size_t datalen)
{
    ...
    return wrap_md_update(&pdsactx->md, data, datalen);
}
                                  ...
                                  size_t *siglen, size_t sigsize)
{
    ...
    if (sig != NULL) {
         ...
         */
        if (!wrap_md_final(&pdsactx->md, digest, &dlen, sizeof(digest)))
            return 0;
    }
                    ...
                    size_t sigsize, const unsigned char *tbs, size_t tbslen)
{
    ...
    if (pdsactx->operation == EVP_PKEY_OP_SIGNMSG) {
        ...
        return dsa_signverify_message_update(pdsactx, tbs, tbslen)
            && dsa_sign_message_final(pdsactx, sig, siglen, sigsize);
    }
                      ...
                      const unsigned char *tbs, size_t tbslen)
{
    ...
    if (pdsactx->operation == EVP_PKEY_OP_VERIFYMSG)
        return dsa_verify_set_sig(pdsactx, sig, siglen)
            && dsa_signverify_message_update(pdsactx, tbs, tbslen)
            && dsa_verify_message_final(pdsactx);
                                      ...
                                      int operation, const char *desc)
{
    ...
    if (mdname != NULL
        ...
        && (mdname[0] == '\0' || OPENSSL_strcasecmp(pdsactx->md.mdname, mdname) != 0)
        && !dsa_setup_md(pdsactx, mdname, NULL, NULL, NID_undef, desc))
        return 0;

    pdsactx->flag_allow_md = 0;

    if (pdsactx->md.mdctx == NULL) {
        if (!wrap_md_newctx(&pdsactx->md, pdsactx->provctx))
            goto error;
    }

    if (!wrap_md_init(&pdsactx->md, params))
        goto error;

    return 1;

 error:
    wrap_md_freectx(&pdsactx->md);
    return 0;
}
...
static int dsa_digest_sign_update(void *vpdsactx, const unsigned char *data,
                                  size_t datalen)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
                                 ...
                                 size_t *siglen, size_t sigsize)
{
    ...
    if (dsa_sign_message_final(pdsactx, sig, siglen, sigsize))
        ok = 1;
...
static int dsa_digest_verify_update(void *vpdsactx, const unsigned char *data,
                                    size_t datalen)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
                            ...
                            size_t siglen)
{
    ...
    if (dsa_verify_set_sig(pdsactx, sig, siglen)
        && dsa_verify_message_final(vpdsactx))
        ok = 1;
...
static void *dsa_dupctx(void *vpdsactx)
{
    ...
    *dstctx = *srcctx;
    ...
    if (!wrap_md_copy(&dstctx->md, &srcctx->md))
        goto err;
...
static int dsa_get_ctx_params(void *vpdsactx, OSSL_PARAM *params)
{
    ...
    if (p != NULL && !OSSL_PARAM_set_utf8_string(p, pdsactx->md.mdname))
        return 0;
...
static int dsa_set_ctx_params(void *vpdsactx, const OSSL_PARAM params[])
{
    ...
    if (p != NULL) {
        ...
        if (propsp != NULL
            ...
            return 0;
        if (!dsa_setup_md(pdsactx, mdname, mdprops, NULL, NID_undef,
                          "DSA Set Ctx"))
            return 0;
    }
...
static int dsa_get_ctx_md_params(void *vpdsactx, OSSL_PARAM *params)
{
    ...
    return wrap_md_get_ctx_params(&pdsactx->md, params);
}
...
static const OSSL_PARAM *dsa_gettable_ctx_md_params(void *vpdsactx)
{
    ...
    return wrap_md_gettable_ctx_params(&pdsactx->md, pdsactx->provctx);
}
...
static int dsa_set_ctx_md_params(void *vpdsactx, const OSSL_PARAM params[])
{
    ...
    return wrap_md_set_ctx_params(&pdsactx->md, params);
}
...
static const OSSL_PARAM *dsa_settable_ctx_md_params(void *vpdsactx)
{
    ...
    return wrap_md_settable_ctx_params(&pdsactx->md, pdsactx->provctx);
}
                                      ...
                                      int operation, const char *desc)
{
    ...
    if (!dsa_setup_md(pdsactx, mdname, NULL, mddispatch, mdnid, desc))
        return 0;
