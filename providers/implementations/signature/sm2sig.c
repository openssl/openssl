                                    ...
                                    size_t datalen)
{
    ...
    return EVP_DigestUpdate(psm2ctx->mdctx, data, datalen);
}
                             ...
                             size_t sigsize)
{
     ...
     */
    if (sig != NULL) {
        if (!EVP_DigestFinal_ex(psm2ctx->mdctx, digest, &dlen))
            return 0;
    }
...
static const OSSL_PARAM *sm2sig_gettable_ctx_params(void)
{
...
static const OSSL_PARAM *sm2sig_settable_ctx_params(void)
{
    /*
