...
static OSSL_FUNC_signature_digest_sign_update_fn ecdsa_digest_sign_update;
...
static OSSL_FUNC_signature_digest_verify_update_fn ecdsa_digest_verify_update;
                          ...
                          const char *desc)
{
    ...
    /* If the MD wrapper fetched an implementation, it needs help setting the NID */
    if (wrp.md != NULL)
...
static int
ecdsa_signverify_init(void *vctx, void *ec,
...
{
    return ecdsa_signverify_init(vctx, ec, ecdsa_set_ctx_params, params,
                               ...
                               const unsigned char *tbs, size_t tbslen)
{
    ...
    size_t mdsize = wrap_md_get_size(&ctx->md);
                                         ...
                                         size_t datalen)
{
    ...
    return wrap_md_update(&ctx->md, data, datalen);
}
                                  ...
                                  size_t *siglen, size_t sigsize)
{
     ...
     */
    if (sig != NULL
        && !wrap_md_final(&ctx->md, digest, &dlen, sizeof(digest)))
        return 0;
    ctx->flag_allow_md = 1;
                    ...
                    size_t sigsize, const unsigned char *tbs, size_t tbslen)
{
    ...
    if (ctx->operation == EVP_PKEY_OP_SIGNMSG) {
        ...
        return ecdsa_signverify_message_update(ctx, tbs, tbslen)
            && ecdsa_sign_message_final(ctx, sig, siglen, sigsize);
    }
...
static int ecdsa_verify_message_final(void *vctx)
{
    ...
    if (!ossl_prov_is_running() || ctx == NULL || ctx->md.mdctx == NULL)
        return 0;
     ...
     */
    if (!wrap_md_final(&ctx->md, digest, &dlen, sizeof(digest)))
        return 0;
                      ...
                      const unsigned char *tbs, size_t tbslen)
{
    ...
    if (ctx->operation == EVP_PKEY_OP_VERIFYMSG)
        return ecdsa_verify_set_sig(ctx, sig, siglen)
            && ecdsa_signverify_message_update(ctx, tbs, tbslen)
            && ecdsa_verify_message_final(ctx);
                                        ...
                                        int operation, const char *desc)
{
    ...
    if (mdname != NULL
        ...
        && (mdname[0] == '\0' || OPENSSL_strcasecmp(ctx->md.mdname, mdname) != 0)
        && !ecdsa_setup_md(ctx, mdname, NULL, NULL, NID_undef, desc))
        return 0;

    ctx->flag_allow_md = 0;

    if (ctx->md.mdctx == NULL) {
        if (!wrap_md_newctx(&ctx->md, ctx->provctx))
            goto error;
    }

    if (!wrap_md_init(&ctx->md, params))
        goto error;
    return 1;
error:
...
static int ecdsa_digest_sign_update(void *vctx, const unsigned char *data,
                                  size_t datalen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
                            ...
                            size_t sigsize)
{
    ...
    if (ecdsa_sign_message_final(ctx, sig, siglen, sigsize))
        ok = 1;
...
int ecdsa_digest_verify_update(void *vctx, const unsigned char *data,
                               size_t datalen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;

    if (ctx == NULL || ctx->md.mdctx == NULL)
        return 0;
...
static int ecdsa_get_ctx_params(void *vctx, OSSL_PARAM *params)
{
    ...
    if (p != NULL && !OSSL_PARAM_set_size_t(p, wrap_md_get_size(&ctx->md)))
        return 0;

    p = OSSL_PARAM_locate(params, OSSL_SIGNATURE_PARAM_DIGEST);
    if (p != NULL && !OSSL_PARAM_set_utf8_string(p, ctx->md.md == NULL
                                                    ? ctx->md.mdname
                                                    : EVP_MD_get0_name(ctx->md.md)))
        return 0;
...
static int ecdsa_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    ...
    if (p != NULL) {
        ...
        if (propsp != NULL
            ...
            return 0;
        if (!ecdsa_setup_md(ctx, mdname, mdprops, NULL, NID_undef,
                            "ECDSA Set Ctx"))
            return 0;
    }
    ...
    if (p != NULL) {
        if (!OSSL_PARAM_get_size_t(p, &mdsize)
            || (!ctx->flag_allow_md && mdsize != wrap_md_get_size(&ctx->md)))
            return 0;
        wrap_md_set_size(&ctx->md, mdsize);
    }
...
static int ecdsa_get_ctx_md_params(void *vctx, OSSL_PARAM *params)
{
    ...
    return wrap_md_get_ctx_params(&ctx->md, params);
}
...
static const OSSL_PARAM *ecdsa_gettable_ctx_md_params(void *vctx)
{
    ...
    return wrap_md_gettable_ctx_params(&ctx->md, ctx->provctx);
}
...
static int ecdsa_set_ctx_md_params(void *vctx, const OSSL_PARAM params[])
{
    ...
    return wrap_md_set_ctx_params(&ctx->md, params);
}
...
static const OSSL_PARAM *ecdsa_settable_ctx_md_params(void *vctx)
{
    ...
    return wrap_md_settable_ctx_params(&ctx->md, ctx->provctx);
}
                                      ...
                                      int operation, const char *desc)
{
    ...
    if (!ecdsa_setup_md(ctx, mdname, NULL, mddispatch, mdnid, desc))
        return 0;
...
#define IMPL_ECDSA_SIGALG(md, MD)                                         \
    ...
    ecdsa_##md##_sign_init(void *vctx, void *vec,                    \
                         const OSSL_PARAM params[])                     \
    {                                                                   \
        static const char desc[] = "ECDSA Sigalg Sign Init";              \
                                                                        \
    ...
    ecdsa_##md##_sign_message_init(void *vctx, void *vec,            \
                                 const OSSL_PARAM params[])             \
    {                                                                   \
        static const char desc[] = "ECDSA Sigalg Sign Message Init";      \
                                                                        \
        return ecdsa_sigalg_signverify_init(vctx, vec,               \
                                          ecdsa_sigalg_set_ctx_params,    \
                                          params,                       \
    ...
    ecdsa_##md##_verify_init(void *vctx, void *vec,                  \
                           const OSSL_PARAM params[])                   \
    {                                                                   \
        static const char desc[] = "ECDSA Sigalg Verify Init";            \
                                                                        \
        return ecdsa_sigalg_signverify_init(vctx, vec,               \
                                          ecdsa_sigalg_set_ctx_params,    \
                                          params,                       \
    ...
    ecdsa_##md##_verify_message_init(void *vctx, void *vec,          \
                                   const OSSL_PARAM params[])           \
    {                                                                   \
        static const char desc[] = "ECDSA Sigalg Verify Message Init";    \
                                                                        \
        return ecdsa_sigalg_signverify_init(vctx, vec,               \
                                          ecdsa_sigalg_set_ctx_params,    \
                                          params,                       \
