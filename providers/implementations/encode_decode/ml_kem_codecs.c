...
static const ML_COMMON_PKCS8_FMT ml_kem_512_p8fmt[NUM_PKCS8_FORMATS] = {
	{ "seed-priv", 0x06aa, 0, 0x308206a6, 0x0440, 6, 0x40, 0x04820660, 0x4a,
	  0x0660, 0, 0 },
	{ "priv-only", 0x0664, 0, 0x04820660, 0, 0, 0, 0, 0x04, 0x0660, 0, 0 },
	{ "oqskeypair", 0x0984, 0, 0x04820980, 0, 0, 0, 0, 0x04, 0x0660, 0x0664,
	  0x0320 },
...
static const ML_COMMON_PKCS8_FMT ml_kem_768_p8fmt[NUM_PKCS8_FORMATS] = {
	...
	{ "oqskeypair", 0x0e04, 0, 0x04820e00, 0, 0, 0, 0, 0x04, 0x0960, 0x0964,
	  0x04a0 },
	{
		"seed-only",
		0x0042,
...
static const ML_COMMON_PKCS8_FMT ml_kem_1024_p8fmt[NUM_PKCS8_FORMATS] = {
	{ "seed-priv", 0x0caa, 0, 0x30820ca6, 0x0440, 6, 0x40, 0x04820c60, 0x4a,
	  0x0c60, 0, 0 },
	{ "priv-only", 0x0c64, 0, 0x04820c60, 0, 0, 0, 0, 0x04, 0x0c60, 0, 0 },
	{ "oqskeypair", 0x1284, 0, 0x04821280, 0, 0, 0, 0, 0x04, 0x0c60, 0x0c64,
	  0x0620 },
	{ "seed-only", 0x0042, 2, 0x8040, 0, 2, 0x40, 0, 0, 0, 0, 0 },
				  ...
				  const char *propq)
{
	...
	fmt_slots = ossl_ml_common_pkcs8_fmt_order(
		v->algorithm_name, codec->p8fmt, "input", formats);
	if (fmt_slots == NULL)
		goto end;
	...
	if (ptype != V_ASN1_UNDEF) {
		ERR_raise_data(
			ERR_LIB_PROV, PROV_R_UNEXPECTED_KEY_PARAMETERS,
	...
	if (p8fmt->seed_length > 0) {
		if (!ossl_ml_kem_set_seed(buf + p8fmt->seed_offset,
			...
			ERR_raise_data(ERR_LIB_OSSL_DECODER,
	...
	if (p8fmt->priv_length > 0) {
		if ((key->encoded_dk = OPENSSL_secure_malloc(
			     p8fmt->priv_length)) == NULL) {
			ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY,
				       "error parsing %s private key",
				       v->algorithm_name);
			goto end;
		}
		memcpy(key->encoded_dk, buf + p8fmt->priv_offset,
		       p8fmt->priv_length);
	}
...
int ossl_ml_kem_i2d_pubkey(const ML_KEM_KEY *key, unsigned char **out)
{
	...
	if (out != NULL && (*out = OPENSSL_malloc(publen)) == NULL)
		return 0;
			   ...
			   PROV_CTX *provctx)
{
	...
	fmt_slots = ossl_ml_common_pkcs8_fmt_order(
		v->algorithm_name, codec->p8fmt, "output", formats);
	if (fmt_slots == NULL)
		return 0;
	...
	if (p8fmt->priv_length != 0) {
		...
		if (pos != buf + p8fmt->priv_offset ||
		    !ossl_ml_kem_encode_private_key(pos, v->prvkey_bytes,
						    key)) {
			ERR_raise_data(ERR_LIB_PROV, ERR_R_INTERNAL_ERROR,
				       "error encoding %s private key",
				       v->algorithm_name);
...
int ossl_ml_kem_key_to_text(BIO *out, const ML_KEM_KEY *key, int selection)
{
	...
	if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0 &&
		...
		if (ossl_ml_kem_have_seed(key)) {
			if (!ossl_ml_kem_encode_seed(seed, sizeof(seed), key))
				goto end;
			if (!ossl_bio_print_labeled_buf(out, "seed:", seed,
							sizeof(seed)))
				goto end;
		}
		if (ossl_ml_kem_have_prvkey(key)) {
			if ((prvenc = OPENSSL_malloc(prvlen)) == NULL)
				return 0;
			if (!ossl_ml_kem_encode_private_key(prvenc, prvlen,
							    key))
				goto end;
			if (!ossl_bio_print_labeled_buf(out, "dk:", prvenc,
							prvlen))
				goto end;
		}
		ret = 1;
	...
	if (ossl_ml_kem_have_pubkey(key)) {
		...
		if ((pubenc = OPENSSL_malloc(key->vinfo->pubkey_bytes)) ==
			    NULL ||
	...
	if (ret == 0)
		ERR_raise_data(ERR_LIB_PROV, PROV_R_MISSING_KEY,
			       "no %s key material available", type_label);
