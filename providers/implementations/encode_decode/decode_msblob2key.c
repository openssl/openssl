			     ...
			     OSSL_PASSPHRASE_CALLBACK *pw_cb, void *pw_cbarg)
{
	...
	if ((selection == 0 ||
	    ...
	    !ispub && ctx->desc->read_private_key != NULL) {
	...
	if (key == NULL &&
	    ...
	    ispub && ctx->desc->read_public_key != NULL) {
		p = buf;
...
next:
	...
	if (key != NULL) {
		...
		params[1] = OSSL_PARAM_construct_utf8_string(
			OSSL_OBJECT_PARAM_DATA_TYPE, (char *)ctx->desc->name,
			0);
...
static int msblob2key_export_object(void *vctx, const void *reference,
				    size_t reference_sz,
...
#define IMPLEMENT_MSBLOB(KEYTYPE, keytype)                                     \
	...
	const OSSL_DISPATCH ossl_msblob_to_##keytype##_decoder_functions[] = { \
