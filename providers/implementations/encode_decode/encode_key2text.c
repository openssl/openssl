...
static int rsa_to_text(BIO *out, const void *key, int selection)
{
	...
	if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
		...
		if (!ossl_bio_print_labeled_bignum(out,
		...
		if (!ossl_bio_print_labeled_bignum(
			    out,
			    "coefficient:", sk_BIGNUM_const_value(coeffs, 0)))
			goto err;
	...
	if ((selection & OSSL_KEYMGMT_SELECT_OTHER_PARAMETERS) != 0) {
		...
		case RSA_FLAG_TYPE_RSA:
			if (!ossl_rsa_pss_params_30_is_unrestricted(
				    pss_params)) {
				if (BIO_printf(out,
			...
			if (ossl_rsa_pss_params_30_is_unrestricted(
				...
				int maskgenhashalg_nid =
					ossl_rsa_pss_params_30_maskgenhashalg(
						pss_params);
				int saltlen = ossl_rsa_pss_params_30_saltlen(
					pss_params);
				int trailerfield =
