...
#define IMPLEMENT_generic_cipher(alg, UCALG, lcmode, UCMODE, flags, kbits, \
				 blkbits, ivbits, typ)                     \
	IMPLEMENT_generic_cipher_genfn(alg, UCALG, lcmode, UCMODE, flags,  \
				       kbits, blkbits, ivbits, typ)        \
	IMPLEMENT_generic_cipher_func(alg, UCALG, lcmode, UCMODE, flags,   \
				      kbits, blkbits, ivbits, typ)
...
#define IMPLEMENT_CIPHER_HW_OFB(MODE, NAME, CTX_NAME, KEY_NAME, FUNC_PREFIX)   \
	...
	{                                                                      \
		...
		KEY_NAME *key = &(((CTX_NAME *)ctx)->ks.ks);                   \
                                                                               \
		while (len >= MAXCHUNK) {                                      \
			FUNC_PREFIX##_encrypt(in, out, MAXCHUNK, key, ctx->iv, \
					      &num);                           \
			len -= MAXCHUNK;                                       \
			in += MAXCHUNK;                                        \
			out += MAXCHUNK;                                       \
		...
		if (len > 0) {                                                 \
			FUNC_PREFIX##_encrypt(in, out, (long)len, key,         \
					      ctx->iv, &num);                  \
		}                                                              \
		ctx->num = num;                                                \
		return 1;                                                      \
...
#define IMPLEMENT_CIPHER_HW_CFB(MODE, NAME, CTX_NAME, KEY_NAME, FUNC_PREFIX)   \
	...
	{                                                                      \
		...
		while (len > 0 && len >= chunk) {                              \
			FUNC_PREFIX##_encrypt(in, out, (long)chunk, key,       \
					      ctx->iv, &num, ctx->enc);        \
			len -= chunk;                                          \
			in += chunk;                                           \
			out += chunk;                                          \
...
#define CIPHER_DEFAULT_GETTABLE_CTX_PARAMS_START(name)                      \
	static const OSSL_PARAM name##_known_gettable_ctx_params[] = {      \
		...
		OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_UPDATED_IV, NULL, \
					0),
...
#define CIPHER_DEFAULT_GETTABLE_CTX_PARAMS_END(name)               \
	...
	;                                                          \
	const OSSL_PARAM *name##_gettable_ctx_params(              \
...
#define CIPHER_DEFAULT_SETTABLE_CTX_PARAMS_END(name)               \
	...
	;                                                          \
	const OSSL_PARAM *name##_settable_ctx_params(              \
		ossl_unused void *cctx, ossl_unused void *provctx) \
	{                                                          \
