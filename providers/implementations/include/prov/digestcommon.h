...
#define PROV_FUNC_DIGEST_GET_PARAM(name, blksize, dgstsize, flags)      \
	...
	{                                                               \
		return ossl_digest_default_get_params(params, blksize,  \
						      dgstsize, flags); \
	}
...
#define PROV_FUNC_DIGEST_FINAL(name, dgstsize, fin)                     \
	...
	{                                                               \
		if (ossl_prov_is_running() && outsz >= dgstsize &&      \
		    fin(out, ctx)) {                                    \
			*outl = dgstsize;                               \
			return 1;                                       \
		}                                                       \
...
#define PROV_DISPATCH_FUNC_DIGEST_CONSTRUCT_START(name, CTX, blksize,         \
						  dgstsize, flags, upd, fin)  \
	static OSSL_FUNC_digest_newctx_fn name##_newctx;                      \
	static OSSL_FUNC_digest_freectx_fn name##_freectx;                    \
	static OSSL_FUNC_digest_dupctx_fn name##_dupctx;                      \
	static void *name##_newctx(void *prov_ctx)                            \
	{                                                                     \
		CTX *ctx = ossl_prov_is_running() ?                           \
	...
	const OSSL_DISPATCH ossl_##name##_functions[] = {                     \
		...
		{ OSSL_FUNC_DIGEST_FINAL,                                     \
		  (void (*)(void))name##_internal_final },                    \
...
#define IMPLEMENT_digest_functions(name, CTX, blksize, dgstsize, flags, init,  \
				   upd, fin)                                   \
	...
	PROV_DISPATCH_FUNC_DIGEST_CONSTRUCT_START(name, CTX, blksize,          \
						  dgstsize, flags, upd, fin),  \
		{ OSSL_FUNC_DIGEST_INIT,                                       \
		  (void (*)(void))name##_internal_init },                      \
