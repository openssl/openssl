				      ...
				      const unsigned char *in, size_t len)
{
	...
	while (len >= MAXCHUNK) {
		DES_ofb64_encrypt(in, out, MAXCHUNK, key, (DES_cblock *)ctx->iv,
				  &num);
		len -= MAXCHUNK;
		in += MAXCHUNK;
		out += MAXCHUNK;
	...
	if (len > 0) {
		DES_ofb64_encrypt(in, out, (long)len, key,
				      ...
				      const unsigned char *in, size_t len)
{
	...
	while (len > 0 && len >= chunk) {
		DES_cfb64_encrypt(in, out, (long)chunk, key,
				     ...
				     const unsigned char *in, size_t inl)
{
	...
	while (inl && inl >= chunk) {
		for (n = 0; n < chunk * 8; ++n) {
			...
			DES_cfb_encrypt(c, d, 1, 1, key, (DES_cblock *)ctx->iv,
					ctx->enc);
			out[n / 8] = (out[n / 8] &
...
#define PROV_CIPHER_HW_des_mode(mode)                                 \
	static const PROV_CIPHER_HW des_##mode = {                    \
		cipher_hw_des_initkey, cipher_hw_des_##mode##_cipher, \
