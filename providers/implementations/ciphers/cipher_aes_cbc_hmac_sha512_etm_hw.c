					  ...
					  size_t keylen)
{
	...
	PROV_AES_HMAC_SHA512_ETM_CTX *sctx =
		(PROV_AES_HMAC_SHA512_ETM_CTX *)vctx;

	if (ctx->base.enc)
		ret = aes_v8_set_encrypt_key(key, ctx->base.keylen * 8,
					     &ctx->ks);
	else
		ret = aes_v8_set_decrypt_key(key, ctx->base.keylen * 8,
					     &ctx->ks);
				     ...
				     const unsigned char *in, size_t len)
{
	...
	if (ctx->base.enc) {
		HWAES_ENC_CBC_SHA512_ETM(in, out, len, out, ctx->tag, len,
					 &arg);
		return 1;
	} else {
		if (ctx->taglen == 0) {
		...
		HWAES_DEC_CBC_SHA512_ETM(in, out, len, out, ctx->tag, len,
					 &arg);
