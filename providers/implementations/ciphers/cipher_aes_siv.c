...
static int aes_siv_get_ctx_params(void *vctx, OSSL_PARAM params[])
{
	...
	if (p != NULL && p->data_type == OSSL_PARAM_OCTET_STRING) {
		if (!ctx->enc || p->data_size != ctx->taglen ||
		    !OSSL_PARAM_set_octet_string(p, &sctx->tag.byte,
						 ctx->taglen)) {
...
#define IMPLEMENT_cipher(alg, lc, UCMODE, flags, kbits, blkbits, ivbits)       \
	...
	{                                                                      \
		return ossl_cipher_generic_get_params(                         \
			params, EVP_CIPH_##UCMODE##_MODE, flags, 2 * kbits,    \
			blkbits, ivbits);                                      \
	}                                                                      \
	static void *alg##kbits##lc##_newctx(void *provctx)                    \
	{                                                                      \
		return alg##_##lc##_newctx(provctx, 2 * kbits,                 \
