					  ...
					  size_t keylen)
{
	...
	if (ctx->base.enc)
		ret = aesni_set_encrypt_key(key, (int)(ctx->base.keylen * 8),
					    &ctx->ks);
	else
		ret = aesni_set_decrypt_key(key, (int)(ctx->base.keylen * 8),
					    &ctx->ks);
...
{ /* n4x is 1 or 2 */
	...
	mctx = (SHA256_MB_CTX *)(storage + 32 -
				 ((size_t)storage % 32)); /* align */
...
#undef MAXCHUNKSIZE
	...
	for (i = 0; i < x4; i++) {
		...
		off = (len - processed) - (64 - 13) -
		      off; /* remainder actually */
	...
	if (ctx->base.enc) {
		if (plen == NO_PAYLOAD_LENGTH)
			plen = len;
			 ...
			 ((plen + SHA256_DIGEST_LENGTH + AES_BLOCK_SIZE) &
			  -AES_BLOCK_SIZE))
			return 0;
		...
		if (plen != len) { /* "TLS" mode of operation */
			...
			sctx->md = sctx->tail;
			sha256_update(&sctx->md, out + plen,
				      SHA256_DIGEST_LENGTH);
			SHA256_Final(out + plen, &sctx->md);

			/* pad the payload|hmac */
			plen += SHA256_DIGEST_LENGTH;
			for (l = (unsigned int)(len - plen - 1); plen < len;
			     plen++)
				out[plen] = l;
			/* encrypt HMAC|padding at once */
			aesni_cbc_encrypt(out + aes_off, out + aes_off,
					  len - aes_off, &ctx->ks, ctx->base.iv,
					  1);
		} else {
		...
		union {
			unsigned int
				u[SHA256_DIGEST_LENGTH / sizeof(unsigned int)];
		...
		if (plen != NO_PAYLOAD_LENGTH) { /* "TLS" mode of operation */
			...
			if ((ctx->aux.tls_aad[plen - 4] << 8 |
			     ctx->aux.tls_aad[plen - 3]) >= TLS1_1_VERSION)
				iv = AES_BLOCK_SIZE;
             ...
             */
			pad = constant_time_select((unsigned int)mask, pad,
						   maxpad);

			inp_len = len - (SHA256_DIGEST_LENGTH + pad + 1);

			ctx->aux.tls_aad[plen - 2] =
				(unsigned char)(inp_len >> 8);
			...
			{
				unsigned char *p = out + len - 1 - maxpad -
						   SHA256_DIGEST_LENGTH;
				size_t off = out - p;
				unsigned int c, cmask;

				for (res = 0, i = 0, j = 0;
				     j < maxpad + SHA256_DIGEST_LENGTH; j++) {
					c = p[j];
					cmask = ((int)(j - off -
						       SHA256_DIGEST_LENGTH)) >>
						(sizeof(int) * 8 - 1);
					      ...
					      int aad_len)
{
	...
	if (ctx->base.enc) {
		...
		ctx->tls_aad_pad =
			(int)(((len + SHA256_DIGEST_LENGTH + AES_BLOCK_SIZE) &
			       -AES_BLOCK_SIZE) -
			      len);
		return 1;
	} else {
...
static int aesni_cbc_hmac_sha256_tls1_multiblock_max_bufsize(void *vctx)
{
	...
	return (int)(5 + 16 +
		     (((int)ctx->multiblock_max_send_fragment + 32 + 16) &
		      -16));
}
...
static const PROV_CIPHER_HW_AES_HMAC_SHA cipher_hw_aes_hmac_sha256 = {
	{ aesni_cbc_hmac_sha256_init_key, aesni_cbc_hmac_sha256_cipher },
