...
static int aes_gcm_siv_initkey(void *vctx)
{
	...
	if (ctx->ecb_ctx == NULL &&
	    (ctx->ecb_ctx = EVP_CIPHER_CTX_new()) == NULL)
		goto err;
	if (!EVP_EncryptInit_ex2(ctx->ecb_ctx, ecb, ctx->key_gen_key, NULL,
				 NULL))
		goto err;
	...
	for (i = 0; i < BLOCK_SIZE; i += 8) {
		...
		out_len = BLOCK_SIZE;
		if (!EVP_EncryptUpdate(ctx->ecb_ctx, output, &out_len,
	...
	if (!EVP_EncryptInit_ex2(ctx->ecb_ctx, ecb, ctx->msg_enc_key, NULL,
				 NULL))
		goto err;
...
static int aes_gcm_siv_aad(PROV_AES_GCM_SIV_CTX *ctx, const unsigned char *aad,
			   size_t len)
{
	size_t to_alloc;
	uint8_t *ptr;
			       ...
			       size_t len)
{
	...
	ossl_polyval_ghash_init(ctx->Htable,
				(const uint64_t *)ctx->msg_auth_key);

	if (ctx->aad != NULL) {
		/* AAD is allocated with padding, but need to adjust length */
		ossl_polyval_ghash_hash(ctx->Htable, S_s, ctx->aad,
					UP16(ctx->aad_len));
	}
	if (DOWN16(len) > 0)
		ossl_polyval_ghash_hash(ctx->Htable, S_s, (uint8_t *)in,
					DOWN16(len));
	if (!IS16(len)) {
		...
		ossl_polyval_ghash_hash(ctx->Htable, S_s, padding,
					sizeof(padding));
	}
	...
	error |= !EVP_EncryptUpdate(ctx->ecb_ctx, ctx->tag, &out_len, S_s,
				    sizeof(S_s));
...
const PROV_CIPHER_HW_AES_GCM_SIV *
ossl_prov_cipher_hw_aes_gcm_siv(size_t keybits)
{
