...
/* Dispatch functions for AES ecb cipher modes */
 ...
 * Since it is still allowed for non confidential operations in protocols
 * (Used in quic, CTR DRBG and DTLS) rather than do a hard error
 * it just sets the indicator without triggering the callback.
 */
#ifdef FIPS_MODULE
...
static void *aes_ecb_newctx(void *provctx, size_t kbits)
{
...
{
    if (!ossl_cipher_generic_init(vctx, key, keylen, iv, ivlen, params, enc))
        return 0;
#ifdef FIPS_MODULE
    ...
    const uint8_t *iv, size_t ivlen, const OSSL_PARAM params[], int enc)
{
    ...
    if (!ossl_cipher_generic_init(vctx, key->data, key->length, iv, ivlen, params, enc))
        return 0;
#ifdef FIPS_MODULE
    if (!aes_ecb_encrypt_check_approved((PROV_AES_ECB_CTX *)vctx, enc))
        return 0;
...
static int aes_ecb_get_ctx_params(void *vctx, OSSL_PARAM params[])
{
    ...
    if (!ossl_cipher_common_get_ctx_params(ctx, &p.common))
        return 0;
#ifdef FIPS_MODULE
    if (p.ind != NULL && !OSSL_PARAM_set_uint(p.ind, ((PROV_AES_ECB_CTX *)ctx)->operation_allowed))
        return 0;
#endif
    return 1;
}
...
#define IMPLEMENT_AES_ECB(name, bits)                                                  \
    ...
    {                                                                                  \
        return aes_ecb_newctx(provctx, bits);                                          \
    }                                                                                  \
    static int aes_##bits##_##name##_get_params(OSSL_PARAM params[])                   \
    {                                                                                  \
        return ossl_cipher_generic_get_params(params, EVP_CIPH_ECB_MODE, 0,            \
            bits, AES_CBC_BLK_BITS, 0);                                                \
    }                                                                                  \
...
IMPLEMENT_AES_ECB(ecb, 256);
/* ossl_aes192ecb_functions */
IMPLEMENT_AES_ECB(ecb, 192);
/* ossl_aes128ecb_functions */
IMPLEMENT_AES_ECB(ecb, 128);
