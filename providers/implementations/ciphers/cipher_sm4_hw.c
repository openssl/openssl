	...
	if (ctx->enc || (ctx->mode != EVP_CIPH_ECB_MODE &&
		...
		if (HWSM4_CAPABLE) {
			...
			if (ctx->mode == EVP_CIPH_CBC_MODE)
				...
				ctx->stream.ctr =
					(ctr128_f)HWSM4_ctr32_encrypt_blocks;
			else
#endif
			...
			if (ctx->mode == EVP_CIPH_CBC_MODE)
				ctx->stream.cbc =
					(cbc128_f)vpsm4_ex_cbc_encrypt;
			else if (ctx->mode == EVP_CIPH_ECB_MODE)
				ctx->stream.ecb =
					(ecb128_f)vpsm4_ex_ecb_encrypt;
			else if (ctx->mode == EVP_CIPH_CTR_MODE)
				ctx->stream.ctr =
					(ctr128_f)vpsm4_ex_ctr32_encrypt_blocks;
		} else
#endif
#ifdef VPSM4_CAPABLE
			if (VPSM4_CAPABLE) {
			...
			if (ctx->mode == EVP_CIPH_CBC_MODE)
				...
				ctx->stream.ctr =
					(ctr128_f)vpsm4_ctr32_encrypt_blocks;
		} else
#endif
		{
...
#define PROV_CIPHER_HW_sm4_mode(mode)                                 \
	static const PROV_CIPHER_HW sm4_##mode = {                    \
		cipher_hw_sm4_initkey, ossl_cipher_hw_generic_##mode, \
		cipher_hw_sm4_copyctx                                 \
	};                                                            \
	PROV_CIPHER_HW_declare(mode) const PROV_CIPHER_HW *           \
...
PROV_CIPHER_HW_sm4_mode(cbc) PROV_CIPHER_HW_sm4_mode(ecb)
	PROV_CIPHER_HW_sm4_mode(ofb128) PROV_CIPHER_HW_sm4_mode(cfb128)
