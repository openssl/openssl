			  ...
			  size_t outsize, const unsigned char *in, size_t inl)
{
	...
	if (ctx->xts_standard) {
		if (ctx->stream != NULL)
			(*ctx->stream)(in, out, inl, ctx->xts.key1,
				       ctx->xts.key2, ctx->base.iv,
				       ctx->base.enc);
		else if (CRYPTO_xts128_encrypt(&ctx->xts, ctx->base.iv, in, out,
					       inl, ctx->base.enc))
			return 0;
	} else {
		if (ctx->stream_gb != NULL)
			(*ctx->stream_gb)(in, out, inl, ctx->xts.key1,
					  ctx->xts.key2, ctx->base.iv,
					  ctx->base.enc);
		else if (ossl_crypto_xts128gb_encrypt(&ctx->xts, ctx->base.iv,
						      in, out, inl,
						      ctx->base.enc))
			return 0;
	}
...
#define IMPLEMENT_cipher(lcmode, UCMODE, kbits, flags)                         \
	static OSSL_FUNC_cipher_get_params_fn                                  \
		sm4_##kbits##_##lcmode##_get_params;                           \
	...
	{                                                                      \
		return sm4_xts_newctx(provctx, EVP_CIPH_##UCMODE##_MODE,       \
	...
	const OSSL_DISPATCH ossl_sm4##kbits##xts_functions[] = {               \
		...
		{ OSSL_FUNC_CIPHER_ENCRYPT_INIT,                               \
		  (void (*)(void))sm4_xts_einit },                             \
		{ OSSL_FUNC_CIPHER_DECRYPT_INIT,                               \
		  (void (*)(void))sm4_xts_dinit },                             \
		{ OSSL_FUNC_CIPHER_UPDATE,                                     \
		  (void (*)(void))sm4_xts_stream_update },                     \
		{ OSSL_FUNC_CIPHER_FINAL,                                      \
		  (void (*)(void))sm4_xts_stream_final },                      \
