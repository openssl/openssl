                          ...
                          size_t outsize, const unsigned char *in, size_t inl)
{
    ...
    if (ctx->xts_standard) {
        ...
        if (ctx->stream_gb != NULL)
            (*ctx->stream_gb)(in, out, inl, ctx->xts.key1, ctx->xts.key2, ctx->base.iv);
        else if (CRYPTO_xts128gb_encrypt(&ctx->xts, ctx->base.iv, in, out, inl,
                                    ctx->base.enc))
            return 0;
    }
    *outl = inl;
...
static int sm4_xts_set_ctx_params(void *vxctx, const OSSL_PARAM params[])
{
    ...
    if (p != NULL) {
        ...
        /* standard can be only 0 for GB/T 17964-2021, or 1 for IEEE Std 1619-2007 */
