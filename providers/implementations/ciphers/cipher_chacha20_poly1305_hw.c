...
static int chacha_poly1305_tls_init(PROV_CIPHER_CTX *bctx, unsigned char *aad,
				    size_t alen)
{
	unsigned int len;
...
static int chacha20_poly1305_initiv(PROV_CIPHER_CTX *bctx)
{
	...
	/* pad on the left */
	memcpy(tempiv + CHACHA_CTR_SIZE - noncelen, bctx->oiv, noncelen);

	if (bctx->enc)
		ret = ossl_chacha20_einit(&ctx->chacha, NULL, 0, tempiv,
					  sizeof(tempiv), NULL);
	else
		ret = ossl_chacha20_dinit(&ctx->chacha, NULL, 0, tempiv,
					  sizeof(tempiv), NULL);
...
static int chacha20_poly1305_tls_cipher(PROV_CIPHER_CTX *bctx,
					unsigned char *out, size_t *out_padlen,
					const unsigned char *in, size_t len)
{
	...
	if (plen <= 3 * CHACHA_BLK_SIZE) {
		...
		buf_len = (plen + 2 * CHACHA_BLK_SIZE - 1) &
			  (0 - CHACHA_BLK_SIZE);
	...
	if (!ctx->mac_inited) {
		if (plen != NO_TLS_PAYLOAD_LENGTH && out != NULL) {
			if (inl != plen + POLY1305_BLOCK_SIZE)
				return 0;
#if !defined(OPENSSL_SMALL_FOOTPRINT)
			return chacha20_poly1305_tls_cipher(bctx, out, outl, in,
							    inl);
#endif
		}
		...
		if (plen != NO_TLS_PAYLOAD_LENGTH) {
			Poly1305_Update(poly, ctx->tls_aad,
	...
	if (in != NULL) { /* aad or text */
		if (out == NULL) { /* aad */
			...
			if (ctx->aad) { /* wrap up aad */
				if ((rem = (size_t)ctx->len.aad %
					   POLY1305_BLOCK_SIZE))
					Poly1305_Update(poly, zero,
							POLY1305_BLOCK_SIZE -
								rem);
				ctx->aad = 0;
			}
			...
			if (bctx->enc) { /* plaintext */
				ctx->chacha.base.hw->cipher(&ctx->chacha.base,
							    out, in, plen);
				Poly1305_Update(poly, out, plen);
				in += plen;
				out += plen;
