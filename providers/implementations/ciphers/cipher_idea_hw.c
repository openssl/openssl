				  ...
				  const unsigned char *key, size_t keylen)
{
	...
	if (ctx->enc || ctx->mode == EVP_CIPH_OFB_MODE ||
...
#define PROV_CIPHER_HW_idea_mode_ex(mode, UCMODE, fname)                      \
	IMPLEMENT_CIPHER_HW_##UCMODE(                                         \
		mode, idea, PROV_IDEA_CTX, IDEA_KEY_SCHEDULE,                 \
		fname) static const PROV_CIPHER_HW idea_##mode = {            \
		cipher_hw_idea_initkey, cipher_hw_idea_##mode##_cipher        \
	};                                                                    \
...
PROV_CIPHER_HW_idea_mode(cbc, CBC) PROV_CIPHER_HW_idea_mode(ofb64, OFB)
