...
{
    struct ascon_aead128_ctx_st *ctx = vctx;
    uint8_t saved_tag[ASCON_AEAD_TAG_LEN];
    int tag_was_set;
    ...
    if (ctx->key_set && ctx->iv_set) {
        ...
        ctx->iv_set = 1;
        if (tag_was_set && direction == DECRYPTION) {
            memcpy(ctx->tag, saved_tag, ASCON_AEAD_TAG_LEN);
            ctx->is_tag_set = 1;
        }
...
static int ascon_aead128_final(void *vctx, unsigned char *out, size_t *outl, size_t outsize)
{
    ...
    if (ctx->direction == ENCRYPTION) {
        uint8_t *ciphertext = out;
        uint8_t *tag = ctx->tag;
        size_t tag_len = ASCON_AEAD_TAG_LEN;
        size_t ret;
        ...
        *outl = ret;
        ...
        if (ctx->is_tag_set) {
            ...
            size_t expected_tag_len = ASCON_AEAD_TAG_LEN;
...
static int ascon_aead128_get_ctx_params(void *vctx, OSSL_PARAM params[])
{
    ...
    if (p.taglen != NULL
        && !OSSL_PARAM_set_size_t(p.taglen, ASCON_AEAD_TAG_LEN)) {
        ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
        return 0;
    }
    ...
    if (p.tag != NULL) {
        ...
        if (!OSSL_PARAM_set_octet_string(p.tag, ctx->tag, ASCON_AEAD_TAG_LEN)) {
            ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
            return 0;
        }
    }
...
static int ascon_aead128_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    ...
    if (p.tag != NULL) {
        ...
        if (p.tag->data == NULL) {
             ...
             * length ASCON_AEAD_TAG_LEN, so do nothing and return success
             */
            return 1;
        }
        ...
        if (p.tag->data_size != ASCON_AEAD_TAG_LEN) {
            ERR_raise(ERR_LIB_PROV, PROV_R_INVALID_TAG_LENGTH);
            return 0;
        }
        memcpy(ctx->tag, p.tag->data, ASCON_AEAD_TAG_LEN);
        ctx->is_tag_set = 1;
    }
