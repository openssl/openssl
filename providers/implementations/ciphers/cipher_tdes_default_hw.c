				   ...
				   const unsigned char *in, size_t inl)
{
	...
	while (inl >= MAXCHUNK) {
		DES_ede3_ofb64_encrypt(in, out, (long)MAXCHUNK, &tctx->ks1,
				       &tctx->ks2, &tctx->ks3,
	...
	if (inl > 0) {
		DES_ede3_ofb64_encrypt(in, out, (long)inl, &tctx->ks1,
				       &tctx->ks2, &tctx->ks3,
				       (DES_cblock *)ctx->iv, &num);
	}
	ctx->num = num;
				   ...
				   const unsigned char *in, size_t inl)
{
	...
	while (inl >= MAXCHUNK) {
		DES_ede3_cfb64_encrypt(in, out, (long)MAXCHUNK, &tctx->ks1,
				       &tctx->ks2, &tctx->ks3,
	...
	if (inl > 0) {
		DES_ede3_cfb64_encrypt(in, out, (long)inl, &tctx->ks1,
				       &tctx->ks2, &tctx->ks3,
				       (DES_cblock *)ctx->iv, &num, ctx->enc);
	}
	ctx->num = num;
				    ...
				    const unsigned char *in, size_t inl)
{
	...
	for (n = 0; n < inl; ++n) {
		...
		DES_ede3_cfb_encrypt(c, d, 1, 1, &tctx->ks1, &tctx->ks2,
				     &tctx->ks3, (DES_cblock *)ctx->iv,
				     ctx->enc);
				    ...
				    const unsigned char *in, size_t inl)
{
	...
	while (inl >= MAXCHUNK) {
		DES_ede3_cfb_encrypt(in, out, 8, (long)MAXCHUNK, &tctx->ks1,
				     &tctx->ks2, &tctx->ks3,
	...
	if (inl > 0)
		DES_ede3_cfb_encrypt(in, out, 8, (long)inl, &tctx->ks1,
				     &tctx->ks2, &tctx->ks3,
				     (DES_cblock *)ctx->iv, ctx->enc);
	return 1;
}
...
PROV_CIPHER_HW_tdes_mode(ede3, ofb) PROV_CIPHER_HW_tdes_mode(ede3, cfb)
	PROV_CIPHER_HW_tdes_mode(ede3, cfb1) PROV_CIPHER_HW_tdes_mode(ede3,
								      cfb8)
