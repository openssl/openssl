...
static int cipher_hw_rc5_initkey(PROV_CIPHER_CTX *ctx, const unsigned char *key,
				 size_t keylen)
{
...
#define PROV_CIPHER_HW_rc5_mode(mode, UCMODE)                                \
	IMPLEMENT_CIPHER_HW_##UCMODE(                                        \
		mode, rc5, PROV_RC5_CTX, RC5_32_KEY,                         \
		RC5_32_##mode) static const PROV_CIPHER_HW rc5_##mode = {    \
		cipher_hw_rc5_initkey, cipher_hw_rc5_##mode##_cipher         \
	};                                                                   \
...
PROV_CIPHER_HW_rc5_mode(cbc, CBC) PROV_CIPHER_HW_rc5_mode(ecb, ECB)
	PROV_CIPHER_HW_rc5_mode(ofb64, OFB) PROV_CIPHER_HW_rc5_mode(cfb64, CFB)
