					     ...
					     size_t keylen)
{
	...
	if (HWAES_CAPABLE) {
		...
		XTS_SET_KEY_FN(HWAES_set_encrypt_key, HWAES_set_decrypt_key,
			       HWAES_encrypt, HWAES_decrypt, stream_enc,
			       stream_dec);
		return 1;
	} else
#endif /* HWAES_CAPABLE */
		...
		XTS_SET_KEY_FN(vpaes_set_encrypt_key, vpaes_set_decrypt_key,
			       vpaes_encrypt, vpaes_decrypt, stream_enc,
			       stream_dec);
		return 1;
	} else
#endif /* VPAES_CAPABLE */
				       ...
				       const unsigned char *key, size_t keylen)
{
	...
	XTS_SET_KEY_FN(aesni_set_encrypt_key, aesni_set_decrypt_key,
		       aesni_encrypt, aesni_decrypt, aesni_xts_enc,
		       aesni_xts_dec);
	return 1;
}
...
#define PROV_CIPHER_HW_declare_xts()                                    \
	...
	static const PROV_CIPHER_HW aes_xts_rv64i_zvkned = {            \
		cipher_hw_aes_xts_rv64i_zvkned_initkey, NULL,           \
		cipher_hw_aes_xts_copyctx                               \
	};                                                              \
						     ...
						     size_t keylen)
{
	...
	XTS_SET_KEY_FN(rv32i_zkne_set_encrypt_key,
		       rv32i_zknd_zkne_set_decrypt_key, rv32i_zkne_encrypt,
