			 ...
			 const OSSL_PARAM params[], int enc)
{
	...
	ctx->enc = enc;
	if (ctx->pad)
		wctx->wrapfn = enc ? CRYPTO_128_wrap_pad :
	...
	if (key != NULL) {
		...
		if (use_forward_transform) {
			AES_set_encrypt_key(key, (int)(keylen * 8),
					    &wctx->ks.ks);
			ctx->block = (block128_f)AES_encrypt;
		} else {
			AES_set_decrypt_key(key, (int)(keylen * 8),
					    &wctx->ks.ks);
...
#define IMPLEMENT_cipher(mode, fname, UCMODE, flags, kbits, blkbits, ivbits) \
	static OSSL_FUNC_cipher_get_params_fn                                \
		aes_##kbits##_##fname##_get_params;                          \
