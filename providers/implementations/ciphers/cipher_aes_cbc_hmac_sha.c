...
static const OSSL_PARAM cipher_aes_known_settable_ctx_params[] = {
	...
	OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_ENC_IN, NULL,
				0),
...
static int aes_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
	...
	if (p != NULL) {
		...
		if (ctx->base.tlsversion == SSL3_VERSION ||
			...
			if (!ossl_assert(ctx->base.removetlsfixed >=
					 AES_BLOCK_SIZE)) {
...
static int aes_get_ctx_params(void *vctx, OSSL_PARAM params[])
{
	...
	if (p != NULL &&
	    !OSSL_PARAM_set_size_t(p, ctx->multiblock_encrypt_len)) {
		ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
		return 0;
	}
#endif /* !defined(OPENSSL_NO_MULTIBLOCK) */
	...
	if (p != NULL && !OSSL_PARAM_set_octet_string_or_ptr(p, ctx->base.oiv,
							     ctx->base.ivlen)) {
		ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
		return 0;
	}
	p = OSSL_PARAM_locate(params, OSSL_CIPHER_PARAM_UPDATED_IV);
	if (p != NULL && !OSSL_PARAM_set_octet_string_or_ptr(p, ctx->base.iv,
							     ctx->base.ivlen)) {
		ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
		return 0;
	}
				      ...
				      uint64_t flags)
{
	...
	if (ctx != NULL)
		base_init(provctx, &ctx->base_ctx,
			  ossl_prov_cipher_hw_aes_cbc_hmac_sha1(), kbits,
			  blkbits, ivbits, flags);
	return ctx;
}
					...
					uint64_t flags)
{
	...
	if (ctx != NULL)
		base_init(provctx, &ctx->base_ctx,
			  ossl_prov_cipher_hw_aes_cbc_hmac_sha256(), kbits,
			  blkbits, ivbits, flags);
	return ctx;
}
...
#define IMPLEMENT_CIPHER(nm, sub, kbits, blkbits, ivbits, flags)               \
	...
	{                                                                      \
		return nm##_##sub##_newctx(provctx, kbits, blkbits, ivbits,    \
					   flags);                             \
	}                                                                      \
	static OSSL_FUNC_cipher_get_params_fn                                  \
		nm##_##kbits##_##sub##_get_params;                             \
	...
	const OSSL_DISPATCH ossl_##nm##kbits##sub##_functions[] = {            \
		{ OSSL_FUNC_CIPHER_NEWCTX,                                     \
		  (void (*)(void))nm##_##kbits##_##sub##_newctx },             \
		{ OSSL_FUNC_CIPHER_FREECTX,                                    \
		  (void (*)(void))nm##_##sub##_freectx },                      \
		{ OSSL_FUNC_CIPHER_DUPCTX,                                     \
		  (void (*)(void))nm##_##sub##_dupctx },                       \
