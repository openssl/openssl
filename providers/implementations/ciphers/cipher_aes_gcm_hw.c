...
static int generic_aes_gcm_cipher_update(PROV_GCM_CTX *ctx,
	...
	if (ctx->enc) {
		if (ctx->ctr != NULL) {
			...
			if (len >= AES_GCM_ENC_BYTES && AES_GCM_ASM(ctx)) {
				...
				if (CRYPTO_gcm128_encrypt(&ctx->gcm, in, out,
							  res))
					return 0;

				bulk = AES_gcm_encrypt(in + res, out + res,
						       len - res, ctx->gcm.key,
						       ctx->gcm.Yi.c,
						       ctx->gcm.Xi.u);
			...
			if (CRYPTO_gcm128_encrypt_ctr32(&ctx->gcm, in + bulk,
							out + bulk, len - bulk,
							ctx->ctr))
				return 0;
#else
			if (CRYPTO_gcm128_encrypt_ctr32(&ctx->gcm, in, out, len,
							ctx->ctr))
				return 0;
#endif /* AES_GCM_ASM */
		} else {
		...
		if (ctx->ctr != NULL) {
			...
			if (len >= AES_GCM_DEC_BYTES && AES_GCM_ASM(ctx)) {
				...
				if (CRYPTO_gcm128_decrypt(&ctx->gcm, in, out,
							  res))
					return 0;

				bulk = AES_gcm_decrypt(in + res, out + res,
						       len - res, ctx->gcm.key,
						       ctx->gcm.Yi.c,
						       ctx->gcm.Xi.u);
			...
			if (CRYPTO_gcm128_decrypt_ctr32(&ctx->gcm, in + bulk,
							out + bulk, len - bulk,
							ctx->ctr))
				return 0;
#else
			if (CRYPTO_gcm128_decrypt_ctr32(&ctx->gcm, in, out, len,
							ctx->ctr))
				return 0;
#endif /* AES_GCM_ASM */
		} else {
...
static const PROV_GCM_HW aes_gcm = {
	aes_gcm_initkey,       ossl_gcm_setiv,
	ossl_gcm_aad_update,   generic_aes_gcm_cipher_update,
	ossl_gcm_cipher_final, ossl_gcm_one_shot
};
