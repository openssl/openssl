				      ...
				      const unsigned char *in, size_t len)
{
	...
	if (ctx->base.enc) {
		...
		if (plen > (sha_off + iv) &&
			...
			aesni_cbc_sha1_enc(in, out, blocks, &ctx->ks,
					   ctx->base.iv, &sctx->md,
					   in + iv + sha_off);
			blocks *= SHA_CBLOCK;
			aes_off += blocks;
		...
		if (plen != NO_PAYLOAD_LENGTH) { /* "TLS" mode of operation */
			...
			aesni_cbc_encrypt(in, out, len, &ctx->ks, ctx->base.iv,
					  0);
					    ...
					    int aad_len)
{
	...
	if (ctx->base.enc) {
		...
		ctx->tls_aad_pad =
			(int)(((len + SHA_DIGEST_LENGTH + AES_BLOCK_SIZE) &
			       -AES_BLOCK_SIZE) -
			      len);
		return 1;
	} else {
...
static int aesni_cbc_hmac_sha1_tls1_multiblock_max_bufsize(void *vctx)
{
	...
	return (int)(5 + 16 +
		     (((int)ctx->multiblock_max_send_fragment + 20 + 16) &
		      -16));
}
