			   ...
			   size_t keylen)
{
	...
	if (HWSM4_CAPABLE) {
		...
		SM4_GCM_HW_SET_KEY_CTR_FN(ks, HWSM4_set_encrypt_key,
					  HWSM4_encrypt,
...
#ifdef VPSM4_CAPABLE
		if (VPSM4_CAPABLE) {
		SM4_GCM_HW_SET_KEY_CTR_FN(ks, vpsm4_set_encrypt_key,
					  vpsm4_encrypt,
					  vpsm4_ctr32_encrypt_blocks);
	} else
#endif /* VPSM4_CAPABLE */
	{
		SM4_GCM_HW_SET_KEY_CTR_FN(ks, ossl_sm4_set_key,
					  ossl_sm4_encrypt, NULL);
	}
...
static const PROV_GCM_HW sm4_gcm = {
	sm4_gcm_initkey,      ossl_gcm_setiv,	     ossl_gcm_aad_update,
