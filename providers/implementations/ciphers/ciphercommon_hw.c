			       ...
			       const unsigned char *in, size_t len)
{
	if (dat->stream.cbc)
		...
		CRYPTO_cbc128_encrypt(in, out, len, dat->ks, dat->iv,
				      dat->block);
	else
		CRYPTO_cbc128_decrypt(in, out, len, dat->ks, dat->iv,
				      dat->block);
				...
				const unsigned char *in, size_t len)
{
	...
	if (len)
		CRYPTO_cfb128_1_encrypt(in, out, len * 8, dat->ks, dat->iv,
			       ...
			       const unsigned char *in, size_t len)
{
	...
	if (dat->stream.ctr)
		CRYPTO_ctr128_encrypt_ctr32(in, out, len, dat->ks, dat->iv,
					    dat->buf, &num, dat->stream.ctr);
	else
