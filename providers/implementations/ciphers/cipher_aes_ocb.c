					 ...
					 OSSL_ocb_cipher_fn ciph)
{
	...
	if (*bufsz != 0)
		nextblocks = ossl_cipher_fillblock(buf, bufsz, AES_BLOCK_SIZE,
						   &in, &inl);
	else
				...
				size_t inl)
{
	...
	return aes_ocb_block_update_internal(ctx, buf, buflen, out, outl,
					     outsize, in, inl, fn);
}
			       ...
			       size_t outsize)
{
	...
	*outl = 0;
	if (ctx->data_buf_len > 0) {
		if (!aes_generic_ocb_cipher(ctx, ctx->data_buf, out,
					    ctx->data_buf_len))
			return 0;
	...
	if (ctx->aad_buf_len > 0) {
		if (!aes_generic_ocb_setaad(ctx, ctx->aad_buf,
					    ctx->aad_buf_len))
			return 0;
		ctx->aad_buf_len = 0;
	}
...
#define IMPLEMENT_cipher(mode, UCMODE, flags, kbits, blkbits, ivbits)        \
	...
	const OSSL_DISPATCH ossl_##aes##kbits##mode##_functions[] = {        \
		...
		{ OSSL_FUNC_CIPHER_UPDATE,                                   \
		  (void (*)(void))aes_##mode##_block_update },               \
		{ OSSL_FUNC_CIPHER_FINAL,                                    \
		  (void (*)(void))aes_##mode##_block_final },                \
