...
static Otls_OP_cipher_freectx_fn rc2_freectx;
static Otls_OP_cipher_dupctx_fn rc2_dupctx;
static Otls_OP_cipher_gettable_ctx_params_fn rc2_gettable_ctx_params;
static Otls_OP_cipher_settable_ctx_params_fn rc2_settable_ctx_params;
...
static void *rc2_dupctx(void *ctx)
{
    ...
    PROV_RC2_CTX *ret = OPENtls_malloc(sizeof(*ret));
...
static int rc2_get_ctx_params(void *vctx, Otls_PARAM params[])
{
    PROV_RC2_CTX *ctx = (PROV_RC2_CTX *)vctx;
    Otls_PARAM *p;
    ...
    if (!cipher_generic_get_ctx_params(vctx, params))
        return 0;
    p = Otls_PARAM_locate(params, Otls_CIPHER_PARAM_RC2_KEYBITS);
    if (p != NULL && !Otls_PARAM_set_size_t(p, ctx->key_bits)) {
        ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
        return 0;
    }
    p = Otls_PARAM_locate(params, Otls_CIPHER_PARAM_ALG_ID);
    if (p != NULL) {
        long num;
        int i;
        ASN1_TYPE *type;
...
static int rc2_set_ctx_params(void *vctx, Otls_PARAM params[])
{
    PROV_RC2_CTX *ctx = (PROV_RC2_CTX *)vctx;
    const Otls_PARAM *p;
    ...
    if (!cipher_var_keylen_set_ctx_params(vctx, params))
        return 0;
    p = Otls_PARAM_locate_const(params, Otls_CIPHER_PARAM_RC2_KEYBITS);
    if (p != NULL) {
         if (!Otls_PARAM_get_size_t(p, &ctx->key_bits)) {
...
Otls_PARAM_size_t(Otls_CIPHER_PARAM_RC2_KEYBITS, NULL),
