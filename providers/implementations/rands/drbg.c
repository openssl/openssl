			  ...
			  int prediction_resistance)
{
	...
	if (drbg->parent == NULL)
         ...
         */
		return ossl_prov_get_entropy(drbg->provctx, pout, entropy,
					     min_len, max_len);
	...
	bytes = drbg->parent_get_seed(
		drbg->parent, pout, entropy > 0 ? entropy : (int)drbg->strength,
		min_len, max_len, prediction_resistance, (unsigned char *)&drbg,
		sizeof(drbg));
	ossl_drbg_unlock_parent(drbg);
	return bytes;
}
				  ...
				  size_t min_len, size_t max_len)
{
	...
	if (drbg->parent != NULL && drbg->parent_nonce != NULL) {
		n = drbg->parent_nonce(drbg->parent, NULL, 0,
			       ...
			       const unsigned char *pers, size_t perslen)
{
	...
	if (drbg->min_noncelen > 0) {
		if (drbg->parent_nonce != NULL) {
			noncelen = drbg->parent_nonce(drbg->parent, NULL,
						      drbg->strength,
						      drbg->min_noncelen,
						      drbg->max_noncelen);
			if (noncelen == 0) {
			...
			if (noncelen !=
			    drbg->parent_nonce(
				    drbg->parent, nonce, drbg->strength,
				    drbg->min_noncelen, drbg->max_noncelen)) {
				ERR_raise(ERR_LIB_PROV,
					  PROV_R_ERROR_RETRIEVING_NONCE);
				goto end;
			...
			noncelen = prov_drbg_get_nonce(drbg, &nonce,
						       drbg->min_noncelen,
						       drbg->max_noncelen);
			    ...
			    const EVP_MD *md)
{
	...
	if (!approved) {
		if (!OSSL_FIPS_IND_ON_UNAPPROVED(
			    drbg, OSSL_FIPS_IND_SETTABLE0, libctx, "DRBG",
			    "Digest",
