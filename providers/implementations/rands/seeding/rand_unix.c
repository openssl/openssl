...
static int wait_random_seeded(void)
{
	...
	if (!seeded) {
		...
		if ((shm_id = shmget(OPENSSL_RAND_SEED_DEVRANDOM_SHM_ID, 1,
				     0)) == -1) {
			/*
...
static int check_random_device(struct random_device *rd)
{
	...
	return rd->fd != -1 && fstat(rd->fd, &st) != -1 &&
	       rd->dev == st.st_dev && rd->ino == st.st_ino &&
	       ((rd->mode ^ st.st_mode) & ~(S_IRWXU | S_IRWXG | S_IRWXO)) ==
		       0 &&
	       rd->rdev == st.st_rdev;
}
...
size_t ossl_pool_acquire_entropy(RAND_POOL *pool)
{
...
#if defined(OPENSSL_RAND_SEED_GETRANDOM)
	{
		...
		bytes_needed =
			ossl_rand_pool_bytes_needed(pool, 1 /*entropy_factor*/);
	...
	if (wait_random_seeded()) {
		...
		bytes_needed =
			ossl_rand_pool_bytes_needed(pool, 1 /*entropy_factor*/);
		for (i = 0;
		     bytes_needed > 0 && i < OSSL_NELEM(random_device_paths);
		     i++) {
			ssize_t bytes = 0;
			...
			while (bytes_needed != 0 && attempts-- > 0) {
				buffer = ossl_rand_pool_add_begin(pool,
								  bytes_needed);
...
#if defined(OPENSSL_RAND_SEED_EGD)
	{
		...
		for (i = 0; bytes_needed > 0 && paths[i] != NULL; i++) {
			...
			buffer = ossl_rand_pool_add_begin(pool, bytes_needed);
			num = RAND_query_egd_bytes(paths[i], buffer,
						   (int)bytes_needed);
