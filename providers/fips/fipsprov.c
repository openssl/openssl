...
void *ossl_fips_prov_ossl_ctx_new(OSSL_LIB_CTX *libctx)
{
    ...
    init_fips_option(&fgbl->fips_dsa_check, 0);
    return fgbl;
}
...
static const OSSL_PARAM fips_param_types[] = {
    ...
    OSSL_PARAM_DEFN(OSSL_PROV_PARAM_DSA_CHECK, OSSL_PARAM_INTEGER, NULL, 0),
    OSSL_PARAM_END
};
...
#define FIPS_FEATURE_OPTION(fgbl, pname, field)                         \
    *p++ = OSSL_PARAM_construct_utf8_ptr(                               \
    ...
    FIPS_FEATURE_OPTION(fgbl, OSSL_PROV_FIPS_PARAM_DSA_CHECK,
                        fips_dsa_check);
...
#define FIPS_FEATURE_GET(fgbl, pname, field)                            \
    ...
    FIPS_FEATURE_GET(fgbl, OSSL_PROV_PARAM_DRBG_TRUNC_DIGEST,
                     fips_restricted_drgb_digests);
    FIPS_FEATURE_GET(fgbl, OSSL_PROV_PARAM_DSA_CHECK,
                     fips_dsa_check);
...
#define FIPS_SET_OPTION(fgbl, field)                                            \
    ...
    FIPS_SET_OPTION(fgbl, fips_dsa_check);
...
FIPS_FEATURE_CHECK(FIPS_restricted_drbg_digests_enabled,
                   fips_restricted_drgb_digests)
FIPS_FEATURE_CHECK(FIPS_dsa_check, fips_dsa_check)
                                 ...
                                 void **cbarg)
{
    ...
    if (c_indcbfn != NULL && c_get_libctx != NULL) {
        /* Get the parent libctx */
        c_indcbfn(c_get_libctx(FIPS_get_core_handle(libctx)), cb, cbarg);
    } else {
        if (cb != NULL)
            *cb = NULL;
        if (cbarg != NULL)
