		       ...
		       const ST_KAT_CIPHER *t, int enc)
{
	...
	if (t->tag == NULL) {
		...
		return EVP_CipherInit_ex(ctx, cipher, NULL, t->key, t->iv,
					 enc) &&
			    ...
			    OSSL_LIB_CTX *libctx)
{
	...
	OSSL_SELF_TEST_onbegin(st, OSSL_SELF_TEST_TYPE_KAT_CIPHER,
			       t->base.desc);
		      ...
		      BN_CTX *ctx)
{
	...
	for (p = params; p->data != NULL; ++p) {
		...
		case OSSL_PARAM_UNSIGNED_INTEGER: {
			...
			if (bn == NULL ||
			    (BN_bin2bn(p->data, (int)p->data_len, bn) ==
			     NULL) ||
		...
		case OSSL_PARAM_INTEGER: {
			if (!OSSL_PARAM_BLD_push_int(bld, p->name,
						     *(int *)p->data))
				goto err;
			break;
		}
			  ...
			  OSSL_LIB_CTX *libctx)
{
	...
	drbg_params[1] = OSSL_PARAM_construct_utf8_string(OSSL_DRBG_PARAM_MAC,
							  "HMAC", 0);
	if (!EVP_RAND_CTX_set_params(drbg, drbg_params))
		goto err;

	drbg_params[0] = OSSL_PARAM_construct_octet_string(
		OSSL_RAND_PARAM_TEST_ENTROPY, (void *)t->entropyin,
		t->entropyinlen);
...
static int self_test_LMS(OSSL_SELF_TEST *st, OSSL_LIB_CTX *libctx)
{
	...
	pm[0] = OSSL_PARAM_construct_octet_string(
		OSSL_PKEY_PARAM_PUB_KEY, (unsigned char *)t->pub, t->publen);
				 ...
				 OSSL_LIB_CTX *libctx)
{
	...
	if ((t->mode & SIGNATURE_MODE_VERIFY_ONLY) != 0) {
		...
		if (digested) {
			if (EVP_PKEY_sign_init_ex2(ctx, sigalg, paramsinit) <=
			    0)
				goto err;
		} else {
			if (EVP_PKEY_sign_message_init(ctx, sigalg,
						       paramsinit) <= 0)
				goto err;
		}
		siglen = sizeof(sig);
		if ((t->mode & SIGNATURE_MODE_SIG_DIGESTED) != 0) {
			if (EVP_PKEY_sign(ctx, NULL, &siglen, t->msg,
					  t->msg_len) <= 0)
				goto err;
	...
	if ((t->mode & SIGNATURE_MODE_SIGN_ONLY) == 0) {
		if (digested) {
			if (EVP_PKEY_verify_init_ex2(ctx, sigalg,
						     paramsverify) <= 0)
				goto err;
		} else {
			if (EVP_PKEY_verify_message_init(ctx, sigalg,
							 paramsverify) <= 0)
				goto err;
		}
				 ...
				 OSSL_SELF_TEST *st, OSSL_LIB_CTX *libctx)
{
	...
	OSSL_SELF_TEST_onbegin(st, OSSL_SELF_TEST_TYPE_KAT_ASYM_KEYGEN,
			       t->desc);
	...
	for (expected = t->expected_params; expected->data != NULL;
	     ++expected) {
			...
			size_t persstr_len)
{
	...
	if (!EVP_RAND_instantiate(parent_rand, strength, 0, NULL, 0,
				  drbg_params))
		goto err;
...
static int self_test_asym_keygens(OSSL_SELF_TEST *st, OSSL_LIB_CTX *libctx)
{
	...
	for (i = 0; i < (int)OSSL_NELEM(st_kat_asym_keygen_tests); ++i) {
		if (!self_test_asym_keygen(&st_kat_asym_keygen_tests[i], st,
					   libctx))
			ret = 0;
	}
	return ret;
