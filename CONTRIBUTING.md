...
- **`build.info` / `build.info.in`** — The top-level build description
  ...
  files has its own `build.info`.  Some subdirectories (e.g., `doc/`) have
  a `build.info.in` template from which `build.info` is generated by
  `make update`; the top-level `build.info` is maintained directly.
...
- **`doc/`** — All documentation source.  See `doc/README.md` for a
  detailed description of this directory.  Key subdirectories:
