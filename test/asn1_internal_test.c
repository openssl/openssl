...
static int test_tbl_standard(void)
{
	...
	for (tmp = tbl_standard, i = 0; i < OSSL_NELEM(tbl_standard);
	     i++, tmp++) {
		if (tmp->nid < last_nid) {
			last_nid = 0;
			break;
	...
	for (tmp = tbl_standard, i = 0; i < OSSL_NELEM(tbl_standard);
	     i++, tmp++)
		TEST_note("asn1 tbl_standard: Index %zu, NID %d, Name=%s", i,
			  tmp->nid, OBJ_nid2ln(tmp->nid));
...
static int test_unicode(const unsigned char *univ, size_t len, int expected)
{
	...
	for (; univ < end; univ += 4) {
		if (!TEST_int_eq(ASN1_mbstring_copy(NULL, univ, 4,
						    MBSTRING_UNIV,
						    B_ASN1_UTF8STRING),
				 expected))
			ok = 0;
...
#define ln_prefix "custom"
	...
	if (!TEST_true(test_obj_create_once(NULL, sn_prefix "1", NULL)) ||
	    ...
	    !TEST_true(test_obj_create_once(arc "7", sn_prefix "7",
					    ln_prefix "7")) ||
	...
	if (!TEST_false(test_obj_create_once(NULL, NULL, NULL)) ||
	    !TEST_false(test_obj_create_once(broken_arc "8", sn_prefix "8",
					     ln_prefix "8")))
		return 0;
