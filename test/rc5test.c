...
static unsigned char RC5key[5][16] = {
	...
	{ 0x91, 0x5f, 0x46, 0x19, 0xbe, 0x41, 0xb2, 0x51, 0x63, 0x55, 0xa5,
	  0x01, 0x10, 0xa9, 0xce, 0x91 },
	{ 0x78, 0x33, 0x48, 0xe7, 0x5a, 0xeb, 0x0f, 0x2f, 0xd7, 0xb1, 0x69,
	  0xbb, 0x8d, 0xc1, 0x67, 0x87 },
	{ 0xdc, 0x49, 0xdb, 0x13, 0x75, 0xa5, 0x58, 0x4f, 0x64, 0x85, 0xb4,
	  0x13, 0xb5, 0xf1, 0x2b, 0xaf },
	{ 0x52, 0x69, 0xf1, 0x49, 0xd4, 0x1b, 0xa0, 0x15, 0x24, 0x97, 0x57,
	  0x4d, 0x7f, 0x15, 0x31, 0x25 },
};
...
static unsigned char rc5_cbc_key[RC5_CBC_NUM][17] = {
	...
	{ 8, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08 },
	{ 8, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08 },
	{ 16, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x10, 0x20, 0x30,
...
static int rc5_cbc_rounds[RC5_CBC_NUM] = {
	0,  0,	0, 0,  0,  1, 2,  2,  8,  8, 12, 16, 8, 12,
	16, 12, 8, 12, 16, 8, 12, 16, 12, 8, 8,	 8,  8,
};
...
static int test_rc5_ecb(int n)
{
	...
	if (!TEST_mem_eq(&RC5cipher[n][0], sizeof(RC5cipher[0]), buf,
			 sizeof(buf)))
		testresult = 0;

	RC5_32_ecb_encrypt(buf, buf2, &key, RC5_DECRYPT);
	if (!TEST_mem_eq(&RC5plain[n][0], sizeof(RC5cipher[0]), buf2,
			 sizeof(buf2)))
		testresult = 0;
...
static int test_rc5_cbc(int n)
{
	...
	if (i >= 8) {
		...
		if (!TEST_mem_eq(&rc5_cbc_cipher[n][0],
				 sizeof(rc5_cbc_cipher[0]), buf, sizeof(buf)))
			testresult = 0;
