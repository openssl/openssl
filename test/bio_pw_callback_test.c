			  ...
			  KEY_ENCODING key_encoding)
{
	...
	case KE_PEM:
		w_ret = PEM_write_bio_PrivateKey(bio, original_pkey,
						 EVP_aes_256_cbc(), NULL, 0,
						 write_callback, &cb_data);
		break;
	case KE_PKCS8:
		w_ret = i2d_PKCS8PrivateKey_bio(bio, original_pkey,
						EVP_aes_256_cbc(), NULL, 0,
						write_callback, &cb_data);
		break;
	}
...
{
	return full_cycle_test(KE_PEM, CB_TEST_WEAK, CB_TEST_NEGATIVE,
			       ER_FAILURE);
}
...
{
	return full_cycle_test(KE_PEM, CB_TEST_16ZERO, CB_TEST_16ZERO,
			       ER_SUCCESS);
}
...
{
	return full_cycle_test(KE_PKCS8, CB_TEST_ZERO_LENGTH,
			       CB_TEST_ZERO_LENGTH, ER_SUCCESS);
}

static int test_pkcs8_weak(void)
{
	return full_cycle_test(KE_PKCS8, CB_TEST_WEAK, CB_TEST_WEAK,
			       ER_SUCCESS);
}
...
{
	return full_cycle_test(KE_PKCS8, CB_TEST_MATCH_SIZE,
			       CB_TEST_EXCEED_SIZE, ER_FAILURE);
}
