...
static void free_race_reader(void)
{
    ...
    for (;;) {
        ...
        if (r < 0 && !BIO_should_retry(reader_bio))
            break;
        /* An explicit peer address makes the send check the peer's caps. */
        ERR_clear_error();
        if (!BIO_sendmmsg(reader_bio, &smsg, sizeof(smsg), 1, 0, &num_processed)
            && ERR_GET_REASON(ERR_peek_error()) != BIO_R_NON_FATAL)
            break;
        ERR_clear_error();
...
static int test_bio_dgram_pair_free_race(void)
{
    ...
    if (!TEST_true(BIO_new_bio_dgram_pair(&bio1, 0, &bio2, 0))
        || !TEST_true(BIO_dgram_set_caps(bio1, BIO_DGRAM_CAP_HANDLES_DST_ADDR))
