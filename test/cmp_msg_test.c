...
{
	EXECUTE_MSG_CREATION_TEST(ossl_cmp_certreq_new(
		fixture->cmp_ctx, fixture->bodytype, NULL));
}

static int execute_errormsg_create_test(CMP_MSG_TEST_FIXTURE *fixture)
{
	EXECUTE_MSG_CREATION_TEST(
		ossl_cmp_error_new(fixture->cmp_ctx, fixture->si,
				   fixture->err_code, "details", 0));
}
...
{
	EXECUTE_MSG_CREATION_TEST(
		ossl_cmp_certConf_new(fixture->cmp_ctx, OSSL_CMP_CERTREQID,
...
static int execute_certrep_create(CMP_MSG_TEST_FIXTURE *fixture)
{
	...
	if (!TEST_ptr(read_cresp))
		goto err;
	if (!TEST_ptr_null(
		    ossl_cmp_certrepmessage_get0_certresponse(crepmsg, 88)))
		goto err;
...
static int execute_rp_create(CMP_MSG_TEST_FIXTURE *fixture)
{
	...
	if (!TEST_ptr(ossl_cmp_revrepcontent_get_CertId(rpmsg->body->value.rp,
							0)))
		goto err;

	if (!TEST_ptr(
		    ossl_cmp_revrepcontent_get_pkisi(rpmsg->body->value.rp, 0)))
		goto err;
...
int setup_tests(void)
{
	...
	if (!test_arg_libctx(&libctx, &default_null_provider, &provider, 3,
			     USAGE))
		return 0;
