 ...
 * are designed not to actually follow the proscribed algoritms themselves, but rather
 ...
 * according to the specific ciphers neds
 *
...
#define ALG(NAMES, FUNC) { NAMES, "provider=p_ossltest", FUNC }
                                     ...
                                     size_t ivlen, const OSSL_PARAM params[])
{
    ...
    /*
     * We need to check to see if we already have a cipher assigned to the context.
     * if we do, then we need to pass a NULL cph to the sub_ctx, otherwise we will
     * reset the key and iv values, and subsequent update operations will fail
     */
    cph = (EVP_CIPHER *)EVP_CIPHER_CTX_get0_cipher(ctx->sub_ctx);
    if (cph == NULL) {
        cph = EVP_CIPHER_fetch(ctx->libctx, "AES-128-CBC", "provider=default");
        if (cph == NULL)
            return 0;
    } else {
        cph = NULL;
    ...
    ret = EVP_CipherInit_ex2(ctx->sub_ctx, cph, key, iv, 1, params);
    EVP_CIPHER_free(cph);
    return ret;
}
                                     ...
                                     size_t ivlen, const OSSL_PARAM params[])
{
    ...
    ret = EVP_CipherInit_ex2(ctx->sub_ctx, cph, key, iv, 0, params);
    EVP_CIPHER_free(cph);
    return ret;
}
                                      ...
                                      size_t inl)
{
    ...
    soutl = EVP_Cipher(ctx->sub_ctx, (unsigned char *)out, in, inl);
    /*
...
{
    PROV_EVP_AES128_CBC_CTX *ctx = (PROV_EVP_AES128_CBC_CTX *)vprovctx;
    fprintf(stderr, "GETTING PARAMS FOR tls1_cipher\n");
                                     ...
                                     size_t ivlen, const OSSL_PARAM params[])
{
    ...
    if (EVP_CIPHER_CTX_get0_cipher(ctx->sub_ctx) == NULL) {
        cph = EVP_CIPHER_fetch(ctx->libctx, "aes-128-gcm", "provider=default");
        if (cph == NULL)
            return 0;
    } else {
        /*
         * If we already have a cipher set in the sub context, don't set a new one
         * as that will reset our key and iv
         */
        cph = NULL;
    }
    ...
    ret = EVP_EncryptInit_ex2(ctx->sub_ctx, cph, key, iv, params);
    EVP_CIPHER_free(cph);
    return ret;
}
                                     ...
                                     size_t ivlen, const OSSL_PARAM params[])
{
    ...
    ret = EVP_DecryptInit_ex2(ctx->sub_ctx, cph, key, iv, params);
    EVP_CIPHER_free(cph);
    return ret;
}
                                     ...
                                     size_t ivlen, const OSSL_PARAM params[])
{
    ...
    /*
     * Check to see if we already have a cipher for this context
     * If we do, don't override it with a new one, as doing so results
     * in the key and iv values getting reset, which we don't want
     */
    cph = (EVP_CIPHER *)EVP_CIPHER_CTX_get0_cipher(ctx->sub_ctx);
    if (cph == NULL) {
        cph = EVP_CIPHER_fetch(ctx->libctx, "AES-128-CBC-HMAC-SHA1", "provider=default");
        if (cph == NULL)
            return 0;
    } else {
        cph = NULL;
    ...
    if (EVP_CIPHER_CTX_is_encrypting(ctx->sub_ctx)) {
        ...
        if (plen != NO_PAYLOAD_LENGTH) { /* "TLS" mode of operation */
            ...
            if (ctx->tls_ver >= TLS1_1_VERSION) {
                ...
                in += AES_BLOCK_SIZE;
                inl -= AES_BLOCK_SIZE;
            } else if (inl < (SHA_DIGEST_LENGTH + 1))
                return 0;
...
static int ossl_test_aes128cbchmacsha1_cipher(void *vprovctx, unsigned char *out, size_t *outl,
                                      size_t outsize, const unsigned char *in, size_t inl)
{
    PROV_EVP_AES128_CBC_HMAC_SHA1_CTX *ctx = (PROV_EVP_AES128_CBC_HMAC_SHA1_CTX *)vprovctx;
...
static int ossl_test_aes128cbchmacsha1_get_params(OSSL_PARAM params[])
{
    ...
    ret = ossl_cipher_generic_get_params(params, EVP_CIPH_CBC_MODE, AES_CBC_HMAC_SHA_FLAGS, 128, 128, 128);
...
static int ossl_test_aes128cbchmacsha1_get_ctx_params(void *vprovctx, OSSL_PARAM params[])
{
    ...
    if (p != NULL)
        OSSL_PARAM_set_size_t(p, ctx->pad_size);
    return ret;
}
