...
#endif
#ifndef OPENSSL_NO_DH
	{ kExampleDHPrivateKeyDER, sizeof(kExampleDHPrivateKeyDER),
	  EVP_PKEY_DH },
#endif
};
			    ...
			    int use_octstring)
{
	...
	if (use_octstring) {
		...
		ret = EVP_PKEY_get_octet_string_param(key, privtag, buf,
						      sizeof(buf), NULL);
	} else {
		BIGNUM *bn = NULL;
...
static int do_pkey_tofrom_data_select(EVP_PKEY *key, const char *keytype)
{
     ...
     */
	if (!TEST_int_eq(EVP_PKEY_todata(key, EVP_PKEY_KEYPAIR,
					 &keypair_params),
			 1) ||
     ...
     */
	if (!TEST_ptr(fromctx = EVP_PKEY_CTX_new_from_name(mainctx, keytype,
							   NULL)) ||
...
static int test_dh_tofrom_data_select(void)
{
	...
	ret = TEST_ptr(gctx = EVP_PKEY_CTX_new_from_name(mainctx, "DHX",
							 NULL)) &&
...
static int test_dh_paramgen(void)
{
	...
	ret = TEST_ptr(
		      gctx = EVP_PKEY_CTX_new_from_name(mainctx, "DH", NULL)) &&
	...
	ret = ret &&
	      TEST_ptr(
		      gctx = EVP_PKEY_CTX_new_from_pkey(mainctx, pkey, NULL)) &&
...
static int test_dh_paramfromdata(void)
{
	...
	ret = TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(mainctx, "DH", NULL)) &&
	      TEST_int_eq(set_fromdata_uint(ctx, OSSL_PKEY_PARAM_GROUP_NAME),
			  0) &&
	      TEST_int_eq(set_fromdata_string(ctx, OSSL_PKEY_PARAM_GROUP_NAME,
					      "bad"),
			  0) &&
	      TEST_int_eq(set_fromdata_string(ctx, OSSL_PKEY_PARAM_FFC_P,
					      "bad"),
			  0) &&
	      TEST_int_eq(set_fromdata_string(ctx, OSSL_PKEY_PARAM_FFC_GINDEX,
					      "bad"),
			  0) &&
	      TEST_int_eq(set_fromdata_string(ctx, OSSL_PKEY_PARAM_FFC_PCOUNTER,
					      "bad"),
			  0) &&
	      TEST_int_eq(set_fromdata_string(ctx, OSSL_PKEY_PARAM_FFC_COFACTOR,
					      "bad"),
			  0) &&
	      TEST_int_eq(set_fromdata_string(ctx, OSSL_PKEY_PARAM_FFC_H,
					      "bad"),
			  0) &&
	      TEST_int_eq(set_fromdata_uint(ctx, OSSL_PKEY_PARAM_FFC_SEED),
			  0) &&
	      TEST_int_eq(set_fromdata_string(
				  ctx, OSSL_PKEY_PARAM_FFC_VALIDATE_PQ, "bad"),
			  0) &&
	      TEST_int_eq(set_fromdata_string(
				  ctx, OSSL_PKEY_PARAM_FFC_VALIDATE_G, "bad"),
			  0) &&
	      TEST_int_eq(set_fromdata_string(
				  ctx, OSSL_PKEY_PARAM_FFC_VALIDATE_LEGACY,
				  "bad"),
			  0) &&
...
static int test_new_keytype(void)
{
	...
	if (!TEST_ptr(ctx = EVP_PKEY_CTX_new(key, NULL)) ||
	    ...
	    !TEST_int_eq(EVP_PKEY_encapsulate(ctx, NULL, &outlen, NULL,
					      &secretlen),
			 1))
		goto err;
	...
	if (out == NULL || secret == NULL || secret2 == NULL ||
	    !TEST_int_eq(EVP_PKEY_encapsulate(ctx, out, &outlen, secret,
					      &secretlen),
			 1) ||
	    !TEST_int_eq(EVP_PKEY_decapsulate_init(ctx, NULL), 1) ||
	    !TEST_int_eq(EVP_PKEY_decapsulate(ctx, secret2, &secretlen2, out,
					      outlen),
			 1) ||
...
static int test_d2i_AutoPrivateKey_ex(int i)
{
	...
	if (ak->evptype == EVP_PKEY_RSA) {
		if (!TEST_true(EVP_PKEY_get_bn_param(
			    pkey, OSSL_PKEY_PARAM_RSA_D, &priv_bn)))
			goto done;
		...
		if (!TEST_true(EVP_PKEY_get_octet_string_param(
			    pkey, OSSL_PKEY_PARAM_PRIV_KEY, buffer,
			    sizeof(buffer), &len)))
			goto done;
	} else {
		if (!TEST_true(EVP_PKEY_get_bn_param(
			    pkey, OSSL_PKEY_PARAM_PRIV_KEY, &priv_bn)))
			goto done;
	}

	if (ak->evptype == EVP_PKEY_DH) {
		if (!TEST_true(EVP_PKEY_get_bn_param(
			    pkey, OSSL_PKEY_PARAM_FFC_P, &p_bn)) ||
		    !TEST_true(EVP_PKEY_get_bn_param(
			    pkey, OSSL_PKEY_PARAM_FFC_G, &g_bn)))
			goto done;
	}
...
static int test_pkcs8key_nid_bio(void)
{
	...
	ret = TEST_ptr(provider = OSSL_PROVIDER_load(NULL, "default")) &&
	      ...
	      TEST_int_eq(i2d_PKCS8PrivateKey_nid_bio(enc_bio, pkey, nid, pwd,
						      ...
						      NULL),
			  1) &&
	      TEST_int_gt(enc_datalen = BIO_get_mem_data(enc_bio, &enc_data),
			  0) &&
...
static int test_PEM_read_bio_negative_wrong_password(int testid)
{
	...
	if (/* Output Encrypted private key in PEM form */
	    !TEST_true(PEM_write_bio_PrivateKey(key_bio, write_pkey,
...
static int do_check_params(OSSL_PARAM key_params[], int expected)
{
	...
	ret = TEST_ptr(gen_ctx = EVP_PKEY_CTX_new_from_name(mainctx, "DSA",
							    NULL)) &&
	      TEST_int_eq(EVP_PKEY_fromdata_init(gen_ctx), 1) &&
	      TEST_int_eq(EVP_PKEY_fromdata(gen_ctx, &pkey, EVP_PKEY_KEYPAIR,
					    key_params),
			  1) &&
		       ...
		       const unsigned char *expected, size_t expected_len)
{
	...
	ret = TEST_ptr(p = OSSL_PARAM_locate(params, key)) &&
	      ...
	      TEST_int_gt(len = BN_bn2binpad(bn, buffer, (int)expected_len),
			  0) &&
...
static int test_dsa_tofrom_data_select(void)
{
	...
	ret = TEST_ptr(key = d2i_AutoPrivateKey_ex(NULL, &pkeydata,
						   sizeof(dsa_key), mainctx,
						   NULL)) &&
...
static int test_dsa_todata(void)
{
	...
	if (!TEST_ptr(pkey = d2i_AutoPrivateKey_ex(NULL, &pkeydata,
						   sizeof(dsa_key), mainctx,
						   NULL)) ||
	    !TEST_int_eq(EVP_PKEY_todata(pkey, EVP_PKEY_KEYPAIR, &to_params),
			 1) ||
	    !do_check_bn(to_params, OSSL_PKEY_PARAM_FFC_P, dsa_p,
			 sizeof(dsa_p)) ||
	    !do_check_bn(to_params, OSSL_PKEY_PARAM_FFC_Q, dsa_q,
			 sizeof(dsa_q)) ||
	    !do_check_bn(to_params, OSSL_PKEY_PARAM_FFC_G, dsa_g,
			 sizeof(dsa_g)) ||
...
static int test_dsa_fromdata_digest_prop(int tstid)
{
	...
	*p++ = OSSL_PARAM_construct_end();
	...
	if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(mainctx, "DSA", NULL)) ||
	    !TEST_int_eq(EVP_PKEY_fromdata_init(ctx), 1) ||
	    !TEST_int_eq(EVP_PKEY_fromdata(ctx, &pkey, EVP_PKEY_KEY_PARAMETERS,
					   params),
			 1))
		goto err;
...
static int test_pkey_todata_null(void)
{
	...
	ret = TEST_ptr(pkey = d2i_AutoPrivateKey_ex(NULL, &pdata,
						    (long)keydata[0].size,
						    mainctx, NULL)) &&
	      TEST_int_eq(EVP_PKEY_todata(NULL, EVP_PKEY_KEYPAIR, &params),
			  0) &&
...
static int test_rsa_pss_sign(void)
{
	...
	ret = TEST_ptr(pkey = d2i_AutoPrivateKey_ex(NULL, &pdata,
						    ...
						    mainctx, NULL)) &&
	      TEST_ptr(
		      pctx = EVP_PKEY_CTX_new_from_pkey(mainctx, pkey, NULL)) &&
...
static int evp_test_name_parsing(void)
{
	...
	if (!TEST_ptr_null(
		    md = EVP_MD_fetch(mainctx, "SHA256:BogusName", NULL))) {
		EVP_MD_free(md);
		return 0;
	}
