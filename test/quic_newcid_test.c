			     ...
			     unsigned char *buf, size_t len, void *cbarg)
{
	...
	static unsigned char new_conn_id_frame[] = {
		...
		0xab, 0xcd, 0xef, 0x01,
		0x12, 0x32, 0x23, 0x45, /* Stateless Reset Token */
		0x56, 0x06, 0x08, 0x89,
		0xa1, 0xb2, 0xc3, 0xd4
	};
...
static int test_ncid_frame(int fail)
{
	...
	static const QUIC_CONN_ID conn_id = {
		0x08, { 0x33, 0x44, 0x55, 0x66, 0xde, 0xad, 0xbe, 0xef }
	};
	...
	if (!TEST_true(qtest_create_quic_objects(NULL, cctx, NULL, cert,
						 privkey, 0, &qtserv, &cssl,
						 &fault, NULL)))
		goto err;
