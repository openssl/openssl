...
#include "../crypto/rand/rand_drbg_lcl.h"
...
#define make_drbg_test_data(nid, flag, pr, p) \
  {p, nid, flag, \
...
{
    TEST_ENT *t = RAND_DRBG_get_app_data(dctx);
...
static int single_kat(DRBG_SELFTEST_DATA *td)
{
    ...
    /*
     * Initial test without PR: Instantiate DRBG with test entropy, nonce
     * and personalisation string.
     */
    if (!TEST_ptr(dctx = RAND_DRBG_new(td->nid, td->flags))
            || !TEST_true(RAND_DRBG_set_callbacks(dctx, test_entropy, NULL, 0,
                                                  test_nonce, NULL)))
        goto err;
    RAND_DRBG_set_app_data(dctx, &t);
    memset(&t, 0, sizeof(t));
    t.ent = td->ent;
    t.entlen = td->entlen;
    t.nonce = td->nonce;
    ...
    if (!TEST_true(RAND_DRBG_instantiate(dctx, td->pers, td->perslen))
            || !TEST_true(RAND_DRBG_generate(dctx, randout, td->katlen, 0,
                                             td->adin, td->adinlen))
            || !TEST_mem_eq(randout, td->katlen, td->kat, td->katlen))
        goto err;
    ...
    if (!TEST_true(RAND_DRBG_reseed(dctx, td->adinreseed, td->adinreseedlen)
            || !TEST_true(RAND_DRBG_generate(dctx, randout, td->kat2len, 0,
                                             td->adin2, td->adin2len))
            || !TEST_mem_eq(randout, td->kat2len, td->kat2, td->kat2len)))
        goto err;
    uninstantiate(dctx);
    ...
    if (!TEST_true(RAND_DRBG_generate(dctx, randout, td->katlen_pr, 1,
                                      td->adin_pr, td->adinlen_pr))
            || !TEST_mem_eq(randout, td->katlen_pr, td->kat_pr, td->katlen_pr))
        goto err;
    ...
    if (!TEST_true(RAND_DRBG_generate(dctx, randout, td->kat2len_pr, 1,
                                      td->ading_pr, td->adinglen_pr))
                || !TEST_mem_eq(randout, td->kat2len_pr,
                                td->kat2_pr, td->kat2len_pr))
        goto err;
 ...
 */
static int init(DRBG_CTX *dctx, DRBG_SELFTEST_DATA *td, TEST_ENT *t)
{
    if (!TEST_true(RAND_DRBG_set(dctx, td->nid, td->flags))
            || !TEST_true(RAND_DRBG_set_callbacks(dctx, test_entropy, NULL, 0,
                                                  test_nonce, NULL)))
...
static int error_check(DRBG_SELFTEST_DATA *td)
{
    ...
    if (!TEST_ptr(dctx = RAND_DRBG_new(0, 0)))
        goto err;
    ...
    if (!TEST_false(RAND_DRBG_generate(dctx, randout, td->katlen, 0,
                                       td->adin, td->adinlen))
            || !uninstantiate(dctx))
        goto err;
    ...
    if (!TEST_false(RAND_DRBG_generate(dctx, randout, dctx->max_request + 1, 0,
                                       td->adin, td->adinlen)))
        goto err;
     ...
     */
    t.entlen = 0;
    if (TEST_false(RAND_DRBG_generate(dctx, randout, td->katlen, 1,
                                      td->adin, td->adinlen))
            || !uninstantiate(dctx))
        goto err;
    ...
    t.entcnt = 0;
    if (!TEST_true(RAND_DRBG_generate(dctx, randout, td->katlen, 0,
                                      td->adin, td->adinlen))
    ...
    if (!TEST_mem_eq(&dctx->d, sizeof(dctx->d), zero, sizeof(dctx->d)))
        goto err;
