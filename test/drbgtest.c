			    ...
			    int expect_private_reseed, time_t reseed_when)
{
	...
	if (expect_success == 1) {
		...
		if (!TEST_time_t_ge(reseed_time(public),
				    reseed_time(primary)) ||
				     ...
				     drbg_fork_result result[2])
{
	...
	if (!TEST_int_ge(pid = fork(), 0)) {
		...
		if (TEST_int_eq(waitpid(pid, &status, 0), pid) &&
			...
			result[1].pid = pid;
			result[1].private = 1;
			memcpy(result[1].random, &random[RANDOM_SIZE],
			       RANDOM_SIZE);
				    ...
				    EVP_RAND_CTX *private)
{
	...
	for (i = 1; i <= DRBG_FORK_COUNT; ++i) {
		...
		BIO_snprintf(presult[0].name, sizeof(presult[0].name),
			     "child %d", i);
		strcpy(presult[1].name, presult[0].name);

		/* collect the random output of the children */
		if (!TEST_true(test_drbg_reseed_in_child(primary, public,
							 private, presult)))
			return 0;
	...
	if (duplicate[0] >= DRBG_FORK_COUNT - 1) {
		...
		TEST_note(
			"ERROR: %d duplicate prefixes in public random output",
			duplicate[0]);
		success = 0;
	}
	...
	if (duplicate[1] >= DRBG_FORK_COUNT - 1) {
		/* just too many duplicates to be a coincidence */
		TEST_note(
			"ERROR: %d duplicate prefixes in private random output",
			duplicate[1]);
		success = 0;
	}
	...
	if (verbose || !success) {
		for (i = 0; i < DRBG_FORK_RESULT_COUNT; ++i) {
			char *rand_hex = OPENSSL_buf2hexstr(result[i].random,
							    RANDOM_SIZE);
...
static int test_rand_reseed(void)
{
     ...
     */
	if (!TEST_true(test_drbg_reseed(1, primary, public, private, NULL, NULL,
					1, 1, 1, 0)))
		goto error;
...
static int set_reseed_time_interval(EVP_RAND_CTX *drbg, int t)
{
	...
	params[0] = OSSL_PARAM_construct_int(
		OSSL_DRBG_PARAM_RESEED_TIME_INTERVAL, &t);
