	...
	for (; ok && p->key != NULL; p++) {
		if (strcmp(p->key, "greeting") == 0) {
			...
			static OSSL_PARAM counter_request[] = {
				...
				{ "greeting", OSSL_PARAM_UTF8_PTR, &greeting,
				  sizeof(&greeting), 0 },
			...
			if (c_get_params(hand, counter_request)) {
				if (greeting) {
					...
					const char *versionp =
						*(void **)counter_request[0]
							 .data;
					const char *namep =
						*(void **)counter_request[1]
							 .data;
            ...
            */
			...
			if (deflt == NULL ||
				...
				p_set_error(ERR_LIB_PROV, 3, ctx->thisfile,
					    OPENSSL_LINE, ctx->thisfunc, NULL);
				ok = 0;
			}
		       ...
		       const OSSL_DISPATCH **out, void **provctx)
{
	...
	if (ctx->libctx == NULL) {
		...
		p_set_error(ERR_LIB_PROV, 2, ctx->thisfile, OPENSSL_LINE,
			    ctx->thisfunc, NULL);
		p_teardown(ctx);
		return 0;
	}
