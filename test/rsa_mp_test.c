...
static int key2048p3_v1(RSA *key)
{
	...
	if (!TEST_int_eq(RSA_set0_crt_params(
				 key, BN_bin2bn(dmp1, sizeof(dmp1) - 1, NULL),
...
static int key2048p3_v2(RSA *key)
{
	...
	if (!TEST_ptr(num = BN_bin2bn(iqmp, sizeof(iqmp) - 1, NULL)) ||
	    ...
	    !TEST_ptr(num = BN_bin2bn(ex_coefficient,
				      sizeof(ex_coefficient) - 1, NULL)) ||
	    !TEST_int_ne(sk_BIGNUM_push(coeffs, num), 0))
		goto err;
...
static int test_rsa_mp_gen_bad_input(void)
{
	...
	if (!TEST_int_eq(RSA_generate_multi_prime_key(rsa, 500, 2, ebn, NULL),
			 0))
		goto err;

	/* Test invalid prime count fails */
	if (!TEST_int_eq(RSA_generate_multi_prime_key(rsa, 1024, 1, ebn, NULL),
			 0))
		goto err;
	ret = 1;
err:
