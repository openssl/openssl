...
static int test_x509v3_addr_get_safi(void)
{
    ...
    if (!TEST_true(ASN1_OCTET_STRING_set(f->addressFamily, testdat, sizeof(testdat))))
        goto err;

    if (!TEST_int_eq(X509v3_addr_get_safi(f), 7))
        goto err;

    if (!TEST_true(ASN1_OCTET_STRING_set(f->addressFamily, testdat1, sizeof(testdat1))))
        goto err;

    if (!TEST_int_eq(X509v3_addr_get_safi(f), 0))
        goto err;
