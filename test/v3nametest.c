...
{
	return set_cn(crt, NID_commonName, "dummy value", NID_commonName, name,
		      0);
}

static int set_cn3(X509 *crt, const char *name)
{
	return set_cn(crt, NID_commonName, name, NID_commonName, "dummy value",
		      0);
}
		    ...
		    const struct set_name_fn *fn)
{
	...
	for (; *pname != NULL; ++pname) {
		...
		match = -1;
		if (!TEST_int_ge(ret = X509_check_host(crt, name, namelen, 0,
						       NULL),
				 0)) {
			failed = 1;
		} else if (fn->host) {
			...
			match = 1;
		if (!TEST_true(check_message(fn, "host", nameincert, match,
					     *pname)))
			failed = 1;

		match = -1;
		if (!TEST_int_ge(ret = X509_check_host(
					 crt, name, namelen,
		...
		if (fn->email) {
			...
			match = 1;
		if (!TEST_true(check_message(fn, "email", nameincert, match,
					     *pname)))
			failed = 1;
		OPENSSL_free(name);
	}
...
static int test_GENERAL_NAME_cmp(void)
{
	...
	for (i = 0; i < OSSL_NELEM(gennames); i++) {
         ...
         */
		namesa[i] =
			d2i_GENERAL_NAME(NULL, &derp, (long)gennames[i].derlen);
		derp = gennames[i].der;
		namesb[i] =
			d2i_GENERAL_NAME(NULL, &derp, (long)gennames[i].derlen);
	...
	for (i = 0; i < OSSL_NELEM(gennames); i++) {
		for (j = 0; j < OSSL_NELEM(gennames); j++) {
			if (i == j) {
				if (!TEST_int_eq(GENERAL_NAME_cmp(namesa[i],
								  namesb[j]),
						 0))
					goto end;
			} else {
				if (!TEST_int_ne(GENERAL_NAME_cmp(namesa[i],
								  namesb[j]),
						 0))
					goto end;
			}
