...
static int test_cmac_bad(void)
{
	...
	if (!TEST_ptr(ctx) ||
	    ...
	    || !TEST_false(CMAC_Init(ctx, xtskey, sizeof(xtskey),
				     EVP_aes_128_xts(), NULL)) ||
	    !TEST_false(CMAC_Update(ctx, test[0].data, test[0].data_len)))
		goto err;
...
static int test_cmac_run(void)
{
	...
	for (case_idx = 0; case_idx < OSSL_NELEM(test); case_idx++) {
		...
		while (fill_len > 0) {
			if (fill_len > str_len) {
				memcpy(&test[case_idx].data[fill_idx],
				       test[case_idx].data, str_len);
				fill_len -= str_len;
				fill_idx += str_len;
			} else {
				memcpy(&test[case_idx].data[fill_idx],
				       test[case_idx].data, fill_len);
				fill_len = 0;
			}
		}
	}
