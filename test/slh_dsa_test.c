...
static EVP_PKEY *slh_dsa_key_from_data(const char *alg,
				       ...
				       size_t datalen, int public)
{
	int ret;
	EVP_PKEY_CTX *ctx = NULL;
	EVP_PKEY *key = NULL;
	OSSL_PARAM params[2];
	const char *keytype = public ? OSSL_PKEY_PARAM_PUB_KEY :
				       OSSL_PKEY_PARAM_PRIV_KEY;
	int selection = public ? EVP_PKEY_PUBLIC_KEY : EVP_PKEY_KEYPAIR;

	params[0] = OSSL_PARAM_construct_octet_string(keytype, (uint8_t *)data,
						      datalen);
				  ...
				  const uint8_t *pub, size_t pub_len)
{
	...
	if (!TEST_ptr(bld = OSSL_PARAM_BLD_new()) ||
	    !TEST_true(OSSL_PARAM_BLD_push_octet_string(
			       bld, OSSL_PKEY_PARAM_PRIV_KEY, priv, priv_len) >
		       0) ||
	    !TEST_true(OSSL_PARAM_BLD_push_octet_string(bld, pub_name, pub,
							pub_len) > 0) ||
...
static int slh_dsa_bad_pub_len_test(void)
{
	...
	if (ret == 0)
		TEST_note(
			"Incorrectly accepted public key of length %u (expected %u)",
...
static int slh_dsa_key_eq_test(void)
{
	...
	if (!TEST_ptr(key[0] = slh_dsa_key_from_data(td1->alg, td1->pub,
						     td1->pub_len, 1)) ||
	    !TEST_ptr(key[1] = slh_dsa_key_from_data(td1->alg, td1->pub,
						     td1->pub_len, 1)) ||
	    !TEST_ptr(key[2] = slh_dsa_key_from_data(td2->alg, td2->pub,
						     td2->pub_len, 1)) ||
...
static int slh_dsa_key_validate_test(void)
{
	...
	if (!TEST_ptr(key = slh_dsa_key_from_data(td->alg, td->pub, td->pub_len,
						  1)))
		return 0;
...
static int slh_dsa_key_validate_failure_test(void)
{
     ...
     * public key.
     */
	if (!TEST_ptr(key = slh_dsa_key_from_data(
			      "SLH-DSA-SHA2-128f",
 ...
 */
static int do_slh_dsa_verify(const SLH_DSA_SIG_TEST_DATA *td, uint8_t *sig,
			     size_t sig_len)
{
	int ret = 0;
	EVP_PKEY_CTX *vctx = NULL;
	...
	*p = OSSL_PARAM_construct_end();
	...
	if (!TEST_ptr(sig_alg = EVP_SIGNATURE_fetch(lib_ctx, td->alg, NULL)))
		goto err;
	if (!TEST_int_eq(EVP_PKEY_verify_message_init(vctx, sig_alg, params),
			 1) ||
	    !TEST_int_eq(EVP_PKEY_verify(vctx, sig, sig_len, td->msg,
					 td->msg_len),
			 1))
		goto err;
	ret = 1;
...
static int slh_dsa_sign_verify_test(int tst_id)
{
	...
	*p++ = OSSL_PARAM_construct_int(OSSL_SIGNATURE_PARAM_DETERMINISTIC,
					&deterministic);
	*p++ = OSSL_PARAM_construct_int(OSSL_SIGNATURE_PARAM_MESSAGE_ENCODING,
					&encode);
	if (td->add_random != NULL)
		*p++ = OSSL_PARAM_construct_octet_string(
			...
			(char *)td->add_random, td->add_random_len);
	*p = OSSL_PARAM_construct_end();
	...
	if (!TEST_ptr(sig_alg = EVP_SIGNATURE_fetch(lib_ctx, td->alg, NULL)))
		goto err;
	if (!TEST_int_eq(EVP_PKEY_sign_message_init(sctx, sig_alg, params),
			 1) ||
	    !TEST_int_eq(EVP_PKEY_sign(sctx, NULL, &psig_len, td->msg,
				       td->msg_len),
			 1) ||
	    ...
	    !TEST_int_eq(EVP_PKEY_sign(sctx, psig, &psig_len, td->msg,
				       td->msg_len),
			 1))
		goto err;
	if (!TEST_int_eq(EVP_Q_digest(lib_ctx, "SHA256", NULL, psig, psig_len,
			 ...
			 1))
		goto err;
	if (!TEST_mem_eq(digest, digest_len, td->sig_digest,
			 td->sig_digest_len))
		goto err;
...
static EVP_PKEY *do_gen_key(const char *alg, const uint8_t *seed,
			    size_t seed_len)
{
	EVP_PKEY *pkey = NULL;
	EVP_PKEY_CTX *ctx = NULL;
	...
	if (seed_len != 0)
		*p++ = OSSL_PARAM_construct_octet_string(
			OSSL_PKEY_PARAM_SLH_DSA_SEED, (char *)seed, seed_len);
	*p = OSSL_PARAM_construct_end();
...
static int slh_dsa_keygen_test(int tst_id)
{
	...
	if (!TEST_true(EVP_PKEY_get_octet_string_param(
		    pkey, OSSL_PKEY_PARAM_PRIV_KEY, priv, sizeof(priv),
		    &priv_len)))
		goto err;
	if (!TEST_true(EVP_PKEY_get_octet_string_param(
		    pkey, OSSL_PKEY_PARAM_PUB_KEY, pub, sizeof(pub), &pub_len)))
		goto err;
	if (!TEST_true(EVP_PKEY_get_int_param(pkey, OSSL_PKEY_PARAM_BITS,
					      &bits)) ||
	    !TEST_size_t_eq((size_t)bits, 8 * 2 * n) ||
	    !TEST_true(EVP_PKEY_get_int_param(
		    pkey, OSSL_PKEY_PARAM_SECURITY_BITS, &sec_bits)) ||
	    !TEST_size_t_eq((size_t)sec_bits, 8 * n) ||
	    !TEST_true(EVP_PKEY_get_int_param(pkey, OSSL_PKEY_PARAM_MAX_SIZE,
					      &sig_len)) ||
	    !TEST_int_ge(sig_len, 7856) || !TEST_int_le(sig_len, 49856))
		goto err;
...
static int slh_dsa_usage_test(void)
{
	...
	/* Generate a key */
	if (!TEST_ptr(gctx = EVP_PKEY_CTX_new_from_name(
			      lib_ctx, "SLH-DSA-SHA2-128s", NULL)) ||
	...
	if (!TEST_ptr(PEM_read_bio_PUBKEY_ex(pub_bio, &pub, NULL, NULL, lib_ctx,
					     NULL)) ||
	    !TEST_ptr(vctx = EVP_PKEY_CTX_new_from_pkey(lib_ctx, pub, NULL)))
		goto err;
	/* verify the signature */
	if (!TEST_int_eq(EVP_PKEY_verify_message_init(vctx, sig_alg, NULL),
			 1) ||
...
static int slh_dsa_deterministic_usage_test(void)
{
	...
	*p = OSSL_PARAM_construct_end();
	...
	if (!TEST_ptr(PEM_read_bio_PrivateKey_ex(priv_bio, &priv, NULL, pass,
	    ...
	    !TEST_ptr(sctx = EVP_PKEY_CTX_new_from_pkey(lib_ctx, priv, NULL))
	    /* Init the signature */
	    || !TEST_ptr(sig_alg = EVP_SIGNATURE_fetch(lib_ctx, tst->name,
						       NULL)) ||
	...
	if (!TEST_ptr(sig = OPENSSL_calloc(2, sig_len)) ||
	    ...
	    !TEST_int_eq(EVP_PKEY_sign(dupctx, sig + sig_len, &len, msg,
				       msg_len),
			 1) ||
	    !TEST_size_t_eq(sig_len, len))
		goto err;
	...
	if (!TEST_int_eq(EVP_PKEY_verify_message_init(vctx, sig_alg, NULL),
	    ...
	    !TEST_int_eq(EVP_PKEY_verify(vctx, sig, sig_len, msg, msg_len),
			 1) ||
	    !TEST_int_eq(EVP_PKEY_verify(dupctx, sig + sig_len, sig_len, msg,
					 msg_len),
			 1))
		goto err;
	ret = 1;
...
static int slh_dsa_digest_sign_verify_test(void)
{
	...
	*p++ = OSSL_PARAM_construct_end();
	...
	if (!TEST_ptr(mctx = EVP_MD_CTX_new()) ||
	    !TEST_int_eq(EVP_DigestSignInit_ex(mctx, NULL, "SHA256", lib_ctx,
	    ...
	    !TEST_int_eq(EVP_DigestSignInit_ex(mctx, NULL, NULL, lib_ctx,
			 ...
			 1))
		goto err;
	if (!TEST_int_eq(EVP_DigestSign(mctx, NULL, &sig_len, msg, msg_len),
			 1) ||
	...
	if (!TEST_int_eq(EVP_DigestSign(mctx, sig, &sig_len, msg, msg_len), 0))
		goto err;
	sig_len++;
	if (!TEST_int_eq(EVP_DigestSignInit_ex(mctx, NULL, NULL, lib_ctx,
					       "?fips=true", key, params),
			 1) ||
	    !TEST_int_eq(EVP_DigestSign(mctx, sig, &sig_len, msg, msg_len),
			 1) ||
	    !TEST_int_eq(EVP_DigestVerifyInit_ex(mctx, NULL, "SHA256", lib_ctx,
						 "?fips=true", key, params),
			 0) ||
	    !TEST_int_eq(EVP_DigestVerifyInit_ex(mctx, NULL, NULL, lib_ctx,
						 "?fips=true", key, params),
			 1) ||
...
static int slh_dsa_keygen_invalid_test(void)
{
	...
	if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(lib_ctx, tst->name,
						       NULL)) ||
	    !TEST_int_eq(EVP_PKEY_keygen_init(ctx), 1))
		goto err;
...
int setup_tests(void)
{
	...
	if (!test_get_libctx(&lib_ctx, &null_prov, config_file, &lib_prov,
			     NULL))
		return 0;
	...
	ADD_ALL_TESTS(slh_dsa_sign_verify_test,
		      OSSL_NELEM(slh_dsa_sig_testdata));
