...
{
	if (!TEST_int_eq(serv->servername_callback,
			 serv2->servername_callback) ||
...
static int execute_test(SSL_TEST_CTX_TEST_FIXTURE *fixture)
{
	...
	if (!TEST_ptr(
		    ctx = SSL_TEST_CTX_create(conf, fixture->test_section,
...
static int test_good_configuration(void)
{
	...
	fixture->expected_ctx->expected_servername =
...
static int test_bad_configuration(int idx)
{
	...
	if (!TEST_ptr_null(ctx = SSL_TEST_CTX_create(
				   conf, bad_configurations[idx], NULL))) {
		SSL_TEST_CTX_free(ctx);
		return 0;
	}
