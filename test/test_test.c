...
static int test_uint(void)
{
	if (!TEST(1, TEST_uint_eq(3u, 3u)) || !TEST(0, TEST_uint_eq(3u, 5u)) ||
	    ...
	    !TEST(1, TEST_uint_ge(11u, 1u)) || !TEST(1, TEST_uint_ge(6u, 6u)) ||
...
static int test_uchar(void)
{
	if (!TEST(1, TEST_uchar_eq(49, 49)) ||
	    !TEST(0, TEST_uchar_eq(49, 60)) || !TEST(1, TEST_uchar_ne(50, 2)) ||
...
static int test_pointer(void)
{
	...
	if (!TEST(1, TEST_ptr(&y)) || !TEST(0, TEST_ptr(NULL)) ||
	    ...
	    !TEST(1, TEST_ptr_ne(&y, NULL)) || !TEST(1, TEST_ptr_ne(&y, &x)) ||
...
static int test_bignum(void)
{
	...
	if (!TEST(1, TEST_int_eq(BN_dec2bn(&a, "0"), 1)) ||
	    ...
	    !TEST(0, TEST_BN_eq_one(a)) || !TEST(1, TEST_BN_eq_zero(a)) ||
	    !TEST(0, TEST_BN_ne_zero(a)) || !TEST(1, TEST_BN_le_zero(a)) ||
	    !TEST(0, TEST_BN_lt_zero(a)) || !TEST(1, TEST_BN_ge_zero(a)) ||
	    !TEST(0, TEST_BN_gt_zero(a)) || !TEST(1, TEST_BN_even(a)) ||
	    !TEST(0, TEST_BN_odd(a)) || !TEST(1, TEST_BN_eq(b, c)) ||
	    !TEST(0, TEST_BN_eq(a, b)) || !TEST(0, TEST_BN_ne(NULL, c)) ||
...
static int test_single_eval(void)
{
	...
	/* int */
	return TEST_int_eq(i++, 4) && TEST_int_eq(i, 5) &&
	       TEST_int_gt(++i, 5) && TEST_int_le(5, i++) &&
	       TEST_int_ne(--i, 5) &&
	       TEST_int_eq(12, i *= 2)
	       /* Long */
