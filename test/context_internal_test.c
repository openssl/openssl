 ...
 * doc/internal/man3/ossl_ctx_get_data.pod
 */
...
static const OSSL_CTX_METHOD foo_method = {
    foo_new,
    foo_free
};
...
static int test_context(OSSL_CTX *ctx)
{
    FOO *data = NULL;

    return TEST_ptr(data = ossl_ctx_get_data(ctx, 0, &foo_method))
