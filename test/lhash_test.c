...
static int test_int_lhash(void)
{
	...
	/* retrieve */
	for (i = 0; i < n_int_tests; i++)
		if (!TEST_int_eq(*lh_int_retrieve(h, int_tests + i),
				 int_tests[i])) {
			TEST_info("lhash int retrieve value %d", i);
			goto end;
		}
	for (i = 0; i < n_int_tests; i++)
		if (!TEST_ptr_eq(lh_int_retrieve(h, int_tests + i),
				 int_tests + i)) {
	...
	if (!TEST_int_eq(int_not_found, 0)) {
		TEST_info(
			"lhash int doall arg encountered a not found condition");
		goto end;
	}
	for (i = 0; i < n_int_tests; i++)
		if (!TEST_int_eq(int_found[i], 1)) {
...
static int test_int_hashtable(void)
{
	...
	if (!TEST_int_eq(int_not_found, 0)) {
		TEST_info(
			"hashtable int foreach encountered a not found condition");
		goto end;
	}
	...
	for (i = 0; i < n_dels; i++) {
		...
		if (dels[i].should_del) {
			...
			if (!TEST_int_eq(todel, 0)) {
				TEST_info(
					"%d found an entry that shouldn't be there\n",
					dels[i].data);
				goto end;
			}
		}
...
static int test_stress(void)
{
	...
	TEST_info("lhash stress runs in %ld.%ld seconds", delta.tv_sec,
		  delta.tv_usec);
#endif
	lh_int_free(h);
	return testresult;
...
static int test_hashtable_stress(int idx)
{
	...
	for (i = 0; i < n; i++) {
		...
		*p = 3 * i + 1;
		...
		if (!TEST_int_eq(ossl_ht_test_int_insert(h, TO_HT_KEY(&key), p,
							 NULL),
				 1)) {
	...
	for (i = 0; i < n; i++) {
		...
		switch (idx) {
		case 0:
			if (!TEST_int_eq((ossl_ht_delete(h, TO_HT_KEY(&key))),
					 1)) {
			...
			break;
		case 1:
			if (!TEST_ptr(p = ossl_ht_test_int_get(
					      h, TO_HT_KEY(&key), &v)) ||
			    !TEST_int_eq(*p, j)) {
	...
	gettimeofday(&end, NULL);
	timeval_subtract(&delta, &end, &start);
	TEST_info("hashtable stress runs in %ld.%ld seconds", delta.tv_sec,
		  delta.tv_usec);
#endif
	ossl_ht_free(h);
	return testresult;
...
static void do_mt_hash_work(void)
{
	...
	for (iter = 0; iter < TEST_THREAD_ITERATIONS; iter++) {
		...
		if (!CRYPTO_atomic_add(&global_iteration, 1, &giter,
				       worker_lock)) {
			worker_exits[num] =
				"Unable to increment global iterator";
			return;
		}
		switch (behavior) {
		case DO_LOOKUP:
			if (!ossl_ht_read_lock(m_ht))
				break;
			m = ossl_ht_mt_TEST_MT_ENTRY_get(m_ht, TO_HT_KEY(&key),
							 &v);
			if (m != NULL && m != expected_m) {
				worker_exits[num] =
					"Read unexpected value from hashtable";
				TEST_info(
					"Iteration %d Read unexpected value %p when %p expected",
		...
		case DO_REPLACE:
			...
			if (expected_rc !=
			    ossl_ht_mt_TEST_MT_ENTRY_insert(
				    m_ht, TO_HT_KEY(&key), expected_m, r)) {
				TEST_info(
					"Iteration %d Expected rc %d on %s of element %u which is %s\n",
					giter, expected_rc,
					behavior == DO_REPLACE ? "replace" :
								 "insert",
					(unsigned int)index,
					expected_m->in_table ? "in table" :
							       "not in table");
		...
		case DO_DELETE:
			...
			if (expected_rc == 1) {
				...
				CRYPTO_atomic_add(&expected_m->pending_delete,
						  1, &ret, worker_lock);
			}
			if (expected_rc !=
			    ossl_ht_delete(m_ht, TO_HT_KEY(&key))) {
				TEST_info(
					"Iteration %d Expected rc %d on delete of element %u which is %s\n",
...
shutdown:
	...
	gettimeofday(&end, NULL);
	timeval_subtract(&delta, &end, &start);
	TEST_info("multithread stress runs 40000 ops in %ld.%ld seconds",
