...
static int pkcs7_stream_enveloped_no_content_test(void)
{
    ...
    const unsigned char data_enveloped_no_body[] = { 0x30, 0x0b, 0x06, 0x09, 0x2a, 0x86, 0x48,
        0x86, 0xf7, 0x0d, 0x01, 0x07, 0x03 };
    const unsigned char *ptr_env_no_body = data_enveloped_no_body;

    ret = TEST_ptr(p7 = d2i_PKCS7(NULL, &ptr_env_no_body, sizeof(data_enveloped_no_body)))
        && TEST_ptr(sink = BIO_new(BIO_s_null()))
        && TEST_ptr_null(bio = BIO_new_PKCS7(sink, p7))
        && TEST_int_eq(ERR_GET_REASON(ERR_peek_last_error()), PKCS7_R_NO_CONTENT);
...
static int pkcs7_stream_enveloped_signed_no_content_test(void)
{
    ...
    const unsigned char data_enveloped_signed_no_body[] = { 0x30, 0x0b, 0x06, 0x09, 0x2a, 0x86, 0x48,
        0x86, 0xf7, 0x0d, 0x01, 0x07, 0x04 };
    const unsigned char *ptr_env_signed_no_body = data_enveloped_signed_no_body;

    ret = TEST_ptr(p7 = d2i_PKCS7(NULL, &ptr_env_signed_no_body, sizeof(data_enveloped_signed_no_body)))
        && TEST_ptr(sink = BIO_new(BIO_s_null()))
        && TEST_ptr_null(bio = BIO_new_PKCS7(sink, p7))
        && TEST_int_eq(ERR_GET_REASON(ERR_peek_last_error()), PKCS7_R_NO_CONTENT);
