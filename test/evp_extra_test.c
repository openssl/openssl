...
const OPTIONS *test_get_options(void)
{
    static const OPTIONS options[] = {
        ...
        { "test-rand", OPT_TEST_RAND, '-',
            "Test non-derandomised ML-KEM, '-config' must also be specified" },
...
static int test_non_derandomised_ml_kem(void)
{
    ...
    for (i = 0; i < 3; ++i) {
        ...
        *p = OSSL_PARAM_construct_end();
        ...
        /* Get the raw public key */
        publen = EVP_PKEY_get1_encoded_public_key(akey, &rawpub);
        if (!TEST_size_t_eq(publen, v[i]->pubkey_bytes))
            goto done;

        res = -3;
        /* Check that we got the expected 'rho' value in the ciphertext */
        if (!TEST_mem_eq(rawpub + v[i]->vector_bytes, ML_KEM_RANDOM_BYTES,
        ...
        *p = OSSL_PARAM_construct_octet_string(OSSL_RAND_PARAM_TEST_ENTROPY,
        ...
        if (!TEST_int_gt(EVP_PKEY_encapsulate(ctx, NULL, &wrpkeylen, NULL,
                                              &bgenkeylen), 0))
            goto done;
        if (!TEST_size_t_eq(wrpkeylen, v[i]->ctext_bytes)
        ...
        if (!TEST_true(EVP_Digest(wrpkey, v[i]->ctext_bytes,
        ...
        memset(wrpkey, 0, v[i]->ctext_bytes);
        if (!TEST_true(EVP_PKEY_decapsulate(ctx, agenkey, &agenkeylen, wrpkey,
                                            wrpkeylen)))
            goto done;
