...
static APK_DATA keydata[] = {
	...
	{ kExampleRSAKeyPKCS8, sizeof(kExampleRSAKeyPKCS8), "RSA",
	  EVP_PKEY_RSA },
#ifndef OPENSSL_NO_EC
...
static APK_DATA keydata_v2[] = {
	{ kExampleRSAKeyPKCS8_v2, sizeof(kExampleRSAKeyPKCS8_v2), "RSA",
	  EVP_PKEY_RSA },
#ifndef OPENSSL_NO_EC
	{ kExampleECKeyPKCS8_v2, sizeof(kExampleECKeyPKCS8_v2), "EC",
	  EVP_PKEY_EC }
#endif
};

static APK_DATA keycheckdata[] = {
	{ kExampleRSAKeyDER, sizeof(kExampleRSAKeyDER), "RSA", EVP_PKEY_RSA, 1,
	  1, 1, 0 },
	{ kExampleBadRSAKeyDER, sizeof(kExampleBadRSAKeyDER), "RSA",
	  EVP_PKEY_RSA, 0, 1, 1, 0 },
	{ kExampleBad2RSAKeyDER, sizeof(kExampleBad2RSAKeyDER), "RSA",
	  EVP_PKEY_RSA, 0, 0,
	  1 /* Since there are no "params" in an RSA key this passes */, 0 },
#ifndef OPENSSL_NO_EC
	{ kExampleECKeyDER, sizeof(kExampleECKeyDER), "EC", EVP_PKEY_EC, 1, 1,
	  1, 0 },
	/* group is also associated in our pub key */
	{ kExampleECPubKeyDER, sizeof(kExampleECPubKeyDER), "EC", EVP_PKEY_EC,
	  0, 1, 1, 1 },
	{ pExampleECParamDER, sizeof(pExampleECParamDER), "EC", EVP_PKEY_EC, 0,
	  0, 1, 2 },
#ifndef OPENSSL_NO_ECX
...
static EVP_PKEY *make_key_fromdata(char *keytype, OSSL_PARAM *params)
{
	...
	if (!TEST_ptr(pctx = EVP_PKEY_CTX_new_from_name(testctx, keytype,
							testpropq)))
		goto err;
...
static int test_EC_priv_pub(void)
{
     ...
     */
	if (!test_selection(params_and_priv,
	...
	if (!TEST_int_gt(EVP_PKEY_parameters_eq(just_params, just_params), 0) ||
	    ...
	    !TEST_int_gt(EVP_PKEY_parameters_eq(just_params, params_and_priv),
			 0) ||
	    !TEST_int_gt(EVP_PKEY_parameters_eq(just_params,
						params_and_keypair),
			 0) ||
	...
	if (!TEST_size_t_gt(EVP_PKEY_get1_encoded_public_key(params_and_pub,
							     &encoded),
			    0))
		goto err;
	OPENSSL_free(encoded);
	encoded = NULL;
	if (!TEST_size_t_eq(EVP_PKEY_get1_encoded_public_key(just_params,
							     &encoded),
			    0)) {
		OPENSSL_free(encoded);
		encoded = NULL;
	...
	if (!TEST_int_eq(EVP_PKEY_get_octet_string_param(
				 params_and_pub,
				 OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, buffer,
				 sizeof(buffer), &len),
			 1) ||
	...
	if (!TEST_int_eq(EVP_PKEY_get_octet_string_param(
				 params_and_pub,
				 OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, buffer, 10,
				 &len),
			 0))
		goto err;
...
static int test_evp_get_ec_pub_legacy(void)
{
	...
	if (!TEST_true(EVP_PKEY_get_bn_param(pkey, OSSL_PKEY_PARAM_EC_PUB_X,
					     &x)) ||
	    !TEST_true(
...
static int test_EVP_Enveloped(int n)
{
	...
	if (!TEST_ptr(keypair = load_example_rsa_key()) ||
	    ...
	    !TEST_true(EVP_SealUpdate(ctx, ciphertext, &ciphertext_len, msg,
				      sizeof(msg))) ||
...
static int test_EVP_DigestSignInit(int tst)
{
	...
	if (tst >= 6 && tst <= 8) {
		...
		if (!TEST_ptr(a_md_ctx = md_ctx = EVP_MD_CTX_new()) ||
		    !TEST_ptr(a_md_ctx_verify = md_ctx_verify =
				      EVP_MD_CTX_new()))
			goto out;
	}
	...
	if (tst >= 9) {
		...
		if (tst <= 11) {
			...
			if (!TEST_false(EVP_DigestSign(md_ctx, sig,
						       &shortsig_len, kMsg,
						       sizeof(kMsg))))
				goto out;
			/*
		...
		if (!TEST_true(EVP_DigestSignFinal(md_ctx, NULL, &sig_len)) ||
		    ...
		    /*
                    ...
                    * buffer should fail.
                    */
		    || !TEST_false(EVP_DigestSignFinal(md_ctx, sig,
						       &shortsig_len)) ||
	...
	if (tst % 3 != 2) {
		if (tst >= 6 && tst <= 8) {
			if (!TEST_int_gt(BIO_reset(mdbio), 0) ||
			    !TEST_int_gt(BIO_get_md_ctx(mdbio, &md_ctx_verify),
					 0))
				goto out;
		}
		...
		if (tst >= 6 && tst <= 8) {
			if (!TEST_true(BIO_write_ex(mdbio, kMsg, sizeof(kMsg),
						    &written)))
				goto out;
		} else {
			if (!TEST_true(EVP_DigestVerifyUpdate(
				    md_ctx_verify, kMsg, sizeof(kMsg))))
				goto out;
		}
		if (!TEST_int_gt(EVP_DigestVerifyFinal(md_ctx_verify, sig,
						       sig_len),
				 0))
			goto out;
...
static int test_EVP_DigestVerifyInit(void)
{
	...
	if (!TEST_true(EVP_DigestVerifyInit(md_ctx, NULL, EVP_sha256(), NULL,
					    pkey)) ||
...
static int test_siphash_digestsign(void)
{
	...
	memset(buf, 0, 8);
	memset(key, 1, 16);
	if (!TEST_ptr(pkey = EVP_PKEY_new_raw_private_key(EVP_PKEY_SIPHASH,
							  NULL, key, 16)))
		goto out;
	...
	if (!TEST_int_eq(EVP_PKEY_CTX_ctrl(ctx, -1, EVP_PKEY_OP_SIGNCTX,
					   EVP_PKEY_CTRL_SET_DIGEST_SIZE, 8,
					   NULL),
			 1))
		goto out;
	/* reinitialize */
...
static int test_EVP_PKCS82PKEY_wrong_tag(void)
{
	...
	if (!TEST_ptr(membio = BIO_new(BIO_s_mem())) ||
	    !TEST_ptr(pkey = load_example_rsa_key()) ||
	    !TEST_int_gt(i2d_PKCS8PrivateKey_bio(membio, pkey, NULL, NULL, 0,
						 NULL, NULL),
			 0) ||
...
static int ec_export_get_encoding_cb(const OSSL_PARAM params[], void *arg)
{
	...
	*enc = -1;
	...
	if (!TEST_ptr(p = OSSL_PARAM_locate_const(
			      params, OSSL_PKEY_PARAM_EC_ENCODING)) ||
	    !TEST_true(OSSL_PARAM_get_utf8_string_ptr(p, &enc_name)))
		return 0;

	for (i = 0; i < OSSL_NELEM(ec_encodings); i++) {
		if (OPENSSL_strcasecmp(enc_name,
				       ec_encodings[i].encoding_name) == 0) {
...
static int test_EC_keygen_with_enc(int idx)
{
	...
	/* Create key */
	if (!TEST_ptr(
		    kctx = EVP_PKEY_CTX_new_from_pkey(testctx, params, NULL)) ||
...
static int test_EVP_SM2_verify(void)
{
	...
	pkey = PEM_read_bio_PUBKEY_ex(bio, NULL, NULL, NULL, testctx,
				      testpropq);
	...
	if (!TEST_ptr(pctx = EVP_PKEY_CTX_new_from_pkey(testctx, pkey,
							testpropq)))
		goto done;
	...
	if (!TEST_int_gt(EVP_DigestVerifyFinal(mctx, signature,
					       sizeof(signature)),
			 0))
		goto done;
	rc = 1;
...
static int test_EVP_SM2(void)
{
	...
	if (!TEST_ptr(pctx = EVP_PKEY_CTX_new_from_name(testctx, "SM2",
							testpropq)))
		goto done;
	...
	if (!TEST_int_gt(EVP_PKEY_CTX_set_ec_paramgen_curve_nid(pctx, NID_sm2),
			 0))
		goto done;
	...
	if (!TEST_ptr(kctx = EVP_PKEY_CTX_new_from_pkey(testctx, pkeyparams,
							testpropq)))
		goto done;
	...
	if (!TEST_ptr(sctx = EVP_PKEY_CTX_new_from_pkey(testctx, pkey,
							testpropq)))
		goto done;
	...
	for (i = 0; i < 2; i++) {
		...
		if (!TEST_ptr(cctx = EVP_PKEY_CTX_new_from_pkey(testctx, pkey,
								testpropq)))
			goto done;
		...
		if (!TEST_true(EVP_PKEY_encrypt(cctx, ciphertext, &ctext_len,
						kMsg, sizeof(kMsg))))
			goto done;
		...
		if (!TEST_ptr(check_md =
				      EVP_MD_fetch(testctx, mdname, testpropq)))
			goto done;
...
static struct keys_st {
	...
	{ EVP_PKEY_POLY1305,
	  (const uint8_t *)"01234567890123456789012345678901", NULL, NULL, 32, 0
#endif
#ifndef OPENSSL_NO_SIPHASH
	},
	{ EVP_PKEY_SIPHASH, (const uint8_t *)"0123456789012345", NULL, NULL, 16,
	  0
#endif
	},
#ifndef OPENSSL_NO_ECX
	{ EVP_PKEY_X25519, (const uint8_t *)"01234567890123456789012345678901",
	  (const unsigned char *)"abcdefghijklmnopqrstuvwxyzabcdef", NULL, 32,
	  32 },
...
#endif
#ifndef OPENSSL_NO_ML_KEM
	{ NID_undef, ml_kem_seed, ml_kem_512_pubkey, "ML-KEM-512",
	  ML_KEM_SEED_BYTES, sizeof(ml_kem_512_pubkey) },
	{ NID_undef, ml_kem_seed, ml_kem_768_pubkey, "ML-KEM-768",
	  ML_KEM_SEED_BYTES, sizeof(ml_kem_768_pubkey) },
			       ...
			       size_t *retlen)
{
	...
	if (!TEST_true(EVP_PKEY_todata(pkey, OSSL_KEYMGMT_SELECT_PRIVATE_KEY,
				       &params)))
		goto done;
...
static int test_set_get_raw_keys_int(int tst, int pub, int uselibctx)
{
	...
	if (pub) {
		...
		if (uselibctx || keys[tst].name != NULL) {
			pkey = EVP_PKEY_new_raw_public_key_ex(testctx, name,
							      NULL, in, inlen);
		} else {
			pkey = EVP_PKEY_new_raw_public_key(keys[tst].type, NULL,
							   in, inlen);
		}
#else
		return 1;
...
#endif
		if (uselibctx || keys[tst].name != NULL) {
			pkey = EVP_PKEY_new_raw_private_key_ex(testctx, name,
							       NULL, in, inlen);
		} else {
	...
	if (!TEST_ptr(pkey) || !TEST_int_eq(EVP_PKEY_eq(pkey, pkey), 1) ||
	    (!pub &&
	     !TEST_true(EVP_PKEY_get_raw_private_key(pkey, NULL, &len))) ||
	    (pub &&
	     !TEST_true(EVP_PKEY_get_raw_public_key(pkey, NULL, &len))) ||
	    !TEST_true(len == inlen))
		goto done;
	if (tst != 1) {
		/*
         ...
         */
		if ((!pub && !TEST_false(EVP_PKEY_get_raw_private_key(
				     pkey, shortbuf, &shortlen))) ||
		    (pub && !TEST_false(EVP_PKEY_get_raw_public_key(
				    pkey, shortbuf, &shortlen))))
			goto done;
	}
	if (!TEST_ptr(buf = OPENSSL_zalloc(len <= 80 ? 80 : len)))
		goto done;
	if ((!pub &&
	     !TEST_true(EVP_PKEY_get_raw_private_key(pkey, buf, &len))) ||
...
{
	static unsigned char key[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
				       0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d,
...
static int test_HKDF(void)
{
	...
	const unsigned char expected[] = { 0xe5, 0x07, 0x70, 0x7f, 0xc6,
					   0x78, 0xd6, 0x54, 0x32, 0x5f,
					   0x7e, 0xc5, 0x7b, 0x59, 0x3e,
					   0xd8, 0x03, 0x6b, 0xed, 0xca };
	size_t expectedlen = sizeof(expected);

	if (!TEST_ptr(pctx = EVP_PKEY_CTX_new_from_name(testctx, "HKDF",
							testpropq)))
		goto done;
...
static int test_emptyikm_HKDF(void)
{
	...
	const unsigned char expected[] = { 0x68, 0x81, 0xa5, 0x3e, 0x5b,
					   0x9c, 0x7b, 0x6f, 0x2e, 0xec,
					   0xc8, 0x47, 0x7c, 0xfa, 0x47,
					   0x35, 0x66, 0x82, 0x15, 0x30 };
	size_t expectedlen = sizeof(expected);
...
static int test_EVP_PKEY_CTX_get_set_params(EVP_PKEY *pkey)
{
	...
	if (!TEST_ptr(params) || !TEST_ptr(OSSL_PARAM_locate_const(
					 params, OSSL_SIGNATURE_PARAM_DIGEST)))
		goto err;
...
static int test_DSA_get_set_params(void)
{
     ...
     */
	if (!TEST_ptr(
		    pctx = EVP_PKEY_CTX_new_from_name(testctx, "DSA", NULL)) ||
...
static int test_RSA_get_set_params(void)
{
     ...
     */
	if (!TEST_ptr(
		    pctx = EVP_PKEY_CTX_new_from_name(testctx, "RSA", NULL)) ||
...
static int test_RSA_OAEP_set_get_params(void)
{
...

	{
		...
		params[0] = OSSL_PARAM_construct_int(
			OSSL_SIGNATURE_PARAM_PAD_MODE, &padding);
	...
	}
	{
		...
		params[0] = OSSL_PARAM_construct_utf8_string(
			OSSL_ASYM_CIPHER_PARAM_OAEP_DIGEST, oaepmd,
			sizeof(oaepmd));
		params[1] = OSSL_PARAM_construct_utf8_string(
			OSSL_ASYM_CIPHER_PARAM_MGF1_DIGEST, mgf1md,
			sizeof(mgf1md));
...
static int test_RSA_OAEP_set_null_label(void)
{
	...
	if (!TEST_ptr(key = load_example_rsa_key()) ||
	    !TEST_ptr(key_ctx =
			      EVP_PKEY_CTX_new_from_pkey(testctx, key, NULL)) ||
	    !TEST_true(EVP_PKEY_encrypt_init(key_ctx)))
		goto err;

	if (!TEST_true(EVP_PKEY_CTX_set_rsa_padding(key_ctx,
...
static int test_decrypt_null_chunks(void)
{
	...
	const unsigned char key[32] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
					0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
...
static int test_rand_agglomeration(void)
{
	...
	*p = OSSL_PARAM_construct_end();
	res = TEST_true(EVP_RAND_CTX_set_params(ctx, params)) &&
	      TEST_true(EVP_RAND_generate(ctx, out, sizeof(out), 0, 1, NULL,
					  0)) &&
...
static int test_evp_iv_aes(int idx)
{
	int ret = 0;
	EVP_CIPHER_CTX *ctx = NULL;
	unsigned char key[16] = { 0x4c, 0x43, 0xdb, 0xdd, 0x42, 0x73,
				  0x47, 0xd1, 0xe5, 0x62, 0x7d, 0xcd,
				  0x4d, 0x76, 0x4d, 0x57 };
	...
	unsigned char ccm_state[7] = {
		0x57, 0x71, 0x7d, 0xad, 0xdb, 0x9b, 0x98
	};
	...
	case 6:
		type = (type != NULL) ? type :
					EVP_CIPHER_fetch(testctx, "aes-128-cbc",
							 testpropq);
		ref_iv = cbc_state;
	...
	case 7:
		type = (type != NULL) ? type :
					EVP_CIPHER_fetch(testctx, "aes-128-ofb",
							 testpropq);
		ref_iv = ofb_state;
	...
	case 8:
		type = (type != NULL) ? type :
					EVP_CIPHER_fetch(testctx, "aes-128-cfb",
							 testpropq);
		ref_iv = cfb_state;
	...
	case 9:
		type = (type != NULL) ? type :
					EVP_CIPHER_fetch(testctx, "aes-128-gcm",
							 testpropq);
		ref_iv = gcm_state;
	...
	case 10:
		type = (type != NULL) ? type :
					EVP_CIPHER_fetch(testctx, "aes-128-ccm",
							 testpropq);
		ref_iv = ccm_state;
	...
	case 11:
		type = (type != NULL) ? type :
					EVP_CIPHER_fetch(testctx, "aes-128-ocb",
							 testpropq);
		ref_iv = ocb_state;
...
static int test_evp_iv_des(int idx)
{
	int ret = 0;
	EVP_CIPHER_CTX *ctx = NULL;
	static const unsigned char key[24] = { 0x01, 0x23, 0x45, 0x67, 0x89,
					       0xab, 0xcd, 0xef, 0xf1, 0xe0,
					       0xd3, 0xc2, 0xb5, 0xa4, 0x97,
					       0x86, 0xfe, 0xdc, 0xba, 0x98,
					       0x76, 0x54, 0x32, 0x10 };
...
{
	int ret = 0;
	static const char *algos[4] = { "bf-ecb", "bf-cbc", "bf-cfb",
					"bf-ofb" };
	int ivlen[4] = { 0, 8, 8, 8 };
	EVP_CIPHER *cipher = NULL;
	...
	if (!TEST_ptr(cipher = EVP_CIPHER_fetch(testctx, algos[idx],
						testpropq)) ||
...
static int test_EVP_rsa_pss_with_keygen_bits(void)
{
	...
	ret = TEST_ptr(md) &&
	      TEST_ptr((ctx = EVP_PKEY_CTX_new_from_name(testctx, "RSA-PSS",
							 testpropq))) &&
...
static int test_EVP_rsa_pss_set_saltlen(void)
{
	...
	ret = TEST_ptr(pkey = load_example_rsa_key()) &&
	      ...
	      TEST_int_gt(EVP_PKEY_CTX_set_rsa_pss_saltlen(pkey_ctx,
							   test_value),
			  0) &&
...
static int evp_init_seq_set_iv(EVP_CIPHER_CTX *ctx, const EVP_INIT_TEST_st *t)
{
	...
	if (t->ivlen != 0) {
		if (!TEST_int_gt(EVP_CIPHER_CTX_ctrl(ctx,
...
static int test_evp_init_seq(int idx)
{
	...
	if (!TEST_true(EVP_CipherInit_ex(ctx, type, NULL, NULL, NULL,
					 t->initenc))) {
	...
	if (!TEST_true(EVP_CipherInit_ex(ctx, NULL, NULL, NULL, NULL,
					 t->finalenc))) {
		errmsg = "FINAL_ENC_INIT";
		goto err;
	}
	if (!TEST_true(EVP_CipherUpdate(ctx, outbuf, &outlen1, t->input,
					(int)t->inlen))) {
	...
	if (!TEST_mem_eq(t->expected, t->expectedlen, outbuf,
			 outlen1 + outlen2)) {
		errmsg = "WRONG_RESULT";
		goto err;
	}
...
static int test_evp_reinit_seq(int idx)
{
	...
	if (!TEST_ptr(ctx = EVP_CIPHER_CTX_new()) ||
	    ...
	    || !TEST_true(EVP_CipherInit_ex2(ctx, type, t->key, t->iv,
					     t->initenc, NULL))
	    /* first iteration */
...
static int test_evp_reset(int idx)
{
	...
	if (!TEST_ptr(type = EVP_CIPHER_fetch(testctx, "aes-128-cfb",
					      testpropq))) {
		errmsg = "CIPHER_FETCH";
		goto err;
	}
	if (!TEST_true(EVP_CipherInit_ex(ctx, type, NULL, kCFBDefaultKey,
					 iCFBIV, t->enc))) {
		errmsg = "CIPHER_INIT";
		goto err;
	}
...
static const TEST_GCM_IV_REINIT_st gcm_reinit_tests[] = {
	{ iGCMResetIV1, iGCMResetIV2, gcmResetCiphertext1, gcmResetCiphertext2,
	  gcmResetTag1, gcmResetTag2, sizeof(iGCMResetIV1),
	  sizeof(iGCMResetIV2), sizeof(gcmResetCiphertext1),
	  sizeof(gcmResetCiphertext2) },
	{ iGCMResetIV2, iGCMResetIV1, gcmResetCiphertext2, gcmResetCiphertext1,
	  gcmResetTag2, gcmResetTag1, sizeof(iGCMResetIV2),
	  sizeof(iGCMResetIV1), sizeof(gcmResetCiphertext2),
...
static int test_gcm_reinit(int idx)
{
	...
	if (!TEST_true(EVP_CipherInit_ex(ctx, NULL, NULL, kGCMResetKey, t->iv1,
					 1))) {
		errmsg = "SET_IV1";
		goto err;
	}
	if (!TEST_true(EVP_CipherUpdate(ctx, NULL, &outlen3, gcmAAD,
					sizeof(gcmAAD)))) {
		errmsg = "AAD1";
		goto err;
	}
...
static int test_ivlen_change(int idx)
{
	...
	if (!TEST_ptr(ciph = EVP_CIPHER_fetch(
			      testctx, ivlen_change_ciphers[idx], testpropq)))
		goto err;

	if (!TEST_true(
		    EVP_CipherInit_ex(ctx, ciph, NULL, kGCMDefaultKey, iv, 1)))
		goto err;
	...
	ERR_set_mark();
	if (!TEST_false(EVP_CipherUpdate(ctx, outbuf, &outlen,
					 gcmDefaultPlaintext,
					 sizeof(gcmDefaultPlaintext)))) {
		ERR_clear_last_mark();
		goto err;
...
static int test_custom_pmeth(int idx)
{
	...
	if (id == EVP_PKEY_ED25519) {
		EVP_PKEY_meth_set_digestsign(custom_pmeth,
					     custom_pmeth_digestsign);
	}
	if (id == EVP_PKEY_DH || id == EVP_PKEY_X25519) {
		EVP_PKEY_meth_set_derive(custom_pmeth, NULL,
					 custom_pmeth_derive);
	} else {
	...
	if (doderive) {
		...
		if (id == EVP_PKEY_ED25519) {
			if (!TEST_true(EVP_DigestSign(ctx, res, &reslen, msg,
						      sizeof(msg))))
				goto err;
		} else {
			if (!TEST_true(
...
static int test_custom_md_meth(void)
{
	...
	nid = OBJ_create("1.3.6.1.4.1.16604.998866.1", "custom-md",
			 "custom-md");
	...
	if (!TEST_int_eq(OBJ_create("1.3.6.1.4.1.16604.998866.1", "custom-md",
				    "custom-md"),
			 NID_undef) ||
	...
	o = ASN1_OBJECT_create(
		nid, (unsigned char *)"\53\6\1\4\1\201\201\134\274\373\122\1",
		12, "custom-md", "custom-md");
...
static int test_custom_ciph_meth(void)
{
	...
	if (!TEST_true(EVP_CIPHER_meth_set_init(tmp, custom_ciph_init)) ||
	    !TEST_true(EVP_CIPHER_meth_set_flags(tmp,
						 EVP_CIPH_ALWAYS_CALL_INIT)) ||
	    !TEST_true(EVP_CIPHER_meth_set_cleanup(tmp, custom_ciph_cleanup)) ||
	    !TEST_true(EVP_CIPHER_meth_set_impl_ctx_size(
		    tmp, sizeof(custom_ciph_ctx))))
		goto err;
	...
	if (!TEST_ptr(ciphctx)
	    ...
	    !TEST_true(EVP_CipherInit_ex(ciphctx, EVP_aes_128_cbc(), NULL, NULL,
					 NULL, 1)) ||
...
static int test_signatures_with_engine(int tst)
{
	...
	const unsigned char cmackey[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
					  0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
					  0x0c, 0x0d, 0x0e, 0x0f };
	const unsigned char ed25519key[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
					     0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
					     0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11,
					     0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
					     0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d,
	...
	ret = EVP_DigestSignInit(ctx, NULL, tst == 2 ? NULL : EVP_sha256(),
				 NULL, pkey);
	if (tst == 0) {
		if (!TEST_true(ret))
			goto err;
...
static int test_cipher_with_engine(void)
{
	...
	if (!TEST_true(EVP_EncryptInit_ex(ctx, EVP_aes_128_cbc(), e, keyiv,
					  keyiv)))
		goto err;
...
static int test_ecx_not_private_key(int tst)
{
	...
	case NID_undef:
		return TEST_skip(
			"signing not supported for X25519/X448/ML-KEM");
	}
	...
	if (!TEST_int_eq(EVP_DigestSign(ctx, mac, &maclen, msg, sizeof(msg)),
			 0))
		goto err;
...
static int test_sign_continuation(void)
{
	...
	if (!TEST_ptr(pctx = EVP_PKEY_CTX_new_from_name(testctx, "RSA",
	    ...
	    !TEST_true(
		    EVP_PKEY_fromdata(pctx, &pkey, EVP_PKEY_KEYPAIR, NULL)) ||
	    !TEST_ptr(pkey))
		goto end;
			   ...
			   const unsigned char *gcm_tag, size_t gcm_tag_s)
{
	...
	if (!TEST_true(EVP_EncryptInit_ex2(ctx, cipher, gcm_key, gcm_iv,
					   params)) ||
	    (gcm_aad != NULL &&
	     !TEST_true(EVP_EncryptUpdate(ctx, NULL, &outlen, gcm_aad,
					  (int)gcm_aad_s))) ||
	    !TEST_true(EVP_EncryptUpdate(ctx, outbuf, &outlen, gcm_pt,
					 (int)gcm_pt_s)) ||
			   ...
			   const unsigned char *gcm_tag, size_t gcm_tag_s)
{
	...
	if (!TEST_true(EVP_DecryptInit_ex2(ctx, cipher, gcm_key, gcm_iv,
					   params)) ||
	    (gcm_aad != NULL &&
	     !TEST_true(EVP_DecryptUpdate(ctx, NULL, &outlen, gcm_aad,
					  (int)gcm_aad_s))) ||
	    !TEST_true(EVP_DecryptUpdate(ctx, outbuf, &outlen, gcm_ct,
					 (int)gcm_ct_s)) ||
...
static int test_aes_gcm_ivlen_change_cve_2023_5363(void)
{
	...
	static const unsigned char gcm_key[] = {
		0xd0, 0xc2, 0x67, 0xc1, 0x9f, 0x30, 0xd8, 0x0b,
		0x89, 0x14, 0xbb, 0xbf, 0xb7, 0x2f, 0x73, 0xb8,
		0xd3, 0xcd, 0x5f, 0x6a, 0x78, 0x70, 0x15, 0x84,
		0x8a, 0x7b, 0x30, 0xe3, 0x8f, 0x16, 0xf1, 0x8b,
	};
	static const unsigned char gcm_iv[] = {
		0xb6, 0xdc, 0xda, 0x95, 0xac, 0x99, 0x77, 0x76, 0x25, 0xae,
		0x87, 0xf8, 0xa3, 0xa9, 0xdd, 0x64, 0xd7, 0x9b, 0xbd, 0x5f,
		0x4a, 0x0e, 0x54, 0xca, 0x1a, 0x9f, 0xa2, 0xe3, 0xf4, 0x5f,
		0x5f, 0xc2, 0xce, 0xa7, 0xb6, 0x14, 0x12, 0x6f, 0xf0, 0xaf,
		0xfd, 0x3e, 0x17, 0x35, 0x6e, 0xa0, 0x16, 0x09, 0xdd, 0xa1,
		0x3f, 0xd8, 0xdd, 0xf3, 0xdf, 0x4f, 0xcb, 0x18, 0x49, 0xb8,
		0xb3, 0x69, 0x2c, 0x5d, 0x4f, 0xad, 0x30, 0x91, 0x08, 0xbc,
		0xbe, 0x24, 0x01, 0x0f, 0xbe, 0x9c, 0xfb, 0x4f, 0x5d, 0x19,
		0x7f, 0x4c, 0x53, 0xb0, 0x95, 0x90, 0xac, 0x7b, 0x1f, 0x7b,
		0xa0, 0x99, 0xe1, 0xf3, 0x48, 0x54, 0xd0, 0xfc, 0xa9, 0xcc,
		0x91, 0xf8, 0x1f, 0x9b, 0x6c, 0x9a, 0xe0, 0xdc, 0x63, 0xea,
		       ...
		       const unsigned char *rc4_ct, size_t rc4_ct_s)
{
	...
	if (!TEST_true(
		    EVP_EncryptInit_ex2(ctx, cipher, rc4_key, NULL, params)) ||
	    !TEST_true(EVP_EncryptUpdate(ctx, outbuf, &outlen, rc4_pt,
					 (int)rc4_pt_s)) ||
	    !TEST_true(EVP_EncryptFinal_ex(ctx, outbuf, &tmplen)))
		goto err;
		       ...
		       const unsigned char *rc4_ct, size_t rc4_ct_s)
{
	...
	if (!TEST_true(
		    EVP_DecryptInit_ex2(ctx, cipher, rc4_key, NULL, params)) ||
	    !TEST_true(EVP_DecryptUpdate(ctx, outbuf, &outlen, rc4_ct,
					 (int)rc4_ct_s)) ||
...
static int test_aes_rc4_keylen_change_cve_2023_5363(void)
{
	...
	static const struct {
		...
		unsigned char padding[11];
			...
			0x32, 0x91 } };
	static const unsigned char rc4_pt[] = { 0x00, 0x00, 0x00, 0x00,
						0x00, 0x00, 0x00, 0x00,
						0x00, 0x00, 0x00, 0x00,
						0x00, 0x00, 0x00, 0x00 };
	static const unsigned char rc4_ct[] = { 0x80, 0xad, 0x97, 0xbd,
						0xc9, 0x73, 0xdf, 0x8a,
						0x2e, 0x87, 0x9e, 0x92,
						0xa4, 0x97, 0xef, 0xda };
...
static int test_invalid_ctx_for_digest(void)
{
	...
	if (!TEST_int_eq(EVP_DigestUpdate(mdctx, "test", sizeof("test") - 1),
			 0))
		ret = 0;
	else
		ret = 1;
...
static int test_evp_cipher_pipeline(void)
{
	...
	if (!TEST_false(EVP_CIPHER_can_pipeline(EVP_aes_256_gcm(), 1)))
		goto end;
	if (!TEST_false(EVP_CipherPipelineEncryptInit(ctx, pipeline_cipher, key,
						      keylen, EVP_MAX_PIPES + 1,
						      NULL, 0)))
		goto end;
	...
	for (numpipes = 1; numpipes <= EVP_MAX_PIPES; numpipes++) {
		for (plaintextlen = 1; plaintextlen <= 256; plaintextlen++) {
			...
			memset(ciphertext_array_p, 0,
			       sizeof(ciphertext_array_p));
			...
			for (i = 0; i < numpipes; i++) {
				if (!TEST_ptr(iv_array[i] =
						      OPENSSL_malloc(ivlen)) ||
				    !TEST_ptr(plaintext_array[i] =
						      OPENSSL_malloc(
							      plaintextlen)) ||
				    !TEST_ptr(
					    ciphertext_array_p[i] = OPENSSL_malloc(
						    plaintextlen +
						    EVP_MAX_BLOCK_LENGTH)) ||
				    !TEST_ptr(tag_array[i] =
						      OPENSSL_malloc(taglen)))
					goto err;

				memset(iv_array[i], (unsigned char)(i + 33),
				       ivlen);
				memset(plaintext_array[i],
				       (unsigned char)(i + 1), plaintextlen);
			...
			if (!TEST_ptr(ciphertext = OPENSSL_malloc(
					      plaintextlen +
					      EVP_MAX_BLOCK_LENGTH)) ||
			    !TEST_ptr(tag = OPENSSL_malloc(taglen)) ||
			    !TEST_ptr(exp_plaintext =
					      OPENSSL_malloc(plaintextlen)))
				goto err;

			/* Encrypt using pipeline API */
			if (!TEST_true(EVP_CIPHER_CTX_reset(ctx)) ||
			    !TEST_true(EVP_CIPHER_can_pipeline(pipeline_cipher,
							       1)) ||
			    ...
			    || !TEST_true(EVP_CipherPipelineUpdate(
				       ...
				       inlen_array)) ||
			    !TEST_true(EVP_CipherPipelineUpdate(
				    ctx, ciphertext_array_p, outlen_array,
				    outsize_array,
				    (const unsigned char **)plaintext_array,
				    inlen_array)))
				goto err;
			...
			for (i = 0; i < numpipes; i++) {
				...
				temp[i] = ciphertext_array_p[i] +
					  ciphertextlen_array[i];
				outsize_array[i] = outsize_array[i] -
						   ciphertextlen_array[i];
			}

			if (!TEST_true(EVP_CipherPipelineFinal(
				    ctx, temp, outlen_array, outsize_array)))
				goto err;
			...
			for (i = 0; i < numpipes; i++) {
				if (!TEST_true(EVP_EncryptInit(ctx, cipher, key,
							       iv_array[i])) ||
				    !TEST_true(EVP_EncryptUpdate(
					    ...
					    (int)plaintextlen)) ||
				    !TEST_true(EVP_EncryptUpdate(
					    ctx, ciphertext, &outlen,
					    plaintext_array[i],
					    (int)plaintextlen)))
					goto err;
				ciphertextlen = outlen;

				if (!TEST_true(EVP_EncryptFinal_ex(
					    ctx, ciphertext + outlen, &outlen)))
					goto err;
				ciphertextlen += outlen;

				params[0] = OSSL_PARAM_construct_octet_string(
					OSSL_CIPHER_PARAM_AEAD_TAG, (void *)tag,
					taglen);
				if (!TEST_true(EVP_CIPHER_CTX_get_params(
					    ctx, params)))
					goto err;
				...
				if (!TEST_mem_eq(ciphertext_array_p[i],
				    ...
				    !TEST_mem_eq(tag_array[i], taglen, tag,
						 taglen))
					goto err;
			}
			...
			if (!TEST_true(EVP_CIPHER_CTX_reset(ctx)) ||
			    ...
			    !TEST_true(EVP_CipherPipelineUpdate(
				    ...
				    inlen_array)) ||
			    !TEST_true(EVP_CipherPipelineUpdate(
				    ctx, plaintext_array, outlen_array,
				    outsize_array,
			...
			for (i = 0; i < numpipes; i++) {
				...
				outsize_array[i] =
					outsize_array[i] - outlen_array[i];
			}
			...
			for (i = 0; i < numpipes; i++) {
				memset(exp_plaintext, (unsigned char)(i + 1),
				       plaintextlen);
				if (!TEST_mem_eq(plaintext_array[i],
						 plaintextlen, exp_plaintext,
						 plaintextlen))
					goto err;
			}
