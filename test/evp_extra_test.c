...
static int test_EVP_SM2_verify(void)
{
    ...
    const uint8_t signature[] = {
       0x30, 0x44, 0x02, 0x20,
       0x40, 0xF1, 0xEC, 0x59, 0xF7, 0x93, 0xD9, 0xF4, 0x9E, 0x09, 0xDC, 0xEF, 0x49, 0x13, 0x0D, 0x41, 0x94, 0xF7, 0x9F, 0xB1, 0xEE, 0xD2, 0xCA, 0xA5, 0x5B, 0xAC, 0xDB, 0x49, 0xC4, 0xE7, 0x55, 0xD1,

       0x02, 0x20,
       0x6F, 0xC6, 0xDA, 0xC3, 0x2C, 0x5D, 0x5C, 0xF1, 0x0C, 0x77, 0xDF, 0xB2, 0x0F, 0x7C, 0x2E, 0xB6, 0x67, 0xA4, 0x57, 0x87, 0x2F, 0xB0, 0x9E, 0xC5, 0x63, 0x27, 0xA6, 0x7E, 0xC7, 0xDE, 0xEB, 0xE7
    };
    ...
    if (!TEST_true(EVP_PKEY_CTX_set_sm2_uid(verify_ctx, user_id) > 0))
        goto done;

    if (!TEST_true(EVP_DigestVerifyInit(md_ctx_verify, NULL, EVP_sm3(), NULL, pkey)))
        goto done;

    if (!TEST_true(EVP_DigestVerifyUpdate(md_ctx_verify, input, strlen(input))))
        goto done;

    if (!TEST_true(EVP_DigestVerifyFinal(md_ctx_verify, signature, sizeof(signature))))
        goto done;
...
static int test_EVP_SM2(void)
{
    ...
    for(use_uid = 0; use_uid <= 1; ++use_uid) {
        ...
        if (use_uid) {
            ...
            if (!TEST_true(EVP_PKEY_CTX_get_sm2_uid(sign_ctx, &uid_output) > 0))
                goto done;

            if (!TEST_true(strcmp(uid_output, "nobody@example.com") == 0))
                goto done;

            OPENSSL_free(uid_output);
