...
#if !defined(OPENSSL_NO_BLAKE2) && !defined(OPENSSL_NO_ARGON2)
static int test_kdf_argon2_basic(void)
{
    int ret;
    EVP_KDF_CTX *kctx;
    OSSL_PARAM p[3];
    ...
    p[0] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_PASSWORD,
                                             (char *)"1234567890", 10);
    p[1] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT,
                                             (char *)"saltsalt", 8);
    p[2] = OSSL_PARAM_construct_end();

    ret =
        TEST_ptr(kctx = get_kdfbyname(SN_argon2d))
        && TEST_true(EVP_KDF_CTX_set_params(kctx, p))
...
static int test_kdf_argon2_custom(void)
{
    int ret;
    EVP_KDF_CTX *kctx;
    OSSL_PARAM p[5];
    ...
    /* Single threaded, multiple lanes, custom memory hardness, 2d. */

    int threads = 2;
    int m_cost = 65536;

    static const unsigned char expected[sizeof(out)] = {
        0x5c, 0xa0, 0xab, 0x13, 0x5d, 0xe1, 0x24, 0x14,
        0x54, 0x84, 0x01, 0x72, 0x69, 0x6c, 0x30, 0x1c,
        0x7b, 0x8f, 0xd9, 0x9a, 0x78, 0x8c, 0xd1, 0x1c,
        0xf9, 0x69, 0x90, 0x44, 0xca, 0xdf, 0x7f, 0xca,
        0x0a, 0x6e, 0x37, 0x62, 0xcb, 0x30, 0x43, 0xa7,
        0x1a, 0xdf, 0x65, 0x53, 0xdb, 0x3f, 0xd7, 0x92,
        0x51, 0x01, 0xb0, 0xcc, 0xf8, 0x86, 0x8b, 0x09,
        0x84, 0x92, 0xa4, 0xad, 0xdb, 0x24, 0x86, 0xbc,
    };
    ...
    p[3] = OSSL_PARAM_construct_int(OSSL_KDF_PARAM_ARGON2_MEMCOST, &m_cost);
...
static int test_kdf_argon2_no_thread(void)
{
    int ret;
    EVP_KDF_CTX *kctx;
    OSSL_PARAM p[6];
    ...
    /* Test m_cost >= 8 lanes constraint. */
...
static int test_kdf_argon2_param1(void)
{
    int ret;
    EVP_KDF_CTX *kctx;
    OSSL_PARAM p[5];
...
static int test_kdf_argon2_param2(void)
{
    int ret;
    EVP_KDF_CTX *kctx;
    OSSL_PARAM p[4];

    /* Test m_cost >= ARGON2_MIN_MEMORY constraint. */
    ...
    p[1] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT,
                                             (char *)"saltsalt", 8);
    p[2] = OSSL_PARAM_construct_int(OSSL_KDF_PARAM_ARGON2_MEMCOST, &m_cost);
    p[3] = OSSL_PARAM_construct_end();

    ret =
        TEST_ptr(kctx = get_kdfbyname(SN_argon2d))
        && TEST_false(EVP_KDF_CTX_set_params(kctx, p));

    EVP_KDF_CTX_free(kctx);
    return ret;
}

static int test_kdf_argon2_threads(void)
{
    int ret;
    EVP_KDF_CTX *kctx;
    OSSL_PARAM p[6];
    ...
    static const unsigned char expected[sizeof(out)] = {
        
    };

# if !defined(OPENSSL_THREADS)
    /*
     * Test threaded Argon2 variant. Note that this requires that threading
     * was enabled during build.
     */
    return 1;
# endif
    ...
    p[1] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT,
                                             (char *)"saltsalt", 8);
    p[2] = OSSL_PARAM_construct_int(OSSL_KDF_PARAM_THREADS, &threads);
    p[3] = OSSL_PARAM_construct_int(OSSL_KDF_PARAM_ARGON2_LANES, &threads);
    p[4] = OSSL_PARAM_construct_int(OSSL_KDF_PARAM_ARGON2_MEMCOST, &m_cost);
    p[5] = OSSL_PARAM_construct_end();
...
#endif /* !defined(OPENSSL_NO_BLAKE2) && !defined(OPENSSL_NO_ARGON2) */
...
#endif
#if !defined(OPENSSL_NO_BLAKE2) && !defined(OPENSSL_NO_ARGON2)
    ADD_TEST(test_kdf_argon2_basic);
    ADD_TEST(test_kdf_argon2_custom);
    ADD_TEST(test_kdf_argon2_no_thread);
    ADD_TEST(test_kdf_argon2_threads);
    ADD_TEST(test_kdf_argon2_param1);
    ADD_TEST(test_kdf_argon2_param2);
#endif
