...
static int do_kdf_hkdf_gettables(int extract_only, int has_digest)
{
	...
	if (!TEST_ptr(gettables = EVP_KDF_CTX_gettable_params(kctx)) ||
	    !TEST_ptr(p = OSSL_PARAM_locate_const(gettables,
						  OSSL_KDF_PARAM_SIZE)) ||
	    !TEST_ptr(p = OSSL_PARAM_locate_const(gettables,
						  OSSL_KDF_PARAM_MODE)) ||
	    !TEST_ptr(p = OSSL_PARAM_locate_const(gettables,
	...
	if (has_digest) {
		if (!TEST_int_eq(EVP_KDF_CTX_get_params(kctx, params_get), 1) ||
		    !TEST_size_t_eq(sz, extract_only ? SHA256_DIGEST_LENGTH :
						       SIZE_MAX))
			goto err;
	} else {
	...
	params_get[0] = OSSL_PARAM_construct_utf8_string(
		OSSL_KDF_PARAM_DIGEST, digest, sizeof(digest));
	params_get[1] = OSSL_PARAM_construct_end();
	if (has_digest) {
		if (!TEST_int_eq(EVP_KDF_CTX_get_params(kctx, params_get), 1) ||
	...
	params_get[0] =
		OSSL_PARAM_construct_int(OSSL_KDF_PARAM_MODE, &mode_int);
	params_get[1] = OSSL_PARAM_construct_end();
	if (!TEST_int_eq(EVP_KDF_CTX_get_params(kctx, params_get), 1) ||
	...
	if (!TEST_int_eq(EVP_KDF_CTX_get_params(kctx, params_get), 1) ||
	    !TEST_str_eq(mode_utf8, extract_only ? "EXTRACT_ONLY" :
						   "EXTRACT_AND_EXPAND") ||
	    !TEST_int_eq(mode_int,
			 extract_only ? EVP_KDF_HKDF_MODE_EXTRACT_ONLY :
					EVP_KDF_HKDF_MODE_EXTRACT_AND_EXPAND) ||
...
static int kdf_hkdf_fixed_digest_change_digest(const char *kdf_name,
					       char *bad_digest)
{
	int ret;
	EVP_KDF_CTX *kctx = NULL;
	...
	params_set =
		construct_hkdf_params(bad_digest, "secret", 6, "salt", "label");
...
{
	return kdf_hkdf_fixed_digest_change_digest(OSSL_KDF_NAME_HKDF_SHA256,
						   "sha512") &&
	       kdf_hkdf_fixed_digest_change_digest(OSSL_KDF_NAME_HKDF_SHA384,
						   "sha512") &&
	       kdf_hkdf_fixed_digest_change_digest(OSSL_KDF_NAME_HKDF_SHA512,
						   "sha256");
}

static int kdf_hkdf_fixed_digest_preserve_digest(const char *kdf_name,
						 ...
						 char *bad_digest)
{
	...
	params_set[0] = OSSL_PARAM_construct_utf8_string(
		OSSL_ALG_PARAM_DIGEST, bad_digest, strlen(bad_digest));
...
{
	return kdf_hkdf_fixed_digest_preserve_digest(OSSL_KDF_NAME_HKDF_SHA256,
						     "SHA2-256", "SHA2-512") &&
	       kdf_hkdf_fixed_digest_preserve_digest(OSSL_KDF_NAME_HKDF_SHA384,
						     "SHA2-384", "SHA2-512") &&
	       kdf_hkdf_fixed_digest_preserve_digest(OSSL_KDF_NAME_HKDF_SHA512,
						     "SHA2-512", "SHA2-256");
}
...
static int do_kdf_hkdf_set_invalid_param(const char *key, int type)
{
	...
	if (type == OSSL_PARAM_UTF8_STRING)
		...
		params[0] = OSSL_PARAM_construct_octet_string(key, buf,
							      sizeof(buf));
	params[1] = OSSL_PARAM_construct_end();
	if (!TEST_int_eq(EVP_KDF_CTX_set_params(kctx, params), 0))
		goto end;
...
static int test_kdf_pbkdf1(void)
{
	...
	if (!TEST_ptr(params) ||
	    !TEST_ptr(kctx = get_kdfbyname_libctx(libctx,
...
static int test_kdf_scrypt(void)
{
	...
	*p = OSSL_PARAM_construct_end();
	...
	for (i = 0; ret && i < 2; ++i) {
		...
		if (i == 0)
			ret = ret &&
			      TEST_int_le(EVP_KDF_derive(kctx, out, sizeof(out),
							 NULL),
					  0) &&
			      TEST_true(OSSL_PARAM_set_uint(
				      p - 1, 10 * 1024 * 1024)) &&
			      TEST_true(EVP_KDF_CTX_set_params(kctx, p - 1));
		ret = ret &&
		      TEST_int_gt(EVP_KDF_derive(kctx, out, sizeof(out), NULL),
				  0) &&
		      TEST_mem_eq(out, sizeof(out), expected, sizeof(expected));
		if (i == 0)
			EVP_KDF_CTX_reset(kctx);
...
static int test_kdf_ss_hash(void)
{
	...
	static unsigned char z[] = { 0x6d, 0xbd, 0xc2, 0x3f, 0x04, 0x54, 0x88,
				     0xe4, 0x06, 0x27, 0x57, 0xb0, 0x6b, 0x9e,
				     0xba, 0xe1, 0x83, 0xfc, 0x5a, 0x59, 0x46,
				     0xd8, 0x0d, 0xb9, 0x3f, 0xec, 0x6f, 0x62,
				     0xec, 0x07, 0xe3, 0x72, 0x7f, 0x01, 0x26,
				     0xae, 0xd1, 0x2c, 0xe4, 0xb2, 0x62, 0xf4,
				     0x7d, 0x48, 0xd5, 0x42, 0x87, 0xf8, 0x1d,
				     0x47, 0x4c, 0x7c, 0x3b, 0x18, 0x50, 0xe9 };
	static unsigned char other[] = {
		0xa1, 0xb2, 0xc3, 0xd4, 0xe5, 0x43, 0x41, 0x56, 0x53, 0x69,
		0x64, 0x3c, 0x83, 0x2e, 0x98, 0x49, 0xdc, 0xdb, 0xa7, 0x1e,
		0x9a, 0x31, 0x39, 0xe6, 0x06, 0xe0, 0x95, 0xde, 0x3c, 0x26,
		0x4a, 0x66, 0xe9, 0x8a, 0x16, 0x58, 0x54, 0xcd, 0x07, 0x98,
...
static int test_kdf_x963(void)
{
	...
	static unsigned char z[] = {
		0x00, 0xaa, 0x5b, 0xb7, 0x9b, 0x33, 0xe3, 0x89, 0xfa, 0x58,
		0xce, 0xad, 0xc0, 0x47, 0x19, 0x7f, 0x14, 0xe7, 0x37, 0x12,
		0xf4, 0x52, 0xca, 0xa9, 0xfc, 0x4c, 0x9a, 0xdb, 0x36, 0x93,
		0x48, 0xb8, 0x15, 0x07, 0x39, 0x2f, 0x1a, 0x86, 0xdd, 0xfd,
		0xb7, 0xc4, 0xff, 0x82, 0x31, 0xc4, 0xbd, 0x0f, 0x44, 0xe4,
		0x4a, 0x1b, 0x55, 0xb1, 0x40, 0x47, 0x47, 0xa9, 0xe2, 0xe7,
	...
	static unsigned char shared[] = { 0xe3, 0xb5, 0xb4, 0xc1, 0xb0, 0xd5,
					  ...
					  0x37, 0x89, 0x5d, 0x31 };
	static const unsigned char expected[sizeof(out)] = {
		0x44, 0x63, 0xf8, 0x69, 0xf3, 0xcc, 0x18, 0x76, 0x9b, 0x52,
		0x26, 0x4b, 0x01, 0x12, 0xb5, 0x85, 0x8f, 0x7a, 0xd3, 0x2a,
		0x5a, 0x2d, 0x96, 0xd8, 0xcf, 0xfa, 0xbf, 0x7f, 0xa7, 0x33,
		0x63, 0x3d, 0x6e, 0x4d, 0xd2, 0xa5, 0x99, 0xac, 0xce, 0xb3,
		0xea, 0x54, 0xa6, 0x21, 0x7c, 0xe0, 0xb5, 0x0e, 0xef, 0x4f,
		0x6b, 0x40, 0xa5, 0xc3, 0x02, 0x50, 0xa5, 0xa8, 0xee, 0xee,
		0x20, 0x80, 0x02, 0x26, 0x70, 0x89, 0xdb, 0xf3, 0x51, 0xf3,
		0xf5, 0x02, 0x2a, 0xa9, 0x63, 0x8b, 0xf1, 0xee, 0x41, 0x9d,
		0xea, 0x9c, 0x4f, 0xf7, 0x45, 0xa2, 0x5a, 0xc2, 0x7b, 0xda,
		0x33, 0xca, 0x08, 0xbd, 0x56, 0xdd, 0x1a, 0x59, 0xb4, 0x10,
		0x6c, 0xf2, 0xdb, 0xbc, 0x0a, 0xb2, 0xaa, 0x8e, 0x2e, 0xfa,
		0x7b, 0x17, 0x90, 0x2d, 0x34, 0x27, 0x69, 0x51, 0xce, 0xcc,
...
static int test_kdf_kbkdf_6803_128(void)
{
	...
	static unsigned char outputs[][16] = {
		{ 0xD1, 0x55, 0x77, 0x5A, 0x20, 0x9D, 0x05, 0xF0, 0x2B, 0x38,
		  0xD4, 0x2A, 0x38, 0x9E, 0x5A, 0x56 },
		{ 0x64, 0xDF, 0x83, 0xF8, 0x5A, 0x53, 0x2F, 0x17, 0x57, 0x7D,
		  0x8C, 0x37, 0x03, 0x57, 0x96, 0xAB },
		{ 0x3E, 0x4F, 0xBD, 0xF3, 0x0F, 0xB8, 0x25, 0x9C, 0x42, 0x5C,
		  0xB6, 0xC9, 0x6F, 0x1F, 0x46, 0x35 }
	};
	...
	for (i = 0; i < 3; i++) {
		...
		params[p++] = OSSL_PARAM_construct_octet_string(
			OSSL_KDF_PARAM_SALT, constants[i],
			sizeof(constants[i]));
		params[p++] = OSSL_PARAM_construct_octet_string(
...
static int test_kdf_kbkdf_empty_key(void)
{
	...
	ret = TEST_ptr(kctx) &&
	      TEST_true(EVP_KDF_CTX_set_params(kctx, params)) &&
	      TEST_int_eq(EVP_KDF_derive(kctx, result, sizeof(result), NULL),
			  0);
...
static int test_kdf_kbkdf_8009_prf1(void)
{
	...
	params[i++] = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_DIGEST,
						       digest, 0);
	params[i++] =
		OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_MAC, mac, 0);
...
static int test_kdf_kbkdf_fixedinfo(void)
{
	...
	*p++ = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_CIPHER, cipher,
						0);
	*p++ = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_MAC, mac, 0);
	...
	*p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, input_key,
						 sizeof(input_key));
	*p++ = OSSL_PARAM_construct_octet_string(
		OSSL_KDF_PARAM_INFO, fixed_input, sizeof(fixed_input));
...
static int test_kdf_kbkdf_kmac(void)
{
	...
	static unsigned char input_key[] = { 0xDD, 0x81, 0xEF, 0xC8, 0x2C, 0xDD,
					     0xEC, 0x51, 0xC4, 0x09, 0xBD, 0x8C,
					     0xCB, 0xAF, 0x94, 0xF6, 0x5F, 0xFA,
					     0x7B, 0x92, 0xF1, 0x11, 0xF9, 0x40,
					     0x2B, 0x0D, 0x6A, 0xE0, 0x5E, 0x44,
					     0x92, 0x34, 0xF0, 0x3B, 0xBA, 0xF5,
					     0x4F, 0xEF, 0x19, 0x45, 0xDA };
	static unsigned char context[] = {
		
	};
	static unsigned char label[] = {
		0xA5, 0xDE, 0x2A, 0x0A, 0xF0, 0xDA, 0x59, 0x04, 0xCC,
		0xFF, 0x50, 0xD3, 0xA5, 0xD2, 0xDE, 0xA3, 0x33, 0xC0,
		0x27, 0xED, 0xDC, 0x6A, 0x54, 0x54, 0x95, 0x78, 0x74,
		0x0D, 0xE7, 0xB7, 0x92, 0xD6, 0x64, 0xD5, 0xFB, 0x1F,
		0x0F, 0x87, 0xFD, 0x65, 0x79, 0x8B, 0x81, 0x83, 0x95,
		0x40, 0x7A, 0x19, 0x8D, 0xCA, 0xE0, 0x4A, 0x93, 0xA8
	};
	static unsigned char output[] = {
		,
...
static int test_kdf_ss_kmac(void)
{
	...
	static unsigned char salt[] = { 0x36, 0x38, 0x27, 0x1c, 0xcd, 0x68,
					...
					0x39, 0xef, 0x3f, 0x89 };
	static const unsigned char expected[sizeof(out)] = {
		0xe9, 0xc1, 0x84, 0x53, 0xa0, 0x62, 0xb5, 0x3b, 0xdb, 0xfc,
		0xbb, 0x5a, 0x34, 0xbd, 0xb8, 0xe5, 0xe7, 0x07, 0xee, 0xbb,
		0x5d, 0xd1, 0x34, 0x42, 0x43, 0xd8, 0xcf, 0xc2, 0xc2, 0xe6,
		0x33, 0x2f, 0x91, 0xbd, 0xa5, 0x86, 0xf3, 0x7d, 0xe4, 0x8a,
		0x65, 0xd4, 0xc5, 0x14, 0xfd, 0xef, 0xaa, 0x1e, 0x67, 0x54,
		0xf3, 0x73, 0xd2, 0x38, 0xe1, 0x95, 0xae, 0x15, 0x7e, 0x1d,
		0xe8, 0x14, 0x98, 0x03
	};

	*p++ = OSSL_PARAM_construct_utf8_string(
		OSSL_KDF_PARAM_MAC, (char *)OSSL_MAC_NAME_KMAC128, 0);
...
static int test_kdf_sshkdf(void)
{
	...
	static unsigned char key[] = {
		
	};
	static unsigned char xcghash[] = { 0x0e, 0x68, 0x3f, 0xc8, 0xa9, 0xed,
					   0x7c, 0x2f, 0xf0, 0x2d, 0xef, 0x23,
					   0xb2, 0x74, 0x5e, 0xbc, 0x99, 0xb2,
					   0x67, 0xda, 0xa8, 0x6a, 0x4a, 0xa7,
					   0x69, 0x72, 0x39, 0x08, 0x82, 0x53,
					   0xf6, 0x42 };
	static unsigned char sessid[] = { 0x0e, 0x68, 0x3f, 0xc8, 0xa9, 0xed,
					  0x7c, 0x2f, 0xf0, 0x2d, 0xef, 0x23,
					  0xb2, 0x74, 0x5e, 0xbc, 0x99, 0xb2,
					  0x67, 0xda, 0xa8, 0x6a, 0x4a, 0xa7,
					  0x69, 0x72, 0x39, 0x08, 0x82, 0x53,
					  0xf6, 0x42 };
	static const unsigned char expected[sizeof(out)] = { 0x41, 0xff, 0x2e,
							     0xad, 0x16, 0x83,
							     0xf1, 0xe6 };
	...
	*p++ = OSSL_PARAM_construct_octet_string(
		OSSL_KDF_PARAM_SSHKDF_SESSION_ID, sessid, sizeof(sessid));
...
static int test_kdf_get_kdf(void)
{
	...
	if (!TEST_ptr(kdf2 = EVP_KDF_fetch(NULL, OBJ_nid2sn(NID_tls1_prf),
					   NULL)) ||
	    !test_kdfs_same(kdf1, kdf2))
		ok = 0;
	EVP_KDF_free(kdf1);
...
static int test_kdf_hmac_drbg_settables(void)
{
	...
	for (i = 0; settableparams[i].key != NULL; ++i) {
		...
		if (OPENSSL_strcasecmp(settableparams[i].key,
			...
			TEST_note("Testing set int into %s fails",
				  settableparams[i].key);
			params[0] = OSSL_PARAM_construct_int(
				settableparams[i].key, &j);
			if (!TEST_int_le(EVP_KDF_CTX_set_params(kctx, params),
					 0))
				goto err;
		}
	}
	...
	params[1] = OSSL_PARAM_construct_octet_string(
		OSSL_KDF_PARAM_HMACDRBG_NONCE, (char *)ent, sizeof(ent));
	params[2] = OSSL_PARAM_construct_utf8_string(OSSL_ALG_PARAM_DIGEST,
						     "SHA256", 0);
...
static int test_kdf_hmac_drbg_gettables(void)
{
	...
	for (i = 0; gettableparams[i].key != NULL; ++i) {
		params[0] = OSSL_PARAM_construct_utf8_string(
			gettableparams[i].key, buf, sizeof(buf));
		if (!TEST_int_le(EVP_KDF_CTX_get_params(kctx, params), 0))
			goto err;
	}
	ret = 1;
...
static int test_kbkdf_mac_change(void)
{
	...
	static /* const */ unsigned char info[] = {
		0x98, 0xe9, 0x98, 0x8b, 0xb4, 0xcc, 0x8b, 0x34, 0xd7, 0x92,
		0x2e, 0x1c, 0x68, 0xad, 0x69, 0x2b, 0xa2, 0xa1, 0xd9, 0xae,
		0x15, 0x14, 0x95, 0x71, 0x67, 0x5f, 0x17, 0xa7, 0x7a, 0xd4,
		0x9e, 0x80, 0xc8, 0xd2, 0xa8, 0x5e, 0x83, 0x1a, 0x26, 0x44,
		0x5b, 0x1f, 0x0f, 0xf4, 0x4d, 0x70, 0x84, 0xa1, 0x72, 0x06,
		0xb4, 0x89, 0x6c, 0x81, 0x12, 0xda, 0xad, 0x18, 0x60, 0x5a
	};
	static const unsigned char output[] = { 0x6c, 0x03, 0x76, 0x52,
						0x99, 0x06, 0x74, 0xa0,
						0x78, 0x44, 0x73, 0x2d,
						0x0a, 0xd9, 0x85, 0xf9 };
