...
ASN1_SEQUENCE(
	ASN1_LONG_DATA) = { ASN1_SIMPLE(ASN1_LONG_DATA, success, ASN1_BOOLEAN),
			    ...
			    ASN1_EXP_OPT(
				    ASN1_LONG_DATA, test_zlong, ZLONG,
...
ASN1_SEQUENCE(
	ASN1_INT32_DATA) = { ASN1_SIMPLE(ASN1_INT32_DATA, success,
					 ASN1_BOOLEAN),
			     ASN1_EMBED(ASN1_INT32_DATA, test_int32, INT32),
			     ASN1_EXP_OPT_EMBED(
				     ASN1_INT32_DATA, test_zint32, ZINT32,
...
ASN1_SEQUENCE(
	ASN1_UINT32_DATA) = { ASN1_SIMPLE(ASN1_UINT32_DATA, success,
					  ASN1_BOOLEAN),
			      ASN1_EMBED(ASN1_UINT32_DATA, test_uint32, UINT32),
			      ASN1_EXP_OPT_EMBED(
				      ASN1_UINT32_DATA, test_zuint32, ZUINT32,
...
ASN1_SEQUENCE(
	ASN1_INT64_DATA) = { ASN1_SIMPLE(ASN1_INT64_DATA, success,
					 ASN1_BOOLEAN),
...
static ASN1_INT64_DATA int64_expected[] = {
	...
	CUSTOM_EXPECTED_FAILURE, /* t_8bytes_5_negpad (illegal padding) */
	CUSTOM_EXPECTED_SUCCESS(0x1ffffffffULL,
...
ASN1_SEQUENCE(
	ASN1_UINT64_DATA) = { ASN1_SIMPLE(ASN1_UINT64_DATA, success,
					  ASN1_BOOLEAN),
...
static ASN1_UINT64_DATA uint64_expected[] = {
	...
	CUSTOM_EXPECTED_FAILURE, /* t_minus_256 (illegal negative value) */
	CUSTOM_EXPECTED_FAILURE, /* t_9bytes_1 */
	CUSTOM_EXPECTED_SUCCESS((uint64_t)INT64_MAX + 1,
			      ...
			      unsigned char **encoding, int explicit_default)
{
	...
	if (explicit_default || i > 0) {
		...
		secondbytesinner =
			...
			custom_data->nbytes2;
		secondbytes = 1 + der_encode_length(secondbytesinner, NULL) +
			      secondbytesinner;
	}
     ...
     */
	seqbytes = 1 +
		   der_encode_length(sizeof(t_true) + firstbytes + secondbytes,
				     NULL) +
			    ...
			    const TEST_PACKAGE *package)
{
	...
	ret = do_decode(encoding, (long)encoding_length, expected,
...
static int test_intern(const TEST_PACKAGE *package)
{
	...
	for (i = 0; i < nelems; i++) {
		...
		EXPECTED *expected = (EXPECTED *)&(
			(unsigned char *)package->encode_expectations)[pos];

		switch (do_encode_custom(expected, &test_custom_data[i],
					 package)) {
		case -1:
			if (expected->success) {
				TEST_error(
	...
	for (i = 0; i < nelems; i++) {
		...
		switch (do_enc_dec(expected,
				   (long)package->encdec_data_elem_size,
				   package)) {
		case -1:
			if (expected->success) {
				TEST_error(
