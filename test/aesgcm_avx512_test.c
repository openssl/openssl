 ...
 * into one CALC_AAD_HASH call: 511/512/513 exercise the 16-block section
 ...
 * 290 halves into 145-byte sub-chunks, so a single call carries 9 blocks: the
 * smallest count whose sub-16-block tail needs H^9..H^16 completed in the
 * context. 240 and 255 halve to at most 8 blocks per call, covering the other
 * side of that boundary.
 */
static const size_t aadlens[] = {
    0, 13, 16, 20, 240, 255, 256, 257, 290, 511, 512, 513, 1024, 1025, 2048
};
...
/*
 * Number of independent seeded data vectors exercised per grid cell. Two
 * covers both the copy and the in-place variant of every cell, and keeps one
 * dump near 100 MB of AAD plus payload.
 */
#define DIFF_REPS 2
...
int setup_tests(void)
{
    ...
    /*
     * Capability probe: report the backend libcrypto would pick and return
     * without running anything. The recipe asks for this before a dump, so a
     * host with no VAES/AVX-512 path to compare against never walks the grid.
     */
    if (mode != NULL && strcmp(mode, "probe") == 0) {
        printf("VAESCAP: %d\n", vaes_avx512_available() ? 1 : 0);
        fflush(stdout);
        return 1;
    }
