...
static int async_write(BIO *bio, const char *in, int inl)
{
	...
	if (ctrs->wctr > 0) {
		...
		if (fragment) {
			...
			if (!PACKET_buf_init(&pkt, (const unsigned char *)in,
					     inl))
				return -1;

			while (PACKET_remaining(&pkt) > 0) {
				PACKET payload, wholebody, sessionid,
					extensions;
				unsigned int contenttype, versionhi, versionlo,
					data;
...
static int test_asyncio(int test)
{
	...
	if (!TEST_true(create_ssl_ctx_pair(
		    NULL, TLS_server_method(), TLS_client_method(),
		    TLS1_VERSION, 0, &serverctx, &clientctx, cert, privkey)))
		goto end;
	...
	if (!TEST_ptr(s_to_c_fbio) || !TEST_ptr(c_to_s_fbio)) {
	...
	for (j = 0; j < 2; j++) {
         ...
         */
		for (ret = -1, i = 0, len = 0;
		     len != sizeof(testdata) && i < MAX_ATTEMPTS; i++) {
