...
static uint8_t ml_kem_expected_rho[3][ML_KEM_RANDOM_BYTES] = {
	{ 0x7e, 0xfb, 0x9e, 0x40, 0xc3, 0xbf, 0x0f, 0xf0, 0x43, 0x29, 0x86,
	  0xae, 0x4b, 0xc1, 0xa2, 0x42, 0xce, 0x99, 0x21, 0xaa, 0x9e, 0x22,
	  0x44, 0x88, 0x19, 0x58, 0x5d, 0xea, 0x30, 0x8e, 0xb0, 0x39 },
	{ 0x16, 0x2e, 0xc0, 0x98, 0xa9, 0x00, 0xb1, 0x2d, 0xd8, 0xfa, 0xbb,
	  0xfb, 0x3f, 0xe8, 0xcb, 0x1d, 0xc4, 0xe8, 0x31, 0x5f, 0x2a, 0xf0,
	  0xd3, 0x2f, 0x00, 0x17, 0xae, 0x13, 0x6e, 0x19, 0xf0, 0x28 },
	{ 0x29, 0xb4, 0xf9, 0xf8, 0xcf, 0xba, 0xdf, 0x2e, 0x41, 0x86, 0x9a,
	  0xbf, 0xba, 0xd1, 0x07, 0x38, 0xad, 0x04, 0xcc, 0x75, 0x2b, 0xc2,
	  0x0c, 0x39, 0x47, 0x46, 0x85, 0x0e, 0x0c, 0x48, 0x47, 0xdb }
};
static uint8_t ml_kem_expected_ctext_sha256[3][32] = {
	{ 0xbc, 0x29, 0xd7, 0xdf, 0x8b, 0xc5, 0x46, 0x5d, 0x98, 0x06, 0x01,
	  0xd8, 0x00, 0x25, 0x97, 0x93, 0xe2, 0x60, 0x38, 0x25, 0xa5, 0x72,
	  0xda, 0x6c, 0xd1, 0x98, 0xa5, 0x12, 0xcc, 0x6d, 0x1a, 0x34 },
	{ 0x36, 0x82, 0x9a, 0x2f, 0x35, 0xcb, 0xf4, 0xde, 0xb6, 0x2c, 0x0a,
	  0x12, 0xa1, 0x5c, 0x22, 0xda, 0xe9, 0xf8, 0xd2, 0xc2, 0x52, 0x56,
	  0x6f, 0xc2, 0x4f, 0x88, 0xab, 0xe8, 0x05, 0xcb, 0x57, 0x5e },
	{ 0x50, 0x81, 0x36, 0xa1, 0x3f, 0x8a, 0x79, 0x20, 0xe3, 0x43, 0x44,
	  0x98, 0xc6, 0x97, 0x5c, 0xbb, 0xab, 0x45, 0x7d, 0x80, 0x93, 0x09,
	  0xeb, 0x2f, 0x92, 0x45, 0x3e, 0x74, 0x09, 0x73, 0x82, 0x10 }
};
static uint8_t ml_kem_expected_shared_secret[3][32] = {
	{ 0x31, 0x98, 0x39, 0xe8, 0x2a, 0xb6, 0xb2, 0x22, 0xde, 0x7b, 0x61,
	  0x9e, 0x80, 0xda, 0x83, 0x91, 0x52, 0x2b, 0xbb, 0x37, 0x67, 0x70,
	  0x18, 0x49, 0x4a, 0x47, 0x42, 0xc5, 0x3f, 0x9a, 0xbf, 0xdf },
	...
	{ 0x48, 0x9d, 0xd1, 0xe9, 0xc2, 0xbe, 0x4a, 0xf3, 0x48, 0x2b, 0xdb,
	  0x35, 0xbb, 0x26, 0xce, 0x76, 0x0e, 0x6e, 0x41, 0x4d, 0xa6, 0xec,
	  0xbe, 0x48, 0x99, 0x85, 0x74, 0x8a, 0x82, 0x5f, 0x1c, 0xd6 },
};
...
static int sanity_test(void)
{
	...
	for (i = 0; i < (int)OSSL_NELEM(alg); ++i) {
		...
		params[0] = OSSL_PARAM_construct_octet_string(
			OSSL_RAND_PARAM_TEST_ENTROPY, ml_kem_private_entropy,
		...
		if (!TEST_mem_eq(encoded_public_key + v->vector_bytes,
				 ML_KEM_RANDOM_BYTES, ml_kem_expected_rho[i],
				 ML_KEM_RANDOM_BYTES))
			goto done;

		ret2 = -4;
		/* Create the expected associated public key */
		if (!ossl_ml_kem_parse_public_key(encoded_public_key,
						  v->pubkey_bytes, public_key))
			goto done;

		/* Configure the public RNG to output the encap and decap seeds */
		params[0] = OSSL_PARAM_construct_octet_string(
			OSSL_RAND_PARAM_TEST_ENTROPY, ml_kem_public_entropy,
		...
		if (!ossl_ml_kem_encap_rand(ciphertext, v->ctext_bytes,
					    shared_secret,
					    sizeof(shared_secret), public_key))
			goto done;

		ret2 = -6;
		/* Check the ciphertext hash */
		if (!TEST_true(EVP_Digest(ciphertext, v->ctext_bytes, hash,
					  NULL, sha256, NULL)) ||
		...
		if (!TEST_true(ossl_ml_kem_decap(
			    shared_secret2, sizeof(shared_secret2), ciphertext,
			    v->ctext_bytes, private_key)))
			goto done;
         ...
         */
		if (!TEST_false(ossl_ml_kem_decap(
			    shared_secret2, sizeof(shared_secret2), ciphertext,
			    v->ctext_bytes - 1, private_key)))
			goto done;
...
{
	if (!TEST_true(RAND_set_DRBG_type(NULL, "TEST-RAND", "fips=no", NULL,
					  NULL)))
		return 0;
