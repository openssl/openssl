...
static int check_rejected(struct helper *h, struct helper_local *hl)
{
	...
	if (!ossl_quic_tserver_stream_has_peer_stop_sending(ACQUIRE_S(),
							    stream_id, NULL) ||
...
static int check_stream_reset(struct helper *h, struct helper_local *hl)
{
	...
	if (!ossl_quic_tserver_stream_has_peer_reset_stream(ACQUIRE_S(),
							    stream_id, &aec)) {
		h->check_spin_again = 1;
		return 0;
	}
...
{
	int *p_checked_out =
		s_checked_out_p(h, hl == NULL ? -1 : hl->thread_idx);
...
static unsigned int server_helper_thread(void *arg)
{
	...
	for (;;) {
		...
		if (!ready) {
			ossl_crypto_condvar_wait(h->server_thread.c,
						 h->server_thread.m);
			continue;
		}
		       ...
		       int free_order, int blocking, int need_injector)
{
	...
	if (!TEST_ptr(h->s_net_bio = h->s_net_bio_own =
			      BIO_new_dgram(h->s_fd, 0)))
		goto err;
	...
	if (!TEST_ptr(h->s_priv = ossl_quic_tserver_new(&s_args, certfile,
							keyfile)))
		goto err;
	...
	if (!TEST_ptr(h->c_net_bio = h->c_net_bio_own =
	...
	if (h->blocking) {
		...
		h->server_thread.t = ossl_crypto_thread_native_start(
			server_helper_thread, h, 1);
			     ...
			     int thread_idx)
{
	...
	if (thread_idx < 0) {
		...
		if (!TEST_ptr(hl->c_streams = lh_STREAM_INFO_new(
				      stream_info_hash, stream_info_cmp)))
			return 0;
	}
...
#endif
	OSSL_TIME op_start_time = ossl_time_zero(),
		  op_deadline = ossl_time_zero();
...
#define C_SPIN_AGAIN()                                             \
	...
	for (;;) {
		...
		if (no_advance) {
			...
			op_deadline = ossl_time_add(op_start_time,
						    ossl_ms2time(60000));
		}

		if (!TEST_int_le(ossl_time_compare(ossl_time_now(),
						   op_deadline),
				 0)) {
			TEST_error("op %zu timed out on thread %d", op_idx + 1,
				   thread_idx);
			goto out;
		}
		...
		if (op->stream_name != NULL) {
			...
			if (thread_idx < 0)
				s_stream_id =
					helper_get_s_stream(h, op->stream_name);
			else
		...
		if (thread_idx >= 0) {
			...
			default:
				TEST_error(
					"opcode %lu not allowed on child thread",
					(unsigned long)op->op);
				goto out;
			}
		}
		...
		case OPK_END:
			...
			TEST_info("script \"%s\" finished on thread %d",
				  script_name, thread_idx);
			testresult = 1;
			goto out;

		case OPK_BEGIN_REPEAT:
			if (!TEST_size_t_lt(repeat_stack_len,
		...
		case OPK_C_SET_ALPN: {
			...
			if (!TEST_size_t_le(alpn_len, UINT8_MAX) ||
			    !TEST_ptr(tmp_buf = (unsigned char *)OPENSSL_malloc(
					      alpn_len + 1)))
				goto out;
			...
			if (!TEST_false(SSL_set_alpn_protos(
				    h->c_conn, tmp_buf,
		...
		case OPK_C_WRITE: {
			...
			r = SSL_write_ex(c_tgt, op->arg0, op->arg1,
					 &bytes_written);
		...
		case OPK_C_WAIT_FOR_DATA: {
			...
			if (!SSL_peek_ex(c_tgt, buf, sizeof(buf),
					 &bytes_read) ||
			    bytes_read == 0)
				C_SPIN_AGAIN();
		} break;
		...
		case OPK_C_READ_EXPECT: {
			...
			r = SSL_read_ex(c_tgt, tmp_buf + offset,
		...
		case OPK_C_EXPECT_FIN: {
			...
			if (!check_consistent_want(c_tgt, r) ||
			    !TEST_false(r) || !TEST_size_t_eq(bytes_read, 0))
				goto out;
		...
		case OPK_S_EXPECT_FIN: {
			...
			if (!ossl_quic_tserver_has_read_ended(ACQUIRE_S(),
							      s_stream_id))
				S_SPIN_AGAIN();
		} break;
		...
		case OPK_C_DETACH: {
			...
			if (!TEST_true(helper_local_set_c_stream(
				    hl, op->stream_name, c_stream)))
				goto out;
		} break;
		...
		case OPK_C_ATTACH: {
			...
			if (!TEST_true(
				    ossl_quic_attach_stream(h->c_conn, c_tgt)))
				goto out;

			if (!TEST_true(helper_local_set_c_stream(
				    hl, op->stream_name, NULL)))
				goto out;
		} break;
		...
		case OPK_C_ACCEPT_STREAM_WAIT: {
			...
			if ((c_stream = SSL_accept_stream(h->c_conn, 0)) ==
			    NULL)
				C_SPIN_AGAIN();
		...
		case OPK_S_ACCEPT_STREAM_WAIT: {
			...
			new_stream_id = ossl_quic_tserver_pop_incoming_stream(
				ACQUIRE_S());
		...
		case OPK_C_SET_DEFAULT_STREAM_MODE: {
			...
			if (!TEST_true(SSL_set_default_stream_mode(
				    c_tgt, (uint32_t)op->arg1)))
				goto out;
		} break;
		...
		case OPK_C_SHUTDOWN: {
			int ret;
			QUIC_CHANNEL *ch =
				ossl_quic_conn_get_channel(h->c_conn);
			SSL_SHUTDOWN_EX_ARGS args = { 0 };

			ossl_quic_engine_set_inhibit_tick(
				ossl_quic_channel_get0_engine(ch), 0);
			...
			ret = SSL_shutdown_ex(c_tgt, op->arg1, &args,
					      sizeof(args));
		...
		case OPK_C_EXPECT_CONN_CLOSE_INFO: {
			...
			int expect_app = (op->arg1 & EXPECT_CONN_CLOSE_APP) !=
					 0;
			int expect_remote =
				(op->arg1 & EXPECT_CONN_CLOSE_REMOTE) != 0;
			...
			if (h->blocking &&
			    !TEST_true(SSL_shutdown_ex(
				    c_tgt, SSL_SHUTDOWN_FLAG_WAIT_PEER, NULL,
				    0)))
				goto out;

			if (!SSL_get_conn_close_info(c_tgt, &cc_info,
						     sizeof(cc_info)))
				C_SPIN_AGAIN();
			...
			if (!TEST_int_eq(expect_app,
				...
				TEST_info("Connection close reason: %s",
					  cc_info.reason);
				goto out;
			}
		} break;
		...
		case OPK_S_EXPECT_CONN_CLOSE_INFO: {
			...
			if (!TEST_ptr(
				    tc = ossl_quic_tserver_get_terminate_cause(
					    ACQUIRE_S())))
				goto out;
		...
		case OPK_C_STREAM_RESET_FAIL: {
			...
			if (op->op == OPK_C_STREAM_RESET) {
				if (!TEST_true(SSL_stream_reset(c_tgt, &args,
								sizeof(args))))
					goto out;
			} else {
				if (!TEST_false(SSL_stream_reset(c_tgt, &args,
								 sizeof(args))))
					goto out;
			}
		} break;
		...
		case OPK_NEW_THREAD: {
			...
			if (!TEST_ptr_null(h->threads)) {
				TEST_error(
					"max one NEW_THREAD operation per script");
				goto out;
			}

			h->threads = OPENSSL_calloc(
				op->arg1, sizeof(struct child_thread_args));
		...
		case OPK_C_EXPECT_SSL_ERR: {
			if (!TEST_size_t_eq((size_t)SSL_get_error(c_tgt, 0),
					    op->arg1))
				goto out;
			if (!TEST_int_eq(SSL_want(c_tgt), SSL_NOTHING))
				goto out;
		...
		case OPK_EXPECT_ERR_REASON: {
			if (!TEST_size_t_eq((size_t)ERR_GET_REASON(
						    ERR_peek_last_error()),
					    op->arg1))
				goto out;
		} break;
		...
		case OPK_EXPECT_ERR_LIB: {
			if (!TEST_size_t_eq(
				    (size_t)ERR_GET_LIB(ERR_peek_last_error()),
				    op->arg1))
				goto out;
		} break;
		...
		case OPK_C_INHIBIT_TICK: {
			...
			ossl_quic_engine_set_inhibit_tick(
				ossl_quic_channel_get0_engine(ch),
				(int)op->arg1);
		} break;
		...
		case OPK_C_SET_WRITE_BUF_SIZE:
			...
			if (!TEST_true(ossl_quic_set_write_buffer_size(
				    c_tgt, op->arg1)))
				goto out;
		...
		case OPK_S_NEW_TICKET:
			if (!TEST_true(
				    ossl_quic_tserver_new_ticket(ACQUIRE_S())))
				goto out;
			break;
...
out:
	...
	if (!testresult) {
		...
		if (h->c_conn != NULL) {
			...
			if (SSL_get_conn_close_info(h->c_conn, &cc_info,
						    sizeof(cc_info))) {
...
static const struct script_op script_2[] = {
	OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_INCOMING_STREAM_POLICY(SSL_INCOMING_STREAM_POLICY_ACCEPT) OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(
		a,
		C_BIDI_ID(
			0)) OP_S_READ_EXPECT(a, "apple", 5) OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(DEFAULT, "orange", 6)

		OP_C_NEW_STREAM_BIDI(b, C_BIDI_ID(1)) OP_C_WRITE(b, "flamingo", 8) OP_C_CONCLUDE(b) OP_S_BIND_STREAM_ID(
			b, C_BIDI_ID(1)) OP_S_READ_EXPECT(b, "flamingo",
							  8) OP_S_EXPECT_FIN(b)
			OP_S_WRITE(b, "gargoyle", 8) OP_S_CONCLUDE(b) OP_C_READ_EXPECT(
				b, "gargoyle",
				8) OP_C_EXPECT_FIN(b)

				OP_C_NEW_STREAM_UNI(c, C_UNI_ID(0)) OP_C_WRITE(c, "elephant", 8) OP_C_CONCLUDE(c) OP_S_BIND_STREAM_ID(
					c,
					C_UNI_ID(0)) OP_S_READ_EXPECT(c,
								      "elephant",
								      8) OP_S_EXPECT_FIN(c) OP_S_WRITE_FAIL(c)

					OP_C_ACCEPT_STREAM_NONE()

						OP_S_NEW_STREAM_BIDI(d, S_BIDI_ID(0)) OP_S_WRITE(
							d, "frog",
							4) OP_S_CONCLUDE(d)
							...
							OP_C_ACCEPT_STREAM_WAIT(d) OP_C_ACCEPT_STREAM_NONE() OP_C_READ_EXPECT(
								...
								OP_S_NEW_STREAM_BIDI(e, S_BIDI_ID(1)) OP_S_WRITE(
									...
									OP_C_ACCEPT_STREAM_WAIT(e) OP_C_READ_EXPECT(e, "mixture", 7) OP_C_EXPECT_FIN(e) OP_C_WRITE(
										e,
										"ramble",
										6) OP_S_READ_EXPECT(e,
												    "ramble",
												    6) OP_C_CONCLUDE(e) OP_S_EXPECT_FIN(e)

										OP_S_NEW_STREAM_UNI(f, S_UNI_ID(0)) OP_S_WRITE(
											f,
											"yonder",
											6) OP_S_CONCLUDE(f)
											...
											OP_C_ACCEPT_STREAM_WAIT(f) OP_C_ACCEPT_STREAM_NONE() OP_C_READ_EXPECT(
												...
												OP_C_SET_INCOMING_STREAM_POLICY(SSL_INCOMING_STREAM_POLICY_REJECT) OP_S_NEW_STREAM_BIDI(
													g,
													S_BIDI_ID(
														2)) OP_S_WRITE(g,
															       "unseen",
													...
													OP_C_ACCEPT_STREAM_NONE()
														...
														OP_C_SET_INCOMING_STREAM_POLICY(SSL_INCOMING_STREAM_POLICY_AUTO) OP_S_NEW_STREAM_BIDI(
															h,
															S_BIDI_ID(
																3))
															OP_S_WRITE(
	...
	OP_C_WRITE_FAIL(DEFAULT)
		...
		OP_C_WRITE(a, "by", 2)
			...
			OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(
				...
				OP_S_WRITE(a, "hello",
					   5) OP_C_READ_EXPECT(a, "hello", 5)

					OP_C_WRITE_FAIL(DEFAULT) OP_C_ATTACH(
						a) OP_C_WRITE(DEFAULT,
							      "is here", 7)
						OP_S_READ_EXPECT(a, "is here",
								 7)
...
static const struct script_op script_4[] = {
	OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
		...
		OP_C_SET_DEFAULT_STREAM_MODE(
			SSL_DEFAULT_STREAM_MODE_NONE) OP_C_WRITE_FAIL(DEFAULT)

			OP_S_NEW_STREAM_BIDI(a, S_BIDI_ID(0)) OP_S_WRITE(
				a, "apple", 5)

				OP_C_READ_FAIL(DEFAULT)

					OP_C_ACCEPT_STREAM_WAIT(
						a) OP_C_READ_EXPECT(a, "apple",
								    5)

						OP_C_ATTACH(a) OP_C_WRITE(
							DEFAULT, "orange", 6)
							OP_S_READ_EXPECT(
								a, "orange", 6)
	...
	OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
		...
		OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
			OP_C_NEW_STREAM_BIDI(
				a,
				C_BIDI_ID(0)) OP_C_NEW_STREAM_BIDI(b,
								   C_BIDI_ID(1))

				OP_C_WRITE(a, "apple", 5) OP_C_STREAM_RESET(a,
									    42)
	...
	OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
		...
		OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_AUTO_UNI)
			OP_S_NEW_STREAM_BIDI(a, S_BIDI_ID(0))
				OP_S_WRITE(a, "apple", 5) OP_C_READ_EXPECT(
					DEFAULT, "apple", 5)
...
static const struct script_op script_11[] = {
	OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
		...
		OP_NEW_THREAD(5, script_11_child)
			...
			OP_S_NEW_STREAM_BIDI(a, ANY_ID) OP_S_WRITE(
				a, "foo", 3) OP_S_CONCLUDE(a)

				OP_S_NEW_STREAM_BIDI(b, ANY_ID) OP_S_WRITE(
					b, "foo", 3) OP_S_CONCLUDE(b)

					OP_S_NEW_STREAM_BIDI(c, ANY_ID) OP_S_WRITE(
						c, "foo", 3) OP_S_CONCLUDE(c)

						OP_S_NEW_STREAM_BIDI(d, ANY_ID) OP_S_WRITE(
							d, "foo",
							3) OP_S_CONCLUDE(d)
...
static const struct script_op script_12[] = {
	OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
		...
		OP_NEW_THREAD(5, script_12_child)
			...
			OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(
				a, "foo",
				3) OP_S_EXPECT_FIN(a) OP_S_BIND_STREAM_ID(b,
									  C_BIDI_ID(
										  1))
				OP_S_READ_EXPECT(b, "foo", 3) OP_S_EXPECT_FIN(
					b) OP_S_BIND_STREAM_ID(c, C_BIDI_ID(2))
					OP_S_READ_EXPECT(c, "foo", 3) OP_S_EXPECT_FIN(
						c) OP_S_BIND_STREAM_ID(d,
								       C_BIDI_ID(
									       3))
						OP_S_READ_EXPECT(
							d, "foo",
		...
		OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
			...
			OP_NEW_THREAD(5, script_14_child)
				...
				OP_BEGIN_REPEAT(50)
					...
					OP_S_ACCEPT_STREAM_WAIT(
						a) OP_S_READ_EXPECT(a, "foo", 3)
						OP_S_EXPECT_FIN(a)
							OP_S_UNBIND_STREAM_ID(a)
...
static const struct script_op script_15[] = {
	...
	OP_BEGIN_REPEAT(200)
		...
		OP_C_NEW_STREAM_BIDI_EX(a, ANY_ID, SSL_STREAM_FLAG_ADVANCE)
			OP_C_WRITE(a, "foo", 3) OP_C_CONCLUDE(a)
				OP_C_FREE_STREAM(a)
			...
			OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(
				...
				OP_CHECK(override_key_update, 1)
					...
					OP_BEGIN_REPEAT(200)
						...
						OP_C_WRITE(
							DEFAULT, "apple",
							5) OP_S_READ_EXPECT(a,
									    "apple",
									    5)
							OP_CHECK(skip_time_ms,
								 8)
	...
	OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
		...
		OP_C_WRITE(DEFAULT, "apple", 5)
			...
			OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(
				...
				OP_C_WRITE(DEFAULT, "orange",
					   6) OP_S_READ_EXPECT(a, "orange", 6)

					OP_S_WRITE(
						a, "strawberry",
						10) OP_C_READ_EXPECT(DEFAULT,
								     "strawberry",
								     10)

						OP_CHECK(
							check_key_update_lt,
							1) OP_CHECK(trigger_key_update,
								    0)

							OP_C_WRITE(DEFAULT, "orange", 6) OP_S_READ_EXPECT(
								a, "orange",
								6) OP_S_WRITE(a,
									      "ok",
									      2)
...
static const struct script_op script_20_child[] = {
	OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "foo", 3)
		...
		OP_CHECK(script_20_trigger1, 0) OP_CHECK(script_20_wait2, 1)
			...
			OP_C_READ_FAIL_WAIT(
				a) OP_C_EXPECT_SSL_ERR(a, SSL_ERROR_SYSCALL)

				OP_EXPECT_ERR_LIB(ERR_LIB_SSL) OP_EXPECT_ERR_REASON(
					SSL_R_PROTOCOL_IS_SHUTDOWN)

					OP_POP_ERR() OP_EXPECT_ERR_LIB(
						ERR_LIB_SSL)
						OP_EXPECT_ERR_REASON(
							SSL_R_QUIC_NETWORK_ERROR)
	...
	OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
		...
		OP_NEW_THREAD(5, script_20_child)
			...
			OP_BEGIN_REPEAT(5)
				...
				OP_S_NEW_STREAM_BIDI(a, ANY_ID) OP_S_WRITE(
					a, "foo", 3) OP_S_UNBIND_STREAM_ID(a)

					OP_END_REPEAT()

						OP_CHECK(script_20_wait1, 5)

							OP_C_CLOSE_SOCKET() OP_CHECK(
								script_20_trigger2,
								0)
...
static const struct script_op script_21[] = {
	OP_S_SET_INJECT_PLAIN(script_21_inject_plain) OP_C_SET_ALPN(
...
static const struct script_op script_22[] = {
	OP_S_SET_INJECT_PLAIN(script_22_inject_plain) OP_C_SET_ALPN(
		"ossltest") OP_C_CONNECT_WAIT()
				  ...
				  unsigned char *buf, size_t len)
{
	...
	if (!TEST_true(WPACKET_quic_write_vlint(
		    &wpkt, OSSL_QUIC_FRAME_TYPE_NEW_TOKEN)) ||
...
static const struct script_op script_23[] = {
	OP_S_SET_INJECT_PLAIN(script_23_inject_plain) OP_C_SET_ALPN(
		"ossltest") OP_C_CONNECT_WAIT()
...
static const struct script_op script_24[] = {
	OP_S_SET_INJECT_PLAIN(script_24_inject_plain) OP_C_SET_ALPN(
		...
		OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(
			...
			OP_SET_INJECT_WORD(
				1, OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_BIDI)
...
static const struct script_op script_26[] = {
	OP_S_SET_INJECT_PLAIN(script_24_inject_plain) OP_C_SET_ALPN(
		...
		OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(
			...
			OP_SET_INJECT_WORD(
				1, OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_BIDI)
...
static const struct script_op script_27[] = {
	OP_S_SET_INJECT_PLAIN(script_24_inject_plain) OP_C_SET_ALPN(
		...
		OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(
			...
			OP_SET_INJECT_WORD(
				1, OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_UNI)
...
static const struct script_op script_28[] = {
	OP_S_SET_INJECT_PLAIN(script_28_inject_plain) OP_C_SET_ALPN(
		"ossltest") OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
		...
		OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "orange", 6)
			...
			OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(
				...
				OP_C_NEW_STREAM_UNI(b, C_UNI_ID(0)) OP_C_WRITE(
					b, "apple", 5)

					OP_S_BIND_STREAM_ID(
						b,
						C_UNI_ID(
							0)) OP_S_READ_EXPECT(b,
									     "apple",
									     5)
...
static const struct script_op script_30[] = {
	OP_S_SET_INJECT_PLAIN(script_28_inject_plain) OP_C_SET_ALPN("ossltest")
		OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
			...
			OP_S_NEW_STREAM_UNI(a, S_UNI_ID(0)) OP_S_WRITE(
				a, "apple", 5)

				OP_C_ACCEPT_STREAM_WAIT(
					a) OP_C_READ_EXPECT(a, "apple", 5)
...
static const struct script_op script_33[] = {
	OP_S_SET_INJECT_PLAIN(script_32_inject_plain) OP_C_SET_ALPN("ossltest")
		OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
			...
			OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(
				a, "apple", 5)

				OP_S_BIND_STREAM_ID(
					a,
					C_BIDI_ID(0)) OP_S_READ_EXPECT(a,
								       "apple",
								       5)
				  ...
				  unsigned char *buf, size_t len)
{
	...
	if (!TEST_true(WPACKET_quic_write_vlint(
		    &wpkt, OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID)) ||
...
static const struct script_op script_39[] = {
	OP_S_SET_INJECT_PLAIN(script_39_inject_plain) OP_C_SET_ALPN("ossltest")
		OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
			...
			OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(
				a, "apple", 5) OP_S_BIND_STREAM_ID(a,
								   C_BIDI_ID(0))
				OP_S_READ_EXPECT(a, "apple", 5)

					OP_SET_INJECT_WORD(0, 1) OP_S_WRITE(
						a, "orange", 5)
...
static const struct script_op script_40[] = {
	OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
		...
		OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
			...
			OP_C_INHIBIT_TICK(
				1) OP_C_SET_WRITE_BUF_SIZE(a, 1024 * 100 * 3)

				OP_BEGIN_REPEAT(100)

					OP_C_WRITE(a, script_40_data,
	...
	OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
		...
		OP_BEGIN_REPEAT(100)
			...
			OP_S_READ_EXPECT(a, script_40_data,
					 sizeof(script_40_data))
			    ...
			    const void *buf, size_t len, SSL *ssl, void *arg)
{
	...
	if (!TEST_true(ossl_quic_wire_decode_frame_path_response(
		    &pkt, &frame_data)) ||
...
static const struct script_op script_41[] = {
	OP_S_SET_INJECT_PLAIN(script_41_inject_plain) OP_C_SET_ALPN(
		"ossltest") OP_C_CONNECT_WAIT() OP_CHECK(script_41_setup, 0)
		...
		OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(
			...
			OP_SET_INJECT_WORD(1,
				...
				OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(
					DEFAULT, "orange", 6)
				  ...
				  unsigned char *buf, size_t len)
{
	...
	if (!TEST_true(WPACKET_quic_write_vlint(&wpkt,
...
static const struct script_op script_44[] = {
	OP_S_SET_INJECT_PLAIN(script_44_inject_plain) OP_C_SET_ALPN(
		"ossltest") OP_C_CONNECT_WAIT()
		...
		OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(
			...
			OP_SET_INJECT_WORD(1, 0)
				...
				OP_S_WRITE(a, "Strawberry", 10)
					OP_C_READ_EXPECT(DEFAULT, "Strawberry",
							 10)
	...
	OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
		...
		OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(
			...
			OP_BEGIN_REPEAT(2)
				...
				OP_CHECK(force_ping, 0) OP_CHECK(
					wait_incoming_acks_increased, 0)
				  ...
				  unsigned char *buf, size_t len)
{
	...
	if (!TEST_true(WPACKET_quic_write_vlint(&wpkt, type)) ||
	    ...
	    !TEST_true(WPACKET_quic_write_vlint(
		    &wpkt, /*ack_range_count=*/range_count)) ||
	    !TEST_true(WPACKET_quic_write_vlint(
		    &wpkt, /*first_ack_range=*/first_range)))
		goto err;

	if (range_count > 0)
		if (!TEST_true(WPACKET_quic_write_vlint(
			    &wpkt, /*range[0].gap=*/agap)) ||
		    !TEST_true(WPACKET_quic_write_vlint(&wpkt,
							/*range[0].len=*/alen)))
			goto err;
...
static const struct script_op script_46[] = {
	OP_S_SET_INJECT_PLAIN(script_46_inject_plain) OP_C_SET_ALPN(
		"ossltest") OP_C_CONNECT_WAIT()
...
static const struct script_op script_53[] = {
	OP_S_SET_INJECT_PLAIN(script_53_inject_plain) OP_C_SET_ALPN(
		"ossltest") OP_C_CONNECT_WAIT()
		...
		OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(
			...
			OP_SET_INJECT_WORD(1, 0) OP_S_WRITE(a, "Strawberry", 10)
				...
				OP_C_EXPECT_CONN_CLOSE_INFO(
					OSSL_QUIC_ERR_CRYPTO_BUFFER_EXCEEDED, 0,
					0)
...
static const struct script_op script_54[] = {
	OP_S_SET_INJECT_HANDSHAKE(script_54_inject_handshake)
		OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT_OR_FAIL()
			...
			OP_C_EXPECT_CONN_CLOSE_INFO(
				OSSL_QUIC_ERR_CRYPTO_UNEXPECTED_MESSAGE, 0, 0)
		...
		OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
			...
			OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(
				...
				OP_S_READ_EXPECT(a, "apple", 5)
					...
					OP_SET_INJECT_WORD(0, 2) OP_S_WRITE(
						a, "orange", 5)
		...
		OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
			...
			OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(
				...
				OP_S_READ_EXPECT(a, "apple", 5)
					...
					OP_SET_INJECT_WORD(0, 3) OP_S_WRITE(
						a, "orange", 5)
		...
		OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
			...
			OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(
				...
				OP_S_READ_EXPECT(a, "apple", 5)
					...
					OP_SET_INJECT_WORD(0, 4) OP_S_WRITE(
						a, "orange",
						5) OP_C_READ_EXPECT(a, "orange",
								    5)
...
static const struct script_op script_58[] = {
	OP_S_SET_INJECT_PLAIN(script_58_inject_plain) OP_C_SET_ALPN(
		"ossltest") OP_C_CONNECT_WAIT()
		...
		OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
			...
			OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(
				...
				OP_S_BIND_STREAM_ID(
					...
					OP_SET_INJECT_WORD(1, 0) OP_S_WRITE(
						a, "orange", 6)
				      ...
				      size_t msglen)
{
	...
	if (!TEST_true(qtest_fault_resize_message(
		    h->qtf, datalen - SSL3_HM_HEADER_LENGTH)))
		return 0;
...
static const struct script_op script_68[] = {
	OP_S_SET_INJECT_HANDSHAKE(script_68_inject_handshake) OP_C_SET_ALPN(
		"ossltest") OP_C_CONNECT_WAIT()
		OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
	...
	OP_S_SET_INJECT_HANDSHAKE(script_68_inject_handshake) OP_C_SET_ALPN(
		...
		OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
			OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(
				...
				OP_SET_INJECT_WORD(
					2,
					0) OP_S_NEW_TICKET() OP_S_WRITE(a,
									"orange",
									6)
...
static const struct script_op script_70[] = {
	OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
		...
		OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(
			a, "apple",
			5) OP_S_BIND_STREAM_ID(a,
					       C_BIDI_ID(
						       0)) OP_S_READ_EXPECT(a,
									    "apple",
									    5)
		...
		OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
			...
			OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(
				...
				OP_S_READ_EXPECT(a, "apple", 5)
					...
					OP_CHECK(set_max_early_data,
						 0xffffffff) OP_S_NEW_TICKET()
						OP_S_WRITE(a, "orange", 6)
							OP_C_READ_EXPECT(
								a, "orange", 6)
...
static const struct script_op script_73[] = {
	...
	OP_BEGIN_REPEAT(200)
		...
		OP_C_NEW_STREAM_UNI_EX(a, ANY_ID,
				       ALLOW_FAIL | SSL_STREAM_FLAG_NO_BLOCK)
			OP_C_SKIP_IF_UNBOUND(a, 2) OP_C_WRITE(a, "apple", 5)
				OP_C_FREE_STREAM(a)
...
static const struct script_op script_74[] = {
	OP_CHECK(script_74_arm_packet_mutator,
		 0) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
	...
	OP_S_SET_INJECT_DATAGRAM(server_gen_version_neg)
		OP_SET_INJECT_WORD(2, 0)
			...
			OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT_OR_FAIL()
				...
				OP_C_EXPECT_CONN_CLOSE_INFO(
					OSSL_QUIC_ERR_CONNECTION_REFUSED, 0, 0)
					  ...
					  unsigned char *buf, size_t len)
{
	...
	if (!TEST_int_eq(SSL_inject_net_dgram(h->c_conn, databuf,
					      sizeof(databuf), NULL,
					      h->s_net_bio_addr),
			 1))
		return 0;
...
static const struct script_op script_80[] = {
	OP_S_SET_INJECT_PLAIN(script_80_inject_pkt) OP_C_SET_ALPN(
		"ossltest") OP_C_CONNECT_WAIT() OP_C_WRITE(DEFAULT, "apple", 5)
		OP_C_CONCLUDE(DEFAULT) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
			OP_S_READ_EXPECT(a, "apple", 5) OP_SET_INJECT_WORD(1, 0)
				OP_S_WRITE(a, "apple", 5) OP_C_READ_EXPECT(
					DEFAULT, "apple",
					5) OP_SET_INJECT_WORD(0, 1)
					OP_S_WRITE(a, "apple", 5)
						OP_C_EXPECT_CONN_CLOSE_INFO(
							0, 0, 1) OP_END
};
...
static int check_idle_timeout(struct helper *h, struct helper_local *hl)
{
	...
	if (!TEST_true(SSL_get_value_uint(h->c_conn,
					  (uint32_t)hl->check_op->arg1,
...
static const struct script_op script_81[] = {
	OP_C_SET_ALPN("ossltest") OP_CHECK(modify_idle_timeout,
					   25000) OP_C_CONNECT_WAIT()
...
static int check_avail_streams(struct helper *h, struct helper_local *hl)
{
	...
	case 0:
		if (!TEST_true(SSL_get_quic_stream_bidi_local_avail(h->c_conn,
								    &v)))
			return 0;
		break;
	case 1:
		if (!TEST_true(SSL_get_quic_stream_bidi_remote_avail(h->c_conn,
								     &v)))
			return 0;
		break;
	case 2:
		if (!TEST_true(
			    SSL_get_quic_stream_uni_local_avail(h->c_conn, &v)))
			return 0;
		break;
	case 3:
		if (!TEST_true(SSL_get_quic_stream_uni_remote_avail(h->c_conn,
								    &v)))
			return 0;
		break;
	default:
		...
		OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
			...
			OP_CHECK2(check_avail_streams, 0, 100) OP_CHECK2(
				...
				100) OP_CHECK2(check_avail_streams, 2,
					       100) OP_CHECK2(check_avail_streams,
							      3, 100)
						...
						OP_C_NEW_STREAM_UNI(b,
							...
							OP_CHECK2(check_avail_streams, 0, 99) OP_CHECK2(
								...
								OP_S_NEW_STREAM_BIDI(
									...
									OP_C_ACCEPT_STREAM_WAIT(c) OP_C_READ_EXPECT(
										c,
										"x",
										1)
										...
										OP_CHECK2(check_avail_streams, 0, 99) OP_CHECK2(
											...
											OP_S_NEW_STREAM_UNI(d, S_UNI_ID(0)) OP_S_WRITE(
												d,
												"x",
												1)

												OP_C_ACCEPT_STREAM_WAIT(d) OP_C_READ_EXPECT(
													d,
													"x",
													1)
													...
													OP_CHECK2(check_avail_streams, 0, 99) OP_CHECK2(
														...
														OP_CHECK2(
															...
															0,
															0) OP_CHECK(set_event_handling_mode_conn,
...
ossl_unused static int script_85_poll(struct helper *h, struct helper_local *hl)
{
	...
	for (i = 0; i < OSSL_NELEM(items); ++i)
		if (!TEST_uint64_t_eq(items[i].revents, expected_revents[i])) {
			TEST_error(
				"mismatch at index %zu in poll results, mode %d",
				i, (int)mode);
			ok = 0;
		}
...
static const struct script_op script_85[] = {
	OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
		...
		OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
			...
			OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(
				...
				OP_C_NEW_STREAM_BIDI(
					b, C_BIDI_ID(1)) OP_C_WRITE(b, "orange",
								    6)
					...
					OP_C_NEW_STREAM_BIDI(
						...
						OP_C_NEW_STREAM_BIDI(
							...
							OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_BIND_STREAM_ID(
								b, C_BIDI_ID(1))
								OP_S_BIND_STREAM_ID(
									c,
									C_BIDI_ID(
	...
	OP_S_SET_INJECT_PLAIN(script_28_inject_plain) OP_SET_INJECT_WORD(
		C_BIDI_ID(1) + 1, OSSL_QUIC_FRAME_TYPE_RESET_STREAM)
	...
	OP_S_SET_INJECT_PLAIN(script_28_inject_plain) OP_SET_INJECT_WORD(
		C_BIDI_ID(2) + 1, OSSL_QUIC_FRAME_TYPE_STOP_SENDING)
...
ossl_unused static int script_88_poll_conly(struct helper *h,
					    struct helper_local *hl)
{
	int ok = 1;
	static const struct timeval timeout = { 0 };
	...
	while (done == 0 && ok == 1) {
		...
		TEST_info("received event " POLL_FMT,
			  POLL_PRINTA(items[0].revents));

		if ((items[0].revents & SSL_POLL_EVENT_EC) == SSL_POLL_EVENT_EC)
			SSL_shutdown(h->c_conn);
		done = ((items[0].revents & SSL_POLL_EVENT_ECD) ==
			SSL_POLL_EVENT_ECD);
...
static const struct script_op script_88[] = {
	OP_SKIP_IF_BLOCKING(16) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
	...
	OP_CHECK(script_88_poll, 0 /* ->arg2 */)
		...
		OP_C_WRITE(a, "flamingo", 8) OP_C_CONCLUDE(a)

	/* Send something that will make client sockets readable. */
	OP_S_READ_EXPECT(a, "flamingo", 8) OP_S_WRITE(a, "flamingo", 8)
		OP_S_CONCLUDE(a)
     ...
     */
	OP_C_SHUTDOWN(NULL, 0) OP_SLEEP(100)
...
static const struct script_op script_86[] = {
	...
	OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
		...
		OP_BEGIN_REPEAT(20) OP_S_READ_FAIL(a, 1) OP_SLEEP(10)
			OP_END_REPEAT()
...
static const struct script_op script_87[] = {
	OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_NEW_STREAM_BIDI(
		a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5) OP_C_CONCLUDE(a)
		OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(
			a, "apple",
			5) OP_S_EXPECT_FIN(a) OP_S_WRITE(a, "orange", 6)
			OP_C_READ_EXPECT(a, "orange", 6) OP_S_CONCLUDE(a)
				OP_C_EXPECT_FIN(a) OP_SLEEP(1000)
...
static const struct script_op *const scripts[] = {
	...
	script_25, script_26, script_27, script_28, script_29, script_30,
	script_31, script_32, script_33, script_34, script_35, script_36,
	script_37, script_38, script_39, script_40, script_41, script_42,
	script_43, script_44, script_45, script_46, script_47, script_48,
	script_49, script_50, script_51, script_52, script_53, script_54,
	script_55, script_56, script_57, script_58, script_59, script_60,
	script_61, script_62, script_63, script_64, script_65, script_66,
	script_67, script_68, script_69, script_70, script_71, script_72,
	script_73, script_74, script_75, script_76, script_77, script_78,
	script_79, script_80, script_81, script_82, script_83, script_84,
	script_85, script_86, script_87, script_88,
};
...
static int test_script(int idx)
{
	...
	BIO_snprintf(script_name, sizeof(script_name), "script %d",
		     script_idx + 1);
...
static struct script_op dyn_frame_types_script[] = {
	...
	OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT_OR_FAIL()
		...
		OP_C_EXPECT_CONN_CLOSE_INFO(OSSL_QUIC_ERR_FRAME_ENCODING_ERROR,
					    0, 0)
