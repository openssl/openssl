...
static int execute_cert_test(CT_TEST_FIXTURE *fixture)
{
	...
	if (fixture->certificate_file != NULL) {
		...
		if (fixture->expected_sct_count > 0) {
			...
			if (fixture->test_validity) {
				if (!assert_validity(fixture, scts,
						     ct_policy_ctx))
					goto end;
			}
	...
	if (fixture->tls_sct_list != NULL) {
		...
		if (!TEST_ptr(
			    o2i_SCT_LIST(&scts, &p, fixture->tls_sct_list_len)))
			goto end;
		...
		if (!TEST_mem_eq(fixture->tls_sct_list,
				 fixture->tls_sct_list_len, tls_sct_list,
				 tls_sct_list_len))
			goto end;
	}
	success = 1;
...
static int test_encode_tls_sct(void)
{
	...
	if (!TEST_ptr(sct = SCT_new_from_base64(
			      SCT_VERSION_V1, log_id, CT_LOG_ENTRY_TYPE_X509,
			      timestamp, extensions, signature))) {
		tear_down(fixture);
		return 0;
	}
