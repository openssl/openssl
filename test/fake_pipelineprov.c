			      ...
			      const unsigned char **iv, size_t ivlen, int enc)
{
	...
	for (i = 0; i < numpipes; i++) {
		...
		if (ctx->cipher_ctxs[i] == NULL)
			return 0;
		if (!EVP_CipherInit(ctx->cipher_ctxs[i], ctx->cipher, key,
				    iv[i], enc))
			return 0;
	}
...
int fake_pipeline_aead_get_ctx_params(void *vctx, OSSL_PARAM params[])
{
	...
	if (p != NULL) {
		if (!OSSL_PARAM_get_octet_ptr(p, (const void **)&aead_tags,
					      &taglen)) {
			ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
			return 0;
		}
		for (i = 0; i < ctx->numpipes; i++) {
			aead_params[0] = OSSL_PARAM_construct_octet_string(
				...
				(void *)aead_tags[i], taglen);
			if (!EVP_CIPHER_CTX_get_params(ctx->cipher_ctxs[i],
						       aead_params)) {
	...
	if (p != NULL) {
		...
		for (i = 0; i < ctx->numpipes; i++) {
			aead_params[0] = OSSL_PARAM_construct_octet_string(
				OSSL_CIPHER_PARAM_AEAD_TAG,
				(void *)aead_tags[i], taglen);
			if (!EVP_CIPHER_CTX_set_params(ctx->cipher_ctxs[i],
						       aead_params)) {
				ERR_raise(ERR_LIB_PROV,
					  PROV_R_FAILED_TO_SET_PARAMETER);
				return 0;
...
#define IMPLEMENT_aead_cipher_pipeline(alg, lc, UCMODE, flags, kbits, blkbits,      \
	...
	static void *fake_pipeline_##alg##_##kbits##_##lc##_newctx(                 \
		void *provctx)                                                      \
	{                                                                           \
		return fake_pipeline_newctx(provctx, ciphername, kbits,             \
					    ivbits);                                \
	}                                                                           \
