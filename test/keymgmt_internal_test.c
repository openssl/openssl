...
static int export_cb(const OSSL_PARAM *params, void *arg)
{
	...
	if (!TEST_ptr(p = OSSL_PARAM_locate_const(
	    ...
	    !TEST_ptr(p = OSSL_PARAM_locate_const(
			      params, OSSL_PKEY_PARAM_RSA_FACTOR3)) ||
	    !TEST_true(get_ulong_via_BN(p, &keydata[F3])))
		return 0;

	if (!TEST_ptr(p = OSSL_PARAM_locate_const(
			      params, OSSL_PKEY_PARAM_RSA_EXPONENT1)) ||
	    !TEST_true(get_ulong_via_BN(p, &keydata[DP])) ||
	    !TEST_ptr(p = OSSL_PARAM_locate_const(
			      params, OSSL_PKEY_PARAM_RSA_EXPONENT2)) ||
	    !TEST_true(get_ulong_via_BN(p, &keydata[DQ])) ||
	    !TEST_ptr(p = OSSL_PARAM_locate_const(
			      params, OSSL_PKEY_PARAM_RSA_EXPONENT3)) ||
...
static int test_pass_rsa(FIXTURE *fixture)
{
	...
	for (;;) {
		...
		if (!TEST_ptr_null(provkey2 = evp_pkey_export_to_provider(
					   pk, NULL, &km, NULL)))
			goto err;

		if (!TEST_ptr(provkey = evp_pkey_export_to_provider(
				      pk, NULL, &km1, NULL)) ||
		...
		for (i = 0; i < OSSL_NELEM(expected); i++) {
			int rv = TEST_uint64_t_eq((uint64_t)expected[i],
						  (uint64_t)keydata[i]);
