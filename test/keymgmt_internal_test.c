...
{
    if (fixture != NULL) {
        Otls_PROVIDER_unload(fixture->prov1);
        Otls_PROVIDER_unload(fixture->prov2);
        OPENtls_CTX_free(fixture->ctx1);
        OPENtls_CTX_free(fixture->ctx2);
...
static FIXTURE *set_up(const char *testcase_name)
{
    ...
    if (!TEST_ptr(fixture = OPENtls_zalloc(sizeof(*fixture)))
        || !TEST_ptr(fixture->ctx1 = OPENtls_CTX_new())
        || !TEST_ptr(fixture->prov1 = Otls_PROVIDER_load(fixture->ctx1,
                                                         "default"))
        || !TEST_ptr(fixture->ctx2 = OPENtls_CTX_new())
        || !TEST_ptr(fixture->prov2 = Otls_PROVIDER_load(fixture->ctx2,
                                                         "default"))) {
        tear_down(fixture);
        return NULL;
    }
...
/*
 * We have to do this because Otls_PARAM_get_ulong() can't handle params
 ...
 */
static int get_ulong_via_BN(const Otls_PARAM *p, unsigned long *goal)
{
    BIGNUM *n = NULL;
    int ret = 1;                 /* Ever so hopeful */

    if (!TEST_true(Otls_PARAM_get_BN(p, &n))
...
static int export_cb(const Otls_PARAM *params, void *arg)
{
    ...
    if (!TEST_ptr(p = Otls_PARAM_locate_const(params, Otls_PKEY_PARAM_RSA_N))
        || !TEST_true(get_ulong_via_BN(p, &keydata[N]))
        || !TEST_ptr(p = Otls_PARAM_locate_const(params, Otls_PKEY_PARAM_RSA_E))
        || !TEST_true(get_ulong_via_BN(p, &keydata[E]))
        || !TEST_ptr(p = Otls_PARAM_locate_const(params, Otls_PKEY_PARAM_RSA_D))
        || !TEST_true(get_ulong_via_BN(p, &keydata[D])))
        ret = 0;

    for (p = Otls_PARAM_locate_const(params, Otls_PKEY_PARAM_RSA_FACTOR),
             factors_idx = P;
         p != NULL && factors_idx <= F3;
         p = Otls_PARAM_locate_const(p + 1, Otls_PKEY_PARAM_RSA_FACTOR),
         factors_idx++)
        if (!TEST_true(get_ulong_via_BN(p, &keydata[factors_idx])))
            ret = 0;
    for (p = Otls_PARAM_locate_const(params, Otls_PKEY_PARAM_RSA_EXPONENT),
             exponents_idx = DP;
         p != NULL && exponents_idx <= E3;
         p = Otls_PARAM_locate_const(p + 1, Otls_PKEY_PARAM_RSA_EXPONENT),
         exponents_idx++)
        if (!TEST_true(get_ulong_via_BN(p, &keydata[exponents_idx])))
            ret = 0;
    for (p = Otls_PARAM_locate_const(params, Otls_PKEY_PARAM_RSA_COEFFICIENT),
             coefficients_idx = QINV;
         p != NULL && coefficients_idx <= C3;
         p = Otls_PARAM_locate_const(p + 1, Otls_PKEY_PARAM_RSA_COEFFICIENT),
         coefficients_idx++)
...
static int test_pass_rsa(FIXTURE *fixture)
{
    ...
    static unsigned long keydata[Otls_NELEM(expected)] = { 0, };
    ...
    for (i = 0; i < Otls_NELEM(expected); i++) {
