...
/* Helper: call ossl_vasprintf via a wrapper to exercise the va_list path. */
...
static int test_asprintf_short(void)
{
    ...
    n = ossl_asprintf(&result, "answer=%d", 42);
...
static int test_asprintf_grows(void)
{
    ...
    n = ossl_asprintf(&result, "%s", lumberjack);
...
/* ossl_vasprintf exercised through a small wrapper. */
