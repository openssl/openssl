...
static int client_cert_cb(SSL *ssl, X509 **x509, EVP_PKEY **pkey)
{
	...
	if (!TEST_ptr(xcert = X509_new_ex(NULL, NULL)) ||
	    ...
	    !TEST_ptr(privpkey = PEM_read_bio_PrivateKey_ex(priv_in, NULL, NULL,
							    NULL, NULL, NULL)))
		goto err;
...
static int ssl_comp_cert(SSL *ssl, int alg)
{
	...
	if (!TEST_size_t_gt(comp_len = SSL_get1_compressed_cert(
				    ssl, alg, &comp_data, &orig_len),
			    0))
		goto err;
...
static int test_ssl_cert_comp(int test)
{
	...
	int server_pref[] = { TLSEXT_comp_cert_zstd, TLSEXT_comp_cert_zlib,
			      TLSEXT_comp_cert_brotli };
	/* default order */
	int client_pref[] = { TLSEXT_comp_cert_brotli, TLSEXT_comp_cert_zlib,
	...
	if (!TEST_true(create_ssl_ctx_pair(NULL, TLS_server_method(),
					   TLS_client_method(), TLS1_3_VERSION,
	...
	if (test == 3) {
		...
		if (!TEST_true(SSL_CTX_set1_cert_comp_preference(
			    sctx, server_pref, 2)))
			goto end;
		...
		if (!TEST_true(SSL_CTX_set1_cert_comp_preference(
			    sctx, server_pref, OSSL_NELEM(server_pref))))
			goto end;
		if (!TEST_true(SSL_CTX_set1_cert_comp_preference(
			    cctx, client_pref, OSSL_NELEM(client_pref))))
			goto end;
	}
	if (test == 2) {
		...
		SSL_CTX_set_verify(
			sctx, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT,
			verify_cb);
	}
