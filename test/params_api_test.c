 ...
 * byte reversal.
 */
static void le_copy(unsigned char *out, size_t outlen, const void *in,
		    size_t inlen)
{
	DECLARE_IS_ENDIAN;
...
static const struct {
	...
	{ 16,
	  { 0x61, 0xe8, 0x7e, 0x31, 0xe9, 0x33, 0x83, 0x3d, 0x87, 0x99, 0xc7,
	    0xd8, 0x5d, 0xa9, 0x8b, 0x42 } },
	{ 16,
	  { 0xee, 0x6e, 0x8b, 0xc3, 0xec, 0xcf, 0x37, 0xcc, 0x89, 0x67, 0xf2,
	    0x68, 0x33, 0xa0, 0x14, 0xb0 } },
};
				 ...
				 size_t width)
{
	...
	if (signd) {
		...
		if ((bit32 && !TEST_true(OSSL_PARAM_get_uint32(
				      param, (uint32_t *)&i32))) ||
		    !TEST_true(
			    OSSL_PARAM_get_uint64(param, (uint64_t *)&i64)) ||
	...
	if (sizeof(size_t) > width) {
		if (signd) {
			...
			if (!TEST_true(OSSL_PARAM_set_uint32(param, 12345)) ||
			    !TEST_true(OSSL_PARAM_get_uint64(
				    param, (uint64_t *)&i64)) ||
			    !TEST_size_t_eq((size_t)i64, 12345))
				return 0;
		}
...
static int test_param_int(int n)
{
	...
	const size_t len = raw_values[n].len >= sizeof(int) ? sizeof(int) :
							      raw_values[n].len;
...
static int test_param_uint(int n)
{
	...
	const size_t len = raw_values[n].len >= sizeof(unsigned int) ?
				   sizeof(unsigned int) :
				   raw_values[n].len;
...
static int test_param_bignum(int n)
{
	...
	OSSL_PARAM param =
		OSSL_PARAM_DEFN("bn", OSSL_PARAM_UNSIGNED_INTEGER, NULL, 0);
	int ret = 0;
	...
	if (!TEST_true(OSSL_PARAM_get_BN(&param, &c)) || !TEST_BN_eq(b, c))
		goto err;
...
static int test_param_signed_bignum(int n)
{
	...
	if (!TEST_ptr(b = BN_signed_lebin2bn(raw_values[n].value, (int)len,
					     NULL)))
		goto err;

	/* raw_values are little endian */
	if (!TEST_false(!!(raw_values[n].value[len - 1] & 0x80) ^
			BN_is_negative(b)))
		goto err;
	if (!TEST_true(OSSL_PARAM_set_BN(&param, b)))
		goto err;
...
{
	static const char *int_names[] = { "int", "long", "int32", "int64" };
	static const char *uint_names[] = { "uint", "ulong", "uint32", "uint64",
					    "size_t" };
	static const unsigned char bn_val[16] = { 0xac, 0x75, 0x22, 0x7d,
						  0x81, 0x06, 0x7a, 0x23,
						  0xa6, 0xed, 0x87, 0xc7,
						  0xab, 0xf4, 0x73, 0x22 };
	...
	params[n++] =
		OSSL_PARAM_construct_utf8_string("utf8str", buf, sizeof(buf));
	params[n++] =
		OSSL_PARAM_construct_octet_string("octstr", buf, sizeof(buf));
	params[n++] =
		OSSL_PARAM_construct_octet_string("octstr2", buf, sizeof(buf));
	...
	if (!TEST_ptr(cp = OSSL_PARAM_locate(p, "double")) ||
	    ...
	    !TEST_double_eq(d2, 3.14) || (tstid <= 1 && !TEST_double_eq(d, d2)))
		goto err;
	/* UTF8 string */
	bufp = NULL;
	...
	if (!TEST_true(OSSL_PARAM_get_octet_string(cp, &vpn, 0, &s)) ||
	    ...
	    !TEST_mem_eq(vpn, sizeof("abcdefghi"), "abcdefghi",
			 sizeof("abcdefghi")))
		goto err;
	vp = buf2;
	if (!TEST_true(
		    OSSL_PARAM_get_octet_string(cp, &vp, sizeof(buf2), &s)) ||
	    !TEST_size_t_eq(s, sizeof("abcdefghi")) ||
	    !TEST_mem_eq(vp, sizeof("abcdefghi"), "abcdefghi",
			 sizeof("abcdefghi")))
		goto err;
	/* OCTET string using OSSL_PARAM_set_octet_string_or_ptr */
	if (!TEST_ptr(cp = OSSL_PARAM_locate(p, "octstr2")) ||
	    !TEST_true(OSSL_PARAM_set_octet_string_or_ptr(
		    cp, "jklmnopqr", sizeof("jklmnopqr"))) ||
	...
	if (!TEST_true(OSSL_PARAM_get_octet_string(cp, &vpn2, 0, &s)) ||
	    ...
	    !TEST_mem_eq(vpn2, sizeof("jklmnopqr"), "jklmnopqr",
			 sizeof("jklmnopqr")))
		goto err;
	vp = buf2;
	if (!TEST_true(
		    OSSL_PARAM_get_octet_string(cp, &vp, sizeof(buf2), &s)) ||
	    !TEST_size_t_eq(s, sizeof("jklmnopqr")) ||
	    !TEST_mem_eq(vp, sizeof("jklmnopqr"), "jklmnopqr",
			 sizeof("jklmnopqr")))
		goto err;
	/* OCTET pointer */
	vp = &l;
	...
	if (!TEST_ptr(cp = OSSL_PARAM_locate(p, "octptr2")) ||
	    !TEST_true(
		    OSSL_PARAM_set_octet_string_or_ptr(cp, &ul, sizeof(ul))) ||
	    !TEST_size_t_eq(cp->return_size, sizeof(ul)) ||
	    (tstid <= 1 && !TEST_ptr_eq(vp, &ul)))
		goto err;
	/* Match the return size to avoid trailing garbage bytes */
	...
	/* Match the return size to avoid trailing garbage bytes */
	cp->data_size = cp->return_size;
	if (!TEST_true(OSSL_PARAM_get_BN(cp, &bn2)) || !TEST_BN_eq(bn, bn2))
		goto err;
	ret = 1;
err:
	if (p != params)
...
static int test_param_modified(void)
{
	OSSL_PARAM param[3] = { OSSL_PARAM_int("a", NULL),
				OSSL_PARAM_int("b", NULL), OSSL_PARAM_END };
	int a, b;
...
static int test_param_copy_null(void)
{
	...
	ret = TEST_ptr_null(OSSL_PARAM_dup(NULL)) &&
	      ...
	      TEST_true(OSSL_PARAM_get_int(p, &val)) && TEST_int_eq(val, 1) &&
	      TEST_ptr(p = OSSL_PARAM_locate(cp1, "b")) &&
	      TEST_true(OSSL_PARAM_get_int(p, &val)) && TEST_int_eq(val, 2) &&
