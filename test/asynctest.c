...
{
    OSSL_CTX *libctx = OSSL_CTX_new();
    OSSL_CTX *oldctx, *tmpctx;
    int ret = 0;

    if (libctx == NULL)
        return 0;

    oldctx = OSSL_CTX_set0_default(libctx);
    ASYNC_pause_job();

    /* Check the libctx is set up as we expect */
    tmpctx = OSSL_CTX_set0_default(oldctx);
    if (tmpctx != libctx)
        goto err;
...
static int test_ASYNC_start_job_with_libctx(void)
{
    ...
    ASYNC_WAIT_CTX *waitctx = NULL;
    OSSL_CTX *libctx = OSSL_CTX_new();
    OSSL_CTX *oldctx, *tmpctx, *globalctx;
    int ret = 0;
    ...
    globalctx = oldctx = OSSL_CTX_set0_default(libctx);
    ...
    tmpctx = OSSL_CTX_set0_default(oldctx);
    oldctx = OSSL_CTX_set0_default(tmpctx);
    if (tmpctx != libctx) {
        fprintf(stderr,
    ...
    /* Reset the libctx temporarily to find out what it is*/
    tmpctx = OSSL_CTX_set0_default(libctx);
    OSSL_CTX_set0_default(tmpctx);
    if (tmpctx != globalctx) {
        fprintf(stderr,
