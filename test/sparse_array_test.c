...
static int test_sparse_array(void)
{
	...
	for (i = 0; i < OSSL_NELEM(cases); i++) {
		...
		for (j = 0; j <= i; j++)
			if (!TEST_str_eq(ossl_sa_char_get(sa, cases[j].n),
					 cases[j].v)) {
...
static int test_sparse_array_num(void)
{
	static const struct {
		...
		ossl_uintmax_t n;
		char *v;
	} cases[] = { { 1, 22, "a" },	 { 2, 1021, "b" },   { 3, 3, "c" },
		      { 2, 22, NULL },	 { 2, 3, "d" },	     { 3, 22, "e" },
		      { 3, 666, NULL },	 { 4, 666, "f" },    { 3, 3, NULL },
		      { 2, 22, NULL },	 { 1, 666, NULL },   { 2, 64000, "g" },
		      { 1, 1021, NULL }, { 0, 64000, NULL }, { 1, 23, "h" },
		      { 0, 23, NULL } };
...
static void leaf_check_all(ossl_uintmax_t n, char *value, void *arg)
{
	...
	for (i = 0; i < doall_data->num_cases; i++)
		if ((doall_data->all || !cases[i].del) && n == cases[i].n &&
		    strcmp(value, cases[i].v) == 0) {
			doall_data->res = 1;
			return;
		}
