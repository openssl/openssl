...
static const OSSL_ALGORITHM testprov_digests[] = { { "testprovmd", MYPROPERTIES,
...
static int test_default_props_and_providers(int propsorder)
{
	...
	if (propsorder == DEFAULT_PROPS_AFTER_FETCH) {
		if (!TEST_true(
			    EVP_set_default_properties(libctx, MYPROPERTIES)))
			goto err;
		EVP_MD_free(testprovmd);
		if (!TEST_ptr(testprovmd =
				      EVP_MD_fetch(libctx, "testprovmd", NULL)))
			goto err;
	}
