...
static int test_xwing_kat(void)
{
    ...
    if (!TEST_ptr(gctx = EVP_PKEY_CTX_new_from_name(libctx, "X-Wing", NULL))
...
static int test_xwing(void)
{
    ...
    if (!TEST_ptr(priv = EVP_PKEY_Q_keygen(libctx, NULL, "X-Wing")))
        return 0;
...
static int test_xwing_hpke(void)
{
    ...
    static const unsigned char info[] = "X-Wing HPKE test";
    static const unsigned char aad[] = "associated data";
    static const unsigned char plaintext[] = "post-quantum plaintext";
    OSSL_HPKE_SUITE suite = {
        OSSL_HPKE_KEM_ID_XWING,
        OSSL_HPKE_KDF_ID_HKDF_SHA256,
        OSSL_HPKE_AEAD_ID_AES_GCM_128
    };
    OSSL_HPKE_CTX *sender = NULL, *receiver = NULL;
    EVP_PKEY *recipient = NULL;
    unsigned char pub[sizeof(expected_pk)];
    unsigned char enc[sizeof(expected_ciphertext)];
    unsigned char ciphertext[sizeof(plaintext) + 16];
    unsigned char recovered[sizeof(plaintext)];
    size_t publen = sizeof(pub), enclen = sizeof(enc);
    ...
    OSSL_HPKE_CTX_free(receiver);
    OSSL_HPKE_CTX_free(sender);
