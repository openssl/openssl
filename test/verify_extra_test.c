...
static int do_test_proxy_pathlen(const char *leaf_file, int assert_flag,
    int assert_pathlen, long pathlen, int expected)
{
    ...
    if (assert_flag) {
        long before = X509_get_proxy_pathlen(pc1);

        X509_set_proxy_flag(pc1);
        /* Asserting the flag must not change the effective path length. */
        if (!TEST_long_eq(X509_get_proxy_pathlen(pc1), before))
            goto err;
    }
    if (assert_pathlen) {
...
{
    return do_test_proxy_pathlen(pc2_cert, 0, 0, 0, 1);
}

static int test_proxy_pathlen_from_extension_exceeded(void)
{
    return do_test_proxy_pathlen(bad_pc4_cert, 0, 0, 0, 0);
}

static int test_proxy_pathlen_asserted_exceeded(void)
{
    return do_test_proxy_pathlen(pc2_cert, 1, 1, 0, 0);
}

static int test_proxy_pathlen_asserted_allows(void)
{
    return do_test_proxy_pathlen(bad_pc4_cert, 1, 1, 2, 1);
}

/* Asserting the proxy flag alone must not lift the encoded path length. */
static int test_proxy_pathlen_flag_only_keeps_extension(void)
{
    return do_test_proxy_pathlen(bad_pc4_cert, 1, 0, 0, 0);
}

/* Setting the proxy path length must take effect without the flag call. */
static int test_proxy_pathlen_set_without_flag(void)
{
    return do_test_proxy_pathlen(pc2_cert, 0, 1, 0, 0);
}
...
int setup_tests(void)
{
    ...
    ADD_TEST(test_proxy_pathlen_flag_only_keeps_extension);
    ADD_TEST(test_proxy_pathlen_set_without_flag);
    return 1;
err:
    cleanup_tests();
    return 0;
