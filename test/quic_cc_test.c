...
static int net_sim_init(struct net_sim *s, const OSSL_CC_METHOD *ccm,
			OSSL_CC_DATA *cc, uint64_t capacity, uint64_t latency)
{
	s->ccm = ccm;
	s->cc = cc;
...
static int net_sim_send(struct net_sim *s, size_t sz)
{
	...
	if (success) {
		...
		pkt->arrive_time =
			ossl_time_add(pkt->tx_time, ossl_ms2time(s->latency));
		/* Assume all received packets are acknowledged immediately. */
		pkt->determination_time = ossl_time_add(
			pkt->arrive_time, ossl_ms2time(s->latency));
         ...
         */
		pkt->determination_time = ossl_time_add(
			pkt->tx_time, ossl_ms2time(3 * s->latency));
		      ...
		      struct net_sim *s)
{
	...
	if (!TEST_true(ccm->get_option_uint(
		    cc, OSSL_CC_OPTION_CUR_BYTES_IN_FLIGHT, &cur_bytes)))
		return 0;

	if (!TEST_true(
		    ccm->get_option_uint(cc, OSSL_CC_OPTION_CUR_STATE, &state)))
		return 0;

	fprintf(logfile,
		"%10lu,%10lu,%10lu,%10lu,%10lu,%10lu,%10lu,%10lu,\"%c\"\n",
		ossl_time2ms(fake_time), ccm->get_tx_allowance(cc), cwnd_size,
		cur_bytes, s->total_acked, s->total_lost, s->capacity,
	...
	*p++ = OSSL_PARAM_construct_end();
	...
	while (total_sent < total_to_send) {
		...
		/* Update our average of the estimated channel capacity. */
		{
			...
			if (!TEST_uint64_t_ne(diag_cur_bytes_in_flight,
					      UINT64_MAX) ||
     ...
     */
	{
		uint64_t estimated_capacity =
			cwnd_sample_sum / cwnd_sample_count;

		double error =
			((double)estimated_capacity / (double)actual_capacity) -
			1.0;
...
static int test_sanity(void)
{
	...
	*p++ = OSSL_PARAM_construct_end();
	...
	if (!TEST_uint64_t_ge(allowance2 = ccm->get_tx_allowance(cc),
			      allowance))
		goto err;
	...
	/* Test loss. */
	if (!TEST_uint64_t_ge(allowance = ccm->get_tx_allowance(cc),
			      1200 + 1300))
		goto err;
