...
static int get_sni_from_client_hello(BIO *bio, char **sni)
{
	...
	while (PACKET_remaining(&pkt2)) {
		...
		if (type == TLSEXT_TYPE_server_name) {
			if (!TEST_true(PACKET_get_length_prefixed_2(&pkt3,
								    &pkt4)) ||
			    !TEST_size_t_ne(PACKET_remaining(&pkt4), 0) ||
			    !TEST_true(PACKET_get_1(&pkt4, &servname_type)) ||
			    !TEST_uint_eq(servname_type,
					  TLSEXT_NAMETYPE_host_name) ||
			    !TEST_true(PACKET_get_length_prefixed_2(&pkt4,
								    &pkt5)) ||
			    !TEST_size_t_le(PACKET_remaining(&pkt5),
...
static int server_setup_sni(void)
{
	...
	if (!TEST_ptr_null(
		    SSL_get_servername(serverssl, TLSEXT_NAMETYPE_host_name))) {
...
static sni_test_fn sni_test_fns[3] = { client_setup_sni_before_state,
