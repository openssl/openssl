...
{
	return fixture != NULL && test_default_cipherlist(fixture->server) &&
...
static int test_default_cipherlist_clear(void)
{
	...
	if (!TEST_int_eq(SSL_CTX_set_cipher_list(fixture->server, "no-such"),
			 0))
		goto end;
...
static int test_stdname_cipherlist(void)
{
	...
	if (!TEST_true(SSL_CTX_set_cipher_list(
		    fixture->server, TLS1_RFC_RSA_WITH_AES_128_SHA)) ||
	    !TEST_true(SSL_CTX_set_cipher_list(
		    fixture->client, TLS1_RFC_RSA_WITH_AES_128_SHA))) {
		goto end;
	}
	result = 1;
end:
