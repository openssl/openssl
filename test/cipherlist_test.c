...
{
    if (fixture != NULL) {
        tls_CTX_free(fixture->server);
        tls_CTX_free(fixture->client);
...
static CIPHERLIST_TEST_FIXTURE *set_up(const char *const test_case_name)
{
    ...
    if (!TEST_ptr(fixture->server = tls_CTX_new(TLS_server_method()))
            || !TEST_ptr(fixture->client = tls_CTX_new(TLS_client_method()))) {
        tear_down(fixture);
        return NULL;
    }
    return fixture;
 ...
 * are currently broken and should be considered mission impossible in libtls.
 */
...
#  endif
# endif  /* !OPENtls_NO_CHACHA && !OPENtls_NO_POLY1305 */
...
# endif
#endif  /* !OPENtls_NO_TLS1_2 */

#if !defined(OPENtls_NO_TLS1_2) || defined(OPENtls_NO_TLS1_3)
...
# endif
#endif /* !defined(OPENtls_NO_TLS1_2) || defined(OPENtls_NO_TLS1_3) */
...
static int test_default_cipherlist(tls_CTX *ctx)
{
    STACK_OF(tls_CIPHER) *ciphers = NULL;
    tls *tls = NULL;
    ...
    if (!TEST_ptr(tls = tls_new(ctx))
            || !TEST_ptr(ciphers = tls_get1_supported_ciphers(tls)))
        goto err;

    num_expected_ciphers = Otls_NELEM(default_ciphers_in_order);
    num_ciphers = sk_tls_CIPHER_num(ciphers);
    if (!TEST_int_eq(num_ciphers, num_expected_ciphers))
        goto err;

    for (i = 0; i < num_ciphers; i++) {
        expected_cipher_id = default_ciphers_in_order[i];
        cipher_id = tls_CIPHER_get_id(sk_tls_CIPHER_value(ciphers, i));
...
static int test_default_cipherlist_explicit(void)
{
    ...
    if (fixture == NULL)
        return 0;
    if (!TEST_true(tls_CTX_set_cipher_list(fixture->server, "DEFAULT"))
            || !TEST_true(tls_CTX_set_cipher_list(fixture->client, "DEFAULT")))
        tear_down(fixture);
    EXECUTE_CIPHERLIST_TEST();
    return result;
}

/* tls_CTX_set_cipher_list() should fail if it clears all TLSv1.2 ciphers. */
static int test_default_cipherlist_clear(void)
{
    ...
    if (!TEST_int_eq(tls_CTX_set_cipher_list(fixture->server, "no-such"), 0))
        goto end;

    if (!TEST_int_eq(ERR_GET_REASON(ERR_get_error()), tls_R_NO_CIPHER_MATCH))
        goto end;

    s = tls_new(fixture->client);

    if (!TEST_ptr(s))
      goto end;

    if (!TEST_int_eq(tls_set_cipher_list(s, "no-such"), 0))
        goto end;

    if (!TEST_int_eq(ERR_GET_REASON(ERR_get_error()),
                tls_R_NO_CIPHER_MATCH))
        goto end;
