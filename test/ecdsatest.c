...
static int x9_62_tests(int n)
{
	...
	if (!TEST_ptr(mctx = EVP_MD_CTX_new())
	    ...
	    !TEST_true(EVP_DigestInit_ex(mctx, EVP_get_digestbynid(md_nid),
					 NULL)) ||
...
static int test_builtin(int n, int as)
{
	...
	if (nid == NID_ipsec4 || nid == NID_ipsec3) {
		TEST_info("skipped: ECDSA unsupported for curve %s",
			  OBJ_nid2sn(nid));
		return 1;
	}
	...
	if (!TEST_int_ge(temp, 0) ||
	    ...
	    || !TEST_true(EVP_DigestVerifyInit(mctx, NULL, NULL, NULL,
					       pkey_neg)) ||
