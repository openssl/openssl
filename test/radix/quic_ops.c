...
static int ssl_ctx_select_alpn(SSL *ssl, const unsigned char **out,
...
static int ssl_create_bound_socket(uint16_t listen_port, int *p_fd,
				   uint16_t *p_result_port)
{
	int ok = 0;
	int fd = -1;
	BIO_ADDR *addr = NULL;
...
static int ssl_attach_bio_dgram(SSL *ssl, uint16_t local_port,
...
DEF_FUNC(hf_new_ssl)
{
	...
	method = is_server ? OSSL_QUIC_server_method() :
			     OSSL_QUIC_client_method();
	if (!TEST_ptr(ctx = SSL_CTX_new(method)))
		goto err;

#if defined(OPENSSL_THREADS)
	if (!TEST_true(SSL_CTX_set_domain_flags(
		    ctx,
		    SSL_DOMAIN_FLAG_MULTI_THREAD | SSL_DOMAIN_FLAG_BLOCKING)))
		goto err;
#endif
...
DEF_FUNC(hf_accept_conn)
{
	...
	if (check_pending_match) {
		if (!pending_cb_called || !hello_cb_called) {
			...
			if (!TEST_ptr_eq(pending_ssl_obj,
					 client_hello_ssl_obj)) {
				SSL_free(conn);
				goto err;
			}
...
DEF_FUNC(hf_set_incoming_stream_policy)
{
	...
	if (!TEST_true(SSL_set_incoming_stream_policy(ssl, (int)policy,
						      error_code)))
		goto err;
...
static int check_consistent_want(SSL *s, int ret)
{
	...
	int ok = TEST_true(
		...
		(ec == SSL_ERROR_WANT_CLIENT_HELLO_CB &&
		 w == SSL_CLIENT_HELLO_CB) ||
...
DEF_FUNC(hf_write)
{
	...
	if (!TEST_true(r) || !check_consistent_want(ssl, r) ||
...
DEF_FUNC(hf_write_fail)
{
	...
	if (!TEST_false(ret) || !TEST_true(check_consistent_want(ssl, ret)) ||
...
DEF_FUNC(hf_read_expect)
{
	...
	r = SSL_read_ex(ssl, RT()->tmp_buf + RT()->tmp_buf_offset,
			buf_len - RT()->tmp_buf_offset, &bytes_read);
...
DEF_FUNC(hf_read_fail)
{
	...
	if (!TEST_false(r) || !TEST_true(check_consistent_want(ssl, r)) ||
...
DEF_FUNC(hf_expect_fin)
{
	...
	if (!TEST_true(check_consistent_want(ssl, ret)) || !TEST_false(ret) ||
	    !TEST_size_t_eq(bytes_read, 0))
		goto err;
...
DEF_FUNC(hf_expect_conn_close_info)
{
	...
	if (!TEST_int_eq((int)expect_app,
			 (cc_info.flags & SSL_CONN_CLOSE_FLAG_TRANSPORT) ==
				 0) ||
...
#define OP_UNBIND(name) (OP_PUSH_PZ(#name), OP_FUNC(hf_unbind))

#define OP_SELECT_SSL(slot, name) \
	(OP_PUSH_U64(slot), OP_PUSH_PZ(#name), OP_FUNC(hf_select_ssl))

#define OP_CLEAR_SLOT(slot) (OP_PUSH_U64(slot), OP_FUNC(hf_clear_slot))

#define OP_CONNECT_WAIT(name) (OP_SELECT_SSL(0, name), OP_FUNC(hf_connect_wait))

#define OP_LISTEN(name) (OP_SELECT_SSL(0, name), OP_FUNC(hf_listen))
...
#define OP_NEW_SSL_D(name) \
	(OP_PUSH_PZ(#name), OP_PUSH_U64(3), OP_FUNC(hf_new_ssl))

#define OP_NEW_SSL_L_LISTEN(name) (OP_NEW_SSL_L(name), OP_LISTEN(name))

#define OP_NEW_SSL_L_FROM(domain_name, listener_name, flags)        \
	(OP_SELECT_SSL(0, domain_name), OP_PUSH_PZ(#listener_name), \
	 OP_PUSH_U64(flags), OP_FUNC(hf_new_ssl_listener_from))
...
#define OP_SET_PEER_ADDR_FROM(dst_name, src_name)                \
	(OP_SELECT_SSL(0, dst_name), OP_SELECT_SSL(1, src_name), \
	 OP_FUNC(hf_set_peer_addr_from))

#define OP_SIMPLE_PAIR_CONN()                                                  \
	(OP_NEW_SSL_L_LISTEN(L), OP_NEW_SSL_C(C), OP_SET_PEER_ADDR_FROM(C, L), \
	 OP_CONNECT_WAIT(C))

#define OP_SIMPLE_PAIR_CONN_D()                                \
	(OP_NEW_SSL_D(Ds), OP_NEW_SSL_L_FROM_LISTEN(Ds, L, 0), \
	 OP_NEW_SSL_C(C), OP_SET_PEER_ADDR_FROM(C, L), OP_CONNECT_WAIT(C))
...
#define OP_NEW_STREAM(conn_name, stream_name, flags)            \
	(OP_SELECT_SSL(0, conn_name), OP_PUSH_PZ(#stream_name), \
...
#define OP_ACCEPT_STREAM_NONE(conn_name) \
	(OP_SELECT_SSL(0, conn_name), OP_FUNC(hf_accept_stream_none))

#define OP_ACCEPT_CONN_WAIT(listener_name, conn_name, flags)      \
	(OP_SELECT_SSL(0, listener_name), OP_PUSH_PZ(#conn_name), \
	 OP_PUSH_U64(flags), OP_FUNC(hf_accept_conn))
...
#define OP_ACCEPT_CONN_NONE(listener_name) \
	(OP_SELECT_SSL(0, listener_name), OP_FUNC(hf_accept_conn_none))
...
#define OP_WRITE(name, buf, buf_len) \
	(OP_SELECT_SSL(0, name), OP_PUSH_BUFP(buf, buf_len), OP_FUNC(hf_write))

#define OP_WRITE_RAND(name, buf_len) \
	(OP_SELECT_SSL(0, name), OP_PUSH_SIZE(buf_len), OP_FUNC(hf_write_rand))

#define OP_WRITE_B(name, buf) OP_WRITE(name, (buf), sizeof(buf))

#define OP_WRITE_EX2(name, buf, buf_len, flags)              \
	(OP_SELECT_SSL(0, name), OP_PUSH_BUFP(buf, buf_len), \
	 OP_PUSH_U64(flags), OP_FUNC(hf_write_ex2))

#define OP_WRITE_FAIL(name) (OP_SELECT_SSL(0, name), OP_FUNC(hf_write_fail))

#define OP_CONCLUDE(name) (OP_SELECT_SSL(0, name), OP_FUNC(hf_conclude))

#define OP_READ_EXPECT(name, buf, buf_len)                   \
	(OP_SELECT_SSL(0, name), OP_PUSH_BUFP(buf, buf_len), \
	 OP_FUNC(hf_read_expect))

#define OP_READ_EXPECT_B(name, buf) OP_READ_EXPECT(name, (buf), sizeof(buf))

#define OP_READ_FAIL() \
	(OP_SELECT_SSL(0, name), OP_PUSH_U64(0), OP_FUNC(hf_read_fail))
...
#define OP_EXPECT_FIN(name) (OP_SELECT_SSL(0, name), OP_FUNC(hf_expect_fin))

#define OP_EXPECT_CONN_CLOSE_INFO(name, error_code, expect_app, expect_remote) \
	(OP_SELECT_SSL(0, name), OP_PUSH_U64(expect_app),                      \
	 OP_PUSH_U64(expect_remote), OP_PUSH_U64(error_code),                  \
	 OP_FUNC(hf_expect_conn_close_info))

#define OP_WAIT_FOR_DATA(name) \
	(OP_SELECT_SSL(0, name), OP_FUNC(hf_wait_for_data))
...
#define OP_EXPECT_SSL_ERR(name, expected)               \
	(OP_SELECT_SSL(0, name), OP_PUSH_U64(expected), \
	 OP_FUNC(hf_expect_ssl_err))

#define OP_EXPECT_STREAM_ID(expected) \
	(OP_PUSH_U64(expected), OP_FUNC(hf_expect_stream_id))

#define OP_SKIP_TIME(ms) (OP_PUSH_U64(ms), OP_FUNC(hf_skip_time))

#define OP_SLEEP(ms) (OP_PUSH_U64(ms), OP_FUNC(hf_sleep))
