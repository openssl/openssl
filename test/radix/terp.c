...
#define DEF_SCRIPT(name, desc)                                           \
	...
	static SCRIPT_INFO script_info_##name = { #name, desc, __FILE__, \
						  __LINE__,              \
						  script_gen_##name };   \
	static void script_gen_##name(GEN_CTX *ctx)
...
#define OPGEN(n) (opgen_set_line(ctx, __FILE__, __LINE__), opgen_##n)
	...
	if (!ok) {
		if (gctx.error)
			TEST_error("script generation failed: %s (at %s:%d)",
				   gctx.first_error_msg, gctx.first_error_file,
...
#define PRINT_OPC(name) \
	print_opc(bio, i, (size_t)(opc_start - srdr->beg), #name)
	...
	case OPK_PUSH_U64: {
		...
		GET_OPERAND(srdr, v);
		PRINT_OPC(PUSH_U64);
		BIO_printf(bio, "%#20llx (%llu)", (unsigned long long)v,
...
{
	if (error)
		TEST_error("%s: script '%s' (%s)", msg, script_info->name,
			   script_info->desc);
	else
		TEST_info("%s: script '%s' (%s)", msg, script_info->name,
			  script_info->desc);
}
...
static int TERP_init(TERP *terp, const TERP_CONFIG *cfg,
		     const SCRIPT_INFO *script_info,
		     const GEN_SCRIPT *gen_script)
{
...
static ossl_inline int TERP_stk_push(TERP *terp, const void *buf,
				     size_t buf_len)
{
...
{
	test_output_memory(header, terp->stk_cur,
...
static int TERP_execute(TERP *terp)
{
	...
	SRDR_init(&terp->srdr, terp->gen_script->buf,
		  terp->gen_script->buf_len);

	terp->start_time = TERP_now(terp);
	terp->deadline_time =
		ossl_time_add(terp->start_time, terp->cfg.max_execution_time);
	...
	for (;;) {
		if (terp->log_execute) {
			...
			if (!TEST_true(SRDR_print_one(&srdr_copy, debug_bio,
						      SIZE_MAX, NULL)))
				goto err;
...
spin_again:
		if (ossl_time_compare(TERP_now(terp), terp->deadline_time) >=
		    0) {
		...
		if (terp->cfg.per_op_cb != NULL)
			if (!TEST_true(terp->cfg.per_op_cb(
				    terp, terp->cfg.per_op_cb_arg))) {
				TEST_error(
					"pre-operation processing failed at op %zu",
					op_num);
				if (terp->log_execute)
					TERP_log_spin(terp, spin_count);
				goto err;
		...
		case OPK_FUNC: {
			...
			if (terp->fctx.skip_rest) {
				...
				if (ret != 1) {
					TEST_error(
						"op %zu (FUNC %s) failed with return value %d",
						op_num, (const char *)f_name,
						ret);
					goto err;
				}
			}
...
err:
	...
	if (!ok) {
		TEST_error(
			"FAILED while executing script: %s at op %zu, error stack:",
...
static int TERP_run(SCRIPT_INFO *script_info, TERP_CONFIG *cfg)
{
	...
	SCRIPT_INFO_print(script_info, debug_bio, /*error=*/0,
			  "generating script");
