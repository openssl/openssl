...
static int RADIX_PROCESS_init(RADIX_PROCESS *rp, size_t node_idx,
			      size_t process_idx)
{
	...
	if (!TEST_ptr(rp->objs =
			      lh_RADIX_OBJ_new(RADIX_OBJ_hash, RADIX_OBJ_cmp)))
		goto err;
...
static void report_ssl_state(BIO *bio, const char *pfx, int is_write, int state,
			     uint64_t ec)
{
	const char *state_s = stream_state_to_str(state);

	BIO_printf(bio, "%s%-15s%s(%d)", pfx,
		   is_write ? "Write state: " : "Read state: ", state_s, state);
	if (ec != UINT64_MAX)
...
static void report_ssl(SSL *ssl, BIO *bio, const char *pfx)
{
	...
	if (is_quic && is_conn &&
		...
		BIO_printf(bio,
			   ...
			   "%s, %s, reason: \"%s\"\n",
			   pfx, e_str, (unsigned long long)cc_info.error_code,
			   f_str, (unsigned long long)cc_info.frame_type,
...
static void RADIX_PROCESS_report_thread_results(RADIX_PROCESS *rp, BIO *bio)
{
	...
	for (i = 1; i < sk_RADIX_THREAD_num(rp->threads); ++i) {
		...
		BIO_printf(bio,
			   ...
			   rp->node_idx, rp->process_idx, rt->thread_idx,
			   rt->thread_idx);

		BIO_snprintf(pfx_buf, sizeof(pfx_buf), "#  -T-%2zu:\t# ",
			     rt->thread_idx);
		...
		BIO_printf(bio,
			   "==> Child thread with index %zu exited with %d\n",
...
static int RADIX_PROCESS_set_obj(RADIX_PROCESS *rp, const char *name,
				 RADIX_OBJ *obj)
{
	RADIX_OBJ *existing;
...
static int bindings_process_init(size_t node_idx, size_t process_idx)
{
	...
	if (!TEST_true(
		    RADIX_PROCESS_init(&radix_process, node_idx, process_idx)))
		return 0;
...
DEF_FUNC(hf_spawn_thread)
{
	...
	if (!TEST_ptr(child_rt->t = ossl_crypto_thread_native_start(
			      RADIX_THREAD_worker_main, child_rt, 1))) {
