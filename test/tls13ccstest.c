		...
		if (rectype == SSL3_RT_HANDSHAKE) {
			...
			if (msgtype == SSL3_MT_CLIENT_HELLO) {
				...
				if (!PACKET_forward(&msgbody, 34) ||
				    !PACKET_get_length_prefixed_1(&msgbody,
								  &sessionid))
					return 0;
				...
				if (chseen == 1) {
					...
					if (!PACKET_copy_bytes(&sessionid,
							       chsessid,
							       chsessidlen))
						return 0;
				} else {
...
static int test_tls13ccs(int tst)
{
	...
	if (tst >= 6) {
		...
		if (!TEST_true(create_ssl_objects(sctx, cctx, &sssl, &cssl,
						  NULL, NULL)) ||
		    !TEST_true(
			    create_ssl_connection(sssl, cssl, SSL_ERROR_NONE)))
			goto err;
     ...
     * correctly.
     */
	if (!TEST_false(badccs) || !TEST_false(badvers) ||
