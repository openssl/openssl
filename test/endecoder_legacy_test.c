...
static int test_membio_str_eq(BIO *bio_provided, BIO *bio_legacy)
{
	...
	return TEST_long_ge(len_legacy, 0) && TEST_long_ge(len_provided, 0) &&
	       TEST_strn2_eq(str_provided, len_provided, str_legacy,
			     len_legacy);
}
			      ...
			      const char *structure)
{
	...
	if (!TEST_ptr(ectx = OSSL_ENCODER_CTX_new_for_pkey(provided_pkey,
							   selection, "PEM",
							   structure, NULL)) ||
	    !TEST_true(OSSL_ENCODER_to_bio(ectx, membio_provided)) ||
	    !TEST_true(pem_write_bio(membio_legacy, legacy_key, NULL, NULL, 0,
				     NULL, NULL)) ||
	...
	if (pem_read_bio != NULL) {
		...
		if (!TEST_ptr(decoded_legacy_pkey = EVP_PKEY_new()) ||
		    !TEST_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(
				      &decoded_provided_pkey, "PEM", structure,
				      keytype, selection, NULL, NULL)) ||
		    !TEST_true(OSSL_DECODER_from_bio(dctx, membio_provided)) ||
		    !TEST_ptr(decoded_legacy_key = pem_read_bio(
				      membio_legacy, NULL, NULL, NULL)) ||
		    ...
		    int selection, const char *structure)
{
	...
	if (!TEST_ptr(ectx = OSSL_ENCODER_CTX_new_for_pkey(provided_pkey,
							   selection, "DER",
							   structure, NULL)) ||
	    !TEST_true(OSSL_ENCODER_to_data(ectx, &der_provided,
					    &der_provided_len)) ||
	...
	if (d2i != NULL) {
		...
		if (!TEST_ptr(decoded_legacy_pkey = EVP_PKEY_new()) ||
		    !TEST_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(
				      &decoded_provided_pkey, "DER", structure,
				      keytype, selection, NULL, NULL)) ||
	...
	if (test_stanza->pem_write_bio_params != NULL) {
		...
		for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
			...
			if (!test_unprotected_PEM(
				    ...
				    EVP_PKEY_print_params, pkey, selection,
				    structure))
				ok = 0;
		}
	}
	...
	if (test_stanza->i2d_params != NULL) {
		...
		for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
			...
			TEST_info(
				...
				test_stanza->keytype, structure);
			if (!test_DER(key->keytype, key->evp_type, legacy_obj,
				      test_stanza->i2d_params,
				      test_stanza->d2i_params,
				      EVP_PKEY_parameters_eq,
				      EVP_PKEY_print_params, pkey, selection,
				      structure))
				ok = 0;
...
int setup_tests(void)
{
	...
	for (i = 0; i < OSSL_NELEM(keys); i++) {
		...
		if (strcmp(keys[i].keytype, "DH") == 0) {
			if (!TEST_ptr(keys[i].key = load_pkey_pem(
					      test_get_argument(1), NULL)))
				return 0;
			continue;
		}
#endif
		...
		if (strcmp(keys[i].keytype, "RSA") == 0) {
			if (!TEST_ptr(keys[i].key = load_pkey_pem(
					      test_get_argument(0), NULL)))
				return 0;
			continue;
		}
#endif
		TEST_info("Generating %s key...", keys[i].keytype);
		if (!TEST_ptr(keys[i].key = make_key(keys[i].keytype,
						     keys[i].template_params)))
			return 0;
	}
