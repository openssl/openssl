...
static int test_asid(void)
{
	...
	ASIdentifiers *asid1 = ASIdentifiers_new(),
		      *asid2 = ASIdentifiers_new(),
		      *asid3 = ASIdentifiers_new(),
		      *asid4 = ASIdentifiers_new();
	int testresult = 0;
	...
	if (!TEST_true(X509v3_asid_add_id_or_range(asid1, V3_ASID_ASNUM, val1,
						   NULL)))
		goto err;
	...
	if (!TEST_true(X509v3_asid_add_id_or_range(asid2, V3_ASID_ASNUM, val2,
						   NULL)))
		goto err;
     ...
     */
	if (!TEST_true(X509v3_asid_add_id_or_range(asid3, V3_ASID_ASNUM, val1,
						   val2)))
		goto err;
	val1 = val2 = NULL;
...
static int check_addr(IPAddrBlocks *addr, int type)
{
	...
	if (!TEST_int_eq(sk_IPAddressOrRange_num(
				 fam->ipAddressChoice->u.addressesOrRanges),
			 1))
		return 0;

	aorr = sk_IPAddressOrRange_value(
		fam->ipAddressChoice->u.addressesOrRanges, 0);
	if (!TEST_ptr(aorr))
		return 0;
...
static int test_ext_syntax(void)
{
	...
	for (i = 0; i < OSSL_NELEM(extvalues); i++) {
		...
		if (!TEST_long_gt(NCONF_load_bio(conf, extbio, &eline), 0)) {
			...
			if (extvalues[i].pass) {
				if (!TEST_true(X509V3_EXT_add_nconf(
					    conf, &ctx, "default", NULL))) {
					TEST_info("Value: %s",
						  extvalues[i].value);
					testresult = 0;
				}
				...
				ERR_set_mark();
				if (!TEST_false(X509V3_EXT_add_nconf(
					    conf, &ctx, "default", NULL))) {
					testresult = 0;
					TEST_info("Value: %s",
						  extvalues[i].value);
...
static int test_addr_subset(void)
{
	...
	for (i = 0; i < sz; ++i) {
		...
		if (!TEST_ptr(addr[i] = sk_IPAddressFamily_new_null()) ||
		    ...
		    !TEST_true(X509v3_addr_add_range(addr[i], ranges[i].afi,
						     NULL, ip1[i]->data,
						     ip2[i]->data)))
			goto end;
	}
