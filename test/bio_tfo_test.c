...
static int test_bio_tfo(int idx)
{
	...
	if (BIO_do_connect(cbio) <= 0) {
		...
		if (sockerr == EOPNOTSUPP) {
			BIO_printf(
				bio_err,
				"Skip: TFO not enabled/supported for client\n");
			goto success;
		} else if (sockerr != EINPROGRESS) {
			BIO_printf(bio_err,
				   "Error: failed without EINPROGRESSn");
			goto err;
		}
	}
	...
	if (BIO_do_accept(abio) <= 0) {
		...
		if (idx == 0)
			BIO_printf(
				bio_err,
				"Success: non-TFO connection accepted without data\n");
		else if (idx == 1)
			BIO_printf(
				bio_err,
				"Ignore: connection accepted before data, possibly no TFO cookie, or TFO may not be enabled\n");
		else if (idx == 4)
			BIO_printf(
				bio_err,
	...
	if (!TEST_true(
		    BIO_write_ex(cbio, SOCKET_DATA, SOCKET_DATA_LEN, &bytes))) {
		sockerr = get_last_socket_error();
		goto err;
	}
	...
	if (!TEST_ptr(sbio = BIO_pop(abio)) ||
	    !TEST_true(BIO_read_ex(sbio, read_buffer, sizeof(read_buffer),
				   &bytes)) ||
...
static int test_fd_tfo(int idx)
{
	...
	case AF_INET6:
		...
		addrlen =
			sizeof(((struct sockaddr_in6 *)ai->ai_addr)->sin6_addr);
	...
	if (!TEST_ptr(baddr = BIO_ADDR_new()) ||
	    !TEST_true(BIO_ADDR_rawmake(baddr, ai->ai_family, addr, addrlen,
					port)))
		goto err;

	/* ACCEPT SOCKET */

	if (!TEST_int_ge(afd = BIO_socket(ai->ai_family, ai->ai_socktype,
					  ai->ai_protocol, 0),
			 0) ||
	...
	if (!TEST_int_ge(getsockname(afd, (struct sockaddr *)&sstorage, &slen),
			 0))
		goto err;
	...
	if (!TEST_true(BIO_ADDR_rawmake(baddr, sstorage.ss_family, addr,
					addrlen, port)))
		goto err;

	/* CLIENT SOCKET */
	if (!TEST_int_ge(cfd = BIO_socket(ai->ai_family, ai->ai_socktype,
					  ai->ai_protocol, 0),
			 0))
		goto err;
	...
	if (!BIO_connect(cfd, baddr, client_flags)) {
		...
		if (sockerr == EOPNOTSUPP) {
			...
			if (sockerr != EINPROGRESS) {
				BIO_printf(
					bio_err,
					"Error: failed without EINPROGRESS\n");
				goto err;
			}
		}
	}
	...
	if (!TEST_int_ge(sendto(cfd, SOCKET_DATA, SOCKET_DATA_LEN,
				OSSL_TFO_SENDTO, (struct sockaddr *)&sstorage,
				slen),
			 0)) {
		sockerr = get_last_socket_error();
		goto err;
