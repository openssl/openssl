...
static int execute_CTX_reinit_test(OSSL_CMP_CTX_TEST_FIXTURE *fixture)
{
	...
	ctx->status = 1;
	ctx->failInfoCode = 1;
	if (!ossl_cmp_ctx_set0_statusString(ctx,
	...
	if (!TEST_true(ctx->status == -1 && ctx->failInfoCode == -1 &&
		       ctx->statusString == NULL && ctx->newCert == NULL &&
		       ctx->newChain == NULL && ctx->caPubs == NULL &&
		       ctx->extraCertsIn == NULL &&
		       ctx->validatedSrvCert == NULL &&
		       ctx->transactionID == NULL && ctx->senderNonce == NULL &&
		       ctx->recipNonce == NULL))
		goto err;
...
static int test_CTX_libctx_propq(void)
{
	...
	int res = TEST_ptr(libctx) && TEST_ptr(cmpctx) &&
...
#define STR509                                    \
	STR64 STR64 STR64 STR64 STR64 STR64 STR64 \
...
static int execute_CTX_print_errors_test(OSSL_CMP_CTX_TEST_FIXTURE *fixture)
{
	...
	if (!TEST_true(ctx->log_cb == msg_total_size_log_cb)) {
		...
		ossl_cmp_add_error_data(
			"data1"); /* should prepend separator ":" */
		expected_size += (int)strlen(":"
					     "data1");
		ossl_cmp_add_error_data(
			"data2"); /* should prepend separator " : " */
		expected_size += (int)strlen(" : "
					     "data2");
		ossl_cmp_add_error_line(
			"new line"); /* should prepend separator "\n" */
		expected_size += (int)strlen("\n"
					     "new line");
		...
		while (expected_size < 4096) { /* force split */
			...
			expected_size += (int)(strlen(STR_SEP) +
					       strlen(max_str_literal));
		}
...
static int
execute_CTX_reqExtensions_have_SAN_test(OSSL_CMP_CTX_TEST_FIXTURE *fixture)
{
	OSSL_CMP_CTX *ctx = fixture->ctx;
	const int len = 16;
	...
	if (!TEST_ptr(ext) || !TEST_ptr(exts = sk_X509_EXTENSION_new_null()) ||
		       ...
		       OSSL_CMP_severity level, const char *msg)
{
	test_log_cb_res =
		...
		(TEST_int_eq(line, test_log_line_start) ||
		 TEST_int_eq(line, 0) ||
...
static int execute_cmp_ctx_log_cb_test(OSSL_CMP_CTX_TEST_FIXTURE *fixture)
{
	...
	if (!TEST_true(OSSL_CMP_CTX_set_log_cb(ctx, NULL))) {
		...
		ossl_cmp_err(ctx,
			     "this should be printed as CMP error message");
		ossl_cmp_warn(ctx,
			      "this should be printed as CMP warning message");
		ossl_cmp_debug(ctx, "this should not be printed");
		TEST_true(OSSL_CMP_CTX_set_log_verbosity(ctx,
							 OSSL_CMP_LOG_DEBUG));
		ossl_cmp_debug(ctx,
			       "this should be printed as CMP debug message");
		TEST_true(
			OSSL_CMP_CTX_set_log_verbosity(ctx, OSSL_CMP_LOG_INFO));
	}
...
#define DEFINE_SET_GET_BASE_TEST(PREFIX, SETN, GETN, DUP, FIELD, TYPE, ERR,             \
	...
	{                                                                               \
		...
		int (*set_fn)(CMP_CTX * ctx, TYPE) =                                    \
			(int (*)(CMP_CTX * ctx, TYPE))                                  \
				PREFIX##_##SETN##_##FIELD;                              \
		/* need type cast in above assignment as TYPE arg sometimes is const */ \
		TYPE (*get_fn)(const CMP_CTX *ctx) =                                    \
			OSSL_CMP_CTX_##GETN##_##FIELD;                                  \
		...
		if (!TEST_int_eq(ERR_peek_error(), 0))                                  \
			res = 0;                                                        \
		if (PREFIX ==                                                           \
		    1) { /* exported setter functions must test ctx == NULL */          \
			if ((*set_fn)(NULL, val1) || ERR_peek_error() == 0) {           \
				TEST_error(                                             \
					"setter did not return error on ctx == NULL");  \
				res = 0;                                                \
			}                                                               \
		}                                                                       \
		ERR_clear_error();                                                      \
		...
		if ((*get_fn)(NULL) != ERR || ERR_peek_error() == 0) {                  \
			TEST_error(                                                     \
				"getter did not return error on ctx == NULL");          \
			res = 0;                                                        \
		}                                                                       \
		ERR_clear_error();                                                      \
                                                                                        \
		...
		if (SETN == 0) {                                                        \
			if (val1_read != val1) {                                        \
				TEST_error(                                             \
					"set/get first value did not match");           \
				res = 0;                                                \
			}                                                               \
		} else {                                                                \
		...
		if (SETN == 0 && GETN == 0) {                                           \
			if (val2_read != val2) {                                        \
				TEST_error(                                             \
					"set/get second value did not match");          \
				res = 0;                                                \
			}                                                               \
		} else {                                                                \
			if (DUP && val2_read == val2) {                                 \
				TEST_error(                                             \
					"second set did not dup the value");            \
				val2_read = 0;                                          \
				res = 0;                                                \
			}                                                               \
			if (val2 == val1) {                                             \
             ...
             */                                                    \
				TEST_error(                                             \
					"second get returned same as first get");       \
				res = 0;                                                \
			}                                                               \
		}                                                                       \
                                                                                        \
		...
		if (GETN == 0) {                                                        \
			...
			if (DUP && val3_read == val2_read) {                            \
				TEST_error(                                             \
					"third get did not create a new dup");          \
				val3_read = 0;                                          \
				res = 0;                                                \
			}                                                               \
		}                                                                       \
	...
	{                                                                               \
		SETUP_TEST_FIXTURE(OSSL_CMP_CTX_TEST_FIXTURE, set_up);                  \
		EXECUTE_TEST(execute_CTX_##SETN##_##GETN##_##FIELD,                     \
			     tear_down);                                                \
		return result;                                                          \
	}
...
#define DEFINE_SET_TEST_DEFAULT(OSSL_CMP, CTX, N, DUP, FIELD, TYPE, DEFAULT) \
	...
	DEFINE_SET_GET_TEST_DEFAULT(OSSL_CMP, CTX, N, 0, DUP, FIELD, TYPE,   \
				    DEFAULT)
...
#define DEFINE_SET_CB_TEST(FIELD)                                  \
	static OSSL_CMP_##FIELD##_t OSSL_CMP_CTX_get_##FIELD(      \
		const CMP_CTX *ctx)                                \
	{                                                          \
		RET_IF_NULL_ARG(ctx, NULL);                        \
		return ctx->FIELD;                                 \
...
#define DEFINE_SET_GET_INT_TEST_DEFAULT(OSSL_CMP, CTX, FIELD, DEFAULT)      \
	DEFINE_SET_GET_BASE_TEST(OSSL_CMP##_##CTX, set, get, 0, FIELD, int, \
				 -1, DEFAULT, 1, DROP)
...
#define DEFINE_SET_GET1_STR_FN(SETN, FIELD)                                 \
	static int OSSL_CMP_CTX_##SETN##_##FIELD##_str(CMP_CTX *ctx,        \
						       char *val)           \
	{                                                                   \
		return OSSL_CMP_CTX_##SETN##_##FIELD(                       \
			ctx, (unsigned char *)val, (int)strlen(val));       \
	}                                                                   \
                                                                            \
	...
	{                                                                   \
		...
		return bytes == NULL ? NULL :                               \
				       OPENSSL_strndup((char *)bytes->data, \
						       bytes->length);      \
	}
...
#define push0 0
#define push1 1
#define DEFINE_PUSH_BASE_TEST(PUSHN, DUP, FIELD, ELEM, TYPE, T, DEFAULT, NEW,             \
			      FREE)                                                       \
	static TYPE sk_top_##FIELD(const CMP_CTX *ctx)                                    \
	{                                                                                 \
		return sk_##T##_value(ctx->FIELD,                                         \
				      sk_##T##_num(ctx->FIELD) - 1);                      \
	}                                                                                 \
                                                                                          \
	static int execute_CTX_##PUSHN##_##ELEM(                                          \
		OSSL_CMP_CTX_TEST_FIXTURE *fixture)                                       \
	{                                                                                 \
		CMP_CTX *ctx = fixture->ctx;                                              \
		int (*push_fn)(CMP_CTX * ctx, TYPE) =                                     \
			(int (*)(CMP_CTX * ctx, TYPE))                                    \
				OSSL_CMP_CTX_##PUSHN##_##ELEM;                            \
		/* \
     ...
     */                                                                      \
		...
		if ((*push_fn)(NULL, val1) || ERR_peek_error() == 0) {                    \
			TEST_error(                                                       \
				"pusher did not return error on ctx == NULL");            \
			res = 0;                                                          \
		}                                                                         \
		ERR_clear_error();                                                        \
                                                                                          \
		...
		if (!DEFAULT(field_read)) {                                               \
			TEST_error(                                                       \
				"did not get default value for stack field");             \
			res = 0;                                                          \
		}                                                                         \
		if (!(*push_fn)(ctx, val1)) {                                             \
		...
		if (sk_##T##_num(ctx->FIELD) != ++n_elem) {                               \
			TEST_error(                                                       \
				"pushing first value did not increment number");          \
			res = 0;                                                          \
		}                                                                         \
		...
		if (PUSHN == 0) {                                                         \
			if (val1_read != val1) {                                          \
				TEST_error(                                               \
					"push/sk_top first value did not match");         \
				res = 0;                                                  \
			}                                                                 \
		} else {                                                                  \
			if (DUP && val1_read == val1) {                                   \
				TEST_error(                                               \
					"first push did not dup the value");              \
				res = 0;                                                  \
			}                                                                 \
		}                                                                         \
                                                                                          \
		...
		if (sk_##T##_num(ctx->FIELD) != ++n_elem) {                               \
			TEST_error(                                                       \
				"pushing second value did not increment number");         \
			res = 0;                                                          \
		}                                                                         \
		...
		if (PUSHN == 0) {                                                         \
			if (val2_read != val2) {                                          \
				TEST_error(                                               \
					"push/sk_top second value did not match");        \
				res = 0;                                                  \
			}                                                                 \
		} else {                                                                  \
			if (DUP && val2_read == val2) {                                   \
				TEST_error(                                               \
					"second push did not dup the value");             \
				res = 0;                                                  \
			}                                                                 \
			if (val2 == val1) {                                               \
				TEST_error(                                               \
...
#define DEFINE_PUSH_TEST(N, DUP, FIELD, ELEM, TYPE)                          \
	DEFINE_PUSH_BASE_TEST(push##N, DUP, FIELD, ELEM, TYPE *, TYPE, IS_0, \
			      TYPE##_new(), TYPE##_free)
...
DEFINE_SET_GET_BASE_TEST(OSSL_CMP_CTX, set0, get0, 0, trusted, X509_STORE *,
