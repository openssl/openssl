...
#define ITS_TRADITION_THAT_MAKES_IT_OK
    ...
    min_tm.tm_year -= 1;
    /* This should now be outside the asn1 time range. */
    expected_days += 365; /* year -1 is not a leap year.. lol */

    /*
     * We could choose not to support these if we validated our inputs..
     * but testing doesn't lie, we currently do not care.
     */
    if (TEST_true(OPENSSL_gmtime_diff(&pday, &psec, &min_tm, &max_tm))) {
        pd = pday;
        ps = psec;
    ...
    max_tm.tm_year += 1;
    expected_days += 366; /* Year 10000 *is* divisible by 400, so is a leap year */
