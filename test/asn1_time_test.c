...
static ASN1_TIME gtime = { 15, V_ASN1_GENERALIZEDTIME,
			   (unsigned char *)"19991231000000Z", 0 };
...
static int test_table(struct testdata *tbl, int idx)
{
	...
	if (!TEST_true(ASN1_TIME_diff(&day, &sec, &gtime, &atime))) {
		...
		error = 1;
	} else if (!((td->cmp_result == 0 &&
		      TEST_true((day == 0 && sec == 0))) ||
	...
	if (!TEST_int_eq(ASN1_TIME_cmp_time_t(&atime, gtime_t),
			 td->cmp_result)) {
	...
	if (!TEST_ptr(ptime)) {
		...
		if (!TEST_int_eq(ptime->type, td->expected_type)) {
			TEST_info("ASN1_TIME_set(%ld) unexpected type",
				  (long)td->t);
			local_error = error = 1;
		}
		if (local_error)
			TEST_info("ASN1_TIME_set() = %*s", ptime->length,
				  ptime->data);
	...
	if (!TEST_ptr(ptime)) {
		...
		if (!TEST_int_eq(ASN1_TIME_set_string(ptime, td->data),
				 td->check_result)) {
			TEST_info("ASN1_TIME_set_string_gmt(%s) failed",
				  td->data);
			local_error = error = 1;
		}
		if (!TEST_int_eq(ASN1_TIME_normalize(ptime),
				 td->check_result)) {
		...
		day = sec = 0;
		if (!TEST_true(ASN1_TIME_diff(&day, &sec, ptime, &atime)) ||
		    !TEST_int_eq(day, 0) || !TEST_int_eq(sec, 0)) {
			TEST_info(
				"ASN1_TIME_diff(day=%d, sec=%d, %s) after ASN1_TIME_set_string_gmt() failed",
				day, sec, td->data);
			local_error = error = 1;
		}
		if (!TEST_int_eq(ASN1_TIME_cmp_time_t(ptime, gtime_t),
				 td->cmp_result)) {
			TEST_info(
	...
	if (!TEST_ptr(ptime)) {
		...
		if (local_error)
			TEST_info("ASN1_TIME_set_string() = %*s", ptime->length,
				  ptime->data);
		ASN1_TIME_free(ptime);
	}
	...
	if (td->type == V_ASN1_UTCTIME) {
		...
		if (ptime != NULL &&
		    !TEST_int_eq(ASN1_TIME_cmp_time_t(ptime, td->t), 0)) {
			TEST_info(
...
#define TOMORROW_GEN                                            \
	{ sizeof(TOMORROW_GEN_STR) - 1, V_ASN1_GENERALIZEDTIME, \
	  TOMORROW_GEN_STR, 0 }
#define TODAY_UTC \
...
static struct compare_testdata tbl_compare_testdata[] = {
	{ TODAY_GEN, TODAY_GEN, 0 },	   { TODAY_GEN, TODAY_UTC, 0 },
	{ TODAY_GEN, TOMORROW_GEN, -1 },   { TODAY_GEN, TOMORROW_UTC, -1 },

	{ TODAY_UTC, TODAY_GEN, 0 },	   { TODAY_UTC, TODAY_UTC, 0 },
	{ TODAY_UTC, TOMORROW_GEN, -1 },   { TODAY_UTC, TOMORROW_UTC, -1 },

	{ TOMORROW_GEN, TODAY_GEN, 1 },	   { TOMORROW_GEN, TODAY_UTC, 1 },
	{ TOMORROW_GEN, TOMORROW_GEN, 0 }, { TOMORROW_GEN, TOMORROW_UTC, 0 },

	{ TOMORROW_UTC, TODAY_GEN, 1 },	   { TOMORROW_UTC, TODAY_UTC, 1 },
	{ TOMORROW_UTC, TOMORROW_GEN, 0 }, { TOMORROW_UTC, TOMORROW_UTC, 0 }
};
