...
static int client_hello_nov12_cb(SSL *s, int *al, void *arg)
{
	...
	if (p == NULL || SSL_client_hello_get0_random(s, &p) == 0 ||
			 ...
			 const unsigned char *in, unsigned int inlen, void *arg)
{
	...
	return TEST_true(ret == OPENSSL_NPN_NEGOTIATED ||
			 ret == OPENSSL_NPN_NO_OVERLAP) ?
		       SSL_TLSEXT_ERR_OK :
		       SSL_TLSEXT_ERR_ALERT_FATAL;
}

static int server_npn_cb(SSL *s, const unsigned char **data, unsigned int *len,
			 void *arg)
{
	CTX_DATA *ctx_data = (CTX_DATA *)(arg);
			  ...
			  unsigned int inlen, void *arg)
{
     ...
     */
	ret = SSL_select_next_proto(&tmp_out, outlen, ctx_data->alpn_protocols,
				    (unsigned int)ctx_data->alpn_protocols_len,
				    in, inlen);
				   ...
				   CTX_DATA *client_ctx_data)
{
	...
	if (!TEST_int_eq(SSL_CTX_set_max_send_fragment(server_ctx,
		...
		goto err;
	if (server2_ctx != NULL) {
		if (!TEST_int_eq(SSL_CTX_set_max_send_fragment(
					 server2_ctx, test->max_fragment_size),
				 1))
			goto err;
	}
	...
	case SSL_TEST_VERIFY_RETRY_ONCE:
		n_retries = 1;
		SSL_CTX_set_cert_verify_callback(client_ctx, &verify_retry_cb,
						 NULL);
		break;
	...
	case SSL_TEST_SERVERNAME_IGNORE_MISMATCH:
		SSL_CTX_set_tlsext_servername_callback(server_ctx,
	...
	case SSL_TEST_SERVERNAME_CLIENT_HELLO_IGNORE_MISMATCH:
		SSL_CTX_set_client_hello_cb(server_ctx, client_hello_ignore_cb,
					    server2_ctx);
		break;
	case SSL_TEST_SERVERNAME_CLIENT_HELLO_REJECT_MISMATCH:
		SSL_CTX_set_client_hello_cb(server_ctx, client_hello_reject_cb,
					    server2_ctx);
		break;
	case SSL_TEST_SERVERNAME_CLIENT_HELLO_NO_V12:
		SSL_CTX_set_client_hello_cb(server_ctx, client_hello_nov12_cb,
					    server2_ctx);
	}

	if (extra->server.cert_status != SSL_TEST_CERT_STATUS_NONE) {
		SSL_CTX_set_tlsext_status_type(client_ctx,
		...
		case SSL_TEST_CERT_STATUS_GOOD_RESPONSE:
			...
			dummy_ocsp_resp = OCSP_response_create(
				OCSP_RESPONSE_STATUS_SUCCESSFUL, NULL);

			SSL_CTX_set_tlsext_status_cb(client_ctx,
						     client_ocsp_cb);
			SSL_CTX_set_tlsext_status_cb(server_ctx,
						     server_ocsp_cb);
	...
	if (server2_ctx != NULL)
		SSL_CTX_set_tlsext_ticket_key_evp_cb(
			server2_ctx, do_not_call_session_ticket_cb);
	...
	if (extra->server2.npn_protocols != NULL) {
		if (!TEST_true(parse_protos(
			    extra->server2.npn_protocols,
	...
	if (extra->server.alpn_protocols != NULL) {
		if (!TEST_true(
			...
			goto err;
		SSL_CTX_set_alpn_select_cb(server_ctx, server_alpn_cb,
					   server_ctx_data);
	}
	if (extra->server2.alpn_protocols != NULL) {
		if (!TEST_ptr(server2_ctx) ||
		    !TEST_true(parse_protos(
			    extra->server2.alpn_protocols,
	...
	if (extra->client.alpn_protocols != NULL) {
		...
		if (!TEST_true(parse_protos(extra->client.alpn_protocols,
		    ...
		    || !TEST_int_eq(SSL_CTX_set_alpn_protos(
	...
	if (extra->server.session_ticket_app_data != NULL) {
		...
		if (!TEST_ptr(server_ctx_data->session_ticket_app_data))
			goto err;
		SSL_CTX_set_session_ticket_cb(server_ctx,
	...
	if (!TEST_ptr(ticket_keys = OPENSSL_zalloc(ticket_key_len)) ||
	    !TEST_long_eq(SSL_CTX_set_tlsext_ticket_keys(
				  server_ctx, ticket_keys, ticket_key_len),
			  1)) {
	...
	if (!configure_handshake_ctx_for_srp(server_ctx, server2_ctx,
					     client_ctx, extra, server_ctx_data,
	...
	if (test->compress_certificates) {
		if (!TEST_true(SSL_CTX_compress_certs(server_ctx, 0)))
			goto err;
		if (server2_ctx != NULL &&
		    !TEST_true(SSL_CTX_compress_certs(server2_ctx, 0)))
			goto err;
	}
#endif
	return 1;
				    ...
				    const SSL_TEST_EXTRA_CONF *extra)
{
	if (extra->client.servername != SSL_TEST_SERVERNAME_NONE)
		SSL_set_tlsext_host_name(
			client, ssl_servername_name(extra->client.servername));
...
static void do_reneg_setup_step(const SSL_TEST_CTX *test_ctx, PEER *peer)
{
	...
	if ((test_ctx->handshake_mode == SSL_TEST_HANDSHAKE_RENEG_SERVER &&
		...
		if (!SSL_renegotiate_pending(peer->ssl)) {
			...
			if (SSL_is_server(peer->ssl)) {
				...
				if (test_ctx->extra.client.reneg_ciphers !=
				    NULL) {
					if (!SSL_set_cipher_list(
						    peer->ssl,
						    test_ctx->extra.client
							    .reneg_ciphers)) {
						peer->status = PEER_ERROR;
						return;
					}
				...
				if (full_reneg)
					...
					ret = SSL_renegotiate_abbreviated(
						peer->ssl);
			}
			if (!ret) {
				peer->status = PEER_ERROR;
	...
	} else if (test_ctx->handshake_mode ==
			   SSL_TEST_HANDSHAKE_KEY_UPDATE_SERVER ||
		   test_ctx->handshake_mode ==
			   SSL_TEST_HANDSHAKE_KEY_UPDATE_CLIENT) {
		if (SSL_is_server(peer->ssl) !=
		...
		if (SSL_is_server(peer->ssl)) {
			SSL_CONNECTION *sc =
				SSL_CONNECTION_FROM_SSL_ONLY(peer->ssl);
					   ...
					   int client_spoke_last)
{
	...
	case PEER_ERROR:
		...
		case PEER_WAITING:
			...
			return client_spoke_last ? CLIENT_ERROR :
						   INTERNAL_ERROR;
		case PEER_SUCCESS:
...
static int pkey_type(EVP_PKEY *pkey)
{
	if (EVP_PKEY_is_a(pkey, "EC")) {
		...
		if (!EVP_PKEY_get_group_name(pkey, name, sizeof(name),
					     &name_len))
			return NID_undef;
...
do_handshake_internal(SSL_CTX *server_ctx, SSL_CTX *server2_ctx,
		      SSL_CTX *client_ctx, const SSL_TEST_CTX *test_ctx,
		      const SSL_TEST_EXTRA_CONF *extra, SSL_SESSION *session_in,
		      SSL_SESSION *serv_sess_in, SSL_SESSION **session_out,
		      SSL_SESSION **serv_sess_out)
{
	PEER server, client;
	...
	if (session_in != NULL) {
		...
		if ((sess_id_len > 0 && !TEST_true(SSL_CTX_add_session(
						server_ctx, serv_sess_in))) ||
	...
	if (!TEST_ptr(client_to_server) || !TEST_ptr(server_to_client))
		goto err;
	...
	if (!TEST_int_ge(ex_data_idx, 0) ||
	    !TEST_int_eq(SSL_set_ex_data(server.ssl, ex_data_idx,
					 &server_ex_data),
			 1) ||
	    !TEST_int_eq(SSL_set_ex_data(client.ssl, ex_data_idx,
					 &client_ex_data),
			 1))
		goto err;
...
err:
	...
	ret->server_num_fatal_alerts_sent =
		server_ex_data.num_fatal_alerts_sent;
	ret->server_alert_received = client_ex_data.alert_received;
	ret->client_alert_sent = client_ex_data.alert_sent;
	ret->client_num_fatal_alerts_sent =
		client_ex_data.num_fatal_alerts_sent;
	...
	ret->session_ticket_do_not_call =
		server_ex_data.session_ticket_do_not_call;
	...
	if ((sess = SSL_get0_session(server.ssl)) != NULL) {
		SSL_SESSION_get0_ticket_appdata(sess, (void **)&tick,
						&tick_len);
		ret->result_session_ticket_app_data =
			OPENSSL_strndup((const char *)tick, tick_len);
	}
			       ...
			       const SSL_TEST_CTX *test_ctx)
{
	...
	result = do_handshake_internal(server_ctx, server2_ctx, client_ctx,
				       test_ctx, &test_ctx->extra, NULL, NULL,
				       &session, &serv_sess);
	if (result == NULL ||
	...
	result = do_handshake_internal(resume_server_ctx, NULL,
				       resume_client_ctx, test_ctx,
				       &test_ctx->resume_extra, session,
				       serv_sess, NULL, NULL);
end:
