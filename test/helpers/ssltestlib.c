...
const BIO_METHOD *bio_s_mempacket_test(void)
{
	if (meth_mem == NULL) {
		if (!TEST_ptr(meth_mem = BIO_meth_new(BIO_TYPE_MEMPACKET_TEST,
						      "Mem Packet Test")) ||
		    !TEST_true(BIO_meth_set_write(meth_mem,
		    ...
		    !TEST_true(BIO_meth_set_create(meth_mem,
						   mempacket_test_new)) ||
		    !TEST_true(BIO_meth_set_destroy(meth_mem,
						    mempacket_test_free)))
			return NULL;
	}
	return meth_mem;
...
static int mempacket_test_read(BIO *bio, char *out, int outl)
{
	...
	if (thispkt->type != INJECT_PACKET_IGNORE_REC_SEQ &&
		...
		for (rem = thispkt->len, rec = thispkt->data; rem > 0;
			...
			if (rem < (int)len)
				return -1;
			if (ctx->droprec == (int)ctx->currrec &&
			    ctx->dropepoch == epoch) {
				if (rem > (int)len)
...
int mempacket_swap_epoch(BIO *bio)
{
	...
	for (rem = thispkt->len, rec = thispkt->data; rem > 0;
		...
		if (epoch != ctx->epoch) {
			...
			if (sk_MEMPACKET_insert(ctx->pkts, thispkt, numpkts) <=
			    0) {
				OPENSSL_free(tmp);
			  ...
			  int type)
{
	...
	for (i = 0; i < (duprec ? 3 : 1); i++) {
		...
		thispkt->num = (pktnum >= 0) ? (unsigned int)pktnum :
					       ctx->lastpkt + i;
		thispkt->type = type;
	}
...
const BIO_METHOD *bio_s_always_retry(void)
{
	if (meth_always_retry == NULL) {
		if (!TEST_ptr(meth_always_retry = BIO_meth_new(
				      BIO_TYPE_ALWAYS_RETRY, "Always Retry")) ||
...
const BIO_METHOD *bio_s_maybe_retry(void)
{
	if (meth_maybe_retry == NULL) {
		if (!TEST_ptr(meth_maybe_retry = BIO_meth_new(
				      BIO_TYPE_MAYBE_RETRY, "Maybe Retry")) ||
			...
			char *certfile, char *privkeyfile)
{
	...
	if (sctx != NULL) {
		if (*sctx != NULL)
			serverctx = *sctx;
		else if (!TEST_ptr(serverctx =
					   SSL_CTX_new_ex(libctx, NULL, sm)) ||
	...
	if (cctx != NULL) {
		if (*cctx != NULL)
			clientctx = *cctx;
		else if (!TEST_ptr(clientctx =
					   SSL_CTX_new_ex(libctx, NULL, cm)))
			goto err;
	}
	...
	if (serverctx != NULL && certfile != NULL && privkeyfile != NULL) {
		if (!TEST_int_eq(SSL_CTX_use_certificate_file(
					 serverctx, certfile, SSL_FILETYPE_PEM),
				 1) ||
...
int create_test_sockets(int *cfdp, int *sfdp, int socktype, BIO_ADDR *saddr)
{
	...
	afd = BIO_socket(AF_INET, socktype,
			 socktype == SOCK_STREAM ? IPPROTO_TCP : IPPROTO_UDP,
			 0);
				  ...
				  int *sm_count)
{
	...
	if (listen) {
		if (!isdtls) {
			TEST_error(
				"DTLSv1_listen requested for non-DTLS object\n");
			return 0;
		}
		peer = BIO_ADDR_new();
		if (!TEST_ptr(peer))
