			       ...
			       const void *buf, size_t len, SSL *ssl, void *arg)
{
	...
	if (content_type == SSL3_RT_QUIC_FRAME_FULL) {
		...
		if (frame_type == OSSL_QUIC_FRAME_TYPE_PING) {
             ...
             */
			(void)BIO_ctrl(noiseargs->cbio, BIO_CTRL_NOISE_BACK_OFF,
				       1, NULL);
			(void)BIO_ctrl(noiseargs->sbio, BIO_CTRL_NOISE_BACK_OFF,
				       1, NULL);
		}
	}
			      ...
			      QTEST_FAULT **fault, BIO **tracebio)
{
	...
	*qtserv = NULL;
	...
	if ((flags & QTEST_FLAG_BLOCK) != 0) {
         ...
         */
		if (!TEST_true(create_test_sockets(&cfd, &sfd, SOCK_DGRAM,
						   peeraddr)))
			goto err;
		...
		if (!TEST_true(BIO_dgram_set_caps(
			    cbio, BIO_DGRAM_CAP_HANDLES_DST_ADDR)) ||
		    !TEST_true(BIO_dgram_set_caps(
			    sbio, BIO_DGRAM_CAP_HANDLES_DST_ADDR)))
			goto err;

		if (!TEST_ptr(localaddr = BIO_ADDR_new()))
			goto err;
		/* Dummy client local addresses */
		if (!TEST_true(BIO_ADDR_rawmake(localaddr, AF_INET, &ina,
						sizeof(ina), htons(0)))) {
			BIO_ADDR_free(localaddr);
			goto err;
		}
		if (!TEST_int_eq(BIO_dgram_set0_local_addr(cbio, localaddr),
				 1)) {
			BIO_ADDR_free(localaddr);
			goto err;
		}
		/* Dummy server address */
		if (!TEST_true(BIO_ADDR_rawmake(peeraddr, AF_INET, &ina,
						sizeof(ina), htons(0))))
			goto err;
	}
	...
	if ((flags & QTEST_FLAG_NOISE) != 0) {
		...
		if ((flags & QTEST_FLAG_FAKE_TIME) != 0) {
			if (!TEST_int_eq(BIO_ctrl(cbio,
						  BIO_CTRL_NOISE_SET_NOW_CB, 0,
						  &now_cb),
					 1))
				goto err;
		}
...
int qtest_wait_for_timeout(SSL *s, QUIC_TSERVER *qtserv)
{
	...
	stimeout =
		ossl_time_subtract(ossl_quic_tserver_get_deadline(qtserv), now);
	...
	do {
		...
		if (qtserv != NULL) {
			...
			for (;;) {
				if (client_ready == 1)
					break;
				ossl_crypto_condvar_wait(client_ready_cond,
							 client_ready_mutex);
			}
			ossl_crypto_mutex_unlock(client_ready_mutex);
#endif
...
{
	return qtest_create_quic_connection_ex(qtserv, clientssl,
					       SSL_ERROR_NONE);
}
...
{
	return qtest_check_server_transport_err(
		qtserv, OSSL_QUIC_ERR_PROTOCOL_VIOLATION);
}

int qtest_check_server_frame_encoding_err(QUIC_TSERVER *qtserv)
{
	return qtest_check_server_transport_err(
		qtserv, OSSL_QUIC_ERR_FRAME_ENCODING_ERROR);
}
			    ...
			    void *arg)
{
	...
	case SSL3_MT_ENCRYPTED_EXTENSIONS: {
		...
		if (!fault->encextcb(fault, &ee, payloadlen,
				     fault->encextcbarg))
			return 0;
	}
...
int qtest_fault_resize_message(QTEST_FAULT *fault, size_t newlen)
{
	...
	if (!qtest_fault_resize_handshake(fault,
					  newlen + SSL3_HM_HEADER_LENGTH))
		return 0;
			    ...
			    uint64_t flags, size_t *num_processed)
{
	...
	if (bdata == NULL || bdata->fault == NULL ||
		...
		return BIO_sendmmsg(next, msg, stride, num_msg, flags,
				    num_processed);
	...
	for (i = 0; i < num_msg; ++i) {
		...
		tmpdata = OPENSSL_malloc(bdata->fault->msg.data_len +
					 GROWTH_ALLOWANCE);
		if (tmpdata == NULL)
			return 0;
		memcpy(tmpdata, bdata->fault->msg.data,
		       bdata->fault->msg.data_len);
		bdata->fault->msg.data = tmpdata;
		bdata->fault->msgalloc =
			bdata->fault->msg.data_len + GROWTH_ALLOWANCE;
...
BIO_METHOD *qtest_get_bio_method(void)
{
	...
	tmp = BIO_meth_new(BIO_TYPE_CIPHER_PACKET_FILTER,
			     ...
			     int noise_rate)
{
	...
	if (!TEST_ptr(sbio) || !TEST_ptr(cbio))
		return 0;
	if (!TEST_int_eq(BIO_ctrl(sbio, BIO_CTRL_NOISE_RATE, noise_rate, NULL),
			 1))
		return 0;
	if (!TEST_int_eq(BIO_ctrl(cbio, BIO_CTRL_NOISE_RATE, noise_rate, NULL),
			 1))
		return 0;
