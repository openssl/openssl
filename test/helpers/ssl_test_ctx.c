...
{
	return enum_name(ssl_test_results, OSSL_NELEM(ssl_test_results),
			 result);
}
...
static const test_enum ssl_protocols[] = {
	{ "TLSv1.3", TLS1_3_VERSION },	 { "TLSv1.2", TLS1_2_VERSION },
	{ "TLSv1.1", TLS1_1_VERSION },	 { "TLSv1", TLS1_VERSION },
	{ "SSLv3", SSL3_VERSION },	 { "DTLSv1", DTLS1_VERSION },
					    ...
					    const char *value)
{
	...
	if (!parse_enum(ssl_servername_callbacks,
			OSSL_NELEM(ssl_servername_callbacks), &ret_value,
			value)) {
		return 0;
	}
...
static const test_enum ssl_test_methods[] = { { "TLS", SSL_TEST_METHOD_TLS },
...
{
	return enum_name(ssl_test_methods, OSSL_NELEM(ssl_test_methods),
			 method);
}
...
IMPLEMENT_SSL_TEST_STRING_OPTION(SSL_TEST_CTX, test,
				 expected_session_ticket_app_data)
				      ...
				      const char *value)
{
	...
	if (!parse_enum(ssl_ct_validation_modes,
			OSSL_NELEM(ssl_ct_validation_modes), &ret_value,
			value)) {
		return 0;
	}
...
IMPLEMENT_SSL_TEST_BOOL_OPTION(SSL_TEST_SERVER_CONF, server,
...
{
	return enum_name(ssl_certstatus, OSSL_NELEM(ssl_certstatus),
			 cert_status);
}
					      ...
					      const char *value)
{
	...
	if (!parse_enum(ssl_max_fragment_len_mode,
			OSSL_NELEM(ssl_max_fragment_len_mode), &ret_value,
			value)) {
		return 0;
	}
...
__owur static int parse_expected_server_ca_names(SSL_TEST_CTX *test_ctx,
						 const char *value)
{
	return parse_expected_ca_names(&test_ctx->expected_server_ca_names,
				       value, test_ctx->libctx);
}
__owur static int parse_expected_client_ca_names(SSL_TEST_CTX *test_ctx,
						 const char *value)
{
	return parse_expected_ca_names(&test_ctx->expected_client_ca_names,
				       value, test_ctx->libctx);
}
	...
	for (i = 0; i < sk_CONF_VALUE_num(sk_conf); i++) {
		...
		for (j = 0; j < OSSL_NELEM(ssl_test_client_options); j++) {
			if (strcmp(option->name,
				   ssl_test_client_options[j].name) == 0) {
				if (!ssl_test_client_options[j].parse(
					    client, option->value)) {
				...
				const char *server_section)
{
	...
	for (i = 0; i < sk_CONF_VALUE_num(sk_conf); i++) {
		...
		for (j = 0; j < OSSL_NELEM(ssl_test_server_options); j++) {
			if (strcmp(option->name,
				   ssl_test_server_options[j].name) == 0) {
				if (!ssl_test_server_options[j].parse(
					    server, option->value)) {
					TEST_info("Bad value %s for option %s",
						  option->value, option->name);
					return 0;
				  ...
				  OSSL_LIB_CTX *libctx)
{
	...
	for (i = 0; i < sk_CONF_VALUE_num(sk_conf); i++) {
		...
		if (strcmp(option->name, "client") == 0) {
			...
			if (!parse_client_options(&ctx->resume_extra.client,
						  conf, option->value))
				goto err;
		} else if (strcmp(option->name, "resume-server") == 0) {
			if (!parse_server_options(&ctx->resume_extra.server,
						  conf, option->value))
				goto err;
		} else if (strcmp(option->name, "resume-server2") == 0) {
			if (!parse_server_options(&ctx->resume_extra.server2,
						  conf, option->value))
				goto err;
		} else {
			for (j = 0; j < OSSL_NELEM(ssl_test_ctx_options); j++) {
				if (strcmp(option->name,
					   ssl_test_ctx_options[j].name) == 0) {
					if (!ssl_test_ctx_options[j].parse(
						    ctx, option->value)) {
						TEST_info(
			...
			if (!found) {
				TEST_info("Unknown test option: %s",
					  option->name);
				goto err;
			}
		}
