			      ...
			      BIO_MSG *msg, size_t num_msg)
{
	...
	for (i = 0; i < limit->num; i++) {
		...
		if (ossl_time_compare(limit->pinfos[idx].timestamp,
				      sampling_start) >= 0)
			break;
	...
	for (i = 0; i < num_msg; ++i) {
		...
		if ((limit->size_sum + pktsize) / SAMPLING_WINDOW_PERIOD >
		    limit->bw) {
			/*
		      ...
		      int *should_drop, uint16_t *flip, size_t *flip_offset)
{
	...
	*should_drop = (type == NOISE_TYPE_DROP || type == NOISE_TYPE_DELAY);
     ...
     */
	*reinject =
		(type == NOISE_TYPE_DUPLICATE || type == NOISE_TYPE_DELAY) ?
	...
	for (i = 0, thismsg = msg; i < msg_cnt;
		...
		if (data->reinject_dgram > 0 &&
			...
			if (msg_cnt < num_msg) {
				...
				BIO_dump_fp(stdout, thismsg->data,
					    thismsg->data_len);
				printf("\n");
#endif
				continue;
		...
		get_noise(
			data->noise_rate,
			/* long header */ (((uint8_t *)thismsg->data)[0] & 0x80) !=
				0,
...
const BIO_METHOD *bio_f_noisy_dgram_filter(void)
{
	if (method_noisy_dgram == NULL) {
		...
		if (method_noisy_dgram == NULL ||
		    ...
		    !BIO_meth_set_sendmmsg(method_noisy_dgram,
					   noisy_dgram_sendmmsg) ||
		    !BIO_meth_set_recvmmsg(method_noisy_dgram,
