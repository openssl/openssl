...
static void generate_p12(PKCS12_BUILDER *pb, const PKCS12_ENC *mac)
{
	...
	if (mac != NULL) {
		...
		if (!TEST_true(PKCS12_set_mac(p12, mac->pass,
					      (int)strlen(mac->pass), NULL, 0,
					      mac->iter, md))) {
			pb->success = 0;
			goto err;
		}
...
static int check_p12_mac(PKCS12 *p12, const PKCS12_ENC *mac)
{
	return TEST_true(PKCS12_mac_present(p12)) &&
	       TEST_true(PKCS12_verify_mac(p12, mac->pass,
					   (int)strlen(mac->pass)));
}
...
void end_contentinfo(PKCS12_BUILDER *pb)
{
	if (pb->success && pb->bags != NULL) {
		if (!TEST_true(
			    PKCS12_add_safe(&pb->safes, pb->bags, -1, 0, NULL)))
			pb->success = 0;
	}
...
void end_contentinfo_encrypted(PKCS12_BUILDER *pb, const PKCS12_ENC *enc)
{
	if (pb->success && pb->bags != NULL) {
		if (legacy) {
			if (!TEST_true(PKCS12_add_safe(&pb->safes, pb->bags,
						       enc->nid, enc->iter,
						       enc->pass)))
				pb->success = 0;
		} else {
			if (!TEST_true(PKCS12_add_safe_ex(
				    &pb->safes, pb->bags, enc->nid, enc->iter,
...
decode_contentinfo(STACK_OF(PKCS7) *safes, int idx, const PKCS12_ENC *enc)
{
	...
	if (enc) {
		if (!TEST_int_eq(bagnid, NID_pkcs7_encrypted))
			goto err;
		bags = PKCS12_unpack_p7encdata(p7, enc->pass,
					       (int)strlen(enc->pass));
	} else {
...
static int add_attributes(PKCS12_SAFEBAG *bag, const PKCS12_ATTR *attr)
{
	...
	while (p_attr->oid != NULL) {
		TEST_info("Adding attribute %s = %s", p_attr->oid,
			  p_attr->value);
		...
		const PKCS12_ATTR *attrs, const PKCS12_ENC *enc)
{
	...
	if (legacy)
		...
		bag = PKCS12_add_key_ex(&pb->bags, pkey, 0 /*keytype*/,
					enc->iter, enc->nid, enc->pass,
					test_ctx, test_propq);
	if (!TEST_ptr(bag)) {
		pb->success = 0;
		goto err;
	...
	for (i = 0; i < sk_X509_ATTRIBUTE_num(bag_attrs); i++) {
		...
		while (p_attr->oid != NULL) {
			...
			if (strcmp(p_attr->oid, attr_txt) == 0) {
				...
				for (j = 0; j < X509_ATTRIBUTE_count(attr);
				     j++) {
					av = X509_ATTRIBUTE_get0_type(attr, j);
					if (!TEST_true(check_asn1_string(
						    av, p_attr->value)))
						goto err;
				}
				break;
		   ...
		   const PKCS12_ATTR *attrs)
{
	...
	if (!check_attrs(PKCS12_SAFEBAG_get0_attrs(bag), attrs) ||
	    ...
	    !TEST_int_eq(PKCS12_SAFEBAG_get_bag_nid(bag),
			 NID_x509Certificate)) {
		pb->success = 0;
		return;
	}
		  ...
		  const PKCS12_ATTR *attrs, const PKCS12_ENC *enc)
{
	...
	case NID_pkcs8ShroudedKeyBag:
		if (legacy)
			p8 = PKCS12_decrypt_skey(bag, enc->pass,
						 (int)strlen(enc->pass));
		else
			p8 = PKCS12_decrypt_skey_ex(bag, enc->pass,
						    (int)strlen(enc->pass),
						    test_ctx, test_propq);
		if (!TEST_ptr(p8)) {
			pb->success = 0;
		     ...
		     const PKCS12_ATTR *attrs)
{
	...
	if (!check_attrs(PKCS12_SAFEBAG_get0_attrs(bag), attrs) ||
	    ...
	    !TEST_true(check_asn1_string(PKCS12_SAFEBAG_get0_bag_obj(bag),
					 secret)))
		pb->success = 0;
}
