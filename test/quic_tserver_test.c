...
static int do_test(int use_thread_assist, int use_fake_time, int use_inject)
{
	...
	if (!TEST_true(
		    BIO_ADDR_rawmake(s_addr_, AF_INET, &ina, sizeof(ina), 0)))
		goto err;
	...
	if (use_fake_time)
		if (!TEST_true(ossl_quic_set_override_now_cb(c_ssl, fake_now,
							     NULL)))
			goto err;
	...
	for (;;) {
		...
		if (c_connected && c_write_done && !s_read_done) {
			if (!ossl_quic_tserver_read(
				...
				if (!TEST_true(ossl_quic_tserver_has_read_ended(
					    tserver, 0)))
					goto err;

				if (!TEST_mem_eq(msg1, sizeof(msg1) - 1, msg2,
						 s_total_read))
					goto err;
				...
				s_total_read += l;
				if (!TEST_size_t_le(s_total_read,
						    sizeof(msg1) - 1))
					goto err;
			}
		}
		...
		if (c_begin_read && c_total_read < sizeof(msg1) - 1) {
			...
			if (c_total_read == sizeof(msg1) - 1) {
				if (!TEST_mem_eq(msg1, sizeof(msg1) - 1, msg3,
						 c_total_read))
					goto err;
		...
		if (c_start_idle_test && !c_done_idle_test) {
			...
			if (idle_units_done < 6000) {
                 ...
                 */
				if (!TEST_true(SSL_get_event_timeout(c_ssl, &tv,
								     &isinf)))
					goto err;
				if (!isinf &&
				    ossl_time_compare(
					...
					OSSL_sleep(
						10); /* Ensure CPU scheduling for test purposes */
			} else {
				c_done_idle_test = 1;
			}
		}
		...
		if (use_inject) {
			...
			for (;;) {
				...
				if (!BIO_recvmmsg(c_net_bio, &rmsg,
						  sizeof(rmsg), 1, 0,
						  &msgs_processed) ||
