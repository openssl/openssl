...
static int test_des_ede_cbc(void)
{
	...
	if (!TEST_mem_eq(cbc_out, sizeof(xcbc_ok), xcbc_ok, sizeof(xcbc_ok)))
		return 0;
	memcpy(iv3, cbc_iv, sizeof(cbc_iv));
	DES_xcbc_encrypt(cbc_out, cbc_in, (long)n, &ks, &iv3, &cbc2_key,
			 &cbc3_key, DES_DECRYPT);
...
static int test_ede_cbc(void)
{
	...
	DES_ede3_cbc_encrypt(cbc_data, cbc_out, 16L, &ks, &ks2, &ks3, &iv3,
			     DES_ENCRYPT);
	DES_ede3_cbc_encrypt(&cbc_data[16], &cbc_out[16], (long)(i - 16), &ks,
			     &ks2, &ks3, &iv3, DES_ENCRYPT);
...
static int test_des_cfb64(void)
{
	...
	memcpy(cfb_tmp, cfb_iv, sizeof(cfb_iv));
	n = 0;
	DES_cfb64_encrypt(cfb_buf1, cfb_buf2, 17, &ks, &cfb_tmp, &n,
			  DES_DECRYPT);
...
static int test_des_ede_ofb64(void)
{
	...
	memcpy(ofb_tmp, ofb_iv, sizeof(ofb_iv));
	num = 0;
	DES_ede3_ofb64_encrypt(ofb_buf1, ofb_buf2, sizeof(ofb_buf1), &ks, &ks,
			       &ks, &ofb_tmp, &num);
	return TEST_mem_eq(plain, sizeof(ofb_buf2), ofb_buf2, sizeof(ofb_buf2));
}
...
static int test_des_check_bad_parity(int n)
{
	...
	return TEST_int_eq(DES_check_key_parity(key),
			   bad_parity_keys[n].expect);
}
