...
static int verify_chain(SSL *ssl, STACK_OF(X509) *chain)
{
	...
	X509_STORE_CTX_set_default(
		store_ctx, SSL_is_server(ssl) ? "ssl_client" : "ssl_server");
	X509_VERIFY_PARAM_set1(X509_STORE_CTX_get0_param(store_ctx),
			       SSL_get0_param(ssl));
	store_ctx_dane_init(store_ctx, ssl);

	if (SSL_get_verify_callback(ssl) != NULL)
		X509_STORE_CTX_set_verify_cb(store_ctx,
...
static STACK_OF(X509) *load_chain(BIO *fp, int nelem)
{
	...
	for (count = 0; count < nelem && errtype == 0 &&
...
static ossl_ssize_t checked_uint8(const char *in, void *out)
{
	...
	if (((v == LONG_MIN || v == LONG_MAX) && e == ERANGE) || endp == cp ||
	    !isspace(_UC(*endp)) || v != (*(uint8_t *)result = (uint8_t)v)) {
		return -1;
	}
...
static int test_tlsafile(SSL_CTX *ctx, const char *base_name, BIO *f,
			 const char *path)
{
	char *line;
	int testno = 0;
	...
	while (ret > 0 && (line = read_to_eol(f)) != NULL) {
		...
		++testno;
		if (sscanf(line, "%d %d %d %d %d%n", &ntlsa, &ncert, &noncheck,
			   &want, &want_depth, &off) != 5 ||
		    !allws(line + off)) {
		...
		if (noncheck)
			SSL_dane_set_flags(ssl,
					   DANE_FLAG_NO_DANE_EE_NAMECHECKS);

		for (i = 0; i < ntlsa; ++i) {
			if ((line = read_to_eol(f)) == NULL ||
		...
		if (!TEST_int_eq(err, want)) {
			if (want == X509_V_OK)
				TEST_info(
					"Verification failure in test %d: %d=%s",
					testno, err,
					X509_verify_cert_error_string(err));
			else
				TEST_info("Unexpected error in test %d",
					  testno);
			ret = 0;
			continue;
		}
		if (!TEST_false(want == 0 && ok == 0)) {
			TEST_info("Verification failure in test %d: ok=0",
				  testno);
			ret = 0;
			continue;
		}
