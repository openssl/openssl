...
static void print_key_details(BIO *out, EVP_PKEY *key)
{
	...
	if (keyid == EVP_PKEY_EC) {
		...
		BIO_printf(out, "%d bits EC (%s)", EVP_PKEY_get_bits(key),
			   group);
	} else
#endif
	{
...
int main(int argc, char *argv[])
{
	...
	SSL_CONF_CTX_set_flags(s_cctx, SSL_CONF_FLAG_CMDLINE |
					       SSL_CONF_FLAG_SERVER |
					       SSL_CONF_FLAG_CERTIFICATE |
					       SSL_CONF_FLAG_REQUIRE_PRIVATE);
	SSL_CONF_CTX_set_flags(s_cctx2, SSL_CONF_FLAG_CMDLINE |
						SSL_CONF_FLAG_SERVER |
						SSL_CONF_FLAG_CERTIFICATE |
						SSL_CONF_FLAG_REQUIRE_PRIVATE);
	...
	SSL_CONF_CTX_set_flags(c_cctx, SSL_CONF_FLAG_CMDLINE |
	...
	while (argc >= 1) {
		if (strcmp(*argv, "-F") == 0) {
			...
			psk_key = *(++argv);
#ifndef OPENSSL_NO_PSK
			if (strspn(psk_key, "abcdefABCDEF1234567890") !=
			    strlen(psk_key)) {
...
bad:
	...
	if (!ssl3 && !tls1 && !tls1_1 && !tls1_2 && !dtls && !dtls1 &&
	    !dtls12 && number > 1 && !reuse && !force) {
		fprintf(stderr,
	...
	if (n) {
		...
		for (j = 0; j < n; j++) {
			const SSL_COMP *c =
				sk_SSL_COMP_value(ssl_comp_methods, j);
			printf("  %s:%d", SSL_COMP_get0_name(c),
			       SSL_COMP_get_id(c));
		}
		printf("\n");
	}
...

	{
		...
		if (!SSL_CTX_set_session_id_context(
			    s_ctx, (void *)&session_id_context,
			    sizeof(session_id_context)) ||
		    !SSL_CTX_set_session_id_context(
			    s_ctx2, (void *)&session_id_context,
			    sizeof(session_id_context))) {
			ERR_print_errors(bio_err);
			goto end;
		}
	...
	if (psk_key != NULL) {
		...
		if (!SSL_CTX_use_psk_identity_hint(
			    s_ctx, "ctx server identity_hint") ||
		    !SSL_CTX_use_psk_identity_hint(
			    s_ctx2, "ctx server identity_hint")) {
			BIO_printf(
				bio_err,
	...
	if (npn_server_reject) {
		SSL_CTX_set_npn_advertised_cb(s_ctx, cb_server_rejects_npn,
					      NULL);
		SSL_CTX_set_npn_advertised_cb(s_ctx2, cb_server_rejects_npn,
					      NULL);
	}
#endif
	...
	if (serverinfo_sct) {
		if (!SSL_CTX_add_client_custom_ext(
			    c_ctx, TLSEXT_TYPE_signed_certificate_timestamp,
			    NULL, NULL, NULL, serverinfo_cli_parse_cb, NULL)) {
	...
	if (serverinfo_tack) {
		if (!SSL_CTX_add_client_custom_ext(
			    c_ctx, TACK_EXT_TYPE, NULL, NULL, NULL,
	...
	if (custom_ext) {
		if (!SSL_CTX_add_client_custom_ext(
			    c_ctx, CUSTOM_EXT_TYPE_0, custom_ext_0_cli_add_cb,
			    NULL, NULL, custom_ext_0_cli_parse_cb, NULL) ||
		    !SSL_CTX_add_client_custom_ext(
			    c_ctx, CUSTOM_EXT_TYPE_1, custom_ext_1_cli_add_cb,
			    NULL, NULL, custom_ext_1_cli_parse_cb, NULL) ||
		    !SSL_CTX_add_client_custom_ext(
			    c_ctx, CUSTOM_EXT_TYPE_2, custom_ext_2_cli_add_cb,
			    NULL, NULL, custom_ext_2_cli_parse_cb, NULL) ||
		    !SSL_CTX_add_client_custom_ext(
			    c_ctx, CUSTOM_EXT_TYPE_3, custom_ext_3_cli_add_cb,
			    NULL, NULL, custom_ext_3_cli_parse_cb, NULL) ||
		    !SSL_CTX_add_server_custom_ext(
			    s_ctx, CUSTOM_EXT_TYPE_0, custom_ext_0_srv_add_cb,
			    NULL, NULL, custom_ext_0_srv_parse_cb, NULL) ||
		    !SSL_CTX_add_server_custom_ext(
			    s_ctx2, CUSTOM_EXT_TYPE_0, custom_ext_0_srv_add_cb,
			    NULL, NULL, custom_ext_0_srv_parse_cb, NULL) ||
		    !SSL_CTX_add_server_custom_ext(
			    s_ctx, CUSTOM_EXT_TYPE_1, custom_ext_1_srv_add_cb,
			    NULL, NULL, custom_ext_1_srv_parse_cb, NULL) ||
		    !SSL_CTX_add_server_custom_ext(
			    s_ctx2, CUSTOM_EXT_TYPE_1, custom_ext_1_srv_add_cb,
			    NULL, NULL, custom_ext_1_srv_parse_cb, NULL) ||
		    !SSL_CTX_add_server_custom_ext(
			    s_ctx, CUSTOM_EXT_TYPE_2, custom_ext_2_srv_add_cb,
			    NULL, NULL, custom_ext_2_srv_parse_cb, NULL) ||
		    !SSL_CTX_add_server_custom_ext(
			    s_ctx2, CUSTOM_EXT_TYPE_2, custom_ext_2_srv_add_cb,
			    NULL, NULL, custom_ext_2_srv_parse_cb, NULL) ||
		    !SSL_CTX_add_server_custom_ext(
	...
	if (alpn_client) {
		...
		if (SSL_CTX_set_alpn_protos(c_ctx, alpn,
	...
	if (server_sess_out != NULL) {
		if (write_session(server_sess_out, SSL_get_session(s_ssl)) ==
		    0) {
			ret = EXIT_FAILURE;
			goto end;
		}
	...
	if (client_sess_out != NULL) {
		if (write_session(client_sess_out, SSL_get_session(c_ssl)) ==
		    0) {
			ret = EXIT_FAILURE;
			goto end;
		}
		   ...
		   clock_t *s_time, clock_t *c_time)
{
	...
	if (verbose) {
		...
		if (BIO_get_ktls_send(SSL_get_wbio(s_ssl)) &&
			...
			BIO_printf(bio_stdout,
				   "Server using Kernel TLS for sending\n");
		else if (BIO_get_ktls_recv(SSL_get_rbio(s_ssl)))
			BIO_printf(bio_stdout,
				   "Server using Kernel TLS for receiving\n");
		...
		if (BIO_get_ktls_send(SSL_get_wbio(c_ssl)) &&
			...
			BIO_printf(bio_stdout,
				   "Client using Kernel TLS for sending\n");
		else if (BIO_get_ktls_recv(SSL_get_rbio(c_ssl)))
			BIO_printf(bio_stdout,
				   "Client using Kernel TLS for receiving\n");
	}
#ifndef OPENSSL_NO_NEXTPROTONEG
...
static int app_verify_callback(X509_STORE_CTX *ctx, void *arg)
{
	...
	if (cb_arg->app_verify) {
		...
		printf("Finished printing do we have a context? 0x%p a cert? 0x%p\n",
		       (void *)ctx, (void *)c);
		if (c)
			s = X509_NAME_oneline(X509_get_subject_name(c), buf,
					      256);
		if (s != NULL) {
