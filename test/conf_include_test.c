...
static int test_load_config(void)
{
	...
	if (!TEST_int_gt(NCONF_load_bio(conf, in, &errline), 0) ||
		...
		if (expect_failure)
			return 1;
		TEST_note("Failure loading the configuration at line %ld",
			  errline);
		return 0;
	}
	if (expect_failure) {
	...
	val = 0;
	if (!TEST_int_eq(NCONF_get_number(conf, "CA_default", "default_days",
					  &val),
			 1) ||
	    !TEST_int_eq(val, 365)) {
	...
	val = 0;
	if (!TEST_int_eq(NCONF_get_number(conf, "req", "default_bits", &val),
			 1) ||
	...
	if (test_providers != 0) {
		...
		val = 0;
		if (!TEST_int_eq(NCONF_get_number(conf, "null_sect", "activate",
						  &val),
				 1) ||
		    !TEST_int_eq(val, 1)) {
		...
		val = 0;
		if (!TEST_int_eq(NCONF_get_number(conf, "default_sect",
						  "activate", &val),
				 1) ||
		    !TEST_int_eq(val, 1)) {
		...
		val = 0;
		if (!TEST_int_eq(NCONF_get_number(conf, "legacy_sect",
						  "activate", &val),
				 1) ||
		    !TEST_int_eq(val, 1)) {
...
static int test_available_providers(void)
{
	...
	if (!TEST_ptr(rel_conf_file) ||
	    !OSSL_LIB_CTX_load_config(libctx, rel_conf_file)) {
