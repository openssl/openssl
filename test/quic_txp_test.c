...
static const QUIC_CONN_ID dcid_1 = { 8,
				     { 0xa1, 0xa2, 0xa3, 0xa4, 0xa5, 0xa6, 0xa7,
				       0xa8 } };

static const QUIC_CONN_ID cid_1 = { 8,
...
static int helper_init(struct helper *h)
{
	...
	if (!TEST_true(ossl_quic_rxfc_init(&h->max_streams_uni_rxfc, NULL, 100,
					   100, fake_now, NULL)))
	...
	if (!TEST_true(ossl_quic_stream_map_init(
		    &h->qsm, NULL, NULL, &h->max_streams_bidi_rxfc,
		    &h->max_streams_uni_rxfc, h->client_ch)))
		goto err;
	...
	if (!TEST_ptr(h->demux =
			      ossl_quic_demux_new(h->bio2, 8, fake_now, NULL)))
		goto err;
...
{
	ossl_quic_tx_packetiser_schedule_ack_eliciting(h->txp,
						       QUIC_PN_SPACE_APP);
	return 1;
}
...
static int schedule_max_data(struct helper *h)
{
	...
	if (!TEST_true(ossl_quic_rxfc_on_rx_stream_frame(&h->stream_rxfc, cwm,
							 0)) ||
...
static const struct script_op script_3[] = {
	OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_1RTT, QRL_SUITE_AES128GCM, secret_1)
		OP_TXP_GENERATE_NONE() OP_CHECK(schedule_max_data)
			OP_TXP_GENERATE() OP_RX_PKT()
	/* Should not be long */
...
static int schedule_cfq_new_conn_id(struct helper *h)
{
	...
	ncid.conn_id = cid_1;
	memcpy(ncid.stateless_reset.token, reset_token_1,
	...
	if (!TEST_ptr(cfq_item = ossl_quic_cfq_add_frame(
			      ...
			      (unsigned char *)buf_mem->data, l, free_buf_mem,
			      buf_mem)))
		goto err;
...
static const struct script_op script_4[] = {
	OP_PROVIDE_SECRET(
		...
		secret_1) OP_TXP_GENERATE_NONE() OP_CHECK(schedule_cfq_new_conn_id)
...
static int schedule_cfq_new_token(struct helper *h)
{
	...
	if (!TEST_true(ossl_quic_wire_encode_frame_new_token(
		    &wpkt, token_1, sizeof(token_1)))) {
		WPACKET_cleanup(&wpkt);
		goto err;
	}
...
static const struct script_op script_8[] = {
	OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_1RTT, QRL_SUITE_AES128GCM,
		...
		OP_CRYPTO_SEND(QUIC_PN_SPACE_APP, crypto_1) OP_TXP_GENERATE()
			OP_RX_PKT() OP_EXPECT_DGRAM_LEN(21, 512) OP_NEXT_FRAME()
	...
	OP_CONN_TXFC_BUMP(1000) OP_STREAM_TXFC_BUMP(42, 1000) OP_TXP_GENERATE()
		OP_RX_PKT() OP_EXPECT_DGRAM_LEN(21, 512) OP_NEXT_FRAME()
			OP_EXPECT_FRAME(OSSL_QUIC_FRAME_TYPE_STREAM)
				OP_CHECK(check_stream_9) OP_EXPECT_NO_FRAME()
...
static const struct script_op script_10[] = {
	OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_1RTT, QRL_SUITE_AES128GCM,
			  secret_1) OP_HANDSHAKE_COMPLETE()
		OP_TXP_GENERATE_NONE() OP_STREAM_NEW(42) OP_STREAM_NEW(43)
			OP_CONN_TXFC_BUMP(10000) OP_STREAM_TXFC_BUMP(42, 5000)
	...
	OP_TXP_GENERATE() OP_RX_PKT() OP_EXPECT_DGRAM_LEN(200,
							  500) OP_NEXT_FRAME()
		OP_EXPECT_FRAME(OSSL_QUIC_FRAME_TYPE_STREAM_OFF_LEN) OP_CHECK(
			check_stream_10c) OP_NEXT_FRAME()
...
static const struct script_op script_11[] = {
	OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_INITIAL, QRL_SUITE_AES128GCM,
			  secret_1) OP_TXP_GENERATE_NONE()
		OP_CRYPTO_SEND(QUIC_PN_SPACE_INITIAL,
			       crypto_1) OP_TXP_GENERATE() OP_RX_PKT()
...
static const struct script_op script_12[] = {
	OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_1RTT, QRL_SUITE_AES128GCM, secret_1)
		OP_HANDSHAKE_COMPLETE() OP_TXP_GENERATE_NONE() OP_STREAM_NEW(
			42) OP_STOP_SENDING(42,
					    4568) OP_TXP_GENERATE() OP_RX_PKT()
...
static const struct script_op script_13[] = {
	OP_PROVIDE_SECRET(
		...
		OP_STREAM_NEW(42) OP_CONN_TXFC_BUMP(8) OP_STREAM_TXFC_BUMP(
			42, 8) OP_STREAM_SEND(42,
					      stream_13) OP_RESET_STREAM(42,
									 4568)
			OP_TXP_GENERATE() OP_RX_PKT() OP_EXPECT_DGRAM_LEN(21,
									  128)
				OP_NEXT_FRAME() OP_EXPECT_FRAME(
					OSSL_QUIC_FRAME_TYPE_RESET_STREAM)
					OP_CHECK(
						check_stream_13) OP_NEXT_FRAME()
						OP_EXPECT_NO_FRAME() OP_RX_PKT_NONE()
							OP_TXP_GENERATE_NONE()
								OP_END
};
...
static const struct script_op script_14[] = {
	OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_1RTT, QRL_SUITE_AES128GCM, secret_1)
		OP_HANDSHAKE_COMPLETE() OP_TXP_GENERATE_NONE() OP_CHECK(
			gen_conn_close) OP_TXP_GENERATE() OP_RX_PKT()
			OP_EXPECT_DGRAM_LEN(21, 512) OP_NEXT_FRAME()
				OP_EXPECT_FRAME(
					OSSL_QUIC_FRAME_TYPE_CONN_CLOSE_TRANSPORT)
					OP_CHECK(check_14) OP_EXPECT_NO_FRAME()
...
static int gen_probe_initial(struct helper *h)
{
	OSSL_ACKM_PROBE_INFO *probe =
		ossl_ackm_get0_probe_request(h->args.ackm);
...
static const struct script_op script_15[] = {
	OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_INITIAL, QRL_SUITE_AES128GCM,
			  secret_1) OP_TXP_GENERATE_NONE()
		OP_CHECK(gen_probe_initial) OP_TXP_GENERATE() OP_RX_PKT()
			OP_EXPECT_DGRAM_LEN(1200, 1200) OP_NEXT_FRAME()
...
static const struct script_op script_16[] = {
	OP_DISCARD_EL(QUIC_ENC_LEVEL_INITIAL) OP_PROVIDE_SECRET(
		...
		secret_1) OP_TXP_GENERATE_NONE() OP_CHECK(gen_probe_handshake)
		OP_TXP_GENERATE() OP_RX_PKT() OP_EXPECT_DGRAM_LEN(21, 512)
			OP_NEXT_FRAME()
				OP_EXPECT_FRAME(OSSL_QUIC_FRAME_TYPE_PING)
					OP_EXPECT_NO_FRAME() OP_RX_PKT_NONE()
...
static const struct script_op script_17[] = {
	OP_DISCARD_EL(QUIC_ENC_LEVEL_INITIAL) OP_DISCARD_EL(
		QUIC_ENC_LEVEL_HANDSHAKE) OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_1RTT,
							    QRL_SUITE_AES128GCM,
							    secret_1)
		OP_TXP_GENERATE_NONE() OP_CHECK(
			gen_probe_1rtt) OP_TXP_GENERATE() OP_RX_PKT()
			OP_EXPECT_DGRAM_LEN(21, 512) OP_NEXT_FRAME()
				OP_EXPECT_FRAME(OSSL_QUIC_FRAME_TYPE_PING)
					OP_EXPECT_NO_FRAME() OP_RX_PKT_NONE()
						OP_TXP_GENERATE_NONE() OP_END
...
static int try_big_token(struct helper *h)
{
	...
	for (i = sizeof(big_token) - 1;; --i) {
		...
		if (ossl_quic_tx_packetiser_set_initial_token(h->txp, big_token,
							      i, NULL, NULL))
			break;
	}
...
static const struct script_op script_18[] = {
	OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_INITIAL, QRL_SUITE_AES128GCM,
			  secret_1) OP_TXP_GENERATE_NONE()
		OP_CHECK(try_big_token) OP_TXP_GENERATE_NONE() OP_CRYPTO_SEND(
			QUIC_PN_SPACE_INITIAL, crypto_1) OP_TXP_GENERATE()
			OP_RX_PKT() OP_EXPECT_DGRAM_LEN(1200,
							1200) OP_NEXT_FRAME()
				OP_EXPECT_FRAME(OSSL_QUIC_FRAME_TYPE_CRYPTO)
					OP_EXPECT_NO_FRAME() OP_RX_PKT_NONE()
						OP_TXP_GENERATE_NONE() OP_END
...
static int run_script(int script_idx, const struct script_op *script)
{
	...
	for (op = script, opn = 0; op->opcode != OPK_END; ++op, ++opn) {
		...
		case OPK_TXP_GENERATE:
			if (!TEST_true(ossl_quic_tx_packetiser_generate(
				    h.txp, &status)) &&
		...
		case OPK_EXPECT_DGRAM_LEN:
			if (!TEST_size_t_ge(h.qrx_pkt->datagram_len,
					    (size_t)op->arg0) ||
			    !TEST_size_t_le(h.qrx_pkt->datagram_len,
					    (size_t)op->arg1))
				goto err;
			break;
		...
		case OPK_EXPECT_INITIAL_TOKEN:
			if (!TEST_mem_eq(h.qrx_pkt->hdr->token,
					 h.qrx_pkt->hdr->token_len, op->buf,
					 (size_t)op->arg0))
				goto err;
			break;
		case OPK_EXPECT_HDR:
		...
		case OPK_NEXT_FRAME:
			skip_padding(&h);
			if (!ossl_quic_wire_peek_frame_header(
				    &h.pkt, &h.frame_type, NULL)) {
				h.frame_type = UINT64_MAX;
				break;
			}
			...
			case OSSL_QUIC_FRAME_TYPE_PING:
				if (!TEST_true(ossl_quic_wire_decode_frame_ping(
					    &h.pkt)))
					goto err;
				break;
			...
			case OSSL_QUIC_FRAME_TYPE_ACK_WITHOUT_ECN:
				...
				h.frame.ack.num_ack_ranges =
					OSSL_NELEM(h.ack_ranges);
				if (!TEST_true(ossl_quic_wire_decode_frame_ack(
					    &h.pkt, h.args.ack_delay_exponent,
					    &h.frame.ack, NULL)))
					goto err;
				break;
		...
		case OPK_STREAM_NEW: {
			...
			if (!TEST_ptr(s = ossl_quic_stream_map_alloc(
					      h.args.qsm, op->arg0,
					      QUIC_STREAM_DIR_BIDI)))
				goto err;

			if (!TEST_ptr(s->sstream = ossl_quic_sstream_new(
					      512 * 1024)) ||
			    !TEST_true(ossl_quic_txfc_init(&s->txfc,
							   &h.conn_txfc)) ||
			    !TEST_true(ossl_quic_rxfc_init(
				    &s->rxfc, &h.conn_rxfc, 1 * 1024 * 1024,
				    16 * 1024 * 1024, fake_now, NULL)) ||
			    !TEST_ptr(s->rstream = ossl_quic_rstream_new(
					      &s->rxfc, NULL, 1024))) {
		...
		case OPK_CONN_TXFC_BUMP:
			if (!TEST_true(ossl_quic_txfc_bump_cwm(h.args.conn_txfc,
							       op->arg0)))
				goto err;
		...
		case OPK_STREAM_TXFC_BUMP: {
			...
			if (!TEST_true(ossl_quic_txfc_bump_cwm(&s->txfc,
							       op->arg0)))
				goto err;
...
err:
	if (!testresult)
		TEST_error("script %d failed at op %zu", script_idx + 1,
			   opn + 1);
	if (have_helper)
		helper_cleanup(&h);
	return testresult;
...
static int test_dyn_script_1(int idx)
{
	...
	if (expect_handshake_pkt_in_same_dgram) {
		...
		dyn_script_1[dyn_script_1_check_idx].check_func =
			check_is_handshake;
	} else {
	...
	if (!run_script(idx, dyn_script_1)) {
		TEST_error("failed dyn script 1 with target size %zu",
			   target_size);
		return 0;
	}
