...
/*
 * Helper functions to detect specific versions of a provider being in use.
 ...
 *      1. the provider version matches the criteria specified or
 *      2. the provider isn't being used.
 */
...
/*
 * This function matches a provider version with (potentially multiple)
 * <operator>maj.min.patch version strings in versions.
 * The operator can be one of = ! < <= >= or > comparison symbols.
 * If the provider matches all the version comparisons (or if there is no
 * provider available) the function returns 1.
 * If the provider does not match the version comparisons, it returns 0.
