			   ...
			   int cmd, long argl, int ret, size_t *processed)
{
	...
	if (oper == (BIO_CB_CTRL | BIO_CB_RETURN) && cmd == BIO_CTRL_FLUSH)
		ret = mock_http_server(bio, args->out, args->version,
				       args->keep_alive, args->content_type,
				       args->txt, (ASN1_VALUE *)x509, x509_it);
	return ret;
}
...
static int test_http_method(int do_get, int do_txt, int suggested_status)
{
	...
	if (do_txt) {
		...
		if (req == NULL || BIO_puts(req, text1) != sizeof(text1) - 1 ||
	...
	rsp = do_get ? OSSL_HTTP_get(real_server ? REAL_SERVER_URL : path,
				     ...
				     0 /* buf_size */, headers,
				     real_server ? "text/html; charset=utf-8" :
						   content_type,
				     !do_txt /* expect_asn1 */,
				     OSSL_HTTP_DEFAULT_MAX_RESP_LEN,
				     0 /* timeout */) :
		       OSSL_HTTP_transfer(NULL, NULL /* host */,
					  NULL /* port */, path,
					  0 /* use_ssl */, NULL /* proxy */,
					  NULL /* no_pr */, wbio, rbio,
					  NULL /* bio_fn */, NULL /* arg */,
					  0 /* buf_size */, headers,
					  content_type, req, content_type,
					  !do_txt /* expect_asn1 */,
					  OSSL_HTTP_DEFAULT_MAX_RESP_LEN,
					  0 /* timeout */, 0 /* keep_alive */);
	if (!TEST_int_eq(suggested_status == HTTP_STATUS_CODES_FATAL_ERROR,
			 rsp == NULL))
		goto err;
	...
	if (rsp != NULL) {
		if (do_get && real_server) {
			...
			char rtext[sizeof(text1) +
				   1 /* more space than needed */];
			...
			res = TEST_ptr(rcert) &&
			      TEST_int_eq(X509_cmp(x509, rcert), 0);
			X509_free(rcert);
		}
		BIO_free(rsp);
	}
...
static int test_http_keep_alive(char version, int keep_alive, int kept_alive)
{
	...
	for (res = 1, i = 1; res && i <= 2; i++) {
		rsp = OSSL_HTTP_transfer(
			&rctx, NULL /* server */, NULL /* port */, RPATH,
			0 /* use_ssl */, NULL /* proxy */, NULL /* no_proxy */,
			wbio, rbio, NULL /* bio_update_fn */, NULL,
			0 /* buf_size */, NULL /* headers */,
			NULL /* content_type */, NULL /* req => GET */,
			content_type, 0 /* ASN.1 not expected */,
			0 /* max_resp_len */, 0 /* timeout */, keep_alive);
			    ...
			    const char *exp_port, const char *exp_path)
{
	...
	if (!TEST_int_eq(sscanf(exp_port, "%d", &exp_num), 1))
		return 0;
	res = TEST_true(OSSL_HTTP_parse_url(url, &ssl, &user, &host, &port,
					    &num, &path, &query, &frag)) &&
	      TEST_str_eq(host, exp_host) && TEST_str_eq(port, exp_port) &&
	      TEST_int_eq(num, exp_num) && TEST_str_eq(path, exp_path) &&
...
static int test_http_url_path_query_ok(const char *url, const char *exp_path_qu)
{
	...
	res = TEST_true(OSSL_HTTP_parse_url(url, NULL, NULL, &host, NULL, NULL,
					    &path, NULL, NULL)) &&
	      TEST_str_eq(host, "host") && TEST_str_eq(path, exp_path_qu);
	OPENSSL_free(host);
	OPENSSL_free(path);
	return res;
}
...
{
	return test_http_url_path_query_ok("http://usr@host:1/p?q=x#frag",
					   "/p?q=x") &&
	       test_http_url_path_query_ok("http://host?query#frag",
					   "/?query") &&
...
{
	return test_http_url_ok("https://1.2.3.4/p/q", 1, "1.2.3.4", "443",
				"/p/q");
}
...
static int test_http_url_invalid(const char *url)
{
	...
	res = TEST_false(OSSL_HTTP_parse_url(url, &ssl, NULL, &host, &port,
					     &num, &path, NULL, NULL)) &&
	      TEST_ptr_null(host) && TEST_ptr_null(port) && TEST_ptr_null(path);
	if (!res) {
		OPENSSL_free(host);
		OPENSSL_free(port);
		OPENSSL_free(path);
...
static int test_http_get_txt_error_status(void)
{
	return test_http_method(1 /* GET */, 1,
				HTTP_STATUS_CODES_NONFATAL_ERROR);
}
