...
static int verify_cookie_cb(SSL *ssl, const unsigned char *cookie,
			    unsigned int cookie_len)
{
	return TEST_mem_eq(cookie, cookie_len, dummy_cookie,
			   sizeof(dummy_cookie));
}
...
static int test_dtls_unprocessed(int testidx)
{
	...
	if (!TEST_true(
		    SSL_CTX_set_cipher_list(sctx, "AES128-SHA:@SECLEVEL=0")) ||
...
static int test_dtls_drop_records(int idx)
{
	...
	if (!TEST_int_eq((int)BIO_ctrl(mempackbio, MEMPACKET_CTRL_GET_DROP_REC,
				       0, NULL),
			 -1))
		goto end;
...
static int test_just_finished(void)
{
	...
	unsigned char buf[] = {
		...
		0,
		DTLS1_HM_HEADER_LENGTH + SHA_DIGEST_LENGTH, /* record length */
	...
	if (!TEST_true(create_ssl_ctx_pair(NULL, DTLS_server_method(), NULL, 0,
					   0, &sctx, NULL, cert, privkey)))
		return 0;
...
static int test_swap_records(int idx)
{
	...
	if (!TEST_true(
		    create_ssl_objects(sctx, cctx, &sssl, &cssl, NULL, NULL)))
		goto end;
