...
my $test_name = "test_dtls13downgrade";
setup($test_name);
...
plan skip_all => "$test_name needs DTLS1.3 and DTLS1.2 enabled"
    if disabled("dtls1_3")
...
#Test 1: Downgrade from DTLSv1.3 to DTLSv1.2
$proxy->filter(\&downgrade_filter);
my $testtype = DOWNGRADE_TO_TLS_1_2;
$proxy->start() or plan skip_all => "Unable to start up Proxy for tests";
plan tests => 6;
ok(TLSProxy::Message->fail(), "Downgrade DTLSv1.3 to DTLSv1.2");

#Test 2: Downgrade from DTLSv1.3 to DTLSv1
$proxy->clear();
$testtype = DOWNGRADE_TO_TLS_1_1;
$proxy->start();
ok(TLSProxy::Message->fail(), "Downgrade DTLSv1.3 to DTLSv1");

#Test 3: Downgrade from DTLSv1.2 to DTLSv1.1
$proxy->clear();
$proxy->clientflags("-max_protocol DTLSv1.2");
$proxy->serverflags("-max_protocol DTLSv1.2");
$proxy->start();
ok(TLSProxy::Message->fail(), "Downgrade DTLSv1.2 to DTLSv1");

#Test 4: Client falls back from TLSv1.3 (server does not support the fallback
#        SCSV)
$proxy->clear();
$testtype = FALLBACK_FROM_TLS_1_3;
$proxy->clientflags("-fallback_scsv -max_protocol DTLSv1.2");
$proxy->start();
my $alert = TLSProxy::Message->alert();
ok(TLSProxy::Message->fail()
   && !$alert->server()
   && $alert->description() == TLSProxy::Message::AL_DESC_ILLEGAL_PARAMETER,
   "Fallback from DTLSv1.3");

SKIP: {
    skip "DTLSv1 disabled", 2 if disabled("dtls1");
    #Test 5: A client side protocol "hole" should not be detected as a downgrade
    $proxy->clear();
    $proxy->filter(undef);
    $proxy->clientflags("-max_protocol DTLSv1");
    $proxy->ciphers("AES128-SHA:\@SECLEVEL=0");
    $proxy->start();
    ok(TLSProxy::Message->success(), "DTLSv1.2 client-side protocol hole");

    #Test 6: A server side protocol "hole" should not be detected as a downgrade
    $proxy->clear();
    $proxy->filter(undef);
    $proxy->serverflags("-max_protocol DTLSv1");
    $proxy->start();
    ok(TLSProxy::Message->success(), "DTLSv1.2 server-side protocol hole");
}
...
sub downgrade_filter
{
    ...
    if ($testtype == FALLBACK_FROM_TLS_1_3) {
        #The default ciphersuite we use for DTLSv1.2 without any SCSV
