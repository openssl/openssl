...
is(run(app([ 'opentls', 'genrsa', '-3', '-out', 'genrsatest.pem', '8'])), 0, "genrsa -3 8");
...
while ($good > $bad + 1) {
    ...
    if (run(app([ 'opentls', 'genrsa', '-3', '-out', 'genrsatest.pem',
...
ok(run(app([ 'opentls', 'genrsa', '-3', '-out', 'genrsatest.pem', $good ])),
   "genrsa -3 $good");
ok(run(app([ 'opentls', 'rsa', '-check', '-in', 'genrsatest.pem', '-noout' ])),
   "rsa -check");
ok(run(app([ 'opentls', 'genrsa', '-f4', '-out', 'genrsatest.pem', $good ])),
   "genrsa -f4 $good");
ok(run(app([ 'opentls', 'rsa', '-check', '-in', 'genrsatest.pem', '-noout' ])),
   "rsa -check");
