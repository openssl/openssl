...
# We use 'opentls verify' for these tests, as it contains everything
...
sub verify {
    ...
    my @args = qw(opentls verify -auth_level 1 -purpose);
...
indir "60-test_x509_store" => sub {
    ...
    ok(run(app([qw(opentls rehash), curdir()])), "Rehashing");

    # Canonical success
    ok(verify("ee-cert", "tlsserver", curdir(), ["ca-cert"], "-show_chain"),
       "verify ee-cert");
