...
#
# The dump grid is deliberately not VAES-gated, because the masked reference run
# has to produce a digest while the VAES path is unavailable to it. Ask the
# binary which backend libcrypto picks before requesting a grid, so a host
# without the VAES/AVX-512 path does not walk one only to be skipped.
#
sub vaes_capable {
    my $status = 0;
    my @out = run(test(["aesgcm_avx512_test", "probe"]),
                  capture => 1, statusvar => \$status);

    return undef unless $status;
    foreach (@out) {
        return $1 if /^VAESCAP:\s+(\d)/;
    }
    return undef;
}
...
sub dump_digest {
 ...
 SKIP: {
    ...
    skip "AVX-512/VAES not available on this host", 1
        if defined $cap && $cap ne "1";

    unless (defined $cap) {
        ok(0, "AES-GCM AVX-512 capability probe succeeded");
    } else {
        my ($nok, $nvaes, $ndig) = dump_digest(undef);

        unless ($nok && defined $nvaes && $nvaes eq "1") {
            ok(0, "AES-GCM AVX-512 dump (native) succeeded and printed "
                . "DIFFDIGEST for the VAES path");
        } else {
            ...
            ok($gok && defined $gvaes && defined $gdig && defined $ndig
               && $gvaes eq "0"
               && $ndig eq $gdig,
               "AVX-512 GCM (VAES=1) byte-identical to non-VAES AES-NI GCM "
               . "(VAES=0) over sweep");
        }
    }
}
