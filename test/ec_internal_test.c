...
static int decoded_flag_test(void)
{
	...
	if (!TEST_int_eq(EC_GROUP_get_asn1_flag(grp), OPENSSL_EC_NAMED_CURVE) ||
	    ...
	    !TEST_ptr(grp_copy =
			      EC_GROUP_new_from_ecpkparameters(ecpkparams)) ||
	    !TEST_int_eq(grp_copy->decoded_from_explicit_params, 0) ||
	...
	if (!TEST_int_gt(encodedlen = i2d_ECPKParameters(grp, &encodedparams),
			 0) ||
...
static int montgomery_correctness_test(EC_GROUP *group)
{
	...
	if (!TEST_ptr(ctx))
		return ret;
	if (!TEST_true(
		    check_bn_mont_ctx(group->mont_data, group->order, ctx))) {
		TEST_error("group order issue");
		goto err;
	}
	if (group->field_data1 != NULL) {
		if (!TEST_true(check_bn_mont_ctx(group->field_data1,
						 group->field, ctx)))
			goto err;
	}
	ret = 1;
...
int setup_tests(void)
{
	...
	if (!TEST_ptr(curves =
			      OPENSSL_malloc_array(crv_len, sizeof(*curves))) ||
