...
static int test_thread_internal(void)
{
	...
	if (threads_supported == 0) {
		...
		t[0] = ossl_crypto_thread_start(NULL, test_thread_native_fn,
						&local[0]);
	...
	for (i = 0; i < OSSL_NELEM(t); ++i) {
		...
		t[i] = ossl_crypto_thread_start(NULL, test_thread_native_fn,
						&local[0]);
		if (!TEST_ptr(t[i]))
			goto cleanup;
		...
		if (!TEST_uint_eq(retval[0], i + 1) ||
		    !TEST_uint_eq(local[0], i + 2))
			goto cleanup;
	...
	for (i = 0; i < OSSL_NELEM(t); ++i) {
		if (!TEST_uint_eq(retval[i], i + 1) ||
		    !TEST_uint_eq(local[i], i + 2))
			goto cleanup;
		if (!TEST_int_eq(ossl_crypto_thread_clean(t[i]), 1))
			goto cleanup;
	}
...
static int test_thread_native_multiple_joins(void)
{
	...
	t = ossl_crypto_thread_native_start(
		test_thread_native_multiple_joins_fn1, NULL, 1);
	t1 = ossl_crypto_thread_native_start(
		test_thread_native_multiple_joins_fn2, t, 1);
	t2 = ossl_crypto_thread_native_start(
		test_thread_native_multiple_joins_fn3, t, 1);
