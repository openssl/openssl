...
static int test_EVP_MD_fetch_freeze_default(void)
{
    ...
    /* Default ctx was frozen in previous test, skip the test */
    if (use_default_ctx == 1)
        return 1;

    if (!load_providers(&ctx, prov))
        goto err;

    ctx2 = OSSL_LIB_CTX_new();

    if (!TEST_ptr(ctx2)
        || !TEST_int_eq(OSSL_LIB_CTX_freeze(ctx, NULL), 1)
        || !TEST_int_eq(OSSL_LIB_CTX_freeze(ctx2, NULL), 1))
        goto err;

    if (!TEST_ptr(md = EVP_MD_fetch(NULL, "SHA1", NULL))
        || !TEST_int_eq(md->origin, EVP_ORIG_FROZEN)
        || !TEST_ptr(md = EVP_MD_fetch(NULL, "SHA1", "?fips=true"))
    ...
    if (!TEST_ptr(md = EVP_MD_fetch(ctx, "SHA1", NULL))
        || !TEST_int_eq(md->origin, EVP_ORIG_FROZEN)
        || !TEST_ptr(md = EVP_MD_fetch(ctx, "SHA1", "?fips=true"))
        || !TEST_int_ne(md->origin, EVP_ORIG_FROZEN))
        goto err;
...
int setup_tests(void)
{
    ...
    if (strcmp(alg, "digest") == 0) {
        ...
        ADD_TEST(test_EVP_MD_fetch_freeze_default);
