...
static int test_certs(int num)
{
	...
	for (c = 0; !err && PEM_read_bio(fp, &name, &header, &data, &len);
	     ++c) {
		const int trusted =
		...
		if (len != enclen) {
			TEST_error(
				"encoded length %ld of %s != input length %ld",
				enclen, name, len);
			err = 1;
			goto next;
		}
		...
		if (enclen != len) {
			TEST_error(
				"unexpected buffer position after encoding %s",
				name);
			err = 1;
			goto next;
		}
		if (memcmp(buf, data, len) != 0) {
			TEST_error("encoded content of %s does not match input",
				   name);
			err = 1;
			goto next;
		}
		...
		if (trusted) {
			...
			if (memcmp(buf, data, enclen) != 0) {
				TEST_error(
					"encoded cert content does not match input");
				err = 1;
				goto next;
			}
		}
