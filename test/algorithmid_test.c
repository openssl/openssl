...
static int test_spki_aid(X509_PUBKEY *pubkey, const char *filename)
{
	...
	OSSL_PARAM params[] = { OSSL_PARAM_octet_string(ALGORITHMID_NAME,
							&algid_prov,
							sizeof(algid_prov)),
				OSSL_PARAM_END };
	int ret = 0;
	...
	if (!TEST_true(
		    X509_PUBKEY_get0_param(NULL, NULL, NULL, &alg, pubkey)) ||
	    !TEST_ptr(pkey = X509_PUBKEY_get0(pubkey)))
		goto end;

	if (!TEST_int_ge(algid_legacy_len = i2d_X509_ALGOR(alg, &algid_legacy),
			 0))
		goto end;
	...
	if ((keydata = evp_pkey_export_to_provider(pkey, NULL, &keymgmt,
						   NULL)) == NULL) {
		TEST_info(
			"The public key found in '%s' doesn't have provider support."
			"  Skipping...",
			filename);
		ret = 1;
		goto end;
	...
	if (!TEST_true(EVP_KEYMGMT_is_a(keymgmt, name))) {
		TEST_info(
			"The AlgorithmID key type (%s) for the public key found in"
	...
	if (!TEST_ptr(gettable_params = EVP_KEYMGMT_gettable_params(keymgmt)) ||
	    !TEST_ptr(OSSL_PARAM_locate_const(gettable_params,
					      ALGORITHMID_NAME))) {
		TEST_info(
			"The %s provider keymgmt appears to lack support for algorithm-id."
			"  Skipping...",
			name);
		ret = 1;
		goto end;
	...
	if (TEST_mem_eq(algid_legacy, algid_legacy_len, algid_prov,
			algid_prov_len))
		ret = 1;
			     ...
			     X509 *cacert, const char *ca_filename)
{
	...
	size_t algid_prov_len = 0;
	const OSSL_PARAM *gettable_params = NULL;
	OSSL_PARAM params[] = { OSSL_PARAM_octet_string("algorithm-id",
							&algid_prov,
							sizeof(algid_prov)),
				OSSL_PARAM_END };
	int ret = 0;
	...
	if (!TEST_true(EVP_PKEY_is_a(pkey, OBJ_nid2sn(pkey_nid)))) {
		TEST_info(
			"The '%s' pubkey can't be used to verify the '%s' signature",
			ca_filename, ee_filename);
		TEST_info(
			"Signature algorithm is %s (pkey type %s, hash type %s)",
...
static int test_x509_files(void)
{
	...
	ret = test_x509_sig_aid(eecert, eecert_filename, cacert,
				cacert_filename) &
...
int setup_tests(void)
{
	...
	if (spki && pubkey_filename == NULL) {
		...
		return 0;
		   ...
		   (eecert_filename == NULL || cacert_filename == NULL)) {
