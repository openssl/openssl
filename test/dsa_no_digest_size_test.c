...
static int sign_and_verify(int len)
{
    ...
    digestlen = BN_num_bytes(q);

    dataToSign = OPENSSL_malloc(len);
    paddedData = OPENSSL_malloc(digestlen);

    if (dataToSign == NULL || paddedData == NULL)
        goto end;

    if (!RAND_bytes(dataToSign, len))
        goto end;
    ...
    if (EVP_PKEY_sign(ctx, NULL, &sigLength, dataToSign, len) != 1) {
        fprintf(stderr, "Failed to get signature length, len=%d", len);
        goto end;
    }
    ...
    if (EVP_PKEY_sign(ctx, signature, &sigLength, dataToSign, len) != 1) {
        fprintf(stderr, "Failed to sign, len=%d", len);
        goto end;
    }
    ...
    if (EVP_PKEY_verify(ctx, signature, sigLength, dataToSign, len) != 1) {
        fprintf(stderr, "EVP verify with unpadded length %d failed\n", len);
        goto end;
    }
...
int main(int argc, char **argv)
{
    ...
    if (!dsa_exact_size_test())
        return 1;
    if (!dsa_small_digest_test())
        return 1;
    if (!dsa_large_digest_test())
        return 1;
    DSA_free(dsakey);
#endif
    return 0;
