...
static DSA *load_dsa_params(void)
{
	...
	static unsigned char dsaq_2048[] = { 0xAD, 0x2D, 0x6E, 0x17, 0xB0, 0xF3,
					     0xEB, 0xC7, 0xB8, 0xEE, 0x95, 0x78,
					     0xF2, 0x17, 0xF5, 0x33, 0x01, 0x67,
					     0xBC, 0xDE, 0x93, 0xFF, 0xEE, 0x40,
					     0xE8, 0x7F, 0xF1, 0x93, 0x6D, 0x4B,
					     0x87, 0x13 };
...
static int sign_and_verify(int len)
{
	...
	if (DSA_verify(0, dataToSign, len, signature, (int)sigLength, dsakey) !=
	    1) {
		TEST_error("Verification with unpadded data failed, len=%d",
			   len);
		goto end;
	}

	if (DSA_verify(0, paddedData, digestlen, signature, (int)sigLength,
		       dsakey) != 1) {
