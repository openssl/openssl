...
static const OSSL_PARAM xor_kemgroup_params[] = {
	...
	OSSL_PARAM_int(OSSL_CAPABILITY_TLS_GROUP_MIN_DTLS,
		       &xor_kemgroup.mindtls),
	OSSL_PARAM_int(OSSL_CAPABILITY_TLS_GROUP_MAX_DTLS,
		       &xor_kemgroup.maxdtls),
				     ...
				     OSSL_CALLBACK *cb, void *arg)
{
	...
	if (strcmp(capability, "TLS-GROUP") == 0) {
		...
		for (i = 0; i < NUM_DUMMY_GROUPS; i++) {
			...
			memcpy(dummygroup, xor_group_params,
		      ...
		      size_t outlen)
{
	...
	*secretlen = XOR_KEY_SIZE;
	...
	for (i = 0; i < XOR_KEY_SIZE; i++)
		secret[i] = pxorctx->key->privkey[i] ^
			    pxorctx->peerkey->pubkey[i];
	...
	if (tokey != NULL && fromkey != NULL) {
		...
		if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) {
			if (fromkey->haspubkey) {
				memcpy(tokey->pubkey, fromkey->pubkey,
				       XOR_KEY_SIZE);
				tokey->haspubkey = 1;
			} else {
				tokey->haspubkey = 0;
		...
		if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
			if (fromkey->hasprivkey) {
				memcpy(tokey->privkey, fromkey->privkey,
				       XOR_KEY_SIZE);
				tokey->hasprivkey = 1;
			} else {
				tokey->hasprivkey = 0;
...
static int xor_match(const void *keydata1, const void *keydata2, int selection)
{
	...
	if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
		if (key1->hasprivkey) {
			if (key2->hasprivkey)
				ok = ok & (CRYPTO_memcmp(key1->privkey,
							 key2->privkey,
							 XOR_KEY_SIZE) == 0);
			else
				ok = ok &
				     xor_recreate(key1->privkey, key2->pubkey);
		} else {
			if (key2->hasprivkey)
				ok = ok &
				     xor_recreate(key2->privkey, key1->pubkey);
			else
				ok = 0;
		}
	}
	...
	if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) {
		if (key1->haspubkey) {
			if (key2->haspubkey)
				...
				ok = ok &
				     xor_recreate(key1->pubkey, key2->privkey);
		} else {
			if (key2->haspubkey)
				ok = ok &
				     xor_recreate(key2->pubkey, key1->privkey);
			else
				ok = 0;
		}
	}
...
static void *xor_gen(void *genctx, OSSL_CALLBACK *osslcb, void *cbarg)
{
	...
	if ((gctx->selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
		if (RAND_bytes_ex(gctx->libctx, key->privkey, XOR_KEY_SIZE,
				  0) <= 0) {
			OPENSSL_free(key);
			return NULL;
		}
		      ...
		      void *cbarg)
{
	...
	*p++ = OSSL_PARAM_construct_octet_string(
		OSSL_PKEY_PARAM_PUB_KEY, key->pubkey, sizeof(key->pubkey));
...
{
	return (select & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0 ? xor_key_types :
							     NULL;
}
					  ...
					  i2d_of_void *k2d)
{
	...
	if ((p8info = PKCS8_PRIV_KEY_INFO_new()) == NULL ||
	    ...
	    !PKCS8_pkey_set0(p8info, OBJ_nid2obj(key_nid), 0, V_ASN1_UNDEF,
			     NULL, der, derlen)) {
		ERR_raise(ERR_LIB_USER, ERR_R_MALLOC_FAILURE);
				 ...
				 struct key2any_ctx_st *ctx)
{
	...
	p8 = PKCS8_encrypt_ex(-1, ctx->cipher, kstr, (int)klen, NULL, 0, 0,
			      p8info, libctx, NULL);
				    ...
				    struct key2any_ctx_st *ctx)
{
	...
	if (p2s != NULL &&
	    !p2s(key, key_nid, ctx->save_parameters, &str, &strtype))
		return 0;
				   ...
				   struct key2any_ctx_st *ctx)
{
	...
	if (ctx->cipher_intent)
		return key_to_epki_der_priv_bio(out, key, key_nid, pemname, p2s,
						k2d, ctx);
				   ...
				   struct key2any_ctx_st *ctx)
{
	...
	if (ctx->cipher_intent)
		return key_to_epki_pem_priv_bio(out, key, key_nid, pemname, p2s,
						k2d, ctx);
...
static int key2any_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
	...
	const OSSL_PARAM *save_paramsp = OSSL_PARAM_locate_const(
		params, OSSL_ENCODER_PARAM_SAVE_PARAMETERS);
	...
	if (cipherp != NULL) {
		...
		if (!OSSL_PARAM_get_utf8_string_ptr(cipherp, &ciphername))
			return 0;
		if (propsp != NULL &&
		    !OSSL_PARAM_get_utf8_string_ptr(propsp, &props))
			return 0;
		...
		if (ciphername != NULL &&
		    ((ctx->cipher = EVP_CIPHER_fetch(libctx, ciphername,
						     props)) == NULL)) {
			return 0;
		}
	}
			  ...
			  i2d_of_void *key2der)
{
	...
	if (key == NULL || type <= 0) {
		...
		if (out != NULL) {
			...
			ret = writer(out, key, type, pemname, key2paramstring,
				     key2der, ctx);
		}
...
#define DO_ENC_PRIVATE_KEY(impl, type, kind, output)                           \
	if ((selection & DO_ENC_PRIVATE_KEY_selection_mask) != 0)              \
		return key2any_encode(ctx, cout, key, impl##_pem_type,         \
				      ...
				      key_to_##kind##_##output##_priv_bio, cb, \
				      cbarg, prepare_##type##_params,          \
...
#define DO_ENC_PUBLIC_KEY(impl, type, kind, output)                           \
	if ((selection & DO_ENC_PUBLIC_KEY_selection_mask) != 0)              \
		return key2any_encode(ctx, cout, key, impl##_pem_type,        \
				      ...
				      key_to_##kind##_##output##_pub_bio, cb, \
				      cbarg, prepare_##type##_params,         \
...
#define MAKE_ENCODER(impl, type, kind, output)                                    \
	...
	static int impl##_to_##kind##_##output##_does_selection(void *ctx,        \
								int selection)    \
	{                                                                         \
		return key2any_check_selection(                                   \
			selection, DO_ENC_##kind##_selection_mask);               \
	}                                                                         \
	...
	static const OSSL_DISPATCH                                                \
		xor_##impl##_to_##kind##_##output##_encoder_functions[] = {       \
			...
			{ OSSL_FUNC_ENCODER_DOES_SELECTION,                       \
			  (void (*)(void))                                        \
				  impl##_to_##kind##_##output##_does_selection }, \
...
ASN1_SEQUENCE(
	X509_PUBKEY_INTERNAL) = { ASN1_SIMPLE(X509_PUBKEY, algor, X509_ALGOR),
				  ASN1_SIMPLE(
					  ...
					  OSSL_LIB_CTX *libctx)
{
	...
	return (X509_PUBKEY *)ASN1_item_d2i_ex(
		(ASN1_VALUE **)&xpub, pp, len,
		ASN1_ITEM_rptr(X509_PUBKEY_INTERNAL), libctx, NULL);
}
				   ...
				   key_from_pkcs8_t *key_from_pkcs8)
{
	...
	if ((p8inf = d2i_PKCS8_PRIV_KEY_INFO(NULL, input_der, input_der_len)) !=
		...
		key = key_from_pkcs8(p8inf, PROV_XOR_LIBCTX_OF(ctx->provctx),
				     NULL);
...
#define DO_PrivateKeyInfo(keytype)                                          \
	"PrivateKeyInfo", 0, (OSSL_KEYMGMT_SELECT_PRIVATE_KEY), NULL, NULL, \
		NULL, xorx_d2i_PKCS8, NULL, NULL, xorx_key_adjust,          \
		(free_key_fn *)xor_freekey

#define DO_SubjectPublicKeyInfo(keytype)                                   \
	"SubjectPublicKeyInfo", 0, (OSSL_KEYMGMT_SELECT_PUBLIC_KEY), NULL, \
		NULL, NULL, NULL, (d2i_of_void *)xor_d2i_PUBKEY, NULL,     \
		xorx_key_adjust, (free_key_fn *)xor_freekey
	...
	static const OSSL_DISPATCH                                         \
		xor_##kind##_der_to_##keytype##_decoder_functions[] = {    \
			...
			{ OSSL_FUNC_DECODER_DOES_SELECTION,                \
			  (void (*)(void))                                 \
				  kind##_der2##keytype##_does_selection }, \
...
static int xor_sig_get_ctx_params(void *vpxor_sigctx, OSSL_PARAM *params)
{
	...
	if (pxor_sigctx->aid == NULL)
		pxor_sigctx->aid_len = xor_get_aid(&(pxor_sigctx->aid),
						   pxor_sigctx->sig->tls_name);

	if (p != NULL && !OSSL_PARAM_set_octet_string(p, pxor_sigctx->aid,
						      pxor_sigctx->aid_len))
		return 0;
