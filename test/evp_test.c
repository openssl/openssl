                        ...
                        uint8_t *out, size_t outlen)
{
    ...
    if (pub != NULL && publen > 0)
        *p++ = OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_PUB_KEY, (uint8_t *)pub, publen);
    if (ctx != NULL && ctxlen > 0)
        *p++ = OSSL_PARAM_construct_octet_string(OSSL_SIGNATURE_PARAM_CONTEXT_STRING, (uint8_t *)ctx, ctxlen);
    if (digestname != NULL)
        *p++ = OSSL_PARAM_construct_utf8_string(OSSL_ALG_PARAM_DIGEST, (char *)digestname, 0);
    *p = OSSL_PARAM_construct_end();

    if (!TEST_ptr(mdctx = EVP_MD_CTX_new()))
        goto err;
    if (!TEST_ptr(md = EVP_MD_fetch(libctx, "ml_dsa_mu", NULL)))
        goto err;
    if (!TEST_true(EVP_DigestInit_ex2(mdctx, md, params))
            || !TEST_true(EVP_DigestUpdate(mdctx, msg, msglen))
            || !TEST_true(EVP_DigestFinalXOF(mdctx, out, outlen)))
        goto err;
    ret = 1;
err:
    EVP_MD_free(md);
...
static int pkey_calculate_mu(EVP_TEST *t, uint8_t *mu, size_t *mulen)
{
    ...
    p = OSSL_PARAM_locate(params, OSSL_ALG_PARAM_DIGEST);
    if (p != NULL && p->data != NULL) {
        ...
        if (EVP_MD_is_a(md, "SHAKE128"))
            ...
            xoflen = 64;
        if (xoflen != 0)
            xofparam[0] = OSSL_PARAM_construct_size_t(OSSL_DIGEST_PARAM_XOFLEN, &xoflen);
        if (!EVP_DigestInit_ex2(mdctx, md, xofparam)
                || !EVP_DigestUpdate(mdctx, in, inlen)
                || !EVP_DigestFinal(mdctx, digest, &len)) {
            goto err;
        }
        in = digest;
        inlen = len;
    ...
    /*
     * ML-DSA uses passes the public key to calculate mu
     * SLH-DSA does not use the public key to calc
     */
