			     ...
			     const OSSL_PARAM params[])
{
	...
	TEST_info("(Indicator Callback received %s : %s is not approved)", type,
		  desc);
	return 1;
}
...
static int mac_check_fips_approved(EVP_MAC_CTX *ctx, EVP_TEST *t)
{
	...
	params[0] = OSSL_PARAM_construct_int(
		OSSL_MAC_PARAM_FIPS_APPROVED_INDICATOR, &approved);
...
static int pkey_check_fips_approved(EVP_PKEY_CTX *ctx, EVP_TEST *t)
{
	...
	if (gettables == NULL ||
	    OSSL_PARAM_locate_const(
		    gettables, OSSL_ALG_PARAM_FIPS_APPROVED_INDICATOR) == NULL)
		return 1;
	...
	params[0] = OSSL_PARAM_construct_int(
		OSSL_ALG_PARAM_FIPS_APPROVED_INDICATOR, &approved);
...
static int rand_check_fips_approved(EVP_RAND_CTX *ctx, EVP_TEST *t)
{
	...
	params[0] = OSSL_PARAM_construct_int(
		OSSL_DRBG_PARAM_FIPS_APPROVED_INDICATOR, &approved);
...
{
	return check_security_category(
		t, pkey, (int (*)(void *, OSSL_PARAM *))EVP_PKEY_get_params);
}
...
static int ctrl2params(EVP_TEST *t, STACK_OF(OPENSSL_STRING) *controls,
		       const OSSL_PARAM *settables, OSSL_PARAM params[],
		       size_t params_sz, size_t *params_n)
{
	int i;
...
static void ctrl2params_free(OSSL_PARAM params[], size_t params_n,
...
static int kdf_check_fips_approved(EVP_KDF_CTX *ctx, EVP_TEST *t)
{
	...
	params[0] = OSSL_PARAM_construct_int(
		OSSL_KDF_PARAM_FIPS_APPROVED_INDICATOR, &approved);
				  ...
				  STACK_OF(EVP_TEST_BUFFER) **sk)
{
	...
	do {
		...
		if (max_len == 0) {
			...
			if ((rv = parse_bin_chunk(value, offset, max_len,
						  &db->buf, &db->buflen,
						  &new_offset)) == -1)
				goto err;
			offset = new_offset;
...
static int evp_test_buffer_do(STACK_OF(EVP_TEST_BUFFER) *sk,
			      int (*fn)(void *ctx, const unsigned char *buf,
					size_t buflen),
			      void *ctx)
{
	int i;
			   ...
			   size_t *out_offset)
{
	size_t vlen;
	size_t chunk_len;
	const char *value_str = value[0] == '"' ? value + offset + 1 :
						  value + offset;
	...
	*out_offset = value[0] == '"' ? offset + (*buflen) :
					offset + 2 * (*buflen);
	return 1;
}
...
static int digest_test_init(EVP_TEST *t, const char *alg)
{
	...
	if ((digest = fetched_digest = EVP_MD_fetch(libctx, alg, propquery)) ==
		    NULL &&
...
static int digest_test_parse(EVP_TEST *t, const char *keyword,
			     const char *value)
{
	DIGEST_DATA *mdata = t->data;

	if (strcmp(keyword, "Input") == 0)
		return evp_test_buffer_append(value, data_chunk_size,
					      &mdata->input);
...
static int digest_test_run(EVP_TEST *t)
{
	...
	*p++ = OSSL_PARAM_construct_end();
	...
	if (!memory_err_compare(t, "DIGEST_MISMATCH", expected->output,
	...
	if (sk_EVP_TEST_BUFFER_num(expected->input) == 1 &&
		...
		if (!TEST_true(EVP_Q_digest(
			    libctx, EVP_MD_get0_name(expected->fetched_digest),
			    NULL, inbuf->buf, inbuf->buflen, got, &size)) ||
		    !TEST_mem_eq(got, size, expected->output,
				 expected->output_len)) {
...
static const EVP_TEST_METHOD digest_test_method = { "Digest", digest_test_init,
						    digest_test_cleanup,
						    digest_test_parse,
						    digest_test_run };
...
static int cipher_test_valid_fragmentation(CIPHER_DATA *cdat)
{
	return (cdat->aead == EVP_CIPH_CCM_MODE ||
		...
		((EVP_CIPHER_get_flags(cdat->cipher) & EVP_CIPH_FLAG_CTS) !=
		 0) ||
...
static int cipher_test_init(EVP_TEST *t, const char *alg)
{
	...
	ERR_set_mark();
	if ((cipher = fetched_cipher =
		     EVP_CIPHER_fetch(libctx, alg, propquery)) == NULL &&
			     ...
			     const char *value)
{
	...
	if (strcmp(keyword, "Ciphertext") == 0)
		return parse_bin(value, &cdat->ciphertext,
				 &cdat->ciphertext_len);
	...
	if (cdat->aead) {
		...
		if (strcmp(keyword, "AAD") == 0 || tls_aad) {
			for (i = 0; i < AAD_NUM; i++) {
				if (cdat->aad[i] == NULL)
					return parse_bin(value, &cdat->aad[i],
							 &cdat->aad_len[i]);
			}
			return -1;
		}
			   ...
			   const OSSL_PARAM initparams[])
{
	...
	if (in_place == 1) {
		...
		tmp = OPENSSL_malloc(out_misalign + in_len +
         ...
         */
		tmp = OPENSSL_malloc(out_misalign + in_len +
				     2 * EVP_MAX_BLOCK_LENGTH + inp_misalign +
				     in_len);
		if (!tmp)
			goto err;
		in = memcpy(tmp + out_misalign + in_len +
				    2 * EVP_MAX_BLOCK_LENGTH + inp_misalign,
			    in, in_len);
	}
	...
	if (expected->cts_mode != NULL) {
		...
		params[0] = OSSL_PARAM_construct_utf8_string(
			OSSL_CIPHER_PARAM_CTS_MODE, (char *)expected->cts_mode,
			0);
	...
	if (expected->iv) {
		if (expected->aead) {
			if (EVP_CIPHER_CTX_ctrl(
				    ctx_base, EVP_CTRL_AEAD_SET_IVLEN,
				    (int)expected->iv_len, 0) <= 0) {
				t->err = "INVALID_IV_LENGTH";
				goto err;
			}
		} else if (expected->iv_len !=
	...
	if (expected->rounds > 0) {
		...
		if (EVP_CIPHER_CTX_ctrl(ctx_base, EVP_CTRL_SET_RC5_ROUNDS,
					rounds, NULL) <= 0) {
	...
	if (expected->key_bits > 0) {
		...
		if (EVP_CIPHER_CTX_ctrl(ctx_base, EVP_CTRL_SET_RC2_KEY_BITS,
					bits, NULL) <= 0) {
			t->err = "INVALID KEY BITS";
			goto err;
		}
	}
	if (!EVP_CipherInit_ex(ctx_base, NULL, NULL, expected->key,
			       expected->iv, -1)) {
		t->err = "KEY_SET_ERROR";
		goto err;
	}
	...
	if (expected->iv != NULL) {
		...
		if (!TEST_true(EVP_CIPHER_CTX_get_updated_iv(ctx_base, iv,
							     sizeof(iv))) ||
		    ((EVP_CIPHER_get_flags(expected->cipher) &
			...
			if (fips_no_silent_error &&
			    !TEST_false(ERR_peek_error())) {
	...
	if (expected->tls_version) {
		...
		params[0] = OSSL_PARAM_construct_int(
			OSSL_CIPHER_PARAM_TLS_VERSION, &expected->tls_version);
		params[1] = OSSL_PARAM_construct_end();
	...
	if (expected->tls_aad) {
		...
		params[0] = OSSL_PARAM_construct_octet_string(
			OSSL_CIPHER_PARAM_AEAD_TLS1_AAD, tls_aad,
			expected->aad_len[0]);
		params[1] = OSSL_PARAM_construct_end();
		if (!EVP_CIPHER_CTX_set_params(ctx, params)) {
	...
	if (!frag) {
		do {
			...
			if (!EVP_CipherUpdate(ctx, tmp + out_misalign + tmplen,
					      &chunklen, in,
					      (int)current_in_len))
				goto err;
			tmplen += chunklen;
		...
		if (in_len > 0) {
			if (!EVP_CipherUpdate(ctx, tmp + out_misalign,
					      &chunklen, in, 1))
				goto err;
			tmplen += chunklen;
			in++;
	...
	if (!enc && expected->tls_aad) {
		if (expected->tls_version >= TLS1_1_VERSION &&
		    (EVP_CIPHER_is_a(expected->cipher,
	...
	if (enc && expected->aead && !expected->tls_aad) {
		...
		if (!memory_err_compare(t, "TAG_VALUE_MISMATCH", expected->tag,
					expected->tag_len, rtag,
					expected->tag_len))
			goto err;
	}
	if (enc && expected->tag) {
		if (EVP_CIPHER_is_a(expected->cipher,
			...
			params[0] = OSSL_PARAM_construct_octet_string(
				OSSL_CIPHER_HMAC_PARAM_MAC, &rtag[0], tag_len);
			params[1] = OSSL_PARAM_construct_end();
			...
			if (!memory_err_compare(
				    t, "TAG_VALUE_MISMATCH", expected->tag,
				    expected->tag_len, rtag, expected->tag_len))
				goto err;
		}
	}
	...
	if (expected->next_iv != NULL) {
		/* Some (e.g., GCM) tests use IVs longer than EVP_MAX_IV_LENGTH. */
		unsigned char iv[128];
		if (!TEST_true(EVP_CIPHER_CTX_get_updated_iv(ctx, iv,
							     sizeof(iv))) ||
...
static int cipher_test_run(EVP_TEST *t)
{
	...
	if (sk_OPENSSL_STRING_num(cdat->init_controls) > 0) {
		if (!ctrl2params(t, cdat->init_controls, NULL, initparams,
				 OSSL_NELEM(initparams), &params_n))
			return 0;
	}
	...
	for (in_place = 1; in_place >= 0; in_place--) {
		...
		for (frag = 0; frag <= fragmax; frag++) {
			...
			for (out_misalign = 0; out_misalign <= 1;
			     out_misalign++) {
				for (inp_misalign = 0; inp_misalign <= 1;
				     inp_misalign++) {
...
static const EVP_TEST_METHOD cipher_test_method = { "Cipher", cipher_test_init,
						    cipher_test_cleanup,
						    cipher_test_parse,
						    cipher_test_run };
...
static int mac_test_parse(EVP_TEST *t, const char *keyword, const char *value)
{
...
static int mac_test_run_pkey(EVP_TEST *t)
{
	...
	if (expected->alg == NULL)
		TEST_info("Trying the EVP_PKEY %s test",
	...
	if (expected->type == EVP_PKEY_CMAC) {
		...
		TEST_info("skipping, PKEY CMAC '%s' is disabled",
			  expected->alg);
		t->skip = 1;
		t->err = NULL;
		goto err;
		...
		if (expected->alg != NULL &&
		    is_cipher_disabled(expected->alg)) {
			TEST_info("skipping, PKEY CMAC '%s' is disabled",
				  expected->alg);
			t->skip = 1;
			t->err = NULL;
		...
		if (!TEST_ptr(cipher = EVP_CIPHER_fetch(libctx, expected->alg,
							propquery))) {
			t->err = "MAC_KEY_CREATE_ERROR";
			goto err;
		}
		tmpctx = OSSL_LIB_CTX_set0_default(libctx);
		key = EVP_PKEY_new_CMAC_key(NULL, expected->key,
					    expected->key_len, cipher);
		OSSL_LIB_CTX_set0_default(tmpctx);
#endif
	} else {
		key = EVP_PKEY_new_raw_private_key_ex(
			libctx, OBJ_nid2sn(expected->type), NULL, expected->key,
			expected->key_len);
	}
	if (key == NULL) {
		t->err = "MAC_KEY_CREATE_ERROR";
	...
	if (expected->type == EVP_PKEY_HMAC && expected->alg != NULL) {
		if (is_digest_disabled(expected->alg)) {
			TEST_info("skipping, HMAC '%s' is disabled",
				  expected->alg);
			t->skip = 1;
			t->err = NULL;
			goto err;
	...
	do {
		...
		if (!EVP_DigestSignUpdate(mctx, expected->input + donelen,
					  current_len)) {
	...
	if (!EVP_DigestSignFinal(mctx, got, &got_len) ||
	    !memory_err_compare(t, "TEST_MAC_ERR", expected->output,
				expected->output_len, got, got_len)) {
		t->err = "TEST_MAC_ERR";
		goto err;
	}
...
static int mac_test_run_mac(EVP_TEST *t)
{
	...
	if (expected->alg != NULL) {
		...
		if (skip) {
			TEST_info("skipping, algorithm '%s' is disabled",
				  expected->alg);
			t->skip = 1;
			t->err = NULL;
			goto err;
	...
	if (expected->custom != NULL)
		params[params_n++] = OSSL_PARAM_construct_octet_string(
			OSSL_MAC_PARAM_CUSTOM, expected->custom,
			expected->custom_len);
	if (expected->salt != NULL)
		params[params_n++] = OSSL_PARAM_construct_octet_string(
			OSSL_MAC_PARAM_SALT, expected->salt,
			expected->salt_len);
	if (expected->iv != NULL)
		params[params_n++] = OSSL_PARAM_construct_octet_string(
			OSSL_MAC_PARAM_IV, expected->iv, expected->iv_len);

	params_n_allocstart = params_n;
	if (!ctrl2params(t, expected->controls, defined_params, params,
			 OSSL_NELEM(params), &params_n))
		goto err;
	...
	if (size_before_init != 0) {
		...
		if (size_val == 0 &&
		    !TEST_size_t_eq(size_before_init, size_after_init)) {
			t->err = "MAC SIZE check failed";
			goto err;
		}
		/* mac-size modified by init params */
		if (size_val != 0 &&
		    !TEST_size_t_eq(size_val, size_after_init)) {
			t->err = "MAC SIZE check failed";
			goto err;
		}
	}
...
retry:
	...
	do {
		...
		if (data_chunk_size == 0 || (size_t)data_chunk_size > input_len)
			current_len = input_len;
		if (!EVP_MAC_update(ctx, expected->input + donelen,
				    current_len)) {
	...
	if (reinit-- && fips_provider_version_gt(libctx, 3, 0, 0)) {
		...
		if (expected->iv != NULL) {
			ivparams[0] = OSSL_PARAM_construct_octet_string(
				OSSL_MAC_PARAM_IV, expected->iv,
				expected->iv_len);
	...
	if (!xof) {
		...
		if (!TEST_true(EVP_Q_mac(libctx, expected->mac_name, NULL,
					 expected->alg, params, expected->key,
					 expected->key_len, expected->input,
					 expected->input_len, got, got_len,
					 &size)) ||
		    !TEST_mem_eq(got, size, expected->output,
				 expected->output_len)) {
...
static const EVP_TEST_METHOD mac_test_method = { "MAC", mac_test_init,
						 mac_test_cleanup,
...
static int kem_test_parse(EVP_TEST *t, const char *keyword, const char *value)
{
	...
	if (strcmp(keyword, "Ciphertext") == 0)
		return parse_bin(value, &kdata->ciphertext,
				 &kdata->ciphertext_len);
		       ...
		       unsigned char **outsecret, size_t *outsecretlen)
{
	...
	const size_t params_max =
		OSSL_NELEM(params) - 1 - (kdata->entropy != NULL);
	...
	if (kdata->entropy != NULL)
		...
		params[params_n++] = OSSL_PARAM_construct_octet_string(
			OSSL_KEM_PARAM_IKME, kdata->entropy, kdata->entropylen);
	params[params_n] = OSSL_PARAM_construct_end();
	...
	if (EVP_PKEY_encapsulate(ctx, NULL, &wrappedlen, NULL, &secretlen) <=
	    0) {
	...
	if (EVP_PKEY_encapsulate(ctx, wrapped, &wrappedlen, secret,
				 &secretlen) <= 0) {
	...
	if (kdata->ciphertext != NULL &&
	    !TEST_mem_eq(wrapped, wrappedlen, kdata->ciphertext,
			 kdata->ciphertext_len)) {
		ret = 0;
		goto err;
	}
		       ...
		       const unsigned char *expected, size_t expectedlen)
{
	...
	if (sk_OPENSSL_STRING_num(kdata->init_ctrls) > 0) {
		if (!ctrl2params(t, kdata->init_ctrls, NULL, params, 2,
				 &params_n))
			goto err;
		p = params;
	}
...
static int kem_test_run(EVP_TEST *t)
{
	...
	if (kdata->keyname != NULL) {
		...
		if (found_key == 0 || pkey == NULL) {
			TEST_info("skipping, key '%s' is disabled",
				  kdata->keyname);
			t->skip = 1;
			goto ok;
		}
		...
		/* Encoded public key */
		if ((pkey = EVP_PKEY_new_raw_public_key_ex(
			     libctx, kdata->algname, propquery,
		...
		/* Encoded private key */
		if ((pkey = EVP_PKEY_new_raw_private_key_ex(
			     libctx, kdata->algname, propquery,
	...
	if (!TEST_ptr(kdata->ctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey,
							      propquery)))
		goto err;
	...
	if (kdata->input == NULL) {
		...
		ret = encapsulate(t, kdata->ctx, kdata->op, &wrapped,
				  &wrappedlen, &secret, &secretlen);
		if (ret == 0 || t->err != NULL)
			goto err;

		/* Also attempt to decrypt if we have the private key */
		if (found_key || kdata->encoded_priv_key != NULL)
			ret = decapsulate(t, kdata->ctx, kdata->op, wrapped,
					  wrappedlen, secret, secretlen);
	} else {
		ret = decapsulate(t, kdata->ctx, kdata->op, kdata->input,
				  kdata->inputlen, kdata->output,
				  kdata->outputlen);
	}
...
static const EVP_TEST_METHOD pkey_kem_test_method = {
	"Kem", kem_test_init, kem_test_cleanup, kem_test_parse, kem_test_run
};
...
static int pkey_test_init(EVP_TEST *t, const char *name, int use_public,
			  int (*keyopinit)(EVP_PKEY_CTX *ctx,
					   const OSSL_PARAM params[]),
			  int (*keyop)(EVP_PKEY_CTX *ctx, unsigned char *sig,
...
static int pkey_test_init_ex2(
	EVP_TEST *t, const char *name, int use_public,
		     ...
		     const unsigned char *tbs, size_t tbslen))
{
	...
	if ((p = strchr(name, ':')) == NULL || p == name || p[1] == '\0' ||
		...
		TEST_info("Can't extract algorithm or key name from '%s'",
			  name);
		return 0;
	}
	...
	kdata->keyop = keyop;
	if (!TEST_ptr(kdata->sigalgo = EVP_SIGNATURE_fetch(libctx, algoname,
							   propquery))) {
...
static int pkey_test_ctrl(EVP_TEST *t, EVP_PKEY_CTX *pctx, const char *value)
{
	int rv = 0;
	char *p, *tmpval;
...
static int pkey_test_parse(EVP_TEST *t, const char *keyword, const char *value)
{
	PKEY_DATA *kdata = t->data;
	if (strcmp(keyword, "Input") == 0)
...
static int pkey_test_run_init(EVP_TEST *t)
{
	...
	int i, ret = 0;
	OSSL_PARAM params[5] = { OSSL_PARAM_END, OSSL_PARAM_END, OSSL_PARAM_END,
				 OSSL_PARAM_END, OSSL_PARAM_END };
	OSSL_PARAM *p = NULL;
	size_t params_n = 0, params_n_allocstart = 0;

	if (sk_OPENSSL_STRING_num(data->init_controls) > 0) {
		if (!ctrl2params(t, data->init_controls, NULL, params,
				 OSSL_NELEM(params), &params_n))
			goto err;
		p = params;
	}
...
static int pkey_test_run(EVP_TEST *t)
{
	...
	if (!pkey_check_security_category(
		    t, EVP_PKEY_CTX_get0_pkey(expected->ctx)))
		goto err;
	...
	if (expected->keyop(expected->ctx, NULL, &got_len, expected->input,
			    expected->input_len) <= 0 ||
	    !TEST_ptr(got = OPENSSL_malloc(got_len))) {
		t->err = "KEYOP_LENGTH_ERROR";
		goto err;
	}
	if (expected->keyop(expected->ctx, got, &got_len, expected->input,
			    expected->input_len) <= 0) {
		t->err = "KEYOP_ERROR";
		goto err;
	}

	if (!memory_err_compare(t, "KEYOP_MISMATCH", expected->output,
				expected->output_len, got, got_len))
		goto err;
...
static int pkey_fromdata_test_parse(EVP_TEST *t, const char *keyword,
				    const char *value)
{
	PKEY_DATA *kdata = t->data;
...
static int pkey_fromdata_test_run(EVP_TEST *t)
{
	...
	static const OSSL_PARAM key_settable_ctx_params[] = {
		...
		OSSL_PARAM_octet_string("pub", NULL, 0), OSSL_PARAM_END
	};
	OSSL_PARAM params[5] = { OSSL_PARAM_END, OSSL_PARAM_END, OSSL_PARAM_END,
				 OSSL_PARAM_END, OSSL_PARAM_END };
	OSSL_PARAM *p = NULL;
...
static const EVP_TEST_METHOD pkey_fromdata_test_method = {
	"KeyFromData", pkey_fromdata_test_init, pkey_fromdata_test_cleanup,
	pkey_fromdata_test_parse, pkey_fromdata_test_run
};
...
{
	if (strchr(name, ':') != NULL)
		return pkey_test_init_ex2(t, name, 0, EVP_PKEY_sign_init_ex2,
					  EVP_PKEY_sign);
	return pkey_test_init(t, name, 0, EVP_PKEY_sign_init_ex, EVP_PKEY_sign);
}

static const EVP_TEST_METHOD psign_test_method = { "Sign", sign_test_init,
						   pkey_test_cleanup,
						   pkey_test_parse,
						   pkey_test_run };
...
{
	return pkey_test_init_ex2(t, name, 0, EVP_PKEY_sign_message_init,
				  EVP_PKEY_sign);
}

static const EVP_TEST_METHOD psign_message_test_method = {
	"Sign-Message", sign_test_message_init, pkey_test_cleanup,
...
static const EVP_TEST_METHOD pverify_recover_test_method = {
	"VerifyRecover", verify_recover_test_init, pkey_test_cleanup,
	pkey_test_parse, pkey_test_run
};
...
static const EVP_TEST_METHOD pdecrypt_test_method = {
	"Decrypt", decrypt_test_init, pkey_test_cleanup, pkey_test_parse,
	pkey_test_run
};
...
{
	if (strchr(name, ':') != NULL)
		return pkey_test_init_ex2(t, name, 1, EVP_PKEY_verify_init_ex2,
					  NULL);
...
static int verify_test_run(EVP_TEST *t)
{
	...
	if (!pkey_test_run_init(t))
		goto err;
	if (!pkey_check_security_category(t,
					  EVP_PKEY_CTX_get0_pkey(kdata->ctx)))
		goto err;
...
static const EVP_TEST_METHOD pverify_test_method = { "Verify", verify_test_init,
						     pkey_test_cleanup,
						     pkey_test_parse,
						     verify_test_run };
...
{
	return pkey_test_init_ex2(t, name, 0, EVP_PKEY_verify_message_init,
				  NULL);
}

static const EVP_TEST_METHOD pverify_message_test_method = {
	"Verify-Message", verify_message_test_init, pkey_test_cleanup,
...
{
	return pkey_test_init_ex2(t, name, 1, EVP_PKEY_verify_message_init,
				  NULL);
}

static const EVP_TEST_METHOD pverify_message_public_test_method = {
	"Verify-Message-Public", verify_message_public_test_init,
	pkey_test_cleanup, pkey_test_parse, verify_test_run
};
...
static int pderive_test_parse(EVP_TEST *t, const char *keyword,
			      const char *value)
{
	PKEY_DATA *kdata = t->data;
	int validate = 0;
...
static int pderive_test_run(EVP_TEST *t)
{
	...
	if (!memory_err_compare(t, "SHARED_SECRET_MISMATCH", expected->output,
				expected->output_len, got, got_len))
		goto err;
...
static const EVP_TEST_METHOD pderive_test_method = {
	"Derive", pderive_test_init, pkey_test_cleanup, pderive_test_parse,
	pderive_test_run
};
...
static int scrypt_test_parse(EVP_TEST *t, const char *keyword,
			     const char *value)
{
...
static int pbkdf2_test_parse(EVP_TEST *t, const char *keyword,
			     const char *value)
{
	PBE_DATA *pdata = t->data;
...
static int pkcs12_test_parse(EVP_TEST *t, const char *keyword,
			     const char *value)
{
	PBE_DATA *pdata = t->data;
...
static int pbe_test_parse(EVP_TEST *t, const char *keyword, const char *value)
{
	PBE_DATA *pdata = t->data;
...
static int pbe_test_run(EVP_TEST *t)
{
	...
	if (expected->pbe_type == PBE_TYPE_PBKDF2) {
		if (PKCS5_PBKDF2_HMAC((char *)expected->pass,
				      (int)expected->pass_len, expected->salt,
				      (int)expected->salt_len, expected->iter,
				      expected->md, (int)expected->key_len,
				      key) == 0) {
			t->err = "PBKDF2_ERROR";
			goto err;
		}
		...
		if (EVP_PBE_scrypt((const char *)expected->pass,
				   expected->pass_len, expected->salt,
				   expected->salt_len, expected->N, expected->r,
				   expected->p, expected->maxmem, key,
				   expected->key_len) == 0) {
			t->err = "SCRYPT_ERROR";
			goto err;
		}
		...
		fetched_digest = EVP_MD_fetch(
			libctx, EVP_MD_get0_name(expected->md), propquery);
		if (fetched_digest == NULL) {
			t->err = "PKCS12_ERROR";
			goto err;
		}
	...
	if (!memory_err_compare(t, "KEY_MISMATCH", expected->key,
				expected->key_len, key, expected->key_len))
		goto err;
...
static const EVP_TEST_METHOD pbe_test_method = { "PBE", pbe_test_init,
						 pbe_test_cleanup,
...
static int encode_test_parse(EVP_TEST *t, const char *keyword,
			     const char *value)
{
	ENCODE_DATA *edata = t->data;
...
static int encode_test_run(EVP_TEST *t)
{
	...
	if (expected->encoding == BASE64_CANONICAL_ENCODING) {
		if (!TEST_ptr(encode_ctx = EVP_ENCODE_CTX_new()) ||
		    !TEST_ptr(encode_out = OPENSSL_malloc(
				      EVP_ENCODE_LENGTH(expected->input_len))))
			goto err;
		...
		do {
			...
			if (data_chunk_size == 0 || current_len > input_len)
				current_len = input_len;
			if (!TEST_true(EVP_EncodeUpdate(
				    encode_ctx, encode_out, &chunk_len,
		...
		EVP_EncodeFinal(encode_ctx, encode_out + output_len,
				&chunk_len);
		output_len += chunk_len;

		if (!memory_err_compare(t, "BAD_ENCODING", expected->output,
					expected->output_len, encode_out,
					output_len))
			goto err;
	}
	...
	do {
		...
		if (data_chunk_size == 0 || (size_t)data_chunk_size > input_len)
			current_len = input_len;
		if (EVP_DecodeUpdate(decode_ctx, decode_out + output_len,
				     &chunk_len, expected->output + donelen,
				     (int)current_len) < 0) {
			t->err = "DECODE_ERROR";
			goto err;
		}
	...
	if (EVP_DecodeFinal(decode_ctx, decode_out + output_len, &chunk_len) !=
	    1) {
		t->err = "DECODE_ERROR";
		goto err;
	}
	...
	if (expected->encoding != BASE64_INVALID_ENCODING &&
	    !memory_err_compare(t, "BAD_DECODING", expected->input,
				expected->input_len, decode_out, output_len)) {
		t->err = "BAD_DECODING";
		goto err;
	}
...
static const EVP_TEST_METHOD encode_test_method = {
	"Encoding",	   encode_test_init, encode_test_cleanup,
	encode_test_parse, encode_test_run,
};
...
typedef struct rand_data_pass_st {
	...
	size_t entropy_len, nonce_len, pers_len, addinA_len, addinB_len,
		pr_entropyA_len, pr_entropyB_len, output_len,
		reseed_entropy_len, reseed_addin_len;
} RAND_DATA_PASS;
...
static int rand_test_parse(EVP_TEST *t, const char *keyword, const char *value)
{
	RAND_DATA *rdata = t->data;
	RAND_DATA_PASS *item;
	const char *p;
	...
	if ((p = strchr(keyword, '.')) != NULL) {
		...
		if (HAS_PREFIX(keyword, "Entropy."))
			return parse_bin(value, &item->entropy,
					 &item->entropy_len);
...
static int rand_test_run(EVP_TEST *t)
{
	...
	if (sk_OPENSSL_STRING_num(expected->init_controls) > 0) {
		if (!ctrl2params(t, expected->init_controls, NULL, params,
				 OSSL_NELEM(params), &params_n))
			goto err;
	}
	p = params + params_n;
	...
	*p = OSSL_PARAM_construct_end();
	...
	for (i = 0; i <= expected->n; i++) {
		...
		*p++ = OSSL_PARAM_construct_octet_string(
			OSSL_RAND_PARAM_TEST_ENTROPY, z, item->entropy_len);
		z = item->nonce != NULL ? item->nonce : (unsigned char *)"";
		*p++ = OSSL_PARAM_construct_octet_string(
			OSSL_RAND_PARAM_TEST_NONCE, z, item->nonce_len);
		*p = OSSL_PARAM_construct_end();
		...
		if (!TEST_true(EVP_RAND_instantiate(
			    ...
			    expected->prediction_resistance, z, item->pers_len,
			    NULL)))
			goto err;
		...
		if (item->reseed_entropy != NULL) {
			params[0] = OSSL_PARAM_construct_octet_string(
				...
				item->reseed_entropy, item->reseed_entropy_len);
			params[1] = OSSL_PARAM_construct_end();
			if (!TEST_true(EVP_RAND_CTX_set_params(expected->parent,
							       params)))
				goto err;

			if (!TEST_true(EVP_RAND_reseed(
				    expected->ctx,
				    expected->prediction_resistance, NULL, 0,
				    item->reseed_addin,
				    item->reseed_addin_len)))
				goto err;
		}
		...
		if (!TEST_true(EVP_RAND_generate(
			    expected->ctx, got, got_len, strength,
...
static const EVP_TEST_METHOD rand_test_method = { "RAND", rand_test_init,
						  rand_test_cleanup,
						  rand_test_parse,
						  rand_test_run };
...
static int kdf_test_ctrl(EVP_TEST *t, EVP_KDF_CTX *kctx, const char *value)
{
	KDF_DATA *kdata = t->data;
	int rv;
	char *p, *name;
	...
	*++kdata->p = OSSL_PARAM_construct_end();
	...
	if ((strcmp(name, "cipher") == 0 || strcmp(name, "cekalg") == 0) &&
...
static int kdf_test_parse(EVP_TEST *t, const char *keyword, const char *value)
{
	KDF_DATA *kdata = t->data;
...
static int kdf_test_run(EVP_TEST *t)
{
	...
	if (!memory_err_compare(t, "KDF_MISMATCH", expected->output,
				expected->output_len, got, got_len))
		goto err;
...
static const EVP_TEST_METHOD kdf_test_method = { "KDF", kdf_test_init,
						 kdf_test_cleanup,
...
static int pkey_kdf_test_parse(EVP_TEST *t, const char *keyword,
			       const char *value)
{
	PKEY_KDF_DATA *kdata = t->data;
...
static int pkey_kdf_test_run(EVP_TEST *t)
{
	...
	if (!TEST_mem_eq(expected->output, expected->output_len, got,
			 got_len)) {
		t->err = "KDF_MISMATCH";
		goto err;
	}
...
static const EVP_TEST_METHOD pkey_kdf_test_method = {
	"PKEYKDF", pkey_kdf_test_init, pkey_kdf_test_cleanup,
	pkey_kdf_test_parse, pkey_kdf_test_run
};
...
static const EVP_TEST_METHOD keypair_test_method = {
	"PrivPubKeyPair", keypair_test_init, keypair_test_cleanup,
	void_test_parse, keypair_test_run
};
...
static int keygen_test_parse(EVP_TEST *t, const char *keyword,
			     const char *value)
{
	KEYGEN_TEST_DATA *keygen = t->data;
				      ...
				      STACK_OF(OPENSSL_STRING) *out_controls)
{
	...
	if (sk_OPENSSL_STRING_num(out_controls) > 0) {
		if (!ctrl2params(t, out_controls,
				 EVP_PKEY_gettable_params(pkey), out_params,
		...
		for (p = out_params; p->key != NULL; ++p) {
			if (p->data_type == OSSL_PARAM_OCTET_STRING) {
				...
				ret = EVP_PKEY_get_octet_string_param(
					      pkey, p->key, data, p->data_size,
					      &len) &&
				      len == p->data_size &&
				      (TEST_mem_eq(p->data, len, data, len) ==
				       1);
				OPENSSL_free(data);
				if (ret == 0) {
					TEST_error(
						"Expected %s value is incorrect",
						p->key);
					goto err;
				}
			}
...
static int keygen_test_run(EVP_TEST *t)
{
	...
	if (keygen->paramname != NULL) {
		...
		if (!TEST_ptr(genctx = EVP_PKEY_CTX_new_from_pkey(
				      libctx, keyparams, propquery)))
			goto err;

	} else {
		if (!TEST_ptr(genctx = EVP_PKEY_CTX_new_from_name(
				      libctx, keygen->alg, propquery)))
			goto err;
	}
	...
	if (sk_OPENSSL_STRING_num(keygen->in_controls) > 0) {
		if ((params = OPENSSL_malloc_array(4, sizeof(OSSL_PARAM))) ==
		    NULL)
			goto err;
		if (!ctrl2params(t, keygen->in_controls,
				 EVP_PKEY_CTX_settable_params(genctx), params,
				 4, &params_n))
			goto err;
...
static const EVP_TEST_METHOD keygen_test_method = {
	"KeyGen",	   keygen_test_init, keygen_test_cleanup,
	keygen_test_parse, keygen_test_run,
};
...
static int digestsigver_test_parse(EVP_TEST *t, const char *keyword,
				   const char *value)
{
	DIGESTSIGN_DATA *mdata = t->data;
...
static int check_deterministic_noncetype(EVP_TEST *t, DIGESTSIGN_DATA *mdata)
{
...
static int signverify_init(EVP_TEST *t, DIGESTSIGN_DATA *data)
{
	...
	if (data->is_verify) {
		if (!EVP_DigestVerifyInit_ex(data->ctx, &data->pctx, name,
					     libctx, NULL, data->key, p)) {
			t->err = "DIGESTVERIFYINIT_ERROR";
			goto err;
		}
	} else {
		if (!EVP_DigestSignInit_ex(data->ctx, &data->pctx, name, libctx,
					   NULL, data->key, p)) {
			t->err = "DIGESTSIGNINIT_ERROR";
			goto err;
		}
...
static int digestsign_test_run(EVP_TEST *t)
{
	...
	if (!memory_err_compare(t, "SIGNATURE_MISMATCH", expected->output,
				expected->output_len, got, got_len))
		goto err;
...
static const EVP_TEST_METHOD digestsign_test_method = {
	"DigestSign", digestsign_test_init, digestsigver_test_cleanup,
...
static int digestverify_test_run(EVP_TEST *t)
{
	...
	if (!evp_test_buffer_do(mdata->input, digestverify_update_fn,
				mdata->ctx)) {
		t->err = "DIGESTUPDATE_ERROR";
		return 1;
	}
...
static const EVP_TEST_METHOD digestverify_test_method = {
	...
	digestsigver_test_parse, digestverify_test_run
};
...
static int oneshot_digestsign_test_run(EVP_TEST *t)
{
	...
	if (!EVP_DigestSign(expected->ctx, NULL, &got_len, expected->osin,
			    expected->osin_len)) {
	...
	got_len *= 2;
	if (!EVP_DigestSign(expected->ctx, got, &got_len, expected->osin,
			    expected->osin_len)) {
...
static const EVP_TEST_METHOD oneshot_digestsign_test_method = {
	"OneShotDigestSign", oneshot_digestsign_test_init,
	digestsigver_test_cleanup, digestsigver_test_parse,
...
static const EVP_TEST_METHOD oneshot_digestverify_test_method = {
	"OneShotDigestVerify", oneshot_digestverify_test_init,
	digestsigver_test_cleanup, digestsigver_test_parse,
...
static int check_test_error(EVP_TEST *t)
{
	...
	if (t->err != NULL && t->expected_err == NULL) {
		if (t->aux_err != NULL) {
			TEST_info(
				"%s:%d: Source of above error (%s); unexpected error %s",
				t->s.test_file, t->s.start, t->aux_err, t->err);
		} else {
			TEST_info(
				"%s:%d: Source of above error; unexpected error %s",
	...
	if (strcmp(t->err, t->expected_err) != 0) {
		TEST_info("%s:%d: Expected %s got %s", t->s.test_file,
	...
	TEST_info("%s:%d: Expected error \"%s\", got \"%s\"", t->s.test_file,
		  t->s.start, t->reason, reason);
	...
	if (t->skip) {
		...
		if (t->entropy != NULL) {
			...
			fake_rand_set_public_private_callbacks(
				libctx, &get_faked_bytes);
		}
		/* run the test */
		if (t->err == NULL && t->meth->run_test(t) != 1) {
			TEST_info("%s:%d %s error", t->s.test_file, t->s.start,
				  t->meth->name);
			goto err;
		}
...
static int key_unsupported(void)
{
     ...
     */
	if (lib == ERR_LIB_EC &&
	    (reason == EC_R_UNKNOWN_GROUP || reason == EC_R_INVALID_CURVE)) {
		ERR_clear_error();
		return 1;
	}
#endif /* OPENSSL_NO_EC */
...
static int prov_available(char *providers)
{
	...
	while (more) {
		...
		for (p = providers; *p != '\0' && !isspace((unsigned char)(*p));
		     p++)
			continue;
		if (*p == '\0')
			more = 0;
...
start:
	if (strcmp(pp->key, "PrivateKey") == 0) {
		pkey = PEM_read_bio_PrivateKey_ex(t->s.key, NULL, 0, NULL,
						  libctx, NULL);
		...
		pkey = PEM_read_bio_PUBKEY_ex(t->s.key, NULL, 0, NULL, libctx,
					      NULL);
		if (pkey == NULL && !key_unsupported()) {
			EVP_PKEY_free(pkey);
		...
		if (klist == &private_keys)
			pkey = EVP_PKEY_new_raw_private_key_ex(
				libctx, strnid, NULL, keybin, keylen);
		else
		...
		if (!prov_available(pp->value)) {
			TEST_info(
				"skipping, '%s' provider not available: %s:%d",
		...
		if (prov_available("fips")) {
			...
			if (j < 0) {
				TEST_info(
					"Line %d: error matching FIPS versions\n",
					t->s.curr);
				return 0;
			} else if (j == 0) {
				TEST_info(
					"skipping, FIPS provider incompatible version: %s:%d",
					t->s.test_file, t->s.start);
				t->skip = 1;
				return 0;
			}
	...
	for (pp++, i = 1; i < (t->s.numpairs - skipped); pp++, i++) {
		if (strcmp(pp->key, "Securitycheck") == 0) {
			...
			{
				TEST_info(
					"skipping, Securitycheck is disabled: %s:%d",
					t->s.test_file, t->s.start);
				t->skip = 1;
				return 0;
			}
			...
			if (t->expected_err != NULL) {
				TEST_info("Line %d: multiple result lines",
					  t->s.curr);
				return 0;
			}
			...
			if (t->reason != NULL) {
				TEST_info("Line %d: multiple reason lines",
					  t->s.curr);
				return 0;
			}
			t->reason = take_value(pp);
		} else if (strcmp(pp->key, "Threads") == 0) {
			if (OSSL_set_max_threads(libctx, atoi(pp->value)) ==
			    0) {
				TEST_info(
					"skipping, '%s' threads not available: %s:%d",
					pp->value, t->s.test_file, t->s.start);
				t->skip = 1;
			}
			...
			if (t->security_category >= 0) {
				TEST_info(
					"Line %d: multiple security category lines",
					t->s.curr);
				return 0;
			}
			...
			if (t->security_category < 0 ||
				...
				TEST_info(
					"Line %d: invalid security category, should be 0..5",
					t->s.curr);
				return 0;
			}
		} else if (strcmp(pp->key, "Test-Entropy") == 0) {
			if (!parse_bin(pp->value, &t->entropy,
				       &t->entropy_len)) {
				TEST_info("Line %d: invalid entropy",
					  t->s.curr);
				return 0;
			}
		} else {
			...
			if (rv == 0) {
				TEST_info("Line %d: unknown keyword %s",
					  t->s.curr, pp->key);
				return 0;
			}
			if (rv < 0) {
...
const OPTIONS *test_get_options(void)
{
	static const OPTIONS test_options[] = {
		...
		{ "chunk", OPT_DATA_CHUNK, 'N',
		  "Size of data chunks to be processed, 0 for default size" },
...
int setup_tests(void)
{
	...
	while ((o = opt_next()) != OPT_EOF) {
		...
		case OPT_CONFIG_FILE:
			...
			break;
		case OPT_IN_PLACE:
			if ((process_mode_in_place =
				     evp_test_process_mode(opt_arg())) == -1)
			case OPT_DATA_CHUNK:
	...
	if (!test_get_libctx(&libctx, &prov_null, config_file, &libprov,
			     provider_name))
		return 0;
