...
static int test_bio_callback(void)
{
    ...
    if (!TEST_int_eq(i, 4)
            ...
            || !TEST_ptr_eq(my_param_argi[0], 4)
            || !TEST_ptr_eq(my_param_argl[0], 0L)
            || !TEST_ptr_eq(my_param_ret[0], 1L)
            || !TEST_ptr_eq(my_param_b[1], bio)
            || !TEST_int_eq(my_param_oper[1], BIO_CB_WRITE | BIO_CB_RETURN)
            || !TEST_ptr_eq(my_param_argp[1], test1)
            || !TEST_ptr_eq(my_param_argi[1], 4)
            || !TEST_ptr_eq(my_param_argl[1], 0L)
            || !TEST_ptr_eq(my_param_ret[1], 4L))
        goto err;
    ...
    if (!TEST_int_eq(i, 5)
            ...
            || !TEST_ptr_eq(my_param_argi[2], 0)
            || !TEST_ptr_eq(my_param_argl[2], 0L)
            || !TEST_ptr_eq(my_param_ret[2], 1L)
            || !TEST_ptr_eq(my_param_b[3], bio)
            || !TEST_int_eq(my_param_oper[3], BIO_CB_PUTS | BIO_CB_RETURN)
            || !TEST_ptr_eq(my_param_argp[3], test2)
            || !TEST_ptr_eq(my_param_argi[3], 0)
            || !TEST_ptr_eq(my_param_argl[3], 0L)
            || !TEST_ptr_eq(my_param_ret[3], 5L))
        goto err;
