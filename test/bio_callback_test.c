...
static int test_bio_callback_ex(void)
{
    ...
    if (!TEST_int_eq(i, 0)
        ...
        || !TEST_int_eq(my_param_oper[3], BIO_CB_READ | BIO_CB_RETURN)
        || !TEST_ptr_eq(my_param_argp[3], buf)
        || !TEST_size_t_eq(my_param_len[3], sizeof(buf))
        || !TEST_long_eq(my_param_argl[3], 0L)
        || !TEST_size_t_eq(my_param_processed[3], 0)
        || !TEST_int_eq((int)my_param_ret[3], 0))
        goto err;
...
static int test_bio_callback(void)
{
    ...
    if (!TEST_int_eq(i, 0)
        ...
        || !TEST_int_eq(my_param_oper[3], BIO_CB_READ | BIO_CB_RETURN)
        || !TEST_ptr_eq(my_param_argp[3], buf)
        || !TEST_int_eq(my_param_argi[3], sizeof(buf))
        || !TEST_long_eq(my_param_argl[3], 0L)
        || !TEST_long_eq(my_param_ret[3], 0L))
        goto err;
