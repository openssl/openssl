...
static int test_bio_callback_ex(void)
{
	...
	if (!TEST_int_eq(i, (int)test1len) || !TEST_int_eq(my_param_count, 2) ||
	...
	my_param_count = 0;
	i = BIO_read(bio, buf, sizeof(buf));
	if (!TEST_int_eq(i, -1) || !TEST_int_eq(my_param_count, 2) ||
	    !TEST_ptr_eq(my_param_b[0], bio) ||
	    !TEST_int_eq(my_param_oper[0], BIO_CB_READ) ||
	    !TEST_ptr_eq(my_param_argp[0], buf) ||
	    !TEST_size_t_eq(my_param_len[0], sizeof(buf)) ||
	...
	my_param_count = 0;
	i = BIO_read(bio, buf, sizeof(buf));
	if (!TEST_int_eq(i, 0) || !TEST_int_eq(my_param_count, 2) ||
	    !TEST_ptr_eq(my_param_b[0], bio) ||
	    !TEST_int_eq(my_param_oper[0], BIO_CB_READ) ||
	    !TEST_ptr_eq(my_param_argp[0], buf) ||
	    !TEST_size_t_eq(my_param_len[0], sizeof(buf)) ||
	...
	if (!TEST_int_eq(i, 5) || !TEST_int_eq(my_param_count, 2) ||
	    !TEST_ptr_eq(my_param_b[0], bio) ||
	...
	my_param_count = 0;
	i = BIO_free(bio);
	if (!TEST_int_eq(i, 1) || !TEST_int_eq(my_param_count, 1) ||
	    !TEST_ptr_eq(my_param_b[0], bio) ||
...
static int test_bio_callback(void)
{
	...
	if (!TEST_int_eq(i, test1len) || !TEST_int_eq(my_param_count, 2) ||
	    !TEST_ptr_eq(my_param_b[0], bio) ||
	    !TEST_int_eq(my_param_oper[0], BIO_CB_WRITE) ||
	    !TEST_ptr_eq(my_param_argp[0], test1) ||
