...
static BIGNUM *getBN(STANZA *s, const char *attribute)
{
	...
	if ((hex = findattr(s, attribute)) == NULL) {
		TEST_error("%s:%d: Can't find %s", s->test_file, s->start,
			   attribute);
		return NULL;
	}
...
static int test_div_recip(void)
{
	...
	for (i = 0; i < NUM0 + NUM1; i++) {
		if (i < NUM1) {
			...
			if (!(TEST_true(BN_bntest_rand(b, 50 + 3 * (i - NUM1),
						       0, 0))))
				goto err;
		}
		BN_set_negative(a, rand_neg());
...
static int test_gf2m_modsolvequad(void)
{
	...
	for (i = 0; i < NUM0; i++) {
		...
		for (j = 0; j < 2; j++) {
			...
			if (t) {
				s++;
				if (!(TEST_true(BN_GF2m_mod_sqr(d, c, b[j],
								ctx)) &&
				      TEST_true(BN_GF2m_add(d, c, d)) &&
...
static int file_sum(STANZA *s)
{
	...
	if (!TEST_true(BN_copy(ret, a)) || !TEST_true(BN_add(ret, ret, b)) ||
	    ...
	    !TEST_true(BN_copy(ret, a)) || !TEST_true(BN_sub(ret, sum, ret)) ||
...
static int file_modmul(STANZA *s)
{
	...
	if (BN_is_odd(m)) {
		...
		if (mont == NULL || a_tmp == NULL || b_tmp == NULL ||
		    ...
		    !TEST_true(BN_mod_mul_montgomery(ret, a_tmp, b_tmp, mont,
						     ctx)) ||
...
static int file_modexp(STANZA *s)
{
	...
	if (BN_is_odd(m)) {
		if (!TEST_true(BN_mod_exp_mont(ret, a, e, m, ctx, NULL)) ||
		    ...
		    !TEST_true(BN_mod_exp_mont_consttime(ret, a, e, m, ctx,
							 NULL)) ||
...
{
	BIGNUM *a = NULL, *p = NULL, *mod_sqrt = NULL, *ret = NULL,
	       *ret2 = NULL;
	int st = 0;
...
#define BOTTOM_BIT_NOTOUCH 0
		if (!TEST_true(BN_rand(n, bytes * 8, TOP_BIT_ON,
				       BOTTOM_BIT_NOTOUCH)))
			goto err;
		...
		if (!TEST_int_eq(BN_bn2binpad(n, out, bytes + 1), bytes + 1) ||
			...
			goto err;
		/* Pad up to 256. */
		if (!TEST_int_eq(BN_bn2binpad(n, out, sizeof(out)),
				 sizeof(out)) ||
		    !TEST_mem_eq(out + sizeof(out) - bytes, bytes, reference,
				 bytes) ||
...
static int test_bn2signed(int i)
{
	...
	if (!TEST_ptr(bn2 = BN_signed_lebin2bn(reversed, sizeof(reversed),
					       NULL)) ||
	    !TEST_BN_eq(bn, bn2))
		goto err;
...
static int test_dec2bn(void)
{
	...
	if (!TEST_int_eq(parsedecBN(&bn, "256"), 3) ||
	    !TEST_BN_eq_word(bn, 256) || !TEST_BN_ge_zero(bn) ||
...
static int test_hex2bn(void)
{
	...
	if (!TEST_int_eq(parseBN(&bn, "256"), 3) ||
	    !TEST_BN_eq_word(bn, 0x256) || !TEST_BN_ge_zero(bn) ||
	    !TEST_BN_gt_zero(bn) || !TEST_BN_ne_zero(bn) || !TEST_BN_even(bn))
		goto err;
	BN_free(bn);
	bn = NULL;
...
static int test_asc2bn(void)
{
	...
	if (!TEST_true(BN_asc2bn(&bn, "0x1234")) ||
	    !TEST_BN_eq_word(bn, 0x1234) || !TEST_BN_ge_zero(bn))
		goto err;
...
static int test_rand_range_single(size_t n)
{
	...
	if (sum > critical) {
		...
		TEST_note("test case %zu  range %u  iterations %u", n + 1,
			  range, iterations);
		goto err;
	}
...
static int test_rand_range(void)
{
	...
	if (TEST_int_ge(n_success, binomial_critical))
		return 1;
	TEST_note(
		"This test is expected to fail by chance 0.01%% of the time.");
	return 0;
}
...
static int test_badmod(void)
{
	...
	if (!TEST_false(BN_mod_exp_mont(a, BN_value_one(), BN_value_one(), b,
					ctx, NULL)))
		goto err;
	ERR_clear_error();
...
static int test_expmodzero(void)
{
	...
	if (!TEST_true(BN_mod_exp(r, a, zero, BN_value_one(), NULL)) ||
	    ...
	    !TEST_BN_eq_zero(r) ||
	    !TEST_true(BN_mod_exp_mont_consttime(r, a, zero, BN_value_one(),
						 NULL, NULL)) ||
	    !TEST_BN_eq_zero(r) ||
	    !TEST_true(BN_mod_exp_mont_word(r, 42, zero, BN_value_one(), NULL,
					    NULL)) ||
	    !TEST_BN_eq_zero(r))
		goto err;
...
static int test_smallprime(int kBits)
{
	...
	if (kBits <= 1) {
		if (!TEST_false(BN_generate_prime_ex(r, kBits, 0, NULL, NULL,
						     NULL)))
			goto err;
	} else {
		if (!TEST_true(BN_generate_prime_ex(r, kBits, 0, NULL, NULL,
						    NULL)) ||
...
static int test_smallsafeprime(int kBits)
{
	...
	if (kBits <= 5 && kBits != 3) {
		if (!TEST_false(BN_generate_prime_ex(r, kBits, 1, NULL, NULL,
						     NULL)))
			goto err;
	} else {
		if (!TEST_true(BN_generate_prime_ex(r, kBits, 1, NULL, NULL,
						    NULL)) ||
		    !TEST_int_eq(BN_num_bits(r), kBits))
			goto err;
	}
...
static int test_gcd_prime(void)
{
	...
	for (i = 0; i < NUM_PRIME_TESTS; i++) {
		if (!TEST_true(BN_generate_prime_ex(b, 1024, 0, NULL, NULL,
						    NULL)) ||
...
static int file_test_run(STANZA *s)
{
	...
	for (; --numtests >= 0; tp++) {
		if (findattr(s, tp->name) != NULL) {
			if (!tp->func(s)) {
				TEST_info("%s:%d: Failed %s test", s->test_file,
					  s->start, tp->name);
				return 0;
			}
			return 1;
...
const OPTIONS *test_get_options(void)
{
	static const OPTIONS test_options[] = {
		...
		{ "stochastic", OPT_STOCHASTIC_TESTS, '-',
