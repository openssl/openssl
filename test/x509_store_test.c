 ...
 * contexts: one in the default (SHA-1-capable) context and one in a context
 * with only the "base" provider loaded, so its SHA-1 fingerprint cannot be
 ...
 *
 * X509_cmp() orders two fingerprinted certs by their SHA-1 hash yet falls back
 ...
 * DER decoded in the default (SHA-1-capable) context and in a base-only context
 * that cannot compute its SHA-1 fingerprint (EXFLAG_NO_FINGERPRINT). Unlike
