...
static int do_PRF(const void *seed1, int seed1_len, const void *seed2,
		  ...
		  unsigned char *out, int olen)
{
	...
	EVP_PKEY_CTX_set1_tls1_prf_secret(pctx, master_secret,
...
#define SS_SESSID_OFS 15 /* Session ID goes here */
		...
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x04, 0x30, /* OCTET_STRING, master secret */
#define SS_SECRET_OFS 49 /* Master secret goes here */
	...
	memcpy(session_asn1 + SS_SECRET_OFS, master_secret,
	       sizeof(master_secret));
...
#define SH_RANDOM_OFS 27 /* Server random goes here */
		...
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x20, /* Session ID length */
#define SH_SESSID_OFS 60 /* Session ID goes here */
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x2f, /* Cipher suite AES128-SHA */
	...
	memcpy(server_hello + SH_RANDOM_OFS, server_random,
		       ...
		       const void *msg, size_t len)
{
	...
	if (!EVP_MAC_init(ctx, mac_key, 20, params) ||
	    !EVP_MAC_update(ctx, epoch, 2) || !EVP_MAC_update(ctx, seq, 6) ||
...
static int send_finished(SSL *s, BIO *rbio)
{
	...
	do_PRF(TLS_MD_KEY_EXPANSION_CONST, TLS_MD_KEY_EXPANSION_CONST_SIZE,
	       server_random, SSL3_RANDOM_SIZE, client_random, SSL3_RANDOM_SIZE,
	...
	do_PRF(TLS_MD_SERVER_FINISH_CONST, TLS_MD_SERVER_FINISH_CONST_SIZE,
	       handshake_hash, md_size, NULL, 0,
	       finished_msg + DTLS1_HM_HEADER_LENGTH, TLS1_FINISH_MAC_LENGTH);

	return send_record(rbio, SSL3_RT_HANDSHAKE, 0, finished_msg,
			   sizeof(finished_msg));
}
...
static struct {
	...
	NODROP(0xffff),	   NODROP(0x10001),   NODROP(0xfffe),
	NODROP(0x10000),   DROP(0x10001),     DROP(0xff),
	NODROP(0x100000),  NODROP(0x800000),  NODROP(0x7fffe1),
	NODROP(0xffffff),  NODROP(0x1000000), NODROP(0xfffffe),
	DROP(0xffffff),	   NODROP(0x1000010), NODROP(0xfffffd),
	NODROP(0x1000011), DROP(0x12),	      NODROP(0x1000012),
	NODROP(0x1ffffff), NODROP(0x2000000), DROP(0x1ff00fe),
	NODROP(0x2000001), NODROP(0x20fffff), NODROP(0x2105500),
	DROP(0x20ffffe),   NODROP(0x21054ff), NODROP(0x211ffff),
	DROP(0x2110000),   NODROP(0x2120000)
...
static int test_bad_dtls(void)
{
	...
	if (!TEST_ptr(handshake_md) ||
	    !TEST_true(EVP_DigestInit_ex(handshake_md, EVP_md5_sha1(), NULL)))
		goto end;
	...
	ret = SSL_do_handshake(con);
	if (!TEST_int_gt(ret, 0) || !TEST_true(validate_ccs(wbio)))
		goto end;
	...
	for (i = 0; i < (int)OSSL_NELEM(tests); i++) {
		...
		if (!TEST_true(send_record(rbio, SSL3_RT_APPLICATION_DATA,
					   tests[i].seq, &tests[i].seq,
					   sizeof(uint64_t)))) {
