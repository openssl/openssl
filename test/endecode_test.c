			  ...
			  OSSL_PARAM *genparams)
{
	...
	EVP_PKEY_CTX *ctx =
		template != NULL ?
			EVP_PKEY_CTX_new_from_pkey(keyctx, template,
						   testpropq) :
			      ...
			      checker *check_cb, dumper *dump_cb, int flags)
{
	...
	if ((flags & FLAG_FAIL_IF_FIPS) != 0 && is_fips && !is_fips_3_0_0) {
		if (TEST_false(decode_cb(
			    file, line, (void **)&pkey2, encoded, encoded_len,
	...
	if (!TEST_true(check_cb(file, line, type, encoded, encoded_len)) ||
	    ...
	    ((output_structure == NULL ||
	     ...
	     !TEST_true(decode_cb(file, line, (void **)&pkey3, encoded,
				  encoded_len, output_type, output_structure,
				  (flags & FLAG_DECODE_WITH_TYPE ? type : NULL),
				  0, pass))) ||
				...
				const char *pcipher)
{
	...
	if (!TEST_FL_ptr(ectx = OSSL_ENCODER_CTX_new_for_pkey(
				 pkey, selection, output_type, output_structure,
				 testpropq)) ||
				...
				int selection, const char *pass)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (!TEST_FL_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(
					 &testpkey, testtype, structure_type,
					 keytype, selection, testctx,
					 testpropq)) ||
		    (pass != NULL && !OSSL_DECODER_CTX_set_passphrase(
		...
		if (i == 0) {
			...
			if (selection ==
				...
				if (!TEST_FL_int_eq(EVP_PKEY_parameters_eq(
							    pkey, testpkey),
						    1))
					goto end;
			} else {
				if (!TEST_FL_int_eq(EVP_PKEY_eq(pkey, testpkey),
						    1))
					goto end;
			}
		}
				      ...
				      const char *pass, const char *pcipher)
{
	...
	if (!TEST_FL_ptr(mem_ser = BIO_new(BIO_s_mem())) ||
	    !TEST_FL_true(PEM_write_bio_PrivateKey_traditional(
		    mem_ser, pkey, cipher, upass, (int)passlen, NULL, NULL)) ||
	    !TEST_FL_true(BIO_get_mem_ptr(mem_ser, &mem_buf) > 0) ||
	    !TEST_FL_ptr(*encoded = mem_buf->data) ||
	    !TEST_FL_long_gt(*encoded_len = (long)mem_buf->length, 0))
		goto end;
	...
	if (TEST_FL_ptr(p8inf)) {
		...
		if (TEST_FL_ptr(pkey)) {
			if (!(ok = TEST_FL_true(EVP_PKEY_is_a(pkey, type)))) {
				EVP_PKEY_type_names_do_all(pkey, collect_name,
							   &namelist);
				if (namelist != NULL)
					TEST_note("%s isn't any of %s", type,
						  namelist);
...
static int test_params_via_DER(const char *type, EVP_PKEY *key)
{
	return test_encode_decode(__FILE__, __LINE__, type, key,
				  ...
				  "type-specific", NULL, NULL,
				  encode_EVP_PKEY_prov, decode_EVP_PKEY_prov,
				  test_mem, check_params_DER, dump_der,
...
static int test_unprotected_via_legacy_PEM(const char *type, EVP_PKEY *key)
{
	...
	return test_encode_decode(__FILE__, __LINE__, type, key,
				  ...
				  decode_EVP_PKEY_prov, test_text,
		     ...
		     const void *data, size_t data_len)
{
	...
	return ossl_do_PVK_header(&in, (unsigned int)data_len, 0, &isdss,
				  &saltlen, &keylen);
}
...
static int test_unprotected_via_PVK(const char *type, EVP_PKEY *key)
{
	return test_encode_decode(__FILE__, __LINE__, type, key,
				  OSSL_KEYMGMT_SELECT_KEYPAIR |
					  OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
				  "PVK", NULL, NULL, NULL, encode_EVP_PKEY_PVK,
...
static int test_protected_via_DER(const char *type, EVP_PKEY *key, int fips)
{
	return test_encode_decode(__FILE__, __LINE__, type, key,
				  OSSL_KEYMGMT_SELECT_KEYPAIR |
					  OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
				  "DER", "EncryptedPrivateKeyInfo", pass,
				  pass_cipher, encode_EVP_PKEY_prov,
				  decode_EVP_PKEY_prov, test_mem,
...
static int test_protected_via_PEM(const char *type, EVP_PKEY *key, int fips)
{
	return test_encode_decode(__FILE__, __LINE__, type, key,
				  OSSL_KEYMGMT_SELECT_KEYPAIR |
					  OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
				  "PEM", "EncryptedPrivateKeyInfo", pass,
				  pass_cipher, encode_EVP_PKEY_prov,
				  decode_EVP_PKEY_prov, test_text,
...
static int test_protected_via_legacy_PEM(const char *type, EVP_PKEY *key)
{
	...
	return test_encode_decode(__FILE__, __LINE__, type, key,
				  ...
				  encode_EVP_PKEY_legacy_PEM,
				  decode_EVP_PKEY_prov, test_text,
				  check_protected_legacy_PEM, dump_pem, 0);
}
...
static int test_protected_via_PVK(const char *type, EVP_PKEY *key)
{
	...
	ret = test_encode_decode(__FILE__, __LINE__, type, key,
				 OSSL_KEYMGMT_SELECT_KEYPAIR |
					 OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
				 "PVK", NULL, pass, NULL, encode_EVP_PKEY_PVK,
				 decode_EVP_PKEY_prov, test_mem, check_PVK,
				 dump_der, 0);
			    ...
			    const void *data, size_t data_len)
{
	const unsigned char *datap = data;
	EVP_PKEY *pkey =
		d2i_PUBKEY_ex(NULL, &datap, (long)data_len, testctx, testpropq);
...
#define IMPLEMENT_TEST_SUITE(KEYTYPE, KEYTYPEstr, fips)                      \
	...
	{                                                                    \
		return test_unprotected_via_i2d(KEYTYPEstr, key_##KEYTYPE,   \
						fips);                       \
	}                                                                    \
					      ...
					      size_t gen_len)
{
	...
	static const unsigned char seed[] = { 0xc4, 0x9d, 0x36, 0x08, 0x86,
					      0xe7, 0x04, 0x93, 0x6a, 0x66,
					      0x78, 0xe1, 0x13, 0x9d, 0x26,
					      0xb7, 0x81, 0x9f, 0x7e, 0x90 };
	static const unsigned char order_data[] = {
		0x00, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00,
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xbc,
		0xe6, 0xfa, 0xad, 0xa7, 0x17, 0x9e, 0x84, 0xf3, 0xb9,
	...
	return TEST_ptr(a = BN_CTX_get(bnctx)) &&
	       ...
	       TEST_true(OSSL_PARAM_BLD_push_utf8_string(
		       bld, OSSL_PKEY_PARAM_EC_FIELD_TYPE, SN_X9_62_prime_field,
		       0)) &&
	       TEST_true(OSSL_PARAM_BLD_push_BN(bld, OSSL_PKEY_PARAM_EC_P,
						prime)) &&
	       TEST_true(
	       ...
	       TEST_true(OSSL_PARAM_BLD_push_BN(
		       bld, OSSL_PKEY_PARAM_EC_COFACTOR, BN_value_one()));
}
...
static int create_ec_explicit_prime_params(OSSL_PARAM_BLD *bld)
{
	...
	static const unsigned char prime256v1_gen2[] = {
		0x04, 0xe4, 0x97, 0x08, 0xbe, 0x7d, 0xfa, 0xa2, 0x9a, 0xa3,
		0x12, 0x6f, 0xe4, 0xe7, 0xd0, 0x25, 0xe3, 0x4a, 0xc1, 0x03,
		0x15, 0x8c, 0xd9, 0x33, 0xc6, 0x97, 0x42, 0xf5, 0xdc, 0x97,
		0xb9, 0xd7, 0x31, 0xe9, 0x7d, 0x74, 0x3d, 0x67, 0x6a, 0x3b,
		0x21, 0x08, 0x9c, 0x31, 0x73, 0xf8, 0xc1, 0x27, 0xc9, 0xd2,
		0xa0, 0xa0, 0x83, 0x66, 0xe0, 0xc9, 0xda, 0xa8, 0xc6, 0x56,
		0x2b, 0x94, 0xb1, 0xae, 0x55
	};
						  ...
						  size_t gen_len)
{
	...
	return TEST_ptr(a = BN_CTX_get(bnctx)) &&
	       ...
	       TEST_ptr(BN_bin2bn(b_data, sizeof(b_data), b)) &&
	       TEST_ptr(BN_bin2bn(order_data, sizeof(order_data), order)) &&
	       TEST_ptr(BN_bin2bn(cofactor_data, sizeof(cofactor_data),
				  cofactor)) &&
	       TEST_true(OSSL_PARAM_BLD_push_utf8_string(
	       ...
	       TEST_true(OSSL_PARAM_BLD_push_octet_string(
		       bld, OSSL_PKEY_PARAM_EC_GENERATOR, gen, gen_len)) &&
	       TEST_true(OSSL_PARAM_BLD_push_BN(
		       bld, OSSL_PKEY_PARAM_EC_COFACTOR, cofactor));
}

static int create_ec_explicit_trinomial_params_namedcurve(OSSL_PARAM_BLD *bld)
{
	static const unsigned char gen[] = {
		0x04, 0x01, 0x72, 0x32, 0xBA, 0x85, 0x3A, 0x7E, 0x73,
		0x1A, 0xF1, 0x29, 0xF2, 0x2F, 0xF4, 0x14, 0x95, 0x63,
		0xA4, 0x19, 0xC2, 0x6B, 0xF5, 0x0A, 0x4C, 0x9D, 0x6E,
		0xEF, 0xAD, 0x61, 0x26, 0x01, 0xDB, 0x53, 0x7D, 0xEC,
		0xE8, 0x19, 0xB7, 0xF7, 0x0F, 0x55, 0x5A, 0x67, 0xC4,
		0x27, 0xA8, 0xCD, 0x9B, 0xF1, 0x8A, 0xEB, 0x9B, 0x56,
		0xE0, 0xC1, 0x10, 0x56, 0xFA, 0xE6, 0xA3
	};
...
static int create_ec_explicit_trinomial_params(OSSL_PARAM_BLD *bld)
{
	static const unsigned char gen2[] = {
		0x04, 0x00, 0xd7, 0xba, 0xd0, 0x26, 0x6c, 0x31, 0x6a,
		0x78, 0x76, 0x01, 0xd1, 0x32, 0x4b, 0x8f, 0x30, 0x29,
		0x2d, 0x78, 0x30, 0xca, 0x43, 0xaa, 0xf0, 0xa2, 0x5a,
		0xd4, 0x0f, 0xb3, 0xf4, 0x00, 0x85, 0x4b, 0x1b, 0x8d,
		0x50, 0x10, 0xa5, 0x1c, 0x80, 0xf7, 0x86, 0x40, 0x62,
		0x4c, 0x87, 0xd1, 0x26, 0x7a, 0x9c, 0x5c, 0xe9, 0x82,
		0x29, 0xd1, 0x67, 0x70, 0x41, 0xea, 0xcb
	};
...
static int ec_encode_to_data_multi(void)
{
	...
	ret = TEST_ptr(key = EVP_PKEY_Q_keygen(testctx, "", "EC", "P-256")) &&
	      TEST_ptr(ectx = OSSL_ENCODER_CTX_new_for_pkey(
			       key, EVP_PKEY_KEYPAIR, "DER", NULL, NULL)) &&
...
int setup_tests(void)
{
	...
	OSSL_PARAM EC_params[] = { OSSL_PARAM_utf8_string("group", groupname,
							  sizeof(groupname) -
								  1),
				   OSSL_PARAM_END };
#endif
	...
	if (default_libctx) {
		if (!test_get_libctx(NULL, NULL, config_file, &deflprov,
				     prov_name))
			return 0;
	} else {
		if (!test_get_libctx(&testctx, &nullprov, config_file,
				     &deflprov, prov_name))
			return 0;
	}
...
#ifndef OPENSSL_NO_EC2M
	MAKE_DOMAIN_KEYS(ECExplicitTriNamedCurve, "EC",
			 ec_explicit_tri_params_nc);
	MAKE_DOMAIN_KEYS(ECExplicitTri2G, "EC",
