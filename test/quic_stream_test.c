...
static int test_sstream_simple(void)
{
	...
	if (!TEST_false(ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr,
							   iov, &num_iov)))
		goto err;
	...
	for (i = 0; i < 2; ++i) {
		num_iov = OSSL_NELEM(iov);
		if (!TEST_true(ossl_quic_sstream_get_stream_frame(
			    sstream, 0, &hdr, iov, &num_iov)) ||
...
static int test_sstream_bulk(int idx)
{
	...
	size_t consumed = 0, total_written = 0, rd, cur_rd, expected = 0,
	       start_at;
	...
	if (!TEST_size_t_eq(ossl_quic_sstream_get_buffer_size(sstream),
			    init_size))
		goto err;
	...
	if (!TEST_size_t_eq(ossl_quic_sstream_get_buffer_used(sstream),
			    init_size) ||
	...
	/* Exercise resize. */
	if (!TEST_true(ossl_quic_sstream_set_buffer_size(sstream,
							 init_size * 2)) ||
...
static int test_rstream_simple(int idx)
{
	...
	if (!TEST_true(ossl_quic_rstream_queue_data(rstream, NULL, 5,
	    ...
	    (use_rbuf &&
	     !TEST_false(ossl_quic_rstream_move_to_rbuf(rstream))) ||
	    (use_rbuf && !TEST_true(ossl_quic_rstream_resize_rbuf(
				 rstream, sizeof(simple_data)))) ||
	    ...
	    !TEST_true(ossl_quic_rstream_queue_data(
		    rstream, NULL, 15, simple_data + 15,
	...
	for (r = 0; r < 100; ++r) {
		for (s = 0; s < 10; ++s) {
			...
			if (!TEST_true(ossl_quic_rstream_queue_data(
				    rstream, NULL, off, bulk_data + off, size,
				    0)))
				goto err;
		...
		if (idx % 2 == 0) {
			if (!TEST_true(test_single_copy_read(rstream, read_buf,
							     data_size,
							     &readbytes, &fin)))
				goto err;
		} else if (!TEST_true(ossl_quic_rstream_read(
				   rstream, read_buf, data_size, &readbytes,
				   &fin))) {
			goto err;
		}
		...
		if (!fin_set && queued_max >= data_size - test_random() % 200) {
			...
			if (!TEST_true(ossl_quic_rstream_queue_data(
				    rstream, NULL, data_size, NULL, 0, 1)))
				goto err;
		}
	}
