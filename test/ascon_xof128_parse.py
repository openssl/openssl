...
# Parse ASCON-XOF128 test vectors from reference format to OpenSSL format.
# 
# Input format:
#    Msg = <hex>
#    MD = <hex>
    ...
    with open(input_file, 'r') as f:
        for line in f:
            ...
            if match:
                ...
                if key == 'Msg':
                    # Start of new vector (Msg is the first field)
                    if current_vector and ('MD' in current_vector):
                        vectors.append(current_vector)
                    current_vector = {}
...
def format_output(vectors):
    ...
    for vec in vectors:
        ...
        # Format output
        output_lines.append("Digest = ASCON-XOF128")
...
def main():
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print("Usage: python3 convert_ascon_xof128.py <input_file> [output_file]", file=sys.stderr)
        ...
        print("  Default output_file: test/recipes/30-test_evp_data/evpmd_ascon_xof128.txt", file=sys.stderr)
        sys.exit(1)
    
    input_file = sys.argv[1]
    output_file = sys.argv[2] if len(sys.argv) == 3 else 'test/recipes/30-test_evp_data/evpmd_ascon_xof128.txt'
