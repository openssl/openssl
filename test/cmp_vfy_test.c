...
static int execute_validate_msg_test(CMP_VFY_TEST_FIXTURE *fixture)
{
	...
	return res &&
	       (!fixture->expected || TEST_ptr_eq(validated, fixture->cert));
}
...
static int execute_validate_cert_path_test(CMP_VFY_TEST_FIXTURE *fixture)
{
	...
	int res = TEST_int_eq(fixture->expected,
			      OSSL_CMP_validate_cert_path(fixture->cmp_ctx, ts,
							  fixture->cert));
...
static int test_validate_msg_signature_partial_chain(int expired)
{
	...
	if (ts == NULL ||
		...
		if (expired)
			X509_VERIFY_PARAM_set_time(vpm,
						   test_time_after_expiration);
	}
	EXECUTE_TEST(execute_validate_msg_test, tear_down);
	return result;
}
...
static int test_validate_msg_signature_sender_cert_extracert(void)
{
	...
	if (!TEST_ptr(fixture->msg =
		...
		fixture->cert = sk_X509_value(fixture->msg->extraCerts,
					      1); /* Insta CA */
	}
	EXECUTE_TEST(execute_validate_msg_test, tear_down);
	return result;
...
static int execute_msg_check_test(CMP_VFY_TEST_FIXTURE *fixture)
{
	...
	if (fixture->expected ==
	    0) /* error expected already during above check */
		return 1;
	return TEST_int_eq(0, ASN1_OCTET_STRING_cmp(
				      ossl_cmp_hdr_get0_senderNonce(hdr),
				      fixture->cmp_ctx->recipNonce)) &&
	       TEST_int_eq(0, ASN1_OCTET_STRING_cmp(
				      tid, fixture->cmp_ctx->transactionID));
}
...
int setup_tests(void)
{
	...
	if (!test_arg_libctx(&libctx, &default_null_provider, &provider, 14,
			     USAGE))
		return 0;
