...
static int test_hmac_md5(int idx)
{
	...
	p = pt(HMAC(EVP_md5(), test[idx].key, test[idx].key_len, test[idx].data,
		    test[idx].data_len, NULL, NULL),
	       MD5_DIGEST_LENGTH);
...
static int test_hmac_bad(void)
{
	...
	ctx = HMAC_CTX_new();
	if (!TEST_ptr(ctx) || !TEST_ptr_null(HMAC_CTX_get_md(ctx)) ||
...
static int test_hmac_run(void)
{
	...
	if (!TEST_true(HMAC_Init_ex(ctx, test[4].key, test[4].key_len,
				    EVP_sha1(), NULL)) ||
	...
	if (!TEST_true(HMAC_Init_ex(ctx, test[6].key, test[6].key_len, NULL,
				    NULL)) ||
...
static int test_hmac_single_shot(void)
{
	...
	p = pt(HMAC(EVP_sha1(), NULL, 0, test[4].data, test[4].data_len, NULL,
		    NULL),
	       SHA_DIGEST_LENGTH);
	if (!TEST_ptr(p) || !TEST_str_eq(p, test[4].digest))
		return 0;
...
static int test_hmac_copy(void)
{
	...
	if (!TEST_true(HMAC_Init_ex(ctx, test[7].key, test[7].key_len,
				    EVP_sha1(), NULL)) ||
...
static int test_hmac_copy_uninited(void)
{
	...
	if (!TEST_ptr(ctx = EVP_MD_CTX_new()) ||
	    !TEST_ptr(pkey = EVP_PKEY_new_mac_key(EVP_PKEY_HMAC, NULL, key,
						  sizeof(key))) ||
...
static char *pt(unsigned char *md, unsigned int len)
{
	...
	if (md == NULL)
		return NULL;
	for (i = 0; i < len && (i + 1) * OSSL_HEX_CHARS_PER_BYTE < sizeof(buf);
	     i++)
...
static int test_hmac_chunks(int idx)
{
	...
	for (i = 0; i < test_chunks[idx].chunks; i++) {
		if (!TEST_true((test_chunks[idx].chunk_size[i] <
				(int)sizeof(buf))))
			goto err;
		memset(buf, i, test_chunks[idx].chunk_size[i]);
		if (!TEST_true(HMAC_Update(ctx, buf,
					   test_chunks[idx].chunk_size[i])))
			goto err;
	}
