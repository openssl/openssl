...
static int check_verify_message(EVP_PKEY_CTX *pkey_ctx, int expected)
{
	...
	*p++ = OSSL_PARAM_construct_int(
		OSSL_SIGNATURE_PARAM_FIPS_VERIFY_MESSAGE, &verify_message);
...
static int ecdsa_keygen_test(int id)
{
	...
	if (!TEST_ptr(pkey = EVP_PKEY_Q_keygen(libctx, NULL, "EC",
					       tst->curve_name)) ||
	...
	if (!TEST_ptr(bld = OSSL_PARAM_BLD_new()) ||
	    (curve_name != NULL &&
	     !TEST_true(OSSL_PARAM_BLD_push_utf8_string(
				bld, OSSL_PKEY_PARAM_GROUP_NAME, curve_name,
				0) > 0)) ||
...
static int ecdsa_sigver_test(int id)
{
	...
	if (!TEST_int_gt((sig_len = i2d_ECDSA_SIG(sign, &sig)), 0) ||
	    ...
	    !TEST_int_eq(EVP_DigestVerify(md_ctx, sig, sig_len, tst->msg,
					  tst->msg_len),
			 tst->pass) ||
	    !check_verify_message(pkey_ctx, 1) ||
...
{
	int ret = 0;
	const struct ecdh_cofactor_derive_st *t =
		&ecdh_cofactor_derive_data[tstid];
	unsigned char secret1[16];
	size_t secret1_len = sizeof(secret1);
	const char *curve =
		"K-283"; /* A curve that has a cofactor that it not 1 */
	...
	if (!TEST_ptr(peer1 = EVP_PKEY_Q_keygen(libctx, NULL, "EC", curve)))
		return TEST_skip("Curve %s not supported by the FIPS provider",
				 curve);
	...
	if (t->key_cofactor != COFACTOR_NOT_SET) {
		params[0] = OSSL_PARAM_construct_int(
			OSSL_PKEY_PARAM_USE_COFACTOR_ECDH, &use_cofactordh);
		prms = params;
	}
...
err:
	if (ret == 0) {
		static const char *state[] = { "unset", "-1", "disabled",
					       "enabled" };
			     ...
			     int expected)
{
	...
	if (!TEST_ptr(bld = OSSL_PARAM_BLD_new()) ||
	    ...
	    !TEST_ptr(params = OSSL_PARAM_BLD_to_param(bld)) ||
	    !TEST_ptr(
		    ctx = EVP_PKEY_CTX_new_from_name(libctx, algname, NULL)) ||
	    !TEST_int_eq(EVP_PKEY_fromdata_init(ctx), 1) ||
	    !TEST_int_eq(EVP_PKEY_fromdata(ctx, pkey, EVP_PKEY_PUBLIC_KEY,
					   params),
			 expected))
...
static int eddsa_keygen_test(int id)
{
	...
	self_test_args.enable = 1;
	if (!TEST_ptr(
		    pkey = EVP_PKEY_Q_keygen(libctx, NULL, tst->curve_name)) ||
	    !TEST_int_ge(self_test_args.called, 3) ||
...
static EVP_PKEY *dsa_paramgen(int L, int N)
{
	...
	if (!TEST_ptr(paramgen_ctx = EVP_PKEY_CTX_new_from_name(libctx, "DSA",
								NULL)) ||
...
static int dsa_paramgen_test(int id)
{
	...
	if (!TEST_ptr(paramgen_ctx = EVP_PKEY_CTX_new_from_name(libctx, "DSA",
	    ...
	    !TEST_int_gt(EVP_PKEY_paramgen_init(paramgen_ctx), 0) ||
	    !TEST_true(
		    EVP_PKEY_CTX_set_dsa_paramgen_bits(paramgen_ctx, tst->L)) ||
	    !TEST_true(
	...
	if (!dsasign_allowed) {
		...
		if (!TEST_true(EVP_PKEY_paramgen(paramgen_ctx, &param_key)) ||
		    !TEST_true(pkey_get_bn_bytes(
			    param_key, OSSL_PKEY_PARAM_FFC_P, &p, &plen)) ||
		    !TEST_true(pkey_get_bn_bytes(
			    param_key, OSSL_PKEY_PARAM_FFC_Q, &q, &qlen)) ||
			   ...
			   BN_CTX *bn_ctx)
{
	...
	if (g != NULL) {
		if (!TEST_ptr(g_bn = BN_CTX_get(bn_ctx)) ||
		    ...
		    !TEST_true(OSSL_PARAM_BLD_push_BN(
			    bld, OSSL_PKEY_PARAM_FFC_G, g_bn)))
			goto err;
	}
	if (seed != NULL) {
	...
	if (pub != NULL) {
		if (!TEST_ptr(pub_bn = BN_CTX_get(bn_ctx)) ||
		    ...
		    !TEST_true(OSSL_PARAM_BLD_push_BN(
			    bld, OSSL_PKEY_PARAM_PUB_KEY, pub_bn)))
			goto err;
	}
...
static int dsa_pqver_test(int id)
{
	...
	if (!TEST_ptr(bn_ctx = BN_CTX_new_ex(libctx)) ||
	    !TEST_true(dsa_create_pkey(&param_key, tst->p, tst->p_len, tst->q,
				       tst->q_len, NULL, 0, tst->seed,
				       tst->seed_len, tst->counter, 1, 0, NULL,
				       0, bn_ctx)) ||
...
static int dsa_siggen_test(int id)
{
	...
	if (!dsasign_allowed) {
		...
		if (!TEST_true(sig_gen(pkey, NULL, tst->digest_alg, tst->msg,
				       tst->msg_len, &sig, &sig_len)) ||
		    !TEST_true(get_dsa_sig_rs_bytes(sig, sig_len, &r, &s, &rlen,
						    &slen)))
			goto err;
		test_output_memory("r", r, rlen);
		test_output_memory("s", s, slen);
...
static int dsa_sigver_test(int id)
{
	...
	if (!TEST_ptr(md = EVP_MD_fetch(libctx, tst->digest_alg, "")) ||
	    !TEST_true(EVP_Digest(tst->msg, tst->msg_len, digest, &digest_len,
				  md, NULL)))
		goto err;
...
static int aes_ccm_enc_dec_test(int id)
{
	...
	return aes_ccm_enc_dec(tst->alg, tst->pt, tst->pt_len, tst->key,
			       ...
			       pass) &&
	       aes_ccm_enc_dec(tst->alg, tst->ct, ct_len, tst->key,
			       tst->key_len, tst->iv, tst->iv_len, tst->aad,
			       tst->aad_len, tst->pt, tst->pt_len, tag, tag_len,
			       !enc, pass)
	       /* test that it fails if the tag is incorrect */
	       && aes_ccm_enc_dec(tst->alg, tst->ct, ct_len, tst->key,
				  tst->key_len, tst->iv, tst->iv_len, tst->aad,
				  tst->aad_len, tst->pt, tst->pt_len, tag - 1,
				  tag_len, !enc, !pass);
}
			   ...
			   unsigned char *outiv)
{
...

	{
		...
		int ivgen =
			(OSSL_PARAM_locate_const(gettables, ivgenkey) != NULL);

		if (ivgen != 0)
			*p++ = OSSL_PARAM_construct_uint(ivgenkey,
							 &iv_generated);
		if (outiv != NULL)
			*p = OSSL_PARAM_construct_octet_string(
				OSSL_CIPHER_PARAM_IV, outiv, iv_len);
		if (!TEST_true(EVP_CIPHER_CTX_get_params(ctx, params)))
			goto err;
		if (ivgen != 0 &&
		    !TEST_uint_eq(iv_generated,
				  (enc == 0 || iv != NULL ? 0 : 1)))
			goto err;
	}
	if (out_len != NULL)
...
static int aes_gcm_gen_iv_internal_test(void)
{
	...
	return aes_gcm_enc_dec(tst->alg, tst->pt, tst->pt_len, tst->key,
			       ...
			       tst->aad_len, NULL, tst->ct_len, NULL,
			       tst->tag_len, enc, pass, out, &out_len, iv) &&
			  ...
			  BN_CTX *bn_ctx, int pass)
{
	...
	if (!TEST_ptr(bld = OSSL_PARAM_BLD_new()) ||
	    (group_name != NULL &&
	     !TEST_int_gt(OSSL_PARAM_BLD_push_utf8_string(
				  bld, OSSL_PKEY_PARAM_GROUP_NAME, group_name,
				  0),
			  0)))
		goto err;
	...
	if (priv != NULL) {
		if (!TEST_ptr(priv_bn = BN_CTX_get(bn_ctx)) ||
		    ...
		    !TEST_true(OSSL_PARAM_BLD_push_BN(
			    bld, OSSL_PKEY_PARAM_PRIV_KEY, priv_bn)))
			goto err;
	}
...
static int dh_safe_prime_keygen_test(int id)
{
	...
	size_t priv_len = 0, pub_len = 0;
	OSSL_PARAM params[2];
	const struct dh_safe_prime_keygen_st *tst =
		&dh_safe_prime_keygen_data[id];
	...
	if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(libctx, "DH", NULL)) ||
	    ...
	    !TEST_true(pkey_get_bn_bytes(pkey, OSSL_PKEY_PARAM_PRIV_KEY, &priv,
					 &priv_len)) ||
	    !TEST_true(pkey_get_bn_bytes(pkey, OSSL_PKEY_PARAM_PUB_KEY, &pub,
					 &pub_len)))
		goto err;
...
static int dh_safe_prime_keyver_test(int id)
{
	...
	EVP_PKEY_CTX *key_ctx = NULL;
	EVP_PKEY *pkey = NULL;
	const struct dh_safe_prime_keyver_st *tst =
		&dh_safe_prime_keyver_data[id];
			   ...
			   const unsigned char *d, size_t d_len, BN_CTX *bn_ctx)
{
	...
	if (e != NULL) {
		if (!TEST_ptr(e_bn = BN_CTX_get(bn_ctx)) ||
		    ...
		    !TEST_true(OSSL_PARAM_BLD_push_BN(
			    bld, OSSL_PKEY_PARAM_RSA_E, e_bn)))
			goto err;
	}
	if (d != NULL) {
		if (!TEST_ptr(d_bn = BN_CTX_get(bn_ctx)) ||
		    ...
		    !TEST_true(OSSL_PARAM_BLD_push_BN(
			    bld, OSSL_PKEY_PARAM_RSA_D, d_bn)))
			goto err;
	}
	if (!TEST_ptr(params = OSSL_PARAM_BLD_to_param(bld)) ||
...
static int rsa_keygen_test(int id)
{
	...
	if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(libctx, "RSA", NULL)) ||
	    ...
	    !TEST_int_gt(EVP_PKEY_keygen(ctx, &pkey), 0) ||
	    !TEST_true(pkey_get_bn_bytes(pkey, OSSL_PKEY_PARAM_RSA_TEST_P1, &p1,
					 &p1_len)) ||
	    !TEST_true(pkey_get_bn_bytes(pkey, OSSL_PKEY_PARAM_RSA_TEST_P2, &p2,
					 &p2_len)) ||
	    !TEST_true(pkey_get_bn_bytes(pkey, OSSL_PKEY_PARAM_RSA_TEST_Q1, &q1,
					 &q1_len)) ||
	    !TEST_true(pkey_get_bn_bytes(pkey, OSSL_PKEY_PARAM_RSA_TEST_Q2, &q2,
					 &q2_len)) ||
	    !TEST_true(pkey_get_bn_bytes(pkey, OSSL_PKEY_PARAM_RSA_FACTOR1, &p,
					 &p_len)) ||
	    !TEST_true(pkey_get_bn_bytes(pkey, OSSL_PKEY_PARAM_RSA_FACTOR2, &q,
					 &q_len)) ||
	    !TEST_true(pkey_get_bn_bytes(pkey, OSSL_PKEY_PARAM_RSA_N, &n,
					 &n_len)) ||
	    !TEST_true(
...
static int rsa_siggen_test(int id)
{
	...
	*p++ = OSSL_PARAM_construct_utf8_string(OSSL_SIGNATURE_PARAM_DIGEST,
	...
	if (salt_len >= 0)
		*p++ = OSSL_PARAM_construct_int(
			OSSL_SIGNATURE_PARAM_PSS_SALTLEN, &salt_len);
	*p++ = OSSL_PARAM_construct_end();

	if (!TEST_ptr(pkey = EVP_PKEY_Q_keygen(libctx, NULL, "RSA",
					       (size_t)tst->mod)) ||
	    !TEST_true(pkey_get_bn_bytes(pkey, OSSL_PKEY_PARAM_RSA_N, &n,
					 &n_len)) ||
	    !TEST_true(pkey_get_bn_bytes(pkey, OSSL_PKEY_PARAM_RSA_E, &e,
					 &e_len)) ||
...
static int rsa_sigver_test(int id)
{
	...
	TEST_note("RSA %s Signature Verify : expected to %s ",
		  tst->sig_pad_mode, tst->pass == PASS ? "pass" : "fail");
...
static int drbg_test(int id)
{
	...
	params[0] = OSSL_PARAM_construct_octet_string(
		OSSL_RAND_PARAM_TEST_ENTROPY, (void *)tst->entropy_input,
...
static int aes_cfb1_bits_test(void)
{
	...
	if (!TEST_ptr(cipher = EVP_CIPHER_fetch(libctx, "AES-128-CFB1",
						"fips=yes")))
		goto err;
	if (!TEST_ptr(ctx = EVP_CIPHER_CTX_new()))
		goto err;
