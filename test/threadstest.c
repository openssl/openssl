...
static int _torture_rw(void)
{
	...
	TEST_info(
		"performed %d reads and %d writes over 2 read and 2 write threads in %e seconds",
...
static void reader_fn(int *iterations)
{
	...
	while (lw1 != 1 || lw2 != 1) {
		...
		if (oldval > val) {
			TEST_info(
				"rcu torture value went backwards! %llu : %llu",
...
static int test_thread_local_multi_key(void)
{
	...
	for (i = 0; i < 1000; i++) {
		if (!TEST_true(CRYPTO_THREAD_init_local(
			    &thread_local_key, thread_local_destructor)))
			return 0;

		if (!TEST_true(
			    CRYPTO_THREAD_set_local(&thread_local_key, &dummy)))
			return 0;
...
static int thread_setup_libctx(int libctx, const char *providers[])
{
	...
	if (libctx && !TEST_true(test_get_libctx(&multi_libctx, NULL,
	...
	if (providers != NULL)
		for (n = 0; providers[n] != NULL; n++)
			if (!TEST_size_t_lt(n, MAXIMUM_PROVIDERS) ||
			    !TEST_ptr(multi_provider[n] = OSSL_PROVIDER_load(
					      multi_libctx, providers[n]))) {
...
static int start_threads(size_t n, void (*thread_func)(void))
{
	...
	for (i = 0; i < n; i++)
		if (!TEST_true(run_thread(multi_threads + multi_num_threads++,
					  thread_func)))
			return 0;
	return 1;
}
...
static int thread_run_test(void (*main_func)(void), size_t num_threads,
			   void (*thread_func)(void), int libctx,
			   const char *providers[])
{
	...
	if (!teardown_threads() || !TEST_true(multi_success))
		goto err;
	testresult = 1;
err:
	thead_teardown_libctx();
...
static void thread_general_worker(void)
{
	...
	const unsigned char key[AES_BLOCK_SIZE] = { 0x00, 0x01, 0x02, 0x03,
						    0x04, 0x05, 0x06, 0x07,
						    0x08, 0x09, 0x0a, 0x0b,
						    0x0c, 0x0d, 0x0e, 0x0f };
	const unsigned char iv[AES_BLOCK_SIZE] = { 0x00, 0x01, 0x02, 0x03,
						   0x04, 0x05, 0x06, 0x07,
						   0x08, 0x09, 0x0a, 0x0b,
						   0x0c, 0x0d, 0x0e, 0x0f };
	unsigned int mdoutl;
	...
	if (!TEST_ptr(mdctx) || !TEST_ptr(md) || !TEST_ptr(cipherctx) ||
	    !TEST_ptr(ciph))
		goto err;
	...
	for (i = 0; i < 5; i++) {
		if (!TEST_true(EVP_EncryptInit_ex(cipherctx, ciph, NULL, key,
						  iv)) ||
     ...
     */
	pkey = EVP_PKEY_Q_keygen(multi_libctx, NULL, "RSA",
				 (size_t)(isfips ? 2048 : 512));
	if (!TEST_ptr(pkey))
		goto err;
...
static void thread_shared_evp_pkey(void)
{
	...
	for (i = 0; i < 1 + do_fips; i++) {
		...
		if (!TEST_int_ge(EVP_PKEY_decrypt_init(ctx), 0) ||
		    !TEST_int_gt(EVP_PKEY_decrypt(ctx, ptbuf, &ptlen, ctbuf,
						  ctlen),
				 0) ||
...
{
	return thread_run_test(&thread_general_worker, 2,
			       &thread_general_worker, 1, default_provider);
}
...
static int test_multi_general_worker_fips_provider(void)
{
	...
	return thread_run_test(&thread_general_worker, 2,
			       &thread_general_worker, 1, fips_provider);
}

static int test_multi_fetch_worker(void)
{
	return thread_run_test(&thread_multi_simple_fetch, 2,
			       &thread_multi_simple_fetch, 1, default_provider);
}
...
static void test_multi_load_worker(void)
{
	...
	if (!TEST_ptr(prov = OSSL_PROVIDER_load(multi_libctx,
						multi_load_provider)) ||
	    !TEST_true(OSSL_PROVIDER_unload(prov)))
		multi_set_success(0);
}
...
static int test_multi_default(void)
{
	...
	return thread_run_test(&thread_multi_simple_fetch, 2,
			       &thread_multi_simple_fetch, 0, NULL);
}
...
static int test_multi_load(void)
{
	...
	return thread_run_test(NULL, MAXIMUM_THREADS, &test_multi_load_worker,
			       0, NULL) &&
	       res;
}
...
static void test_obj_create_one(void)
{
	...
	if (!TEST_int_ne(id, 0) || !TEST_true(id = OBJ_create(oid, sn, ln)) ||
...
{
	return thread_run_test(&test_obj_create_one, MAXIMUM_THREADS,
			       &test_obj_create_one, 1, default_provider);
}
...
static int test_bio_dgram_pair(void)
{
	...
	r = thread_run_test(&test_bio_dgram_pair_worker, MAXIMUM_THREADS,
			    &test_bio_dgram_pair_worker, 1, default_provider);
