...
static int test_crypto_buffer(void)
{
    ...
    if (!TEST_str_eq((const char *)CRYPTO_BUFFER_data(b1),
        ...
        ret = 0;
    if (!TEST_size_t_eq(CRYPTO_BUFFER_len(b1), 5))
        ret = 0;
    if (!TEST_size_t_eq(CRYPTO_BUFFER_len(b2), 5))
        ret = 0;
    if (!TEST_size_t_eq(CRYPTO_BUFFER_len(b3), 5))
        ret = 0;
    if (!TEST_size_t_eq(CRYPTO_BUFFER_len(b4), 5))
        ret = 0;
