...
static int ffc_params_gen_test(void)
{
    ...
    FFC_PARAMS_init(&params);
    if (!TEST_true(FFC_PARAMS_FIPS186_4_generate(&params, FFC_PARAM_TYPE_DH,
...
static int ffc_params_gen_canonicalg_test(void)
{
    ...
    FFC_PARAMS_init(&params);
    FFC_PARAMS_set0_gindex(&params, 1);
    if (!TEST_true(FFC_PARAMS_FIPS186_4_generate(&params, FFC_PARAM_TYPE_DH,
                                                 2048, 256, NULL, &res, NULL)))
        goto err;
    if (!TEST_true(FFC_PARAMS_FIPS186_4_validate(&params, FFC_PARAM_TYPE_DH,
...
static int ffc_params_fips186_2_gen_validate_test(void)
{
    ...
    if (!TEST_ptr(bn = BN_new()))
        goto err;
    FFC_PARAMS_init(&params);
    if (!TEST_true(FFC_PARAMS_FIPS186_2_generate(&params, FFC_PARAM_TYPE_DH,
...
static int ffc_public_validate_test(void)
{
    ...
    if (!TEST_false(FFC_validate_pub_key(params, pub, &res)))
        goto err;
    ...
    if (!TEST_false(FFC_validate_pub_key(params, BN_value_one(), &res)))
        goto err;
    if (!TEST_int_eq(FFC_ERROR_PUBKEY_TOO_SMALL, res))
        goto err;
    ...
    if (!TEST_true(FFC_validate_pub_key(params, pub, &res)))
        goto err;
...
static int ffc_private_validate_test(void)
{
    ...
    if (!TEST_false(FFC_validate_priv_key(params->q, priv, &res)))
        goto err;
    ...
    /* Pass if priv key >= 1 */
    if (!TEST_true(FFC_validate_priv_key(params->q, BN_value_one(), &res)))
        goto err;
    ...
    if (!TEST_true(FFC_validate_priv_key(params->q, priv, &res)))
        goto err;
...
/* TODO: We need some way to select tests to run in fips mode only */
#ifdef FIPS_MODE
    {
        int N = BN_num_bits(params->q);
        /* Fail since N < 2*s - where s = 112*/
        if (!TEST_false(FFC_generate_priv_key(params, 220, 112, priv)))
            goto err;
        /* fail since N > len(q) */
        if (!TEST_false(FFC_generate_priv_key(params, N+1, 112, priv)))
            goto err;
        /* pass since 2s <= N <= len(q) */
        if (!TEST_true(FFC_generate_priv_key(params, N, 112, priv)))
            goto err;
        /* pass since N = len(q) */
        if (!TEST_true(FFC_validate_priv_key(params->q, priv, &res)))
            goto err;
        /* pass since 2s <= N < len(q) */
        if (!TEST_true(FFC_generate_priv_key(params, N/2, 112, priv)))
            goto err;
        if (!TEST_true(FFC_validate_priv_key(params->q, priv, &res)))
            goto err;
    }
    ...
    if (!TEST_true(FFC_generate_priv_key(params, 0, 0, priv)))
        goto err;
    if (!TEST_true(FFC_validate_priv_key(params->q, priv, &res)))
        goto err;
#endif
