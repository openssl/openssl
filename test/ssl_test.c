...
static int test_handshake(int idx)
{
    ...
    if (test_ctx->method == SSL_TEST_METHOD_DTLS) {
        ...
        if (!TEST_(SSL_CTX_set_max_proto_version(server_ctx, DTLS_MAX_VERSION)))
            goto err;
        ...
        if (!TEST_(SSL_CTX_set_max_proto_version(client_ctx, DTLS_MAX_VERSION)))
            goto err;
        if (test_ctx->handshake_mode == SSL_TEST_HANDSHAKE_RESUME) {
            resume_server_ctx = SSL_CTX_new(DTLS_server_method());
            if (!TEST_(SSL_CTX_set_max_proto_version(resume_server_ctx,
                                                     DTLS_MAX_VERSION)))
                goto err;
            resume_client_ctx = SSL_CTX_new(DTLS_client_method());
            if (!TEST_(SSL_CTX_set_max_proto_version(resume_client_ctx,
                                                     DTLS_MAX_VERSION)))
                goto err;
    ...
    if (test_ctx->method == SSL_TEST_METHOD_TLS) {
        ...
        if (!TEST_(SSL_CTX_set_max_proto_version(server_ctx, TLS_MAX_VERSION)))
            goto err;
        ...
        if (test_ctx->extra.server.servername_callback !=
            ...
            if (!TEST_ptr(server2_ctx = SSL_CTX_new(TLS_server_method())))
                goto err;
            if (!TEST_(SSL_CTX_set_max_proto_version(server2_ctx, TLS_MAX_VERSION)))
                goto err;
        }
        client_ctx = SSL_CTX_new(TLS_client_method());
        if (!TEST_(SSL_CTX_set_max_proto_version(client_ctx, TLS_MAX_VERSION)))
            goto err;
