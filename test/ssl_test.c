					 ...
					 SSL_TEST_CTX *test_ctx)
{
	...
	if (test_ctx->expected_session_ticket_app_data != NULL)
		expected_len =
			strlen(test_ctx->expected_session_ticket_app_data);
			  ...
			  STACK_OF(X509_NAME) *names)
{
	...
	if (sk_X509_NAME_num(names) != sk_X509_NAME_num(expected_names))
		goto err;
	for (i = 0; i < sk_X509_NAME_num(names); i++) {
		if (!TEST_int_eq(X509_NAME_cmp(
					 sk_X509_NAME_value(names, i),
...
static int test_handshake(int idx)
{
	...
	if (test_ctx->method == SSL_TEST_METHOD_DTLS) {
		...
		if (test_ctx->extra.server.servername_callback !=
			...
			if (!TEST_ptr(server2_ctx = SSL_CTX_new_ex(
					      libctx, NULL,
					      DTLS_server_method())) ||
		...
		if (test_ctx->handshake_mode == SSL_TEST_HANDSHAKE_RESUME) {
			resume_server_ctx = SSL_CTX_new_ex(
				libctx, NULL, DTLS_server_method());
			if (!TEST_true(SSL_CTX_set_max_proto_version(
			    ...
			    !TEST_true(SSL_CTX_set_options(
				    ...
				    SSL_OP_ALLOW_CLIENT_RENEGOTIATION)))
				goto err;
			resume_client_ctx = SSL_CTX_new_ex(
				libctx, NULL, DTLS_client_method());
			if (!TEST_true(SSL_CTX_set_max_proto_version(
				    resume_client_ctx, 0)))
				goto err;
