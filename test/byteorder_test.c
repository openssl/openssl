...
static int test_byteorder(void)
{
	...
	if (!TEST_true(u16 == 0x0100U && memcmp(in, out, (size_t)2) == 0 &&
		       restin == in + 2 && restout == out + 2)) {
	...
	if (!TEST_true(u16 == 0x0001U && memcmp(in, out, (size_t)2) == 0 &&
		       restin == in + 2 && restout == out + 2)) {
	...
	if (!TEST_true(u32 == 0x03020100UL && memcmp(in, out, (size_t)4) == 0 &&
		       restin == in + 4 && restout == out + 4)) {
	...
	if (!TEST_true(u32 == 0x00010203UL && memcmp(in, out, (size_t)4) == 0 &&
		       restin == in + 4 && restout == out + 4)) {
	...
	if (!TEST_true(u64 == 0x0706050403020100ULL &&
		       memcmp(in, out, (size_t)8) == 0 && restin == in + 8 &&
		       restout == out + 8)) {
