...
static int group_order_tests(EC_GROUP *group)
{
	...
	for (i = 1; i <= 2; i++) {
		...
		if (!TEST_true(BN_set_word(n1, i))
		    ...
		    (i == 1 &&
		     !TEST_int_eq(0, EC_POINT_cmp(group, P, G, ctx))) ||
		    !TEST_true(BN_one(n1))
		    /* n1 = 1 - order */
		...
		if (!TEST_true(EC_POINTs_mul(group, R, NULL, 2, points, scalars,
					     ctx)) ||
		...
		scalars[5] = n2;
		points[5] = Q; /* => infinity */
		if (!TEST_true(EC_POINTs_mul(group, P, NULL, 6, points, scalars,
					     ctx)) ||
...
static int prime_field_tests(void)
{
	...
	if (!TEST_int_gt(EC_POINT_is_on_curve(group, Q, ctx), 0)) {
		if (!TEST_true(EC_POINT_get_affine_coordinates(group, Q, x, y,
							       ctx)))
			goto err;
	...
	do {
		...
		if (EC_POINT_is_at_infinity(group, P)) {
			...
			if (!TEST_true(EC_POINT_get_affine_coordinates(
				    group, P, x, y, ctx)))
				goto err;
	...
	if (!TEST_size_t_ne(len, 0) ||
		...
		goto err;
	test_output_memory("Generator as octet string, compressed form:", buf,
			   len);
     ...
     */
	    || !TEST_false(EC_POINT_set_affine_coordinates(group, P, x,
							   yplusone, ctx)) ||
...
static struct c2_curve_test {
	...
	{ "NIST curve K-163", "0800000000000000000000000000000000000000C9", "1",
...
/* Change test based on whether binary point compression is enabled or not. */
     ...
     * and therefore setting the coordinates should fail.
     */
	if (!TEST_false(EC_POINT_set_affine_coordinates(group, P, x, yplusone,
							ctx)) ||
	...
	if (n == OSSL_NELEM(char2_curve_tests) - 1) {
		if (!TEST_true(EC_POINT_set_affine_coordinates(group, P, x, y,
							       ctx)) ||
		    !TEST_true(EC_POINT_copy(Q, P)) ||
		    !TEST_false(EC_POINT_is_at_infinity(group, Q)) ||
		    !TEST_true(EC_POINT_dbl(group, P, P, ctx)) ||
		    ...
		    !TEST_true(EC_POINT_add(group, R, R, Q, ctx)) ||
		    !TEST_true(
			    EC_POINT_is_at_infinity(group, R)) /* R = P + 2Q */
		    || !TEST_false(EC_POINT_is_at_infinity(group, Q)))
			goto err;
		...
		if (!TEST_true(BN_add(y, z, BN_value_one())) ||
		    !TEST_BN_even(y) || !TEST_true(BN_rshift1(y, y)))
			goto err;
		scalars[0] = y; /* (group order + 1)/2, so y*Q + y*Q = Q */
		scalars[1] = y;

		/* z is still the group order */
		if (!TEST_true(EC_POINTs_mul(group, P, NULL, 2, points, scalars,
					     ctx)) ||
		    !TEST_true(EC_POINTs_mul(group, R, z, 2, points, scalars,
					     ctx)) ||
		    !TEST_int_eq(0, EC_POINT_cmp(group, P, R, ctx)) ||
		    !TEST_int_eq(0, EC_POINT_cmp(group, R, Q, ctx)))
			goto err;
		...
		if (!TEST_true(EC_POINTs_mul(group, P, NULL, 3, points, scalars,
					     ctx)) ||
		    !TEST_true(EC_POINT_is_at_infinity(group, P)))
			goto err;
#endif
...
static int hybrid_point_encoding_test(void)
{
	...
	if (!TEST_true(BN_dec2bn(&x, "0")) || !TEST_true(BN_dec2bn(&y, "1")) ||
	    ...
	    !TEST_true(EC_POINT_set_affine_coordinates(group, point, x, y,
						       NULL)) ||
	    !TEST_size_t_ne(0, (len = EC_POINT_point2oct(
					group, point, POINT_CONVERSION_HYBRID,
					NULL, 0, NULL))) ||
...
static int check_named_curve_test(int id)
{
	...
	if (!TEST_ptr(bn_ctx = BN_CTX_new()) ||
	    ...
	    !TEST_true(EC_POINT_add(group, other_gen, group_gen, group_gen,
				    NULL)) ||
	...
	if (field_nid == NID_X9_62_characteristic_two_field) {
         ...
         */
		if (!TEST_ptr(
			    BN_copy(other_p, BN_ucmp(BN_get0_nist_prime_192(),
						     other_p) == 0 ?
	...
	if (!TEST_size_t_eq(EC_GROUP_set_seed(group, invalid_seed,
					      invalid_seed_len),
			    invalid_seed_len))
		goto err;
	...
	if (has_seed) {
         ...
         */
		if (!TEST_int_eq(EC_GROUP_check_named_curve(group, 0, NULL),
				 nid))
			goto err;
	}
	...
	if (!TEST_true(EC_GROUP_set_curve(gtest, group_p, group_a, group_b,
					  NULL)) ||
	    !TEST_int_eq(EC_GROUP_check_named_curve(gtest, 0, NULL), nid))
		goto err;
...
static ossl_inline int are_ec_nids_compatible(int n1d, int n2d)
{
	...
	case NID_wap_wsg_idm_ecid_wtls4:
		ret = (n2d == NID_sect113r1 ||
		       n2d == NID_wap_wsg_idm_ecid_wtls4);
		break;
	case NID_sect163k1:
	case NID_wap_wsg_idm_ecid_wtls3:
		ret = (n2d == NID_sect163k1 ||
		       n2d == NID_wap_wsg_idm_ecid_wtls3);
		break;
	case NID_sect233k1:
	case NID_wap_wsg_idm_ecid_wtls10:
		ret = (n2d == NID_sect233k1 ||
		       n2d == NID_wap_wsg_idm_ecid_wtls10);
		break;
	case NID_sect233r1:
	case NID_wap_wsg_idm_ecid_wtls11:
		ret = (n2d == NID_sect233r1 ||
		       n2d == NID_wap_wsg_idm_ecid_wtls11);
		break;
	...
	case NID_wap_wsg_idm_ecid_wtls6:
		ret = (n2d == NID_secp112r1 ||
		       n2d == NID_wap_wsg_idm_ecid_wtls6);
		break;
	case NID_secp160r2:
	case NID_wap_wsg_idm_ecid_wtls7:
		ret = (n2d == NID_secp160r2 ||
		       n2d == NID_wap_wsg_idm_ecid_wtls7);
		break;
#ifdef OPENSSL_NO_EC_NISTP_64_GCC_128
	case NID_secp224r1:
	case NID_wap_wsg_idm_ecid_wtls12:
		ret = (n2d == NID_secp224r1 ||
		       n2d == NID_wap_wsg_idm_ecid_wtls12);
		break;
#else
	/*
...
static int check_named_curve_from_ecparameters(int id)
{
	...
	if (!TEST_true(are_ec_nids_compatible(nid, tnid))) {
		TEST_info("nid = %s, tnid = %s", OBJ_nid2sn(nid),
			  OBJ_nid2sn(tnid));
		goto err;
	}
	/* Ensure that the OPENSSL_EC_EXPLICIT_CURVE ASN1 flag is set. */
     ...
     * group.
     */
	if (!TEST_size_t_eq(EC_GROUP_set_seed(tmpg, invalid_seed,
					      invalid_seed_len),
			    invalid_seed_len) ||
...
static int ossl_parameter_test(void)
{
	...
	if (!TEST_ptr(group_nmd = EC_GROUP_new_by_curve_name(NID_secp384r1))
	    ...
	    || !TEST_ptr(params_nmd = EC_GROUP_to_params(group_nmd, NULL, NULL,
							 NULL)) ||
	    !TEST_ptr(group_nmd2 = EC_GROUP_new_from_params(params_nmd, NULL,
							    NULL)) ||
	    !TEST_int_eq(EC_GROUP_cmp(group_nmd, group_nmd2, NULL), 0)
	    /* test with BN_CTX set */
	    || !TEST_ptr(params_nmd2 = EC_GROUP_to_params(group_nmd, NULL, NULL,
							  bn_ctx)) ||
	    !TEST_ptr(group_nmd3 = EC_GROUP_new_from_params(params_nmd2, NULL,
							    NULL)) ||
	...
	if (!TEST_ptr(params_exp = OSSL_PARAM_BLD_to_param(bld)) ||
	    !TEST_ptr(group_exp = EC_GROUP_new_from_params(params_exp, NULL,
							   NULL)) ||
	    !TEST_ptr(params_exp2 = EC_GROUP_to_params(group_exp, NULL, NULL,
						       NULL)) ||
	    !TEST_ptr(group_exp2 = EC_GROUP_new_from_params(params_exp2, NULL,
							    NULL)) ||
...
static int cofactor_range_test(void)
{
	...
	if (!TEST_ptr(group = d2i_ECPKParameters(NULL, &b1,
	    ...
	    !TEST_int_gt(BN_hex2bn(&cf, "12bc94785251297abfafddf1565100da"),
			 0) ||
...
static int cardinality_test(int n)
{
	...
	BIGNUM *g1_p = NULL, *g1_a = NULL, *g1_b = NULL, *g1_x = NULL,
	       *g1_y = NULL, *g1_order = NULL, *g1_cf = NULL, *g2_cf = NULL;
	...
	is_binary = (EC_GROUP_get_field_type(g1) ==
	...
	/* construct g2 manually with g1 parameters */
#ifndef OPENSSL_NO_EC2M
	    || !TEST_ptr(g2 = (is_binary) ? EC_GROUP_new_curve_GF2m(g1_p, g1_a,
								    g1_b, ctx) :
					    EC_GROUP_new_curve_GFp(g1_p, g1_a,
								   g1_b, ctx))
#else
	    ...
	    !TEST_true(EC_POINT_set_affine_coordinates(g2, g2_gen, g1_x, g1_y,
						       ctx))
...
static int check_ec_key_field_public_range_test(int id)
{
	...
	if (!TEST_int_le(EC_KEY_set_public_key_affine_coordinates(key, x, y),
			 0))
		goto err;
					    ...
					    unsigned char *gen, size_t gen_size)
{
	...
	if (EC_GROUP_get_field_type(group) == NID_X9_62_prime_field) {
		...
		if (EC_GROUP_get_basis_type(group) == NID_X9_62_tpBasis) {
			...
			if (!TEST_true(
				    EC_GROUP_get_trinomial_basis(group, &k1)))
				goto err;
		} else {
	...
	if (!TEST_true(EVP_PKEY_get_utf8_string_param(
		    pkeyparam, OSSL_PKEY_PARAM_EC_FIELD_TYPE, name,
	...
	if (EC_GROUP_get0_cofactor(group) != NULL) {
		if (!TEST_true(EVP_PKEY_get_bn_param(
			    pkeyparam, OSSL_PKEY_PARAM_EC_COFACTOR,
			    &cofactor_out)) ||
	...
	if (EC_GROUP_get0_seed(group) != NULL) {
		if (!TEST_true(EVP_PKEY_get_octet_string_param(
			    pkeyparam, OSSL_PKEY_PARAM_EC_SEED, buf,
			    sizeof(buf), &buf_len)) ||
	...
	if (EC_GROUP_get_field_type(group) == NID_X9_62_prime_field) {
		...
		if (!TEST_false(EVP_PKEY_get_int_param(
		    ...
		    !TEST_false(EVP_PKEY_get_int_param(
			    pkeyparam, OSSL_PKEY_PARAM_EC_CHAR2_TP_BASIS,
			    &i_out)) ||
		    !TEST_false(EVP_PKEY_get_int_param(
			    pkeyparam, OSSL_PKEY_PARAM_EC_CHAR2_PP_K1,
			    &i_out)) ||
		    !TEST_false(EVP_PKEY_get_int_param(
			    pkeyparam, OSSL_PKEY_PARAM_EC_CHAR2_PP_K2,
			    &i_out)) ||
		    !TEST_false(EVP_PKEY_get_int_param(
			    pkeyparam, OSSL_PKEY_PARAM_EC_CHAR2_PP_K3,
			    &i_out)) ||
		    !TEST_false(EVP_PKEY_get_utf8_string_param(
			    pkeyparam, OSSL_PKEY_PARAM_EC_CHAR2_TYPE, name,
			    sizeof(name), &name_len)))
			goto err;
	} else {
#ifndef OPENSSL_NO_EC2M
	...
	if (!TEST_ptr(gettable = EVP_PKEY_gettable_params(pkeyparam)) ||
	    ...
	    !TEST_ptr(OSSL_PARAM_locate_const(
		    gettable, OSSL_PKEY_PARAM_EC_CHAR2_PP_K1)) ||
	    !TEST_ptr(OSSL_PARAM_locate_const(
		    gettable, OSSL_PKEY_PARAM_EC_CHAR2_PP_K2)) ||
	    !TEST_ptr(OSSL_PARAM_locate_const(gettable,
...
static int custom_params_test(int id)
{
	...
	if (!TEST_ptr(param_bld = OSSL_PARAM_BLD_new()) ||
	    ...
	    !TEST_true(OSSL_PARAM_BLD_push_BN(
		    param_bld, OSSL_PKEY_PARAM_PRIV_KEY, priv1)) ||
