...
static int test_client_hello(int currtest)
{
	...
	case TEST_PADDING_NOT_NEEDED:
		...
		if (!TEST_true(SSL_CTX_set1_groups_list(
			    ctx, "?X25519:?secp256r1:?ffdhe2048:?ffdhe3072")))
			goto end;
		/*
		...
		if (currtest == TEST_ADD_PADDING) {
			if (!TEST_false(SSL_CTX_set_alpn_protos(
				    ctx, (unsigned char *)alpn_prots,
				    sizeof(alpn_prots) - 1)))
				goto end;
			/*
	...
	if (currtest == TEST_SET_SESSION_TICK_DATA_VER_NEG) {
		if (!TEST_true(SSL_set_session_ticket_ext(
			    con, dummytick, (int)strlen(dummytick))))
			goto end;
	}
