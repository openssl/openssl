...
static int rpk_verify_client_cb(int ok, X509_STORE_CTX *ctx)
{
	...
	if (X509_STORE_CTX_get0_rpk(ctx) != NULL) {
		if (err != X509_V_OK) {
			TEST_info("rpk_verify_client_cb: ok=%d err=%d", ok,
				  err);
			return 0;
		}
	}
...
static int rpk_verify_server_cb(int ok, X509_STORE_CTX *ctx)
{
	...
	if (X509_STORE_CTX_get0_rpk(ctx) != NULL) {
		if (err != X509_V_OK) {
			TEST_info("rpk_verify_server_cb: ok=%d err=%d", ok,
				  err);
			return 0;
		}
	}
...
#define RPK_DIMS (2 * 4 * 2 * 2 * 2 * 2)
	...
	if (!TEST_true(create_ssl_ctx_pair(
		    test_libctx, TLS_server_method(), TLS_client_method(),
	...
	if (idx_client_client_rpk)
		if (!TEST_true(SSL_CTX_set1_client_cert_type(
			    cctx, cert_type_rpk, sizeof(cert_type_rpk))))
			goto end;
	if (!TEST_true(SSL_CTX_set_session_id_context(sctx, SID_CTX,
						      sizeof(SID_CTX))))
		goto end;
	if (!TEST_true(SSL_CTX_set_session_id_context(cctx, SID_CTX,
						      sizeof(SID_CTX))))
		goto end;
	...
	case 2:
		...
		if (!TEST_int_eq(SSL_use_PrivateKey_file(clientssl,
							 privkey_file,
							 SSL_FILETYPE_PEM),
				 1))
			goto end;
	...
	case 7:
		...
		if (!TEST_true(SSL_add_expected_rpk(clientssl, other_pkey)))
			goto end;
		SSL_set_verify(clientssl, SSL_VERIFY_NONE,
	...
	if (client_auth) {
		if (idx_server_client_rpk && idx_client_client_rpk) {
			...
			if (!TEST_ptr(SSL_get0_peer_rpk(serverssl)))
				goto end;
			if (!TEST_int_eq(SSL_get_negotiated_client_cert_type(
						 serverssl),
					 TLSEXT_cert_type_rpk))
				goto end;
			if (!TEST_int_eq(SSL_get_negotiated_client_cert_type(
	...
	if (resumption) {
		...
		if (!TEST_int_eq(SSL_use_PrivateKey_file(serverssl,
							 privkey_file,
							 SSL_FILETYPE_PEM),
				 1))
			goto end;
		...
		default:
			break;
		case 11:
			if (!TEST_true(SSL_add_expected_rpk(clientssl,
							    client_pkey)))
				goto end;
			break;
		case 12:
		...
		case 13:
			if (!TEST_true(SSL_add_expected_rpk(clientssl,
							    client_pkey)))
				goto end;
			if (!TEST_true(SSL_add_expected_rpk(serverssl,
							    server_pkey)))
				goto end;
			/* Use the same key for client auth */
			if (!TEST_int_eq(SSL_use_PrivateKey_file(
						 clientssl, privkey_file,
						 SSL_FILETYPE_PEM),
					 1))
				goto end;
			if (!TEST_int_eq(SSL_use_certificate_file(
						 clientssl, cert_file,
						 SSL_FILETYPE_PEM),
					 1))
				goto end;
		...
		ret = create_ssl_connection(serverssl, clientssl,
					    SSL_ERROR_NONE);
		if (!TEST_true(ret))
			goto end;
