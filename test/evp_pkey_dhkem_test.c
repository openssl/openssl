...
static int test_dhkem_encapsulate(int tstid)
{
	...
	if (t->spriv != NULL) {
		if (!TEST_ptr(spriv = new_raw_private_key(t->curve, t->spriv,
							  t->sprivlen, t->spub,
							  t->spublen)))
			goto err;
	}
...
static int test_dhkem_decapsulate(int tstid)
{
	...
	if (!TEST_ptr(rpriv = new_raw_private_key(t->curve, t->rpriv,
						  t->rprivlen, t->rpub,
						  t->rpublen)))
		goto err;
	if (t->spub != NULL) {
		if (!TEST_ptr(spub = new_raw_public_key(t->curve, t->spub,
							t->spublen)))
			goto err;
	}
...
static int test_settables(int tstid)
{
	...
	return TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, NULL), 1) &&
	       ...
	       TEST_ptr(p = OSSL_PARAM_locate_const(
				settableparams, OSSL_KEM_PARAM_OPERATION)) &&
...
static int test_ec_dhkem_derivekey_fail(void)
{
	...
	if (!TEST_ptr(genctx =
			      EVP_PKEY_CTX_new_from_name(libctx, "EC", NULL)) ||
...
static int test_no_operation_set(int tstid)
{
	...
	return TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, NULL), 1) &&
	       TEST_int_eq(EVP_PKEY_encapsulate(ctx, NULL, &len, NULL, NULL),
			   1) &&
	       TEST_int_eq(EVP_PKEY_decapsulate_init(ctx, NULL), 1) &&
...
static int test_ikm_small(int tstid)
{
	...
	return TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, params), 1) &&
	       TEST_int_eq(EVP_PKEY_encapsulate(ctx, enc, &enclen, secret,
						&secretlen),
			   0);
}
...
static int test_input_size_small(int tstid)
{
	...
	seclen--;
	if (!TEST_int_eq(EVP_PKEY_decapsulate(ctx, sec, &seclen, enc, enclen),
			 0))
		goto err;
	seclen++;
	/* incorrect enclen passed to decap  */
	enclen--;
	ret = TEST_int_eq(EVP_PKEY_decapsulate(ctx, sec, &seclen, enc, enclen),
			  0);
err:
	return ret;
}
...
static int test_ec_auth_key_curve_mismatch(void)
{
	...
	ret = TEST_int_eq(
		EVP_PKEY_auth_encapsulate_init(rctx[0], auth, opparam), 0);
	EVP_PKEY_free(auth);
	return ret;
}
...
static int test_auth_key_type_mismatch(int tstid)
{
	...
	return TEST_int_eq(EVP_PKEY_auth_encapsulate_init(rctx[id1], rkey[id2],
							  opparam),
			   0);
}
...
static int test_ec_public_key_infinity(void)
{
	...
	unsigned char tmp[1] = {
		0
	}; /* The encoding for an EC point at infinity */
	EVP_PKEY_CTX *ctx = rctx[0];
	const TEST_ENCAPDATA *t = &ec_encapdata[0];

	ret = TEST_ptr(key = new_raw_private_key(t->curve, t->rpriv,
						 t->rprivlen, tmp,
						 sizeof(tmp))) &&
	      ...
	      && TEST_int_eq(EVP_PKEY_auth_encapsulate_init(ctx, key, opparam),
			     1) &&
...
static int test_null_params(int tstid)
{
	...
	return TEST_int_eq(EVP_PKEY_auth_encapsulate_init(ctx, NULL, opparam),
			   0) &&
...
static int test_set_params(int tstid)
{
	...
	badparams[0] = OSSL_PARAM_construct_utf8_string(
		OSSL_KEM_PARAM_OPERATION, "unknown_op", 0);
	badparams[1] = OSSL_PARAM_construct_end();
	if (!TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, badparams), 0))
		goto err;
	...
	badparams[0] = OSSL_PARAM_construct_utf8_string(
		OSSL_KEM_PARAM_OPERATION, NULL, 0);
	badparams[1] = OSSL_PARAM_construct_end();
	if (!TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, badparams), 0))
		goto err;
...
static int test_nopublic(int tstid)
{
	...
	if (!TEST_ptr(priv = new_raw_private_key(t->curve, t->rpriv,
						 t->rprivlen, NULL, 0)))
		goto err;
	...
	if (encap) {
		if (!TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, opparam),
				 expected))
			goto err;
	} else {
		if (!TEST_int_eq(EVP_PKEY_decapsulate_init(ctx, opparam),
				 expected))
			goto err;
	}
	if (expected == 0 && !TEST_int_eq(ERR_GET_REASON(ERR_get_error()),
...
static int test_ec_dhkem_derivekey(int tstid)
{
	...
	ret = TEST_ptr(genctx = EVP_PKEY_CTX_new_from_name(libctx, "EC",
							   NULL)) &&
	      TEST_int_eq(EVP_PKEY_keygen_init(genctx), 1) &&
	      TEST_int_eq(EVP_PKEY_CTX_set_params(genctx, params), 1) &&
...
static int test_ec_noikme(int tstid)
{
	...
	if (curve[0] == 'X') {
		if (!TEST_ptr(recip = EVP_PKEY_Q_keygen(libctx, NULL, curve)) ||
		    (auth && !TEST_ptr(sender_auth = EVP_PKEY_Q_keygen(
					       libctx, NULL, curve))))
			goto err;
	} else {
		if (!TEST_ptr(recip = EVP_PKEY_Q_keygen(libctx, NULL, "EC",
							curve)) ||
		    (auth && !TEST_ptr(sender_auth = EVP_PKEY_Q_keygen(
					       libctx, NULL, "EC", curve))))
			goto err;
	}
...
static int test_ec_badpublic(int tstid)
{
	...
	if (!TEST_ptr(
		    ctx = EVP_PKEY_CTX_new_from_pkey(libctx, recippriv, NULL)))
		goto err;
	...
	if (encap) {
		...
		if (!TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, opparam), 1))
			goto err;
		if (!TEST_int_eq(EVP_PKEY_encapsulate(ctx, enc, &enclen, secret,
						      &secretlen),
				 0))
			goto err;
	} else {
...
static int test_ecx_dhkem_derivekey(int tstid)
{
	...
	ret = TEST_ptr(genctx = EVP_PKEY_CTX_new_from_name(libctx, t->curvename,
							   NULL)) &&
	      TEST_int_eq(EVP_PKEY_keygen_init(genctx), 1) &&
	      TEST_int_eq(EVP_PKEY_CTX_set_params(genctx, params), 1) &&
...
static int test_ecx_auth_key_curve_mismatch(void)
{
	...
	ret = TEST_int_eq(EVP_PKEY_auth_encapsulate_init(
				  rctx[TEST_KEYTYPE_X25519], auth, opparam),
			  0);
	EVP_PKEY_free(auth);
	return ret;
}
...
int setup_tests(void)
{
	...
	ADD_ALL_TESTS(test_ec_noikme,
		      TEST_TYPE_AUTH_NOAUTH *
			      OSSL_NELEM(dhkem_supported_curves));
