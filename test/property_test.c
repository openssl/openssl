...
static const struct {
	...
	{ "n=0777777777777777777777", "n=0777777777777777777777",
	  1 }, /* INT64_MAX */
	{ "n=033", "n=27", 1 },
	{ "n=0", "n=00", 1 },
	{ "n=0x0", "n=0", 1 },
...
static const struct {
	...
	{ "alpha", "alpha=yes", 1 },	   { "alpha=no", "alpha", -1 },
	{ "alpha=1", "alpha=1", 1 },	   { "alpha=2", "alpha=1", -1 },
	{ "alpha", "omega", -1 },	   { "alpha", "?omega", 0 },
	{ "alpha", "?omega=1", 0 },	   { "alpha", "?omega=no", 1 },
	{ "alpha", "?omega=yes", 0 },	   { "alpha, omega", "?omega=yes", 1 },
...
static int test_definition_compares(int n)
{
	...
	r = TEST_ptr(store = ossl_method_store_new(NULL)) &&
	    ...
	    TEST_ptr(
		    q = ossl_parse_query(NULL, definition_tests[n].query, 0)) &&
...
static int test_register_deregister(void)
{
	...
	for (i = 0; i < OSSL_NELEM(impls); i++)
		if (!TEST_true(ossl_method_store_add(
			    store, &prov, impls[i].nid, impls[i].prop,
			    impls[i].impl, &up_ref, &down_ref))) {
...
static int test_property(void)
{
	...
	for (i = 0; i < OSSL_NELEM(impls); i++)
		if (!TEST_true(ossl_method_store_add(
			    store, *impls[i].prov, impls[i].nid, impls[i].prop,
			    impls[i].impl, &up_ref, &down_ref))) {
			TEST_note("iteration %zd", i + 1);
			goto err;
		}
	...
	for (i = 0; i < OSSL_NELEM(queries); i++) {
		...
		if (queries[i].prov == &fake_prov1) {
			if (!TEST_true(ossl_method_store_fetch(
				    store, queries[i].nid, queries[i].prop,
				    &fake_prov1, &result)) ||
			...
			if (!TEST_false(ossl_method_store_fetch(
				    store, queries[i].nid, queries[i].prop,
				    &fake_prov1, &result)) ||
			    !TEST_ptr_eq(fake_prov1, &fake_provider1) ||
...
static int test_query_cache_stochastic(void)
{
	...
	for (i = 1; i <= max; i++) {
		...
		if (!TEST_true(ossl_method_store_add(
			    store, &prov, i, buf, "abc", &up_ref, &down_ref)) ||
		    !TEST_true(ossl_method_store_cache_set(
			    store, &prov, i, buf, v + i, &up_ref, &down_ref)) ||
		    !TEST_true(ossl_method_store_cache_set(
			    store, &prov, i, "n=1234", "miss", &up_ref,
			    &down_ref))) {
	...
	for (i = 1; i <= max; i++) {
		BIO_snprintf(buf, sizeof(buf), "n=%d\n", i);
		if (!ossl_method_store_cache_get(store, NULL, i, buf,
						 &result) ||
		    result != v + i)
			errors++;
	}
...
static int test_fips_mode(void)
{
	...
	ret = TEST_true(EVP_set_default_properties(ctx,
...
int setup_tests(void)
{
	...
	ADD_ALL_TESTS(test_property_list_to_string,
		      OSSL_NELEM(to_string_tests));
	return 1;
}
