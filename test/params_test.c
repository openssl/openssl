...
static int raw_set_params(void *vobj, const OSSL_PARAM *params)
{
	...
	for (; params->key != NULL; params++)
		if (strcmp(params->key, "p1") == 0) {
			...
			BN_free(obj->p3);
			if (!TEST_ptr(obj->p3 = BN_native2bn(
					      params->data,
					      (int)params->data_size, NULL)))
				return 0;
		} else if (strcmp(params->key, "p4") == 0) {
			OPENSSL_free(obj->p4);
			if (!TEST_ptr(obj->p4 = OPENSSL_strndup(
             ...
             */
			size_t data_length = OPENSSL_strnlen(params->data,
							     params->data_size);
	...
	for (; params->key != NULL; params++)
		if (strcmp(params->key, "p1") == 0) {
			...
			*(double *)params->data = obj->p2;
			...
			if (!TEST_size_t_ge(params->data_size,
					    params->return_size))
				return 0;
			BN_bn2nativepad(obj->p3, params->data,
					(int)params->return_size);
		} else if (strcmp(params->key, "p4") == 0) {
			params->return_size = strlen(obj->p4);
			if (!TEST_size_t_gt(params->data_size,
					    params->return_size))
				return 0;
...
static int api_set_params(void *vobj, const OSSL_PARAM *params)
{
	...
	if ((p = OSSL_PARAM_locate_const(params, "p5")) != NULL) {
		char *p5_ptr = obj->p5;
		if (!TEST_true(OSSL_PARAM_get_utf8_string(p, &p5_ptr,
							  sizeof(obj->p5))))
			return 0;
		obj->p5_l = strlen(obj->p5);
	}
...
/* "raw" provider */
static const struct provider_dispatch_st provider_raw = { raw_set_params,
							  raw_get_params };

/* "api" provider */
static const struct provider_dispatch_st provider_api = { api_set_params,
							  api_get_params };
...
static OSSL_PARAM *construct_api_params(void)
{
	...
	params[n++] =
		OSSL_PARAM_construct_BN("p3", bignumbin, sizeof(bignumbin));
	params[n++] =
		OSSL_PARAM_construct_utf8_string("p4", app_p4, sizeof(app_p4));
	params[n++] =
		OSSL_PARAM_construct_utf8_string("p5", app_p5, sizeof(app_p5));
	/* sizeof(app_p6_init), because we know that's what we're using */
	params[n++] = OSSL_PARAM_construct_utf8_ptr("p6", (char **)&app_p6,
						    sizeof(app_p6_init));
	params[n++] =
		OSSL_PARAM_construct_octet_string("foo", &foo, sizeof(foo));
...
static const struct param_owner_st api_params = { static_api_params,
			     ...
			     const struct provider_dispatch_st *prov)
{
	...
	if (!TEST_true(prov->set_params(obj, params))) {
		...
		if (!TEST_int_eq(sneakpeek->p1, app_p1) /* app value set */
		    || !TEST_double_eq(sneakpeek->p2,
				       p2_init) /* Should remain untouched */
...
static int check_int_from_text(const struct int_from_text_test_st a)
{
	...
	if (!OSSL_PARAM_allocate_from_text(&param, params_from_text, a.argname,
					   a.strval, 0, NULL)) {
		if (a.expected_res)
			TEST_error(
	...
	if (param.data_size == 0) {
		...
		TEST_error("unexpected zero size for %s \"%s\"", a.argname,
			   a.strval);
		return 0;
	}
	...
	if (res ^ a.expected_res) {
		TEST_error(
			"unexpected OSSL_PARAM_get_long() return for %s \"%s\": "
			"%d != %d",
	...
	if (param.data_size != a.expected_bufsize) {
		TEST_error("unexpected size for %s \"%s\":  %d != %d",
			   a.argname, a.strval, (int)a.expected_bufsize,
			   (int)param.data_size);
		return 0;
	}
...
static int check_octetstr_from_hexstr(void)
{
	...
	for (i = 0; values[i] != NULL; i++) {
		...
		if (expected)
			result = TEST_true(OSSL_PARAM_allocate_from_text(
				&param, params_from_text, "hexoctets",
				values[i], 0, NULL));
		else
			result = TEST_false(OSSL_PARAM_allocate_from_text(
				&param, params_from_text, "hexoctets",
				values[i], 0, NULL));
		if (!result) {
			TEST_error(
				"unexpected OSSL_PARAM_allocate_from_text() %s for 'octets' \"%s\"",
