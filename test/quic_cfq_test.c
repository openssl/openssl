...
static int check(QUIC_CFQ *cfq)
{
	...
	for (pn_space = QUIC_PN_SPACE_INITIAL; pn_space < QUIC_PN_SPACE_NUM;
	     ++pn_space)
		for (i = 0,
		    item = ossl_quic_cfq_get_priority_head(cfq, pn_space);
		     ; ++i, item = ossl_quic_cfq_item_get_priority_next(
				    item, pn_space)) {
			...
			if (!TEST_ptr(item) ||
			    ...
			    !TEST_int_eq(ossl_quic_cfq_item_get_pn_space(item),
					 pn_space) ||
...
static int test_cfq(void)
{
	...
	for (i = 0; i < OSSL_NELEM(ref_buf); ++i) {
		if (!TEST_ptr(item = ossl_quic_cfq_add_frame(
				      ...
				      ref_frame_type[i], 0, ref_buf + i, 1,
				      free_cb, NULL)) ||
		    !TEST_int_eq(ossl_quic_cfq_item_get_state(item),
				 QUIC_CFQ_STATE_NEW) ||
	...
	for (pn_space = QUIC_PN_SPACE_INITIAL; pn_space < QUIC_PN_SPACE_NUM;
		...
		for (item = ossl_quic_cfq_get_priority_head(cfq, pn_space);
			...
			inext = ossl_quic_cfq_item_get_priority_next(item,
								     pn_space);
