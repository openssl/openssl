			  ...
			  const unsigned char *exp, const int exp_len)
{
	...
	if (!TEST_true(PKCS5_pbe_set0_algor(algor, EVP_CIPHER_nid(cipher),
					    pbe_iter, pbe_salt,
					    sizeof(pbe_salt))) ||
	    !TEST_true(PKCS5_PBE_keyivgen(ctx, pbe_password,
					  (int)strlen(pbe_password),
	...
	if (!TEST_true(PKCS5_PBE_keyivgen(ctx, pbe_password,
					  (int)strlen(pbe_password),
...
{
	return test_pkcs5_pbe(EVP_des_cbc(), EVP_sha1(),
