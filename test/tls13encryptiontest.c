...
static int test_tls13_encryption(void)
{
	...
	for (ctr = 0; ctr < OSSL_NELEM(refdata); ctr++) {
		...
		if (!load_record(&rec, &refdata[ctr], &key, iv, ivlen,
				 seqbuf)) {
		...
		if (!TEST_true(ossl_tls_record_method.new_record_layer(
			    ...
			    OSSL_RECORD_PROTECTION_LEVEL_APPLICATION, 0, NULL,
			    0, key, 16, iv, ivlen, NULL, 0, EVP_aes_128_gcm(),
			    EVP_GCM_TLS_TAG_LEN, 0, NULL, NULL, NULL, NULL,
		...
		/* Encrypt it */
		if (!TEST_size_t_eq(
			    wrl->funcs->cipher(wrl, &rec, 1, 1, NULL, 0), 1)) {
		...
		/* Decrypt it */
		if (!TEST_int_eq(rrl->funcs->cipher(rrl, &rec, 1, 0, NULL, 0),
				 1)) {
