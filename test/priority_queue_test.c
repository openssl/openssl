					  ...
					  int remove, int random, int popfree)
{
	...
	static size_t values[MAX_SAMPLES], sorted[MAX_SAMPLES],
		ref[MAX_SAMPLES];
	size_t n;
	int i, res = 0;
	static const char *orders[3] = { "unordered", "ascending",
					 "descending" };
	...
	if (order == 1)
		...
		qsort(values, count, sizeof(*values),
	...
	for (i = 0; i < count; i++)
		if (!TEST_true(
			    ossl_pqueue_size_t_push(pq, values + i, ref + i)))
			goto err;
	...
	if (remove) {
		while (remove-- > 0) {
			...
			if (values[i] != SIZE_MAX) {
				if (!TEST_ptr_eq(ossl_pqueue_size_t_remove(
							 pq, ref[i]),
						 values + i))
					goto err;
				values[i] = SIZE_MAX;
...
{
	return test_size_t_priority_queue_int(0, 0, MAX_SAMPLES,
					      MAX_SAMPLES / 100, 1, 1);
}
