...
static int test_skey_cipher(void)
{
	...
	fake_cipher = EVP_CIPHER_fetch(libctx, "fake_cipher",
	...
	params[1] = OSSL_PARAM_construct_octet_string(
		OSSL_SKEY_PARAM_RAW_BYTES, (void *)import_key, KEY_SIZE);
	...
	if (!TEST_ptr(ctx = EVP_CIPHER_CTX_new()) ||
	    !TEST_int_gt(EVP_CipherInit_SKEY(ctx, fake_cipher, key, NULL, 0, 1,
					     NULL),
			 0))
		goto end;
...
static int test_skey_skeymgmt(void)
{
	...
	if (!TEST_ptr(skeymgmt = EVP_SKEYMGMT_fetch(
			      libctx, OSSL_SKEY_TYPE_GENERIC, NULL)))
		goto end;

	/* Check the parameter we need is available */
	if (!TEST_ptr(imp_params = EVP_SKEYMGMT_get0_imp_settable_params(
			      skeymgmt)) ||
	...
	if (!TEST_ptr(key = EVP_SKEY_import(
			      libctx, EVP_SKEYMGMT_get0_name(skeymgmt), NULL,
			      OSSL_SKEYMGMT_SELECT_ALL, params)))
		goto end;
...
static int test_aes_raw_skey(void)
{
	...
	/* Create EVP_SKEY */
	skey = EVP_SKEY_import_raw_key(libctx, "AES-128", aes_key, KEY_SIZE,
				       NULL);
	if (!TEST_ptr(skey))
		goto end;

	if (!TEST_int_gt(EVP_SKEY_get0_raw_key(skey, &export_key,
					       &export_length),
			 0) ||
	...
	fin_len = 0;
	if (!TEST_ptr(ctx = EVP_CIPHER_CTX_new()) ||
	    !TEST_int_gt(EVP_CipherInit_SKEY(ctx, aes_cbc, skey, aes_iv,
					     IV_SIZE, 1, NULL),
			 0) ||
	    !TEST_int_gt(EVP_CipherUpdate(ctx, encrypted_skey, &enc_len, data,
					  DATA_SIZE),
			 0) ||
	    !TEST_int_gt(EVP_CipherFinal(ctx, encrypted_skey + enc_len,
					 &fin_len),
			 0))
		goto end;
	...
	fin_len = 0;
	if (!TEST_int_gt(EVP_CipherInit_ex2(ctx, aes_cbc, aes_key, aes_iv, 1,
					    NULL),
			 0) ||
	    !TEST_int_gt(EVP_CipherUpdate(ctx, encrypted_raw, &enc_len, data,
					  DATA_SIZE),
			 0) ||
	    !TEST_int_gt(EVP_CipherFinal(ctx, encrypted_raw + enc_len,
					 &fin_len),
			 0) ||
	    !TEST_mem_eq(encrypted_skey, DATA_SIZE + IV_SIZE, encrypted_raw,
			 DATA_SIZE + IV_SIZE))
		goto end;
...
static int test_des_raw_skey(void)
{
	...
	fin_len = 0;
	if (!TEST_ptr(ctx = EVP_CIPHER_CTX_new()) ||
	    !TEST_int_gt(EVP_CipherInit_SKEY(ctx, des_cbc, skey, des_iv,
					     DES_IV_SIZE, 1, NULL),
			 0) ||
	    !TEST_int_gt(EVP_CipherUpdate(ctx, encrypted_skey, &enc_len, data,
					  DATA_SIZE),
	...
	enc_len = sizeof(encrypted_raw);
	fin_len = 0;
	if (!TEST_int_gt(EVP_CipherInit_ex2(ctx, des_cbc, des_key, des_iv, 1,
					    NULL),
			 0) ||
	    !TEST_int_gt(EVP_CipherUpdate(ctx, encrypted_raw, &enc_len, data,
					  DATA_SIZE),
