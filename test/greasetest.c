...
static int setup_grease(SSL_CTX *ctx)
{
    ...
    if (!TEST_true(SSL_CTX_add_grease_to_extension(ctx, TLSEXT_TYPE_supported_versions, 0x5A5A)))
        return 0;
    if (!TEST_true(SSL_CTX_add_grease_to_extension(ctx, TLSEXT_TYPE_signature_algorithms, 0x6A6A)))
        return 0;
    if (!TEST_true(SSL_CTX_add_grease_to_extension(ctx, TLSEXT_TYPE_signature_algorithms, 0x7A7A)))
        return 0;
    if (!TEST_true(SSL_CTX_add_grease_to_extension(ctx, TLSEXT_TYPE_supported_groups, 0x8A8A)))
        return 0;
    if (!TEST_true(SSL_CTX_add_grease_to_extension(ctx, TLSEXT_TYPE_compress_certificate, 0x9A9A)))
        return 0;
    ...
    if (!TEST_true(SSL_CTX_add_grease_to_extension(ctx, TLSEXT_TYPE_server_cert_type, 0x7A)))
        return 0;
    if (!TEST_true(SSL_CTX_add_grease_to_extension(ctx, TLSEXT_TYPE_key_share, 0x11119A9A)))
        return 0;
    /* Duplicate */
    if (!TEST_false(SSL_CTX_add_grease_to_extension(ctx, TLSEXT_TYPE_supported_versions, 0x5A5A)))
        return 0;

    if (!TEST_true(SSL_CTX_add_grease_extension(ctx, 0x1111, extension_data, sizeof(extension_data))))
        return 0;
    if (!TEST_true(SSL_CTX_add_grease_extension(ctx, 0x2222, extension_data, sizeof(extension_data))))
        return 0;
    /* Duplicate */
    if (!TEST_false(SSL_CTX_add_grease_extension(ctx, 0x2222, extension_data, sizeof(extension_data))))
        return 0;
...
# define GREASE_DIMS 2
    ...
    /* Configure Grease */
    if (!TEST_true(setup_grease(sctx))
            || !TEST_true(setup_grease(cctx)))
        goto end;
    ...
    if (resumption) {
        ...
        if (idx == 5 || idx == 6) {
            if (!TEST_true(setup_cert(clientssl)))
                goto end;
            SSL_set_verify(serverssl, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, my_verify_cb);
        }
