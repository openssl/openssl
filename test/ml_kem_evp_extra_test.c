...
const OPTIONS *test_get_options(void)
{
	static const OPTIONS options[] = { OPT_TEST_OPTIONS_DEFAULT_USAGE,
					   { "test-rand", OPT_TEST_RAND, '-',
					     "Test non-derandomised ML-KEM" },
					   { NULL } };
	return options;
}
...
static uint8_t dec_seed[32] = {
	0x4e, 0x6f, 0x74, 0x20, 0x74, 0x68, 0x65, 0x20, 0x64, 0x72, 0x6f,
...
static int test_non_derandomised_ml_kem(void)
{
	...
	for (i = 0; i < (int)OSSL_NELEM(alg); ++i) {
		...
		*p++ = OSSL_PARAM_construct_octet_string(
			OSSL_RAND_PARAM_TEST_ENTROPY, gen_seed,
			sizeof(gen_seed));
		...
		*p = OSSL_PARAM_construct_end();
		...
		if (!TEST_int_gt(EVP_PKEY_copy_parameters(bkey, akey), 0))
			goto done;
		if (!TEST_true(EVP_PKEY_set1_encoded_public_key(bkey, rawpub,
								publen)))
			goto done;
		...
		p = params;
		*p = OSSL_PARAM_construct_octet_string(
			OSSL_RAND_PARAM_TEST_ENTROPY, enc_seed,
			sizeof(enc_seed));
		...
		if (!TEST_int_gt(EVP_PKEY_encapsulate_init(ctx, NULL), 0))
			goto done;
		if (!TEST_int_gt(EVP_PKEY_encapsulate(ctx, NULL, &wrpkeylen,
						      NULL, &bgenkeylen),
				 0))
			goto done;
		...
		if (!TEST_ptr(wrpkey) || !TEST_ptr(bgenkey))
			goto done;
		if (!TEST_true(EVP_PKEY_encapsulate(ctx, wrpkey, &wrpkeylen,
						    bgenkey, &bgenkeylen)))
			goto done;
		EVP_PKEY_CTX_free(ctx);
		ctx = NULL;
		...
		if (!TEST_true(EVP_Digest(wrpkey, v->ctext_bytes, hash, NULL,
					  sha256, NULL)) ||
		...
		if (!TEST_int_gt(EVP_PKEY_decapsulate_init(ctx, NULL), 0))
			goto done;
		if (!TEST_true(EVP_PKEY_decapsulate(ctx, NULL, &agenkeylen,
						    wrpkey, wrpkeylen)))
			goto done;
		...
		if (!TEST_ptr(agenkey))
			goto done;
		if (!TEST_true(EVP_PKEY_decapsulate(ctx, agenkey, &agenkeylen,
						    wrpkey, wrpkeylen)))
			goto done;
		/* Hopefully we ended up with a shared key */
		if (!TEST_mem_eq(agenkey, agenkeylen, bgenkey, bgenkeylen))
		...
		p = params;
		*p = OSSL_PARAM_construct_octet_string(
			OSSL_RAND_PARAM_TEST_ENTROPY, dec_seed,
			sizeof(dec_seed));
		if (!TEST_true(EVP_RAND_CTX_set_params(pubctx, params)))
			goto done;

		/* This time decap should fail, and return the decap entropy */
		if (!TEST_false(EVP_PKEY_decapsulate(ctx, agenkey, &agenkeylen,
						     wrpkey, wrpkeylen - 1)))
			goto done;
		if (!TEST_mem_eq(agenkey, agenkeylen, dec_seed,
				 sizeof(dec_seed)))
			goto done;
