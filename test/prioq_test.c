...
static int elem_compare(const OSSL_PRIOQ_NODE *a, const OSSL_PRIOQ_NODE *b)
{
...
static void free_checker(ossl_unused OSSL_PRIOQ_NODE *n)
{
    num_rec_freed++;
}
    ...
    int remove, int random, int popfree)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        ossl_prioq_node_init(&elems[i].pqn);
    ...
    if (!TEST_ptr(pq = ossl_prioq_new(elem_compare))
        || !TEST_size_t_eq(ossl_prioq_num(pq), 0))
        goto err;

    if (reserve && !TEST_true(ossl_prioq_reserve(pq, count)))
        goto err;

    for (i = 0; i < count; i++)
        if (!TEST_true(ossl_prioq_push(pq, &elems[i].pqn)))
            goto err;

    if (!TEST_ptr(ossl_prioq_peek(pq))
        || !TEST_size_t_eq(CONTAINER_OF(ossl_prioq_peek(pq), ELEM, pqn)->value,
            sorted[0])
        || !TEST_size_t_eq(ossl_prioq_num(pq), count))
        goto err;

    if (remove) {
        while (remove-- > 0) {
            i = test_random() % count;
            if (ossl_prioq_node_in_queue(&elems[i].pqn)) {
                if (!TEST_ptr_eq(ossl_prioq_remove(pq, &elems[i].pqn),
                        &elems[i].pqn))
                    goto err;
                /*
    ...
    for (i = 0; ossl_prioq_peek(pq) != NULL; i++) {
        OSSL_PRIOQ_NODE *peeked = ossl_prioq_peek(pq);
        OSSL_PRIOQ_NODE *popped;

        if (!TEST_size_t_eq(CONTAINER_OF(peeked, ELEM, pqn)->value, sorted[i]))
            goto err;
        popped = ossl_prioq_pop(pq);
    ...
    if (popfree) {
        num_rec_freed = 0;
        n = ossl_prioq_num(pq);
        ossl_prioq_pop_free(pq, &free_checker);
        pq = NULL;
...
static int info_cmp(const OSSL_PRIOQ_NODE *a, const OSSL_PRIOQ_NODE *b)
{
...
static int test_22644(void)
{
    ...
    INFO infos[32];
    int res = 0;
    OSSL_PRIOQ *pq = ossl_prioq_new(info_cmp);

    memset(infos, 0, sizeof(infos));
    for (i = 0; i < 32; i++) {
        infos[i].sub_seq = i;
        ossl_prioq_node_init(&infos[i].pqn);
    }

    infos[0].seq_num = 70650219160667140;
    if (!TEST_true(ossl_prioq_push(pq, &infos[0].pqn))
        || !TEST_ptr_eq(ossl_prioq_remove(pq, &infos[0].pqn), &infos[0].pqn))
        goto err;

    infos[1].seq_num = 289360691352306692;
    if (!TEST_true(ossl_prioq_push(pq, &infos[1].pqn))
        || !TEST_ptr_eq(ossl_prioq_remove(pq, &infos[1].pqn), &infos[1].pqn))
        goto err;

    for (i = 2; i <= 8; i++) {
        infos[i].seq_num = 289360691352306692;
        if (!TEST_true(ossl_prioq_push(pq, &infos[i].pqn)))
            goto err;
    }

    if (!TEST_ptr(ossl_prioq_pop(pq))
        || !TEST_ptr(ossl_prioq_pop(pq))) /* crash if bug present */
        goto err;
    res = 1;
...
static int test_edge_cases(void)
{
    ...
    if (!TEST_ptr_null(ossl_prioq_new(NULL)))
        goto err;

    if (!TEST_size_t_eq(ossl_prioq_num(NULL), 0)
        || !TEST_ptr_null(ossl_prioq_peek(NULL))
        || !TEST_ptr_null(ossl_prioq_pop(NULL)))
        goto err;

    if (!TEST_ptr(pq = ossl_prioq_new(elem_compare)))
        goto err;

    if (!TEST_ptr_null(ossl_prioq_peek(pq))
        || !TEST_ptr_null(ossl_prioq_pop(pq))
        || !TEST_size_t_eq(ossl_prioq_num(pq), 0))
        goto err;

    /* Remove on a detached node is a no-op. */
    e.value = 42;
    ossl_prioq_node_init(&e.pqn);
    if (!TEST_ptr_null(ossl_prioq_remove(pq, &e.pqn))
        || !TEST_false(ossl_prioq_node_in_queue(&e.pqn)))
        goto err;

    /* Single-element push/pop. */
    if (!TEST_true(ossl_prioq_push(pq, &e.pqn))
        || !TEST_size_t_eq(ossl_prioq_num(pq), 1)
        || !TEST_ptr_eq(ossl_prioq_peek(pq), &e.pqn)
        || !TEST_ptr_eq(ossl_prioq_pop(pq), &e.pqn)
        || !TEST_size_t_eq(ossl_prioq_num(pq), 0)
        || !TEST_false(ossl_prioq_node_in_queue(&e.pqn)))
        goto err;

    ossl_prioq_free(NULL); /* must not crash */
    res = 1;
err:
    ossl_prioq_free(pq);
    return res;
