...
#undef DO_PRINT
    ...
    if (!TEST_str_eq(bio_buf, data->expected)
        ...
        + !(data->skip_libc_check || data->skip_libc_ret_check
...
static const struct n_data {
    ...
    { "|%#2147483639x|%2147483639s|0123456789abcdef|%ln",
      ...
      .skip_libc_check = true, .exp_ret = -1 },
    { "|%-9876543210s|%012345678901o|0123456789|%lln",
      "|echo test test test                                           ",
      AT_LLONG, 4294967308ULL,
      AT_STR, { .s = "echo test test test" }, AT_INT, { .i = 0xbad },
      /* glibc caps %n value (1 << 32) - 1 */
      .skip_libc_check = true, .exp_ret = -1 },
    { "|%11111111111111111s|%22222222222222222u|33333333333333333|%zn",
      "|                                                              ",
      AT_SIZE, sizeof(size_t) == 8 ? 4294967315ULL : 19,
      AT_STR, { .s = "according to all known laws of aviation" },
      AT_INT, { .i = 0xbee },
      /* glibc caps %n value (1 << 32) - 1 */
      .skip_libc_check = true, .exp_ret = -1 },
    { "=%11111111111.9999999999s=%+8888888888d=7777777777=%tn=",
      "=                                                              ",
      AT_PTRDIFF, sizeof(ptrdiff_t) == 8 ? 4294967308ULL : 12,
...
#undef DO_PRINT
    ...
    if (!TEST_str_eq(bio_buf, data->expected)
        ...
        + !TEST_int_eq(bio_ret, exp_ret)
        + !(skip_libc_check || TEST_str_eq(bio_buf, std_buf))
        + !(skip_libc_check || TEST_uint64_t_eq(n.val, std_n.val))
        + !(skip_libc_check || data->skip_libc_ret_check
            || TEST_int_eq(bio_ret, std_ret))) {
        TEST_note("Format: \"%s\"", data->format);
        return 0;
    }
