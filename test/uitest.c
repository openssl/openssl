...
static int test_old(void)
{
	...
	if (!TEST_ptr(ui_method = UI_UTIL_wrap_read_pem_callback(
			      test_pem_password_cb, 0)) ||
	...
	if (UI_add_input_string(ui, "prompt", UI_INPUT_FLAG_DEFAULT_PWD, pass,
				0, sizeof(pass) - 1) <= 0)
		goto err;
...
static int test_new_ui(void)
{
	...
	if (TEST_int_gt(password_callback(pass, sizeof(pass), 0, &cb_data),
			0) &&
