...
static EVP_PKEY *hss_pubkey_from_data(const unsigned char *data, size_t datalen)
{
    int ret;
    ...
    ret = TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(libctx, "HSS", NULL))
...
static int hss_bad_pub_len_test(void)
{
    ...
    for (publen = 0; publen <= td->publen + 16; publen += 3) {
        if (publen == td->publen)
            continue;
        if (!TEST_ptr_null(pkey = hss_pubkey_from_data(pubdata, publen)))
            goto end;
    }
    ret = 1;
end:
...
static int hss_key_eq_test(void)
{
    ...
    if (!TEST_ptr(key[0] = hss_pubkey_from_data(td1->pub, td1->publen))
            || !TEST_ptr(key[1] = hss_pubkey_from_data(td1->pub, td1->publen))
            || !TEST_ptr(key[2] = hss_pubkey_from_data(td2->pub, td2->publen)))
        goto end;
...
static int hss_key_validate_test(void)
{
    ...
    if (!TEST_ptr(key = hss_pubkey_from_data(td->pub, td->publen)))
        return 0;
