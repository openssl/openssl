...
static int check_single_test_params(char *name, char *testname, char *itname)
{
	...
	if (single_iter != -1) {
		if (all_tests[single_test - 1].num == -1) {
			test_printf_stderr(
				"-%s option is not valid for test %d:%s\n",
				itname, single_test,
