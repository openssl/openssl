		    ...
		    const char *module_name)
{
	...
	if (default_null_prov != NULL &&
		...
		opt_printf_stderr(
			"Failed to load null provider into default libctx\n");
		goto err;
	}
		    ...
		    OSSL_PROVIDER **provider, int argn, const char *usage)
{
	...
	return test_get_libctx(libctx, default_null_prov,
			       test_get_argument(argn + 1), provider,
			       module_name);
}
...
static int fips_provider_version(OSSL_LIB_CTX *libctx, FIPS_VERSION *vers)
{
	...
	if (!OSSL_PROVIDER_available(libctx, "fips"))
		return 0;
	*params =
		OSSL_PARAM_construct_utf8_ptr(OSSL_PROV_PARAM_VERSION, &vs, 0);
			     ...
			     int patch)
{
	...
	if ((res = fips_provider_version(libctx, &prov)) <= 0)
		return res == 0;
	return major == prov.major && minor == prov.minor &&
	       patch == prov.patch;
}
...
int fips_provider_version_match(OSSL_LIB_CTX *libctx, const char *versions)
{
	...
	while (*versions != '\0') {
		...
		switch (mode) {
		case MODE_EQ:
			r = fips_provider_version_eq(libctx, major, minor,
						     patch);
			break;
		case MODE_NE:
			r = fips_provider_version_ne(libctx, major, minor,
						     patch);
			break;
		case MODE_LE:
			r = fips_provider_version_le(libctx, major, minor,
						     patch);
			break;
		case MODE_LT:
			r = fips_provider_version_lt(libctx, major, minor,
						     patch);
			break;
		case MODE_GT:
			r = fips_provider_version_gt(libctx, major, minor,
						     patch);
			break;
		case MODE_GE:
			r = fips_provider_version_ge(libctx, major, minor,
						     patch);
			break;
		}
		if (r < 0) {
			TEST_info(
				"Error matching FIPS version: internal error\n");
			return -1;
		}
		if (r == 0)
			return 0;
