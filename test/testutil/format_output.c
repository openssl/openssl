 ...
 */
void test_fail_string_message(const char *prefix, const char *file, int line,
			      const char *type, const char *left,
			      const char *right, const char *op, const char *m1,
			      size_t l1, const char *m2, size_t l2)
{
	test_fail_string_common(prefix, file, line, type, left, right, op, m1,
				l1, m2, l2);
	test_printf_stderr("\n");
}
...
{
	test_fail_string_common("string", NULL, 0, NULL, NULL, NULL, name, m, l,
				m, l);
}
 ...
 */
static const int bn_bytes =
	(MAX_STRING_WIDTH - 9) / (BN_OUTPUT_SIZE * 2 + 1) * BN_OUTPUT_SIZE;
 ...
 */
static int convert_bn_memory(const unsigned char *in, size_t bytes, char *out,
			     int *lz, const BIGNUM *bn)
{
	int n = (int)(bytes * 2), i;
	char *p = out, *q = NULL;
	const char *r;
	...
	for (i = 0; i < n; i++) {
		*p++ = ' ';
		if (i % (2 * BN_OUTPUT_SIZE) == 2 * BN_OUTPUT_SIZE - 1 &&
		    i != n - 1)
			*p++ = ' ';
	}
	*p = '\0';
...
static void test_fail_bignum_common(const char *prefix, const char *file,
				    int line, const char *type,
				    const char *left, const char *right,
				    const char *op, const BIGNUM *bn1,
				    const BIGNUM *bn2)
{
	...
	test_fail_message_prefix(prefix, file, line, type, left, right, op);
	l1 = bn1 == NULL ? 0 :
			   (BN_num_bytes(bn1) + (BN_is_negative(bn1) ? 1 : 0));
	l2 = bn2 == NULL ? 0 :
			   (BN_num_bytes(bn2) + (BN_is_negative(bn2) ? 1 : 0));
	if (l1 == 0 && l2 == 0) {
	...
	if (len > MEM_BUFFER_SIZE &&
	    (bufp = OPENSSL_malloc_array(2, len)) == NULL) {
		bufp = buffer;
		len = MEM_BUFFER_SIZE;
		test_printf_stderr(
			"WARNING: these BIGNUMs have been truncated\n");
	}
 ...
 */
void test_fail_bignum_message(const char *prefix, const char *file, int line,
			      const char *type, const char *left,
...
void test_fail_bignum_mono_message(const char *prefix, const char *file,
				   int line, const char *type, const char *left,
				   const char *right, const char *op,
				   const BIGNUM *bn)
{
	test_fail_bignum_common(prefix, file, line, type, left, right, op, bn,
				bn);
	test_printf_stderr("\n");
}
...
void test_output_bignum(const char *name, const BIGNUM *bn)
{
	if (bn == NULL || BN_is_zero(bn)) {
		...
		test_fail_bignum_common("bignum", NULL, 0, NULL, NULL, NULL,
					name, bn, bn);
	}
}
 ...
 */
void test_fail_memory_message(const char *prefix, const char *file, int line,
			      const char *type, const char *left,
			      const char *right, const char *op,
			      const unsigned char *m1, size_t l1,
			      const unsigned char *m2, size_t l2)
{
	test_fail_memory_common(prefix, file, line, type, left, right, op, m1,
				l1, m2, l2);
	test_printf_stderr("\n");
}
...
{
	test_fail_memory_common("memory", NULL, 0, NULL, NULL, NULL, name, m, l,
				m, l);
}
