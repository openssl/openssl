...
{
	if (!TEST_int_eq(ossl_quic_wire_encode_frame_ack(pkt, 3,
							 &encode_case_3_f),
			 1))
		return 0;
...
static int encode_case_3_dec(PACKET *pkt, ossl_ssize_t fail)
{
	...
	if (!TEST_int_eq(ossl_quic_wire_decode_frame_ack(pkt, 3, &f,
							 &total_ranges),
			 fail < 0))
		return 0;
...
{
	if (!TEST_int_eq(ossl_quic_wire_encode_frame_reset_stream(
				 pkt, &encode_case_4_f),
			 1))
		return 0;
...
{
	if (!TEST_int_eq(ossl_quic_wire_encode_frame_stop_sending(
				 pkt, &encode_case_5_f),
			 1))
		return 0;
...
{
	if (!TEST_ptr(
		    ossl_quic_wire_encode_frame_crypto(pkt, &encode_case_6_f)))
		return 0;
...
static int encode_case_6_dec(PACKET *pkt, ossl_ssize_t fail)
{
	...
	if (!TEST_int_eq(ossl_quic_wire_decode_frame_crypto(pkt, 0, &f),
			 fail < 0))
		return 0;
...
static int encode_case_7_dec(PACKET *pkt, ossl_ssize_t fail)
{
	...
	if (!TEST_int_eq(ossl_quic_wire_decode_frame_new_token(pkt, &token,
							       &token_len),
			 fail < 0))
		return 0;
...
{
	if (!TEST_ptr(
		    ossl_quic_wire_encode_frame_stream(pkt, &encode_case_8_f)))
		return 0;
...
static int encode_case_8_dec(PACKET *pkt, ossl_ssize_t fail)
{
	...
	if (!TEST_int_eq(ossl_quic_wire_decode_frame_stream(pkt, 0, &f),
			 fail < 0))
		return 0;
...
{
	if (!TEST_ptr(
		    ossl_quic_wire_encode_frame_stream(pkt, &encode_case_9_f)))
		return 0;
...
{
	if (!TEST_int_eq(ossl_quic_wire_encode_frame_max_stream_data(
				 pkt, 0x1234, 0x9781),
			 1))
		return 0;
...
{
	if (!TEST_int_eq(
		    ossl_quic_wire_encode_frame_max_streams(pkt, 0, 0x1234), 1))
		return 0;

	if (!TEST_int_eq(
		    ossl_quic_wire_encode_frame_max_streams(pkt, 1, 0x9781), 1))
		return 0;
...
{
	if (!TEST_int_eq(ossl_quic_wire_encode_frame_data_blocked(pkt, 0x1234),
			 1))
		return 0;
...
static const unsigned char encode_case_16_conn_id[] = { 0x33, 0x44, 0x55,
							0x66 };
...
{
	if (!TEST_int_eq(ossl_quic_wire_encode_frame_new_conn_id(
				 pkt, &encode_case_16_f),
			 1))
		return 0;
...
static const unsigned char encode_case_16_expect[] = {
	...
	0xde, 0x06, 0xcb, 0x76,
	0x5d, 0xb1, 0xa7, 0x71, /* Stateless Reset Token */
	0x78, 0x09, 0xbb, 0xe8,
	0x50, 0x19, 0x12, 0x9a
};
...
{
	if (!TEST_int_eq(
		    ossl_quic_wire_encode_frame_retire_conn_id(pkt, 0x1234), 1))
		return 0;
...
static int encode_case_17_dec(PACKET *pkt, ossl_ssize_t fail)
{
	...
	if (!TEST_int_eq(ossl_quic_wire_decode_frame_retire_conn_id(pkt,
								    &seq_num),
			 fail < 0))
		return 0;
...
/* 18. PATH_CHALLENGE */
static const uint64_t encode_case_18_data = (((uint64_t)0x5f4b12) << 40) |
					    (uint64_t)0x731834UL;
...
/* 19. PATH_RESPONSE */
static const uint64_t encode_case_19_data = (((uint64_t)0x5f4b12) << 40) |
					    (uint64_t)0x731834UL;

static int encode_case_19_enc(WPACKET *pkt)
{
	if (!TEST_int_eq(ossl_quic_wire_encode_frame_path_response(
				 pkt, encode_case_19_data),
			 1))
		return 0;
...
static int encode_case_19_dec(PACKET *pkt, ossl_ssize_t fail)
{
	...
	if (!TEST_int_eq(ossl_quic_wire_decode_frame_path_response(pkt,
								   &challenge),
			 fail < 0))
		return 0;
...
{
	if (!TEST_int_eq(ossl_quic_wire_encode_frame_conn_close(
				 pkt, &encode_case_20_f),
			 1))
		return 0;
...
static int encode_case_22_enc(WPACKET *pkt)
{
	...
	if (!TEST_ptr(ossl_quic_wire_encode_transport_param_bytes(
		    pkt, 0x1234, encode_case_22_data,
...
static int encode_case_23_dec(PACKET *pkt, ossl_ssize_t fail)
{
	...
	if (!TEST_int_eq(ossl_quic_wire_decode_transport_param_int(pkt, &id,
								   &value),
			 fail < 0 || fail >= 7))
		return 0;
...
static const struct encode_test_case encode_cases[] = {
	ENCODE_CASE(1) ENCODE_CASE(2) ENCODE_CASE(3) ENCODE_CASE(4) ENCODE_CASE(
		5) ENCODE_CASE(6) ENCODE_CASE(7) ENCODE_CASE(8) ENCODE_CASE(9)
		ENCODE_CASE(10) ENCODE_CASE(11) ENCODE_CASE(12) ENCODE_CASE(13)
			ENCODE_CASE(14) ENCODE_CASE(15) ENCODE_CASE(16)
				ENCODE_CASE(16b) ENCODE_CASE(17) ENCODE_CASE(18)
					ENCODE_CASE(19) ENCODE_CASE(20)
						ENCODE_CASE(21) ENCODE_CASE(22)
							ENCODE_CASE(23)
};
...
static int test_wire_encode(int idx)
{
	...
	if (!TEST_int_eq(PACKET_buf_init(&pkt, (unsigned char *)buf->data,
					 written),
			 1))
		goto err;
	...
	for (i = 0; i < c->expect_buf_len; ++i) {
         ...
         */
		if (!TEST_int_eq(PACKET_buf_init(&pkt2,
						 (unsigned char *)c->expect_buf,
						 i),
				 1))
			goto err;
...
static int ack_generic_decode(PACKET *pkt)
{
	...
	if (!TEST_int_eq(ossl_quic_wire_peek_frame_ack_num_ranges(
	...
	for (i = 0; i < f.num_ack_ranges; ++i) {
		if (!TEST_uint64_t_le(f.ack_ranges[i].start,
				      f.ack_ranges[i].end))
			return 0;
...
static const struct ack_test_case ack_cases[] = {
	ACK_CASE(1, 1, ack_generic_decode) ACK_CASE(2, 0, ack_generic_decode)
		ACK_CASE(3, 1, ack_generic_decode) ACK_CASE(4, 0,
							    ack_generic_decode)
...
/* RFC 9001 s. A.4 */
static const QUIC_CONN_ID retry_orig_dcid = { 8,
...
static int test_wire_retry_integrity_tag(void)
{
	...
	if (!TEST_true(PACKET_buf_init(&pkt, retry_encoded,
