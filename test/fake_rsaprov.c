...
#define PROV_FAKE_RSA_LIBCTX_OF(provctx) \
	(((PROV_FAKE_RSA_CTX *)provctx)->libctx)
	...
	if (priv) {
		OSSL_PARAM params[] = {
			...
			OSSL_PARAM_BN(OSSL_PKEY_PARAM_RSA_FACTOR2, fake_rsa_q,
				      sizeof(fake_rsa_q) - 1),
			OSSL_PARAM_BN(OSSL_PKEY_PARAM_RSA_EXPONENT1,
				      fake_rsa_dmp1, sizeof(fake_rsa_dmp1) - 1),
			OSSL_PARAM_BN(OSSL_PKEY_PARAM_RSA_EXPONENT2,
				      fake_rsa_dmq1, sizeof(fake_rsa_dmq1) - 1),
			OSSL_PARAM_BN(OSSL_PKEY_PARAM_RSA_COEFFICIENT1,
				      fake_rsa_iqmp, sizeof(fake_rsa_iqmp) - 1),
			OSSL_PARAM_END
		};
...
static void *fake_rsa_keymgmt_load(const void *reference, size_t reference_sz)
{
	...
	if (key->status != FAKE_RSA_STATUS_IMPORTED &&
...
static int fake_rsa_dgstsgn_init(void *ctx, const char *mdname, void *provkey,
				 const OSSL_PARAM params[])
{
	return fake_rsa_dgstsgnvfy_init(ctx, FAKE_DGSTSGN_SIGN, provkey,
					params);
}
...
static int fake_rsa_dgstvfy_init(void *ctx, const char *mdname, void *provkey,
				 const OSSL_PARAM params[])
{
	return fake_rsa_dgstsgnvfy_init(ctx, FAKE_DGSTSGN_VERIFY, provkey,
					params);
}
...
static const OSSL_DISPATCH fake_rsa_sig_funcs[] = {
	...
	{ OSSL_FUNC_SIGNATURE_SIGN_INIT,
	  (void (*)(void))fake_rsa_sig_sign_init },
				 ...
				 void *pw_cbarg)
{
	...
	if (strncmp(uri, fake_rsa_openpwtest,
		...
		if (!pw_cb(fakepw, sizeof(fakepw), &fakepw_len, pw_params,
			   pw_cbarg)) {
			TEST_info(
				"fake_rsa_open_ex failed passphrase callback");
			return NULL;
		}
...
ASN1_SEQUENCE(
	X509_PUBKEY_INTERNAL) = { ASN1_SIMPLE(X509_PUBKEY, algor, X509_ALGOR),
				  ASN1_SIMPLE(
					...
					static X509_PUBKEY
				* fake_rsa_d2i_X509_PUBKEY_INTERNAL(
					...
					key_from_pkcs8_t *key_from_pkcs8)
{
	...
	if ((p8inf = d2i_PKCS8_PRIV_KEY_INFO(NULL, input_der, input_der_len)) !=
		...
		key = key_from_pkcs8(
			p8inf, PROV_FAKE_RSA_LIBCTX_OF(ctx->provctx), NULL);
...
static int fake_rsa_der2key_decode(void *vctx, OSSL_CORE_BIO *cin,
				 ...
				 void *export_cbarg)
{
	...
	OSSL_FUNC_keymgmt_export_fn *export =
		fake_rsa_prov_get_keymgmt_export(ctx->desc->fns);
	void *keydata;
...
static struct fake_rsa_keydata *
fake_rsa_key_from_pkcs8(const PKCS8_PRIV_KEY_INFO *p8inf, OSSL_LIB_CTX *libctx,
			const char *propq)
{
...
static void *fake_rsa_d2i_PKCS8(void **key, const unsigned char **der,
				long der_len, struct der2key_ctx_st *ctx)
{
	return fake_rsa_der2key_decode_p8(
		der, der_len, ctx, (key_from_pkcs8_t *)fake_rsa_key_from_pkcs8);
}
...
#define DO_PrivateKeyInfo(keytype)                                   \
	"PrivateKeyInfo", keytype##_evp_type,                        \
		(OSSL_KEYMGMT_SELECT_PRIVATE_KEY), NULL, NULL, NULL, \
		fake_rsa_d2i_PKCS8, NULL, NULL, fake_rsa_key_adjust, \
...
#define DO_SubjectPublicKeyInfo(keytype)                                       \
	"SubjectPublicKeyInfo", keytype##_evp_type,                            \
		...
		(d2i_of_void *)fake_rsa_d2i_PUBKEY, NULL, fake_rsa_key_adjust, \
		(free_key_fn *)fake_rsa_keymgmt_free
...
#define MAKE_DECODER(keytype_name, keytype, type, kind)                      \
	static struct keytype_desc_st kind##_##keytype##_desc = {            \
		keytype_name, fake_rsa_keymgmt_funcs, DO_##kind(keytype)     \
	};                                                                   \
                                                                             \
...
#define DECODER_w_structure(_name, _input, _structure, _output)   \
	...
	DECODER_w_structure("RSA:rsaEncryption", der, SubjectPublicKeyInfo,
			    rsa),
