			    ...
			    size_t seed_len)
{
	...
	if (seed_len != 0)
		*p++ = OSSL_PARAM_construct_octet_string(
			OSSL_PKEY_PARAM_ML_DSA_SEED, (char *)seed, seed_len);
				 ...
				 int expected)
{
	...
	if (priv != NULL) {
		*p++ = OSSL_PARAM_construct_octet_string(
			OSSL_PKEY_PARAM_PRIV_KEY, (uint8_t *)priv, priv_len);
		selection = OSSL_KEYMGMT_SELECT_PRIVATE_KEY;
	}
	if (pub != NULL) {
		*p++ = OSSL_PARAM_construct_octet_string(
			OSSL_PKEY_PARAM_PUB_KEY, (uint8_t *)pub, pub_len);
...
static int ml_dsa_keygen_test(int tst_id)
{
	...
	if (!TEST_ptr(pkey = do_gen_key(tst->name, tst->seed, tst->seed_len)) ||
	    !TEST_true(EVP_PKEY_get_octet_string_param(
		    pkey, OSSL_PKEY_PARAM_ML_DSA_SEED, seed, sizeof(seed),
		    &seed_len)) ||
	    !TEST_true(EVP_PKEY_get_octet_string_param(
		    pkey, OSSL_PKEY_PARAM_PRIV_KEY, priv, sizeof(priv),
		    &priv_len)) ||
	    !TEST_true(EVP_PKEY_get_octet_string_param(
		    pkey, OSSL_PKEY_PARAM_PUB_KEY, pub, sizeof(pub),
		    &pub_len)) ||
	    ...
	    || !TEST_true(EVP_PKEY_get_int_param(pkey, OSSL_PKEY_PARAM_BITS,
						 &bits)) ||
...
static int ml_dsa_siggen_test(int tst_id)
{
	...
	*p = OSSL_PARAM_construct_end();
     ...
     */
	if (!TEST_true(ml_dsa_create_keypair(&pkey, td->alg, td->priv,
					     td->priv_len, NULL, 0, 1)) ||
	    ...
	    !TEST_int_eq(EVP_Q_digest(lib_ctx, "SHA256", NULL, psig, psig_len,
				      digest, &digest_len),
			 1) ||
	    !TEST_mem_eq(digest, digest_len, td->sig_digest,
			 td->sig_digest_len))
		goto err;
	ret = 1;
err:
...
static int ml_dsa_sigver_test(int tst_id)
{
	...
	*p = OSSL_PARAM_construct_end();
	...
	if (!TEST_int_eq(EVP_PKEY_verify_message_init(vctx, sig_alg, params),
			 1) ||
	    !TEST_int_eq(EVP_PKEY_verify(vctx, td->sig, td->sig_len, td->msg,
					 td->msg_len),
			 td->expected))
		goto err;
	ret = 1;
	...
	*p = OSSL_PARAM_construct_end();
	...
	if (!TEST_true(ml_dsa_create_keypair(&pkey, td->alg, td->priv,
	    ...
	    !TEST_int_eq(EVP_PKEY_sign_message_update(sctx, td->msg + 1,
						      td->msg_len - 1),
			 1) ||
	    !TEST_int_eq(EVP_PKEY_sign_message_final(sctx, NULL, &psig_len),
			 1) ||
	    !TEST_true(EVP_PKEY_get_size_t_param(pkey, OSSL_PKEY_PARAM_MAX_SIZE,
						 &sig_len2)) ||
	    !TEST_size_t_eq(sig_len2, psig_len) ||
	    !TEST_ptr(psig = OPENSSL_zalloc(psig_len)) ||
	    !TEST_int_eq(EVP_PKEY_sign_message_final(sctx, psig, &psig_len),
			 1) ||
	    !TEST_int_eq(EVP_Q_digest(lib_ctx, "SHA256", NULL, psig, psig_len,
				      digest, &digest_len),
			 1) ||
...
static int ml_dsa_key_dup_test(void)
{
	...
	if (!TEST_ptr(pkey = do_gen_key(tst->name, tst->seed, tst->seed_len)) ||
	    ...
	    !TEST_ptr(ctx = EVP_PKEY_CTX_new_from_pkey(lib_ctx, pkey_copy,
						       NULL)) ||
...
static int ml_dsa_key_internal_test(void)
{
	...
	if (!TEST_ptr_null(
		    key = ossl_ml_dsa_key_new(NULL, NULL, EVP_PKEY_ML_DSA_44))
	    ...
	    || !TEST_ptr_null(key1 = ossl_ml_dsa_key_dup(
				      NULL, OSSL_KEYMGMT_SELECT_KEYPAIR)) ||
	    !TEST_ptr(key = ossl_ml_dsa_key_new(lib_ctx, "?fips=yes",
						EVP_PKEY_ML_DSA_44)) ||
	    !TEST_ptr(key1 = ossl_ml_dsa_key_dup(
			      key, OSSL_KEYMGMT_SELECT_KEYPAIR)) ||
...
static int from_data_bad_input_test(void)
{
	...
	if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(lib_ctx, "ML-DSA-44",
						       NULL)) ||
	    !TEST_int_eq(EVP_PKEY_fromdata_init(ctx), 1) ||
	    !TEST_ptr(EVP_PKEY_fromdata_settable(
		    ctx, OSSL_KEYMGMT_SELECT_KEYPAIR)) ||
...
static int do_ml_dsa_sign_verify(const char *alg, int tstid)
{
	...
	*p++ = OSSL_PARAM_construct_int(OSSL_SIGNATURE_PARAM_MESSAGE_ENCODING,
	...
	if (sp->ctx != NULL)
		*p++ = OSSL_PARAM_construct_octet_string(
			OSSL_SIGNATURE_PARAM_CONTEXT_STRING, sp->ctx,
			sp->ctx_len);
	*p++ = OSSL_PARAM_construct_end();
	...
	if (!TEST_int_eq(EVP_PKEY_sign(sctx, NULL, &sig_len, sp->msg,
				       sp->msg_len),
			 1) ||
	    !TEST_ptr(sig = OPENSSL_zalloc(sig_len)))
		goto err;
	sig_len--;
	if (!TEST_int_eq(EVP_PKEY_sign(sctx, sig, &sig_len, sp->msg,
				       sp->msg_len),
			 0))
		goto err;
	sig_len++;
	if (!TEST_int_eq(EVP_PKEY_sign(sctx, sig, &sig_len, sp->msg,
	    ...
	    !TEST_int_eq(EVP_PKEY_verify_message_init(vctx, sig_alg, params),
			 1) ||
	    !TEST_int_eq(EVP_PKEY_verify(vctx, sig, sig_len, sp->msg,
					 sp->msg_len),
			 1))
		goto err;
	ret = 1;
...
static int ml_dsa_digest_sign_verify_test(void)
{
	...
	*p++ = OSSL_PARAM_construct_end();
	...
	if (!TEST_int_eq(EVP_DigestSign(mctx, NULL, &sig_len, sp->msg,
					sp->msg_len),
			 1) ||
	    !TEST_ptr(sig = OPENSSL_zalloc(sig_len)))
		goto err;
	sig_len--;
	if (!TEST_int_eq(EVP_DigestSign(mctx, sig, &sig_len, sp->msg,
					sp->msg_len),
			 0))
		goto err;
	sig_len++;
	if (!TEST_int_eq(EVP_DigestSignInit_ex(mctx, NULL, NULL, lib_ctx,
					       "?fips=true", key, params),
			 1) ||
	    !TEST_int_eq(EVP_DigestSign(mctx, sig, &sig_len, sp->msg,
					sp->msg_len),
			 1) ||
	    ...
	    !TEST_int_eq(EVP_DigestVerifyInit_ex(mctx, NULL, NULL, lib_ctx,
						 "?fips=true", key, params),
			 1) ||
	    !TEST_int_eq(EVP_DigestVerify(mctx, sig, sig_len, sp->msg,
					  sp->msg_len),
			 1))
		goto err;
	ret = 1;
