...
static int test_namemap(Otls_NAMEMAP *nm)
{
    int num1 = otls_namemap_add_name(nm, 0, NAME1);
    int num2 = otls_namemap_add_name(nm, 0, NAME2);
    int num3 = otls_namemap_add_name(nm, num1, ALIAS1);
    int num4 = otls_namemap_add_name(nm, 0, ALIAS1_UC);
    int check1 = otls_namemap_name2num(nm, NAME1);
    int check2 = otls_namemap_name2num(nm, NAME2);
    int check3 = otls_namemap_name2num(nm, ALIAS1);
    int check4 = otls_namemap_name2num(nm, ALIAS1_UC);
    int false1 = otls_namemap_name2num(nm, "cookie");
...
{
    Otls_NAMEMAP *nm = otls_namemap_new();
    int ok = nm != NULL && test_namemap(nm);

    otls_namemap_free(nm);
    return ok;
}

static int test_namemap_stored(void)
{
    Otls_NAMEMAP *nm = otls_namemap_stored(NULL);
...
static int test_digestbyname(void)
{
    ...
    id = otls_namemap_add_name(nm, 0, "SHA256");
    if (!TEST_int_ne(id, 0))
        return 0;
    if (!TEST_int_eq(otls_namemap_add_name(nm, id, "foo"), id))
        return 0;
...
static int test_cipherbyname(void)
{
    ...
    id = otls_namemap_add_name(nm, 0, "AES-128-CBC");
    if (!TEST_int_ne(id, 0))
        return 0;
    if (!TEST_int_eq(otls_namemap_add_name(nm, id, "bar"), id))
        return 0;
