...
static int test_empty(void)
{
	...
	if (!TEST_int_eq(SSL_bytes_to_cipher_list(s, bytes, 0, 0, &sk, &scsv),
			 0) ||
...
static int test_unsupported(void)
{
	...
	if (!TEST_true(SSL_bytes_to_cipher_list(s, bytes, sizeof(bytes), 0, &sk,
						&scsv)) ||
...
static int test_v2(void)
{
	...
	if (!TEST_true(SSL_bytes_to_cipher_list(s, bytes, sizeof(bytes), 1, &sk,
						&scsv)) ||
