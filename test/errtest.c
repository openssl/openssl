...
static int test_print_error_format(void)
{
	...
	BIO_snprintf(expected, sizeof(expected), expected_format, errorcode,
...
static int raised_error(void)
{
	...
	file = __FILE__;
	line = __LINE__ +
	       2; /* The error is generated on the ERR_raise_data line */
#endif
	ERR_raise_data(ERR_LIB_NONE, ERR_R_INTERNAL_ERROR, "calling exit()");
	if (!TEST_ulong_ne(e = ERR_get_error_all(&f, &l, NULL, &data, NULL),
			   0) ||
...
static int test_marks(void)
{
	...
	if (!TEST_true(ERR_set_mark()) || !TEST_true(ERR_pop_to_mark()) ||
	    ...
	    !TEST_true(ERR_set_mark()) || !TEST_true(ERR_clear_last_mark()) ||
	    !TEST_ulong_eq(mallocfail, ERR_peek_last_error()))
		return 0;
	...
	ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
	if (!TEST_ulong_ne(mallocfail, ERR_peek_last_error()) ||
	    !TEST_true(ERR_pop_to_mark()) || !TEST_true(ERR_pop_to_mark()) ||
	    !TEST_ulong_eq(mallocfail, ERR_peek_last_error()))
		return 0;
	...
	for (i = 0; i < 2; i++) {
		...
		if (idx == 0 || i == 0) {
			if (!TEST_ulong_eq(ERR_get_error_all(NULL, NULL, NULL,
					   ...
					   mallocfail) ||
			    !TEST_int_ne(flags,
					 ERR_TXT_STRING | ERR_TXT_MALLOCED))
				goto err;
		}

		if (!TEST_ulong_eq(ERR_get_error_all(NULL, NULL, NULL, &data,
						     &flags),
				   interr) ||
