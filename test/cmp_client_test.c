...
static int execute_exec_RR_ses_test(CMP_SES_TEST_FIXTURE *fixt)
{
	return TEST_int_eq(OSSL_CMP_CTX_get_status(fixt->cmp_ctx),
	       ...
	       TEST_int_eq(OSSL_CMP_CTX_get_status(fixt->cmp_ctx),
			   fixt->expected);
}
...
static int execute_exec_certrequest_ses_test(CMP_SES_TEST_FIXTURE *fixture)
{
	...
	if (fixture->caPubs != NULL) {
		...
		int ret = TEST_int_eq(
			STACK_OF_X509_cmp(fixture->caPubs, caPubs), 0);
...
static int test_exec_REQ_ses_poll(int req_type, int check_after, int poll_count,
				  int total_timeout, int expect)
{
	...
	OSSL_CMP_CTX_set_option(fixture->cmp_ctx, OSSL_CMP_OPT_TOTAL_TIMEOUT,
				total_timeout);
...
{
	return test_exec_REQ_ses_poll(
		OSSL_CMP_PKIBODY_IR, checkAfter, 2 /* pollCount */,
...
static int test_exec_CR_ses(int implicit_confirm, int granted, int reject)
{
	...
	OSSL_CMP_CTX_set_option(fixture->cmp_ctx, OSSL_CMP_OPT_IMPLICIT_CONFIRM,
				implicit_confirm);
	OSSL_CMP_SRV_CTX_set_grant_implicit_confirm(fixture->srv_ctx, granted);
	ossl_cmp_mock_srv_set_sendError(
		fixture->srv_ctx, reject ? OSSL_CMP_PKIBODY_CERTCONF : -1);
...
static int test_exec_KUR_ses(int transfer_error, int pubkey, int raverified)
{
	...
	if (pubkey) {
		EVP_PKEY *key = raverified /* wrong key */ ? server_key :
							     client_key;

		if (!EVP_PKEY_up_ref(key))
			return 0;

		OSSL_CMP_CTX_set0_newPkey(fixture->cmp_ctx, 0 /* not priv */,
					  key);
...
static int execute_try_certreq_poll_test(CMP_SES_TEST_FIXTURE *fixture)
{
	...
	ossl_cmp_mock_srv_set_checkAfterTime(fixture->srv_ctx, CHECK_AFTER);
	return TEST_int_eq(-1, OSSL_CMP_try_certreq(ctx, TYPE, NULL,
						    &check_after)) &&
	       check_after == CHECK_AFTER &&
	       TEST_ptr_eq(OSSL_CMP_CTX_get0_newCert(ctx), NULL) &&
	       TEST_int_eq(-1, OSSL_CMP_try_certreq(ctx, TYPE, NULL,
						    &check_after)) &&
	       check_after == CHECK_AFTER &&
	       TEST_ptr_eq(OSSL_CMP_CTX_get0_newCert(ctx), NULL) &&
	       TEST_int_eq(fixture->expected,
			   OSSL_CMP_try_certreq(ctx, TYPE, NULL, NULL)) &&
	       TEST_int_eq(0, X509_cmp(OSSL_CMP_CTX_get0_newCert(ctx),
				       client_cert));
}
...
static int execute_try_certreq_poll_abort_test(CMP_SES_TEST_FIXTURE *fixture)
{
	...
	return TEST_int_eq(-1, OSSL_CMP_try_certreq(ctx, TYPE, NULL,
	       ...
	       TEST_ptr_eq(OSSL_CMP_CTX_get0_newCert(ctx), NULL) &&
	       TEST_int_eq(fixture->expected,
			   OSSL_CMP_try_certreq(ctx, -1 /* abort */, NULL,
						NULL)) &&
...
{
	int res = ossl_cmp_exchange_certConf(
		fixture->cmp_ctx, OSSL_CMP_CERTREQID,
		OSSL_CMP_PKIFAILUREINFO_addInfoNotAvailable, "abcdefg");
...
{
	int res = ossl_cmp_exchange_error(
		fixture->cmp_ctx, OSSL_CMP_PKISTATUS_rejection,
		1 << OSSL_CMP_PKIFAILUREINFO_unsupportedVersion, "foo_status",
		999, "foo_details");
...
int setup_tests(void)
{
	...
	if (!test_arg_libctx(&libctx, &default_null_provider, &provider, 5,
			     USAGE))
		return 0;
