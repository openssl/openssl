...
#if !defined(OPENSSL_NO_SSL_TRACE) && defined(OPENSSL_NO_BROTLI) && \
	defined(OPENSSL_NO_ZSTD) && !defined(OPENSSL_NO_ECX) &&     \
	!defined(OPENSSL_NO_DH) && !defined(OPENSSL_NO_ML_DSA) &&   \
...
static int test_quic_write_read(int idx)
{
	...
	for (k = 0; k < 2; k++) {
		if (!TEST_ptr(cctx) ||
		    !TEST_true(qtest_create_quic_objects(
			    libctx, cctx, sctx, cert, privkey,
			    idx >= 1 ? QTEST_FLAG_BLOCK : 0, &qtserv,
			    &clientquic, NULL, NULL)) ||
		    !TEST_true(
			    SSL_set_tlsext_host_name(clientquic, "localhost")))
			goto end;

		if (sess != NULL &&
		    !TEST_true(SSL_set_session(clientquic, sess)))
			goto end;
		...
		for (j = 0; j < 2; j++) {
			...
			if (!TEST_true(SSL_write_ex(clientquic, msg, msglen,
						    &numbytes)) ||
			    !TEST_size_t_eq(numbytes, msglen))
				goto end;
			if (idx >= 1) {
				do {
					if (!TEST_true(wait_until_sock_readable(
						    ssock)))
						goto end;

					ossl_quic_tserver_tick(qtserv);

					if (!TEST_true(ossl_quic_tserver_read(
						    qtserv, sid, buf,
						    sizeof(buf), &numbytes)))
						goto end;
				} while (numbytes == 0);
			...
			if (!TEST_true(ossl_quic_tserver_write(
				    qtserv, sid, (unsigned char *)msg, msglen,
				    &numbytes)))
				goto end;
			ossl_quic_tserver_tick(qtserv);
			...
			if (idx >= 2 && j > 0) {
				if (!TEST_false(SSL_read_ex(clientquic, buf, 1,
							    &numbytes)) ||
				    !TEST_int_eq(SSL_get_error(clientquic, 0),
						 SSL_ERROR_SYSCALL) ||
				    !TEST_false(SSL_write_ex(clientquic, msg,
							     msglen,
							     &numbytes)) ||
				    !TEST_int_eq(SSL_get_error(clientquic, 0),
						 SSL_ERROR_SYSCALL))
            ...
            */
			if (!TEST_true(SSL_read_ex(clientquic, buf, 1,
						   &numbytes)) ||
			    !TEST_size_t_eq(numbytes, 1) ||
			    !TEST_true(SSL_has_pending(clientquic)) ||
			    !TEST_int_eq(SSL_pending(clientquic),
					 (int)(msglen - 1)) ||
		...
		if (!TEST_true(SSL_export_keying_material(
			    clientquic, scratch, sizeof(scratch), "test", 4,
			    (unsigned char *)"ctx", 3, 1)))
			goto end;
...
static int test_fin_only_blocking(void)
{
	...
	if (!TEST_ptr(cctx) ||
	    !TEST_true(qtest_create_quic_objects(
		    libctx, cctx, sctx, cert, privkey, QTEST_FLAG_BLOCK,
		    &qtserv, &clientquic, NULL, NULL)) ||
...
static int test_ciphersuites(void)
{
	...
	int cipherids[] = { TLS1_3_CK_AES_256_GCM_SHA384,
...
static int test_cipher_find(void)
{
	...
	struct {
		...
		int ok;
	} testciphers[] = { { TLS13_AES_128_GCM_SHA256_BYTES, 1 },
	...
	for (i = 0; i < OSSL_NELEM(testciphers); i++)
		if (testciphers[i].ok) {
			if (!TEST_ptr(SSL_CIPHER_find(
				    clientquic, testciphers[i].cipherbytes)))
				goto err;
		} else {
...
static int compare_with_file(BIO *membio)
{
	...
	if (!TEST_true(BIO_eof(file)) || !TEST_true(BIO_eof(membio)))
		goto err;
...
static int test_ssl_trace(void)
{
	...
	if (!TEST_ptr(cctx = SSL_CTX_new_ex(libctx, NULL,
	    ...
	    !TEST_true(
		    SSL_CTX_set_ciphersuites(cctx, "TLS_AES_128_GCM_SHA256")) ||
	    !TEST_true(qtest_create_quic_objects(
		    libctx, cctx, NULL, cert, privkey, QTEST_FLAG_FAKE_TIME,
		    &qtserv, &clientquic, NULL, NULL)))
		goto err;
...
static int find_new_token_data(BIO *membio)
{
	...
	while (state != SUCCESS && state != FAILED &&
		...
		case GATHER_TOKEN:
			...
			if (tmpstring == NULL) {
				TEST_info(
					"Next line did not contain a new token\n");
				state = FAILED;
			} else {
		...
		case CHECK_TOKEN:
			tmpstring = strstr(buf, "Token: ");
			if (tmpstring != NULL && !strcmp(tmpstring, tokenval)) {
				state = SUCCESS;
...
static int test_new_token(void)
{
	...
	if (!TEST_true(qtest_create_quic_objects(
		    libctx, cctx, NULL, cert, privkey, QTEST_FLAG_FAKE_TIME,
		    &qtserv2, &clientquic2, NULL, NULL)))
		goto err;
...
static int test_quic_forbidden_apis_ctx(void)
{
	...
	if (!TEST_true(
		    SSL_CTX_set_tlsext_use_srtp(ctx, "SRTP_AEAD_AES_128_GCM")))
		goto err;
#endif
...
#define NON_QUIC_CIPHERSUITES       \
	...
	if (!TEST_true(SSL_CTX_set_ciphersuites(ctx, QUIC_CIPHERSUITES
...
static int test_quic_forbidden_apis(void)
{
	...
	/* Set TLSv1.3 ciphersuite list for the SSL_CTX. */
	if (!TEST_true(SSL_set_ciphersuites(ssl, QUIC_CIPHERSUITES
					    ":" NON_QUIC_CIPHERSUITES)))
		goto err;
...
static int test_quic_forbidden_options(void)
{
	...
	if (!TEST_uint64_t_eq(SSL_get_options(ssl),
	...
	if (!TEST_true(SSL_set_tlsext_max_fragment_length(
		    ssl, TLSEXT_max_fragment_length_DISABLED)) ||
	    !TEST_false(SSL_set_tlsext_max_fragment_length(
		    ssl, TLSEXT_max_fragment_length_512)))
		goto err;
...
static int test_quic_set_fd(int idx)
{
	...
	if (!TEST_int_ge(fd = BIO_socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP, 0),
			 0))
		goto err;
	...
	if (!TEST_true(BIO_get_fd(bio, &resfd)) || !TEST_int_eq(resfd, fd))
		goto err;
...
static int test_bio_ssl(void)
{
	...
	if (!TEST_true(qtest_create_quic_objects(
		    libctx, NULL, NULL, cert, privkey, QTEST_FLAG_FAKE_TIME,
		    &qtserv, &clientquic, NULL, NULL)))
		goto err;
	...
	do {
		...
		if (!servererr && rets <= 0) {
			...
			if (!servererr)
				rets = ossl_quic_tserver_is_handshake_confirmed(
					qtserv);
		}
	...
	for (i = 0, thisbio = cbio; i < 2; i++) {
		if (!TEST_true(ossl_quic_tserver_read(
			    qtserv, sid, buf, sizeof(buf), &readbytes)) ||
		...
		if (!TEST_true(BIO_read_ex(thisbio, buf, sizeof(buf),
					   &readbytes)) ||
		    !TEST_mem_eq(msg, msglen, buf, readbytes))
			goto err;
...
static int test_back_pressure(void)
{
	...
	for (i = 0; i < BACK_PRESSURE_NUM_LOOPS; i++) {
		...
		/* Receive data at the server */
		ossl_quic_tserver_tick(qtserv);
		if (!TEST_true(ossl_quic_tserver_read(qtserv, 0, buf,
...
static int test_non_io_retry(int idx)
{
	...
	SSL_CTX_set_cert_verify_callback(cctx, non_io_retry_cert_verify_cb,
					 &allow);

	flags = (idx >= 1) ? QTEST_FLAG_BLOCK : 0;
	if (!TEST_true(qtest_create_quic_objects(libctx, cctx, NULL, cert,
						 privkey, flags, &qtserv,
						 &clientquic, NULL, NULL)) ||
	    !TEST_true(qtest_create_quic_connection_ex(
		    qtserv, clientquic, SSL_ERROR_WANT_RETRY_VERIFY)) ||
...
static int test_client_auth(int idx)
{
	...
	if (idx > 1) {
		if (!TEST_true(ssl_ctx_add_large_cert_chain(libctx, cctx,
							    ccert)) ||
		    !TEST_true(
			    ssl_ctx_add_large_cert_chain(libctx, sctx, cert)))
			goto err;
	}
	...
	ossl_quic_tserver_tick(qtserv);
	if (!TEST_true(ossl_quic_tserver_write(qtserv, 0, (unsigned char *)msg,
					       msglen, &numbytes)))
		goto err;
...
static int test_alpn(int idx)
{
	...
	if (idx == 0) {
		...
		if (!TEST_int_eq(SSL_get_error(clientquic, ret),
				 SSL_ERROR_WANT_READ) ||
		    !TEST_true(
			    qtest_create_quic_connection(qtserv, clientquic)))
			goto err;
	...
	for (abortctr = 0; abortctr < MAX_LOOPS; abortctr++) {
		...
		if (*stream != NULL) {
			if (SSL_read_ex(*stream, buf, buflen, readbytes))
				return 1;
			if (!TEST_int_eq(SSL_get_error(*stream, 0),
					 SSL_ERROR_WANT_READ))
				return 0;
		}
		ossl_quic_tserver_tick(qtserv);
...
static int unreliable_server_read(QUIC_TSERVER *qtserv, uint64_t sid, void *buf,
				  size_t buflen, size_t *readbytes,
				  SSL *clientquic)
{
	int abortctr;
	...
	/* We just do this in a loop with a sleep for simplicity */
	for (abortctr = 0; abortctr < MAX_LOOPS; abortctr++) {
		if (ossl_quic_tserver_read(qtserv, sid, buf, buflen,
					   readbytes) &&
		    *readbytes > 1)
			return 1;
		ossl_quic_tserver_tick(qtserv);
...
static int test_noisy_dgram(int idx)
{
	...
	if (!TEST_ptr(cctx) || !TEST_true(qtest_create_quic_objects(
				       libctx, cctx, NULL, cert, privkey, flags,
				       &qtserv, &clientquic, &fault, NULL)))
		goto err;

	if (!TEST_true(qtest_create_quic_connection(qtserv, clientquic)))
		goto err;

	if (!TEST_true(SSL_set_incoming_stream_policy(
		    clientquic, SSL_INCOMING_STREAM_POLICY_ACCEPT, 0)) ||
	    !TEST_true(SSL_set_default_stream_mode(
		    clientquic, SSL_DEFAULT_STREAM_MODE_NONE)))
		goto err;
	...
	for (j = 0; j < 2; j++) {
		...
		for (i = 0; i < 20; i++) {
             ...
             * handle that
             */
			if (!TEST_true(unreliable_client_read(
				    clientquic, &stream[j], buf, sizeof(buf),
				    &readbytes, qtserv)) ||
			    !TEST_mem_eq(msg, msglen, buf, readbytes))
				goto err;
		}
		...
		for (i = 0; i < 20; i++) {
			if (!TEST_true(SSL_write_ex(stream[j],
						    (unsigned char *)msg,
						    msglen, &written)) ||
			    !TEST_size_t_eq(msglen, written))
				goto err;
             ...
             * handle that
             */
			if (!TEST_true(unreliable_server_read(
				    qtserv, sid, buf, sizeof(buf), &readbytes,
				    clientquic)) ||
			    !TEST_mem_eq(msg, msglen, buf, readbytes))
				goto err;
	...
	while (recvlen > 0) {
		...
		if (sendlen > 0) {
			if (!SSL_write_ex(clientquic, msg,
					  sendlen > TEST_SINGLE_WRITE_SIZE ?
						  TEST_SINGLE_WRITE_SIZE :
						  sendlen,
					  &written)) {
				TEST_info("Retrying to send: %llu",
					  (unsigned long long)sendlen);
				if (!TEST_int_eq(SSL_get_error(clientquic, 0),
						 SSL_ERROR_WANT_WRITE))
					goto err;
			} else {
				sendlen -= written;
				TEST_info("Remaining to send: %llu",
					  (unsigned long long)sendlen);
			}
		} else {
			SSL_handle_events(clientquic);
		...
		if (ossl_quic_tserver_read(qtserv, 0, recvbuf,
					   recvlen > TEST_SINGLE_WRITE_SIZE ?
						   TEST_SINGLE_WRITE_SIZE :
						   recvlen,
					   &readbytes) &&
		    readbytes > 1) {
			recvlen -= readbytes;
			TEST_info("Remaining to recv: %llu",
				  (unsigned long long)recvlen);
		} else {
			TEST_info("No progress on recv: %llu",
				  (unsigned long long)recvlen);
		}
		ossl_quic_tserver_tick(qtserv);
	}
...
#define TPARAM_CHECK_INJECT(name, buf, buf_len, reason) \
	{ QUIC_TPARAM_##name, TPARAM_OP_INJECT, (reason), (buf), (buf_len) },
...
#define TPARAM_CHECK_DROP_INJECT(name, buf, buf_len, reason)          \
	{ QUIC_TPARAM_##name, TPARAM_OP_DROP_INJECT, (reason), (buf), \
	  (buf_len) },
...
#define TPARAM_CHECK_INJECT_TWICE(name, buf, buf_len, reason)          \
	{ QUIC_TPARAM_##name, TPARAM_OP_INJECT_TWICE, (reason), (buf), \
	  (buf_len) },
...
#define TPARAM_CHECK_INJECT_RAW(buf, buf_len, reason) \
	{ 0, TPARAM_OP_INJECT_RAW, (reason), (buf), (buf_len) },
...
static const unsigned char disable_active_migration_1[] = { 0x00 };

static const unsigned char malformed_stateless_reset_token_1[] = { 0x02, 0xff };

static const unsigned char malformed_stateless_reset_token_2[] = { 0x01 };
...
static const unsigned char malformed_preferred_addr_1[] = { 0x0d, 0xff };
...
static const unsigned char malformed_preferred_addr_4[] = {
	...
	0x0d, 0x29, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x01, 0x55, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};

static const unsigned char malformed_unknown_1[] = { 0xff };
...
static const unsigned char ack_delay_exp[] = { 0x03 };

static const unsigned char stateless_reset_token[16] = { 0x42 };

static const unsigned char preferred_addr[] = {
	0x44, 0x44, 0x44, 0x44, 0x55, 0x55, 0x66, 0x66, 0x66, 0x66, 0x66,
	0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66,
	0x77, 0x77, 0x02, 0xAA, 0xBB, 0x99, 0x99, 0x99, 0x99, 0x99, 0x99,
...
static const struct tparam_test tparam_tests[] = {
	TPARAM_CHECK_DUP(ORIG_DCID, "ORIG_DCID appears multiple times") TPARAM_CHECK_DUP(INITIAL_SCID, "INITIAL_SCID appears multiple times") TPARAM_CHECK_DUP(
		INITIAL_MAX_DATA,
		"INITIAL_MAX_DATA appears multiple times") TPARAM_CHECK_DUP(INITIAL_MAX_STREAM_DATA_BIDI_LOCAL,
									    "INITIAL_MAX_STREAM_DATA_BIDI_LOCAL appears multiple times") TPARAM_CHECK_DUP(INITIAL_MAX_STREAM_DATA_BIDI_REMOTE,
																			  "INITIAL_MAX_STREAM_DATA_BIDI_REMOTE appears multiple times")
		TPARAM_CHECK_DUP(INITIAL_MAX_STREAM_DATA_UNI, "INITIAL_MAX_STREAM_DATA_UNI appears multiple times") TPARAM_CHECK_DUP(
			INITIAL_MAX_STREAMS_BIDI,
			"INITIAL_MAX_STREAMS_BIDI appears multiple times") TPARAM_CHECK_DUP(INITIAL_MAX_STREAMS_UNI,
											    "INITIAL_MAX_STREAMS_UNI appears multiple times")
			TPARAM_CHECK_DUP(MAX_IDLE_TIMEOUT, "MAX_IDLE_TIMEOUT appears multiple times") TPARAM_CHECK_DUP(MAX_UDP_PAYLOAD_SIZE, "MAX_UDP_PAYLOAD_SIZE appears multiple times") TPARAM_CHECK_DUP(
				ACTIVE_CONN_ID_LIMIT,
				"ACTIVE_CONN_ID_LIMIT appears multiple times") TPARAM_CHECK_DUP(DISABLE_ACTIVE_MIGRATION, "DISABLE_ACTIVE_MIGRATION appears multiple times")

				TPARAM_CHECK_DROP(INITIAL_SCID, "INITIAL_SCID was not sent but is required") TPARAM_CHECK_DROP(
					ORIG_DCID,
					"ORIG_DCID was not sent but is required")

					TPARAM_CHECK_DROP_INJECT_A(DISABLE_ACTIVE_MIGRATION, disable_active_migration_1, "DISABLE_ACTIVE_MIGRATION is malformed") TPARAM_CHECK_INJECT(
						UNKNOWN_1, NULL,
						0, NULL) TPARAM_CHECK_INJECT_RAW_A(malformed_stateless_reset_token_1,
										   "STATELESS_RESET_TOKEN is malformed") TPARAM_CHECK_INJECT_A(STATELESS_RESET_TOKEN, malformed_stateless_reset_token_2,
																	       "STATELESS_RESET_TOKEN is malformed")
						TPARAM_CHECK_INJECT_A(STATELESS_RESET_TOKEN, malformed_stateless_reset_token_3, "STATELESS_RESET_TOKEN is malformed") TPARAM_CHECK_INJECT_A(
							STATELESS_RESET_TOKEN,
							malformed_stateless_reset_token_4,
							"STATELESS_RESET_TOKEN is malformed") TPARAM_CHECK_INJECT(STATELESS_RESET_TOKEN, NULL, 0, "STATELESS_RESET_TOKEN is malformed")
							TPARAM_CHECK_INJECT_RAW_A(malformed_preferred_addr_1, "PREFERRED_ADDR is malformed") TPARAM_CHECK_INJECT_RAW_A(malformed_preferred_addr_2, "PREFERRED_ADDR is malformed") TPARAM_CHECK_INJECT_RAW_A(
								malformed_preferred_addr_3,
								"PREFERRED_ADDR is malformed") TPARAM_CHECK_INJECT_RAW_A(malformed_preferred_addr_4, "PREFERRED_ADDR is malformed")
								TPARAM_CHECK_INJECT_RAW_A(malformed_unknown_1, "bad transport parameter") TPARAM_CHECK_INJECT_RAW_A(
									malformed_unknown_2,
									"bad transport parameter") TPARAM_CHECK_INJECT_RAW_A(malformed_unknown_3, "bad transport parameter")

									TPARAM_CHECK_INJECT_A(ACK_DELAY_EXP, excess_ack_delay_exp, "ACK_DELAY_EXP is malformed") TPARAM_CHECK_INJECT_A(
										MAX_ACK_DELAY,
										excess_max_ack_delay,
										"MAX_ACK_DELAY is malformed") TPARAM_CHECK_DROP_INJECT_A(INITIAL_MAX_STREAMS_BIDI,
																	 excess_initial_max_streams,
																	 "INITIAL_MAX_STREAMS_BIDI is malformed") TPARAM_CHECK_DROP_INJECT_A(INITIAL_MAX_STREAMS_UNI,
																									     excess_initial_max_streams, "INITIAL_MAX_STREAMS_UNI is malformed")

										TPARAM_CHECK_DROP_INJECT_A(MAX_UDP_PAYLOAD_SIZE, undersize_udp_payload_size, "MAX_UDP_PAYLOAD_SIZE is malformed") TPARAM_CHECK_DROP_INJECT_A(
											ACTIVE_CONN_ID_LIMIT,
											undersize_active_conn_id_limit,
											"ACTIVE_CONN_ID_LIMIT is malformed")

											TPARAM_CHECK_INJECT_TWICE_A(ACK_DELAY_EXP, ack_delay_exp, "ACK_DELAY_EXP appears multiple times") TPARAM_CHECK_INJECT_TWICE_A(
												MAX_ACK_DELAY,
												ack_delay_exp,
												"MAX_ACK_DELAY appears multiple times")
												TPARAM_CHECK_INJECT_TWICE_A(STATELESS_RESET_TOKEN, stateless_reset_token, "STATELESS_RESET_TOKEN appears multiple times") TPARAM_CHECK_INJECT_TWICE_A(
													PREFERRED_ADDR,
													preferred_addr,
													"PREFERRED_ADDR appears multiple times")

													TPARAM_CHECK_MUTATE(ORIG_DCID, "ORIG_DCID does not match expected value") TPARAM_CHECK_MUTATE(
														INITIAL_SCID,
														"INITIAL_SCID does not match expected value")

														TPARAM_CHECK_DROP_INJECT_A(ORIG_DCID, long_cid, "ORIG_DCID is malformed") TPARAM_CHECK_DROP_INJECT_A(
															INITIAL_SCID,
															long_cid,
															"INITIAL_SCID is malformed")

															TPARAM_CHECK_INT(INITIAL_MAX_DATA, "INITIAL_MAX_DATA is malformed") TPARAM_CHECK_INT(
																INITIAL_MAX_STREAM_DATA_BIDI_LOCAL,
																"INITIAL_MAX_STREAM_DATA_BIDI_LOCAL is malformed")
																TPARAM_CHECK_INT(INITIAL_MAX_STREAM_DATA_BIDI_REMOTE, "INITIAL_MAX_STREAM_DATA_BIDI_REMOTE is malformed") TPARAM_CHECK_INT(
																	INITIAL_MAX_STREAM_DATA_UNI,
																	"INITIAL_MAX_STREAM_DATA_UNI is malformed")
																	TPARAM_CHECK_INT(ACK_DELAY_EXP, "ACK_DELAY_EXP is malformed") TPARAM_CHECK_INT(
																		MAX_ACK_DELAY,
...
static int tparam_handle(struct tparam_ctx *ctx, uint64_t id,
			 unsigned char *data, size_t data_len, WPACKET *wpkt)
{
			     ...
			     size_t ee_len, void *arg)
{
	...
	if (!TEST_true(qtest_fault_delete_extension(
		    qtf, TLSEXT_TYPE_quic_transport_parameters, ee->extensions,
		    &ee->extensionslen, old_bufm)))
		goto err;
     ...
     *
     */
	if (!TEST_true(WPACKET_put_bytes_u16(
		    &wpkt, TLSEXT_TYPE_quic_transport_parameters)) ||
	    !TEST_true(WPACKET_start_sub_packet_u16(&wpkt)))
		goto err;

	for (; PACKET_remaining(&pkt) > 0;) {
		tp_p = (unsigned char *)
			ossl_quic_wire_decode_transport_param_bytes(&pkt, &id,
								    &tp_len);
		if (!TEST_ptr(tp_p)) {
	...
	if (ctx->t->op == TPARAM_OP_INJECT ||
	    ctx->t->op == TPARAM_OP_DROP_INJECT ||
	    ctx->t->op == TPARAM_OP_INJECT_TWICE) {
		if (!TEST_ptr(ossl_quic_wire_encode_transport_param_bytes(
			    &wpkt, ctx->t->id, ctx->t->buf, ctx->t->buf_len)))
			goto err;
		...
		if (!TEST_true(WPACKET_memcpy(&wpkt, ctx->t->buf,
					      ctx->t->buf_len)))
			goto err;
	}

	if (!TEST_true(
		    WPACKET_close(&wpkt))) /* end extension data, set length */
		goto err;
	...
	eb_len = (((uint16_t)ee->extensions[0]) << 8) +
		 (uint16_t)ee->extensions[1];
	eb_len += written;
...
static int test_tparam(int idx)
{
	...
	if (!TEST_true(qtest_create_quic_objects(libctx, c_ctx, NULL, cert,
						 privkey, 0, &s, &c_ssl, &qtf,
						 NULL)))
		goto err;

	if (!TEST_true(qtest_fault_set_hand_enc_ext_listener(
		    qtf, tparam_on_enc_ext, &ctx)))
		goto err;

	if (!TEST_true(qtest_create_quic_connection_ex(
		    s, c_ssl, ctx.t->expect_fail != NULL)))
		goto err;

	if (ctx.t->expect_fail != NULL) {
		SSL_CONN_CLOSE_INFO info = { 0 };

		if (!TEST_true(SSL_get_conn_close_info(c_ssl, &info,
						       sizeof(info))))
			goto err;

		if (!TEST_true((info.flags & SSL_CONN_CLOSE_FLAG_TRANSPORT) !=
			       0) ||
		    !TEST_uint64_t_eq(info.error_code,
				      OSSL_QUIC_ERR_TRANSPORT_PARAMETER_ERROR) ||
		    !TEST_ptr(strstr(info.reason, ctx.t->expect_fail))) {
			TEST_error(
				"expected connection closure information mismatch"
...
err:
	if (!testresult) {
		if (ctx.t->expect_fail != NULL)
			TEST_info(
				"failed during test for id=%llu, op=%d, bl=%zu, "
				"expected failure='%s'",
				(unsigned long long)ctx.t->id, ctx.t->op,
				ctx.t->buf_len, ctx.t->expect_fail);
		else
			TEST_info(
				"failed during test for id=%llu, op=%d, bl=%zu",
				(unsigned long long)ctx.t->id, ctx.t->op,
...
static int test_session_cb(void)
{
     ...
     */
	if (!TEST_int_eq(new_called, 2) || !TEST_ptr_eq(clientquic, cbssl))
		goto err;
...
static int test_domain_flags(void)
{
	...
	if (!TEST_ptr(ctx = SSL_CTX_new_ex(libctx, NULL,
	    ...
	    !TEST_uint64_t_ne(domain_flags & (SSL_DOMAIN_FLAG_SINGLE_THREAD |
			      ...
			      0) ||
	    !TEST_uint64_t_ne(domain_flags & SSL_DOMAIN_FLAG_LEGACY_BLOCKING,
			      0) ||
	    !TEST_true(SSL_CTX_set_domain_flags(
		    ctx, SSL_DOMAIN_FLAG_SINGLE_THREAD)) ||
	    !TEST_true(SSL_CTX_get_domain_flags(ctx, &domain_flags)) ||
...
{
	static unsigned char alpn[] = { 8,   'o', 's', 's', 'l',
					't', 'e', 's', 't' };

	if (SSL_select_next_proto((unsigned char **)out, out_len, alpn,
				  sizeof(alpn), in,
...
static SSL_CTX *create_server_ctx(void)
{
	...
	if (!TEST_ptr(ssl_ctx = SSL_CTX_new_ex(libctx, NULL,
	    ...
	    !TEST_true(SSL_CTX_use_certificate_file(ssl_ctx, cert,
						    SSL_FILETYPE_PEM)) ||
	    !TEST_true(SSL_CTX_use_PrivateKey_file(ssl_ctx, privkey,
						   SSL_FILETYPE_PEM))) {
		SSL_CTX_free(ssl_ctx);
		ssl_ctx = NULL;
	} else {
...
static BIO_ADDR *create_addr(struct in_addr *ina, short int port)
{
	...
	if (!TEST_true(BIO_ADDR_rawmake(addr, AF_INET, ina,
					sizeof(struct in_addr), htons(port)))) {
		BIO_ADDR_free(addr);
		return NULL;
	}
...
static int create_quic_ssl_objects(SSL_CTX *sctx, SSL_CTX *cctx, SSL **lssl,
				   SSL **cssl)
{
	BIO_ADDR *addr = NULL;
	struct in_addr ina;
...
{
	SSL *cssl = (SSL *)X509_STORE_CTX_get_ex_data(
				...
				uint64_t accept_flags, int stream_opt)
{
	...
	if (create_uni && (!TEST_ptr(serverstream = SSL_new_stream(
					     serverssl, SSL_STREAM_FLAG_UNI)) ||
			   !TEST_int_gt(SSL_write_ex(serverstream, msg,
						     sizeof(msg), &nwritten),
					0) ||
	...
	if (create_bidi &&
	    (!TEST_ptr(serverstream2 = SSL_new_stream(serverssl, 0)) ||
	     !TEST_int_gt(SSL_write_ex(serverstream2, msg, sizeof(msg),
				       &nwritten),
			  0) ||
	     !TEST_size_t_eq(nwritten, sizeof(msg)) ||
	     !TEST_int_eq(SSL_handle_events(clientssl), 1)))
	...
	if (!create_uni && create_bidi && stream_type != SSL_STREAM_TYPE_BIDI)
		goto err;
	else if (create_uni && !create_bidi &&
		 stream_type != SSL_STREAM_TYPE_READ)
		goto err;

	if (!TEST_int_gt(SSL_read_ex(clientstream, buf, sizeof(buf), &nread),
			 0) ||
...
static int test_accept_stream(void)
{
	...
	int ret, i;
	long unsigned int j;
	uint64_t accept_flags[] = { 0, SSL_ACCEPT_STREAM_UNI,
	...
	if (!TEST_int_eq(SSL_set_default_stream_mode(
				 clientssl, SSL_DEFAULT_STREAM_MODE_NONE),
			 1) ||
	    !TEST_int_eq(SSL_set_default_stream_mode(
				 serverssl, SSL_DEFAULT_STREAM_MODE_NONE),
			 1) ||
	    !TEST_int_eq(SSL_set_incoming_stream_policy(
				 clientssl, SSL_INCOMING_STREAM_POLICY_ACCEPT,
				 0),
			 1) ||
	    !TEST_int_eq(SSL_set_incoming_stream_policy(
				 serverssl, SSL_INCOMING_STREAM_POLICY_ACCEPT,
				 0),
			 1))
		goto err;
