...
static EVP_PKEY *lms_pubkey_from_data(const unsigned char *data, size_t datalen)
{
	...
	params[0] = OSSL_PARAM_construct_octet_string(
		OSSL_PKEY_PARAM_PUB_KEY, (unsigned char *)data, datalen);
...
static int lms_bad_pub_len_test(void)
{
	...
	for (publen = 0; publen <= td->publen + 16; publen += 3) {
		if (publen == td->publen)
			continue;
		if (!TEST_ptr_null(
			    pkey = lms_pubkey_from_data(pubdata, publen)))
			goto end;
	}
	ret = 1;
end:
...
static int lms_pubkey_decoder_fail_test(void)
{
	...
	size_t pdatalen;
	static const unsigned char pub_bad_LMSType[] = { 0x00, 0x00, 0x00,
							 0xAA };
...
static int lms_pubkey_decoder_test(void)
{
	...
	if (!TEST_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(
			      &pub, "xdr", NULL, "LMS",
...
static int lms_key_eq_test(void)
{
	...
	if (!TEST_ptr(key[0] = lms_pubkey_from_data(td1->pub, td1->publen)) ||
	    ...
	    !TEST_ptr(
		    key[3] = key_decode_from_data(td1->pub, td1->publen, NULL)))
		goto end;
...
static int lms_verify_test(int tst)
{
	...
	ret = TEST_ptr(pkey = lms_pubkey_from_data(td->pub, td->publen)) &&
	      ...
	      TEST_int_eq(EVP_PKEY_verify(ctx, td->sig, td->siglen, td->msg,
					  td->msglen),
			  1);
...
static int lms_verify_bad_sig_test(void)
{
	...
	for (i = 0; i < (int)td->siglen; i += step) {
		...
		if (i > 0)
			sig_data[i - step] ^=
				corrupt_mask; /* Reset the previously corrupt byte */

		if (!TEST_int_eq(EVP_PKEY_verify(ctx, sig_data, td->siglen,
						 td->msg, td->msglen),
				 0)) {
			TEST_note(
				"Incorrectly passed when %dth byte of signature"
				" was corrupted",
				i);
			goto end;
		}
...
static int lms_verify_bad_sig_len_test(void)
{
	...
	for (siglen = 0; siglen < td->siglen + 16; siglen += step) {
		if (siglen == td->siglen) /* ignore the size that should pass */
			continue;
		if (!TEST_int_eq(EVP_PKEY_verify_message_init(ctx, sig, NULL),
				 1))
			goto end;
		if (!TEST_int_eq(EVP_PKEY_verify(ctx, sigdata, siglen, td->msg,
						 td->msglen),
				 0)) {
...
static int lms_verify_bad_pub_sig_test(void)
{
	...
	for (i = 0; i < (int)td->publen; i += step) {
		...
		if (pkey != NULL) {
			...
			if ((EVP_PKEY_verify_message_init(ctx, sig, NULL) ==
			     1) &&
			    !TEST_int_eq(EVP_PKEY_verify(ctx, td->sig,
							 td->siglen, td->msg,
							 td->msglen),
					 0)) {
				TEST_note(
					"Incorrectly passed when byte %d of the public key"
					" was corrupted",
					i);
				goto end;
			}
