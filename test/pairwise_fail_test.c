...
static int test_keygen_pairwise_failure(void)
{
	...
	if (strcmp(pairwise_name, "rsa") == 0) {
		if (!TEST_true(setup_selftest_pairwise_failure(type)))
			goto err;
		if (!TEST_ptr_null(pkey = EVP_PKEY_Q_keygen(libctx, NULL, "RSA",
							    (size_t)2048)))
			goto err;
		...
		if (!TEST_ptr(bio = BIO_new_file(dsaparam_file, "r")))
			goto err;
		if (!TEST_ptr(pParams = PEM_read_bio_Parameters_ex(
				      bio, NULL, libctx, NULL)))
			goto err;
		if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_from_pkey(libctx, pParams,
							       NULL)))
			goto err;
		...
		if (!TEST_true(setup_selftest_pairwise_failure(type)))
			goto err;
		if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(
				      libctx, "ED25519", NULL)))
			goto err;
		if (!TEST_int_eq(EVP_PKEY_keygen_init(ctx), 1))
			goto err;
