...
static X509 *endentity1 = NULL, *endentity2 = NULL, *root = NULL,
	    *intermediate = NULL;
 ...
 */
static int verify_signature(OSSL_CMP_MSG *msg, ASN1_BIT_STRING *protection,
...
static int
execute_calc_protection_signature_test(CMP_PROTECT_TEST_FIXTURE *fixture)
{
	ASN1_BIT_STRING *protection =
		ossl_cmp_calc_protection(fixture->cmp_ctx, fixture->msg);
...
static int test_cmp_calc_protection_no_key_no_secret(void)
{
	...
	if (!TEST_ptr(fixture->msg = load_pkimsg(ir_unprotected_f, libctx)) ||
	    !TEST_ptr(
		    fixture->msg->header->protectionAlg =
...
static int test_cmp_calc_protection_pkey(void)
{
	...
	if (!TEST_true(
		    OSSL_CMP_CTX_set1_pkey(fixture->cmp_ctx, prot_RSA_key)) ||
...
static int test_cmp_calc_protection_pbmac(void)
{
	...
	SETUP_TEST_FIXTURE(CMP_PROTECT_TEST_FIXTURE, set_up);
	if (!TEST_true(OSSL_CMP_CTX_set1_secretValue(
		    fixture->cmp_ctx, sec_insta, sizeof(sec_insta))) ||
...
static int execute_MSG_protect_test(CMP_PROTECT_TEST_FIXTURE *fixture)
{
	return TEST_int_eq(fixture->expected,
			   ossl_cmp_msg_protect(fixture->cmp_ctx,
						fixture->msg));
}
...
static int test_MSG_protect_with_certificate_and_key(void)
{
	...
	if (!TEST_ptr(fixture->msg = OSSL_CMP_MSG_dup(ir_unprotected)) ||
	    ...
	    !TEST_true(OSSL_CMP_CTX_set1_cert(fixture->cmp_ctx, server_cert))) {
		tear_down(fixture);
		fixture = NULL;
	}
	EXECUTE_TEST(execute_MSG_protect_test, tear_down);
...
static int test_MSG_protect_pbmac_no_sender(int with_ref)
{
	...
	if (!TEST_ptr(fixture->msg = OSSL_CMP_MSG_dup(ir_unprotected)) ||
	    ...
	    !OSSL_CMP_CTX_set1_secretValue(fixture->cmp_ctx, secret,
					   sizeof(secret)) ||
	    (!OSSL_CMP_CTX_set1_referenceValue(
		    fixture->cmp_ctx, with_ref ? ref : NULL, sizeof(ref)))) {
		tear_down(fixture);
		fixture = NULL;
	}
	EXECUTE_TEST(execute_MSG_protect_test, tear_down);
...
{
	return TEST_true(
		ossl_cmp_msg_add_extraCerts(fixture->cmp_ctx, fixture->msg));
}
...
static int execute_cmp_build_cert_chain_test(CMP_PROTECT_TEST_FIXTURE *fixture)
{
	...
	X509_STORE *store;
	STACK_OF(X509) *chain = X509_build_chain(fixture->cert, fixture->certs,
						 NULL, fixture->with_ss,
	...
	if (TEST_ptr(store = X509_STORE_new()) &&
		...
		chain = X509_build_chain(fixture->cert, fixture->certs, store,
					 fixture->with_ss, ctx->libctx,
					 ctx->propq);
		ret = TEST_int_eq(fixture->expected, chain != NULL);
		if (ret && chain != NULL) {
			/* Check whether chain built is equal to the expected one */
			ret = TEST_int_eq(0, STACK_OF_X509_cmp(chain,
							       fixture->chain));
			OSSL_STACK_OF_X509_free(chain);
		}
	}
...
static int execute_X509_STORE_test(CMP_PROTECT_TEST_FIXTURE *fixture)
{
	...
	if (!TEST_true(ossl_cmp_X509_STORE_add1_certs(store, fixture->certs,
...
#define USAGE                                                              \
	"prot_RSA.pem IR_protected.der prot_Ed.pem "                       \
...
int setup_tests(void)
{
	...
	if (!test_arg_libctx(&libctx, &default_null_provider, &provider, 12,
			     USAGE))
		return 0;
