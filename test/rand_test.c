...
static int test_rand(void)
{
	...
	*params = OSSL_PARAM_construct_octet_string(
		OSSL_RAND_PARAM_TEST_ENTROPY, entropy2, sizeof(entropy2));
	...
	*params = OSSL_PARAM_construct_octet_string(OSSL_RAND_PARAM_TEST_NONCE,
	...
	if (prov != NULL &&
		...
		params[0] = OSSL_PARAM_construct_int(
			OSSL_RAND_PARAM_FIPS_APPROVED_INDICATOR, &indicator);
...
static int test_rand_uniform(void)
{
	...
	for (i = 1; i < 100; i += 13) {
		...
		if (!TEST_int_eq(err, 0) || !TEST_uint_ge(x, 0) ||
		    !TEST_uint_lt(x, i))
			return 0;
	}
	for (i = 1; i < 100; i += 17)
		for (j = i + 1; j < 150; j += 11) {
			...
			if (!TEST_int_eq(err, 0) || !TEST_uint_ge(x, i) ||
			    !TEST_uint_lt(x, j))
				return 0;
		}
...
static int fips_health_test_one(const uint8_t *buf, size_t n, size_t gen)
{
	...
	if (!TEST_ptr(parent_alg =
			      EVP_RAND_fetch(NULL, "TEST-RAND", "-fips")) ||
	    ...
	    !TEST_true(EVP_RAND_instantiate(crngt, 0, 0, (unsigned char *)"def",
					    3, NULL)) ||
...
int setup_tests(void)
{
	...
	if (!TEST_ptr(configfile = test_get_argument(0)) ||
	    !TEST_true(RAND_set_DRBG_type(NULL, "TEST-RAND", "fips=no", NULL,
					  NULL)) ||
