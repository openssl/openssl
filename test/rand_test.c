...
static int test_rand_set_default_parameters(void)
{
    ...
    if (!TEST_ptr(rctx = RAND_get0_primary(ctx)))
        goto err;
    if (!TEST_int_eq(EVP_RAND_CTX_get_params(rctx, test_par), 1))
        goto err;
    if (!TEST_uint_eq(reseed_request_prim, reseed_request_test))
        goto err;
    if (!TEST_int_eq(reseed_time_interval_prim, reseed_time_interval_test))
        goto err;
    if (!TEST_ptr(rctx = RAND_get0_private(ctx)))
        goto err;
    if (!TEST_int_eq(EVP_RAND_CTX_get_params(rctx, test_par), 1))
        goto err;
    if (!TEST_uint_eq(reseed_request_sec, reseed_request_test))
        goto err;
    if (!TEST_int_eq(reseed_time_interval_sec, reseed_time_interval_test))
        goto err;
