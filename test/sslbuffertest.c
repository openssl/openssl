...
static int test_func(int test)
{
	...
	for (j = 0; j < 2; j++) {
		...
		for (ret = -1, i = 0, len = 0; len != sizeof(testdata) && i < 2;
			...
			if (test >= 1 &&
			    (!TEST_true(SSL_free_buffers(clientssl)) ||
			     !TEST_true(checkbuffers(clientssl, 0))))
				goto end;
			if (test >= 2 &&
			    (!TEST_true(SSL_alloc_buffers(clientssl)) ||
		...
		for (ret = -1, i = 0, len = 0;
			...
			if (test >= 5 &&
			    (!TEST_true(SSL_free_buffers(serverssl)) ||
			...
			if (test >= 6 &&
				...
				goto end;
			if (test >= 7 &&
			    (!TEST_true(SSL_alloc_buffers(serverssl)) ||
...
static int test_free_buffers(int test)
{
	...
	for (i = 0; i <= pipeline; i++) {
		if (!TEST_true(SSL_write_ex(clientssl, testdata,
	...
	if (test == 0) {
		...
		if (!TEST_true(
			    SSL_read_ex(serverssl, buf, readlen, &readbytes)) ||
		...
		if (!TEST_true(
			    BIO_read_ex(tmp, buf, sizeof(buf), &readbytes)) ||
		...
		if (pipeline) {
             ...
             * never fail.
             */
			if (!TEST_int_eq(buf[first_rec_len],
...
int setup_tests(void)
{
	...
	if (!create_ssl_ctx_pair(NULL, TLS_server_method(), TLS_client_method(),
				 TLS1_VERSION, 0, &serverctx, &clientctx, cert,
				 pkey)) {
