...
static void client_keylog_callback(const SSL *ssl, const char *line)
{
	...
	if (client_log_buffer_index + line_length >
	    sizeof(client_log_buffer) - 1) {
...
static void server_keylog_callback(const SSL *ssl, const char *line)
{
	...
	/* If the log doesn't fit, error out. */
	if (server_log_buffer_index + line_length >
	    sizeof(server_log_buffer) - 1) {
			      ...
			      struct sslapitest_log_counts *expected)
{
	...
	for (token = strtok(buffer, " \n"); token != NULL;
		...
		if (strcmp(token, "RSA") == 0) {
			...
			if (!TEST_size_t_eq(client_random_size,
					    SSL3_RANDOM_SIZE))
				return 0;
			...
			if (!TEST_size_t_ne(master_key_size, 0))
				return 0;
			if (!TEST_false(compare_hex_encoded_buffer(
				    token, strlen(token), actual_master_key,
				    master_key_size)))
				return 0;
			master_secret_count++;
		} else if (strcmp(token, "CLIENT_EARLY_TRAFFIC_SECRET") == 0 ||
			   strcmp(token, "CLIENT_HANDSHAKE_TRAFFIC_SECRET") ==
				   0 ||
			   strcmp(token, "SERVER_HANDSHAKE_TRAFFIC_SECRET") ==
				   0 ||
...
static int test_ssl_ctx_build_cert_chain(void)
{
	...
	/* leaf_chain contains leaf + subinterCA + interCA + rootCA */
	if (!TEST_int_eq(SSL_CTX_use_certificate_chain_file(ctx, leaf_chain),
			 1) ||
...
static int full_client_hello_callback(SSL *s, int *al, void *arg)
{
	...
	if (!TEST_mem_eq(p, len, expected_ciphers, sizeof(expected_ciphers)) ||
	    !TEST_size_t_eq(SSL_client_hello_get0_compression_methods(s, &p),
			    1) ||
	    !TEST_int_eq(*p, 0))
...
static int test_no_ems(void)
{
	...
	if (!create_ssl_ctx_pair(libctx, TLS_server_method(),
	...
	if (!create_ssl_objects(sctx, cctx, &serverssl, &clientssl, NULL,
				NULL)) {
...
static int ping_pong_query(SSL *clientssl, SSL *serverssl)
{
	...
	if (!TEST_true(SSL_write(clientssl, cbuf, sizeof(cbuf)) ==
		       sizeof(cbuf)))
		goto end;

	while ((err = SSL_read(serverssl, &sbuf, sizeof(sbuf))) !=
	       sizeof(sbuf)) {
	...
	if (!TEST_true(SSL_write(serverssl, sbuf, sizeof(sbuf)) ==
		       sizeof(sbuf)))
		goto end;

	while ((err = SSL_read(clientssl, &cbuf, sizeof(cbuf))) !=
	       sizeof(cbuf)) {
			     ...
			     const char *cipher)
{
	...
	if (!TEST_true(create_ssl_ctx_pair(
		    ...
		    tls_version, tls_version, &sctx, &cctx, cert, privkey)))
		goto end;
	...
	if (!TEST_true(create_ssl_objects2(sctx, cctx, &serverssl, &clientssl,
					   sfd, cfd)))
		goto end;
	...
	while ((ret = SSL_write(clientssl, buf, bufsz)) != (int)bufsz) {
		if (!TEST_true(SSL_get_error(clientssl, ret) ==
				      ...
				      int zerocopy)
{
	...
	while (chunk_off < SENDFILE_SZ) {
		...
		while ((err = SSL_sendfile(serverssl, ffd, chunk_off,
					   chunk_size, 0)) != chunk_size) {
			if (SSL_get_error(serverssl, err) !=
			    SSL_ERROR_WANT_WRITE)
				goto end;
		}
		while ((err = SSL_read(clientssl, buf_dst + chunk_off,
		...
		/* verify the payload */
		if (!TEST_mem_eq(buf_dst + chunk_off, chunk_size,
				 buf + chunk_off, chunk_size))
			goto end;
...
static int test_large_app_data(int tst)
{
	...
	case 6:
		...
		if (is_fips)
			return TEST_skip(
				"DTLS 1 not supported by FIPS provider");
		prot = DTLS1_VERSION;
		smeth = DTLS_server_method();
		cmeth = DTLS_client_method();
		break;
	...
	if (is_fips && prot < TLS1_2_VERSION)
		return TEST_skip(
			"TLS versions < 1.2 not supported by FIPS provider");
	...
	if ((tst & 1) != 0) {
		...
		if (!TEST_true(SSL_set_options(
			    serverssl, SSL_OP_DONT_INSERT_EMPTY_FRAGMENTS)) ||
		    !TEST_true(SSL_set_options(
			    clientssl, SSL_OP_DONT_INSERT_EMPTY_FRAGMENTS)))
			goto end;
	}
				     ...
				     int max_version)
{
	...
	if (!TEST_int_eq(SSL_write(clientssl, cbuf, sizeof(cbuf)),
			 sizeof(cbuf)))
		goto end;

	if (!TEST_int_eq(SSL_peek(serverssl, &sbuf, sizeof(sbuf)),
			 sizeof(sbuf)))
		goto end;
	...
	if (!TEST_int_eq(SSL_read(serverssl, &sbuf, sizeof(sbuf)),
			 sizeof(sbuf)))
		goto end;
...
static int test_cleanse_plaintext(void)
{
	...
	if (!TEST_true(execute_cleanse_plaintext(
		    TLS_server_method(), TLS_client_method(), TLS1_2_VERSION,
		    TLS1_2_VERSION)))
		return 0;
				       ...
				       char *signer_cert_files)
{
	...
	if (signer_key_files != NULL && signer_cert_files != NULL) {
		...
		if (!TEST_ptr(signer_key =
				      load_pkey_pem(signer_key_file, libctx)))
			goto end;
		signer_cert_file =
			test_mk_file_path(certsdir, signer_cert_files);
		if (!TEST_ptr(signer_cert = load_cert_pem(signer_cert_file,
							  libctx)) ||
...
static int ocsp_server_cb_single(SSL *s, void *arg)
{
	...
	if (*argi == 2) {
		...
		if (id == NULL ||
		    !OCSP_RESPID_match_ex(id, ocspcert, libctx, NULL))
			return SSL_TLSEXT_ERR_ALERT_FATAL;
	} else if (*argi != 1) {
		return SSL_TLSEXT_ERR_ALERT_FATAL;
	}
	...
	if (!TEST_true(SSL_set_tlsext_status_ocsp_resp(s, ocsp_resp_der,
						       resplen))) {
...
static int ocsp_server_cb_multi(SSL *s, void *arg)
{
	...
	char *signer_key_files[] = { "subinterCA.key", "interCA.key",
				     "rootCA.key" };
	char *signer_cert_files[] = { "subinterCA.pem", "interCA.pem",
				      "rootCA.pem" };
...
static int ocsp_client_cb_multi(SSL *s, void *arg)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (ocsp_client_called != 3) {
			...
			OCSP_id_get0_info(NULL, &cert_id_md_oid, NULL, NULL,
					  cid);
			...
			cert_id = OCSP_cert_to_id(cert_id_md, ssl_cert,
						  issuer_cert);
			if (cert_id == NULL) {
				resp = NULL;
				break;
...
static int test_tlsext_status_type_multi(void)
{
	...
	if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(),
					   ...
					   &sctx, &cctx, leaf, skey)))
		goto end;
	if (TEST_int_lt(SSL_CTX_use_certificate_chain_file(sctx, leaf_chain),
			0))
		goto end;
	if (!TEST_true(SSL_CTX_load_verify_locations(cctx, root, NULL)))
		goto end;
	...
	if (!TEST_true(SSL_CTX_set_tlsext_status_type(
		    cctx, TLSEXT_STATUSTYPE_ocsp)) ||
...
static int test_extra_tickets(int idx)
{
	...
	if (!TEST_ptr(bretry) ||
	    !setup_ticket_test(stateful, idx, &sctx, &cctx))
		goto end;
	SSL_CTX_sess_set_new_cb(sctx, new_session_cb);
...
static int test_set_sigalgs(int idx)
{
	...
	if (testctx) {
		...
		if (curr->list != NULL)
			ret = SSL_CTX_set1_sigalgs(cctx, curr->list,
						   (long)curr->listlen);
		else
	...
	if (!testctx) {
		...
		if (curr->list != NULL)
			ret = SSL_set1_sigalgs(clientssl, curr->list,
					       (long)curr->listlen);
		else
...
static int sub_session_time(SSL_SESSION *sess)
{
	...
	return SSL_SESSION_set_time_ex(sess, ossl_time_to_time_t(tick_time)) !=
	       0;
}
...
static int test_early_data_replay_int(int idx, int usecb, int confopt)
{
	...
	if (usecb > 0) {
		if (confopt == 0) {
			...
			if (!TEST_int_eq(SSL_CONF_cmd(confctx, "Options",
						      "-AntiReplay"),
					 2)) {
		...
		SSL_CTX_set_allow_early_data_cb(sctx, allow_early_data_cb,
						&usecb);
	}
...
static int test_early_data_psk(int idx)
{
	...
	int testresult = 0;
	SSL_SESSION *sess = NULL;
	unsigned char alpnlist[] = { 0x08, 'g', 'o', 'o',  'd', 'a',
				     'l',  'p', 'n', 0x07, 'b', 'a',
				     'd',  'a', 'l', 'p',  'n' };
...
#define BADALPN (alpnlist + GOODALPNLEN)
	...
	case 2:
         ...
         */
		err = SSL_R_BAD_PSK;
		if (!TEST_true(SSL_SESSION_set_protocol_version(
			    sess, TLS1_2_VERSION)))
			goto end;
		break;
	...
	case 4:
		...
		if (!TEST_true(SSL_SESSION_set1_hostname(sess, "goodhost")) ||
		    ...
		    !TEST_true(SSL_CTX_set_tlsext_servername_callback(
			    sctx, hostname_cb)))
			goto end;
		break;
	...
	SSL_set_connect_state(clientssl);
	if (err != 0) {
		if (!TEST_false(SSL_write_early_data(clientssl, MSG1,
						     strlen(MSG1), &written)) ||
		...
		if (!TEST_true(SSL_write_early_data(clientssl, MSG1,
						    strlen(MSG1), &written)))
			goto end;
...
static int test_early_data_tls1_2(int idx)
{
     ...
     */
	if (!TEST_false(
		    SSL_write_ex(clientssl, MSG1, strlen(MSG1), &written)) ||
	    !TEST_int_eq(SSL_read_early_data(serverssl, buf, sizeof(buf),
					     &readbytes),
			 SSL_READ_EARLY_DATA_FINISH) ||
	    !TEST_size_t_eq(readbytes, 0) ||
...
static int test_set_ciphersuite(int idx)
{
	...
	if (idx >= 4 && idx <= 7) {
		...
		if (!TEST_true(
			    SSL_CTX_set_cipher_list(cctx, "AES256-GCM-SHA384")))
			goto end;
	}
	...
	if (idx == 8 || idx == 9) {
		...
		if (!TEST_true(SSL_set_cipher_list(clientssl,
						   "AES256-GCM-SHA384")))
			goto end;
	}
...
static int test_key_exchange(int idx)
{
	...
	case 4:
		if (is_fips)
			return TEST_skip(
				"X25519 might not be supported by fips provider.");
	...
	case 5:
		if (is_fips)
			return TEST_skip(
				"X448 might not be supported by fips provider.");
		kexch_alg = NID_X448;
		kexch_name0 = "x448";
		break;
#endif
	...
	if (!TEST_true(create_ssl_ctx_pair(
		    libctx, TLS_server_method(), TLS_client_method(),
		    TLS1_VERSION, max_version, &sctx, &cctx, cert, privkey)))
		goto end;
	...
	if (kexch_groups != NULL) {
		if (!TEST_true(SSL_set1_groups(serverssl, kexch_groups,
					       kexch_groups_size)) ||
		    !TEST_true(SSL_set1_groups(clientssl, kexch_groups,
					       kexch_groups_size)))
			goto end;
	} else {
	...
	if (clientmulti) {
		...
		if (isecdhe) {
			if (!TEST_true(SSL_set1_groups(
				    clientssl, ecdhe_kexch_groups, numec)))
				return 0;
		} else {
			if (!TEST_true(SSL_set1_groups(
				    clientssl, ffdhe_kexch_groups, numff)))
				return 0;
		}
	} else {
		...
		if (isecdhe) {
			if (!TEST_true(SSL_set1_groups(
				    serverssl, ecdhe_kexch_groups, numec)))
				return 0;
		} else {
			if (!TEST_true(SSL_set1_groups(
				    serverssl, ffdhe_kexch_groups, numff)))
				return 0;
		}
	}
	return 1;
...
static int test_negotiated_group(int idx)
{
	...
	if (!TEST_true(set_ssl_groups(serverssl, clientssl, clientmulti,
				      isecdhe, idx)))
		goto end;
...
static int test_tls13_ciphersuite(int idx)
{
	...
	for (max_ver = TLS1_2_VERSION; max_ver <= TLS1_3_VERSION; max_ver++) {
		...
		for (i = 0; i < OSSL_NELEM(t13_ciphers); i++) {
			...
			if (!TEST_true(create_ssl_ctx_pair(
				    libctx, TLS_server_method(),
				    TLS_client_method(), TLS1_VERSION, max_ver,
				    &sctx, &cctx, cert, privkey)))
				goto end;
			...
			if (!TEST_true(
				    create_ssl_objects(sctx, cctx, &serverssl,
						       &clientssl, NULL, NULL)))
				goto end;

			if (set_at_ssl) {
				if (!TEST_true(SSL_set_ciphersuites(
					    serverssl, t13_cipher)) ||
				    !TEST_true(SSL_set_ciphersuites(
					    clientssl, t13_cipher)))
					goto end;
				if (t12_cipher != NULL) {
					if (!TEST_true(SSL_set_cipher_list(
						    serverssl, t12_cipher)) ||
					    !TEST_true(SSL_set_cipher_list(
						    clientssl, t12_cipher)))
						goto end;
				}
			}

			if (!TEST_true(create_ssl_connection(
				    serverssl, clientssl, SSL_ERROR_NONE)))
				goto end;

			negotiated_scipher = SSL_CIPHER_get_name(
				SSL_get_current_cipher(serverssl));
			negotiated_ccipher = SSL_CIPHER_get_name(
				SSL_get_current_cipher(clientssl));
			if (!TEST_str_eq(negotiated_scipher,
					 negotiated_ccipher))
				goto end;
...
static int test_tls13_psk(int idx)
{
	...
	const unsigned char key[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
...
static int test_stateless(void)
{
	...
	SSL_CTX_set_stateless_cookie_verify_cb(
		sctx, verify_stateless_cookie_callback);
...
static int test_custom_exts(int tst)
{
	...
	if (tst == 2 && !TEST_true(create_ssl_ctx_pair(
				libctx, TLS_server_method(), NULL, TLS1_VERSION,
				0, &sctx2, NULL, cert, privkey)))
		goto end;
	...
	if (tst == 0) {
		if (!TEST_true(SSL_CTX_add_client_custom_ext(
			    cctx, TEST_EXT_TYPE1, old_add_cb, old_free_cb,
			    &client, old_parse_cb, &client)))
			goto end;
	} else {
		if (!TEST_true(SSL_CTX_add_custom_ext(
			    cctx, TEST_EXT_TYPE1, context, new_add_cb,
	...
	if (tst == 0) {
		if (!TEST_true(SSL_CTX_add_server_custom_ext(
			    sctx, TEST_EXT_TYPE1, old_add_cb, old_free_cb,
			    &server, old_parse_cb, &server)))
			goto end;
	} else {
		if (!TEST_true(SSL_CTX_add_custom_ext(
			    sctx, TEST_EXT_TYPE1, context, new_add_cb,
	...
	if (tst == 2) {
		/* Set up SNI */
		if (!TEST_true(SSL_CTX_set_tlsext_servername_callback(
			    sctx, sni_cb)) ||
	...
	if (tst == 0) {
		...
		if (clntaddnewcb != 1 || clntparsenewcb != 5 ||
		    srvaddnewcb != 5 || srvparsenewcb != 1)
			goto end;
	}
	...
	if (tst == 0) {
         ...
         */
		if (clntaddnewcb != 2 || clntparsenewcb != 8 ||
		    srvaddnewcb != 8 || srvparsenewcb != 2)
			goto end;
	}
...
#define SERVERINFO_CUSTOM                                                 \
	0x00, (char)TLSEXT_TYPE_signed_certificate_timestamp, 0x00, 0x03, \
		0x04, 0x05, 0x06
...
static int test_serverinfo_custom(const int idx)
{
	...
	if (call_use_serverinfo_ex) {
		if (!TEST_true(SSL_CTX_use_serverinfo_ex(
			    sctx, serverinfo_version, si, si_len)))
			goto end;
	} else {
	...
	if (!TEST_true(SSL_CTX_add_custom_ext(
		    cctx, TLSEXT_TYPE_signed_certificate_timestamp,
		    extension_context, NULL, NULL, NULL,
		    serverinfo_custom_parse_cb, &cb_result)) ||
	    !TEST_true(create_ssl_objects(sctx, cctx, &serverssl, &clientssl,
					  NULL, NULL)) ||
	    !TEST_true(create_ssl_connection(serverssl, clientssl,
					     SSL_ERROR_NONE)) ||
...
static int test_export_key_mat(int tst)
{
	...
	size_t labellen;
	const int protocols[] = { TLS1_VERSION,	  TLS1_1_VERSION,
	...
	if (tst == 5) {
         ...
         */
		if (!TEST_int_le(SSL_export_keying_material(
					 clientssl, ckeymat1, sizeof(ckeymat1),
					 label, LONG_LABEL_LEN + 1, context,
					 sizeof(context) - 1, 1),
				 0))
			goto end;
...
static int test_key_update_peer_in_read(int tst)
{
     ...
     */
	if (!TEST_int_eq(SSL_write(local, lwbuf, sizeof(lwbuf)),
			 sizeof(lwbuf)) ||
...
static int test_key_update_local_in_read(int tst)
{
     ...
     */
	if (!TEST_int_eq(SSL_write(peer, pwbuf, sizeof(pwbuf)),
			 sizeof(pwbuf)) ||
...
static int create_new_vbase(char *userid, char *password)
{
	...
	if (!TEST_true(SRP_create_verifier_BN_ex(userid, password, &salt,
						 &verifier, lgN->N, lgN->g,
						 libctx, NULL)))
		goto end;
...
static int test_srp(int tst)
{
	...
	if (tst == 0 || tst == 1) {
		...
		if (tst == 4 || tst == 5) {
			if (!TEST_true(create_new_vfile(userid, password,
							tmpfilename)))
				goto end;
		...
		if (!TEST_int_eq(SRP_VBASE_init(vbase, tstsrpfile),
				 SRP_NO_ERROR))
			goto end;
	}
	...
	if (!TEST_int_gt(SSL_CTX_set_srp_username_callback(sctx, ssl_srp_cb),
			 0) ||
...
/* We need either ECDHE or DHE for the TLSv1.2 test to work */
	...
	if (!TEST_true(create_ssl_ctx_pair(
		    libctx, TLS_server_method(), TLS_client_method(), tlsvers,
		    tlsvers, &sctx, &cctx, cert, privkey)))
		goto end;
 ...
 */
	...
	{ TLS1_3_VERSION, "AES128-SHA:AES256-SHA", NULL,
...
static int test_ticket_callbacks(int tst)
{
	...
	if (tst >= 14) {
		if (!TEST_true(SSL_CTX_set_tlsext_ticket_key_evp_cb(
			    sctx, tick_key_evp_cb)))
			goto end;
#ifndef OPENSSL_NO_DEPRECATED_3_0
	} else if (tst >= 8) {
		if (!TEST_true(SSL_CTX_set_tlsext_ticket_key_cb(sctx,
								tick_key_cb)))
			goto end;
#endif
	}
...
static int test_shutdown(int tst)
{
	...
	if (tst >= 4) {
		...
		if (!TEST_false(SSL_read_ex(serverssl, buf, sizeof(buf),
			...
			goto end;
		if (tst == 4 && !TEST_true(SSL_key_update(
					serverssl, SSL_KEY_UPDATE_REQUESTED)))
			goto end;
		if (tst == 5) {
			SSL_set_verify(serverssl, SSL_VERIFY_PEER, NULL);
	...
	if (tst < 4) {
		...
		if (!TEST_int_eq(SSL_shutdown(serverssl), 0)
		    /*
                    ...
                    * be possible.
                    */
		    || !TEST_false(SSL_write_ex(serverssl, msg, sizeof(msg),
						&written)) ||
	...
	if (pkey == NULL) {
		...
		if (chain == NULL)
			*x509 = x;
			...
			goto out;
	} else if (PEM_read_bio_PrivateKey_ex(in, pkey, NULL, NULL, libctx,
					      NULL) == NULL) {
		goto out;
	}
...
static int cert_cb(SSL *s, void *arg)
{
	...
	if (cert_cb_cnt == 0) {
		...
		if (!TEST_true(SSL_use_certificate_file(s, cert,
							SSL_FILETYPE_PEM)) ||
		...
		if (!TEST_ptr(chain) ||
		    ...
		    !TEST_true(
			    load_chain("root-cert.pem", NULL, NULL, chain)) ||
		    !TEST_true(load_chain("p256-ee-rsa-ca-cert.pem", NULL,
					  &x509, NULL)) ||
		    !TEST_true(load_chain("p256-ee-rsa-ca-key.pem", &pkey, NULL,
					  NULL)))
			goto out;
...
static int test_client_cert_cb(int tst)
{
	...
	if (!TEST_true(create_ssl_ctx_pair(
		    libctx, TLS_server_method(), TLS_client_method(),
		    TLS1_VERSION, tst == 0 ? TLS1_2_VERSION : TLS1_3_VERSION,
		    &sctx, &cctx, cert, privkey)))
		goto end;
...
static int test_ca_names_int(int prot, int tst)
{
	...
	if (tst == 0 || tst == 1) {
		if (!TEST_ptr(sk1 = sk_X509_NAME_new_null()) ||
		    !TEST_true(
			    sk_X509_NAME_push(sk1, X509_NAME_dup(name[0]))) ||
		    !TEST_true(
			    sk_X509_NAME_push(sk1, X509_NAME_dup(name[1]))) ||
		    !TEST_ptr(sk2 = sk_X509_NAME_new_null()) ||
		    !TEST_true(
			    sk_X509_NAME_push(sk2, X509_NAME_dup(name[0]))) ||
	...
	if (tst == 1 || tst == 2) {
		if (!TEST_ptr(sk1 = sk_X509_NAME_new_null()) ||
		    !TEST_true(
			    sk_X509_NAME_push(sk1, X509_NAME_dup(name[2]))) ||
		    !TEST_true(
			    sk_X509_NAME_push(sk1, X509_NAME_dup(name[3]))) ||
		    !TEST_ptr(sk2 = sk_X509_NAME_new_null()) ||
		    !TEST_true(
			    sk_X509_NAME_push(sk2, X509_NAME_dup(name[2]))) ||
...
static int test_multiblock_write(int test_index)
{
	...
	if (!TEST_true(
		    SSL_CTX_set_max_send_fragment(sctx, MULTIBLOCK_FRAGSIZE)))
		goto end;
	...
	len = written;
	while (len > 0) {
		if (!TEST_true(SSL_read_ex(clientssl, p, MULTIBLOCK_FRAGSIZE,
					   &readbytes)))
			goto end;
		p += readbytes;
		len -= readbytes;
...
#define TIMEOUT 10
	...
	(void)SSL_CTX_set_session_cache_mode(
		ctx, SSL_SESS_CACHE_UPDATE_TIME |
...
static int test_servername(int tst)
{
	...
	if (!TEST_true(create_ssl_ctx_pair(
		    libctx, TLS_server_method(), TLS_client_method(),
		    TLS1_VERSION, (tst <= 4) ? TLS1_2_VERSION : TLS1_3_VERSION,
		    &sctx, &cctx, cert, privkey)) ||
	    !TEST_true(create_ssl_objects(sctx, cctx, &serverssl, &clientssl,
					  NULL, NULL)))
		goto end;
...
static int test_sigalgs_available(int idx)
{
	...
	int sig, hash, numshared, numshared_expected, hash_expected,
		sig_expected;
	...
	if (idx != 0 && idx != 3) {
		...
		if (idx < 3) {
             ...
             */
			if (!TEST_true(filter_provider_set_filter(
				    OSSL_OP_DIGEST, "SHA2-256:SHA1")))
				goto end;
		} else {
			if (!TEST_true(filter_provider_set_filter(
				    OSSL_OP_SIGNATURE, "ECDSA"))
#ifdef OPENSSL_NO_ECX
			    || !TEST_true(filter_provider_set_filter(
				       OSSL_OP_KEYMGMT, "EC"))
#else
			    || !TEST_true(filter_provider_set_filter(
				       OSSL_OP_KEYMGMT, "EC:X25519:X448"))
#endif
			)
				goto end;
		}
	...
	if (idx != 5) {
		...
		if (!TEST_true(create_ssl_ctx_pair(
			    libctx, TLS_server_method(), TLS_client_method(),
			    TLS1_VERSION, 0, &sctx, &cctx, cert2, privkey2)))
			goto end;
	}
	...
	if (idx < 4) {
		if (!TEST_true(SSL_CTX_set_cipher_list(
			    cctx, "ECDHE-RSA-AES128-GCM-SHA256")))
			goto end;
	} else {
		if (!TEST_true(SSL_CTX_set_cipher_list(
			    cctx, "ECDHE-ECDSA-AES128-GCM-SHA256")))
			goto end;
	}
	...
	if (idx < 3) {
		...
		if (!SSL_CTX_set1_sigalgs_list(
			    cctx, "rsa_pss_rsae_sha256:ECDSA+SHA256") ||
		    !SSL_CTX_set1_sigalgs_list(
			    sctx, "rsa_pss_rsae_sha256:ECDSA+SHA256"))
			goto end;
	}

	/* ECDSA P-256 second server key, unless already first */
	if (idx != 5 && (!TEST_int_eq(SSL_CTX_use_certificate_file(
					      sctx, cert2, SSL_FILETYPE_PEM),
				      1) ||
			 !TEST_int_eq(SSL_CTX_use_PrivateKey_file(
					      sctx, privkey2, SSL_FILETYPE_PEM),
				      1) ||
			 !TEST_int_eq(SSL_CTX_check_private_key(sctx), 1)))
		goto end;
	...
	numshared = SSL_get_shared_sigalgs(serverssl, 0, &sig, &hash, NULL,
					   NULL, NULL);
...
static int create_cert_key(int idx, char *certfilename, char *privkeyfilename)
{
	...
	if (!TEST_ptr(evpctx) ||
	    ...
	    !TEST_true(PEM_write_bio_PrivateKey(keybio, pkey, NULL, NULL, 0,
						NULL, NULL)) ||
...
static int test_pluggable_signature(int idx)
{
	...
	if (!TEST_true(create_ssl_ctx_pair(
		    libctx, TLS_server_method(), TLS_client_method(),
		    TLS1_3_VERSION, TLS1_3_VERSION, &sctx, &cctx, NULL, NULL)))
		goto end;
	...
	if (do_conf_cmd) {
		...
		SSL_CONF_CTX_set_ssl_ctx(confctx, sctx);
		if (!TEST_int_gt(SSL_CONF_cmd(confctx, "Certificate",
					      certfilename),
				 0) ||
		    !TEST_int_gt(SSL_CONF_cmd(confctx, "PrivateKey",
					      privkeyfilename),
				 0) ||
		...
		if (!TEST_int_eq(SSL_CTX_use_certificate_file(
					 sctx, certfilename, SSL_FILETYPE_PEM),
				 1) ||
...
static int test_ssl_dup(void)
{
	...
	if (!TEST_int_eq(SSL_get_min_proto_version(client2ssl),
			 TLS1_2_VERSION) ||
	    !TEST_int_eq(SSL_get_max_proto_version(client2ssl), TLS1_2_VERSION))
		goto end;

	if (!TEST_true(create_ssl_connection(serverssl, client2ssl,
					     SSL_ERROR_NONE)))
		goto end;
	...
	if (tmp_dh_params == NULL) {
		...
		if (!TEST_ptr(tmpl) ||
		    !TEST_true(OSSL_PARAM_BLD_push_BN(
			    tmpl, OSSL_PKEY_PARAM_FFC_P, p)) ||
		    !TEST_true(OSSL_PARAM_BLD_push_uint(
			    tmpl, OSSL_PKEY_PARAM_FFC_G, 2)))
			goto end;
...
static int test_dh_auto(int idx)
{
	...
	if (!TEST_true(create_ssl_ctx_pair(libctx, NULL, NULL, 0, 0, &sctx,
					   &cctx, thiscert, thiskey)))
		goto end;
...
#define TWO_WEEK_SEC (2 * ONE_WEEK_SEC)
	...
	if (!TEST_true(create_ssl_ctx_pair(
		    libctx, TLS_server_method(), TLS_client_method(), version,
...
static int test_legacy_ec_point_formats(void)
{
	...
	if (!TEST_true(
		    SSL_CTX_set_options(sctx, SSL_OP_LEGACY_EC_POINT_FORMATS)))
		goto end;

	if (!TEST_true(
		    SSL_CTX_set_options(cctx, SSL_OP_LEGACY_EC_POINT_FORMATS)))
		goto end;
...
static int test_tls13_record_padding(int idx)
{
	...
	if (idx == 0) {
		...
		if (!TEST_false(SSL_CTX_set_block_padding(
			    cctx, SSL3_RT_MAX_PLAIN_LENGTH + 1)))
			goto end;
	...
	if (idx == 1) {
		...
		/* Exceeding the max plain length should fail */
		if (!TEST_false(SSL_set_block_padding(
			    clientssl, SSL3_RT_MAX_PLAIN_LENGTH + 1)))
			goto end;
     ...
     */
	for (i = 0; i < 4; i++) {
		if (!TEST_true(SSL_write_ex(clientssl, msg, strlen(msg),
					    &written)) ||
		    !TEST_size_t_eq(written, strlen(msg)))
			goto end;
...
static int test_pipelining(int idx)
{
	...
	if (!TEST_true(create_ssl_ctx_pair(
		    libctx, TLS_server_method(), TLS_client_method(), 0,
		    TLS1_2_VERSION, &sctx, &cctx, cert, privkey)))
		goto end;
	...
	for (offset = 0, numreads = 0; offset < msglen;
		...
		if (!TEST_true(SSL_read_ex(peerb, buf + offset, msglen - offset,
					   &readbytes)))
			goto end;
	}
	...
	for (offset = 0; offset < msglen; offset += fragsize) {
		...
		if (sendlen > fragsize)
			sendlen = fragsize;
		if (!TEST_true(SSL_write_ex(peerb, msg + offset, sendlen,
					    &written)) ||
...
static int test_version(int idx)
{
	...
	if (!TEST_true(create_ssl_ctx_pair(libctx, servmeth, clientmeth,
					   version, version, &sctx, &cctx, cert,
					   privkey)))
		goto end;
...
static int test_data_retry(void)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (!TEST_false(SSL_write_ex(clientssl, inbuf, sizeof(inbuf),
					     &written)))
			goto end;
		if (!TEST_int_eq(SSL_get_error(clientssl, 0),
				 SSL_ERROR_WANT_WRITE))
			goto end;

		/* Allow one write to progress, but the next one to signal retry */
		if (!TEST_true(BIO_ctrl(bretry,
					MAYBE_RETRY_CTRL_SET_RETRY_AFTER_CNT, 1,
					NULL)))
			goto end;
	...
	while (SSL_read_ex(serverssl, outbuf + totread,
			   sizeof(outbuf) - totread, &readbytes))
		totread += readbytes;
...
static int resume_servername_cb(SSL *s, int *ad, void *arg)
{
	...
	if (!TEST_true(create_ssl_objects(cbdata->sctx, cbdata->cctx,
					  &serverssl, &clientssl, NULL,
					  NULL)) ||
...
static int test_multi_resume(int idx)
{
	...
	if (idx == 3) {
		SSL_CTX_set_tlsext_servername_callback(sctx,
						       resume_servername_cb);
...
static int check_secret_history(SSL *s)
{
	...
	for (i = 0; secret_history[i].recorded == 1; i++) {
		...
		TEST_info(
			...
			last_state, secret_history[i].sm_generation);
...
static int test_quic_tls(int idx)
{
	...
	SSL *serverssl = NULL, *clientssl = NULL;
	int testresult = 0;
	OSSL_DISPATCH qtdis[] = { { OSSL_FUNC_SSL_QUIC_TLS_CRYPTO_SEND,
				    (void (*)(void))crypto_send_cb },
	...
	if (idx == 5) {
		if (!TEST_true(create_ssl_ctx_pair(
			    libctx, TLS_server_method(), NULL, TLS1_3_VERSION,
			    0, &sctx2, NULL, cert, privkey)))
			goto end;
	...
	if (idx == 2) {
		if (!TEST_true(SSL_set_ciphersuites(
			    serverssl, "TLS_AES_128_CCM_SHA256")) ||
	...
	if (idx != 1 && idx != 4) {
		if (!TEST_true(create_ssl_connection_ex(
			    serverssl, clientssl, SSL_ERROR_NONE,
			    &cdata.sm_count, &sdata.sm_count)))
			goto end;
	} else {
		/* We expect this connection to fail */
		if (!TEST_false(create_ssl_connection_ex(
			    serverssl, clientssl, SSL_ERROR_NONE,
			    &cdata.sm_count, &sdata.sm_count)))
			goto end;
		testresult = 1;
