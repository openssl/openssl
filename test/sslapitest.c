...
static int early_data_skip_helper(int testtype, int idx)
{
    ...
    if (testtype == 1 || testtype == 2) {
         ...
         */
        if (!TEST_time_t_ne(SSL_SESSION_set_time_ex(sess, time(NULL) - 20), 0))
            goto end;
    }
...
#define TIMEOUT 10
    ...
    if (!TEST_int_ne(SSL_SESSION_set_timeout_ex(early, TIMEOUT), 0)
        || !TEST_int_ne(SSL_SESSION_set_timeout_ex(middle, TIMEOUT), 0)
        || !TEST_int_ne(SSL_SESSION_set_timeout_ex(late, TIMEOUT), 0))
        goto end;
