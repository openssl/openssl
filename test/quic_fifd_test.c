...
static int test_generic(INFO *info, int kind)
{
	...
	for (i = 0; i < 2; ++i) {
		...
		if (!TEST_true(ossl_quic_sstream_append(
			    info->sstream[i], (unsigned char *)"Test message",
			    12, &consumed)) ||
	...
	if (!TEST_ptr(cfq_item = ossl_quic_cfq_add_frame(
			      info->cfq, 10, pn_space,
			      OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID, 0,
			      placeholder_data, sizeof(placeholder_data),
			      cfq_free_cb_, NULL)) ||
	...
	case TEST_KIND_ACK:
		...
		for (i = 0; i < 2; ++i)
			if (!TEST_size_t_eq(ossl_quic_sstream_get_buffer_used(
						    info->sstream[i]),
					    0))
				goto err;
	...
	case TEST_KIND_LOSS:
		...
		if (!TEST_ptr_eq(cfq_item, ossl_quic_cfq_get_priority_head(
						   info->cfq, pn_space)))
			goto err;
...
static int test_fifd(int idx)
{
	...
	if (!TEST_true(ossl_statm_init(&info.statm)) ||
	    ...
	    !TEST_true(ossl_quic_fifd_init(
		    &info.fifd, info.cfq, info.ackm, info.txpim,
		    get_sstream_by_id, NULL, regen_frame, NULL, confirm_frame,
		    NULL, sstream_updated, NULL, NULL, NULL)))
		goto err;
