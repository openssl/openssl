...
static int put_trace_output(void)
{
	...
	{
		res = TEST_int_eq(BIO_printf(trc_out, OSSL_HELLO),
				  (int)strlen(OSSL_HELLO));
		res += TEST_int_eq(trace_string(0, 0, OSSL_STR80),
				   (int)strlen(OSSL_STR80));
		res += TEST_int_eq(trace_string(0, 0, OSSL_STR81),
				   (int)strlen(OSSL_STR80));
		res += TEST_int_eq(trace_string(1, 1, OSSL_CTRL),
				   (int)strlen(OSSL_CTRL));
		res += TEST_int_eq(trace_string(0, 1, OSSL_MASKED),
				   (int)strlen(OSSL_MASKED) +
					   1); /* newline added */
		res += TEST_int_eq(BIO_printf(trc_out, OSSL_BYE),
				   (int)strlen(OSSL_BYE));
		res = res == 6;
...
static int test_trace_channel(void)
{
	static const char expected[] = OSSL_START
		"\n" OSSL_HELLO OSSL_STR80
		"[len 81 limited to 80]: " OSSL_STR80 OSSL_CTRL OSSL_MASKED
		"\n" OSSL_BYE OSSL_END "\n";
	...
	if (!TEST_strn2_eq(p_buf, len, expected, expected_len))
		ret = 0;
	ret = TEST_int_eq(OSSL_trace_set_channel(OSSL_TRACE_CATEGORY_HTTP,
						 NULL),
			  1) &&
	      ret;
...
static size_t trace_cb(const char *buffer, size_t count, int category, int cmd,
		       void *data)
{
	trace_cb_called = 1;
...
static int test_trace_callback(void)
{
	...
	if (!TEST_true(OSSL_trace_set_callback(OSSL_TRACE_CATEGORY_TRACE,
					       trace_cb, NULL)))
		goto end;
