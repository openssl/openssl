...
static int test_quic_client_ex(int fd_arg)
{
	...
	if (fd_arg == INVALID_SOCKET) {
		...
		if (!TEST_true(BIO_ADDR_rawmake(s_addr_, AF_INET, &ina,
						sizeof(ina), htons(port))))
			goto err;
	} else {
		c_fd = fd_arg;
	}
	...
	for (;;) {
		if (ossl_time_compare(ossl_time_subtract(ossl_time_now(),
							 start_time),
		...
		if (c_connected && !c_write_done) {
			if (!TEST_int_eq(SSL_write(c_ssl, msg1,
						   sizeof(msg1) - 1),
					 (int)sizeof(msg1) - 1))
				goto err;
		...
		if (c_write_done && !c_shutdown &&
			...
			if (ret != 1) {
				...
				if (!TEST_size_t_lt(c_total_read,
						    sizeof(msg2) - 1))
					goto err;
			}
		}
...
static int test_quic_client_connect_first(void)
{
	...
	if (!TEST_int_eq(connect(c_fd, (const struct sockaddr *)&sin,
				 sizeof(sin)),
			 0))
		goto err;
